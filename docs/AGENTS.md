...
- `scripts/docs-sync-publish.mjs` excludes and prunes `docs/internal/**` from the public `nodoassist/docs` publish repo if a page is force-added later.
...
`.github/workflows/maturity-scorecard.yml` renders artifact previews and can open generated-doc PRs; `.github/workflows/nodoassist-release-checks.yml` dispatches it for release QA.
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `nodoassist/docs` repo (often cloned locally as `../nodoassist-docs`).
...
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `nodoassist/docs`.
