...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In NodoAssist it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to NodoAssist primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In NodoAssist it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    nodoassist plugins enable open-prose
    ```
  ...
  <Step title="Verify">
    ```bash
    nodoassist plugins list | grep prose
    ```
...
`nodoassist plugins install ./extensions/open-prose`
...
## NodoAssist runtime mapping

OpenProse programs map to NodoAssist primitives:

| OpenProse concept         | NodoAssist tool                                 |
| ------------------------- | ----------------------------------------------- |
...
executed. Use NodoAssist tool allowlists and approval gates to control side
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    NodoAssist's native multi-agent coordination layer.
  </Card>
