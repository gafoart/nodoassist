---
summary: "Run NodoAssist embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the bundled Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to NodoAssist
---

The bundled `codex` plugin runs embedded OpenAI agent turns through Codex
app-server instead of the built-in NodoAssist harness. Codex owns the
low-level agent session: native thread resume, native tool continuation,
native compaction, and app-server execution. NodoAssist still owns chat
channels, session files, model selection, NodoAssist dynamic tools, approvals,
...
repaired by `nodoassist doctor --fix`.

When no NodoAssist sandbox is active, NodoAssist starts Codex app-server threads
with Codex native code mode enabled (code-mode-only stays off by default), so
native workspace/code capabilities remain available alongside NodoAssist
dynamic tools routed through the app-server `item/tool/call` bridge. An
active NodoAssist sandbox or restricted tool policy disables native code mode
entirely unless you opt into the experimental sandbox exec-server path.

This Codex-native feature is separate from
[NodoAssist code mode](/reference/code-mode), an opt-in QuickJS-WASI runtime
for generic NodoAssist runs with a different `exec` input shape. For the
...
- NodoAssist with the bundled `codex` plugin available. Include `codex` in
...
- Codex app-server `0.142.0` or newer. The plugin manages a compatible
  ...
  startup.
- Codex auth through `nodoassist models auth login --provider openai`, an
...
```bash
nodoassist models auth login --provider openai
```
...
The default `appServer.homeScope: "agent"` isolates each NodoAssist agent from
...
config, plugins, and thread store. NodoAssist does not inject an NodoAssist auth
...
NodoAssist; the fork attaches to the current NodoAssist session and stays
visible to other native Codex clients. Archiving requires explicit
confirmation that the thread is closed elsewhere.

Do not resume or write the same thread concurrently from NodoAssist and
another Codex client. Codex coordinates live writers inside one app-server
process, not across independent Desktop, CLI, and NodoAssist processes.
...
| Need                                   | Set                                                                              | Where                                |
| -------------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------ |
| Enable the harness                     | `plugins.entries.codex.enabled: true`                                            | NodoAssist config                    |
| Keep an allowlisted plugin install     | Include `codex` in `plugins.allow`                                               | NodoAssist config                    |
| Route OpenAI agent turns through Codex | `agents.defaults.model` or `agents.list[].model` as `openai/gpt-*`               | NodoAssist agent config              |
| Sign in with ChatGPT/Codex OAuth       | `nodoassist models auth login --provider openai`                                 | CLI auth profile                     |
| Add API-key backup for Codex runs      | `openai:*` API-key profile listed after subscription auth in `auth.order.openai` | CLI auth profile + NodoAssist config |
| Fail closed when Codex is unavailable  | Provider or model `agentRuntime.id: "codex"`                                     | NodoAssist model/provider config     |
| Use direct OpenAI API traffic          | Provider or model `agentRuntime.id: "nodoassist"` with normal OpenAI auth        | NodoAssist model/provider config     |
...
The API key is only an auth fallback, not a request to switch to NodoAssist or
...
NodoAssist ignores those local summarizer overrides at runtime, and
`nodoassist doctor --fix` removes them when the agent uses Codex.
...
`agents.defaults.compaction.provider`. `nodoassist doctor --fix` migrates the
...
compaction on the bound thread. NodoAssist does not wait for completion,
impose an NodoAssist timeout, restart the shared app-server, or fall back to a
...
- Do not use legacy Codex GPT refs in config; run `nodoassist doctor --fix` to
...
- `agentRuntime.id: "nodoassist"` opts a provider or model into the embedded
  NodoAssist runtime when that is intentional.
...
| Direct OpenAI API-key traffic through NodoAssist     | Provider or model `agentRuntime.id: "nodoassist"` and normal OpenAI auth | `/status` shows NodoAssist runtime      | Use only when NodoAssist is intentional |
| Legacy config                                        | legacy Codex GPT refs                                                    | `nodoassist doctor --fix` rewrites it   | Do not write new config this way        |
...
With Codex forced, NodoAssist fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start.

## App-server policy

By default, the plugin starts NodoAssist's managed Codex binary locally with
...
implicit YOLO posture, NodoAssist selects allowed guardian permissions
instead. When an NodoAssist sandbox is active for the session, NodoAssist
...
Shell access instead goes through NodoAssist sandbox-backed dynamic tools such
as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

Use normalized NodoAssist exec mode for Codex native auto-review before
...
NodoAssist does not preserve legacy unsafe Codex `approvalPolicy: "never"` or
...
works, but `tools.exec.mode: "auto"` is the normalized NodoAssist surface.
...
supports NodoAssist text commands.
...
- `/codex resume <thread-id>` attaches the current NodoAssist session to an
...
1. Ordered OpenAI auth profiles for the agent, preferably under
   `auth.order.openai`. Run `nodoassist doctor --fix` to migrate older legacy
...
When NodoAssist sees a ChatGPT subscription-style Codex auth profile, it
...
If a subscription profile hits a Codex usage limit, NodoAssist records the
...
When native Codex plugins are configured, NodoAssist installs or refreshes
...
ids, accessibility, and metadata, but NodoAssist owns the per-thread
enablement decision: if policy allows a listed accessible app, NodoAssist
...
installation for unknown ids; NodoAssist only activates marketplace plugins
with `plugin/install` and then refreshes inventory.

### Environment isolation

For local stdio app-server launches, NodoAssist sets `CODEX_HOME` to a
...
`~/.codex` by default. NodoAssist preserves the normal process `HOME`;
...
`appServer.homeScope: "user"`, NodoAssist instead uses the native user Codex
home and its existing account without injecting an NodoAssist auth profile.
...
process. NodoAssist removes `CODEX_HOME` and `HOME` from this list during
...
Codex dynamic tools default to `searchable` loading. NodoAssist does not
...
remaining NodoAssist integration tools, such as messaging, media, cron,
browser, nodes, gateway, and `heartbeat_respond`, are available through
Codex tool search under the `nodoassist` namespace, keeping the initial model
context smaller.
...
NodoAssist's managed `web_search` dynamic tool are mutually exclusive so
managed search cannot bypass native domain restrictions. NodoAssist uses the
managed tool when hosted search is unavailable, explicitly disabled, or
replaced by a selected managed provider. NodoAssist keeps Codex's standalone
...
while explicit NodoAssist or ACP delegation is still available through the
`nodoassist` dynamic tool namespace. Heartbeat collaboration instructions
...
| -------------------------- | -------------- | ------------------------------------------------------------------------------------------ |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put NodoAssist dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional NodoAssist dynamic tool names to omit from Codex app-server turns.              |
...
| `homeScope`                                   | `"agent"`                                              | `"agent"` isolates Codex state per NodoAssist agent. `"user"` shares the native `$CODEX_HOME` or `~/.codex`, uses native auth, and enables owner-only thread management. User scope requires stdio.                                                                                                                                                                                                     |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after NodoAssist builds its inherited environment. NodoAssist keeps the selected `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                                                               |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. NodoAssist dynamic tools remain registered with Codex so nested `tools.*` calls return through the app-server `item/tool/call` bridge.                                                                                                                                                                                                                    |
| `remoteWorkspaceRoot`                         | unset                                                  | Remote Codex app-server workspace root. When set, NodoAssist infers the local workspace root from the resolved NodoAssist workspace, preserves the current cwd suffix under this remote root, and sends only the final app-server cwd to Codex. If the cwd is outside the resolved NodoAssist workspace root, NodoAssist fails closed instead of sending a gateway-local path to the remote app-server. |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                                                                                                             |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while NodoAssist waits for `turn/completed`.                                                                                                                                                                                                                                                                          |
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, post-tool raw assistant progress, raw reasoning completion, or reasoning progress while NodoAssist waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget.                                      |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an NodoAssist sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the NodoAssist sandbox egress setting.                                                                                         |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                                                                                                      |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                                                                                                         |
| `networkProxy`                                | disabled                                               | Opt into Codex permissions-profile networking for app-server commands. NodoAssist defines the selected `permissions.<profile>.network` config and selects it with `default_permissions` instead of sending `sandbox`.                                                                                                                                                                                   |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an NodoAssist sandbox-backed Codex environment with the supported Codex app-server so native Codex execution can run inside the active NodoAssist sandbox.                                                                                                                                                                                                                |

`appServer.networkProxy` is explicit because it changes the Codex sandbox
contract. When enabled, NodoAssist also sets `features.network_proxy.enabled`
and `default_permissions` in the Codex thread config so the generated
permission profile can start Codex managed networking. By default, NodoAssist
generates a collision-resistant `nodoassist-network-<fingerprint>` profile
...
NodoAssist-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 90
second NodoAssist watchdog by default. A positive per-call `timeoutMs`
...
reduced by earlier preparation work. On timeout, NodoAssist aborts the tool
...
After Codex accepts a turn, and after NodoAssist responds to a turn-scoped
...
app-server goes quiet for `appServer.turnCompletionIdleTimeoutMs`, NodoAssist
best-effort interrupts the Codex turn, records a diagnostic timeout, and
releases the NodoAssist session lane so follow-up chat messages are not
...
Tool handoffs use a longer post-tool idle budget: after NodoAssist returns an
...
quiet without `turn/completed`, NodoAssist best-effort interrupts the native
turn and releases the session lane. If another turn watch wins that release
race, NodoAssist still accepts the completed final assistant item once no
...
stuck app-server client and release the NodoAssist session lane; they also
...
- `NODOASSIST_CODEX_APP_SERVER_BIN` bypasses the managed binary when
  `appServer.command` is unset.
- `NODOASSIST_CODEX_APP_SERVER_ARGS`
- `NODOASSIST_CODEX_APP_SERVER_MODE=yolo|guardian`
- `NODOASSIST_CODEX_APP_SERVER_APPROVAL_POLICY`
- `NODOASSIST_CODEX_APP_SERVER_SANDBOX`

`NODOASSIST_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`NODOASSIST_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config
...
capabilities in the same Codex thread as the NodoAssist harness turn. NodoAssist
does not translate Codex plugins into synthetic `codex_plugin_*` NodoAssist
dynamic tools.
...
Thread app config is computed when NodoAssist establishes a Codex harness
...
Short version: NodoAssist does not vendor the desktop-control app or execute
...
- NodoAssist dynamic tools are supported. Codex asks NodoAssist to execute
  those tools, so NodoAssist remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  NodoAssist can observe or block selected native events through the
  supported relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. NodoAssist keeps a transcript mirror for
  channel history, search, `/new`, `/reset`, and future model or harness
  switching, but does not replace Codex compaction with an NodoAssist or
  context-engine summarizer.
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching NodoAssist provider/model settings.
- `tool_result_persist` applies to NodoAssist-owned transcript tool results,
...
**NodoAssist uses the built-in harness instead of Codex:** confirm the model
...
instead of falling back to NodoAssist.
...
(
  ...
  if ls /tmp/nodoassist/nodoassist-*.log >/dev/null 2>&1; then
    grep -E -i -n "$pattern" /tmp/nodoassist/nodoassist-*.log 2>/dev/null || true
  else
    journalctl --user -u nodoassist-gateway --since today --no-pager 2>/dev/null \
...
**Legacy Codex model refs config remains:** run `nodoassist doctor --fix`.
...
`0.142.0-alpha.2` or `0.142.0+custom` are rejected because NodoAssist tests
...
id that NodoAssist no longer has registered. This is a native Codex hook
...
NodoAssist Gateway so old threads are dropped and native hook registrations
are recreated.
