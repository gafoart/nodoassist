...
Installed plugins point `package.json` `nodoassist` fields at both source and
built entries:
...
- If a `runtimeExtensions`/`runtimeSetupEntry` artifact is declared but
  missing, install/discovery fails with a packaging error; NodoAssist does not
  silently fall back to source. Source fallback (below) only applies when no
  runtime entry is declared at all.
- If an installed package declares only a TypeScript source entry, NodoAssist
...
**Import:** `nodoassist/plugin-sdk/tool-plugin`
...
the NodoAssist tool-result format, and exposes static metadata that
`nodoassist plugins build` writes into the plugin manifest (`contracts.tools`,
`configSchema`).
...
- For custom tool results, `nodoassist/plugin-sdk/tool-results` exports
  `textResult` and `jsonResult`.
- Tool names are static, so `nodoassist plugins build` derives
...
- Runtime loading stays strict: installed plugins still need
  `nodoassist.plugin.json` and `package.json` `nodoassist.extensions`. NodoAssist
  never executes plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `nodoassist/plugin-sdk/plugin-entry`
...
| `configSchema`            | `NodoAssistPluginConfigSchema \| () => NodoAssistPluginConfigSchema` | No       | Empty object schema |
| `reload`                  | `NodoAssistPluginReloadRegistration`                                 | No       | -                   |
| `nodeHostCommands`        | `NodoAssistPluginNodeHostCommand[]`                                  | No       | -                   |
| `securityAuditCollectors` | `NodoAssistPluginSecurityAuditCollector[]`                           | No       | -                   |
| `register`                | `(api: NodoAssistPluginApi) => void`                                 | Yes      | -                   |

- `id` must match your `nodoassist.plugin.json` manifest.
- `kind` is deprecated: declare an exclusive slot (`"memory"` or
  `"context-engine"`) in the `nodoassist.plugin.json` manifest `kind` field
  instead. Runtime-entry `kind` remains only as a compatibility fallback for
  older plugins.
- `configSchema` can be a function for lazy evaluation. NodoAssist resolves and
  memoizes the schema on first access, so expensive schema builders only run
  once.

## `defineChannelPluginEntry`

**Import:** `nodoassist/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: NodoAssistPluginApi) => void`                                 | No       | -                   |
| `registerFull`        | `(api: NodoAssistPluginApi) => void`                                 | No       | -                   |
...
- `registerFull` runs only for `"full"` and `"tool-discovery"`. For
  `"tool-discovery"` it runs _instead of_ channel registration: NodoAssist
  ...
  channel setup.
- Discovery registration is non-activating, not import-free: NodoAssist may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory; NodoAssist
...
- Use `api.registerCli(..., { descriptors: [...] })` for plugin-owned root
  ...
  underscore, starting with a letter or number; NodoAssist rejects other
...
- Use `api.registerNodeCliFeature(...)` for paired-node feature commands so
  they land under `nodoassist nodes` (equivalent to
  `registerCli(registrar, { parentPath: ["nodes"], ... })`).
- For other nested plugin commands, add `parentPath` and register commands
  on the `program` object passed to the registrar; NodoAssist resolves it to
...
NodoAssist loads this instead of the full entry when a channel is disabled,
...
| Import                                | Use for                                                                                                                                                                            |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nodoassist/plugin-sdk/setup-runtime` | Runtime-safe setup helpers: `createSetupTranslator`, import-safe setup patch adapters, lookup-note output, `promptResolvedAllowFrom`, `splitSetupEntries`, delegated setup proxies |
| `nodoassist/plugin-sdk/channel-setup` | Optional-install setup surfaces                                                                                                                                                    |
| `nodoassist/plugin-sdk/setup-tools`   | Setup/install CLI, archive, and docs helpers                                                                                                                                       |
...
`nodoassist/plugin-sdk/channel-entry-contract` instead. It lets the setup
entry keep setup-safe plugin/secrets exports while still exposing a runtime
setter:

```typescript
import { defineBundledChannelSetupEntry } from "nodoassist/plugin-sdk/channel-entry-contract";
...
evaluate the plugin entry and the channel plugin object so NodoAssist can
...
NodoAssist classifies loaded plugins by their registration behavior:
...
Use `nodoassist plugins inspect <id>` to see a plugin's shape.
