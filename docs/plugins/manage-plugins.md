---
summary: "Quick examples for listing, installing, updating, inspecting, and uninstalling NodoAssist plugins"
read_when:
...
source-selection rules, and edge cases, see [`nodoassist plugins`](/cli/plugins).
...
`plugins list` is a cold inventory check: what NodoAssist can discover from
...
an install hint (`nodoassist plugins install clawhub:<package>`) per result.

## Enable and disable plugins

```bash
nodoassist plugins enable <plugin-id>
nodoassist plugins disable <plugin-id>
```
...
nodoassist plugins install clawhub:<package>@1.2.3
nodoassist plugins install clawhub:<package>@beta

# Install from npm.
nodoassist plugins install npm:<package>
nodoassist plugins install npm:@scope/nodoassist-plugin@1.2.3
nodoassist plugins install npm:@nodoassist/codex

# Install from a local npm-pack artifact.
nodoassist plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
nodoassist plugins install git:github.com/acme/nodoassist-plugin@v1.0.0
...
name matches a bundled or official plugin id, in which case NodoAssist uses
...
use `nodoassist plugins update` instead; `--force` is not supported with
`--link`.
...
```bash
nodoassist plugins update <plugin-id>
nodoassist plugins update <npm-package-or-spec>
nodoassist plugins update --all
nodoassist plugins update <plugin-id> --dry-run
```
...
`nodoassist plugins update --all` is the bulk maintenance path. It still
respects ordinary tracked install specs, but trusted official NodoAssist
...
```bash
nodoassist plugins update @scope/nodoassist-plugin@beta
nodoassist plugins update @scope/nodoassist-plugin
```
...
See [`nodoassist plugins`](/cli/plugins#update) for the exact fallback and
pinning rules.

## Uninstall plugins

```bash
nodoassist plugins uninstall <plugin-id> --dry-run
nodoassist plugins uninstall <plugin-id>
nodoassist plugins uninstall <plugin-id> --keep-files
```
...
In Nix mode (`NODOASSIST_NIX_MODE=1`), plugin install, update, uninstall,
...
| ----------- | --------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| ClawHub     | You want NodoAssist-native discovery, scan summaries, versions, and hints   | `nodoassist plugins install clawhub:<package>`                   |
| git         | You want a branch, tag, or commit from a repository                         | `nodoassist plugins install git:github.com/<owner>/<repo>@<ref>` |
...
| npm pack    | You are proving a local package artifact through npm install semantics      | `nodoassist plugins install npm-pack:<path.tgz>`                 |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `nodoassist plugins install npm:@acme/nodoassist-plugin`         |
...
ClawHub is the primary public discovery surface for NodoAssist plugins. Publish
...
Native npm plugins must ship a plugin manifest (`nodoassist.plugin.json`) plus
`package.json` metadata before publishing:

```json package.json
{
  "name": "@acme/nodoassist-plugin",
  "version": "1.0.0",
  "type": "module",
  "nodoassist": {
...
nodoassist plugins install npm:@acme/nodoassist-plugin
nodoassist plugins install npm:@acme/nodoassist-plugin@beta
nodoassist plugins install npm:@acme/nodoassist-plugin@1.0.0
```
...
- [Building plugins](/plugins/building-plugins) shows the full plugin
  package shape (including `nodoassist.plugin.json`) and first publish
  workflow.
