...
The plugin is published to npm; it is not bundled into the NodoAssist runtime
...
NodoAssist has a built-in dimension for `text-embedding-3-small` (1536) and
...
`memory_recall` tool, the `memory_forget` query path, and `nodoassist ltm
...
```bash
nodoassist ltm list [--limit <n>] [--order-by-created-at]
nodoassist ltm search <query> [--limit <n>]
nodoassist ltm stats
```

`ltm query` runs a non-vector query directly against the LanceDB table:

```bash
nodoassist ltm query --cols id,text,createdAt --limit 20
nodoassist ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
LanceDB data defaults to `~/.nodoassist/memory/lancedb`. Override with `dbPath`:
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "~/.nodoassist/memory/lancedb",
          embedding: {
  ...
  plugins: {
    entries: {
      "memory-lancedb": {
        enabled: true,
        config: {
          dbPath: "s3://memory-bucket/nodoassist",
          storageOptions: {
...
plugin package (not the NodoAssist core dist). Gateway startup does not repair
...
```bash
nodoassist ltm stats
nodoassist ltm search "recent preference"
```
