...
`api.runtime.version` is the current NodoAssist product version, sourced from the shared version resolver so plugins see the same value the CLI reports.
...
Use `nodoassist/plugin-sdk/pair-loop-guard-runtime` directly only for custom
...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    `runEmbeddedAgent(...)` is the neutral helper for starting a normal NodoAssist agent turn from plugin code. It uses the same provider/model resolution and agent-harness selection as channel-triggered replies.
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    For transcript reads and writes, import `nodoassist/plugin-sdk/session-transcript-runtime` and use `resolveSessionTranscriptIdentity(...)`, `resolveSessionTranscriptTarget(...)`, `readSessionTranscriptEvents(...)`, `appendSessionTranscriptMessageByIdentity(...)`, `publishSessionTranscriptUpdateByIdentity(...)`, or `withSessionTranscriptWriteLock(...)` with `{ agentId, sessionKey, sessionId }`. These APIs let plugins identify a transcript, read its events, append messages, publish updates, and run related operations under the same transcript write lock. Passing `sessionFile`, using `resolveSessionTranscriptLegacyFileTarget(...)`, or importing low-level `appendSessionTranscriptMessage(...)` / `emitSessionTranscriptUpdate(...)` from `nodoassist/plugin-sdk/agent-harness-runtime` is deprecated; those paths exist only for legacy code that already receives an active transcript artifact.
  ...
  <Accordion title="api.runtime.llm">
    ...
    duplicating NodoAssist model/auth/base URL preparation.
    ...
    The helper uses the same simple-completion preparation path as NodoAssist's
  ...
  <Accordion title="api.runtime.nodes">
    ...
    Inside the Gateway this runtime is in-process. In plugin CLI commands it calls the configured Gateway over RPC, so commands such as `nodoassist googlemeet recover-tab` can inspect paired nodes from the terminal. Node commands still go through normal Gateway node pairing, command allowlists, plugin node-invoke policies, and node-local command handling.
  ...
  <Accordion title="api.runtime.tasks">
    ...
    Use `bindSession({ sessionKey, requesterOrigin })` when you already have a trusted NodoAssist session key from your own binding layer. Do not bind from raw user input.
  ...
  <Accordion title="api.runtime.webSearch">
    ...
    const result = await api.runtime.webSearch.search({
      config: api.config,
      args: { query: "NodoAssist plugin SDK", count: 5 },
    });
    ```
  ...
  <Accordion title="api.runtime.media">
    ...
    const tmpRoot = resolvePreferredNodoAssistTmpDir();
...
<ParamField path="api.config" type="NodoAssistConfig">
