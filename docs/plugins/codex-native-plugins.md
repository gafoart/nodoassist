---
summary: "Configure migrated native Codex plugins for Codex-mode NodoAssist agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode NodoAssist agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode NodoAssist agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the NodoAssist turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. NodoAssist does not translate
Codex plugins into synthetic `codex_plugin_*` NodoAssist dynamic tools.
...
`codexPlugins` has no effect on NodoAssist-provider runs, ACP conversation
...
```bash
nodoassist migrate codex --dry-run --verify-plugin-apps
```
...
`enable`/`disable` write only to `~/.nodoassist/nodoassist.json`; they never edit
...
`auth_required`, reauthorize the app in Codex before enabling it in NodoAssist.
...
| Enabled    | NodoAssist config allows the plugin for Codex harness turns.                                                                     |
...
- During planning, NodoAssist reads source Codex `plugin/read` details and
...
NodoAssist reads Codex app inventory through app-server `app/list`, caches it
...
drops it, and NodoAssist rebuilds it from the next `app/list` read.
...
A plugin app is exposed only when NodoAssist can map it back to the migrated
...
NodoAssist injects a restrictive `config.apps` patch for the Codex thread:
...
get `open_world_enabled: true`. NodoAssist does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: NodoAssist returns a deterministic decline.
- `true`: NodoAssist auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: NodoAssist exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into NodoAssist plugin
  approvals before returning the Codex approval response.
- `"ask"`: NodoAssist uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, NodoAssist selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  NodoAssist; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                     |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in NodoAssist.                                                     |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | NodoAssist schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known. |
...
Codex thread bindings keep the app config they started with until NodoAssist
