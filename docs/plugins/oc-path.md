---
summary: "Bundled `oc-path` plugin: ships the `nodoassist path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`nodoassist path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the NodoAssist repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `nodoassist path … --json` instead of carrying separate markdown, JSONC,
...
nodoassist path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
nodoassist path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `nodoassist` CLI** on the host where you
...
Plugin metadata lives in `extensions/oc-path/nodoassist.plugin.json`:
...
lazily the first time you run `nodoassist path …`, so installs that never use
...
state. Bare `nodoassist path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
nodoassist plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `nodoassist path` CLI          | `extensions/oc-path/cli-registration.ts`                |
...
`__NODOASSIST_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`nodoassist path` CLI reference](/cli/path)
