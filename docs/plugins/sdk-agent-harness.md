...
An **agent harness** is the low level executor for one prepared NodoAssist agent
...
runtime and the normal NodoAssist provider transport is the wrong abstraction:
...
- a model runtime that needs its own resume id in addition to the NodoAssist
  session transcript
...
Before a harness is selected, NodoAssist has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the NodoAssist transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an NodoAssist-owned
policy bundle for runtime decisions that must stay shared across NodoAssist and
native harnesses:
...
Harnesses may use the plan for decisions that need to match NodoAssist behavior,
but treat it as host-owned attempt state: do not mutate it or use it to switch
providers/models inside a turn.

## Register a harness

**Import:** `nodoassist/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "nodoassist/plugin-sdk/agent-harness";
...
NodoAssist chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, NodoAssist uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, NodoAssist does not
...
`agents.list[].agentRuntime`, and `NODOASSIST_AGENT_RUNTIME`. `/status` shows the
...
NodoAssist. The harness then claims that provider in `supports(...)`.
...
- app-server request: NodoAssist sends the bare model id to Codex and lets the
...
NodoAssist requires Codex app-server `0.142.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers,
so NodoAssist only runs against the protocol surface it has tested.
...
seam is for async tool-result transforms that must run before NodoAssist or
...
`nodoassist/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so NodoAssist's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; NodoAssist does not infer it from assistant prose. The helper
...
`nodoassist/plugin-sdk/agent-harness-runtime` after they finalize an attempt. It
dispatches the portable `agent_end` hook and NodoAssist's research capture
...
user-input helpers from `nodoassist/plugin-sdk/agent-harness-runtime` to format
the prompt, deliver it through NodoAssist's blocking reply path, and normalize
...
`nodoassist/plugin-sdk/agent-harness-tool-runtime`. It owns
...
The bundled `codex` harness is the native Codex mode for embedded NodoAssist
...
`nodoassist doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. NodoAssist still owns the chat channel,
...
By default, NodoAssist uses `auto` provider/model runtime policy: registered
...
token. Keep that binding explicitly associated with the NodoAssist session, and
keep mirroring user-visible assistant/tool output into the NodoAssist
transcript.

The NodoAssist transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in NodoAssist harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so NodoAssist
can clear it when the owning NodoAssist session is reset.

## Tool and media results

Core constructs the NodoAssist tool list and passes it into the prepared
...
outputs on the same delivery path as NodoAssist-backed runs.
