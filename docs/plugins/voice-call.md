...
read_when:
  - You want to place an outbound voice call from NodoAssist
...
Voice calls for NodoAssist via a plugin: outbound notifications, multi-turn
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        nodoassist plugins install @nodoassist/voice-call
        ```
      </Tab>
  ...
  <Step title="Verify setup">
    ...
    nodoassist voicecall setup --json
    ```
...
| `store`                         | unset        | Overrides the default `~/.nodoassist/voice-calls` call-log path.                       |
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    release (`2026.6.0`), so run `nodoassist doctor --fix` to rewrite committed
...
- Voice Call exposes the shared `nodoassist_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal NodoAssist tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `nodoassist_agent_consult`.
...
configured NodoAssist agent without paying a full agent-consult round trip on
...
`nodoassist_agent_consult` still run the full NodoAssist agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call nodoassist_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `nodoassist doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
nodoassist voicecall call --to "+15555550123" --message "Hello from NodoAssist"
nodoassist voicecall start --to "+15555550123"   # alias for call
nodoassist voicecall continue --call-id <id> --message "Any questions?"
nodoassist voicecall speak --call-id <id> --message "One moment"
nodoassist voicecall dtmf --call-id <id> --digits "ww123456#"
nodoassist voicecall end --call-id <id>
nodoassist voicecall status --call-id <id>
nodoassist voicecall tail
nodoassist voicecall latency                      # summarize turn latency from logs
nodoassist voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL NodoAssist reconstructs
...
Use `nodoassist logs --follow` for the live phase trace. A healthy Twilio Meet
...
`nodoassist voicecall tail` still shows persisted call records; useful for
...
- `nodoassist logs --follow` shows realtime TwiML served, the realtime bridge started, and the initial greeting queued.
