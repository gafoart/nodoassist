...
read_when:
  ...
  - You need the NodoAssist plugin that owns the node-llama-cpp runtime
...
The main `nodoassist` npm package does not include `node-llama-cpp`. Keeping the
native dependency in this plugin prevents normal NodoAssist npm updates from
deleting a manually installed runtime inside the NodoAssist package directory.
...
If `node-llama-cpp` is missing or fails to load, NodoAssist reports the failure
with:

1. Install the plugin: `nodoassist plugins install @nodoassist/llama-cpp-provider`.
