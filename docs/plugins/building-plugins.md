---
summary: "Create your first NodoAssist plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new NodoAssist plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend NodoAssist without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the NodoAssist repository. Publish
the package to [ClawHub](/clawhub) and users install it with:

```bash
nodoassist plugins install clawhub:<package-name>
```
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because NodoAssist discovers
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect NodoAssist to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through NodoAssist model fallback.
  </Card>
...
```json nodoassist.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to NodoAssist",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    appear in `contracts.tools` so NodoAssist can discover ownership without
  ...
  <Step title="Register the tool">
    ...
    `defineChannelPluginEntry` from `nodoassist/plugin-sdk/core` instead.
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    nodoassist plugins inspect my-plugin --runtime --json
    ```

    If the plugin registers a CLI command, run that command too and confirm
    output, for example `nodoassist demo-plugin ping`.

    For a bundled plugin in this repository, NodoAssist discovers source-checkout
  ...
  <Step title="Test the package install">
    ...
    `nodoassist.extensions` at built JavaScript like `./dist/index.js`, and make
    ...
    nodoassist plugins install npm-pack:/tmp/<plugin-package>.tgz --force
    nodoassist plugins inspect my-plugin --runtime --json
    ```

    `npm-pack:` uses NodoAssist's managed per-plugin npm project, so it catches
  ...
  <Step title="Install">
    ...
    ```bash
    nodoassist plugins install clawhub:your-org/your-plugin
    ```
...
plugin is enabled. Optional tools need explicit user opt-in before NodoAssist
...
operator, installed plugin code, or a modified NodoAssist runtime. Sensitive
...
aligned with `api.registerTool(..., { optional: true })` so NodoAssist can avoid
...
```typescript
import { definePluginEntry } from "nodoassist/plugin-sdk";
```
...
`nodoassist/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `nodoassist` metadata</Check>
<Check>**nodoassist.plugin.json** manifest is present and valid</Check>
...
1. Watch [nodoassist/nodoassist](https://github.com/openclaw/openclaw/releases) releases (`Watch` > `Releases`). Beta tags look like `v2026.3.N-beta.1`. You can also follow [@nodoassist](https://x.com/openclaw) on X for release announcements.
