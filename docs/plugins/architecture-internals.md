...
At startup, NodoAssist does roughly this:
...
The manifest is the control-plane source of truth. NodoAssist uses it to:
...
derived set is empty, NodoAssist keeps the scope empty instead of widening to
...
NodoAssist does not cache plugin discovery results or direct manifest registry
...
NodoAssist still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, NodoAssist calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that NodoAssist no longer calls, such as
...
| _(built-in model lookup)_         | NodoAssist tries the normal registry/catalog path first                                                        | _(not a plugin hook)_                                                                                                                         |
...
| `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared NodoAssist refreshers                                                                                        |
...
that still runs on NodoAssist's normal inference loop.

`resolveUsageAuth` decides whether NodoAssist should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of NodoAssist's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as NodoAssist adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  schemas while NodoAssist owns the provider/runtime boundary.
...
- NodoAssist only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "NodoAssist plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope. The default surface (`gatewayRuntimeScopeSurface: "write-default"`) is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) and any non-trusted-proxy auth method get a single `operator.write` scope, even if the caller sends `x-nodoassist-scopes`
  - `trusted-proxy` callers without an explicit `x-nodoassist-scopes` header also keep the legacy `operator.write`-only surface
  - `trusted-proxy` callers that do send `x-nodoassist-scopes` get the declared scopes instead
  - a route can opt into `gatewayRuntimeScopeSurface: "trusted-operator"` to always honor `x-nodoassist-scopes` for identity-bearing auth modes (falling back to the full CLI default scope set when the header is absent)
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, opt into `trusted-operator` scope surface, require an identity-bearing auth mode, and document the explicit `x-nodoassist-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `nodoassist/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                               | Purpose                                                |
| ------------------------------------- | ------------------------------------------------------ |
| `nodoassist/plugin-sdk/plugin-entry`  | Plugin registration primitives                         |
| `nodoassist/plugin-sdk/channel-core`  | Channel entry/build helpers                            |
| `nodoassist/plugin-sdk/core`          | Generic shared helpers and umbrella contract           |
| `nodoassist/plugin-sdk/config-schema` | Root `nodoassist.json` Zod schema (`NodoAssistSchema`) |
...
<Info>
`nodoassist/plugin-sdk/channel-runtime`, `nodoassist/plugin-sdk/channel-lifecycle`,
small channel helper facades, `nodoassist/plugin-sdk/outbound-runtime`,
`nodoassist/plugin-sdk/outbound-send-deps`, `nodoassist/plugin-sdk/config-runtime`,
and `nodoassist/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `nodoassist/plugin-sdk/*` subpaths. Never
...
`nodoassist/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape NodoAssist writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to NodoAssist's
...
- if both `catalog` and `discovery` are registered, NodoAssist uses `catalog`
  and emits a warning
...
- Read-only command paths such as `nodoassist status`, `nodoassist status --all`,
  `nodoassist channels status`, `nodoassist channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `nodoassist.extensions`:
...
Security guardrail: every `nodoassist.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `nodoassist plugins install` installs plugin dependencies with a
...
Optional: `nodoassist.setupEntry` can point at a lightweight setup-only module.
When NodoAssist needs setup surfaces for a disabled channel plugin, or
...
Optional: `nodoassist.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let NodoAssist load the
...
Channel plugins can advertise setup/discovery metadata via `nodoassist.channel` and
install hints via `nodoassist.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@nodoassist/nextcloud-talk",
  "nodoassist": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@nodoassist/nextcloud-talk",
...
Useful `nodoassist.channel` fields beyond the minimal example:
...
NodoAssist can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.nodoassist/mpm/plugins.json`
- `~/.nodoassist/mpm/catalog.json`
- `~/.nodoassist/plugins/catalog.json`

Or point `NODOASSIST_PLUGIN_CATALOG_PATHS` (or `NODOASSIST_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "nodoassist": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `nodoassist.install` block. The
...
2. **Add typed plugin registration/runtime surfaces.** Extend
   `NodoAssistPluginApi` and/or `api.runtime` with the smallest useful typed
   capability surface.
...
This is how NodoAssist stays opinionated without becoming hardcoded to one
