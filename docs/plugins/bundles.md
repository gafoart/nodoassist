---
summary: "Install and use Codex, Claude, and Cursor bundles as NodoAssist plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how NodoAssist maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

NodoAssist can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** - content and metadata packs that
NodoAssist maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native NodoAssist plugins. Native plugins run
...
of requiring authors to rewrite them as native NodoAssist plugins, NodoAssist
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    nodoassist plugins install ./my-bundle

    # Archive
    nodoassist plugins install ./my-bundle.tgz

    # Claude marketplace
    nodoassist plugins marketplace list <source>
    nodoassist plugins install <plugin> --marketplace <source>
    ```
...
## What NodoAssist maps from bundles

Not every bundle feature runs in NodoAssist today. Here is what works and what
...
| ------------- | --------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal NodoAssist skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                          | Claude, Cursor |
| Hook packs    | NodoAssist-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded NodoAssist settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded NodoAssist LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded NodoAssist defaults                                     | Claude         |

#### Skill content

- Bundle skill roots load as normal NodoAssist skill roots.
- Claude `commands/` roots are treated as additional skill roots.
- Cursor `.cursor/commands/` roots are treated as additional skill roots.

Claude markdown command files and Cursor command markdown both work through the
normal NodoAssist skill loader.

#### Hook packs

Bundle hook roots work **only** when they use the normal NodoAssist hook-pack
layout: `HOOK.md` plus `handler.ts` or `handler.js`. Today this is primarily
the Codex-compatible case.

#### MCP for embedded NodoAssist

- Enabled bundles can contribute MCP server config.
- NodoAssist merges bundle MCP config into the effective embedded NodoAssist
  settings as `mcpServers`.
- NodoAssist exposes supported bundle MCP tools during embedded NodoAssist agent
...
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in NodoAssist config. `nodoassist mcp set` and `nodoassist doctor --fix` normalize the common alias.
...
NodoAssist registers bundle MCP tools with provider-safe names in the form
...
#### Embedded NodoAssist settings

Claude `settings.json` is imported as default embedded NodoAssist settings when
the bundle is enabled. NodoAssist sanitizes shell override keys before applying
them:

- `shellPath`
- `shellCommandPrefix`

#### Embedded NodoAssist LSP

- Enabled Claude bundles can contribute LSP server config.
- NodoAssist loads `.lsp.json` plus any manifest-declared `lspServers` paths.
- Bundle LSP config is merged into the effective embedded NodoAssist LSP
  defaults.
- Only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `nodoassist plugins inspect <id>`.

### Detected but not executed

These are recognized and shown in diagnostics, but NodoAssist does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit NodoAssist best when they use skill roots and NodoAssist-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded NodoAssist settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded NodoAssist
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded NodoAssist LSP defaults
...
NodoAssist checks for native plugin format first:

1. `nodoassist.plugin.json` or a valid `package.json` with `nodoassist.extensions` - treated as a **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) - treated as a **bundle**

If a directory contains both, NodoAssist uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `nodoassist plugins install` and ship everything
  they need in the installed plugin directory.
- NodoAssist-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `nodoassist doctor --fix` removes stale local bundled-plugin install records
...
- NodoAssist does **not** load arbitrary bundle runtime modules in-process.
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `nodoassist plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded NodoAssist settings from `settings.json` are supported. NodoAssist does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    NodoAssist hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
