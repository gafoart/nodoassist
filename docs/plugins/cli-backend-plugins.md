...
read_when:
  ...
  - You need to map a third-party CLI into NodoAssist's text fallback runner
---

CLI backend plugins let NodoAssist call a local AI CLI as a text inference
...
| -------------------- | ------------------------ | --------------------------------------------------------- |
| Package entry        | `package.json`           | Points NodoAssist at the plugin runtime module            |
| Manifest ownership   | `nodoassist.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `nodoassist.runtimeExtensions` pointing at the
  ...
  <Step title="Declare backend ownership">
    ```json nodoassist.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through NodoAssist",
    ...
    `cliBackends` is the runtime ownership list; it lets NodoAssist auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "nodoassist/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how NodoAssist should launch and parse the CLI:
...
| `modelAliases`                                            | Map NodoAssist model ids to CLI-native ids                                        |
| `sessionArg` / `sessionArgs`                              | How to pass a session id                                                          |
| `sessionMode`                                             | `always`, `existing`, or `none`                                                   |
| `sessionIdFields`                                         | JSON fields NodoAssist reads from CLI output                                      |
...
| `defaultAuthProfileId`             | Prefer a specific NodoAssist auth profile                                   |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into NodoAssist's loopback MCP tool bridge                              |
| `ownsNativeCompaction`             | Backend owns its own compaction - NodoAssist defers                         |
...
`sideQuestionToolMode: "disabled"`; otherwise NodoAssist fails closed when BTW
requires a no-tools CLI run.

### `ownsNativeCompaction`: opting out of NodoAssist compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so NodoAssist's safeguard summarizer never runs
...
over-budget session can stay over budget or go stale (NodoAssist no longer
rescues it):
...
CLI backends do not receive NodoAssist tools by default. If the CLI can consume
...
"always-on"` so NodoAssist can fail closed when a caller requires no native
tools.
...
```bash
nodoassist plugins inspect acme-cli --runtime --json
nodoassist agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `nodoassist.extensions` and built runtime entries for published packages</Check>
<Check>`nodoassist.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
