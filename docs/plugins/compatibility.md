...
read_when:
  - You maintain an NodoAssist plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

NodoAssist keeps older plugin contracts wired through named compatibility
...
NodoAssist should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `nodoassist/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `nodoassist/extension-api`,
  `nodoassist/plugin-sdk/channel-runtime`, `nodoassist/plugin-sdk/command-auth`
  status builders, `nodoassist/plugin-sdk/test-utils` (replaced by focused
  `nodoassist/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `NodoAssistSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while
  plugins move to `nodoassist/plugin-sdk/channel-route`
...
- channel `showConfigured` / `showInSetup` metadata while channel packages
  move to `nodoassist.channel.exposure`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `nodoassist plugins registry --refresh`
...
machine-readable output in CI annotations. NodoAssist core should expose
contracts and fixtures the inspector can consume, but should not publish the
inspector binary from the main `nodoassist` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against NodoAssist plugin
packages. Run it from a clean NodoAssist checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @nodoassist/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @nodoassist/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @nodoassist/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
