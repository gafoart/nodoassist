...
Plugin hooks are in-process extension points for NodoAssist plugins: inspect or
...
plugin that registered that handler. NodoAssist injects it per handler without
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when NodoAssist has resolved the child session's native model before launch.
...
use `nodoassist sessions` or the Gateway session/status surfaces. To debug
...
- NodoAssist strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, NodoAssist stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when NodoAssist can
...
the conversation target identifier when NodoAssist can derive one from the
...
`nodoassist/plugin-sdk/channel-inbound`:

```ts
declare module "nodoassist/plugin-sdk/channel-inbound" {
...
pending forever. A timeout is logged and NodoAssist continues; it does not
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when NodoAssist can derive a
...
Handlers have a 15s default budget; on timeout, NodoAssist logs the failure and
continues with the original final answer.
Codex native `Stop` hooks are relayed into this hook as NodoAssist
...
behavior). NodoAssist drains queued injections before prompt hooks, drops
...
policy runs from NodoAssist config, covers CLI install and update paths, and
fails closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the NodoAssist process where plugin hooks have
...
NodoAssist no longer runs built-in install-time dangerous-code blocking, so it
...
syncing external wake schedulers, and keep NodoAssist as the source of truth
