...
read_when:
  - You see the NODOASSIST_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the NODOASSIST_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before NodoAssist 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external NodoAssist plugin
---

NodoAssist replaced a broad backwards-compatibility layer with a modern plugin
...
- **`nodoassist/plugin-sdk/compat`** - re-exported dozens of helpers to keep
  older hook-based plugins working while the new architecture was built.
- **`nodoassist/plugin-sdk/infra-runtime`** - a broad barrel mixing system
  events, heartbeat state, delivery queues, fetch/proxy helpers, file helpers,
  approval types, and unrelated utilities.
- **`nodoassist/plugin-sdk/config-runtime`** - a broad config barrel still
  carrying deprecated direct load/write helpers during the migration window.
- **`nodoassist/extension-api`** - a bridge giving plugins direct access to
...
NodoAssist does not remove or reinterpret documented plugin behavior in the same
...
Each `nodoassist/plugin-sdk/<subpath>` is now a small, self-contained module with
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    New plugin code should avoid the broad `nodoassist/plugin-sdk/config-runtime`
    barrel. Use the narrow subpath for the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `NodoAssistConfig` | `nodoassist/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `nodoassist/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `nodoassist/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `nodoassist/plugin-sdk/config-mutation` |
    | Session store helpers | `nodoassist/plugin-sdk/session-store-runtime` |
    | Markdown table config | `nodoassist/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `nodoassist/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `nodoassist/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `nodoassist/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // NodoAssist and Codex runtime dynamic tools
    api.registerAgentToolResultMiddleware(async (event) => {
      return compactToolResult(event);
    }, {
      runtimes: ["nodoassist", "codex"],
    });
    ```
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects (clients, tokens, Bolt apps)
      through `nodoassist/plugin-sdk/channel-runtime-context`.
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `nodoassist/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "nodoassist/extension-api" my-plugin/
    ```
  </Step>
  ...
  <Step title="Replace with focused imports">
    ...
    } from "nodoassist/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "nodoassist/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "nodoassist/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "nodoassist/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "nodoassist/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `nodoassist/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused surface it actually
    needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `nodoassist/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `nodoassist/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `nodoassist/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `nodoassist/plugin-sdk/channel-activity-runtime` |
    | In-memory and persistent-backed dedupe caches | `nodoassist/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `nodoassist/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `nodoassist/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `nodoassist/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `nodoassist/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `nodoassist/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `nodoassist/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `nodoassist/plugin-sdk/error-runtime` |
    | Transport readiness waits | `nodoassist/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `nodoassist/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `nodoassist/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `nodoassist/plugin-sdk/number-runtime` |
    | Process-local async lock | `nodoassist/plugin-sdk/async-lock-runtime` |
    | File locks | `nodoassist/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code uses `nodoassist/plugin-sdk/channel-route`. The older
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `NodoAssistSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | NodoAssist-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
import the published `@nodoassist/discord` package directly. Owner-specific
...
<AccordionGroup>
  <Accordion title="command-auth help builders -> command-status">
    **Old (`nodoassist/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`nodoassist/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "nodoassist/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "nodoassist/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers -> resolveInboundMentionDecision">
    **Old**: `resolveMentionGating(params)` and
    ...
    `nodoassist/plugin-sdk/channel-inbound` or
    `nodoassist/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `nodoassist/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `nodoassist/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `nodoassist/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper -> createTool() on the plugin">
    **Old**: `tool()` factory from `nodoassist/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    NodoAssist no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks -> resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. NodoAssist downgrades stale stored values by profile rank
    automatically.
  ...
  <Accordion title="NodoAssistSchemaType alias -> NodoAssistConfig">
    `NodoAssistSchemaType` re-exported from `nodoassist/plugin-sdk` is now a
    one-line alias for `NodoAssistConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { NodoAssistSchemaType } from "nodoassist/plugin-sdk";
    // After
    import type { NodoAssistConfig } from "nodoassist/plugin-sdk/config-schema";
    ```
...
session controller exported by `nodoassist/plugin-sdk/realtime-voice`. The
...
`talk.providers` should be repaired with `nodoassist doctor --fix`; runtime Talk
...
```bash
NODOASSIST_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 nodoassist gateway run
NODOASSIST_SUPPRESS_EXTENSION_API_WARNING=1 nodoassist gateway run
```
