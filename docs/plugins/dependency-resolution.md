---
summary: "How NodoAssist installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged NodoAssist installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

NodoAssist handles plugin dependencies at install/update time only. Runtime
loading never runs a package manager, repairs a dependency tree, or mutates
the NodoAssist package directory.
...
- SDK/core imports are peer or supplied NodoAssist imports.
- Local development plugins bring their own already-installed dependencies.
- npm and git plugins install into NodoAssist-owned package roots.

NodoAssist owns only the plugin lifecycle:
...
NodoAssist uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.nodoassist/npm/projects/<encoded-package>`.
- git packages clone under `~/.nodoassist/git`.
...
```bash
cd ~/.nodoassist/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`nodoassist plugins install npm-pack:<path.tgz>` uses the same per-plugin npm
project root for a local npm-pack tarball: NodoAssist reads the tarball's npm
...
`~/.nodoassist/npm/projects/<encoded-package>` can unblock a temporary
...
`node_modules` beside the plugin package. NodoAssist scans the managed project
...
publishable lockfile during install, and NodoAssist's managed npm project root
supports it through the normal install path. NodoAssist-owned publishable
...
`nodoassist.release.publishToNpm: true`. Third-party plugin packages may also
ship a shrinkwrap; NodoAssist does not require one for community packages, but
...
NodoAssist-owned npm plugin packages can also publish with explicit
...
`nodoassist.release.bundleRuntimeDependencies: false`; they still ship a
shrinkwrap, but npm resolves runtime dependencies during install instead of
embedding every platform binary in the plugin tarball. The root `nodoassist`
package does not bundle its full dependency tree.

Plugins that import `nodoassist/plugin-sdk/*` declare `nodoassist` as a peer
dependency. NodoAssist does not let npm install a separate registry copy of the
...
resolution/materialization, and NodoAssist reasserts plugin-local
`node_modules/nodoassist` links for installed packages that declare the host
...
Local plugins are developer-controlled directories. NodoAssist never runs
...
```bash
nodoassist plugins update <id>
nodoassist plugins install <source>
nodoassist doctor --fix
```

`doctor --fix` cleans legacy NodoAssist-generated dependency state and can
...
Lightweight and core-critical bundled plugins ship as part of NodoAssist. They
...
In source checkouts, NodoAssist treats the repository as a pnpm monorepo.
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g nodoassist`      | Built runtime tree inside the package | NodoAssist package and explicit plugin install/update/doctor flows   |
...
| `nodoassist plugins install ...` | Managed npm project/git/ClawHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older NodoAssist versions generated bundled-plugin dependency roots at startup
...
`plugin-runtime-deps` targets, `.nodoassist-runtime-deps*` manifests, generated
...
Older npm installs also used a shared `~/.nodoassist/npm/node_modules` root.
