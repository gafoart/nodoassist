...
NodoAssist-owned state stays on the Gateway under `<state-dir>/logbook/`, but
...
- A connected node that exposes `screen.snapshot` or `logbook.snapshot`. The
  ...
  (`nodoassist node host run`) gets the plugin-provided `logbook.snapshot`
...
- The bundled Codex plugin enabled and authenticated. Codex currently provides
  ...
  `nodoassist models auth login --provider openai`; see
...
```bash
nodoassist plugins enable codex
nodoassist plugins enable logbook
```
...
nodoassist plugins inspect logbook --runtime --json
...
1. `nodoassist plugins list --enabled` includes `logbook`.
