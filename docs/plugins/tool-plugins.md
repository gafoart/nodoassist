---
summary: "Build simple typed agent tools with defineToolPlugin and nodoassist plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple NodoAssist plugin that only adds agent tools
...
manifest metadata NodoAssist needs to discover tools without loading plugin
runtime code.
...
- `nodoassist >=2026.5.17`, the first version that exports
  `nodoassist/plugin-sdk/tool-plugin`.
- A package root that ships `dist/`, `nodoassist.plugin.json`, and
  `package.json`.
...
| `package.json`           | Scripts, runtime deps, `nodoassist.extensions: ["./dist/index.js"]` |
| `nodoassist.plugin.json` | Generated manifest metadata for the initial tool                    |

`npm run plugin:build` runs `npm run build` (tsc) then
`nodoassist plugins build --entry ./dist/index.js`. `npm run plugin:validate`
rebuilds and runs `nodoassist plugins validate --entry ./dist/index.js`.
...
`nodoassist plugins init <id>` options:
...
is sent to a model. `nodoassist plugins build` writes the matching
`toolMetadata.<tool>.optional` manifest entry, so NodoAssist can see that the
...
`defineToolPlugin` wraps plain return values into the NodoAssist tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and NodoAssist to keep the original value in `details`.
...
`configSchema` is optional. Omit it and NodoAssist applies a strict empty object
...
NodoAssist reads plugin config from the plugin's entry in the Gateway config. Do
...
NodoAssist must read the plugin manifest before importing plugin runtime code.
`defineToolPlugin` exposes static metadata for this, and
`nodoassist plugins build` writes it into the package. Rerun the generator after
changing plugin id, name, description, config schema, activation, or tool
names:

```bash
npm run build
nodoassist plugins build --entry ./dist/index.js
```
...
`contracts.tools` is the important discovery contract: it tells NodoAssist which
...
`nodoassist plugins build` also aligns `package.json` to the selected runtime
entry:
...
```bash
npm run build
nodoassist plugins build --entry ./dist/index.js --check
nodoassist plugins validate --entry ./dist/index.js
npm test
```

`plugins validate` checks that:

- `nodoassist.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `nodoassist.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate NodoAssist checkout or installed CLI, install the package path:

```bash
nodoassist plugins install ./stock-quotes
nodoassist plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
nodoassist plugins install npm-pack:./nodoassist-plugin-stock-quotes-0.1.0.tgz
nodoassist plugins inspect stock-quotes --runtime --json
```
...
```bash
nodoassist plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs still install from npm during the launch cutover, but
ClawHub is the preferred discovery and distribution surface for NodoAssist
...
`nodoassist plugins build --entry ./dist/index.js` or
`nodoassist plugins validate --entry ./dist/index.js`.
...
### `nodoassist.plugin.json generated metadata is stale`
...
Commit both `nodoassist.plugin.json` and `package.json` changes.

### `package.json nodoassist.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`nodoassist plugins build --entry ./dist/index.js` so the generator aligns
...
1. `nodoassist plugins inspect <plugin-id> --runtime`
2. `nodoassist plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `nodoassist.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `nodoassist.extensions: ["./dist/index.js"]`.
