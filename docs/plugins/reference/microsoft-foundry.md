---
summary: "Adds Microsoft Foundry model provider support to NodoAssist."
read_when:
...
Adds Microsoft Foundry model provider support to NodoAssist.

## Distribution

- Package: `@nodoassist/microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. NodoAssist refreshes Microsoft Foundry runtime tokens through
...
NodoAssist uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so NodoAssist
...
deployment through onboarding or include model metadata so NodoAssist can verify
