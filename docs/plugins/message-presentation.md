...
Message presentation is NodoAssist's shared contract for rich outbound chat UI.
...
```bash
nodoassist message send --channel slack \
  --target channel:C123 \
...
```bash
nodoassist message send --channel telegram \
...
| Matrix          | Text fallback plus structured event field | Buttons/selects advertise as supported, but every block currently renders as `renderMessagePresentationFallbackText` output carried in a `com.nodoassist.presentation` event field, not native interactive widgets. |
...
Use helpers from `nodoassist/plugin-sdk/interactive-runtime` when bridging older
code:
