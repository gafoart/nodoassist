...
NodoAssist Gateway. It is not a replacement for GitHub Issues, Linear, Jira, or
...
```bash
nodoassist plugins disable workboard
nodoassist gateway restart
```
...
that Gateway's NodoAssist state (see [Storage](#storage)).
...
NodoAssist subagent sessions still own execution. One dispatch pass:
...
Gateway (`NODOASSIST_GATEWAY_URL` or `gateway.mode: remote`) apply, the CLI runs
...
`defaultAssignee`, and `orchestratorProfile`. NodoAssist records this intent and
...
nodoassist workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
nodoassist workboard show <card-id> [--json]
nodoassist workboard dispatch [--board <id>] [--json]
```
...
under the NodoAssist state directory: boards, cards, labels, lifecycle events,
...
`nodoassist doctor --fix` to migrate the shipped legacy plugin-state namespaces
