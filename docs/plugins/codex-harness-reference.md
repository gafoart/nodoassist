...
By default NodoAssist starts the managed Codex binary shipped with the bundled
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after NodoAssist builds its inherited environment.                                                                                                                                                                                                                                                                   |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active NodoAssist sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows NodoAssist sandbox egress.                                                                                                                                                                                           |
...
contract. When enabled, NodoAssist also sets `features.network_proxy.enabled` and
`default_permissions` in the Codex thread config so the generated permission
profile can start Codex-managed networking. NodoAssist generates a
collision-resistant `nodoassist-network-<fingerprint>` profile name from the
...
NodoAssist treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before NodoAssist builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent. When native
Codex plugins are configured, NodoAssist uses the connected app-server's plugin
...
still the authoritative inventory and metadata source, but NodoAssist policy
...
plugins via `plugin/install` and refreshes inventory. Only connect NodoAssist to
remote app-servers that are trusted to accept NodoAssist-managed plugin installs
...
unattended NodoAssist turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, NodoAssist treats the implicit default as guardian
...
When an NodoAssist sandbox is active, the local Codex app-server process still
runs on the Gateway host. NodoAssist therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the NodoAssist sandbox
backend. Shell access is exposed through NodoAssist sandbox-backed dynamic tools
such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

<Note>
On Docker-backed NodoAssist sandbox hosts (`agents.defaults.sandbox.mode` set to
a Docker backend), `nodoassist doctor` probes whether the host allows the
...
Ubuntu/AppArmor hosts. Fix the reported host namespace policy for the NodoAssist
...
The stable default is fail-closed: active NodoAssist sandboxing disables native
...
to try Codex's remote environment support with NodoAssist's sandbox backend.
...
When the flag is on and the current NodoAssist session is sandboxed, NodoAssist
...
NodoAssist-owned environment. If the app-server cannot register the environment,
...
the loopback exec-server unless it is running on the same host, so NodoAssist
...
1. An explicit NodoAssist Codex auth profile for the agent.
2. The app-server's existing account in that agent's Codex home.
3. For local stdio app-server launches only, `CODEX_API_KEY`, then
   `OPENAI_API_KEY`, when no app-server account is present and OpenAI auth is
   still required.

When NodoAssist sees a ChatGPT subscription-style Codex auth profile (OAuth or
...
Stdio app-server launches inherit NodoAssist's process environment by default.
NodoAssist owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's NodoAssist state. That keeps Codex
config, accounts, plugin cache/data, and thread state scoped to the NodoAssist
...
NodoAssist skips its auth-profile bridge for the app-server. Verified owner
...
continuing it in NodoAssist; independent Codex processes do not coordinate
concurrent writers for the same thread.

NodoAssist does not rewrite `HOME` for normal local app-server launches.
Codex-run subprocesses such as `nodoassist`, `gh`, `git`, cloud CLIs, and shell
...
In the default agent scope, NodoAssist plugins and NodoAssist skill snapshots
still flow through NodoAssist's own plugin registry and skill loader; personal
Codex `~/.codex` assets do not. If you have useful Codex CLI skills or
plugins from a Codex home that should become part of an isolated NodoAssist
agent, inventory them explicitly:

```bash
nodoassist migrate codex --dry-run
nodoassist migrate apply codex --yes
```
...
NodoAssist removes `CODEX_HOME` and `HOME` from this list during local launch
...
`nodoassist` namespace with `deferLoading: true`. NodoAssist does not expose
...
Most remaining NodoAssist integration tools, such as messaging, media, cron,
...
`sessions_spawn` remains available for explicit NodoAssist or ACP delegation.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, NodoAssist aborts the
...
releases the NodoAssist session lane so follow-up chat messages are not queued
...
post-tool idle budget: after NodoAssist returns an `item/tool/call` response,
...
NodoAssist best-effort interrupts the native turn and releases the session
...
NodoAssist upgrades the bundled `@openai/codex` version or when a deployment
...
If discovery fails or times out, NodoAssist uses a bundled fallback catalog:
...
NodoAssist does not write synthetic Codex project-doc files or depend on Codex
fallback filenames for persona files, because Codex fallbacks only apply when
`AGENTS.md` is missing.

For NodoAssist workspace parity, the Codex harness forwards the other
...
- `SOUL.md`, `IDENTITY.md`, and `USER.md` are forwarded as **turn-scoped**
  ...
  user profile.
- The compact loaded NodoAssist skills list is also forwarded as turn-scoped
...
- `BOOTSTRAP.md`, when present, is forwarded as NodoAssist turn input reference
  context.
...
`NODOASSIST_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
