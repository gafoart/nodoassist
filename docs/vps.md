---
summary: "Run NodoAssist on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for NodoAssist
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the NodoAssist Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install NodoAssist on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep NodoAssist bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/nodoassist-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `NODOASSIST_NO_RESPAWN=1` keeps routine Gateway restarts in-process, which avoids extra process handoffs and keeps PID tracking simple on small hosts.
...
- Service env for a stable startup path: `NODOASSIST_NO_RESPAWN=1` and
  `NODE_COMPILE_CACHE=/var/tmp/nodoassist-compile-cache`
...
The standard `nodoassist onboard --install-daemon` path installs a systemd user
unit; edit it with:
...
`sudo systemctl edit nodoassist-gateway.service`.
