...
read_when:
  - You want to use Featherless AI with NodoAssist
...
OpenAI-compatible API. NodoAssist installs Featherless as an official external
...
| Package         | `@nodoassist/featherless-provider`       |
...
```bash
nodoassist onboard --auth-choice featherless-api-key
```
...
```bash
nodoassist models list --provider featherless
```

## Default model

The plugin uses `Qwen/Qwen3-32B` as the setup default because Featherless
documents native tool calling for the Qwen 3 family. NodoAssist configures its
...
modes and NodoAssist does not embed account-specific plan or request-pricing
rates.
...
NodoAssist deliberately does not copy Featherless's full public model index into
...
- Managed Gateway cannot see the key: put it in `~/.nodoassist/.env` or another
