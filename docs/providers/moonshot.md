...
Kimi K2.7 Code always uses native thinking. NodoAssist exposes only the `on`
    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        nodoassist onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        nodoassist onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        nodoassist models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        NODOASSIST_CONFIG_PATH=/tmp/nodoassist-kimi/nodoassist.json \
        NODOASSIST_STATE_DIR=/tmp/nodoassist-kimi \
  ...
  <Tab title="Kimi Coding">
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      <Step title="Install the plugin">
        ```bash
        nodoassist plugins install @nodoassist/kimi-provider
        nodoassist gateway restart
        ```
      </Step>
      <Step title="Run onboarding">
        ```bash
        nodoassist onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        nodoassist models list --provider kimi
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    omit the `thinking` field for this model, so NodoAssist exposes only `on` and
    ignores stale `off` settings. K2.7 also fixes `temperature`, `top_p`, `n`,
    `presence_penalty`, and `frequency_penalty`; NodoAssist omits configured
    ...
    NodoAssist maps runtime `/think` levels for those models:
    ...
    default strategy. NodoAssist only forwards `thinking.keep` for
    `moonshot/kimi-k2.6` and strips it from other models. Kimi K2.7 Code
    preserves full reasoning history by default while NodoAssist omits the entire
    `thinking` field.
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves native tool_call ids shaped like `functions.<name>:<index>`. NodoAssist preserves the first occurrence of each native Kimi id and rewrites later duplicates to deterministic OpenAI-style `call_*` ids. Matching tool results are remapped with the same id so replay remains unique without stripping Kimi's first native id. This behavior is wired into the bundled Moonshot provider and is not a user-configurable setting.
  </Accordion>
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    NodoAssist keys this off the endpoint host, not the provider id, so a custom
