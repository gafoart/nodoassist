---
summary: "Use Qianfan's unified API to access many models in NodoAssist"
read_when:
...
Qianfan is Baidu's MaaS platform: a unified, OpenAI-compatible API that routes requests to many models behind a single endpoint and API key. NodoAssist ships it as the official external plugin `@nodoassist/qianfan-provider`.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nodoassist onboard --auth-choice qianfan-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    nodoassist models list --provider qianfan
    ```
  </Step>
</Steps>
