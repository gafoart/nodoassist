---
summary: "Use the Qwen Portal provider id with NodoAssist"
read_when:
...
(`@nodoassist/qwen-provider`). It targets the Qwen Portal endpoint at
...
NodoAssist has two Qwen-facing provider ids:
...
nodoassist models set qwen/qwen3-coder-plus
```

## Migration

Legacy Qwen Portal OAuth profiles are not refreshable; `nodoassist doctor` flags
