---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with NodoAssist"
read_when:
  - You want Xiaomi MiMo models in NodoAssist
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    ```bash
    nodoassist onboard --auth-choice xiaomi-api-key
    ```

    Token Plan:

    ```bash
    nodoassist onboard --auth-choice xiaomi-token-plan-sgp
    ```

    Or pass the keys directly:

    ```bash
    nodoassist onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    nodoassist onboard --auth-choice xiaomi-token-plan-sgp --xiaomi-token-plan-api-key "$XIAOMI_TOKEN_PLAN_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    nodoassist models list --provider xiaomi
    nodoassist models list --provider xiaomi-token-plan
    ```
  </Step>
</Steps>
...
NodoAssist's [`/think` directive](/tools/thinking) with levels `off`,
...
`audio.voice`, so NodoAssist sends `speakerVoice` for those models.
...
the desired voice description; NodoAssist sends it as the `user` message, sends
...
Matrix, Telegram, and WhatsApp), NodoAssist transcodes Xiaomi output to 48kHz
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.nodoassist/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.nodoassist/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full NodoAssist configuration reference.
  </Card>
