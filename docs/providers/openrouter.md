---
summary: "Use OpenRouter's unified API to access many models in NodoAssist"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in NodoAssist
...
OpenAI-compatible, so NodoAssist talks to it over the same
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        nodoassist onboard --auth-choice openrouter-oauth
        ```

        NodoAssist opens OpenRouter's browser sign-in flow (PKCE), exchanges the
        code for an OpenRouter API key, and stores it in the default
        OpenRouter auth profile. On remote/headless hosts, NodoAssist prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        nodoassist models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        nodoassist onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
NodoAssist sends image requests to OpenRouter's chat-completions image API with
...
NodoAssist submits text-to-video and image-to-video jobs, polls the returned
...
and also exposes `google/lyria-3-clip-preview`. NodoAssist sends `modalities:
...
NodoAssist sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter's STT contract), not as multipart OpenAI form
uploads.

## Fusion router

OpenRouter Fusion sends one NodoAssist model ref to several OpenRouter models in
...
`openrouter/fusion`, so the NodoAssist model ref carries both the NodoAssist
provider prefix and the upstream OpenRouter namespace:

```bash
nodoassist models set openrouter/openrouter/fusion
```
...
in normal agent/chat turns to try to force Fusion: NodoAssist turns can include
...
NodoAssist adds a sanitized system-prompt note listing the configured analysis
...
```bash
nodoassist infer model run --local \
...
login flow that issues an OpenRouter API key, so NodoAssist stores the result in
...
```bash
nodoassist models auth login --provider openrouter --method oauth
nodoassist models auth login --provider openrouter --method api-key
```

On verified OpenRouter requests (`https://openrouter.ai/api/v1`), NodoAssist adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, NodoAssist
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    NodoAssist sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, NodoAssist maps the selected thinking level
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    V4's required follow-up shape. NodoAssist sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: NodoAssist keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    NodoAssist forwards that object to OpenRouter as the request `provider`
