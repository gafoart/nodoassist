---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with NodoAssist"
read_when:
  - You want ElevenLabs text-to-speech in NodoAssist
...
NodoAssist uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | NodoAssist surface                                                   | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. NodoAssist keeps
...
returned audio stream instead of waiting for NodoAssist to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; NodoAssist omits that parameter for
...
NodoAssist sends multipart audio to ElevenLabs `/v1/speech-to-text` with
