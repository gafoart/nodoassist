...
title: "LongCat"
read_when:
  - You want to use LongCat-2.0 with NodoAssist
  - You need the LongCat API key or model limits
---

[LongCat](https://longcat.ai) provides a hosted API for LongCat-2.0, a
reasoning model built for coding and agentic workloads. NodoAssist provides the
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nodoassist onboard --auth-choice longcat-api-key
    ```
  </Step>
  <Step title="Verify the model">
    ```bash
    nodoassist models list --provider longcat
    ```
  </Step>
</Steps>
...
LongCat exposes binary thinking control. NodoAssist maps enabled thinking levels
to `thinking: { type: "enabled" }` and `/think off` to
`thinking: { type: "disabled" }`. LongCat does not currently document
`reasoning_effort`, so NodoAssist does not send it.

LongCat returns reasoning in `reasoning_content`. NodoAssist preserves that field
...
model through an OpenAI-compatible runtime and use NodoAssist's existing
...
<AccordionGroup>
  <Accordion title="The key works in a shell but not in the Gateway">
    ...
    variable. Put `LONGCAT_API_KEY` in `~/.nodoassist/.env`, configure it through
  ...
  <Accordion title="The model does not appear">
    Run `nodoassist plugins list` and confirm the `longcat` plugin is
    enabled, then run `nodoassist models list --provider longcat`.
  </Accordion>
</AccordionGroup>
