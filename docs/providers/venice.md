---
summary: "Use Venice AI privacy-focused models in NodoAssist"
read_when:
  - You want privacy-focused inference in NodoAssist
...
<Steps>
  ...
  <Step title="Configure NodoAssist">
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        nodoassist onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    nodoassist agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
```bash
nodoassist models set venice/kimi-k2-5
nodoassist models list --all --provider venice
```

You can also run `nodoassist configure` and pick **Model/auth provider > Venice AI**.
...
The bundled catalog above is a manifest-backed seed list. At runtime NodoAssist
...
`deepseek-v4-flash`, NodoAssist fills the required `reasoning_content` replay
...
nodoassist agent --model venice/kimi-k2-5 --message "Quick health check"

# Claude Opus via Venice (anonymized)
nodoassist agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Uncensored model
nodoassist agent --model venice/venice-uncensored --message "Draft options"

# Vision model with image
nodoassist agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Coding model
nodoassist agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  <Accordion title="API key not recognized">
    ```bash
    echo $VENICE_API_KEY
    nodoassist models list | grep venice
    ```
  ...
  <Accordion title="Model not available">
    Run `nodoassist models list --all --provider venice` to see currently
