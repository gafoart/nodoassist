---
summary: "Use xAI Grok models in NodoAssist"
read_when:
  - You want to use Grok models in NodoAssist
  - You are configuring xAI auth or model ids
title: "xAI"
---

NodoAssist ships a bundled `xai` provider plugin for Grok models. The
...
show Grok Build on the consent screen because NodoAssist uses xAI's shared
OAuth client.
...
<Steps>
  ...
  <Step title="Existing install">
    ...
    ```bash
    nodoassist models set xai/grok-4.3
    ```
...
<Note>
NodoAssist uses the xAI Responses API as the bundled xAI transport. The same
credential from `nodoassist models auth login --provider xai --method oauth` or
...
- For SSH, Docker, VPS, or other remote setups, use
  `nodoassist models auth login --provider xai --method oauth`; it uses
...
- Inspect saved xAI auth profiles:
  ...
  ```bash
  nodoassist models auth list --provider xai
...
<Tip>
Use `xai-oauth` when signing in from SSH, Docker, or a VPS. NodoAssist prints a
...
The bundled plugin maps xAI's current public API surface onto NodoAssist's
...
| xAI capability             | NodoAssist surface                      | Status                                                                |
| -------------------------- | --------------------------------------- | --------------------------------------------------------------------- |
...
| Streaming TTS              | -                                       | Not exposed; NodoAssist's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` media understanding | Yes                                                                   |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`  | Yes                                                                   |
| Realtime voice             | -                                       | Not exposed yet; needs a different session/WebSocket contract         |
| Files / batches            | Generic model API compatibility only    | Not a first-class NodoAssist tool                                     |

<Note>
NodoAssist uses xAI's REST image/video/TTS/STT APIs for media generation and
...
<AccordionGroup>
  <Accordion title="Web search">
    ...
    ```bash
    nodoassist models auth login --provider xai --method oauth
    nodoassist config set tools.web.search.provider grok
    ```
  ...
  <Accordion title="Video generation">
    ...
    NodoAssist encodes those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    NodoAssist asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. NodoAssist forwards only the shared
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    NodoAssist uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming
    TTS over WebSocket, but the NodoAssist speech provider contract currently
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through NodoAssist's
    ...
    transcription request. Prompt hints are accepted by the shared NodoAssist
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an NodoAssist tool for
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an NodoAssist tool for
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config
      ...
      even though NodoAssist does not require the Grok Build app.
    - NodoAssist does not currently expose the xAI multi-agent model family. xAI
      serves these models through the Responses API, but they do not accept
      the client-side or custom tools used by NodoAssist's shared agent loop.
      See the
      [xAI multi-agent limitations](https://docs.x.ai/developers/model-capabilities/text/multi-agent#limitations).
    - xAI Realtime voice is not registered as an NodoAssist provider yet. It
  ...
  <Accordion title="Advanced notes">
    - NodoAssist applies xAI-specific tool-schema and tool-call compatibility
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as NodoAssist
      tools. NodoAssist attaches only the specific xAI built-in each tool needs
...
NODOASSIST_LIVE_TEST=1 NODOASSIST_LIVE_TEST_QUIET=1 pnpm test:live -- extensions/xai/xai.live.test.ts
NODOASSIST_LIVE_TEST=1 NODOASSIST_LIVE_TEST_QUIET=1 NODOASSIST_LIVE_IMAGE_GENERATION_PROVIDERS=xai pnpm test:live -- test/image-generation.runtime.live.test.ts
```
...
The shared image live file verifies the same xAI provider through NodoAssist's
