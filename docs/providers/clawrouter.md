...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in NodoAssist
---

ClawRouter gives NodoAssist one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on NodoAssist usage surfaces.
...
NodoAssist host. The plugin ships bundled with NodoAssist (`enabledByDefault: true`);
you only need an issued ClawRouter credential.

| Property      | Value                                    |
| ------------- | ---------------------------------------- |
| Provider      | `clawrouter`                             |
| Plugin        | bundled (included in NodoAssist)         |
...
<Steps>
  ...
  <Step title="Configure NodoAssist">
    ...
    nodoassist onboard --auth-choice clawrouter-api-key
    nodoassist plugins enable clawrouter
    ```
  ...
  <Step title="List granted models">
    ```bash
    nodoassist models list --all --provider clawrouter
    ```
  ...
  <Step title="Select a model">
    ```bash
    nodoassist models set clawrouter/<provider>/<model>
    ```

    You can also select a returned model for one run with
    `nodoassist agent --model clawrouter/<provider>/<model> --message "..."`.
...
supported request routes. NodoAssist does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as an NodoAssist model when:
...
Adding a model to a supported ClawRouter provider needs no NodoAssist release:
...
ClawRouter owns upstream credentials; its catalog tells NodoAssist which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | NodoAssist transport   |
| -------------------------------------------------------- | ---------------------- |
...
unsupported request format is intentionally not advertised as an NodoAssist
...
ClawRouter's `/v1/usage` response feeds the normal NodoAssist provider-usage
...
```bash
nodoassist status --usage
nodoassist models status
```

The same provider snapshot is available to `/status` in chat and NodoAssist's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter credential; NodoAssist does not fall back to upstream provider keys.                                        |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    NodoAssist usage and status surfaces.
  </Card>
</CardGroup>
