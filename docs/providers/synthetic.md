---
summary: "Use Synthetic's Anthropic-compatible API in NodoAssist"
read_when:
...
NodoAssist bundles it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nodoassist onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
NodoAssist's Anthropic client appends `/v1` to the base URL automatically, so use
...
`nodoassist models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    NodoAssist still appends `/v1` automatically.
