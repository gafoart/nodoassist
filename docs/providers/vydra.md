---
summary: "Use Vydra image, video, and speech in NodoAssist"
read_when:
  - You want Vydra media generation in NodoAssist
...
NodoAssist uses the same `VYDRA_API_KEY` for all three capabilities.
...
<Steps>
  <Step title="Run interactive onboarding">
    ```bash
    nodoassist onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    export NODOASSIST_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
