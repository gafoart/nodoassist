---
summary: "Use Ollama Cloud directly with NodoAssist"
read_when:
...
NodoAssist registers `ollama-cloud` as its own provider id so cloud-only
...
```bash
nodoassist onboard --auth-choice ollama-cloud --ollama-cloud-api-key "<key>"
```
...
- You want the same native Ollama chat API shape NodoAssist uses for local
...
NodoAssist discovers Ollama Cloud models live from the hosted catalog:

```bash
nodoassist models list --provider ollama-cloud
nodoassist models set ollama-cloud/kimi-k2.6
```
...
nothing, NodoAssist falls back to the bundled rows `kimi-k2.5:cloud`,
...
NODOASSIST_LIVE_OLLAMA_MODEL=kimi-k2.6 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

The cloud smoke runs text, native stream, and web search; set
`NODOASSIST_LIVE_OLLAMA_WEB_SEARCH=0` to skip web search. It skips embeddings by
default for `https://ollama.com` because Ollama Cloud API keys may not
authorize `/api/embed`; force them with `NODOASSIST_LIVE_OLLAMA_EMBEDDINGS=1`.
...
- Unknown model errors: run `nodoassist models list --provider ollama-cloud` and
