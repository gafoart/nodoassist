---
summary: "Run NodoAssist with LM Studio"
read_when:
  - You want to run NodoAssist with open source models via LM Studio
...
```bash
nodoassist models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys use an `author/model-name` format (e.g. `qwen/qwen3.5-9b`); NodoAssist model refs
...
```bash
nodoassist onboard --non-interactive --accept-risk --auth-choice lmstudio
```
...
servers; omit it for unauthenticated servers and NodoAssist stores a local non-secret marker instead.
...
LM Studio doesn't always emit an OpenAI-shaped `usage` object on streamed responses. NodoAssist
...
When LM Studio's `/api/v1/models` discovery reports model-specific reasoning options, NodoAssist
...
"on"]`) while rejecting those literal values on `/v1/chat/completions`; NodoAssist normalizes that
...
LM Studio supports just-in-time (JIT) model loading, loading models on first request. NodoAssist
...
disable NodoAssist's preload step:
