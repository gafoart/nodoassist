---
summary: "Use NovitaAI's OpenAI-compatible API with NodoAssist"
read_when:
  - You want to run NodoAssist with NovitaAI models
...
It ships as a bundled NodoAssist provider (no separate plugin install), so
...
```bash
nodoassist onboard --auth-choice novita-api-key
```
...
```bash
nodoassist models list --provider novita
```
...
- `401`/`403`: verify the key in Novita's key management page and re-run
  `nodoassist onboard --auth-choice novita-api-key` if the stored profile is
  stale.
