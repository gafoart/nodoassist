---
summary: "Sign in to GitHub Copilot from NodoAssist using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `nodoassist models auth login-github-copilot` flow
  - You are choosing between the built-in Copilot provider, Copilot SDK harness, and Copilot Proxy
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. NodoAssist can use Copilot as a model
provider or agent runtime in three different ways.

## Three ways to use Copilot in NodoAssist

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when NodoAssist runs. This is the **default** and simplest path
    ...
    <Steps>
      ...
      <Step title="Set a default model">
        ```bash
        nodoassist models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot SDK harness plugin (copilot)">
    Install the external `@nodoassist/copilot` plugin when you want GitHub's
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. NodoAssist talks to
    the proxy's `/v1` endpoint (default `http://localhost:3000/v1`) and uses the
    model list you configure.

    The `copilot-proxy` plugin ships with NodoAssist and is enabled by default.
...
endpoints rather than public `github.com`. NodoAssist exposes this as a
first-class auth choice so you do not have to hand-edit URLs.

<Steps>
  <Step title="Pick the Enterprise auth choice">
    In onboarding or `nodoassist models auth`, choose
    **GitHub Copilot (Enterprise / data residency)**. You will be prompted for
    ...
    NodoAssist derives those endpoints from the tenant root automatically.

    ```bash
    nodoassist models auth login --provider github-copilot --method device-enterprise
    ```
...
tenant, or one tenant to another), NodoAssist will not reuse the existing token —
...
`nodoassist models auth login-github-copilot` shortcut, token refresh, embeddings,
...
| ------------------------------------------------------------------------ | --------------- | ---------------------------------------------------- |
| `nodoassist models auth login-github-copilot`                            | `--yes`         | Overwrite an existing auth profile without prompting |
| `nodoassist models auth login --provider github-copilot --method device` | `--set-default` | Also apply the provider's recommended default model  |

```bash
# Skip the re-login confirmation
nodoassist models auth login-github-copilot --yes

# Login and set the default model in one step
nodoassist models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

The device-login flow requires an interactive TTY. For headless setup, import
an existing GitHub OAuth access token with `nodoassist onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    NodoAssist refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    churn. Newly published Copilot models become visible without an NodoAssist
  ...
  <Accordion title="Transport selection">
    ...
    models keep the OpenAI Responses transport. NodoAssist selects the correct
    transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    NodoAssist sends Copilot IDE-style request headers on Copilot transports
  ...
  <Accordion title="Environment variable resolution order">
    NodoAssist resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, NodoAssist uses the highest-priority one.
    The device-login flow (`nodoassist models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    token when NodoAssist runs. You do not need to manage the token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, NodoAssist can use it for embeddings without a separate API key.

### Config

Set `memorySearch.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, NodoAssist discovers available embedding models from
...
1. NodoAssist resolves your GitHub token (from env vars or auth profile).
...
available, NodoAssist skips Copilot and tries the next provider.
