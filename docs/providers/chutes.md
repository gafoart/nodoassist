...
title: "Chutes"
read_when:
  - You want to use Chutes with NodoAssist
...
OpenAI-compatible API. NodoAssist supports both browser OAuth and API-key auth.

| Property         | Value                                                     |
| ---------------- | --------------------------------------------------------- |
| Provider         | `chutes`                                                  |
| Plugin           | official external package (`@nodoassist/chutes-provider`) |
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run the OAuth onboarding flow">
        ```bash
        nodoassist onboard --auth-choice chutes
        ```
        NodoAssist launches the browser flow locally, or shows a URL + redirect-paste
        flow on remote/headless hosts. OAuth tokens auto-refresh through NodoAssist auth
        profiles.
      </Step>
    </Steps>
  </Tab>
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run the API key onboarding flow">
        ```bash
        nodoassist onboard --auth-choice chutes-api-key
        ```
      </Step>
    </Steps>
  </Tab>
...
When Chutes auth is available, NodoAssist queries `GET /v1/models` with that
credential and uses the discovered models, cached for 5 minutes per
credential. On an expired/unauthorized key (HTTP 401), NodoAssist retries once
...
NodoAssist registers three convenience aliases for the Chutes catalog:
...
Run `nodoassist models list --all --provider chutes` for the full list.
