...
read_when:
  - You want to use Vercel AI Gateway with NodoAssist
...
| Package       | `@nodoassist/vercel-ai-gateway-provider` |
...
<Tip>
NodoAssist auto-discovers the Gateway `/v1/models` catalog, so both the
`/models vercel-ai-gateway` chat command and
`nodoassist models list --provider vercel-ai-gateway` include current model
...
<Steps>
  <Step title="Install the plugin">
    ```bash
    nodoassist plugins install @nodoassist/vercel-ai-gateway-provider
    ```
  </Step>
  <Step title="Set the API key">
    ```bash
    nodoassist onboard --auth-choice ai-gateway-api-key
    ```
  </Step>
  ...
  <Step title="Verify the model is available">
    ```bash
    nodoassist models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
NodoAssist normalizes Claude shorthand model refs at runtime:
...
<Tip>
Use either form in your configuration; NodoAssist resolves the canonical
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the NodoAssist Gateway runs as a daemon (launchd/systemd), make sure
    ...
    the key in `~/.nodoassist/.env` or via `env.shellEnv` to ensure the gateway
    process can read it.
    </Warning>
  ...
  <Accordion title="Thinking levels">
    `/think` options follow the upstream model prefix when NodoAssist recognizes
