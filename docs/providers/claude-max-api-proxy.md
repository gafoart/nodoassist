...
**claude-max-api-proxy** is a community npm package (not an NodoAssist plugin) that
...
plan articles, and [Anthropic provider](/providers/anthropic) for NodoAssist's
...
<Steps>
  ...
  <Step title="Configure NodoAssist">
    Point NodoAssist at the proxy as a custom OpenAI-compatible endpoint:
...
<Note>
The model ids below are the proxy's own catalog, not NodoAssist's Anthropic
...
<AccordionGroup>
  <Accordion title="Proxy-style OpenAI-compatible notes">
    This uses NodoAssist's generic custom `/v1` OpenAI-compatible route, the same
    ...
    - No Responses `store`, prompt-cache hints, or OpenAI reasoning-compat
      payload shaping.
    - NodoAssist's OpenAI/Codex attribution headers (`originator`, `version`,
...
<CardGroup cols={2}>
  <Card title="Anthropic provider" href="/providers/anthropic" icon="bolt">
    Native NodoAssist integration with Claude CLI or API keys.
  </Card>
