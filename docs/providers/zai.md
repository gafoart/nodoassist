---
summary: "Use Z.AI (GLM models) with NodoAssist"
read_when:
  - You want Z.AI / GLM models in NodoAssist
...
NodoAssist uses the `zai` provider with a Z.AI API key.

| Property | Value                                        |
| -------- | -------------------------------------------- |
| Provider | `zai`                                        |
| Package  | `@nodoassist/zai-provider`                   |
| Auth     | `ZAI_API_KEY` (legacy alias: `Z_AI_API_KEY`) |
| API      | Z.AI Chat Completions (Bearer auth)          |

## GLM models

GLM is a model family, not a separate provider. In NodoAssist, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. NodoAssist probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        nodoassist onboard --auth-choice zai-api-key
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
        nodoassist models list --all --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        nodoassist onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        nodoassist onboard --auth-choice zai-coding-cn

        # General API
        nodoassist onboard --auth-choice zai-global

        # General API CN (China region)
        nodoassist onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Verify the model is listed">
        ```bash
...
<Tip>
`zai-api-key` lets NodoAssist detect the matching Z.AI endpoint from the key and
...
versions and availability can change; run `nodoassist models list --all --provider zai`
...
<Tabs>
  <Tab title="GLM-5.2">
    Full range: `off`, `low`, `high`, `max` (default `off`). NodoAssist maps
...
<AccordionGroup>
  ...
  <Accordion title="Preserved thinking">
    ...
    When enabled and thinking is on, NodoAssist sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; NodoAssist copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full NodoAssist config schema, including provider and model settings.
  </Card>
</CardGroup>
