...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with NodoAssist
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    nodoassist onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    nodoassist models list --provider deepseek
    ```

    To inspect the plugin's static catalog without a running Gateway:

    ```bash
    nodoassist models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. NodoAssist also replays
...
NodoAssist's DeepSeek plugin backfills that field automatically, so normal
...
When thinking is disabled (including the UI **None** selection), NodoAssist
...
```bash
NODOASSIST_LIVE_PROVIDERS=deepseek \
NODOASSIST_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
