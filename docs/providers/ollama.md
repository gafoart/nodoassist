---
summary: "Run NodoAssist with Ollama (cloud and local models)"
read_when:
  - You want to run NodoAssist with cloud or local models via Ollama
  - You need Ollama setup and configuration guidance
  - You want Ollama vision models for image understanding
title: "Ollama"
---

NodoAssist talks to Ollama's native API (`/api/chat`), not the OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Local and LAN hosts">
    Loopback, private-network, `.local`, and bare-hostname Ollama URLs do not need a real bearer token. NodoAssist uses the `ollama-local` marker for these.
  </Accordion>
  ...
  <Accordion title="Auth profiles">
    `auth-profiles.json` stores the credential for a provider id; put endpoint settings (`baseUrl`, `api`, models, headers, timeouts) in `models.providers.<id>`. Older flat files such as `{ "ollama-windows": { "apiKey": "ollama-local" } }` are not a runtime format; `nodoassist doctor --fix` rewrites them into a canonical `ollama-windows:default` API-key profile with a backup. A `baseUrl` value in that legacy file is noise and should move to provider config.
  </Accordion>
...
<Tabs>
  <Tab title="Onboarding (recommended)">
    <Steps>
      ...
      <Step title="Verify">
        ```bash
        nodoassist models list --provider ollama
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Manual setup">
    <Steps>
      ...
      <Step title="Set a credential">
        ...
        Or in config: `nodoassist config set models.providers.ollama.apiKey "OLLAMA_API_KEY"`.
      </Step>
      <Step title="Select the model">
        ```bash
        nodoassist models list
        nodoassist models set ollama/gemma4
        ```
...
NodoAssist prompts for the base URL, discovers local models, and checks
...
For cloud-only access without a local daemon, use `nodoassist onboard --auth-choice ollama-cloud` and see [Ollama Cloud](/providers/ollama-cloud) — that path does not need `ollama signin` or a running server:

```bash
nodoassist onboard --auth-choice ollama-cloud
nodoassist models set ollama-cloud/kimi-k2.5:cloud
```

The cloud model list shown during `nodoassist onboard` is populated live from
...
models at setup time, NodoAssist falls back to its hardcoded suggested list so
...
defined, NodoAssist discovers models from `http://127.0.0.1:11434`:
...
| Token limits         | `maxTokens` defaults to NodoAssist's Ollama max-token cap                                                                                                                                                                                                                                     |
...
hand-written `models.json` entry; NodoAssist resolves it live. For signed-in
...
`nodoassist infer audio transcribe` for audio):
...
provider and `/api/tags` is unreachable, NodoAssist records that run as
...
```bash
NODOASSIST_LIVE_TEST=1 NODOASSIST_LIVE_OLLAMA=1 NODOASSIST_LIVE_OLLAMA_WEB_SEARCH=0 \
  pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

For Ollama Cloud, point the same live test at the hosted endpoint (skips
embeddings by default; force with `NODOASSIST_LIVE_OLLAMA_EMBEDDINGS=1` since a
cloud key may not authorize `/api/embed`):

```bash
export OLLAMA_API_KEY='<your-ollama-cloud-api-key>'
NODOASSIST_LIVE_TEST=1 NODOASSIST_LIVE_OLLAMA=1 \
NODOASSIST_LIVE_OLLAMA_BASE_URL=https://ollama.com \
NODOASSIST_LIVE_OLLAMA_MODEL=glm-5.1:cloud \
NODOASSIST_LIVE_OLLAMA_WEB_SEARCH=1 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```
...
<Steps>
  ...
  <Step title="Connect the node host">
    ...
    nodoassist devices approve <deviceRequestId>
    nodoassist nodes pending
    nodoassist nodes approve <nodeRequestId>
    ...
    `ollama.models` and `ollama.chat`, check `nodoassist nodes pending` again.
...
```bash
nodoassist config set plugins.entries.ollama.config.nodeInference.enabled false
```

Restart the node (`nodoassist node restart`, or stop/rerun `nodoassist node run`
...
surface may need `nodoassist nodes pending` approval again after reconnect.
...
media-understanding provider, so NodoAssist can route explicit image-description
...
nodoassist infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --json
```
...
that already support native vision. If the call fails, NodoAssist can continue
through `agents.defaults.imageModel.fallbacks`; file/URL preparation errors
fail before fallback is attempted. Use `infer image describe` for NodoAssist's
...
```bash
NODOASSIST_LIVE_TEST=1 NODOASSIST_LIVE_OLLAMA_IMAGE=1 \
...
NodoAssist rejects image-description requests for models not marked
...
<Tabs>
  <Tab title="Basic (implicit discovery)">
    ...
    <Tip>
    If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry; NodoAssist fills it in for availability checks.
    </Tip>
...
<AccordionGroup>
  ...
  <Accordion title="LAN Ollama host with manual models">
    ...
    `contextWindow` is NodoAssist's context budget; `params.num_ctx` is sent to
  ...
  <Accordion title="Multiple Ollama hosts">
    ...
    NodoAssist strips the active provider prefix (falling back to a bare
...
prefix, such as `ollama-spark/qwen3:32b`, NodoAssist strips that prefix before
...
# NodoAssist catalog and selected model
nodoassist models list --provider ollama
...
works but NodoAssist does not, check whether the Gateway runs on a different
machine, container, or service account.

## Ollama Web Search

NodoAssist bundles **Ollama Web Search** as a `web_search` provider.
...
Choose it during `nodoassist onboard` or `nodoassist configure --section web`, or set:
...
For a self-hosted host, NodoAssist first tries the local `/api/experimental/web_search`
...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in this mode.** Use it only when a proxy needs OpenAI format and you do not depend on native tool calling.
    ...
    NodoAssist injects `options.num_ctx` by default in this mode so Ollama does
  ...
  <Accordion title="Context windows">
    For auto-discovered models, NodoAssist uses the context window `/api/show`
    reports, including larger `PARAMETER num_ctx` values from custom
    Modelfiles; otherwise it falls back to NodoAssist's default Ollama context
    window.
    ...
    model. `contextWindow` is NodoAssist's own prompt/compaction budget. Native
    ...
    `nodoassist doctor --fix` to copy those into `params.num_ctx`. The
    ...
    top-level request fields instead of nested `options`. NodoAssist only
  ...
  <Accordion title="Thinking control">
    NodoAssist forwards thinking as Ollama expects it: top-level `think`, not
    ...
    ```bash
    nodoassist agent --model ollama/gemma4 --thinking off
    nodoassist agent --model ollama/gemma4 --thinking low
    ```
    ...
    thinking for a specific model. NodoAssist preserves that explicit config
  ...
  <Accordion title="Memory embeddings">
    ...
    loopback origin derived from the configured `baseUrl` use NodoAssist's
  ...
  <Accordion title="Streaming configuration">
    ...
    and `nodoassist agent --thinking off` send top-level `think: false` unless
...
<AccordionGroup>
  <Accordion title="WSL2 crash loop (repeated reboots)">
    ...
    NodoAssist logs a startup warning when it detects WSL2, `ollama.service`
  ...
  <Accordion title="Remote host works with curl but not NodoAssist">
  ...
  <Accordion title="Kimi or GLM returns garbled symbols">
    ...
    ```bash
    nodoassist infer model run --model ollama/kimi-k2.5:cloud --prompt "Reply with exactly: ok" --json
    nodoassist models set ollama/gemma4
    ```
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    ...
    `params.num_ctx` is set. Cap both NodoAssist's budget and Ollama's request
    ...
    Lower `contextWindow` if NodoAssist sends too much prompt. Lower
