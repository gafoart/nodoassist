...
This hub links the core docs for how NodoAssist connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`nodoassist gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Canvas host** is served on the same port as the Gateway (`/__nodoassist__/canvas/`, `/__nodoassist__/a2ui/`), protected by Gateway auth when bound beyond loopback.
