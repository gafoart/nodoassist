...
- Install: [Google Play](https://play.google.com/store/apps/details?id=ai.openclaw.app&hl=en_IN) or `NodoAssist-Android.apk` from a supported [GitHub Release](https://github.com/openclaw/openclaw/releases), [Getting Started](/start/getting-started) for the Gateway, then [Pairing](/channels/pairing).
...
Regular final and correction GitHub Releases include a universal `NodoAssist-Android.apk` and `NodoAssist-Android-SHA256SUMS.txt`. The APK is built from the release tag, signed with the NodoAssist Android release key, and carries GitHub Actions provenance.
...
gh release download "$release_tag" \
  ...
  --pattern NodoAssist-Android.apk \
  --pattern NodoAssist-Android-SHA256SUMS.txt
sha256sum --check NodoAssist-Android-SHA256SUMS.txt
gh attestation verify NodoAssist-Android.apk \
  --repo nodoassist/nodoassist \
  --signer-workflow nodoassist/nodoassist/.github/workflows/android-release.yml \
...
workflow, separate from the NodoAssist node connection. It is useful when the Android device and the
...
- The `nodoassist` CLI available on the gateway machine (or via SSH), to approve pairing requests.
...
```bash
nodoassist gateway discover --json
```
...
1. Set up a DNS-SD zone (example `nodoassist.internal.`) on the gateway host and publish `_nodoassist-gw._tcp` records.
...
1. Create `~/.nodoassist/workspace/canvas/index.html` on the gateway host.
2. Navigate the node to it (LAN):

```bash
nodoassist nodes invoke --node "<Android Node>" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18789/__nodoassist__/canvas/"}'
```

Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18789/__nodoassist__/canvas/`.

This server injects a live-reload client into HTML and reloads on file changes. The Gateway also serves `/__nodoassist__/a2ui/`, but the Android app treats remote A2UI pages as render-only. Action-capable A2UI commands use the bundled app-owned A2UI page.
...
Android supports launching NodoAssist from the system assistant trigger (Google Assistant). Holding the home button (or another `ACTION_ASSIST` trigger) opens the app; saying "Hey Google, ask NodoAssist `<prompt>`" matches the app's declared App Actions query pattern and hands the prompt into the chat composer without auto-sending it.
...
<Note>
App Actions availability depends on the device, Google Play Services version, and whether the user has set NodoAssist as the default assistant app.
</Note>

## Notification forwarding

Android can forward device notifications to the gateway as `node.event` items. This is configured **on the device**, in the app's Settings sheet — not in gateway/`nodoassist.json` config.

| Setting                     | Description                                                                                                                                                                                              |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Forward Notification Events | Master toggle. Off by default; requires Notification Listener Access to be granted first.                                                                                                                |
| Package Filter              | **Allowlist** (only listed package IDs forwarded) or **Blocklist** (default: all packages except listed IDs). NodoAssist's own package is always excluded in Blocklist mode to prevent forwarding loops. |
...
WhatsApp, WhatsApp Business, Telegram, Telegram X, Discord, and Signal notifications are always excluded. Their messages are already owned by native NodoAssist channel sessions; forwarding the Android notification as a separate node event could route a reply through the wrong conversation.
