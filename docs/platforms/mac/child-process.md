...
enables the launchd service via the external `nodoassist` CLI (no embedded
...
- The app installs a per-user LaunchAgent labeled `ai.nodoassist.gateway` (or
  `ai.nodoassist.<profile>` when using `--profile`/`NODOASSIST_PROFILE`).
...
Replace the label with `ai.nodoassist.<profile>` when running a named profile.
...
`~/.nodoassist/disable-launchagent`, so the app only attaches to an already
