...
The macOS app surfaces NodoAssist skills via the gateway; it does not parse skills locally.
...
- Requirements come from `metadata.nodoassist.requires` in each `SKILL.md`.

## Install actions

- `metadata.nodoassist.install` defines install options (brew/node/go/uv/download).
...
- The app stores keys in `~/.nodoassist/nodoassist.json` under `skills.entries.<skillKey>`.
