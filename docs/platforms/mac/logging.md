---
summary: "NodoAssist logging: rolling diagnostics file log + unified log privacy flags"
read_when:
...
- Location: `~/Library/Logs/NodoAssist/diagnostics.jsonl`.
...
## Enable for NodoAssist (`ai.nodoassist`)

Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.nodoassist.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.nodoassist.plist /Library/Preferences/Logging/Subsystems/ai.nodoassist.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.nodoassist.plist`.
