...
- Same path: run the app from a fixed location (for NodoAssist, `dist/NodoAssist.app`).
- Same bundle identifier: NodoAssist's bundle ID is `ai.nodoassist.mac`; changing it creates a new permission identity.
...
Prefer granting Accessibility to NodoAssist.app, Peekaboo.app, or another signed helper with its own bundle identifier instead of a generic `node` binary.
...
Example resets (using NodoAssist's bundle ID, `ai.nodoassist.mac`):

```bash
sudo tccutil reset Accessibility ai.nodoassist.mac
sudo tccutil reset ScreenCapture ai.nodoassist.mac
...
Workaround: move files into the NodoAssist workspace (`~/.nodoassist/workspace`) if you want to avoid per-folder grants.
