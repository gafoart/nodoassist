...
2. `npm i -g nodoassist@latest`
3. `nodoassist onboard --install-daemon`
...
nodoassist configure   # select "Gateway service" when prompted
```
...
`nodoassist gateway install` renders a systemd **user** unit by default. Full
...
(`~/.config/systemd/user/nodoassist-gateway[-<profile>].service`):

```ini
[Unit]
Description=NodoAssist Gateway (profile: <profile>, v<version>)
...
sessions and channel connections, so NodoAssist biases transient child
processes to be killed first when possible.

For eligible Linux child spawns, NodoAssist wraps the command in a short
...
- NodoAssist-launched browser/Chrome processes (via the plugin SDK process runtime)

The wrapper is Linux-only and skipped when `/bin/sh` is unavailable, or when
the child env sets `NODOASSIST_CHILD_OOM_SCORE_ADJ` to `0`, `false`, `no`, or
`off`.
