...
read_when:
  - Moving NodoAssist runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.nodoassist/state/nodoassist.sqlite`
...
- Configuration stays file-backed: `nodoassist.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside NodoAssist's database.
...
- Raw stream logging uses `NODOASSIST_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of NodoAssist
  runtime or tests.
...
- One global SQLite database owns control-plane state:
  `state/nodoassist.sqlite`.
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/nodoassist-agent.sqlite`.
- Config remains file-backed. `nodoassist.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/nodoassist.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/nodoassist-state-schema.sql`,
      `src/state/nodoassist-agent-schema.sql`,
      `src/state/*generated*`.
...
- `nodoassist doctor --fix` owns the legacy file-to-database migration step.
  Runtime startup and `nodoassist migrate` should not carry legacy NodoAssist
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the NodoAssist `sessionId` as the canonical
...
former stores now write to `state/nodoassist.sqlite` or the owning
`nodoassist-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `nodoassist-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/nodoassist-agent-db.ts` opens
  `agents/<agentId>/agent/nodoassist-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one NodoAssist session to one or more external
  ...
  NodoAssist session can represent multiple external DM peers without demoting
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `nodoassist.json` remains the config store;
...
- Workspace bootstrap completion markers now live in typed shared
  ...
  `.nodoassist/workspace-state.json`; runtime no longer reads or rewrites the
  legacy workspace marker, and helper APIs no longer pass around a fake
  `.nodoassist/setup-state` path just to derive storage identity.
- Exec approvals now live in the typed shared SQLite `exec_approvals_config`
  singleton row. Doctor imports legacy `~/.nodoassist/exec-approvals.json`;
  ...
  `state/nodoassist.sqlite` table row; it keeps only the Unix prompt socket on disk
...
- GitHub Copilot compaction no longer writes `nodoassist-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and NodoAssist keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`NodoAssistKit`) uses the same
  `state/nodoassist.sqlite` rows for device identity and device auth. macOS app
...
- Android device identity uses the same TypeScript-compatible key material
  stored in typed `state/nodoassist.sqlite#table/device_identities` rows. It never
  reads or writes `nodoassist/identity/device.json`; a leftover legacy file blocks
  startup until doctor imports it into SQLite.
- Android cached device auth tokens also use typed
  `state/nodoassist.sqlite#table/device_auth_tokens` rows and share the same
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `nodoassist update --channel` now fails on invalid legacy config and asks the
...
- Plugin state now uses the shared `state/nodoassist.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/nodoassist.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `nodoassist sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `NODOASSIST_CACHE_TRACE_FILE`,
  `NODOASSIST_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `NODOASSIST_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.nodoassist-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.nodoassist-wiki/state.json` or the unused
  `.nodoassist-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `nodoassist.json`. The config
...
- Codex app-server conversation bindings now key SQLite plugin state by
  NodoAssist session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `nodoassist-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the NodoAssist temp root, not durable NodoAssist state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$NODOASSIST_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$NODOASSIST_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.nodoassist/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `nodoassist-cli-images` cache
...
- `nodoassist secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/nodoassist.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.nodoassist-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.nodoassist-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store SQLite plugin blob rows instead of
  writing `.nodoassist-wiki/cache/agent-digest.json` and
  `.nodoassist-wiki/cache/claims.jsonl`. The migration provider imports old cache
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  NodoAssist temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `nodoassist backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `nodoassist backup restore` validates the archive before extraction, reuses the
...
- `nodoassist reset --scope config+creds+sessions` removes per-agent
  `nodoassist-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureNodoAssistModelCatalog`; there is no `models.json` compatibility API in
...
- Memory-core runtime imports SQLite transcript indexing helpers from
  `nodoassist/plugin-sdk/memory-core-host-engine-session-transcripts`, not the
...
- QMD's own `index.sqlite` is now a temp runtime materialization backed by the
  ...
  `~/.nodoassist/agents/<agentId>/qmd` sidecar.
- The optional `memory-lancedb` plugin no longer creates
  `~/.nodoassist/memory/lancedb` as an implicit NodoAssist-managed store. It is an
...
`nodoassist doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `nodoassist migrate` must not import legacy NodoAssist state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `nodoassist doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/nodoassist.sqlite`:
...
```text
~/.nodoassist/state/nodoassist.sqlite
~/.nodoassist/agents/<agentId>/agent/nodoassist-agent.sqlite
```
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `nodoassist backup create` does this archive verification by default;
...
5. Verify the archive by opening every included SQLite snapshot and running
   ...
   `nodoassist backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `nodoassist sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `nodoassist backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  NodoAssist test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `nodoassist-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- Memory Wiki `.nodoassist-wiki/log.jsonl`
- Memory Wiki `.nodoassist-wiki/state.json`
- Memory Wiki `.nodoassist-wiki/locks/`
- Memory Wiki `.nodoassist-wiki/source-sync.json`
- Memory Wiki `.nodoassist-wiki/import-runs/*.json`
- Memory Wiki `.nodoassist-wiki/cache/agent-digest.json`
- Memory Wiki `.nodoassist-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.nodoassist-profile-decorated`
