---
summary: "First-run setup flow for NodoAssist (macOS app)"
read_when:
...
- By default, NodoAssist is a personal agent: one trusted operator boundary.
...
<Step title="CLI">
  Local setup installs the global `nodoassist` CLI via npm, pnpm, or bun,
...
list. The selected provider supplies its starter model and config; NodoAssist
...
<Frame caption="Choose what permissions do you want to give NodoAssist">
