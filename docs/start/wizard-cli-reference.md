---
summary: "Step-by-step behavior for nodoassist onboard: what each step does, config it writes, and internals"
read_when:
  - You need detailed behavior for a specific nodoassist onboard step
...
commands), see [`nodoassist onboard`](/cli/onboard).
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.nodoassist/nodoassist.json` exists, choose **Keep current values**, **Review and update**, or **Reset before setup**.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove the workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `nodoassist doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.nodoassist/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `nodoassist pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Skip this step with `--skip-search`; reconfigure later with `nodoassist configure --section web`.
  ...
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first
      - If task creation is denied, NodoAssist falls back to a per-user Startup-folder login item and starts the gateway immediately.
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `nodoassist health`.
    - `nodoassist status --deep` adds the live gateway health probe to status output, including channel probes when supported.
  ...
  <Step title="Skills">
    ...
    - Skips unavailable Homebrew, uv, and Go installers, then groups the affected
      skills with manual setup guidance. Run `nodoassist doctor` after installing
...
Plaintext `ws://` is accepted for loopback, private IP literals, `.local`, and Tailnet `*.ts.net` URLs; other private-DNS names need `NODOASSIST_ALLOW_INSECURE_PRIVATE_WS=1`.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="xAI (Grok) OAuth">
    ...
    recommended xAI path for most users. NodoAssist stores the resulting auth
...
- Auth profiles (API keys + OAuth): `~/.nodoassist/agents/<agentId>/agent/auth-profiles.json`
...
`~/.nodoassist/agents/<agentId>/agent/auth-profiles.json`, or the matching
`$NODOASSIST_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
is only a legacy import source.
</Note>

## Outputs and internals

Typical fields in `~/.nodoassist/nodoassist.json`:
...
`nodoassist agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.nodoassist/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.nodoassist/agents/<agentId>/sessions/`.
...
Full flag reference and provider-specific examples: [`nodoassist onboard`](/cli/onboard), [CLI automation](/start/wizard-cli-automation).
...
- Stores the release-asset install under `~/.nodoassist/tools/signal-cli/<version>/`
...
- Command reference: [`nodoassist onboard`](/cli/onboard)
