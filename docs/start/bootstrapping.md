...
On the first run against a brand-new workspace (default `~/.nodoassist/workspace`),
NodoAssist:
...
For embedded or local-model runs, NodoAssist keeps `BOOTSTRAP.md` out of the
...
host (for example, `user@gateway-host:~/.nodoassist/workspace`).
</Note>
