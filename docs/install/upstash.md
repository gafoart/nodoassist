---
summary: "Host NodoAssist on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying NodoAssist to Upstash Box
  - You want a managed Linux environment for NodoAssist with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent NodoAssist Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current NodoAssist Box walkthrough at
[NodoAssist Setup](https://upstash.com/docs/box/guides/openclaw-setup).

## Connect with an SSH tunnel

Forward the NodoAssist dashboard port to your local machine. Use your Box API key
...
```bash
nodoassist config set gateway.bind lan
nohup nodoassist gateway > gateway.log 2>&1 &
```
...
- [Updating NodoAssist](/install/updating)
