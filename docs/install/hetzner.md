---
summary: "Run NodoAssist Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want NodoAssist running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running NodoAssist in Docker on Hetzner or a similar provider
title: "Hetzner"
---

Run a persistent NodoAssist Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
2. Install Docker
3. Clone the NodoAssist repository
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.nodoassist/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.nodoassist
    ```
  ...
  <Step title="Configure environment variables">
    ...
    NODOASSIST_CONFIG_DIR=/root/.nodoassist
    NODOASSIST_WORKSPACE_DIR=/root/.nodoassist/workspace
...
- Infrastructure: [nodoassist-terraform-hetzner](https://github.com/andreesg/openclaw-terraform-hetzner)
- Docker config: [nodoassist-docker-config](https://github.com/andreesg/openclaw-docker-config)
