---
summary: "Host NodoAssist on a DigitalOcean Droplet"
read_when:
  - Setting up NodoAssist on DigitalOcean
  - Looking for a simple paid VPS for NodoAssist
title: "DigitalOcean"
---

Run a persistent NodoAssist Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own NodoAssist state and services.
    adduser nodoassist
    usermod -aG sudo nodoassist
    loginctl enable-linger nodoassist

    su - nodoassist
    nodoassist --version
    ```

    Use the root shell only for system bootstrap. Run NodoAssist commands as the non-root `nodoassist` user so state lives under `/home/nodoassist/.nodoassist/` and the Gateway installs as that user's systemd `--user` service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    nodoassist config set gateway.bind tailnet
    nodoassist gateway restart
    ```
...
- `~/.nodoassist/` -- `nodoassist.json`, channel/provider credentials, per-agent `auth-profiles.json`, and session data.
...
DigitalOcean snapshots back up the whole Droplet; `nodoassist backup create` is portable across hosts.
