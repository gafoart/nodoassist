...
NodoAssist ships four update channels:
...
| `extended-stable` | resolves the public npm `extended-stable` selector, verifies the exact selected package, and installs that exact version. Fails closed with no fallback to `latest`, `beta`, or `dev`. | unsupported: NodoAssist leaves the checkout unchanged and asks you to use a package installation                                                                   |
...
For `dev` git installs, the default checkout is `~/nodoassist` (or
...
nodoassist update --tag 2026.4.1-beta.1
...
nodoassist update --tag nodoassist@2026.4.1-beta.1
...
- The tag is not persisted; the next `nodoassist update` uses the configured
  channel.
- `--tag main` maps to the npm-compatible spec `github:nodoassist/nodoassist#main`
  for that one run. For a persistent moving `main` install, use
  `nodoassist update --channel dev` (package installs switch to a git checkout)
...
- Downgrade protection: if the target version is older than the current
  version, NodoAssist prompts for confirmation (skip with `--yes`).
...
Preview what `nodoassist update` would do without making changes:

```bash
nodoassist update --dry-run
nodoassist update --channel beta --dry-run
nodoassist update --tag 2026.4.1-beta.1 --dry-run
nodoassist update --dry-run --json
```
...
Switching channels with `nodoassist update` also syncs plugin sources:
