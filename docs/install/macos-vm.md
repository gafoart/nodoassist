---
summary: "Run NodoAssist in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want NodoAssist isolated from your main macOS environment
...
Run NodoAssist in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume). This gives you:
...
Once you have SSH access to a macOS VM, continue at [Install NodoAssist](#6-install-nodoassist) below.

## Quick path (Lume, experienced users)

1. Install Lume.
2. `lume create nodoassist --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH).
4. `lume run nodoassist --no-display`
5. SSH in, install NodoAssist, configure channels.
6. Done.
...
```bash
lume create nodoassist --os macos --ipsw latest
```
...
```bash
nano ~/.nodoassist/nodoassist.json
```
...
```bash
lume stop nodoassist
lume run nodoassist --no-display
```

The VM runs in the background; NodoAssist's daemon keeps the gateway running. To check status:

```bash
ssh youruser@192.168.64.X "nodoassist status"
```

## Bonus: iMessage integration

This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to NodoAssist.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running NodoAssist/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your NodoAssist config:
...
```bash
lume stop nodoassist
lume clone nodoassist nodoassist-golden
```

Reset anytime:

```bash
lume stop nodoassist && lume delete nodoassist
lume clone nodoassist-golden nodoassist
lume run nodoassist --no-display
```
...
| VM IP not showing        | Wait for VM to fully boot, run `lume get nodoassist` again                            |
| Lume command not found   | Add `~/.local/bin` to your PATH                                                       |
| WhatsApp QR not scanning | Ensure you are logged into the VM (not host) when running `nodoassist channels login` |
