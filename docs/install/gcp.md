---
summary: "Run NodoAssist Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want NodoAssist running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent NodoAssist Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
4. Clone the NodoAssist repository
...
<Steps>
  ...
  <Step title="Create a GCP project">
    ```bash
    gcloud projects create my-nodoassist-project --name="NodoAssist Gateway"
    gcloud config set project my-nodoassist-project
  ...
  <Step title="Create the VM">
    ...
    ```bash
    gcloud compute instances create nodoassist-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    ```bash
    gcloud compute ssh nodoassist-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the NodoAssist repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    NODOASSIST_IMAGE=nodoassist:latest
    NODOASSIST_GATEWAY_TOKEN=
    NODOASSIST_GATEWAY_BIND=lan
    NODOASSIST_GATEWAY_PORT=18789

    NODOASSIST_CONFIG_DIR=/home/$USER/.nodoassist
    NODOASSIST_WORKSPACE_DIR=/home/$USER/.nodoassist/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.nodoassist
    ```

    Set `NODOASSIST_GATEWAY_TOKEN` to manage the stable gateway token through
    `.env`; otherwise configure `gateway.auth.token` before relying on clients
    across restarts. If neither is set, NodoAssist uses a runtime-only token for
    ...
    **Do not commit this file.** It holds container/runtime env such as
    `NODOASSIST_GATEWAY_TOKEN`. Stored provider OAuth/API-key auth lives in the
    mounted `~/.nodoassist/agents/<agentId>/agent/auth-profiles.json`.
    ...
    services:
      nodoassist-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - NODOASSIST_GATEWAY_BIND=${NODOASSIST_GATEWAY_BIND}
          - NODOASSIST_GATEWAY_PORT=${NODOASSIST_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${NODOASSIST_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`NODOASSIST_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm nodoassist-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh nodoassist-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop nodoassist-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type nodoassist-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start nodoassist-gateway --zone=us-central1-a
```
...
```bash
gcloud iam service-accounts create nodoassist-deploy \
  --display-name="NodoAssist Deployment"

gcloud projects add-iam-policy-binding my-nodoassist-project \
  --member="serviceAccount:nodoassist-deploy@my-nodoassist-project.iam.gserviceaccount.com" \
