---
summary: "Host NodoAssist on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up NodoAssist on Oracle Cloud
  - Looking for free VPS hosting for NodoAssist
  - Want 24/7 NodoAssist on a small server
title: "Oracle Cloud"
---

Run a persistent NodoAssist Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname nodoassist
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=nodoassist
    ```

    From now on, connect via Tailscale: `ssh ubuntu@nodoassist`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    nodoassist config set gateway.bind loopback
    nodoassist config set gateway.auth.mode token
    nodoassist doctor --generate-gateway-token
    nodoassist config set gateway.tailscale.mode serve
    nodoassist config set gateway.trustedProxies '["127.0.0.1"]'
...
- `chmod 700 ~/.nodoassist` to restrict credential file permissions.
- `nodoassist security audit` for an NodoAssist-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most NodoAssist features work fine; a small number of native binaries need ARM builds:
...
- `~/.nodoassist/` -- `nodoassist.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.nodoassist/workspace/` -- the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@nodoassist
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=nodoassist --reset` to re-authenticate.

**Gateway will not start** -- Run `nodoassist doctor --non-interactive` and check logs with `journalctl --user -u nodoassist-gateway.service -n 50`.
