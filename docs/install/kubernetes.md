---
summary: "Deploy NodoAssist Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run NodoAssist on a Kubernetes cluster
  - You want to test NodoAssist in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running NodoAssist on Kubernetes, not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm

NodoAssist is a single container with some config files. The interesting customization is in agent content (Markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. Layer a Helm chart on top of these manifests if your deployment grows more complex.
...
kubectl port-forward svc/nodoassist 18789:18789 -n nodoassist
...
```bash
kubectl get secret nodoassist-secrets -n nodoassist -o jsonpath='{.data.NODOASSIST_GATEWAY_TOKEN}' | base64 -d
```
...
```text
Namespace: nodoassist (configurable via NODOASSIST_NAMESPACE)
├── Deployment/nodoassist        # Single pod, init container + gateway
├── Service/nodoassist           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/nodoassist-config  # nodoassist.json + AGENTS.md
└── Secret/nodoassist-secrets    # Gateway token + API keys
```
...
Edit `nodoassist.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret nodoassist-secrets -n nodoassist \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/nodoassist -n nodoassist
```

### Custom namespace

```bash
NODOASSIST_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/nodoassist/nodoassist:slim # primary; official Docker Hub mirror: nodoassist/nodoassist
```
...
└── manifests/
    ...
    ├── configmap.yaml          # nodoassist.json + AGENTS.md
