---
summary: "Automated, hardened NodoAssist installation with Ansible, Tailscale VPN, and firewall isolation"
read_when:
...
Deploy NodoAssist to production servers with **[nodoassist-ansible](https://github.com/openclaw/openclaw-ansible)**, an automated installer with a security-first architecture.

<Info>
The [nodoassist-ansible](https://github.com/openclaw/openclaw-ansible) repo is the source of truth for Ansible deployment. This page is a quick overview.
</Info>
...
4. Node.js and pnpm (NodoAssist requires Node 22.19+ or 23.11+; Node 24 is recommended)
5. NodoAssist, installed host-based, not containerized
...
<Steps>
  <Step title="Switch to the nodoassist user">
    ```bash
    sudo -i -u nodoassist
    ```
  </Step>
  <Step title="Run the onboarding wizard">
    The post-install script guides you through configuring NodoAssist.
  </Step>
  <Step title="Connect messaging channels">
    Log in to WhatsApp, Telegram, Discord, or Signal:
    ```bash
    nodoassist channels login --channel <name>
    ```
  </Step>
  <Step title="Verify the installation">
    ```bash
    sudo systemctl status nodoassist
    sudo journalctl -u nodoassist -f
    ```
  </Step>
...
# Restart gateway
sudo systemctl restart nodoassist

# Channel login (run as nodoassist user)
sudo -i -u nodoassist
nodoassist channels login --channel <name>
```
...
<Steps>
  ...
  <Step title="Run the playbook">
    ...
    # Then run: /tmp/nodoassist-setup.sh
    ```
...
The Ansible installer sets up NodoAssist for manual updates; see [Updating](/install/updating) for the standard flow.
...
<AccordionGroup>
  ...
  <Accordion title="Service will not start">
    ```bash
    # Check logs
    sudo journalctl -u nodoassist -n 100

    # Verify permissions
    sudo ls -la /opt/nodoassist
  ...
  <Accordion title="Docker sandbox issues">
    ...
    # Check sandbox image
    sudo docker images | grep nodoassist-sandbox

    # Build the sandbox image if missing (requires a source checkout)
    cd /opt/nodoassist/nodoassist
    sudo -u nodoassist ./scripts/sandbox-setup.sh
  ...
  <Accordion title="Channel login fails">
    Make sure you are running as the `nodoassist` user:
    ```bash
    sudo -i -u nodoassist
    nodoassist channels login --channel <name>
    ```
...
For detailed security architecture and troubleshooting, see the nodoassist-ansible repo:
...
- [nodoassist-ansible](https://github.com/openclaw/openclaw-ansible): full deployment guide
