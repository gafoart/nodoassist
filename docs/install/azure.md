---
summary: "Run NodoAssist Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want NodoAssist running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on NodoAssist Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

Set up an Azure Linux VM with the Azure CLI, apply Network Security Group (NSG) hardening, configure Azure Bastion for SSH access, and install NodoAssist.
...
- Install NodoAssist with the installer script
- Verify the gateway
...
<Steps>
  ...
  <Step title="Set deployment variables">
    ...
    RG="rg-nodoassist"
    LOCATION="westus2"
    VNET_NAME="vnet-nodoassist"
    VNET_PREFIX="10.40.0.0/16"
    VM_SUBNET_NAME="snet-nodoassist-vm"
    VM_SUBNET_PREFIX="10.40.2.0/24"
    BASTION_SUBNET_PREFIX="10.40.1.0/26"
    NSG_NAME="nsg-nodoassist-vm"
    VM_NAME="vm-nodoassist"
    ADMIN_USERNAME="nodoassist"
    BASTION_NAME="bas-nodoassist"
    BASTION_PIP_NAME="pip-nodoassist-bastion"
    ```
...
<Steps>
  ...
  <Step title="Install NodoAssist (in the VM shell)">
    ```bash
    ...
    The installer installs Node and dependencies if not already present, installs NodoAssist, and launches onboarding. See [Install](/install) for details.
...
- More detail on Azure deployment with the GitHub Copilot model provider: [NodoAssist on Azure with GitHub Copilot](https://github.com/johnsonshi/openclaw-azure-github-copilot)
