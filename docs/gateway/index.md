...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and a `Capability` line that matches what you expect. Use `nodoassist gateway status --require-rpc` for read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `NODOASSIST_CONFIG_PATH` when set). Default mode is `gateway.reload.mode="hybrid"`. After the first successful load, the running process serves the active in-memory config snapshot; a successful reload swaps that snapshot atomically.
</Note>
...
- Auth is required by default. Shared-secret setups use `gateway.auth.token` / `gateway.auth.password` (or `NODOASSIST_GATEWAY_TOKEN` / `NODOASSIST_GATEWAY_PASSWORD`), and non-loopback reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

NodoAssist's highest-leverage compatibility surface:
...
`/v1/models` is agent-first: it returns `nodoassist`, `nodoassist/default`, and `nodoassist/<agentId>` for every configured agent. `nodoassist/default` is the stable alias that always maps to the configured default agent. Send `x-nodoassist-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | -------------------------------------------------------------------- |
| Gateway port | `--port` → `NODOASSIST_GATEWAY_PORT` → `gateway.port` → `18789`      |
| Bind mode    | CLI/override → `gateway.bind` → `loopback` (or `auto` in containers) |

Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `nodoassist doctor --fix` or `nodoassist gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
```bash
nodoassist gateway status
nodoassist gateway status --deep   # adds a system-level service scan
...
- `gateway probe` can warn about `multiple reachable gateway identities` when distinct gateways answer, or when NodoAssist cannot prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
- If that is intentional, isolate ports, config/state, and workspace roots per gateway.

Checklist per instance:

- Unique `gateway.port`
- Unique `NODOASSIST_CONFIG_PATH`
- Unique `NODOASSIST_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
NODOASSIST_CONFIG_PATH=~/.nodoassist/a.json NODOASSIST_STATE_DIR=~/.nodoassist-a nodoassist gateway --port 19001
NODOASSIST_CONFIG_PATH=~/.nodoassist/b.json NODOASSIST_STATE_DIR=~/.nodoassist-b nodoassist gateway --port 19002
```
...
Use `nodoassist gateway restart` for restarts. Do not chain `nodoassist gateway stop` and `nodoassist gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default. This removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `nodoassist gateway stop --disable`.

LaunchAgent labels are `ai.nodoassist.gateway` (default) or `ai.nodoassist.<profile>` (named profile). `nodoassist doctor` audits and repairs service config drift.
...
```bash
nodoassist gateway install
systemctl --user enable --now nodoassist-gateway[-<profile>].service
nodoassist gateway status
```
...
[Service]
ExecStart=/usr/local/bin/nodoassist gateway --port 18789
Restart=always
RestartSec=5
...
Native Windows managed startup uses a Scheduled Task named `NodoAssist Gateway`
(or `NodoAssist Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, NodoAssist falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now nodoassist-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/nodoassist-gateway[-<profile>].service` and adjust
`ExecStart=` if your `nodoassist` binary lives elsewhere.

Do not also let `nodoassist doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level NodoAssist gateway service; use `NODOASSIST_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
nodoassist --dev setup
nodoassist --dev gateway --allow-unconfigured
nodoassist --dev status
```
