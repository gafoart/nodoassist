...
Each channel starts automatically when its config section exists (unless `enabled: false`). Telegram and iMessage ship inside the core `nodoassist` package. Other official channels (Discord, Slack, WhatsApp, Matrix, Microsoft Teams, IRC, Google Chat, Signal, Mattermost, and more) install as separate plugins with `nodoassist plugins install <spec>`; see [Channels](/channels) for the full list and install specs.
...
- Legacy single-account Baileys auth dir is migrated by `nodoassist doctor` into `whatsapp/default`.
...
- `apiRoot` is the Telegram Bot API root only. Use `https://api.telegram.org` or your self-hosted/proxy root, not `https://api.telegram.org/bot<TOKEN>`; `nodoassist doctor --fix` removes an accidental trailing `/bot<TOKEN>` suffix.
- For a self-hosted Bot API server in `--local` mode, `trustedLocalFileRoots` lists host paths NodoAssist may read. Mount the server data volume on the NodoAssist host and configure either its data root or per-token directory; container paths under `/var/lib/telegram-bot-api` are mapped into those roots. Other absolute paths remain rejected.
- Optional `channels.telegram.defaultAccount` overrides default account selection when it matches a configured account id.
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `nodoassist doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["nodoassist-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-nodoassist",
          requireMention: false,
...
- `channels.discord.voice.reconnectGraceMs` controls how long a disconnected voice session may take to enter reconnect signalling before NodoAssist destroys it (default `15000`).
- Discord voice playback is not interrupted by another user's speaking-start event. To avoid feedback loops, NodoAssist ignores new voice capture while TTS is playing.
- NodoAssist additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
- `channels.discord.streaming` is the canonical stream mode key. Discord defaults to `streaming.mode: "progress"` so tool/work progress appears in one edited preview message; set `streaming.mode: "off"` to disable it. Legacy `streamMode` and boolean `streaming` values remain runtime aliases; run `nodoassist doctor --fix` to rewrite persisted config.
...
- `channels.slack.streaming.mode` is the canonical Slack stream mode key (default `"partial"`). `channels.slack.streaming.nativeTransport` controls Slack's native streaming transport (default `true`). Legacy `streamMode`, boolean `streaming`, `chunkMode`, `blockStreaming`, `blockStreamingCoalesce`, and `nativeStreaming` values remain runtime aliases; run `nodoassist doctor --fix` to rewrite persisted config to `streaming.{mode,chunkMode,block.enabled,block.coalesce,nativeTransport}`.
...
Check [npmjs.com/package/@nodoassist/mattermost](https://www.npmjs.com/package/@openclaw/mattermost) for the current dist-tags before pinning a version.
...
- `commands.callbackUrl` must resolve to the NodoAssist gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, NodoAssist rejects callbacks with
...
- `channels.imessage.actions.*`: enable private API actions that are also gated by `imsg status` / `nodoassist channels status --probe`.
...
- If you add a non-default account via `nodoassist channels add` (or channel onboarding) while still on a single-account top-level channel config, NodoAssist promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `nodoassist doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
If the message tool is unavailable under the active tool policy, NodoAssist falls back to automatic visible replies instead of silently suppressing the response. `nodoassist doctor` warns about this mismatch.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@nodoassist", "nodoassist"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@nodoassist"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `nodoassist.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
- `mcp: true` enables `/mcp` for NodoAssist-managed MCP server config under `mcp.servers`.
