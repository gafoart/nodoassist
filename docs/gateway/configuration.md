...
read_when:
  - Setting up NodoAssist for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

NodoAssist reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.nodoassist/nodoassist.json`. If the file is missing, NodoAssist uses safe defaults.

The active config path must be a regular file. NodoAssist-owned writes replace it atomically (rename onto the path), so a symlinked `nodoassist.json` gets its target replaced rather than written through - avoid symlinked config layouts. If you keep config outside the default state directory, point `NODOASSIST_CONFIG_PATH` directly at the real file.
...
<Tip>
**New to configuration?** Start with `nodoassist onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    nodoassist onboard       # full onboarding flow
    nodoassist configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.nodoassist/nodoassist.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
NodoAssist only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`nodoassist config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`nodoassist doctor`, `nodoassist logs`, `nodoassist health`, `nodoassist status`)
- Run `nodoassist doctor` to see exact issues
- Run `nodoassist doctor --fix` (`--repair` is the same flag; `--yes` skips prompts) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically - only `nodoassist doctor --fix`
does. If `nodoassist.json` fails validation (including plugin-local validation), Gateway
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `nodoassist config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@nodoassist", "nodoassist"],
            },
          },
        ],
      },
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `NODOASSIST_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push for public App Store builds uses the hosted NodoAssist relay: `https://ios-push-relay.openclaw.ai`.
    ...
    ```bash
    nodoassist config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `NODOASSIST_APNS_RELAY_BASE_URL` and `NODOASSIST_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    ...
    - `NODOASSIST_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `NODOASSIST_GATEWAY_TOKEN` or `gateway.auth.password` / `NODOASSIST_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-nodoassist-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **NodoAssist-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      NodoAssist updates that included file and leaves `nodoassist.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for NodoAssist-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `nodoassist.json`. To share a tree across machines or users, set
      `NODOASSIST_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.nodoassist/nodoassist.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `nodoassist.json`. NodoAssist-owned config
...
config`, inspect the config, run `nodoassist config validate`, then run `nodoassist
...
When you edit a source file that is referenced through `$include`, NodoAssist plans
...
```bash
nodoassist gateway call config.get --params '{}'  # capture payload.hash
nodoassist gateway call config.patch --params '{
...
NodoAssist reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.nodoassist/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, NodoAssist runs your login shell and imports only the missing keys:
...
Env var equivalent: `NODOASSIST_LOAD_SHELL_ENV=1`. Default `timeoutMs`: `15000`.
</Accordion>
