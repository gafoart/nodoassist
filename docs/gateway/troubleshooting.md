...
- `nodoassist gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `nodoassist doctor` reports no blocking config/service issues.
- `nodoassist channels status --probe` shows live per-account transport status and, where supported, `works` or `audit ok`.
...
- `Update restart` in `nodoassist status` / `nodoassist status --all`. Pending or failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run nodoassist doctor --fix` under Channels: the channel config still exists, but plugin registration failed before the channel could load.
- Provider 401s after re-auth: `nodoassist doctor --fix` checks for stale per-agent OAuth auth shadows and removes old copies so all agents resolve the current shared profile.

## Split brain installs and newer config guard

Use when a gateway service unexpectedly stops after an update, or logs show one `nodoassist` binary is older than the version that last wrote `nodoassist.json`.

NodoAssist stamps config writes with `meta.lastTouchedVersion`. Read-only commands can inspect a config written by a newer NodoAssist, but process and service mutations refuse to run from an older binary. Blocked actions: gateway service start/stop/restart/uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which nodoassist
nodoassist --version
nodoassist gateway status --deep
nodoassist config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `nodoassist` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `nodoassist` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
- `Established clients:` in `nodoassist gateway status --deep` or `Gateway clients` in `nodoassist doctor --deep`: active TCP clients connected to the Gateway port, with PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer NodoAssist install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale NodoAssist client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed NodoAssist: local dashboards, editors, app-server helpers, or long-running `nodoassist logs --follow` shells.
3. Re-run `nodoassist gateway status --deep` or `nodoassist doctor --deep` and confirm the stale client PID is gone.
...
Every skill root is a containment boundary. A symlink under `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or `~/.nodoassist/skills` is skipped when its real target resolves outside that root, unless the target is explicitly trusted.
...
nodoassist config get skills.load
```
...
nodoassist config get agents.defaults.models
```
...
Do not assume this is always an NodoAssist configuration issue. The response can come from an upstream security layer such as a CDN, WAF, bot-management rule, or reverse proxy in front of an OpenAI-compatible endpoint.
...
Fix the provider-side filtering first when evidence points to a WAF/CDN block. Prefer a narrowly scoped allow or skip rule for the API path NodoAssist uses, and avoid disabling protection for the whole site.
...
- NodoAssist model runs fail only on normal agent turns.
...
nodoassist infer model run --model <provider/model> --prompt "hi" --json
nodoassist logs --follow
```

Look for:

- Direct tiny calls succeed, but NodoAssist runs fail only on larger prompts.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0`: the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. NodoAssist retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - Direct tiny requests succeed, but NodoAssist agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds): NodoAssist transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle NodoAssist's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while NodoAssist agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
nodoassist config get channels
nodoassist logs --follow
```
...
If `curl` returns NodoAssist HTML, the Gateway is working and the remaining issue is likely browser cache, an old deep link, or stale tab state. Open `http://127.0.0.1:18789` directly and navigate from the dashboard. If restart does not leave the service running, run `nodoassist gateway start` and recheck `nodoassist gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `nodoassist config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                            |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `nodoassist devices list` then `nodoassist devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                           |
...
If `nodoassist devices rotate` / `revoke` / `remove` is denied unexpectedly:

- Paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`.
- `nodoassist devices rotate --scope ...` can only request operator scopes that the caller session already holds.
...
nodoassist logs --follow
nodoassist doctor
nodoassist gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `nodoassist onboard --mode local` / `nodoassist setup` to restamp the expected local-mode config. If you are running NodoAssist via Podman, the default config path is `~/.nodoassist/nodoassist.json`.
    ...
    - `System-level NodoAssist gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `NODOASSIST_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `nodoassist doctor --fix` or `nodoassist gateway install --force`, then restart the gateway service.
...
Use when channels (Telegram, WhatsApp, etc.) on a macOS host go quiet for minutes to hours at a time, and the gateway appears to come back the moment you open the Control UI, SSH in, or otherwise interact with the host. There is usually no obvious symptom in `nodoassist status` because by the time you look the gateway is alive again.

```bash
ls ~/.nodoassist/logs/stability/ | tail -5
nodoassist gateway stability --bundle latest
pmset -g log | grep -iE "sleep|wake|maintenance" | tail -50
launchctl print gui/$UID/ai.nodoassist.gateway | grep -E "state|last exit|runs"
```

Look for:

- One or more `*-uncaught_exception.json` bundles in `~/.nodoassist/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
- `runs` count incrementing across the day with no corresponding `received SIG*; shutting down` line in `~/Library/Logs/nodoassist/gateway.log`: clean shutdowns log a signal; transient crashes do not.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/ai.nodoassist.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
   if [ "$state" != "running" ]; then
     launchctl kickstart -k gui/$UID/ai.nodoassist.gateway
   fi
   ```
...
- `critical memory pressure bundle written` appears shortly before restart → NodoAssist captured a pre-OOM stability bundle. Inspect it with `nodoassist gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → NodoAssist detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `nodoassist.json.rejected.*` file beside the active config.
- A timestamped `nodoassist.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit.
- NodoAssist keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an NodoAssist-owned write.
    - Gateway startup fails closed instead of rewriting `nodoassist.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - NodoAssist-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `nodoassist doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, NodoAssist rotates older `.clobbered.*` files so the newest repaired payload is still available.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(nodoassist config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an NodoAssist-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an NodoAssist-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `nodoassist doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `nodoassist config set` or `config.patch`.
    3. Run `nodoassist config validate` before restarting.
...
Use when `nodoassist gateway probe` reaches something, but still prints a warning block.
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or NodoAssist could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
nodoassist system heartbeat last
nodoassist logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank, comment, header, fence, or empty-checklist scaffolding, so NodoAssist skips the model call.
...
nodoassist browser start --browser-profile nodoassist
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - Stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `nodoassist browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (NodoAssist-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    nodoassist gateway status
    nodoassist config get gateway.mode
    nodoassist config get gateway.remote.url
    nodoassist config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    nodoassist config get gateway.bind
    nodoassist config get gateway.auth.mode
    nodoassist config get gateway.auth.token
    nodoassist gateway status
    nodoassist logs --follow
    ```
