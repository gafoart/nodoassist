---
summary: "Schema-accurate configuration examples for common NodoAssist setups"
read_when:
  - Learning how to configure NodoAssist
...
Save to `~/.nodoassist/nodoassist.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.nodoassist/agents/main/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/nodoassist" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.nodoassist/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
