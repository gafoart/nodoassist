---
summary: "Pre-flight and rollback checklist before exposing an NodoAssist Gateway beyond loopback"
...
- Gateway host, OS user, and state directory (default `~/.nodoassist`).
...
- Backup location for `~/.nodoassist/nodoassist.json` and credentials.
...
```bash
nodoassist gateway probe --url ws://127.0.0.1:18789 --token "$NODOASSIST_GATEWAY_TOKEN"
```
...
Run `nodoassist security audit --deep` after proxy changes. Trusted-proxy
...
1. Re-run `nodoassist security audit --deep`.
...
5. Re-run `nodoassist security audit --deep`.
