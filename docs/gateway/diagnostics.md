...
NodoAssist can build a local diagnostics `.zip` for bug reports: sanitized Gateway
...
2. NodoAssist sends a preamble and asks for one explicit exec approval, which runs
   `nodoassist gateway diagnostics export --json`. Do not approve diagnostics via
   an allow-all rule.
3. After approval, NodoAssist replies with the local bundle path, manifest
   summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but NodoAssist sends the
...
approval also covers an OpenAI feedback upload for the Codex threads NodoAssist
...
approval, the reply lists channels, NodoAssist session ids, Codex thread ids, and
...
Persisted bundles live under `~/.nodoassist/logs/stability/` when events exist.

## Useful options

```bash
nodoassist gateway diagnostics export \
  --output nodoassist-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
...
| ----------------------- | --------------------------------------------------------------------------------- | -------------------------------------------------- |
| `--output <path>`       | `$NODOASSIST_STATE_DIR/logs/support/nodoassist-diagnostics-<timestamp>-<pid>.zip` | Write to a specific zip path (or directory).       |
