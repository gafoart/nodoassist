---
summary: "Use OpenShell as a managed sandbox backend for NodoAssist agents"
title: OpenShell
read_when:
...
locally, NodoAssist delegates sandbox lifecycle to the `openshell` CLI, which
...
- OpenShell plugin installed (`nodoassist plugins install @nodoassist/openshell-sandbox`)
- `openshell` CLI on `PATH` (or a custom path via
  `plugins.entries.openshell.config.command`)
- An OpenShell account with sandbox access
- NodoAssist Gateway running on the host
...
Restart the Gateway. On the next agent turn NodoAssist creates an OpenShell
...
- Before `exec`, NodoAssist syncs the local workspace into the sandbox.
- After `exec`, NodoAssist syncs the remote workspace back to local.
- File tools go through the sandbox bridge, but local stays source of truth
  between turns.

Best for development workflows: local edits outside NodoAssist show up on the
...
- On first sandbox creation, NodoAssist seeds the remote workspace from local
  once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. NodoAssist does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside NodoAssist after the initial seed is invisible to the remote sandbox. Run `nodoassist sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"nodoassist"` | Sandbox source for first-time create                                                   |
...
1. NodoAssist runs `sandbox get` for the sandbox name (with any configured
...
2. NodoAssist runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
...
- [Sandbox CLI](/cli/sandbox) - `nodoassist sandbox` commands
