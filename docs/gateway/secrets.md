...
NodoAssist supports additive SecretRefs so supported credentials do not need to live as plaintext in configuration.
...
<Warning>
Plaintext credentials remain agent-readable if they sit in files the agent can inspect, including `nodoassist.json`, `auth-profiles.json`, `.env`, or generated `agents/*/agent/models.json` files. SecretRefs only reduce that local blast radius once every supported credential is migrated and `nodoassist secrets audit --check` reports no plaintext residue.
</Warning>
...
For model-provider credentials backed by SecretRefs, NodoAssist mints an opaque, process-local sentinel during model-auth resolution. Auth storage, stream options, SDK configuration, logs, error objects, and most runtime introspection therefore see a value such as `oc-sent-v1-...`, not the provider credential. The guarded model fetch and managed local-provider health probes replace known sentinels in URL and header values immediately before each request leaves the process.

Unknown sentinel-shaped values fail closed before network activity. NodoAssist refuses to send the request rather than forwarding an unresolved sentinel to a provider. Resolved secret values are also registered for exact-value log redaction as a defense in depth measure.

Provider adapters use the latest injection point their SDK supports:

- SDKs with a custom fetch option receive NodoAssist's guarded fetch, so the SDK retains the sentinel.
- SDKs without a custom fetch option unwrap the sentinel immediately before client construction. Plugin-owned provider streams and agent harnesses unwrap at the final core-owned handoff because those transports do not share NodoAssist's guarded fetch.
...
Set `NODOASSIST_SECRET_SENTINELS=off` (also accepts `0` or `false`, case-insensitive) to disable sentinel minting during incident response or compatibility troubleshooting. The kill switch does not disable exact-value redaction registration.
...
- Legacy plaintext residue is scrubbed from `nodoassist.json`, `auth-profiles.json`, `.env`, and generated `models.json` files.
- `nodoassist secrets audit --check` is clean after migration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `NODOASSIST_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
<AccordionGroup>
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/NodoAssist QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    Use a resolver wrapper to map SecretRef ids to Bitwarden Secrets Manager item keys. The repository includes `scripts/secrets/nodoassist-bws-resolver.mjs`; install or copy it to an absolute trusted path on the host that runs the Gateway.
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/nodoassist-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "nodoassist/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    The resolver batches requested ids, runs `bws secret list`, and returns values for matching secret `key` fields. Use keys that satisfy the exec SecretRef id contract, such as `nodoassist/providers/openai/apiKey`; env-var-style keys with underscores are rejected before the resolver runs. If more than one visible Bitwarden secret shares the requested key, the resolver fails that id as ambiguous instead of guessing. After updating config, verify the resolver path:
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/nodoassist"],
  ...
  <Accordion title="password-store (`pass`)">
    Use a small resolver wrapper to map SecretRef ids directly to `pass` entries. Save this as an executable at an absolute path that passes your exec-provider path checks, for example `/usr/local/bin/nodoassist-pass-resolver`. The `#!/usr/bin/env node` shebang resolves `node` from the resolver process `PATH`, so include `PATH` in `passEnv`. If `pass` is not on that `PATH`, set `PASS_BIN` in the parent environment and include it in `passEnv` too:
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/nodoassist-pass-resolver",
...
- NodoAssist resolves these refs during sandbox activation, not lazily on each SSH call.
...
- The redaction sentinel `__NODOASSIST_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `nodoassist.json` refs)
...
When reload-time activation fails after a healthy state, NodoAssist enters degraded secrets state, emitting one-shot system events and log codes:
...
<Tabs>
  <Tab title="Strict command paths">
    For example `nodoassist memory` remote-memory paths and `nodoassist qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `nodoassist status`, `nodoassist status --all`, `nodoassist channels status`, `nodoassist channels resolve`, `nodoassist security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable.
...
- Snapshot refresh after backend secret rotation is handled by `nodoassist secrets reload`.
...
<Steps>
  ...
  <Step title="Configure and apply SecretRefs">
    ```bash
    nodoassist secrets configure --apply
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
If you save a plan instead of applying during `configure`, apply that saved plan with `nodoassist secrets apply --from <plan-path>` before the re-audit.

<AccordionGroup>
  <Accordion title="secrets audit">
    Findings include:

    - Plaintext values at rest (`nodoassist.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`).
    - Plaintext sensitive provider header residues in generated `models.json` entries.
    - Unresolved refs.
    - Precedence shadowing (`auth-profiles.json` taking priority over `nodoassist.json` refs).
    - Legacy residues (`auth.json`, OAuth reminders).

    Exec note: by default, audit skips exec SecretRef resolvability checks to avoid command side effects. Use `nodoassist secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - Lets you select supported secret-bearing fields in `nodoassist.json` plus `auth-profiles.json` for one agent scope.
    ...
    - `nodoassist secrets configure --agent <id>`
...
<Warning>
NodoAssist intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
