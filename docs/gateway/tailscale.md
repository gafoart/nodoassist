...
NodoAssist can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the Gateway dashboard and WebSocket port. This keeps the gateway bound to loopback while Tailscale provides HTTPS, routing, and (for Serve) identity headers.
...
Status and audit output use **Tailscale exposure** for this NodoAssist Serve/Funnel mode. `off` means NodoAssist is not managing Serve or Funnel; it does not mean the local Tailscale daemon is stopped or logged out.
...
```json5
{
  gateway: {
    bind: "loopback",
    tailscale: { mode: "serve", serviceName: "svc:nodoassist" },
  },
}
```
...
Prefer `NODOASSIST_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
nodoassist gateway --tailscale serve
nodoassist gateway --tailscale funnel --auth password
```
...
| `token` (default when `NODOASSIST_GATEWAY_TOKEN` is set) | Shared token                                                                        |
| `password`                                               | Shared secret via `NODOASSIST_GATEWAY_PASSWORD` or config                           |
...
- `gateway.tailscale.serviceName` applies only to Serve mode and is passed to `tailscale serve --service=<name>`. The value must use Tailscale's `svc:<dns-label>` format, for example `svc:nodoassist`. Tailscale requires Service hosts to be tagged nodes, and the Service may need admin-console approval before Serve can publish it.
- `gateway.tailscale.resetOnExit` undoes `tailscale serve`/`tailscale funnel` configuration on shutdown.
- `gateway.tailscale.preserveFunnel: true` keeps an externally configured `tailscale funnel` route alive across gateway restarts. With `mode: "serve"`, NodoAssist checks `tailscale funnel status` before re-applying Serve and skips it when a Funnel route already covers the gateway port. The NodoAssist-managed Funnel password-only policy is unchanged.
