---
summary: "ClawHub CLI entry points for discovering, installing, publishing, and verifying NodoAssist skills and plugins."
read_when:
  - You want to use ClawHub from the command line
  - You want to install ClawHub skills or plugins through NodoAssist
...
- `nodoassist skills` / `nodoassist plugins` - discover, install, and update
  packages for a local NodoAssist agent or Gateway.
...
nodoassist skills install @owner/<slug> --version <version> --global
nodoassist skills update @owner/<slug>
nodoassist skills update --all --acknowledge-clawhub-risk
...
local path. Full flag reference: [`nodoassist skills`](/cli/skills) and
[`nodoassist plugins`](/cli/plugins).

### Release trust

NodoAssist checks a release's ClawHub trust state before downloading it, for
...
- **Risky** releases (non-clean scan, non-blocking moderation state) print a
  ...
  non-interactively.
- **Official ClawHub publishers/packages and bundled NodoAssist sources** skip
...
- [`nodoassist skills`](/cli/skills) - local skill search, install, update, and
  verification
- [`nodoassist plugins`](/cli/plugins) - plugin search, install, update, and
  inspection
