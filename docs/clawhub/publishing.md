...
An owner is a ClawHub publisher handle, such as `@alice` or `@nodoassist`.
...
`@nodoassist/dronzer` can only be published as `@nodoassist`. To publish as
...
```text
Package scope "@nodoassist" must match selected owner "@vintageayu".
Publish as "@nodoassist" or rename this package to "@vintageayu/dronzer".
```
...
```sh
clawhub package transfer @opik/opik-nodoassist --to opik
```
...
`@nodoassist/dronzer` claims the `@nodoassist` namespace, so only publishers with
access to `@nodoassist` can publish or transfer into it.
