...
- `location.get` is a node command, invoked via `node.invoke` or `nodoassist nodes location get`.
- Off by default.
...
- CLI: `nodoassist nodes location get --node <id>`.
- Agent guidelines: only call when the user enabled location and understands the scope.

## UX copy (suggested)

- Off: "Location sharing is disabled."
- While Using: "Only when NodoAssist is open."
- Always: "Allow requested location checks while NodoAssist is in the background."
