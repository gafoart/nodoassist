...
NodoAssist supports camera capture for agent workflows on paired **iOS**, **Android**, and **macOS** nodes: capture a photo (`jpg`) or a short video clip (`mp4`, with optional audio) via Gateway `node.invoke`.
...
```bash
nodoassist nodes camera snap --node <id>                 # default: both front + back (2 MEDIA lines)
nodoassist nodes camera snap --node <id> --facing front
nodoassist nodes camera clip --node <id> --duration 3000
nodoassist nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`nodoassist.cameraEnabled`).
  - Default: **off**.
  - When off: camera requests return `CAMERA_DISABLED: enable Camera in Settings`.

### CLI helper (node invoke)

Use the main `nodoassist` CLI to invoke camera commands on the macOS node.

```bash
nodoassist nodes camera list --node <id>                     # list camera ids
nodoassist nodes camera snap --node <id>                     # prints saved path
nodoassist nodes camera snap --node <id> --max-width 1280
nodoassist nodes camera snap --node <id> --delay-ms 2000
nodoassist nodes camera snap --node <id> --device-id <id>
nodoassist nodes camera clip --node <id> --duration 10s       # prints saved path
nodoassist nodes camera clip --node <id> --duration-ms 3000   # prints saved path (legacy flag)
nodoassist nodes camera clip --node <id> --device-id <id>
nodoassist nodes camera clip --node <id> --no-audio
```

- `nodoassist nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
nodoassist nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
