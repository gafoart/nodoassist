...
## Config (`~/.nodoassist/nodoassist.json`)
...
| `consultThinkingLevel`                   | unset                                      | Thinking level override for the agent run behind realtime `nodoassist_agent_consult` calls.                                                                                                                                                                                |
| `consultFastMode`                        | unset                                      | Fast-mode override for realtime `nodoassist_agent_consult` calls.                                                                                                                                                                                                          |
...
| `realtime.consultRouting`                | -                                          | `provider-direct` preserves the provider's direct reply when it skips `nodoassist_agent_consult`; `force-agent-consult` routes finalized user transcripts through NodoAssist instead.                                                                                      |
| `realtime.instructions`                  | -                                          | Appends provider-facing system instructions to NodoAssist's built-in realtime prompt (voice style/tone); the default `nodoassist_agent_consult` guidance stays.                                                                                                            |
...
- macOS local MLX playback uses the bundled `nodoassist-mlx-tts` helper when present, or an executable on `PATH`. Set `NODOASSIST_MLX_TTS_BIN` to point at a custom helper binary during development.
