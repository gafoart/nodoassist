...
summary: "Fleet supervision plan for Codex app-server sessions controlled by NodoAssist."
read_when:
  - Designing Codex fleet supervision
  - Building NodoAssist tools that read, steer, or spawn Codex sessions
...
Claw Supervisor lets one always-on NodoAssist instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into NodoAssist through MCP.

## Product Model

Codex remains the primary work surface. NodoAssist supervises Codex rather than hiding Codex inside an opaque NodoAssist subagent.

The NodoAssist plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Claw: an always-on NodoAssist agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

NodoAssist may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- NodoAssist tools for Claw-to-Codex control.
...
Every supervised Codex gets an MCP server named `nodoassist-codex-supervisor`.
...
- Supervisor state lives in NodoAssist Gateway storage.
...
- Transcript reads redact secrets at NodoAssist boundary.
...
Phase 2: NodoAssist integration
...
- Durable state owner: NodoAssist Gateway DB, Cloudflare Durable Object, or VPS database.
