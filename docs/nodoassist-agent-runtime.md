---
summary: "Developer workflow for NodoAssist agent runtime: build, test, and live validation"
title: "NodoAssist agent runtime workflow"
read_when:
  - Working on NodoAssist agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

Developer workflow for the agent runtime (`src/agents/`) in the NodoAssist repo.
...
wrapper (sets `NODOASSIST_LIVE_TEST=1` and needs provider credentials):
...
- Run the Gateway in dev mode (skips channel connections via `NODOASSIST_SKIP_CHANNELS=1`): `pnpm gateway:dev`
- Trigger one agent turn through the Gateway: `pnpm nodoassist agent --message "Hello" --thinking low`
...
State lives in the NodoAssist state directory: `~/.nodoassist` by default, or
`$NODOASSIST_STATE_DIR` when set. Paths relative to that directory:

| Path                                             | Holds                                                              |
| ------------------------------------------------ | ------------------------------------------------------------------ |
| `nodoassist.json`                                | Config                                                             |
| `state/nodoassist.sqlite`                        | Shared runtime state database                                      |
| `agents/<agentId>/agent/nodoassist-agent.sqlite` | Per-agent model auth profiles (API keys + OAuth) and runtime state |
...
- Keep auth: leave `agents/<agentId>/agent/nodoassist-agent.sqlite` and `credentials/` in place.

Legacy `auth-profiles.json` files are no longer read at runtime;
`nodoassist doctor --fix` imports them into the SQLite store.
...
- [NodoAssist agent runtime architecture](/agent-runtime-architecture)
