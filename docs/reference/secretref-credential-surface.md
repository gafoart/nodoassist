...
- In scope: strictly user-supplied credentials that NodoAssist does not mint or rotate.
...
### `nodoassist.json` targets (`secrets configure` + `secrets apply` + `secrets audit`)
...
- In `nodoassist.json`, SecretRefs must use structured objects such as `{"source":"env","provider":"default","id":"DISCORD_BOT_TOKEN"}`. Legacy `secretref-env:<ENV_VAR>` marker strings are rejected on SecretRef credential paths; run `nodoassist doctor --fix` to migrate valid markers.
