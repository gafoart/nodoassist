---
summary: "NodoAssist code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable NodoAssist code mode for an agent run
...
Code mode is an experimental, opt-in NodoAssist agent-runtime feature. When
...
This page documents NodoAssist code mode, not Codex Code Mode. The two features
...
- NodoAssist code mode runs in the generic NodoAssist agent runtime and is
...
- Every other enabled tool (NodoAssist core, plugin, MCP, client) is hidden from
...
- Provider neutral: works for NodoAssist, plugin, MCP, and client tools without
...
```bash
NODOASSIST_DEBUG_CODE_MODE=1 \
NODOASSIST_DEBUG_MODEL_TRANSPORT=1 \
NODOASSIST_DEBUG_MODEL_PAYLOAD=tools \
nodoassist gateway
```
...
`NODOASSIST_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
| Stability           | experimental NodoAssist surface (Codex Code Mode is a separate, stable Codex harness surface) |
| Target surface      | generic NodoAssist agent runs                                                                 |
...
- **Code mode**: the NodoAssist runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, NodoAssist fails closed
...
1. Resolve the agent, model, provider, sandbox, channel, sender, and run
   policy.
2. Build the effective NodoAssist tool list, adding eligible plugin, MCP, and
   client tools.
...
when `tools.codeMode.enabled: true` is configured. Code mode and NodoAssist Tool
...
- `command` is accepted as an exec-compatible alias for hook policies and
  trusted rewrites (the normal NodoAssist shell exec tool also uses a `command`
...
- If `language` is `"typescript"`, NodoAssist transpiles before evaluation.
...
final value is JSON-compatible after NodoAssist's output adapter runs.
...
`wait` exists because nested NodoAssist tools can be slow, interactive, approval
...
2. On suspension, NodoAssist snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot and
   re-registers host callbacks by stable names.
4. NodoAssist delivers nested tool results into the restored VM and drains
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "nodoassist" | "mcp" | "client";
  sourceName?: string;
};
```

Plugin tools use `source: "nodoassist"` with `sourceName` set to the owning
...
const hits = await tools.web_search({ query: "NodoAssist code mode" });
```
...
directory. For each code-mode `exec` call, NodoAssist builds the run-scoped tool
...
catalog for arbitrary enabled NodoAssist, plugin, and client tools; use `MCP`
...
ordinary NodoAssist tools, not through the code-mode bridge.

## Tool catalog

The hidden catalog includes tools after effective policy filtering, in this
order: NodoAssist core tools, bundled plugin tools, external plugin tools, MCP
...
where `<source>` is `nodoassist`, `mcp`, or `client` (plugin tools use
`nodoassist` with the plugin id as `<owner>`; core tools use `nodoassist:core:*`).
Examples:

```text
nodoassist:core:message
nodoassist:browser:browser_request
...
Code mode supersedes the NodoAssist Tool Search model surface for runs where it
is active.

When `tools.codeMode.enabled` is true and code mode activates:

- NodoAssist does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same NodoAssist executor path that Tool
  Search uses.

See [Tool Search](/tools/tool-search) for the NodoAssist compact catalog bridge
that code mode supersedes for active runs.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal NodoAssist
shell `exec` tool is enabled, it is hidden from the model and cataloged like
any other tool.

Inside the guest runtime:

- `tools.call("nodoassist:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, NodoAssist omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters NodoAssist,
...
- NodoAssist caps the number of concurrently suspended runs per process (64) and
...
NodoAssist loads `quickjs-wasi` as a direct dependency in the owning package; it
...
The runtime executes in a Node.js worker thread, outside NodoAssist's main
...
breakdown (`nodoassist`/`mcp`/`client` counts), cumulative search/describe/call
...
tool inputs beyond existing NodoAssist trajectory policy.
...
NODOASSIST_DEBUG_MODEL_TRANSPORT=1 \
NODOASSIST_DEBUG_MODEL_PAYLOAD=tools \
NODOASSIST_DEBUG_SSE=events \
nodoassist gateway
```

For payload-shape debugging, use `NODOASSIST_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `NODOASSIST_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for NodoAssist tools
...
5. Send an agent turn with NodoAssist, plugin, MCP, and client test tools.
...
7. In `exec`, read `ALL_TOOLS` and assert the effective test tools are
   present.
8. In `exec`, call NodoAssist/plugin/client tools through `tools.search`,
