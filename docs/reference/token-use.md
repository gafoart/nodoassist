---
summary: "How NodoAssist builds prompt context and reports token usage + costs"
read_when:
...
NodoAssist tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

NodoAssist assembles its own system prompt on every run. It includes:
...
- Workspace + bootstrap files (`AGENTS.md`, `SOUL.md`, `TOOLS.md`,
  ...
  - Lowercase root `memory.md` is never injected. It is legacy repair input
    for `nodoassist doctor --fix`, which migrates it into `MEMORY.md`.
...
`toolResultMaxChars` is unset by default, so NodoAssist derives the live
...
For images, NodoAssist downscales transcript/tool image payloads before
...
- `/usage off|tokens|full` -> appends a per-response usage footer to every
  ...
  - `/usage full` shows compact model/context/cost details; estimated cost
    appears only when NodoAssist has usage metadata and local pricing for the
    active model. Custom `messages.usageTemplate` layouts can include
    token/cache fields.
- `/usage cost` -> local cost summary from NodoAssist session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` and `/usage` are supported.
- **CLI:** `nodoassist status --usage` and `nodoassist channels list` show
...
NodoAssist falls back to matching OAuth/API-key credentials from auth
...
NodoAssist keeps provider usage accounting separate from the current context
...
After sidecars and channels reach the Gateway ready path, NodoAssist starts an
...
Provider prompt caching only applies within the cache TTL window. NodoAssist
...
NodoAssist sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus
...
Older configs can keep `context1m: true`, but NodoAssist no longer sends
...
(`sk-ant-oat-*`), NodoAssist preserves the OAuth-required Anthropic beta
