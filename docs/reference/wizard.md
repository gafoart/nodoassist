...
This is the full reference for `nodoassist onboard`.
...
<Steps>
  <Step title="Reset (optional)">
    ...
    - If the config file is invalid, onboarding stops and tells you to run
      `nodoassist doctor` first, then re-run setup.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic Claude CLI**: preferred local path when a Claude CLI sign-in already exists; NodoAssist still supports Anthropic setup-token auth as an alternative.
    ...
    - Auth profiles live in `~/.nodoassist/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.nodoassist/credentials/oauth.json` is legacy import-only.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `nodoassist pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `nodoassist configure --section web`.
  ...
  <Step title="Daemon install">
    ...
    - Native Windows: Scheduled Task first; if task creation is denied, NodoAssist falls back to a per-user Startup-folder login item and starts the Gateway immediately.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `nodoassist health`.
    - Tip: `nodoassist status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  ...
  <Step title="Skills (recommended)">
    ...
    - Skips skills whose Homebrew, uv, or Go installer prerequisite is unavailable, groups them with manual setup guidance, and points you at `nodoassist doctor` once the prerequisite is installed.
...
`main` is a reserved agent id and cannot be used for `nodoassist agents add`.
...
- Linux x86-64: downloads the official native GraalVM build from the `signal-cli` GitHub releases and stores it under `~/.nodoassist/tools/signal-cli/<version>/`.
