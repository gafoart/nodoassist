...
- Flags are case-insensitive strings, resolved from `diagnostics.flags` in
  config plus the `NODOASSIST_DIAGNOSTICS` env override, deduped and lowercased.
...
```bash
NODOASSIST_DIAGNOSTICS=telegram.http,brave.http
```
...
`NODOASSIST_DIAGNOSTICS=0` disables flags from both env and config for that
...
```bash
NODOASSIST_DIAGNOSTICS=profiler nodoassist gateway run
```

Enable only reply-dispatch profiler spans:

```bash
NODOASSIST_DIAGNOSTICS=reply.profiler nodoassist gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
NODOASSIST_DIAGNOSTICS=codex.profiler nodoassist gateway run
```
...
`NODOASSIST_DIAGNOSTICS=0` to override every diagnostics flag for that run.
...
NODOASSIST_DIAGNOSTICS_TIMELINE_PATH=/tmp/nodoassist-timeline.jsonl \
...
The output path always comes from `NODOASSIST_DIAGNOSTICS_TIMELINE_PATH`, even
...
are missing because NodoAssist has not read config yet; subsequent startup spans
are captured normally.

`NODOASSIST_DIAGNOSTICS=1`, `=all`, and `=*` also enable the timeline, since they
...
`timeline`: set `NODOASSIST_DIAGNOSTICS_EVENT_LOOP=1` (or `on`/`true`/`yes`) on
top of enabling the timeline.

Timeline records use the `nodoassist.diagnostics.v1` envelope and can include
...
```bash
ls -t /tmp/nodoassist/nodoassist-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/nodoassist/nodoassist-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/nodoassist/nodoassist-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/nodoassist/nodoassist-$(date +%F).log | rg "telegram http error"
```

For remote gateways, use `nodoassist logs --follow` instead (see
[/cli/logs](/cli/logs)).
