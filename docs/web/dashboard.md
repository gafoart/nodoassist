...
- Re-open anytime: `nodoassist dashboard` (copies the link, opens a browser if possible, prints an SSH hint if headless).
- If clipboard and browser delivery both fail, `nodoassist dashboard` still prints the clean URL and tells you to append your token (from `NODOASSIST_GATEWAY_TOKEN` or `gateway.auth.token`) as the URL fragment key `token`; it never prints the token value in logs.
...
- **Shared-secret token source**: `gateway.auth.token` (or `NODOASSIST_GATEWAY_TOKEN`). `nodoassist dashboard` can pass it via URL fragment for one-time bootstrap; the Control UI keeps it in sessionStorage for the current tab and selected gateway URL, not localStorage.
- If `gateway.auth.token` is SecretRef-managed, `nodoassist dashboard` prints/copies/opens a non-tokenized URL by design, to avoid exposing externally managed tokens in shell logs, clipboard history, or browser-launch arguments. If the ref is unresolved in your current shell, it still prints the non-tokenized URL plus actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or `NODOASSIST_GATEWAY_PASSWORD`). The dashboard does not persist passwords across reloads.
...
- Confirm the gateway is reachable: local `nodoassist status`; remote, SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@gateway-host` then open `http://127.0.0.1:18789/`.
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `nodoassist config get gateway.auth.token`
  - Password: resolve the configured `gateway.auth.password` or `NODOASSIST_GATEWAY_PASSWORD`
  - SecretRef-managed token: resolve the external secret provider, or export `NODOASSIST_GATEWAY_TOKEN` in this shell and rerun `nodoassist dashboard`
  - No shared secret configured: `nodoassist doctor --generate-gateway-token`
