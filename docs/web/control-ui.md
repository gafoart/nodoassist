...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/nodoassist`)
...
If the page fails to load, start the Gateway first: `nodoassist gateway`.

<Note>
On native Windows LAN binds, Windows Firewall or organization-managed Group Policy can still block the advertised LAN URL even when `127.0.0.1` works on the Gateway host. Run `nodoassist gateway status --deep` on the Windows host; it reports likely-blocked ports, profile mismatches, and local firewall rules that policy may ignore.
</Note>
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created; re-run `nodoassist devices list` before approving.

Switching an already-paired browser from read access to write/admin access is treated as an approval upgrade, not a silent reconnect: NodoAssist keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `nodoassist devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation, revocation, and the Paperclip / `nodoassist_gateway` first-run approval flow.
...
<Steps>
  ...
  <Step title="Connect the phone">
    In the NodoAssist mobile app, open **Settings** → **Gateway** and scan the QR code. You can copy and paste the setup code instead.
  </Step>
...
The Control UI fetches its runtime settings from `/control-ui-config.json`, resolved relative to the gateway's Control UI base path (for example `/__nodoassist__/control-ui-config.json` under base path `/__nodoassist__/`). That endpoint is gated by the same gateway auth as the rest of the HTTP surface: unauthenticated browsers cannot fetch it, and a successful fetch requires a valid gateway token/password, Tailscale Serve identity, or a trusted-proxy identity.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.nodoassist/nodoassist.json` (`config.get`, `config.set`).
  ...
  <Accordion title="Cron jobs panel notes">
    ...
    - `cron.webhook` is a deprecated legacy fallback: run `nodoassist doctor --fix` to migrate stored jobs that still use `notify: true` to explicit per-job webhook or completion delivery.
...
The dedicated MCP page is an operator view for NodoAssist-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `nodoassist mcp doctor --probe` when you need live server proof.
...
7. Run `nodoassist mcp status --verbose`, `nodoassist mcp doctor --probe`, or `nodoassist mcp reload` from a terminal for static diagnostics, live proof, or cached-runtime disposal.
...
<Warning>
The terminal is an unconfined host shell and inherits the Gateway process environment. Enable it only for trusted operator deployments. NodoAssist refuses terminal sessions for agents with `sandbox.mode: "all"`; changing an active agent to that mode closes its existing and in-flight terminal sessions.
</Warning>
...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - In the macOS app, the NodoAssist mark uses the otherwise-empty native titlebar strip next to the window controls instead of consuming a sidebar row.
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop nodoassist`, `please stop`) to abort out-of-band.
...
token), first opens and reloads show a small animated NodoAssist mark while the connection is
...
If the page shows **Protocol mismatch** right after an NodoAssist update, first reopen the dashboard with `nodoassist dashboard` and hard-refresh. If it still fails, clear site data for the dashboard origin or test in a private browser window; an old tab or browser service-worker cache can keep running a pre-update Control UI bundle against the newer Gateway.
...
| `push/vapid-keys.json` (under the NodoAssist state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `NODOASSIST_VAPID_PUBLIC_KEY`
- `NODOASSIST_VAPID_PRIVATE_KEY`
- `NODOASSIST_VAPID_SUBJECT` (defaults to `https://openclaw.ai`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    nodoassist gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, NodoAssist **blocks** Control UI connections without device identity.
...
- `GET /__nodoassist__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth; the browser sends the gateway token as a bearer header when checking availability.
...
```bash
NODOASSIST_CONTROL_UI_BASE_PATH=/nodoassist/ pnpm ui:build
```
...
If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<nodoassist-app>` is not registered after startup.
