...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    **local config or environment issues** a remote helper cannot inspect, so this beats
    ...
    | Command | Shows |
    | --- | --- |
    | `nodoassist status` | Gateway/agent health + basic config snapshot |
    | `nodoassist status --all` | Full read-only diagnosis, pasteable |
    | `nodoassist models status` | Provider auth + model availability |
    | `nodoassist doctor` | Validates and repairs common config/state issues |
    | `nodoassist logs --follow` | Live log tail |
    | `nodoassist gateway status --deep` | Deep gateway/config/plugin health check |
    | `nodoassist health --verbose` | Detailed health report |
  ...
  <Accordion title="Recommended way to install and set up NodoAssist">
    ```bash
    ...
    No global install yet? Run `pnpm nodoassist onboard` instead. If Control UI assets are
  ...
  <Accordion title="How do I authenticate the dashboard on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Token source: `gateway.auth.token` (or `NODOASSIST_GATEWAY_TOKEN`).
    - Password source: `gateway.auth.password` (or `NODOASSIST_GATEWAY_PASSWORD`).
    - No shared secret configured yet? Run `nodoassist doctor --generate-gateway-token` (or `nodoassist doctor --fix --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `nodoassist gateway --tailscale serve`, open `https://<magicdns>/`. With `gateway.auth.allowTailscale: true`, identity headers satisfy Control UI/WebSocket auth (no pasted shared secret, assumes a trusted gateway host); HTTP APIs still need shared-secret auth unless you deliberately use private-ingress `none` or trusted-proxy HTTP auth.
    ...
    - **Tailnet bind**: run `nodoassist gateway --bind tailnet --token "<token>"` (or configure password auth), open `http://<tailscale-ip>:18789/`, paste the matching shared secret in dashboard settings.
  ...
  <Accordion title="Why are there two exec approval configs for chat approvals?">
    ...
    - When a supported native channel can infer approvers safely, NodoAssist auto-enables DM-first native approvals if `channels.<channel>.execApprovals.enabled` is unset or `"auto"`.
  ...
  <Accordion title="It is stuck on wake up my friend / onboarding will not hatch. What now?">
    ...
    TUI without sending anything — add a provider with `nodoassist configure --section model`.
  ...
  <Accordion title="Can I migrate my setup to a new machine without redoing onboarding?">
    ...
    1. Install NodoAssist on the new machine.
    2. Copy `$NODOASSIST_STATE_DIR` (default: `~/.nodoassist`) from the old machine.
    3. Copy your workspace (default: `~/.nodoassist/workspace`).
    4. Run `nodoassist doctor` and restart the Gateway service.
    ...
    **memory + bootstrap files**, but not session history or auth. Those live under
    `~/.nodoassist/` (for example `~/.nodoassist/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Windows install says git not found or nodoassist not recognized">
    ...
    **2) nodoassist is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    Still reproducing this on latest NodoAssist? Track/report it: [Issue #30640](https://github.com/openclaw/openclaw/issues/30640).
  ...
  <Accordion title="How do I install NodoAssist on Linux?">
  ...
  <Accordion title="How do I install NodoAssist on a VPS?">
  ...
  <Accordion title="Can I ask NodoAssist to update itself?">
    ...
    ```bash
    nodoassist update
    nodoassist update status
    nodoassist update --channel stable|extended-stable|beta|dev
    nodoassist update --tag <dist-tag|version>
    nodoassist update --no-restart
    ```

    Automating from an agent:

    ```bash
    nodoassist update --yes --no-restart
    nodoassist gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `nodoassist onboard` is the recommended setup path. In **local mode** it walks through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. Run NodoAssist with **API keys** (Anthropic/OpenAI/others) or **local-only models**
    ...
    NodoAssist also supports hosted subscription-style options including **Qwen Cloud
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    Yes. NodoAssist supports Claude CLI reuse for Pro/Max/Team/Enterprise plans. Anthropic
    currently treats the `claude -p` path NodoAssist uses as subscription-plan usage subject
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic setup-token auth is also still a supported token path, but NodoAssist prefers
...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Set a **fallback model** so NodoAssist keeps replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. NodoAssist has a bundled **Amazon Bedrock (Converse)** provider. With AWS env
    markers present (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, `AWS_BEARER_TOKEN_BEDROCK`),
    NodoAssist auto-enables the implicit Bedrock provider for model discovery; otherwise
  ...
  <Accordion title="How does Codex auth work?">
    NodoAssist supports **OpenAI Codex** via OAuth (ChatGPT sign-in). Use `openai/gpt-5.5`
    ...
    `nodoassist doctor --fix`. Direct OpenAI API-key access remains available for non-agent
  ...
  <Accordion title="Why does NodoAssist still mention legacy OpenAI Codex prefix?">
    ...
    - Legacy `openai-codex/*` model refs = legacy route repaired by `nodoassist doctor --fix`.
    - `openai/gpt-5.5` plus an ordered `openai` API-key profile = API-key auth for an OpenAI agent model.
    - Legacy `openai-codex` auth profile ids = legacy ids migrated by `nodoassist doctor --fix`.

    Want direct OpenAI Platform billing? Set `OPENAI_API_KEY`. Want ChatGPT/Codex
    subscription auth? Run `nodoassist models auth login --provider openai`. Keep the model
    ref as `openai/gpt-5.5`; legacy Codex-prefixed refs are what `nodoassist doctor --fix` rewrites.
  ...
  <Accordion title="Why can Codex OAuth limits differ from ChatGPT web?">
    ...
    `nodoassist models status` shows the currently visible provider usage/quota windows, but
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    ...
    tools/workflows like NodoAssist. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `nodoassist.json`.

    1. Install Gemini CLI locally so `gemini` is on `PATH`:
       - Homebrew: `brew install gemini-cli`
       - npm: `npm install -g @google/gemini-cli`
    2. Enable the plugin: `nodoassist plugins enable google`
    3. Login: `nodoassist models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. NodoAssist needs large context + strong safety; small cards truncate context
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. NodoAssist runs on macOS or Linux (Windows via WSL2). A Mac mini is a popular
  ...
  <Accordion title="If I buy a Mac mini to run NodoAssist, can I connect it to my MacBook Pro?">
    ...
    node host and pairs to the Gateway. Check with `nodoassist nodes status` / `nodoassist nodes list`.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    not the bot username. Setup asks for numeric user IDs only; `nodoassist doctor --fix`
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different NodoAssist instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    Running NodoAssist via systemd: make sure the service PATH includes
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    Yes, with `nodoassist update --channel ...` on an existing install. This does **not
    delete your data** - only the NodoAssist code install changes. State (`~/.nodoassist`) and
    workspace (`~/.nodoassist/workspace`) stay untouched.
  ...
  <Accordion title="How important is it to run NodoAssist on a dedicated machine?">
  ...
  <Accordion title="Can I run NodoAssist in a VM and what are the requirements?">
