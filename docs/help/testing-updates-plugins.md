---
summary: "How NodoAssist validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing NodoAssist update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `nodoassist doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` covers plugin install smoke, local folder installs,
  ...
  and Claude-bundle enable/inspect. Set `NODOASSIST_PLUGINS_E2E_CLAWHUB=0` to
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `nodoassist plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `nodoassist config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `nodoassist update --yes --json`, and requires the candidate update command to
...
```bash
NODOASSIST_UPGRADE_SURVIVOR_BASELINE_SPEC=nodoassist@2026.4.23 \
NODOASSIST_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

NODOASSIST_UPGRADE_SURVIVOR_BASELINE_SPEC=nodoassist@latest \
NODOASSIST_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```

Available scenarios: `base`, `acpx-nodoassist-tools-bridge`, `feishu-channel`,
`bootstrap-persona`, `channel-post-core-restore`, `plugin-deps-cleanup`,
`configured-plugin-installs`, `stale-source-plugin-shadow`, `tilde-log-path`,
and `versioned-runtime-deps`. In aggregate runs, `NODOASSIST_UPGRADE_SURVIVOR_SCENARIOS=reported-issues`
...
- `source=npm`: validate `nodoassist@extended-stable`, `nodoassist@beta`,
  `nodoassist@latest`, or an exact published version.
...
- `source=trusted-url`: validate an HTTPS tarball with required
  ...
  `NODOASSIST_TRUSTED_PACKAGE_TOKEN` secret.
...
`package_acceptance_package_spec=nodoassist@YYYY.M.PATCH` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published NodoAssist
...
`package_spec=nodoassist@extended-stable`. Package Acceptance resolves that
