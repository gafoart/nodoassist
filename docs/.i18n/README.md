...
- repo: `nodoassist/docs`
- local checkout: `~/Projects/nodoassist-docs`

## Source of truth

- English docs are authored in `nodoassist/nodoassist`.
...
1. Edit English docs in `nodoassist/nodoassist`.
2. Push to `main`.
3. `nodoassist/nodoassist/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `nodoassist/docs`.
...
5. `nodoassist/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `nodoassist/docs`.
...
- Do not treat locale visibility in generated `docs/docs.json` as proof that translation artifacts exist. Verify each generated locale folder and its translation memory in `nodoassist/docs`.
...
- Sync metadata is written to `.nodoassist-sync/source.json` in the publish repo.
- Source repo secret: `NODOASSIST_DOCS_SYNC_TOKEN`
- Publish repo secret: `NODOASSIST_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `nodoassist/docs` first.
