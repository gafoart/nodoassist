...
summary: "Specification for making the bundled Codex app-server harness honor NodoAssist context-engine plugins"
read_when:
  ...
  - You are comparing embedded NodoAssist and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same NodoAssist context-engine
lifecycle contract that embedded NodoAssist turns already honor.
...
NodoAssist-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in NodoAssist attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in NodoAssist-controlled code. The external
boundary is the Codex app-server protocol itself: NodoAssist can control what it
...
Built-in NodoAssist attempts call the context-engine lifecycle directly:
...
For Codex harness turns, NodoAssist should preserve this lifecycle:

1. Read the mirrored NodoAssist session transcript.
...
8. Mirror the Codex result back into the NodoAssist transcript.
...
11. Preserve Codex native compaction signals and NodoAssist compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. NodoAssist should
not try to mutate the app-server's internal history except through supported
protocol calls.

NodoAssist's transcript mirror remains the source for NodoAssist features:
...
The context-engine interface returns NodoAssist `AgentMessage[]`, not a Codex
...
- `runtime: "nodoassist"` selects the built-in NodoAssist harness
- `runtime: "codex"` selects the registered Codex harness
- `runtime: "auto"` lets plugin harnesses claim supported providers
- unmatched `auto` runs use the built-in NodoAssist harness
...
- Label it clearly as NodoAssist assembled context.
...
inside NodoAssist and preserves context-engine semantics.
...
we need strict NodoAssist parity, run context-engine assembly before hook
...
1. NodoAssist context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit NodoAssist compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
NodoAssist compaction should prefer the context engine's `compact()` result for
the NodoAssist transcript mirror and plugin state.
...
the NodoAssist session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
NodoAssist session lifecycle paths. Do not add Codex-specific cleanup unless the
context-engine lifecycle currently misses reset/delete events for all harnesses.

### 10. Error handling

Follow built-in NodoAssist semantics:
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole NodoAssist compaction when the context engine is primary.
...
Avoid requiring lossless-claw in NodoAssist core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for NodoAssist/plugin state.
