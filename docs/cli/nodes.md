---
summary: "CLI reference for `nodoassist nodes` (status, pairing, invoke, camera/canvas/screen/location/notify)"
read_when:
...
nodoassist nodes status --last-connected 24h
nodoassist nodes list
...
nodoassist nodes reject <requestId>
nodoassist nodes remove --node <id|name|ip>
nodoassist nodes rename --node <id|name|ip> --name <displayName>
```

These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`nodoassist devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/nodes) for how the two relate.
...
```bash
nodoassist nodes invoke --node <id> --command system.which --params '{"name":"uname"}'
```
...
```bash
nodoassist nodes notify --node <id> --title "Build" --body "Done" --priority timeSensitive
nodoassist nodes push --node <id> --title "NodoAssist" --environment sandbox
nodoassist nodes location get --node <id> --accuracy precise
nodoassist nodes screen record --node <id> --duration 10s --fps 10 --out ./clip.mp4
```
...
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `NodoAssist`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment.
...
Camera and Canvas commands have their own docs: [Camera nodes](/nodes/camera), [Canvas](/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `nodoassist nodes canvas` as a compatibility mount point.
