---
summary: "CLI reference for `nodoassist sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `nodoassist channels status --probe`,
`nodoassist status --deep`, or `nodoassist health --verbose` when you need live
channel connectivity.

```bash
nodoassist sessions
nodoassist sessions --agent work
nodoassist sessions --all-agents
nodoassist sessions --active 120
nodoassist sessions --limit 25
nodoassist sessions --store ./tmp/sessions.json
nodoassist sessions --json
```
...
`nodoassist sessions` and the Gateway `sessions.list` RPC are bounded by default
...
`nodoassist sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.nodoassist/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.nodoassist/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
```bash
nodoassist sessions tail
nodoassist sessions tail --follow
nodoassist sessions tail --session-key "agent:main:telegram:direct:123" --tail 25
nodoassist sessions --agent work tail --follow
nodoassist sessions --all-agents tail --follow
```

`nodoassist sessions tail` renders recent trajectory JSONL events as compact
...
```bash
nodoassist sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
nodoassist sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.nodoassist/trajectory-exports/` under the selected workspace.
...
nodoassist sessions cleanup --agent work --dry-run
nodoassist sessions cleanup --all-agents --dry-run
nodoassist sessions cleanup --enforce
nodoassist sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
nodoassist sessions cleanup --dry-run --fix-dm-scope
nodoassist sessions cleanup --json
```

`nodoassist sessions cleanup` uses `session.maintenance` settings from config
([Configuration reference](/gateway/config-agents#session)):

- Scope note: `nodoassist sessions cleanup` maintains session stores,
...
`nodoassist sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.nodoassist/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.nodoassist/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
...
Reclaim context budget for a wedged or oversized session. `nodoassist sessions
...
<Note>
`nodoassist agent --message '/compact ...'` is **not** a compaction path. Slash
...
`nodoassist gateway call sessions.compact --params '<json>'` accepts:
...
```json
{
  ...
  "key": "agent:main:main",
  "compacted": true,
  "archived": "/home/user/.nodoassist/agents/main/sessions/transcripts/<id>.jsonl.bak",
  "kept": 200
}
```
