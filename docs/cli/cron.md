---
summary: "CLI reference for `nodoassist cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `nodoassist cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`nodoassist cron create` is an alias for `nodoassist cron add`. For new jobs, put the schedule first and the prompt second:
...
Use `--command` for deterministic shell-style jobs that run inside NodoAssist cron without starting an isolated agent/model run:
...
`nodoassist cron list` and `nodoassist cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `nodoassist cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
Cron jobs, pending runtime state, and run history live in the shared SQLite state database. Legacy `jobs.json`, `<name>-state.json`, and `runs/*.jsonl` files are imported once and renamed with a `.migrated` suffix. After import, edit schedules with `nodoassist cron add|edit|remove` instead of editing JSON files.

### Manual runs

`nodoassist cron run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
nodoassist cron run <job-id>
nodoassist cron runs --id <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
nodoassist cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, NodoAssist passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
- Local-provider preflight checks walk configured fallbacks before marking a cron run `skipped`.

`nodoassist doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
nodoassist cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
nodoassist cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
nodoassist cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
nodoassist cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
nodoassist cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
nodoassist cron list --agent ops
nodoassist cron get <job-id>
nodoassist cron show <job-id>
nodoassist cron run <job-id>
nodoassist cron run <job-id> --due
nodoassist cron run <job-id> --wait --wait-timeout 10m
nodoassist cron run <job-id> --wait --wait-timeout 10m --poll-interval 2s
nodoassist cron runs --id <job-id> --limit 50
nodoassist cron runs --id <job-id> --run-id <run-id>
```

`nodoassist cron list` shows all matching jobs by default. Pass `--agent <id>` to show only jobs whose effective normalized agent id matches; jobs without a stored agent id count as the configured default agent.

`nodoassist cron get <job-id>` returns the stored job JSON directly. Use `cron show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
nodoassist cron edit <job-id> --agent ops
nodoassist cron edit <job-id> --clear-agent
nodoassist cron edit <job-id> --session current
nodoassist cron edit <job-id> --session "session:daily-brief"
```

`nodoassist cron add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
nodoassist cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
nodoassist cron edit <job-id> --webhook "https://example.invalid/openclaw/cron"
nodoassist cron edit <job-id> --best-effort-deliver
nodoassist cron edit <job-id> --no-best-effort-deliver
nodoassist cron edit <job-id> --no-deliver
```
