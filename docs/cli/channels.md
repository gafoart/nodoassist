---
summary: "CLI reference for `nodoassist channels` (accounts, status, capabilities, resolve, logs, login/logout)"
read_when:
...
```bash
nodoassist channels list
nodoassist channels list --all
nodoassist channels status
nodoassist channels capabilities
nodoassist channels capabilities --channel discord --target channel:123
nodoassist channels resolve --channel slack "#general" "@jane"
nodoassist channels logs --channel all
```

`channels list` shows chat channels only: configured accounts by default, with `installed`, `configured`, and `enabled` status tags per account (`--json` for machine output). Pass `--all` to also surface bundled channels that have no configured account yet and installable catalog channels that are not yet on disk. Provider auth and model usage live elsewhere: `nodoassist models auth list` for provider auth profiles, `nodoassist status` or `nodoassist models list` for usage/quota.
...
Do not use `nodoassist sessions`, Gateway `sessions.list`, or the agent
...
```bash
nodoassist channels add --channel telegram --token <bot-token>
nodoassist channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
nodoassist channels remove --channel telegram --delete
```

<Tip>
`nodoassist channels add --help` shows per-channel flags (token, private key, app token, signal-cli paths, etc).
</Tip>
...
If a channel plugin needs to be installed during a flag-driven add command, NodoAssist uses the channel's default install source without opening the interactive plugin install prompt.

When you run `nodoassist channels add` without flags, the interactive wizard can prompt:
...
You can also manage the same routing rules later with `nodoassist agents bindings`, `nodoassist agents bind`, and `nodoassist agents unbind` (see [agents](/cli/agents)).

When you add a non-default account to a channel that is still using single-account top-level settings, NodoAssist promotes those top-level values into the channel's account map before writing the new account. Promotion reuses an existing named account when the channel has exactly one, or when `defaultAccount` points at one; otherwise the values land in `channels.<channel>.accounts.default`.
...
If your config was already in a mixed state (named accounts present and top-level single-account values still set), run `nodoassist doctor --fix` to move account-scoped values into the promoted account chosen for that channel.

## Login and logout (interactive)

```bash
nodoassist channels login --channel whatsapp
nodoassist channels logout --channel whatsapp
```
...
- Run `nodoassist status --deep` for a broad probe.
- Use `nodoassist doctor` for guided fixes.
- `nodoassist channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
nodoassist channels resolve --channel slack "#general" "@jane"
nodoassist channels resolve --channel discord "My Server/#support" "@someone"
nodoassist channels resolve --channel matrix "Project Room"
```
