...
The Gateway writes records to the shared NodoAssist state database through a
...
```bash
nodoassist audit
nodoassist audit --agent main --status failed
nodoassist audit --session "agent:main:main" --after 2026-07-01T00:00:00Z
nodoassist audit --run 8c69f72e-8b11-4c54-98d5-1a3dd67450c3
nodoassist audit --kind tool_action --limit 50 --json
```
...
```bash
nodoassist gateway call audit.list --params '{"agentId":"main","status":"failed","limit":50}'
```
