---
summary: "CLI reference for `nodoassist path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
nodoassist plugins enable oc-path
```
...
NodoAssist state is spread across human-edited markdown, commented JSONC
...
`nodoassist path` gives those callers a stable address instead of a one-off
...
Skip `nodoassist path` for ordinary whole-file edits, rich config migrations, or
...
```bash
nodoassist path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
nodoassist path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
nodoassist path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
nodoassist path resolve 'oc://AGENTS.md/runtime-safety/nodoassist-gateway'
```
...
```bash
nodoassist path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
nodoassist path resolve 'oc://gateway.jsonc/version'

# Wildcard search
nodoassist path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
nodoassist path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
nodoassist path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
nodoassist path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
nodoassist path emit ./AGENTS.md
```
...
nodoassist path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
nodoassist path set 'oc://nodoassist.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
nodoassist path set 'oc://nodoassist.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
nodoassist path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
nodoassist path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
nodoassist path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
nodoassist path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
nodoassist path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
nodoassist path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
nodoassist path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
nodoassist path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
nodoassist path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
nodoassist path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
nodoassist path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ nodoassist path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ nodoassist path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ nodoassist path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ nodoassist path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ nodoassist path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ nodoassist path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ nodoassist path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: nodoassist.invoke
```

```bash
$ nodoassist path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ nodoassist path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
nodoassist path resolve 'oc://AGENTS.md/tools/gh/risk' --human
nodoassist path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
nodoassist path find 'oc://AGENTS.md/tools/**/risk'
nodoassist path find 'oc://session.jsonl/[event=action]/userId'
nodoassist path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
nodoassist path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
nodoassist path set 'oc://gateway.jsonc/version' '2.0'
nodoassist path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ nodoassist path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
nodoassist path emit ./AGENTS.md
nodoassist path emit ./gateway.jsonc --json
```
...
`nodoassist path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__NODOASSIST_REDACTED__` (verbatim or as a substring) is refused at write
  time.
