...
read_when:
  - Adding or modifying `nodoassist infer` commands
  - Designing stable headless capability automation
title: "Inference CLI"
---

`nodoassist infer` is the canonical headless surface for provider-backed inference. It exposes capability families (`model`, `image`, `audio`, `tts`, `video`, `web`, `embedding`), not raw gateway RPC names or agent tool ids. `nodoassist capability ...` is an alias for the same command tree.

Reasons to prefer it over a one-off provider wrapper:

- Reuses providers and models already configured in NodoAssist.
...
```text
Read https://docs.openclaw.ai/cli/infer, then create a skill that routes my common workflows to `nodoassist infer`.
Focus on model runs, image generation, video generation, audio transcription, TTS, web search, and embeddings.
```

A good infer-based skill maps common user intents to the right subcommand, includes a few canonical examples per workflow, prefers `nodoassist infer ...` over lower-level alternatives, and does not re-document the entire infer surface in the skill body.
...
| Task                          | Command                                                                                         | Notes                                                 |
| ----------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| Run a text/model prompt       | `nodoassist infer model run --prompt "..." --json`                                              | Local by default                                      |
| Run a model prompt on images  | `nodoassist infer model run --prompt "Describe this" --file ./image.png --model provider/model` | Repeat `--file` for multiple images                   |
| Generate an image             | `nodoassist infer image generate --prompt "..." --json`                                         | Use `image edit` when starting from an existing file  |
| Describe an image file or URL | `nodoassist infer image describe --file ./image.png --prompt "..." --json`                      | `--model` must be an image-capable `<provider/model>` |
| Transcribe audio              | `nodoassist infer audio transcribe --file ./memo.m4a --json`                                    | `--model` must be `<provider/model>`                  |
| Synthesize speech             | `nodoassist infer tts convert --text "..." --output ./speech.mp3 --json`                        | `tts status` only runs through the gateway            |
| Generate a video              | `nodoassist infer video generate --prompt "..." --json`                                         | Supports provider hints such as `--resolution`        |
| Describe a video file         | `nodoassist infer video describe --file ./clip.mp4 --json`                                      | `--model` must be `<provider/model>`                  |
| Search the web                | `nodoassist infer web search --query "..." --json`                                              |                                                       |
| Fetch a web page              | `nodoassist infer web fetch --url https://example.com --json`                                   |                                                       |
| Create embeddings             | `nodoassist infer embedding create --text "..." --json`                                         |                                                       |
...
```bash
nodoassist infer model run --prompt "Reply with exactly: smoke-ok" --json
nodoassist infer model run --prompt "Summarize this changelog entry" --model openai/gpt-5.4 --json
nodoassist infer model run --prompt "Describe this image in one sentence" --file ./photo.jpg --model google/gemini-2.5-flash --json
nodoassist infer model run --prompt "Use more reasoning here" --thinking high --json
nodoassist infer model providers --json
nodoassist infer model inspect --model gpt-5.5 --json
```
...
```bash
nodoassist infer model run --local --model anthropic/claude-sonnet-4-6 --prompt "Reply with exactly: pong" --json
nodoassist infer model run --local --model cerebras/zai-glm-4.7 --prompt "Reply with exactly: pong" --json
nodoassist infer model run --local --model google/gemini-2.5-flash --prompt "Reply with exactly: pong" --json
nodoassist infer model run --local --model groq/llama-3.1-8b-instant --prompt "Reply with exactly: pong" --json
nodoassist infer model run --local --model mistral/mistral-medium-3-5 --prompt "Reply with exactly: pong" --json
nodoassist infer model run --local --model mistral/mistral-small-latest --prompt "Reply with exactly: pong" --json
nodoassist infer model run --local --model openai/gpt-5.5 --prompt "Reply with exactly: pong" --json
nodoassist infer model run --local --model ollama/qwen2.5vl:7b --prompt "Describe this image." --file ./photo.jpg --json
```
...
- Local `model run --model <provider/model>` can resolve exact bundled static-catalog rows (the same rows `nodoassist models list --all` shows) before that provider is written to config. Provider auth is still required; missing credentials fail as auth errors, not `Unknown model`.
...
- Use `model run --gateway` to test Gateway routing or agent-runtime setup while keeping the model input raw. Use `nodoassist agent` or a chat surface for full agent context, tools, memory, and session transcript.
...
nodoassist infer image generate --prompt "slow image backend" --timeout-ms 180000 --json
nodoassist infer image edit --file ./logo.png --model openai/gpt-image-1.5 --output-format png --background transparent --prompt "keep the logo, remove the background" --json
nodoassist infer image edit --file ./poster.png --prompt "make this a vertical story ad" --size 2160x3840 --aspect-ratio 9:16 --resolution 4K --json
nodoassist infer image describe --file ./photo.jpg --json
nodoassist infer image describe --file https://example.com/photo.png --json
nodoassist infer image describe --file ./receipt.jpg --prompt "Extract the merchant, date, and total" --json
nodoassist infer image describe-many --file ./before.png --file ./after.png --prompt "Compare the screenshots and list visible UI changes" --json
nodoassist infer image describe --file ./ui-screenshot.png --model openai/gpt-5.4-mini --json
nodoassist infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --prompt "Describe the image in one sentence" --timeout-ms 300000 --json
```
...
```bash
nodoassist infer audio transcribe --file ./memo.m4a --json
nodoassist infer audio transcribe --file ./team-sync.m4a --language en --prompt "Focus on names and action items" --json
nodoassist infer audio transcribe --file ./memo.m4a --model openai/whisper-1 --json
```
...
```bash
nodoassist infer tts convert --text "hello from nodoassist" --output ./hello.mp3 --json
nodoassist infer tts convert --text "Your build is complete" --output ./build-complete.mp3 --json
nodoassist infer tts providers --json
nodoassist infer tts personas --json
nodoassist infer tts status --json
```
...
```bash
nodoassist infer video generate --prompt "cinematic sunset over the ocean" --json
nodoassist infer video generate --prompt "slow drone shot over a forest lake" --resolution 768P --duration 6 --json
nodoassist infer video describe --file ./clip.mp4 --json
nodoassist infer video describe --file ./clip.mp4 --model openai/gpt-5.4-mini --json
```
...
```bash
nodoassist infer web search --query "NodoAssist docs" --json
nodoassist infer web search --query "NodoAssist infer web providers" --json
nodoassist infer web fetch --url https://docs.openclaw.ai/cli/infer --json
nodoassist infer web providers --json
```
...
For generated media commands, `outputs` contains files written by NodoAssist. Use the `path`, `mimeType`, `size`, and any media-specific dimensions in that array for automation instead of parsing human-readable stdout.
...
```bash
# Bad
nodoassist infer audio transcribe --file ./memo.m4a --model whisper-1 --json
