---
summary: "CLI reference for `nodoassist migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into NodoAssist
...
```bash
nodoassist migrate list
nodoassist migrate claude --dry-run
nodoassist migrate codex --dry-run
nodoassist migrate codex --skill gog-vault77-google-workspace
nodoassist migrate codex --plugin google-calendar --dry-run
nodoassist migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
nodoassist migrate hermes --dry-run
nodoassist migrate hermes
nodoassist migrate apply codex --yes --skill gog-vault77-google-workspace
nodoassist migrate apply codex --yes --plugin google-calendar
nodoassist migrate apply codex --yes
nodoassist migrate apply claude --yes
nodoassist migrate apply hermes --yes
nodoassist migrate apply hermes --include-secrets --yes
nodoassist onboard --flow import
nodoassist onboard --import-from claude --import-source ~/.claude
nodoassist onboard --import-from hermes --import-source ~/.hermes
```

Running `nodoassist migrate <provider>` with no other flags plans, previews, and (in a TTY) prompts before applying. `nodoassist migrate plan <provider>` and `nodoassist migrate apply <provider>` split preview and apply into separate subcommands with the same flags.

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `nodoassist migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--backup-output <path>" type="string">
  Pre-migration backup archive path or directory. Passed through to `nodoassist backup create`.
</ParamField>
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local NodoAssist state exists.
</ParamField>
...
`nodoassist migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `nodoassist migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an NodoAssist backup before applying the migration. If no local NodoAssist state exists yet, the backup step is skipped and the migration continues. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the NodoAssist agent workspace (`AGENTS.md`).
...
- Claude command Markdown files (user `~/.claude/commands` and project `.claude/commands`) converted into NodoAssist skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, project `CLAUDE.local.md`, `.claude/rules`, user and project `agents/` directories, and project history (`projects`, `cache`, `plans` under `~/.claude`) are preserved in the migration report or reported as manual-review items. NodoAssist does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the NodoAssist Codex harness and you want to promote useful personal Codex CLI assets deliberately. Local Codex app-server launches use a per-agent `CODEX_HOME`, so they do not read your personal `~/.codex` by default. The normal process `HOME` is still inherited, so Codex can see shared `$HOME/.agents/*` skills/plugin marketplace entries and subprocesses can find user-home config and tokens.

Running `nodoassist migrate codex` in an interactive terminal previews the full plan, then opens checkbox selectors before the final apply confirmation. Skill copy items are prompted first. Use `Toggle all on` or `Toggle all off` for bulk selection. Press Space to toggle rows, or Enter to activate the highlighted row and continue. Planned skills start checked, conflict skills start unchecked, and `Skip for now` skips skill copies for this run while still continuing to plugin selection. When source-installed curated Codex plugins are migratable and `--plugin` was not supplied, migration then prompts for native Codex plugin activation by plugin name. Plugin items start checked unless the target NodoAssist Codex plugin config already has that plugin. Existing target plugins start unchecked and show a conflict hint such as `conflict: plugin exists`; choose `Toggle all off` to migrate no native Codex plugins in that run, or `Skip for now` to stop before applying.

For scripted or exact runs, select one or more skills or plugins explicitly:

```bash
nodoassist migrate codex --dry-run --skill gog-vault77-google-workspace
nodoassist migrate apply codex --yes --skill gog-vault77-google-workspace
nodoassist migrate codex --dry-run --plugin google-calendar
nodoassist migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current NodoAssist agent workspace for per-agent ownership.
...
- `SOUL.md` and `AGENTS.md` into the NodoAssist agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for NodoAssist file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that NodoAssist cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live NodoAssist config or credentials. This preserves opaque or unsafe state without pretending NodoAssist can execute or trust it automatically: `plugins/`, `sessions/`, `logs/`, `cron/`, `mcp-tokens/`, `state.db`.
...
Migration sources are plugins. A plugin declares its provider ids in `nodoassist.plugin.json`:
...
Provider plugins can use `nodoassist/plugin-sdk/migration` for item construction and summary counts, plus `nodoassist/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `nodoassist onboard --flow import` and `nodoassist setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh NodoAssist setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move NodoAssist to a new machine.
