---
summary: "CLI reference for `nodoassist promos` (list and claim promotional model offers)"
read_when:
...
```bash
nodoassist promos list
nodoassist promos claim <slug>
nodoassist promos claim <slug> --api-key <key> --set-default
```
...
2. Validates the promotion's provider, auth choice, and declared plugin packages
   against your installed NodoAssist version. Unknown ids or package mismatches are
...
3. Reuses your existing provider credentials when you have them. Otherwise it
   ...
   prompts, matching the `nodoassist onboard` non-interactive flags; to keep the
...
`nodoassist models set <model>`.

## Passive discovery in `models list`

`nodoassist models list` also surfaces promotions without you asking ClawHub
directly:
...
- The first time a new offer is seen, a one-time notice points at
  `nodoassist promos list`. Offers you have already listed or claimed are never
  announced again.
