---
summary: "CLI reference for `nodoassist logs` (tail gateway logs via RPC)"
read_when:
...
nodoassist logs --follow --interval 2000
nodoassist logs --limit 500 --max-bytes 500000
nodoassist logs --json
nodoassist logs --plain
nodoassist logs --no-color
nodoassist logs --utc
nodoassist logs --follow --local-time
nodoassist logs --url ws://127.0.0.1:18789 --token "$NODOASSIST_GATEWAY_TOKEN"
```

## Fallback and recovery behavior

- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `nodoassist logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets never use this fallback.
