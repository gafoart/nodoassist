---
summary: "CLI reference for `nodoassist config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Non-interactive helpers for `nodoassist.json`: get/set/patch/unset a value by path, print the schema, validate, or print the active file path. Run `nodoassist config` with no subcommand to open the same guided wizard as `nodoassist configure`.

<Note>
When `NODOASSIST_NIX_MODE=1`, NodoAssist treats `nodoassist.json` as immutable. Read-only commands (`config get`, `config file`, `config schema`, `config validate`) still work; config writers refuse. Edit the Nix source for the install instead; for the first-party nix-nodoassist distribution, use the [nix-nodoassist Quick Start](https://github.com/openclaw/nix-openclaw#quick-start) and set values under `programs.nodoassist.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `nodoassist config` without a subcommand.
</ParamField>
...
nodoassist config --section model
nodoassist config --section gateway --section daemon
nodoassist config schema
nodoassist config get browser.executablePath
...
nodoassist config set agents.defaults.heartbeat.every "2h"
nodoassist config set 'agents.list[0].tools.exec.node' "node-id-or-name"
nodoassist config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
nodoassist config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
nodoassist config set secrets.providers.vaultfile --provider-source file --provider-path /etc/nodoassist/secrets.json --provider-mode json
nodoassist config patch --file ./nodoassist.patch.json5 --dry-run
nodoassist config unset plugins.entries.brave.config.webSearch.apiKey
nodoassist config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
nodoassist config validate
nodoassist config validate --json
```
...
```bash
nodoassist config get agents.defaults.workspace
nodoassist config get 'agents.list[0].id'
nodoassist config get agents.list
nodoassist config set 'agents.list[1].tools.exec.node' "node-id-or-name"
```
...
Prints the active config file path, resolved from `NODOASSIST_CONFIG_PATH` or the default location. The path names a regular file, not a symlink; see [Write safety](#write-safety).

### `config schema`

Prints the generated JSON schema for `nodoassist.json` to stdout.
...
```bash
nodoassist config schema
nodoassist config schema > nodoassist.schema.json
```
...
<Note>
If validation is already failing, start with `nodoassist configure` or `nodoassist doctor --fix`. `nodoassist chat` does not bypass the invalid-config guard.
</Note>
...
```bash
nodoassist config set agents.defaults.heartbeat.every "0m"
nodoassist config set gateway.port 19001 --strict-json
nodoassist config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
nodoassist config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
nodoassist config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
<Tabs>
  <Tab title="Value mode">
    ```bash
    nodoassist config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    nodoassist config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    nodoassist config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/nodoassist-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ```bash
    nodoassist config set --batch-json '[
      {
    ...
    ```bash
    nodoassist config set --batch-file ./config-set.batch.json --dry-run
    ```
...
nodoassist config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/nodoassist/secrets.json","mode":"json"}' \
  --strict-json
```
...
```bash
nodoassist config patch --file ./nodoassist.patch.json5 --dry-run
nodoassist config patch --file ./nodoassist.patch.json5
```

Pipe a patch over stdin for remote setup scripts:

```bash
ssh user@gateway-host 'nodoassist config patch --stdin --dry-run' < ./nodoassist.patch.json5
ssh user@gateway-host 'nodoassist config patch --stdin' < ./nodoassist.patch.json5
```
...
```bash
nodoassist config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
`--dry-run` validates changes without writing `nodoassist.json`. Available on `config set`, `config patch`, and `config unset`.
...
`nodoassist config set` and other NodoAssist-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `nodoassist.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `nodoassist.json` layouts are unsupported for writes; use `NODOASSIST_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
nodoassist config set gateway.reload.mode hybrid --dry-run
nodoassist config set gateway.reload.mode hybrid
nodoassist config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `nodoassist.json`. Run `nodoassist doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
After `nodoassist config validate` passes, use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `nodoassist config set` or `nodoassist configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `nodoassist config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `nodoassist doctor` or `nodoassist doctor --fix` for migration and repair help.
  </Step>
</Steps>
