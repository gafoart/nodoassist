---
summary: "CLI reference for `nodoassist tasks` (background task ledger and Task Flow state)"
read_when:
  - You want to inspect, audit, or cancel background task records
  - You are documenting Task Flow commands under `nodoassist tasks flow`
title: "`nodoassist tasks`"
---

Inspect durable background tasks and Task Flow state. With no subcommand,
`nodoassist tasks` is equivalent to `nodoassist tasks list`.
...
```bash
nodoassist tasks list [--runtime <name>] [--status <name>] [--json]
```
...
```bash
nodoassist tasks flow list [--status <name>] [--json]
