---
summary: "CLI reference for `nodoassist devices` (device pairing + token rotation/revocation)"
read_when:
...
### `nodoassist devices approve [requestId] [--latest]`
...
```bash
nodoassist devices approve
nodoassist devices approve <requestId>
nodoassist devices approve --latest
```

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), NodoAssist supersedes the previous pending entry with a new `requestId`. Run `nodoassist devices list` right before approval to get the current id.
</Note>

Approval behavior:

- If the device is already paired and requests broader scopes or role, NodoAssist keeps the existing approval and creates a new pending upgrade request. Compare `Requested` vs `Approved` in `nodoassist devices list`, or preview with `--latest`, before approving.
...
### `nodoassist devices remove <deviceId>`

Remove one paired device entry.

```bash
nodoassist devices remove <deviceId>
nodoassist devices remove <deviceId> --json
```
...
### `nodoassist devices clear --yes [--pending]`

Clear paired devices in bulk. Gated by `--yes`.

```bash
nodoassist devices clear --yes
nodoassist devices clear --yes --pending
nodoassist devices clear --yes --pending --json
```

`--pending` also rejects all pending pairing requests.

### `nodoassist devices rotate --device <id> --role <role> [--scope <scope...>]`

Rotate a device token for a role, optionally updating its scopes.

```bash
nodoassist devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `nodoassist devices revoke --device <id> --role <role>`

Revoke a device token for a role.

```bash
nodoassist devices revoke --device <deviceId> --role node
```
...
3. Rotate the operator token for the affected device:
   ...
   ```bash
   nodoassist devices rotate --device <deviceId> --role operator
   ```
...
## Paperclip / `nodoassist_gateway` first-run approval

Paperclip agents connecting through the `nodoassist_gateway` adapter go through the same first-run device pairing approval as any other new client. If Paperclip reports `nodoassist_gateway_pairing_required`, approve the pending device and retry.

```bash
nodoassist devices approve --latest
```

The preview prints the exact `nodoassist devices approve <requestId>` command; verify the details, then rerun that command with the request ID to approve it. For a remote gateway or explicit credentials, pass the same options while previewing and approving:

```bash
nodoassist devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `nodoassist devices list` first to confirm a pending request exists.
