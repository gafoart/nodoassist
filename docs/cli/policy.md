---
summary: "CLI reference for `nodoassist policy` conformance checks"
read_when:
  - You want to check NodoAssist settings against an authored policy.jsonc
...
`nodoassist policy` is provided by the bundled Policy plugin. It is an enterprise
conformance layer over existing NodoAssist settings, not a second configuration
system. You author requirements in `policy.jsonc`; NodoAssist observes the active
...
```bash
nodoassist plugins enable policy
```
...
observed state is existing NodoAssist config or workspace metadata.
...
If an `agentIds` entry is not present in `agents.list[]`, NodoAssist evaluates
...
| `gateway.nodes.denyCommands`            | `gateway.nodes.denyCommands`                   | Require exact node command ids such as `system.run` to be denied in NodoAssist config. |
...
denied by NodoAssist config. A deployment that intentionally allows a privileged
...
`~/.nodoassist/exec-approvals.json` by default, or
`$NODOASSIST_STATE_DIR/exec-approvals.json` when `NODOASSIST_STATE_DIR` is set.
...
```bash
nodoassist policy check
nodoassist policy check --json
nodoassist policy check --severity-min error
```
...
`nodoassist doctor --lint` when the Policy plugin is enabled.

Compare an operator policy file against an authored baseline:

```bash
nodoassist policy compare --baseline official.policy.jsonc
nodoassist policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```
...
```json
{
  ...
  "evidence": {
    "channels": [
      {
        "id": "telegram",
        "provider": "telegram",
        "source": "oc://nodoassist.config/channels/telegram",
        "enabled": false
      }
    ],
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://nodoassist.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://nodoassist.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.5",
        "source": "oc://nodoassist.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://nodoassist.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://nodoassist.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://nodoassist.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://nodoassist.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://nodoassist.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed NodoAssist state used by the checks, and
...
`nodoassist policy watch` re-runs the check and reports when current evidence no
longer matches `expectedAttestationHash`:

```bash
nodoassist policy watch --json
```
...
| `policy/gateway-node-command-denied`                     | A node command denied by policy is not denied by NodoAssist config.               |
...
```json
{
  ...
  "target": "oc://nodoassist.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://nodoassist.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://nodoassist.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://nodoassist.config/gateway/bind",
...
```json
{
  ...
  "severity": "error",
  "message": "Gateway node command 'system.run' is denied by policy but not denied by NodoAssist config.",
  "source": "policy",
  "path": "nodoassist config",
  "ocPath": "oc://nodoassist.config/gateway/nodes/denyCommands",
  "target": "oc://nodoassist.config/gateway/nodes/denyCommands",
...
```json
{
  ...
  "target": "oc://nodoassist.config/agents/defaults/sandbox/workspaceAccess",
