---
summary: "CLI reference for `nodoassist update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `nodoassist update` output or options
...
Update NodoAssist and switch between stable/extended-stable/beta/dev channels.
...
nodoassist update repair
nodoassist update wizard
nodoassist update --channel extended-stable
nodoassist update --channel beta
nodoassist update --channel dev
nodoassist update --tag beta
nodoassist update --tag main
nodoassist update --dry-run
nodoassist update --no-restart
nodoassist update --yes
nodoassist update --acknowledge-clawhub-risk
nodoassist update --json
nodoassist --update
```

`nodoassist --update` rewrites to `nodoassist update` (useful for shells and
launcher scripts).
...
| `--acknowledge-clawhub-risk`                     | Allow post-update plugin sync to continue past community ClawHub trust warnings without an interactive prompt. Without it, risky community releases are skipped and left unchanged when NodoAssist cannot prompt. Official ClawHub packages and bundled plugin sources bypass this prompt.                                                       |

There is no `--verbose` flag. Use `--dry-run` to preview planned actions,
`--json` for machine-readable results, and `nodoassist update status --json`
...
<Note>
In Nix mode (`NODOASSIST_NIX_MODE=1`), mutating `nodoassist update` runs are disabled. Update the Nix source or flake input for this install instead; for nix-nodoassist, use the agent-first [Quick Start](https://github.com/openclaw/nix-openclaw#quick-start). `nodoassist update status` and `nodoassist update --dry-run` remain read-only.
</Note>
...
nodoassist update status --timeout 10
```
...
`nodoassist update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair did not
converge.

```bash
nodoassist update repair
nodoassist update repair --channel beta
nodoassist update repair --acknowledge-clawhub-risk
nodoassist update repair --json
```
...
| `--acknowledge-clawhub-risk`                     | Same behavior as on `nodoassist update`.                                                                                                                                                                                                                            |
| `--no-restart`                                   | Accepted for parity; repair never restarts the Gateway.                                                                                                                                                                                                             |

`update repair` runs `nodoassist doctor --fix`, reloads the repaired config and
...
- `dev` -> ensures a git checkout (default `~/nodoassist`, or
  `$NODOASSIST_HOME/nodoassist` when `NODOASSIST_HOME` is set; override with
  `NODOASSIST_GIT_DIR`), updates it, and installs the global CLI from that
  checkout.
...
detached helper and exits, and that helper runs `nodoassist update --yes --json`
...
healthy. If the plist is installed but launchd is not supervising it, NodoAssist
...
`Gateway: restart failed: ...` with a manual `nodoassist gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `nodoassist update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"`: NodoAssist could not find a supervising
  service boundary and durable service identity for a safe handoff (for
  example, systemd handoff requires the `NODOASSIST_SYSTEMD_UNIT` unit identity,
...
`nodoassist status` and `nodoassist status --all` show an `Update restart` row
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `NODOASSIST_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `nodoassist doctor` runs as the final safe-update check.
  </Step>
...
beta release, NodoAssist falls back to the recorded default/latest spec and
reports a warning. For npm plugins, NodoAssist also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `nodoassist update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (for example an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `nodoassist update repair` and `nodoassist plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `nodoassist update repair`.

After the per-plugin sync step, `nodoassist update` runs a mandatory **post-core convergence** pass before the gateway restarts: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly declared `main` exists). Failures from this pass, and an invalid config snapshot, return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `nodoassist update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `nodoassist update repair` and `nodoassist plugins inspect <id> --runtime --json`. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here (mirroring the `skipDisabledPlugins` policy used by the missing-payload check), so a stale disabled plugin record cannot block an otherwise valid update.
...
version. For default/`latest` intent, NodoAssist does not query plugin
...
For package-manager installs, `nodoassist update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: NodoAssist installs the new package into a temporary npm prefix,
...
plugin records aligned with the installed NodoAssist build, while leaving full
plugin-command completion rebuilds to explicit
`nodoassist completion --write-state` runs.

## Related

- `nodoassist doctor` (offers to run update first on git checkouts)
