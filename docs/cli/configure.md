---
summary: "CLI reference for `nodoassist configure` (interactive configuration prompts)"
read_when:
...
Use `nodoassist onboard` or `nodoassist setup` for the full guided first-run journey, `nodoassist setup --baseline` for the baseline config/workspace only, and `nodoassist channels add` when you only need channel account setup.

<Tip>
`nodoassist config` with no subcommand opens the same wizard. Use `nodoassist config get|set|unset` for non-interactive edits.
</Tip>
...
nodoassist configure --section model --section channels
nodoassist configure --section gateway --section daemon
```
...
<Note>
`nodoassist configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `nodoassist config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `nodoassist models auth login --provider <id> --set-default` or `nodoassist models set <model>` to intentionally change the default model.
</Note>
...
`nodoassist configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
