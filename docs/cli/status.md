---
summary: "CLI reference for `nodoassist status` (diagnostics, probes, usage snapshots)"
read_when:
  - You want a quick diagnosis of channel health + recent session recipients
  - You want a pasteable "all" status for debugging
title: "nodoassist status"
---
...
Plain `nodoassist status` stays on the fast read-only path and marks memory as
...
`nodoassist status --all`, `nodoassist status --deep`, `nodoassist security audit`,
and `nodoassist memory status --deep`.
...
- Session status output separates `Execution:` from `Runtime:`. `Execution`
  ...
  whether the session is using `NodoAssist Default`, `OpenAI Codex`, a CLI
...
- MiniMax's raw `usage_percent` / `usagePercent` fields are remaining quota,
  so NodoAssist inverts them before display; count-based fields win when
...
- Update info surfaces in the Overview; if an update is available, status
  prints a hint to run `nodoassist update` (see [Updating](/install/updating)).
