---
summary: "CLI reference for `nodoassist commitments` (inspect and dismiss inferred follow-ups)"
read_when:
...
With no subcommand, `nodoassist commitments` lists pending commitments.

## Usage

```bash
nodoassist commitments [--all] [--agent <id>] [--status <status>] [--json]
nodoassist commitments list [--all] [--agent <id>] [--status <status>] [--json]
nodoassist commitments dismiss <id...> [--json]
```
...
```bash
nodoassist commitments dismiss cm_abc123 cm_def456
```

Export as JSON:

```bash
nodoassist commitments --all --json
```
