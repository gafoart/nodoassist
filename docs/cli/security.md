---
summary: "CLI reference for `nodoassist security` (audit and fix common security footguns)"
read_when:
...
nodoassist security audit --deep --password <password>
nodoassist security audit --deep --token <token>
nodoassist security audit --auth password --password <password>
nodoassist security audit --fix
...
Run `nodoassist doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new token.
...
- Warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `nodoassist.browserConfigEpoch`) and recommends `nodoassist sandbox recreate --browser --all`.
...
```bash
nodoassist security audit --json | jq '.summary'
nodoassist security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

With `--fix --json`, output includes both fix actions and the final report:

```bash
nodoassist security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- also tightens config include files referenced from `nodoassist.json`
