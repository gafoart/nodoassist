---
summary: "CLI reference for `nodoassist workboard` cards, dispatch, and worker runs"
read_when:
...
`nodoassist workboard` is the terminal surface for the bundled [Workboard plugin](/plugins/workboard). It lets an operator list cards, create a card, inspect one card, and ask the running Gateway to dispatch ready work into subagent worker runs.

Enable the plugin before using the command:

```bash
nodoassist plugins enable workboard
...
```bash
nodoassist workboard list [--board <id>] [--status <status>] [--include-archived] [--json]
nodoassist workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
nodoassist workboard show <id> [--json]
nodoassist workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
nodoassist workboard list
nodoassist workboard list --board default --status ready
nodoassist workboard list --json
```
...
```bash
nodoassist workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
nodoassist workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
nodoassist workboard show 7f4a2c10
nodoassist workboard show 7f4a2c10 --json
```
...
```bash
nodoassist workboard dispatch
nodoassist workboard dispatch --json
nodoassist workboard dispatch --url http://127.0.0.1:18789 --token "$NODOASSIST_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local NodoAssist state directory used by the current profile. Use `--dev` or `--profile <name>` on the top-level `nodoassist` command when you need a different state root.
...
```bash
nodoassist plugins inspect workboard --runtime --json
```
...
Then retry `nodoassist workboard dispatch`. Data-only fallback is useful for local state cleanup, but worker runs need a live Gateway.
...
```bash
nodoassist workboard list --status ready
```
