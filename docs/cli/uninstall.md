---
summary: "CLI reference for `nodoassist uninstall` (remove gateway service + local data)"
read_when:
...
nodoassist uninstall
nodoassist uninstall --service --yes --non-interactive
nodoassist uninstall --state --workspace --yes --non-interactive
nodoassist uninstall --all --yes
nodoassist uninstall --dry-run
```

## Notes

- Run `nodoassist backup create` first for a restorable snapshot before removing
  state or workspaces.
