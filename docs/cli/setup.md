---
summary: "CLI reference for `nodoassist setup` (alias for onboarding, with baseline setup available by flag)"
read_when:
...
`nodoassist setup` runs the same guided onboarding flow as `nodoassist onboard`
...
`setup` accepts the same onboarding flags as `nodoassist onboard`, including
...
non-interactive examples; `nodoassist onboard --modern` (the Crestodian
conversational assistant) has no `setup` equivalent.

<Note>
`nodoassist setup` is for mutable config installs. In Nix mode (`NODOASSIST_NIX_MODE=1`) NodoAssist refuses setup writes because the config file is managed by Nix. Use the first-party [nix-nodoassist Quick Start](https://github.com/openclaw/nix-openclaw#quick-start) or the equivalent source config for another Nix package.
</Note>
...
| Flag                       | Description                                                                                           |
| -------------------------- | ----------------------------------------------------------------------------------------------------- |
| `--workspace <dir>`        | Agent workspace directory (default `~/.nodoassist/workspace`; stored as `agents.defaults.workspace`). |
...
`nodoassist setup --baseline` preserves the older baseline-only behavior: it
...
nodoassist setup --workspace ~/.nodoassist/workspace
nodoassist setup --import-from hermes --import-source ~/.hermes
nodoassist setup --non-interactive --accept-risk --mode remote --remote-url wss://gateway-host:18789 --remote-token <token>
```

## Notes

- After baseline setup, run `nodoassist setup` or `nodoassist onboard` for the full guided journey, `nodoassist configure` for targeted changes, or `nodoassist channels add` to add channel accounts.
