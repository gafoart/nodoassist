---
summary: "CLI reference for `nodoassist message` (send + channel actions)"
read_when:
...
```bash
nodoassist message <subcommand> [flags]
```
...
`nodoassist message` resolves channel SecretRefs before running the action,
...
```bash
nodoassist message send --channel discord \
...
```bash
nodoassist message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
nodoassist message send --channel telegram --target 123456789 --message "Open app:" \
  --presentation '{"blocks":[{"type":"buttons","buttons":[{"label":"Launch","webApp":{"url":"https://example.com/app"}}]}]}'
```

```bash
nodoassist message send --channel telegram --target @mychat \
...
```bash
nodoassist message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```bash
nodoassist message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```bash
nodoassist message poll --channel msteams \
  --target conversation:19:abc@thread.tacv2 \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi
```
...
```bash
nodoassist message broadcast --targets <target...> [--channel all] [--message <text>] [--media <url>] [--dry-run]
```
