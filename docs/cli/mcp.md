---
summary: "Expose NodoAssist channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to NodoAssist-backed channels
  - Running `nodoassist mcp serve`
  - Managing NodoAssist-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`nodoassist mcp` has two jobs:

- run NodoAssist as an MCP server with `nodoassist mcp serve`
- manage NodoAssist-managed outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

`serve` is NodoAssist acting as an MCP server. The other subcommands are NodoAssist acting as an MCP client-side registry for servers its own runtimes may consume later.

<Note>
  `list`, `show`, `set`, and `unset` only read and write NodoAssist-managed `mcp.servers` entries in NodoAssist config. They do not include mcporter servers from `config/mcporter.json`; use `mcporter list` for that registry.
</Note>

Use [`nodoassist acp`](/cli/acp) when NodoAssist should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

| Goal                                                                  | Use                                                                    | Why                                                                                                             |
| --------------------------------------------------------------------- | ---------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send NodoAssist channel conversations | `nodoassist mcp serve`                                                 | NodoAssist is the MCP server and exposes Gateway-backed conversations over stdio.                               |
| Save third-party MCP servers for NodoAssist-managed agent runs        | `nodoassist mcp add`, `set`, `configure`, `tools`, `login`             | NodoAssist is the MCP client-side registry and later projects those servers into eligible runtimes.             |
| Check a saved server without running an agent turn                    | `nodoassist mcp status`, `doctor`, `probe`                             | `status` and `doctor` inspect config; `probe` opens a live MCP connection and lists capabilities.               |
...
| Run ACP-hosted harness sessions                                       | [`nodoassist acp`](/cli/acp) and [ACP Agents](/tools/acp-agents-setup) | ACP bridge mode does not accept per-session MCP server injection; configure gateway/plugin bridges instead.     |

<Tip>
If you are not sure which path you need, start with `nodoassist mcp status --verbose`. It shows what NodoAssist has saved without starting any MCP servers.
</Tip>

## NodoAssist as an MCP server

This is the `nodoassist mcp serve` path.

### When to use serve

Use `nodoassist mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to NodoAssist-backed channel conversations
- you already have a local or remote NodoAssist Gateway with routed sessions
- you want one MCP server that works across NodoAssist's channel backends instead of running separate per-channel bridges

Use [`nodoassist acp`](/cli/acp) instead when NodoAssist should host the coding runtime itself and keep the agent session inside NodoAssist.

### How it works

`nodoassist mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote NodoAssist Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `nodoassist mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the NodoAssist Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `nodoassist agent` and `nodoassist infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by NodoAssist (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when NodoAssist already has session state with a known route such as:
...
<Tabs>
  <Tab title="Local Gateway">
    ```bash
    nodoassist mcp serve
    ```
  </Tab>
  <Tab title="Remote Gateway (token)">
    ```bash
    nodoassist mcp serve --url wss://gateway-host:18789 --token-file ~/.nodoassist/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    nodoassist mcp serve --url wss://gateway-host:18789 --password-file ~/.nodoassist/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    nodoassist mcp serve --verbose
    nodoassist mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the NodoAssist equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying NodoAssist channel configuration
...
NodoAssist ships a deterministic Docker smoke for this bridge:

```bash
pnpm test:docker:mcp-channels
```

That smoke runs a single container: it seeds conversation state, starts the Gateway, then spawns `nodoassist mcp serve` as a stdio child process and drives it as an MCP client. It verifies conversation discovery, transcript reads, attachment metadata reads, live event queue behavior, and Claude-style channel and permission notifications over the real stdio MCP bridge. Outbound send routing (`messages_send` reusing the stored conversation route) is covered separately by unit tests in `src/mcp/channel-server.test.ts`.
...
## NodoAssist as an MCP client registry

This is the `nodoassist mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose NodoAssist over MCP. They manage NodoAssist-managed MCP server definitions under `mcp.servers` in NodoAssist config. They do not read mcporter servers from `config/mcporter.json`.

Those saved definitions are for runtimes that NodoAssist launches or configures later, such as embedded NodoAssist and other runtime adapters. NodoAssist stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write NodoAssist config
    ...
    - embedded NodoAssist exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become NodoAssist tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded NodoAssist consumes NodoAssist `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
NodoAssist projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific NodoAssist
...
NodoAssist strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.
...
- `nodoassist mcp list`
- `nodoassist mcp show [name]`
- `nodoassist mcp status [--verbose]`
- `nodoassist mcp doctor [name] [--probe]`
- `nodoassist mcp probe [name]`
- `nodoassist mcp add <name> [flags]`
- `nodoassist mcp set <name> <json>`
- `nodoassist mcp configure <name> [flags]`
- `nodoassist mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `nodoassist mcp login <name> [--code code]`
- `nodoassist mcp logout <name>`
- `nodoassist mcp reload`
- `nodoassist mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `nodoassist mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
nodoassist mcp list
nodoassist mcp show context7 --json
nodoassist mcp status --verbose
nodoassist mcp doctor --probe
nodoassist mcp probe context7 --json
nodoassist mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
nodoassist mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
nodoassist mcp tools context7 --include 'resolve-library-id,get-library-docs'
nodoassist mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
nodoassist mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
nodoassist mcp configure docs --auth oauth --oauth-scope 'docs.read'
nodoassist mcp login docs
nodoassist mcp logout docs
nodoassist mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `nodoassist mcp doctor --probe` afterward to prove that the server starts and exposes tools.

<Tabs>
  <Tab title="Filesystem">
    ```bash
    nodoassist mcp add files \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-filesystem \
      --arg "$HOME/Documents" \
      --include 'read_file,list_directory,search_files'
    nodoassist mcp doctor files --probe
    ```
  ...
  <Tab title="Memory">
    ```bash
    nodoassist mcp add memory \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-memory
    nodoassist mcp probe memory --json
    ```
  ...
  <Tab title="Local script">
    ```bash
    nodoassist mcp add local-tools \
      --command node \
      --arg ./dist/mcp-server.js \
      --cwd /srv/nodoassist-tools \
  ...
  <Tab title="Remote HTTP">
    ...
    nodoassist mcp doctor docs --probe
    ```
  ...
  <Tab title="Desktop/CUA">
    ...
    nodoassist mcp tools cua-driver --include 'list_apps,observe,click,type'
    nodoassist mcp doctor cua-driver --probe
    ```
...
<AccordionGroup>
  <Accordion title="status --json">
    ```json
    {
      "path": "/home/user/.nodoassist/nodoassist.json",
      "servers": [
        {
          "name": "docs",
          "configured": true,
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "warning",
              "message": "OAuth credentials are not authorized; run nodoassist mcp login docs"
            }
          ]
        }
      ]
...
| `auth: "oauth"`                | Use MCP OAuth token storage and `nodoassist mcp login`           |
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `nodoassist mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    nodoassist mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```
  ...
  <Step title="Start login">
    ...
    NodoAssist prints the authorization URL and stores temporary OAuth verifier state under the NodoAssist state directory.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to NodoAssist.

    ```bash
    nodoassist mcp login docs --code abc123
    ```
...
If the provider rotates tokens or the authorization state gets stuck, run `nodoassist mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
| ------------------------------ | ---------------------------------------------------------------------------------------- |
| `url`                          | HTTP or HTTPS URL of the remote server (required)                                        |
| `transport`                    | Set to `"streamable-http"` to select this transport; when omitted, NodoAssist uses `sse` |
| `headers`                      | Optional key-value map of HTTP headers (for example auth tokens)                         |
| `connectionTimeoutMs`          | Per-server connection timeout in ms (optional)                                           |
| `connectTimeout`               | Per-server connection timeout in seconds (optional)                                      |
| `timeout` / `requestTimeoutMs` | Per-server MCP request timeout in seconds or ms                                          |
...
NodoAssist config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `nodoassist mcp set` and repaired by `nodoassist doctor --fix` in existing config, but `transport` is what embedded NodoAssist consumes directly.
...
Use the page for operator edits and quick inventory. Use `nodoassist mcp doctor --probe` or `nodoassist mcp probe` when you need live server proof.
...
7. Run `nodoassist mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `nodoassist mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients
