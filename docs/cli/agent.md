---
summary: "CLI reference for `nodoassist agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
nodoassist agent --to +15555550123 --message "status update" --deliver
nodoassist agent --agent ops --message "Summarize logs"
nodoassist agent --agent ops --message-file ./task.md
nodoassist agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
nodoassist agent --session-key agent:ops:incident-42 --message "Summarize status"
nodoassist agent --agent ops --session-key incident-42 --message "Summarize status"
nodoassist agent --session-id 1234 --message "Summarize inbox" --thinking medium
nodoassist agent --to +15555550123 --message "Trace logs" --verbose on --json
nodoassist agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
nodoassist agent --agent ops --message "Run locally" --local
```
...
- Slash commands (for example `/compact`) cannot run through `--message`. The CLI rejects them and points you at the first-class command instead (`nodoassist sessions compact <key>` for compaction).
