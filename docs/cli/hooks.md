---
summary: "CLI reference for `nodoassist hooks` (agent hooks)"
read_when:
...
Manage agent hooks (event-driven automations for commands like `/new`, `/reset`, and gateway startup). Bare `nodoassist hooks` is equivalent to `nodoassist hooks list`.
...
```bash
nodoassist hooks list [--eligible] [--json] [-v|--verbose]
```
...
```bash
nodoassist hooks info <name> [--json]
```
...
```bash
nodoassist plugins install <package>        # npm by default
nodoassist plugins install npm:<package>    # npm only
nodoassist plugins install <package> --pin  # pin resolved version
nodoassist plugins install <path>           # local directory or archive
nodoassist plugins install -l <path>        # link a local directory instead of copying

nodoassist plugins update <id>
nodoassist plugins update --all
nodoassist plugins update --dry-run
```

Hook packs install through the unified plugins installer/updater; `nodoassist hooks install` / `nodoassist hooks update` still work as deprecated aliases that print a warning and forward to the `plugins` commands.
...
- Bare specs and `@latest` stay on the stable track; if npm resolves to a prerelease, NodoAssist stops and asks you to opt in explicitly (`@beta`, `@rc`, or an exact prerelease version).
...
- Install copies the pack into `~/.nodoassist/hooks/<id>`, enables its hooks under `hooks.internal.entries.*`, and records the install under `hooks.internal.installs`.
- If a stored integrity hash no longer matches the fetched artifact, NodoAssist warns and prompts before continuing; pass global `--yes` to bypass the prompt (for example in CI).
...
| command-logger        | `command`                                         | Logs command events to `~/.nodoassist/logs/commands.log`                                           |
...
Enable any bundled hook with `nodoassist hooks enable <hook-name>`. Full details, config keys, and defaults: [Bundled hooks](/automation/hooks#bundled-hooks).

### command-logger log file

```bash
tail -n 20 ~/.nodoassist/logs/commands.log        # recent commands
cat ~/.nodoassist/logs/commands.log | jq .          # pretty-print
grep '"action":"new"' ~/.nodoassist/logs/commands.log | jq .   # filter by action
```
