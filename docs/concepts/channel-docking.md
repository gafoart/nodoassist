---
summary: "Move one NodoAssist session's reply route between linked chat channels"
...
Channel docking is call forwarding for one NodoAssist session. It keeps the same
...
Alice can message NodoAssist on Telegram and Discord:
...
NodoAssist keeps the current session context and changes the reply route:
...
4. Keep the same NodoAssist session, but receive future replies in Discord.
...
NodoAssist generates one `/dock-<channel>` command for every loaded channel plugin
...
Send the dock command from a direct chat with NodoAssist, not from a group chat.
