...
`nodoassist/plugin-sdk/channel-outbound` / `channel-inbound` subpaths. For the
...
Public surface: `nodoassist/plugin-sdk/channel-outbound` (send/receipt/durable/live/reply-pipeline
helpers) and `nodoassist/plugin-sdk/channel-inbound` (inbound context, `runChannelInboundEvent`,
...
NodoAssist only advances the persisted restart watermark past updates that
...
- **No `MessageOrigin` / `shouldDropNodoAssistEcho`.** The original plan called
  for a `source: "nodoassist"` origin tag on gateway-failure messages plus a
...
- **iMessage** (`extensions/imessage/src/monitor/echo-cache.ts`,
  ...
  cache, or NodoAssist can re-ingest its own replies as inbound user messages.
...
- Whether Telegram should eventually replace the grammY (`1.43.0`) polling
  ...
  redelivery, not only NodoAssist's persisted restart watermark
