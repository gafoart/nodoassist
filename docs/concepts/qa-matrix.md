...
read_when:
  - Running pnpm nodoassist qa matrix locally
  - Adding or selecting Matrix QA scenarios
  - Triaging Matrix QA failures, timeouts, or stuck cleanup
title: "Matrix QA"
---

The Matrix QA lane runs the bundled `@nodoassist/matrix` plugin against a disposable Tuwunel homeserver in Docker, with temporary driver, SUT, and observer accounts plus seeded rooms. It is the live transport-real coverage for Matrix.

Maintainer-only tooling. Packaged NodoAssist releases omit `qa-lab`, so `nodoassist qa` only runs from a source checkout, which loads the bundled runner directly with no plugin install step.
...
```bash
pnpm nodoassist qa matrix --profile fast --fail-fast
```

Plain `pnpm nodoassist qa matrix` runs `--profile all` and does not stop on first failure. Shard the full inventory across parallel jobs with `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`.
...
2. Registers three temporary users: `driver` (sends inbound traffic), `sut` (the NodoAssist Matrix account under test), `observer` (third-party traffic capture).
...
5. Starts a child NodoAssist gateway with the real Matrix plugin scoped to the SUT account.
...
```text
pnpm nodoassist qa matrix [options]
```
...
| `e2ee-cli`      | `nodoassist matrix encryption setup` and `verify *` CLI scenarios driven through the QA harness.                                                                                                                      |

The exact mapping lives in `extensions/qa-matrix/src/runners/contract/scenario-catalog.ts`.

## Scenarios

The shared Matrix adapter exposes these canonical YAML scenarios through `nodoassist qa suite --channel-driver live --channel matrix`:
...
| ----------------------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NODOASSIST_QA_MATRIX_TIMEOUT_MS`         | `1800000` (30 min)                        | Hard upper bound on the entire run.                                                                                                                                                            |
| `NODOASSIST_QA_MATRIX_CANARY_TIMEOUT_MS`  | `45000`                                   | Bound for the initial canary reply. Release CI raises this on shared runners so a slow first gateway turn does not fail before scenario coverage starts.                                       |
| `NODOASSIST_QA_MATRIX_NO_REPLY_WINDOW_MS` | `8000`                                    | Quiet window for negative no-reply assertions. Clamped to `<=` the run timeout.                                                                                                                |
| `NODOASSIST_QA_MATRIX_CLEANUP_TIMEOUT_MS` | `90000`                                   | Bound for Docker teardown. Failure surfaces include the recovery `docker compose ... down --remove-orphans` command.                                                                           |
| `NODOASSIST_QA_MATRIX_TUWUNEL_IMAGE`      | `ghcr.io/matrix-construct/tuwunel:v1.5.1` | Override the homeserver image when validating against a different Tuwunel version.                                                                                                             |
| `NODOASSIST_QA_MATRIX_PROGRESS`           | on                                        | `0` silences `[matrix-qa] ...` progress lines on stderr. `1` forces them on.                                                                                                                   |
| `NODOASSIST_QA_MATRIX_CAPTURE_CONTENT`    | redacted                                  | `1` keeps message body and `formatted_body` in `matrix-qa-observed-events.json`. Default redacts to keep CI artifacts safe.                                                                    |
| `NODOASSIST_QA_MATRIX_DISABLE_FORCE_EXIT` | off                                       | `1` skips the deterministic `process.exit` after artifact write. The default forces exit because matrix-js-sdk's native crypto handles can keep the event loop alive past artifact completion. |
| `NODOASSIST_RUN_NODE_OUTPUT_LOG`          | unset                                     | When set by an outer launcher (e.g. `scripts/run-node.mjs`), Matrix QA reuses that log path instead of starting its own tee.                                                                   |
...
- `matrix-qa-observed-events.json`: Observed Matrix events from the driver and observer clients. Bodies are redacted unless `NODOASSIST_QA_MATRIX_CAPTURE_CONTENT=1`; approval metadata is summarized with selected safe fields and a truncated command preview.
- `matrix-qa-output.log`: Combined stdout/stderr from the run. If `NODOASSIST_RUN_NODE_OUTPUT_LOG` is set, the outer launcher's log is reused instead.

## Triage tips

- **Run hangs near the end:** `matrix-js-sdk` native crypto handles can outlive the harness. The default forces a clean `process.exit` after artifact write; if you set `NODOASSIST_QA_MATRIX_DISABLE_FORCE_EXIT=1`, expect the process to linger.
...
- **Flaky negative-assertion windows in CI:** lower `NODOASSIST_QA_MATRIX_NO_REPLY_WINDOW_MS` (default 8 s) when CI is fast; raise it on slow shared runners.
- **Need redacted bodies for a bug report:** rerun with `NODOASSIST_QA_MATRIX_CAPTURE_CONTENT=1` and attach `matrix-qa-observed-events.json`. Treat the resulting artifact as sensitive.
- **Different Tuwunel version:** point `NODOASSIST_QA_MATRIX_TUWUNEL_IMAGE` at the version under test. The lane checks in only the pinned default image.
