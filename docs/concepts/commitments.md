...
read_when:
  - You want NodoAssist to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, NodoAssist can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. NodoAssist may check in afterward.
- You say you are exhausted. NodoAssist may ask later whether you slept.
- The agent says it will follow up after something changes. NodoAssist may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: NodoAssist remembers a
...
```bash
nodoassist config set commitments.enabled true
nodoassist config set commitments.maxPerDay 3
```

Equivalent `nodoassist.json`:
...
After an agent reply, NodoAssist may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, NodoAssist stores a commitment with:
...
metadata, and due-commitment heartbeat turns run without NodoAssist tools.

NodoAssist never delivers an inferred commitment immediately after writing it.
...
```bash
nodoassist commitments
nodoassist commitments --all
nodoassist commitments --agent main
nodoassist commitments --status snoozed
nodoassist commitments dismiss cm_abc123
```

See [`nodoassist commitments`](/cli/commitments) for the full command reference.
...
Stored commitments are local NodoAssist state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
nodoassist config set commitments.enabled false
```
...
- Check `nodoassist commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`nodoassist commitments`](/cli/commitments)
