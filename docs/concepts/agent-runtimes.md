---
summary: "How NodoAssist separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between NodoAssist, Codex, ACP, or another native agent runtime
...
| ------------- | ---------------------------------------------- | --------------------------------------------------------------------- |
| Provider      | `anthropic`, `github-copilot`, `openai`        | How NodoAssist authenticates, discovers models, and names model refs. |
| Model         | `claude-opus-4-6`, `gpt-5.5`                   | The model selected for the agent turn.                                |
| Agent runtime | `claude-cli`, `codex`, `copilot`, `nodoassist` | The low-level loop or backend that executes the prepared turn.        |
| Channel       | Discord, Slack, Telegram, WhatsApp             | Where messages enter and leave NodoAssist.                            |
...
runtime keys are legacy and ignored. `nodoassist doctor --fix` removes old
...
- **Embedded harnesses** run inside NodoAssist's prepared agent loop: the
  built-in `nodoassist` runtime, plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | NodoAssist name/config               | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
makes native app-server features available; `nodoassist doctor --fix` owns
...
That means NodoAssist selects an OpenAI model ref, then asks the Codex
...
NodoAssist session store becomes Codex.
...
3. **NodoAssist explicitly chosen for an OpenAI model** -> keep the model ref as `openai/<model>` and set provider/model runtime policy to `agentRuntime.id: "nodoassist"`. A selected `openai` OAuth profile is routed internally through NodoAssist's Codex-auth transport.
4. **Legacy Codex model refs in config** -> repair with `nodoassist doctor --fix` to `openai/<model>`; doctor keeps the Codex auth route by adding provider/model-scoped `agentRuntime.id: "codex"` where the old model ref implied it. Legacy **`codex-cli/*`** model refs repair to the same `openai/<model>` Codex app-server route; NodoAssist no longer keeps a bundled Codex CLI backend.
...
| Surface                     | NodoAssist embedded                                | Codex app-server                                                              |
| --------------------------- | -------------------------------------------------- | ----------------------------------------------------------------------------- |
| Model loop owner            | NodoAssist, through the NodoAssist embedded runner | Codex app-server                                                              |
| Canonical thread state      | NodoAssist transcript                              | Codex thread, plus NodoAssist transcript mirror                               |
| NodoAssist dynamic tools    | Native NodoAssist tool loop                        | Bridged through the Codex adapter                                             |
| Native shell and file tools | NodoAssist path                                    | Codex-native tools, bridged through native hooks where supported              |
| Context engine              | Native NodoAssist context assembly                 | NodoAssist projects assembled context into the Codex turn                     |
| Compaction                  | NodoAssist or selected context engine              | Codex-native compaction, with NodoAssist notifications and mirror maintenance |
| Channel delivery            | NodoAssist                                         | NodoAssist                                                                    |

Design rule: if NodoAssist owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, NodoAssist needs runtime
events or native hooks. If the native runtime owns canonical thread state,
NodoAssist mirrors and projects context rather than rewriting unsupported
internals.

## Runtime selection

NodoAssist resolves an embedded runtime after provider and model resolution, in
this order:
...
4. If nothing claims the turn in `auto` mode, NodoAssist falls back to
   `nodoassist` as the compatibility runtime. Use an explicit runtime id when
   the run must be strict.

Whole-session and whole-agent runtime pins are ignored: `NODOASSIST_AGENT_RUNTIME`,
session `agentHarnessId`/`agentRuntimeOverride` state, `agents.defaults.agentRuntime`,
and `agents.list[].agentRuntime`. Run `nodoassist doctor --fix` to remove stale
...
never silently routed back to NodoAssist. Only `auto` may route an unmatched
turn to NodoAssist.
...
harness. Explicit NodoAssist runtime config remains an opt-in compatibility
route for `openai/*` agent turns; when paired with a selected `openai` OAuth
profile, NodoAssist routes that path internally through the Codex-auth
...
If `nodoassist doctor` warns that the `codex` plugin is enabled while legacy
Codex model refs remain in config, treat that as legacy route state and run
`nodoassist doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@nodoassist/copilot` plugin registers an opt-in `copilot` runtime
...
prefix in `extensions/copilot/doctor-contract-api.ts`, which `nodoassist doctor`
...
When a runtime is not NodoAssist, its docs should state which NodoAssist surfaces
it supports:

| Question                               | Why it matters                                                                                      |
| -------------------------------------- | --------------------------------------------------------------------------------------------------- |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                     |
| Who owns canonical thread history?     | Determines whether NodoAssist can edit history or only mirror it.                                   |
| Do NodoAssist dynamic tools work?      | Messaging, sessions, cron, and NodoAssist-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around NodoAssist-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume NodoAssist equivalence where the native runtime owns more state.            |
