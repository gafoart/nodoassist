...
title: "Mantis"
read_when:
  - Building or running live visual QA for NodoAssist bugs
...
Mantis publishes visual CI evidence and a PR comment for NodoAssist behavior.
...
- NodoAssist (`extensions/qa-lab/src/mantis/*`): scenario runtime, `pnpm nodoassist qa mantis <command>` CLI, evidence schema.
- QA Lab (`extensions/qa-lab/src/live-transports/*`): live transport harness, driver/SUT bots, report/evidence writers.
- Crabbox (`nodoassist/crabbox`): warmed Linux machines, leases, VNC, `crabbox media preview`.
...
All commands are `pnpm nodoassist qa mantis <command>`, defined in
`extensions/qa-lab/src/mantis/cli.ts`. Requires `NODOASSIST_ENABLE_PRIVATE_QA_CLI=1`
at build/run time (bundled workflows set `NODOASSIST_BUILD_PRIVATE_QA=1` and
`NODOASSIST_ENABLE_PRIVATE_QA_CLI=1` before building).
...
| `telegram-desktop-builder`      | Lease/reuse a Crabbox desktop, install Telegram Desktop, optionally configure an NodoAssist gateway.                                                      |
...
```bash
pnpm nodoassist qa mantis discord-smoke \
...
Token resolution order: `--token-file` value, then `NODOASSIST_QA_DISCORD_MANTIS_BOT_TOKEN`
(override with `--token-env`), then a file named by `NODOASSIST_QA_DISCORD_MANTIS_BOT_TOKEN_FILE`
(override with `--token-file-env`). Guild/channel ids come from
`NODOASSIST_QA_DISCORD_GUILD_ID` / `NODOASSIST_QA_DISCORD_CHANNEL_ID` (override with
`--guild-id` / `--channel-id`) and must be 17-20 digit Discord snowflakes. Set
`NODOASSIST_QA_REDACT_PUBLIC_METADATA=1` to replace bot/guild/channel/message ids
and names with `<redacted>` in the published summary and report.

### `run`

```bash
pnpm nodoassist qa mantis run \
  --transport discord \
...
`pnpm nodoassist qa discord --scenario <id> --model openai/gpt-5.4 --alt-model openai/gpt-5.4 --allow-failures`
...
```bash
pnpm nodoassist qa mantis desktop-browser-smoke \
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome profile archive from that env var before launch (default `NODOASSIST_MANTIS_BROWSER_PROFILE_TGZ_B64`); used for logged-in witnesses like Discord Web.
- `--video-duration <seconds>` controls MP4 capture length (default 10s).
- `--keep-lease` (or `NODOASSIST_MANTIS_KEEP_VM=1`) keeps a lease this run created open for VNC inspection; failed runs that created a lease also keep it by default.
...
`NODOASSIST_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also writes a
Discord Web URL artifact, and `NODOASSIST_QA_DISCORD_KEEP_THREADS=1` leaves the
...
`pnpm nodoassist qa slack` inside it, opens Slack Web in the VNC browser,
...
With `--gateway-setup`, the command creates a persistent disposable NodoAssist
home at `$HOME/.nodoassist-mantis/slack-nodoassist` in the VM, patches Slack
Socket Mode config for the target channel, starts
`nodoassist gateway run --dev --allow-unconfigured --port 38973`, and leaves
...
- `NODOASSIST_LIVE_OPENAI_KEY` for the remote model lane (if only `OPENAI_API_KEY`
  is set locally, Mantis copies it to `NODOASSIST_LIVE_OPENAI_KEY` before
  invoking Crabbox)
...
bot token into the VM as `NODOASSIST_MANTIS_SLACK_*` env vars, so GitHub
...
`NODOASSIST_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome
profile inside the VM (default `$HOME/.config/nodoassist-mantis/slack-chrome-profile`);
...
optionally restores a user-session archive, configures NodoAssist with the
leased Telegram SUT bot token, starts
`nodoassist gateway run --dev --allow-unconfigured --port 38974`, posts a
driver-bot readiness message to the leased private group, then captures a
screenshot and MP4. A bot token only configures NodoAssist; it never logs
...
`NODOASSIST_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads; it is
...
- `MANTIS_ARTIFACT_R2_BUCKET` (workflows set `nodoassist-crabbox-artifacts`)
...
| `Mantis Telegram Live`            | PR comment or manual dispatch                                                              | Runs the bot-API Telegram live QA lane (`nodoassist qa telegram`), writes `mantis-evidence.json` from the QA summary, renders redacted evidence HTML through a Crabbox desktop browser, generates a motion GIF, posts PR evidence. Telegram Web login is not required for this lane.                             |
...
| `Mantis Web UI Chat Proof`        | PR comment or manual dispatch                                                              | Runs the focused NodoAssist Control UI chat Playwright proof against the candidate, verifies the browser sends through the mocked Gateway, captures screenshot/video artifacts, and posts PR evidence. This lane is web chat proof only, not WinUI/native-app or arbitrary visual proof.                         |
...
```text
@nodoassist-mantis discord status reactions
@nodoassist-mantis discord status reactions baseline=origin/main candidate=HEAD
...
@nodoassist-mantis web ui chat
@nodoassist-mantis web-ui-chat candidate=HEAD
```
...
`NODOASSIST_MANTIS_CRABBOX_PROVIDER` / `NODOASSIST_MANTIS_CRABBOX_CLASS`. GitHub
...
noVNC, Node 22+ and pnpm, an NodoAssist checkout, and outbound access to the
target transport, GitHub, model providers, and the credential broker.

Secret names used across the Mantis workflows:

- `NODOASSIST_QA_DISCORD_MANTIS_BOT_TOKEN`
...
- `NODOASSIST_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads
- `NODOASSIST_QA_CONVEX_SITE_URL`, `NODOASSIST_QA_CONVEX_SECRET_CI`
- `CRABBOX_COORDINATOR` / `CRABBOX_COORDINATOR_TOKEN` (workflows also accept
  `NODOASSIST_QA_MANTIS_CRABBOX_COORDINATOR` / `_TOKEN` as a fallback and map
...
ref policy, candidate ref policy, NodoAssist config patch, setup/stimulus steps,
