...
| Local             | `local`             | `@nodoassist/llama-cpp-provider`    |
| Mistral           | `mistral`           |                                     |
...
NodoAssist indexes `MEMORY.md` and `memory/*.md` into chunks (400 tokens with
...
- **Reindex on demand:** `nodoassist memory index --force`
...
**Memory search disabled?** Check `nodoassist memory status`. If no provider is
...
**Stale results?** Run `nodoassist memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** NodoAssist falls back to in-process cosine
similarity automatically. `nodoassist memory status --deep` reports the local
