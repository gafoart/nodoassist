...
When a normal prompt arrives while a session run is already streaming and the queue mode is `steer` (the default, no config needed), NodoAssist tries to send that prompt into the active runtime. NodoAssist and the native Codex app-server harness implement the delivery details differently.
...
Steering does not interrupt a tool call that is already running. NodoAssist checks for queued steering messages at model boundaries:

1. The assistant asks for tool calls.
2. NodoAssist executes the current assistant message's tool-call batch.
3. NodoAssist emits the turn end event.
4. NodoAssist drains queued steering messages.
5. NodoAssist appends those messages as user messages before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of NodoAssist runtime's internal steering queue. NodoAssist batches queued prompts for the configured quiet window, then sends a single `turn/steer` request with all collected user input in arrival order.

Codex review and manual compaction turns reject same-turn steering. When a runtime cannot accept steering in `steer` mode, NodoAssist waits for the active run to finish before starting the prompt.
...
- With default behavior, the active runtime receives all four messages in arrival order before its next model decision. NodoAssist drains them at the next model boundary; Codex receives them as one batched `turn/steer`.
- With `/queue collect`, NodoAssist does not steer. It waits until the active run ends, then creates a followup turn with compatible queued messages after the debounce window.
- With `/queue interrupt`, NodoAssist aborts the active run and starts the newest message instead of steering.
