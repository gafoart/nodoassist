---
summary: "Delegate architecture: running NodoAssist as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Run NodoAssist as a **named delegate**: an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human - it sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate

A delegate is an NodoAssist agent that:
...
NodoAssist's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of NodoAssist's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.nodoassist/workspace-delegate",
  tools: {
...
- Cron run history: NodoAssist's shared SQLite state database.
- Session transcripts: `~/.nodoassist/agents/delegate/sessions`.
- Identity provider audit logs (Exchange, Google Workspace).

All delegate actions flow through NodoAssist's session store. For compliance, retain and review these logs.
...
```bash
nodoassist agents add delegate --workspace ~/.nodoassist/workspace-delegate
```

This creates:

- Workspace: `~/.nodoassist/workspace-delegate`
- Agent state: `~/.nodoassist/agents/delegate/agent`
- Sessions: `~/.nodoassist/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.nodoassist/workspace" },
      {
        id: "delegate",
        workspace: "~/.nodoassist/workspace-delegate",
        tools: {
...
~/.nodoassist/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.nodoassist/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.nodoassist/workspace-org",
        agentDir: "~/.nodoassist/agents/org-assistant/agent",
