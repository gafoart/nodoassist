...
NodoAssist gives agents tools to work across sessions, inspect status, and orchestrate sub-agents.
...
- `deliveryContext` is the persisted delivery route stored on the session, which NodoAssist can reuse for later delivery even when the active surface differs.
...
`subagents` is the visibility helper for already spawned NodoAssist sub-agents. It supports `action: "list"` to inspect active/recent runs.
