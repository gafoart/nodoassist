...
NodoAssist ships one **embedded agent runtime**: a built-in agent loop, tool
...
Recommended: use `nodoassist setup` to create `~/.nodoassist/nodoassist.json` if missing and initialize the workspace files.
...
Inside the workspace, NodoAssist expects these user-editable files:
...
On the first turn of a new session, NodoAssist injects the contents of these files into the system prompt's Project Context. `MEMORY.md` is only injected when it exists at the workspace root.

Blank files are skipped. Large files are trimmed and truncated with a marker so prompts stay lean (read the file for full content). A missing file (other than `MEMORY.md`) injects a single "missing file" marker line instead; `nodoassist setup` creates a safe default template for it.

`BOOTSTRAP.md` is only created for a **brand new workspace** (no other bootstrap files present). While it is pending, NodoAssist keeps it in Project Context and adds system-prompt bootstrap guidance for the initial ritual instead of copying it into the user message. If you delete it after completing the ritual, it is not recreated on later restarts.

After a workspace has been observed, NodoAssist also keeps a state-dir attestation marker for the workspace path. If a recently attested workspace disappears or is wiped, startup refuses to silently reseed `BOOTSTRAP.md`; restore the workspace or use a full onboard reset so the workspace and marker are cleared together.
...
NodoAssist loads skills from these locations (highest precedence first):

- Workspace: `<workspace>/skills`
- Project agent skills: `<workspace>/.agents/skills`
- Personal agent skills: `~/.agents/skills`
- Managed/local: `~/.nodoassist/skills`
...
The embedded agent runtime is NodoAssist-owned: model discovery, tool wiring,
...
- `~/.nodoassist/agents/<agentId>/sessions/<SessionId>.jsonl`

The session ID is stable and chosen by NodoAssist. NodoAssist does not read session folders from other tools.
...
- If you omit the provider, NodoAssist tries an alias first, then a unique
  ...
  configured default model, NodoAssist falls back to the first configured
