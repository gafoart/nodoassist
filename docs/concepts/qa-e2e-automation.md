...
The private QA stack exercises NodoAssist in a realistic, channel-shaped way that
a unit test cannot.
...
Every QA flow runs under `pnpm nodoassist qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files for a model-axis parity gate, or use `--runtime-axis --token-efficiency` to write Codex-vs-NodoAssist runtime parity and token-efficiency reports.                                                                        |
...
command also needs an NodoAssist root profile, put the root profile before the
QA command:

```bash
pnpm nodoassist --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm nodoassist qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`nodoassist.run`, `nodoassist.harness.run`, a latest GenAI semantic-convention
model-call span, `nodoassist.context.assembled`, and `nodoassist.message.delivery`
...
IDs and `nodoassist.content.*` attributes must stay out of the trace. The scenario
...
```bash
NODOASSIST_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
  pnpm nodoassist qa matrix --provider-mode mock-openai --profile fast --fail-fast
```
...
```bash
NODOASSIST_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
NODOASSIST_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
  pnpm nodoassist qa matrix --provider-mode live-frontier --profile fast --fail-fast
```
...
E2EE CLI profile also drives `nodoassist matrix encryption setup` and
...
credentials, `--fast`, and `NODOASSIST_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`.
...
```bash
pnpm nodoassist qa discord
pnpm nodoassist qa slack
pnpm nodoassist qa telegram
pnpm nodoassist qa whatsapp
```
...
```bash
pnpm nodoassist qa mantis slack-desktop-smoke \
  --gateway-setup \
...
NodoAssist Slack gateway running inside the VM on port `38973`; without it, the
...
```bash
pnpm nodoassist qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `nodoassist infer image
...
The doctor checks Convex broker env (`NODOASSIST_QA_CONVEX_SITE_URL`,
`NODOASSIST_QA_CONVEX_ENDPOINT_PREFIX`), validates endpoint settings, reports
only set/missing status for `NODOASSIST_QA_CONVEX_SECRET_CI` and
`NODOASSIST_QA_CONVEX_SECRET_MAINTAINER`, and verifies admin/list reachability
...
`nodoassist/plugin-sdk/qa-live-transport-scenarios`.
...
```bash
pnpm nodoassist qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds NodoAssist
...
- `NODOASSIST_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
of the `/usage` footer on tool-only replies. Use `pnpm nodoassist qa telegram
...
```bash
NODOASSIST_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `NODOASSIST_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`NODOASSIST_NPM_TELEGRAM_RTT_SAMPLES`, `NODOASSIST_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `NODOASSIST_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
controlled by the harness and a SUT bot started by the child NodoAssist gateway
...
- `NODOASSIST_QA_DISCORD_GUILD_ID`
- `NODOASSIST_QA_DISCORD_CHANNEL_ID`
- `NODOASSIST_QA_DISCORD_DRIVER_BOT_TOKEN`
- `NODOASSIST_QA_DISCORD_SUT_BOT_TOKEN`
- `NODOASSIST_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id
  returned by Discord (the lane fails fast otherwise).

Optional:

- `NODOASSIST_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in
  observed-message artifacts.
- `NODOASSIST_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for
...
- `discord-qa-observed-messages.json` - bodies redacted unless
  `NODOASSIST_QA_DISCORD_CAPTURE_CONTENT=1`.
...
- `NODOASSIST_QA_SLACK_CHANNEL_ID`
- `NODOASSIST_QA_SLACK_DRIVER_BOT_TOKEN`
- `NODOASSIST_QA_SLACK_SUT_BOT_TOKEN`
- `NODOASSIST_QA_SLACK_SUT_APP_TOKEN`

Optional:

- `NODOASSIST_QA_SLACK_CAPTURE_CONTENT=1` keeps message bodies in
  observed-message artifacts.
- `NODOASSIST_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `NODOASSIST_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-codex-approval-exec-native` - opt-in Codex Guardian command approval
  ...
  `nodoassist-codex-app-server`, resolves it, and verifies the Codex turn
...
- `slack-qa-observed-messages.json` - bodies redacted unless
  `NODOASSIST_QA_SLACK_CAPTURE_CONTENT=1`.
- `approval-checkpoints/` - only when Mantis sets
  `NODOASSIST_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "NodoAssist QA Driver",
    "description": "Test driver bot for NodoAssist QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "NodoAssist QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "NodoAssist QA SUT",
    "description": "NodoAssist QA SUT connector for NodoAssist"
  },
  "features": {
    "bot_user": {
      "display_name": "NodoAssist QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#nodoassist-qa`) and invite both
bots from inside the channel:

```text
/invite @NodoAssist QA Driver
/invite @NodoAssist QA SUT
```
...
`NODOASSIST_QA_SLACK_*` variables and pass `--credential-source env`), or seed
...
With `NODOASSIST_QA_CONVEX_SITE_URL` and `NODOASSIST_QA_CONVEX_SECRET_MAINTAINER`
exported in your shell, register and verify:

```bash
pnpm nodoassist qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm nodoassist qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child NodoAssist gateway through
...
- `NODOASSIST_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
...
- `NODOASSIST_QA_WHATSAPP_CAPTURE_CONTENT=1` keeps message bodies in
  observed-message artifacts.
...
`@nodoassist/whatsapp/api.js` package surface instead of reaching into private
...
- `whatsapp-qa-observed-messages.json` - bodies redacted unless
  `NODOASSIST_QA_WHATSAPP_CAPTURE_CONTENT=1`.
...
`--credential-source convex` (or set `NODOASSIST_QA_CREDENTIAL_SOURCE=convex`);
...
- `mock-openai` is the scenario-aware NodoAssist mock. It remains the default
...
runs NodoAssist's normal channel plugins against them. `live` is reserved for
...
- the `nodoassist qa` command root
...
- how `nodoassist qa <runner>` is mounted beneath the shared `qa` root
...
3. Keep transport-specific mechanics inside the runner plugin or channel
   harness.
4. Mount the runner as `nodoassist qa <runner>` instead of registering a
   competing root command. Runner plugins should declare `qaRunners` in
   `nodoassist.plugin.json` and export a matching `qaRunnerCliRegistrations`
...
or wiring a new transport - run `pnpm nodoassist qa coverage` (add `--json`
for machine-readable output). When choosing focused proof for a touched
behavior or file path, run `pnpm nodoassist qa coverage --match <query>`. The
...
```bash
pnpm nodoassist qa character-eval \
