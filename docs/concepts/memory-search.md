...
NodoAssist uses OpenAI embeddings by default. To use another provider, set it
explicitly:
...
NodoAssist runs two retrieval paths in parallel and merges the results:
...
**No results?** Run `nodoassist memory status` to check the index. If empty, run
`nodoassist memory index --force`.

**Only keyword matches?** Your embedding provider may not be configured. Check
`nodoassist memory status --deep`.
