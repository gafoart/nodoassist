...
read_when:
  ...
  - Explaining how NodoAssist updates one channel message while work is in progress
...
`channels.<channel>.streaming.mode: "progress"` and NodoAssist creates the
...
that; otherwise NodoAssist sends the final answer through normal delivery and
...
default `label` is `"auto"`, which picks from NodoAssist's built-in single-word
label pool:
...
NodoAssist renders only `progress.text` in the channel progress UI. The normal
...
NodoAssist uses the same formatter for progress drafts and `/verbose`:
...
the draft is edited, and NodoAssist truncates long lines so repeated draft edits
...
With `toolProgress: false`, NodoAssist still suppresses the older standalone
...
When the final answer is ready, NodoAssist tries to keep the chat clean:

- If the draft can safely become the final answer, NodoAssist edits it in place.
- If the channel uses native progress streaming, NodoAssist finalizes that
...
- Otherwise (media, an approval prompt, an explicit reply target, too many
  chunks, or a failed edit/send) NodoAssist sends the final answer through the
...
Check `streaming.progress.toolProgress`. If it is `false`, NodoAssist keeps the
