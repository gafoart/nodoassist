---
summary: "NodoAssist capabilities across channels, routing, media, and UX."
read_when:
  - You want a full list of what NodoAssist supports
title: "Features"
---
...
- iMessage, Telegram, and WebChat ship with the core install; every other channel is an
  official plugin installed with `nodoassist plugins install @nodoassist/<id>` (or on demand
  during `nodoassist onboard` / `nodoassist channels add`)
...
- External plugin channels maintained outside the NodoAssist repo: WeChat, Yuanbao, and Zalo ClawBot
...
<CardGroup cols={2}>
  ...
  <Card title="Plugins" href="/tools/plugin" icon="plug">
    Official and external plugins that extend NodoAssist.
  </Card>
</CardGroup>
