...
alongside NodoAssist. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** - if QMD is unavailable, NodoAssist falls back to the
...
NodoAssist creates a self-contained QMD home under
`~/.nodoassist/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- NodoAssist creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so NodoAssist skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, NodoAssist retries with `qmd query`.
- When `searchMode` is `query`, set `memory.qmd.rerank` to `false` to use
  ...
  NodoAssist passes `--no-rerank` to the direct QMD CLI path and
  `rerank: false` to QMD's MCP query tool.
- With QMD releases that advertise multi-collection filters, NodoAssist groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, NodoAssist falls back to the builtin SQLite engine.
  ...
  `nodoassist memory status` and one-shot CLI probes still recheck QMD
  directly.
...
NodoAssist keeps the QMD search path compatible with both current and older QMD
installs.

On startup, NodoAssist checks the installed QMD help text once per manager. If
the binary advertises support for multiple collection filters, NodoAssist
...
Older QMD builds only accept one collection filter. When NodoAssist detects one
...
process, so you can tune QMD globally without adding new NodoAssist config:
...
collection under `~/.nodoassist/agents/<id>/qmd/sessions/`. Setting only
...
NodoAssist logs a warning with the derived channel and chat type so empty
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If NodoAssist
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but NodoAssist still reports
...
with `nodoassist memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs NodoAssist uses.

**Many QMD subprocesses during search?** Update QMD if possible. NodoAssist
...
`memory.qmd.searchMode = "search"`. NodoAssist treats that mode as
...
for the next startup reconciliation. NodoAssist recreates stale managed
...
QMD traversal follows the underlying QMD scanner rather than NodoAssist's
