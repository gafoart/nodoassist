---
summary: "How NodoAssist manages conversation sessions"
read_when:
...
NodoAssist routes every inbound message to a **session** based on where it came
...
Verify your setup with `nodoassist security audit`.
...
NodoAssist bounds session storage over time via `session.maintenance`, defaults
shown:
...
`nodoassist sessions cleanup --enforce` applies the cap immediately.
...
`nodoassist sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview any maintenance run with `nodoassist sessions cleanup --dry-run`.

## Inspecting sessions

| Command                      | Shows                                           |
| ---------------------------- | ----------------------------------------------- |
| `nodoassist status`          | Session store path and recent activity          |
| `nodoassist sessions --json` | All sessions (filter with `--active <minutes>`) |
