---
summary: "OAuth in NodoAssist: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand NodoAssist OAuth end-to-end
...
NodoAssist supports OAuth ("subscription auth") for providers that offer it,
notably **OpenAI Codex (ChatGPT OAuth)** and **Anthropic Claude CLI reuse**.
For Anthropic, the practical split is:

- **Anthropic API key**: normal Anthropic API billing.
- **Anthropic Claude CLI / subscription auth inside NodoAssist**: Anthropic staff
  told us this usage is allowed again, so NodoAssist treats Claude CLI reuse and
...
NodoAssist stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`nodoassist doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
issued for the same user/app. Practical symptom: log in via NodoAssist _and_
via Claude Code / Codex CLI, and one of them randomly gets logged out later.

To reduce that, NodoAssist treats the auth profile store as a **token sink**:

- the runtime reads credentials from one place per agent
- multiple profiles can coexist and route deterministically
- external CLI reuse is provider-specific: once NodoAssist owns a local OAuth
  profile for a provider, the local refresh token is canonical. If that local
  refresh token is rejected, NodoAssist reports the profile for
  ...
  `openai:default`-style profile before NodoAssist owns OAuth for that
  provider; after that, NodoAssist-owned refreshes stay canonical
...
- Legacy compatibility file: `~/.nodoassist/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.nodoassist/credentials/oauth.json` (imported into the auth profile store on first use)

All of the above also respect `$NODOASSIST_STATE_DIR` (state dir override). Full reference: [/gateway/configuration-reference#auth-storage](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, NodoAssist uses read-through
...
NodoAssist supports Anthropic Claude CLI reuse and `claude -p` as a sanctioned
...
available as a supported token-auth path, but NodoAssist prefers Claude CLI
reuse when it is available.

<Warning>
Anthropic's public Claude Code docs say direct Claude Code use stays within
Claude subscription limits, and Anthropic staff told us NodoAssist-style Claude
CLI usage is allowed again. NodoAssist therefore treats Claude CLI reuse and
`claude -p` usage as sanctioned for this integration unless Anthropic
...
If you want other subscription-style options in NodoAssist, see [OpenAI
...
NodoAssist's interactive login flows are implemented in `nodoassist/plugin-sdk/llm.ts` and wired into the wizards/commands.

### Anthropic setup-token

Flow shape:

1. start Anthropic setup-token or paste-token from NodoAssist
2. NodoAssist stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including NodoAssist workflows.
...
`nodoassist models auth list --provider openai` after repair before copying
...
3. try to capture the callback on `http://localhost:1455/auth/callback` (the
   ...
   override with `NODOASSIST_OAUTH_CALLBACK_HOST`)
...
Wizard path is `nodoassist onboard` → auth choice `openai`.
...
- externally managed CLI credentials (Claude CLI, narrow Codex CLI bootstrap;
  ...
  spending a copied refresh token. If a managed refresh fails, NodoAssist
...
nodoassist agents add personal
```
...
```bash
nodoassist models auth list --provider <id>
```
