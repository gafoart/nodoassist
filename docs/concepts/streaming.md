...
NodoAssist has two independent streaming layers, and there is **no true
...
streaming sends media early, NodoAssist remembers that delivery for the turn. If
...
distinct text around media that was already streamed, NodoAssist still sends the
...
When block streaming is enabled, NodoAssist can **merge consecutive block
...
| Discord  | `streamMode`, boolean `streaming`                           | Runtime aliases for the `streaming` enum; run `nodoassist doctor --fix` to rewrite persisted config                                                            |
| Slack    | `streamMode`; boolean `streaming`; legacy `nativeStreaming` | Runtime aliases for `streaming.mode` (and `streaming.nativeTransport` for the boolean/legacy forms); run `nodoassist doctor --fix` to rewrite persisted config |
...
- If the final edit fails before the completed text is confirmed, NodoAssist uses
...
- Telegram selected quote replies are an exception: when `replyToMode` is not
  `"off"` and selected quote text is present, NodoAssist skips the answer preview
...
- To keep preview streaming but hide tool-progress lines, set
  ...
  that use NodoAssist's compact progress renderer, including Discord, Matrix,
