...
Paste into `nodoassist.json` for a safe default: plugin on, scoped to `main`,
...
case-insensitive. If `allowedChatIds` is non-empty and NodoAssist cannot
...
With those on, NodoAssist appends diagnostic lines after the normal reply (as a
...
external context-engine plugin (`nodoassist plugins install
...
5. Verify memory search itself works with `nodoassist status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, NodoAssist uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `nodoassist status --deep` to inspect the memory-search backend and
      index health.
