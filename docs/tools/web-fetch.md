...
`plugins.entries.firecrawl.config.webFetch` via `nodoassist doctor --fix`.
...
- If `provider` is omitted, NodoAssist auto-detects the first ready web-fetch
...
In this mode, NodoAssist still applies hostname-based SSRF checks before sending
