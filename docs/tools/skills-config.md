...
`~/.nodoassist/nodoassist.json`. Agent-specific visibility lives under
...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  recommended for WhatsApp/Telegram). `nodoassist setup --node-manager` and
  `nodoassist onboard --node-manager` accept `npm`, `pnpm`, or `bun`; set
...
policy runs after NodoAssist has staged source material and before the install
  ...
  security: {
    installPolicy: {
      ...
      exec: {
        ...
        passEnv: ["NODOASSIST_STATE_DIR", "PATH"],
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. NodoAssist runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the NodoAssist process into the
...
`nodoassistVersion`, `targetType`, `targetName`, `sourcePath`, `sourcePathKind`,
...
NodoAssist does not execute install policy during normal Gateway startup.
Installs and updates fail closed when policy is enabled but unavailable.
`nodoassist doctor` performs static validation; `nodoassist doctor --deep`
...
{
  "protocolVersion": 1,
  "nodoassistVersion": "2026.6.1",
  "targetType": "skill",
  "targetName": "weather",
  "sourcePath": "/var/folders/.../nodoassist-skill-clawhub/root",
...
`metadata.nodoassist.skillKey`, use that key instead. Quote hyphenated names
...
<Warning>
  Agent skill allowlists are a visibility and loading filter for NodoAssist
  ...
  registries such as `~/.nodoassist/skills/config/mcporter.json`. For
...
Managed `~/.nodoassist/skills` and personal `~/.agents/skills` directories
