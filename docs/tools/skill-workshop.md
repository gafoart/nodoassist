...
Skill Workshop is NodoAssist's governed path for creating and updating workspace
...
```bash
nodoassist skills curator status
nodoassist skills curator pin <skill>
nodoassist skills curator unpin <skill>
nodoassist skills curator restore <skill>
```
...
apply it through the normal approval flow or with `nodoassist skills workshop`.
...
`nodoassist skills workshop apply|reject|quarantine <proposal-id>`. Agents should
...
# Update an existing workspace skill
nodoassist skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
...
NodoAssist detects durable instructions such as “next time,” “remember to,” and reactive corrections
...
```text
<NODOASSIST_STATE_DIR>/skill-workshop/
  proposals.json
...
Default state directory: `~/.nodoassist`.
...
| Proposal does not show in list                 | Check the selected `--agent` workspace and `NODOASSIST_STATE_DIR`.                                                                                                                                          |
...
When autonomous capture is enabled, `nodoassist doctor` runs the
...
`nodoassist plugins inspect skill-workshop`; that command now explains that Skill
...
- [Skills CLI](/cli/skills) for `nodoassist skills` commands
