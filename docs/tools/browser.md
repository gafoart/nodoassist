...
read_when:
  ...
  - Debugging why nodoassist is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (NodoAssist-managed)"
---

NodoAssist can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls. It runs through a small local control service inside the Gateway (loopback only) and is isolated from your personal browser.

- Think of it as a **separate, agent-only browser**. The `nodoassist` profile never touches your personal browser profile.
...
- A separate browser profile named **nodoassist** (orange accent by default).
...
- A bundled `browser-automation` skill that teaches agents the snapshot,
  ...
  plugin is enabled.
- Optional multi-profile support (`nodoassist`, `work`, `remote`, ...).
...
nodoassist browser --browser-profile nodoassist doctor --deep
nodoassist browser --browser-profile nodoassist status
...
If `nodoassist browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `nodoassist browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `nodoassist browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
## Profiles: `nodoassist`, `user`, `chrome`

- `nodoassist`: managed, isolated browser (no extension required).
- `user`: built-in Chrome DevTools MCP attach profile for your **real
  ...
  prompt the first time NodoAssist attaches, so someone must be at the computer.
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the NodoAssist browser extension instead of
  the remote-debugging port, so there is no "Allow remote debugging?" prompt.

For agent browser tool calls:

- Default: use the isolated `nodoassist` browser.
...
Set `browser.defaultProfile: "nodoassist"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.nodoassist/nodoassist.json`.
...
but NodoAssist does not automatically attach them to channel replies. To share a
...
understanding model is configured, NodoAssist keeps the normal image result so the
...
- Control service binds to loopback on a port derived from `gateway.port` (default `18791` = gateway + 2). `NODOASSIST_GATEWAY_PORT` takes priority over `gateway.port`; either shifts the derived ports in the same family.
- Local `nodoassist` profiles auto-assign `cdpPort`/`cdpUrl` from a range starting 9 ports above the control port (default `18800`-`18899`); set those only for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, NodoAssist pauses new launch
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the NodoAssist-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- NodoAssist-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `nodoassist browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `nodoassist browser start --headless` request a
  ...
  remote CDP profiles reject the override because NodoAssist does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  default to headless automatically when neither the environment nor profile/global
  config explicitly chooses headed mode. `nodoassist browser status --json`
  reports `headlessSource` as `env`, `profile`, `config`,
  `request`, `linux-display-fallback`, or `default`.
- `NODOASSIST_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `NODOASSIST_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- Default profile is `nodoassist` (managed standalone). Use `defaultProfile: "user"` to opt into the signed-in user browser.
...
- `driver: "extension"` drives your signed-in Chrome through the [NodoAssist Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
NodoAssist uses it automatically. Set `browser.executablePath` to override
auto-detection. Top-level and per-profile `executablePath` values accept `~`
for your OS home directory:

```bash
nodoassist config set browser.executablePath "/usr/bin/google-chrome"
nodoassist config set browser.profiles.work.executablePath "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
```
...
Per-profile `executablePath` only affects local managed profiles that NodoAssist
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, NodoAssist will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local NodoAssist-managed browser profile.
- `headless` only affects local managed profiles that NodoAssist launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `nodoassist browser stop` stops the browser process that
  NodoAssist launched
- attach-only and remote CDP profiles: `nodoassist browser stop` closes the active
  ...
  though no browser process was launched by NodoAssist
...
NodoAssist preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, NodoAssist can
...
- If you set `nodeHost.browserProxy.allowProfiles`, NodoAssist treats it as a least-privilege boundary limiting which profile names the proxy will target.
...
CDP connection URLs over HTTPS and WebSocket. NodoAssist can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let NodoAssist
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and NodoAssist runs on the host, treat
...
NodoAssist process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-NodoAssist WebSocket base, such
...
an address NodoAssist cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, NodoAssist treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by NodoAssist.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). NodoAssist accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  NodoAssist calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. NodoAssist connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). NodoAssist tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise NodoAssist
  ...
  accepts it, NodoAssist falls back to that root as well. This lets a bare `ws://`
...
`nodoassist browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-nodoassist-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, NodoAssist
...
- Configure `gateway.auth.token`, `gateway.auth.password`, `NODOASSIST_GATEWAY_TOKEN`, or
  `NODOASSIST_GATEWAY_PASSWORD` explicitly if you want a stable secret you control
...
NodoAssist supports multiple named profiles (routing configs). Profiles can be:

- **nodoassist-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `nodoassist` profile is auto-created if missing.
...
NodoAssist can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when NodoAssist attaches.
...
```bash
nodoassist browser --browser-profile user start
nodoassist browser --browser-profile user status
nodoassist browser --browser-profile user tabs
nodoassist browser --browser-profile user snapshot --format ai
```
...
- `nodoassist doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `nodoassist` profile because it can
  act inside your signed-in browser session.
- NodoAssist does not launch the browser for this driver; it only attaches.
- NodoAssist uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
When `cdpUrl` is set on an existing-session profile, NodoAssist skips
...
Compared to the managed `nodoassist` profile, existing-session drivers are more constrained:
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside NodoAssist appear under `browserState.dialogs.recent`.
...
When launching locally, NodoAssist picks the first available:
...
control API** plus a matching `nodoassist browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means NodoAssist cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "nodoassist" is not reachable after start`
  - `Remote CDP for profile "<name>" is not reachable at <cdpUrl>`
  - `Port <port> is in use for profile "<name>" but not by nodoassist` when a
...
- For the local loopback `nodoassist` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for NodoAssist's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (nodoassist, chrome, or remote CDP).
