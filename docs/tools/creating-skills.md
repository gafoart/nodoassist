...
summary: "Build, test, and publish custom SKILL.md workspace skills for your NodoAssist agents."
read_when:
...
NodoAssist loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.nodoassist/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.nodoassist/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    NodoAssist watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ```bash
    nodoassist agent --message "give me a greeting"
    ```
...
metadata: { "nodoassist": { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" } }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `nodoassist.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `nodoassist.json`:
...
nodoassist skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.nodoassist` gating fields
...
<Tip>
  ...
  - **Test locally** — use `nodoassist agent --message "..."` before sharing.
