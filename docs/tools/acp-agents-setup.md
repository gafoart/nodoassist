...
read_when:
  ...
  - Enabling the plugin-tools or NodoAssist-tools MCP bridge
...
Codex has two NodoAssist routes:
...
| `nodoassist` | NodoAssist ACP bridge (native `nodoassist acp`)                                                                 |
...
When NodoAssist uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal NodoAssist `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by NodoAssist before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, NodoAssist/acpx cannot force a model selection.
...
Packaged installs use the official `@nodoassist/acpx` runtime plugin for ACP.
...
```bash
nodoassist plugins install ./path/to/local/acpx-plugin
```
...
signal. Set `NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE=0` or
`NODOASSIST_SKIP_ACPX_RUNTIME_PROBE=1` only for scripts or environments that
...
- `agents.<id>.args` is optional. Each array item is shell-quoted before NodoAssist passes it through the current acpx command-string registry.
...
manually, and there is no separate postinstall step for NodoAssist itself. If an
adapter download or spawn fails, `/acp doctor` reports the failure.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose NodoAssist plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
NodoAssist plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
nodoassist config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `nodoassist-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled NodoAssist
  plugins.
...
### NodoAssist tools MCP bridge

By default, ACPX sessions also do **not** expose built-in NodoAssist tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
nodoassist config set plugins.entries.acpx.config.nodoAssistToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `nodoassist-tools` into ACPX session
  bootstrap.
- Exposes selected built-in NodoAssist tools. The initial server exposes `cron`.
...
```bash
nodoassist config set plugins.entries.acpx.config.timeoutSeconds 180
```

Runtime turns use NodoAssist agent/run timeouts, including `/acp timeout`.
...
```bash
nodoassist config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from NodoAssist exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between NodoAssist `tools.exec.mode`, Codex Guardian
...
<Warning>
NodoAssist defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode`.
