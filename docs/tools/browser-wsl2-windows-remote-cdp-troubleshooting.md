...
read_when:
  - Running NodoAssist Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, NodoAssist Gateway runs inside WSL2, Chrome runs
...
- NodoAssist points a browser profile at the address reachable from WSL2
...
```powershell
chrome.exe --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\NodoAssist\ChromeCDP"
```
...
If this fails, diagnose the Windows listeners below. NodoAssist is not the
problem yet.
...
that before touching NodoAssist config.

### Layer 3: configure the correct browser profile

Point NodoAssist at the address reachable from WSL2:
...
- use HTTP(S) when you want NodoAssist to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools
  socket URL
- test the same URL with `curl` before expecting NodoAssist to succeed
...
```bash
nodoassist browser --browser-profile remote open https://example.com
nodoassist browser --browser-profile remote tabs
```
...
| stale viewport / dark-mode / locale / offline overrides after a remote session          | run `nodoassist browser --browser-profile remote stop` to close the session and release the cached Playwright/CDP connection without restarting the Gateway or the external browser |
...
3. NodoAssist config: does `browser.profiles.<name>.cdpUrl` use that exact
...
from WSL2 second, and only then debug NodoAssist config or Control UI auth.
