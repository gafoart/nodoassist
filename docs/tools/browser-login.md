...
When a site requires login, sign in manually in the host browser's `nodoassist`
...
NodoAssist controls a dedicated Chrome profile named `nodoassist` (orange-tinted
UI), separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent uses its isolated `nodoassist` browser.
...
Two ways to access the `nodoassist` profile:
...
nodoassist browser open https://x.com
```

For a non-default profile, put `--browser-profile <name>` before the
subcommand (default is `nodoassist`):

```bash
nodoassist browser --browser-profile <name> open https://x.com
```
...
```bash
nodoassist browser --browser-profile nodoassist open https://x.com
```
