---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for NodoAssist browser control on Linux"
...
```json
{ "error": "Error: Failed to start Chrome CDP on port 18800 for profile \"nodoassist\"." }
```
...
Snap's AppArmor confinement interferes with how NodoAssist spawns and monitors
the browser process.

Other common Linux launch failures:

- `The profile appears to be in use by another Chromium process`: stale
  `Singleton*` lock files in the managed profile directory. NodoAssist removes
...
- `Missing X server or $DISPLAY`: a visible browser was explicitly requested
  ...
  If you set `NODOASSIST_BROWSER_HEADLESS=0`, `browser.headless: false`, or
  `browser.profiles.<name>.headless: false`, remove that headed override, set
  `NODOASSIST_BROWSER_HEADLESS=1`, start `Xvfb`, run
  `nodoassist browser start --headless` for a one-shot managed launch, or run
  NodoAssist in a real desktop session.
...
Update `~/.nodoassist/nodoassist.json`:
...
If you must keep snap Chromium, configure NodoAssist to attach to a
...
chromium-browser --headless --no-sandbox --disable-gpu \
  ...
  --user-data-dir=$HOME/.nodoassist/browser/nodoassist/user-data \
  about:blank &
```
...
```ini
# ~/.config/systemd/user/nodoassist-browser.service
[Unit]
Description=NodoAssist Browser (Chrome CDP)
After=network.target

[Service]
ExecStart=/snap/bin/chromium --headless --no-sandbox --disable-gpu --remote-debugging-port=18800 --user-data-dir=%h/.nodoassist/browser/nodoassist/user-data about:blank
Restart=on-failure
RestartSec=5
...
```bash
systemctl --user enable --now nodoassist-browser.service
```
...
| `NODOASSIST_BROWSER_HEADLESS`    | Per-process override for local managed browser headless mode         | unset                                                              |
...
launch succeeds but `nodoassist browser start` still reports `not reachable
after start`.
...
1. Use the managed browser instead:
   `nodoassist browser --browser-profile nodoassist start` (or set
   `browser.defaultProfile: "nodoassist"`).
...
- Local `nodoassist`-driver profiles auto-assign `cdpPort`/`cdpUrl`; only set
