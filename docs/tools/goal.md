...
read_when:
  - You want NodoAssist to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current NodoAssist session.
...
stale or unknown token snapshot when the goal starts, NodoAssist waits for the
...
NodoAssist usage and model controls.

## Model tools

NodoAssist exposes three goal tools to agent harnesses:
...
NodoAssist keeps the line compact by truncating long objectives. Paused,
...
`/goal` works in command-capable NodoAssist sessions, including the TUI and
...
fresh token snapshot yet. Usage refreshes as NodoAssist records session usage
