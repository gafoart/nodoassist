---
summary: "Typed workflow runtime for NodoAssist with resumable approval gates."
title: Lobster
read_when:
...
see [Task Flow](/automation/taskflow) (`nodoassist tasks flow`); for the task
...
→ nodoassist calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ nodoassist calls gmail.send
...
NodoAssist runs Lobster workflows **in-process** using the bundled
...
gog.gmail.search --query 'newer_than:1d' \
  | nodoassist.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
### Important limitation: embedded Lobster vs `nodoassist.invoke`

The bundled Lobster plugin runs workflows **in-process** inside the gateway.
In that embedded mode, `nodoassist.invoke` does **not** automatically inherit a
gateway URL/auth context for nested NodoAssist CLI tool calls.

That means this pattern is **not currently reliable in the embedded runner**:

```lobster
nodoassist.invoke --tool llm-task --action json --args-json '{ ... }'
```

Use the example below only when running the **standalone Lobster CLI** in an
environment where `nodoassist.invoke` is already configured with the correct
...
- non-`nodoassist.invoke` steps inside the Lobster pipeline until a supported
...
a bare envelope: NodoAssist creates or resumes a durable flow record, applies the
...
- **No secrets** - Lobster doesn't manage OAuth; it calls NodoAssist tools that
  do.
