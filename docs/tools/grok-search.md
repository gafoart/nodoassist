...
NodoAssist supports Grok as a `web_search` provider, using xAI web-grounded
...
lets NodoAssist reuse it as a fallback for the bundled xAI model provider.
...
Choosing **Grok** during `nodoassist onboard` or `nodoassist configure --section
web` lets NodoAssist reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

NodoAssist then offers a follow-up step to enable `x_search` with the same xAI
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    nodoassist onboard --auth-choice xai-oauth
...
**Credential alternatives:** `nodoassist models auth login --provider xai
...
through an operator proxy or xAI-compatible Responses endpoint. NodoAssist
