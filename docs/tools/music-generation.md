...
generated audio is still missing from the reply, NodoAssist sends an
...
<Note>
Not all providers support all parameters. NodoAssist still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, NodoAssist
...
Provider request timeouts are operator configuration only. NodoAssist uses
...
- **Status lookup:** `nodoassist tasks list` or `nodoassist tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** NodoAssist injects an internal completion event back
...
nodoassist tasks show <taskId>
nodoassist tasks cancel <taskId>
```
...
```bash
NODOASSIST_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
