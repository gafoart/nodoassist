      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; NodoAssist appends /v1/search
          },
        },
      },
    },
...
environment. For a gateway install, put it in `~/.nodoassist/.env`.
...
Cloudflare AI Gateway). NodoAssist normalizes bare hosts by prepending
...
- Both providers return a `session_id`; NodoAssist surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- NodoAssist always forwards a resolved result count to Parallel as
  ...
  NodoAssist's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
