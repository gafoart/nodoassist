...
NodoAssist converts outbound replies into audio across **14 speech providers**:
...
<Steps>
  ...
  <Step title="Try it in chat">
    `/tts status` shows the current state. `/tts audio Hello from NodoAssist`
...
NodoAssist picks the first configured provider in registry auto-select order.
...
normalized to `microsoft` and `nodoassist doctor --fix` rewrites persisted
config; new configs should always use `microsoft`.
</Warning>

## Configuration

TTS config lives under `messages.tts` in `~/.nodoassist/nodoassist.json`. Pick a
...
the voice-design prompt. NodoAssist sends that prompt as the TTS `user` message
...
For each provider attempt, NodoAssist merges configs in this order:
...
<AccordionGroup>
  <Accordion title="Google Gemini">
    ...
    inside the Gemini transcript; NodoAssist does not generate these tags.
  </Accordion>
...
Single command `/tts`. On Discord, NodoAssist also registers `/voice` because
...
`~/.nodoassist/settings/tts.json`; override with the `NODOASSIST_TTS_PREFS` env var
...
- **Google Gemini:** returns raw 24 kHz PCM. NodoAssist wraps it as WAV for audio attachments, transcodes it to 48 kHz Opus for voice-note targets, and returns PCM directly for Talk/telephony.
...
- **Microsoft:** uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, NodoAssist retries with MP3.
  - When no explicit voice override is set and the default English voice is used, NodoAssist auto-switches to a Chinese neural voice (`zh-CN-XiaoxiaoNeural`, `zh-CN` locale) if the reply text is CJK-dominant.

OpenAI and ElevenLabs output formats are fixed per channel as listed above.

## Auto-TTS behavior

When `messages.tts.auto` is enabled, NodoAssist:
...
If the reply exceeds `maxLength`, NodoAssist never skips audio outright:
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="enabled" type="boolean" deprecated>
      Legacy toggle. `nodoassist doctor --fix` migrates this to `auto`.
    </ParamField>
    <ParamField path="mode" type='"final" | "all"' default="final">
      `"all"` includes tool/block replies in addition to final replies.
    </ParamField>
    <ParamField path="provider" type="string">
      Speech provider id. When unset, NodoAssist uses the first configured provider in registry auto-select order. Legacy `provider: "edge"` is rewritten to `"microsoft"` by `nodoassist doctor --fix`.
    </ParamField>
    ...
    <ParamField path="providers.<id>" type="object">
      Provider-owned settings keyed by speech provider id. Legacy direct blocks (`messages.tts.openai`, `.elevenlabs`, `.microsoft`, `.edge`) are rewritten by `nodoassist doctor --fix`; commit only `messages.tts.providers.<id>`.
    </ParamField>
    ...
    <ParamField path="prefsPath" type="string">
      Override the local prefs JSON path (provider/limit/summary). Default `~/.nodoassist/settings/tts.json`.
    </ParamField>
  </Accordion>
  ...
  <Accordion title="Microsoft (no API key)">
    ...
    <ParamField path="speakerVoice" type="string">Microsoft neural voice name (e.g. `en-US-MichelleNeural`). Legacy alias: `voice`. If the default English voice is in effect and reply text is CJK-dominant, NodoAssist auto-switches to `zh-CN-XiaoxiaoNeural`.</ParamField>
    ...
    <ParamField path="edge.*" type="object" deprecated>Legacy alias. Run `nodoassist doctor --fix` to rewrite persisted config to `providers.microsoft`.</ParamField>
  </Accordion>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="style" type="string">Optional natural-language style instruction sent as the user message; not spoken. For `mimo-v2.5-tts-voicedesign`, this is the voice-design prompt; NodoAssist supplies a default when omitted.</ParamField>
  </Accordion>
</AccordionGroup>
