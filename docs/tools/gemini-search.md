...
NodoAssist supports Gemini models with built-in
...
For a gateway install, put env keys in `~/.nodoassist/.env`.
...
- Citation URLs from Gemini grounding are automatically resolved from Google
  redirect URLs to direct URLs via a HEAD request through NodoAssist's SSRF-guarded
