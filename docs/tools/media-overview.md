...
read_when:
  - Looking for an overview of NodoAssist's media capabilities
...
NodoAssist generates images, videos, and music, understands inbound media
...
NodoAssist until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, NodoAssist submits the request to the provider, returns a task
...
NodoAssist wakes the agent with the generated media paths so it can tell the
...
NodoAssist sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; NodoAssist has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in NodoAssist until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
