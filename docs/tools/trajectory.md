---
summary: "Export redacted trajectory bundles for debugging an NodoAssist agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an NodoAssist session
...
Trajectory capture is NodoAssist's per-session flight recorder. It records a
...
NodoAssist writes the bundle under the workspace:

```text
.nodoassist/trajectory-exports/nodoassist-trajectory-<session>-<timestamp>/
```
...
The name resolves inside `.nodoassist/trajectory-exports/`. Absolute paths and
`~` paths are rejected.
...
bundle; do not use allow-all. In group chats, NodoAssist sends the approval
...
`.nodoassist/trajectory-exports`), `--store <path>` (session store override),
...
Trajectory capture is on by default for NodoAssist agent runs.
...
```json
{
  "traceSchema": "nodoassist-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | NodoAssist version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata   |
...
NodoAssist also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `NODOASSIST_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory instead, one JSONL file per session id:

```bash
export NODOASSIST_TRAJECTORY_DIR=/var/lib/nodoassist/trajectories
```
...
```bash
export NODOASSIST_TRAJECTORY=0
```

This disables runtime trajectory capture before starting NodoAssist.
...
NodoAssist flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`NODOASSIST_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting NodoAssist:

```bash
export NODOASSIST_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when NodoAssist logs an `nodoassist-trajectory-flush` timeout and
...
`NODOASSIST_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are for support and debugging, not public posting. NodoAssist
...
- confirm NodoAssist was started without `NODOASSIST_TRAJECTORY=0`
- check whether `NODOASSIST_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.nodoassist/trajectory-exports/`
