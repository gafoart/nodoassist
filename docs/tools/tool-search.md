---
summary: "Tool Search: compact large NodoAssist tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want NodoAssist agents to use a large tool catalog without adding every tool schema to the prompt
  - You want NodoAssist tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for NodoAssist runs
---

Tool Search is an experimental NodoAssist agent runtime feature. It gives agents one
...
This page documents NodoAssist Tool Search. It is not the Codex-native tool
...
When enabled for NodoAssist runs, the model receives one `tool_search_code` tool
by default. That tool runs a short JavaScript body in an isolated Node
subprocess with an `nodoassist.tools` bridge:

```js
const hits = await nodoassist.tools.search("create a GitHub issue");
const tool = await nodoassist.tools.describe(hits[0].id);
return await nodoassist.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include NodoAssist tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through NodoAssist.

Codex harness runs do not receive these experimental NodoAssist Tool Search
controls. NodoAssist passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the NodoAssist embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible NodoAssist and plugin tools.
...
6. Expose the NodoAssist code bridge, the structured fallback tools, or the
   compact directory surface to the model.

At execution time every real tool call returns to NodoAssist. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`nodoassist.tools.call(...)` crosses the bridge back into the Gateway, where the
...
- `directory`: exposes `tool_search`, `tool_describe`, and `tool_call` plus a
  ...
  providers that should see tool names without every full schema. NodoAssist can
  also expose a small bounded set of likely or required tool schemas directly
  for the current turn.

All modes use the same policy-filtered catalog and normal NodoAssist execution
...
for the current run while NodoAssist tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small NodoAssist tool
...
catalog includes eligible NodoAssist, MCP, and client tools after normal policy
filtering.
...
`nodoassist.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await nodoassist.tools.search("calendar event", { limit: 5 });
```

`nodoassist.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await nodoassist.tools.describe("mcp:calendar:create_event");
```

`nodoassist.tools.call(id, args)`

Calls a selected tool through NodoAssist.

```js
await nodoassist.tools.call(calendarCreate.id, {
  summary: "Planning",
...
the model requests an exact hidden directory tool name directly, NodoAssist
hydrates it from the authorized catalog before normal execution.
Directory-mode client tool names must not collide with NodoAssist, plugin, or MCP
...
network grants, and no child-process or worker grants. NodoAssist enforces a
...
- `nodoassist.tools.search`
- `nodoassist.tools.describe`
- `nodoassist.tools.call`

Normal NodoAssist behavior still applies to final calls:
...
Enable Tool Search for NodoAssist runs with the default code bridge:

```bash
nodoassist config set tools.toolSearch true
```
...
Use the structured fallback tools instead for NodoAssist runs:
...
Use the compact directory surface instead for NodoAssist runs:
...
- final tool calls executed through NodoAssist
...
- whether the result came from NodoAssist, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves both paths with the NodoAssist runtime:

```bash
pnpm nodoassist qa suite --provider-mode mock-openai --scenario tool-search-gateway-e2e
```
