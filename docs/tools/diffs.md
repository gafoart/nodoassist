...
<Steps>
  <Step title="Install the plugin">
    ```bash
    nodoassist plugins install diffs
    ```
  </Step>
...
```bash
nodoassist plugins install clawhub:@nodoassist/diffs-language-pack
```
...
- Artifacts live under `$TMPDIR/nodoassist-diffs`.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in NodoAssist config.
  </Step>
