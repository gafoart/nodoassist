...
normalized results, cached by query for 15 minutes (configurable). NodoAssist
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    nodoassist configure --section web
    ```
  ...
  <Step title="Use it">
    ```javascript
    await web_search({ query: "NodoAssist plugin SDK" });
    ```
...
no `provider` is set, NodoAssist checks providers in this order and uses the
...
`nodoassist configure --section web`. NodoAssist does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, NodoAssist resolves only the
...
`web_search` tool automatically when NodoAssist web search is enabled and no
...
search and NodoAssist's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. NodoAssist uses the
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  NodoAssist's managed `web_search` instead
...
use the managed `web_search` fallback through NodoAssist's dynamic tool namespace.
Use an explicit managed provider when you need NodoAssist's provider-specific
...
Codex app-server first with `nodoassist models auth login --provider openai`.
The parent agent can use any model or runtime; only the bounded search worker
runs through Codex.

## Network safety

Managed HTTP `web_search` provider calls use NodoAssist's guarded fetch path,
scoped to the current provider's own hostname. For that hostname only,
NodoAssist allows Surge, Clash, and sing-box fake-IP DNS answers in
...
Grok can also reuse an xAI OAuth auth profile from `nodoassist models auth login
...
NodoAssist keeps startup resilient and reports a warning so you can reinstall the
plugin or run `nodoassist doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let NodoAssist auto-detect the first ready web-fetch
...
When you choose **Kimi** during `nodoassist onboard` or
`nodoassist configure --section web`, NodoAssist can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `nodoassist doctor --fix`.
When you choose Grok during `nodoassist onboard` or `nodoassist configure --section web`,
NodoAssist also offers optional `x_search` setup with the same credential right
...
provider, NodoAssist does not show the `x_search` prompt.

### Storing API keys

<Tabs>
  <Tab title="Config file">
    Run `nodoassist configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.nodoassist/.env`.
...
optional structured filters. NodoAssist constructs the built-in xAI `x_search`
