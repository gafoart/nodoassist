...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in NodoAssist
...
Today this applies to NodoAssist embedded runs and NodoAssist dynamic tools in the Codex
app-server harness. Tokenjuice hooks NodoAssist's tool-result middleware and
...
```bash
nodoassist plugins install clawhub:@nodoassist/tokenjuice
```
...
```bash
nodoassist config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
nodoassist plugins disable tokenjuice
```
