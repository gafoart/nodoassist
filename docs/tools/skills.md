...
frontmatter and a markdown body. NodoAssist loads bundled skills plus any local
...
NodoAssist loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.nodoassist/skills`                  |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. NodoAssist discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an NodoAssist
  skill root. Use `nodoassist migrate plan codex` to inventory those skills, then
  `nodoassist migrate codex` to copy them into your NodoAssist workspace.
</Note>
...
| Shared managed | `~/.nodoassist/skills`       | All agents on this machine  |
...
`nodoassist.plugin.json` (paths relative to the plugin root). Plugin skills load
...
`metadata.nodoassist.requires` in its frontmatter, same as any other skill.
...
```bash
nodoassist skills workshop list
nodoassist skills workshop inspect <proposal-id>
nodoassist skills workshop apply <proposal-id>
```
...
`nodoassist skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                  |
| ---------------------------------- | -------------------------------------------------------- |
| Install a skill into the workspace | `nodoassist skills install @owner/<slug>`                |
| Install from a Git repository      | `nodoassist skills install git:owner/repo@ref`           |
| Install a local skill directory    | `nodoassist skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `nodoassist skills install @owner/<slug> --global`       |
| Update all workspace skills        | `nodoassist skills update --all`                         |
| Update a shared managed skill      | `nodoassist skills update @owner/<slug> --global`        |
| Update all shared managed skills   | `nodoassist skills update --all --global`                |
| Verify a skill's trust envelope    | `nodoassist skills verify @owner/<slug>`                 |
| Print the generated Skill Card     | `nodoassist skills verify @owner/<slug> --card`          |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                     |

<AccordionGroup>
  <Accordion title="Install details">
    `nodoassist skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.nodoassist/skills` directory, visible to all local agents unless agent
    ...
    `nodoassist skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `nodoassist skills verify @owner/<slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `nodoassist.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.nodoassist/skills` and personal `~/.agents/skills` may contain
...
<Note>
  NodoAssist follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, NodoAssist keeps the skill's instructions out of the agent's normal
...
NodoAssist filters skills at load time using `metadata.nodoassist` (JSON5 object
embedded in the frontmatter, see the parsing note above). A skill with no
`metadata.nodoassist` block is always eligible unless explicitly disabled.
...
<ParamField path="requires.config" type="string[]">
  Each `nodoassist.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.nodoassist` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, NodoAssist lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `nodoassist.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** NodoAssist does not auto-install Homebrew or translate brew
    ...
    - **Go:** NodoAssist requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, NodoAssist installs Go via
      ...
      dependency always targets a dedicated NodoAssist-managed bin directory
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.nodoassist/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.nodoassist.primaryEnv`.
...
<Note>
  ...
  `metadata.nodoassist.skillKey`, use that key under `skills.entries` instead.
  Quote hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, NodoAssist:

<Steps>
  <Step title="Reads skill metadata">
    NodoAssist resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, NodoAssist also materializes the same
...
NodoAssist snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, NodoAssist refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, NodoAssist watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, NodoAssist can treat macOS-only skills as eligible when
    ...
    answering bin probes, NodoAssist clears its cached bin matches.
...
When skills are eligible, NodoAssist injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), NodoAssist first drops descriptions
(compact format: name + location only), then truncates the skill list and adds
a note pointing at `nodoassist skills check`.
