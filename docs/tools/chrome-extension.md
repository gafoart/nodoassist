---
summary: "Chrome extension: let NodoAssist drive your signed-in Chrome with no remote-debugging prompt"
read_when:
...
The NodoAssist Chrome extension lets an agent control your **signed-in Chrome
tabs** without launching a separate managed browser, and **without** Chrome's
blocking "Allow remote debugging?" prompt.

This matters when you drive NodoAssist from a phone (Telegram, WhatsApp, etc.):
the [`user` profile](/tools/browser#profiles-nodoassist-user-chrome) attaches over
...
so the only in-page hint is Chrome's dismissible "NodoAssist started debugging
this browser" banner.
...
- **Extension relay** (loopback WebSocket): a small server the control service
  ...
  NodoAssist and speaks to the extension. Both sides authenticate with a
  host-local token (see below).
- **NodoAssist Chrome extension** (MV3): attaches to tabs with `chrome.debugger`,
  forwards CDP traffic, and manages the **NodoAssist tab group**.

NodoAssist only sees and controls tabs that are in the **NodoAssist tab group**. The
...
1. Print the unpacked extension path:
   ...
   ```bash
   nodoassist browser extension path
   ```
...
3. Print the pairing string:
   ...
   ```bash
   nodoassist browser extension pair
   ```

4. Click the NodoAssist toolbar icon and paste the pairing string into the popup.
...
```bash
nodoassist config set browser.defaultProfile chrome
```
...
- Share a tab: click the NodoAssist toolbar button on that tab (it joins the
  NodoAssist tab group), or drag any tab into the group.
...
- **Same host** (Gateway + Chrome on one machine): pair on that machine with
  `nodoassist browser extension pair`. The relay is loopback-only.
- **Direct to a remote Gateway** (Chrome on your laptop, Gateway on a VPS, and
  **nothing else on the laptop**): on the Gateway, run
  `nodoassist browser extension pair --gateway-url wss://your-gateway.example.com`.
  ...
  over `wss://` — no NodoAssist install, Node, CLI, or open inbound port on the
  laptop. This is the managed-hosting path.
- **Via a browser node host** (Chrome on a machine already running an NodoAssist
...
```bash
nodoassist browser status --browser-profile chrome
nodoassist browser doctor --browser-profile chrome
```
...
- The agent can only see and drive tabs in the **NodoAssist tab group**. Your
...
managed `nodoassist` and Chrome MCP `user` profiles.
