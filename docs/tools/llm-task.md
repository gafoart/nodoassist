...
`nodoassist.invoke` already has the correct gateway URL/auth context.

For the bundled **embedded** Lobster runner inside NodoAssist, this nested CLI
...
- Lobster steps that do not rely on nested `nodoassist.invoke` calls.
