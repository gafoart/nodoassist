...
NodoAssist supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  NodoAssist never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, NodoAssist defaults to
...
- If the resolved host is `https://ollama.com`, NodoAssist calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise NodoAssist calls the local proxy endpoint
  ...
  the local host.
- NodoAssist warns during setup if Ollama is unreachable or not signed in, but
