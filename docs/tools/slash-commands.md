...
the NodoAssist command handler, and `/status` plus `/unfocus` stay local whenever
...
<ParamField path="commands.config" type="boolean" default="false">
  Enables `/config` (reads/writes `nodoassist.json`). Owner-only.
</ParamField>

<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes NodoAssist-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | --- | --- | --- |
    | `/config show\|get\|set\|unset` | `commands.config: true` | Read or write `nodoassist.json`. Owner-only |
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write NodoAssist-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart NodoAssist |
...
/config set messages.responsePrefix="[nodoassist]"
...
`/mcp` stores config in NodoAssist config, not embedded-agent project settings.
...
<AccordionGroup>
  ...
  <Accordion title="Slack specifics">
    `channels.slack.slashCommand` supports a single `/nodoassist`-style command.
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `NodoAssist Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
