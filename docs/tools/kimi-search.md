...
<Steps>
  ...
  <Step title="Store the key">
    ...
    gateway install, add it to `~/.nodoassist/.env`), or configure via:
...
Choosing **Kimi** during `nodoassist onboard` or `nodoassist configure --section web`
also prompts for:
...
NodoAssist only returns a Kimi `web_search` result after Moonshot's response
...
grounding (for example "I cannot browse the internet"), NodoAssist returns a
