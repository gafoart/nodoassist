...
`nodoassist agent` runs a single agent turn from the command line without an
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    nodoassist agent --agent main --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    nodoassist agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    nodoassist agent --session-id abc123 --message "Continue the task"
  ...
  <Step title="Deliver the reply to a channel">
    ...
    nodoassist agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    nodoassist agent --agent ops --message "Generate report" \
...
nodoassist agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
...
<CardGroup cols={2}>
  <Card title="Agent CLI reference" href="/cli/agent" icon="terminal">
    Full `nodoassist agent` flag and option reference.
  </Card>
