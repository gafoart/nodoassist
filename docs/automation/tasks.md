...
- `nodoassist tasks list` shows all tasks; `nodoassist tasks audit` surfaces issues.
...
<Tabs>
  <Tab title="List and filter">
    ...
    nodoassist tasks list

    # Filter by runtime or status
    nodoassist tasks list --runtime acp
    nodoassist tasks list --status running
    ```
  ...
  <Tab title="Cancel and notify">
    ...
    nodoassist tasks cancel <lookup>

    # Change notification policy for a task
    nodoassist tasks notify <lookup> state_changes
    ```
  ...
  <Tab title="Audit and maintenance">
    ```bash
    # Run a health audit
    nodoassist tasks audit

    # Preview or apply maintenance
    nodoassist tasks maintenance
    nodoassist tasks maintenance --apply
    ```
  ...
  <Tab title="Task flow">
    ...
    nodoassist tasks flow show <lookup>
...
| CLI operations         | `cli`        | `nodoassist agent` commands that run through the gateway               | `silent`              |
...
| `cancelled` | Stopped by the operator via `nodoassist tasks cancel`, or the run was aborted |
...
- CLI tasks: tasks with a run id/source id use the live run context, so lingering child-session or chat-session rows do not keep them alive after the gateway-owned run disappears. Legacy CLI tasks without run identity still fall back to the child session. Gateway-backed `nodoassist agent` runs also finalize from their run result, so completed runs do not sit active until the sweeper marks them `lost`.

## Delivery and notifications

When a task reaches a terminal state, NodoAssist notifies you. There are two delivery paths:

**Direct delivery** - if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Discord, Slack, Telegram, etc.). Group and channel task completions are instead routed through the requester session so the parent agent can write the visible reply. For subagent completions, NodoAssist also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
<AccordionGroup>
  <Accordion title="tasks list">
    ```bash
    nodoassist tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
    ```

    Output columns: Task, Kind, Status, Delivery, Run, Child Session, Summary. Bare `nodoassist tasks` behaves like `nodoassist tasks list`.

  </Accordion>
  <Accordion title="tasks show">
    ```bash
    nodoassist tasks show <lookup> [--json]
    ```
  ...
  <Accordion title="tasks notify">
    ```bash
    nodoassist tasks notify <lookup> <done_only|state_changes|silent>
    ```
  </Accordion>
  <Accordion title="tasks audit">
    ```bash
    nodoassist tasks audit [--severity <warn|error>] [--code <name>] [--limit <n>] [--json]
    ```

    Surfaces operational issues for tasks **and** TaskFlows in one report. Findings also appear in `nodoassist status` when issues are detected.
  ...
  <Accordion title="tasks maintenance">
    ```bash
    nodoassist tasks maintenance [--json]
    nodoassist tasks maintenance --apply [--json]
    ```
    ...
    When applying maintenance, NodoAssist also removes stale `cron:<jobId>:run:<runId>` session registry rows older than 7 days, while preserving rows for currently running cron jobs and leaving non-cron session rows untouched.

  </Accordion>
  <Accordion title="tasks flow list | show | cancel">
    ```bash
    nodoassist tasks flow list [--status <status>] [--json]
...
For the full operator ledger, use the CLI: `nodoassist tasks list`.
...
`nodoassist status` includes an at-a-glance task line:
...
Task records and delivery state persist in the shared NodoAssist SQLite state database:

```
~/.nodoassist/state/nodoassist.sqlite   (tables: task_runs, task_delivery_state, flow_runs)
```

Set `NODOASSIST_STATE_DIR` to move the whole state root (default `~/.nodoassist`) elsewhere; the shared database path moves with it.
...
Legacy sidecar stores from older installs (`tasks/runs.sqlite`, `flows/registry.sqlite`) are imported into the shared database by `nodoassist doctor`.
...
<AccordionGroup>
  <Accordion title="Tasks and Task Flow">
    [Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `nodoassist tasks` to inspect individual task records and `nodoassist tasks flow` to inspect the orchestrating flow.

  </Accordion>
  <Accordion title="Tasks and cron">
    Cron job definitions, runtime execution state, and run history live in NodoAssist's shared SQLite state database. **Every** cron execution creates a task record - both main-session and isolated - with `silent` notify policy, so cron runs are tracked without generating task notifications of their own.
