...
read_when:
  - You want to connect NodoAssist to QQ
  - You need QQ Bot credential setup
  - You want QQ Bot group or private chat support
title: QQ bot
---

QQ Bot connects to NodoAssist via the official QQ Bot API (WebSocket gateway).
...
```bash
nodoassist plugins install @nodoassist/qqbot
```
...
```bash
nodoassist channels add --channel qqbot --token "AppID:AppSecret"
```
...
binding. NodoAssist persists the returned credentials under the account's config
scope.
...
- `nodoassist channels add --channel qqbot --token-file ...` sets the AppSecret
...
Run multiple QQ bots under a single NodoAssist instance:
...
```bash
nodoassist channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
Old QQBot `toolPolicy` entries are retired. Run `nodoassist doctor --fix` to migrate them to `tools`.
...
| `/bot-version`       | —         | private only | Show the NodoAssist framework version and plugin version                       |
...
- Inbound, outbound, and gateway-bridge media share one payload root under
  `~/.nodoassist/media/qqbot` (honoring `NODOASSIST_HOME` when set), so uploads,
...
- If a hot upgrade interrupts the Gateway before it finishes writing
  `nodoassist.json`, the plugin restores the last-known `appId` / `clientSecret`
