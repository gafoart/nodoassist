---
summary: "Zalo ClawBot channel setup through the external nodoassist-zaloclawbot plugin"
read_when:
  - You want a personal Zalo assistant bot with QR-code login
  - You are installing or troubleshooting the nodoassist-zaloclawbot channel plugin
title: "Zalo ClawBot"
---

NodoAssist connects to Zalo ClawBot through the catalog-listed external `@zalo-platforms/nodoassist-zaloclawbot` plugin. Login uses a Zalo Mini App QR code; the plugin id in config is `nodoassist-zaloclawbot`.

## Compatibility

| Plugin Version | NodoAssist Version | npm dist-tag | Status        |
| -------------- | ------------------ | ------------ | ------------- |
...
- Node.js >= 22
- [NodoAssist](https://docs.openclaw.ai/install) installed (`nodoassist` CLI available)
...
```bash
nodoassist plugins install "@zalo-platforms/nodoassist-zaloclawbot@0.1.4"
```

Use the exact pinned version so NodoAssist verifies the package against the catalog integrity hash during install.

### 2. Enable the plugin in config

```bash
nodoassist config set plugins.entries.nodoassist-zaloclawbot.enabled true
```

### 3. Generate a QR code and log in

```bash
nodoassist channels login --channel nodoassist-zaloclawbot
```
...
The plugin manages bot credentials under the NodoAssist state directory. Treat that directory as sensitive and cover it under the same access-control and backup policy as the rest of NodoAssist state.

This plugin's runtime lives entirely in the external `@zalo-platforms/nodoassist-zaloclawbot` package; behavior details below beyond install/config are as reported by the plugin's maintainers and are not verified against NodoAssist core source.
...
- **Gateway fails to load:** confirm your NodoAssist host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger this ID requires.
