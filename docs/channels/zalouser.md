...
read_when:
  - Setting up Zalo Personal for NodoAssist
...
- Pin a version: `nodoassist plugins install @nodoassist/zalouser@<version>`
- From a source checkout: `nodoassist plugins install ./path/to/local/zalouser-plugin`
...
- `nodoassist pairing list zalouser`
- `nodoassist pairing approve zalouser <code>`
...
<Note>
`channels.zalouser.groups.<id>.allow` is a legacy field name; current config uses `enabled`. `nodoassist doctor --fix` migrates `allow` to `enabled` automatically.
</Note>
...
- When a group message is skipped because a mention is required, NodoAssist stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in NodoAssist state. Example:
...
Profile names select the saved Zalo login credentials in NodoAssist state. Resolution order:
...
- NodoAssist sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, NodoAssist sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `nodoassist channels logout --channel zalouser && nodoassist channels login --channel zalouser`
