...
Google Chat runs as the official `@nodoassist/googlechat` plugin: DMs and spaces through Google Chat API webhooks (HTTP endpoint only, no Pub/Sub).

## Install

```bash
nodoassist plugins install @nodoassist/googlechat
```

Local checkout (when running from a git repo):

```bash
nodoassist plugins install ./path/to/local/googlechat-plugin
```
...
2. Create a **Service Account**:
   ...
   - Name it whatever you want (e.g., `nodoassist-chat`).
...
4. Store the downloaded JSON file on your gateway host (e.g., `~/.nodoassist/googlechat-service-account.json`).
...
7. Configure NodoAssist with the service account and the webhook audience (must match the Chat app config):
   - Env: `GOOGLE_CHAT_SERVICE_ACCOUNT_FILE=/path/to/service-account.json` (default account only), or
   - Config: see [Config highlights](#config-highlights). `nodoassist channels add --channel googlechat` also accepts `--audience-type`, `--audience`, `--webhook-path`, and `--webhook-url`.
...
Google Chat webhooks require a public HTTPS endpoint. For security, expose **only the `/googlechat` path** to the internet and keep the NodoAssist dashboard and other endpoints private.
...
Requests to `your-domain.com/` are ignored or 404, while `your-domain.com/googlechat` routes to NodoAssist.
...
2. NodoAssist authenticates every request before dispatch:
...
5. DM access is pairing by default. Unknown senders receive a pairing code; approve with:
   - `nodoassist pairing approve googlechat <code>`
...
1. **Channel not configured**: the `channels.googlechat` section is missing. Verify with:
   ...
   ```bash
   nodoassist config get channels.googlechat
   ```
...
2. **Plugin not enabled**: check plugin status:
   ...
   ```bash
   nodoassist plugins list | grep googlechat
   ```
...
- `nodoassist channels status --probe` surfaces auth errors and missing audience config (`audience` and `audienceType` are both required).
...
- `nodoassist logs --follow` while sending a test message shows whether requests reach the gateway.
