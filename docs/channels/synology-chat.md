---
summary: "Synology Chat webhook setup and NodoAssist config"
read_when:
  - Setting up Synology Chat with NodoAssist
  - Debugging Synology Chat webhook routing
title: "Synology Chat"
---

Synology Chat connects to NodoAssist through a webhook pair: a Synology Chat outgoing webhook posts inbound direct messages to the Gateway, and replies go back through a Synology Chat incoming webhook.
...
```bash
nodoassist plugins install @nodoassist/synology-chat
```

Local checkout (when running from a git repo):

```bash
nodoassist plugins install ./path/to/local/synology-chat-plugin
```
...
3. Point the outgoing webhook URL to your NodoAssist Gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in NodoAssist. Synology Chat appears in the same channel setup list in both flows:
   - Guided: `nodoassist onboard` or `nodoassist channels add`
   - Direct: `nodoassist channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart the Gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- NodoAssist accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
```bash
nodoassist message send --channel synology-chat --target 123456 --message "Hello from NodoAssist"
nodoassist message send --channel synology-chat --target synology-chat:123456 --message "Hello again"
nodoassist message send --channel synology-chat --target synology:123456 --message "Short prefix"
```

Outbound text is chunked at 2000 characters. Media sends are supported by URL-based file delivery: the NAS downloads and attaches the file (max 32 MB). Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before NodoAssist forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. NodoAssist rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached NodoAssist
- `Rate limit exceeded`:
