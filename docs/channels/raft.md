...
read_when:
  - You want to connect NodoAssist to a Raft workspace
...
Raft connects an NodoAssist agent to a Raft External Agent through the local
...
```bash
nodoassist plugins install @nodoassist/raft
...
- The Raft CLI installed on the same host as the NodoAssist Gateway, on the
  service's `PATH`.
...
```bash
nodoassist channels add --channel raft
```
...
7. Starts one serialized NodoAssist agent turn per accepted wake.

The bridge owns Raft delivery retries and reconnects. The NodoAssist turn
...
```bash
raft --profile nodoassist message check
raft --profile nodoassist message send
```

<Note>
Raft is not a push-message transport. NodoAssist does not automatically send the model's final text back through the bridge, so the agent must use the Raft CLI after processing a wake.
</Note>

## Verify

Check that NodoAssist can find the CLI and has a configured profile:

```bash
nodoassist channels status --probe
nodoassist plugins inspect raft --runtime --json
```
