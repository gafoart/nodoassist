...
Nextcloud Talk is a downloadable channel plugin (`@nodoassist/nextcloud-talk`) that connects NodoAssist to a self-hosted Nextcloud instance through a Talk webhook bot. Direct messages, rooms, reactions, and markdown messages are supported; media goes out as URLs.

## Install

```bash
nodoassist plugins install @nodoassist/nextcloud-talk
```
...
```bash
nodoassist plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "NodoAssist" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure NodoAssist:
   ...
   ```bash
   nodoassist channels add --channel nextcloud-talk \
...
- With `apiUser`/`apiPassword` and `webhookPublicUrl` set, `nodoassist channels status` probes the bot and warns when the `response` feature is missing.

## Access control (DMs)

- Default: `channels.nextcloud-talk.dmPolicy = "pairing"`. Unknown senders get a pairing code.
- Approve via:
  - `nodoassist pairing list nextcloud-talk`
  - `nodoassist pairing approve nextcloud-talk <CODE>`
