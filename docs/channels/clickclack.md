...
read_when:
  - Connecting NodoAssist to a ClickClack workspace
  - Testing ClickClack bot identities
title: "ClickClack"
---

ClickClack connects NodoAssist to a self-hosted ClickClack workspace through first-class ClickClack bot tokens.

Use this when you want an NodoAssist agent to appear as a ClickClack bot user. ClickClack supports independent service bots and user-owned bots; user-owned bots keep an `owner_user_id` and receive only the token scopes you grant.
...
ClickClack in channel setup or running `nodoassist plugins enable clickclack`
...
`nodoassist plugins install @nodoassist/clickclack` follows the normal
...
```bash
nodoassist message send --channel clickclack --target channel:general --message "hello"
nodoassist message send --channel clickclack --target dm:usr_123 --message "hello"
nodoassist message send --channel clickclack --target thread:msg_123 --message "following up"
```
...
NodoAssist only needs `bot:write` for normal agent chat. Add `agent_activity:write` when enabling [agent activity rows](#agent-activity-rows).
