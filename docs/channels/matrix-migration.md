---
summary: "How NodoAssist upgrades the previous Matrix plugin in place, including encrypted-state recovery limits and manual recovery steps."
read_when:
...
- the plugin stays `@nodoassist/matrix`
- the channel stays `matrix`
- your config stays under `channels.matrix`
- cached credentials stay under `~/.nodoassist/credentials/matrix/`
- runtime state stays under `~/.nodoassist/matrix/`

You do not need to rename config keys or reinstall the plugin under a new name.
The root `nodoassist` package no longer bundles Matrix runtime code or Matrix SDK
dependencies. If `nodoassist channels status` shows Matrix is configured but the
plugin is not installed, run `nodoassist doctor --fix` or
`nodoassist plugins install @nodoassist/matrix`; do not install Matrix SDK packages
into the root NodoAssist package.

## What the migration does automatically

Matrix migration runs when the gateway starts (through the loaded Matrix plugin), when you run [`nodoassist doctor --fix`](/gateway/doctor), and as a fallback when the Matrix client starts and still finds old on-disk state. Before any actionable migration step mutates on-disk state, NodoAssist creates or reuses a focused recovery snapshot.

When you use `nodoassist update`, the exact trigger depends on how NodoAssist is installed:

- source installs run a non-interactive `nodoassist doctor --fix` pass during the update flow, then restart the gateway by default
- package-manager installs update the package, run `nodoassist doctor --non-interactive --fix`, then rely on the default gateway restart so startup can finish Matrix migration
- if you use `nodoassist update --no-restart`, startup-backed Matrix migration is deferred until you later run `nodoassist doctor --fix` and restart the gateway

Automatic migration covers:

- creating or reusing a pre-migration snapshot under `~/Backups/nodoassist-migrations/`
...
- NodoAssist writes a marker file at `~/.nodoassist/matrix/migration-snapshot.json` after a successful snapshot so later startup and repair passes can reuse the same archive.
- These automatic Matrix migration snapshots back up config + state only (`includeWorkspace: false`).
- If Matrix only has warning-only migration state, for example because `userId` or `accessToken` is still missing, NodoAssist does not create the snapshot yet because no Matrix mutation is actionable.
- If the snapshot step fails, NodoAssist skips Matrix migration for that run instead of mutating state without a recovery point.

About multi-account upgrades:

- the flat Matrix store (`~/.nodoassist/matrix/bot-storage.json` and `~/.nodoassist/matrix/crypto/`) came from a single-store layout, so NodoAssist can only migrate it into one resolved Matrix account target
...
NodoAssist cannot automatically recover:
...
- custom plugin path installs that are pinned to a repo path instead of the standard Matrix package (surfaced by `nodoassist doctor`)
...
1. Update NodoAssist and the Matrix plugin normally.
   Prefer plain `nodoassist update` without `--no-restart` so startup can finish the Matrix migration immediately.
2. Run:
...
4. Check current verification and backup state:
   ...
   ```bash
   nodoassist matrix verify status
   nodoassist matrix verify backup status
   ```
...
6. If NodoAssist tells you a recovery key is needed, run the command for the matching account:

   ```bash
   printf '%s\n' "$MATRIX_RECOVERY_KEY" | nodoassist matrix verify backup restore --recovery-key-stdin
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | nodoassist matrix verify backup restore --recovery-key-stdin --account assistant
   ```

7. If this device is still unverified, run the command for the matching account:

   ```bash
   printf '%s\n' "$MATRIX_RECOVERY_KEY" | nodoassist matrix verify device --recovery-key-stdin
   printf '%s\n' "$MATRIX_RECOVERY_KEY_ASSISTANT" | nodoassist matrix verify device --recovery-key-stdin --account assistant
   ```
...
8. If you are intentionally abandoning unrecoverable old history and want a fresh backup baseline for future messages, run:
   ...
   ```bash
   nodoassist matrix verify backup reset --yes
   ```
...
9. If no server-side key backup exists yet, create one for future recoveries:
   ...
   ```bash
   nodoassist matrix verify bootstrap
   ```
...
1. Startup or `nodoassist doctor --fix` creates or reuses the pre-migration snapshot if encrypted migration is actionable, then inspects the old Matrix rust crypto store through the crypto inspector bundled with the Matrix plugin.
2. If a backup decryption key is found, NodoAssist imports it into Matrix SQLite state and marks room-key restore as pending.
3. On the next Matrix startup, NodoAssist restores backed-up room keys into the new crypto store automatically. Pending restore state is also picked up from sibling token-hash storage roots when the access token rotated in between.

If the old store reports room keys that were never backed up, NodoAssist warns instead of pretending recovery succeeded.
...
- Meaning: old Matrix state exists, but NodoAssist cannot map it to a current Matrix account because Matrix is not configured.
- What to do: configure `channels.matrix`, then rerun `nodoassist doctor --fix` or restart the gateway.

`Legacy Matrix state detected at ... but the new account-scoped target could not be resolved yet (need homeserver, userId, and access token for channels.matrix...).`

- Meaning: NodoAssist found old state, but it still cannot determine the exact current account/device root.
- What to do: start the gateway once with a working Matrix login, or rerun `nodoassist doctor --fix` after cached credentials exist.

`Legacy Matrix state detected at ... but multiple Matrix accounts are configured and channels.matrix.defaultAccount is not set.`

- Meaning: NodoAssist found one shared flat Matrix store, but it refuses to guess which named Matrix account should receive it.
- What to do: set `channels.matrix.defaultAccount` to the intended account, then rerun `nodoassist doctor --fix` or restart the gateway.
...
- Meaning: the new account-scoped location already has a sync or crypto store, so NodoAssist did not overwrite it automatically.
...
- Meaning: NodoAssist tried to move old Matrix state but the filesystem operation failed.
- What to do: inspect filesystem permissions and disk state, then rerun `nodoassist doctor --fix`.

`Matrix migration warnings are present, but no on-disk Matrix mutation is actionable yet. No pre-migration snapshot was needed.`

- Meaning: NodoAssist detected old Matrix state, but the migration is still blocked on missing identity or credential data. Startup logs this as `matrix: migration remains in a warning-only state; no pre-migration snapshot was needed yet`.
- What to do: finish Matrix login or config setup, then rerun `nodoassist doctor --fix` or restart the gateway.

`Legacy Matrix encrypted state was detected, but the Matrix crypto inspector is unavailable.`

- Meaning: NodoAssist found old encrypted Matrix state, but the Matrix plugin build is missing the crypto inspector module that inspects the old rust crypto store.
- What to do: reinstall or repair the Matrix plugin (`nodoassist plugins install @nodoassist/matrix`, or `nodoassist plugins install ./path/to/local/matrix-plugin` for a repo checkout), then rerun `nodoassist doctor --fix` or restart the gateway.

`- Failed creating a Matrix migration snapshot before repair: ...`

`- Skipping Matrix migration changes for now. Resolve the snapshot failure, then rerun "nodoassist doctor --fix".`

- Meaning: NodoAssist refused to mutate Matrix state because it could not create the recovery snapshot first.
- What to do: resolve the backup error, then rerun `nodoassist doctor --fix` or restart the gateway.

`Failed migrating legacy Matrix client storage: ...`

- Meaning: the Matrix client-side fallback found old storage, but the migration failed. NodoAssist rolls back completed moves and aborts that fallback instead of silently starting with a fresh store. This error also appears when the flat store targets a different account than the one currently starting.
...
- What to do: reinstall with `nodoassist plugins install @nodoassist/matrix` when you want to return to the default Matrix plugin.
...
- Meaning: the old crypto store does not record which Matrix device it belonged to, so NodoAssist cannot inspect it safely.
- What to do: old encrypted history cannot be recovered automatically; NodoAssist continues without it.

`Legacy Matrix encrypted state for account "..." has backed-up room keys, but no local backup decryption key was found. Ask the operator to run "nodoassist matrix verify backup restore --recovery-key <key>" after upgrade if they have the recovery key.`

- Meaning: backup exists, but NodoAssist could not recover the recovery key automatically.
- What to do: run `printf '%s\n' "$MATRIX_RECOVERY_KEY" | nodoassist matrix verify backup restore --recovery-key-stdin` (preferred over passing the key as an argument).

`Failed inspecting legacy Matrix encrypted state for account "..." (...): ...`

- Meaning: NodoAssist found the old encrypted store, but it could not inspect it safely enough to prepare recovery.
- What to do: rerun `nodoassist doctor --fix`. If it repeats, keep the old state directory intact and recover using another verified Matrix client plus `printf '%s\n' "$MATRIX_RECOVERY_KEY" | nodoassist matrix verify backup restore --recovery-key-stdin`.
...
- Meaning: NodoAssist detected a backup key conflict and refused to overwrite the current recovery-key state automatically.
...
- What to do: run `nodoassist matrix verify backup status`, then retry with `printf '%s\n' "$MATRIX_RECOVERY_KEY" | nodoassist matrix verify backup restore --recovery-key-stdin` if needed.

### Manual recovery messages

`nodoassist matrix verify status` and `nodoassist matrix verify backup status` print a `Backup issue:` line plus `Next steps:` guidance when the room-key backup is not healthy on this device:

| Backup issue                                                          | Meaning                                            | Fix                                                                                                                                       |
| --------------------------------------------------------------------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `no room-key backup exists on the homeserver`                         | nothing to restore from                            | `nodoassist matrix verify bootstrap` to create a room key backup                                                                          |
| `backup decryption key is not loaded on this device`                  | key exists but is not active here                  | `nodoassist matrix verify backup restore`; if it still cannot load the key, pipe the recovery key via `--recovery-key-stdin`              |
...
| `backup exists but is not active on this device`                      | server backup present, local session inactive      | verify the device first, then recheck with `nodoassist matrix verify backup status`                                                       |
| `backup trust state could not be fully determined`                    | diagnostics were inconclusive                      | `nodoassist matrix verify status --verbose`                                                                                               |
...
- What to do: run `nodoassist matrix verify self`, accept the request in another Matrix client, compare the SAS, and type `yes` only when it matches. Use `printf '%s\n' "$MATRIX_RECOVERY_KEY" | nodoassist matrix verify bootstrap --recovery-key-stdin --force-reset-cross-signing` only when you intentionally want to replace the current cross-signing identity.

If you accept losing unrecoverable old encrypted history, you can instead reset the
current backup baseline with `nodoassist matrix verify backup reset --yes`. When the
...
- What to do: reinstall with `nodoassist plugins install @nodoassist/matrix`, or if you are running from a repo checkout, `nodoassist plugins install ./path/to/local/matrix-plugin`. `nodoassist doctor --fix` can also remove the stale Matrix plugin references for you.
...
```bash
nodoassist matrix verify status --verbose
nodoassist matrix verify backup status --verbose
printf '%s\n' "$MATRIX_RECOVERY_KEY" | nodoassist matrix verify backup restore --recovery-key-stdin --verbose
```
