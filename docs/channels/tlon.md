...
Tlon is a decentralized messenger built on Urbit. NodoAssist connects to your Urbit ship and
...
Tlon ships bundled in current NodoAssist releases; packaged builds do not need a separate install.

On an older build or custom install that excludes it, install from npm:

```bash
nodoassist plugins install @nodoassist/tlon
```

Use the bare package name to track the current release tag. Pin a version (`@nodoassist/tlon@x.y.z`)
only for reproducible installs.

From a local checkout:

```bash
nodoassist plugins install ./path/to/local/tlon-plugin
```
...
```bash
nodoassist channels add --channel tlon --ship ~sampel-palnet --url https://your-ship-host --code lidlut-tabwed-pillex-ridrup
```
...
NodoAssist blocks private/internal hostnames and IP ranges for SSRF protection by default. If your
...
<Note>
`channels.tlon.allowPrivateNetwork` (flat key) is retired. `nodoassist doctor --fix` moves it to
...
prompt to yes and writes `true` explicitly. With it on, NodoAssist scries joined groups on startup,
...
Use with `nodoassist message send` or cron delivery:
