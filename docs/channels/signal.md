...
Signal is a downloadable channel plugin (`@nodoassist/signal`). The gateway talks to `signal-cli` over HTTP: either the native daemon (JSON-RPC + SSE) or the [bbernhard/signal-cli-rest-api](https://github.com/bbernhard/signal-cli-rest-api) container (REST + WebSocket). NodoAssist does not embed libsignal.
...
Bare plugin specs try ClawHub first, then npm fallback. Force a source with `nodoassist plugins install clawhub:@nodoassist/signal` or `npm:@nodoassist/signal`. `plugins install` registers and enables the plugin; no separate `enable` step is needed. See [Plugins](/tools/plugin) for general install rules.
...
<Steps>
  ...
  <Step title="Link or register the account">
    - **QR link (fastest):** `signal-cli link -n "NodoAssist"`, then scan with Signal. See [Path A](#setup-path-a-link-existing-signal-account-qr).
  ...
  <Step title="Verify and pair">
    ```bash
    nodoassist gateway call channels.status --params '{"probe":true}'
    ```
    Send a first DM and approve pairing: `nodoassist pairing approve signal <CODE>`.
  </Step>
</Steps>
...
1. Install `signal-cli` (JVM or native build), or let `nodoassist channels add` install it for you.
2. Link a bot account: `signal-cli link -n "NodoAssist"`, then scan the QR in Signal.
...
4. Configure NodoAssist, restart the gateway, verify the channel:

```bash
# If you run the gateway as a user systemd service:
systemctl --user restart nodoassist-gateway.service
...
5. Pair your DM sender:
   ...
   - Approve on the server: `nodoassist pairing approve signal <PAIRING_CODE>`.
...
To manage `signal-cli` yourself (slow JVM cold starts, container init, shared CPUs), run the daemon separately and point NodoAssist at it:
...
This skips auto-spawn and NodoAssist's startup wait. For slow auto-spawned starts, set `channels.signal.startupTimeoutMs`.
...
- Register or link your Signal account inside the container before connecting NodoAssist.
...
`apiMode` controls which protocol NodoAssist uses:
...
When `apiMode` is `"auto"`, NodoAssist caches the detected mode for 30 seconds per daemon URL to avoid repeated probes (native wins when both transports are healthy). Container receive is only selected for streaming after `/v1/receive/{account}` upgrades to WebSocket, which requires `MODE=json-rpc`.

Container mode supports the same Signal operations as native mode where the container exposes matching APIs: sends, receives, attachments, typing indicators, read/viewed receipts, reactions, groups, and styled text. NodoAssist translates native Signal RPC calls into the container's REST payloads, including `group.{base64(internal_id)}` group IDs and `text_mode: "styled"` for formatted text.

Operational notes:

- Use `autoStart: false` with container mode; NodoAssist should not spawn a native daemon when `apiMode: "container"` is selected.
- Use `MODE=json-rpc` for receiving. `MODE=normal` can make `/v1/about` look healthy, but `/v1/receive/{account}` will not WebSocket-upgrade, so NodoAssist will not select container receive streaming in `auto` mode.
...
- Approve via `nodoassist pairing list signal` and `nodoassist pairing approve signal <CODE>`.
...
- Replies to inbound messages include native Signal quote metadata when the backend accepts the inbound timestamp and author; if quote metadata is missing or rejected, NodoAssist sends the reply as a normal message.
...
- **Typing indicators**: NodoAssist sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, NodoAssist forwards read receipts for allowed DMs.
...
Configure aliases for stable names on recurring Signal targets. Aliases are NodoAssist-side config only; they do not create or edit Signal contacts.
...
```bash
nodoassist message send --channel signal --target signal:ops --message "Deployment is complete"
```
...
`nodoassist directory peers list --channel signal` and `nodoassist directory groups list --channel signal` list configured aliases. The Signal directory is config-backed; it does not live-query Signal contacts or mutate the Signal account.
...
```bash
nodoassist pairing list signal
```
...
- Config validation errors after edits: run `nodoassist doctor --fix`.
...
nodoassist pairing list signal
pgrep -af signal-cli
grep -i "signal" "/tmp/nodoassist/nodoassist-$(date +%Y-%m-%d).log" | tail -20
```
...
- `channels.signal.aliases`: NodoAssist-side aliases for DM or group delivery targets.
