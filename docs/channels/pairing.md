...
read_when:
  ...
  - Reviewing NodoAssist security posture
title: "Pairing"
---

"Pairing" is NodoAssist's explicit access approval step.
...
Supported channels (any installed channel plugin that declares pairing; external plugins such as `nodoassist-weixin` can add more): `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `signal`, `slack`, `sms`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.
...
Stored under `~/.nodoassist/credentials/`:
...
3. On your phone, open the NodoAssist app → **Settings** → **Gateway**.
4. Scan the QR code or paste the setup code, then connect.

Official NodoAssist iOS and Android apps are approved automatically when their
...
3. On your phone, open the NodoAssist iOS app → Settings → Gateway.
...
For `gateway.bind=lan` setup URLs, NodoAssist detects persistent Tailscale Serve
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, NodoAssist keeps the existing approval as-is and creates a fresh pending upgrade request. Use `nodoassist devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
Stored under `~/.nodoassist/devices/`:
...
- The legacy `node.pair.*` API (CLI: `nodoassist nodes pending|approve|reject|remove|rename`) is a
