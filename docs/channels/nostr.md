...
read_when:
  - You want NodoAssist to receive DMs via Nostr
  - You're setting up decentralized messaging
title: "Nostr"
---

Nostr is a downloadable channel plugin (`@nodoassist/nostr`) that lets NodoAssist receive and answer NIP-04 encrypted direct messages over Nostr relays. One account per gateway; DMs only.

## Install

```bash
nodoassist plugins install @nodoassist/nostr
```
...
```bash
nodoassist plugins install --link <path-to-local-nostr-plugin>
```

Restart the gateway after installing or enabling plugins. Onboarding (`nodoassist onboard`) and `nodoassist channels add` surface Nostr from the shared channel catalog once the plugin is installed.

### Non-interactive setup

```bash
nodoassist channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY"
nodoassist channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "nodoassist@example.com",
        lud16: "nodoassist@example.com",
      },
    },
  },
}
...
1. Note the bot pubkey from gateway logs or `nodoassist channels status` (hex; convert to npub in your client if needed).
