...
NodoAssist routes replies **back to the channel where a message came from**. The
...
NodoAssist infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, NodoAssist still records inbound session metadata, but it
...
when a guarded path must not create a new NodoAssist session. In that mode,
NodoAssist may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when NodoAssist would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.nodoassist/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.nodoassist`):

- `~/.nodoassist/agents/<agentId>/sessions/sessions.json`
