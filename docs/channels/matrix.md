...
read_when:
  - Setting up Matrix in NodoAssist
  - Configuring Matrix E2EE and verification
title: "Matrix"
---

Matrix is a downloadable channel plugin (`@nodoassist/matrix`) built on the official `matrix-js-sdk`. It supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Install

```bash
nodoassist plugins install @nodoassist/matrix
```

Bare plugin specs try ClawHub first, then npm fallback. Force a source with `nodoassist plugins install clawhub:@nodoassist/matrix` or `npm:@nodoassist/matrix`. From a local checkout: `nodoassist plugins install ./path/to/local/matrix-plugin`.
...
nodoassist configure --section channels
```

The wizard asks for homeserver URL, auth method (token or password), user ID (password auth only), optional device name, whether to enable E2EE, and room access/auto-join. If matching `MATRIX_*` env vars already exist and the account has no saved auth, the wizard offers an env-var shortcut. Resolve room names before saving an allowlist with `nodoassist channels resolve --channel matrix "Project Room"`. Enabling E2EE in the wizard runs the same bootstrap as [`nodoassist matrix encryption setup`](#encryption-and-verification).
...
Matrix caches credentials under `~/.nodoassist/credentials/matrix/`: `credentials.json` for the default account, `credentials-<account>.json` for named accounts. When cached credentials exist, NodoAssist treats Matrix as configured even without an `accessToken` in the config file - this covers setup, `nodoassist doctor`, and channel-status probes.
...
Matrix reply streaming is opt-in. `streaming` controls how NodoAssist delivers the in-flight assistant reply; `blockStreaming` controls whether each completed block is kept as its own Matrix message.
...
- If a preview grows past Matrix's per-event size limit, NodoAssist stops preview streaming and falls back to final-only delivery.
- Media replies always send attachments normally; if a stale preview cannot be reused safely, NodoAssist redacts it before sending the final media reply.
...
- In encrypted rooms, NodoAssist decrypts the attachment through the existing Matrix media path before transcription.
...
Matrix native approval prompts are normal `m.room.message` events with NodoAssist-specific content under the `com.nodoassist.approval` key. Stock clients still render the text body; NodoAssist-aware clients can read the structured approval id, kind, state, decisions, and exec/plugin details.

When a prompt is too long for one Matrix event, NodoAssist chunks the visible text and attaches `com.nodoassist.approval` to the first chunk only. Allow/deny reactions bind to that first event, so long prompts keep the same approval target as single-event prompts.
...
By default, Matrix messages from other configured NodoAssist Matrix accounts are ignored. Use `allowBots` to intentionally allow inter-agent traffic:
...
- NodoAssist still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix has no native bot flag; NodoAssist treats "bot-authored" as "sent by another configured Matrix account on this NodoAssist gateway".
...
All `nodoassist matrix` commands accept `--verbose` (full diagnostics), `--json` (machine-readable output), and `--account <id>` (multi-account setups). Output is concise by default.

### Enable encryption

```bash
nodoassist matrix encryption setup
```
...
nodoassist matrix verify status --include-recovery-key --json
```
...
The literal-key form `nodoassist matrix verify device "<recovery-key>"` also works, but the key ends up in shell history.
...
If the homeserver requires UIA to upload cross-signing keys, NodoAssist tries no-auth first, then `m.login.dummy`, then `m.login.password` (requires `channels.matrix.password`).
...
```bash
nodoassist matrix verify list
```

Lists pending verification requests for the selected account.

```bash
nodoassist matrix verify request --own-user
nodoassist matrix verify request --user-id @ops:example.org --device-id ABCDEF
```
...
| Command                                      | Purpose                                                             |
| -------------------------------------------- | ------------------------------------------------------------------- |
| `nodoassist matrix verify accept <id>`       | Accept an inbound request                                           |
| `nodoassist matrix verify start <id>`        | Start the SAS flow                                                  |
| `nodoassist matrix verify sas <id>`          | Print the SAS emoji or decimals                                     |
| `nodoassist matrix verify confirm-sas <id>`  | Confirm that the SAS matches what the other client shows            |
| `nodoassist matrix verify mismatch-sas <id>` | Reject the SAS when the emoji or decimals do not match              |
| `nodoassist matrix verify cancel <id>`       | Cancel; takes optional `--reason <text>` and `--code <matrix-code>` |
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    ...
    Startup also runs a conservative crypto bootstrap pass reusing the current secret storage and cross-signing identity. If bootstrap state is broken, NodoAssist attempts a guarded repair even without `channels.matrix.password`; if the homeserver requires password UIA, startup logs a warning and stays non-fatal. Already-owner-signed devices are preserved.
  ...
  <Accordion title="Verification notices">
    ...
    Incoming requests from another Matrix client are tracked and auto-accepted. For self-verification, NodoAssist starts the SAS flow automatically and confirms its own side once emoji verification is available - you still need to compare and confirm "They match" in your Matrix client.
  ...
  <Accordion title="Deleted or invalid Matrix device">
    If `verify status` says the current device is no longer listed on the homeserver, create a new NodoAssist Matrix device. For password login:
...
nodoassist matrix account add \
  ...
  --password '<password>' \
  --device-name NodoAssist-Gateway
```

    For token auth, create a fresh access token in your Matrix client or admin UI, then update NodoAssist:
...
```
  ...
  <Accordion title="Device hygiene">
    Old NodoAssist-managed devices can accumulate. List and prune:

```bash
nodoassist matrix devices list
nodoassist matrix devices prune-stale
```
  ...
  <Accordion title="Crypto store">
    ...
    Encrypted runtime state lives under `~/.nodoassist/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/` and includes the sync store, crypto store, recovery key, IDB snapshot, thread bindings, and startup verification state. When the token changes but the account identity stays the same, NodoAssist reuses the best existing root so prior state remains visible.

    A single older token-hash root can be a normal token-rotation continuity path. If NodoAssist logs `matrix: multiple populated token-hash storage roots detected`, inspect the account directory and archive stale sibling roots only after confirming the selected active root is healthy. Prefer moving stale roots into an `_archive/` directory over deleting them immediately.
...
```bash
nodoassist matrix profile set --name "NodoAssist Assistant"
nodoassist matrix profile set --avatar-url https://cdn.example.org/avatar.png
```
...
`dm.sessionScope` decides how Matrix DM rooms map to NodoAssist sessions:
...
- DM user-target reuse only kicks in when current session metadata proves the same DM peer on the same Matrix account; otherwise NodoAssist falls back to normal user-scoped routing.
...
When NodoAssist detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` pointing to the `/focus` escape hatch and suggesting a `dm.sessionScope` change. The notice only appears when thread bindings are enabled.
...
`--bind here` does not create a child Matrix thread. `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where NodoAssist needs to create or bind a child thread.
...
- Room history is pending-only: NodoAssist buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
nodoassist pairing list matrix
nodoassist pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging before approval, NodoAssist reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state drifts, NodoAssist can end up with stale `m.direct` mappings pointing at old solo rooms instead of the live DM. Inspect the current mapping for a peer:

```bash
nodoassist matrix direct inspect --user-id @alice:example.org
```

Repair it:

```bash
nodoassist matrix direct repair --user-id @alice:example.org
```
...
- If you have multiple accounts and one is literally named `default`, NodoAssist uses it implicitly even when `defaultAccount` is unset.
...
- When NodoAssist promotes a single-account config to multi-account during repair or setup, it preserves the existing named account if one exists or `defaultAccount` already points at one. Only Matrix auth/bootstrap keys move into the promoted account; shared delivery-policy keys stay at the top level.

See [Configuration reference](/gateway/config-channels#multi-account-all-channels) for the shared multi-account pattern.

## Private/LAN homeservers

By default, NodoAssist blocks private/internal Matrix homeservers for SSRF protection unless you opt in per account.
...
Named accounts can override the top-level default with `channels.matrix.accounts.<id>.proxy`. NodoAssist uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere NodoAssist asks for a room or user target:
...
Matrix room IDs are case-sensitive. Use the exact room ID casing from Matrix when configuring explicit delivery targets, cron jobs, bindings, or allowlists. NodoAssist keeps internal session keys canonical for storage, so those lowercase keys are not a reliable source for Matrix delivery IDs.
