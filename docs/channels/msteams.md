...
Microsoft Teams ships as a bundled plugin in current NodoAssist releases; no separate install is required in the normal packaged build.

On an older build or a custom install that excludes bundled Teams, install the npm package directly:

```bash
nodoassist plugins install @nodoassist/msteams
```
...
```bash
nodoassist plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-nodoassist-bot --allow-anonymous
devtunnel port create my-nodoassist-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-nodoassist-bot
...
**4. Configure NodoAssist** using the credentials from the output:
...
- Talk to NodoAssist via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists; they can change. NodoAssist disables direct name matching by default; opt in with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, NodoAssist resolves team/channel and user allowlist names to IDs (when Graph permissions allow) and logs the mapping. Unresolved names are kept as typed but ignored for routing unless `channels.msteams.dangerouslyAllowNameMatching: true` is set.
...
5. Configure `msteams` in `~/.nodoassist/nodoassist.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                      |
   | ------------------ | ---------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `nodoassist-msteams` (must be unique) |
...
For production, NodoAssist supports **federated authentication** as an alternative to client secrets, via `channels.msteams.authType: "federated"`. Two methods:
...
3. At runtime, NodoAssist uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint.
...
**Default:** when `authType` is unset, NodoAssist uses client-secret authentication (`appPassword`). Existing configs keep working unchanged.
...
These auth-related config keys can be set via environment variables instead of `nodoassist.json` (other config keys, such as `groupPolicy` or `historyLimit`, are config-only):
...
NodoAssist exposes a Graph-backed `member-info` message action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, job title, UPN, office location) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "NodoAssist in Teams", full: "NodoAssist in Teams" },
...
Teams delivers messages via HTTP webhook. NodoAssist applies fixed HTTP server timeouts to that webhook listener: 30s inactivity, 30s total request, 15s to receive headers. If agent processing takes longer than the client's own retry window, you may see:

- Teams retrying the message (causing duplicates).
- Dropped replies.

NodoAssist acks the webhook quickly (before agent processing finishes) and sends replies proactively once the agent responds, but very slow agent runs can still surface retries/duplicates on the Teams side.
...
| Teams environment | NodoAssist config                                           | Proactive `serviceUrl`                             |
| ----------------- | ----------------------------------------------------------- | -------------------------------------------------- |
...
When `replyStyle: "thread"` is in effect and the bot was @mentioned from inside a channel thread, NodoAssist re-attaches the original thread root to the outbound conversation reference (`19:...@thread.tacv2;messageid=<root>`) so the reply lands inside the same thread. This holds for both live (in-turn) sends and proactive sends made after the Bot Framework turn context has expired (e.g., long-running agents, queued tool-call replies via `mcp__nodoassist__message`).
...
By default, NodoAssist only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/NodoAssistShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

NodoAssist sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `nodoassist message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in NodoAssist plugin-state SQLite under `state/nodoassist.sqlite`.
- Existing `msteams-polls.json` files are imported by `nodoassist doctor --fix`, not by the running plugin.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries, and there is no poll-results CLI yet.

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. NodoAssist renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so NodoAssist downgrades them to readable text before delivery.
...
```bash
nodoassist message send --channel msteams \
...
nodoassist message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
nodoassist message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
nodoassist message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
nodoassist message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- Proactive messages are only possible **after** a user has interacted, because NodoAssist stores conversation references at that point.
...
- **Ignore** the `groupId` query parameter for NodoAssist routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
