// Bench Gateway Restart script supports NodoAssist repository automation.
...
const GATEWAY_CASES: readonly GatewayBenchCase[] = [
  {
    ...
    env: { NODOASSIST_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe on",
    env: { NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE: "1", NODOASSIST_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "skipChannelsNoAcpxProbe",
    name: "gateway restart, skip channels, ACPX startup probe off",
    env: { NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE: "0", NODOASSIST_SKIP_CHANNELS: "1" },
    config: BASE_CONFIG,
  },
  {
    id: "default",
...
function printUsage(): void {
  console.log(`NodoAssist Gateway restart benchmark
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "nodoassist-gateway-restart-bench-"));
