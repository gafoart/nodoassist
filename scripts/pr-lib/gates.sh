...
pin_worktree_bundled_plugins_dir() {
  ...
  # PR branches without the nodoassist-root node_modules-boundary fix still test
  # their own extensions instead of the primary checkout's stale trees.
  export NODOASSIST_BUNDLED_PLUGINS_DIR="${NODOASSIST_BUNDLED_PLUGINS_DIR:-$PWD/extensions}"
}

resolve_pr_gates_remote_mode() {
  case "${NODOASSIST_PR_GATES_REMOTE:-}" in
    "")
      printf 'local\n'
      ;;
    testbox)
      ...
      ;;
    *)
      echo "Unsupported NODOASSIST_PR_GATES_REMOTE=${NODOASSIST_PR_GATES_REMOTE} (supported: testbox)." >&2
      return 1
      ;;
  esac
}
...
acquire_pr_gates_lock() {
  ...
  if [ "${NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD:-}" = "1" ]; then
    return 0
  fi
  ...
  export NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD=1
  export NODOASSIST_TSGO_HEAVY_CHECK_LOCK_HELD=1
  export NODOASSIST_OXLINT_SKIP_LOCK=1
}
...
release_pr_gates_lock() {
  ...
  PR_GATES_LOCK_STATUS_FILE=""
  unset NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD NODOASSIST_TSGO_HEAVY_CHECK_LOCK_HELD NODOASSIST_OXLINT_SKIP_LOCK
}
...
read_remote_testbox_gate_run_url() {
  ...
  if [ -z "$expected_repo" ] || [ "$expected_repo" = "$pr_url" ]; then
    expected_repo="nodoassist/nodoassist"
  fi
...
prepare_gates() {
  ...
  local gates_remote_mode
  gates_remote_mode=$(resolve_pr_gates_remote_mode)
  if [ "$gates_remote_mode" = "testbox" ] && [ "${NODOASSIST_TESTBOX:-}" = "1" ]; then
    echo "NODOASSIST_PR_GATES_REMOTE=testbox conflicts with NODOASSIST_TESTBOX=1; hosted PR gates already own remote proof."
    exit 2
  fi
  ...
  if [ "$has_changelog_update" = "true" ]; then
    if ! root_changelog_update_allowed_for_pr; then
      ...
      echo "Set NODOASSIST_ALLOW_ROOT_CHANGELOG_PR=1 only for explicit release automation or maintainer release closeout."
      exit 1
    fi
  ...
  if [ "${NODOASSIST_TESTBOX:-}" != "1" ] && [ "$docs_only" = "true" ] && [ -n "$previous_last_verified_head" ] && git merge-base --is-ancestor "$previous_last_verified_head" HEAD 2>/dev/null; then
  ...
  if [ "${NODOASSIST_TESTBOX:-}" = "1" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      release_pr_gates_lock
      gates_mode="remote_testbox"
      echo "Running pnpm test on Blacksmith Testbox (NODOASSIST_PR_GATES_REMOTE=testbox)."
      run_remote_testbox_full_test_gate \
      ...
      gates_mode="full"
      if [ -n "${NODOASSIST_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with NODOASSIST_VITEST_MAX_WORKERS=$NODOASSIST_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env NODOASSIST_VITEST_MAX_WORKERS="$NODOASSIST_VITEST_MAX_WORKERS" pnpm test
      else
