...
root_changelog_update_allowed_for_pr() {
  case "${NODOASSIST_ALLOW_ROOT_CHANGELOG_PR:-}" in
...
pr_contributor_allows_human_trailers() {
  ...
  case "$normalized" in
    ""|"null"|"app/"*|"codex"|"nodoassist"|"clawsweeper"|"nodoassist-clawsweeper"|"clawsweeper[bot]"|"nodoassist-clawsweeper[bot]"|"steipete")
      return 1
      ;;
  esac
