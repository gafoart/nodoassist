// Sync Plugin Versions script supports NodoAssist repository automation.
...
const NODOASSIST_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;
const VERSION_ALIGNED_PACKAGE_DIRS = ["packages/ai"] as const;

function syncNodoAssistDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.nodoassist;
  if (!current || current === "workspace:*" || !NODOASSIST_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  ...
  deps.nodoassist = next;
  return true;
}

function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.nodoassist?.compat;
  const current = compat?.pluginApi;
  if (!current || !NODOASSIST_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildNodoAssistVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.nodoassist?.build;
  const current = build?.nodoassistVersion;
  if (!current) {
    return false;
  }
  ...
  build.nodoassistVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncNodoAssistDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncNodoAssistDependencyRange(
    ...
    const buildNodoAssistVersionChanged = syncBuildNodoAssistVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildNodoAssistVersionChanged;
    if (!packageChanged) {
