// Write Plugin Sdk Entry Dts script supports NodoAssist repository automation.
...
const USE_CANONICAL_DECLARATIONS = process.env.NODOASSIST_PLUGIN_SDK_CANONICAL_DTS === "1";

function isBareImportSpecifier(id: string): boolean {
  if (
    id === "@nodoassist/llm-core" ||
    id.startsWith("@nodoassist/llm-core/") ||
    id === "@nodoassist/model-catalog-core/model-catalog-types" ||
...
if (USE_CANONICAL_DECLARATIONS) {
  ...
  const flatDeclarationTempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "nodoassist-plugin-sdk-dts-"),
  );
  try {
    await build({
      clean: true,
