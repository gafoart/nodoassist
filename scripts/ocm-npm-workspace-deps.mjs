...
const WORKSPACE_DIRS_ENV = "NODOASSIST_OCM_WORKSPACE_DEPENDENCY_DIRS";
const REAL_NPM_ENV = "NODOASSIST_OCM_REAL_NPM_BIN";
...
export function buildInstallManifest(rootArchive, workspacePackages) {
  return {
    private: true,
    dependencies: {
      nodoassist: pathToFileURL(rootArchive).href,
      ...Object.fromEntries(
...
function patchRootArchiveWorkspaceDependencies(rootArchive, workspacePackages, outputDir) {
  ...
  const patchedArchive = join(outputDir, "nodoassist-root-patched.tgz");
...
function main() {
  ...
  const packDir = mkdtempSync(join(tmpdir(), "nodoassist-ocm-workspace-deps-"));
  try {
