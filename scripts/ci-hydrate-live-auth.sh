...
profile_path="${1:-${RUNNER_TEMP:-/tmp}/nodoassist-live.profile}"
...
activate_claude_oauth_access_token() {
  local credentials="${NODOASSIST_CLAUDE_CREDENTIALS_JSON:-}"
...
write_secret_file "$HOME/.codex/auth.json" NODOASSIST_CODEX_AUTH_JSON
write_secret_file "$HOME/.codex/config.toml" NODOASSIST_CODEX_CONFIG_TOML
write_secret_file "$HOME/.claude.json" NODOASSIST_CLAUDE_JSON
write_secret_file "$HOME/.claude/.credentials.json" NODOASSIST_CLAUDE_CREDENTIALS_JSON
write_secret_file "$HOME/.claude/settings.json" NODOASSIST_CLAUDE_SETTINGS_JSON
write_secret_file "$HOME/.claude/settings.local.json" NODOASSIST_CLAUDE_SETTINGS_LOCAL_JSON
write_secret_file "$HOME/.gemini/settings.json" NODOASSIST_GEMINI_SETTINGS_JSON

if [[ -n "${GITHUB_ENV:-}" ]]; then
  {
    echo "NODOASSIST_PROFILE_FILE=$profile_path"
  } >>"$GITHUB_ENV"
fi
