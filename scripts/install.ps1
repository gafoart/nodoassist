# NodoAssist Installer for Windows
...
function Complete-Install {
    ...
    throw "NodoAssist installation failed with exit code $($script:InstallExitCode)."
}

Write-Host ""
Write-Host "  NodoAssist Installer" -ForegroundColor Cyan
Write-Host ""
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:NODOASSIST_INSTALL_METHOD)) {
        $InstallMethod = $env:NODOASSIST_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:NODOASSIST_GIT_DIR)) {
        $GitDir = $env:NODOASSIST_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:NODOASSIST_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:NODOASSIST_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:NODOASSIST_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "nodoassist")
}
...
function Get-NodoAssistDepsRoot {
    ...
    return (Join-Path $localAppData "NodoAssist\deps")
}

function Get-PortableNodeRoot {
    return (Join-Path (Get-NodoAssistDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or nodoassist is not found)" -ForegroundColor Yellow
    }
}
...
function Check-ExistingNodoAssist {
    if (Get-NodoAssistCommandPath) {
        Write-Host "[*] Existing NodoAssist installation detected" -ForegroundColor Yellow
        return $true
    }
    return $false
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-NodoAssistDepsRoot) "portable-git")
}
...
function Resolve-PortableGitDownload {
    ...
    $headers = @{
        "User-Agent" = "nodoassist-installer"
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("nodoassist-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Ensure-Git {
    ...
    Write-Host ""
    Write-Host "Error: Git is required to install NodoAssist." -ForegroundColor Red
...
function Get-NodoAssistCommandPath {
    $nodoassistCmd = Get-Command nodoassist.cmd -ErrorAction SilentlyContinue
    if ($nodoassistCmd -and $nodoassistCmd.Source) {
        return $nodoassistCmd.Source
    }

    $nodoassist = Get-Command nodoassist -ErrorAction SilentlyContinue
    if ($nodoassist -and $nodoassist.Source) {
        return $nodoassist.Source
    }

    return $null
}

function Invoke-NodoAssistCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-NodoAssistCommandPath
    if (-not $commandPath) {
        throw "nodoassist command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveNodoAssistCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "nodoassist $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-NodoAssistOnPath {
    if (Get-NodoAssistCommandPath) {
        return $true
    }
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "nodoassist.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] nodoassist is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmNodoAssistInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-NodoAssistSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("nodoassist@")) {
        $normalizedTag = $normalizedTag.Substring("nodoassist@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:nodoassist/nodoassist($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/nodoassist/nodoassist(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]nodoassist/nodoassist(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/nodoassist/nodoassist(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:nodoassist/nodoassist(\.git)?($|[?#])'
    )
}
...
function Install-NodoAssist {
    if ([string]::IsNullOrWhiteSpace($Tag)) {
        $Tag = "latest"
    }
    if (Test-NodoAssistSourcePackageInstallSpec -RequestedTag $Tag) {
        Write-Host "Error: npm installs do not support NodoAssist GitHub source targets like '$Tag'." -ForegroundColor Red
    ...
    # Use nodoassist package for beta, nodoassist for stable
    $packageName = "nodoassist"
    if ($Tag -eq "beta" -or $Tag -match "^beta\.") {
        $packageName = "nodoassist"
    }
    $installSpec = Resolve-NpmNodoAssistInstallSpec -PackageName $packageName -RequestedTag $Tag
    Write-Host "[*] Installing NodoAssist ($installSpec)..." -ForegroundColor Yellow
    ...
    Write-Host "[OK] NodoAssist installed" -ForegroundColor Green
    return $true
}

# Install NodoAssist from GitHub
function Install-NodoAssistFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    Write-Host "[*] Installing NodoAssist from GitHub ($repoUrl)..." -ForegroundColor Yellow
    ...
    if (-not (Test-Path $entryPath)) {
        Write-Host "[!] NodoAssist build did not produce $entryPath" -ForegroundColor Red
        return $false
    }
    ...
    $cmdPath = Join-Path $binDir "nodoassist.cmd"
    ...
    Write-Host "[OK] NodoAssist wrapper installed to $cmdPath" -ForegroundColor Green
...
function Run-Doctor {
    ...
    try {
        Invoke-NodoAssistCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-NodoAssistCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    if (-not (Get-NodoAssistCommandPath)) {
        return
    }
    ...
    try {
        Invoke-NodoAssistCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-NodoAssistCommand gateway restart | Out-Null
        Invoke-NodoAssistCommand gateway status --json | Out-Null
        Write-Host "[OK] Gateway service refreshed" -ForegroundColor Green
    } catch {
        Write-Host "[!] Gateway service restart failed; continuing. Run: nodoassist gateway restart" -ForegroundColor Yellow
    }
}

function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:NODOASSIST_GIT_DIR)) {
        return $env:NODOASSIST_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "nodoassist")
}
...
function Main {
    ...
    $isUpgrade = Check-ExistingNodoAssist
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "nodoassist") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-NodoAssistFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "nodoassist.cmd"
        ...
        $npmInstallResults = @(Install-NodoAssist)
        if (-not (Test-BooleanSuccessResult -Results $npmInstallResults)) {
            return (Fail-Install)
        }
    }

    if (-not (Ensure-NodoAssistOnPath)) {
        Write-Host "Install completed, but NodoAssist is not on PATH yet." -ForegroundColor Yellow
        Write-Host "Open a new terminal, then run: nodoassist doctor" -ForegroundColor Cyan
        return
    }
    ...
    try {
        $installedVersion = (Invoke-NodoAssistCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.nodoassist -and $npmList.dependencies.nodoassist.version) {
                $installedVersion = $npmList.dependencies.nodoassist.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    Write-Host ""
    if ($installedVersion) {
        Write-Host "NodoAssist installed successfully ($installedVersion)!" -ForegroundColor Green
    } else {
        Write-Host "NodoAssist installed successfully!" -ForegroundColor Green
    }
    Write-Host ""
    if ($isUpgrade) {
        $updateMessages = @(
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\nodoassist.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "nodoassist doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "nodoassist onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveNodoAssistCommand onboard
        }
    }
