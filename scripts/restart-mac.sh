#!/usr/bin/env bash
# Reset NodoAssist like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${ROOT_DIR}/scripts/lib/restart-mac-gateway.sh"
APP_BUNDLE="${NODOASSIST_APP_BUNDLE:-}"
APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/NodoAssist"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/NodoAssist"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/NodoAssist"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/NodoAssist"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.nodoassist.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/nodoassist-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
LOCK_HELD=0
WAIT_FOR_LOCK=0
LOG_PATH="${NODOASSIST_RESTART_LOG:-${TMPDIR:-/tmp}/nodoassist-restart-${LOCK_KEY}.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${NODOASSIST_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.nodoassist/disable-launchagent"
ATTACH_ONLY=1
...
canonicalize_app_bundle() {
  ...
  if [[ ! -d "${APP_BUNDLE}" ]]; then
    fail "NODOASSIST_APP_BUNDLE does not exist: ${APP_BUNDLE}"
  fi
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  NODOASSIST_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node nodoassist.mjs daemon install --force --runtime node"
      log "  node nodoassist.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.nodoassist/disable-launchagent"
      log ""
...
kill_all_nodoassist() {
  ...
  [[ -z "$(nodoassist_process_pids)" ]]
}

known_nodoassist_executables() {
  if [[ -n "${APP_BUNDLE}" ]]; then
    printf '%s\n' "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
  fi
  printf '%s\n' \
    "${ROOT_DIR}/dist/NodoAssist.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "/Applications/NodoAssist.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "${DEBUG_PROCESS_PATTERN}" \
    "${LOCAL_PROCESS_PATTERN}" \
    "${RELEASE_PROCESS_PATTERN}"
}

nodoassist_process_pids() {
  local pattern=""
  while IFS= read -r pattern; do
    [[ -n "${pattern}" ]] || continue
    process_pids_matching "${pattern}"
  done < <(known_nodoassist_executables) | sort -u
}
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.nodoassist.mac 2>/dev/null || true
}

# 1) Stop launchd supervision, then kill all running instances.
stop_launch_agent
log "==> Killing existing NodoAssist instances"
if ! kill_all_nodoassist; then
  fail "NodoAssist instances did not exit after cleanup attempts"
fi
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product NodoAssist"
...
choose_app_bundle() {
  ...
  if [[ -d "${ROOT_DIR}/dist/NodoAssist.app" ]]; then
    APP_BUNDLE="$(cd "${ROOT_DIR}/dist/NodoAssist.app" && pwd -P)"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/NodoAssist.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/NodoAssist.app" ]]; then
    APP_BUNDLE="$(cd "/Applications/NodoAssist.app" && pwd -P)"
    return 0
  fi

  fail "App bundle not found. Set NODOASSIST_APP_BUNDLE to your installed NodoAssist.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nodoassist.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node nodoassist.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".nodoassist", "nodoassist.json"), "utf8");
...
if [[ -n "$(process_pids_matching "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}")" ]]; then
  log "OK: NodoAssist is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.nodoassist.gateway.plist' | head -n 40 || true"
fi
