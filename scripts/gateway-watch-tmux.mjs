...
const TMUX_CWD_ENV_KEY = "NODOASSIST_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@nodoassist.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "NODOASSIST_CONFIG_PATH",
  "NODOASSIST_DIAGNOSTICS",
  "NODOASSIST_DIAGNOSTICS_EVENT_LOOP",
  "NODOASSIST_DIAGNOSTICS_TIMELINE_PATH",
  "NODOASSIST_GATEWAY_PORT",
  "NODOASSIST_GATEWAY_RESTART_TRACE",
  "NODOASSIST_GATEWAY_STARTUP_TRACE",
  "NODOASSIST_HOME",
  "NODOASSIST_PROFILE",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.NODOASSIST_TRACE_SYNC_IO ??= "0";
    if (nextEnv.NODOASSIST_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  const profile =
    env.NODOASSIST_PROFILE ||
    readArgValue(args, "--profile") ||
    (args.includes("--dev") ? "dev" : null);
  const port = env.NODOASSIST_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "nodoassist",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `NODOASSIST_GATEWAY_WATCH_TMUX_CHILD=1`,
    `NODOASSIST_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const log = (stderr, message) => {
  stderr.write(`[nodoassist] ${message}\n`);
};
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.NODOASSIST_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.NODOASSIST_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.NODOASSIST_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
