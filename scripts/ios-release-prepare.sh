...
- regenerates apps/ios/NodoAssist.xcodeproj via xcodegen
EOF
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical NodoAssist Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi

if [[ -n "${NODOASSIST_PUSH_RELAY_BASE_URL:-}" || -n "${IOS_PUSH_RELAY_BASE_URL:-}" ]]; then
...
NODOASSIST_APP_BUNDLE_ID = ai.nodoassistfoundation.app
NODOASSIST_SHARE_BUNDLE_ID = ai.nodoassistfoundation.app.share
NODOASSIST_ACTIVITY_WIDGET_BUNDLE_ID = ai.nodoassistfoundation.app.activitywidget
NODOASSIST_WATCH_APP_BUNDLE_ID = ai.nodoassistfoundation.app.watchkitapp
NODOASSIST_CODE_SIGN_ENTITLEMENTS = Sources/NodoAssistAppAttest.entitlements
NODOASSIST_APNS_ENTITLEMENT_ENVIRONMENT = production
NODOASSIST_APP_ATTEST_ENVIRONMENT = production
NODOASSIST_PUSH_MODE = appStore
NODOASSIST_PUSH_RELAY_BASE_URL =
EOF
