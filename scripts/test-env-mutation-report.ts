#!/usr/bin/env node
// Test Env Mutation Report script supports NodoAssist repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/nodoassist-test-state.ts", "canonical NodoAssist test state helper"],
  ...
  ["test/setup-nodoassist-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "NodoAssist test env mutation report",
...
function printHelp(): void {
  process.stdout.write(`NodoAssist test env mutation report
