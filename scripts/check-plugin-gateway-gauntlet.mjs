...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
Environment:
  NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  NODOASSIST_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function nodoassistCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function sourceNodoAssistCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    NODOASSIST_LOG_DIR: path.join(runRoot, "logs"),
    NODOASSIST_QA_SUITE_PROGRESS: process.env.NODOASSIST_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...nodoassistCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(params) {
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...nodoassistCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceNodoAssistCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-gauntlet-"));
