...
TRUSTED_HARNESS_DIR="${NODOASSIST_LIVE_DOCKER_TRUSTED_HARNESS_DIR:-${NODOASSIST_LIVE_CODEX_TRUSTED_HARNESS_DIR:-$SCRIPT_ROOT_DIR}}"
...
CODEX_HARNESS_AUTH_MODE="${NODOASSIST_LIVE_CODEX_HARNESS_AUTH:-codex-auth}"
CODEX_CLI_PACKAGE_SPEC="${NODOASSIST_LIVE_CODEX_CLI_PACKAGE_SPEC:-}"
...
DOCKER_CACHE_CONTAINER_DIR="/tmp/nodoassist-cache"
DOCKER_CLI_TOOLS_CONTAINER_DIR="/tmp/nodoassist-npm-global"
DOCKER_EXTRA_ENV_FILES=()
DOCKER_AUTH_PRESTAGED=0

nodoassist_live_codex_harness_is_ci() {
  nodoassist_live_is_ci
}

nodoassist_live_codex_harness_append_build_extension() {
...
case "$CODEX_HARNESS_AUTH_MODE" in
  codex-auth | api-key)
    ;;
  *)
    echo "ERROR: NODOASSIST_LIVE_CODEX_HARNESS_AUTH must be one of: codex-auth, api-key." >&2
    exit 1
    ;;
esac
...
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" && -z "${OPENAI_API_KEY:-}" ]]; then
  echo "ERROR: NODOASSIST_LIVE_CODEX_HARNESS_AUTH=api-key requires OPENAI_API_KEY." >&2
  exit 1
fi
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" && ! -s "$HOME/.codex/auth.json" ]]; then
  echo "ERROR: NODOASSIST_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image." >&2
  if [[ -n "${OPENAI_API_KEY:-}" ]]; then
    echo "If this is a Testbox/API-key run, set NODOASSIST_LIVE_CODEX_HARNESS_AUTH=api-key and run through nodoassist-testbox-env." >&2
  fi
  exit 1
fi
...
elif nodoassist_live_codex_harness_is_ci; then
...
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" ]]; then
  ...
  CONFIG_DIR="$(mktemp -d "${RUNNER_TEMP:-/tmp}/nodoassist-docker-config.XXXXXX")"
  WORKSPACE_DIR="$(mktemp -d "${RUNNER_TEMP:-/tmp}/nodoassist-docker-workspace.XXXXXX")"
...
if [[ "$CODEX_HARNESS_AUTH_MODE" != "api-key" ]]; then
  ...
  done < <(nodoassist_live_collect_auth_files_from_csv "openai")
fi
...
if [[ -n "${DOCKER_HOME_DIR:-}" ]]; then
  nodoassist_live_stage_auth_into_home "$DOCKER_HOME_DIR" --files "${AUTH_FILES[@]}"
...
DOCKER_AUTH_ENV=()
if [[ "$CODEX_HARNESS_AUTH_MODE" == "api-key" ]]; then
  docker_env_dir="$(mktemp -d "${RUNNER_TEMP:-/tmp}/nodoassist-codex-harness-env.XXXXXX")"
...
if [ "${NODOASSIST_LIVE_CODEX_HARNESS_DEBUG:-}" = "1" ]; then
  id
  mount | grep -E 'nodoassist-cache|nodoassist-npm|/home/node' || true
...
if [ "${NODOASSIST_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ]; then
...
run_setup_command() {
  local timeout_value="${NODOASSIST_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:?missing live Codex harness setup timeout seconds}s"
  local timeout_bin=""
...
if [ "${NODOASSIST_LIVE_CODEX_HARNESS_AUTH:-codex-auth}" != "api-key" ] && [ ! -s "$HOME/.codex/auth.json" ]; then
...
if [ -d /app/dist-runtime/extensions/codex ]; then
  export NODOASSIST_BUNDLED_PLUGINS_DIR=/app/dist-runtime/extensions
elif [ -d /app/dist/extensions/codex ]; then
  export NODOASSIST_BUNDLED_PLUGINS_DIR=/app/dist/extensions
elif [ -f "$tmp_dir/extensions/codex/nodoassist.plugin.json" ]; then
  export NODOASSIST_BUNDLED_PLUGINS_DIR="$tmp_dir/extensions"
else
  echo "ERROR: staged Codex plugin not found for live harness." >&2
  exit 1
fi
nodoassist_live_stage_state_dir "$tmp_dir/.nodoassist-state"
if [ -n "${NODOASSIST_LIVE_CODEX_TRUSTED_HARNESS_DIR:-}" ] && [ -d "$NODOASSIST_LIVE_CODEX_TRUSTED_HARNESS_DIR" ]; then
  for harness_file in src/gateway/gateway-codex-harness.live-helpers.ts; do
    if [ -f "$NODOASSIST_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" ]; then
      mkdir -p "$(dirname "$tmp_dir/$harness_file")"
      cp "$NODOASSIST_LIVE_CODEX_TRUSTED_HARNESS_DIR/$harness_file" "$tmp_dir/$harness_file"
    fi
  done
fi
nodoassist_live_prepare_staged_config
cd "$tmp_dir"
if [ "${NODOASSIST_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" = "1" ]; then
...
if ! "$NPM_CONFIG_PREFIX/bin/codex" exec \
  ...
  if grep -q "Failed to extract accountId from token" "$codex_preflight_log"; then
    echo "ERROR: Codex auth cannot extract accountId from the available token; refresh NODOASSIST_CODEX_AUTH_JSON or use NODOASSIST_LIVE_CODEX_HARNESS_AUTH=api-key." >&2
    exit 1
  fi
  tail -c 262144 "$codex_preflight_log" >&2 || true
  exit 1
fi
node scripts/test-live.mjs -- ${NODOASSIST_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}
EOF

nodoassist_live_codex_harness_append_build_extension codex
...
NODOASSIST_SKIP_DOCKER_BUILD=0
export NODOASSIST_SKIP_DOCKER_BUILD
...
echo "==> Model: ${NODOASSIST_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}"
echo "==> Chat image probe: ${NODOASSIST_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE:-0}"
echo "==> Image probe: ${NODOASSIST_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}"
echo "==> MCP probe: ${NODOASSIST_LIVE_CODEX_HARNESS_MCP_PROBE:-1}"
echo "==> Subagent probe: ${NODOASSIST_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}"
echo "==> Subagent-only fast path: ${NODOASSIST_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-auto}"
echo "==> Guardian probe: ${NODOASSIST_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}"
echo "==> Auth mode: $CODEX_HARNESS_AUTH_MODE"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> CI-safe Codex config: ${NODOASSIST_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}"
echo "==> Test files: ${NODOASSIST_LIVE_CODEX_TEST_FILES:-src/gateway/gateway-codex-harness.live.test.ts}"
echo "==> Codex CLI package: $CODEX_CLI_PACKAGE_SPEC"
echo "==> Harness fallback: none"
echo "==> Auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
nodoassist_live_init_docker_run_args DOCKER_RUN_ARGS "${NODOASSIST_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e NODOASSIST_AGENT_HARNESS_FALLBACK=none \
  -e NODOASSIST_DOCKER_AUTH_PRESTAGED="$DOCKER_AUTH_PRESTAGED" \
  -e NODOASSIST_CODEX_APP_SERVER_BIN="${NODOASSIST_CODEX_APP_SERVER_BIN:-codex}" \
  ...
  -e NODOASSIST_LIVE_CODEX_HARNESS_AUTH="$CODEX_HARNESS_AUTH_MODE" \
  -e NODOASSIST_LIVE_CODEX_HARNESS=1 \
  -e NODOASSIST_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE="${NODOASSIST_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE:-0}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_DEBUG="${NODOASSIST_LIVE_CODEX_HARNESS_DEBUG:-}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_GUARDIAN_PROBE="${NODOASSIST_LIVE_CODEX_HARNESS_GUARDIAN_PROBE:-1}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_IMAGE_PROBE="${NODOASSIST_LIVE_CODEX_HARNESS_IMAGE_PROBE:-1}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_MCP_PROBE="${NODOASSIST_LIVE_CODEX_HARNESS_MCP_PROBE:-1}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_MODEL="${NODOASSIST_LIVE_CODEX_HARNESS_MODEL:-codex/gpt-5.5}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS="${NODOASSIST_LIVE_CODEX_HARNESS_REQUIRE_GUARDIAN_EVENTS:-1}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS="${NODOASSIST_LIVE_CODEX_HARNESS_REQUEST_TIMEOUT_MS:-}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS="$CODEX_HARNESS_SETUP_TIMEOUT_SECONDS" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_SUBAGENT_ONLY="${NODOASSIST_LIVE_CODEX_HARNESS_SUBAGENT_ONLY:-}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_SUBAGENT_PROBE="${NODOASSIST_LIVE_CODEX_HARNESS_SUBAGENT_PROBE:-1}" \
  -e NODOASSIST_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG="${NODOASSIST_LIVE_CODEX_HARNESS_USE_CI_SAFE_CODEX_CONFIG:-1}" \
  -e NODOASSIST_LIVE_CODEX_CLI_PACKAGE_SPEC="$CODEX_CLI_PACKAGE_SPEC" \
  ...
  -e NODOASSIST_LIVE_DOCKER_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e NODOASSIST_LIVE_CODEX_TRUSTED_HARNESS_DIR="$DOCKER_TRUSTED_HARNESS_CONTAINER_DIR" \
  -e NODOASSIST_LIVE_CODEX_BIND="${NODOASSIST_LIVE_CODEX_BIND:-}" \
  -e NODOASSIST_LIVE_CODEX_BIND_MODEL="${NODOASSIST_LIVE_CODEX_BIND_MODEL:-}" \
  -e NODOASSIST_LIVE_CODEX_BIND_PROVIDER="${NODOASSIST_LIVE_CODEX_BIND_PROVIDER:-}" \
  -e NODOASSIST_LIVE_CODEX_TEST_FILES="${NODOASSIST_LIVE_CODEX_TEST_FILES:-}" \
