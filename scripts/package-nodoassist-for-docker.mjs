#!/usr/bin/env node
// Builds the NodoAssist package artifact used by Docker E2E.
...
const AI_RUNTIME_PACKAGE = "@nodoassist/ai";
const AI_RUNTIME_BACKUP_DIR = ".nodoassist-ai-package-backup";
...
function resolvePackedNodoAssistFileName(value) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("nodoassist-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  if (
    !/^nodoassist-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
    filename !== path.basename(filename) ||
    filename !== path.win32.basename(filename)
  ) {
    throw new Error(`npm pack reported unsafe NodoAssist tarball filename: ${filename}`);
  }
  return filename;
}
...
const PACKAGE_ARTIFACT_BUILD_STEPS = [
  {
    label: "Building NodoAssist package artifacts",
    command: "node",
...
export async function buildPackageArtifacts(sourceDir, options = {}) {
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      env: {
        ...process.env,
        NODOASSIST_BUILD_ALL_NO_PNPM: "1",
        NODOASSIST_RUN_NODE_SKIP_DTS_BUILD: "0",
      },
      timeoutMs: resolveTimeoutMs(
        "NODOASSIST_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestNodoAssistTarball(outputDir, packOutput) {
  let fromOutput = "";
  try {
    ...
    if (Array.isArray(parsed)) {
      for (const entry of parsed) {
        ...
        const filename = resolvePackedNodoAssistFileName(entry.filename);
        if (filename) {
          fromOutput = filename;
        }
      }
  ...
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedNodoAssistFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedNodoAssistFileName(entry) === entry;
      } catch {
        return false;
      }
    })
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed NodoAssist tarball in ${outputDir}`);
  }
...
async function cleanPackedNodoAssistTarballs(outputDir) {
  let entries;
  try {
...
function isPackedAiRuntimeTarball(filename) {
  return /^nodoassist-ai-[A-Za-z0-9._-]+\.tgz$/u.test(filename);
}
...
export async function prepareBundledAiRuntimePackage(
  ...
  const aiRuntimePath = path.join(sourceDir, "node_modules", "@nodoassist", "ai");
  ...
  if (!hasAiRuntimeWorkspace) {
    throw new Error("@nodoassist/ai dependency requires the packages/ai workspace");
  }
  if (typeof aiRuntimeDependency !== "string") {
    throw new Error("root package.json must declare @nodoassist/ai as a dependency");
  }
  ...
  try {
    await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "NODOASSIST_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
    ...
    if (packedAiTarballs.length !== 1) {
      throw new Error(
        `expected one packed @nodoassist/ai tarball in ${outputDir}, found ${packedAiTarballs.length}`,
      );
    }
    ...
    if (typeof stagedPackageJson.version !== "string" || !stagedPackageJson.version) {
      throw new Error("packed @nodoassist/ai package must declare a version");
    }
    for (const [name, version] of Object.entries(stagedPackageJson.dependencies ?? {})) {
      if (packageJson.dependencies[name] !== version) {
        throw new Error(
          `root package.json must declare ${name}@${version} to bundle @nodoassist/ai without duplicate dependencies`,
        );
      }
    }
...
export async function packNodoAssistPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  console.error("==> Packing NodoAssist package");
  await prepareChangelog(sourceDir);
  let packOutput;
  let cleanupBundledAiRuntime = async () => {};
  try {
    await cleanPackedNodoAssistTarballs(outputDir);
  ...
  let tarball = await newestNodoAssistTarball(outputDir, packOutput);
  if (options.outputName) {
...
async function main() {
  ...
  console.error("==> Writing NodoAssist package inventory");
  await run(
    "node",
    [
      "--import",
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "NODOASSIST_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  const tarball = await packNodoAssistPackageForDocker(sourceDir, outputDir, {
    outputName: options.outputName,
    packJsonPath: options.packJson,
  });

  console.error("==> Checking NodoAssist package tarball");
  const checkStartedAt = Date.now();
  await run(
    "node",
    [
      path.join(ROOT_DIR, "scripts/check-nodoassist-package-tarball.mjs"),
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "NODOASSIST_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
        DEFAULT_PACKAGE_TARBALL_CHECK_TIMEOUT_MS,
      ),
    },
  );
  console.error(
    `==> NodoAssist package tarball check finished in ${Math.round((Date.now() - checkStartedAt) / 1000)}s`,
  );
