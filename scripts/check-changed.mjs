...
const CANVAS_A2UI_NATIVE_RESOURCE_PATH_RE =
  /^(?:pnpm-lock\.yaml$|apps\/shared\/NodoAssistKit\/Sources\/NodoAssistKit\/Resources\/CanvasA2UI\/|extensions\/canvas\/(?:package\.json$|scripts\/bundle-a2ui\.mjs$|src\/host\/a2ui(?:\/(?:index\.html|a2ui\.bundle\.js|\.bundle\.hash)$|-app\/))|scripts\/(?:bundle-a2ui|sync-native-a2ui)\.mjs$)/u;
...
export function shouldDelegateChangedCheckToCrabbox(argv = [], env = process.env) {
  if (isTruthyEnvFlag(env.NODOASSIST_CHECK_CHANGED_REMOTE_CHILD)) {
    return false;
  }
...
export function createChangedCheckPlan(result, options = {}) {
  ...
  if (lanes.liveDockerTooling) {
    ...
    addCommand("live Docker scheduler dry run", "node", ["scripts/test-docker-all.mjs"], {
      ...
      NODOASSIST_DOCKER_ALL_LIVE_MODE: "only",
    });
  }
...
function ensureCorepackPnpmShimDir() {
  ...
  const dir = mkdtempSync(path.join(tmpdir(), "nodoassist-corepack-pnpm-"));
