...
EXTRA_MOUNTS="${NODOASSIST_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${NODOASSIST_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${NODOASSIST_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${NODOASSIST_DOCKER_SOCKET:-}"
TIMEZONE="${NODOASSIST_TZ:-}"
RAW_SKIP_ONBOARDING="${NODOASSIST_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
DOCKER_PULL_TIMEOUT="${NODOASSIST_DOCKER_SETUP_PULL_TIMEOUT:-600s}"
OFFLINE_MODE=""
DEFAULT_SANDBOX_IMAGE="nodoassist-sandbox:bookworm-slim"
DEFAULT_SANDBOX_BROWSER_IMAGE="nodoassist-sandbox-browser:bookworm-slim"
...
read_config_gateway_token() {
  local config_path="$NODOASSIST_CONFIG_DIR/nodoassist.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == NODOASSIST_GATEWAY_TOKEN=* ]]; then
      token="${line#NODOASSIST_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${NODOASSIST_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$NODOASSIST_GATEWAY_PORT" "$NODOASSIST_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$NODOASSIST_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$NODOASSIST_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network nodoassist-cli service because it
  ...
  # Host NODOASSIST_* paths are Compose bind-mount sources. Setup-time CLI writes
  ...
  run_prestart_gateway \
    ...
    --entrypoint node nodoassist-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" nodoassist-cli "$@"
}
...
run_gateway_up() {
  ...
  docker compose "${compose_args[@]}" "${up_args[@]}" nodoassist-gateway
}
...
resolve_offline_sandbox_images() {
  ...
  printf '%s' "$agents_json" | run_prestart_gateway \
    -T --entrypoint node nodoassist-gateway -e '
const fs = require("node:fs");
...
const matchesBrowser = (rawPattern) => {
  ...
  if (pattern === "group:nodoassist" || pattern === "group:ui") {
    return true;
  }
  if (!pattern) {
    return false;
...
validate_offline_sandbox_prerequisites() {
      ...
      browser)
        if ! browser_contract="$(
          docker --host "unix://$DOCKER_SOCKET_PATH" image inspect \
            -f '{{ index .Config.Labels "org.nodoassist.sandbox-browser.contract" }}' \
  ...
  if ! run_compose_one_off --rm --entrypoint docker nodoassist-gateway --version >/dev/null 2>&1; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "NODOASSIST_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "NODOASSIST_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
NODOASSIST_CONFIG_DIR="${NODOASSIST_CONFIG_DIR:-$HOME/.nodoassist}"
NODOASSIST_WORKSPACE_DIR="${NODOASSIST_WORKSPACE_DIR:-$HOME/.nodoassist/workspace}"
NODOASSIST_AUTH_PROFILE_SECRET_DIR="${NODOASSIST_AUTH_PROFILE_SECRET_DIR:-$HOME/.nodoassist-auth-profile-secrets}"

validate_mount_path_value "NODOASSIST_CONFIG_DIR" "$NODOASSIST_CONFIG_DIR"
validate_mount_path_value "NODOASSIST_WORKSPACE_DIR" "$NODOASSIST_WORKSPACE_DIR"
validate_mount_path_value "NODOASSIST_AUTH_PROFILE_SECRET_DIR" "$NODOASSIST_AUTH_PROFILE_SECRET_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "NODOASSIST_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "NODOASSIST_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "NODOASSIST_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "NODOASSIST_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "NODOASSIST_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "NODOASSIST_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$NODOASSIST_CONFIG_DIR"
mkdir -p "$NODOASSIST_WORKSPACE_DIR"
mkdir -p "$NODOASSIST_AUTH_PROFILE_SECRET_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$NODOASSIST_CONFIG_DIR/identity"
mkdir -p "$NODOASSIST_CONFIG_DIR/agents/main/agent"
mkdir -p "$NODOASSIST_CONFIG_DIR/agents/main/sessions"

export NODOASSIST_CONFIG_DIR
export NODOASSIST_WORKSPACE_DIR
export NODOASSIST_AUTH_PROFILE_SECRET_DIR
export NODOASSIST_GATEWAY_PORT="${NODOASSIST_GATEWAY_PORT:-18789}"
export NODOASSIST_BRIDGE_PORT="${NODOASSIST_BRIDGE_PORT:-18790}"
export NODOASSIST_GATEWAY_BIND="${NODOASSIST_GATEWAY_BIND:-lan}"
export NODOASSIST_DISABLE_BONJOUR="${NODOASSIST_DISABLE_BONJOUR:-}"
export NODOASSIST_IMAGE="$IMAGE_NAME"
export NODOASSIST_IMAGE_APT_PACKAGES="${NODOASSIST_IMAGE_APT_PACKAGES-${NODOASSIST_DOCKER_APT_PACKAGES:-}}"
export NODOASSIST_IMAGE_PIP_PACKAGES="${NODOASSIST_IMAGE_PIP_PACKAGES:-}"
export NODOASSIST_EXTENSIONS="${NODOASSIST_EXTENSIONS:-}"
export NODOASSIST_DOCKER_BUILD_NODE_OPTIONS="${NODOASSIST_DOCKER_BUILD_NODE_OPTIONS---max-old-space-size=8192}"
export NODOASSIST_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB="${NODOASSIST_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB:-}"
export NODOASSIST_DOCKER_BUILD_SKIP_DTS="${NODOASSIST_DOCKER_BUILD_SKIP_DTS:-1}"
export NODOASSIST_INSTALL_BROWSER="${NODOASSIST_INSTALL_BROWSER:-}"
export NODOASSIST_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export NODOASSIST_HOME_VOLUME="$HOME_VOLUME_NAME"
export NODOASSIST_ALLOW_INSECURE_PRIVATE_WS="${NODOASSIST_ALLOW_INSECURE_PRIVATE_WS:-}"
export NODOASSIST_SANDBOX="$SANDBOX_ENABLED"
export NODOASSIST_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export NODOASSIST_DOCKER_SETUP=1
export NODOASSIST_TZ="$TIMEZONE"
...
export NODOASSIST_OTEL_PRELOADED="${NODOASSIST_OTEL_PRELOADED:-}"
export NODOASSIST_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${NODOASSIST_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    NODOASSIST_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $NODOASSIST_CONFIG_DIR/nodoassist.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      NODOASSIST_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      NODOASSIST_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      NODOASSIST_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
export NODOASSIST_GATEWAY_TOKEN
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${NODOASSIST_CONFIG_DIR}:/home/node/.nodoassist"
    gateway_workspace_mount="${NODOASSIST_WORKSPACE_DIR}:/home/node/.nodoassist/workspace"
    gateway_auth_profile_secret_mount="${NODOASSIST_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/nodoassist"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${NODOASSIST_INSTALL_DOCKER_CLI:-}" ]]; then
    export NODOASSIST_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ...
  NODOASSIST_AUTH_PROFILE_SECRET_DIR \
  NODOASSIST_GATEWAY_PORT \
  NODOASSIST_BRIDGE_PORT \
  NODOASSIST_GATEWAY_BIND \
  NODOASSIST_DISABLE_BONJOUR \
  NODOASSIST_GATEWAY_TOKEN \
  NODOASSIST_IMAGE \
  NODOASSIST_EXTRA_MOUNTS \
  NODOASSIST_HOME_VOLUME \
  NODOASSIST_IMAGE_APT_PACKAGES \
  NODOASSIST_IMAGE_PIP_PACKAGES \
  NODOASSIST_EXTENSIONS \
  NODOASSIST_DOCKER_BUILD_NODE_OPTIONS \
  NODOASSIST_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB \
  NODOASSIST_DOCKER_BUILD_SKIP_DTS \
  NODOASSIST_INSTALL_BROWSER \
  NODOASSIST_SANDBOX \
  NODOASSIST_DOCKER_SOCKET \
  DOCKER_GID \
  NODOASSIST_INSTALL_DOCKER_CLI \
  NODOASSIST_ALLOW_INSECURE_PRIVATE_WS \
  NODOASSIST_TZ \
  ...
  NODOASSIST_OTEL_PRELOADED \
  NODOASSIST_SKIP_ONBOARDING

if [[ -n "$OFFLINE_MODE" ]]; then
  require_local_docker_image "$IMAGE_NAME"
  echo "==> Using preloaded Docker image: $IMAGE_NAME"
elif [[ "$IMAGE_NAME" == "nodoassist:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "NODOASSIST_IMAGE_APT_PACKAGES=${NODOASSIST_IMAGE_APT_PACKAGES}" \
    --build-arg "NODOASSIST_IMAGE_PIP_PACKAGES=${NODOASSIST_IMAGE_PIP_PACKAGES}" \
    --build-arg "NODOASSIST_EXTENSIONS=${NODOASSIST_EXTENSIONS}" \
    --build-arg "NODOASSIST_DOCKER_BUILD_NODE_OPTIONS=${NODOASSIST_DOCKER_BUILD_NODE_OPTIONS}" \
    --build-arg "NODOASSIST_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=${NODOASSIST_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB}" \
    --build-arg "NODOASSIST_DOCKER_BUILD_SKIP_DTS=${NODOASSIST_DOCKER_BUILD_SKIP_DTS}" \
    --build-arg "NODOASSIST_INSTALL_BROWSER=${NODOASSIST_INSTALL_BROWSER}" \
    --build-arg "NODOASSIST_INSTALL_DOCKER_CLI=${NODOASSIST_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the NodoAssist metadata subdirectory
# (.nodoassist/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh nodoassist-gateway -c \
  'find /home/node/.nodoassist -xdev -exec chown node:node {} +; \
   chown node:node /home/node/.config; \
   find /home/node/.config/nodoassist -xdev -exec chown node:node {} +; \
   [ -d /home/node/.nodoassist/workspace/.nodoassist ] && chown -R node:node /home/node/.nodoassist/workspace/.nodoassist || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (NODOASSIST_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from NODOASSIST_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $NODOASSIST_GATEWAY_BIND"
  if is_truthy_value "$NODOASSIST_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (NODOASSIST_DISABLE_BONJOUR=$NODOASSIST_DISABLE_BONJOUR)."
  elif [[ -z "$NODOASSIST_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (NODOASSIST_DISABLE_BONJOUR=$NODOASSIST_DISABLE_BONJOUR)."
  fi
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm nodoassist-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm nodoassist-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm nodoassist-cli channels add --channel discord --token <token>"
...
# --- Sandbox setup (opt-in via NODOASSIST_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -z "$OFFLINE_MODE" && ! -S "$DOCKER_SOCKET_PATH" ]]; then
    echo "WARNING: NODOASSIST_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
  ...
  if [[ -n "$SANDBOX_ENABLED" && -z "$OFFLINE_MODE" ]] &&
    ! run_compose_one_off --rm --entrypoint docker nodoassist-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg NODOASSIST_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (NODOASSIST_IMAGE=nodoassist:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in NodoAssist config.
...
echo "Config: $NODOASSIST_CONFIG_DIR"
echo "Workspace: $NODOASSIST_WORKSPACE_DIR"
echo "Token: stored in Docker environment/config (not printed)."
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f nodoassist-gateway"
echo "  ${COMPOSE_HINT} exec nodoassist-gateway sh -lc 'node dist/index.js health --token \"\$NODOASSIST_GATEWAY_TOKEN\"'"
