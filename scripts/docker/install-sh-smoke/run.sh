...
SMOKE_MODE="${NODOASSIST_INSTALL_SMOKE_MODE:-install}"
SMOKE_PREVIOUS_VERSION="${NODOASSIST_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${NODOASSIST_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="nodoassist"
PACKAGE_NAME="${NODOASSIST_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
FRESH_VERSION="${NODOASSIST_INSTALL_FRESH_VERSION:-}"
FRESH_TAG_URL="${NODOASSIST_INSTALL_FRESH_TAG_URL:-}"
UPDATE_BASELINE_VERSION="${NODOASSIST_INSTALL_UPDATE_BASELINE:-latest}"
UPDATE_BASELINE_TAG_URL="${NODOASSIST_INSTALL_UPDATE_BASELINE_TAG_URL:-}"
UPDATE_EXPECT_VERSION="${NODOASSIST_INSTALL_UPDATE_EXPECT_VERSION:-}"
UPDATE_TAG_URL="${NODOASSIST_INSTALL_UPDATE_TAG_URL:-}"
SELF_UPDATE_WARNING_FIXED_VERSION="${NODOASSIST_INSTALL_SELF_UPDATE_WARNING_FIXED_VERSION:-2026.5.25}"
FRESHNESS_VERSION="${NODOASSIST_INSTALL_FRESHNESS_VERSION:-latest}"
...
is_self_swapped_package_process_exit() {
  local stderr="$1"
  [[ "$stderr" == *"[nodoassist] Failed to start CLI:"* ]] &&
    [[ "$stderr" == *"ERR_MODULE_NOT_FOUND"* ]] &&
    [[ "$stderr" == *"/node_modules/nodoassist/dist/"* ]]
}
...
allow_legacy_update_warning() {
  [[ "${NODOASSIST_INSTALL_ALLOW_LEGACY_UPDATE_WARNING:-0}" == "1" ]] && return 0
...
run_install_smoke() {
  if [[ -n "$FRESH_VERSION" && -n "$FRESH_TAG_URL" ]]; then
    ...
    NODOASSIST_NO_ONBOARD=1 NODOASSIST_NO_PROMPT=1 \
    ...
    if [[ -n "${NODOASSIST_INSTALL_LATEST_OUT:-}" ]]; then
      ...
      if [[ -n "$latest_npm_version" ]]; then
        printf "%s" "$latest_npm_version" > "${NODOASSIST_INSTALL_LATEST_OUT:-}"
      else
        printf "%s" "$FRESH_VERSION" > "${NODOASSIST_INSTALL_LATEST_OUT:-}"
      fi
    fi
...
NODE
  ...
  if [[ "$SKIP_PREVIOUS" == "1" ]]; then
    echo "==> Skip preinstall previous (NODOASSIST_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
  else
  ...
  echo "==> Verify installed version"
  if [[ -n "${NODOASSIST_INSTALL_LATEST_OUT:-}" ]]; then
    printf "%s" "$LATEST_VERSION" > "${NODOASSIST_INSTALL_LATEST_OUT:-}"
  fi
...
run_update_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: NODOASSIST_INSTALL_UPDATE_EXPECT_VERSION is required for update mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: NODOASSIST_INSTALL_UPDATE_TAG_URL is required for update mode" >&2
    return 1
  fi
  ...
  echo "==> Run nodoassist update from host-served tgz"
  local update_status
  ...
  if allow_legacy_update_warning; then
    update_env+=(NODOASSIST_UPDATE_IN_PROGRESS=1)
  fi
  update_stderr_file="$(mktemp)"
  set +e
  UPDATE_JSON="$(
    run_with_heartbeat "nodoassist update" \
      "${update_env[@]}" \
      nodoassist update --tag "$UPDATE_TAG_URL" --yes --json 2>"$update_stderr_file"
  )"
  update_status=$?
  set -e
  ...
  if [[ "$update_stderr" == *"config was written by version"* ]] && allow_legacy_update_warning; then
    ...
    echo "ERROR: nodoassist update emitted a self-update version-skew warning" >&2
    return 1
  fi
  if [[ "$update_status" -ne 0 ]]; then
    if is_self_swapped_package_process_exit "$update_stderr"; then
      ...
      echo "ERROR: nodoassist update failed with exit code $update_status" >&2
...
run_npm_global_smoke() {
  if [[ -z "$UPDATE_EXPECT_VERSION" ]]; then
    echo "ERROR: NODOASSIST_INSTALL_UPDATE_EXPECT_VERSION is required for npm-global mode" >&2
    return 1
  fi
  if [[ -z "$UPDATE_TAG_URL" ]]; then
    echo "ERROR: NODOASSIST_INSTALL_UPDATE_TAG_URL is required for npm-global mode" >&2
    return 1
  fi
...
case "$SMOKE_MODE" in
  ...
  freshness)
    ...
    ;;
  *)
    echo "ERROR: unsupported NODOASSIST_INSTALL_SMOKE_MODE=$SMOKE_MODE" >&2
    exit 1
    ;;
esac
