...
process.env.NODOASSIST_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK ??= "1";

const { packageRoot } = parsePackageRootArg(
  process.argv.slice(2),
  "NODOASSIST_BUNDLED_CHANNEL_SMOKE_ROOT",
);
...
const installedLayoutEnv = "NODOASSIST_BUNDLED_CHANNEL_SMOKE_INSTALLED_LAYOUT";
...
function packageRootLooksInstalled(root) {
  return root.replaceAll("\\", "/").endsWith("/node_modules/nodoassist");
}
...
function smokeInInstalledLayoutIfNeeded() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-channel-entry-smoke-"));
  const nodeModulesRoot = path.join(tempRoot, "node_modules");
  const installedPackageRoot = path.join(nodeModulesRoot, "nodoassist");
...
function collectBundledChannelEntryFiles() {
  ...
  for (const dirent of fs.readdirSync(distExtensionsRoot, { withFileTypes: true })) {
    ...
    const packageJson = readJson(packageJsonPath);
    if (!packageJson.nodoassist?.channel) {
      continue;
    }
    ...
    const extensionEntries =
      Array.isArray(packageJson.nodoassist.extensions) &&
      packageJson.nodoassist.extensions.length > 0
    ...
    const setupEntry = packageJson.nodoassist.setupEntry;
