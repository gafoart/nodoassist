// Debug Claude Usage script supports NodoAssist repository automation.
...
Options:
  --agent <id>          NodoAssist agent id to inspect (default: main)
...
const resolveFetchTimeoutMs = (
  raw = process.env.NODOASSIST_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS,
) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "NODOASSIST_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
