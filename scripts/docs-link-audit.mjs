...
export function prepareMirroredDocsDir(sourceDir = DOCS_DIR, options = {}) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docs-link-audit-"));
  try {
...
export function prepareAnchorAuditDocsDir(sourceDir = DOCS_DIR) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docs-anchor-audit-"));
  try {
