...
// NodoAssist Npm Release Check script supports NodoAssist repository automation.
...
const skipPackValidationEnv = "NODOASSIST_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "nodoassist") {
    errors.push(`package.json name must be "nodoassist"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.nodoassist !== "nodoassist.mjs") {
    errors.push(
      `package.json bin.nodoassist must be "nodoassist.mjs"; found "${pkg.bin?.nodoassist ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`nodoassist-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `nodoassist-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
