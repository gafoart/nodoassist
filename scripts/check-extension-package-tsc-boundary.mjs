...
export function resolveCompileConcurrency(
  ...
  const raw = env.NODOASSIST_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "NODOASSIST_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.NODOASSIST_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
