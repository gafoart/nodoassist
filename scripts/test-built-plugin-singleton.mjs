...
const { clearPluginCommands, getPluginCommandSpecs, loadNodoAssistPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadNodoAssistPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@nodoassist/build-smoke-plugin",
      type: "module",
      nodoassist: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'nodoassist/plugin-sdk';",
...
const registry = loadNodoAssistPlugins({
  ...
  env: {
    ...process.env,
    NODOASSIST_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
