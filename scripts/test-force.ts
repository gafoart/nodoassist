...
// Test Force script supports NodoAssist repository automation.
...
function usage(): string {
  return [
    ...
    "",
    "Clears the configured NodoAssist gateway port, then runs the local test suite.",
    "",
    "Options:",
...
function runTests() {
  const isolatedLock =
    process.env.NODOASSIST_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `nodoassist-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["scripts/test-projects.mjs"], {
    stdio: "inherit",
    env: {
      ...process.env,
      NODOASSIST_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
