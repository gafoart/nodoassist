...
IMAGE_NAME="${NODOASSIST_INSTALL_E2E_IMAGE:-nodoassist-install-e2e:local}"
...
PROFILE_FILE="${NODOASSIST_INSTALL_E2E_PROFILE_FILE:-${NODOASSIST_PROFILE_FILE:-${NODOASSIST_TESTBOX_PROFILE_FILE:-$HOME/.nodoassist-testbox-live.profile}}}"

OPENAI_API_KEY="${OPENAI_API_KEY:-}"
ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
ANTHROPIC_API_TOKEN="${ANTHROPIC_API_TOKEN:-}"
NODOASSIST_E2E_MODELS="${NODOASSIST_E2E_MODELS:-}"
...
docker_e2e_docker_run_cmd run --rm \
  ...
  -e NODOASSIST_INSTALL_TAG="${NODOASSIST_INSTALL_TAG:-latest}" \
  -e NODOASSIST_E2E_MODELS="$NODOASSIST_E2E_MODELS" \
  -e NODOASSIST_INSTALL_E2E_OPENAI_MODEL="${NODOASSIST_INSTALL_E2E_OPENAI_MODEL:-}" \
  -e NODOASSIST_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS="$OPENAI_PROVIDER_TIMEOUT_SECONDS" \
  -e NODOASSIST_INSTALL_E2E_PREVIOUS="${NODOASSIST_INSTALL_E2E_PREVIOUS:-}" \
  -e NODOASSIST_INSTALL_E2E_SKIP_PREVIOUS="${NODOASSIST_INSTALL_E2E_SKIP_PREVIOUS:-0}" \
  ...
  -e NODOASSIST_INSTALL_E2E_AGENT_TOOL_SMOKE="$AGENT_TOOL_SMOKE" \
  -e NODOASSIST_INSTALL_E2E_SESSION_SCAN_BYTES="$SESSION_SCAN_BYTES" \
  -e NODOASSIST_INSTALL_E2E_SESSION_LINE_BYTES="$SESSION_LINE_BYTES" \
  -e NODOASSIST_INSTALL_E2E_SESSION_SCAN_DEPTH="$SESSION_SCAN_DEPTH" \
  -e NODOASSIST_INSTALL_E2E_SESSION_SCAN_NODES="$SESSION_SCAN_NODES" \
