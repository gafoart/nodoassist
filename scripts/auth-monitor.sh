...
#
# Suggested cron: */30 * * * * /path/to/nodoassist/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send NodoAssist notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., nodoassist-alerts)
...
STATE_FILE="$HOME/.nodoassist/auth-monitor-state"
...
send_notification() {
    ...
    # Send via NodoAssist if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use nodoassist
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via NodoAssist to $NOTIFY_PHONE..."
            nodoassist send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! NodoAssist is down. Run on the NodoAssist host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
