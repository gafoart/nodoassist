// Ios Version script supports NodoAssist repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `NODOASSIST_IOS_VERSION=${version.canonicalVersion}`,
        `NODOASSIST_MARKETING_VERSION=${version.marketingVersion}`,
        `NODOASSIST_BUILD_VERSION=${version.buildVersion}`,
      ].join("\n") + "\n",
    );
  } else {
