// Write Cli Startup Metadata script supports NodoAssist repository automation.
...
import type { NodoAssistConfig } from "../src/config/config.js";
...
function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.nodoassist?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.nodoassist?.channel?.order;
      const labelRaw = parsed.nodoassist?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".nodoassist-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "nodoassist-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "nodoassist-build",
...
async function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "nodoassist", "browser", "--help"]);`,
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["nodoassist.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      NODOASSIST_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
