// Gateway Smoke script supports NodoAssist repository automation.
...
function parseGatewaySmokeCli(
  ...
  return {
    help: has("--help") || has("-h"),
    token: getArg("--token") ?? env.NODOASSIST_GATEWAY_TOKEN,
    urlRaw: getArg("--url") ?? env.NODOASSIST_GATEWAY_URL,
  };
}
...
export async function runGatewaySmoke(
  ...
  try {
    ...
    const connectRes = await request("connect", {
      ...
      client: {
        id: "nodoassist-ios",
        displayName: "nodoassist gateway smoke test",
        version: "dev",
        platform: "dev",
        mode: "ui",
        instanceId: "nodoassist-dev-smoke",
      },
      locale: "en-US",
