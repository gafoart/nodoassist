#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports NodoAssist repository automation.
...
type DriverMode = "token" | "webhook" | "nodoassist";
...
const DEFAULT_NODOASSIST_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.NODOASSIST_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.NODOASSIST_HOME?.trim() || process.env.HOME || "";
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "nodoassist") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or nodoassist.`,
  );
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver nodoassist] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) NodoAssist spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|nodoassist> Driver transport mode (default: token)\n" +
    ...
    "  --state-dir <p>              Override NodoAssist state dir for plugin-state polling\n" +
    "  --nodoassist-bin <path>        NodoAssist CLI binary for driver=nodoassist (default: nodoassist)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  NODOASSIST_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  NODOASSIST_DISCORD_SMOKE_DRIVER\n" +
    "  NODOASSIST_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  NODOASSIST_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  NODOASSIST_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  NODOASSIST_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  NODOASSIST_DISCORD_SMOKE_AGENT\n" +
    "  NODOASSIST_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  NODOASSIST_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  NODOASSIST_DISCORD_SMOKE_POLL_MS\n" +
    "  NODOASSIST_STATE_DIR\n" +
    "  NODOASSIST_DISCORD_SMOKE_NODOASSIST_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.NODOASSIST_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.NODOASSIST_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.NODOASSIST_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.NODOASSIST_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.NODOASSIST_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix", argv) ||
    process.env.NODOASSIST_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.NODOASSIST_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention", argv) ||
    process.env.NODOASSIST_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction", argv) ||
    process.env.NODOASSIST_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.NODOASSIST_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.NODOASSIST_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const nodoassistBin =
    resolveArg("--nodoassist-bin", argv) ||
    process.env.NODOASSIST_DISCORD_SMOKE_NODOASSIST_BIN ||
    "nodoassist";
...
async function nodoassistCliJson<T>(params: {
  nodoassistBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.nodoassistBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_NODOASSIST_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`nodoassist ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithNodoAssist(params: {
  nodoassistBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await nodoassistCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    nodoassistBin: params.nodoassistBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "nodoassist") {
    return await readMessagesWithNodoAssist({
      nodoassistBin: params.args.nodoassistBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `nodoassist-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      minBindingBoundAt = Date.now() - 3_000;
      const sent = await nodoassistCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        nodoassistBin: args.nodoassistBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("nodoassist message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "nodoassist"
            ? await readMessagesWithNodoAssist({
                nodoassistBin: args.nodoassistBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from NodoAssist.`,
        diagnostics: {
          bindingCandidates: [
            {
