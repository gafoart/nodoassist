// Ios Node E2E script supports NodoAssist repository automation.
...
function usage(): string {
  return [
    ...
    "Or set env: NODOASSIST_GATEWAY_URL / NODOASSIST_GATEWAY_TOKEN",
    "",
    "Options:",
...
const urlRaw = getArg("--url") ?? process.env.NODOASSIST_GATEWAY_URL;
const token = getArg("--token") ?? process.env.NODOASSIST_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.NODOASSIST_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "nodoassist ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "nodoassist-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "NodoAssist E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
