...
const MANUAL_SECTION_START = "<!-- nodoassist-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- nodoassist-plugin-reference:manual-end -->";
...
function resolveDescription({ manifest, packageJson }) {
  ...
  if (channels.length > 0) {
    ...
    return `Adds the ${channelLabel}${channelNoun} surface for sending and receiving NodoAssist messages.`;
  }

  const providers = Array.isArray(manifest.providers) ? manifest.providers : [];
  if (providers.length > 0) {
    return `Adds ${displayList(providers)} model provider support to NodoAssist.`;
  }
  ...
  return packageDescription ? `${packageDescription}.` : "Provides an NodoAssist plugin.";
}
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.nodoassist?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  if (status === "core") {
    const release = packageJson.nodoassist?.release;
    if (release?.publishToClawHub === true || release?.publishToNpm === true) {
      return `included in NodoAssist; ${resolveInstallRoute(packageJson, "external")}`;
    }
    return "included in NodoAssist";
  }
  const install = packageJson.nodoassist?.install;
  const release = packageJson.nodoassist?.release;
  const clawhubSpec =
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.nodoassist?.install?.clawhubSpec === "string" ||
    typeof packageJson.nodoassist?.install?.npmSpec === "string";
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "nodoassist.plugin.json");
