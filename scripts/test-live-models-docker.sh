...
LIVE_MAX_MODELS="${NODOASSIST_LIVE_MAX_MODELS:-}"
if [[ -n "$LIVE_MAX_MODELS" && ! "$LIVE_MAX_MODELS" =~ ^\+?[0-9]+$ ]]; then
  echo "invalid NODOASSIST_LIVE_MAX_MODELS: $LIVE_MAX_MODELS" >&2
  exit 2
fi
LIVE_MODEL_TIMEOUT_MS="${NODOASSIST_LIVE_MODEL_TIMEOUT_MS:-}"
if [[ -n "$LIVE_MODEL_TIMEOUT_MS" ]]; then
  LIVE_MODEL_TIMEOUT_MS="$(nodoassist_live_read_positive_int_env NODOASSIST_LIVE_MODEL_TIMEOUT_MS "$LIVE_MODEL_TIMEOUT_MS")"
fi
TEMP_DIRS=()
DOCKER_HOME_MOUNT=()
cleanup_temp_dirs() {
...
if nodoassist_live_truthy "${NODOASSIST_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  NODOASSIST_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${NODOASSIST_LIVE_PROVIDERS:-}" || -n "${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      nodoassist_live_collect_auth_dirs_from_csv "${NODOASSIST_LIVE_PROVIDERS:-}"
      nodoassist_live_collect_auth_dirs_from_csv "${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      nodoassist_live_collect_auth_files_from_csv "${NODOASSIST_LIVE_PROVIDERS:-}"
      nodoassist_live_collect_auth_files_from_csv "${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${NODOASSIST_DOCKER_PROFILE_ENV_ONLY:-0}"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> External auth dirs: ${AUTH_DIRS_CSV:-none}"
echo "==> External auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
nodoassist_live_init_docker_run_args DOCKER_RUN_ARGS "${NODOASSIST_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e NODOASSIST_LIVE_MODELS="${NODOASSIST_LIVE_MODELS:-modern}" \
  -e NODOASSIST_LIVE_PROVIDERS="${NODOASSIST_LIVE_PROVIDERS:-}" \
  -e NODOASSIST_LIVE_MAX_MODELS="$LIVE_MAX_MODELS" \
  -e NODOASSIST_LIVE_MODEL_TIMEOUT_MS="$LIVE_MODEL_TIMEOUT_MS" \
  -e NODOASSIST_LIVE_REQUIRE_PROFILE_KEYS="${NODOASSIST_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e NODOASSIST_LIVE_GATEWAY_MODELS="${NODOASSIST_LIVE_GATEWAY_MODELS:-}" \
  -e NODOASSIST_LIVE_GATEWAY_PROVIDERS="${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}" \
  -e NODOASSIST_LIVE_GATEWAY_MAX_MODELS="${NODOASSIST_LIVE_GATEWAY_MAX_MODELS:-}" \
