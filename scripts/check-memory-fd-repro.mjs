...
const SKIP_GATEWAY_ENV = {
  ...
  NODOASSIST_SKIP_ACPX_RUNTIME: "1",
  NODOASSIST_SKIP_ACPX_RUNTIME_PROBE: "1",
...
Options:
  ...
  --keep                         Keep the synthetic NODOASSIST_HOME and workspace after the run.
...
export function parseArgs(argv) {
  ...
  const options = {
    fileCount: undefined,
    mode: process.env.NODOASSIST_MEMORY_FD_REPRO_MODE || "fixed",
    ...
    keep: process.env.NODOASSIST_MEMORY_FD_REPRO_KEEP === "1",
    allowNonDarwin: process.env.NODOASSIST_MEMORY_FD_REPRO_ALLOW_NON_DARWIN === "1",
  };
...
/**
 * Writes isolated NodoAssist config for the synthetic memory workspace.
 */
export function writeConfig({ homeDir, workspaceDir, port, token }) {
  const configDir = path.join(homeDir, ".nodoassist");
...
async function main() {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-memory-fd-repro-"));
  ...
  const env = {
    ...
    ...SKIP_GATEWAY_ENV,
    HOME: homeDir,
    NODOASSIST_STATE_DIR: path.join(homeDir, ".nodoassist"),
