...
// Release Check script supports NodoAssist repository automation.
...
} from "./nodoassist-npm-postpublish-verify.ts";
...
export const RELEASE_CHECK_LOCAL_PACKAGE_TARBALL_DIR_ENV =
  "NODOASSIST_RELEASE_CHECK_LOCAL_PACKAGE_TARBALL_DIR";
...
const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "nodoassist/plugin-sdk/core",
  "nodoassist/plugin-sdk/provider-entry",
  "nodoassist/plugin-sdk/runtime",
] as const;
const CRITICAL_PLUGIN_SDK_IMPORT_SMOKE_SPECIFIERS = ["nodoassist/plugin-sdk/core"] as const;
...
export function runReleaseCheckCommand(
  ...
  const output = execFileSync(invocation.command, invocation.args, {
    ...
    maxBuffer:
      options.maxBuffer ??
      positiveEnvInt(
        "NODOASSIST_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES",
    ...
    timeout:
      options.timeoutMs ??
      positiveEnvInt(
        "NODOASSIST_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
...
export function writePackedTarballInstallManifest(
  ...
  if (localPackageTarballs.length > 1) {
    throw new Error(
      `release-check: packed install accepts at most one @nodoassist/ai tarball; found ${localPackageTarballs.length}.`,
    );
  }
  const dependencies: Record<string, string> = {
    nodoassist: pathToFileURL(tarballPath).href,
  };
  if (localPackageTarballs[0]) {
    dependencies["@nodoassist/ai"] = pathToFileURL(localPackageTarballs[0]).href;
  }
...
export function resolvePackedInstalledBinaryPath(
  ...
  return join(
    ...
    "node_modules",
    ".bin",
    platform === "win32" ? "nodoassist.cmd" : "nodoassist",
  );
}
...
export function collectPackedInstalledPackageVerificationErrors(params: {
  ...
  if (
    ...
    errors.push(
      `installed nodoassist binary version mismatch: expected ${params.expectedVersion}, found ${params.installedBinaryVersion || "<missing>"}.`,
    );
  }
...
export function createPackedPluginSdkTypescriptSmokeProject(params: {
  ...
  const dependencies: Record<string, string> = {
    nodoassist: params.packageSpec,
  };
  if (params.aiPackageSpec) {
    dependencies["@nodoassist/ai"] = params.aiPackageSpec;
  }
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: "nodoassist-plugin-sdk-type-smoke",
        private: true,
        type: "module",
        dependencies,
      },
...
function runPackedPluginSdkTypescriptSmoke(
  ...
  const installedNodoAssistRoot = join(consumerDir, "node_modules", "nodoassist");
  const tscPath = [
    join(consumerDir, "node_modules", "typescript", "bin", "tsc"),
    join(installedNodoAssistRoot, "node_modules", "typescript", "bin", "tsc"),
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".nodoassist", "nodoassist.json");
  mkdirSync(join(homeDir, ".nodoassist"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-nodoassist-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-nodoassist-release-check",
  });
  ...
  runReleaseCheckCommand(
    {
      command: process.execPath,
      args: [join(packageRoot, "nodoassist.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    },
    {
      cwd: packageRoot,
      stdio: "inherit",
  ...
  runReleaseCheckCommand(
    { command: process.execPath, args: [join(packageRoot, "nodoassist.mjs"), "plugins", "doctor"] },
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "nodoassist-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(prefixDir, "node_modules", "nodoassist");
    ...
    runReleaseCheckCommand(
      {
        command: process.execPath,
        args: [join(packageRoot, "nodoassist.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      },
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.nodoassist-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.nodoassist-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from nodoassist/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("nodoassist/plugin-sdk/".length);
