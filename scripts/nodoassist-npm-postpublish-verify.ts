...
// NodoAssist Npm Postpublish Verify script supports NodoAssist repository automation.
...
} from "./nodoassist-npm-release-check.ts";
...
type NodoAssistNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function nodoAssistNpmPostpublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/nodoassist-npm-postpublish-verify.ts <version>";
}

export function parseNodoAssistNpmPostpublishVerifyArgs(
  argv: readonly string[],
): NodoAssistNpmPostpublishVerifyArgs {
  ...
  if (!version) {
    throw new Error(nodoAssistNpmPostpublishVerifyUsage());
  }
  if (version.startsWith("-")) {
    throw new Error(`Unknown nodoassist npm postpublish verifier option: ${version}`);
  }
  const extraArg = args[1]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected nodoassist npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  const exactSpec = `nodoassist@${version}`;
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`nodoassist@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
const NPM_PROVENANCE_WORKFLOW_PATH = ".github/workflows/nodoassist-npm-release.yml";
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  if (
    ...
    throw new Error(
      `npm provenance attestation does not bind ${version} to the trusted NodoAssist GitHub release workflow.`,
    );
  }
...
function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@nodoassist/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "nodoassist.cmd")
    : pathPosix.join(prefixDir, "bin", "nodoassist");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  console.log(
    `nodoassist-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `nodoassist-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "nodoassist");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed nodoassist binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`nodoassist-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  const args = parseNodoAssistNpmPostpublishVerifyArgs(argv);
  if (args.help) {
    console.log(nodoAssistNpmPostpublishVerifyUsage());
    return;
  }
  ...
  console.log(
    `nodoassist-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`nodoassist-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
