...
usage() {
  cat <<'EOF'
Usage:
  scripts/ios-validate-app-store-ipa.sh --ipa apps/ios/build/app-store/NodoAssist-<version>.ipa
...
EXPECTED_BUNDLE_ID="ai.nodoassistfoundation.app"
EXPECTED_PROFILE_NAME="NodoAssist App Store ai.nodoassistfoundation.app"
EXPECTED_APP_GROUP="group.ai.nodoassistfoundation.app.shared"
...
tmp_dir="$(mktemp -d -t nodoassist-ios-ipa.XXXXXX)"
...
assert_plist_string "${info_plist}" "NodoAssistPushMode" "${EXPECTED_PUSH_MODE}" "push mode mismatch"
assert_plist_empty_or_absent "${info_plist}" "NodoAssistPushRelayBaseURL" "push relay URL override"
assert_plist_key_absent "${info_plist}" "NodoAssistPushTransport" "legacy push transport"
assert_plist_key_absent "${info_plist}" "NodoAssistPushDistribution" "legacy push distribution"
assert_plist_key_absent "${info_plist}" "NodoAssistPushAPNsEnvironment" "legacy APNs environment"
assert_plist_key_absent "${info_plist}" "NodoAssistPushRelayProfile" "legacy relay profile"
assert_plist_key_absent "${info_plist}" "NodoAssistPushProofPolicy" "legacy proof policy"
