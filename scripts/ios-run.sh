...
APP_NAME="${IOS_APP_NAME:-NodoAssist}"
...
Options:
  ...
  --simulator-proof-secret-env <name>
      ...
      Defaults to NODOASSIST_SIMULATOR_PUSH_PROOF_SECRET.
  -h, --help
      Show this help.
EOF
}
...
push_relay_base_url="${NODOASSIST_PUSH_SANDBOX_RELAY_BASE_URL:-https://ios-push-relay-sandbox.openclaw.ai}"
simulator_proof_secret_env="${NODOASSIST_SIMULATOR_PUSH_PROOF_SECRET_ENV:-NODOASSIST_SIMULATOR_PUSH_PROOF_SECRET}"
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  ...
  xcodebuild_overrides+=(
    "NODOASSIST_PUSH_MODE=simulatorSandbox"
    "NODOASSIST_PUSH_RELAY_BASE_URL=${push_relay_base_url}"
    "NODOASSIST_APNS_ENTITLEMENT_ENVIRONMENT=development"
  )
fi

unset "${simulator_proof_secret_env}"
if [[ "${simulator_proof_secret_env}" != "NODOASSIST_SIMULATOR_PUSH_PROOF_SECRET" ]]; then
  unset NODOASSIST_SIMULATOR_PUSH_PROOF_SECRET
fi
...
if [[ "${push_sandbox_simulator}" == "1" ]]; then
  # shellcheck disable=SC2086
  SIMCTL_CHILD_NODOASSIST_SIMULATOR_PUSH_PROOF_SECRET="${simulator_proof_secret}" \
