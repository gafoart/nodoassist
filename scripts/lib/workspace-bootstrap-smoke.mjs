// Verifies installed packages can bootstrap the default NodoAssist workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "nodoassist-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "nodoassist.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".nodoassist", "workspace");
