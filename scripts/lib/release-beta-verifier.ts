// Release Beta Verifier script supports NodoAssist repository automation.
...
const DEFAULT_REPO = "nodoassist/nodoassist";
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--clawhub-workflow-ref REF] [--full-release-validation-run ID] [--nodoassist-npm-run ID] [--plugin-npm-run ID] [--plugin-clawhub-run ID] [--plugin-clawhub-bootstrap-run ID] [--npm-telegram-run ID] [--skip-github-release] [--skip-clawhub]",
    );
  }
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--nodoassist-npm-run":
        parsed.workflowRuns.nodoassistNpm = next();
        break;
      case "--plugin-npm-run":
...
export async function verifyBetaRelease(
  ...
  const nodoassistNpm = await verifyNpmPackage("nodoassist", args.version, args.distTag);
  lines.push(`nodoassist npm OK: ${args.version} (${args.distTag})`);
  ...
  if (!args.skipPostpublish) {
    ...
    lines.push("nodoassist postpublish verifier OK");
  }
  ...
  if (args.workflowRuns.nodoassistNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.nodoassistNpm,
        label: "NodoAssist NPM Release",
        repo: args.repo,
        expectedWorkflowName: "NodoAssist NPM Release",
        expectedHeadBranch: args.workflowRef,
        rerunFailed: false,
      }),
    );
      ...
      `${JSON.stringify(
        {
          ...
          nodoassistNpmIntegrity: nodoassistNpm.integrity,
          nodoassistNpmTarball: nodoassistNpm.tarball,
