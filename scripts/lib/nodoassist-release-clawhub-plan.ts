// NodoAssist release ClawHub plan script supports release workflow routing.
...
type NodoAssistReleaseClawHubPlanArgs = {
  releaseTag: string;
...
type NodoAssistReleaseClawHubPlan = {
...
type NodoAssistReleaseClawHubRuntimeStateArgs = {
  repository: string;
...
type NodoAssistReleaseClawHubRuntimeState = {
  verifierArgs: string[];
  proofLines: {
    normal: string;
    bootstrap: string;
...
export function buildNodoAssistReleaseClawHubRuntimeState(
  args: NodoAssistReleaseClawHubRuntimeStateArgs,
): NodoAssistReleaseClawHubRuntimeState {
...
export function parseNodoAssistReleaseClawHubPlanArgs(
  argv: string[],
): NodoAssistReleaseClawHubPlanArgs {
  const values = [...argv];
  if (values[0] === "--") {
    values.shift();
  }
...
export async function buildNodoAssistReleaseClawHubPlan(
  args: NodoAssistReleaseClawHubPlanArgs,
  options: {
    rootDir?: string;
    fetchImpl?: typeof fetch;
    registryBaseUrl?: string;
  } = {},
): Promise<NodoAssistReleaseClawHubPlan> {
