...
docker_build_on_missing_enabled() {
  case "${NODOASSIST_DOCKER_BUILD_ON_MISSING:-}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
    0 | false | FALSE | no | NO)
  ...
  [ "${NODOASSIST_TESTBOX:-0}" = "1" ]
}

docker_build_command() {
  local build_cmd=(docker build)
  if [ "${NODOASSIST_DOCKER_BUILD_USE_BUILDX:-0}" = "1" ] || docker_build_args_need_buildx "$@"; then
    build_cmd=(docker buildx build --load)
    if [ -n "${NODOASSIST_DOCKER_BUILD_CACHE_FROM:-}" ]; then
      build_cmd+=(--cache-from "${NODOASSIST_DOCKER_BUILD_CACHE_FROM}")
    fi
    if [ -n "${NODOASSIST_DOCKER_BUILD_CACHE_TO:-}" ]; then
      build_cmd+=(--cache-to "${NODOASSIST_DOCKER_BUILD_CACHE_TO}")
    fi
  fi
...
Try increasing the Docker/BuildKit memory limit, closing other memory-heavy processes, or rebuilding with a smaller NodoAssist build heap, for example:
  NODOASSIST_DOCKER_BUILD_NODE_OPTIONS=--max-old-space-size=4096 NODOASSIST_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=4096 ./scripts/docker/setup.sh
EOF
}

docker_build_retry_count() {
  local configured="${NODOASSIST_DOCKER_BUILD_RETRIES:-2}"
  if [[ "$configured" =~ ^[0-9]+$ ]]; then
    echo "$((10#$configured))"
    return 0
  fi
  echo "invalid NODOASSIST_DOCKER_BUILD_RETRIES: $configured" >&2
  return 2
}

docker_build_timeout_required() {
  case "${NODOASSIST_DOCKER_BUILD_REQUIRE_TIMEOUT:-0}" in
    1 | true | TRUE | yes | YES)
      return 0
      ;;
  esac
...
docker_build_heartbeat_seconds() {
  local configured="${NODOASSIST_DOCKER_BUILD_HEARTBEAT_SECONDS:-30}"
  if [[ "$configured" =~ ^[0-9]+$ ]] && [ "$configured" -ge 1 ]; then
    echo "$((10#$configured))"
    return 0
  fi
  echo "invalid NODOASSIST_DOCKER_BUILD_HEARTBEAT_SECONDS: $configured" >&2
  return 2
}
...
docker_build_with_retries() {
  ...
  local timeout_value="${NODOASSIST_DOCKER_BUILD_TIMEOUT:-3600s}"
  while true; do
...
docker_build_run() {
  ...
  NODOASSIST_DOCKER_BUILD_REQUIRE_TIMEOUT="${NODOASSIST_DOCKER_BUILD_REQUIRE_TIMEOUT:-1}" \
