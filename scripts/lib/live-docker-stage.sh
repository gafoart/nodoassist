...
nodoassist_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${NODOASSIST_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "NODOASSIST_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
nodoassist_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
...
nodoassist_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
nodoassist_live_scrub_staged_plugin_index() {
  local dest_dir="${1:?destination directory required}"
  local db_path="$dest_dir/state/nodoassist.sqlite"
...
nodoassist_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.nodoassist"
  ...
  if [ -d "$source_dir" ]; then
    ...
    nodoassist_live_scrub_staged_plugin_index "$dest_dir"
    if [ -d "$source_dir/workspace" ] && [ ! -e "$dest_dir/workspace" ]; then
      ln -s "$source_dir/workspace" "$dest_dir/workspace"
    fi
  fi

  export NODOASSIST_STATE_DIR="$dest_dir"
  export NODOASSIST_CONFIG_PATH="$dest_dir/nodoassist.json"
}

nodoassist_live_prepare_staged_config() {
  if [ ! -f "${NODOASSIST_CONFIG_PATH:-}" ]; then
    return 0
  fi
