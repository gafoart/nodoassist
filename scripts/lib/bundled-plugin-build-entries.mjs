...
const BUNDLED_PLUGIN_BUILD_IDS_ENV = "NODOASSIST_BUNDLED_PLUGIN_BUILD_IDS";
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  if (params.packageJson?.nodoassist?.release?.publishToNpm === true) {
    return false;
  }
  const packageName = typeof params.packageJson?.name === "string" ? params.packageJson.name : "";
  if (packageName !== `@nodoassist/${params.dirName}`) {
    return false;
  }
...
function shouldBuildBundledDistEntry(packageJson) {
  return packageJson?.nodoassist?.build?.bundledDist !== false;
}
...
export function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.nodoassist?.extensions)
    ? packageJson.nodoassist.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.nodoassist?.setupEntry === "string" &&
    packageJson.nodoassist.setupEntry.trim().length > 0
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const candidate of collectBundledPluginCandidates(cwd, extensionsRoot)) {
    ...
    const hasManifest =
      relativeFiles?.includes("nodoassist.plugin.json") ?? fs.existsSync(manifestPath);
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "nodoassist.plugin.json"));
    }
    if (hasPackageJson) {
