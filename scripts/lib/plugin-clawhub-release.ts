// Plugin Clawhub Release script supports NodoAssist repository automation.
...
const NODOASSIST_PLUGIN_CLAWHUB_REPOSITORY = "nodoassist/nodoassist";
const NODOASSIST_PLUGIN_CLAWHUB_WORKFLOW_FILENAME = "plugin-clawhub-release.yml";
...
const CLAWHUB_SHARED_RELEASE_INPUT_PATHS = [
  ...
  "scripts/nodoassist-npm-release-check.ts",
...
export function collectClawHubPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    if (packageJson.nodoassist?.release?.publishToClawHub !== true) {
      continue;
    }
...
export function collectClawHubVersionGateErrors(params: {
  ...
  for (const plugin of changedPlugins) {
    ...
    if (baseManifest?.nodoassist?.release?.publishToClawHub !== true) {
      continue;
    }
    const baseVersion =
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    try {
      if (!retryRateLimit) {
        ...
        return isNodoAssistPluginTrustedPublisher(trustedPublisherDetail.trustedPublisher);
      }
    } finally {
      request.clearTimeout();
    }
...
function isNodoAssistPluginTrustedPublisher(value: unknown): boolean {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return false;
  }
  const trustedPublisher = value as ClawHubTrustedPublisherConfig;
  return (
    trustedPublisher.repository === NODOASSIST_PLUGIN_CLAWHUB_REPOSITORY &&
    trustedPublisher.workflowFilename === NODOASSIST_PLUGIN_CLAWHUB_WORKFLOW_FILENAME &&
