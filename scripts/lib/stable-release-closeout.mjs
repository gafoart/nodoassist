...
function isCloseoutEvidenceAsset(assetName, tag) {
  ...
  return (
    assetName === `nodoassist-${releaseVersion}-stable-main-closeout.json` ||
    assetName === `nodoassist-${releaseVersion}-stable-main-closeout.json.sha256`
  );
}
...
export function verifyStableMainCloseout(params) {
  ...
  const expectedMacAssets = [
    `NodoAssist-${macAssetVersion}.zip`,
    `NodoAssist-${macAssetVersion}.dmg`,
    `NodoAssist-${macAssetVersion}.dSYM.zip`,
  ];
