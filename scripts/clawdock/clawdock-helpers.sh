#!/usr/bin/env bash
# ClawDock - Docker helpers for NodoAssist
# Inspired by Simon Willison's "Running NodoAssist in Docker"
...
# Common paths to check for NodoAssist
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/nodoassist"
  "${HOME}/workspace/nodoassist"
  "${HOME}/projects/nodoassist"
  "${HOME}/dev/nodoassist"
  "${HOME}/code/nodoassist"
  "${HOME}/src/nodoassist"
)
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port nodoassist-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "👾 Found NodoAssist at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/nodoassist"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ NodoAssist not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/openclaw/openclaw.git ~/nodoassist"
    echo "  cd ~/nodoassist && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^NODOASSIST_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d nodoassist-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart nodoassist-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f nodoassist-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.nodoassist"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show nodoassist.json
  if [[ -f "${config_dir}/nodoassist.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/nodoassist.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/nodoassist.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No nodoassist.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec nodoassist-gateway \
    bash -c 'echo "alias nodoassist=\"./nodoassist.mjs\"" > /tmp/.bashrc_nodoassist && bash --rcfile /tmp/.bashrc_nodoassist'
}
...
clawdock-cli() {
  _clawdock_compose run --rm nodoassist-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build nodoassist-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d nodoassist-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build nodoassist-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "NODOASSIST_GATEWAY_TOKEN=$token" nodoassist-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" nodoassist-gateway \
    bash -c './nodoassist.mjs config set gateway.remote.token "$TOKEN" && ./nodoassist.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec nodoassist-gateway \
    bash -c "./nodoassist.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart nodoassist-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps nodoassist-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec nodoassist-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'nodoassist config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}👾 ClawDock - Docker Helpers for NodoAssist${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (nodoassist alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to nodoassist project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.nodoassist)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'nodoassist channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'nodoassist status')"
  echo ""
