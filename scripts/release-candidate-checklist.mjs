...
const WINDOWS_NODE_REPO = "nodoassist/nodoassist-windows-node";
const WINDOWS_NODE_REQUIRED_ASSETS = [
  "NodoAssistCompanion-Setup-x64.exe",
  "NodoAssistCompanion-Setup-arm64.exe",
];
...
NodoAssist Release Publish command only after everything is green.
...
Options:
  ...
  --npm-preflight-run <id>            Reuse successful NodoAssist NPM Release preflight run.
...
export function parseArgs(argv) {
  ...
  if (options.pluginPublishScope === "selected") {
    throw new Error(
      "--plugin-publish-scope selected is only for plugin-only repair publishes; release candidates publish NodoAssist with --plugin-publish-scope all-publishable",
    );
  }
...
function githubApiTimeoutMs() {
  const raw = process.env.NODOASSIST_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
...
export function buildPublishCommand(options) {
  ...
  return [
    ...
    "workflow",
    "run",
    "nodoassist-release-publish.yml",
    "--repo",
    options.repo,
    "--ref",
    options.workflowRef,
...
async function runTelegramIfNeeded(options, artifactName) {
  ...
  const runId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `nodoassist@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "nodoassist-npm-release.yml";
  ...
  const npmRun = await waitForSuccessfulRun(options.repo, options.npmPreflightRunId, {
    workflowName: "NodoAssist NPM Release",
    workflowRef: options.workflowRef,
  });
  ...
  const npmArtifactName = await downloadResolvedArtifact(
    ...
    `nodoassist-npm-preflight-${options.tag}`,
