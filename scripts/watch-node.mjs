...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.NODOASSIST_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[nodoassist] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[nodoassist] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[nodoassist] This usually means a file in node_modules is empty or truncated.");
  console.error("[nodoassist] Recommended recovery:");
  console.error("[nodoassist]   rm -rf node_modules");
  console.error("[nodoassist]   pnpm store prune");
  console.error("[nodoassist]   pnpm install");
  console.error("");
  console.error("[nodoassist] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.NODOASSIST_WATCH_MODE = "1";
  childEnv.NODOASSIST_WATCH_SESSION = watchSession;
  ...
  childEnv.NODOASSIST_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.NODOASSIST_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `nodoassist doctor --fix --non-interactive` once.",
        deps,
      );
