...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[nodoassist] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".nodoassist");
  const isolatedConfigPath = path.join(isolatedStateDir, "nodoassist.json");
  const shellSource = [
    'echo "$$" > "$NODOASSIST_WATCH_PID_FILE"',
    'mkdir -p "$NODOASSIST_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$NODOASSIST_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    NODOASSIST_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    NODOASSIST_HOME: isolatedHomeDir,
    NODOASSIST_CONFIG_PATH: isolatedConfigPath,
    NODOASSIST_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-gateway-watch-"));
