...
const DEFAULT_OUTPUT_NAME = "nodoassist-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "NODOASSIST_TRUSTED_PACKAGE_TOKEN";
...
export const NODOASSIST_PACKAGE_SPEC_RE =
  /^nodoassist@(alpha|beta|extended-stable|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-nodoassist-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedNodoAssistTarballFilename(value) {
  ...
  if (
    ...
    throw new Error(
      `npm pack reported unsafe NodoAssist tarball filename: ${JSON.stringify(filename)}`,
    );
  }
  return filename;
}

export function validateNodoAssistPackageSpec(spec) {
  if (!NODOASSIST_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be nodoassist@alpha, nodoassist@beta, nodoassist@extended-stable, nodoassist@latest, or an exact NodoAssist release version; got: ${spec}`,
    );
  }
}

export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateNodoAssistPackageSpec(packageSpec);
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an NodoAssist branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `nodoassist-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (packedFilename) {
      filename = resolvePackedNodoAssistTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        (trimmed.startsWith("nodoassist-") ||
          trimmed.includes(":") ||
          trimmed.includes("/") ||
          trimmed.includes("\\"))
      ) {
        filename = resolvePackedNodoAssistTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => {
        try {
          return resolvePackedNodoAssistTarballFilename(entry) === entry;
        } catch {
          return false;
        }
      })
  ...
  if (!filename) {
    throw new Error(`npm pack produced no NodoAssist tarball in ${outputDir}`);
  }
...
export const cleanPackedNodoAssistTarballsForTest = cleanPackedNodoAssistTarballs;
...
async function resolveCandidate(options) {
  ...
  console.error(`Checking NodoAssist package tarball: ${target}`);
  const checkStartedAt = Date.now();
  await run("node", ["scripts/check-nodoassist-package-tarball.mjs", target], {
    timeoutMs: 5 * 60 * 1000,
  });
  console.error(
  ...
  if (pkg.name !== "nodoassist") {
    throw new Error(
      `package candidate must be named "nodoassist"; got: ${pkg.name || "<missing>"}`,
    );
  }
  if (!pkg.version) {
