...
// Telegram User Crabbox Proof script supports NodoAssist repository automation.
...
import { clampTimerTimeoutMs } from "@nodoassist/normalization-core/number-coercion";
...
const REMOTE_ROOT = "/tmp/nodoassist-telegram-user-crabbox";
...
export function readTelegramUserProofLogTailBytes(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("NODOASSIST_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES", 256 * 1024, env);
}
...
function usageText() {
  return [
    "Usage:",
    "  node --import tsx scripts/e2e/telegram-user-crabbox-proof.ts [probe] [--text /status] [--expect NodoAssist]",
    ...
    "  --repo <owner/name>           GitHub repo for publish. Default: nodoassist/nodoassist.",
...
export function parseArgs(argvInput: string[]): Options {
  ...
  const opts: Options = {
    ...
    command,
    crabboxBin: trimToValue(process.env.NODOASSIST_TELEGRAM_USER_CRABBOX_BIN) ?? "crabbox",
    desktopChatTitle:
      trimToValue(process.env.NODOASSIST_TELEGRAM_USER_DESKTOP_CHAT_TITLE) ?? "NodoAssist Testing",
    dryRun: false,
    expect: ["NodoAssist"],
    gatewayPort: 19_879,
    idleTimeout: "60m",
    keepBox: false,
    mockResponseText: "NODOASSIST_E2E_OK",
    mockPort: 19_882,
    outputDir: path.join(DEFAULT_OUTPUT_ROOT, createTelegramProofRunId()),
    previewCropWidth: TELEGRAM_PROOF_CROP.cropWidth,
    previewFps: 24,
    previewWidth: 1920,
    provider: process.env.NODOASSIST_TELEGRAM_USER_CRABBOX_PROVIDER?.trim() || "aws",
    publishFullArtifacts: false,
    publishRepo: "nodoassist/nodoassist",
    recordFps: 24,
    recordSeconds: 35,
    remoteCommand: [],
    target: "linux",
    ...
    ttl: "120m",
    userDriverScript:
      trimToValue(process.env.NODOASSIST_TELEGRAM_USER_DRIVER_SCRIPT) ?? DEFAULT_USER_DRIVER,
  };
...
function repoRoot() {
  ...
  if (
    ...
    throw new Error("Run from the NodoAssist repo root.");
  }
  return cwd;
}
...
function childProcessBaseEnv() {
  const keys = [
    ...
    "LC_ALL",
    "NODE_OPTIONS",
    "NODOASSIST_BUILD_PRIVATE_QA",
    "NODOASSIST_ENABLE_PRIVATE_QA_CLI",
    "PATH",
    "PNPM_HOME",
    "SHELL",
    "TEMP",
...
function gatewayEnv(params: { configPath: string; stateDir: string; sutToken: string }) {
  return {
    ...childProcessBaseEnv(),
    OPENAI_API_KEY: "sk-nodoassist-e2e-mock",
...
export function createNodoAssistGatewaySpawnSpec(params: {
  ...
  const spec = createPnpmRunnerSpawnSpec({
    ...
    pnpmArgs: ["nodoassist", "gateway", "--port", String(params.gatewayPort)],
  });
  return {
    args: spec.args,
...
function writeSutConfig(params: {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-tg-crabbox-sut-"));
...
type StartLocalSutDeps = {
  createGatewaySpawnSpec?: typeof createNodoAssistGatewaySpawnSpec;
...
export async function startLocalSut(
  ...
  const createGatewaySpawnSpec = deps.createGatewaySpawnSpec ?? createNodoAssistGatewaySpawnSpec;
...
async function startLocalSutDaemon(params: {
  ...
  try {
    ...
    const gatewaySpec = createNodoAssistGatewaySpawnSpec({
      env: gatewayEnvVars,
      gatewayPort: params.gatewayPort,
      repoRoot: params.repoRoot,
    });
...
tdlib_url=${tdlibUrl}
setup_step_timeout_kill_after="\${NODOASSIST_TELEGRAM_USER_SETUP_KILL_AFTER_SECONDS:-30}s"
apt_timeout="\${NODOASSIST_TELEGRAM_USER_APT_TIMEOUT_SECONDS:-900}s"
download_timeout="\${NODOASSIST_TELEGRAM_USER_DOWNLOAD_TIMEOUT_SECONDS:-600}"
download_connect_timeout="\${NODOASSIST_TELEGRAM_USER_DOWNLOAD_CONNECT_TIMEOUT_SECONDS:-15}"
download_retries="\${NODOASSIST_TELEGRAM_USER_DOWNLOAD_RETRIES:-3}"
download_retry_delay="\${NODOASSIST_TELEGRAM_USER_DOWNLOAD_RETRY_DELAY_SECONDS:-5}"
tdlib_clone_timeout="\${NODOASSIST_TELEGRAM_USER_TDLIB_CLONE_TIMEOUT_SECONDS:-600}s"
tdlib_build_timeout="\${NODOASSIST_TELEGRAM_USER_TDLIB_BUILD_TIMEOUT_SECONDS:-1800}s"
run_setup_step() {
  local label="$1"
...
async function startSession(root: string, opts: Options, outputDir: string) {
  ...
  const hasConvexEnv =
    trimToValue(process.env.NODOASSIST_QA_CONVEX_SITE_URL) &&
    trimToValue(process.env.NODOASSIST_QA_CONVEX_SECRET_CI);
  ...
  try {
    ...
    return {
      ...
      commands: {
        send: `nodoassist-telegram-user-crabbox-proof send --session ${path.relative(root, pathname)} --text '/status'`,
        view: `nodoassist-telegram-user-crabbox-proof view --session ${path.relative(root, pathname)} --message-id <message-id>`,
        run: `nodoassist-telegram-user-crabbox-proof run --session ${path.relative(root, pathname)} -- bash -lc 'source ${REMOTE_ROOT}/env.sh && python3 ${REMOTE_ROOT}/user-driver.py transcript --limit 20 --json'`,
        finish: `nodoassist-telegram-user-crabbox-proof finish --session ${path.relative(root, pathname)} --preview-crop telegram-window`,
      },
    };
  } catch (error) {
...
async function main() {
  ...
  const localRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-telegram-crabbox-"));
  const summary: JsonObject = {
    artifacts: {},
