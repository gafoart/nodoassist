// Filesystem script supports NodoAssist repository automation.
...
const NODOASSIST_VERSION_PATTERN = /NodoAssist\s+([0-9][^\s]*)/gi;
...
export async function extractLastNodoAssistVersionFromLog(
  logPath: string,
  pattern = NODOASSIST_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.NODOASSIST_PARALLELS_ARTIFACT_ROOT ||
