// Powershell script supports NodoAssist repository automation.
...
export function windowsAgentTurnConfigPatchScript(modelId: string): string {
  ...
  return `$agentTurnConfigPatchPath = $env:NODOASSIST_CONFIG_PATH
if (-not $agentTurnConfigPatchPath) { $agentTurnConfigPatchPath = Join-Path $env:USERPROFILE '.nodoassist\\nodoassist.json' }
$agentTurnVersionText = Invoke-NodoAssist --version 2>$null | Out-String
$agentTurnRuntimePolicySupported = $false
if ($agentTurnVersionText -match 'NodoAssist\\s+(\\d{4})\\.(\\d{1,2})\\.(\\d{1,2})') {
...
$env:NODOASSIST_PARALLELS_AGENT_CONFIG_PATCH = @'
${payloadJson}
'@
$env:NODOASSIST_PARALLELS_AGENT_CONFIG_PATH = $agentTurnConfigPatchPath
$env:NODOASSIST_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED = if ($agentTurnRuntimePolicySupported) { '1' } else { '0' }
$agentTurnConfigPatchScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'nodoassist-agent-turn-config-patch.cjs'
@'
const fs = require("node:fs");
const path = require("node:path");
const configPath = process.env.NODOASSIST_PARALLELS_AGENT_CONFIG_PATH;
const payload = JSON.parse(process.env.NODOASSIST_PARALLELS_AGENT_CONFIG_PATCH || "{}");
const canWriteAgentRuntime = process.env.NODOASSIST_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED === "1";
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@nodoassist", "codex"), { recursive: true, force: true });
...
if (selectedModelEntry && typeof selectedModelEntry === "object" && !Array.isArray(selectedModelEntry)) {
  if (canWriteAgentRuntime) {
    selectedModelEntry.agentRuntime = { id: "nodoassist" };
  } else {
...
Remove-Item Env:NODOASSIST_PARALLELS_AGENT_CONFIG_PATCH -Force -ErrorAction SilentlyContinue
Remove-Item Env:NODOASSIST_PARALLELS_AGENT_CONFIG_PATH -Force -ErrorAction SilentlyContinue
Remove-Item Env:NODOASSIST_PARALLELS_AGENT_RUNTIME_POLICY_SUPPORTED -Force -ErrorAction SilentlyContinue
if ($agentTurnConfigPatchExit -ne 0) { throw "agent turn config patch failed" }`;
}

export const windowsNodoAssistResolver = String.raw`$portableNode = if ($env:LOCALAPPDATA) { Join-Path $env:LOCALAPPDATA 'Programs\nodejs' } else { $null }
if ($portableNode -and (Test-Path (Join-Path $portableNode 'node.exe'))) {
  $env:PATH = "$portableNode;$env:PATH"
}
function Resolve-NodoAssistCommand {
  if ($script:NodoAssistResolvedCommand) { return $script:NodoAssistResolvedCommand }
  $shimCandidates = @()
  if ($env:APPDATA) {
    $shimCandidates += Join-Path $env:APPDATA 'npm\nodoassist.cmd'
    $shimCandidates += Join-Path $env:APPDATA 'npm\nodoassist.ps1'
  }
  foreach ($name in @('nodoassist.cmd', 'nodoassist.ps1', 'nodoassist')) {
  ...
  if ($npmPrefix) {
    $shimCandidates += Join-Path $npmPrefix 'nodoassist.cmd'
    $shimCandidates += Join-Path $npmPrefix 'nodoassist.ps1'
  }
  foreach ($candidate in $shimCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:NodoAssistResolvedCommand = @{ Kind = 'shim'; Path = $candidate }
      return $script:NodoAssistResolvedCommand
    }
  }
  $entryCandidates = @()
  if ($env:APPDATA) {
  ...
  if ($npmPrefix) {
    $entryCandidates += Join-Path $npmPrefix 'node_modules\nodoassist\nodoassist.mjs'
  }
  foreach ($candidate in $entryCandidates) {
    if ($candidate -and (Test-Path $candidate)) {
      $script:NodoAssistResolvedCommand = @{ Kind = 'node'; Path = $candidate }
      return $script:NodoAssistResolvedCommand
    }
  }
  throw 'nodoassist command not found in PATH, APPDATA npm, or npm global prefix'
}
function Invoke-NodoAssist {
  param([Parameter(ValueFromRemainingArguments = $true)][string[]] $NodoAssistArgs)
  $command = Resolve-NodoAssistCommand
  ...
  try {
    if ($command.Kind -eq 'node') {
      & node.exe $command.Path @NodoAssistArgs
    } else {
      & $command.Path @NodoAssistArgs
    }
  } finally {
