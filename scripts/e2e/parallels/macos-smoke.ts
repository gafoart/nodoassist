...
// Macos Smoke script supports NodoAssist repository automation.
...
const guestNodoAssist = "nodoassist";
const guestNodoAssistEntry = '"$(npm root -g)/nodoassist/nodoassist.mjs"';
const guestNodoAssistEntryRunner = `node ${guestNodoAssistEntry}`;
...
class MacosSmoke {
  ...
  constructor(private options: MacosOptions) {
    ...
    this.agentTimeoutSeconds = readPositiveIntEnv(
      "NODOASSIST_PARALLELS_MACOS_AGENT_TIMEOUT_S",
      2700,
    );
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("nodoassist-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("nodoassist-parallels-macos-tgz.");
    try {
  ...
  private guestNodoAssistEntryExec(
    args: string[],
    options: { check?: boolean; env?: Record<string, string> } = {},
  ): string {
    ...
    return this.guestSh(
      `set -e
entry="$(npm root -g)/nodoassist/nodoassist.mjs"
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'nodoassist.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nodoassist-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nodoassist.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g nodoassist >/dev/null 2>&1 || true
rm -rf "$HOME/.nodoassist"
...
curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 ${shellQuote(
      ...
      )} -o /tmp/nodoassist-install.sh
bash /tmp/nodoassist-install.sh --version ${shellQuote(this.installVersion)}
${guestNodoAssist} --version`,
    );
  }
...
${guestNodoAssist} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestNodoAssist, "--version"]);
...
check_path "$root/nodoassist"
check_path "$root/nodoassist/extensions"
if [ -d "$root/nodoassist/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/nodoassist/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/nodoassist-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/nodoassist`)}
export PATH=${shellQuote(`/tmp/nodoassist-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".nodoassist", "nodoassist.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 NODOASSIST_DISABLE_BUNDLED_PLUGINS=1 ${guestNodoAssistEntryRunner} update --channel dev --yes --json
${guestNodoAssistEntryRunner} --version
${guestNodoAssistEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestNodoAssistEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'nodoassist.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nodoassist-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'nodoassist.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} NODOASSIST_HOME=${shellQuote(home)} NODOASSIST_STATE_DIR=${shellQuote(`${home}/.nodoassist`)} NODOASSIST_CONFIG_PATH=${shellQuote(
        `${home}/.nodoassist/nodoassist.json`,
      )} ${guestNodoAssistEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/nodoassist-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestNodoAssist(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestNodoAssist, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestNodoAssist(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestNodoAssist(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`nodoassist ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/nodoassist-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>NodoAssist Control</title>' /tmp/nodoassist-dashboard-smoke.html >/dev/null &&
      grep -F '<nodoassist-app></nodoassist-app>' /tmp/nodoassist-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/nodoassist-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestNodoAssistEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestNodoAssistEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestNodoAssistEntryExec([
      "config",
      "set",
    ...
    this.guestNodoAssistEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestNodoAssistEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestNodoAssistEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastNodoAssistVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
