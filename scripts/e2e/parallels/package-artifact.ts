// Package Artifact script supports NodoAssist repository automation.
...
export function resolveNodoAssistRegistryVersion(specOrAlias: string): string {
  const rawValue = specOrAlias.trim();
  const value = rawValue.startsWith("nodoassist@")
    ? rawValue.slice("nodoassist@".length)
    : rawValue;
  if (!value) {
    return "";
  }
  if (value === "latest" || value === "beta" || /^\d/.test(value)) {
    return npmViewVersion(`nodoassist@${value}`);
  }
  const betaMatch = /^beta(\d+)$/u.exec(value);
  if (betaMatch) {
    const betaSuffix = `-beta.${betaMatch[1]}`;
    const versions = JSON.parse(
      run("npm", ["view", "nodoassist", "versions", "--json"], { quiet: true }).stdout,
    ) as string[];
    const match = versions
      ...
      .at(-1);
    if (!match) {
      die(`no nodoassist registry version found for alias ${value}`);
    }
    return match;
  }
  return "";
...
export async function packNodoAssist(input: {
  destination: string;
  packageSpec?: string;
  requireControlUi?: boolean;
  ...
  return await withPackageLock(path.join(tmpdir(), "nodoassist-parallels-build.lock"), async () => {
    ...
    const packed = resolveNpmPackTarballFilename(JSON.parse(output).at(-1)?.filename);
    const tgzPath = path.join(input.destination, `nodoassist-main-${shortHead}.tgz`);
...
async function acquirePackageLock(
  ...
  const timeoutMs = readPositiveIntEnv("NODOASSIST_PARALLELS_PACKAGE_LOCK_TIMEOUT_MS", 30 * 60_000);
  const staleMs = readPositiveIntEnv("NODOASSIST_PARALLELS_PACKAGE_LOCK_STALE_MS", 2 * 60 * 60_000);
