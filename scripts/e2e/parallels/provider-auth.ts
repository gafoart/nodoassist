// Provider Auth script supports NodoAssist repository automation.
...
export function resolveProviderAuth(input: {
  ...
  const providerDefaults: Record<Provider, Omit<ProviderAuth, "apiKeyValue">> = {
    anthropic: {
      ...
      modelId:
        input.modelId ||
        process.env.NODOASSIST_PARALLELS_ANTHROPIC_MODEL ||
    ...
    minimax: {
      ...
      modelId:
        input.modelId || process.env.NODOASSIST_PARALLELS_MINIMAX_MODEL || "minimax/MiniMax-M2.7",
    },
    openai: {
      apiKeyEnv: input.apiKeyEnv || "OPENAI_API_KEY",
      authChoice: "openai-api-key",
      authKeyFlag: "openai-api-key",
      modelId: input.modelId || process.env.NODOASSIST_PARALLELS_OPENAI_MODEL || "openai/gpt-5.5",
    },
  };
...
export function resolveWindowsProviderAuth(input: {
  ...
  const windowsModel = process.env.NODOASSIST_PARALLELS_WINDOWS_OPENAI_MODEL?.trim();
  if (windowsModel) {
    return { ...auth, modelId: windowsModel };
  }
  if (process.env.NODOASSIST_PARALLELS_OPENAI_MODEL?.trim()) {
    return auth;
  }
...
export function resolveParallelsModelTimeoutSeconds(platform?: Platform): number {
  const platformEnvName =
    platform === undefined
      ? undefined
      : `NODOASSIST_PARALLELS_${platform.toUpperCase()}_MODEL_TIMEOUT_S`;
  ...
  return readPositiveIntEnv("NODOASSIST_PARALLELS_MODEL_TIMEOUT_S", defaultSeconds);
}
...
export function resolveLatestVersion(
  ...
  const userConfigDir = createTempDir(path.join(resolveTempDir(), "nodoassist-npm-"));
  const userConfigPath = path.join(userConfigDir, "npmrc");
  try {
    writeFile(userConfigPath, "", "utf8");
    return runCommand("npm", ["view", "nodoassist", "version", "--userconfig", userConfigPath], {
      quiet: true,
    }).stdout.trim();
  } finally {
