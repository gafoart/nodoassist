// Smoke Common helper supports NodoAssist script workflows.
import { readFile, rm } from "node:fs/promises";
import path from "node:path";
import { extractLastNodoAssistVersionFromLog } from "./filesystem.ts";
...
export async function packAndServeSmokeArtifact(
  ...
  const artifact = await packNodoAssist({
    destination: tgzDir,
    packageSpec,
    requireControlUi,
  });
...
export async function extractLastNodoAssistVersion(
  runDir: string,
  phaseName: string,
  pattern: RegExp,
): Promise<string> {
  return await extractLastNodoAssistVersionFromLog(path.join(runDir, `${phaseName}.log`), pattern);
}
