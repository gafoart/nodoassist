...
const PLUGIN_SPEC =
  process.env.NODOASSIST_KITCHEN_SINK_NPM_SPEC || "npm:@nodoassist/kitchen-sink@latest";
const PLUGIN_ID =
  process.env.NODOASSIST_KITCHEN_SINK_PLUGIN_ID || "nodoassist-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built NodoAssist entry.

Environment:
  NODOASSIST_ENTRY                         Built NodoAssist entrypoint. Defaults to dist/index.mjs or dist/index.js.
  NODOASSIST_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@nodoassist/kitchen-sink@latest.
  NODOASSIST_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: nodoassist-kitchen-sink-fixture.
  NODOASSIST_KITCHEN_SINK_PERSONALITY      Plugin fixture personality. Default: conformance.
  NODOASSIST_KITCHEN_SINK_RPC_PORT         Gateway loopback port. Default: OS-selected free port.
  NODOASSIST_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  NODOASSIST_KITCHEN_SINK_RPC_COMMAND_MS   NodoAssist command timeout.
  NODOASSIST_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  NODOASSIST_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  NODOASSIST_KITCHEN_SINK_RPC_FETCH_MS     HTTP readiness probe timeout.
  NODOASSIST_KITCHEN_SINK_RPC_FETCH_BODY_BYTES  HTTP readiness probe response ceiling.
  NODOASSIST_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  NODOASSIST_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  NODOASSIST_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS  Per-command stdout/stderr capture ceiling.
  NODOASSIST_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function resolveKitchenSinkRpcConfig(env = process.env) {
  const commandTimeoutMs = readPositiveTimerMs(
    env.NODOASSIST_KITCHEN_SINK_RPC_COMMAND_MS,
    DEFAULT_COMMAND_TIMEOUT_MS,
    "NODOASSIST_KITCHEN_SINK_RPC_COMMAND_MS",
  );
  return {
    commandMaxRssMiB: readPositiveInt(
      env.NODOASSIST_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
      DEFAULT_MAX_COMMAND_RSS_MIB,
      "NODOASSIST_KITCHEN_SINK_COMMAND_MAX_RSS_MIB",
    ),
    commandTimeoutMs,
    fetchBodyMaxBytes: readPositiveInt(
      env.NODOASSIST_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
      DEFAULT_FETCH_BODY_MAX_BYTES,
      "NODOASSIST_KITCHEN_SINK_RPC_FETCH_BODY_BYTES",
    ),
    fetchTimeoutMs: readPositiveTimerMs(
      env.NODOASSIST_KITCHEN_SINK_RPC_FETCH_MS,
      DEFAULT_FETCH_TIMEOUT_MS,
      "NODOASSIST_KITCHEN_SINK_RPC_FETCH_MS",
    ),
    installTimeoutMs: readPositiveTimerMs(
      env.NODOASSIST_KITCHEN_SINK_RPC_INSTALL_MS,
      Math.max(commandTimeoutMs, DEFAULT_INSTALL_TIMEOUT_MS),
      "NODOASSIST_KITCHEN_SINK_RPC_INSTALL_MS",
    ),
    maxRssMiB: readPositiveInt(
      env.NODOASSIST_KITCHEN_SINK_MAX_RSS_MIB,
      DEFAULT_MAX_RSS_MIB,
      "NODOASSIST_KITCHEN_SINK_MAX_RSS_MIB",
    ),
    outputCaptureChars: readPositiveInt(
      env.NODOASSIST_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
      DEFAULT_OUTPUT_CAPTURE_CHARS,
      "NODOASSIST_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS",
    ),
    readyTimeoutMs: readPositiveTimerMs(
      env.NODOASSIST_KITCHEN_SINK_RPC_READY_MS,
      DEFAULT_READY_TIMEOUT_MS,
      "NODOASSIST_KITCHEN_SINK_RPC_READY_MS",
    ),
    rpcTimeoutMs: readPositiveTimerMs(
      env.NODOASSIST_KITCHEN_SINK_RPC_CALL_MS,
      DEFAULT_RPC_TIMEOUT_MS,
      "NODOASSIST_KITCHEN_SINK_RPC_CALL_MS",
    ),
  };
}
...
export async function resolveKitchenSinkRpcPort(env = process.env, options = {}) {
  const rawPort = (env.NODOASSIST_KITCHEN_SINK_RPC_PORT || "").trim();
  if (rawPort) {
    const port = readPositiveInt(rawPort, 0, "NODOASSIST_KITCHEN_SINK_RPC_PORT");
    if (port > 65535) {
      throw new Error(
        `NODOASSIST_KITCHEN_SINK_RPC_PORT must be a TCP port from 1 to 65535. Got: ${JSON.stringify(rawPort)}`,
      );
    }
    return port;
  }
...
function resolveNodoAssistRunner() {
  if (process.env.NODOASSIST_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.NODOASSIST_ENTRY],
      label: process.env.NODOASSIST_ENTRY,
    };
  }
  ...
  return { pnpm: true, baseArgs: ["nodoassist"], label: "pnpm nodoassist" };
}

export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      NODOASSIST_SKIP_PROVIDERS: "0",
      NODOASSIST_KITCHEN_SINK_PERSONALITY:
        process.env.NODOASSIST_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
function resolveCommandParentSignalKillGraceMs(env) {
  const raw = env.VITEST && env.NODOASSIST_TEST_KITCHEN_SINK_PARENT_SIGNAL_KILL_GRACE_MS;
  if (!raw) {
...
async function runNodoAssist(runner, args, env, options = {}) {
  const config = resolveKitchenSinkRpcConfig(env);
  const command = await resolveNodoAssistCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveNodoAssistCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.NODOASSIST_CONFIG_PATH),
        configPath: options.env.NODOASSIST_CONFIG_PATH,
...
async function loadCallGatewayModule(runner) {
  if (!usesBuiltNodoAssistEntry(runner)) {
    return null;
  }
...
export function usesBuiltNodoAssistEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.NODOASSIST_ENTRY && entry === env.NODOASSIST_ENTRY) {
    return true;
  }
...
function configureKitchenSink(env, port) {
  const configPath = env.NODOASSIST_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.NODOASSIST_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function startGateway(runner, port, env, logPath) {
  ...
  const command = await resolveNodoAssistCommand(
    runner,
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("nodoassist-gateway"),
    );
...
export async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveNodoAssistRunner();
  const port = await resolveKitchenSinkRpcPort();
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.NODOASSIST_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runNodoAssist(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.NODOASSIST_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
