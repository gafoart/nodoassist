// Cron Mcp Cleanup Docker Client script supports NodoAssist repository automation.
...
export function readCronMcpCleanupProbePidWaitMs(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("NODOASSIST_CRON_MCP_CLEANUP_PID_WAIT_MS", 120_000, env);
}
...
async function waitForProbeExit(params: {
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (!args || !args.includes("nodoassist-cron-mcp-cleanup-probe")) {
      return;
    }
    await delay(100);
  }
  ...
  while (Date.now() - startedAt < params.timeoutMs) {
    ...
    if (observed.length > 0) {
      ...
      for (const pid of observed) {
        const args = await describeProbePid(pid);
        if (args?.includes("nodoassist-cron-mcp-cleanup-probe")) {
          allExited = false;
          break;
        }
      }
...
async function runCronCleanupScenario(params: {
  ...
  assert(
    initialArgs === undefined || initialArgs.includes("nodoassist-cron-mcp-cleanup-probe"),
...
async function main() {
  ...
  const stateDir =
    process.env.NODOASSIST_STATE_DIR?.trim() || path.join(os.homedir(), ".nodoassist");
