// Mcp Code Mode Gateway Client script supports NodoAssist repository automation.
...
export function readMcpCodeModeClientFetchLimits(
  ...
  return {
    ...
    timeoutMs: readPositiveIntEnv("NODOASSIST_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", 300_000, env),
  };
}
...
async function main() {
  ...
  const model = process.env.NODOASSIST_MCP_CODE_MODE_MODEL?.trim() || "nodoassist/main";
  assert(gatewayUrl, "missing GW_URL");
  assert(gatewayToken, "missing GW_TOKEN");
  assert(stateDir, "missing NODOASSIST_STATE_DIR");
