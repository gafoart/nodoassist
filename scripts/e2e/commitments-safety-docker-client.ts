...
async function withStateDir<T>(name: string, fn: (stateDir: string) => Promise<T>): Promise<T> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `nodoassist-${name}-`));
  const previousStateDir = process.env.NODOASSIST_STATE_DIR;
  try {
    setEnvValue("NODOASSIST_STATE_DIR", root);
    return await fn(root);
  } finally {
    resetCommitmentExtractionRuntimeForTests();
    if (previousStateDir === undefined) {
      deleteEnvValue("NODOASSIST_STATE_DIR");
    } else {
      setEnvValue("NODOASSIST_STATE_DIR", previousStateDir);
    }
