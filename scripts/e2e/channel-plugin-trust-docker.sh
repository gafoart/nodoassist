...
#   mode against the installed NodoAssist package.
#
# Parameters:
#   --container: run the in-container scenario. Host mode is the default.
#   NODOASSIST_CHANNEL_PLUGIN_TRUST_E2E_IMAGE: override the Docker image name.
#   NODOASSIST_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1: reuse/pull the image.
#
# Outputs:
...
Description:
  Proves the packaged NodoAssist CLI enforces local channel plugin trust for
...
Environment:
  NODOASSIST_CHANNEL_PLUGIN_TRUST_E2E_IMAGE       Override Docker image name.
  NODOASSIST_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD  Reuse/pull image instead of building.
  NODOASSIST_TEST_STATE_SCRIPT_B64                Required in --container mode.
...
Examples:
  ...
  NODOASSIST_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1 bash scripts/e2e/channel-plugin-trust-docker.sh
EOF
}
...
run_nodoassist() {
  ...
  if [ -f /app/nodoassist.mjs ]; then
    node /app/nodoassist.mjs "$@"
    return
  fi
  echo "nodoassist CLI not found in Docker image" >&2
  exit 1
}
...
EOF
  ...
  cat >"$plugin_dir/nodoassist.plugin.json" <<EOF
{
  "id": "$plugin_id",
...
write_case_config() {
  ...
  if [ "$trusted" = "1" ]; then
    cat >"$NODOASSIST_CONFIG_PATH" <<EOF
{
  "plugins": {
    "enabled": true,
    "allow": ["$plugin_id"],
...
run_case() {
  ...
  local trusted="${2:?missing trusted flag}"
  local scratch
  scratch="$(mktemp -d "/tmp/nodoassist-channel-plugin-trust-$case_id.XXXXXX")"
  ...
  PLUGINTRUST_IMPORT_MARKER="$marker_dir/import.marker" \
    ...
    run_nodoassist channels add --channel e2e-load-paths --token "$canary" \
...
run_container() {
  ...
  export NODOASSIST_WORKSPACE_DIR="$HOME/.nodoassist/workspace"

  run_nodoassist --version
...
run_host() {
  ...
  image_name="$(
    docker_e2e_resolve_image \
      "nodoassist-channel-plugin-trust-e2e:local" \
      NODOASSIST_CHANNEL_PLUGIN_TRUST_E2E_IMAGE
  )"
  local skip_build="${NODOASSIST_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD:-0}"
  ...
  docker_e2e_run_logged_print_with_harness \
    ...
    -e "NODOASSIST_TEST_STATE_SCRIPT_B64=$state_script_b64" \
    "$image_name" \
    bash scripts/e2e/channel-plugin-trust-docker.sh --container
}
