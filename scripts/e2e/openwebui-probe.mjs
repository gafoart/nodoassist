...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.NODOASSIST_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "nodoassist/default") ??
      modelIds.find((id) => id === "nodoassist");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing nodoassist model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `nodoassist model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
