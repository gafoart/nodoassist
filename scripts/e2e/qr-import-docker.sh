...
IMAGE_NAME="${NODOASSIST_QR_SMOKE_IMAGE:-nodoassist-qr-smoke}"
DOCKER_BUILD_ARGS=()

if [[ "${NODOASSIST_QR_SMOKE_FORCE_INSTALL:-0}" == "1" ]]; then
  INSTALL_CACHE_BUSTER="${GITHUB_SHA:-manual}-${GITHUB_RUN_ID:-$(date +%s)}-${GITHUB_RUN_ATTEMPT:-0}"
  DOCKER_BUILD_ARGS+=(
    --build-arg
    "NODOASSIST_QR_INSTALL_CACHE_BUSTER=${INSTALL_CACHE_BUSTER}"
  )
fi
