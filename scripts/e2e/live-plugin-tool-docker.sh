...
IMAGE_NAME="$(docker_e2e_resolve_image "nodoassist-live-plugin-tool-e2e" NODOASSIST_LIVE_PLUGIN_TOOL_E2E_IMAGE)"
DOCKER_TARGET="${NODOASSIST_LIVE_PLUGIN_TOOL_DOCKER_TARGET:-bare}"
HOST_BUILD="${NODOASSIST_LIVE_PLUGIN_TOOL_HOST_BUILD:-1}"
PACKAGE_TGZ="${NODOASSIST_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${NODOASSIST_LIVE_PLUGIN_TOOL_PROFILE_FILE:-${NODOASSIST_TESTBOX_PROFILE_FILE:-$HOME/.nodoassist-testbox-live.profile}}"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${NODOASSIST_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "NODOASSIST_LIVE_PLUGIN_TOOL_HOST_BUILD=0 requires NODOASSIST_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
NODOASSIST_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 live-plugin-tool empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e NODOASSIST_LIVE_PLUGIN_TOOL_MODEL="${NODOASSIST_LIVE_PLUGIN_TOOL_MODEL:-openai/gpt-5.5}" \
  ...
  -e "NODOASSIST_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
...
MODEL_REF="${NODOASSIST_LIVE_PLUGIN_TOOL_MODEL:?missing NODOASSIST_LIVE_PLUGIN_TOOL_MODEL}"
PLUGIN_ID="e2e-slug-tool"
PLUGIN_NAME="@nodoassist/e2e-slug-tool"
PLUGIN_VERSION="0.0.0-e2e.1"
TOOL_NAME="e2e_slug_probe"
SEED="NodoAssist E2E Plugin Tool $(date +%s)-$RANDOM"
...
dump_debug_logs() {
  local status="$1"
  local agent_output_dump_bytes="${NODOASSIST_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES:-16384}"
  echo "Live plugin tool scenario failed with exit code $status" >&2
  if [ -f /tmp/nodoassist-agent.json ]; then
    echo "--- /tmp/nodoassist-agent.json (last ${agent_output_dump_bytes} bytes) ---" >&2
    tail -c "$agent_output_dump_bytes" /tmp/nodoassist-agent.json >&2 || true
    echo >&2
  fi
  nodoassist_e2e_dump_logs \
    /tmp/nodoassist-install.log \
    /tmp/nodoassist-plugin-install.log \
    /tmp/nodoassist-plugin-enable.log \
    /tmp/nodoassist-plugins-list.json \
    /tmp/nodoassist-plugin-inspect.json \
    /tmp/nodoassist-live-plugin-tool-pack.log \
...
fixture_dir="$(mktemp -d /tmp/nodoassist-live-plugin-tool.XXXXXX)"
...
(cd "$plugin_dir" && npm pack --pack-destination "$fixture_dir" --silent) \
  >/tmp/nodoassist-live-plugin-tool-pack.log 2>&1
plugin_tgzs=()
...
if [ "${#plugin_tgzs[@]}" -ne 1 ]; then
  ...
  nodoassist_e2e_dump_logs /tmp/nodoassist-live-plugin-tool-pack.log
  exit 1
fi
plugin_tgz="${plugin_tgzs[0]}"

echo "Installing fixture plugin from npm-pack: $plugin_tgz"
nodoassist plugins install "npm-pack:$plugin_tgz" --force >/tmp/nodoassist-plugin-install.log 2>&1
node scripts/e2e/lib/live-plugin-tool/assertions.mjs configure
nodoassist plugins enable "$PLUGIN_ID" >/tmp/nodoassist-plugin-enable.log 2>&1
nodoassist plugins list --json >/tmp/nodoassist-plugins-list.json
nodoassist plugins inspect "$PLUGIN_ID" --runtime --json >/tmp/nodoassist-plugin-inspect.json
...
nodoassist agent --local \
  ...
  --thinking off \
  --timeout "${NODOASSIST_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS:-300}" \
  --json >/tmp/nodoassist-agent.json 2>/tmp/nodoassist-agent.err
