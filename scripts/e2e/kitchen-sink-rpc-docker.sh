...
IMAGE_NAME="$(docker_e2e_resolve_image "nodoassist-kitchen-sink-rpc-e2e" NODOASSIST_KITCHEN_SINK_RPC_E2E_IMAGE)"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env NODOASSIST_KITCHEN_SINK_MAX_MEMORY_MIB 2048)"
...
CONTAINER_NAME="nodoassist-kitchen-sink-rpc-e2e-$$"
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/nodoassist-kitchen-sink-rpc.XXXXXX")"
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/nodoassist-kitchen-sink-rpc-stats.XXXXXX")"
...
DOCKER_ENV_ARGS=(
  ...
  -e NODOASSIST_ENTRY=/app/nodoassist.mjs
)

for env_name in \
  NODOASSIST_KITCHEN_SINK_NPM_SPEC \
  NODOASSIST_KITCHEN_SINK_PLUGIN_ID \
  ...
  NODOASSIST_KITCHEN_SINK_RPC_PORT \
  ...
  NODOASSIST_KITCHEN_SINK_KEEP_TMP \
  NODOASSIST_KITCHEN_SINK_MAX_RSS_MIB \
  NODOASSIST_KITCHEN_SINK_COMMAND_MAX_RSS_MIB; do
  env_value="${!env_name:-}"
