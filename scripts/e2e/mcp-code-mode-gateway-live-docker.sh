...
IMAGE_NAME="$(docker_e2e_resolve_image "nodoassist-mcp-code-mode-gateway-live-e2e" NODOASSIST_IMAGE)"
PORT="$(docker_e2e_read_tcp_port_env NODOASSIST_MCP_CODE_MODE_LIVE_GATEWAY_PORT 18789)"
...
CONTAINER_NAME="nodoassist-mcp-code-mode-live-e2e-$$"
PROFILE_FILE="${NODOASSIST_MCP_CODE_MODE_LIVE_PROFILE_FILE:-${NODOASSIST_TESTBOX_PROFILE_FILE:-$HOME/.nodoassist-testbox-live.profile}}"

CLIENT_LOG="$(mktemp -t nodoassist-mcp-code-mode-live-log.XXXXXX)"
...
NODOASSIST_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-code-mode-gateway-live empty)"

# The profile is only a credential source. Keep this lane's NodoAssist runtime
# isolated from host/testbox mode flags that can change packaged behavior.
unset NODOASSIST_TESTBOX
...
docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "NODOASSIST_DOCKER_OPENAI_BASE_URL=${OPENAI_BASE_URL:-https://api.openai.com/v1}" \
  ...
  -e "NODOASSIST_SKIP_ACPX_RUNTIME=1" \
  -e "NODOASSIST_SKIP_ACPX_RUNTIME_PROBE=1" \
  ...
  -e "NODOASSIST_MCP_CODE_MODE_MODEL=${NODOASSIST_MCP_CODE_MODE_LIVE_MODEL:-nodoassist/main}" \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
  bash -lc "set -euo pipefail
    ...
    gateway_pid=\"\$(nodoassist_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-live-gateway.log)\"
    nodoassist_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-live-gateway.log 480 $PORT
