...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nodoassist-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.NODOASSIST_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".nodoassist-rename-tmp");
...
run_nodoassist() {
  HOME="$HOME_DIR" \
  NODOASSIST_HOME="$STATE_DIR" \
  NODOASSIST_STATE_DIR="$STATE_DIR" \
  NODOASSIST_CONFIG_PATH="$CONFIG_PATH" \
  NODOASSIST_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_nodoassist status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run nodoassist doctor --fix" "$BEFORE" >/dev/null

run_nodoassist doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_nodoassist status --all --timeout 1 > "$AFTER"
