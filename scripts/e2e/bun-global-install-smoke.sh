...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${NODOASSIST_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${NODOASSIST_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${NODOASSIST_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "NODOASSIST_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (NODOASSIST_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set NODOASSIST_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/nodoassist-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack NodoAssist tarball"
...
main() {
  ...
  echo "==> Bun global install packed NodoAssist"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  nodoassist_bin="$BUN_INSTALL/bin/nodoassist"
  if [ ! -x "$nodoassist_bin" ]; then
    nodoassist_bin="$(command -v nodoassist || true)"
  fi
  if [ -z "$nodoassist_bin" ] || [ ! -x "$nodoassist_bin" ]; then
    echo "Bun global install did not create an executable nodoassist binary" >&2
    exit 1
  fi

  echo "==> NodoAssist version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$nodoassist_bin" --version

  echo "==> NodoAssist image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$nodoassist_bin" infer image providers --json)"
  NODOASSIST_IMAGE_PROVIDERS_JSON="$providers_json" node scripts/e2e/lib/bun-global-install/assertions.mjs assert-image-providers
}
