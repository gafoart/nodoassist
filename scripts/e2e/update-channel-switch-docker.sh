...
IMAGE_NAME="$(docker_e2e_resolve_image "nodoassist-update-channel-switch-e2e" NODOASSIST_UPDATE_CHANNEL_SWITCH_E2E_IMAGE)"
SKIP_BUILD="${NODOASSIST_UPDATE_CHANNEL_SWITCH_E2E_SKIP_BUILD:-0}"
cleanup() {
  docker_e2e_cleanup_package_tgz "${PACKAGE_TGZ:-}"
}
trap cleanup EXIT

PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz update-channel-switch "${NODOASSIST_CURRENT_PACKAGE_TGZ:-}")"
# Bare lanes mount the package artifact instead of baking app sources into the image.
docker_e2e_package_mount_args "$PACKAGE_TGZ"
NODOASSIST_TEST_STATE_SCRIPT_B64="$(
  node "$ROOT_DIR/scripts/lib/nodoassist-test-state.mjs" shell \
...
(
  cd "$git_root"
  if ! nodoassist_e2e_maybe_timeout "${NODOASSIST_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/nodoassist-git-install.log 2>&1; then
    nodoassist_e2e_print_log /tmp/nodoassist-git-install.log >&2
    exit 1
  fi
)
node scripts/e2e/lib/update-channel-switch/assertions.mjs write-control-ui "$git_root"

git config --global user.email "docker-e2e@nodoassist.local"
git config --global user.name "NodoAssist Docker E2E"
...
package_install_log="/tmp/nodoassist-update-channel-switch-package-install.log"
if ! nodoassist_e2e_maybe_timeout "${NODOASSIST_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path" >"$package_install_log" 2>&1; then
  nodoassist_e2e_print_log "$package_install_log" >&2
  exit 1
fi
package_version="$(node -p "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/npm-prefix/lib/node_modules/nodoassist/package.json\", \"utf8\")).version")"
...
export NODOASSIST_GIT_DIR="$git_root"
export NODOASSIST_UPDATE_DEV_TARGET_REF="$fixture_sha"

echo "==> package -> git dev channel"
set +e
dev_json="$(nodoassist update --channel dev --yes --json --no-restart)"
dev_status=$?
set -e
printf "%s\n" "$dev_json"
...
status_json="$(nodoassist update status --json)"
printf "%s\n" "$status_json"
...
set +e
stable_json="$(nodoassist update --channel stable --tag "$pkg_tgz_path" --yes --json --no-restart)"
stable_status=$?
set -e
printf "%s\n" "$stable_json"
