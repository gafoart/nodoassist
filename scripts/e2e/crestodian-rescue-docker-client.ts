...
function makeParams(commandBody: string, cfg: NodoAssistConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function main() {
  const tempState = await createE2eStateDir("nodoassist-crestodian-");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env NODOASSIST_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke(
    "/crestodian create agent work workspace /tmp/nodoassist-work",
    cfg,
  );
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/nodoassist-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as NodoAssistConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "NODOASSIST_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/nodoassist-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/nodoassist-work",
    ),
