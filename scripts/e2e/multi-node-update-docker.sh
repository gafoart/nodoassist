...
# NodoAssist under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `nodoassist update`. Verifies that:
#
...
#
# Requires: Docker, a built nodoassist-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="$(docker_e2e_resolve_image "nodoassist-multi-node-update-e2e" NODOASSIST_MULTI_NODE_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${NODOASSIST_MULTI_NODE_UPDATE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${NODOASSIST_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${NODOASSIST_CURRENT_PACKAGE_TGZ:-}")"
...
echo ""
echo "── Step 2: Install NodoAssist under node-A ──"

# Use node-A to install nodoassist with npm prefix A.
...
echo "Installing NodoAssist package under node-A prefix: $NPM_PREFIX_A"
nodoassist_e2e_install_package "$ARTIFACTS/install-a.log" "NodoAssist package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking nodoassist location..."

NODOASSIST_A="$(command -v nodoassist)"
echo "nodoassist binary: $NODOASSIST_A"
echo "nodoassist version: $(nodoassist --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/nodoassist"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/nodoassist-gateway.service"
...
if ! nodoassist gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but nodoassist is NOT installed there.
...
echo "which nodoassist: $(command -v nodoassist)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run nodoassist update (this is the bug) ──"
...
echo "Running nodoassist update --yes --json..."
UPDATE_EXIT=0
nodoassist update --yes --json \
  --tag /tmp/nodoassist-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: nodoassist update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but NodoAssist package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the NodoAssist package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/nodoassist/package.json" ]; then
  echo "WARNING: NodoAssist was ALSO installed under node-B prefix (split install)"
else
  echo "OK: NodoAssist is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell NodoAssist root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
