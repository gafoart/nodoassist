...
const port =
  process.env.MOCK_PORT != null
    ...
    : readTcpPortEnv("NODOASSIST_MOCK_OPENAI_PORT");
const successMarker = process.env.SUCCESS_MARKER ?? "NODOASSIST_E2E_OK";
    ...
    data: [
      {
        ...
        mime_type: "image/png",
        revised_prompt: "nodoassist mock image",
      },
    ],
  });
}
...
function resolveResponseText(bodyText) {
  const matches = Array.from(bodyText.matchAll(/\bNODOASSIST_E2E_OK(?:_\d+)?\b/gu));
...
const server = http.createServer((req, res) => {
  void (async () => {
    ...
    if (req.method === "GET" && url.pathname === "/v1/models") {
      writeJson(res, 200, {
        object: "list",
        data: [{ id: "gpt-5.5", object: "model", owned_by: "nodoassist-e2e" }],
      });
      return;
    }
