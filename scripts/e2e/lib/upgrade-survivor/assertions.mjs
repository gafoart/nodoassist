        ...
        resolvedSkills: [
          {
            ...
            filePath: "/tmp/nodoassist-old-package/skills/legacy-heavy-skill-cache/SKILL.md",
          },
        ],
      },
    },
...
function getScenario() {
  const scenario = process.env.NODOASSIST_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("NODOASSIST_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.NODOASSIST_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  ...
  const workspace = requireEnv("NODOASSIST_TEST_WORKSPACE_DIR");
  ...
  writeJson(path.join(workspace, ".nodoassist", "workspace-state.json"), {
    version: 1,
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".nodoassist-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
    write(
      path.join(
        runtimeRoot,
        plugin,
        ".nodoassist-runtime-deps-copy-stale",
        "node_modules",
        "stale-sentinel",
        "package.json",
      ),
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.NODOASSIST_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `nodoassist-${version}-${plugin}`,
          ".nodoassist-runtime-deps-stamp.json",
        ),
        {
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "acpx-nodoassist-tools-bridge")) {
    const pluginAllow = config.plugins?.allow ?? [];
    ...
    assert(
      config.plugins?.entries?.acpx?.config?.nodoAssistToolsMcpBridge === true,
      "ACPX NodoAssist tools bridge config changed",
    );
  }
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/nodoassist-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const workspace = requireEnv("NODOASSIST_TEST_WORKSPACE_DIR");
  const scenario = getScenario();
  const stage = process.env.NODOASSIST_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-nodoassist");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`nodoassist-${version}-`))
      : [];
    assert(
...
function readMigratedSessionStore(stateDir, targetStorePath) {
  ...
  const dbPath = path.join(stateDir, "agents", "main", "agent", "nodoassist-agent.sqlite");
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  const extensionsRoot = path.join(requireEnv("NODOASSIST_STATE_DIR"), "extensions");
  assert(
