...
ARTIFACT_ROOT="$(dirname "${NODOASSIST_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/nodoassist-upgrade-survivor-artifacts/summary.json}")"
...
RUNTIME_ROOT="$NODOASSIST_UPGRADE_SURVIVOR_RUNTIME_ROOT"
STATE_HOME_ROOT="${NODOASSIST_UPGRADE_SURVIVOR_STATE_HOME_ROOT:-$RUNTIME_ROOT/state-home}"
mkdir -p "$ARTIFACT_ROOT"
mkdir -p "$RUNTIME_ROOT"
export TMPDIR="${NODOASSIST_UPGRADE_SURVIVOR_TMPDIR:-$RUNTIME_ROOT/tmp}"
export NODOASSIST_TEST_STATE_TMPDIR="${NODOASSIST_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR:-$RUNTIME_ROOT/state-tmp}"
mkdir -p "$TMPDIR" "$NODOASSIST_TEST_STATE_TMPDIR"
...
SUMMARY_JSON="${NODOASSIST_UPGRADE_SURVIVOR_SUMMARY_JSON:-$ARTIFACT_ROOT/summary.json}"
PHASE_LOG="$ARTIFACT_ROOT/phases.jsonl"
BASELINE_RAW="${NODOASSIST_UPGRADE_SURVIVOR_BASELINE:?missing NODOASSIST_UPGRADE_SURVIVOR_BASELINE}"
CANDIDATE_KIND="${NODOASSIST_UPGRADE_SURVIVOR_CANDIDATE_KIND:-tarball}"
CANDIDATE_SPEC="${NODOASSIST_UPGRADE_SURVIVOR_CANDIDATE_SPEC:-${NODOASSIST_CURRENT_PACKAGE_TGZ:-}}"
...
export NODOASSIST_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON="$CONFIG_COVERAGE_JSON"
rm -f "$SUMMARY_JSON" "$CONFIG_COVERAGE_JSON"
: >"$PHASE_LOG"

validate_baseline_package_spec() {
  local spec="$1"
  if [[ "$spec" =~ ^nodoassist@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$ ]]; then
    return 0
  fi
  echo "NODOASSIST_UPGRADE_SURVIVOR_BASELINE must be nodoassist@latest, nodoassist@beta, nodoassist@alpha, an exact NodoAssist release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  if [ -z "$raw" ]; then
    echo "NODOASSIST_UPGRADE_SURVIVOR_BASELINE cannot be empty" >&2
    return 1
  fi
  case "$raw" in
    nodoassist@*)
      baseline_spec="$raw"
      baseline_version="${raw#nodoassist@}"
      ;;
    *@*)
      echo "NODOASSIST_UPGRADE_SURVIVOR_BASELINE must be nodoassist@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="nodoassist@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "NODOASSIST_UPGRADE_SURVIVOR_BASELINE must be nodoassist@latest, nodoassist@beta, nodoassist@alpha, nodoassist@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
validate_update_restart_mode() {
  case "$UPDATE_RESTART_MODE" in
    manual | auto-auth)
      ;;
    *)
      echo "NODOASSIST_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE must be manual or auto-auth; got: $UPDATE_RESTART_MODE" >&2
      return 1
      ;;
  esac
}
...
const summary = {
  ...
  candidate: {
    kind: process.env.NODOASSIST_UPGRADE_SURVIVOR_CANDIDATE_KIND || null,
    spec: process.env.NODOASSIST_UPGRADE_SURVIVOR_CANDIDATE_SPEC || process.env.NODOASSIST_CURRENT_PACKAGE_TGZ || null,
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      nodoassist_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/nodoassist\n' "$npm_config_prefix"
}

legacy_runtime_deps_symlink_plugin() {
  local plugin="${NODOASSIST_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK:-}"
  if [ -z "$plugin" ]; then
    return 1
  fi
  case "$plugin" in
    *[!A-Za-z0-9._-]*)
      echo "NODOASSIST_UPGRADE_SURVIVOR_LEGACY_RUNTIME_DEPS_SYMLINK must be a plugin id, got: $plugin" >&2
      return 2
      ;;
  esac
  printf '%s\n' "$plugin"
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@nodoassist-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
plugin_deps_cleanup_plugins() {
  printf '%s\n' "${NODOASSIST_UPGRADE_SURVIVOR_PLUGIN_DEPS_CLEANUP_PLUGINS:-discord telegram}"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$NODOASSIST_STATE_DIR/extensions/opik-nodoassist"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-nodoassist",
...
JSON
  cat >"$shadow_root/nodoassist.plugin.json" <<'JSON'
{
  "id": "opik-nodoassist",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.nodoassist-runtime-deps.json" \
      "$plugin_dir/.nodoassist-runtime-deps-stamp.json" \
      "$plugin_dir/.nodoassist-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.nodoassist-install-stage-upgrade-survivor" \
      "$plugin_dir/.nodoassist-pnpm-store"
  done < <(plugin_deps_cleanup_plugin_dirs "$plugin")
  printf '%s\n' \
    "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$NODOASSIST_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor" \
    "$NODOASSIST_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor"
}
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/nodoassist-upgrade-survivor-dep" \
      "$plugin_dir/.nodoassist-runtime-deps-copy-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep" \
      "$plugin_dir/.nodoassist-install-stage-upgrade-survivor" \
      "$plugin_dir/.nodoassist-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep" \
      "$NODOASSIST_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep" \
      "$NODOASSIST_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep"
    printf '{"name":"nodoassist-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/nodoassist-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.nodoassist-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.nodoassist-runtime-deps-stamp.json"
    printf '{"name":"nodoassist-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.nodoassist-runtime-deps-copy-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep/package.json"
    printf '{"name":"nodoassist-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep/package.json"
    printf '{"name":"nodoassist-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$NODOASSIST_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep/package.json"
    printf '{"name":"nodoassist-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$NODOASSIST_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/nodoassist-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"nodoassist-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
reset_run_state() {
  rm_rf_retry "$npm_config_prefix" "$TMPDIR" "$NODOASSIST_TEST_STATE_TMPDIR" "$STATE_HOME_ROOT"
  rm -f "$SYSTEMCTL_SHIM_PID_FILE" "$SYSTEMCTL_SHIM_DAEMON_LOG"
  mkdir -p "$npm_config_prefix" "$npm_config_cache" "$TMPDIR" "$NODOASSIST_TEST_STATE_TMPDIR"
}

install_baseline() {
  normalize_baseline
  echo "Installing baseline package: $baseline_spec"
  if ! nodoassist_e2e_maybe_timeout "${NODOASSIST_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    nodoassist_e2e_print_log "$BASELINE_INSTALL_LOG" >&2
    return 1
  fi
  if ! command -v nodoassist >/dev/null; then
    echo "baseline install did not expose nodoassist on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" nodoassist --version 2>&1)"; then
    echo "baseline nodoassist --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline nodoassist --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  nodoassist_e2e_eval_test_state_from_b64 "${NODOASSIST_TEST_STATE_FUNCTION_B64:?missing NODOASSIST_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.nodoassist /root/workspace
    nodoassist_test_state_create /root minimal
  else
    nodoassist_test_state_create "$STATE_HOME_ROOT" minimal
  fi
  export NODOASSIST_UPGRADE_SURVIVOR_BASELINE_VERSION="$baseline_version"
...
validate_baseline_config() {
  if ! nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" nodoassist config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
    echo "generated baseline config failed baseline validation" >&2
    nodoassist_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG" >&2
    return 1
  fi
}
...
log_file="${NODOASSIST_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_LOG:-/tmp/nodoassist-systemctl-shim.log}"
pid_file="${NODOASSIST_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_PID_FILE:-/tmp/nodoassist-systemctl-shim.pid}"
daemon_log="${NODOASSIST_UPGRADE_SURVIVOR_SYSTEMCTL_SHIM_DAEMON_LOG:-/tmp/nodoassist-systemctl-shim-gateway.log}"
printf '%s\n' "$*" >>"$log_file"
...
unit_path() {
  printf '%s/.config/systemd/user/nodoassist-gateway.service\n' "${HOME:?missing HOME}"
}
...
install_update_restart_service_unit() {
  if ! nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u NODOASSIST_GATEWAY_TOKEN -u NODOASSIST_GATEWAY_PASSWORD nodoassist gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
    echo "baseline gateway service install failed" >&2
    nodoassist_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR" >&2
    nodoassist_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON" >&2
    return 1
  fi
}
...
if (!stateDir) {
  throw new Error("missing NODOASSIST_STATE_DIR");
}
...
write_update_restart_service_secretref_env() {
  ...
  local dotenv_path="$NODOASSIST_STATE_DIR/.env"
...
assert_baseline_state() {
  NODOASSIST_UPGRADE_SURVIVOR_ASSERT_STAGE=baseline \
...
resolve_candidate_version() {
  if [ -z "$CANDIDATE_SPEC" ]; then
    echo "missing NODOASSIST_UPGRADE_SURVIVOR_CANDIDATE_SPEC" >&2
    return 1
  fi
...
update_candidate() {
  ...
  local update_env=(
    env
    -u NODOASSIST_GATEWAY_TOKEN
    -u NODOASSIST_GATEWAY_PASSWORD
    -u NODOASSIST_ALLOW_ROOT
  )
  ...
  if [ "$ROOT_MANAGED_VPS" != "1" ]; then
    update_env+=(NODOASSIST_ALLOW_ROOT=1)
  fi
  if ! nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" nodoassist "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
    echo "nodoassist update failed" >&2
    nodoassist_e2e_print_log "$UPDATE_ERR" >&2
    nodoassist_e2e_print_log "$UPDATE_JSON" >&2
    return 1
  fi
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
assert_root_managed_vps_cli_usable() {
  ...
  nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" nodoassist config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" nodoassist plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" nodoassist doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "nodoassist doctor failed" >&2
    nodoassist_e2e_print_log "$DOCTOR_LOG" >&2
    return 1
  fi
}

validate_post_doctor_config() {
  if ! nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" nodoassist config validate >>"$DOCTOR_LOG" 2>&1; then
...
probe_gateway_endpoint() {
  ...
  if [ -n "${NODOASSIST_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING:-}" ]; then
    args+=(--allow-failing "$NODOASSIST_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")
  fi
...
start_gateway() {
  ...
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u NODOASSIST_GATEWAY_TOKEN -u NODOASSIST_GATEWAY_PASSWORD nodoassist gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_status() {
  ...
  if ! nodoassist_e2e_maybe_timeout "$COMMAND_TIMEOUT" nodoassist gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
    echo "gateway status failed" >&2
    nodoassist_e2e_print_log "$STATUS_ERR" >&2
    nodoassist_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
  ...
  if [ "$status_seconds" -gt "$budget" ]; then
    ...
    nodoassist_e2e_print_log "$STATUS_JSON" >&2
    return 1
  fi
