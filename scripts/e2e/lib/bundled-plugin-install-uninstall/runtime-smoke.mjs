...
const RUNTIME_PORT_BASE_ENV = "NODOASSIST_BUNDLED_PLUGIN_RUNTIME_PORT_BASE";
...
const WATCHDOG_MS = readPositiveIntEnv("NODOASSIST_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS", 1000);
const READY_TIMEOUT_MS = readPositiveIntEnv("NODOASSIST_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000);
const RPC_TIMEOUT_MS = readPositiveIntEnv("NODOASSIST_BUNDLED_PLUGIN_RUNTIME_RPC_MS", 60000);
...
const HTTP_PROBE_TIMEOUT_MS = readPositiveIntEnv("NODOASSIST_BUNDLED_PLUGIN_RUNTIME_HTTP_MS", 5000);
...
function manifestPath(pluginDir, pluginRoot) {
  const candidates = [
    ...(isNonEmptyString(pluginRoot) ? [path.join(pluginRoot, "nodoassist.plugin.json")] : []),
    path.join(process.cwd(), "dist", "extensions", pluginDir, "nodoassist.plugin.json"),
    path.join(process.cwd(), "dist-runtime", "extensions", pluginDir, "nodoassist.plugin.json"),
  ];
...
function configPathFromEnv(env = process.env) {
  return (
    env.NODOASSIST_CONFIG_PATH ||
    path.join(env.HOME || os.homedir(), ".nodoassist", "nodoassist.json")
  );
}
  ...
  const child = childProcess.spawn(
    ...
    ],
    {
      env: {
        ...
        NODOASSIST_SKIP_CHANNELS: params.skipChannels ? "1" : "0",
...
export async function rpcCall(method, params, options) {
  const rpcStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-runtime-rpc-"));
  const args = [
    options.entrypoint,
    "gateway",
    "call",
  ...
  try {
    const { stdout } = await runCommand("node", args, {
      env: {
        ...
        ...options.env,
        NODOASSIST_NO_ONBOARD: "1",
        NODOASSIST_STATE_DIR: rpcStateDir,
      },
    });
...
async function smokePlugin(pluginId, pluginDir, requiresConfig, pluginIndex, pluginRoot) {
  ...
  const entrypoint = process.env.NODOASSIST_ENTRY;
  if (!entrypoint) {
    throw new Error("missing NODOASSIST_ENTRY");
  }
  ...
  const logPath = `/tmp/nodoassist-plugin-runtime-${pluginIndex}-${pluginId}.log`;
...
async function smokeTtsGlobalDisable(pluginId, pluginDir, provider, pluginIndex, pluginRoot) {
  ...
  const logPath = `/tmp/nodoassist-plugin-runtime-${pluginIndex}-${pluginId}-tts-disabled.log`;
...
async function smokeOpenAiTts(pluginIndex) {
  ...
  const logPath = `/tmp/nodoassist-plugin-runtime-${pluginIndex}-openai-tts-live.log`;
  const child = startGateway({ entrypoint, port, logPath, env, skipChannels: true });
  try {
    await waitForReady({ child, port, logPath });
    await assertBaseGatewayProbes({ entrypoint, port, env });
...
export function createIsolatedStateEnv(label) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `nodoassist-${label}-`));
