...
function resolveStateDir() {
  if (process.env.NODOASSIST_STATE_DIR) {
    return process.env.NODOASSIST_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".nodoassist");
}
...
function resolveNodoAssistEntry() {
  if (process.env.NODOASSIST_ENTRY) {
    return process.env.NODOASSIST_ENTRY;
  }
  ...
  throw new Error("Missing NODOASSIST_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveNodoAssistEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.NODOASSIST_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `NODOASSIST_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("NODOASSIST_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("NODOASSIST_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `NODOASSIST_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.NODOASSIST_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
