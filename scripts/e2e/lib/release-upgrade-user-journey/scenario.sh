...
export OPENAI_API_KEY="sk-nodoassist-release-upgrade-user-journey"
export CLICKCLACK_BOT_TOKEN="clickclack-release-upgrade-token"

PORT="18789"
MOCK_PORT="44210"
CLICKCLACK_PORT="44211"
SUCCESS_MARKER="NODOASSIST_E2E_OK_RELEASE_UPGRADE"
...
candidate_version="$(
  tar -xOf "${NODOASSIST_CURRENT_PACKAGE_TGZ:?missing NODOASSIST_CURRENT_PACKAGE_TGZ}" package/package.json |
...
if [ -n "${NODOASSIST_RELEASE_UPGRADE_BASELINE_SPEC:-}" ]; then
  BASELINE_SPEC="$NODOASSIST_RELEASE_UPGRADE_BASELINE_SPEC"
else
...
cleanup() {
  ...
  nodoassist_e2e_stop_process "${clickclack_pid:-}"
...
start_gateway() {
  local log_path="$1"
  gateway_pid="$(nodoassist_e2e_start_gateway "$entry" "$PORT" "$log_path")"
  nodoassist_e2e_wait_gateway_ready "$gateway_pid" "$log_path" 300 "$PORT"
}

echo "Installing published baseline $BASELINE_SPEC..."
if ! nodoassist_e2e_maybe_timeout "${NODOASSIST_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v nodoassist >/dev/null
baseline_root="$(nodoassist_e2e_package_root)"
baseline_entry="$(nodoassist_e2e_package_entrypoint "$baseline_root")"
nodoassist_e2e_enable_nodoassist_cli_timeout

mock_pid="$(nodoassist_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if nodoassist_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
nodoassist_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
nodoassist plugins install "$plugin_dir" >"$PLUGIN_INSTALL_LOG" 2>&1
nodoassist release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
nodoassist_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate NodoAssist package"
...
nodoassist release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
...
nodoassist plugins install "$clickclack_plugin_dir" >"$CLICKCLACK_PLUGIN_INSTALL_LOG" 2>&1

nodoassist channels status --json >"$STATUS_JSON" 2>"$STATUS_ERR"
node scripts/e2e/lib/release-user-journey/assertions.mjs assert-channel-status clickclack "$STATUS_JSON"
nodoassist message send \
  --channel clickclack \
