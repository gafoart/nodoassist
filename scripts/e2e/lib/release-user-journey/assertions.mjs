...
function clickClackHttpTimeoutMs() {
  return readPositiveInt(
    process.env.NODOASSIST_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS,
    5000,
    "NODOASSIST_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS",
  );
}

function clickClackHttpBodyMaxBytes() {
  return readPositiveInt(
    process.env.NODOASSIST_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES,
    1024 * 1024,
...
function configPath() {
  return (
    process.env.NODOASSIST_CONFIG_PATH ??
    path.join(process.env.HOME ?? "", ".nodoassist", "nodoassist.json")
  );
}
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write nodoassist.json");
  const stateRaw =
    fs.readFileSync(configPath(), "utf8") +
    (fs.existsSync(authPath) ? fs.readFileSync(authPath, "utf8") : "");
  assert(
    !stateRaw.includes("sk-nodoassist-release-user-journey"),
