...
function withoutAiRuntimeDependency(value) {
  ...
  const next = value.filter((entry) => entry !== "@nodoassist/ai");
...
function prepare(root) {
  ...
  const aiRuntimeSource = path.join(root, "node_modules", "@nodoassist", "ai");
  const aiRuntimePackageJson = path.join(aiRuntimeSource, "package.json");
  if (!fs.existsSync(aiRuntimePackageJson)) {
    return;
  }

  const aiRuntimeTarget = path.join(root, ".nodoassist-fixture", "packages", "ai");
  ...
  packageJson.dependencies["@nodoassist/ai"] = "file:.nodoassist-fixture/packages/ai";
