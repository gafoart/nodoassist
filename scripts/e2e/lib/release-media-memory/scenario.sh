...
export OPENAI_API_KEY="sk-nodoassist-release-media-memory"
export NODOASSIST_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="NODOASSIST_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/nodoassist-release-media-memory.XXXXXX)"
...
entry="$(nodoassist_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'entry=%s\n' "$entry"
  printf 'HOME=%s\n' "$HOME"
  printf 'NODOASSIST_HOME=%s\n' "$NODOASSIST_HOME"
  printf 'NODOASSIST_STATE_DIR=%s\n' "$NODOASSIST_STATE_DIR"
  printf 'NODOASSIST_CONFIG_PATH=%s\n' "$NODOASSIST_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(nodoassist_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$NODOASSIST_CONFIG_PATH" "$CONFIG_JSON"
nodoassist plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$NODOASSIST_STATE_DIR/workspace/memory"
...
nodoassist infer image describe \
...
cat >"$NODOASSIST_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

nodoassist memory index --force >"$INDEX_LOG" 2>&1
nodoassist memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
nodoassist memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
