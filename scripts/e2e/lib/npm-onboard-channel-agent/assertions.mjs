...
const STATUS_TEXT_MAX_BYTES = readPositiveIntEnv(
  "NODOASSIST_NPM_ONBOARD_STATUS_TEXT_MAX_BYTES",
  1024 * 1024,
);
...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write nodoassist.json");
  }
  ...
  assertOpenAiEnvAuthProfileStore(authStoreText, {
    ...
    rawKeyNeedle: "sk-nodoassist-npm-onboard-e2e",
  });
}
...
function assertMockModelConfig() {
  ...
  if (provider?.agentRuntime?.id !== "nodoassist") {
  ...
  if (agent && agentRuntime !== "nodoassist") {
...
function assertAgentTurn() {
  ...
  assertAgentReplyContainsMarker(marker, "/tmp/nodoassist-agent.combined");
