...
const AGENTS_DELETE_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "NODOASSIST_FIXTURE_AGENTS_DELETE_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
...
function writeOpenWebUiWorkspace() {
  const workspace =
    process.env.NODOASSIST_WORKSPACE_DIR || path.join(process.env.HOME, ".nodoassist", "workspace");
  write(
...
function writeAgentsDeleteConfig() {
  const stateDir = requireArg(process.env.NODOASSIST_STATE_DIR, "NODOASSIST_STATE_DIR");
  const sharedWorkspace = requireArg(process.env.SHARED_WORKSPACE, "SHARED_WORKSPACE");
  const gatewayToken = process.env.NODOASSIST_GATEWAY_TOKEN?.trim();
...
function assertAgentsDeleteResult([outputPath]) {
  ...
  const remaining =
    readJson(path.join(process.env.NODOASSIST_STATE_DIR, "nodoassist.json"))?.agents?.list ?? [];
