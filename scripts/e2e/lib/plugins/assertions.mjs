...
const scratchRoot = process.env.NODOASSIST_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "NODOASSIST_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("NODOASSIST_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  const configPath = nodoAssistConfigPath();
  const config = readNodoAssistConfig();
  const allowLegacyCompat = process.env.NODOASSIST_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function nodoAssistConfigPath() {
  return path.join(process.env.HOME, ".nodoassist", "nodoassist.json");
}

function readNodoAssistConfig() {
  const configPath = nodoAssistConfigPath();
  return fs.existsSync(configPath) ? readRequiredNodoAssistConfig() : {};
}

function readRequiredNodoAssistConfig() {
  const configPath = nodoAssistConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read NodoAssist config ${configPath}: ${message}`, { cause: error });
  }
}
...
function recordFixturePluginTrust() {
  ...
  const configPath = path.join(process.env.HOME, ".nodoassist", "nodoassist.json");
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".nodoassist", "git");
...
function assertClawHubExternalInstallContract(installPath) {
  const nodoassistPeerPath = path.join(installPath, "node_modules", "nodoassist");
  if (!fs.existsSync(nodoassistPeerPath)) {
    throw new Error(`missing ClawHub nodoassist peer symlink: ${nodoassistPeerPath}`);
  }
  if (!fs.lstatSync(nodoassistPeerPath).isSymbolicLink()) {
    throw new Error(`ClawHub nodoassist peer is not a symlink: ${nodoassistPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(nodoassistPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected ClawHub nodoassist peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@nodoassist/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@nodoassist/demo-plugin-npm") {
...
function assertInvalidNodoAssistExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["nodoassist.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-nodoassist-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-nodoassist-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".nodoassist", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.NODOASSIST_CLAWHUB_URL ||
    process.env.CLAWHUB_URL ||
    "https://clawhub.ai"
  ).replace(/\/+$/, "");
  const token =
    process.env.NODOASSIST_CLAWHUB_TOKEN ||
...
function assertClawHubInstalled() {
  ...
  const extensionsRoot = path.join(process.env.HOME, ".nodoassist", "extensions");
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".nodoassist", "nodoassist.json");
...
const commands = {
  ...
  "invalid-nodoassist-extensions": assertInvalidNodoAssistExtensionsRejected,
