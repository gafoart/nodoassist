NODOASSIST_PLUGINS_FIXTURE_PID_FILES=()
NODOASSIST_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=0
NODOASSIST_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=""

nodoassist_plugins_read_positive_int_env() {
...
nodoassist_plugins_read_nonnegative_decimal_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [[ -z "${!name+x}" ]]; then
...
nodoassist_plugins_cleanup_fixture_servers() {
  local pid_file
  local pid
  for pid_file in "${NODOASSIST_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    [[ -f "$pid_file" ]] || continue
    pid="$(cat "$pid_file" 2>/dev/null || true)"
    if [[ "$pid" =~ ^[0-9]+$ ]]; then
      nodoassist_plugins_stop_fixture_process "$pid"
    fi
    rm -f "$pid_file"
  done
}

nodoassist_plugins_signal_fixture_process() {
  local pid="$1"
  local signal="$2"
...
nodoassist_plugins_fixture_process_alive() {
  local pid="$1"
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}

nodoassist_plugins_stop_fixture_process() {
  local pid="$1"
  local _
  local attempts interval
  attempts="$(nodoassist_plugins_read_positive_int_env NODOASSIST_PLUGINS_FIXTURE_STOP_ATTEMPTS 40)" || return $?
  interval="$(nodoassist_plugins_read_nonnegative_decimal_env NODOASSIST_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS 0.25)" || return $?
  if declare -F nodoassist_e2e_stop_process >/dev/null 2>&1; then
    nodoassist_e2e_stop_process "$pid"
    return
  fi
  nodoassist_plugins_signal_fixture_process "$pid" TERM
  for _ in $(seq 1 "$attempts"); do
    ! nodoassist_plugins_fixture_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return; }
    sleep "$interval"
  done
  nodoassist_plugins_signal_fixture_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}

nodoassist_plugins_print_fixture_log() {
  local log_file="$1"
  ...
  local max_bytes
  max_bytes="$(nodoassist_plugins_read_positive_int_env NODOASSIST_DOCKER_E2E_LOG_PRINT_BYTES 65536)" || return $?
...
nodoassist_plugins_validate_fixture_log_print_bytes() {
  nodoassist_plugins_read_positive_int_env NODOASSIST_DOCKER_E2E_LOG_PRINT_BYTES 65536 >/dev/null
}

nodoassist_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  NODOASSIST_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  nodoassist_plugins_install_fixture_cleanup_trap
}

nodoassist_plugins_install_fixture_cleanup_trap() {
  if [[ "${NODOASSIST_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED:-0}" = "1" ]]; then
    return
  fi

  local existing_trap
  existing_trap="$(trap -p EXIT || true)"
  if [[ -n "$existing_trap" && "$existing_trap" != *nodoassist_plugins_fixture_exit_trap* ]]; then
    local existing_action="${existing_trap#trap -- }"
    existing_action="${existing_action% EXIT}"
    eval "NODOASSIST_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION=$existing_action"
  fi

  NODOASSIST_PLUGINS_FIXTURE_EXIT_TRAP_INSTALLED=1
  trap nodoassist_plugins_fixture_exit_trap EXIT
}

nodoassist_plugins_fixture_exit_trap() {
  local status="$?"
  nodoassist_plugins_cleanup_fixture_servers
  if [[ -n "${NODOASSIST_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION:-}" ]]; then
    eval "$NODOASSIST_PLUGINS_FIXTURE_PREVIOUS_EXIT_ACTION"
  fi
  exit "$status"
}
...
packageJson.nodoassist.extensions = ["./index.js", " "];
