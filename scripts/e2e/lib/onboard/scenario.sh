...
NODOASSIST_ONBOARD_SCENARIO_SOURCE_ONLY="${NODOASSIST_ONBOARD_SCENARIO_SOURCE_ONLY:-0}"
if [ "$NODOASSIST_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
...
if [ -z "${NODOASSIST_ENTRY:-}" ] && [ "$NODOASSIST_ONBOARD_SCENARIO_SOURCE_ONLY" != "1" ]; then
  NODOASSIST_ENTRY="$(nodoassist_e2e_resolve_entrypoint)"
fi
export NODOASSIST_ENTRY
ONBOARD_TMP_ROOT="${NODOASSIST_ONBOARD_E2E_TMPDIR:-${TMPDIR:-/tmp}}"
ONBOARD_TMP_ROOT="${ONBOARD_TMP_ROOT%/}"
[ -n "$ONBOARD_TMP_ROOT" ] || ONBOARD_TMP_ROOT="/tmp"
mkdir -p "$ONBOARD_TMP_ROOT"
ONBOARD_TMP_DIR="$(mktemp -d "$ONBOARD_TMP_ROOT/nodoassist-onboard.XXXXXX")"
NODOASSIST_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"
GATEWAY_LOG_PATH="$ONBOARD_TMP_DIR/gateway-e2e.log"
export NODOASSIST_E2E_LOG_DIR
export GATEWAY_LOG_PATH
mkdir -p "$NODOASSIST_E2E_LOG_DIR"
...
start_gateway() {
  GATEWAY_PID="$(nodoassist_e2e_start_gateway "$NODOASSIST_ENTRY" 18789 "$GATEWAY_LOG_PATH")"
}

wait_for_gateway() {
  local wait_attempts
  wait_attempts="$(nodoassist_e2e_read_positive_int_env NODOASSIST_ONBOARD_GATEWAY_WAIT_ATTEMPTS 20)" || return $?
  local wait_interval_s="${NODOASSIST_ONBOARD_GATEWAY_WAIT_INTERVAL_S:-1}"
  local saw_listening_log="false"
  for _ in $(seq 1 "$wait_attempts"); do
    if nodoassist_e2e_probe_tcp 127.0.0.1 18789 500 >/dev/null 2>&1; then
      return 0
    fi
...
cleanup_wizard_case() {
  ...
  nodoassist_e2e_stop_process "${wizard_pid:-}"
  stop_gateway "${gw_pid:-}"
...
run_wizard_cmd() {
  ...
  set_isolated_nodoassist_env "$state_ref"
  ...
  local log_path="$NODOASSIST_E2E_LOG_DIR/${case_name}.log"
  WIZARD_LOG_PATH="$log_path"
  export WIZARD_LOG_PATH
  # Run under script to keep an interactive TTY for clack prompts.
  nodoassist_e2e_run_script_with_pty "$command" "$log_path" <"$input_fifo" >/dev/null 2>&1 &
  wizard_pid=$!
...
assert_onboard_config() {
  local scenario="$1"
  shift
  nodoassist_e2e_assert_file "$NODOASSIST_CONFIG_PATH"
  node scripts/e2e/lib/onboard/assert-config.mjs "$scenario" "$NODOASSIST_CONFIG_PATH" "$@"
}

set_isolated_nodoassist_env() {
  local state_ref="$1"
  nodoassist_test_state_create "$state_ref" empty
}
...
run_case_local_basic() {
  set_isolated_nodoassist_env local-basic
  nodoassist_e2e_run_logged local-basic node "$NODOASSIST_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
  ...
  validate_local_basic_log "$NODOASSIST_E2E_LAST_LOG_PATH"

  # Assert config + workspace scaffolding.
  workspace_dir="$NODOASSIST_STATE_DIR/workspace"
  sessions_dir="$NODOASSIST_STATE_DIR/agents/main/sessions"

  nodoassist_e2e_assert_dir "$sessions_dir"
  for file in AGENTS.md BOOTSTRAP.md IDENTITY.md SOUL.md TOOLS.md USER.md; do
    nodoassist_e2e_assert_file "$workspace_dir/$file"
  done
...
run_case_remote_non_interactive() {
  set_isolated_nodoassist_env remote-non-interactive
  # Smoke test non-interactive remote config write.
  nodoassist_e2e_run_logged remote-non-interactive node "$NODOASSIST_ENTRY" onboard --non-interactive --accept-risk \
    --mode remote \
...
run_case_reset() {
  set_isolated_nodoassist_env reset-config
  node scripts/e2e/lib/onboard/write-config.mjs reset "$NODOASSIST_CONFIG_PATH"

  nodoassist_e2e_run_logged reset-config node "$NODOASSIST_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_channels() {
  ...
  run_wizard_cmd channels channels "node \"$NODOASSIST_ENTRY\" configure --section channels" send_channels_flow
...
run_case_skills() {
  local home_dir
  set_isolated_nodoassist_env skills
  home_dir="$HOME"
  node scripts/e2e/lib/onboard/write-config.mjs skills "$NODOASSIST_CONFIG_PATH"

  run_wizard_cmd skills "$home_dir" "node \"$NODOASSIST_ENTRY\" configure --section skills" send_skills_flow
...
validate_local_basic_log() {
  local log_path="$1"
  nodoassist_e2e_assert_log_not_contains "$log_path" "systemctl --user unavailable"
}
