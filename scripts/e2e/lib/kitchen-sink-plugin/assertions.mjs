...
function shouldScanLogFile(entry) {
  if (
    !(/\.(?:log|jsonl)$/u.test(entry) || /nodoassist-kitchen-sink-/u.test(path.basename(entry)))
  ) {
    return false;
  }
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".nodoassist")];
  const deny = [
  ...
  if (scannedFiles === 0) {
    throw new Error(
      "kitchen-sink log scan found no files under the isolated scratch root or NodoAssist home",
    );
  }
...
function assertClawHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(nodoassistPeerPath)) {
    throw new Error(`missing kitchen-sink nodoassist peer symlink: ${nodoassistPeerPath}`);
  }
  if (!fs.lstatSync(nodoassistPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink nodoassist peer is not a symlink: ${nodoassistPeerPath}`);
  }
  ...
  if (linkedHostRoot !== hostRoot) {
    throw new Error(
      `expected kitchen-sink nodoassist peer ${linkedHostRoot} to target ${hostRoot}`,
    );
  }
