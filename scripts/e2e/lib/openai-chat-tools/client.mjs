...
const timeoutSeconds = readPositiveIntEnv("NODOASSIST_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", 180);
const maxBodyBytes = readPositiveIntEnv("NODOASSIST_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", 1048576);

if (!portText || !token) {
  throw new Error("missing PORT/NODOASSIST_GATEWAY_TOKEN");
}
const port = readTcpPortEnv("PORT", portText);
if (!Number.isFinite(timeoutSeconds) || timeoutSeconds <= 0) {
  throw new Error(`invalid NODOASSIST_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: ${timeoutSeconds}`);
}
if (!Number.isFinite(maxBodyBytes) || maxBodyBytes <= 0) {
  throw new Error(`invalid NODOASSIST_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: ${maxBodyBytes}`);
}
  ...
  response = await Promise.race([
    fetch(`http://127.0.0.1:${port}/v1/chat/completions`, {
      ...
      headers: {
        ...
        "x-nodoassist-model": backendModel,
      },
      body: JSON.stringify({
        model: "nodoassist",
        stream: false,
