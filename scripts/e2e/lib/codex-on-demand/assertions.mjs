...
const inspect = readJson("/tmp/nodoassist-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@nodoassist/codex")) {
  throw new Error(`expected @nodoassist/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @nodoassist/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@nodoassist/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@nodoassist/codex");
...
function readAuthProfileStoreText(agentDir) {
  const dbPath = path.join(agentDir, "nodoassist-agent.sqlite");
...
assertOpenAiEnvAuthProfileStore(authRaw, {
  ...
  rawKeyNeedle: "sk-nodoassist-codex-on-demand-e2e",
});
