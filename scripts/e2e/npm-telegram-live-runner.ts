...
function resolveCredentialSource(env: NodeJS.ProcessEnv) {
  return env.NODOASSIST_NPM_TELEGRAM_CREDENTIAL_SOURCE ?? env.NODOASSIST_QA_CREDENTIAL_SOURCE;
}

function resolveCredentialRole(env: NodeJS.ProcessEnv) {
  return env.NODOASSIST_NPM_TELEGRAM_CREDENTIAL_ROLE ?? env.NODOASSIST_QA_CREDENTIAL_ROLE;
}
...
function resolvePackageTelegramOutputDir(env: NodeJS.ProcessEnv, repoRoot: string) {
  return (
    env.NODOASSIST_NPM_TELEGRAM_OUTPUT_DIR?.trim() ||
...
function resolveRttOptions(env: NodeJS.ProcessEnv, selectedScenarioIds: readonly string[] = []) {
  const explicitCheckIds = splitCsv(env.NODOASSIST_NPM_TELEGRAM_RTT_CHECKS);
  if (
  ...
  const rttCount = parsePositiveIntegerEnv(env, "NODOASSIST_NPM_TELEGRAM_RTT_SAMPLES") ?? 20;
  return {
    rttCount,
    rttTimeoutMs: parsePositiveIntegerEnv(env, "NODOASSIST_NPM_TELEGRAM_RTT_TIMEOUT_MS"),
    maxRttFailures:
      parsePositiveIntegerEnv(env, "NODOASSIST_NPM_TELEGRAM_RTT_MAX_FAILURES") ?? rttCount,
...
async function shouldFailPackageTelegramRun(
  ...
  if (parseBoolean(env.NODOASSIST_NPM_TELEGRAM_ALLOW_FAILURES)) {
    return false;
  }
...
async function resolveTrustedNodoAssistCommand(rawCommand: string) {
  if (!path.isAbsolute(rawCommand)) {
    throw new Error("NODOASSIST_NPM_TELEGRAM_SUT_COMMAND must be an absolute path.");
  }
  const commandName = path.basename(rawCommand);
  if (commandName !== "nodoassist" && commandName !== "nodoassist.cmd") {
    throw new Error(
      `NODOASSIST_NPM_TELEGRAM_SUT_COMMAND must point to nodoassist; got: ${commandName}`,
    );
  }
  const npmPrefix = process.env.NPM_CONFIG_PREFIX?.trim();
  if (!npmPrefix) {
    throw new Error("Missing NPM_CONFIG_PREFIX for installed nodoassist command validation.");
  }
  ...
  if (realCommand !== realPrefix && !realCommand.startsWith(`${realPrefix}${path.sep}`)) {
    throw new Error("NODOASSIST_NPM_TELEGRAM_SUT_COMMAND must resolve inside NPM_CONFIG_PREFIX.");
  }
  return rawCommand;
}
...
async function main() {
  ...
  const rawSutNodoAssistCommand = process.env.NODOASSIST_NPM_TELEGRAM_SUT_COMMAND?.trim();
  if (!rawSutNodoAssistCommand) {
    throw new Error("Missing NODOASSIST_NPM_TELEGRAM_SUT_COMMAND.");
  }
  const sutNodoAssistCommand = await resolveTrustedNodoAssistCommand(rawSutNodoAssistCommand);

  const repoRoot = path.resolve(process.env.NODOASSIST_NPM_TELEGRAM_REPO_ROOT ?? process.cwd());
  const outputDir = resolvePackageTelegramOutputDir(process.env, repoRoot);
  const scenarioIds = splitCsv(process.env.NODOASSIST_NPM_TELEGRAM_SCENARIOS);
  const result = await runTelegramQaLive({
    env: process.env,
    repoRoot,
    outputDir,
    sutNodoAssistCommand,
    providerMode: process.env.NODOASSIST_NPM_TELEGRAM_PROVIDER_MODE,
    primaryModel: process.env.NODOASSIST_NPM_TELEGRAM_MODEL,
    alternateModel: process.env.NODOASSIST_NPM_TELEGRAM_ALT_MODEL,
    fastMode: parseBoolean(process.env.NODOASSIST_NPM_TELEGRAM_FAST),
    scenarioIds,
    ...resolveRttOptions(process.env, scenarioIds),
    sutAccountId: process.env.NODOASSIST_NPM_TELEGRAM_SUT_ACCOUNT,
