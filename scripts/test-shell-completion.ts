 ...
 * `nodoassist update`. Use it to verify the completion installation flow
...
\`nodoassist update\`, \`nodoassist doctor\`, and \`nodoassist onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# NodoAssist Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`nodoassist completion --install\` later to enable.`));
    return;
  }
