...
const TOOLING_ISOLATED_TEST_TARGET = "test/scripts/nodoassist-e2e-instance.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "NODOASSIST_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "NODOASSIST_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-nodoassist-runtime)?\.ts$/u,
...
const GITHUB_WORKFLOW_OWNER_TEST_TARGETS = new Map([
  ...
  [
    ...
    [
      ...
      "test/scripts/nodoassist-cross-os-release-workflow.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
    ],
  ],
  [
  ...
  [
    ".github/workflows/nodoassist-performance.yml",
    ["test/scripts/nodoassist-performance-workflow.test.ts"],
  ],
  [
    ".github/workflows/nodoassist-release-checks.yml",
    [
      "test/scripts/nodoassist-cross-os-release-checks.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
      "test/scripts/plugin-prerelease-test-plan.test.ts",
  ...
  [
    ".github/workflows/nodoassist-scheduled-live-checks.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
    ".github/workflows/nodoassist-stable-main-closeout.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/github/resolve-nodoassist-ref.sh", ["test/scripts/resolve-nodoassist-ref.test.ts"]],
  ["scripts/ci-hydrate-testbox-env.sh", ["test/scripts/ci-hydrate-testbox-env.test.ts"]],
  [
    "scripts/github/run-nodoassist-cross-os-release-checks.sh",
    ["test/scripts/nodoassist-cross-os-release-workflow.test.ts"],
  ],
  ...
  ["scripts/nodoassist-release-clawhub-plan.ts", ["test/scripts/release-wrapper-scripts.test.ts"]],
  [
  ...
  ["scripts/lib/nodoassist-test-state.mjs", ["test/scripts/nodoassist-test-state.test.ts"]],
  [
  ...
  [
    "scripts/nodoassist-npm-prepublish-verify.ts",
    ["test/nodoassist-npm-prepublish-verify.test.ts"],
  ],
  [
  ...
  [
    "scripts/check-nodoassist-package-tarball.mjs",
    ["test/scripts/check-nodoassist-package-tarball.test.ts"],
  ],
  ...
  [
    ...
    ["test/scripts/docker-build-helper.test.ts", "test/scripts/nodoassist-test-state.test.ts"],
  ],
  [
  ...
  [
    "scripts/nodoassist-performance-source-summary.mjs",
    ["test/scripts/nodoassist-performance-source-summary.test.ts"],
  ],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/nodoassist-test-state.test.ts", ["test/scripts/nodoassist-test-state.test.ts"]],
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/nodoassist-test-state.ts", ["src/test-utils/nodoassist-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "NODOASSIST_TEST_CHANGED_BROAD";
...
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "NODOASSIST_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.NODOASSIST_TEST_PROJECTS_PARALLEL,
    "NODOASSIST_TEST_PROJECTS_PARALLEL",
  );
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.NODOASSIST_TEST_PROJECTS_SERIAL !== "1" &&
    env.CI !== "true" &&
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.NODOASSIST_VITEST_MAX_WORKERS ?? env.NODOASSIST_TEST_WORKERS,
    env.NODOASSIST_VITEST_MAX_WORKERS === undefined
      ? "NODOASSIST_TEST_WORKERS"
...
export function resolveParallelFullSuiteConcurrency(specCount, envInput, hostInfo) {
  ...
  const override = parsePositiveInt(
    env.NODOASSIST_TEST_PROJECTS_PARALLEL,
    "NODOASSIST_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.NODOASSIST_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.NODOASSIST_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `nodoassist-vitest-include-${randomUUID()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.NODOASSIST_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
