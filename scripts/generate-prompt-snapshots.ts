// Generate Prompt Snapshots script supports NodoAssist repository automation.
...
export async function createFormattedPromptSnapshotFiles(): Promise<PromptSnapshotFile[]> {
  ...
  const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-prompt-snapshots-"));
  try {
