// Load Channel Config Surface script supports NodoAssist repository automation.
...
export async function loadChannelConfigSurfaceModule(
  ...
  const loadViaBun = (candidatePath: string) => {
    const script = `
      ...
      const modulePath = process.env.NODOASSIST_CONFIG_SURFACE_MODULE;
      if (!modulePath) {
        throw new Error("missing NODOASSIST_CONFIG_SURFACE_MODULE");
      }
    ...
    const result = spawnSync("bun", ["-e", script], {
      ...
      env: {
        ...process.env,
        NODOASSIST_CONFIG_SURFACE_MODULE: path.resolve(candidatePath),
      },
    });
    if (result.error) {
  ...
  const loadViaJiti = (candidatePath: string) => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "nodoassist/plugin-sdk": pluginSdkAlias } : {}),
      ...resolvePluginSdkScopedAliasMap({
        modulePath: resolvedPath,
        pluginSdkResolution: "src",
      }),
