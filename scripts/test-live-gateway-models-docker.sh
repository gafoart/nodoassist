...
LIVE_GATEWAY_MAX_MODELS="$(nodoassist_live_read_positive_int_env NODOASSIST_LIVE_GATEWAY_MAX_MODELS 8)"
LIVE_GATEWAY_STEP_TIMEOUT_MS="$(nodoassist_live_read_positive_int_env NODOASSIST_LIVE_GATEWAY_STEP_TIMEOUT_MS 45000)"
LIVE_GATEWAY_MODEL_TIMEOUT_MS="$(nodoassist_live_read_positive_int_env NODOASSIST_LIVE_GATEWAY_MODEL_TIMEOUT_MS 90000)"
TEMP_DIRS=()
DOCKER_HOME_MOUNT=()
...
elif [[ -n "${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(nodoassist_live_collect_auth_dirs_from_csv "${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(nodoassist_live_collect_auth_files_from_csv "${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}")
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
...
DOCKER_RUN_ARGS=()
nodoassist_live_init_docker_run_args DOCKER_RUN_ARGS "${NODOASSIST_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e NODOASSIST_LIVE_GATEWAY_MODELS="${NODOASSIST_LIVE_GATEWAY_MODELS:-modern}" \
  -e NODOASSIST_LIVE_GATEWAY_PROVIDERS="${NODOASSIST_LIVE_GATEWAY_PROVIDERS:-}" \
  -e NODOASSIST_LIVE_GATEWAY_THINKING="${NODOASSIST_LIVE_GATEWAY_THINKING:-}" \
  -e NODOASSIST_LIVE_GATEWAY_SMOKE="${NODOASSIST_LIVE_GATEWAY_SMOKE:-1}" \
  -e NODOASSIST_LIVE_GATEWAY_MAX_MODELS="$LIVE_GATEWAY_MAX_MODELS" \
  -e NODOASSIST_LIVE_GATEWAY_HEARTBEAT_MS="${NODOASSIST_LIVE_GATEWAY_HEARTBEAT_MS:-}" \
  -e NODOASSIST_LIVE_GATEWAY_STEP_TIMEOUT_MS="$LIVE_GATEWAY_STEP_TIMEOUT_MS" \
  -e NODOASSIST_LIVE_GATEWAY_MODEL_TIMEOUT_MS="$LIVE_GATEWAY_MODEL_TIMEOUT_MS" \
