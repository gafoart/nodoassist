...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "NODOASSIST_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "NODOASSIST_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `NODOASSIST_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model =
    env[providerEnvKey]?.trim() || env.NODOASSIST_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
function buildReleaseProviderConfigOverride(providerMeta) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "nodoassist" } } : {}),
    models: [],
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.nodoassist-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "NODOASSIST_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "NODOASSIST_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("NODOASSIST_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.NODOASSIST_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.NODOASSIST_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.NODOASSIST_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `nodoassist@${previousVersion}` : "nodoassist@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.NODOASSIST_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.NODOASSIST_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
...
export function resolvePackageCandidatePackCommand(sourceDir, packDir) {
  const packageHelper = join(sourceDir, "scripts", "package-nodoassist-for-docker.mjs");
...
async function runUpgradeLane(params) {
  ...
  try {
    ...
    await runTimedLanePhase(lane, "update", async () => {
      try {
        updateResult = await runNodoAssist({
          lane,
          env: updateEnv,
          args: updateArgs,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        NODOASSIST_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `nodoassist-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".nodoassist");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    NODOASSIST_HOME: lane.homeDir,
    NODOASSIST_STATE_DIR: lane.stateDir,
    NODOASSIST_CONFIG_PATH: join(lane.stateDir, "nodoassist.json"),
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.NODOASSIST_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]nodoassist\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/nodoassist[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "nodoassist@latest") {
    return "";
  }
  if (trimmed.startsWith("nodoassist@")) {
    return trimmed.slice("nodoassist@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "nodoassist@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view nodoassist@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'nodoassist.cmd'),
      (Join-Path $npmPrefix 'nodoassist.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command nodoassist -ErrorAction Stop
...
Write-Output "__NODOASSIST_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__NODOASSIST_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed nodoassist path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v nodoassist >/dev/null 2>&1",
    'printf "__NODOASSIST_PATH__=%s\\n" "$(command -v nodoassist)"',
    "nodoassist --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__NODOASSIST_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed nodoassist path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `nodoassist update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.nodoassist/logs/${params.lane.name}-discord-gateway.log`),
    });
...
export function dashboardHtmlMarkerStatus(html: string): {
  ...
  const title = html.includes("<title>NodoAssist Control</title>");
  const app = html.includes("<nodoassist-app></nodoassist-app>");
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.NODOASSIST_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.NODOASSIST_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.NODOASSIST_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.NODOASSIST_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "nodoassist/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.NODOASSIST_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.NODOASSIST_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.NODOASSIST_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing NODOASSIST_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "NODOASSIST_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.NODOASSIST_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.NODOASSIST_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      NODOASSIST_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      NODOASSIST_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
async function resolveGatewayStatusArgs(lane, env, logPath) {
  try {
    const help = await runNodoAssist({
      lane,
      env,
      args: ["gateway", "status", "--help"],
      logPath,
...
async function runAgentTurn(params) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      const logOffset = readLogFileSize(params.logPath);
      const result = await runNodoAssist({
        lane: params.lane,
        env: params.env,
        args: buildReleaseAgentTurnArgs(sessionId),
        logPath: params.logPath,
...
async function runNodoAssist(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "nodoassist.mjs");
}
