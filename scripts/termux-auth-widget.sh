...
# NodoAssist Auth Widget for Termux
...
# Check auth status
termux-toast "Checking NodoAssist auth..."

STATUS=$(ssh "$SERVER" '$HOME/nodoassist/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/nodoassist/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|NODOASSIST_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/nodoassist/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|NODOASSIST_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: NodoAssist won't work until re-auth"
                ;;
        esac
        ;;
