#!/usr/bin/env node
// Development runner that rebuilds NodoAssist, runs runtime postbuild steps, and
...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "nodoassist.plugin.json"));
    }
...
const listRequiredNodoAssistExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
...
const listRequiredStaticExtensionAssetOutputs = (deps) => {
  if (deps.env.NODOASSIST_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  const runtimeRoot = resolveRuntimePostBuildRuntimeRoot(deps);
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredNodoAssistExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.NODOASSIST_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.NODOASSIST_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.NODOASSIST_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by NODOASSIST_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by NODOASSIST_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "NODOASSIST_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "NODOASSIST_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "NODOASSIST_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "NODOASSIST_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "NODOASSIST_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "NODOASSIST_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "NODOASSIST_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "NODOASSIST_RUN_NODE_SKIP_DTS_BUILD";
...
const logRunner = (message, deps) => {
  if (deps.env.NODOASSIST_RUNNER_LOG === "0") {
    return;
  }
  const line = `[nodoassist] ${message}\n`;
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.NODOASSIST_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const createRunNodeProgress = (label, deps) => {
  ...
  const render = () => {
    ...
    frameIndex += 1;
    deps.stderr.write(`\r[nodoassist] ${frame} ${label} (${elapsedSeconds}s)`);
    visible = true;
  };
...
const listRunNodeCpuProfiles = (deps, absoluteProfileDir, commandName) => {
  ...
  const prefix = `nodoassist-${commandName}-`;
  return entries
    .filter(
      (entry) =>
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `nodoassist-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.NODOASSIST_TRACE_SYNC_IO === "1") {
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[nodoassist] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldSkipWatchRuntimeSync = (deps, requirement) =>
  deps.env.NODOASSIST_WATCH_MODE === "1" &&
...
const shouldFastPathExistingDistForGatewayClient = (deps) =>
  ...
  deps.env.NODOASSIST_FORCE_BUILD !== "1" &&
...
const canUseStampedGatewayClientDist = (deps) => {
  ...
  if (
    ...
    deps.env.NODOASSIST_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.NODOASSIST_BUILD_PRIVATE_QA = "1";
    deps.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.NODOASSIST_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
  ...
  try {
    ...
    if (shouldFastPathExistingDistForGatewayClient(deps)) {
      exitCode = await runNodoAssist(deps);
