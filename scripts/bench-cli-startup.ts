// Bench Cli Startup script supports NodoAssist repository automation.
...
const DEFAULT_ENTRY = "nodoassist.mjs";
const MAX_RSS_MARKER = "__NODOASSIST_MAX_RSS_KB__=";

function resolveTimeoutKillGraceMs(env: NodeJS.ProcessEnv): number {
  const raw = env.VITEST ? env.NODOASSIST_TEST_CLI_STARTUP_TIMEOUT_KILL_GRACE_MS : undefined;
  if (!raw || !/^\d+$/u.test(raw)) {
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "NODOASSIST_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "NODOASSIST_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.NODOASSIST_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "nodoassist-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".nodoassist");
...
function printUsage(): void {
  console.log(`NodoAssist CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: nodoassist.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "nodoassist-cli-bench-"));
