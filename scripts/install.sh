...
# NodoAssist Installer for macOS and Linux
...
resolve_nodoassist_effective_home() {
    local nodoassist_home="${NODOASSIST_HOME:-}"
    if [[ -z "$nodoassist_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$nodoassist_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$nodoassist_home" == \~/* ]]; then
        echo "${HOME}${nodoassist_home:1}"
        return
    fi
    echo "$nodoassist_home"
}

resolve_nodoassist_user_path() {
    local input="$1"
    local effective_home
    effective_home="$(resolve_nodoassist_effective_home)"
    if [[ "$input" == "~" ]]; then
        echo "$effective_home"
    elif [[ "$input" == \~/* ]]; then
...
GUM_VERSION="${NODOASSIST_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$NODOASSIST_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
cleanup_npm_nodoassist_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.nodoassist-* "$npm_root"/nodoassist 2>/dev/null || true
}

extract_nodoassist_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_nodoassist_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/nodoassist" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/nodoassist"|"/usr/local/bin/nodoassist")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/nodoassist/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale nodoassist symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing nodoassist binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing NodoAssist package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing NodoAssist package"
    "${cmd[@]}" >"$log" 2>&1
}
...
install_nodoassist_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "NodoAssist npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*nodoassist" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_nodoassist_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_nodoassist_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "NodoAssist npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an nodoassist binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${NODOASSIST_TAGLINE_INDEX:-}" ]]; then
        if [[ "${NODOASSIST_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((NODOASSIST_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${NODOASSIST_NO_ONBOARD:-0}
NO_PROMPT=${NODOASSIST_NO_PROMPT:-0}
DRY_RUN=${NODOASSIST_DRY_RUN:-0}
INSTALL_METHOD=${NODOASSIST_INSTALL_METHOD:-}
NODOASSIST_VERSION=${NODOASSIST_VERSION:-latest}
USE_BETA=${NODOASSIST_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_nodoassist_effective_home)/nodoassist"
GIT_DIR=${NODOASSIST_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${NODOASSIST_GIT_UPDATE:-1}
NPM_LOGLEVEL="${NODOASSIST_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${NODOASSIST_VERBOSE:-0}"
VERIFY_INSTALL="${NODOASSIST_VERIFY_INSTALL:-0}"
NODOASSIST_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
NodoAssist installer (macOS + Linux)
...
Environment variables:
  NODOASSIST_INSTALL_METHOD=git|npm
  NODOASSIST_VERSION=latest|next|<semver>|<spec>
  NODOASSIST_BETA=0|1
  NODOASSIST_GIT_DIR=...
  NODOASSIST_GIT_UPDATE=0|1
  NODOASSIST_NO_PROMPT=1
  NODOASSIST_VERIFY_INSTALL=1
  NODOASSIST_DRY_RUN=1
  NODOASSIST_NO_ONBOARD=1
  NODOASSIST_VERBOSE=1
  NODOASSIST_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected NodoAssist checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a NodoAssist source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_nodoassist_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"nodoassist"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
    ...
    for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
        if [[ -f "$rc" ]]; then
            if [[ "$(sed -n '1p' "$rc")" != "$path_line" ]]; then
                local tmp_rc="${rc}.nodoassist-tmp"
                {
                    printf '%s\n' "$path_line"
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future NodoAssist updates; use npm i -g nodoassist@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_nodoassist_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/nodoassist" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/nodoassist" ]]; then
        ln -sf "$npm_root/nodoassist/dist/entry.js" "${npm_bin}/nodoassist"
        ui_info "Created nodoassist bin link at ${npm_bin}/nodoassist"
    fi
    return 0
}

# Check for existing NodoAssist installation
check_existing_nodoassist() {
    if [[ -n "$(type -P nodoassist 2>/dev/null || true)" ]]; then
        ui_info "Existing NodoAssist installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_nodoassist_ref() {
    local requested="${NODOASSIST_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "nodoassist" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_nodoassist_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${NODOASSIST_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_nodoassist_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/nodoassist"
        ...
        local version=""
        version="$(nodoassist_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_nodoassist_npm_root_candidates)
}
...
warn_duplicate_nodoassist_global_installs() {
    ...
    done < <(find_nodoassist_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple NodoAssist global installs detected"
    echo "  Different Node/npm environments can run different NodoAssist versions."

    local active_node active_npm active_nodoassist
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_nodoassist="${NODOASSIST_BIN:-}"
    if [[ -z "$active_nodoassist" ]]; then
        active_nodoassist="$(type -P nodoassist 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active nodoassist: ${INFO}${active_nodoassist:-none}${NC}"
    echo ""
    echo "  Found installs:"
    ...
    echo "    npm uninstall -g nodoassist"
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make nodoassist show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

nodoassist_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "nodoassist"
        return 0
    ...
    local original_claw=""
    original_claw="$(PATH="$ORIGINAL_PATH" type -P nodoassist 2>/dev/null || true)"
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${NODOASSIST_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_nodoassist_not_found() {
    ui_warn "Installed, but nodoassist is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t nodoassist 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named nodoassist; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_nodoassist_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P nodoassist 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/nodoassist" ]]; then
        echo "${npm_bin}/nodoassist"
        return 0
    fi
...
resolve_installed_nodoassist_bin() {
    local installed_bin=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        installed_bin="$HOME/.local/bin/nodoassist"
    elif [[ "$INSTALL_METHOD" == "npm" ]]; then
        local npm_bin=""
        npm_bin="$(npm_global_bin_dir || true)"
        if [[ -n "$npm_bin" ]]; then
            installed_bin="${npm_bin}/nodoassist"
        fi
    fi
...
install_nodoassist_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing NodoAssist from git checkout: ${repo_dir}"
    else
        ui_info "Installing NodoAssist from GitHub (${repo_url})"
    fi
    ...
    local git_ref
    git_ref="$(resolve_git_nodoassist_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_nodoassist_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building NodoAssist" run_pnpm -C "$repo_dir" build
    ...
    cat > "$HOME/.local/bin/nodoassist" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec ${node_bin_quoted} ${entry_path_quoted} "\$@"
EOF
    chmod +x "$HOME/.local/bin/nodoassist"
    ui_success "NodoAssist wrapper installed to \$HOME/.local/bin/nodoassist"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view nodoassist dist-tags.beta 2>/dev/null || true)"
...
is_nodoassist_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#nodoassist@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:nodoassist/nodoassist($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/nodoassist/nodoassist(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]nodoassist/nodoassist(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/nodoassist/nodoassist(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:nodoassist/nodoassist(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:nodoassist/nodoassist#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_nodoassist() {
    local package_name="nodoassist"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            NODOASSIST_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="nodoassist"
        else
            NODOASSIST_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${NODOASSIST_VERSION}" ]]; then
        NODOASSIST_VERSION="latest"
    fi

    if is_nodoassist_source_package_install_spec "${NODOASSIST_VERSION}"; then
        ui_error "npm installs do not support NodoAssist GitHub source targets like '${NODOASSIST_VERSION}'."
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${NODOASSIST_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${NODOASSIST_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing NodoAssist v${resolved_version}"
    else
        ui_info "Installing NodoAssist (${NODOASSIST_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${NODOASSIST_VERSION}")"

    if ! install_nodoassist_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_nodoassist_paths
        install_nodoassist_npm "${install_spec}"
    fi

    if [[ "${NODOASSIST_VERSION}" == "latest" && "${package_name}" == "nodoassist" ]]; then
        if ! resolve_nodoassist_bin &> /dev/null; then
            ui_warn "npm install nodoassist@latest failed; retrying nodoassist@next"
            cleanup_npm_nodoassist_paths
            install_nodoassist_npm "nodoassist@next"
        fi
    fi

    ensure_nodoassist_bin_link || true

    ui_success "NodoAssist installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${NODOASSIST_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_nodoassist_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (nodoassist not on PATH yet)"
        warn_nodoassist_not_found
        return 0
    fi
    local doctor_exit=0
...
has_nodoassist_config() {
    local effective_home
    effective_home="$(resolve_nodoassist_effective_home)"
    if [[ -n "${NODOASSIST_CONFIG_PATH:-}" ]]; then
        local config_path
        config_path="$(resolve_nodoassist_user_path "$NODOASSIST_CONFIG_PATH")"
        [[ -f "$config_path" ]]
        return
    fi

    if [[ -n "${NODOASSIST_STATE_DIR:-}" ]]; then
        local state_dir
        state_dir="$(resolve_nodoassist_user_path "$NODOASSIST_STATE_DIR")"
        if [[ -f "$state_dir/nodoassist.json" || -f "$state_dir/clawdbot.json" ]]; then
            return 0
        fi
        return 1
    fi

    if [[ -f "$effective_home/.nodoassist/nodoassist.json" ||
        -f "$effective_home/.nodoassist/clawdbot.json" ||
        -f "$effective_home/.clawdbot/nodoassist.json" ||
...
if ! declare -F extract_nodoassist_semver >/dev/null 2>&1; then
...
resolve_nodoassist_version() {
    ...
    local claw="${NODOASSIST_BIN:-}"
    if [[ -z "$claw" ]] && command -v nodoassist &> /dev/null; then
        claw="$(command -v nodoassist)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_nodoassist_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/nodoassist/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/nodoassist/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "nodoassist daemon status --json" "$claw" daemon status --json || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if run_quiet_step "Restarting gateway service" "$claw" gateway restart; then
        ...
        local user_claw
        user_claw="$(nodoassist_command_for_user "$claw")"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: nodoassist not on PATH yet"
        warn_nodoassist_not_found
        return 1
    fi

    run_quiet_step "Checking NodoAssist version" "$claw" --version || return 1
...
main() {
    ...
    detected_checkout="$(detect_nodoassist_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found NodoAssist checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set NODOASSIST_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_nodoassist; then
        is_upgrade=true
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        if npm list -g nodoassist &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g nodoassist 2>/dev/null || true
        ...
        install_nodoassist_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/nodoassist" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/nodoassist"
    ...
    NODOASSIST_BIN="$(resolve_installed_nodoassist_bin || true)"
    warn_duplicate_nodoassist_global_installs || true
    ...
    if has_nodoassist_config; then
        config_present=true
        refresh_gateway_service_if_loaded
    fi

    local installed_version
    installed_version=$(resolve_nodoassist_version)
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        local user_claw
        user_claw="$(nodoassist_command_for_user "${NODOASSIST_BIN:-}")"
        ui_section "Source install details"
        ui_kv "Checkout" "$final_git_dir"
        ui_kv "Wrapper" "$HOME/.local/bin/nodoassist"
    ...
    if [[ "$config_present" != "true" ]]; then
        if [[ "$NO_ONBOARD" == "1" ]]; then
            ...
            if is_promptable; then
                local claw="${NODOASSIST_BIN:-}"
                if [[ -z "$claw" ]]; then
                    claw="$(resolve_installed_nodoassist_bin || true)"
                fi
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (nodoassist not on PATH yet)"
                    warn_nodoassist_not_found
                    return 0
                fi
                exec </dev/tty
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running nodoassist doctor"
            local doctor_ok=0
            local doctor_exit=0
            if (( ${#doctor_args[@]} )); then
                NODOASSIST_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null || doctor_exit=$?
            else
                NODOASSIST_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty || doctor_exit=$?
            fi
            if (( doctor_exit == 130 )); then
                abort_install_int
            fi
            ...
            if (( doctor_ok )); then
                ...
                NODOASSIST_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
    ...
    if [[ "$config_present" == "true" ]]; then
        ...
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            ...
            if [[ "$DRY_RUN" == "1" ]]; then
                ...
                if NODOASSIST_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
...
if [[ "${NODOASSIST_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
