...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.nodoassist) ? packageJson.nodoassist : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.nodoassist?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.nodoassist?.channel?.id) || left.name;
    const rightId = trimString(right.nodoassist?.channel?.id) || right.name;
