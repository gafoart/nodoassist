...
// Code Mode Namespace Live script supports NodoAssist repository automation.
...
async function runOne(mode: Mode, task: Task, model: string, apiKey: string): Promise<RunMetrics> {
  ...
  return {
    ...
    ...(process.env.NODOASSIST_CODE_MODE_LIVE_DEBUG === "1" ? { toolResults } : {}),
  };
}
...
export async function main() {
  const model = readArg("model") ?? process.env.NODOASSIST_CODE_MODE_LIVE_MODEL ?? "gpt-5.4-mini";
  ...
  const taskLimit = parseTaskLimit(
    taskArg ?? process.env.NODOASSIST_CODE_MODE_LIVE_TASKS,
    taskArg === undefined ? "NODOASSIST_CODE_MODE_LIVE_TASKS" : "--tasks",
  );
