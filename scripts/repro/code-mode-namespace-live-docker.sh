...
IMAGE_NAME="$(docker_e2e_resolve_image "nodoassist-code-mode-namespace-live-e2e" NODOASSIST_CODE_MODE_NAMESPACE_LIVE_E2E_IMAGE)"
SKIP_BUILD="${NODOASSIST_CODE_MODE_NAMESPACE_LIVE_E2E_SKIP_BUILD:-0}"
PROFILE_FILE="${NODOASSIST_CODE_MODE_NAMESPACE_LIVE_PROFILE_FILE:-${NODOASSIST_TESTBOX_PROFILE_FILE:-$HOME/.nodoassist-testbox-live.profile}}"
run_log=""
...
if ! docker_e2e_run_with_harness \
  ...
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
  -e "NODOASSIST_CODE_MODE_LIVE_MODEL=${NODOASSIST_CODE_MODE_LIVE_MODEL:-gpt-5.4-mini}" \
  -e "NODOASSIST_CODE_MODE_LIVE_TASKS=${NODOASSIST_CODE_MODE_LIVE_TASKS:-3}" \
  -v "$ROOT_DIR":/src:ro \
  "${PROFILE_MOUNT[@]}" \
  "$IMAGE_NAME" \
