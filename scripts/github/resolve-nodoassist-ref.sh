...
REMOTE_URL="${NODOASSIST_REF_REMOTE:-https://github.com/openclaw/openclaw.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-nodoassist-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves NodoAssist branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve NodoAssist ref: ${REF}" >&2
exit 1
