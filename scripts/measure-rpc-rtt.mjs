...
function resolveNodoAssistLaunchArgs(repoRoot, sourceEntryExists = existsSync) {
  const sourceEntry = path.join(repoRoot, "src", "entry.ts");
  if (sourceEntryExists(sourceEntry)) {
    return ["--import", "tsx", sourceEntry];
  }
  return [path.join(repoRoot, "nodoassist.mjs")];
}
...
export async function startGateway({
  ...
  let child;
  const launcherArgs = resolveNodoAssistLaunchArgs(repoRoot, sourceEntryExists);
  try {
    child = spawnImpl(
      process.execPath,
      [
      ...
      ],
      {
        ...
        env: {
          ...
          NODOASSIST_STATE_DIR: path.join(tempRoot, "state"),
...
async function main() {
  ...
  const repoRoot = path.resolve(args.repoRoot ?? process.env.NODOASSIST_REPO_ROOT ?? process.cwd());
  ...
  try {
    ...
    const requireFromNodoAssist = createRequire(path.join(repoRoot, "package.json"));
    const WebSocket = requireFromNodoAssist("ws");
    ...
    const connect = await client.request(
      "connect",
      {
        ...
        client: {
          id: "gateway-client",
          displayName: "nodoassist-rtt rpc probe",
          version: "rtt",
          platform: process.platform,
          mode: "backend",
          instanceId: `nodoassist-rtt-rpc-${randomUUID()}`,
        },
        locale: "en-US",
        userAgent: "nodoassist-rtt-rpc",
        role: "operator",
