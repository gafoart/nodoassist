#!/usr/bin/env node
// Builds NodoAssist packages and plugin SDK artifacts with cache-aware orchestration.
...
export const BUILD_ALL_STEPS = [
  ...
  },
  {
    ...
    env: {
      NODOASSIST_PLUGIN_SDK_CANONICAL_DTS: "1",
    },
    cache: {
  ...
  },
  {
    ...
    // from package.json, git HEAD, and NODOASSIST_CONTROL_UI_BUILD_ID env, so a
...
export function buildAllUsage() {
  return [
    ...
    "",
    "Builds NodoAssist artifacts for the selected profile.",
    "",
    "Profiles:",
...
if (isMainModule()) {
  ...
  if (args?.help) {
    ...
    for (const step of resolveBuildAllSteps(args.profile)) {
      ...
      if (process.env.NODOASSIST_BUILD_CACHE !== "0" && cacheState.fresh) {
