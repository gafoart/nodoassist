...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.NODOASSIST_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "nodoassist.plugin.json");
    ...
    if (packageJson.nodoassist && "extensions" in packageJson.nodoassist) {
      packageJson.nodoassist = {
        ...packageJson.nodoassist,
        extensions: rewritePackageExtensions(packageJson.nodoassist.extensions),
        ...(typeof packageJson.nodoassist.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.nodoassist.setupEntry) }
          : {}),
      };
    }
