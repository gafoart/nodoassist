...
// NodoAssist Npm Prepublish Verify script supports NodoAssist repository automation.
...
import { resolveNpmCommandInvocation } from "./nodoassist-npm-release-check.ts";
...
type NodoAssistNpmPrepublishVerifyArgs =
  | {
...
export function nodoAssistNpmPrepublishVerifyUsage(): string {
  return "Usage: node --import tsx scripts/nodoassist-npm-prepublish-verify.ts <tarball.tgz> [expected-version] [dependency.tgz ...]";
}

export function parseNodoAssistNpmPrepublishVerifyArgs(
  argv: readonly string[],
): NodoAssistNpmPrepublishVerifyArgs {
  ...
  if (!tarballPath) {
    throw new Error(nodoAssistNpmPrepublishVerifyUsage());
  }
  if (tarballPath.startsWith("-")) {
    throw new Error(`Unknown nodoassist npm prepublish verifier option: ${tarballPath}`);
  }

  const expectedVersion = args[1]?.trim();
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown nodoassist npm prepublish verifier option: ${expectedVersion}`);
  }
...
function main(argv = process.argv.slice(2)): void {
  const args = parseNodoAssistNpmPrepublishVerifyArgs(argv);
  if (args.help) {
    console.log(nodoAssistNpmPrepublishVerifyUsage());
    return;
  }

  const workingDir = mkdtempSync(join(tmpdir(), "nodoassist-prepublish-"));
  ...
  try {
    ...
    if (usesPreparedLocalDependencyInstall(args.dependencyTarballPaths.length)) {
        ...
        `${JSON.stringify(
          {
            private: true,
            dependencies: {
              "@nodoassist/ai": pathToFileURL(realpathSync(args.dependencyTarballPaths[0])).href,
              nodoassist: pathToFileURL(realpathSync(args.tarballPath)).href,
            },
          },
          null,
          2,
      ...
      packageRoot = join(prefixDir, "node_modules", "nodoassist");
      const binaryPath = join(
        prefixDir,
        "node_modules",
        ".bin",
        process.platform === "win32" ? "nodoassist.cmd" : "nodoassist",
      );
      binaryInvocation =
      ...
      packageRoot = join(globalRoot, "nodoassist");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed nodoassist binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `nodoassist-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`nodoassist-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
