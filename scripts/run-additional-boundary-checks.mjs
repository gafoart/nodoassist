...
export function parseCliArgs(args, env = process.env) {
  let shardSpec = env.NODOASSIST_ADDITIONAL_BOUNDARY_SHARD ?? "";
  let help = false;
  ...
  try {
    ...
    if (cliArgs.help) {
      ...
      const concurrencyRaw =
        process.env.NODOASSIST_ADDITIONAL_BOUNDARY_CONCURRENCY ??
        process.env.NODOASSIST_EXTENSION_BOUNDARY_CONCURRENCY;
      const concurrencyLabel =
        process.env.NODOASSIST_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
          ? "NODOASSIST_EXTENSION_BOUNDARY_CONCURRENCY"
          : "NODOASSIST_ADDITIONAL_BOUNDARY_CONCURRENCY";
      const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
      const checkTimeoutMs = resolvePositiveInteger(
        process.env.NODOASSIST_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
        DEFAULT_CHECK_TIMEOUT_MS,
        "NODOASSIST_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
      );
      const outputMaxBytes = resolvePositiveInteger(
        process.env.NODOASSIST_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
        DEFAULT_OUTPUT_MAX_BYTES,
        "NODOASSIST_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
      );
