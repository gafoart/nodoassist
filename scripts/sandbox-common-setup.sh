...
BASE_IMAGE="${BASE_IMAGE:-nodoassist-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-nodoassist-sandbox-common:bookworm-slim}"
...
NODOASSIST_DOCKER_BUILD_USE_BUILDX="${NODOASSIST_DOCKER_BUILD_USE_BUILDX:-0}"
NODOASSIST_DOCKER_BUILD_CACHE_FROM="${NODOASSIST_DOCKER_BUILD_CACHE_FROM:-}"
NODOASSIST_DOCKER_BUILD_CACHE_TO="${NODOASSIST_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=nodoassist.sandbox=1)
NOTE
