...
const DEFAULT_WORKFLOW = "nodoassist-live-and-e2e-checks-reusable.yml";
...
function ghWorkflowCommand(lanes, ref, workflow, reuseInputs = {}) {
  const workflowRef =
    ...
    process.env.NODOASSIST_DOCKER_E2E_WORKFLOW_REF ||
...
function defaultOutputDir(input) {
  return fs.mkdtempSync(
    path.join(os.tmpdir(), `nodoassist-docker-e2e-rerun-${safePathSegment(input)}-`),
  );
}
