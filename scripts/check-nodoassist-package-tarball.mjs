...
function usage() {
  return "Usage: node scripts/check-nodoassist-package-tarball.mjs [--require-bundled-workspace-deps] <nodoassist.tgz>";
}
...
function parseArgs(argv) {
  ...
  for (const rawArg of args) {
    ...
    if (arg.startsWith("-")) {
      throw new Error(`Unknown NodoAssist package tarball check option: ${arg}`);
    }
    if (tarball) {
      throw new Error(`Unexpected NodoAssist package tarball check argument: ${arg}`);
    }
    tarball = arg;
  }
  if (!tarball) {
...
if (!fs.existsSync(tarball)) {
  fail(`NodoAssist package tarball does not exist: ${tarball}`);
}
...
const REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES = ["@nodoassist/ai"];
...
function collectRequiredBundledWorkspaceDependencyErrors(packageJson, entrySet) {
  ...
  for (const name of REQUIRED_BUNDLED_WORKSPACE_DEPENDENCIES) {
    ...
    if (!bundledDependencies.has(name)) {
      errors.push(
        `package.json dependencies.${name} must be listed in bundleDependencies because it is private to the NodoAssist workspace`,
      );
    }
...
const phaseTimingsEnabled = process.env.NODOASSIST_PACKAGE_TARBALL_CHECK_TIMINGS !== "0";
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-nodoassist-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "nodoassist") {
      errors.push("npm-shrinkwrap.json root name must be nodoassist");
    }
    ...
    if (!rootPackage || rootPackage.name !== "nodoassist") {
      errors.push("npm-shrinkwrap.json packages root must name nodoassist");
    }
...
if (errors.length > 0) {
  fs.rmSync(extractDir, { recursive: true, force: true });
  fail(`NodoAssist package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`NodoAssist package tarball integrity warning: ${warning}`);
}
fs.rmSync(extractDir, { recursive: true, force: true });
console.log("NodoAssist package tarball integrity passed.");
