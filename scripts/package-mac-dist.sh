...
# Output:
# - dist/NodoAssist.app
# - dist/NodoAssist-<version>.zip
# - dist/NodoAssist-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.nodoassist.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/nodoassist-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/NodoAssist.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/NodoAssist-$VERSION.zip"
DMG="$ROOT_DIR/dist/NodoAssist-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/NodoAssist-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/NodoAssist-$VERSION.dSYM.zip"
