...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/nodoassist/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "NodoAssist Re-Auth" -c "After getting key, run: ssh $SERVER '~/nodoassist/scripts/mobile-reauth.sh'" --id nodoassist-auth
        ;;
    *)
