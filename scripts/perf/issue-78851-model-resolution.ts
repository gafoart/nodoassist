// Issue 78851 Model Resolution script supports NodoAssist repository automation.
...
function buildConfig(options: Options, workspaceDir: string): NodoAssistConfig {
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureNodoAssistModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "nodoassist-issue-78851-"));
