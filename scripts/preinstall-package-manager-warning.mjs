...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[nodoassist] warning: detected ${packageManager} for install lifecycle.`,
    "[nodoassist] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[nodoassist] prefer: corepack pnpm install",
  ].join("\n");
}
