// Check No Monolithic Plugin Sdk Entry Imports script supports NodoAssist repository automation.
import fs from "node:fs";
import path from "node:path";
import { discoverNodoAssistPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']nodoassist\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']nodoassist\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']nodoassist\/plugin-sdk\/channel-runtime["']/,
    label: "nodoassist/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']nodoassist\/plugin-sdk\/config-runtime["']/,
    label: "nodoassist/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']nodoassist\/plugin-sdk\/infra-runtime["']/,
    label: "nodoassist/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import monolithic nodoassist/plugin-sdk.",
      );
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy nodoassist/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused nodoassist/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
