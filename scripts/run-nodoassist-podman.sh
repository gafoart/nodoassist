#!/usr/bin/env bash
# Rootless NodoAssist in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-nodoassist-podman.sh launch        # Start gateway
#   ./scripts/run-nodoassist-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   nodoassist --container nodoassist dashboard --no-open
#   nodoassist --container nodoassist channels login
#
...
run_podman_detached() {
  nodoassist_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      NODOASSIST_GATEWAY_TOKEN|NODOASSIST_PODMAN_CONTAINER|NODOASSIST_PODMAN_IMAGE|NODOASSIST_IMAGE|NODOASSIST_PODMAN_PULL|NODOASSIST_PODMAN_RUN_TIMEOUT|NODOASSIST_PODMAN_GATEWAY_HOST_PORT|NODOASSIST_GATEWAY_PORT|NODOASSIST_PODMAN_BRIDGE_HOST_PORT|NODOASSIST_BRIDGE_PORT|NODOASSIST_GATEWAY_BIND|NODOASSIST_PODMAN_USERNS|NODOASSIST_BIND_MOUNT_OPTIONS|NODOASSIST_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-nodoassist-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${NODOASSIST_CONFIG_DIR:-$EFFECTIVE_HOME/.nodoassist}"
ENV_FILE="${NODOASSIST_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${NODOASSIST_CONFIG_DIR:-$EFFECTIVE_HOME/.nodoassist}"
ENV_FILE="${NODOASSIST_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${NODOASSIST_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${NODOASSIST_PODMAN_CONTAINER:-nodoassist}"
NODOASSIST_IMAGE="${NODOASSIST_PODMAN_IMAGE:-${NODOASSIST_IMAGE:-nodoassist:local}}"
PODMAN_PULL="${NODOASSIST_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${NODOASSIST_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${NODOASSIST_PODMAN_GATEWAY_HOST_PORT:-${NODOASSIST_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${NODOASSIST_PODMAN_BRIDGE_HOST_PORT:-${NODOASSIST_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${NODOASSIST_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$NODOASSIST_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v nodoassist >/dev/null 2>&1; then
    return 0
  fi
  NODOASSIST_CONTAINER="" NODOASSIST_CONFIG_DIR="$config_dir" \
    nodoassist config get gateway.bind 2>/dev/null || true
}
...
# NODOASSIST_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${NODOASSIST_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'NODOASSIST_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v nodoassist >/dev/null 2>&1; then
    echo "Warning: nodoassist not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    NODOASSIST_CONTAINER="" NODOASSIST_CONFIG_DIR="$config_dir" \
      nodoassist config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    NODOASSIST_CONTAINER="" NODOASSIST_CONFIG_DIR="$config_dir" \
      nodoassist config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  NODOASSIST_CONTAINER="" NODOASSIST_CONFIG_DIR="$config_dir" \
    nodoassist config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${NODOASSIST_GATEWAY_TOKEN:-}" ]]; then
  export NODOASSIST_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "NODOASSIST_GATEWAY_TOKEN" "$NODOASSIST_GATEWAY_TOKEN"
  echo "Generated NODOASSIST_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/nodoassist.json"
...
PODMAN_USERNS="${NODOASSIST_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported NODOASSIST_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (NODOASSIST_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${NODOASSIST_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${NODOASSIST_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$NODOASSIST_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.nodoassist/.npm \
    -e NODOASSIST_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.nodoassist:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.nodoassist/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$NODOASSIST_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME nodoassist dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME nodoassist devices approve --latest  # if pairing required"
