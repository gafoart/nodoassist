...
const ignoreRepoBinary = process.env.NODOASSIST_CRABBOX_WRAPPER_IGNORE_REPO_BINARY === "1";
...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.NODOASSIST_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function enforceBrokeredAws(commandArgs, providerName) {
  ...
  console.error(
    [
      "[crabbox] provider=aws requires a configured Crabbox broker for NodoAssist proof.",
      "[crabbox] run `crabbox login --url https://crabbox.openclaw.ai --provider aws`, then retry.",
      "[crabbox] for intentional direct AWS provider debugging, set NODOASSIST_CRABBOX_ALLOW_DIRECT_AWS=1.",
    ].join("\n"),
  );
  process.exit(2);
}
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    "nodoassist_changed_gate_base=${NODOASSIST_CHANGED_GATE_BASE:-" + quotedBase + "};",
    'if ! command -v git >/dev/null 2>&1; then echo "git is required for NodoAssist remote changed-gate sync" >&2; exit 2; fi;',
    'nodoassist_changed_gate_remote_base="$(git rev-parse --verify refs/remotes/origin/main 2>/dev/null || true)";',
    'if ! git status --short >/dev/null 2>&1 || [ "$nodoassist_changed_gate_remote_base" != "$nodoassist_changed_gate_base" ]; then',
    "rm -rf .git;",
    "git init -q;",
    ...
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=NodoAssist -c user.email=ci@nodoassist.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$nodoassistModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($nodoassistModulesDir) {",
    'if (-not (Test-Path $nodoassistModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $nodoassistModulesDir" }',
    '$nodoassistWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$nodoassistSelfModules = Join-Path $nodoassistModulesDir "node_modules"',
    'if (-not (Test-Path $nodoassistSelfModules)) { cmd /c mklink /J "$nodoassistSelfModules" "$nodoassistModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $nodoassistWorkspaceModules)) { cmd /c mklink /J "$nodoassistWorkspaceModules" "$nodoassistModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remotePosixJsEnvBootstrap() {
  return [
    ...
    "nodoassist_env_args=();",
    "nodoassist_env_ignore=0;",
    "nodoassist_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) nodoassist_env_ignore=1; nodoassist_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${nodoassist_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) nodoassist_env_ignore=1; nodoassist_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) nodoassist_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then nodoassist_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) nodoassist_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$nodoassist_env_ignore" = "1" ]; then nodoassist_env_args+=("PATH=${NODOASSIST_CRABBOX_BOOTSTRAP_PATH:-$PATH}:${1#PATH=}"); else nodoassist_env_args+=("$1"); fi; nodoassist_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) nodoassist_env_args+=("$1"); shift ;;',
    '--) nodoassist_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    ...
    'command env "${nodoassist_env_args[@]}" "$@";',
    "};",
  ];
}

function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  const nodeVersion = process.env.NODOASSIST_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "nodoassist_crabbox_bootstrap_macos_js() {",
    'tool_root="${NODOASSIST_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.nodoassist-crabbox-toolchain}";',
    ...
    'ready_marker="$node_dir/.nodoassist-crabbox-node-ready";',
  ...
  if (bun) {
    bootstrap.push(
      ...
      'bun_ready_marker="$bun_root/.nodoassist-crabbox-bun-ready";',
  ...
  bootstrap.push('export NODOASSIST_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "nodoassist_crabbox_bootstrap_macos_js");
  return bootstrap.join(" ");
}

function remoteWsl2JsBootstrap({ packageManager = false } = {}) {
  const nodeVersion = process.env.NODOASSIST_CRABBOX_WSL2_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "nodoassist_crabbox_bootstrap_wsl2_js() {",
    'tool_root="${NODOASSIST_CRABBOX_WSL2_TOOLCHAIN_DIR:-$HOME/.nodoassist-crabbox-toolchain}";',
    `node_version=${shellQuote(nodeVersion)};`,
    'arch="$(uname -m)";',
  ...
  bootstrap.push('export NODOASSIST_CRABBOX_BOOTSTRAP_PATH="$PATH";');
  bootstrap.push("};", "nodoassist_crabbox_bootstrap_wsl2_js");
  return bootstrap.join(" ");
}
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    bun: needsBun,
    shellCommand: `nodoassist_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function shellCommandWithEnvShim(command, eligibleSegments) {
  ...
  for (const segment of shellCommandSegmentsWithBounds(command)) {
    ...
    rewritten += "nodoassist_crabbox_env";
...
function prepareRemoteWsl2JsBootstrapScript(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "nodoassist-crabbox-wsl2-script-"));
...
function remoteAwsMacosSwiftBootstrap() {
  return [
    "nodoassist_crabbox_require_macos_swift_62() {",
    'nodoassist_xcode="";',
    'for nodoassist_candidate in /Applications/Xcode_26.1.app /Applications/Xcode_26*.app /Applications/Xcode-26*.app; do if [ -d "$nodoassist_candidate" ]; then nodoassist_xcode="$nodoassist_candidate"; fi; done;',
    'if [ -n "$nodoassist_xcode" ]; then nodoassist_developer="$nodoassist_xcode/Contents/Developer"; if [ ! -d "$nodoassist_developer" ]; then nodoassist_developer="$nodoassist_xcode"; fi; sudo xcode-select -s "$nodoassist_developer" || return 1; fi;',
    'nodoassist_swift_version="$(swift --version 2>&1)" || { status=$?; printf "%s\\n" "$nodoassist_swift_version" >&2; return "$status"; };',
    'printf "%s\\n" "$nodoassist_swift_version" >&2;',
    'nodoassist_swift_major_minor="$(printf "%s\\n" "$nodoassist_swift_version" | sed -nE "s/.*Apple Swift version ([0-9]+)\\.([0-9]+).*/\\1 \\2/p" | head -n 1)";',
    'if [ -z "$nodoassist_swift_major_minor" ]; then echo "[crabbox] NodoAssist macOS app proof requires Swift tools 6.2+; unable to parse swift --version." >&2; return 2; fi;',
    "set -- $nodoassist_swift_major_minor;",
    'if [ "$1" -lt 6 ] || { [ "$1" -eq 6 ] && [ "$2" -lt 2 ]; }; then',
    'echo "[crabbox] NodoAssist macOS app proof requires Swift tools 6.2+ (Xcode 26.x)." >&2;',
    'echo "[crabbox] current Swift is $1.$2; select/install Xcode 26.x or use a Blacksmith macOS runner with Xcode_26.1.app." >&2;',
    "return 2;",
    "fi;",
    'nodoassist_xcodebuild_version="$(xcodebuild -version 2>&1)" || { printf "%s\\n" "$nodoassist_xcodebuild_version" >&2; echo "[crabbox] NodoAssist macOS app proof requires Xcode 26.x; active developer directory does not provide usable xcodebuild." >&2; return 2; };',
    'printf "%s\\n" "$nodoassist_xcodebuild_version" >&2;',
    'nodoassist_xcode_major="$(printf "%s\\n" "$nodoassist_xcodebuild_version" | sed -nE "s/^Xcode ([0-9]+)(\\..*)?$/\\1/p" | head -n 1)";',
    'if [ "$nodoassist_xcode_major" != "26" ]; then echo "[crabbox] NodoAssist macOS app proof requires Xcode 26.x; current xcodebuild is ${nodoassist_xcode_major:-unknown}." >&2; return 2; fi;',
    "};",
    "nodoassist_crabbox_require_macos_swift_62",
  ].join(" ");
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "nodoassist-crabbox-macos-script-"));
  const scriptPath = resolve(scriptRoot, "script.sh");
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/nodoassist-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_nodoassist_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_nodoassist_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `NODOASSIST_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "nodoassist", "crabbox-sync");
  }
  return resolve(tmpdir(), "nodoassist-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.NODOASSIST_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "NODOASSIST_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set NODOASSIST_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower NODOASSIST_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "nodoassist-crabbox-sync-"));
  let active = false;
...
function fullCheckoutKeepaliveIntervalMs() {
  return parseNonNegativeIntegerEnv(
    "NODOASSIST_CRABBOX_SYNC_KEEPALIVE_MS",
    5000,
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm/Bun for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for NodoAssist CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for NodoAssist Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/nodoassist-crabbox-docker-work";
  console.error(
...
function resolveChildKillGraceMs(env) {
  if (!env.VITEST || !env.NODOASSIST_TEST_CRABBOX_CHILD_KILL_GRACE_MS) {
    return 5_000;
  }
  const value = Number.parseInt(env.NODOASSIST_TEST_CRABBOX_CHILD_KILL_GRACE_MS, 10);
  return Number.isFinite(value) && value >= 0 ? value : 5_000;
}

function resolveMetadataProbeTimeoutMs(env) {
  if (!env.VITEST || !env.NODOASSIST_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS) {
    return CRABBOX_METADATA_PROBE_TIMEOUT_MS;
  }
  const value = Number.parseInt(env.NODOASSIST_TEST_CRABBOX_METADATA_PROBE_TIMEOUT_MS, 10);
