...
- iOS SwiftUI text should use branded typography helpers, not bare system fonts. Use `NodoAssistType` in `apps/ios/Sources/**`, `NodoAssistChatTypography` in shared chat UI, `WatchClawType` in watch UI, and `NodoAssistActivityType` in Live Activity UI.
...
- When touching iOS text surfaces, update/keep `apps/ios/Tests/NodoAssistTypographyTests.swift` so bare text/control regressions are caught, then run the focused typography test.
...
- Do not add NodoAssist, NodoAssist Foundation, or other first-party/self-owned license entries. The screen is for third-party/open-source dependency acknowledgements.
- When adding, removing, or upgrading iOS dependencies, audit whether `apps/ios/Resources/Licenses/` needs updates. Exclude dependencies owned by NodoAssist Foundation from the published license list.
