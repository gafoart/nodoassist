...
@MainActor
final class NodoAssistSnapshotUITests: XCTestCase {
    ...
    func testOnboardingExplainsCapabilitiesAndTrust() {
        ...
        app.launchArguments += ["--nodoassist-reset-onboarding"]
        app.launch()
        self.app = app
    ...
    func testEmptyChatStarterPromptSendsMessage() throws {
        self.launchApp(
            ...
            additionalArguments: ["--nodoassist-empty-chat-fixture"])
        ...
        starter.tap()
        let sentText = "Summarize the current NodoAssist status and tell me what needs attention."
    ...
    func testEmptyChatStarterPromptsLocalizeInGerman() throws {
        ...
        let starter = try XCTUnwrap(self.app?.buttons["NodoAssist-Status prüfen"])
        XCTAssertTrue(starter.exists)
        starter.tap()
        XCTAssertTrue(
            self.app?.staticTexts[
                "Fasse den aktuellen NodoAssist-Status zusammen und sage mir, was Aufmerksamkeit erfordert.",
    ...
    func testChatReturnsToOriginatingControlDetail() throws {
        ...
        let returnButton = try XCTUnwrap(self.app?.buttons["NodoAssistChatBackToControlDetailButton"])
    ...
    func testLiveGatewayChatRoundTripAndControlOverview() throws {
        ...
        for index in 0..<3 {
            let seedMarker = "NODOASSIST_E2E_SEED_\(index)_\(Int(Date().timeIntervalSince1970 * 1000))"
        ...
        let replyMarker = "NODOASSIST_E2E_OK_\(Int(Date().timeIntervalSince1970 * 1000))"
        self.sendLiveGatewayMessage(
    ...
    func testManualAuthRetryUsesEditedToken() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["NODOASSIST_IOS_RETRY_E2E"] == "1",
            "Set NODOASSIST_IOS_RETRY_E2E=1 with a local token-auth Gateway on port 18920")
        let token = try XCTUnwrap(ProcessInfo.processInfo.environment["NODOASSIST_IOS_RETRY_TOKEN"])
    ...
    func testPhotosLimitedAccess() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["NODOASSIST_IOS_PHOTOS_E2E"] == "1",
            "Set NODOASSIST_IOS_PHOTOS_E2E=1 to exercise the system Photos prompt")
        ...
        additionalArguments: [String] = [])
    {
        ...
        if screenshotMode {
            app.launchArguments.append("--nodoassist-screenshot-mode")
        }
        app.launchArguments += additionalArguments
        if let appearance {
            app.launchArguments += ["--nodoassist-appearance", appearance]
        }
        app.launch()
        self.app = app
    ...
    {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["NODOASSIST_IOS_LIVE_GATEWAY"] == "1",
            "Set NODOASSIST_IOS_LIVE_GATEWAY=1 and provide a fresh setup code")

        if let setupCode = ProcessInfo.processInfo.environment["NODOASSIST_IOS_LIVE_SETUP_CODE"] {
