...
NodoAssist iOS is the officially released iPhone app. It connects to an NodoAssist Gateway as a `role: node` for chat, voice, approvals, sharing, and device-aware automation.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical NodoAssist team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical NodoAssist team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `ai.nodoassistfoundation.app`
...
- Local development uses the canonical `ai.nodoassistfoundation.app*` bundle IDs when the NodoAssist team is available, and unique `ai.nodoassistfoundation.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `ai.nodoassistfoundation.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `NodoAssistPushMode=appStore`, which derives relay transport, official distribution, the canonical production relay, production APNs, production relay profile, `appleStrict` proof, and the App-Attest-capable entitlement file.
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `ai.nodoassistfoundation.app`
   - `ai.nodoassistfoundation.app.share`
   - `ai.nodoassistfoundation.app.activitywidget`
   - `ai.nodoassistfoundation.app.watchkitapp`
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/NodoAssist-<version>.ipa`
   - `apps/ios/build/app-store/NodoAssist-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/NodoAssist.entitlements` derives `aps-environment` from the active build configuration/signing override.
- App Attest relay builds use `apps/ios/Sources/NodoAssistAppAttest.entitlements`; local/direct builds do not require App Attest provisioning.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual Debug builds default to `NodoAssistPushMode=localSandbox`, direct APNs registration, and a development `aps-environment` entitlement. Local/manual Release builds default to `NodoAssistPushMode=localProduction` and direct production APNs registration.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `NODOASSIST_APNS_TEAM_ID`, `NODOASSIST_APNS_KEY_ID`, and either `NODOASSIST_APNS_PRIVATE_KEY_P8` or `NODOASSIST_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.nodoassist/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `NODOASSIST_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to sandbox APNs through `NodoAssistPushMode=localSandbox`; Release builds default to production APNs through `NodoAssistPushMode=localProduction`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `nodoassist.json`. `NODOASSIST_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine NodoAssist official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as NodoAssist node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
