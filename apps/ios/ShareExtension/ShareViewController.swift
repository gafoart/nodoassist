...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.nodoassistfoundation.app", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle(
            NSLocalizedString("Send to NodoAssist", comment: "Share extension send action"),
            for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus(NSLocalizedString("Sending to NodoAssist gateway…", comment: "Share extension sending status"))
        ...
        do {
            ...
            self.showStatus(NSLocalizedString("Sent to NodoAssist.", comment: "Share extension success status"))
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfigDiscardingUnscopedDeviceAuth() else {
            throw NSError(
                domain: "NodoAssistShare",
                code: 10,
                userInfo: [
                    NSLocalizedDescriptionKey: NSLocalizedString(
                        "NodoAssist is not connected to a gateway yet.",
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "NodoAssist Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("nodoassist-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
