...
enum WatchMessagingPayloadCodec {
    private static let durableSnapshotTypes = [
        NodoAssistWatchPayloadType.appSnapshot.rawValue,
        NodoAssistWatchPayloadType.execApprovalSnapshot.rawValue,
    ]
        ...
        gatewayStableID: String?) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": NodoAssistWatchPayloadType.notify.rawValue,
            "id": id,
            "title": params.title,
            "body": params.body,
            "priority": params.priority?.rawValue ?? NodoAssistNotificationPriority.active.rawValue,
            "sentAtMs": self.nowMs(),
        ]
    ...
    static func encodeExecApprovalItem(_ item: NodoAssistWatchExecApprovalItem) -> [String: Any] {
        var payload: [String: Any] = [
            "id": item.id,
    ...
    static func encodeExecApprovalPromptPayload(
        _ message: NodoAssistWatchExecApprovalPromptMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": NodoAssistWatchPayloadType.execApprovalPrompt.rawValue,
    ...
    static func encodeExecApprovalResolvedPayload(
        _ message: NodoAssistWatchExecApprovalResolvedMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": NodoAssistWatchPayloadType.execApprovalResolved.rawValue,
    ...
    static func encodeExecApprovalExpiredPayload(
        _ message: NodoAssistWatchExecApprovalExpiredMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": NodoAssistWatchPayloadType.execApprovalExpired.rawValue,
            "approvalId": message.approvalId,
    ...
    static func encodeExecApprovalSnapshotPayload(
        _ message: NodoAssistWatchExecApprovalSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": NodoAssistWatchPayloadType.execApprovalSnapshot.rawValue,
    ...
    static func encodeAppSnapshotPayload(
        _ message: NodoAssistWatchAppSnapshotMessage) -> [String: Any]
    {
        var payload: [String: Any] = [
            "type": NodoAssistWatchPayloadType.appSnapshot.rawValue,
    ...
    static func encodeChatCompletionPayload(
        _ message: NodoAssistWatchChatCompletionMessage) -> [String: Any]
    {
        [
    ...
    {
        guard (payload["type"] as? String) == NodoAssistWatchPayloadType.reply.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == NodoAssistWatchPayloadType.execApprovalResolve.rawValue else {
            return nil
        }
        guard let approvalId = nonEmpty(payload["approvalId"] as? String),
              let rawDecision = nonEmpty(payload["decision"] as? String),
              let decision = NodoAssistWatchExecApprovalDecision(rawValue: rawDecision)
        else {
            return nil
        }
        let replyId = self.nonEmpty(payload["replyId"] as? String) ?? UUID().uuidString
    ...
    {
        guard (payload["type"] as? String) == NodoAssistWatchPayloadType.execApprovalSnapshotRequest.rawValue else {
            return nil
        }
    ...
    {
        guard (payload["type"] as? String) == NodoAssistWatchPayloadType.appSnapshotRequest.rawValue else {
            return nil
        }
        let requestId = self.nonEmpty(payload["requestId"] as? String) ?? UUID().uuidString
        let sentAtMs = (payload["sentAtMs"] as? Int) ?? (payload["sentAtMs"] as? NSNumber)?.intValue
    ...
    {
        guard (payload["type"] as? String) == NodoAssistWatchPayloadType.appCommand.rawValue else {
            return nil
        }
        guard let rawCommand = nonEmpty(payload["command"] as? String),
              let command = NodoAssistWatchAppCommand(rawValue: rawCommand)
        else {
            return nil
        }
