...
extension AgentProTab {
    var rosterHeader: some View {
        VStack(alignment: .leading, spacing: 10) {
            NodoAssistAdaptiveHeaderRow(
                ...
                titleFont: NodoAssistType.title2SemiBold,
    ...
    var gatewayToolbarButton: some View {
        if let openSettings {
            ...
            .tint(self.gatewayConnected ? NodoAssistBrand.ok : .secondary)
    ...
    func agentAvatar(_ agent: AgentSummary, state: AgentRosterState) -> some View {
        ZStack(alignment: .bottomTrailing) {
            Text(self.agentBadge(for: agent))
                .font(NodoAssistType.avatar(size: self.agentBadge(for: agent).count > 2 ? 14 : 18))
    ...
    func cronJobRow(_ job: CronJob) -> some View {
        HStack(spacing: 12) {
            ProIconBadge(
                ...
                color: job.enabled ? NodoAssistBrand.accent : .secondary)
            VStack(alignment: .leading, spacing: 3) {
                Text(job.name)
                    .font(NodoAssistType.subheadSemiBold)
                    .lineLimit(1)
            ...
            Text(self.cronJobState(job))
                .font(NodoAssistType.caption2SemiBold)
                .foregroundStyle(job.enabled ? NodoAssistBrand.accent : .secondary)
                .lineLimit(1)
        }
        .padding(.vertical, 10)
    ...
    var instancesColor: Color {
        ...
        return (overview?.presence.isEmpty == false) ? NodoAssistBrand.accent : .secondary
    }
    ...
    var cronColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return overview?.cronStatus?.enabled == true ? NodoAssistBrand.accent : .secondary
    }
    ...
    var dreamingColor: Color {
        guard self.gatewayConnected else { return .secondary }
        return overview?.dreaming?.enabled == true ? NodoAssistBrand.accent : .secondary
    }
