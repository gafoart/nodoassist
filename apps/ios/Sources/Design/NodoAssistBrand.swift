...
enum AppAppearancePreference: String, CaseIterable, Identifiable {
    ...
    static var launchArgumentPreference: AppAppearancePreference? {
        ...
        guard let flagIndex = arguments.firstIndex(of: "--nodoassist-appearance") else {
            return nil
        }
...
struct NodoAssistActivationGlyph: View {
    let size: CGFloat

    var body: some View {
        NodoAssistMascotView(floats: false)
            .frame(width: self.size, height: self.size)
            .shadow(
                color: NodoAssistBrand.activationGlow.opacity(0.18),
...
extension View {
    func nodoAssistSheetChrome() -> some View {
        self
            .tint(NodoAssistBrand.accent)
            .background {
                NodoAssistBrand.sheetBackground
                    .ignoresSafeArea()
            }
    }

    func nodoAssistCraftSurface(cornerRadius: CGFloat = 24, shadow: Bool = true) -> some View {
        self.modifier(NodoAssistCraftSurfaceModifier(cornerRadius: cornerRadius, shadow: shadow))
    }
}

private struct NodoAssistCraftSurfaceModifier: ViewModifier {
        ...
        content
            .background {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .fill(NodoAssistBrand.activationSurface)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .stroke(NodoAssistBrand.activationHairline, lineWidth: 0.5)
            }
    }
}

struct NodoAssistPrimaryActionButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(self.isEnabled ? NodoAssistBrand.activationPrimaryActionText : Color.secondary)
            .tint(self.isEnabled ? NodoAssistBrand.activationPrimaryActionText : Color.secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .fill(self.isEnabled ? Self.primaryFill : NodoAssistBrand.activationDisabledGradient)
                    .shadow(
                        color: self.isEnabled ? NodoAssistBrand.activationPrimaryAction.opacity(0.08) : .clear,
                        radius: 1,
                        x: 0,
                        y: 1)
                    .shadow(
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(
                        self.isEnabled
                            ? NodoAssistBrand.activationPrimaryAction.opacity(0.72)
                            : NodoAssistBrand.activationNeutralStroke,
                        lineWidth: 0.75)
            }
...
struct NodoAssistSecondaryActionButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            .font(NodoAssistType.subheadSemiBold)
            .foregroundStyle(self.isEnabled ? NodoAssistBrand.activationPrimaryAction : .secondary)
            .frame(maxWidth: .infinity)
            .frame(height: self.height)
            .background {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: self.resolvedCornerRadius, style: .continuous)
                    .stroke(NodoAssistBrand.activationHairline, lineWidth: 0.75)
            }
            .scaleEffect(configuration.isPressed && self.isEnabled ? 0.98 : 1)
            .animation(.smooth(duration: 0.14), value: configuration.isPressed)
    }
    ...
    private static var secondaryFill: LinearGradient {
        LinearGradient(
            colors: [
                NodoAssistBrand.activationSecondaryActionTop,
                NodoAssistBrand.activationSecondaryActionBottom,
            ],
            startPoint: .top,
            endPoint: .bottom)
    }
...
struct NodoAssistCloseButtonStyle: ButtonStyle {
    @Environment(\.isEnabled) private var isEnabled
        ...
        configuration.label
            ...
            .background {
                Capsule(style: .continuous)
                    .fill(NodoAssistBrand.activationNeutralGradient)
                    .shadow(
            ...
            .overlay {
                Capsule(style: .continuous)
                    .stroke(NodoAssistBrand.activationNeutralStroke, lineWidth: 0.6)
            }
