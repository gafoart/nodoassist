...
struct CommandCenterTab: View {
    ...
    @State private var defaultChatSessionEntry: NodoAssistChatSessionEntry?
    @State private var recentChatSessions: [NodoAssistChatSessionEntry] = []
    var ownsNavigationStack: Bool = true
    var usesNativeNavigationChrome: Bool = false
    var headerTitle: String = "NodoAssist"
    var headerLeadingAction: NodoAssistSidebarHeaderAction?
    ...
    private var header: some View {
            ...
            subtitleLineLimit: 1)
        {
            ...
            {
                NodoAssistProMark(size: 28, shadowRadius: 5)
            }
        } accessory: {
            Button(action: self.openSettings) {
                Image(systemName: "gearshape.fill")
                    .font(NodoAssistType.subheadSemiBold)
                    .frame(width: NodoAssistProMetric.compactControlSize, height: NodoAssistProMetric.compactControlSize)
            }
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 0) {
                    ...
                    self.gatewayFact(
                        ...
                        color: NodoAssistBrand.accentHotForeground)
                }
                .padding(.vertical, 7)
            }
        }
    ...
    private var defaultChatWorkItem: WorkItem {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isOpen ? NodoAssistBrand.accent : NodoAssistBrand.ok,
            progress: nil,
            route: .chat(nil),
    ...
    private var recentSessionPreviewSessions: [NodoAssistChatSessionEntry] {
    ...
    private func open(_ session: NodoAssistChatSessionEntry) {
    ...
    private func deleteSession(_ session: NodoAssistChatSessionEntry) {
    ...
    private func archiveSession(_ session: NodoAssistChatSessionEntry) {
        self.performSessionMutation(resetActiveSessionKey: session.key) { transport in
            try await transport.patchSession(
                key: session.key,
                label: nil,
    ...
    private func forkSession(_ session: NodoAssistChatSessionEntry) {
        Task {
            do {
    ...
    private func performSessionMutation(
        ...
        _ operation: @escaping (any NodoAssistChatTransport) async throws -> Void)
    {
        Task {
            do {
    ...
    private func applySessions(_ sessions: [NodoAssistChatSessionEntry]) {
    ...
    private static func sessionChoices(
        _ sessions: [NodoAssistChatSessionEntry],
        defaultSessionKey: String) -> [NodoAssistChatSessionEntry]
    {
        sessions.filter {
    ...
    static func sessionWorkItem(
        for session: NodoAssistChatSessionEntry,
        currentSessionKey: String) -> WorkItem
    {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isCurrent ? NodoAssistBrand.accent : NodoAssistBrand.ok,
            progress: nil,
    ...
    fileprivate static func sessionTitle(_ session: NodoAssistChatSessionEntry) -> String {
    ...
    fileprivate static func sessionDetail(_ session: NodoAssistChatSessionEntry) -> String {
...
struct CommandSessionsScreen: View {
    ...
    @State private var sessions: [NodoAssistChatSessionEntry] = []
            ...
            VStack(spacing: 0) {
                HStack(spacing: 8) {
                    Text(self.showArchived ? "Archived sessions" : "Recent sessions")
                        .font(NodoAssistType.subheadBold)
                    Spacer(minLength: 8)
                    if self.isLoading {
                        ProgressView()
                            .controlSize(.small)
    ...
    private var visibleSessions: [NodoAssistChatSessionEntry] {
        self.sessions
    ...
    private func sessionRow(_ session: NodoAssistChatSessionEntry) -> some View {
    ...
    private func toggleArchived(_ session: NodoAssistChatSessionEntry) {
