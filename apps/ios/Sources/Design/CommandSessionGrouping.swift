...
struct CommandSessionSection: Identifiable {
    ...
    let id: ID
    let title: String
    let entries: [NodoAssistChatSessionEntry]
    let showsHeader: Bool
}

enum CommandSessionGrouping {
    static func sections(
        from entries: [NodoAssistChatSessionEntry],
    ...
    static func previewOrder(_ entries: [NodoAssistChatSessionEntry]) -> [NodoAssistChatSessionEntry] {
    ...
    static func previewSelection(
        _ entries: [NodoAssistChatSessionEntry],
        currentKey: String,
        limit: Int = 3) -> [NodoAssistChatSessionEntry]
    {
    ...
    static func members(
        of group: String,
        in lists: [[NodoAssistChatSessionEntry]]) -> [NodoAssistChatSessionEntry]
    {
    ...
    static func activityTimestamp(_ entry: NodoAssistChatSessionEntry) -> Double {
        entry.lastActivityAt ?? entry.updatedAt ?? 0
    }

    private static func sortedByActivity(
        _ entries: [NodoAssistChatSessionEntry]) -> [NodoAssistChatSessionEntry]
    {
        entries.sorted { lhs, rhs in
            let left = self.activityTimestamp(lhs)
            let right = self.activityTimestamp(rhs)
