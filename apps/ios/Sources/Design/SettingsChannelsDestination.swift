...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return NodoAssistBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? NodoAssistBrand.ok : NodoAssistBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return NodoAssistBrand.ok }
        if self.lastError != nil { return NodoAssistBrand.warn }
        return self.configured ? NodoAssistBrand.accent : .secondary
    }
