...
enum NodoAssistRadius {
    static let xs: CGFloat = 8
    static let sm: CGFloat = 10
    static let md: CGFloat = 12
}

struct NodoAssistProBackground: View {
    var body: some View {
...
struct ProSectionHeader: View {
    ...
    var body: some View {
        HStack {
            Text(self.title)
                .font(NodoAssistType.footnoteMedium)
...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = NodoAssistProMetric.cardRadius
...
private struct NodoAssistGlassButtonModifier: ViewModifier {
    let prominent: Bool
    let tint: Color?
        ...
        if #available(iOS 26.0, *) {
            if self.prominent {
                content
                    ...
                    .tint(self.tint ?? NodoAssistBrand.accent)
            } else {
                content
                    .font(NodoAssistType.subheadSemiBold)
                    .buttonStyle(.glass)
...
private struct NodoAssistTabBarBehaviorModifier: ViewModifier {
    func body(content: Content) -> some View {
        if #available(iOS 26.0, *) {
...
private struct NodoAssistGlassSurfaceModifier: ViewModifier {
    let radius: CGFloat
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = NodoAssistProMetric.cardRadius,
    ...
    func nodoAssistGlassButton(prominent: Bool = false, tint: Color? = nil) -> some View {
        modifier(NodoAssistGlassButtonModifier(prominent: prominent, tint: tint))
    }

    func nodoAssistTabBarBehavior() -> some View {
        modifier(NodoAssistTabBarBehaviorModifier())
    }

    func nodoAssistGlassSurface(radius: CGFloat = NodoAssistProMetric.controlRadius) -> some View {
        modifier(NodoAssistGlassSurfaceModifier(radius: radius))
    }
}
...
struct ProIconBadge: View {
    ...
    var body: some View {
        Image(systemName: self.systemName)
            ...
            .background {
                RoundedRectangle(cornerRadius: NodoAssistRadius.xs, style: .continuous)
                    .fill(self.color.opacity(0.12))
            }
    }
}

struct NodoAssistSidebarHeaderAction {
    let systemName: String
...
struct NodoAssistSidebarRevealButton: View {
    let headerAction: NodoAssistSidebarHeaderAction

    init(action: NodoAssistSidebarHeaderAction) {
    ...
    var body: some View {
        let button = Button(action: headerAction.action) {
            Image(systemName: self.headerAction.systemName)
                .font(NodoAssistType.subheadSemiBold)
                .frame(
                    width: NodoAssistProMetric.compactControlSize,
                    height: NodoAssistProMetric.compactControlSize)
                .contentShape(Rectangle())
        }
        .buttonBorderShape(.circle)
        .nodoAssistGlassButton(tint: NodoAssistBrand.accent)
...
struct NodoAssistSidebarHeaderLeadingSlot: View {
    let action: NodoAssistSidebarHeaderAction

    var body: some View {
        NodoAssistSidebarRevealButton(action: self.action)
            .frame(width: 44, height: 44, alignment: .center)
    }
}

struct NodoAssistGlassControlGroup<Content: View>: View {
...
enum NodoAssistNoticeDetail {
    case accent(String)
    case requestID(String)
}

struct NodoAssistNoticeBanner: View {
    let icon: String
    let title: String
    let message: String
    let ownerLabel: String
    let tint: Color
    var detail: NodoAssistNoticeDetail?
                    ...
                    NodoAssistGlassControlGroup {
                        HStack(spacing: 10) {
                            if let primaryActionTitle, let onPrimaryAction {
                                ...
                                .font(NodoAssistType.captionSemiBold)
                                .nodoAssistGlassButton(prominent: true)
                                .controlSize(.small)
                            }
        ...
        if let detail {
            ...
            case let .requestID(value):
                Text("Request ID: \(value)")
                    .font(NodoAssistType.monoSmallMedium)
                    .foregroundStyle(.secondary)
...
struct NodoAssistAdaptiveHeaderRow<Leading: View, Accessory: View>: View {
    let title: String
    let subtitle: String?
    var titleFont: Font = NodoAssistType.title3SemiBold
    var subtitleFont: Font = NodoAssistType.subhead
    ...
    init(
        title: String,
        subtitle: String? = nil,
        titleFont: Font = NodoAssistType.title3SemiBold,
        subtitleFont: Font = NodoAssistType.subhead,
...
struct NodoAssistToggleIndicator: View {
    let isOn: Bool

    var body: some View {
        Capsule()
            .fill(self.isOn ? NodoAssistBrand.accent : Color.secondary.opacity(0.35))
...
enum NodoAssistStatusTone {
    ...
    var color: Color {
        switch self {
        case .ok: NodoAssistBrand.ok
        case .warn: NodoAssistBrand.warn
        case .danger: NodoAssistBrand.danger
        case .info: NodoAssistBrand.info
        case .accent: NodoAssistBrand.accent
        case .teal: NodoAssistBrand.teal
        case .muted: NodoAssistBrand.textSecondary
        }
    }
}

struct NodoAssistStatusBadge: View {
    @Environment(\.colorScheme) private var colorScheme
    let label: String
    let tone: NodoAssistStatusTone

    var body: some View {
        HStack(spacing: NodoAssistSpacing.space1 + 2) {
            Circle()
        ...
        .padding(.horizontal, NodoAssistSpacing.space2)
        .padding(.vertical, 5)
        .background {
            Capsule()
...
struct NodoAssistProMark: View {
    var size: CGFloat = 42
    var shadowRadius: CGFloat = 10

    var body: some View {
        NodoAssistMascotView()
            .frame(width: self.size, height: self.size)
            .shadow(color: NodoAssistBrand.accent.opacity(0.18), radius: self.shadowRadius, y: self.shadowRadius / 3)
            .accessibilityLabel("NodoAssist")
    }
}

struct ProProgressBar: View {
    let progress: Double
    var color: Color = NodoAssistBrand.accentHot
...
struct NodoAssistGatewayCompactPill: View {
    @Environment(NodeAppModel.self) private var appModel

    var body: some View {
        NodoAssistStatusBadge(label: self.title, tone: self.tone)
    ...
    private var tone: NodoAssistStatusTone {
        switch GatewayStatusBuilder.build(appModel: self.appModel) {
        case .connected:
            .ok
        case .connecting:
...
struct ProMetricTile: View {
    ...
    var body: some View {
        ...
        .padding(11)
        .frame(maxWidth: .infinity, alignment: .leading)
        .proInsetSurface(tint: self.color, radius: NodoAssistProMetric.controlRadius)
    }
}
...
struct ProPanelHeader: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            if let value {
                Text(value)
                    .font(NodoAssistType.caption2Bold)
                    .foregroundStyle(.secondary)
            }
            Spacer(minLength: 8)
            self.actionControl
