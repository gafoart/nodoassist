            ...
            Text(IPadWorkboardDefaults.label(for: status))
                ...
                .overlay {
                    Capsule()
                        .strokeBorder(
                            self.selectedStatus == status
                                ? NodoAssistBrand.accent.opacity(0.42)
                                : Color.primary.opacity(0.08),
                            lineWidth: 1)
                }
        }
        .buttonStyle(.plain)
        .foregroundStyle(self.selectedStatus == status ? NodoAssistBrand.accent : .primary)
...
private struct IPadWorkboardKanbanCard: View {
    ...
    private var color: Color {
        switch self.card.status {
        case "running": NodoAssistBrand.ok
        case "review": NodoAssistBrand.accentForeground
        case "blocked": NodoAssistBrand.warn
        case "done": .secondary
        default: NodoAssistBrand.accentHotForeground
        }
    }
    ...
    var body: some View {
        ...
        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
            if let nextStatus {
                ...
                .tint(NodoAssistBrand.accentHot)
            }
            Button(action: self.archive) {
                Text(self.card.metadata?.archivedAt == nil ? "Archive" : "Unarchive")
                    .font(NodoAssistType.subheadSemiBold)
