...
final class RemindersService: RemindersServicing {
    ...
    func list(params: NodoAssistRemindersListParams) async throws -> NodoAssistRemindersListPayload {
        ...
        let payload: [NodoAssistReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return NodoAssistReminderPayload(
        ...
        return NodoAssistRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: NodoAssistRemindersAddParams) async throws -> NodoAssistRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = NodoAssistReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return NodoAssistRemindersAddPayload(reminder: payload)
    }
