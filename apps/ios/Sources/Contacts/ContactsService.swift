...
final class ContactsService: ContactsServicing {
    ...
    func search(params: NodoAssistContactsSearchParams) async throws -> NodoAssistContactsSearchPayload {
        ...
        return NodoAssistContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: NodoAssistContactsAddParams) async throws -> NodoAssistContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return NodoAssistContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return NodoAssistContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> NodoAssistContactPayload {
        NodoAssistContactPayload(
