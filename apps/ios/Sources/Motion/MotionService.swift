...
final class MotionService: MotionServicing {
    func activities(params: NodoAssistMotionActivityParams) async throws -> NodoAssistMotionActivityPayload {
        ...
        let mapped: [NodoAssistMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return NodoAssistMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = NodoAssistPedometerPayload(
