...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> NodoAssistDeviceStatusPayload {
        ...
        return NodoAssistDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> NodoAssistDeviceInfoPayload {
        ...
        return NodoAssistDeviceInfoPayload(
    ...
    private func batteryStatus() -> NodoAssistBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: NodoAssistBatteryState = switch device.batteryState {
        ...
        return NodoAssistBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> NodoAssistThermalStatusPayload {
        let state: NodoAssistThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return NodoAssistThermalStatusPayload(state: state)
    }

    private func storageStatus() -> NodoAssistStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return NodoAssistStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
