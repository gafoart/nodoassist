...
@MainActor
enum NodoAssistAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class NodoAssistAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.nodoassistfoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.nodoassistfoundation.app", category: "BackgroundWake")
    ...
    private static var appBundleIdentifier: String {
        guard let bundleId = Bundle.main.bundleIdentifier?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            return "ai.nodoassistfoundation.app"
        }
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? NodoAssistAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "nodoassist.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "nodoassist.watch.promptId"
    static let sessionKeyKey = "nodoassist.watch.sessionKey"
    static let gatewayStableIDKey = "nodoassist.watch.gatewayStableID"
    static let actionPrimaryIDKey = "nodoassist.watch.action.primary.id"
    static let actionPrimaryLabelKey = "nodoassist.watch.action.primary.label"
    static let actionSecondaryIDKey = "nodoassist.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "nodoassist.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "nodoassist.watch.action.primary"
    static let actionSecondaryIdentifier = "nodoassist.watch.action.secondary"
    static let actionIdentifierPrefix = "nodoassist.watch.action."
    static let actionIDKeyPrefix = "nodoassist.watch.action.id."
    static let actionLabelKeyPrefix = "nodoassist.watch.action.label."
    static let categoryPrefix = "nodoassist.watch.prompt.category."
        ...
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> NodoAssistWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return NodoAssistWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "NodoAssist" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [NodoAssistWatchAction]) -> [UNNotificationAction] {
...
@main
struct NodoAssistApp: App {
    @State private var appearanceModel: AppAppearanceModel
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(NodoAssistAppDelegate.self) private var appDelegate
    @Environment(\.scenePhase) private var scenePhase

    init() {
        Self.installUncaughtExceptionLogger()
        GatewaySettingsStore.bootstrapPersistence()
        NodoAssistType.installUIKitAppearance()
        let appModel = NodeAppModel()
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--nodoassist-reset-onboarding") {
        ...
        #endif
        NodoAssistAppModelRegistry.appModel = appModel
        ...
        WindowGroup {
            RootTabs()
                ...
                .onReceive(
                    ...
                    perform: { _ in
                        NodoAssistType.refreshUIKitAppearance(in: Self.connectedWindows())
                    })
                .onOpenURL { url in
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--nodoassist-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--nodoassist-screenshot-notification-guidance")
        #else
        false
        #endif
    }
    ...
    private func applyWindowTint() {
        for window in Self.connectedWindows() {
            window.tintColor = NodoAssistBrand.uiAccent
        }
    }
...
extension NodoAssistApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("NodoAssist: installing uncaught exception handler")
