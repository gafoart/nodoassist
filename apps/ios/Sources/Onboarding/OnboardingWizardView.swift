...
struct OnboardingWizardView: View {
    ...
    private var modeStep: some View {
        ...
        Section {
            ...
            .buttonStyle(NodoAssistPrimaryActionButtonStyle(height: 48, cornerRadius: 16))
        ...
        Group {
            Section {
                ...
                if let problem = self.currentProblem {
                    ...
                    Text("NodoAssist is checking gateway and node access.")
                        .font(NodoAssistType.footnote)
                        .foregroundStyle(.secondary)
                }
            } header: {
            ...
            if self.issue.needsPairing {
                Section {
                    ...
                    let requestLine: String = {
                        ...
                        return "Request ID: check `nodoassist devices list`."
                    }()
                    let commandLine = self.currentProblem?.actionCommand ?? "nodoassist devices approve <requestId>"
                    Text(
...
extension OnboardingWizardView {
    ...
    private func initializeState() {
        if self.manualHost.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
               ...
               let port = active.port
            {
                ...
                self.manualHost = "nodoassist.local"
        ...
        if self.selectedMode == .developerLocal, self.manualHost == "nodoassist.local" {
    ...
    private func applyModeDefaults(_ mode: OnboardingConnectionMode) {
        ...
        let hostIsDefaultLike = host.isEmpty || host == "nodoassist.local" || host == "localhost"

        switch mode {
        case .homeNetwork:
            if hostIsDefaultLike { self.manualHost = "nodoassist.local" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .remoteDomain:
            if host == "nodoassist.local" || host == "localhost" { self.manualHost = "" }
            self.manualTLS = true
            if self.manualPort <= 0 || self.manualPort > 65535 { self.manualPort = 18789 }
        case .developerLocal:
