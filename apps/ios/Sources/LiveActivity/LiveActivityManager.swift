...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.nodoassistfoundation.app", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<NodoAssistActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = NodoAssistActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<NodoAssistActivityAttributes>.activities
    ...
    private func updateCurrent(state: NodoAssistActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<NodoAssistActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> NodoAssistActivityAttributes.ContentState {
        NodoAssistActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> NodoAssistActivityAttributes.ContentState {
        NodoAssistActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func disconnectedState() -> NodoAssistActivityAttributes.ContentState {
        NodoAssistActivityAttributes.ContentState(
