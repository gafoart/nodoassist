...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: NodoAssistWatchNotifyParams) -> NodoAssistWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [NodoAssistWatchAction]?,
        kind: String?,
        promptId: String?) -> [NodoAssistWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> NodoAssistWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return NodoAssistWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                NodoAssistWatchAction(id: "approve", label: "Approve"),
                NodoAssistWatchAction(id: "decline", label: "Decline", style: "destructive"),
                NodoAssistWatchAction(id: "open_phone", label: "Open iPhone"),
                NodoAssistWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            NodoAssistWatchAction(id: "done", label: "Done"),
            NodoAssistWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            NodoAssistWatchAction(id: "open_phone", label: "Open iPhone"),
            NodoAssistWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: NodoAssistWatchRisk?,
        priority: NodoAssistNotificationPriority?) -> NodoAssistWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: NodoAssistNotificationPriority?,
        risk: NodoAssistWatchRisk?) -> NodoAssistNotificationPriority?
    {
