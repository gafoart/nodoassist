    ...
    var body: some View {
        VStack(alignment: .leading, spacing: self.style == .sheet ? 16 : 12) {
            HStack(alignment: .top, spacing: 12) {
                Image(systemName: self.iconSystemName)
                    ...
                    .foregroundStyle(self.requestIsPending ? NodoAssistBrand.warn : NodoAssistBrand.accent)
                    .frame(width: 28, height: 28)

                VStack(alignment: .leading, spacing: 6) {
                    Text(self.titleText)
                        .font(self.style == .sheet ? NodoAssistType.title3SemiBold : NodoAssistType.headline)
        ...
        .background {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: NodoAssistRadius.md, style: .continuous)
                    .fill(.thinMaterial)
            }
        }
        .overlay {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: NodoAssistRadius.md, style: .continuous)
                    .stroke(NodoAssistBrand.accent.opacity(0.20), lineWidth: 1)
            }
        }
