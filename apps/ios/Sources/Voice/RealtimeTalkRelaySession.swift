...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "nodoassist_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.nodoassistfoundation.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "NodoAssist tool call failed"]
                : ["text": completion.text ?? "NodoAssist finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: NodoAssistChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
