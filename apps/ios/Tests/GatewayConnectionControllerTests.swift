...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func `current caps reflect toggles`() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": NodoAssistLocationMode.always.rawValue,
            ...
            #expect(caps.contains(NodoAssistCapability.canvas.rawValue))
            #expect(caps.contains(NodoAssistCapability.screen.rawValue))
            #expect(caps.contains(NodoAssistCapability.camera.rawValue))
            #expect(caps.contains(NodoAssistCapability.location.rawValue))
            #expect(caps.contains(NodoAssistCapability.voiceWake.rawValue))
            #expect(caps.contains(NodoAssistCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func `current commands include location when enabled`() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": NodoAssistLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(NodoAssistLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func `current commands exclude dangerous system exec commands`() {
        withUserDefaults([
            ...
            #expect(commands.contains(NodoAssistSystemCommand.notify.rawValue))
            #expect(!commands.contains(NodoAssistSystemCommand.run.rawValue))
            #expect(!commands.contains(NodoAssistSystemCommand.which.rawValue))
            #expect(!commands.contains(NodoAssistSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(NodoAssistSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func `chat cache remains isolated when active gateway switches`() async throws {
        ...
        _ = GatewaySettingsStore.upsertGatewayRegistryEntry(.init(
            ...
            lastConnectedAtMs: nil))
        let appModel = NodeAppModel()
        let session = NodoAssistChatSessionEntry(
            key: "agent:main:a",
            kind: nil,
