...
struct NodoAssistTypographyTests {
    @Test func `session controls use branded typography`() throws {
        ...
        #expect(support.contains(".font(NodoAssistType.subhead)"))
        #expect(support.contains(".font(NodoAssistType.subheadSemiBold)"))
        #expect(commandCenter.contains("Toggle(isOn: self.$showArchived)"))
        #expect(commandCenter.contains("Text(\"Show Archived\")"))
        #expect(commandCenter.contains(".font(NodoAssistType.captionMedium)"))
    }

    @Test func `bundled fonts load from app bundle`() {
        for name in NodoAssistType.registeredPostScriptNames {
    ...
    @Test func `app extensions register bundled branded fonts`() throws {
        ...
        for targetName in ["NodoAssistActivityWidget", "NodoAssistWatchApp"] {
            let target = try Self.extract(
                project,
                from: "  \(targetName):",
                to: targetName == "NodoAssistActivityWidget" ? "  NodoAssistWatchApp:" : "  NodoAssistTests:")
    ...
    @Test func `extension text surfaces use branded typography helpers`() throws {
        ...
        #expect(activitySource.contains("NodoAssistActivityType.subheadSemiBold"))
        #expect(activitySource.contains("NodoAssistActivityType.subheadBold"))
        #expect(activitySource.contains("NodoAssistActivityType.caption"))
    ...
    @Test func `UIKit typography refreshes when Dynamic Type changes`() throws {
        ...
        let typographySource = try String(
            contentsOf: Self.sourceURL("Design/NodoAssistTypography.swift"),
            encoding: .utf8)

        #expect(appSource.contains("UIContentSizeCategory.didChangeNotification"))
        #expect(appSource.contains("NodoAssistType.refreshUIKitAppearance(in: Self.connectedWindows())"))
    ...
    @Test func `listed iOS app surfaces enforce branded control typography`() throws {
        let proComponents = try String(
            contentsOf: Self.sourceURL("Design/NodoAssistProComponents.swift"),
            encoding: .utf8)
        ...
        let channels = try String(
            ...
            encoding: .utf8)
        let docs = try String(contentsOf: Self.sourceURL("Design/NodoAssistDocsScreen.swift"), encoding: .utf8)
        ...
        let chatTypography = try String(
            contentsOf: Self.iosRootURL()
                ...
                .appendingPathComponent("shared/NodoAssistKit/Sources/NodoAssistChatUI/ChatTypography.swift"),
            encoding: .utf8)
        let chatMessageViews = try String(
            contentsOf: Self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/NodoAssistKit/Sources/NodoAssistChatUI/ChatMessageViews.swift"),
            encoding: .utf8)

        #expect(proComponents.contains(".font(NodoAssistType.subheadSemiBold)"))
        ...
        #expect(chatTab.contains(".font(NodoAssistType.body)"))
        ...
        #expect(quickSetup.contains(".font(NodoAssistType.subheadSemiBold)"))
        ...
        #expect(gatewayProblem.contains(".font(NodoAssistType.subheadSemiBold)"))
        ...
        #expect(onboardingSteps.contains(".font(NodoAssistType.subheadSemiBold)"))
        ...
        #expect(onboardingWizard.contains(".font(NodoAssistType.subheadSemiBold)"))
        ...
        let onboardingSecureOption = try Self.extract(
            ...
            to: ".tag(true)")
        #expect(onboardingUnencryptedOption.contains(".font(NodoAssistType.captionSemiBold)"))
        #expect(onboardingSecureOption.contains(".font(NodoAssistType.captionSemiBold)"))

        #expect(settingsSections.contains(".font(NodoAssistType.body)"))
        #expect(settingsSections.contains("self.settingsToggle(\"Show Talk Control\", isOn: self.$talkButtonEnabled)"))
        #expect(settingsSections.contains("NodoAssistToggleIndicator(isOn: isOn.wrappedValue)"))
        #expect(settingsSections.contains("TextField(\"Default Share Instruction\""))
        #expect(settingsSections.contains(".font(NodoAssistType.subhead)"))
        ...
        #expect(settingsSections.contains(".font(NodoAssistType.subheadSemiBold)"))
        ...
        let settingsSecureOption = try Self.extract(
            ...
            from: "Text(\"Secure (TLS)\")",
            to: ".tag(true)")
        #expect(settingsUnencryptedOption.contains(".font(NodoAssistType.captionSemiBold)"))
        #expect(settingsSecureOption.contains(".font(NodoAssistType.captionSemiBold)"))
        ...
        #expect(privacyAccess.contains(".font(NodoAssistType.footnoteSemiBold)"))
        ...
        for source in [agentDestinations, dreaming, instances, channels, docs] {
            #expect(source.contains(".font(NodoAssistType.body)"))
        }

        #expect(chatMessageViews.contains("font: NodoAssistChatTypography.body"))
        #expect(chatMessageViews.contains("NodoAssistChatTypography.callout.italic()"))
    ...
    private static func activityWidgetSourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/NodoAssistLiveActivity.swift")
    }

    private static func activityWidgetTypographySourceURL() -> URL {
        self.iosRootURL().appendingPathComponent("ActivityWidget/NodoAssistActivityTypography.swift")
    }
    ...
    private static func appSourceURL() -> URL {
        self.sourceURL("NodoAssistApp.swift")
    }
    ...
    private static func swiftSourcesForTypographyAudit() throws -> [URL] {
        let roots = [
            ...
            self.iosRootURL()
                .deletingLastPathComponent()
                .appendingPathComponent("shared/NodoAssistKit/Sources/NodoAssistChatUI"),
        ]
    ...
    private static func unbrandedTextCallOffenders() throws -> [String] {
        let fontTokens = ["NodoAssistType", "NodoAssistChatTypography"]
    ...
    private static func hasAllowedBrandedFontParameter(_ window: String, in url: URL) -> Bool {
        ...
        case "apps/ios/Sources/Design/NodoAssistProComponents.swift":
            window.contains(".font(self.titleFont)") || window.contains(".font(self.subtitleFont)")
        case "apps/shared/NodoAssistKit/Sources/NodoAssistChatUI/ChatMarkdownRenderer.swift":
