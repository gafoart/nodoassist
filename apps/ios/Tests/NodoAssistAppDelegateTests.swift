...
@Suite(.serialized) struct NodoAssistAppDelegateTests {
    @Test @MainActor func `resolves registry model before view task assigns delegate model`() {
        let registryModel = NodeAppModel()
        NodoAssistAppModelRegistry.appModel = registryModel
        defer { NodoAssistAppModelRegistry.appModel = nil }

        let delegate = NodoAssistAppDelegate()
    ...
    @Test @MainActor func `derives background refresh task identifier from app bundle identifier`() {
        let delegate = NodoAssistAppDelegate()
        let bundleIdentifier = Bundle.main.bundleIdentifier ?? "ai.nodoassistfoundation.app.tests"
    ...
    @Test @MainActor func `stages a gateway URL when the model is ready`() async throws {
        NodoAssistAppModelRegistry.appModel = nil
        defer { NodoAssistAppModelRegistry.appModel = nil }
        let model = NodeAppModel()
        let delegate = NodoAssistAppDelegate()
        delegate.appModel = model
        let url = try #require(URL(
            string: "nodoassist://gateway?host=gateway.example.com&port=443&tls=1&token=tok"))
