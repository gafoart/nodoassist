...
@Suite struct ShareToAgentDeepLinkTests {
    @Test func appGroupIdentifierUsesCanonicalNodoAssistGroup() {
        #expect(NodoAssistAppGroup.canonicalIdentifier == "group.ai.nodoassistfoundation.app.shared")
    }
    ...
    @Test func buildURLEncodesAgentRoute() {
        ...
        guard case let .agent(agent)? = parsed else {
            Issue.record("Expected nodoassist://agent deep link")
            return
        }
