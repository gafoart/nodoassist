...
struct NodoAssistBrandTests {
    @Test func `brand colors meet text contrast in both appearances`() {
        let foregroundColors = [
            ("accent", NodoAssistBrand.uiAccentForeground),
            ("accentHot", NodoAssistBrand.uiAccentHotForeground),
            ("ok", NodoAssistBrand.uiOK),
            ("warn", NodoAssistBrand.uiWarn),
            ("danger", NodoAssistBrand.uiDanger),
            ("info", NodoAssistBrand.uiInfo),
        ]
        ...
        for style in [UIUserInterfaceStyle.light, .dark] {
            ...
            let pillBackground = Self.composite(
                NodoAssistBrand.uiAccentForeground,
                alpha: style == .dark ? 0.12 : 0.08,
                over: .secondarySystemGroupedBackground,
                traits: traits)
            #expect(Self.contrastRatio(NodoAssistBrand.uiAccentForeground, pillBackground, traits: traits) >= 4.5)
            #expect(Self.contrastRatio(NodoAssistBrand.uiAccent, .white, traits: traits) >= 4.5)
        }
    }
