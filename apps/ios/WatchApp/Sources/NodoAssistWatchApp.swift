...
@main
struct NodoAssistWatchApp: App {
    @Environment(\.scenePhase) private var scenePhase
    @State private var inboxStore = WatchInboxStore(
        requestNotificationAuthorization: !NodoAssistWatchApp.isScreenshotMode)
    @State private var receiver: WatchConnectivityReceiver?
    @State private var execApprovalRefreshTask: Task<Void, Never>?

    private static let screenshotModeDefaultsKey = "nodoassist.watch.screenshotMode"
    private static let isScreenshotMode = ProcessInfo.processInfo.arguments.contains(
        "--nodoassist-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["NODOASSIST_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: NodoAssistWatchApp.screenshotModeDefaultsKey)
    ...
    var body: some Scene {
        WindowGroup {
            WatchInboxView(
                ...
                .task {
                    if NodoAssistWatchApp.isScreenshotMode {
