...
Use these steps to exercise the live NodoAssist iOS App Review Gateway.

## Demo Account / Setup

Use the NodoAssist iOS app with the live review Gateway setup code included in
...
9. On `Connected`, tap `Open NodoAssist`.
10. Confirm the `Control` screen shows `Gateway Online`.
11. Tap `Settings`.
12. Tap `Approvals`.
13. Tap `Open Notifications`.
14. Tap `Enable Notifications`.
15. On `Enable NodoAssist Hosted Push Relay?`, tap `Continue`.
16. If iOS asks whether NodoAssist may send notifications, tap `Allow`.
...
command `printf 'NodoAssist App Review approval demo complete\n'`. Tap
...
9. While Talk is active, send NodoAssist to the background by returning to the
   Home Screen or locking the iPhone. Do not force quit the app.
10. Continue speaking then wait for assistant audio reply.

Expected result: realtime Talk audio continues while NodoAssist is backgrounded.
Reopen NodoAssist, confirm Talk is still active, then tap `Stop Talk`.
...
1. Tap `Settings`.
2. Tap `Reconnect`.
3. Immediately send NodoAssist to the background by returning to the Home Screen
   or locking the iPhone.
4. Watch the Lock Screen or Dynamic Island while the Gateway reconnects.

Expected result: while reconnecting, iOS can show an `NodoAssist` Live Activity
with connection status such as `Connecting...` or `Reconnecting...`. On a fast
network this status may be brief because NodoAssist ends the Live Activity after
...
4. Immediately send NodoAssist to the background and lock the iPhone. Do not
   force quit the app.

Expected result: the iPhone Lock Screen receives a visible `NodoAssist`
notification with this body:

```text
NodoAssist App Review push notification demo
```

Tap the notification and unlock the iPhone if prompted. If NodoAssist opens on
...
3. Immediately send NodoAssist to the background and lock the iPhone. Do not
   force quit the app.
4. Wait for the `NodoAssist` notification on the Lock Screen. It normally appears
   about 10 seconds after the message is sent.
5. Tap the notification and unlock the iPhone if prompted. If NodoAssist opens on
   `Control`, tap `Chat`.
...
4. Tap `Share`.
5. Tap `NodoAssist`.
6. Confirm the NodoAssist share extension appears and shows
   `Edit text, then tap Send.` and `Send to NodoAssist`.
7. Tap `Send to NodoAssist`.

Expected result: the NodoAssist share extension sends the shared Safari page to
the live review Gateway and shows `Sent to NodoAssist.` Returning to NodoAssist
