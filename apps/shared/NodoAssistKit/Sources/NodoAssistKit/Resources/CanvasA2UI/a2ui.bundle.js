...
var NodoAssistA2UIHost = class extends i$7 {
	static properties = {
		surfaces: { state: true },
    ...
    :host {
      ...
      position: relative;
      box-sizing: border-box;
      padding: var(--nodoassist-a2ui-inset-top, 0px) var(--nodoassist-a2ui-inset-right, 0px)
        var(--nodoassist-a2ui-inset-bottom, 0px) var(--nodoassist-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--nodoassist-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--nodoassist-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--nodoassist-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--nodoassist-a2ui-empty-top, var(--nodoassist-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["nodoassist:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["nodoassist:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		globalThis["__nodoassistLastA2UIAction"] = userAction;
		const handler = globalThis.webkit?.messageHandlers?.nodoassistCanvasA2UIAction ?? globalThis.nodoassistCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.nodoassistCanvasA2UIAction) postNativeMessage(handler, JSON.stringify({ userAction }));
...
if (!customElements.get("nodoassist-a2ui-host")) customElements.define("nodoassist-a2ui-host", NodoAssistA2UIHost);
