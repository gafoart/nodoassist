...
enum DeviceIdentityPaths {
    private static let stateDirEnv = ["NODOASSIST_STATE_DIR"]
    ...
    private static let appGroupStateDirAvailable =
        DeviceIdentityPaths.hasAppGroupEntitlement(NodoAssistAppGroup.identifier)
        ...
        temporaryDirectory: URL) -> URL
    {
        ...
        return temporaryDirectory.appendingPathComponent("nodoassist", isDirectory: true)
    }
    ...
    private static func legacyStateDirURL() -> URL? {
        if let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first {
            return appSupport.appendingPathComponent("NodoAssist", isDirectory: true)
        }
        return nil
    }
    ...
    private static func appGroupStateDirURL() -> URL? {
        guard
            let containerURL = FileManager.default
                .containerURL(forSecurityApplicationGroupIdentifier: NodoAssistAppGroup.identifier)
        else {
            return nil
        }
        return containerURL.appendingPathComponent("NodoAssist", isDirectory: true)
    }
    ...
    /// An explicit NODOASSIST_STATE_DIR override selects a caller-chosen store, not the legacy
