...
extension NodoAssistChatViewModel {
    ...
    func replaceMessages(_ messages: [NodoAssistChatMessage]) {
    ...
    func persistSessionsToCache(_ sessions: [NodoAssistChatSessionEntry]) {
        guard let transcriptCache else { return }
        let previous = pendingCacheWriteTask
        pendingCacheWriteTask = Task.detached {
            await previous?.value
    ...
    func paintFromCacheIfNeeded(session: SessionSnapshot) {
        ...
        if sessions.isEmpty, !hasAppliedLiveSessions {
            Task { [weak self] in
                ...
                self.sessions = NodoAssistChatSessionListOrganizer.organize(cached)
            }
        }
