...
extension NodoAssistChatViewModel {
    static func decodeMessages(_ raw: [AnyCodable]) -> [NodoAssistChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: NodoAssistChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    static func stripInboundMetadata(from message: NodoAssistChatMessage) -> NodoAssistChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> NodoAssistChatMessageContent in
    ...
    static func messageContentFingerprint(for message: NodoAssistChatMessage) -> String {
    ...
    static func finalMessageContentFingerprint(for message: NodoAssistChatMessage) -> String {
        message.content.map { item in
            let type = (item.type ?? "text").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let text = (item.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    static func messageIdentityKey(for message: NodoAssistChatMessage) -> String? {
    ...
    static func userRefreshIdentityKey(for message: NodoAssistChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func finalMessageReconciliationKey(for message: NodoAssistChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func adoptingCanonicalMessage(
        _ incoming: NodoAssistChatMessage,
        over existing: NodoAssistChatMessage) -> NodoAssistChatMessage
    {
    ...
    private static func preservingLocalAudioDurations(
        in incoming: [NodoAssistChatMessageContent],
        from existing: [NodoAssistChatMessageContent]) -> [NodoAssistChatMessageContent]
    {
    ...
    static func messageRange(
        ...
        in messages: [NodoAssistChatMessage]) -> Range<[NodoAssistChatMessage].Index>
    {
    ...
    func adoptingProvisionalFinalMessageIDs(
        in incoming: [NodoAssistChatMessage]) -> [NodoAssistChatMessage]
    {
    ...
    func clearProvisionalFinalMarkersAdoptedByHistory(_ incoming: [NodoAssistChatMessage]) {
    ...
    func adoptCorrelatedUserMessage(incoming: NodoAssistChatMessage) -> Bool {
    ...
    func adoptProvisionalFinalMessage(incoming: NodoAssistChatMessage) -> Bool {
    ...
    static func reconcileMessageIDs(
        previous: [NodoAssistChatMessage],
        incoming: [NodoAssistChatMessage]) -> [NodoAssistChatMessage]
    {
        guard !previous.isEmpty, !incoming.isEmpty else { return incoming }

        var previousMessagesByKey: [String: [NodoAssistChatMessage]] = [:]
    ...
    static func reconcileRunRefreshMessages(
        previous: [NodoAssistChatMessage],
        incoming: [NodoAssistChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [NodoAssistChatMessage]
    {
    ...
    static func dedupeMessages(_ messages: [NodoAssistChatMessage]) -> [NodoAssistChatMessage] {
    ...
    static func dedupeKey(for message: NodoAssistChatMessage) -> String? {
