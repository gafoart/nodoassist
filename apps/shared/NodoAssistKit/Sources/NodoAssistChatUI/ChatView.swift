...
@MainActor
public struct NodoAssistChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: NodoAssistChatViewModel
    ...
    private let talkControl: NodoAssistChatTalkControl?
    private let voiceNoteControl: NodoAssistChatVoiceNoteControl?
    private let speech: NodoAssistChatSpeechController?
    ...
    public init(
        viewModel: NodoAssistChatViewModel,
        ...
        talkControl: NodoAssistChatTalkControl? = nil,
        voiceNoteControl: NodoAssistChatVoiceNoteControl? = nil,
        speech: NodoAssistChatSpeechController? = nil)
    {
    ...
    public var body: some View {
        ZStack {
            if self.drawsBackground, self.style == .standard {
                NodoAssistChatTheme.background
                    .ignoresSafeArea()
            }
    ...
    @ViewBuilder
    private func messageRow(for msg: NodoAssistChatMessage) -> some View {
    ...
    private func isListenable(_ msg: NodoAssistChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: NodoAssistChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    @ViewBuilder
    private func copyMessageButton(for message: NodoAssistChatMessage) -> some View {
    ...
    private var visibleMessages: [NodoAssistChatMessage] {
        let base: [NodoAssistChatMessage]
    ...
    private var jumpToLatestButton: some View {
        Button {
            ...
            Label("Jump to latest", systemImage: "arrow.down")
                .font(NodoAssistChatTypography.body(size: 16, weight: .semibold, relativeTo: .callout))
                .padding(.horizontal, 13)
                .padding(.vertical, 8)
        }
        .buttonStyle(.plain)
        .foregroundStyle(NodoAssistChatTheme.assistantText)
        .background(
            Capsule()
    ...
    private func mergeToolResults(in messages: [NodoAssistChatMessage]) -> [NodoAssistChatMessage] {
        var result: [NodoAssistChatMessage] = []
        ...
        for message in messages {
            ...
            let merged = NodoAssistChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: NodoAssistChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: NodoAssistChatMessage) -> Bool {
    ...
    private func primaryText(in message: NodoAssistChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return NodoAssistChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: NodoAssistChatMessage) -> Bool {
    ...
    private func toolCalls(in message: NodoAssistChatMessage) -> [NodoAssistChatMessageContent] {
    ...
    private func inlineToolResults(in message: NodoAssistChatMessage) -> [NodoAssistChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: NodoAssistChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: NodoAssistChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [NodoAssistChatView.StarterPrompt]
    let onPrompt: (NodoAssistChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(self.text)
                ...
                .background(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .fill(NodoAssistChatTheme.assistantBubble))
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(NodoAssistChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(NodoAssistChatTypography.headline)
        } description: {
            Text(self.message)
                .font(NodoAssistChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(NodoAssistChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
