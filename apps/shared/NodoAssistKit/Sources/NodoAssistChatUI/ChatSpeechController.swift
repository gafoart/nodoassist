...
public struct NodoAssistChatSpeechClip: Equatable, Sendable {
...
public typealias NodoAssistChatSpeechSynthesis =
    @Sendable (_ text: String) async throws -> NodoAssistChatSpeechClip
...
@MainActor
@Observable
public final class NodoAssistChatSpeechController {
    ...
    private let synthesize: NodoAssistChatSpeechSynthesis
    ...
    public convenience init(synthesize: @escaping NodoAssistChatSpeechSynthesis) {
        self.init(
    ...
    init(
        synthesize: @escaping NodoAssistChatSpeechSynthesis,
    ...
    private func run(messageID: UUID, text: String, generation: UInt64) async {
        let clip: NodoAssistChatSpeechClip? = await {
            do {
