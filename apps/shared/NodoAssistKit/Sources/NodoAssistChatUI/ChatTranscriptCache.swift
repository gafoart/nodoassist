...
private let cacheLogger = Logger(subsystem: "ai.nodoassist", category: "NodoAssistChatTranscriptCache")

private final class OutboxChangeHub: @unchecked Sendable {
    private let lock = NSLock()
    private var continuations: [UUID: AsyncStream<NodoAssistChatOutboxChange>.Continuation] = [:]

    func stream() -> AsyncStream<NodoAssistChatOutboxChange> {
        let id = UUID()
        let pair = AsyncStream<NodoAssistChatOutboxChange>.makeStream()
        self.lock.lock()
    ...
    func yield(_ change: NodoAssistChatOutboxChange) {
        self.lock.lock()
...
public protocol NodoAssistChatTranscriptCache: Sendable {
    func loadSessions() async -> [NodoAssistChatSessionEntry]
    func loadTranscript(sessionKey: String) async -> [NodoAssistChatMessage]
    func loadTranscript(sessionKey: String, agentID: String?) async -> [NodoAssistChatMessage]
    func storeSessions(_ sessions: [NodoAssistChatSessionEntry]) async
    func storeTranscript(sessionKey: String, messages: [NodoAssistChatMessage]) async
    func storeTranscript(sessionKey: String, agentID: String?, messages: [NodoAssistChatMessage]) async
    ...
    func storeCanonicalTranscript(
        sessionKey: String,
        messages: [NodoAssistChatMessage],
...
extension NodoAssistChatTranscriptCache {
    public func loadTranscript(sessionKey: String, agentID: String?) async -> [NodoAssistChatMessage] {
    ...
    public func storeTranscript(
        sessionKey: String,
        agentID: String?,
        messages: [NodoAssistChatMessage]) async
    {
        guard agentID == nil else { return }
...
public protocol NodoAssistChatCanonicalTranscriptMerging: NodoAssistChatTranscriptCache {
...
public struct NodoAssistChatOutboxAttachment: Codable, Hashable, Sendable {
...
public struct NodoAssistChatOutboxCommand: Hashable, Sendable, Identifiable {
    ...
    public let attachments: [NodoAssistChatOutboxAttachment]
    ...
    public init(
        ...
        agentID: String? = nil,
        text: String,
        attachments: [NodoAssistChatOutboxAttachment] = [],
        thinking: String,
        createdAt: Double,
        status: Status,
        retryCount: Int,
...
public enum NodoAssistChatOutboxUpdateResult: Equatable, Sendable {
    case updated
    case confirmed
    case missing
    case unavailable
...
public enum NodoAssistChatOutboxChange: Equatable, Sendable {
...
public protocol NodoAssistChatCommandOutbox: Sendable {
    ...
    func enqueueCommand(_ command: NodoAssistChatOutboxCommand) async -> Bool
    ...
    func claimNextCommand() async -> NodoAssistChatOutboxCommand?
    func markCommandQueued(id: String, retryCount: Int, lastError: String?) async
    func markCommandAwaitingConfirmation(id: String) async -> NodoAssistChatOutboxUpdateResult
    ...
    func markCommandFailedIfPresent(
        id: String,
        retryCount: Int,
        lastError: String?) async -> NodoAssistChatOutboxUpdateResult
    ...
    func markCommandRetriedIfPresent(
        ...
        routingContract: String) async -> NodoAssistChatOutboxUpdateResult
    ...
    func cancelCommand(id: String) async -> NodoAssistChatOutboxUpdateResult
    /// Canonical gateway history may complete any row, including a sending
    /// row whose request ACK was lost.
    func confirmCommand(id: String) async -> NodoAssistChatOutboxUpdateResult
    /// Cross-view-model invalidation.
    func changes() -> AsyncStream<NodoAssistChatOutboxChange>
}

public struct NodoAssistChatSessionRoutingIdentity: Equatable, Sendable {
    ...
    public init?(contract: String?) {
        guard let components = NodoAssistChatSessionRoutingContract.parse(contract) else { return nil }
    ...
    public init?(scope: String?, mainSessionKey: String?, defaultAgentID: String?) {
        guard let contract = NodoAssistChatSessionRoutingContract.make(
            scope: scope,
...
public actor NodoAssistChatSQLiteTranscriptCache: NodoAssistChatTranscriptCache,
    NodoAssistChatCanonicalTranscriptMerging,
    NodoAssistChatCommandOutbox
{
    ...
    public nonisolated static func loadSessionRoutingIdentity(
        databaseURL: URL,
        gatewayID: String) -> NodoAssistChatSessionRoutingIdentity?
    {
        ...
        return NodoAssistChatSessionRoutingIdentity(
    ...
    // MARK: - NodoAssistChatTranscriptCache

    public func loadSessions() async -> [NodoAssistChatSessionEntry] {
        ...
        guard let decoded = try? JSONDecoder().decode(
            [NodoAssistChatSessionEntry].self,
    ...
    public func loadTranscript(sessionKey: String) async -> [NodoAssistChatMessage] {
    ...
    private func readTranscript(
        _ db: OpaquePointer,
        sessionKey: String,
        agentID: String?) -> [NodoAssistChatMessage]
    {
    ...
    public func storeSessions(_ sessions: [NodoAssistChatSessionEntry]) async {
        guard !self.isRetired else { return }
        guard let db = await handle() else { return }
    ...
    public func loadSessionRoutingIdentity() async -> NodoAssistChatSessionRoutingIdentity? {
    ...
    public func storeSessionRoutingIdentity(_ identity: NodoAssistChatSessionRoutingIdentity) async {
    ...
    public func storeTranscript(sessionKey: String, messages: [NodoAssistChatMessage]) async {
    ...
    // MARK: - NodoAssistChatCommandOutbox

    public nonisolated func changes() -> AsyncStream<NodoAssistChatOutboxChange> {
        self.outboxChangeHub.stream()
    }

    public func enqueueCommand(_ command: NodoAssistChatOutboxCommand) async -> Bool {
    ...
    public func loadCommands() async -> [NodoAssistChatOutboxCommand] {
        await self.loadCommandsIfAvailable() ?? []
    }

    public func loadCommandsIfAvailable() async -> [NodoAssistChatOutboxCommand]? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func claimNextCommand() async -> NodoAssistChatOutboxCommand? {
        guard !self.isRetired, let db = await handle() else { return nil }
    ...
    public func markCommandAwaitingConfirmation(id: String) async -> NodoAssistChatOutboxUpdateResult {
        ...
        routingContract: String) async -> NodoAssistChatOutboxUpdateResult
    {
        ...
        let normalizedRoutingContract = routingContract.trimmingCharacters(in: .whitespacesAndNewlines)
        let allowsUntargetedAgent = normalizedRoutingContract == NodoAssistChatOutboxCommand
    ...
    public func cancelCommand(id: String) async -> NodoAssistChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(db, sql: "BEGIN IMMEDIATE", bindings: []) else { return .unavailable }
        var committed = false
        defer {
        ...
        let result = self.canonicalMessageProofHub.withProofDecision(for: messageKey) { isProven in
            if isProven {
                committed = self.execute(db, sql: "COMMIT", bindings: [])
                return committed ? NodoAssistChatOutboxUpdateResult.confirmed : .unavailable
            }
    ...
    public func confirmCommand(id: String) async -> NodoAssistChatOutboxUpdateResult {
        guard !self.isRetired, let db = await handle() else { return .unavailable }
        guard self.execute(
            db,
    ...
    private func emitOutboxChange(_ change: NodoAssistChatOutboxChange) {
    ...
    static func cacheableMessages(_ messages: [NodoAssistChatMessage]) -> [NodoAssistChatMessage] {
    ...
    static func boundedSessions(_ sessions: [NodoAssistChatSessionEntry]) -> [NodoAssistChatSessionEntry] {
    ...
    private static func attachmentByteCount(_ attachments: [NodoAssistChatOutboxAttachment]) -> Int? {
        var total = 0
    ...
    private func readCommands(_ db: OpaquePointer) -> [NodoAssistChatOutboxCommand]? {
        var statement: OpaquePointer?
        let sql = """
        ...
        var commands: [NodoAssistChatOutboxCommand] = []
        ...
        while step == SQLITE_ROW {
               ...
               let text = sqlite3_column_text(statement, 5)
            {
                ...
                guard let attachments = try? JSONDecoder().decode(
                    [NodoAssistChatOutboxAttachment].self,
                ...
                if let status = NodoAssistChatOutboxCommand.Status(rawValue: statusRaw) {
                    commands.append(
                        NodoAssistChatOutboxCommand(
