...
extension NodoAssistChatViewModel {
    ...
    private func thinkingPickerIsAvailable(
        ...
        modelChoice: NodoAssistChatModelChoice?) -> Bool
    {
    ...
    private struct ThinkingLevelOptionsResolution {
        let options: [NodoAssistChatThinkingLevelOption]
        let isGatewayMetadata: Bool
    }

    private func resolvedThinkingLevelOptions(
        for currentSession: NodoAssistChatSessionEntry?) -> ThinkingLevelOptionsResolution
    {
    ...
    private func selectedModelChoice(
        for currentSession: NodoAssistChatSessionEntry?) -> NodoAssistChatModelChoice?
    {
    ...
    private func modelChoice(modelID: String?, provider: String?) -> NodoAssistChatModelChoice? {
    ...
    private static func sessionModelMatchesDefaults(
        ...
        defaults: NodoAssistChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [NodoAssistChatThinkingLevelOption]?) -> [NodoAssistChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return NodoAssistChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [NodoAssistChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return NodoAssistChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    static func withCurrentThinkingOption(
        _ options: [NodoAssistChatThinkingLevelOption],
        current: String) -> [NodoAssistChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [NodoAssistChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [NodoAssistChatThinkingLevelOption]) -> [NodoAssistChatThinkingLevelOption]
    {
        var result: [NodoAssistChatThinkingLevelOption] = []
        var seen = Set<String>()
