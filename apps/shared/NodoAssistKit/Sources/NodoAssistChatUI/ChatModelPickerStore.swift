...
public struct ChatModelPickerSections: Sendable, Equatable {
    public let pinned: [NodoAssistChatModelChoice]
    public let recent: [NodoAssistChatModelChoice]
    public let remaining: [NodoAssistChatModelChoice]
}

@MainActor
public final class ChatModelPickerStore {
    private static let favoritesKey = "nodoassist.chat.modelFavorites"
    private static let recentsKey = "nodoassist.chat.modelRecents"
    ...
    public func recordRecent(_ selectionID: String) {
        guard !selectionID.isEmpty, selectionID != NodoAssistChatViewModel.defaultModelSelectionID else { return }
