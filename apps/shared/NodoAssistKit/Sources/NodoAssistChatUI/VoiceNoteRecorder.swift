...
public struct NodoAssistVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class NodoAssistVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: NodoAssistVoiceNoteRecording)
        case staging(recording: NodoAssistVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: NodoAssistVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = NodoAssistVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: NodoAssistVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> NodoAssistVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: NodoAssistVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> NodoAssistVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = NodoAssistVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class NodoAssistVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "NodoAssistVoiceNoteAudioCapture",
                    code: 1,
...
func nodoAssistVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
