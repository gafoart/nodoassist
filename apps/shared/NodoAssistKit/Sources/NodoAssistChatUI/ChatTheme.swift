...
enum NodoAssistChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("NodoAssistChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("NodoAssistChatTheme.onboardingAssistantBubble"),
...
enum NodoAssistPlatformImageFactory {
    static func image(_ image: NodoAssistPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
