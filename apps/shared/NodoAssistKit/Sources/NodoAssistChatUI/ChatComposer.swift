...
public struct NodoAssistChatTalkControl {
...
private struct CleanChatComposerSurface: ViewModifier {
    ...
    func body(content: Content) -> some View {
        ...
        content
            ...
            .overlay(
                RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                    .strokeBorder(NodoAssistChatTheme.composerBorder, lineWidth: 1))
        #else
...
@MainActor
struct NodoAssistChatComposer: View {
    @Bindable var viewModel: NodoAssistChatViewModel
    let style: NodoAssistChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: NodoAssistChatView.ComposerChrome
    let isComposerEnabled: Bool
    let isAttachmentInputEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: NodoAssistChatTalkControl?
    let voiceNoteControl: NodoAssistChatVoiceNoteControl?
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let voiceNoteControl, voiceNoteControl.recorder.isRecording {
                NodoAssistVoiceNoteRecordingRow(recorder: voiceNoteControl.recorder)
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(NodoAssistChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(NodoAssistChatTheme.composerBorder, lineWidth: 1))
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 5) {
                    ...
                    if self.viewModel.showsModelPicker {
                        self.modelPicker
                        if self.viewModel.modelSelectionID != NodoAssistChatViewModel.defaultModelSelectionID {
                            self.modelPinButton
                        }
                    }
                }
    ...
    private func contextUsageIndicator(_ fraction: Double) -> some View {
        ...
        let color = fraction >= 0.8 ? NodoAssistChatTheme.warning : NodoAssistChatTheme.muted
        return ZStack {
            Circle()
                .stroke(NodoAssistChatTheme.muted.opacity(0.2), lineWidth: 2)
            Circle()
    ...
    private var thinkingPicker: some View {
                ...
                set: { next in self.viewModel.selectThinkingLevel(next) }))
        {
            ForEach(self.viewModel.thinkingLevelOptions) { option in
                Text(option.label)
                    .font(NodoAssistChatTypography.captionSemiBold)
                    .tag(option.id)
            }
        }
        .labelsHidden()
    ...
    private var modelPicker: some View {
                ...
                set: { next in self.viewModel.selectModel(next) }))
        {
            Text(self.viewModel.defaultModelLabel)
                .font(NodoAssistChatTypography.captionSemiBold)
                .tag(NodoAssistChatViewModel.defaultModelSelectionID)
    ...
    private func modelOptions(_ models: [NodoAssistChatModelChoice]) -> some View {
                ...
                set: { next in self.viewModel.switchSession(to: next) }))
        {
            ForEach(self.viewModel.sessionChoices, id: \.key) { session in
                Text(session.displayName ?? session.key)
                    .font(NodoAssistChatTypography.mono(size: 12, relativeTo: .caption))
                    .tag(session.key)
            }
        }
        .labelsHidden()
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \NodoAssistPendingAttachment.id)
                { (att: NodoAssistPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            NodoAssistPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
                            ...
                            Text("Voice note")
                                .font(NodoAssistChatTypography.caption)
                            if let durationSeconds = att.durationSeconds {
                                Text(nodoAssistVoiceNoteDurationLabel(durationSeconds))
                                    .font(NodoAssistChatTypography.caption)
                                    .foregroundStyle(.secondary)
                            }
                        } else {
                    ...
                    .padding(.vertical, 5)
                    .background(NodoAssistChatTheme.accent.opacity(0.08))
                    .clipShape(Capsule())
                }
            }
        }
    ...
    private var fullEditor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(NodoAssistChatTheme.divider)
                .frame(height: 1)
    ...
    private func talkButton(_ talkControl: NodoAssistChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: NodoAssistChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
                .font(NodoAssistChatTypography.body(size: 14, weight: .semibold, relativeTo: .subheadline))
                ...
                .background {
                    Circle()
                        .fill(talkControl.isEnabled
                            ...
                            : AnyShapeStyle(NodoAssistChatTheme.accent))
    ...
    private func talkButtonFill(_ talkControl: NodoAssistChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(NodoAssistChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return NodoAssistChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: NodoAssistChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return NodoAssistChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: NodoAssistChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: NodoAssistChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        HStack(spacing: 6) {
            ...
            Text(self.connectionStatusText)
                .font(NodoAssistChatTypography.caption2)
                .foregroundStyle(.secondary)
        }
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(NodoAssistChatTheme.subtleCard)
            }
        }
    }
        ...
        ZStack(alignment: self.editorOverlayAlignment) {
            if self.viewModel.input.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text(self.placeholderText)
                    .font(NodoAssistChatTypography.body)
        ...
        HStack(alignment: .top, spacing: 0) {
            VStack(alignment: .leading, spacing: 3) {
                Text(command.displayInvocation)
                    .font(NodoAssistChatTypography.mono(
                        size: 15,
                        weight: .semibold,
    ...
    private func selectSlashCommand(_ command: NodoAssistChatCommandChoice) {
                ...
                Button {
                    ...
                    if self.viewModel.isAborting {
                        ...
                        Image(systemName: "stop.fill")
                            .font(NodoAssistChatTypography.display(size: 13, weight: .semibold, relativeTo: .caption))
                    }
                }
                .buttonStyle(.plain)
                .foregroundStyle(.white)
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? NodoAssistChatTheme.userBubble
    }
