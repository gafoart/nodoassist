...
extension NodoAssistChatViewModel {
    nonisolated static func chatContextUsageFraction(for session: NodoAssistChatSessionEntry?) -> Double? {
    ...
    public var sessionChoices: [NodoAssistChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = NodoAssistChatSessionListOrganizer.organize(sessions)
        let mainSessionKey = resolvedMainSessionKey

        var result: [NodoAssistChatSessionEntry] = []
