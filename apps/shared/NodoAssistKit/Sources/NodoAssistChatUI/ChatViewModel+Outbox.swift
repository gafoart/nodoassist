...
private let outboxLogger = Logger(subsystem: "ai.nodoassist", category: "NodoAssistChatOutbox")

/// Display state for a transcript row backed by a durable outbox command.
public enum NodoAssistChatOutboxMessageState: Equatable, Sendable {
    case queued
    case sending
    case confirming
...
extension NodoAssistChatViewModel {
    ...
    public func outboxState(for messageID: UUID) -> NodoAssistChatOutboxMessageState? {
    ...
    func enqueueOutboxCommand(
        text: String,
        draftInput: String,
        draftAttachments: [NodoAssistPendingAttachment] = [],
        session: SessionSnapshot) async
    {
        ...
        let command = NodoAssistChatOutboxCommand(
            id: UUID().uuidString,
            ...
            attachments: draftAttachments.map {
                NodoAssistChatOutboxAttachment(
                    type: $0.type,
                    mimeType: $0.mimeType,
                    fileName: $0.fileName,
                    data: $0.data,
    ...
    func confirmOutboxCommands(in messages: [NodoAssistChatMessage]) {
    ...
    func confirmOutboxCommandsNow(in messages: [NodoAssistChatMessage]) async {
        self.observeCanonicalOutboxMessageKeys(in: messages)
        guard let outbox else { return }
    ...
    private func persistCanonicalOutboxEvidence(
        _ message: NodoAssistChatMessage,
        for command: NodoAssistChatOutboxCommand) async
    {
        guard let transcriptCache = transcriptCache as? any NodoAssistChatCanonicalTranscriptMerging else { return }
    ...
    private func observeCanonicalOutboxMessageKeys(in messages: [NodoAssistChatMessage]) {
    ...
    private func presentOutboxCommands(_ commands: [NodoAssistChatOutboxCommand]) {
    ...
    private static func outboxUserMessage(for command: NodoAssistChatOutboxCommand) -> NodoAssistChatMessage {
        var content = [
    ...
    private func mapOutboxCommand(_ command: NodoAssistChatOutboxCommand, to messageID: UUID) {
    ...
    private func performOutboxFlush() async {
        ...
        while self.healthOK {
            ...
            do {
            ...
            } catch is NodoAssistChatTransportSendError {
                ...
                if error.detailsReason == NodoAssistChatSessionRoutingContract.changedErrorReason {
                    guard await self.parkOutboxCommandForChangedTarget(next, outbox: outbox) else { break }
                    continue
                }
    ...
    private static func needsOutboxDeliveryReconciliation(_ command: NodoAssistChatOutboxCommand) -> Bool {
        command.status == .awaitingConfirmation ||
            (command.status == .failed &&
                command.lastError == NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }

    private static func deliveryTarget(for command: NodoAssistChatOutboxCommand) -> OutboxDeliveryTarget {
    ...
    private func parkOutboxCommandWithUnconfirmedDelivery(
        _ command: NodoAssistChatOutboxCommand,
        outbox: any NodoAssistChatCommandOutbox) async -> NodoAssistChatOutboxUpdateResult
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError)
        switch update {
        case .updated:
            self.setOutboxState(
                .failed(reason: NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError),
    ...
    private func parkOutboxCommandForChangedTarget(
        _ command: NodoAssistChatOutboxCommand,
        outbox: any NodoAssistChatCommandOutbox) async -> Bool
    {
        let update = await outbox.markCommandFailedIfPresent(
            id: command.id,
            retryCount: command.retryCount,
            lastError: NodoAssistChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    private func recordOutboxRejection(
        of command: NodoAssistChatOutboxCommand,
    ...
    private func spliceSentCommandIntoCachedTranscript(_ command: NodoAssistChatOutboxCommand) async {
    ...
    private func outboxRoutingContract(for session: SessionSnapshot) -> String? {
        if !self.transport.outboxRequiresSessionRoutingContract {
            return NodoAssistChatOutboxCommand.legacyUnboundRoutingContract
        }
    ...
    private func setOutboxState(_ state: NodoAssistChatOutboxMessageState, forCommandID commandID: String) {
    ...
    func handleOutboxChange(_ change: NodoAssistChatOutboxChange) {
    ...
    private static func outboxDisplayState(for command: NodoAssistChatOutboxCommand)
        -> NodoAssistChatOutboxMessageState
    {
