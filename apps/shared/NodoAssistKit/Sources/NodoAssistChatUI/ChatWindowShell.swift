...
/// panel keeps using `NodoAssistChatView` directly; this shell is the full
/// window experience.
@MainActor
public struct NodoAssistChatWindowShell: View {
    @State private var viewModel: NodoAssistChatViewModel
    @State private var sessionQuery = ""
    @State private var isConfirmingClearHistory = false
    private let userAccent: Color?

    public init(viewModel: NodoAssistChatViewModel, userAccent: Color? = nil) {
...
private struct ChatContextUsageMenu: View {
    let usage: NodoAssistChatContextUsage
    let onCompact: () -> Void
...
private struct ChatSessionSidebar: View {
    ...
    @State private var sessionPendingDeletion: NodoAssistChatSessionEntry?
    ...
    private func row(for session: NodoAssistChatSessionEntry) -> some View {
        HStack(spacing: 6) {
            VStack(alignment: .leading, spacing: 2) {
                Text(ChatSessionSidebarModel.displayName(for: session))
                    .font(NodoAssistChatTypography.body(size: 13, weight: .medium, relativeTo: .body))
                    .lineLimit(1)
    ...
    private func rowSubtitle(for session: NodoAssistChatSessionEntry) -> String? {
