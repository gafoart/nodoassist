...
struct ChatSessionsSheet: View {
    ...
    @State private var scopedSessions: [NodoAssistChatSessionEntry] = []
    @State private var isLoadingScoped = false
    @State private var renameTarget: NodoAssistChatSessionEntry?
    ...
    private var displayedSessions: [NodoAssistChatSessionEntry] {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                self.viewModel.refreshSessions(limit: NodoAssistChatViewModel.sessionListFetchLimit)
            }
            .alert(
                "Rename Session",
                isPresented: Binding(
    ...
    private func sessionRow(_ session: NodoAssistChatSessionEntry) -> some View {
            ...
            HStack(spacing: 8) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(session.displayName ?? session.key)
                        .font(NodoAssistChatTypography.mono(size: 17, relativeTo: .body))
                        .lineLimit(1)
        ...
        .swipeActions(edge: .leading, allowsFullSwipe: true) {
            if !session.isArchived {
                ...
                .tint(NodoAssistChatTheme.accent)
            }
        }
        .swipeActions(edge: .trailing, allowsFullSwipe: false) {
            ...
            .tint(session.isArchived ? NodoAssistChatTheme.accent : NodoAssistChatTheme.danger)
        }
        .contextMenu {
            Button {
