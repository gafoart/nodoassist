...
private let chatUILogger = Logger(subsystem: "ai.nodoassist", category: "NodoAssistChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class NodoAssistChatViewModel {
    ...
    public internal(set) var messages: [NodoAssistChatMessage] = []

    public var input: String = ""
    public private(set) var thinkingLevel: String
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var thinkingLevelOptions: [NodoAssistChatThinkingLevelOption]
    /// Setter is module-internal for the thinking-level extension only.
    public internal(set) var showsThinkingPicker = true
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [NodoAssistChatModelChoice] = []
    private var modelPickerFavorites: [String]
    private var modelPickerRecents: [String]
    public private(set) var slashCommands: [NodoAssistChatCommandChoice] = []
    ...
    private struct SlashFilterCache {
        let query: String
        let filter: NodoAssistChatCommandFilter
        let result: [NodoAssistChatCommandChoice]
    }
    ...
    public var attachments: [NodoAssistPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [NodoAssistChatPendingToolCall] = []

    private(set) var timelineRevision: UInt64 = 0
    /// Setter is module-internal for the transcript-cache extension only.
    public internal(set) var sessions: [NodoAssistChatSessionEntry] = [] {
        didSet { self.syncContextUsageFraction() }
    }
    ...
    let transport: any NodoAssistChatTransport
    let haptics: NodoAssistChatHaptics
    let transcriptCache: (any NodoAssistChatTranscriptCache)?
    let outbox: (any NodoAssistChatCommandOutbox)?
    @ObservationIgnored
    ...
    public internal(set) var outboxStatesByMessageID: [UUID: NodoAssistChatOutboxMessageState] = [:]
    @ObservationIgnored
    ...
    var sessionDefaults: NodoAssistChatSessionsDefaults? {
        didSet { self.syncContextUsageFraction() }
    }
    ...
    private struct HistoryRequest {
        ...
        var visibleMessagesByID: [UUID: NodoAssistChatMessage]
    ...
    private var pendingToolCallsById: [String: NodoAssistChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any NodoAssistChatTransport,
        activeAgentId: String? = nil,
        sessionRoutingContract: String? = nil,
        attachmentOwnerIsActive: @escaping @MainActor () -> Bool = { false },
        haptics: NodoAssistChatHaptics = NodoAssistChatHaptics(),
        transcriptCache: (any NodoAssistChatTranscriptCache)? = nil,
        outbox: (any NodoAssistChatCommandOutbox)? = nil,
    ...
    public func slashCommandMatches(
        query: String,
        filter: NodoAssistChatCommandFilter) -> [NodoAssistChatCommandChoice]
    {
    ...
    public func applySlashCommandSelection(_ command: NodoAssistChatCommandChoice) {
    ...
    private func usesMutableContractRouting(for contract: String?) -> Bool {
        ...
        let contractMainKey = NodoAssistChatSessionRoutingContract.parse(contract)?.mainKey ?? ""
    ...
    static let baseThinkingLevelOptions: [NodoAssistChatThinkingLevelOption] = [
        NodoAssistChatThinkingLevelOption(id: "off", label: "off"),
        NodoAssistChatThinkingLevelOption(id: "minimal", label: "minimal"),
        NodoAssistChatThinkingLevelOption(id: "low", label: "low"),
        NodoAssistChatThinkingLevelOption(id: "medium", label: "medium"),
        NodoAssistChatThinkingLevelOption(id: "high", label: "high"),
    ]
    ...
    public func removeAttachment(_ id: NodoAssistPendingAttachment.ID) {
    ...
    private func appendMessage(_ message: NodoAssistChatMessage) {
    ...
    private func applyHistoryPayload(
        _ payload: NodoAssistChatHistoryPayload,
    ...
    private func provisionalFinalMessagesMissing(
        from incoming: [NodoAssistChatMessage]) -> [NodoAssistChatMessage]
    {
    ...
    private func applyInFlightRunSnapshot(
        _ snapshot: NodoAssistChatInFlightRun?,
        for request: HistoryRequest)
    {
    ...
    private static func isKnownSlashCommandText(
        _ text: String,
        commands: [NodoAssistChatCommandChoice]) -> Bool
    {
    ...
    private static func commands(
        _ commands: [NodoAssistChatCommandChoice],
    ...
    private static func command(
        _ command: NodoAssistChatCommandChoice,
        ...
        filter: NodoAssistChatCommandFilter) -> [NodoAssistChatCommandChoice]
    {
        ...
        let effectiveFilter: NodoAssistChatCommandFilter =
            self.queryTargetsSkills(trimmed) && filter == .all ? .skills : filter
        return commands.enumerated()
            .compactMap { index, command -> (Int, Int, NodoAssistChatCommandChoice)? in
    ...
    private static func command(
        _ command: NodoAssistChatCommandChoice,
        isIncludedIn filter: NodoAssistChatCommandFilter) -> Bool
    {
        switch filter {
        case .all:
            true
    ...
    private func performSend() async {
           ...
           self.outbox != nil
        {
            ...
            if case let .unavailable(reason) = routeResult,
               reason == NodoAssistChatTransportUpgradeMessage.routingContract
            {
        ...
        var userContent: [NodoAssistChatMessageContent] = [
        ...
        let encodedAttachments = draftAttachments.map { attachment in
            NodoAssistChatAttachmentPayload(
        ...
        do {
            ...
            if encodedAttachments.isEmpty, !(error is GatewayResponseError) {
                ...
                self.clearPendingRun(runId)
                let deliveryIsAmbiguous = !(error is NodoAssistChatTransportSendError)
    ...
    func fetchSessions(limit: Int?, sessionSnapshot: SessionSnapshot? = nil) async {
        do {
            ...
            let organized = NodoAssistChatSessionListOrganizer.organize(res.sessions)
    ...
    public func fetchSessionList(search: String?, archived: Bool) async -> [NodoAssistChatSessionEntry] {
        ...
        do {
            let res = try await self.transport.listSessions(
                ...
                search: query,
                archived: archived)
            return NodoAssistChatSessionListOrganizer.organize(res.sessions)
        } catch {
            ...
            return NodoAssistChatSessionListOrganizer.filter(self.sessions, search: query)
        }
    }
    ...
    public func setSessionPinned(key: String, pinned: Bool) {
        ...
        if let index = self.sessions.firstIndex(where: { $0.key == key }) {
            ...
            self.sessions = NodoAssistChatSessionListOrganizer.organize(self.sessions)
        }
        Task {
            do {
                try await self.transport.patchSession(
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "NodoAssistChatTransport"
    ...
    func placeholderSession(key: String) -> NodoAssistChatSessionEntry {
    ...
    private func handleTransportEvent(_ evt: NodoAssistChatTransportEvent) {
        switch evt {
        case let .health(ok):
    ...
    private func handleSessionMessageEvent(_ payload: NodoAssistSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: NodoAssistChatEventPayload) {
           ...
           let runId = Self.normalizedRunID(chat.runId)
        {
            ...
            self.adoptRun(
                runId: runId,
                bufferedText: NodoAssistChatEventText.assistantText(from: chat) ?? "")
            return
        }
        ...
        case "final", "aborted", "error":
            ...
            let hapticEvent: NodoAssistChatHaptics.Event? = switch chat.state {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: NodoAssistChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = NodoAssistChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: NodoAssistChatMessage.self)
        }
    ...
    static func isAssistantMessage(_ message: NodoAssistChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: NodoAssistChatMessage) -> NodoAssistChatMessage {
        guard message.timestamp == nil else { return message }
        return NodoAssistChatMessage(
            id: message.id,
            role: message.role,
    ...
    private func handleAgentEvent(_ evt: NodoAssistAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = NodoAssistChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: NodoAssistAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: NodoAssistAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    private func finishPendingRunAfterTerminalOkSendAck(_ response: NodoAssistChatSendResponse) {
        self.clearPendingRun(response.runId, hapticEvent: .runCompleted)
        self.pendingToolCallsById = [:]
        self.updateStreamingAssistantText(nil)
        self.logDiagnostic(
    ...
    private func finishPendingRunIfTerminalSendAck(_ response: NodoAssistChatSendResponse) -> Bool {
    ...
    private static func hasUnansweredLatestUser(in messages: [NodoAssistChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    static func latestUserTurn(in messages: [NodoAssistChatMessage]) -> LatestUserTurn? {
    ...
    static func userTurn(
        at userIndex: [NodoAssistChatMessage].Index,
        in messages: [NodoAssistChatMessage]) -> LatestUserTurn?
    {
    ...
    private static func hasAnsweredUser(
        ...
        in messages: [NodoAssistChatMessage])
        -> Bool
    {
        ...
        var occurrence = 0
        var latestMatchingUserIndex: [NodoAssistChatMessage].Index?
    ...
    private static func hasAssistantMessage(
        after userIndex: [NodoAssistChatMessage].Index,
        in messages: [NodoAssistChatMessage]) -> Bool
    {
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [NodoAssistChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
    ...
    private static func assistantHapticEvent(
        for message: NodoAssistChatMessage) -> NodoAssistChatHaptics.Event?
    {
    ...
    private func assistantHapticEventAfterLatestUser() -> NodoAssistChatHaptics.Event? {
    ...
    private func assistantHapticEvent(after timestamp: Double) -> NodoAssistChatHaptics.Event? {
    ...
    func refreshHistoriesAfterOutboxFlush(
        ...
        routeLease: NodoAssistChatTransportRouteLease) async
    {
    ...
    private func clearPendingRun(
        _ runId: String,
        hapticEvent: NodoAssistChatHaptics.Event? = nil)
    {
