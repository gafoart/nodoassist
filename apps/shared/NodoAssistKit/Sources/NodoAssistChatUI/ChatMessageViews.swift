...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            .font(NodoAssistChatTypography.avatar(size: self.fontSize))
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? NodoAssistChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? NodoAssistChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
private struct ChatMessageBody: View {
    @Environment(\.nodoAssistAssistantBubblesInCleanChrome) private var assistantBubblesInClean
    let message: NodoAssistChatMessage
    let isUser: Bool
    let style: NodoAssistChatView.Style
    let markdownVariant: ChatMarkdownVariant
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? NodoAssistChatTheme.userText : NodoAssistChatTheme.assistantText
    ...
    private var inlineAttachments: [NodoAssistChatMessageContent] {
    ...
    private var toolCalls: [NodoAssistChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [NodoAssistChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? NodoAssistChatTheme.userBubble
        }
        if self.style == .onboarding {
            return NodoAssistChatTheme.onboardingAssistantBubble
        }
        return NodoAssistChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return NodoAssistChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: NodoAssistChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.isAudio ? "Voice note" : (self.att.fileName ?? "Attachment"))
                .font(NodoAssistChatTypography.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? NodoAssistChatTheme.userText : NodoAssistChatTheme.assistantText)
            if self.isAudio, let durationSeconds = self.att.durationSeconds {
                Text(nodoAssistVoiceNoteDurationLabel(durationSeconds))
                    .font(NodoAssistChatTypography.footnote)
                    .foregroundStyle(
                        self.isUser
                            ? NodoAssistChatTheme.userText.opacity(0.72)
                            : NodoAssistChatTheme.assistantText.opacity(0.72))
            }
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: NodoAssistChatMessageContent
    let isUser: Bool
...
struct ChatOutboxStatusLabel: View {
    let state: NodoAssistChatOutboxMessageState
    ...
    var body: some View {
        ...
        .foregroundStyle(self.state.isFailed ? AnyShapeStyle(NodoAssistChatTheme.danger) : AnyShapeStyle(.secondary))
    ...
    private var title: String {
        ...
        case .confirming:
            "Confirming…"
        case let .failed(reason) where reason == NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError:
            "Delivery unknown"
        case .failed:
            "Not sent"
        }
...
extension EnvironmentValues {
    ...
    @Entry public var nodoAssistAssistantBubblesInCleanChrome: Bool = false
}
...
private struct AssistantBubbleContainerStyle: ViewModifier {
    ...
    @Environment(\.nodoAssistAssistantBubblesInCleanChrome) private var bubblesInClean
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [NodoAssistChatPendingToolCall]
    let isClean: Bool
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                let font = segment.kind == .thinking
                    ? NodoAssistChatTypography.callout.italic()
                    : NodoAssistChatTypography.body
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: NodoAssistChatTheme.assistantText,
