...
/// Animated NodoAssist mascot. Redraws the canonical 120x120 vector from
...
public struct NodoAssistMascotView: View {
    ...
    public var body: some View {
        let palette = NodoAssistMascotPalette.forScheme(self.colorScheme)
        if self.reduceMotion {
            NodoAssistMascotCanvas(pose: .still, palette: palette)
        } else {
            TimelineView(.animation(minimumInterval: 1.0 / 30.0)) { timeline in
                let pose = NodoAssistMascotPose.at(time: timeline.date.timeIntervalSinceReferenceDate)
                ...
                GeometryReader { proxy in
                    NodoAssistMascotCanvas(pose: pose, palette: palette)
                        .offset(
                            y: self.floats
...
struct NodoAssistMascotPalette: Equatable {
    let gradientTop: Color
    let gradientBottom: Color
    let antenna: Color

    static let dark = NodoAssistMascotPalette(
    ...
    static let light = NodoAssistMascotPalette(
    ...
    static func forScheme(_ colorScheme: ColorScheme) -> NodoAssistMascotPalette {
...
struct NodoAssistMascotPose: Equatable {
    ...
    static let still = NodoAssistMascotPose()

    static func at(time: TimeInterval) -> NodoAssistMascotPose {
        // Float depth matches the hero: -8px on a 100px mark = 8% of the 120 box.
        NodoAssistMascotPose(
...
private struct NodoAssistMascotCanvas: View {
    let pose: NodoAssistMascotPose
    let palette: NodoAssistMascotPalette
    ...
    private static func draw(
        ...
        size: CGSize,
        pose: NodoAssistMascotPose,
        palette: NodoAssistMascotPalette)
    {
    ...
    private static func gradient(
        for path: Path,
        palette: NodoAssistMascotPalette) -> GraphicsContext.Shading
    {
