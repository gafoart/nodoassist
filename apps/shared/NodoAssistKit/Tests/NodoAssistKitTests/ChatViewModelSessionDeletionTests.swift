...
private final class DeleteSessionTestTransport: @unchecked Sendable, NodoAssistChatTransport {
    ...
    func requestHistory(sessionKey: String) async throws -> NodoAssistChatHistoryPayload {
        ...
        """
        return try JSONDecoder().decode(NodoAssistChatHistoryPayload.self, from: Data(json.utf8))
    }
        ...
        attachments _: [NodoAssistChatAttachmentPayload]) async throws -> NodoAssistChatSendResponse
    {
        ...
        """
        return try JSONDecoder().decode(NodoAssistChatSendResponse.self, from: Data(json.utf8))
    }
...
struct ChatViewModelSessionDeletionTests {
    @Test func `deleting the active main session re-bootstraps in place`() async throws {
        ...
        let vm = NodoAssistChatViewModel(sessionKey: "main", transport: transport)
        vm.load()
    ...
    @Test func `deleting the active non-main session switches to main`() async throws {
        let transport = DeleteSessionTestTransport()
        let vm = NodoAssistChatViewModel(sessionKey: "scratch", transport: transport)
        vm.load()
        try await waitUntil("initial bootstrap history") {
