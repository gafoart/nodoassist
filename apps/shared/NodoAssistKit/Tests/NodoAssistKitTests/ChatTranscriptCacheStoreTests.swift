...
private func cacheSessionEntry(key: String, updatedAt: Double) -> NodoAssistChatSessionEntry {
...
private func messageTexts(_ messages: [NodoAssistChatMessage]) -> [String] {
...
private func outboxCommand(
    ...
    thinking: String = "off",
    createdAt: Double = Date().timeIntervalSince1970) -> NodoAssistChatOutboxCommand
{
...
struct ChatTranscriptCacheStoreTests {
    @Test func `verified routing identity survives a cold store reopen`() async throws {
        ...
        let identity = try #require(NodoAssistChatSessionRoutingIdentity(
            scope: " Per-Sender ",
            mainSessionKey: " Work ",
            defaultAgentID: " Main "))
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")

        await store.storeSessionRoutingIdentity(identity)
        #expect(NodoAssistChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
            databaseURL: url,
            gatewayID: "gw-a") == identity)

        let reopened = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `transcript keeps only most recent messages within bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = NodoAssistChatSQLiteTranscriptCache.maxCachedMessagesPerSession
    ...
    @Test func `sessions list is bounded to most recently updated`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = NodoAssistChatSQLiteTranscriptCache.maxCachedSessions
    ...
    @Test func `transcript eviction keeps most recent sessions`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = NodoAssistChatSQLiteTranscriptCache.maxCachedTranscripts
    ...
    @Test func `transcripts are scoped per gateway identity`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let storeA = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let storeB = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `reset retirement permits physical removal of all gateway rows`() async throws {
        ...
        await storeA.retire()
        await storeB.retire()
        NodoAssistChatSQLiteTranscriptCache.removeDatabaseFiles(at: url)
        await storeA.storeTranscript(
            sessionKey: "main",
            messages: [cacheMessage(role: "user", text: "late write", timestamp: 3)])

        let readerA = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let readerB = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-b")
    ...
    @Test func `removing one gateway database preserves another`() async throws {
        ...
        let storeA = NodoAssistChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
        let storeB = NodoAssistChatSQLiteTranscriptCache(databaseURL: urlB, gatewayID: "gw-b")
        ...
        await storeA.retire()
        NodoAssistChatSQLiteTranscriptCache.removeDatabaseFiles(at: urlA)

        let readerA = NodoAssistChatSQLiteTranscriptCache(databaseURL: urlA, gatewayID: "gw-a")
    ...
    @Test func `v1 transcript cache migrates to durable outbox schema`() async throws {
        ...
        let migrated = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `v2 migration parks rows without a routing contract`() async throws {
        ...
        #expect(commands.map(\.lastError) == [
            NodoAssistChatSQLiteTranscriptCache.outboxUnknownTargetError,
            NodoAssistChatSQLiteTranscriptCache.outboxUnknownTargetError,
            NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError,
            NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError,
            NodoAssistChatSQLiteTranscriptCache.outboxUnknownTargetError,
            NodoAssistChatSQLiteTranscriptCache.outboxUnknownTargetError,
        ])
    ...
    @Test func `unknown schema preserves durable outbox bytes and fails closed`() async throws {
        ...
        let blocked = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        ...
        #expect(sqlite3_exec(
            raw,
            "PRAGMA user_version = \(NodoAssistChatSQLiteTranscriptCache.schemaVersion)",
            nil,
            nil,
            nil) == SQLITE_OK)
        sqlite3_close_v2(raw)
        let recovered = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
    ...
    @Test func `undecodable row is dropped and treated as miss`() async throws {
        ...
        let reader = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
...
struct ChatCommandOutboxStoreTests {
    ...
    @Test func `interrupted sending rows fail closed on recovery`() async throws {
        ...
        #expect(recovered.map(\.lastError) == [NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError])
    ...
    @Test func `queued commands expire to failed at the staleness boundary`() async throws {
        ...
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let now = Date().timeIntervalSince1970
        let maxAge = NodoAssistChatSQLiteTranscriptCache.outboxCommandMaxAge
        #expect(await store.enqueueCommand(
        ...
        #expect(stale.lastError == NodoAssistChatSQLiteTranscriptCache.outboxExpiredError)
        #expect(fresh.status == .queued)
        #expect(fresh.lastError == nil)
        #expect(unconfirmed.status == .failed)
        #expect(unconfirmed.lastError == NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError)
    }
    ...
    @Test func `enqueue refuses beyond the queue bound`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let bound = NodoAssistChatSQLiteTranscriptCache.maxQueuedCommands
    ...
    @Test func `attachment byte admission enforces command and gateway bounds`() {
        let commandBound = NodoAssistChatSQLiteTranscriptCache.maxAttachmentBytesPerCommand
        let gatewayBound = NodoAssistChatSQLiteTranscriptCache.maxQueuedAttachmentBytes

        #expect(NodoAssistChatSQLiteTranscriptCache.canEnqueueAttachmentBytes(
    ...
    @Test func `enqueue persists attachment bytes and refuses a full byte budget`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let attachment = NodoAssistChatOutboxAttachment(
            type: "file",
            mimeType: "audio/mp4",
        ...
        #expect(sqlite3_exec(
            ...
            SET attachment_bytes = \(NodoAssistChatSQLiteTranscriptCache.maxQueuedAttachmentBytes)
        ...
        let fullStore = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
