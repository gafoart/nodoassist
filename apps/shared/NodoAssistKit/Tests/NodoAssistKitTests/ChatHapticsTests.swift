...
private final class HapticRecorder: @unchecked Sendable {
    ...
    private var recordedEvents: [NodoAssistChatHaptics.Event] = []

    var events: [NodoAssistChatHaptics.Event] {
        self.lock.lock()
        defer { self.lock.unlock() }
        return self.recordedEvents
    }

    func record(_ event: NodoAssistChatHaptics.Event) {
        self.lock.lock()
        defer { self.lock.unlock() }
...
private final class HapticsTestTransport: @unchecked Sendable, NodoAssistChatTransport {
    private let response: NodoAssistChatSendResponse
    ...
    init(status: String, historyMessages: [AnyCodable] = []) {
        self.response = NodoAssistChatSendResponse(runId: "run-1", status: status)
        self.historyMessages = historyMessages
        var continuation: AsyncStream<NodoAssistChatTransportEvent>.Continuation!
    ...
    HapticRecorder)
{
    ...
    let haptics = NodoAssistChatHaptics(performer: recorder.record)
    let viewModel = await MainActor.run {
        NodoAssistChatViewModel(sessionKey: "main", transport: transport, haptics: haptics)
    }
    return (transport, viewModel, recorder)
}

private func sendHapticsTestMessage(_ viewModel: NodoAssistChatViewModel) async {
    await MainActor.run {
...
struct ChatHapticsTests {
    ...
    @Test func `completion fires once for duplicate terminal events`() async throws {
        ...
        let final = NodoAssistChatTransportEvent.chat(NodoAssistChatEventPayload(
            runId: "run-1",
            sessionKey: "main",
            state: "final",
            message: nil,
