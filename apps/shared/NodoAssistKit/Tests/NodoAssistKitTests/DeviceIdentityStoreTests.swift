...
struct DeviceIdentityStoreTests {
    ...
    func `device auth store reports failed durable writes`() throws {
        ...
        setenv("NODOASSIST_STATE_DIR", blocker.path, 1)
    ...
    func `secondary profiles use separate identity and auth files`() throws {
        ...
        // getenv, not ProcessInfo: the trait pins NODOASSIST_STATE_DIR via setenv and
        // ProcessInfo.environment can serve a stale snapshot on Darwin.
        let stateDirPath = try #require(getenv("NODOASSIST_STATE_DIR").map { String(cString: $0) })
    ...
    // Regression: an explicit NODOASSIST_STATE_DIR override must never import the
