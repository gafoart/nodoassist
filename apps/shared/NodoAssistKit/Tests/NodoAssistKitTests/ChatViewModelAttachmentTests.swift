...
private actor AttachmentSendCapture {
    private(set) var attachments: [NodoAssistChatAttachmentPayload] = []

    func store(_ attachments: [NodoAssistChatAttachmentPayload]) {
    ...
    func first() -> NodoAssistChatAttachmentPayload? {
...
private struct AttachmentProcessingTransport: NodoAssistChatTransport {
        ...
        attachments: [NodoAssistChatAttachmentPayload]) async throws -> NodoAssistChatSendResponse
    {
        ...
        return NodoAssistChatSendResponse(runId: idempotencyKey, status: self.responseStatus)
    }
...
private func makeAttachmentOutbox() -> NodoAssistChatSQLiteTranscriptCache {
    NodoAssistChatSQLiteTranscriptCache(
...
private func makeDurableAttachmentViewModel(
    ...
    outbox: NodoAssistChatSQLiteTranscriptCache) -> NodoAssistChatViewModel
{
...
final class ChatViewModelAttachmentTests: XCTestCase {
    func testImageAttachmentsAreProcessedBeforeStaging() async throws {
        ...
        let viewModel = await MainActor.run {
            NodoAssistChatViewModel(sessionKey: "main", transport: AttachmentProcessingTransport())
        }
    ...
    func testPartialIdentitySyncPreservesTheOtherDeferredComponent() {
        ...
        let contractViewModel = NodoAssistChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let contractAttachment = NodoAssistPendingAttachment(
            url: nil,
        ...
        let agentViewModel = NodoAssistChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
            sessionRoutingContract: oldContract)
        let agentAttachment = NodoAssistPendingAttachment(
            url: nil,
    ...
    func testAttachmentStagingPinsSessionAndIdentityUntilItFinishes() {
        let viewModel = NodoAssistChatViewModel(
            sessionKey: "main",
            transport: AttachmentProcessingTransport(),
            activeAgentId: "main",
    ...
    func testFailedAttachmentSendWithoutOutboxRestoresDraft() async throws {
        ...
        let attachmentID = await MainActor.run {
            let attachment = NodoAssistPendingAttachment(
                url: nil,
                data: attachmentData,
                fileName: "retry.m4a",
                mimeType: "audio/mp4",
    ...
    func testVoiceNoteSendKeepsCapturedDurationWhenDraftChangesDuringHealthCheck() async throws {
        ...
        let (viewModel, draftAttachmentID) = await MainActor.run {
            let viewModel = NodoAssistChatViewModel(sessionKey: "main", transport: transport)
            let draftAttachment = NodoAssistPendingAttachment(
                url: nil,
    ...
    func testAmbiguousVoiceNoteSurvivesViewModelRecreation() async throws {
        let outbox = makeAttachmentOutbox()
        var firstViewModel: NodoAssistChatViewModel? = await MainActor.run {
            let viewModel = makeDurableAttachmentViewModel(
    ...
    func testCanonicalVoiceNoteConfirmationPreservesDurationAndDeletesDurableBytes() async throws {
        ...
        let canonical = try JSONDecoder().decode(
            ...
            from: Data(
                """
                {"role":"user","content":"See attached.","__nodoassist":{"idempotencyKey":"\(command
    ...
    func testCanonicalVoiceNotePreservesOptimisticDuration() throws {
        let localAudio = NodoAssistChatMessageContent(
            type: "file",
            text: nil,
            mimeType: "audio/mp4",
        ...
        let existing = NodoAssistChatMessage(
            role: "user",
            content: [localAudio],
            timestamp: nil,
        ...
        let incoming = try JSONDecoder().decode(
            NodoAssistChatMessage.self,
            from: Data(
                #"{"role":"user","content":"See attached.","__nodoassist":{"idempotencyKey":"run:user"},"MediaPaths":["media/inbound/media-1.m4a"],"MediaTypes":["audio/mp4"]}"#
                    .utf8))

        let adopted = NodoAssistChatViewModel.adoptingCanonicalMessage(incoming, over: existing)
