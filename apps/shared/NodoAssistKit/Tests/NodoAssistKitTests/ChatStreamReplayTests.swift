...
/// Replays scripted gateway traffic against `NodoAssistChatViewModel` with deterministic
...
private final class ScriptedChatTransport: @unchecked Sendable, NodoAssistChatTransport {
    private actor State {
        var history: NodoAssistChatHistoryPayload
        var historyRequestCount = 0
        var sentRunIds: [String] = []

        init(history: NodoAssistChatHistoryPayload) {
            self.history = history
        }

        func setHistory(_ payload: NodoAssistChatHistoryPayload) {
            self.history = payload
        }

        func recordHistoryRequest() -> NodoAssistChatHistoryPayload {
    ...
    func setHistory(_ payload: NodoAssistChatHistoryPayload) async {
    ...
    // MARK: NodoAssistChatTransport

    func requestHistory(sessionKey _: String) async throws -> NodoAssistChatHistoryPayload {
        ...
        attachments _: [NodoAssistChatAttachmentPayload]) async throws -> NodoAssistChatSendResponse
    {
        ...
        return NodoAssistChatSendResponse(runId: idempotencyKey, status: "pending")
    }
    ...
    {
        NodoAssistChatSessionsListResponse(ts: nil, path: nil, count: 0, defaults: nil, sessions: [])
    }
...
private func replayHistory(
    ...
    messages: [AnyCodable] = []) -> NodoAssistChatHistoryPayload
{
...
/// `__nodoassist.idempotencyKey` metadata used for turn correlation.
...
private func replaySessionMessageEvent(
    ...
    messageId: String) -> NodoAssistChatTransportEvent
{
    .sessionMessage(
...
private func replayFinalEvent(
    runId: String,
    text: String,
    timestamp: Double) -> NodoAssistChatTransportEvent
{
    .chat(
...
private func replayAssistantDeltaEvent(
    ...
    seq: Int) -> NodoAssistChatTransportEvent
{
    .agent(
...
private struct StreamReplayHarness {
    ...
    static func bootstrapped(
        initialHistory: NodoAssistChatHistoryPayload = replayHistory()) async throws -> StreamReplayHarness
    {
        let transport = ScriptedChatTransport(history: initialHistory)
        let vm = await MainActor.run {
            NodoAssistChatViewModel(sessionKey: "main", transport: transport)
        }
    ...
    func converge(
        _ label: String,
        _ condition: @escaping @MainActor @Sendable (NodoAssistChatViewModel) -> Bool) async throws
    {
        let vm = self.vm
...
extension NodoAssistChatViewModel {
    fileprivate var replayAssistantRows: [NodoAssistChatMessage] {
        self.messages.filter { $0.role == "assistant" }
    }

    fileprivate func replayAssistantRows(text: String) -> [NodoAssistChatMessage] {
        self.replayAssistantRows.filter { message in
            message.content.compactMap(\.text).joined() == text
        }
    }

    fileprivate var replayUserRows: [NodoAssistChatMessage] {
