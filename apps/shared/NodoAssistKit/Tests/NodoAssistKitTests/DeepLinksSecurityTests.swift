...
@Suite struct DeepLinksSecurityTests {
    ...
    @Test func gatewayDeepLinkAllowsLoopbackWs() {
        let url = URL(
            string: "nodoassist://gateway?host=127.0.0.1&port=18789&tls=0&token=abc")!
        #expect(
    ...
    @Test func setupCodeAllowsPrivateLanHostPayload() {
        let payload = #"{"host":"nodoassist.local","port":18789,"tls":false,"bootstrapToken":"tok"}"#
        #expect(
            GatewayConnectDeepLink.fromSetupCode(setupCode(from: payload)) == .init(
                host: "nodoassist.local",
                port: 18789,
