...
private func outboxTestCommand(id: String, text: String, createdAt: Double) -> NodoAssistChatOutboxCommand {
...
private func userTexts(_ vm: NodoAssistChatViewModel) async -> [String] {
    await MainActor.run {
        vm.messages
            .filter { $0.role == "user" }
...
private final class OutboxTestTransport: @unchecked Sendable, NodoAssistChatTransport {
    let state: OutboxTransportState
    private let sessions: [NodoAssistChatSessionEntry]
    ...
    init(
        ...
        sessions: [NodoAssistChatSessionEntry] = [],
    ...
    func emit(_ event: NodoAssistChatTransportEvent) {
    ...
    func listCommands(sessionKey _: String) async throws -> [NodoAssistChatCommandChoice] {
    ...
    private func requestHistory(
        sessionKey: String,
        agentID: String?,
        expectedRoute: Int?) async throws -> NodoAssistChatHistoryPayload
    {
        ...
        if let stale = await state.staleHistoryRows {
            ...
            return NodoAssistChatHistoryPayload(
        ...
        let durableUserRows = keys.indices.compactMap { index -> AnyCodable? in
            ...
            return AnyCodable([
                ...
                "__nodoassist": ["idempotencyKey": "\(key):user"],
            ] as [String: Any])
        }
        return NodoAssistChatHistoryPayload(
            sessionKey: sessionKey,
    ...
    private func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        expectedRoute: Int?) async throws -> NodoAssistChatSendResponse
    {
        if let expectedRoute, await state.routeGeneration != expectedRoute {
            throw NodoAssistChatTransportSendError.notDispatched
        }
        ...
        if await self.state.sendRoutingChanged {
            throw GatewayResponseError(
                ...
                details: [
                    "reason": AnyCodable(NodoAssistChatSessionRoutingContract.changedErrorReason),
                ])
        }
        if await self.state.sendRejects {
            // Gateway responded but refused to start the run.
            return NodoAssistChatSendResponse(runId: idempotencyKey, status: "error")
        }
        ...
        return NodoAssistChatSendResponse(runId: idempotencyKey, status: "accepted")
    }

    func acquireOutboxRouteLease() async -> NodoAssistChatTransportRouteLeaseResult {
        if let routeUnavailableReason {
            return .unavailable(reason: routeUnavailableReason)
        }
        if !self.requiresRoutingContract {
            let transport = self
            return .available(NodoAssistChatTransportRouteLease(
    ...
    func listSessions(
        limit _: Int?,
        search _: String?,
        archived _: Bool) async throws -> NodoAssistChatSessionsListResponse
    {
...
private func outboxSessionEntry(
    key: String,
    thinkingLevels: [String]) -> NodoAssistChatSessionEntry
{
    NodoAssistChatSessionEntry(
        ...
        model: nil,
        contextTokens: nil,
        thinkingLevels: thinkingLevels.map { NodoAssistChatThinkingLevelOption(id: $0, label: $0) })
}

private func makeOutboxViewModel(
    transport: OutboxTestTransport,
    outbox: any NodoAssistChatCommandOutbox,
    transcriptCache: (any NodoAssistChatTranscriptCache)? = nil,
    retryDelaysMs: [UInt64] = [1, 1],
    sessionKey: String = "main",
    activeAgentID: String? = "main",
    sessionRoutingContract: String? = "per-sender|main|main") async -> NodoAssistChatViewModel
{
    await MainActor.run {
...
private func sendWhileOffline(_ vm: NodoAssistChatViewModel, text: String) async throws {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
@MainActor
private func queuedStateCount(_ vm: NodoAssistChatViewModel) -> Int {
...
private actor DelayingOutbox: NodoAssistChatCommandOutbox {
    private nonisolated let base: NodoAssistChatSQLiteTranscriptCache
    ...
    init(base: NodoAssistChatSQLiteTranscriptCache) {
        self.base = base
    }

    nonisolated func changes() -> AsyncStream<NodoAssistChatOutboxChange> {
        self.base.changes()
    }
...
private actor SnapshotHoldingOutbox: NodoAssistChatCommandOutbox {
    private nonisolated let base: NodoAssistChatSQLiteTranscriptCache
...
private actor CancellationHoldingOutbox: NodoAssistChatCommandOutbox {
    private nonisolated let base: NodoAssistChatSQLiteTranscriptCache
...
struct ChatViewModelOutboxTests {
    @Test func `offline send queues durably and renders queued row`() async throws {
        ...
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
    ...
    @Test func `unsupported gateway keeps queued work and surfaces upgrade action`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let message = NodoAssistChatTransportUpgradeMessage.routingContract
    ...
    @Test func `inert outbox does not capability gate healthy live chat`() async throws {
        ...
        let transport = OutboxTestTransport(
            healthy: true,
            routeUnavailableReason: NodoAssistChatTransportUpgradeMessage.routingContract)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        ...
        parked.lastError = NodoAssistChatSQLiteTranscriptCache.outboxChangedTargetError
    ...
    @Test func `changed default agent parks command visibly for retry`() async throws {
        ...
        #expect(await store.loadCommands().first?.lastError ==
            NodoAssistChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `atomic gateway routing rejection parks without retrying`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == NodoAssistChatSQLiteTranscriptCache.outboxChangedTargetError)
    ...
    @Test func `failed alias row remains reachable after owner change`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.enqueueCommand(NodoAssistChatOutboxCommand(
            id: "c-old-failure",
            sessionKey: "main",
            ...
            status: .failed,
            retryCount: 1,
            lastError: NodoAssistChatSQLiteTranscriptCache.outboxExpiredError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `ownerless global retry stays failed without a selected agent`() async throws {
        ...
        #expect(await store.enqueueCommand(NodoAssistChatOutboxCommand(
            ...
            status: .failed,
            retryCount: 0,
            lastError: NodoAssistChatSQLiteTranscriptCache.outboxUnknownTargetError)))
        let transport = OutboxTestTransport(healthy: false)
        let vm = await makeOutboxViewModel(
            transport: transport,
            outbox: store,
    ...
    @Test func `delete race preserves a turn already proven by canonical history`() async throws {
        ...
        let canonicalMessage = NodoAssistChatMessage(
            role: "user",
            content: [NodoAssistChatMessageContent(
                type: "text",
                text: "delivered already",
                mimeType: nil,
                fileName: nil,
    ...
    @Test func `gateway rejections burn attempts then fail terminally and support tap retry`() async throws {
        ...
        #expect(failed.retryCount == NodoAssistChatViewModel.maxOutboxSendAttempts)
    ...
    @Test func `unavailable terminal write drops health instead of advancing FIFO`() async throws {
        ...
        #expect(await store.enqueueCommand(NodoAssistChatOutboxCommand(
            ...
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: NodoAssistChatViewModel.maxOutboxSendAttempts - 1,
            lastError: "rejected")))
        let outbox = DelayingOutbox(base: store)
    ...
    @Test func `ambiguous live transport failure requires explicit retry`() async throws {
        ...
        #expect(preserved.lastError == NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError)
    ...
    @Test func `tap retry refreshes createdAt so an expired command can resend`() async throws {
        ...
        let staleCreatedAt = Date().timeIntervalSince1970 -
            NodoAssistChatSQLiteTranscriptCache.outboxCommandMaxAge - 60
        ...
        #expect(await store.loadCommands().map(\.lastError) == [
            NodoAssistChatSQLiteTranscriptCache.outboxExpiredError,
        ])
    ...
    @Test func `background canonical alias event confirms by idempotency key`() async throws {
        ...
        transport.emit(.sessionMessage(NodoAssistSessionMessageEventPayload(
    ...
    @Test func `full queue refuses enqueue and keeps the draft`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        for index in 0..<NodoAssistChatSQLiteTranscriptCache.maxQueuedCommands {
        ...
        #expect(await userTexts(vm).isEmpty)
        #expect(await store.loadCommands().count == NodoAssistChatSQLiteTranscriptCache.maxQueuedCommands)
    }
    ...
    @Test func `queued send transport failure fails closed until explicit retry`() async throws {
        ...
        let command = try #require(await store.loadCommands().first)
        #expect(command.lastError == NodoAssistChatSQLiteTranscriptCache.outboxUnconfirmedError)
        #expect(command.retryCount == 0)
        #expect(await transport.state.sentIdempotencyKeys.isEmpty)
