    ...
    idempotencyKey: String? = nil) -> AnyCodable
{
    ...
    if let idempotencyKey {
        message["__nodoassist"] = ["idempotencyKey": idempotencyKey]
    }
...
private func chatTextModelMessage(
    ...
    text: String,
    timestamp: Double,
    idempotencyKey: String? = nil) -> NodoAssistChatMessage
{
    NodoAssistChatMessage(
        role: role,
        content: [
...
extension [NodoAssistChatMessage] {
...
private func historyPayload(
    ...
    inFlightRun: NodoAssistChatInFlightRun? = nil) -> NodoAssistChatHistoryPayload
{
    NodoAssistChatHistoryPayload(
        sessionKey: sessionKey,
        sessionId: sessionId,
        messages: messages,
        thinkingLevel: "off",
        sessionInfo: supportsActiveRunState
            ? NodoAssistChatSessionInfo(hasActiveRun: hasActiveRun ?? (inFlightRun != nil))
            : nil,
...
private func sessionEntry(
    ...
    contextTokens: Int? = nil) -> NodoAssistChatSessionEntry
{
...
private func sessionsListResponse(_ sessions: [NodoAssistChatSessionEntry]) -> NodoAssistChatSessionsListResponse {
    NodoAssistChatSessionsListResponse(
        ts: nil,
        path: nil,
...
private func thinkingOption(_ id: String, label: String? = nil) -> NodoAssistChatThinkingLevelOption {
    NodoAssistChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    thinkingLevels: [NodoAssistChatThinkingLevelOption]? = nil,
    thinkingOptions: [String]? = nil,
    thinkingDefault: String? = nil) -> NodoAssistChatSessionEntry
{
    NodoAssistChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(
    ...
    reasoning: Bool? = nil) -> NodoAssistChatModelChoice
{
    NodoAssistChatModelChoice(
        modelID: id,
        name: name,
        provider: provider,
        contextWindow: nil,
...
private func sessionsResponse(
    _ session: NodoAssistChatSessionEntry,
    defaults: NodoAssistChatSessionsDefaults? = nil) -> NodoAssistChatSessionsListResponse
{
    NodoAssistChatSessionsListResponse(
        ts: 1,
        path: nil,
...
private func commandChoice(
    ...
    aliases: [String],
    description: String = "",
    source: NodoAssistChatCommandChoice.Source = .command,
    acceptsArgs: Bool = false) -> NodoAssistChatCommandChoice
{
    NodoAssistChatCommandChoice(
...
private func makeViewModel(
    ...
    historyResponses: [NodoAssistChatHistoryPayload],
    sessionRoutingContract: String? = nil,
    sessionsResponses: [NodoAssistChatSessionsListResponse] = [],
    modelResponses: [[NodoAssistChatModelChoice]] = [],
    commandResponses: [[NodoAssistChatCommandChoice]] = [],
    requestHistoryHook: (@Sendable (String) async throws -> Void)? = nil,
    historyResponseHook: (@Sendable (String, Int, [String]) async throws -> NodoAssistChatHistoryPayload?)? = nil,
    ...
    listSessionsHook: (
        @Sendable (TestSessionListQuery) async throws -> NodoAssistChatSessionsListResponse?)? = nil,
    sendMessageHook: (@Sendable (String) async throws -> NodoAssistChatSendResponse)? = nil,
    ...
    -> (TestChatTransport, NodoAssistChatViewModel)
{
    ...
    let vm = NodoAssistChatViewModel(
        sessionKey: sessionKey,
        transport: transport,
...
private func sendUserMessage(_ vm: NodoAssistChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, NodoAssistChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [NodoAssistChatHistoryPayload]
    private let sessionsResponses: [NodoAssistChatSessionsListResponse]
    private let modelResponses: [[NodoAssistChatModelChoice]]
    private let commandResponses: [[NodoAssistChatCommandChoice]]
    private let requestHistoryHook: (@Sendable (String) async throws -> Void)?
    private let historyResponseHook:
        (@Sendable (String, Int, [String]) async throws -> NodoAssistChatHistoryPayload?)?
    ...
    private let listSessionsHook:
        (@Sendable (TestSessionListQuery) async throws -> NodoAssistChatSessionsListResponse?)?
    private let sendMessageHook: (@Sendable (String) async throws -> NodoAssistChatSendResponse)?
    ...
    private let stream: AsyncStream<NodoAssistChatTransportEvent>
    private let continuation: AsyncStream<NodoAssistChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<NodoAssistChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<NodoAssistChatTransportEvent> {
        self.stream
    }
    ...
    func createSession(
        ...
        worktree _: Bool?) async throws -> NodoAssistChatCreateSessionResponse
    {
        ...
        return NodoAssistChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }

    func requestHistory(sessionKey: String) async throws -> NodoAssistChatHistoryPayload {
        ...
        return self.historyResponses.last ?? NodoAssistChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments: [NodoAssistChatAttachmentPayload]) async throws -> NodoAssistChatSendResponse
    {
    ...
    func sendMessage(
        ...
        thinking: String,
        idempotencyKey: String,
        attachments _: [NodoAssistChatAttachmentPayload]) async throws -> NodoAssistChatSendResponse
    {
        ...
        return NodoAssistChatSendResponse(runId: idempotencyKey, status: self.sendMessageStatus)
    }
    ...
    func listSessions(
        limit: Int?,
        search: String?,
        archived: Bool) async throws -> NodoAssistChatSessionsListResponse
    {
        ...
        return self.sessionsResponses.last ?? NodoAssistChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [NodoAssistChatModelChoice] {
    ...
    func listCommands(sessionKey: String) async throws -> [NodoAssistChatCommandChoice] {
    ...
    func emit(_ evt: NodoAssistChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test func `context usage fraction validates freshness and token bounds`() {
        func fraction(total: Int?, fresh: Bool? = true, context: Int?) -> Double? {
            NodoAssistChatViewModel.chatContextUsageFraction(
                for: sessionEntry(
                    key: "main",
                    updatedAt: 1,
                    totalTokens: total,
    ...
    @Test @MainActor func `event listener does not retain discarded view model`() async throws {
        ...
        var viewModel: NodoAssistChatViewModel? = NodoAssistChatViewModel(
            sessionKey: "main",
            transport: transport)
    ...
    @Test func `decodes in-flight run from chat history`() throws {
        ...
        let payload = try JSONDecoder().decode(NodoAssistChatHistoryPayload.self, from: data)
    ...
    @Test func `decodes agent scope from chat event`() throws {
        ...
        let payload = try JSONDecoder().decode(NodoAssistChatEventPayload.self, from: data)
    ...
    @Test func `bootstrap adopts active history run and consumes live events`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "partial reply"))
    ...
    @Test func `adopts Codex history run without buffered text`() async throws {
        let history = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-codex", text: ""))
    ...
    @Test func `foreground history refreshes adopted run snapshot`() async throws {
        let firstHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "first partial"))
        let resumedHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "resumed partial"))
        ...
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                return historyPayload(
                    ...
                    inFlightRun: NodoAssistChatInFlightRun(runId: runId, text: "working"))
            },
    ...
    @Test func `foreground discovers run started without local ownership`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-external", text: "external partial"))
    ...
    @Test func `foreground keeps active run with intermediate assistant history`() async throws {
        ...
        let activeHistory = historyPayload(
            ...
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-tool", text: "still working"))
        let (_, vm) = await makeViewModel(historyResponses: [idleHistory, activeHistory])
    ...
    @Test func `foreground clears completed run without assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-quiet", text: ""))
        let completedHistory = historyPayload(
            messages: [chatTextMessage(role: "user", text: "quiet task", timestamp: 1)])
    ...
    @Test func `foreground missing snapshot does not clear an in-flight send`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "accepted"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), historyPayload(), activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            sendMessageHook: { _ in
                await sendGate.wait()
                return NodoAssistChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `post-send history keeps active run with intermediate assistant output`() async throws {
        let activeHistory = historyPayload(
            ...
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "working"))
        let (_, vm) = await makeViewModel(
            historyResponses: [historyPayload(), activeHistory],
            sendMessageHook: { _ in
                NodoAssistChatSendResponse(runId: "run-active", status: "pending")
            })
    ...
    @Test func `live chat delta owns run while bootstrap history is pending`() async throws {
        ...
        let staleHistory = historyPayload(
            ...
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-stale", text: "stale partial"))
        let (transport, vm) = await makeViewModel(
    ...
    @Test func `global chat delta adopts only selected agent run`() async throws {
        let bareGlobalMatches = await MainActor.run {
            (
                NodoAssistChatViewModel.matchesCurrentSessionKey(
                    incoming: "global",
                    agentId: "work",
                    current: "global",
                    mainSessionKey: "main",
    ...
    @Test func `intermediate session message preserves pending recovery snapshot`() async throws {
        ...
        let historyCalls = AsyncCounter()
        let activeHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "active partial"))
        let (transport, vm) = await makeViewModel(
        ...
        transport.emit(
            .sessionMessage(
                NodoAssistSessionMessageEventPayload(
                    sessionKey: "main",
    ...
    @Test func `manual refresh re-adopts active run after clearing local ownership`() async throws {
        ...
        let refreshedHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "refreshed partial"))
    ...
    @Test func `older history cannot replace newer run snapshot`() async throws {
        ...
        let initialHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-initial", text: "initial"))
        let (_, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    return historyPayload(
                        inFlightRun: NodoAssistChatInFlightRun(runId: "run-older", text: "older"))
                }
                if index == 2 {
                    return historyPayload(
                        inFlightRun: NodoAssistChatInFlightRun(runId: "run-newer", text: "newer"))
                }
                return nil
            })
    ...
    @Test func `delayed history cannot overwrite newer live run text`() async throws {
        ...
        let activeHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "initial"))
        let (transport, vm) = await makeViewModel(
            historyResponses: [activeHistory],
            requestHistoryHook: { _ in _ = await historyCalls.increment() },
            historyResponseHook: { _, index, _ in
                ...
                return historyPayload(
                    inFlightRun: NodoAssistChatInFlightRun(runId: "run-active", text: "stale"))
            })
    ...
    @Test func `external terminal event protects current run from delayed snapshot`() async throws {
        ...
        let currentHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-current", text: "current"))
        let (transport, vm) = await makeViewModel(
            ...
            historyResponseHook: { _, index, _ in
                if index == 1 {
                    ...
                    _ = await staleCompletions.increment()
                    return historyPayload(
                        inFlightRun: NodoAssistChatInFlightRun(runId: "run-finished", text: "stale"))
                }
                if index == 2 {
                    throw NSError(domain: "test", code: 1)
                }
    ...
    @Test func `sequence gap re-adopts active run from history`() async throws {
        ...
        let recoveredHistory = historyPayload(
            inFlightRun: NodoAssistChatInFlightRun(runId: "run-recovered", text: "recovered partial"))
    ...
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> NodoAssistChatMessage {
        ...
        #expect(
            NodoAssistChatMessage.displayText(
                contentText: "",
                role: assistantError.role,
                stopReason: assistantError.stopReason,
                errorMessage: assistantError.errorMessage) == "stale provider failure")
    ...
    @Test func `duplicate final events append one provisional reply`() async throws {
        ...
        await sendUserMessage(vm, text: "hello")
        let runId = try await waitForLastSentRunId(transport)
        let final = NodoAssistChatEventPayload(
            runId: runId,
            sessionKey: "main",
            state: "final",
            message: chatTextMessage(
    ...
    @Test func `later identical session reply does not adopt prior turn provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                NodoAssistChatSendResponse(runId: runId, status: "pending")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `rekeys optimistic user message when gateway reuses active run`() async throws {
        ...
        let (_, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                ...
                return NodoAssistChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `canonical projected reply after steering adopts reused provisional final`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { _ in
                NodoAssistChatSendResponse(runId: remoteRunId, status: "in_flight")
            })
        try await loadAndWaitBootstrap(vm: vm, sessionId: sessionId)
    ...
    @Test func `terminal timeout send ack surfaces error and allows next send`() async throws {
        ...
        let (transport, vm) = await makeViewModel(
            ...
            sendMessageHook: { runId in
                ...
                return NodoAssistChatSendResponse(
                    runId: runId,
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(NodoAssistChatTransportEvent.seqGap)
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let history = historyPayload()
        let sessions = NodoAssistChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `session choices use resolved main session key instead of literal main`() async throws {
        ...
        let sessions = NodoAssistChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: NodoAssistChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func `new trigger falls back to reset when create session is unsupported`() async throws {
        ...
        let unsupported = NSError(
            domain: "NodoAssistChatTransport",
            code: 0,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = NodoAssistChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: NodoAssistChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(NodoAssistChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == NodoAssistChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = NodoAssistChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: NodoAssistChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
            ...
            historyResponseHook: { _, index, sentRunIds in
                ...
                if (1...3).contains(index) {
                    ...
                    return historyPayload(
                        ...
                        sessionId: sessionId,
                        messages: [chatTextMessage(role: "user", text: "hello", timestamp: now)],
                        inFlightRun: NodoAssistChatInFlightRun(runId: runId, text: ""))
                }
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = NodoAssistChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = NodoAssistChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = NodoAssistChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            NodoAssistChatSessionsListResponse.self,
            from: Data(json.utf8))
        ...
        try await waitUntil("models loaded with default selection") {
            await MainActor.run {
                vm.modelChoices.count == 2 &&
                    vm.modelSelectionID == NodoAssistChatViewModel.defaultModelSelectionID
            }
        }
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { NodoAssistChatViewModel(sessionKey: "main", transport: transport) }
...
struct ChatViewModelSessionManagementTests {
    @Test @MainActor func `session list organizer orders pinned first with key tiebreak`() {
        let organized = NodoAssistChatSessionListOrganizer.organize([
    ...
    @Test @MainActor func `session list organizer filters across display fields`() {
        ...
        let matched = NodoAssistChatSessionListOrganizer.filter(sessions, search: "TRIP")
        #expect(matched.map(\.key) == ["agent:main:topic-a", "agent:main:trip-notes"])
        #expect(NodoAssistChatSessionListOrganizer.filter(sessions, search: "  ") == sessions)
    }
