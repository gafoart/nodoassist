...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(nodoAssistVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(nodoAssistVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(nodoAssistVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(nodoAssistVoiceNoteDurationLabel(-1), "0:00")
    }

    @MainActor
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = NodoAssistVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = NodoAssistChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = NodoAssistVoiceNoteButton(
            control: control,
            compact: false,
