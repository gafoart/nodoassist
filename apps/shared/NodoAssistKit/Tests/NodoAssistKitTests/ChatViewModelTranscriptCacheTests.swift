...
private func cachedMessage(role: String, text: String, timestamp: Double) -> NodoAssistChatMessage {
...
private func visibleTexts(_ vm: NodoAssistChatViewModel) async -> [String] {
    await MainActor.run { vm.messages.map { $0.content.compactMap(\.text).joined() } }
}

private func cachedSessionEntry(key: String, updatedAt: Double) -> NodoAssistChatSessionEntry {
...
private actor TestTranscriptCache: NodoAssistChatTranscriptCache {
    private var transcripts: [String: [NodoAssistChatMessage]]
    private var sessions: [NodoAssistChatSessionEntry]
    private let loadSessionsHook: (@Sendable () async -> Void)?
    private(set) var storedTranscriptSessionKeys: [String] = []
    private(set) var storedTranscripts: [[NodoAssistChatMessage]] = []
    private(set) var storedSessionsCallCount = 0

    init(
        transcripts: [String: [NodoAssistChatMessage]] = [:],
...
private final class GatedHistoryChatTransport: @unchecked Sendable, NodoAssistChatTransport {
    private let historyGate: AsyncStream<Void>.Continuation?
    private let historyGateStream: AsyncStream<Void>?
    private let historyResult: @Sendable (String, Int) throws -> NodoAssistChatHistoryPayload
    ...
    init(
        gated: Bool,
        historyResult: @escaping @Sendable (String, Int) throws -> NodoAssistChatHistoryPayload)
    {
        ...
        var eventCont: AsyncStream<NodoAssistChatTransportEvent>.Continuation!
    ...
    {
        NodoAssistChatSendResponse(runId: idempotencyKey, status: "accepted")
    }
...
struct ChatViewModelTranscriptCacheTests {
    @Test func `cold open paints cached transcript then live history replaces it`() async throws {
        ...
        let vm = await MainActor.run {
            NodoAssistChatViewModel(sessionKey: "main", transport: transport, transcriptCache: cache)
        }
