...
struct GatewayErrorsTests {
    ...
    @Test func `connect auth error preserves structured metadata`() {
        let error = GatewayConnectAuthError(
            ...
            actionCommand: "nodoassist devices approve req-123",
        ...
        #expect(error.actionCommand == "nodoassist devices approve req-123")
    ...
    @Test func `protocol mismatch maps older app to update problem`() {
        ...
        #expect(problem?.message == "This app is older than the gateway. Update NodoAssist on this device, then retry.")
    ...
    @Test func `protocol mismatch maps older gateway to update problem`() {
        ...
        #expect(problem?
            .message == "The gateway is older than this app. Update NodoAssist on the gateway host, then retry.")
        #expect(problem?.actionLabel == "Copy update command")
        #expect(problem?.actionCommand == "nodoassist update")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `protocol mismatch without versions still gives actionable fallback`() {
        ...
        #expect(problem?
            .message == "The app and gateway use incompatible protocol versions. Update NodoAssist on both, then retry.")
        #expect(problem?.retryable == false)
        #expect(problem?.pauseReconnect == true)
    }
    ...
    @Test func `pairing problem uses structured request metadata`() {
        ...
        #expect(problem?.pauseReconnect == true)
        #expect(problem?.actionCommand == "nodoassist devices approve req-123")
    }
