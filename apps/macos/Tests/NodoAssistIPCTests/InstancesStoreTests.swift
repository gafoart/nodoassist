...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: NodoAssistProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: NodoAssistProtocol.AnyCodable] = [
            "presence": .init([NodoAssistProtocol.AnyCodable(entry)]),
        ]
        let payload = NodoAssistProtocol.AnyCodable(payloadMap)
