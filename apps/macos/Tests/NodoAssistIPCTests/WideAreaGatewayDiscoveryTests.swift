import Foundation
import Testing
@testable import NodoAssistDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() async {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._nodoassist-gw._tcp.nodoassist.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.nodoassist.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (NodoAssist)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/nodoassist/src/entry.ts\""
                }
                return ""
            })
        ...
        let beacons = await TestIsolation.withEnvValues(
            ["NODOASSIST_WIDE_AREA_DOMAIN": "nodoassist.internal"])
        {
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (NodoAssist)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/nodoassist/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._nodoassist-gw._tcp.nodoassist.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
