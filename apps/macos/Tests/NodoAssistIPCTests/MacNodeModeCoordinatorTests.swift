...
struct MacNodeModeCoordinatorTests {
    ...
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(NodoAssistCapability.browser.rawValue))
        #expect(!commands.contains(NodoAssistBrowserCommand.proxy.rawValue))
        #expect(commands.contains(NodoAssistCanvasCommand.present.rawValue))
