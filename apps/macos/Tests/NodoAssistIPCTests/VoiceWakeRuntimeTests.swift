...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "nodoassist"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["nodoassist"]
        let text = "hey nodoassist"
    ...
    @Test func `trigger only rejects trailing wake word mentions in ordinary speech`() {
        let triggers = ["nodoassist"]
        let text = "tell me about nodoassist"
        #expect(!VoiceWakeRuntime._testIsTriggerOnly(text, triggers: triggers))
    }
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["nodoassist"]
        let text = "uh nodoassist"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "nodoassist")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["nodoassist", "hey nodoassist"]
        let text = "hey nodoassist"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey nodoassist")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey nodoassist do thing"
        let config = WakeWordGateConfig(triggers: ["nodoassist"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["nodoassist"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "nodoassist")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "nodoassist tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["nodoassist", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["nodoassist", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "nodoassist")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["nodoassist"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "nodoassist", start: 0.2, duration: 0.1, range: transcript.range(of: "nodoassist")),
