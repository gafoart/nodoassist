...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers nodo assist binary`() throws {
        ...
        let cmd = CommandResolver.nodoassistCommand(
            subcommand: "gateway",
            defaults: defaults,
            configRoot: [:],
            searchPaths: searchPaths,
            projectRoot: tmp)
        #expect(cmd.prefix(2).elementsEqual([nodoassistPath.path, "gateway"]))
    }
    ...
    @Test func `falls back to node and script`() throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/nodoassist.js")
    ...
    @Test func `prefers nodo assist binary over pnpm`() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()

        let binDir = tmp.appendingPathComponent("bin")
        let nodoassistPath = binDir.appendingPathComponent("nodoassist")
        ...
        #expect(cmd.prefix(2).elementsEqual([nodoassistPath.path, "rpc"]))
    }

    @Test func `uses nodo assist binary without node runtime`() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "nodoassist", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "nodoassist", "health", "--json"]))
    ...
    @Test func `managed install only precedes external installs after validation`() throws {
        ...
        let managedBin = home.appendingPathComponent(".nodoassist/bin")
        try FileManager().createDirectory(at: managedBin, withIntermediateDirectories: true)
        let managedExecutable = managedBin.appendingPathComponent("nodoassist")
    ...
    @Test func `validated CLI preference expires when the app requires a newer version`() throws {
        ...
        #expect(CommandResolver.validatedNodoAssistExecutable(
            defaults: defaults,
            fileManager: .default,
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("nodoassist@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/nodoassist", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "nodoassist@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/nodoassist'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("nodoassist"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == nodoassistPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
