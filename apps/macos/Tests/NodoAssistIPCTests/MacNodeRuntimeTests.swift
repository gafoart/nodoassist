...
struct MacNodeRuntimeTests {
    ...
    final class ScreenSnapshotProbeServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
        typealias SnapshotResult = (
            data: Data,
            format: NodoAssistScreenSnapshotFormat,
            width: Int,
            height: Int)
        ...
        func snapshotScreen(
            ...
            maxWidth: Int?,
            quality: Double?,
            format: NodoAssistScreenSnapshotFormat?) async throws -> SnapshotResult
        {
            ...
            outPath: String?) async throws -> (path: String, hasAudio: Bool)
        {
            let url = FileManager().temporaryDirectory
                .appendingPathComponent("nodoassist-test-screen-record-\(UUID().uuidString).mp4")
    ...
    @Test func `handle invoke rejects empty system run`() async throws {
        let runtime = MacNodeRuntime()
        let params = NodoAssistSystemRunParams(command: [])
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2", command: NodoAssistSystemCommand.run.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }
    ...
    @Test func `system run denied event preserves gateway run id`() async throws {
        ...
        try await TestIsolation.withEnvValues(["NODOASSIST_STATE_DIR": stateDir.path]) {
            let probe = ExecEventProbe()
            let runtime = MacNodeRuntime()
            await runtime.setEventSender { event, json in
                await probe.append(event: event, json: json)
            }
            let params = NodoAssistSystemRunParams(
            ...
            let response = await runtime.handleInvoke(
                BridgeInvokeRequest(
                    id: "req-run-id",
                    command: NodoAssistSystemCommand.run.rawValue,
                    paramsJSON: json))
    ...
    @Test func `handle invoke rejects blocked system run env override before execution`() async throws {
        ...
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2c", command: NodoAssistSystemCommand.run.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    ...
    @Test func `handle invoke rejects invalid system run env override key before execution`() async throws {
        ...
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2d", command: NodoAssistSystemCommand.run.rawValue, paramsJSON: json))
        #expect(response.ok == false)
        #expect(response.error?.message.contains("SYSTEM_RUN_DENIED: environment override rejected") == true)
    ...
    @Test func `handle invoke rejects empty system which`() async throws {
        let runtime = MacNodeRuntime()
        let params = NodoAssistSystemWhichParams(bins: [])
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-2b", command: NodoAssistSystemCommand.which.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }

    @Test func `handle invoke rejects empty notification`() async throws {
        let runtime = MacNodeRuntime()
        let params = NodoAssistSystemNotifyParams(title: "", body: "")
        let json = try String(data: JSONEncoder().encode(params), encoding: .utf8)
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(id: "req-3", command: NodoAssistSystemCommand.notify.rawValue, paramsJSON: json))
        #expect(response.ok == false)
    }
    ...
    @Test func `handle invoke camera list requires enabled camera`() async {
        await TestIsolation.withUserDefaultsValues([cameraEnabledKey: false]) {
            let runtime = MacNodeRuntime()
            let response = await runtime.handleInvoke(
                BridgeInvokeRequest(id: "req-4", command: NodoAssistCameraCommand.list.rawValue))
            #expect(response.ok == false)
    ...
    @Test func `handle invoke screen record uses injected services`() async throws {
        ...
        final class FakeMainActorServices: MacNodeRuntimeMainActorServices, @unchecked Sendable {
            func snapshotScreen(
                ...
                maxWidth: Int?,
                quality: Double?,
                format: NodoAssistScreenSnapshotFormat?) async throws
                -> (data: Data, format: NodoAssistScreenSnapshotFormat, width: Int, height: Int)
            {
                _ = screenIndex
                _ = maxWidth
                _ = quality
    ...
    @Test func `handle invoke browser proxy uses injected request`() async {
        ...
        let response = await runtime.handleInvoke(
            BridgeInvokeRequest(
                id: "req-browser",
                command: NodoAssistBrowserCommand.proxy.rawValue,
