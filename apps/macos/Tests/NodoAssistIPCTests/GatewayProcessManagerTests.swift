...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["NODOASSIST_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "nodoassist-gateway",
                executablePath: "/tmp/nodoassist-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 nodoassist-gateway @ /tmp/nodoassist-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
