import Foundation
import Testing
@testable import NodoAssistMacCLI
...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-configure-remote-\(UUID().uuidString).json")
        ...
        try await TestIsolation.withIsolatedState(env: ["NODOASSIST_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "nodoassist.connectionMode") == "remote")
                #expect(defaults.string(forKey: "nodoassist.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "nodoassist.onboardingSeen") == true)
                #expect(defaults.string(forKey: "nodoassist.remoteCliPath") == "/opt/homebrew/bin/nodoassist")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote defaults SSH host key policy to strict`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-configure-remote-strict-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["NODOASSIST_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
