...
@Suite(.serialized)
struct NodoAssistConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-config-\(UUID().uuidString)")
            .appendingPathComponent("nodoassist.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["NODOASSIST_CONFIG_PATH": override]) {
            #expect(NodoAssistConfigFile.url().path == override)
        }
    }
    ...
    func `browser control enabled reads config flag`() async {
        ...
        await TestIsolation.withEnvValues(["NODOASSIST_CONFIG_PATH": override]) {
            #expect(NodoAssistConfigFile.browserControlEnabled() == true)
            NodoAssistConfigFile.saveDict(["browser": ["enabled": false]])
            #expect(NodoAssistConfigFile.browserControlEnabled() == false)
            NodoAssistConfigFile.setBrowserControlEnabled(true)
            #expect(NodoAssistConfigFile.browserControlEnabled() == true)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["NODOASSIST_CONFIG_PATH": override]) {
            ...
            #expect(NodoAssistConfigFile.remoteGatewayPort() == 19999)
            #expect(NodoAssistConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(NodoAssistConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(NodoAssistConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(NodoAssistConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(NodoAssistConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["NODOASSIST_CONFIG_PATH": override]) {
            ...
            NodoAssistConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = NodoAssistConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["NODOASSIST_CONFIG_PATH": override]) {
            ...
            NodoAssistConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = NodoAssistConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["NODOASSIST_CONFIG_PATH": override]) {
            ...
            NodoAssistConfigFile.clearRemoteGatewayUrl()
            let root = NodoAssistConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "NODOASSIST_CONFIG_PATH": nil,
            "NODOASSIST_STATE_DIR": dir,
        ]) {
            #expect(NodoAssistConfigFile.stateDirURL().path == dir)
            #expect(NodoAssistConfigFile.url().path == "\(dir)/nodoassist.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("nodoassist-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("nodoassist.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "NODOASSIST_STATE_DIR": stateDir.path,
            "NODOASSIST_CONFIG_PATH": configPath.path,
        ]) {
            NodoAssistConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-nodoassist-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = NodoAssistConfigFile.loadDict()
    ...
    func `load dict ignores legacy config health sidecar`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try NodoAssistConfigFile.withTestingFileLock {
                let loaded = NodoAssistConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try NodoAssistConfigFile.withTestingFileLock {
                ...
                _ = NodoAssistConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = NodoAssistConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
