...
struct ChatTranscriptCacheIdentityTests {
    private static let defaultStateDir = URL(fileURLWithPath: "/Users/tester/.nodoassist", isDirectory: true)
    ...
    @Test @MainActor func `windows share one outbox owner per gateway`() {
        let databaseURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("nodoassist-cache-owner-\(UUID().uuidString).sqlite")
    ...
    @Test func `local mode keys on state dir so profiles never collide`() {
        ...
        let devProfile = MacChatTranscriptCache.gatewayID(
            mode: .local,
            localStateDir: URL(fileURLWithPath: "/Users/tester/.nodoassist-dev", isDirectory: true),
            remoteTransport: .ssh,
            directURL: nil,
            sshTarget: "",
            sshRemotePort: 18789)
        #expect(defaultProfile == "local:/Users/tester/.nodoassist")
        #expect(devProfile == "local:/Users/tester/.nodoassist-dev")
    ...
    @Test func `local state dir aliases resolve to one identity`() throws {
        ...
        let canonical = try FileManager.default.temporaryDirectory
            .appendingPathComponent("nodoassist-cache-identity-\(UUID().uuidString)", isDirectory: true)
