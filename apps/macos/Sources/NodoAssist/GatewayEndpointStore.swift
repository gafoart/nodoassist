...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.nodoassist", category: "gateway-endpoint")
    ...
    {
        let raw = env["NODOASSIST_GATEWAY_PASSWORD"] ?? ""
        ...
        if !trimmed.isEmpty {
               ...
               !configPassword.isEmpty
            {
                self.warnEnvOverrideOnce(
                    kind: .password,
                    envVar: "NODOASSIST_GATEWAY_PASSWORD",
    ...
    private static func resolveGatewayToken(
        ...
        launchdSnapshot: LaunchAgentPlistSnapshot?) -> String?
    {
        let raw = env["NODOASSIST_GATEWAY_TOKEN"] ?? ""
        let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
        if !trimmed.isEmpty {
               ...
               configToken != trimmed
            {
                self.warnEnvOverrideOnce(
                    kind: .token,
                    envVar: "NODOASSIST_GATEWAY_TOKEN",
    ...
    private let logger = Logger(subsystem: "ai.nodoassist", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "nodoassist.onboardingSeen")
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: NodoAssistConfigFile.loadDict())
    ...
    {
        if let envBind = env["NODOASSIST_GATEWAY_BIND"] {
    ...
    private static func resolveGatewayScheme(
        ...
        env: [String: String]) -> String
    {
        if let envValue = env["NODOASSIST_GATEWAY_TLS"]?.trimmingCharacters(in: .whitespacesAndNewlines),
           !envValue.isEmpty
        {
