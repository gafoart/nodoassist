...
import NodoAssistDiscovery
import NodoAssistIPC
...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        self.onboardingPage {
            VStack(spacing: 18) {
                ...
                Label {
                    Text(
                        "NodoAssist can take actions using the permissions and services you enable. " +
    ...
    func connectionPage() -> some View {
        self.onboardingPage {
            Text("Where should your assistant live?")
                .font(.largeTitle.weight(.semibold))
            Text(
                "Most people pick this Mac — NodoAssist installs everything and keeps it " +
            ...
            if self.selectedConnectionMode == .unconfigured {
                Text("OK — NodoAssist won’t start anything yet. Pick Local or Remote later in Settings → General.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
                    ...
                    if self.state.remoteTransport == .ssh {
                        ...
                        GridRow {
                            Text("Project root")
                                .font(.callout.weight(.semibold))
                                .frame(width: labelWidth, alignment: .leading)
                            TextField("/home/you/Projects/nodoassist", text: self.$state.remoteProjectRoot)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: fieldWidth)
                        }
                        GridRow {
                            Text("CLI path")
                                ...
                                .frame(width: labelWidth, alignment: .leading)
                            TextField(
                                "/Applications/NodoAssist.app/.../nodoassist",
            ...
            if self.state.remoteTokenUnsupported {
                Text(
                    "The current gateway.remote.token value is not plain text. "
                        + "NodoAssist for macOS cannot use it directly; "
        ...
        action: @escaping () -> Void) -> some View
    {
        Button {
            ...
            .nodoAssistSelectableRowChrome(selected: selected)
        }
        .buttonStyle(.plain)
    }
    ...
    func permissionsPage() -> some View {
        ...
        VStack(spacing: 12) {
            ...
            Text(
                "These macOS permissions let NodoAssist automate apps and capture context on this Mac. " +
    ...
    func cliPage() -> some View {
        self.onboardingPage {
            Text("Getting things ready")
                .font(.largeTitle.weight(.semibold))
            Text(
                "NodoAssist is setting up its background service on this Mac. " +
    ...
    func workspacePage() -> some View {
        self.onboardingPage {
            Text("Agent workspace")
                .font(.largeTitle.weight(.semibold))
            Text(
                "NodoAssist runs the agent from a dedicated workspace so it can load `AGENTS.md` " +
                ...
                if self.state.connectionMode == .remote {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Workspace folder")
                            .font(.headline)
                        TextField(
                            AgentWorkspace.displayPath(for: NodoAssistConfigFile.defaultWorkspaceURL()),
                                ...
                                Task {
                                    ...
                                    if saved {
                                        self.workspaceStatus =
                                            "Saved to ~/.nodoassist/nodoassist.json (agents.defaults.workspace)"
                                    }
                                }
                            }
                            .buttonStyle(.bordered)
    ...
    func onboardingChatPage() -> some View {
        VStack(spacing: 12) {
            ...
            self.onboardingGlassCard(padding: 8) {
                NodoAssistChatView(viewModel: self.onboardingChatModel, style: .onboarding)
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `nodoassist` package and make sure credentials exist
                        (typically `~/.nodoassist/credentials/oauth.json`). Then connect again if needed.
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the NodoAssist menu bar icon for quick chat and status.",
