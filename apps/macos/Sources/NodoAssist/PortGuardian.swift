...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "ai.nodoassist", category: "portguard")
    #if DEBUG
    private var testingDescriptors: [Int: Descriptor] = [:]
    #endif
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("NodoAssist", isDirectory: true)
    }()
    ...
    static func classifyTunnelRecord(_ record: Record, process: TunnelProcessInfo?) -> TunnelRecordAction {
        ...
        // live parent may be a concurrent NodoAssist instance (prod + dev), so hands off.
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "nodoassist", "tsx", "pnpm", "bun"]
    ...
    private static func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .local:
            ...
            if full.contains("gateway-daemon") || full.contains("nodoassist-gateway")
                || cmd.contains("nodoassist-gateway")
            {
                return true
            }
            if self.isNodeNodoAssistGatewayCommand(full) { return true }
            // If args are unavailable, treat a CLI listener as expected.
            if cmd.contains("nodoassist"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
    ...
    private static func isNodeNodoAssistGatewayCommand(_ fullCommand: String) -> Bool {
        ...
        return self.isNodoAssistDistEntrypointToken(tokens[1])
            && tokens[2].lowercased() == "gateway"
    }

    private static func isNodoAssistDistEntrypointToken(_ token: String) -> Bool {
