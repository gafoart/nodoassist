...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.nodoassist.mac`) even if the shipped
    ...
    {
        if environment["NODOASSIST_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "nodoassist.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "nodoassist.nixMode") { return true }
