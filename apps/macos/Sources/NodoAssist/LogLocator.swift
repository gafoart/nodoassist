...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["NODOASSIST_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/nodoassist")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("nodoassist-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("nodoassist-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/nodoassist/ (rolling or stdout), or nil if none exist.
