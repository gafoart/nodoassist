...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.nodoassist/nodoassist.json. Prefer NODOASSIST_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
        ...
        isPaused: Bool) -> GatewayTailscaleApplyMessages
    {
        ...
        let statusMessage = if connectionMode == .local, !isPaused {
            "Saved to ~/.nodoassist/nodoassist.json. Restarting gateway…"
        } else {
            "Saved to ~/.nodoassist/nodoassist.json. Restart the gateway to apply."
        }
        return GatewayTailscaleApplyMessages(
