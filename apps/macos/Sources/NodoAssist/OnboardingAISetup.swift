...
final class OnboardingAISetupModel {
    ...
    private(set) var phase: Phase = .idle {
        didSet {
            ...
            OnboardingController.shared.busyReason = self.phase == .testing
                ? "NodoAssist is testing your AI connection."
                : nil
        }
    }
    ...
    static func friendlyTransportError(_ raw: String) -> String {
        if raw.localizedCaseInsensitiveContains("unknown method") {
            return "The Gateway is running an older NodoAssist version that doesn’t support " +
                "app-guided setup. Update NodoAssist on the gateway, then try again."
        }
        return raw
    }
...
private enum OnboardingAISetupError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .providerCatalogUnavailable:
            "The Gateway is running an older NodoAssist version that doesn’t provide the " +
                "supported provider list. Update NodoAssist on the gateway, then try again."
        }
    }
}
...
struct OnboardingAISetupView: View {
    ...
    private func candidateRow(_ candidate: OnboardingAISetupModel.Candidate) -> some View {
        ...
        return Button {
            ...
            .nodoAssistSelectableRowChrome(selected: selected && status != .failed(message: ""))
        }
        .buttonStyle(.plain)
    ...
    private var manualProviderHelp: String {
        ...
        guard let hint, !hint.isEmpty else {
            return "Paste the key or token here, and NodoAssist checks it with a real test question."
        }
        return "\(hint). Paste it here, and NodoAssist checks it with a real test question."
    }
