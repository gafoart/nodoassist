...
extension ChannelsStore {
    ...
    func applyConfigSnapshot(_ snap: ConfigSnapshot, sourceKey: String, force: Bool) {
        ...
        self.configStatus = snap.valid == false
            ? "Config invalid; fix it in ~/.nodoassist/nodoassist.json."
            : nil
    ...
    private func currentConfigCacheSourceKey() -> String {
        ...
        return [
            ...
            "token:\(Self.configFingerprint(env["NODOASSIST_GATEWAY_TOKEN"]))",
            "password:\(Self.configFingerprint(env["NODOASSIST_GATEWAY_PASSWORD"]))",
        ].joined(separator: "|")
    }
