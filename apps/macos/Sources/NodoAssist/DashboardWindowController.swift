...
final class DashboardWindowController: NSWindowController, WKNavigationDelegate, WKUIDelegate, NSWindowDelegate {
    ...
    private static func installNativeChromeScript(into userContentController: WKUserContentController) {
        let css = """
        html.nodoassist-native-macos {
          --nodoassist-native-titlebar-height: 50px;
        }
        @media (min-width: 700px) {
          html.nodoassist-native-macos .sidebar-shell {
            padding-top: max(14px, var(--nodoassist-native-titlebar-height)) !important;
          }
          html.nodoassist-native-macos .sidebar-shell__header {
        ...
        (() => {
          try {
            if (document.getElementById("nodoassist-native-macos-chrome")) return;
            const style = document.createElement("style");
            style.id = "nodoassist-native-macos-chrome";
            style.textContent = \(Self.jsStringLiteral(css));
            document.documentElement.classList.add("nodoassist-native-macos");
