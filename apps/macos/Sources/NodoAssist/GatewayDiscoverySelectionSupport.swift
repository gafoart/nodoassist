...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            NodoAssistConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                NodoAssistConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                NodoAssistConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            NodoAssistConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            NodoAssistConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
