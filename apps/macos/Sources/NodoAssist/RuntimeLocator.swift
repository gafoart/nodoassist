...
enum RuntimeLocator {
    private static let logger = Logger(subsystem: "ai.nodoassist", category: "runtime")
    ...
    static func describeFailure(_ error: RuntimeResolutionError) -> String {
        ...
        case let .notFound(searchPaths):
            [
                "nodoassist needs Node \(self.supportedNodeRange) but found no runtime.",
        ...
        case let .unsupported(kind, found, path, searchPaths):
            [
                ...
                "PATH searched: \(searchPaths.joined(separator: ":"))",
                "Upgrade Node and rerun nodoassist.",
            ].joined(separator: "\n")
