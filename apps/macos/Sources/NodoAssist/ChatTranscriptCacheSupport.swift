...
/// (`~/Library/Application Support/NodoAssist/chat-cache.sqlite`), matching the
/// existing NodoAssist app-support layout. macOS has no per-file Data Protection
...
enum MacChatTranscriptCache {
    struct Context {
        let store: NodoAssistChatSQLiteTranscriptCache
    ...
    @MainActor private static var storesByGatewayID: [String: NodoAssistChatSQLiteTranscriptCache] = [:]
    ...
    ///   session data: distinct profiles (distinct `NODOASSIST_STATE_DIR`) never
    ...
    static func currentGatewayID() -> String? {
        ...
        return self.gatewayID(
            mode: mode,
            localStateDir: NodoAssistConfigFile.stateDirURL(),
    ...
    @MainActor
    static func make() -> NodoAssistChatSQLiteTranscriptCache? {
    ...
    static func makeContext() -> Context? {
        ...
        let databaseURL = base.appendingPathComponent("NodoAssist/chat-cache.sqlite", isDirectory: false)
        return Context(
            store: self.store(databaseURL: databaseURL, gatewayID: gatewayID),
            routingIdentity: NodoAssistChatSQLiteTranscriptCache.loadSessionRoutingIdentity(
                databaseURL: databaseURL,
                gatewayID: gatewayID))
    }

    @MainActor
    static func store(databaseURL: URL, gatewayID: String) -> NodoAssistChatSQLiteTranscriptCache {
        if let store = storesByGatewayID[gatewayID] {
            return store
        }
        let store = NodoAssistChatSQLiteTranscriptCache(databaseURL: databaseURL, gatewayID: gatewayID)
