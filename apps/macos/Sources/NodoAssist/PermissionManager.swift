...
extension Notification.Name {
    static let nodoassistPermissionsChanged = Notification.Name("nodoassist.permissions.changed")
}
...
enum PermissionManager {
    ...
    static func ensure(_ caps: [Capability], interactive: Bool) async -> [Capability: Bool] {
        ...
        if interactive {
            await MainActor.run {
                NotificationCenter.default.post(name: .nodoassistPermissionsChanged, object: nil)
            }
        }
        return results
    }
...
enum AppleScriptPermission {
    private static let logger = Logger(subsystem: "ai.nodoassist", category: "AppleScriptPermission")
