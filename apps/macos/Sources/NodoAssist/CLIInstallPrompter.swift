...
final class CLIInstallPrompter {
    ...
    private let logger = Logger(subsystem: "ai.nodoassist", category: "cli.prompt")
    ...
    private func checkAndPromptIfNeededAsync(reason: String) async {
        ...
        let alert = NSAlert()
        alert.messageText = "Install NodoAssist CLI?"
    ...
    private func installCLI() async {
        ...
        if installed {
            await status.set("Starting NodoAssist Gateway…")
            let activation = await CLIInstaller.activateLocalGateway()
            let message = switch activation {
            case .ready:
                "NodoAssist Gateway is ready."
            case .deferred:
                "NodoAssist is installed. The Gateway will start when This Mac is active and resumed."
            case .failed:
                "NodoAssist was installed, but the Gateway did not start. Open Settings to retry."
            }
            await status.set(message)
        }
    ...
    private func openSettings(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .nodoassistSelectSettingsTab, object: tab)
        }
    }
