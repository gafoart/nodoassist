...
final class RemotePortTunnel: @unchecked Sendable {
    private static let logger = Logger(subsystem: "ai.nodoassist", category: "remote.tunnel")
    ...
    static func resolveRemotePortOverride(defaultRemotePort: Int, for sshHost: String) -> Int? {
        ...
        guard let sshKey = NodoAssistConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = NodoAssistConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.nodoassist.remote.tunnel.port", qos: .utility)
            do {
