...
private let webChatSwiftLogger = Logger(subsystem: "ai.nodoassist", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "nodoassist.webchat.thinkingLevel"
...
private enum WebChatSwiftUILayout {
    ...
    static let windowFrameAutosaveName = "NodoAssistChatWindow"
    static let anchorPadding: CGFloat = 8
}

struct MacGatewayChatTransport: NodoAssistChatTransport {
        ...
        archived: Bool) async throws -> NodoAssistChatSessionsListResponse
    {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(NodoAssistChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            NodoAssistChatSessionsDefaults(
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return NodoAssistChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
        ...
        attachments: [NodoAssistChatAttachmentPayload]) async throws -> NodoAssistChatSendResponse
    {
        ...
        else { throw NodoAssistChatTransportSendError.notDispatched }
        ...
        let guardedContract = NodoAssistChatSessionRoutingContract.expectedValue(
    ...
    func acquireOutboxRouteLease() async -> NodoAssistChatTransportRouteLeaseResult {
        ...
        guard supportsRoutingContract else {
            return .unavailable(reason: NodoAssistChatTransportUpgradeMessage.routingContract)
        }
    ...
    func createSession(
        ...
        worktree: Bool?) async throws -> NodoAssistChatCreateSessionResponse
    {
        var params: [String: AnyCodable] = [
            "key": AnyCodable(key),
        ]
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(NodoAssistChatCreateSessionResponse.self, from: data)
    }
    ...
    func compactSession(sessionKey: String) async throws {
        ...
        try NodoAssistSessionsCompactResponse.requireSuccess(from: response)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> NodoAssistChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                NodoAssistGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    NodoAssistChatEventPayload.self,
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    NodoAssistSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    NodoAssistAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: NodoAssistProtocol.ModelChoice) -> NodoAssistChatModelChoice {
    ...
    private static func mapCommandChoice(_ entry: CommandEntry) -> NodoAssistChatCommandChoice {
        let sourceValue = (entry.source.value as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
        let source: NodoAssistChatCommandChoice.Source = switch sourceValue {
        case "native":
            .command
        case "skill":
            .skill
        ...
        return NodoAssistChatCommandChoice(
            id: id,
            name: entry.name,
            textAliases: aliases,
...
final class WebChatSwiftUIWindowController {
        ...
        outbox: (any NodoAssistChatCommandOutbox)? = nil)
    {
        ...
        Task { @MainActor [weak vm] in
            ...
            for await push in pushes {
                ...
                if let routingIdentity {
                    ...
                    if let store = transcriptCache as? NodoAssistChatSQLiteTranscriptCache,
                       store.gatewayID == MacChatTranscriptCache.currentGatewayID(),
                       let persistedIdentity = NodoAssistChatSessionRoutingIdentity(
        ...
        case .window:
            ...
            let hosting = NSHostingController(rootView: NodoAssistChatWindowShell(
                viewModel: vm,
                userAccent: accent))
        ...
        case .panel:
            ...
            let hosting = NSHostingController(rootView: NodoAssistChatView(
                viewModel: vm,
    ...
    private static func makePanelContentController(
        hosting: NSHostingController<NodoAssistChatView>) -> NSViewController
    {
