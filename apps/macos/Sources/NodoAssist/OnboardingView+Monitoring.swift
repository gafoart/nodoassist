...
extension OnboardingView {
    ...
    func startCLIInstall() {
        ...
        OnboardingController.shared.busyReason = "NodoAssist is installing the Gateway service."
    ...
    func runCLIInstall() async {
        ...
        cliStatus = "Starting NodoAssist Gateway…"
        ...
        case .ready:
            cliStatus = "NodoAssist Gateway is ready."
        case .deferred:
            cliStatus = "NodoAssist is installed. The Gateway will start when This Mac is active and resumed."
        case .failed:
            cliStatus = "NodoAssist was installed, but the Gateway did not start. Retry setup."
            return
        }
        cliInstalled = true
    }
    ...
    func refreshLocalGatewayProbe() async {
        ...
        await MainActor.run {
            ...
            let expectedTokens = ["node", "nodoassist", "tsx", "pnpm", "bun"]
