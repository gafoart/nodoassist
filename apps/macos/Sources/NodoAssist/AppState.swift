...
final class AppState {
    private static let logger = Logger(subsystem: "ai.nodoassist", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = NodoAssistConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            NodoAssistConfigFile.canonicalHostForComparison(host) ==
                NodoAssistConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = NodoAssistConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: NodoAssistConfigFile.loadDict(),
            draft: .init(
        ...
        guard NodoAssistConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/nodoassist"
        state.remoteCliPath = ""
        return state
    }
}
