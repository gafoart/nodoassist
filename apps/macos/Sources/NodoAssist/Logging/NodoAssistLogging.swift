...
enum NodoAssistLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = NodoAssistOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = NodoAssistFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        NodoAssistLogging.bootstrapIfNeeded()
        let label = NodoAssistLogging.makeLabel(subsystem: subsystem, category: category)
...
struct NodoAssistOSLogHandler: AppLogLevelBackedHandler {
...
struct NodoAssistFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = NodoAssistLogging.parseLabel(self.label)
