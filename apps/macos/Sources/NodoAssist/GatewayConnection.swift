...
private let gatewayConnectionLogger = Logger(subsystem: "ai.nodoassist", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: NodoAssistProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(NodoAssistGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> NodoAssistSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return NodoAssistSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        ...
        maxChars: Int? = nil,
        timeoutMs: Int? = nil,
        ifCurrentRoute route: Route? = nil) async throws -> NodoAssistChatHistoryPayload
    {
        ...
        if let route {
            let data = try await request(
                ...
                timeoutMs: timeout,
                ifCurrentRoute: route)
            return try self.decoder.decode(NodoAssistChatHistoryPayload.self, from: data)
        }
        ...
        distinguishPreDispatchRouteChange: Bool = false) async throws -> NodoAssistChatSendResponse
    {
        ...
        if let route {
            ...
            return try self.decoder.decode(NodoAssistChatSendResponse.self, from: data)
        }
