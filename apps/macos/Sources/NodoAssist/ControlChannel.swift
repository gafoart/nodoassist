...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: NodoAssistProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.nodoassist", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: NodoAssistKit.AnyCodable]()) {
                $0[$1.key] = NodoAssistKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking NodoAssist Local Network access.
                    Allow NodoAssist in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: NodoAssistProtocol.AnyCodable?) -> [String: NodoAssistProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: NodoAssistProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: NodoAssistKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: NodoAssistProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("nodoassist.control.heartbeat")
    static let controlAgentEvent = Notification.Name("nodoassist.control.agent")
}
