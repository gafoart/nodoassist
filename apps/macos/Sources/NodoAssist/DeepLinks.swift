...
private let deepLinkLogger = Logger(subsystem: "ai.nodoassist", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        case let .agent(link):
            guard !AppStateStore.shared.isPaused else {
                self.presentAlert(title: "NodoAssist is paused", message: "Unpause NodoAssist to run agent actions.")
                return
            }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run NodoAssist agent?", message: body) else { return }
        }
