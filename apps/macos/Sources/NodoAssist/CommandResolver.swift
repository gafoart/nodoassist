...
enum CommandResolver {
    private static let projectRootDefaultsKey = "nodoassist.gatewayProjectRootPath"
    private static let helperName = "nodoassist"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let nodoassistEntry = root.appendingPathComponent("nodoassist.mjs").path
        if FileManager().isReadableFile(atPath: nodoassistEntry) { return nodoassistEntry }
        let binEntry = root.appendingPathComponent("bin/nodoassist.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/nodoassist")
    ...
    static func preferredPaths() -> [String] {
        ...
        let validatedExecutable = self.validatedNodoAssistExecutable(
            defaults: .standard,
            fileManager: .default,
        ...
        validatedExecutable: String? = nil) -> [String]
    {
        ...
        let managedPaths = self.nodoassistManagedPaths(home: home)
    ...
    static func validatedNodoAssistExecutable(
    ...
    private static func nodoassistManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".nodoassist"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func nodoassistExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectNodoAssistExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("nodoassist.mjs").path,
            root.appendingPathComponent("bin/nodoassist.js").path,
        ]
    ...
    static func hasAnyNodoAssistInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.nodoassistExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func nodoassistNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let nodoassistPath = self.projectNodoAssistExecutable(projectRoot: root) {
            return [nodoassistPath, subcommand] + extraArgs
        }
        if let nodoassistPath = self.nodoassistExecutable(searchPaths: searchPaths) {
            return [nodoassistPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "nodoassist", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            nodoassist entrypoint missing (looked for dist/index.js or nodoassist.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real nodoassist CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/nodoassist"
        ...
        if command -v nodoassist >/dev/null 2>&1; then
          CLI="$(command -v nodoassist)"
          nodoassist \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/nodoassist.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/nodoassist.mjs"
            node "$PRJ/nodoassist.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/nodoassist.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/nodoassist.js"
            node "$PRJ/bin/nodoassist.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent nodoassist"
          pnpm --silent nodoassist \(quotedArgs);
        else
          echo "nodoassist CLI missing on remote host"; exit 127;
        fi
        """
    ...
    {
        let root = configRoot ?? NodoAssistConfigFile.loadDict()
