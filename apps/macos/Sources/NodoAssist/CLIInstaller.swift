...
enum CLIInstaller {
    ...
    enum Status: Equatable {
        ...
        var message: String {
            ...
            case let .ready(_, version):
                "NodoAssist Gateway \(version) is ready."
            case .missing:
                "NodoAssist Gateway is not installed yet."
            case .unusable:
                "The NodoAssist Gateway could not be verified. Setup will repair it."
        ...
        fileManager: FileManager) -> [String]
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("nodoassist").path
    ...
    static func managedExecutableLocation() -> String {
        URL(fileURLWithPath: self.installPrefix())
            .appendingPathComponent("bin/nodoassist")
            .path
    }
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async -> Bool {
        ...
        await statusHandler("Installing nodoassist CLI…")
        guard let installerURL = Bundle.main.url(forResource: "install-cli", withExtension: "sh") else {
            await statusHandler("Install failed: installer resource is missing. Reinstall NodoAssist.")
            return false
        }
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed nodoassist \($0)." } ?? "Installed nodoassist."
    ...
    private static func installPrefix() -> String {
        FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".nodoassist")
            .path
    }
