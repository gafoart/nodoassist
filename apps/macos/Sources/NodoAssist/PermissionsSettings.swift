        ...
        return HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(complete ? "All access granted" : "\(granted) of \(total) permissions granted")
                    .font(.headline)
                Text("NodoAssist only asks for macOS capabilities when a feature needs them.")
                    .font(.footnote)
...
private struct LocationAccessSettings: View {
    ...
    @AppStorage(locationModeKey) private var locationModeRaw: String = NodoAssistLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = NodoAssistLocationMode.off.rawValue
        ...
        VStack(spacing: 0) {
                ...
                subtitle: "Allow agents to use device location when a tool asks for it.")
            {
                Picker("Location Access", selection: self.$locationModeRaw) {
                    Text("Off").tag(NodoAssistLocationMode.off.rawValue)
                    Text("While Using").tag(NodoAssistLocationMode.whileUsing.rawValue)
                    Text("Always").tag(NodoAssistLocationMode.always.rawValue)
                }
                .labelsHidden()
                .pickerStyle(.menu)
    ...
    private var locationMode: NodoAssistLocationMode {
        NodoAssistLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: NodoAssistLocationMode) async -> Bool {
