...
final class DevicePairingApprovalPrompter {
    ...
    private let logger = Logger(subsystem: "ai.nodoassist", category: "device-pairing")
    ...
    static func alertSummary(for req: PendingRequest) -> String {
        ...
        return "Approve \(subject) to control NodoAssist. Only approve if this is yours; you can remove it later in Settings."
    }
    ...
    static func deviceName(for req: PendingRequest) -> String {
        ...
        return self.isMac(req.platform) ? "NodoAssist Mac app" : "New device"
    }
