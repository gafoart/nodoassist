...
final class MacNodeModeCoordinator: NSObject {
    ...
    private let logger = Logger(subsystem: "ai.nodoassist", category: "mac-node")
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(self.refreshNodeConfiguration),
            name: .nodoassistPermissionsChanged,
            object: nil)
    }
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = NodoAssistConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: NodoAssistNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: NodoAssistLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        ...
        if browserControlEnabled, connectionMode == .local {
            caps.append(NodoAssistCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(NodoAssistCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(NodoAssistCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps(browserControlEnabled: Bool, cameraEnabled: Bool) -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: browserControlEnabled,
            cameraEnabled: cameraEnabled,
            locationMode: NodoAssistLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            NodoAssistSystemCommand.which.rawValue,
            NodoAssistSystemCommand.run.rawValue,
            NodoAssistSystemCommand.execApprovalsGet.rawValue,
            NodoAssistSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(NodoAssistCapability.browser.rawValue) {
            commands.append(NodoAssistBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(NodoAssistCapability.camera.rawValue) {
        ...
        if capsSet.contains(NodoAssistCapability.location.rawValue) {
    ...
    private func buildSessionBox(url: URL, connectionMode: AppState.ConnectionMode) -> WebSocketSessionBox? {
        ...
        guard let params = Self.tlsParams(
            ...
            root: NodoAssistConfigFile.loadDict(),
