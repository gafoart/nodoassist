...
struct NodoAssistMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("nodoassist-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      nodoassist-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      nodoassist-mac discover [--timeout <ms>] [--json] [--include-local]
      nodoassist-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      nodoassist-mac connect
      nodoassist-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      nodoassist-mac connect --url ws://127.0.0.1:18789 --json
      nodoassist-mac discover --timeout 3000 --json
      nodoassist-mac wizard --mode local
    """)
}
