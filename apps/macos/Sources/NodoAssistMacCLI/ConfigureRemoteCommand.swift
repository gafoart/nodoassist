...
private let appDefaultsSuites = ["ai.nodoassist.mac", "ai.nodoassist.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              nodoassist-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
              ...
              nodoassist-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote NodoAssist checkout for CLI commands.
              --cli-path <path>   Remote nodoassist executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
    ...
    defaultsSuites: [String]) throws -> ConfigureRemoteOutput
{
    ...
    let configURL = nodoAssistConfigURL()
...
private func nodoAssistConfigURL() -> URL {
    if let raw = ProcessInfo.processInfo.environment["NODOASSIST_CONFIG_PATH"],
       !raw.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    {
        return URL(fileURLWithPath: NSString(string: raw).expandingTildeInPath)
    }
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".nodoassist/nodoassist.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String, suites: [String]) {
    for suite in suites {
        ...
        defaults.set("remote", forKey: "nodoassist.connectionMode")
        setDefaultString(defaults, key: "nodoassist.remoteTarget", value: target)
        defaults.set(true, forKey: "nodoassist.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "nodoassist.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "nodoassist.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "nodoassist.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "nodoassist.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("NodoAssist macOS Remote Config")
