...
```text
refs/nodoassist/mobile-releases/android/<versionName>-<versionCode>
```

Example:

```text
refs/nodoassist/mobile-releases/android/2026.6.10-2026061008
```
...
core NodoAssist release machinery.
...
If `MATCH_PASSWORD` is not set, the existing manual Gradle-property signing path still works: provide `NODOASSIST_ANDROID_STORE_FILE`, `NODOASSIST_ANDROID_STORE_PASSWORD`, `NODOASSIST_ANDROID_KEY_ALIAS`, and `NODOASSIST_ANDROID_KEY_PASSWORD` through your local Gradle user properties before running release tasks.
