...
val nodoAssistAndroidVersionFile = rootProject.file("Config/Version.properties")
val thirdPartyLicensesDir = rootProject.file("THIRD_PARTY_LICENSES")
val nodoAssistAndroidVersionProperties =
  Properties().apply {
    if (!nodoAssistAndroidVersionFile.isFile) {
      error("Missing Android version properties. Run `pnpm android:version:sync`.")
    }
    nodoAssistAndroidVersionFile.inputStream().use(::load)
  }

fun requireNodoAssistAndroidVersionProperty(name: String): String =
  nodoAssistAndroidVersionProperties.getProperty(name)?.trim()?.takeIf { it.isNotEmpty() }
    ?: error("Missing $name in Config/Version.properties. Run `pnpm android:version:sync`.")

val nodoAssistAndroidVersionName = requireNodoAssistAndroidVersionProperty("NODOASSIST_ANDROID_VERSION_NAME")
val nodoAssistAndroidVersionCode =
  requireNodoAssistAndroidVersionProperty("NODOASSIST_ANDROID_VERSION_CODE").toIntOrNull()
    ?: error("NODOASSIST_ANDROID_VERSION_CODE must be an integer in Config/Version.properties.")

val androidStoreFile = providers.gradleProperty("NODOASSIST_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("NODOASSIST_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("NODOASSIST_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("NODOASSIST_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set NODOASSIST_ANDROID_STORE_FILE, " +
      "NODOASSIST_ANDROID_STORE_PASSWORD, NODOASSIST_ANDROID_KEY_ALIAS, and " +
      "NODOASSIST_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/NodoAssistKit/Sources/NodoAssistKit/Resources")
      assets.directories.add(thirdPartyLicensesDir.path)
    }
  }

  defaultConfig {
    applicationId = "ai.nodoassist.app"
    minSdk = 31
    targetSdk = 36
    versionCode = nodoAssistAndroidVersionCode
    versionName = nodoAssistAndroidVersionName
    ndk {
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "nodoassist-$versionName-$buildType.apk"
          } else {
            "nodoassist-$versionName-$flavorName-$buildType.apk"
          }
