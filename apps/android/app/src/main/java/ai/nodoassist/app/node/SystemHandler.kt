...
import ai.nodoassist.app.mainActivityPendingIntent
...
private const val NOTIFICATION_CHANNEL_BASE_ID = "nodoassist.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "NodoAssist Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "NodoAssist Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "NodoAssist Active")
      }
