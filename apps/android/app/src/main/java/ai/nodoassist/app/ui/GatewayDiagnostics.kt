...
internal fun nodoAssistAndroidVersionLabel(): String {
...
internal fun gatewayNodeApprovalCommand(
  ...
  when (state) {
    ...
    -> normalizeGatewayApprovalRequestId(requestId)?.let { "nodoassist nodes approve $it" } ?: "nodoassist nodes status"
    GatewayNodeApprovalState.Unapproved -> "nodoassist nodes status"
...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this NodoAssist Android gateway connection failure.
    ...
    - tell me whether `nodoassist devices list` should show a pending pairing request
    - if more signal is needed, ask for `nodoassist qr --json`, `nodoassist devices list`, and `nodoassist nodes status`
    - give the next exact command or tap

    Debug info:
    - screen: $screen
    - app version: ${nodoAssistAndroidVersionLabel()}
    - device: $device
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("NodoAssist gateway diagnostics", report))
