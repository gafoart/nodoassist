...
import ai.nodoassist.app.protocol.NodoAssistCanvasA2UICommand
import ai.nodoassist.app.protocol.NodoAssistCanvasCommand
...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    // Command strings come from NodoAssistProtocolConstants; the registry above owns advertised availability.
    return when (command) {
      // Canvas commands
      ...
      NodoAssistCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      NodoAssistCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      NodoAssistCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      NodoAssistCanvasA2UICommand.Push.rawValue, NodoAssistCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      NodoAssistCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      NodoAssistCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      NodoAssistCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      NodoAssistLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      NodoAssistDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      NodoAssistDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      NodoAssistDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      NodoAssistDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)
      NodoAssistDeviceCommand.Apps.rawValue -> deviceHandler.handleDeviceApps(paramsJson)

      // Notifications command
      NodoAssistNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      NodoAssistNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      NodoAssistSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      NodoAssistTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      NodoAssistTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      NodoAssistTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      NodoAssistTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.nodoassist.app.protocol.NodoAssistPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      NodoAssistContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      NodoAssistContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      NodoAssistCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      NodoAssistCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      NodoAssistMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      NodoAssistMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      NodoAssistSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      NodoAssistSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      NodoAssistCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
