...
import ai.nodoassist.app.MainActivity
...
class SystemHandlerTest {
  ...
  fun handleSystemNotify_rejectsUnauthorized() {
    ...
    val result = handler.handleSystemNotify("""{"title":"NodoAssist","body":"hi"}""")
  ...
  fun handleSystemNotify_rejectsInvalidRequestObject() {
    ...
    val result = handler.handleSystemNotify("""{"title":"NodoAssist"}""")
  ...
  fun handleSystemNotify_postsNotification() {
    ...
    val result = handler.handleSystemNotify("""{"title":"NodoAssist","body":"done","priority":"active"}""")
  ...
  fun handleSystemNotify_trimsAndPassesOptionalFields() {
    ...
    val result =
      handler.handleSystemNotify(
        """{"title":" NodoAssist ","body":" done ","priority":" passive ","sound":" silent "}""",
      )

    assertTrue(result.ok)
    assertEquals("NodoAssist", poster.lastRequest?.title)
  ...
  fun buildSystemNotificationSetsImmutableAppLaunchIntent() {
    ...
    val notification =
      buildSystemNotification(
        appContext = context,
        channelId = "test",
        request = SystemNotifyRequest("NodoAssist", "done", sound = null, priority = null),
      )
  ...
  fun handleSystemNotify_returnsUnauthorizedWhenPostFailsPermission() {
    ...
    val result = handler.handleSystemNotify("""{"title":"NodoAssist","body":"done"}""")
