...
class InvokeDispatcherTest {
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(NodoAssistSmsCommand.Search.rawValue, "not-json")
    ...
    runTest {
      val result =
        newDispatcher(
          ...
          smsFeatureEnabled = true,
          smsTelephonyAvailable = true,
        ).handleInvoke(NodoAssistSmsCommand.Send.rawValue, """{"to":"+15551234567","message":"hi"}""")
  ...
  fun handleInvoke_blocksCameraCommandsWhenCameraDisabled() =
    runTest {
      val result = newDispatcher(cameraEnabled = false).handleInvoke(NodoAssistCameraCommand.List.rawValue, null)
  ...
  fun handleInvoke_blocksLocationCommandWhenLocationDisabled() =
    runTest {
      val result = newDispatcher(locationEnabled = false).handleInvoke(NodoAssistLocationCommand.Get.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(installedAppsSharingEnabled = false)
          .handleInvoke(NodoAssistDeviceCommand.Apps.rawValue, """{"limit":1}""")
    ...
    runTest {
      val result =
        newDispatcher(motionActivityAvailable = false)
          .handleInvoke(NodoAssistMotionCommand.Activity.rawValue, null)
    ...
    runTest {
      val result =
        newDispatcher(motionPedometerAvailable = false)
          .handleInvoke(NodoAssistMotionCommand.Pedometer.rawValue, null)
  ...
  fun handleInvoke_blocksCallLogWhenUnavailable() =
    runTest {
      val result =
        newDispatcher(callLogAvailable = false).handleInvoke(NodoAssistCallLogCommand.Search.rawValue, null)
  ...
  fun handleInvoke_blocksPhotosWhenUnavailable() =
    runTest {
      val result = newDispatcher(photosAvailable = false).handleInvoke(NodoAssistPhotosCommand.Latest.rawValue, null)
  ...
  fun handleInvoke_routesTalkPttCommands() =
    runTest {
      ...
      val start = dispatcher.handleInvoke(NodoAssistTalkCommand.PttStart.rawValue, null)
      val stop = dispatcher.handleInvoke(NodoAssistTalkCommand.PttStop.rawValue, null)
      val cancel = dispatcher.handleInvoke(NodoAssistTalkCommand.PttCancel.rawValue, null)
      val once = dispatcher.handleInvoke(NodoAssistTalkCommand.PttOnce.rawValue, null)
