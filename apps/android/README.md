## NodoAssist Android App

NodoAssist Android is the officially released Google Play app. It connects to an NodoAssist Gateway as a companion node for chat, voice, approvals, screen, and device-aware automation.
...
```bash
ANDROID_SCREENSHOT_AVD=NodoAssist_QA_API35 pnpm android:screenshots
```
...
- Play build: `nodoassist-<version>-play-release.aab`
- Third-party build: `nodoassist-<version>-third-party-release.apk`

`pnpm android:bundle:release` is an alias for the same Fastlane archive lane.

Regular final and correction NodoAssist releases publish the signed third-party APK as `NodoAssist-Android.apk` with a checksum manifest and GitHub Actions provenance. `.github/workflows/android-release.yml` is the only automated GitHub Release upload path; `NodoAssist Release Publish` dispatches it while the canonical release is still a draft and blocks publication until the uploaded asset contract verifies.

The protected `android-release` environment supplies `MATCH_PASSWORD`; the repository's read-only GitHub App token checks out encrypted material from `nodoassist/apps-signing`. The workflow builds the exact release tag, refuses to replace different existing bytes, and re-downloads the APK for checksum, certificate, and provenance verification.
...
```bash
pnpm nodoassist gateway --port 18789 --verbose
```
...
- Canvas web content already supports live reload when loaded from Gateway `__nodoassist__/canvas/` (see `docs/platforms/android.md`).
...
Current NodoAssist Android implication:
...
2) Android app is connected to that gateway and `nodoassist nodes status` shows it as paired + connected.
3) App stays unlocked and in foreground for the whole run.
4) Open the app **Screen** tab and keep it active during the run (canvas/A2UI commands require the canvas WebView attached there).
5) Grant runtime permissions for capabilities you expect to pass (camera/mic/location/notification listener/location, etc.).
6) No interactive system dialogs should be pending before test start.
7) Canvas host is enabled and reachable from the device for remote Canvas checks (do not run gateway with `NODOASSIST_SKIP_CANVAS_HOST=1`; startup logs should include `canvas host mounted at .../__nodoassist__/`).
...
```bash
nodoassist devices list
nodoassist devices approve --latest   # preview only; copy the requestId from output
nodoassist devices approve <requestId>
```
...
- `NODOASSIST_ANDROID_GATEWAY_URL=ws://...` (default: from your local NodoAssist config)
- `NODOASSIST_ANDROID_GATEWAY_TOKEN=...`
- `NODOASSIST_ANDROID_GATEWAY_PASSWORD=...`
...
- `pairing required` before tests start:
  - list pending requests (`nodoassist devices list`), then approve with the exact ID (`nodoassist devices approve <requestId>`) and rerun.
