# fastlane setup (NodoAssist Android)
...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the NodoAssist app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.nodoassist.app
```
...
NODOASSIST_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
NODOASSIST_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
NODOASSIST_ANDROID_STORE_PASSWORD=<store-password>
NODOASSIST_ANDROID_KEY_ALIAS=<upload-key-alias>
NODOASSIST_ANDROID_KEY_PASSWORD=<key-password>
```
...
- Stable GitHub Release APK publication is separate from Google Play: `NodoAssist Release Publish` dispatches `.github/workflows/android-release.yml`, whose protected `android-release` environment provides `MATCH_PASSWORD`; the repository GitHub App reads the encrypted signing repo.
