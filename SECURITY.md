...
Security work is shared across a number of NodoAssist maintainers, including engineers and security researchers from organizations such as NVIDIA and Tencent. See the [maintainer list](CONTRIBUTING.md#maintainers).

## Report a Security Issue

Report vulnerabilities directly to the repository where the issue lives:

- **Core CLI and gateway** — [nodoassist/nodoassist](https://github.com/openclaw/openclaw)
- **macOS desktop app** — [nodoassist/nodoassist](https://github.com/openclaw/openclaw) (apps/macos)
- **iOS app** — [nodoassist/nodoassist](https://github.com/openclaw/openclaw) (apps/ios)
- **Android app** — [nodoassist/nodoassist](https://github.com/openclaw/openclaw) (apps/android)
- **ClawHub** — [nodoassist/clawhub](https://github.com/openclaw/clawhub)
- **Trust and threat model** — [nodoassist/trust](https://github.com/openclaw/trust)
...
For NodoAssist core issues, submit through a private [GitHub Security Advisory](https://github.com/openclaw/openclaw/security/advisories/new). Do not open a public issue or PR that discloses an unpatched vulnerability, exploit path, secret, or security-sensitive proof of concept.
...
NodoAssist does not currently run a paid bug bounty program. Please still disclose responsibly so we can fix real issues quickly. The best way to help the project right now is to send high-signal reports and, when practical, focused PRs.
...
- The actual impact, including which NodoAssist trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an NodoAssist security boundary.
...
- A report whose only primitive is changing the process or child-process environment before running NodoAssist or an executable NodoAssist invokes.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated NodoAssist impact.
...
- Tested version details (NodoAssist version and/or commit SHA).
...
- For dependency CVE reports, evidence that the shipped dependency version is actually affected, plus a PoC that reproduces impact through NodoAssist. Showing that NodoAssist can reach a native parser is not enough by itself.
- Demonstrated impact tied to NodoAssist's documented trust boundaries.
- For exposed-secret reports: proof the credential is NodoAssist-owned (or grants access to NodoAssist-operated infrastructure/services).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example image codec libraries such as libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through NodoAssist. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under NodoAssist's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
- Reports that depend on attacker-controlled environment variables changing executable behavior, including variables that redirect lookup paths, preload code, select wrappers/interpreters, alter package-manager or runtime hooks, or make one executable call another executable. Control of the process or child-process environment is trusted host/operator control in NodoAssist's model; these reports need a separate NodoAssist boundary bypass that lets untrusted input set or mutate that environment.
...
NodoAssist does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- Concretely, on the OpenAI-compatible HTTP surface:
  ...
  - narrower `x-nodoassist-scopes` headers are ignored for that shared-secret path
...
- NodoAssist can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need NodoAssist, use one VPS (or host/OS user boundary) per user.
...
- This is expected in NodoAssist's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of NodoAssist's trusted computing base for a gateway.
...
- Using NodoAssist in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped NodoAssist production code or a published package artifact intended for users.
...
- Reports that require write access to trusted local state (`~/.nodoassist`, workspace files like `MEMORY.md` / `memory/*.md`)
...
- Exposed secrets that are third-party/user-controlled credentials (not NodoAssist-owned and not granting access to NodoAssist-operated infrastructure/services) without demonstrated NodoAssist impact
- Reports whose only claim is host-side exec when sandbox runtime is disabled/unavailable (documented default behavior in the trusted-operator model), without a boundary bypass.
- Reports whose only claim is that a platform-provided upload destination URL is untrusted (for example Microsoft Teams `fileConsent/invoke` `uploadInfo.uploadUrl`) without proving attacker control in an authenticated production flow.
- SSRF reports limited to the operator-managed HTTP/WebSocket proxy-routing feature where the demonstrated mitigation is to enable/configure `proxy.enabled` with a filtering `proxy.proxyUrl`/`NODOASSIST_PROXY_URL`, or where impact depends on a permissive/misconfigured operator proxy. This only covers normal process-local HTTP(S)/WebSocket egress (`fetch`, Node HTTP(S), and similar JavaScript clients); non-HTTP egress and other features are assessed separately. See [Network proxy](https://docs.openclaw.ai/security/network-proxy).

### Deployment Assumptions

NodoAssist security guidance assumes:

- The host where NodoAssist runs is within a trusted OS/admin boundary.
- Anyone who can set or mutate the NodoAssist process environment, launcher environment, or child-process environment is inside that trusted host/operator boundary.
- Anyone who can modify `~/.nodoassist` state/config (including `nodoassist.json`) is effectively a trusted operator.
...
NodoAssist's security model is "personal assistant" (one trusted operator, potentially many agents), not "shared multi-tenant bus."
...
NodoAssist separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when NodoAssist can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the NodoAssist process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

NodoAssist uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:

- Preferred temp root: `/tmp/nodoassist` (when available and safe on the host).
- Fallback temp root: `os.tmpdir()/nodoassist` (or `nodoassist-<uid>` on multi-user hosts).

Security boundary notes:

- Sandbox media validation allows absolute temp paths only under the NodoAssist-managed temp root.
- Arbitrary host tmp paths are not treated as trusted media roots.
- Plugin/extension code should use NodoAssist temp helpers (`resolvePreferredNodoAssistTmpDir`, `buildRandomTempFilePath`, `withTempDownloadPath`) rather than raw `os.tmpdir()` defaults when handling media files.
- Enforcement reference points:
  - temp root resolver: `src/infra/tmp-nodoassist-dir.ts`
...
For threat model + hardening guidance (including `nodoassist security audit --deep` and `--fix`), see:
...
NodoAssist's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.

- Recommended: keep the Gateway **loopback-only** (`127.0.0.1` / `::1`).
  - Config: `gateway.bind="loopback"` (default).
  - CLI: `nodoassist gateway run --bind loopback`.
- `gateway.controlUi.dangerouslyDisableDeviceAuth` is intended for localhost-only break-glass use.
  - NodoAssist keeps deployment flexibility by design and does not hard-forbid non-local setups.
  - Non-local and other risky configurations are surfaced by `nodoassist security audit` as dangerous findings.
...
- Canvas host note: network-visible canvas is **intentional** for trusted node scenarios (LAN/tailnet).
  ...
  - Expected routes: `/__nodoassist__/canvas/`, `/__nodoassist__/a2ui/`.
  - This deployment model alone is not a security vulnerability.
- Do **not** expose it to the public internet (no direct bind to `0.0.0.0`, no public reverse proxy). It is not hardened for public exposure.
- If you need remote access, prefer an SSH tunnel or Tailscale serve/funnel (so the Gateway still binds to loopback), plus strong Gateway auth.
- The Gateway HTTP surface includes the canvas host (`/__nodoassist__/canvas/`, `/__nodoassist__/a2ui/`). Treat canvas content as sensitive/untrusted and avoid exposing it beyond loopback unless you understand the risk.

## Runtime Requirements

### Node.js Version

NodoAssist requires **Node.js 22.19+, Node.js 23.11+, or Node.js 24+**. Node 24 is the recommended default runtime for new installs. The minimum supported Node 22 version includes important security patches:
...
When running NodoAssist in Docker:
...
docker run --read-only --cap-drop=ALL \
  -v nodoassist-data:/app/data \
  nodoassist/nodoassist:latest
```

## Security Scanning

NodoAssist uses several security and release-validation layers. No single scanner is treated as the boundary.

### Secret Detection

NodoAssist runs the pre-commit `detect-private-key` hook in CI and keeps secret-resolution behavior covered by the dedicated secrets test surface.
