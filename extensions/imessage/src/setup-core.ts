...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: NodoAssistConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: NodoAssistConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: NodoAssistConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "For the usual setup, run NodoAssist on the Mac signed into Messages.",
    ...
    "After Private API setup, run `imsg launch`, then `nodoassist channels status --probe`.",
    "Ensure NodoAssist has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: NodoAssistConfig; accountId?: string }) =>
