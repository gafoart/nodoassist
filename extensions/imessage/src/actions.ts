...
import { readBooleanParam } from "nodoassist/plugin-sdk/boolean-param";
import {
  createActionGate,
  jsonResult,
...
import { normalizePollInput } from "nodoassist/plugin-sdk/poll-runtime";
...
import { extractToolSend } from "nodoassist/plugin-sdk/tool-send";
  ...
  handleAction: async ({
    ...
    const assertPrivateApiEnabled = async () => {
      ...
      if (!privateApiStatus?.available) {
        ...
        const reason = privateApiStatus?.statusMessage
          ...
          : "";
        log.warn(
          `iMessage ${action} blocked: private API bridge unavailable (accountId=${account.accountId}, cliPath=${cliPathForProbe}). Run \`imsg launch\` to re-inject the dylib, then \`nodoassist channels status --probe\` to refresh.${reason}`,
        );
        throw new Error(
          `iMessage ${action} requires the imsg private API bridge. Run imsg launch, then nodoassist channels status --probe to refresh capability detection.${reason}`,
        );
      }
    };
    const opts = {
    ...
    if (action === "reply") {
      ...
      if (attachment) {
        ...
        // (nodoassist/imsg#114). Older imsg builds reject the option, so
        ...
        // of nodoassist/nodoassist#79822.
        if (privateApiStatus?.cliCapabilities?.sendRichSupportsAttachment !== true) {
          throw new Error(
            "iMessage reply with an attachment needs an imsg build that exposes `send-rich --file` " +
              "(nodoassist/imsg#114). Upgrade imsg, or use action 'upload-file' (with filePath/filename) " +
    ...
    if (action === "poll") {
      ...
      if (privateApiStatus?.selectors?.pollPayloadMessage !== true) {
        throw new Error(
          "iMessage poll requires an imsg bridge that advertises the pollPayloadMessage selector. Update imsg, run imsg launch to re-inject the bridge, then run nodoassist channels status --probe to refresh capability detection.",
        );
      }
    ...
    if (action === "poll-vote") {
      ...
      if (privateApiStatus?.selectors?.pollVoteMessage !== true) {
        throw new Error(
          "iMessage poll-vote requires an imsg bridge that advertises the pollVoteMessage selector. Update imsg, run imsg launch to re-inject the bridge, then run nodoassist channels status --probe to refresh capability detection.",
        );
      }
      ...
      if (!imessageRpcSupportsMethod(privateApiStatus, "poll.vote")) {
        throw new Error(
          "iMessage poll-vote requires an imsg build that advertises the poll.vote capability. Update imsg, then run nodoassist channels status --probe to refresh capability detection.",
        );
      }
