/**
 * NodoAssist Memory (LanceDB) Plugin
 *
...
import type { AgentToolResult } from "nodoassist/plugin-sdk/agent-core";
import {
  optionalFiniteNumberSchema,
  optionalPositiveIntegerSchema,
} from "nodoassist/plugin-sdk/channel-actions";
import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "nodoassist/plugin-sdk/chat-channel-ids";
import type { NodoAssistConfig } from "nodoassist/plugin-sdk/config-contracts";
import { createLazyRuntimeModule } from "nodoassist/plugin-sdk/lazy-runtime";
import type { MemoryEmbeddingProvider } from "nodoassist/plugin-sdk/memory-core-host-engine-embeddings";
...
import { resolveLivePluginConfigObject } from "nodoassist/plugin-sdk/plugin-config-runtime";
...
import { definePluginEntry, type NodoAssistPluginApi } from "./api.js";
import {
...
const loadMemoryEmbeddingProviderModule = createLazyRuntimeModule(
  () => import("nodoassist/plugin-sdk/memory-core-host-engine-embeddings"),
);
const loadMemoryHostCoreModule = createLazyRuntimeModule(
  () => import("nodoassist/plugin-sdk/memory-host-core"),
);
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  constructor(
    private api: NodoAssistPluginApi,
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as NodoAssistConfig;
...
function createEmbeddings(api: NodoAssistPluginApi, cfg: MemoryConfig): Embeddings {
...
/**
 * Matches JSON blobs that look like NodoAssist transport envelope metadata.
...
/**
 * Returns true if `text` looks like it contains NodoAssist-injected envelope or
...
/**
 * Strips NodoAssist-injected envelope metadata from a user message so that only
    ...
    const resolveCurrentHookConfig = () => {
      const runtimePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as NodoAssistConfig
          : undefined,
        "memory-lancedb",
