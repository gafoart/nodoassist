...
import { addTimerTimeoutGraceMs } from "nodoassist/plugin-sdk/number-runtime";
import type { PluginRuntime } from "nodoassist/plugin-sdk/plugin-runtime";
import type { RuntimeLogger } from "nodoassist/plugin-sdk/plugin-runtime";
  ...
  if (result.code !== 0 || !outputMentionsBlackHole2ch(output)) {
    ...
    throw new Error(
      [
        ...
        "Install BlackHole 2ch and route Chrome input/output through the NodoAssist audio bridge.",
        hint,
      ]
        .filter(Boolean)
        .join(" "),
  ...
  > => {
    ...
    if (params.config.chrome.audioBridgeCommand) {
      if (params.mode === "agent") {
        throw new Error(
          "Chrome agent mode requires chrome.audioInputCommand and chrome.audioOutputCommand so NodoAssist can run STT and regular TTS directly.",
        );
      }
...
function meetStatusScript(params: {
  ...
  const captionState = (() => {
    ...
    const w = window;
    if (!inCall && !w.__nodoassistMeetCaptions) return undefined;
    if (!w.__nodoassistMeetCaptions) {
      w.__nodoassistMeetCaptions = {
    ...
    return w.__nodoassistMeetCaptions;
  })();
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    ...
    manualActionMessage = "Sign in to Google in the NodoAssist browser profile, then retry the Meet join.";
    ...
    manualActionMessage = "Admit the NodoAssist browser participant in Google Meet, then retry speech.";
  } else if (permissionNeeded) {
    manualActionReason = "meet-permission-required";
    manualActionMessage = allowMicrophone
      ? "Allow microphone/camera/speaker permissions for Meet in the NodoAssist browser profile, then retry."
      : "Join without microphone/camera permissions in the NodoAssist browser profile, then retry.";
    ...
    manualActionMessage = allowMicrophone
      ? "Meet is showing the microphone choice. Click Use microphone in the NodoAssist browser profile, then retry."
      : "Meet is showing the microphone choice. Choose the no-microphone option in the NodoAssist browser profile, then retry.";
  }
  ...
  do {
    try {
      ...
      browser = {
        ...
        manualActionMessage:
          "Open the NodoAssist browser profile, finish Google Meet login, admission, or permission prompts, then retry.",
        notes: [
          ...permissionNotes,
