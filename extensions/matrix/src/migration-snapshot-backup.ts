...
import { writeJsonFileAtomically } from "nodoassist/plugin-sdk/json-store";
import { resolveRequiredHomeDir, resolveStateDir } from "nodoassist/plugin-sdk/state-paths";

const MATRIX_MIGRATION_SNAPSHOT_DIRNAME = "nodoassist-migrations";
...
export async function maybeCreateMatrixMigrationSnapshot(params: {
  ...
  outputDir?: string;
  createBackupArchive?: typeof import("nodoassist/plugin-sdk/runtime").createBackupArchive;
  log?: { info?: (message: string) => void; warn?: (message: string) => void };
}): Promise<MatrixMigrationSnapshotResult> {
  const env = params.env ?? process.env;
  const createBackupArchive =
    params.createBackupArchive ??
    (await import("nodoassist/plugin-sdk/runtime")).createBackupArchive;
