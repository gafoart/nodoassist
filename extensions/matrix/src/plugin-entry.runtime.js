...
function hasTrustedNodoAssistRootIndicator(packageRoot, packageJson) {
  ...
  const hasNodoAssistBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("nodoassist")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.nodoassist === "string");
  const hasNodoAssistEntrypoint = fs.existsSync(path.join(packageRoot, "nodoassist.mjs"));
  return hasCliEntryExport || hasNodoAssistBin || hasNodoAssistEntrypoint;
}

function findNodoAssistPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "nodoassist" && hasTrustedNodoAssistRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findNodoAssistPackageRoot(moduleDir);
  if (location) {
