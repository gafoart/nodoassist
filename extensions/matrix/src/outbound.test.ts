...
describe("matrixOutbound cfg threading", () => {
  ...
  it("passes resolved cfg to sendMessageMatrix for media sends", async () => {
    ...
    await matrixOutbound.sendMedia!({
      ...
      mediaLocalRoots: ["/tmp/nodoassist"],
      accountId: "default",
      audioAsVoice: true,
    });
    ...
    expect(options.mediaLocalRoots).toEqual(["/tmp/nodoassist"]);
  ...
  it("renders MessagePresentation into Matrix custom content metadata", async () => {
    ...
    expect(matrixData.extraContent?.["com.nodoassist.presentation"]).toEqual({
      ...presentation,
      version: 1,
    ...
    await matrixOutbound.sendPayload!({
      ...
      payload: {
        ...
        channelData: {
          matrix: {
            extraContent: {
              "com.nodoassist.presentation": presentationContent,
            },
          },
        },
      },
