...
const createBackupArchiveMock = vi.hoisted(() =>
  vi.fn(async (_params: unknown) => ({
    ...
    archiveRoot: "2026-03-17-nodoassist-backup",
...
describe("matrix client storage paths", () => {
  ...
  function setupStateDir(
    ...
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-matrix-storage-"));
  ...
  function createMigrationEnv(stateDir: string): NodeJS.ProcessEnv {
    return {
      ...
      NODOASSIST_HOME: path.dirname(stateDir),
  ...
  it("reads legacy storage metadata until doctor migrates it to SQLite", () => {
    ...
    expect(rotatedStoragePaths.rootDir).toBe(oldStoragePaths.rootDir);
    expect(fs.existsSync(path.join(oldStoragePaths.rootDir, "state", "nodoassist.sqlite"))).toBe(
      false,
    );
  });
