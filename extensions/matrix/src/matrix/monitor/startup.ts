...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleNodoAssistDevices.length > 0) {
      params.logger.warn(
        `matrix: stale NodoAssist devices detected for ${params.auth.userId}: ${deviceHealth.staleNodoAssistDevices.map((device) => device.deviceId).join(", ")}. Run 'nodoassist matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
