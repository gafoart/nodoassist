...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'nodoassist matrix verify status --verbose --account ${accountId}' and 'nodoassist matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another NodoAssist runtime is using the same account. " +
    `Check 'nodoassist matrix verify status --verbose --account ${accountId}' and 'nodoassist matrix devices list --account ${accountId}'.`
  );
}
