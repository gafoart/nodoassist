...
import { createReplyToFanout } from "nodoassist/plugin-sdk/channel-outbound";
import {
  renderMessagePresentationFallbackText,
  type MessagePresentation,
} from "nodoassist/plugin-sdk/interactive-runtime";
import { resolvePayloadMediaUrls } from "nodoassist/plugin-sdk/reply-payload";
...
const MATRIX_NODOASSIST_PRESENTATION_KEY = "com.nodoassist.presentation" as const;
const MATRIX_NODOASSIST_PRESENTATION_TYPE = "message.presentation" as const;
...
function buildMatrixPresentationContent(presentation: MessagePresentation) {
  return {
    ...presentation,
    version: 1,
    type: MATRIX_NODOASSIST_PRESENTATION_TYPE,
  };
}
...
function resolveMatrixPresentationContent(
  ...
  const presentation = toRecord(extraContent?.[MATRIX_NODOASSIST_PRESENTATION_KEY]);
  if (
    !presentation ||
    presentation.version !== 1 ||
    presentation.type !== MATRIX_NODOASSIST_PRESENTATION_TYPE
  ) {
    return undefined;
  }
  return presentation;
  ...
  return {
    ...
    channelData: {
      ...
      matrix: {
        ...matrixData,
        extraContent: {
          [MATRIX_NODOASSIST_PRESENTATION_KEY]: buildMatrixPresentationContent(params.presentation),
        },
      },
    },
  };
...
function resolveMatrixExtraContent(payload: ReplyPayload): MatrixExtraContentFields | undefined {
  ...
  return presentation ? { [MATRIX_NODOASSIST_PRESENTATION_KEY]: presentation } : undefined;
}
