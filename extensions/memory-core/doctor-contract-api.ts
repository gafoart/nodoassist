...
import { resolveUserPath } from "nodoassist/plugin-sdk/memory-core-host-engine-foundation";
import {
...
import { resolveMemoryDreamingWorkspaces } from "nodoassist/plugin-sdk/memory-core-host-status";
...
} from "nodoassist/plugin-sdk/sqlite-runtime";
import {
...
async function collectLegacyMemorySidecarSources(params: {
  ...
  const migrationEnv = { ...params.env, NODOASSIST_STATE_DIR: params.stateDir };
  ...
  async function addSource(agentId: string, legacyPath: string): Promise<void> {
    ...
    sources.push({
      ...
      agentDatabasePath: resolveNodoAssistAgentSqlitePath({ agentId, env: migrationEnv }),
    });
  }
...
async function migrateLegacyMemorySidecarSource(params: {
  ...
  try {
    const migrationEnv = {
      ...params.env,
      NODOASSIST_STATE_DIR: params.source.stateDir,
    };
    ensureNodoAssistAgentDatabaseSchema(db, {
