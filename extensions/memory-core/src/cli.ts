    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nodoassist memory status", "Show index and provider status."],
          [
            "nodoassist memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["nodoassist memory status --deep", "Probe embedding provider readiness."],
          ["nodoassist memory index --force", "Force a full reindex."],
          ['nodoassist memory search "meeting notes"', "Quick search using positional query."],
          [
            'nodoassist memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `nodoassist memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
            "Review weighted short-term candidates for long-term memory.",
          ],
          [
            "nodoassist memory promote --apply",
            "Append top-ranked short-term candidates into MEMORY.md.",
          ],
          [
            'nodoassist memory promote-explain "router vlan"',
            "Explain why a specific candidate would or would not promote.",
          ],
          [
            "nodoassist memory rem-harness --json",
            "Preview REM reflections, candidate truths, and deep promotion output.",
          ],
          [
            "nodoassist memory rem-backfill --path ./memory",
          ...
          ["nodoassist memory status --json", "Output machine-readable JSON (good for scripts)."],
