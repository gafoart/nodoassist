...
import { emitSessionTranscriptUpdate } from "nodoassist/plugin-sdk/agent-harness-runtime";
import {
...
const MEMORY_CORE_TRANSCRIPT_UPDATE_SUBSCRIBER_KEY = Symbol.for(
  "nodoassist.memoryCore.sessionTranscriptUpdateSubscriber",
);
const originalStartupStateDir = process.env.NODOASSIST_STATE_DIR;
const originalStartupConfigPath = process.env.NODOASSIST_CONFIG_PATH;
...
function setStartupConfigPath(configPath: string): void {
  Reflect.set(process.env, "NODOASSIST_CONFIG_PATH", configPath);
}
...
function restoreStartupEnv(): void {
  if (originalStartupStateDir === undefined) {
    ...
    Reflect.set(process.env, "NODOASSIST_STATE_DIR", originalStartupStateDir);
  }
  if (originalStartupConfigPath === undefined) {
    Reflect.deleteProperty(process.env, "NODOASSIST_CONFIG_PATH");
  } else {
    Reflect.set(process.env, "NODOASSIST_CONFIG_PATH", originalStartupConfigPath);
  }
}
...
describe("session startup catch-up", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-session-startup-"));
