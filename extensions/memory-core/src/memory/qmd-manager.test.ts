...
import { withMockedWindowsPlatform } from "nodoassist/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("nodoassist.memoryEmbeddingProviders");
...
vi.mock("nodoassist/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("nodoassist/plugin-sdk/memory-core-host-engine-foundation")
  >("nodoassist/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/file-lock")>(
    "nodoassist/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { NodoAssistConfig } from "nodoassist/plugin-sdk/memory-core-host-engine-foundation";
import {
...
const originalQmdStateDir = process.env.NODOASSIST_STATE_DIR;
...
function restoreQmdStateDir(): void {
  if (originalQmdStateDir === undefined) {
    ...
    Reflect.set(process.env, "NODOASSIST_STATE_DIR", originalQmdStateDir);
  }
}
...
describe("QmdMemoryManager", () => {
  ...
  it("rebinds a stale in-container collection root to the host workspace (sandbox-mode transition)", async () => {
    ...
    // /home/node/.nodoassist/... stored that in-container path as the collection root. Resolved
    ...
    const containerRoot = "/home/node/.nodoassist/teams/x/workspace";
  ...
  it("parseShownCollection extracts path and pattern from qmd collection show output", async () => {
    ...
    const sampleOutput = [
      ...
      "  Path:     /home/node/.nodoassist/teams/example-team/workspace-example/memory",
      "  Pattern:  **/*.md",
      "  Include:  yes (default)",
    ].join("\n");

    const result = parser(sampleOutput);
    expect(result.path).toBe("/home/node/.nodoassist/teams/example-team/workspace-example/memory");
