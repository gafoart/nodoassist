...
const originalReindexStateDir = process.env.NODOASSIST_STATE_DIR;
...
function restoreReindexStateDir(): void {
  if (originalReindexStateDir === undefined) {
    ...
    Reflect.set(process.env, "NODOASSIST_STATE_DIR", originalReindexStateDir);
  }
}
...
describe("memory manager reindex recovery", () => {
  ...
  beforeEach(async () => {
    resetEmbeddingMocks();
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-mem-reindex-recovery-"));
  ...
  async function openManager(cfg: NodoAssistConfig): Promise<MemoryIndexManager> {
  ...
  it("rejects a full reindex while another process owns the build lock", async () => {
    ...
    const harness = memoryManager as unknown as ReindexHarness;
    const databasePath = resolveNodoAssistAgentSqlitePath({ agentId: "main" });
