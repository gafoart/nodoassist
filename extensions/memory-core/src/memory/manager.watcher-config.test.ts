...
const {
  ...
  const chokidarKey = Symbol.for("nodoassist.test.memoryWatchFactory");
  const nativeKey = Symbol.for("nodoassist.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("nodoassist.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("nodoassist.test.memoryNativeWatchFactory");
const originalWatcherStateDir = process.env.NODOASSIST_STATE_DIR;
...
function restoreWatcherStateDir(): void {
  if (originalWatcherStateDir === undefined) {
    ...
    Reflect.set(process.env, "NODOASSIST_STATE_DIR", originalWatcherStateDir);
  }
}

vi.mock("nodoassist/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("nodoassist/plugin-sdk/memory-core-host-engine-foundation")
    >();
  return {
    ...actual,
...
import { clearMemoryEmbeddingProviders as clearRegistry } from "nodoassist/plugin-sdk/memory-core-host-engine-embeddings";
import {
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): NodoAssistConfig {
    const defaults: NonNullable<NonNullable<NodoAssistConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: NodoAssistConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. NodoAssist watches directories only so
