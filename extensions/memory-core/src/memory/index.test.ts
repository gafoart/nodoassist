...
import { hashText } from "nodoassist/plugin-sdk/memory-core-host-engine-storage";
import { resolveSessionTranscriptsDirForAgent } from "nodoassist/plugin-sdk/memory-core-host-runtime-core";
import { resolveNodoAssistAgentSqlitePath } from "nodoassist/plugin-sdk/sqlite-runtime";
import {
  closeNodoAssistAgentDatabasesForTest,
  closeNodoAssistStateDatabaseForTest,
  openNodoAssistAgentDatabase,
} from "nodoassist/plugin-sdk/sqlite-runtime-testing";
...
const originalMemoryIndexStateDir = process.env.NODOASSIST_STATE_DIR;
...
function restoreMemoryIndexStateDir(): void {
  if (originalMemoryIndexStateDir === undefined) {
    ...
    Reflect.set(process.env, "NODOASSIST_STATE_DIR", originalMemoryIndexStateDir);
  }
}
...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-mem-fixtures-"));
  ...
  it("reindexes memory tables in place without deleting unrelated agent rows", async () => {
    ...
    const agentDbPath = resolveNodoAssistAgentSqlitePath({ agentId: "main" });
    const agentDb = openNodoAssistAgentDatabase({ agentId: "main" });
    agentDb.db
    ...
    const reopened = openNodoAssistAgentDatabase({ agentId: "main" });
    expect(
      reopened.db
  ...
  it("status-purpose manager detects unindexed session transcripts as dirty", async () => {
    // Regression test for #97814: plain nodoassist memory status (purpose: status)
