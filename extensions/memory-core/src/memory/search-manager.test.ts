...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "nodoassist/plugin-sdk/memory-core-host-engine-qmd";
...
function createBuiltinCfg(agentId: string): NodoAssistConfig {
  return {
    agents: {
      defaults: {
        workspace: "/tmp/workspace",
...
async function expectPendingQmdReplacement(params: {
  agentId: string;
  firstCfg: NodoAssistConfig;
  secondCfg: NodoAssistConfig;
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("nodoassist.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-qmd-workspace-"));
