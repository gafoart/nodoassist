...
import type { PluginCommandContext } from "nodoassist/plugin-sdk/core";
...
function resolveStoredDreaming(config: NodoAssistConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: NodoAssistConfig = {}) {
  let runtimeConfig: NodoAssistConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: NodoAssistConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: NodoAssistConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
