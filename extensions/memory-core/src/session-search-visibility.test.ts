...
import * as sessionTranscriptHit from "nodoassist/plugin-sdk/session-transcript-hit";
...
import { asNodoAssistConfig } from "./tools.test-helpers.js";
...
describe("filterMemorySearchHitsBySessionVisibility", () => {
  ...
  async function createQmdArtifactIndex(params: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-qmd-session-artifact-"));
    tempRoots.push(root);
  ...
  it("drops sessions-sourced hits when requester key is missing (fail closed)", async () => {
    const cfg = asNodoAssistConfig({ tools: { sessions: { visibility: "all" } } });
