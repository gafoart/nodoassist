...
import { requireNodeSqlite } from "nodoassist/plugin-sdk/memory-core-host-engine-storage";
import type { MemorySearchResult } from "nodoassist/plugin-sdk/memory-core-host-runtime-files";

const QMD_SESSION_ARTIFACT_TABLE = "nodoassist_qmd_session_artifacts";

const QMD_SESSION_ARTIFACT_HIT: unique symbol = Symbol("nodoassist.qmdSessionArtifactHit");
...
function ensureQmdSessionArtifactSchema(db: DatabaseSync): void {
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_nodoassist_qmd_session_artifacts_docid
     ON ${QMD_SESSION_ARTIFACT_TABLE} (docid)`,
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_nodoassist_qmd_session_artifacts_search_path
