...
import type { MemorySearchRuntimeDebug } from "nodoassist/plugin-sdk/memory-core-host-runtime-files";
...
describe("memory_search unavailable payloads", () => {
  ...
  it("returns unavailable metadata when the index identity is paused", async () => {
    ...
    expectUnavailableMemorySearchDetails(result.details, {
      ...
      action:
        "Tell the user to run: nodoassist memory status --index or nodoassist memory index --force.",
    });
    expect(searchCalls).toBe(1);
    expect(getMemorySyncMockCalls()).toBe(0);
  });
...
describe("memory_search corpus labels", () => {
  ...
  it("re-resolves config when executing a previously created tool", async () => {
    const startupConfig = asNodoAssistConfig({
      agents: {
        defaults: {
          memorySearch: {
            provider: "ollama",
    ...
    const patchedConfig = asNodoAssistConfig({
      agents: {
        defaults: {
          memorySearch: {
            provider: "openai",
