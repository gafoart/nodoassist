...
function previewGatewayLoopbackBind(
  ...
  if (
    finding.ocPath !== "oc://nodoassist.config/gateway/bind" &&
    finding.ocPath !== "oc://nodoassist.config/gateway/customBindHost"
  ) {
    return [];
  }
  return [
...
function previewGatewayNodeDenyCommand(
  ...
  if (
    command === undefined ||
    command === "" ||
    finding.ocPath !== "oc://nodoassist.config/gateway/nodes/denyCommands"
  ) {
    return [];
  }
  return [
