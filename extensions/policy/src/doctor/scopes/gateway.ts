...
import type { HealthCheck, HealthFinding } from "nodoassist/plugin-sdk/health";
  ...
  return policyDenied
    ...
    .map((command): HealthFinding => {
      return {
        ...
        severity: "error",
        message: `Gateway node command '${command}' is denied by policy but not denied by NodoAssist config.`,
        source: "policy",
        path: "nodoassist config",
