...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://nodoassist.config/plugins/entries/policy/config/expectedHash",
  ...
  return [
    {
      ...
      requirement: "oc://nodoassist.config/plugins/entries/policy/config/expectedAttestationHash",
...
function isGroupIngressDisabled(
  ...
  const channelDefaultsParent = "oc://nodoassist.config/channels/defaults";
    ...
    if (invalidIndex >= 0) {
      findings.push(
        policyShapeFinding(
          ...
          "Use non-empty source names such as env, file, exec, or nodoassist.",
        ),
      );
    }
  }
...
function resolvePolicyArtifactHomeDir(): string | undefined {
  const explicitHome = normalizedEnvValue(process.env.NODOASSIST_HOME);
  ...
  return [
    ...new Set(
      findings
        ...
        .map((finding) => finding.ocPath?.match(/^oc:\/\/nodoassist\.config\/channels\/(.+)$/)?.[1])
...
function canonicalExecApprovalsPath(): string {
  return "~/.nodoassist/exec-approvals.json";
}
...
function execApprovalsArtifactLocation(ctx: HealthCheckContext): {
  ...
  const stateDir = normalizedEnvValue(process.env.NODOASSIST_STATE_DIR);
