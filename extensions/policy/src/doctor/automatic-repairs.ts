...
type RepairPatch = {
  readonly config: NodoAssistConfig;
...
function mergeRequiredDenyTools(
  ...
  for (const finding of findings) {
    ...
    if (
      ...
      finding.ocPath === "oc://nodoassist.config/tools/deny"
    ) {
      warnings.push(
  ...
  return changes.length > 0
    ? { config: next as NodoAssistConfig, changes: uniqueStrings(changes), warnings }
...
function disableElevatedTools(
  ...
  if (
    !findings.some((finding) => finding.ocPath === "oc://nodoassist.config/tools/elevated/enabled")
  ) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
  ...
  return {
    config: next as NodoAssistConfig,
...
function disableInsecureControlUi(
  ...
  const changes: string[] = [];
  const fields = [
    ["allowInsecureAuth", "oc://nodoassist.config/gateway/controlUi/allowInsecureAuth"],
    [
  ...
  return changes.length > 0
    ? { config: next as NodoAssistConfig, changes }
    : { config: cfg, changes };
}
...
function disableRemoteGatewayMode(
  ...
  if (!findings.some((finding) => finding.ocPath === "oc://nodoassist.config/gateway/mode")) {
    return { config: cfg, changes: [] };
  }
  const next = cloneConfig(cfg);
  const gateway = ensureRecord(next, "gateway");
...
function enableSensitiveLoggingRedaction(cfg: NodoAssistConfig): RepairPatch {
  const next = cloneConfig(cfg);
...
function disableTelemetryContentCapture(cfg: NodoAssistConfig): RepairPatch {
  const next = cloneConfig(cfg);
...
function cloneConfig(cfg: NodoAssistConfig): ConfigRecord {
...
function configPathSegments(ocPath: string): readonly string[] {
  const prefix = "oc://nodoassist.config/";
...
function skippedUnsafeScopedRepair(cfg: NodoAssistConfig, warning: string): RepairPatch {
...
function isScopedInheritedChannelDefaultFinding(finding: HealthFinding): boolean {
  return (
    hasScopedPolicyRequirement([finding]) &&
    finding.ocPath?.startsWith("oc://nodoassist.config/channels/defaults/") === true
  );
}
