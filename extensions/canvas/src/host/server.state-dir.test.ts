...
import { withStateDirEnv } from "nodoassist/plugin-sdk/test-env";
...
describe("canvas host state dir defaults", () => {
  ...
  it("uses NODOASSIST_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("nodoassist-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("NodoAssist Canvas");
      } finally {
        await handler.close();
      }
    });
