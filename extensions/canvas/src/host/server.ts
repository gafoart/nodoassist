...
import { detectMime } from "nodoassist/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "nodoassist/plugin-sdk/runtime-env";
import { resolveStateDir } from "nodoassist/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "nodoassist/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "nodoassist/plugin-sdk/text-utility-runtime";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.nodoassistCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.nodoassistCanvasA2UIAction &&
        typeof window.nodoassistCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.nodoassistSendUserAction === "function";
  ...
  window.addEventListener("nodoassist:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android NodoAssist node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.nodoassistSendUserAction === "function"
        ? window.nodoassistSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.NODOASSIST_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>NodoAssist Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
