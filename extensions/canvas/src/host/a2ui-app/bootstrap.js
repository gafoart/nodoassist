...
import { themeContext } from "@nodoassist/a2ui-theme-context";
...
class NodoAssistA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.nodoassistA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["nodoassist:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.nodoassistCanvasA2UIAction ??
      globalThis.nodoassistCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.nodoassistCanvasA2UIAction) {
...
if (!customElements.get("nodoassist-a2ui-host")) {
  customElements.define("nodoassist-a2ui-host", NodoAssistA2UIHost);
}
