...
export function isCanvasPluginEnabled(config?: NodoAssistConfig): boolean {
  if (!config) {
    return true;
  }
...
export function isCanvasHostEnabled(config?: NodoAssistConfig): boolean {
...
export const canvasConfigSchema: CanvasPluginConfigSchema = {
  ...
  uiHints: {
    ...
    "host.root": {
      ...
      help: "Directory to serve. Defaults to the NodoAssist state canvas directory.",
      advanced: true,
    },
    "host.port": {
