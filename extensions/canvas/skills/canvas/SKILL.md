---
name: canvas
description: "Present HTML on connected NodoAssist node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "nodoassist": { "emoji": "🖼️" } }
---
...
Active config: `$NODOASSIST_CONFIG_PATH` or `~/.nodoassist/nodoassist.json`.
...
4. Present the hosted URL: `/__nodoassist__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__nodoassist__/canvas/index.html
http://<gateway-host>:<gateway.port>/__nodoassist__/canvas/games/snake.html
```

Path mapping:

- `/__nodoassist__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__nodoassist__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
