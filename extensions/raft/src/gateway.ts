...
import type { ChannelGatewayContext } from "nodoassist/plugin-sdk/channel-contract";
import { keepHttpServerTaskAlive, waitUntilAbort } from "nodoassist/plugin-sdk/channel-outbound";
...
function spawnRaftBridge(params: {
  ...
  // Raft owns the fixed bridge command. NodoAssist passes profile/loopback
...
export async function startRaftGatewayAccount(
  ...
  if (!ctx.channelRuntime) {
    throw new Error(
      "Raft requires NodoAssist channel runtime support. Update NodoAssist and retry.",
    );
  }
  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        // Raft drains runtime activity after each wake pass. NodoAssist has no
      ...
      // Raft owns wake metadata and its schema evolution. NodoAssist accepts only
