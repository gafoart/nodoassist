...
import { createClaimableDedupe } from "nodoassist/plugin-sdk/persistent-dedupe";
import { resetPluginStateStoreForTests } from "nodoassist/plugin-sdk/plugin-state-test-runtime";
...
function makeTempDir(prefix: string): string {
  // nodoassist-temp-dir: allow extension tests cannot import root test helpers
...
function createContext(accountId = "default") {
  ...
  const ctx = {
    ...
    channelRuntime: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/nodoassist-agent.sqlite"),
...
describe("Raft wake gateway", () => {
  ...
  it("persists accepted wake dedupe across restarts without crossing accounts", async () => {
    const stateDir = makeTempDir("nodoassist-raft-wake-dedupe-");
    try {
