...
import { verifyChannelMessageAdapterCapabilityProofs } from "nodoassist/plugin-sdk/channel-outbound";
import { createSendCfgThreadingRuntime } from "nodoassist/plugin-sdk/channel-test-helpers";
...
vi.mock("nodoassist/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("nodoassist/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("nodoassist/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("nodoassist/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("nodoassist/plugin-sdk/plugin-config-runtime");
  vi.doUnmock("nodoassist/plugin-sdk/text-chunking");
  vi.resetModules();
});
