...
describe("irc normalize", () => {
  it("normalizes targets", () => {
    expect(normalizeIrcMessagingTarget("irc:channel:nodoassist")).toBe("#nodoassist");
  ...
  it("builds canonical channel and direct session routes", () => {
    ...
    expect(
      resolveIrcOutboundSessionRoute({
        cfg,
        agentId: "main",
        target: "irc:channel:nodoassist",
      }),
    ).toMatchObject({
      sessionKey: "agent:main:irc:group:#nodoassist",
      peer: { kind: "group", id: "#nodoassist" },
      chatType: "group",
