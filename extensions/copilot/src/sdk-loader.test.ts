...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { nodoassistDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { nodoassistDefaultImporterSentinel?: boolean };

      expect(sdk.nodoassistDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("nodoassist plugins install @nodoassist/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("nodoassist plugins install @nodoassist/copilot");
  });
  ...
  it("resolves the fallback dir from NODOASSIST_STATE_DIR for relocated profiles", () => {
    expect(
      resolveCopilotSdkFallbackDir({
        ...process.env,
        NODOASSIST_STATE_DIR: "/tmp/nodoassist-state",
      }),
    ).toBe(path.join("/tmp/nodoassist-state", "npm-runtime", "copilot"));
  });
