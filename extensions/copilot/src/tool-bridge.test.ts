...
import type { AnyAgentTool, SandboxContext } from "nodoassist/plugin-sdk/agent-harness-runtime";
import { afterEach, describe, expect, it, vi } from "vitest";
import {
  createCopilotToolBridge,
  convertNodoAssistToolToSdkTool,
  supportsModelTools,
} from "./tool-bridge.js";
...
describe("supportsModelTools", () => {
  it("returns true for github-copilot and false otherwise", () => {
    ...
    expect(supportsModelTools("nodoassist")).toBe(false);
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createNodoAssistCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createNodoAssistCodingTools).toHaveBeenCalledTimes(0);
  });

  it("allows vetted BYOK providers to expose model tools", async () => {
    const sourceTools = [makeTool()];
    const createNodoAssistCodingTools = vi.fn(async () => sourceTools);
    ...
    expect(createNodoAssistCodingTools).toHaveBeenCalledWith(
  ...
  it("forwards supported fields to injected createNodoAssistCodingTools", async () => {
    ...
    expect(createNodoAssistCodingTools).toHaveBeenCalledTimes(1);
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createNodoAssistCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("compacts the Copilot tool surface behind tool_search controls when enabled", async () => {
    const createNodoAssistCodingTools = vi.fn(async (opts: unknown) => {
  ...
  it("compacts the Copilot tool surface behind code-mode exec/wait when enabled", async () => {
    const createNodoAssistCodingTools = vi.fn(async () => [
      makeTool({ name: "fake_hidden" }),
      makeTool({ name: "read" }),
    ]);
  ...
  it("throws when createNodoAssistCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createNodoAssistCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createNodoAssistCodingTools must return an array");
  });

  it("throws when createNodoAssistCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createNodoAssistCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged NodoAssist tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createNodoAssistCodingTools,
        getOpts: () =>
          (createNodoAssistCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createNodoAssistCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building NodoAssist tools", async () => {
      const { createNodoAssistCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createNodoAssistCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createNodoAssistCodingTools", async () => {
  ...
  // NodoAssist attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createNodoAssistCodingTools", async () => {
      const createNodoAssistCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createNodoAssistCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("does not discard lean-mode overrides after tool construction", async () => {
      const result = await createCopilotToolBridge({
        ...
        createNodoAssistCodingTools: async () => [makeTool({ name: "image_generate" })],
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      });
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createNodoAssistCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeNodoAssistTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeNodoAssistTools).toBe(true);
    });
...
describe("convertNodoAssistToolToSdkTool", () => {
  it("throws on empty and non-string names", () => {
    expect(() => convertNodoAssistToolToSdkTool(makeTool({ name: "" as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
    expect(() => convertNodoAssistToolToSdkTool(makeTool({ name: 42 as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
  });
  ...
  it("throws on non-function execute", () => {
    expect(() =>
      convertNodoAssistToolToSdkTool(makeTool({ execute: "nope" as never }), {}),
  ...
  it("preserves name, description, and parameters exactly", () => {
    ...
    const result = convertNodoAssistToolToSdkTool(sourceTool, {});
  ...
  it("sets skipPermission: true so NodoAssist's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", () => {
    // Per the harness docs: every bridged NodoAssist tool comes from
    // `createNodoAssistCodingTools`, which already wraps each tool with
    ...
    // for kind: "custom-tool" would either short-circuit NodoAssist's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // NodoAssist's hook with the right context — mirrors codex
    // (`extensions/codex/src/app-server/dynamic-tools.ts`).
    const result = convertNodoAssistToolToSdkTool(makeTool(), {}) as SdkTool & {
  ...
  it("marks every bridged tool as overridesBuiltInTool so NodoAssist owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", () => {
    // Real-world dogfood found that nodoassist's createNodoAssistCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = convertNodoAssistToolToSdkTool(makeTool({ name }), {}) as SdkTool & {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = convertNodoAssistToolToSdkTool(sourceTool, { abortSignal: controller.signal });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertNodoAssistToolToSdkTool(sourceTool, { beforeExecute });
  ...
  it("returns a failure result when beforeExecute throws", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertNodoAssistToolToSdkTool(sourceTool, {
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = convertNodoAssistToolToSdkTool(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = convertNodoAssistToolToSdkTool(makeTool({}, sourceResult), {
      onAgentToolResult,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = convertNodoAssistToolToSdkTool(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned NodoAssist error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
