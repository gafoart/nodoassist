...
import { createAgentHarnessToolSurfaceRuntime } from "nodoassist/plugin-sdk/agent-harness-tool-runtime";

type CreateNodoAssistCodingTools =
  (typeof import("nodoassist/plugin-sdk/agent-harness"))["createNodoAssistCodingTools"];
type NodoAssistCodingToolsOptions = NonNullable<Parameters<CreateNodoAssistCodingTools>[0]>;
 ...
 * authoritative `createNodoAssistCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `nodoassist/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
export interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createNodoAssistCodingTools` so the
  ...
  createNodoAssistCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createNodoAssistCodingTools =
    input.createNodoAssistCodingTools ??
    (await import("nodoassist/plugin-sdk/agent-harness")).createNodoAssistCodingTools;
  ...
  const toolOptions = buildNodoAssistCodingToolsOptions(
    input,
    {
      ...effectiveToolPlan,
  ...
  try {
    sourceTools = await createNodoAssistCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createNodoAssistCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createNodoAssistCodingTools must return an array of tools",
    );
  }
  ...
  return {
    ...
    sdkTools: filteredTools.map((sourceTool) =>
      convertNodoAssistToolToSdkTool(sourceTool, {
        abortSignal: input.abortSignal,
...
/**
 * Builds the full `createNodoAssistCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged NodoAssist tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertNodoAssistToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
 ...
 */
function buildNodoAssistCodingToolsOptions(
  input: CopilotToolBridgeInput,
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as NodoAssistCodingToolsOptions["modelCompat"])
      : undefined;
...
export function convertNodoAssistToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  return {
    ...
    // NodoAssist owns its bridged tools by design (the harness docs:
    // "NodoAssist still owns ... NodoAssist dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with NodoAssist's coding-tool set. Mark every bridged tool as
    ...
    // same name." NodoAssist's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged NodoAssist tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit NodoAssist's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged NodoAssist tools are wrapped with
