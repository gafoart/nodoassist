...
// Filtering them out of the NodoAssist bootstrap injection avoids
...
/**
 * Loads NodoAssist workspace bootstrap files (IDENTITY.md, SOUL.md,
...
export async function resolveCopilotWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    // SOUL.md / IDENTITY.md / .nodoassist conventions live) onto the
...
export function renderCopilotWorkspaceBootstrapInstructions(
  ...
  const lines: string[] = [
    "NodoAssist loaded these user-editable workspace files. Treat them as project/user context. The Copilot SDK loads AGENTS.md natively from its instruction directories, so AGENTS.md is not repeated here.",
    "",
    "# Project Context",
    "",
