...
import type { AgentHarnessAttemptParams } from "nodoassist/plugin-sdk/agent-harness-runtime";
import { isLiveTestEnabled } from "nodoassist/plugin-sdk/test-env";
...
vi.mock("nodoassist/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nodoassist/plugin-sdk/agent-harness")>();
...
const LIVE = isLiveTestEnabled(["NODOASSIST_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.NODOASSIST_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "nodoassist-copilot-live-"));
