...
// nodoassist/plugin-sdk/agent-harness-runtime declares auth, messages,
// onAssistantDelta, and initialReplayState.sdkSessionId fields. Tracked by
// project nodoassist-copilot-harness; reviewer-attempt-bridge note.
...
type AttemptParamsLike = AgentHarnessAttemptParams & {
  ...
  // current-turn user message for the NodoAssist audit transcript so
...
interface CopilotAttemptDeps {
   ...
   * `nodoassist/plugin-sdk/agent-harness-runtime#resolveSandboxContext`, which is
   ...
   * harness uses this to track the nodoassistSessionId -> sdkSessionId
...
export async function runCopilotAttempt(
  ...
  try {
    ...
    try {
      const toolBridge = await createToolBridge({
        ...
        // tool-bridge.ts buildNodoAssistCodingToolsOptions().
        attemptParams: input,
        sessionRef,
        onYieldDetected: () => {
          yieldDetected = true;
    ...
    // Load NodoAssist workspace bootstrap files (SOUL.md, IDENTITY.md,
    ...
    // NodoAssist bootstrap block (SDK still loads AGENTS.md natively).
    ...
    const emitLlmInput = (prompt: string, additionalContext?: string) => {
      runAgentHarnessLlmInputHook({
        event: {
          ...
          // Copilot SDK sessions own their own transcript. NodoAssist's
    ...
    if (abortRequested || params.abortSignal?.aborted) {
      ...
      if (!bridge.recordSendResult(result) && !aborted) {
        ...
        // capability inventory. Do not call session.abort() here: NodoAssist may
  ...
  //
  // Without this synthesis the NodoAssist audit transcript never sees
  ...
  // (user/assistant/toolResult) into the NodoAssist audit transcript at
  // params.sessionFile, alongside the SDK's own session storage. The
  // NodoAssist shell (attempt-execution.ts) writes only the user prompt
  ...
  const nodoAssistSessionIdForMirror = readString(input.sessionId);
  const mirrorScopeSessionId = sessionIdUsed ?? nodoAssistSessionIdForMirror;
  if (sessionFileForMirror && nodoAssistSessionIdForMirror && messagesSnapshot.length > 0) {
    ...
    await dualWriteCopilotTranscriptBestEffort({
      ...
      sessionId: nodoAssistSessionIdForMirror,
      sessionKey: readString((input as { sessionKey?: unknown }).sessionKey),
      ...
      // authoritative; only the NodoAssist audit transcript would be
...
function createSessionConfig(
  ...
  return {
    ...
    // tool-bridge.ts) so 100% of tool calls go through NodoAssist's
    ...
    // bypass NodoAssist's wrapped-tool enforcement under any permissive
    ...
    ...(resolvedAuth.authMode === "gitHubToken" && resolvedAuth.gitHubToken
      ...
      : {}),
    // NodoAssist workspace bootstrap plus per-turn runtime guidance
    // injected via the SDK's `systemMessage` field in append mode:
    // SDK foundation + NodoAssist context. Append keeps every SDK
    ...
    // L1036). Omitted when there is no NodoAssist-owned context so the
...
function hasMirrorIdentity(message: AgentMessage): boolean {
  const record = message as unknown as { __nodoassist?: unknown };
