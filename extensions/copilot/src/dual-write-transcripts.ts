 ...
 * NodoAssist audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The NodoAssist shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the NodoAssist transcript via the same
...
export function attachCopilotMirrorIdentity<T extends AgentMessage>(
  ...
  const existing = record["__nodoassist"];
  const baseMeta =
  ...
  return {
    ...record,
    __nodoassist: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}
 ...
 * the NodoAssist audit trail just misses the intermediate messages for
 * this turn.
 */
