...
import { isBlockedHostnameOrIp } from "nodoassist/plugin-sdk/ssrf-runtime";
...
export const COPILOT_BYOK_TRANSPORT_POLICY_ERROR =
  "[copilot-attempt] BYOK does not support NodoAssist provider request proxy, TLS, or private-network policy overrides";
export const COPILOT_BYOK_ENDPOINT_POLICY_ERROR =
  "[copilot-attempt] BYOK endpoint is blocked by NodoAssist SSRF policy";
...
/**
 * Maps NodoAssist's prepared model facts into the Copilot SDK's session-level
 * provider contract. The SDK owns the wire request; NodoAssist only supplies
