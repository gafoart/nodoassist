 ...
 *     use cases.
 *   - Falls back to NODOASSIST_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *     (`<nodoAssistHome>/.nodoassist/agents/<agentId>/copilot`, or
 *     `<agentDir>/copilot` when an agent directory is supplied) that
 *     respects `NODOASSIST_HOME` for the home directory root.
 ...
 *   4. NODOASSIST_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with NodoAssist's own files
  ...
  const nodoAssistHome = readString(args.env.NODOASSIST_HOME);
  const rootHome = nodoAssistHome ? resolve(nodoAssistHome) : safeHomeDir(args.homeDir);
  // Per-agent isolation per proposal section 3.6:
  //   <nodoAssistHome>/.nodoassist/agents/<agentId>/copilot
  return resolve(join(rootHome, ".nodoassist", "agents", args.agentId, "copilot"));
}
...
function readEnvTokenFallback(
  ...
  // NODOASSIST_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "NODOASSIST_GITHUB_TOKEN", value: readString(env.NODOASSIST_GITHUB_TOKEN) },
