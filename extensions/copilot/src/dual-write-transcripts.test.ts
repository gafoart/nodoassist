...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-copilot-mirror-"));
  tempDirs.push(dir);
...
describe("mirrorCopilotTranscript", () => {
  it("mirrors user, assistant, and tool result messages into the NodoAssist transcript", async () => {
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("nodoassist-copilot-mirror-missing-dir-");
  ...
  it("uses attached identity instead of content fingerprint when provided", async () => {
    ...
    await mirrorCopilotTranscript({
      ...
      sessionId: "session-1",
      messages: [tagged],
      idempotencyScope: "copilot:nodoassist-session-1",
    });

    const raw = await fs.readFile(sessionFile, "utf8");
    expect(raw).toContain(
      '"idempotencyKey":"copilot:nodoassist-session-1:sdk-session-1:assistant:0"',
    );
...
describe("dualWriteCopilotTranscriptBestEffort", () => {
  ...
  it("swallows infrastructure failures and never rejects", async () => {
    const root = await makeRoot("nodoassist-copilot-mirror-invalid-");
