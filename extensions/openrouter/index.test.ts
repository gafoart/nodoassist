...
vi.mock("nodoassist/plugin-sdk/provider-stream-family", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("nodoassist/plugin-sdk/provider-stream-family")>();
  return {
    ...actual,
...
function readManifest(): OpenRouterManifest {
  return JSON.parse(readFileSync(new URL("./nodoassist.plugin.json", import.meta.url), "utf8"));
}
...
describe("openrouter provider hooks", () => {
  ...
  it("injects provider routing into compat before applying stream wrappers", async () => {
    ...
    const baseStreamFn = vi.fn(
      (
        ...args: Parameters<import("nodoassist/plugin-sdk/agent-core").StreamFn>
      ): ReturnType<import("nodoassist/plugin-sdk/agent-core").StreamFn> => {
  ...
  it("forwards resolved API keys as explicit OpenRouter auth headers", async () => {
    ...
    expect(headers.get("x-openrouter-title")).toBe("NodoAssist");
  });
