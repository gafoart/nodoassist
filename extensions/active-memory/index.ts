...
import { closeActiveMemorySearchManager } from "nodoassist/plugin-sdk/memory-host-search";
import {
  asDateTimestampMs,
...
} from "nodoassist/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type NodoAssistPluginApi } from "nodoassist/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "nodoassist/plugin-sdk/routing";
import { isPathInside } from "nodoassist/plugin-sdk/security-runtime";
import {
...
import { tempWorkspace, resolvePreferredNodoAssistTmpDir } from "nodoassist/plugin-sdk/temp-path";
...
function resolveDefaultToolsAllow(cfg: NodoAssistConfig | undefined): string[] {
...
function resolvePersistentTranscriptBaseDir(api: NodoAssistPluginApi, agentId: string): string {
  return path.join(
...
function openActiveMemoryToggleStore(api: NodoAssistPluginApi) {
...
function isActiveMemoryGloballyEnabled(cfg: NodoAssistConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: NodoAssistPluginApi): NodoAssistConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as NodoAssistConfig | undefined) ??
      (api.config as NodoAssistConfig | undefined)
    );
  } catch {
    return api.config as NodoAssistConfig | undefined;
  }
}
...
async function runRecallSubagent(params: {
  ...
  const transientWorkspace = params.config.persistTranscripts
    ...
    : await tempWorkspace({
        ...
        prefix: "nodoassist-active-memory-",
      });
...
export default definePluginEntry({
  ...
  register(api: NodoAssistPluginApi) {
    const readCurrentConfig = (): NodoAssistConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as NodoAssistConfig | undefined) ??
          (api.config as NodoAssistConfig | undefined)
