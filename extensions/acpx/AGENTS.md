...
The ACPX extension is a thin NodoAssist wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `nodoassist/acpx`, not in this extension.
...
Use this flow when NodoAssist needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `nodoassist/acpx` repo first.
2. In NodoAssist, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild NodoAssist and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for NodoAssist ACP validation.
...
If a change feels like shared ACP runtime behavior instead of NodoAssist-specific glue, move it to `nodoassist/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
