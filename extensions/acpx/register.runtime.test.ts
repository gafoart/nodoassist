...
vi.mock("nodoassist/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.NODOASSIST_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.NODOASSIST_SKIP_ACPX_RUNTIME;
  } else {
    process.env.NODOASSIST_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/nodoassist-acpx-register-test",
    stateDir: "/tmp/nodoassist-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
