 ...
 * NodoAssist-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const NODOASSIST_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-nodoassist-owned" | "unverified-root";
};
...
function resolveNodoAssistInstallRoot(pluginRoot: string): string {
  if (
...
function resolveOwnedAcpPackageRootCandidates(packageName: string): string[] {
  ...
  const nodoAssistRoot = resolveNodoAssistInstallRoot(pluginRoot);
  return [
    resolvePackageRoot(packageName),
    path.join(pluginRoot, "node_modules", packageName),
    path.join(nodoAssistRoot, "node_modules", packageName),
...
/** Check whether a command references an NodoAssist-generated ACPX wrapper path. */
export function isNodoAssistLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, NODOASSIST_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, NODOASSIST_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

/** Check whether a command is owned by NodoAssist ACPX runtime packages or wrappers. */
export function isNodoAssistOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(NODOASSIST_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated NodoAssist-owned ACPX wrapper process tree. */
export async function cleanupNodoAssistOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an NodoAssist-owned wrapper, fail closed to
  ...
  if (!liveCommandWasGeneratedWrapper && storedCommandWasGeneratedWrapper) {
    return {
      ...
      terminatedPids: [],
      skippedReason: "not-nodoassist-owned",
    };
  }
  if (
  ...
  if (
    !isNodoAssistOwnedAcpxProcessCommand({
      command: rootCommand,
      wrapperRoot: params.wrapperRoot,
    })
  ) {
...
/** Reap orphaned NodoAssist-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleNodoAssistOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
