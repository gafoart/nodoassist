...
const { cleanupNodoAssistOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupNodoAssistOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleNodoAssistOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleNodoAssistOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupNodoAssistOwnedAcpxProcessTree: cleanupNodoAssistOwnedAcpxProcessTreeMock,
  reapStaleNodoAssistOwnedAcpxOrphans: reapStaleNodoAssistOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { NodoAssistPluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE: process.env.NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE,
  NODOASSIST_SKIP_ACPX_RUNTIME: process.env.NODOASSIST_SKIP_ACPX_RUNTIME,
  NODOASSIST_SKIP_ACPX_RUNTIME_PROBE: process.env.NODOASSIST_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupNodoAssistOwnedAcpxProcessTreeMock.mockClear();
  reapStaleNodoAssistOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("NODOASSIST_SKIP_ACPX_RUNTIME");
  restoreEnv("NODOASSIST_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): NodoAssistPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".nodoassist-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: NodoAssistPluginServiceContext) {
  const env = { ...process.env, NODOASSIST_STATE_DIR: ctx.stateDir };
...
function openGatewayInstanceStore(ctx: NodoAssistPluginServiceContext) {
...
function openProcessLeaseStore(ctx: NodoAssistPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.NODOASSIST_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupNodoAssistOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupNodoAssistOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale NodoAssist-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleNodoAssistOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupNodoAssistOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleNodoAssistOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale NodoAssist-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.NODOASSIST_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.NODOASSIST_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (NODOASSIST_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
