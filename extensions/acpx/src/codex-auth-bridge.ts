...
import { readJsonFileWithFallback } from "nodoassist/plugin-sdk/json-store";
import {
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--nodoassist-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @nodoassist/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep NodoAssist's isolated CODEX_HOME wrapper, but launch the plugin-local
...
const nodoAssistWrapperArgs = new Set([
  ${quoteCommandPart(NODOASSIST_ACPX_LEASE_ID_ARG)},
  ${quoteCommandPart(NODOASSIST_GATEWAY_INSTANCE_ID_ARG)},
]);

function readNodoAssistWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(NODOASSIST_ACPX_LEASE_ID_ENV)}] ||
    readNodoAssistWrapperArg(args, ${quoteCommandPart(NODOASSIST_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripNodoAssistWrapperArgs(args) {
  const stripped = [];
  for (let index = 0; index < args.length; index += 1) {
    const value = args[index];
    if (nodoAssistWrapperArgs.has(value)) {
      index += 1;
      continue;
    }
    stripped.push(value);
...
const configuredArgs = stripNodoAssistWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[nodoassist] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[nodoassist] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in NodoAssist; fallback must not float to an unpatched newer release.
