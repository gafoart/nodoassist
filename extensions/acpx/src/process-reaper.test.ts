...
const WRAPPER_ROOT = "/tmp/nodoassist-state/acpx";
const CODEX_WRAPPER_COMMAND = `node ${WRAPPER_ROOT}/codex-acp-wrapper.mjs`;
const CODEX_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_WRAPPER_COMMAND} ${NODOASSIST_ACPX_LEASE_ID_ARG} lease-1 ${NODOASSIST_GATEWAY_INSTANCE_ID_ARG} gateway-1`;
...
describe("process reaper", () => {
  ...
  it("only treats generated wrappers as launch-lease aware", () => {
    ...
    expect(
      isNodoAssistLeaseAwareAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND }),
    ).toBe(false);
    expect(isNodoAssistLeaseAwareAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(
      false,
    );
  });

  it("recognizes NodoAssist plugin-runtime-deps ACP adapter children", () => {
    expect(isNodoAssistOwnedAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(true);
    expect(isNodoAssistOwnedAcpxProcessCommand({ command: "npx @zed-industries/codex-acp" })).toBe(
      false,
    );
  });

  it("recognizes plugin-local ACP adapter package paths without trusting arbitrary installs", () => {
    expect(isNodoAssistOwnedAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND })).toBe(
      true,
    );
    expect(
      isNodoAssistOwnedAcpxProcessCommand({
    ...
    const { deps, killed } = cleanupDeps([
      {
        pid: 113,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${NODOASSIST_ACPX_LEASE_ID_ARG} other-lease ${NODOASSIST_GATEWAY_INSTANCE_ID_ARG} gateway-1`,
      },
    ]);
  ...
  it("does not kill a reused pid when the live command is not NodoAssist-owned", async () => {
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-nodoassist-owned");
    expect(killed).toStrictEqual([]);
  });

  it("reaps stale NodoAssist-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
    ...
    const result = await reapStaleNodoAssistOwnedAcpxOrphans({
      wrapperRoot: WRAPPER_ROOT,
      deps,
    });
