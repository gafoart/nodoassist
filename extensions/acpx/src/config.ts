...
import { formatPluginConfigIssue } from "nodoassist/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "nodoassist-plugin-tools";
const ACPX_NODOASSIST_TOOLS_MCP_SERVER_NAME = "nodoassist-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "nodoassist.plugin.json")) &&
...
function resolveAcpxPluginRootFromNodoAssistLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common NodoAssist layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromNodoAssistLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveNodoAssistRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
function resolvePluginToolsMcpServerConfig(moduleUrl: string = import.meta.url): McpServerConfig {
  ...
  const nodoAssistRoot = resolveNodoAssistRoot(pluginRoot);
  const distEntry = path.join(nodoAssistRoot, "dist", "mcp", "plugin-tools-serve.js");
  ...
  const sourceEntry = path.join(nodoAssistRoot, "src", "mcp", "plugin-tools-serve.ts");
  return {
    command: process.execPath,
...
function resolveNodoAssistToolsMcpServerConfig(
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const nodoAssistRoot = resolveNodoAssistRoot(pluginRoot);
  const distEntry = path.join(nodoAssistRoot, "dist", "mcp", "nodoassist-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(nodoAssistRoot, "src", "mcp", "nodoassist-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  nodoAssistToolsMcpBridge: boolean;
  moduleUrl?: string;
  ...
  if (params.nodoAssistToolsMcpBridge && resolved[ACPX_NODOASSIST_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_NODOASSIST_TOOLS_MCP_SERVER_NAME} is reserved when nodoAssistToolsMcpBridge=true`,
    );
  }
  ...
  if (params.nodoAssistToolsMcpBridge) {
    resolved[ACPX_NODOASSIST_TOOLS_MCP_SERVER_NAME] = resolveNodoAssistToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
...
/** Convert NodoAssist MCP server config into ACPX runtime MCP server entries. */
...
export function resolveAcpxPluginConfig(params: {
  ...
  const nodoAssistToolsMcpBridge = normalized.nodoAssistToolsMcpBridge === true;
  const mcpServers = resolveConfiguredMcpServers({
    mcpServers: normalized.mcpServers,
    pluginToolsMcpBridge,
    nodoAssistToolsMcpBridge,
