...
import { upsertAuthProfile } from "nodoassist/plugin-sdk/provider-auth";
...
async function makeTempRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-migrate-codex-"));
  tempRoots.add(root);
  return root;
}
...
async function createCodexFixture(): Promise<{
  ...
  vi.stubEnv("NODOASSIST_AGENT_DIR", "");
...
describe("buildCodexMigrationProvider", () => {
  it("parses target marketplace discovery timeout env strictly", () => {
    expect(
      targetCodexMarketplaceDiscoveryTimeoutMs({
        NODOASSIST_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS: "0",
      }),
    ).toBe(0);
    expect(
      targetCodexMarketplaceDiscoveryTimeoutMs({
        NODOASSIST_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS: "250",
      }),
    ).toBe(250);
    ...
    for (const value of ["0x10", "1e3", "2.5"]) {
      expect(
        targetCodexMarketplaceDiscoveryTimeoutMs({
          NODOASSIST_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS: value,
        }),
      ).toBe(30_000);
    }
  });
  ...
  it("warns and skips app-backed plugins when source Codex account is not ChatGPT subscription auth", async () => {
    ...
    expect(plan.warnings).toEqual([
      "Codex app-backed plugin migration requires the Codex app-server source account to be logged in with a ChatGPT subscription account. Log in to the Codex app with subscription auth; NodoAssist auth or API-key auth does not satisfy Codex app connector access.",
    ]);
    expect(appServerRequest.mock.calls.filter(([arg]) => arg.method === "app/list")).toHaveLength(
      0,
    );
  ...
  it("leaves selected Codex plugins as warnings when target curated plugins never load", async () => {
    vi.stubEnv("NODOASSIST_CODEX_MIGRATION_PLUGIN_LIST_TIMEOUT_MS", "1");
    const fixture = await createCodexFixture();
    const configState: MigrationProviderContext["config"] = {
      agents: { defaults: { workspace: fixture.workspaceDir } },
    } as MigrationProviderContext["config"];
    ...
    expect(result.warnings).toContain(
      "Some Codex plugins could not be migrated. Run `nodoassist migrate codex` after onboarding.",
    );
