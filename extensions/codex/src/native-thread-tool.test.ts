...
import { createPluginRuntimeMock } from "nodoassist/plugin-sdk/plugin-test-runtime";
...
describe("native Codex thread tool", () => {
  ...
  async function withFixture(run: () => void | Promise<void>): Promise<void> {
    await withTempDir("nodoassist-codex-threads-", async (tempRoot) => {
      root = tempRoot;
  ...
  function createTool(params?: {
    ...
    const context: NodoAssistPluginToolContext = {
      config: {},
      agentId: "main",
  ...
  it("forks a native thread and attaches the fork to the NodoAssist session", () =>
    withFixture(async () => {
  ...
  it("refuses to archive the active thread bound to this NodoAssist session", () =>
    withFixture(async () => {
      ...
      ).rejects.toThrow("cannot archive the Codex thread active in this NodoAssist session");
