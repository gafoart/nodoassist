...
const ForkParamsSchema = Type.Object(
  {
    ...
    attach: Type.Optional(
      Type.Boolean({
        default: true,
        description: "Attach the fork to this NodoAssist session for its next turn.",
      }),
    ),
  },
  { additionalProperties: false },
...
export function createCodexThreadsTool(options: CodexThreadsToolOptions): AnyAgentTool | null {
  ...
  return {
    ...
    async execute(_toolCallId, rawParams) {
      ...
      if (action === "archive") {
        ...
        if (binding?.threadId === threadId) {
          ...
          if (readThreadStatusType(current) === "active") {
            throw new Error("cannot archive the Codex thread active in this NodoAssist session");
          }
        }
        await request(
          pluginConfig,
      ...
      if (attach && !session) {
        throw new Error("cannot attach a Codex fork without an active NodoAssist session");
      }
      if (attach && binding?.threadId === threadId) {
        ...
        if (readThreadStatusType(current) === "active") {
          throw new Error("cannot replace the Codex thread active in this NodoAssist turn");
        }
      }
      const response = await request(
        pluginConfig,
