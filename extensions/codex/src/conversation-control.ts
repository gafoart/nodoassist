...
const CODEX_CONVERSATION_CONTROL_STATE = Symbol.for("nodoassist.codex.conversationControl");
...
async function requireThreadBinding(
  ...
  if (!binding?.threadId) {
    throw new Error("No Codex thread is attached to this NodoAssist session yet.");
  }
  return binding;
}
