...
/**
 * Lightweight read/write surface over the NodoAssist config file. Plugged in by
...
// Plugin lifecycle changes (enable/disable) write to nodoassist.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in nodoassist.json. ${POLICY_REFRESH_HINT}`,
    };
  }
...
export function buildPluginsHelp(): string {
  return [
    "Codex sub-plugin management (writes only to ~/.nodoassist/nodoassist.json, never to ~/.codex/config.toml):",
...
export function formatPluginList(
  ...
  return [
    "Codex sub-plugins in NodoAssist config (~/.nodoassist/nodoassist.json):",
    "",
    ...rows.map(
      (r) =>
