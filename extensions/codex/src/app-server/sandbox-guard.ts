/**
 * Blocks direct Codex app-server requests that would bypass NodoAssist sandbox or
...
type DirectMethodPolicy =
  ...
  | "requires-nodoassist-environment";
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-nodoassist-environment"],
...
/** Returns a block message when a direct app-server method would bypass NodoAssist execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-nodoassist-environment" &&
    hasNodoAssistSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasNodoAssistSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("nodoassist-sandbox-") &&
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because NodoAssist sandboxing is active for this session.`,
    "This mode cannot route execution through the NodoAssist sandbox backend.",
