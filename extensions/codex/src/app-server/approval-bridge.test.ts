...
import { reviewExecRequestWithConfiguredModel } from "nodoassist/plugin-sdk/agent-harness-exec-review-runtime";
import {
  callGatewayTool,
...
vi.mock("nodoassist/plugin-sdk/agent-harness-exec-review-runtime", async (importOriginal) => ({
  ...(await importOriginal<
    typeof import("nodoassist/plugin-sdk/agent-harness-exec-review-runtime")
  >()),
...
describe("Codex app-server approval bridge", () => {
  ...
  it("routes command approvals through plugin approvals and accepts allowed commands", async () => {
    ...
    expect(requestPayload.pluginId).toBe("nodoassist-codex-app-server");
  ...
  it("uses the configured NodoAssist exec auto-review model before plugin approvals", async () => {
    const params = createParams();
    ...
    findApprovalEvent(params, {
      status: "approved",
      message:
        "Codex app-server command approval granted by NodoAssist exec auto-reviewer: read-only version check",
    });
  });
  ...
  it("falls back to plugin approval when Codex native OpenAI config uses a local base URL", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-codex-approval-"));
    try {
  ...
  it.each([
    ...
    "sudo -EH bash -lc 'nodoassist channels login --channel whatsapp'",
  ...
  it("normalizes prefixed channel targets for NodoAssist tool policy context", async () => {
    const params = createParams();
  ...
  it("denies command approvals before prompting when NodoAssist tool policy blocks", async () => {
    const params = createParams();
  ...
  it("fails closed when the native hook relay returns unreadable approval output", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "NodoAssist native hook relay returned an unreadable Codex app-server approval result.",
    });
  });
  ...
  it("fails closed when the native hook relay returns a non-deny decision", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "NodoAssist native hook relay returned a non-deny Codex app-server approval decision.",
    });
  });
  ...
  it("fails closed when the expected native hook relay cannot be invoked", async () => {
    ...
    findApprovalEvent(params, {
      status: "denied",
      message:
        "NodoAssist native hook relay unavailable for Codex app-server approval: native hook relay not found",
    });
  });
  ...
  it("denies command approvals when NodoAssist tool policy rewrites params", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: false,
      params: {
  ...
  it("keeps NodoAssist plugin allow-always approvals scoped to one Codex request", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: false,
      params: {
  ...
  it("denies command approvals when NodoAssist tool policy requires approval", async () => {
    const params = createParams();
    mockRunBeforeToolCallHook.mockResolvedValueOnce({
      blocked: true,
      kind: "failure",
  ...
  it("fails closed for unsupported native approval methods without requesting plugin approval", async () => {
    ...
    expect(result).toEqual({
      decision: "decline",
      reason: "NodoAssist codex app-server bridge does not grant native approvals yet.",
    });
    expect(mockCallGatewayTool).not.toHaveBeenCalled();
