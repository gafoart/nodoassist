...
describe("maybeCompactCodexAppServerSession", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-codex-compact-"));
  });
  ...
  it("blocks native app-server compaction when the current NodoAssist session is sandboxed", async () => {
    const fake = createFakeCodexClient();
    setCodexAppServerClientFactoryForTest(async () => fake.client);
    const sessionFile = await writeTestBinding();
    ...
    expect(result.reason).toContain(
      "Codex-native native compaction is unavailable because NodoAssist sandboxing is active for this session.",
    );
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("blocks native app-server compaction when exec host=node is active", async () => {
    ...
    expect(result.compacted).toBe(false);
    expect(result.reason).toContain(
      "Codex-native native compaction is unavailable because NodoAssist exec host=node is active for this session.",
    );
    expect(fake.request).not.toHaveBeenCalled();
  });
  ...
  it("warns when stale NodoAssist compaction overrides are ignored", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "ignoring NodoAssist compaction overrides for Codex app-server compaction; Codex uses native server-side compaction",
      {
        sessionId: "session-1",
        sessionKey: "agent:main:session-1",
