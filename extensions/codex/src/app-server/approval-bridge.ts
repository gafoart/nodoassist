...
} from "nodoassist/plugin-sdk/agent-harness-exec-review-runtime";
/**
 * Bridges Codex app-server approval requests into NodoAssist policy hooks and
 * plugin approval UX.
 */
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    const policyOutcome = await runNodoAssistToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
    ...
    // Native hook/model policy did not decide; fall back to the NodoAssist
...
/** Converts an NodoAssist approval outcome into the app-server method response. */
...
async function runInternalExecAutoReviewForApprovalRequest(params: {
  ...
  return {
    ...
    reason: `Codex app-server command approval granted by NodoAssist exec auto-reviewer: ${formatCodexDisplayText(
      decision.rationale,
    )}`,
  };
}
...
async function runNodoAssistToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  context: ApprovalContext;
  ...
  const policyRequest = buildNodoAssistToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `NodoAssist native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        formatErrorMessage(error),
      )}`,
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "NodoAssist native hook relay failed for Codex app-server approval.",
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "NodoAssist native hook policy denied Codex app-server approval.",
...
function buildNodoAssistToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
