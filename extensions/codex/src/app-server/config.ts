...
} from "nodoassist/plugin-sdk/exec-approvals-runtime";
...
import { normalizeTrimmedStringList } from "nodoassist/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "nodoassist/plugin-sdk/windows-spawn";
import { z } from "zod";
import type {
  CodexApprovalPolicy,
  CodexSandboxPolicy,
...
const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for(
  "nodoassist.codexAppServerStartOptionsKeySecret",
);
...
type NodoAssistExecMode = "deny" | "allowlist" | "ask" | "auto" | "full";
type NodoAssistExecSecurity = "deny" | "allowlist" | "full";
type NodoAssistExecAsk = "off" | "on-miss" | "always";
type NodoAssistExecApprovalFloorsForCodexAppServer = {
  security?: NodoAssistExecSecurity;
  ask?: NodoAssistExecAsk;
};
export type NodoAssistExecPolicyForCodexAppServer = {
  mode?: NodoAssistExecMode;
  security: NodoAssistExecSecurity;
  ask: NodoAssistExecAsk;
  touched: boolean;
};
type NodoAssistExecPolicy = NodoAssistExecPolicyForCodexAppServer;
...
const DEFAULT_CODEX_APP_SERVER_NETWORK_PROXY_PROFILE_PREFIX = "nodoassist-network";
...
export function resolveCodexAppServerRuntimeOptions(
  params: {
    ...
    hostName?: string;
    nodoAssistSandboxActive?: boolean;
  } = {},
  ...
  const envCommand = readNonEmptyString(env.NODOASSIST_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.NODOASSIST_CODEX_APP_SERVER_ARGS);
  ...
  const execMode = resolveEffectiveNodoAssistExecModeForCodexAppServer({
    execMode: params.execMode,
    execPolicy: params.execPolicy,
  });
  assertCodexAppServerAllowedForNodoAssistExecMode(execMode);
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.NODOASSIST_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.NODOASSIST_CODEX_APP_SERVER_SANDBOX);
  const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
  const normalizedPolicyMode = resolveCodexPolicyModeForNodoAssistExecMode(execMode);
  ...
  const execModeRequiringPromptingApprovals:
    | Extract<NodoAssistExecMode, "auto" | "ask">
    | undefined =
        ...
        sandbox: preserveExplicitAutoSandbox
          ...
          : forceDangerFullAccessSandbox
            ? selectForcedDangerFullAccessSandbox({
                configuredSandbox,
                defaultPolicy,
                nodoAssistSandboxActive: Boolean(params.nodoAssistSandboxActive),
              })
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.NODOASSIST_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.NODOASSIST_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.NODOASSIST_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.NODOASSIST_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.NODOASSIST_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.NODOASSIST_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.NODOASSIST_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.NODOASSIST_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.NODOASSIST_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
...
function resolveDefaultCodexAppServerPolicy(params: {
  ...
  execModeRequiringPromptingApprovals?: Extract<NodoAssistExecMode, "auto" | "ask">;
  execModeRequiringUserReviewer?: NodoAssistExecMode;
  env?: NodeJS.ProcessEnv;
  requirementsToml?: string | null;
  requirementsPath?: string;
  readRequirementsFile?: (path: string) => string | undefined;
...
function selectGuardianApprovalsReviewer(
  ...
  execModeRequiringAutoReviewer?: Extract<NodoAssistExecMode, "auto">,
...
function selectForcedDangerFullAccessSandbox(params: {
  ...
  if (params.defaultPolicy?.dangerFullAccessAllowed === false) {
    if (params.nodoAssistSandboxActive) {
...
export function resolveNodoAssistExecModeFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): NodoAssistExecMode | undefined {
  const policy = resolveNodoAssistExecPolicyFromConfig(params);
  return policy.touched ? policy.mode : undefined;
}

function resolveNodoAssistExecPolicyFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): NodoAssistExecPolicy {
  const root = readRecord(params.config);
  const globalExec = readRecord(readRecord(root?.tools)?.exec);
  const globalPolicy = applyNodoAssistExecPolicyLayer(
    createDefaultNodoAssistExecPolicy(),
    globalExec,
  );
  ...
  return applyNodoAssistExecPolicyLayer(globalPolicy, agentExec);
}

export function resolveNodoAssistExecModeForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const policy = resolveNodoAssistExecPolicyForCodexAppServer(params);
  return policy.touched ? policy.mode : undefined;
}

export function resolveNodoAssistExecPolicyForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  config?: unknown;
  agentId?: string;
}): NodoAssistExecPolicyForCodexAppServer {
  const basePolicy = resolveNodoAssistExecPolicyFromConfig({
    config: params.config,
    agentId: params.agentId,
  });
  const overridePolicy = applyNodoAssistExecPolicyLayer(basePolicy, params.execOverrides);
  const approvalFloors = resolveNodoAssistExecApprovalFloorsForCodexAppServer({
    approvals: params.approvals,
    agentId: params.agentId,
    policy: overridePolicy,
  });
  return applyNodoAssistExecApprovalFloors(overridePolicy, approvalFloors);
}

function resolveEffectiveNodoAssistExecModeForCodexAppServer(params: {
  execMode?: NodoAssistExecMode;
  execPolicy?: NodoAssistExecPolicyForCodexAppServer;
}): NodoAssistExecMode | undefined {
...
function resolveCodexPolicyModeForNodoAssistExecMode(
  mode: NodoAssistExecMode | undefined,
...
function assertCodexAppServerAllowedForNodoAssistExecMode(
  mode: NodoAssistExecMode | undefined,
): void {
...
function createDefaultNodoAssistExecPolicy(): NodoAssistExecPolicy {
  return {
    security: "full",
    ask: "off",
    touched: false,
...
function applyNodoAssistExecPolicyLayer(
  base: NodoAssistExecPolicy,
  ...
  if (mode !== undefined) {
    return {
      ...resolveNodoAssistExecPolicyForMode(mode),
      touched: true,
    };
  }
  ...
  return {
    mode: resolveNodoAssistExecModeFromPolicy({ security: nextSecurity, ask: nextAsk }),
...
function resolveNodoAssistExecApprovalFloorsForCodexAppServer(params: {
  approvals?: ExecApprovalsFile;
  agentId?: string;
  policy: NodoAssistExecPolicy;
}): NodoAssistExecApprovalFloorsForCodexAppServer | undefined {
  if (!params.approvals) {
    return undefined;
  }
...
function applyNodoAssistExecApprovalFloors(
  base: NodoAssistExecPolicy,
  approvalFloors?: NodoAssistExecApprovalFloorsForCodexAppServer,
): NodoAssistExecPolicy {
  if (!approvalFloors) {
    return base;
  }
  const nextSecurity = approvalFloors.security
    ? minNodoAssistExecSecurity(base.security, approvalFloors.security)
    : base.security;
  const nextAsk = approvalFloors.ask
    ? maxNodoAssistExecAsk(base.ask, approvalFloors.ask)
    : base.ask;
...
function resolveNodoAssistExecPolicyForMode(
  mode: NodoAssistExecMode,
): Omit<NodoAssistExecPolicy, "touched"> {
  switch (mode) {
    case "deny":
...
function resolveNodoAssistExecModeFromPolicy(params: {
  security: NodoAssistExecSecurity;
  ask: NodoAssistExecAsk;
}): NodoAssistExecMode {
...
function minNodoAssistExecSecurity(
  left: NodoAssistExecSecurity,
  right: NodoAssistExecSecurity,
): NodoAssistExecSecurity {
  const order: Record<NodoAssistExecSecurity, number> = { deny: 0, allowlist: 1, full: 2 };
  return order[left] <= order[right] ? left : right;
}

function maxNodoAssistExecAsk(
  left: NodoAssistExecAsk,
  right: NodoAssistExecAsk,
): NodoAssistExecAsk {
  const order: Record<NodoAssistExecAsk, number> = { off: 0, "on-miss": 1, always: 2 };
  return order[left] >= order[right] ? left : right;
}

function readExecMode(value: unknown): NodoAssistExecMode | undefined {
  return value === "deny" ||
    value === "allowlist" ||
    value === "ask" ||
    value === "auto" ||
...
function readExecSecurity(value: unknown): NodoAssistExecSecurity | undefined {
  return value === "deny" || value === "allowlist" || value === "full" ? value : undefined;
}

function readExecAsk(value: unknown): NodoAssistExecAsk | undefined {
