...
import type { ManagedProcess, NodoAssistExecServer, ProcessChunk } from "./types.js";
...
async function runProcess(
  ...
  if (!backend) {
    throw new Error("NodoAssist sandbox backend is unavailable.");
  }
  ...
  if (!command) {
    throw new Error("NodoAssist sandbox exec spec did not provide a command.");
  }
