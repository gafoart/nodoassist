 ...
 * access through the active NodoAssist sandbox backend.
 */
...
import { SsrFBlockedError, isBlockedHostnameOrIp } from "nodoassist/plugin-sdk/ssrf-runtime";
...
import type { HttpHeader, NodoAssistExecServer } from "./types.js";
...
async function runStreamingSandboxHttpRequest(
  ...
  if (!command) {
    throw new Error("NodoAssist sandbox HTTP exec spec did not provide a command.");
  }
...
tmp=$(mktemp "$TMPDIR/nodoassist-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/nodoassist-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
