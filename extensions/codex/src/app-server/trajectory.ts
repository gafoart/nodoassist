...
import { resolveUserPath } from "nodoassist/plugin-sdk/agent-harness-runtime";
import type {
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.NODOASSIST_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.NODOASSIST_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
