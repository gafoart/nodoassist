...
describe("Codex plugin thread config", () => {
  ...
  it("re-enables an NodoAssist-allowed app even when app/list reports it disabled", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
  ...
  it("does not expose apps for plugins that NodoAssist policy leaves disabled", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
  ...
  it("uses app-level destructive policy for plugins without NodoAssist tool-name knowledge", async () => {
    const appCache = new CodexAppInventoryCache();
    await appCache.refreshNow({
      key: "runtime",
      nowMs: 0,
