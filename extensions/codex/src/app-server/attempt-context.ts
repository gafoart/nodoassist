...
import { resolveAgentWorkspaceDir } from "nodoassist/plugin-sdk/agent-runtime";
...
export async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const developerInstructionFiles = shouldInjectCodexNodoAssistPromptContext(params.params)
      ? selectCodexWorkspaceInheritedDeveloperInstructionFiles(contextFiles)
      : [];
    const turnScopedDeveloperInstructionFiles = shouldInjectCodexNodoAssistPromptContext(
      params.params,
    )
    ...
    return {
      ...
      memoryCollaborationInstructions: shouldInjectCodexNodoAssistPromptContext(params.params)
...
/**
 * Builds NodoAssist-provided workspace prompt context for the current Codex turn.
 */
export function buildCodexNodoAssistPromptContext(params: {
  params: EmbeddedRunAttemptParams;
  workspacePromptContext?: string;
}): string | undefined {
  if (!shouldInjectCodexNodoAssistPromptContext(params.params)) {
    return undefined;
  }
  const sections = [
    params.workspacePromptContext?.trim()
      ? ["## NodoAssist Workspace Context", "", params.workspacePromptContext.trim()].join("\n")
      : undefined,
  ].filter(isNonEmptyString);
...
function shouldInjectCodexNodoAssistPromptContext(params: EmbeddedRunAttemptParams): boolean {
...
/** Renders loaded NodoAssist skill prompts as Codex collaboration instructions. */
export function renderCodexSkillsCollaborationInstructions(params: {
  attempt: EmbeddedRunAttemptParams;
  skillsPrompt?: string;
}): string | undefined {
  if (!shouldInjectCodexNodoAssistPromptContext(params.attempt)) {
    return undefined;
  }
  return params.skillsPrompt?.trim()
    ? ["## NodoAssist Skills", "", params.skillsPrompt.trim()].join("\n")
    : undefined;
}

/**
 * Prepends NodoAssist context while preserving leading delivery metadata as
 * routing guidance instead of user request text.
 */
export function prependCodexNodoAssistPromptContext(
  prompt: string,
...
function renderCodexWorkspaceThreadDeveloperInstructions(
  ...
  return renderCodexWorkspaceDeveloperInstructions({
    files,
    header: "## NodoAssist Workspace Instructions",
    preamble:
...
export function renderCodexWorkspaceMemoryReference(params: {
  ...
  const lines = [
    "## NodoAssist Workspace Memory",
    "",
    `MEMORY.md exists in the active agent workspace as a memory file, not an instruction file. NodoAssist does not paste its contents into native Codex turns; use ${toolNames.join(" or ")} when durable memory is relevant and the tools are available.`,
    "",
  ];
