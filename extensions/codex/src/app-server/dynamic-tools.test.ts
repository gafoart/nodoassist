...
import type { AnyAgentTool } from "nodoassist/plugin-sdk/agent-harness";
import {
  HEARTBEAT_RESPONSE_TOOL_NAME,
  embeddedAgentLog,
  wrapToolWithBeforeToolCallHook,
} from "nodoassist/plugin-sdk/agent-harness-runtime";
import { createTerminalPresentationContractTool } from "nodoassist/plugin-sdk/agent-runtime-test-contracts";
import {
...
describe("createCodexDynamicToolBridge", () => {
  it("keeps NodoAssist control-path tools direct while deferring broad tools", () => {
    ...
    expectDynamicSpec(webSearch, {
      name: "web_search",
      namespace: CODEX_NODOASSIST_DYNAMIC_TOOL_NAMESPACE,
      deferLoading: true,
    });
  ...
  it("can register a durable tool schema while denying execution for the current turn", async () => {
    ...
    expect(result).toEqual({
      ...
      contentItems: [
        {
          type: "inputText",
          text: `NodoAssist tool is not available for this turn: ${HEARTBEAT_RESPONSE_TOOL_NAME}`,
        },
      ],
    });
    ...
    expect(onAgentToolResult).toHaveBeenCalledWith({
      ...
      result: {
        ...
        details: {
          status: "failed",
          error: `NodoAssist tool is not available for this turn: ${HEARTBEAT_RESPONSE_TOOL_NAME}`,
        },
      },
      isError: true,
    });
  ...
  it("quarantines dynamic tools with unsupported input schemas", async () => {
    ...
    expect(result).toEqual({
      success: false,
      contentItems: [
        { type: "inputText", text: "Unknown NodoAssist tool: fuzzplugin_move_angles" },
      ],
    });
  ...
  it("truncates configured text tool results before returning them to Codex", async () => {
    ...
    expect(text).toContain("NodoAssist truncated dynamic tool result");
  ...
  it("honors normalized per-agent dynamic tool result caps", async () => {
    ...
    expect(firstItem.text).toContain("NodoAssist truncated dynamic tool result");
  });
  ...
  it("keeps truncation notices within tiny configured caps", async () => {
    ...
    expect(firstItem.text).toBe("...(NodoAssist truncated dynamic tool".slice(0, 32));
  });
  ...
  it("marks executed dynamic tool results as side-effect evidence", async () => {
    ...
    const result = await bridge.handleToolCall({
      ...
      namespace: null,
      tool: "exec",
      arguments: { command: "touch /tmp/nodoassist-replay-test" },
    });
  ...
  it("shares replay-safe classification with NodoAssist for read-only dynamic tools", async () => {
  ...
  it("keeps config out of Codex tool-result contexts", async () => {
    const config = { session: { store: "/tmp/nodoassist-session-store.json" } };
    const registry = createEmptyPluginRegistry();
