 ...
 * or whether NodoAssist must keep exec/process on a configured node host.
 */
import type { NodoAssistConfig } from "nodoassist/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "nodoassist/plugin-sdk/sandbox";
import { getSessionEntry, type SessionEntry } from "nodoassist/plugin-sdk/session-store-runtime";
...
type AgentEntry = NonNullable<NonNullable<NodoAssistConfig["agents"]>["list"]>[number];
...
export function resolveCodexNativeExecutionPolicy(params: {
  ...
  return {
    ...
    node,
    blockReason:
      "NodoAssist exec host=node is active for this session. Codex app-server native execution cannot route shell, filesystem, MCP, or app-backed work through the selected NodoAssist node.",
  };
}
...
export function formatCodexNativeNodeExecBlock(params: {
  ...
  return [
    `Codex-native ${params.surface} is unavailable because NodoAssist exec host=node is active for this session.`,
    params.reason ??
      "Codex app-server native execution cannot route execution through the selected NodoAssist node.",
    "Use a normal Codex harness turn so NodoAssist exec/process tools run on the node, or switch exec host to gateway for native Codex app-server execution.",
  ].join(" ");
}
...
function listAgentEntries(config: NodoAssistConfig): AgentEntry[] {
