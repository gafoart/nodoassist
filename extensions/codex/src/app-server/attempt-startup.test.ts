...
function createAttemptPaths(): AttemptPaths {
  const root = path.join(os.tmpdir(), `nodoassist-codex-attempt-startup-${randomUUID()}`);
  tempRoots.add(root);
  return {
...
async function answerInitialize(harness: ClientHarness): Promise<void> {
  ...
  harness.send({ id: initialize.id, result: { userAgent: "nodoassist/0.142.0 (macOS; test)" } });
}
