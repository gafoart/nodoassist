...
function buildCurrentCodexApprovalElicitation() {
  return {
    ...
    _meta: {
      ...
      tool_params_display: [
        { name: "repo", display_name: "Repository", value: "nodoassist/nodoassist" },
      ],
    },
    requestedSchema: {
      type: "object",
...
describe("Codex app-server elicitation bridge", () => {
  ...
  it("accepts current Codex MCP approval elicitations with an empty form schema", async () => {
    ...
    expect(approvalRequest.description).toContain("Repository: nodoassist/nodoassist");
  });
  ...
  it("falls back to stable names when display labels sanitize to empty", async () => {
    ...
    expect(approvalRequest.description).toContain("- repo: nodoassist/nodoassist");
