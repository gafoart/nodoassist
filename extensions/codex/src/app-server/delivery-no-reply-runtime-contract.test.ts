...
import { isSilentReplyPayloadText } from "nodoassist/plugin-sdk/reply-chunking";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-codex-delivery-contract-"));
