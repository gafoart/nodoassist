...
import { resetDiagnosticEventsForTest } from "nodoassist/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "nodoassist/plugin-sdk/hook-runtime";
import { clearMemoryPluginState } from "nodoassist/plugin-sdk/memory-core-host-runtime-core";
import { clearPluginCommands } from "nodoassist/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("NODOASSIST_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-codex-run-"),
    );
  });
  ...
  afterEach(async () => {
    ...
    testing.resetNodoAssistCodingToolsFactoryForTests();
