/**
 * Bridges Codex native hook callbacks into NodoAssist's native hook relay so
 * app-server tool events can still run NodoAssist policy and diagnostics.
 */
...
/** Codex hook events that can be registered through NodoAssist's native relay. */
...
/** Registers an NodoAssist native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian NodoAssist plugin approval prompt.
...
export function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("nodoassist:codex:native-hook-relay:v1");
  hash.update("\0");
