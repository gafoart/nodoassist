...
/** Resolves the same agent scope NodoAssist uses for transcript/session ownership. */
...
/** Lets the authoritative NodoAssist session generation claim a stale stable binding row. */
...
export function createCodexAppServerBindingStore(
  ...
  return {
    ...
    async adoptSessionGeneration(identity, expectedPreviousSessionId) {
      ...
      // Context-engine compaction rotates the physical NodoAssist session before
