...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `NodoAssist dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `NodoAssist dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
export async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    const message = "NodoAssist dynamic tool call aborted before execution.";
  ...
  const abortFromRun = () => {
    const message = "NodoAssist dynamic tool call aborted.";
    const terminalReason = resolveCodexToolAbortTerminalReason(params.signal);
    params.onFallbackSelected?.();
  ...
  try {
    ...
    const message = formatToolExecutionErrorMessage(error, "NodoAssist dynamic tool call failed.");
    notifyFailedToolResult(message, terminalReason);
    return failedDynamicToolResponse(message, {
      sideEffectEvidence: true,
      terminalReason,
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("NodoAssist dynamic tool call finished."));
    }
  }
}
...
function readDynamicToolResponseText(response: CodexDynamicToolCallResponse): string {
  const text = response.contentItems
    ...
    .join("\n")
    .trim();
  return text || "NodoAssist dynamic tool call failed.";
}
...
/** Strips NodoAssist-only metadata before sending a dynamic tool response to Codex. */
...
type TerminalDynamicToolReleaseState = {
  ...
  pendingNodoAssistDynamicToolCompletionIdsCount: number;
};
...
export function shouldReleaseTurnAfterTerminalDynamicTool(
  ...
  return (
    ...
    state.pendingNodoAssistDynamicToolCompletionIdsCount === 0
  );
}
