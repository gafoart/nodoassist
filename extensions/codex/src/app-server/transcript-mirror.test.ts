...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-codex-transcript-"));
  tempDirs.push(dir);
...
describe("mirrorCodexAppServerTranscript", () => {
  ...
  it("leaves the assistant unowned when transcript persistence fails", async () => {
    const root = await makeRoot("nodoassist-codex-transcript-failure-");
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("nodoassist-codex-transcript-missing-dir-");
