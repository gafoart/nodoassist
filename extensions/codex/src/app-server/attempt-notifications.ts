...
/** Detects completion for an NodoAssist dynamic tool result still awaited by Codex. */
export function isPendingNodoAssistDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingNodoAssistDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingNodoAssistDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;
