...
function hasManagedSearchProvider(config: NodoAssistConfig | undefined): boolean {
  return normalizeOptionalString(config?.tools?.web?.search?.provider) !== undefined;
}

function hasNativeDomainRestrictions(config: NodoAssistConfig | undefined): boolean {
  return (
