...
/** Tool names owned by Codex app-server and normally excluded from NodoAssist dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  NODOASSIST_BUILD_PRIVATE_QA?: string;
  NODOASSIST_QA_FORCE_RUNTIME?: string;
};

/** Normalizes NodoAssist/Codex tool names before filtering and allowlist checks. */
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    ...
    env.NODOASSIST_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters NodoAssist tools that Codex owns natively or config explicitly excludes. */
