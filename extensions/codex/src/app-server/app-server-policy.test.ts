...
describe("Codex app-server policy", () => {
  it("keeps implicit Codex yolo approval policy when untrusted approvals are disallowed", () => {
    ...
    const resolved = resolveCodexAppServerForNodoAssistToolPolicy({
      appServer,
  ...
  it("promotes implicit yolo approval policy when NodoAssist tool policy requires review", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({ env: {}, requirementsToml: null });
  ...
  it("preserves explicit operator app-server policy", () => {
    ...
    const explicitConfig = resolveCodexAppServerForNodoAssistToolPolicy({
      appServer,
    ...
    const explicitEnv = resolveCodexAppServerForNodoAssistToolPolicy({
      appServer,
      pluginConfig: readCodexPluginConfig({}),
      env: { NODOASSIST_CODEX_APP_SERVER_APPROVAL_POLICY: "never" },
      shouldPromote: true,
      canUseUntrustedApprovalPolicy: true,
    });
    const explicitRequirements = resolveCodexAppServerForNodoAssistToolPolicy({
