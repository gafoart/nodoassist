...
import {
  ...
  isPendingNodoAssistDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingNodoAssistDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingNodoAssistDynamicToolCompletionNotification(
    notification,
    params.pendingNodoAssistDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after NodoAssist hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served NodoAssist
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingNodoAssistDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
