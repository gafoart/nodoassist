...
import type { ExecApprovalsFile } from "nodoassist/plugin-sdk/exec-approvals-runtime";
import { upsertSessionEntry } from "nodoassist/plugin-sdk/session-store-runtime";
...
vi.mock("nodoassist/plugin-sdk/exec-approvals-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("nodoassist/plugin-sdk/exec-approvals-runtime")>();
  return {
    ...actual,
    loadExecApprovals: execApprovalsRuntimeMocks.loadExecApprovals,
  };
});
vi.mock("nodoassist/plugin-sdk/agent-runtime", () => agentRuntimeMocks);
...
describe("codex conversation binding", () => {
  beforeEach(async () => {
    resetCodexTestBindingStore();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-codex-binding-"));
  });
  ...
  it("rejects binding when configured exec auto mode may need unrouted human approvals", async () => {
    ...
    await expect(
      ...
      "NodoAssist native Codex conversation binding cannot route interactive approvals yet",
    );
  ...
  it("blocks bound Codex app-server turns when the current NodoAssist session is sandboxed", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    await writeTestConversationBinding(sessionFile, { threadId: "thread-1", cwd: tempDir });
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because NodoAssist sandboxing is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when exec host=node is active", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because NodoAssist exec host=node is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the binding agent uses node exec without a session key", async () => {
    ...
    expect(result?.reply?.text).toContain("NodoAssist exec host=node is active");
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  });
  ...
  it("blocks bound Codex CLI node turns when the current NodoAssist session is sandboxed", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex CLI node conversation binding is unavailable because NodoAssist sandboxing is active for this session.",
        ),
      },
    });
  ...
  it("refreshes stale network-proxy bound app-server threads before the turn", async () => {
    ...
    await writeTestConversationBinding(sessionFile, {
      threadId: "thread-old",
      cwd: tempDir,
      networkProxyProfileName: "nodoassist-network-stale",
