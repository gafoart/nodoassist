...
vi.mock("nodoassist/plugin-sdk/memory-host-core", () => ({
  resolveDefaultAgentId: resolveDefaultAgentIdMock,
  resolveSessionAgentId: resolveSessionAgentIdMock,
}));

vi.mock("nodoassist/plugin-sdk/session-transcript-hit", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("nodoassist/plugin-sdk/session-transcript-hit")>();
  return {
    ...actual,
...
function createAppConfig(): NodoAssistConfig {
  return {
    agents: {
...
function createSessionVisibilityAppConfig(): NodoAssistConfig {
  return {
    agents: {
  ...
  loadCombinedSessionStoreForGatewayMock.mockReturnValue({
    ...
    store: {
      "agent:main:child-session": {
        ...
        updatedAt: 1,
        sessionFile: "/tmp/nodoassist/child-session.jsonl",
      },
      "agent:main:sibling-session": {
        sessionId: "sibling-session",
        updatedAt: 2,
        sessionFile: "/tmp/nodoassist/sibling-session.jsonl",
      },
    },
  });
}
...
describe("searchMemoryWiki", () => {
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Related",
          "<!-- nodoassist:wiki:related:start -->",
          "### Related Pages",
          "- [Needle Person](entities/needle-person.md)",
          "<!-- nodoassist:wiki:related:end -->",
          "",
        ].join("\n"),
      }),
      "utf8",
    ...
    await fs.writeFile(
      ...
      renderWikiMarkdown({
        ...
        body: [
          ...
          "",
          "## Agent Card",
          "- Maintainer lane: CEO; Microsoft-facing NodoAssist maintainer",
          "",
          "## AI Notes",
  ...
  it("reports a contract error when the shared manager lacks search()", async () => {
    ...
    // Partial manager as registered by @mem0/nodoassist-mem0 <= 1.0.14.
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:main:abc-uuid": {
          sessionId: "abc-uuid",
          updatedAt: 1,
          sessionFile: "/tmp/nodoassist/abc-uuid.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:visible-session": {
          ...
          updatedAt: 1,
          sessionFile: "/tmp/nodoassist/visible-session.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
    ...
    loadCombinedSessionStoreForGatewayMock.mockReturnValue({
      ...
      store: {
        "agent:secondary:main": {
          sessionId: "main",
          updatedAt: 1,
          sessionFile: "/tmp/nodoassist/main.jsonl",
        },
      },
    });
    const manager = createMemoryManager({
