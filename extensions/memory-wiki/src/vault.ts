...
import { FsSafeError, pathExists, root as fsRoot } from "nodoassist/plugin-sdk/security-runtime";
...
- Use \`.nodoassist-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the NodoAssist memory-wiki plugin.
...
- \`.nodoassist-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- nodoassist:human:start -->
<!-- nodoassist:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(rootDir, ".nodoassist-wiki/log.jsonl", "", createdFiles);
