...
describe("initializeMemoryWikiVault", () => {
  it("creates the wiki layout and seed files", async () => {
    ...
    await expect(fs.access(path.join(rootDir, ".nodoassist-wiki", "state.json"))).rejects.toThrow(
      /ENOENT/,
    );
    await expect(fs.access(path.join(rootDir, ".nodoassist-wiki", "locks"))).rejects.toThrow(
      /ENOENT/,
    );
  });
