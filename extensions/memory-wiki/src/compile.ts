...
import { retryTransientMemoryRead } from "nodoassist/plugin-sdk/memory-core-host-engine-storage";
import {
...
const AGENT_DIGEST_PATH = ".nodoassist-wiki/cache/agent-digest.json";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- nodoassist:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- nodoassist:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- nodoassist:wiki:index:start -->",
      endMarker: "<!-- nodoassist:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- nodoassist:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- nodoassist:wiki:${group.dir}:index:end -->`,
