...
description: Maintain the NodoAssist memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `nodoassist wiki ingest`, `nodoassist wiki compile`, and `nodoassist wiki lint` as the default maintenance loop.
- In `bridge` mode, run `nodoassist wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `nodoassist wiki unsafe-local import` only when the user explicitly opted into private local path access.
