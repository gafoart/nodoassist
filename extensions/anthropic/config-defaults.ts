...
function usesClaudeCliModelSelection(config: NodoAssistConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: NodoAssistConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: NodoAssistConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an NodoAssist config object. */
