 ...
 * thinking levels, and keeps NodoAssist-managed CLI runs isolated from shell env.
 */
import type {
  CliBackendConfig,
...
// steer NodoAssist-managed Claude CLI runs toward a different provider,
// endpoint, token source, plugin/config tree, or telemetry bootstrap mode.
/** Environment variables removed before launching NodoAssist-managed Claude CLI runs. */
...
function isNodoAssistRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
/** Resolve Claude permission mode from NodoAssist exec security settings. */
export function resolveClaudePermissionMode(context?: CliBackendNormalizeConfigContext): {
  mode?: string;
  overrideExisting: boolean;
} {
  return isNodoAssistRequestedYolo(context)
...
/** Map NodoAssist thinking levels to Claude CLI effort flags for a model id. */
