...
import { streamSimple } from "nodoassist/plugin-sdk/llm";
import type { ProviderWrapStreamFnContext } from "nodoassist/plugin-sdk/plugin-entry";
import {
...
const NODOASSIST_DEFAULT_ANTHROPIC_BETAS = [
  "fine-grained-tool-streaming-2025-05-14",
  "interleaved-thinking-2025-05-14",
] as const;
const NODOASSIST_OAUTH_ANTHROPIC_BETAS = [
  "claude-code-20250219",
  "oauth-2025-04-20",
  ...NODOASSIST_DEFAULT_ANTHROPIC_BETAS,
] as const;
...
/** Wrap a stream function to merge NodoAssist and configured Anthropic beta headers. */
export function createAnthropicBetaHeadersWrapper(
  ...
  return (model, context, options) => {
    ...
    const nodoAssistBetas = isOauth
      ? (NODOASSIST_OAUTH_ANTHROPIC_BETAS as readonly string[])
      : (NODOASSIST_DEFAULT_ANTHROPIC_BETAS as readonly string[]);
    const allBetas = [...new Set([...nodoAssistBetas, ...effectiveBetas])];
