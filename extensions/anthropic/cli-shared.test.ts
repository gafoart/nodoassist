...
describe("resolveClaudeCliExecutionArgs", () => {
  ...
  it("maps NodoAssist thinking levels to Claude effort args", () => {
    expect(
      resolveClaudeCliExecutionArgs({
        workspaceDir: "/tmp",
        provider: "claude-cli",
    ...
    expect(
      resolveClaudeCliExecutionArgs({
        ...
        baseArgs: [
          ...
          "--output-format",
          "stream-json",
          "--allowedTools=mcp__nodoassist__*",
          "--allowedTools",
          "Read",
          "Grep",
          "--permission-mode",
...
describe("normalizeClaudeBackendConfig", () => {
  ...
  it("derives Claude bypass from NodoAssist YOLO policy and disables it for safer policy", () => {
  ...
  it("derives Claude bypass from per-agent NodoAssist exec policy", () => {
    expect(
      resolveClaudePermissionMode({
        backendId: "claude-cli",
        agentId: "safe-agent",
  ...
  it("passes system prompt on every turn (issue #80374 — systemPromptWhen must be 'always')", () => {
    ...
    // NodoAssist system prompt context on resumed / compacted claude-cli sessions.
    const backend = buildAnthropicCliBackend();
