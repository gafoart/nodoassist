...
import { resolvePositiveTimeoutSeconds } from "nodoassist/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "nodoassist/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<NodoAssistConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<NodoAssistConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: NodoAssistConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: NodoAssistConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: NodoAssistConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: NodoAssistConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: NodoAssistConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: NodoAssistConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlMaxAgeMs(cfg?: NodoAssistConfig, override?: number): number {
