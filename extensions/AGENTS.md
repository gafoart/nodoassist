...
- Extension production code should import from `nodoassist/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `nodoassist.plugin.json` and the package
  `nodoassist` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `nodoassist/plugin-sdk/*`.
