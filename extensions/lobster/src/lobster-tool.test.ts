...
import type { NodoAssistPluginApi, NodoAssistPluginToolContext } from "../runtime-api.js";
import { createLobsterTool } from "./lobster-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<NodoAssistPluginApi> = {}): NodoAssistPluginApi {
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: NodoAssistPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
