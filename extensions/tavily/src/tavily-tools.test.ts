...
function fakeApi(): NodoAssistPluginApi {
  return {
    config: {},
...
describe("tavily tools", () => {
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<NodoAssistPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<NodoAssistPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "nodoassist" });
    ...
    expect(searchParams.query).toBe("nodoassist");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as NodoAssistConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
