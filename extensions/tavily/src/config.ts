...
import { resolvePositiveTimeoutSeconds } from "nodoassist/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: NodoAssistConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: NodoAssistConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: NodoAssistConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
