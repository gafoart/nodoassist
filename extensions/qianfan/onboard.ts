...
function resolveQianfanPreset(cfg: NodoAssistConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: NodoAssistConfig) => {
...
export function applyQianfanProviderConfig(cfg: NodoAssistConfig): NodoAssistConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: NodoAssistConfig): NodoAssistConfig {
