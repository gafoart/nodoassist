...
import type { NodoAssistPluginNodeInvokePolicyContext } from "nodoassist/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: NodoAssistPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<NodoAssistPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<NodoAssistPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<NodoAssistPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
