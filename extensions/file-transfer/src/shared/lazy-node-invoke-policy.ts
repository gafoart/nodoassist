...
import type { NodoAssistPluginNodeInvokePolicy } from "nodoassist/plugin-sdk/plugin-entry";
import { FILE_TRANSFER_NODE_INVOKE_COMMANDS } from "./node-invoke-policy-commands.js";

type LoadFileTransferNodeInvokePolicy = () => Promise<NodoAssistPluginNodeInvokePolicy>;
...
export function createLazyFileTransferNodeInvokePolicy(
  ...
  let policyPromise: Promise<NodoAssistPluginNodeInvokePolicy> | undefined;

  return {
    commands: [...FILE_TRANSFER_NODE_INVOKE_COMMANDS],
    async handle(ctx) {
      let policy: NodoAssistPluginNodeInvokePolicy;
      try {
