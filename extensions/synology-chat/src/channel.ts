/**
 * Synology Chat Channel Plugin for NodoAssist.
 *
 * Implements the ChannelPlugin interface following the LINE pattern.
 */

import { DEFAULT_ACCOUNT_ID } from "nodoassist/plugin-sdk/account-id";
import type { NodoAssistConfig } from "nodoassist/plugin-sdk/account-resolution";
import {
...
import { waitUntilAbort } from "nodoassist/plugin-sdk/channel-outbound";
import {
...
import { createEmptyChannelDirectoryAdapter } from "nodoassist/plugin-sdk/directory-runtime";
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "nodoassist pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: NodoAssistConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: NodoAssistConfig; account: ResolvedSynologyChatAccount }) => {
  ...
  return createChatChannelPlugin({
    base: {
      ...
      meta: {
        ...
        blurb: "Connect your Synology NAS Chat to NodoAssist",
        order: 90,
      },
      capabilities: {
    ...
    pairing: {
      text: {
        ...
        message: "NodoAssist: your access has been approved.",
