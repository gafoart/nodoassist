...
vi.mock("nodoassist/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("nodoassist/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("nodoassist/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("nodoassist/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/nodoassist/synology-chat-sessions.json"),
