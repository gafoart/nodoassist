...
const TELEGRAM_PROFILE_ARCHIVE_ENV = "NODOASSIST_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64";
const TELEGRAM_PROFILE_ARCHIVE_ENV_NAME_ENV =
  "NODOASSIST_MANTIS_TELEGRAM_DESKTOP_PROFILE_ARCHIVE_ENV";
const TELEGRAM_PROFILE_DIR_ENV = "NODOASSIST_MANTIS_TELEGRAM_DESKTOP_PROFILE_DIR";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.NODOASSIST_MANTIS_TELEGRAM_GROUP_ID)) {
    next.NODOASSIST_MANTIS_TELEGRAM_GROUP_ID = trimToValue(next.NODOASSIST_QA_TELEGRAM_GROUP_ID);
  }
  if (!trimToValue(next.NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN)) {
    next.NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = trimToValue(
      next.NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN,
    );
  }
  if (!trimToValue(next.NODOASSIST_MANTIS_TELEGRAM_SUT_BOT_TOKEN)) {
    next.NODOASSIST_MANTIS_TELEGRAM_SUT_BOT_TOKEN = trimToValue(
      next.NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN,
    );
  }
  return next;
}
...
function resolveTelegramGatewayEnvPayload(
  ...
  const groupId = trimToValue(env.NODOASSIST_QA_TELEGRAM_GROUP_ID);
  const driverToken = trimToValue(env.NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN);
  const sutToken = trimToValue(env.NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN);
  if (!groupId || !driverToken || !sutToken) {
    throw new Error(
      "Telegram desktop builder requires NODOASSIST_QA_TELEGRAM_GROUP_ID, NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN, and NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN when using --credential-source env.",
    );
  }
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.NODOASSIST_MANTIS_TELEGRAM_GROUP_ID) &&
    trimToValue(params.env.NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN) &&
    trimToValue(params.env.NODOASSIST_MANTIS_TELEGRAM_SUT_BOT_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.NODOASSIST_MANTIS_TELEGRAM_GROUP_ID = payload.groupId;
  params.env.NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN = payload.driverToken;
  params.env.NODOASSIST_MANTIS_TELEGRAM_SUT_BOT_TOKEN = payload.sutToken;
  params.env.NODOASSIST_QA_TELEGRAM_GROUP_ID =
    trimToValue(params.env.NODOASSIST_QA_TELEGRAM_GROUP_ID) ?? payload.groupId;
  params.env.NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN =
    trimToValue(params.env.NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN) ?? payload.driverToken;
  params.env.NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN =
    trimToValue(params.env.NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN) ?? payload.sutToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
telegram_root="$HOME/.local/share/nodoassist-mantis/telegram-desktop-bin"
...
if [ -n "\${NODOASSIST_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64:-}" ]; then
  printf '%s' "$NODOASSIST_MANTIS_TELEGRAM_DESKTOP_PROFILE_TGZ_B64" | base64 -d >"$out/telegram-profile.tgz"
...
qa_status=0
{
  ...
  if [ "$setup_gateway" = "1" ]; then
    export TELEGRAM_BOT_TOKEN="\${NODOASSIST_MANTIS_TELEGRAM_SUT_BOT_TOKEN:-\${TELEGRAM_BOT_TOKEN:-}}"
    telegram_group_id="\${NODOASSIST_MANTIS_TELEGRAM_GROUP_ID:-}"
    driver_token="\${NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN:-}"
    if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$telegram_group_id" ] || [ -z "$driver_token" ]; then
      echo "Gateway setup requires NODOASSIST_MANTIS_TELEGRAM_GROUP_ID, NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN, and NODOASSIST_MANTIS_TELEGRAM_SUT_BOT_TOKEN." >&2
      exit 2
    fi
...
const token = process.env.NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
...
node --input-type=module -e 'import fs from "node:fs"; const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(value.id || ""));' "$out/telegram-driver-getme.json")"
    export NODOASSIST_HOME="$HOME/.nodoassist-mantis/telegram-nodoassist"
...
MANTIS_TELEGRAM_PATCH
    pnpm nodoassist config patch --file "$out/telegram.patch.json5" --dry-run
    pnpm nodoassist config patch --file "$out/telegram.patch.json5"
    node --input-type=module >"$out/telegram-ready-message.json" 2>"$out/telegram-ready-message.err" <<'MANTIS_TELEGRAM_READY'
const token = process.env.NODOASSIST_MANTIS_TELEGRAM_DRIVER_BOT_TOKEN;
const chatId = process.env.NODOASSIST_MANTIS_TELEGRAM_GROUP_ID;
...
MANTIS_TELEGRAM_READY
    nohup pnpm nodoassist gateway run --dev --allow-unconfigured --port 38974 --cli-backend-logs </dev/null >"$out/nodoassist-gateway.log" 2>&1 &
    gateway_pid="$!"
...
function renderReport(summary: MantisTelegramDesktopBuilderSummary) {
  const lines = [
    ...
    "- NodoAssist gateway log: `nodoassist-gateway.log`",
...
export async function runMantisTelegramDesktopBuilder(
  ...
  const remoteOutputDir = `/tmp/nodoassist-mantis-telegram-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Telegram desktop builder did not report a live NodoAssist gateway.");
    }
    summary = {
      artifacts: {
        reportPath,
