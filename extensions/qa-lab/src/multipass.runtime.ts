...
import type { NodoAssistCrablineChannelDriverSelection } from "@openclaw/crabline";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/nodoassist-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/nodoassist";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/nodoassist-codex-home";
...
type QaMultipassPlan = {
  ...
  channelDriverSelection?: NodoAssistCrablineChannelDriverSelection;
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `nodoassist-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "NODOASSIST_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `NODOASSIST_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `NODOASSIST_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# NodoAssist QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm nodoassist qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredNodoAssistTmpDir(), `${plan.vmName}-qa-suite-`),
  );
