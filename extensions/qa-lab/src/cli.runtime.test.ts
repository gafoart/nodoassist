...
describe("qa cli runtime", () => {
  ...
  beforeEach(async () => {
    ...
    writeQaDockerHarnessFiles.mockResolvedValue({
      outputDir: "/tmp/nodoassist-repo/.artifacts/qa-docker",
    });
    buildQaDockerHarnessImage.mockResolvedValue({
      imageName: "nodoassist:qa-local-prebaked",
    });
  ...
  it("dispatches a taxonomy-backed profile category through the suite runner", async () => {
    ...
    process.env.NODOASSIST_QA_PROFILE = "release";
    try {
      runQaSuite.mockImplementationOnce(async () => {
        expect(process.env.NODOASSIST_QA_PROFILE).toBe("smoke-ci");
        await fs.writeFile(
          suiteEvidencePath,
          JSON.stringify(
            makeQaEvidence([
      ...
      expectFields(suiteArgs, {
        repoRoot: path.resolve("/tmp/nodoassist-repo"),
        outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa-e2e/smoke-ci"),
        transportId: "qa-channel",
      ...
      expect(process.env.NODOASSIST_QA_PROFILE).toBe("release");
  ...
  it("resolves suite repo-root-relative paths before dispatching", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/nodoassist-repo"),
      outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa/frontier"),
      transportId: "qa-channel",
      channelDriver: undefined,
      channelDriverSelection: undefined,
      providerMode: "live-frontier",
  ...
  it("uses the Crabline default channel when selected scenarios do not request one", async () => {
    ...
    expect(runQaSuite).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/nodoassist-repo"),
      outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa/multipass-telegram"),
      transportId: "qa-channel",
      channelDriver: "crabline",
  ...
  it("rejects unknown runtime-pair ids at the CLI boundary", async () => {
    ...
    ).rejects.toThrow('--runtime-pair only supports "nodoassist" and "codex".');
    expect(runQaSuite).not.toHaveBeenCalled();
  });
    ...
    expect(runCanonicalLiveScenarios).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.objectContaining({
          repoRoot: path.resolve("/tmp/nodoassist-repo"),
          outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa/telegram"),
          providerMode: "live-frontier",
          primaryModel: "openai/gpt-5.5",
          alternateModel: "openai/gpt-5.5",
          fastMode: true,
  ...
  it("rejects output dirs that escape the repo root", () => {
    expect(() => resolveRepoRelativeOutputDir("/tmp/nodoassist-repo", "../outside")).toThrow(
      "--output-dir must stay within the repo root.",
    );
    expect(() => resolveRepoRelativeOutputDir("/tmp/nodoassist-repo", "/tmp/outside")).toThrow(
  ...
  it("prints telegram scenario catalog without starting the live lane", async () => {
    ...
    expectWriteContains(
      stdoutWrite,
      "telegram-status-command\tdefault\tTelegram status command reply\tVerify Telegram status returns model, session, and activation details. refs=nodoassist/nodoassist#74698",
    );
  });
  ...
  it("runs a host-only parity preflight against the sentinel scenario", async () => {
    const repoRoot = path.resolve("/tmp/nodoassist-repo");
    await runQaSuiteCommand({
      repoRoot: "/tmp/nodoassist-repo",
      providerMode: "mock-openai",
      primaryModel: "openai/gpt-5.5",
  ...
  it("prints a markdown tool coverage report from runtime tool fixtures", async () => {
    ...
    expectWriteContains(stdoutWrite, "# NodoAssist Runtime Tool Coverage");
  ...
  it("writes a curated mock JSONL replay report and summary", async () => {
    ...
    try {
      ...
      expect(report).toContain("# NodoAssist JSONL Replay Report - nodoassist vs codex");
    ...
    try {
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          run: { runtimePair: ["nodoassist", "codex"] },
        }),
        "utf8",
      );
  ...
  it("resolves character eval paths and passes model refs through", async () => {
    ...
    expectFields(characterEvalArgs, {
      repoRoot: path.resolve("/tmp/nodoassist-repo"),
      outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa/character"),
  ...
  it("routes suite runs through multipass when the runner is selected", async () => {
    ...
    expect(runQaMultipass).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/nodoassist-repo"),
      outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa-multipass"),
      transportId: "qa-channel",
      providerMode: "mock-openai",
      primaryModel: undefined,
      alternateModel: undefined,
  ...
  it("resolves self-check repo-root-relative paths before starting the lab server", async () => {
    ...
    expect(startQaLabServer).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/nodoassist-repo"),
      outputPath: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa/self-check.md"),
    });
  });
  ...
  it("rejects oversized credential payload files before broker setup", async () => {
    const previousMaxBytes = process.env.NODOASSIST_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
    ...
    process.env.NODOASSIST_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = "32";
    ...
    try {
      await expect(
        ...
        "Payload file exceeds NODOASSIST_QA_CREDENTIAL_PAYLOAD_MAX_BYTES (32 bytes).",
      );
    } finally {
      if (previousMaxBytes === undefined) {
        delete process.env.NODOASSIST_QA_CREDENTIAL_PAYLOAD_MAX_BYTES;
      } else {
        process.env.NODOASSIST_QA_CREDENTIAL_PAYLOAD_MAX_BYTES = previousMaxBytes;
      }
    }
  });
  ...
  it("resolves docker scaffold paths relative to the explicit repo root", async () => {
    ...
    expect(writeQaDockerHarnessFiles).toHaveBeenCalledWith({
      outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa-docker"),
      repoRoot: path.resolve("/tmp/nodoassist-repo"),
  ...
  it("passes the explicit repo root into docker image builds", async () => {
    await runQaDockerBuildImageCommand({
      repoRoot: "/tmp/nodoassist-repo",
      image: "nodoassist:qa-local-prebaked",
    });
  ...
  it("resolves docker up paths relative to the explicit repo root", async () => {
    ...
    expect(runQaDockerUp).toHaveBeenCalledWith({
      repoRoot: path.resolve("/tmp/nodoassist-repo"),
      outputDir: path.resolve("/tmp/nodoassist-repo", ".artifacts/qa-up"),
      gatewayPort: undefined,
      qaLabPort: undefined,
