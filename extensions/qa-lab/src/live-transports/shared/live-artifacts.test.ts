...
describe("live transport artifacts", () => {
  ...
  it("preserves cleanup phase labels while redacting details", () => {
    expect(
      ...
      "credential lease release: details redacted (NODOASSIST_QA_REDACT_PUBLIC_METADATA=1)",
      "live gateway cleanup: details redacted (NODOASSIST_QA_REDACT_PUBLIC_METADATA=1)",
    ]);
  });
    ...
    expect(
      redactQaLiveLaneIssues([
        [
          ...
          "Artifacts:",
          "- gatewayDebug: /tmp/nodoassist-whatsapp-qa/gateway-debug",
        ].join("\n"),
      ]),
    ).toEqual(["details redacted (NODOASSIST_QA_REDACT_PUBLIC_METADATA=1)"]);
  });
});
