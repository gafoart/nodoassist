...
vi.mock("nodoassist/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/ssrf-runtime")>(
    "nodoassist/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
describe("telegram live qa runtime", () => {
  ...
  it("resolves required Telegram QA env vars", () => {
    expect(
      testing.resolveTelegramQaRuntimeEnv({
        NODOASSIST_QA_TELEGRAM_GROUP_ID: "-100123",
        NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toEqual({
      groupId: "-100123",
      driverToken: "driver",
  ...
  it("fails when a required Telegram QA env var is missing", () => {
    ...
    ).toThrow("NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN");
  });

  it("fails when the Telegram group id is not numeric", () => {
    expect(() =>
      testing.resolveTelegramQaRuntimeEnv({
        NODOASSIST_QA_TELEGRAM_GROUP_ID: "qa-group",
        NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN: "driver",
        NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN: "sut",
      }),
    ).toThrow("NODOASSIST_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  });
  ...
  it("applies NODOASSIST_QA_SUITE_PROGRESS override to Telegram live logging", () => {
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "false",
        NODOASSIST_QA_SUITE_PROGRESS: "true",
      }),
    ).toBe(true);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        NODOASSIST_QA_SUITE_PROGRESS: "false",
      }),
    ).toBe(false);
    expect(
      testing.shouldLogTelegramQaLiveProgress({
        CI: "true",
        NODOASSIST_QA_SUITE_PROGRESS: "definitely",
      }),
    ).toBe(true);
  });
  ...
  it("normalizes the Telegram QA transport ready timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaReadyTimeoutMs({
        NODOASSIST_QA_TRANSPORT_READY_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveTelegramQaReadyTimeoutMs({
        NODOASSIST_QA_TRANSPORT_READY_TIMEOUT_MS: "bad",
      }),
    ).toBe(45_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaReadyTimeoutMs({
          NODOASSIST_QA_TRANSPORT_READY_TIMEOUT_MS: value,
        }),
      ).toBe(45_000);
    }
  });
  ...
  it("normalizes the Telegram QA canary timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        NODOASSIST_QA_TELEGRAM_CANARY_TIMEOUT_MS: "90000",
      }),
    ).toBe(90_000);
    expect(
      testing.resolveTelegramQaCanaryTimeoutMs({
        NODOASSIST_QA_TELEGRAM_CANARY_TIMEOUT_MS: "nope",
      }),
    ).toBe(30_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaCanaryTimeoutMs({
          NODOASSIST_QA_TELEGRAM_CANARY_TIMEOUT_MS: value,
        }),
      ).toBe(30_000);
    }
  });
  ...
  it("normalizes the Telegram QA scenario timeout env", () => {
    ...
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        NODOASSIST_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "180000",
      }),
    ).toBe(180_000);
    expect(
      testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
        NODOASSIST_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: "nope",
      }),
    ).toBe(45_000);
    for (const value of ["0x10", "1e3", "10.5"]) {
      expect(
        testing.resolveTelegramQaScenarioTimeoutMs(45_000, {
          NODOASSIST_QA_TELEGRAM_SCENARIO_TIMEOUT_MS: value,
        }),
      ).toBe(45_000);
    }
  });
  ...
  it("includes mention gating in the Telegram live scenario catalog", () => {
    ...
    expect(otherBotStep?.input).toBe("/status@NodoAssistQaOtherBot");
    expect(
      scenarios
  ...
  it("lists remaining optional Telegram scenario metadata", () => {
    ...
    expect(streamSingle.regressionRefs).toEqual(["nodoassist/nodoassist#39905"]);
  });
