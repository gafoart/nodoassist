...
import type { TelegramBotMessage, TelegramBotUpdate } from "@nodoassist/telegram/api.js";
...
import { isRecord, uniqueStrings } from "nodoassist/plugin-sdk/string-coerce-runtime";
import { z } from "zod";
...
const TELEGRAM_QA_SCENARIOS: TelegramQaScenarioDefinition[] = [
  {
    ...
    buildRun: () =>
      telegramQaStepRun({
        expectReply: false,
        input: "/status@NodoAssistQaOtherBot",
      }),
  },
  {
  ...
  },
  {
    ...
    regressionRefs: ["nodoassist/nodoassist#39905"],
    timeoutMs: 75_000,
    buildRun: (sutUsername) =>
      telegramQaStepRun({
...
const QA_SUITE_PROGRESS_ENV = "NODOASSIST_QA_SUITE_PROGRESS";
...
function resolveTelegramQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): TelegramQaRuntimeEnv {
  const groupId = resolveEnvValue(env, "NODOASSIST_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("NODOASSIST_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
export async function runTelegramQaLive(params: {
  ...
  try {
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutNodoAssistCommand
        ? {
            executablePath: params.sutNodoAssistCommand,
