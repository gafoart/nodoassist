...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        NODOASSIST_QA_SLACK_CHANNEL_ID: "C123456789",
        NODOASSIST_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        NODOASSIST_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        NODOASSIST_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        NODOASSIST_QA_SLACK_CHANNEL_ID: "qa-channel",
        NODOASSIST_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        NODOASSIST_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        NODOASSIST_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("NODOASSIST_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
  ...
  it("rejects an incompatible Codex approval model before credential acquisition", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "nodoassist-slack-codex-model-"));
    await expect(
      runSlackQaLive({
  ...
  it("builds Codex approval instructions for command and file-change routes", () => {
    ...
    expect(testing.resolveCodexFileApprovalTargetPath("MARKER")).toMatch(
      /\.nodoassist-qa-codex-file-approval-marker\.txt$/u,
    );
  });
  ...
  it("matches resolved Codex approvals without pending-only marker text", () => {
    expect(
      testing.matchesSlackApprovalResolvedUpdate({
        ...
        approvalKind: "plugin",
        decision: "allow-once",
        extraTextMatches: ["nodoassist-codex-app-server", "Codex app-server file approval"],
        text: [
          "Plugin approval: Allowed once",
          "Codex app-server file approval",
          "Plugin: nodoassist-codex-app-server",
        ].join("\n"),
      }),
    ).toBe(true);
    expect(
  ...
  it("matches pending Codex approvals by stable renderer fields without marker text", () => {
    expect(
      testing.matchesSlackApprovalPromptText({
        approvalKind: "plugin",
        extraTextMatches: ["nodoassist-codex-app-server", "Codex app-server command approval"],
        text: [
  ...
  it("resolves Slack approval checkpoint configuration from env", () => {
    expect(
      testing.resolveSlackApprovalCheckpointConfig({
        NODOASSIST_QA_SLACK_APPROVAL_CHECKPOINT_DIR: "/tmp/checkpoints",
        NODOASSIST_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
  ...
  it("writes artifacts when Convex credential acquisition fails", async () => {
    const outputDir = await fs.mkdtemp(path.join(tmpdir(), "nodoassist-slack-qa-"));
    ...
    expect(result.scenarios[0]?.details).toContain("Missing NODOASSIST_QA_CONVEX_SITE_URL");
    ...
    expect(summary.entries[0]).toMatchObject({
      ...
      result: {
        status: "fail",
        failure: {
          reason: expect.stringContaining("Missing NODOASSIST_QA_CONVEX_SITE_URL"),
        },
      },
    });
  });
