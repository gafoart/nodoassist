...
import { createSlackWebClient, createSlackWriteClient } from "@nodoassist/slack/api.js";
...
export async function createSlackQaTransportAdapter(
  ...
  return {
    ...
    async sendInbound(input) {
      ...
      const text = input.text.replaceAll("@nodoassist", `<@${sutIdentity.userId}>`);
    ...
    createGatewayConfig: () =>
      slackLive.buildSlackQaConfig({} as NodoAssistConfig, {
        channelId: runtimeEnv.channelId,
