...
import { startWhatsAppQaDriverSession } from "@nodoassist/whatsapp/api.js";
...
export async function createWhatsAppQaTransportAdapter(
  ...
  try {
    authRoot = await fs.mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-whatsapp-qa-adapter-"),
    );
  ...
  return {
    ...
    createGatewayConfig: () =>
      whatsappLive.buildWhatsAppQaConfig({} as NodoAssistConfig, {
