...
describe("tool search gateway e2e fetch helper", () => {
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readToolSearchGatewayFetchLimits({
        NODOASSIST_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid NODOASSIST_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: 1e3");
    expect(() =>
      readToolSearchGatewayFetchLimits({
        NODOASSIST_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid NODOASSIST_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: 1000ms");
    expect(
      readToolSearchGatewayFetchLimits({
        NODOASSIST_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "4096",
        NODOASSIST_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "5000",
      }),
    ).toEqual({
      bodyMaxBytes: 4096,
      timeoutMs: 5_000,
...
describe("tool search gateway e2e session log scanner", () => {
  it("does not count target mentions from user prompt records", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-tool-search-log-"));
    try {
