...
vi.mock("nodoassist/plugin-sdk/gateway-runtime", () => ({
...
describe("startQaGatewayRpcClient", () => {
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    const originalHome = process.env.NODOASSIST_HOME;
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.NODOASSIST_HOME = originalHome;
      }
    }

    expect(process.env.NODOASSIST_HOME).toBe(originalHome);
  });
