...
export const QA_EVIDENCE_SUMMARY_KIND = "nodoassist.qa.evidence-summary";
...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["NODOASSIST_E2E_PROFILE", params.env?.NODOASSIST_E2E_PROFILE],
    ["NODOASSIST_QA_PROFILE", params.env?.NODOASSIST_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.NODOASSIST_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidenceChannelDriver(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  const id =
    params.fallback?.trim() ||
    params.env?.NODOASSIST_QA_CHANNEL_DRIVER?.trim() ||
    params.env?.NODOASSIST_E2E_CHANNEL_DRIVER?.trim();
...
export function resolveQaEvidenceEnvironment(params: {
  ...
  return {
    ...
    ref:
      params.env?.NODOASSIST_QA_REF?.trim() ||
...
function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.NODOASSIST_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.NODOASSIST_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.NODOASSIST_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
