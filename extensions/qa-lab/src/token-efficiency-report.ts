...
export type TokenEfficiencyRow = {
  ...
  nodoassist: TokenEfficiencyRuntimeUsage;
...
export type TokenEfficiencyReport = {
  ...
  aggregate: {
    nodoassist: { totalTokens: number; p50PerScenario: number; p90PerScenario: number };
...
const ZERO_AGGREGATE: TokenEfficiencyReport["aggregate"] = {
  nodoassist: { totalTokens: 0, p50PerScenario: 0, p90PerScenario: 0 },
...
function deltaPercent(nodoassistTotalTokens: number, codexTotalTokens: number): number {
  if (nodoassistTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - nodoassistTotalTokens) / nodoassistTotalTokens) * 100;
}
...
function toolNamesForCells(nodoassist: RuntimeParityCell, codex: RuntimeParityCell): string[] {
  return [
    ...new Set([...nodoassist.toolCalls, ...codex.toolCalls].map((call) => call.tool)),
...
function buildRow(params: {
  ...
  const nodoassist = runtimeUsage(params.result.cells.nodoassist);
  const codex = runtimeUsage(params.result.cells.codex);
  const delta = deltaPercent(nodoassist.totalTokens, codex.totalTokens);
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.nodoassist, params.result.cells.codex),
  };
}

function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const nodoassistTotals = rows.map((row) => row.nodoassist.totalTokens);
  const codexTotals = rows.map((row) => row.codex.totalTokens);
  const nodoassistTotalTokens = nodoassistTotals.reduce((sum, value) => sum + value, 0);
  const codexTotalTokens = codexTotals.reduce((sum, value) => sum + value, 0);
  return {
    nodoassist: {
      totalTokens: nodoassistTotalTokens,
      p50PerScenario: percentile(nodoassistTotals, 50),
      p90PerScenario: percentile(nodoassistTotals, 90),
    },
    codex: {
    ...
    deltaPercent: deltaPercent(nodoassistTotalTokens, codexTotalTokens),
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.nodoassist.totalTokens <= 0) {
    failures.push(
      `${row.scenarioId} nodoassist live usage totalTokens=${row.nodoassist.totalTokens}`,
    );
  }
...
export function buildTokenEfficiencyReport(
  ...
  const failures = rows.flatMap((row, index) => {
    ...
    const rowFailures =
      liveUsage && result
        ? [
            ...liveUsageShapeFailures(row.scenarioId, "nodoassist", result.cells.nodoassist.usage),
  ...
  return {
    ...
    notes: [
      ...
      "Codex savings are reported as savings and do not fail the gate; only positive Codex-over-NodoAssist live deltas exceed the threshold.",
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  const lines = [
    `# NodoAssist Runtime Token Efficiency - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: |",
    `| nodoassist | ${report.aggregate.nodoassist.totalTokens} | ${report.aggregate.nodoassist.p50PerScenario} | ${report.aggregate.nodoassist.p90PerScenario} |`,
  ...
  if (report.rows.length > 0) {
    lines.push(
      "## Scenario Efficiency",
      "",
      "| Scenario | Source | NodoAssist in/out/total/tools | Codex in/out/total/tools | Token delta | Classification | Flagged | Tools used |",
      "| --- | --- | ---: | ---: | ---: | --- | --- | --- |",
    );
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${row.nodoassist.inputTokens}/${row.nodoassist.outputTokens}/${row.nodoassist.totalTokens}/${row.nodoassist.toolCallCount} | ${row.codex.inputTokens}/${row.codex.outputTokens}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatPercent(row.deltaPercent)} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }
