...
describe("evidence gallery", () => {
  it("builds a generic gallery model for non-UX QA Lab evidence", async () => {
    ...
    const evidence: QaEvidenceSummaryJson = buildVitestEvidenceSummary({
      ...
      env: {
        NODOASSIST_QA_REF: "gallery-test",
  ...
  it("sanitizes local roots from gallery failure reasons", async () => {
    ...
    evidence.entries[0] = {
      ...
      result: {
        ...
        failure: {
          class: "blocked",
          reason: `Command failed at ${repoRoot}/nodoassist.mjs and file://${repoRoot}/trace.log`,
        },
      },
    };
    ...
    expect(model.entries[0].failureReason).toBe(
      "Command failed at <repo-root>/nodoassist.mjs and file://<repo-root>/trace.log",
    );
    ...
    await writeJson(path.join(runDir, "matrix.json"), {
      ...
      cells: [
        null,
        {
          ...
          runner: {
            availability: "local",
            command: `${repoRoot}/nodoassist.mjs qa suite --scenario ux-matrix-evidence-dashboard`,
        ...
        },
        {
          ...
          runner: {
            availability: "local",
            command: "pnpm nodoassist qa suite --scenario ux-matrix-evidence-dashboard",
            lane: "cli-status",
    ...
    expect(model.producerContext?.matrix?.cells).toEqual([
      {
        ...
        runner: {
          availability: "local",
          command: "<repo-root>/nodoassist.mjs qa suite --scenario ux-matrix-evidence-dashboard",
          lane: "web-ui-playwright",
  ...
  it("resolves evidence and declared artifacts inside the repo root only", async () => {
    ...
    await expect(
      resolveQaEvidenceFile({ inputPath: "/tmp/not-nodoassist-evidence.json", repoRoot }),
