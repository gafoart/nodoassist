...
import { NODOASSIST_CRABLINE_DEFAULT_CHANNEL } from "@openclaw/crabline";
...
export function createQaSmokeCiMatrix(): { include: QaSmokeCiShard[] } {
  ...
  for (const scenario of scenarios) {
    const channel = scenario.execution.channel ?? NODOASSIST_CRABLINE_DEFAULT_CHANNEL;
  ...
  const shards = [...scenariosByChannel.entries()]
    ...
    .flatMap(([channel, channelScenarios]) => {
      const shardCount =
        channel === NODOASSIST_CRABLINE_DEFAULT_CHANNEL
