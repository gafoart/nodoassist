...
const QA_LAB_UI_OVERLAY_DIR = "/opt/nodoassist-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${yamlDoubleQuoted(context)}\n      dockerfile: Dockerfile\n      args:\n        NODOASSIST_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    volumes:
      - ./state:/opt/nodoassist-scaffold:ro
${params.bindUiDist ? `      - ${yamlDoubleQuoted(`${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro`)}\n` : ""}    healthcheck:
    ...
    command:
      - sh
      - -lc
      - NODOASSIST_QA_CONTROL_UI_PROXY_TOKEN="$(node -e 'const fs=require("node:fs");const cfg=JSON.parse(fs.readFileSync("/opt/nodoassist-scaffold/nodoassist.json","utf8"));process.stdout.write(cfg.gateway?.auth?.token ?? "")')" exec node dist/index.js qa ui --host 0.0.0.0 --port ${QA_LAB_INTERNAL_PORT} --advertise-host 127.0.0.1 --advertise-port ${params.qaLabPort} --control-ui-url http://127.0.0.1:${params.gatewayPort}/ --control-ui-proxy-target http://openclaw-qa-gateway:18789/${params.bindUiDist ? ` --ui-dist-dir ${QA_LAB_UI_OVERLAY_DIR}` : ""} --auto-kickoff-target direct --send-kickoff-on-start --embedded-gateway disabled
    depends_on:
      qa-mock-openai:
        condition: service_healthy
`
    : ""
}  nodoassist-qa-gateway:
${imageBlock}    pull_policy: never
    extra_hosts:
    ...
    environment:
      NODOASSIST_CONFIG_PATH: /tmp/nodoassist/nodoassist.json
      NODOASSIST_STATE_DIR: /tmp/nodoassist/state
    ...
    volumes:
      - ./state:/opt/nodoassist-scaffold:ro
      - ${yamlDoubleQuoted(`${repoMount}:/opt/nodoassist-repo:ro`)}
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/nodoassist/workspace /tmp/nodoassist/state && cp /opt/nodoassist-scaffold/nodoassist.json /tmp/nodoassist/nodoassist.json && cp -R /opt/nodoassist-scaffold/seed-workspace/. /tmp/nodoassist/workspace/ && rm -rf /tmp/nodoassist/workspace/repo && ln -s /opt/nodoassist-repo /tmp/nodoassist/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
- \`.env.example\`
- \`state/nodoassist.json\`

Suggested flow:

1. Build the prebaked image once:
   - \`docker build -t nodoassist:qa-local-prebaked --build-arg NODOASSIST_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "nodoassist:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "nodoassist.json"),
  ];
