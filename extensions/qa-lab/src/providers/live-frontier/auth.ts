...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "NODOASSIST_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "NODOASSIST_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV =
  "NODOASSIST_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["NODOASSIST_LIVE_ANTHROPIC_KEY"],
  gemini: ["NODOASSIST_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "NODOASSIST_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: NodoAssistConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: NodoAssistConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: NodoAssistConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.NODOASSIST_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "nodoassist") {
    return false;
  }
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or NODOASSIST_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host NodoAssist OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
