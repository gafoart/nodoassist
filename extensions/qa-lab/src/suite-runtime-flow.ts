...
import { formatMemoryDreamingDay } from "nodoassist/plugin-sdk/memory-core-host-status";
import { resolveSessionTranscriptsDirForAgent } from "nodoassist/plugin-sdk/memory-host-core";
...
function setActiveMemorySessionDisabled(
  ...
  const store = createPluginStateSyncKeyedStore<{
    ...
    env: {
      ...process.env,
      NODOASSIST_STATE_DIR: path.join(env.gateway.tempRoot, "state"),
    },
  });
