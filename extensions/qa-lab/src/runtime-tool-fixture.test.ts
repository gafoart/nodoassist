...
describe("runtime tool fixture", () => {
  ...
  it("does not fail Codex-native fixtures solely because NodoAssist dynamic exposure is absent", async () => {
    const env = await makeEnv({
      mock: { baseUrl: "http://127.0.0.1:9999" },
      gateway: {
        baseUrl: "http://127.0.0.1:1",
        tempRoot: "",
        workspaceDir: "",
        runtimeEnv: { NODOASSIST_QA_FORCE_RUNTIME: "codex" },
        call: vi.fn(),
      },
    });
    ...
    expect(details).toContain("NodoAssist dynamic exposure is intentionally omitted");
  ...
  it.each([
    ...
    },
    {
      ...
      toolName: "web_search",
      happyArgs: { query: "NodoAssist runtime parity fixed query" },
      happyOutput: "result",
  ...
  it("still fails required NodoAssist dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
