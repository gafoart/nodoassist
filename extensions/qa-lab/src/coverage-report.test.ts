...
describe("qa coverage report", () => {
  ...
  it("renders Playwright matches as qa suite targets", () => {
    ...
    expect(report).toContain(
      "- Suite command: `pnpm nodoassist qa suite --scenario control-ui-chat-flow-playwright`",
    );
  ...
  it("includes required channel driver flags in scenario match commands", () => {
    ...
    expect(report).toContain(
      "- Suite command: `pnpm nodoassist qa suite --channel-driver live --channel whatsapp --scenario whatsapp-access-control-group-disabled`",
    );
  });
  ...
  it("splits qa suite targets when matches mix execution kinds", () => {
    ...
    expect(report).toContain("  - flow: `pnpm nodoassist qa suite --scenario flow-proof`");
    expect(report).toContain(
      "  - playwright: `pnpm nodoassist qa suite --scenario playwright-proof`",
    );
  });
    ...
    const report = buildQaScorecardTaxonomyReport({
      ...
      scenarios: [
        scenarioWithCoverage({
          primary: [TEST_EXECUTABLE_COVERAGE_ID],
          executionKind: "playwright",
          executionPath: "../outside-nodoassist.test.ts",
        }),
      ],
    });
