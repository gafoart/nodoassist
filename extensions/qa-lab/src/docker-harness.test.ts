...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    for (const expectedFile of [
      ...
      path.join(outputDir, "state", "nodoassist.json"),
    ...
    expect(compose).toContain("image: nodoassist:qa-local-prebaked");
    ...
    expect(services["qa-mock-openai"]?.environment).not.toHaveProperty("NODOASSIST_CONFIG_PATH");
    expect(services["qa-mock-openai"]?.volumes).toBeUndefined();
    expect(services["qa-lab"]?.environment).toMatchObject({
      NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1",
      NODOASSIST_CONFIG_PATH: "/opt/nodoassist-scaffold/nodoassist.json",
      NODOASSIST_STATE_DIR: "/tmp/nodoassist/state",
    });
    expect(services["qa-lab"]?.volumes).toContain("./state:/opt/nodoassist-scaffold:ro");
    expect(compose).toContain('      - "127.0.0.1:18889:18789"');
    expect(compose).toContain('      - "127.0.0.1:43124:43123"');
    expect(compose).toContain(":/opt/nodoassist-qa-lab-ui:ro");
    ...
    expect(compose).toContain("--ui-dist-dir /opt/nodoassist-qa-lab-ui");
    expect(compose).toContain(":/opt/nodoassist-repo:ro");
    expect(compose).toContain("./state:/opt/nodoassist-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/nodoassist-scaffold/seed-workspace/. /tmp/nodoassist/workspace/ && rm -rf /tmp/nodoassist/workspace/repo && ln -s /opt/nodoassist-repo /tmp/nodoassist/workspace/repo",
    );
    expect(compose).toContain("NODOASSIST_CONFIG_PATH: /tmp/nodoassist/nodoassist.json");
    expect(compose).toContain("NODOASSIST_STATE_DIR: /tmp/nodoassist/state");
    expect(compose).toContain('NODOASSIST_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("NODOASSIST_GATEWAY_TOKEN=qa-token");
    ...
    const configText = await readFile(path.join(outputDir, "state", "nodoassist.json"), "utf8");
    ...
    expect(configText).toContain('"nodoAssistToolsMcpBridge": true');
    expect(configText).toContain("/app/dist/control-ui");
    expect(configText).toContain("C-3PO QA");
    expect(configText).toContain('"/tmp/nodoassist/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    expect(result.imageName).toBe("nodoassist:qa-local-prebaked");
    expect(calls).toEqual([
      "docker build -t nodoassist:qa-local-prebaked --build-arg NODOASSIST_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/nodoassist",
    ]);
  });
  ...
  it("quotes generated compose paths so shell-sensitive repo paths survive YAML parsing", async () => {
    ...
    expect(services["qa-lab"]?.volumes).toContain(
      "../repo #hash/extensions/qa-lab/web/dist:/opt/nodoassist-qa-lab-ui:ro",
    );
    expect(services["nodoassist-qa-gateway"]?.volumes).toContain(
      "../repo #hash:/opt/nodoassist-repo:ro",
    );
  });
});
