...
export type QaToolCoverageRow = {
  ...
  scenarios: string[];
  sourcePaths: string[];
  nodoassist: QaToolCoverageStatus;
...
function countRuntimeToolCalls(
  ...
  const cell = runtime === "nodoassist" ? result.cells.nodoassist : result.cells.codex;
...
function buildRow(params: {
  ...
  return {
    ...
    nodoassist: result ? cellStatus(result.cells.nodoassist) : "not-run",
    codex: result ? cellStatus(result.cells.codex) : "not-run",
    drift: result?.drift ?? "not-run",
    nodoassistToolCalls: countRuntimeToolCalls(result, "nodoassist", runtimeToolName),
...
function coverageFailureForRow(row: QaToolCoverageRow): string | undefined {
  ...
  if (row.nodoassist !== "pass" || row.codex !== "pass") {
    return `${row.tool} status nodoassist=${row.nodoassist} codex=${row.codex}`;
  }
  if (row.drift === "failure-mode") {
    return `${row.tool} drift=failure-mode${row.details ? ` (${row.details})` : ""}`;
  }
  if (row.runtimeToolName && row.nodoassistToolCalls === 0) {
    return `${row.tool} missing nodoassist tool call ${row.runtimeToolName}`;
  }
...
export function buildQaToolCoverageReport(params: {
  ...
  return {
    ...
    dynamicIntegrationTools: rows.filter((row) => row.bucket === "nodoassist-dynamic-integration")
      .length,
    searchableDynamicTools: rows.filter(
      (row) => row.capabilityLayer === "nodoassist-dynamic-searchable",
    ).length,
...
export function renderQaToolCoverageMarkdownReport(report: QaToolCoverageReport): string {
  const lines = [
    `# NodoAssist Runtime Tool Coverage — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
    ...
    `- NodoAssist dynamic integration tools: ${report.dynamicIntegrationTools}`,
    ...
    "",
    "| Tool | Bucket | Expected layer | Capability layer | Required | Fixtures | NodoAssist | Codex | Drift | Codex default impact | QA impact | Action | Tracking |",
    "| --- | --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- |",
  ];
