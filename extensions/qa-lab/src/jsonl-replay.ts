...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { nodoassist: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { nodoassist: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      nodoassist: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.nodoassist.push(parity.cells.nodoassist);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# NodoAssist JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
