...
function createSelection(
  channel: NodoAssistCrablineChannelDriverSelection["channel"] = "telegram",
) {
  return {
...
describe("crabline transport", () => {
  it("configures NodoAssist's Telegram plugin against a Crabline local provider server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      ...
      try {
        ...
        const manifest = JSON.parse(
          await fs.readFile(path.join(outputDir, NODOASSIST_CRABLINE_MANIFEST_PATH), "utf8"),
        ) as {
          provider?: string;
        };
  ...
  it("configures NodoAssist's Slack plugin against a Crabline local provider server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
  ...
  it("configures NodoAssist's WhatsApp plugin against a Crabline Baileys WebSocket server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
      ...
      try {
        ...
        expect(env).toMatchObject({
          ...
          NODOASSIST_WHATSAPP_WEB_SOCKET_URL: expect.stringMatching(
    ...
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      ...
      try {
        ...
        expect(transport.createRuntimeEnvPatch?.()).toMatchObject({
          ...
          MATRIX_USER_ID: "@nodoassist:matrix.test",
        });
