...
import { resolveChannelConfigWrites } from "nodoassist/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "nodoassist/plugin-sdk/routing";
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: NodoAssistConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: NodoAssistConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as NodoAssistConfig;
  ...
  let skippedCfg: NodoAssistConfig | undefined;
      ...
      mutate: async (draft) => {
        ...
        if (!agentExists) {
          const workspaceTemplate =
            dynamicCfg.workspaceTemplate ?? "~/.nodoassist/workspace-{agentId}";
          const agentDirTemplate =
            dynamicCfg.agentDirTemplate ?? "~/.nodoassist/agents/{agentId}/agent";
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as NodoAssistConfig,
