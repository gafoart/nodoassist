...
function collectConfiguredAgentIds(cfg: NodoAssistConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: NodoAssistConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "nodoassist doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: NodoAssistConfig): boolean {
