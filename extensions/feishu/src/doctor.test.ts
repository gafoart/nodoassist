...
function captureEnv(): EnvSnapshot {
  return {
    ...
    NODOASSIST_HOME: process.env.NODOASSIST_HOME,
...
function feishuConfig(): NodoAssistConfig {
  return {
    channels: {
      feishu: {
        appId: "cli_xxx",
...
function stateDir(): string {
  const dir = process.env.NODOASSIST_STATE_DIR;
  if (!dir) {
...
describe("Feishu doctor state repair", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-feishu-doctor-"));
    process.env.HOME = tempHome;
    process.env.NODOASSIST_HOME = tempHome;
    process.env.NODOASSIST_STATE_DIR = path.join(tempHome, ".nodoassist");
    fs.mkdirSync(process.env.NODOASSIST_STATE_DIR, { recursive: true, mode: 0o700 });
  });
  ...
  it("warns before repair when Feishu local state is corrupt", async () => {
    ...
    expect(result.warningNotes.join("\n")).toContain("nodoassist doctor --fix");
  });
