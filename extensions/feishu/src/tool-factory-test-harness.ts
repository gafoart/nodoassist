...
export function createToolFactoryHarness(cfg: NodoAssistPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<NodoAssistPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
  ...
  return {
    api: api as NodoAssistPluginApi,
    resolveTool,
    registered,
  };
}
