...
beforeEach(async () => {
  tempRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nodoassist-feishu-agent-"));
});
...
function createRuntime(
  currentCfg?: NodoAssistConfig,
  persistedCfg?: NodoAssistConfig,
  mutationCfg?: NodoAssistConfig,
) {
  let runtimeCfg = structuredClone(currentCfg ?? ({} as NodoAssistConfig));
  const commitConfig = vi.fn();
  const mutateConfigFile = vi.fn(
    async (params: {
      mutate: (
        draft: NodoAssistConfig,
...
describe("maybeCreateDynamicAgent", () => {
  ...
  it("rechecks current ingress inside the config mutation lock", async () => {
    ...
    const canCreateForConfig = vi
      .fn<(cfg: NodoAssistConfig) => Promise<boolean>>()
  ...
  it("preserves concurrent runtime config when creating from a stale request snapshot", async () => {
    ...
    const result = await maybeCreateDynamicAgent({
      cfg: { agents: { list: [] }, bindings: [] } as NodoAssistConfig,
      runtime,
      accountId: "default",
      senderOpenId: "ou_sender",
      canCreateForConfig: async () => true,
