...
type Registered = {
  ...
  tools: unknown[];
  service?: Parameters<NodoAssistPluginApi["registerService"]>[0];
};
type MockCallSource = {
  mock: {
...
describe("voice-call plugin", () => {
  ...
  afterEach(() => {
    ...
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("nodoassist.voice-call.runtime")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("nodoassist.voice-call.runtimePromise")
    ];
    delete (globalThis as Record<PropertyKey, unknown>)[
      Symbol.for("nodoassist.voice-call.runtimeStopPromise")
    ];
  });
  ...
  it("still starts the webhook runtime for gateway CLI processes", async () => {
    ...
    vi.stubEnv("NODOASSIST_CLI", "1");
    process.argv = ["node", "nodoassist", "gateway", "run"];
    const { service } = setup({ provider: "mock" });
  ...
  it("normalizes legacy config through runtime creation and warns to run doctor", async () => {
    ...
    expectWarningIncludes('Run "nodoassist doctor --fix"');
  });
  ...
  it("CLI smoke can place a live notify call with --yes", async () => {
    ...
    try {
      ...
      expect(runtimeStub.manager["initiateCall"]).toHaveBeenCalledWith("+15550009999", undefined, {
        message: "NodoAssist voice call smoke test.",
        mode: "notify",
      });
