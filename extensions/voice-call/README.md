...
Official Voice Call plugin for **NodoAssist**.
...
```bash
PLUGIN_HOME=~/.nodoassist/extensions
mkdir -p "$PLUGIN_HOME"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `nodoassist doctor --fix` to rewrite them.
...
- `realtime.consultThinkingLevel` is optional. When set, it overrides the thinking level used by the model behind realtime `nodoassist_agent_consult` calls.
- `realtime.consultFastMode` is optional. When set, it toggles fast mode for realtime `nodoassist_agent_consult` calls.
