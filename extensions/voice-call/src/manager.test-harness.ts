...
import { createPluginStateSyncKeyedStoreForTests } from "nodoassist/plugin-sdk/plugin-state-test-runtime";
...
export function createTestStorePath(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-voice-call-test-"));
}
