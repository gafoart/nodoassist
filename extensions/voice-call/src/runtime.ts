...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured NodoAssist agent receiving delegated requests from a live phone voice bridge.",
...
function resolveVoiceCallConsultSessionKey(call: {
  config: VoiceCallConfig;
  coreSession?: NodoAssistConfig["session"];
  sessionKey?: string;
  from?: string;
  to?: string;
...
function listRealtimeAgentIds(config: VoiceCallConfig, coreConfig: NodoAssistConfig): string[] {
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as NodoAssistConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as NodoAssistConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {
