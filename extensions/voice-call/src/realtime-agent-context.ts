...
import { buildRealtimeVoiceAgentConsultPolicyInstructions } from "nodoassist/plugin-sdk/realtime-voice";
import { root } from "nodoassist/plugin-sdk/security-runtime";
import { normalizeOptionalString as normalizeString } from "nodoassist/plugin-sdk/string-coerce-runtime";
...
export async function buildRealtimeVoiceInstructions(params: {
  ...
  const capsule: string[] = [
    "NodoAssist agent voice context:",
    `- Agent id: ${agentId}`,
    "- Use this context to match the NodoAssist agent's personality and standing preferences on fast voice turns.",
    "- Treat this as compact context only; call nodoassist_agent_consult when the caller needs the full agent brain, tools, memory, or workspace state.",
  ];
