...
import { registerApiProvider, streamSimple } from "nodoassist/plugin-sdk/llm";
...
import { streamWithPayloadPatch } from "nodoassist/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When NodoAssist's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if NodoAssist identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher NodoAssist uses for direct model IDs.
 */
 ...
 * the NodoAssist config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: NodoAssistPluginApi): void {
