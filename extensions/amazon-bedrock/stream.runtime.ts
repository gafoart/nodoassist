/**
 * Amazon Bedrock Converse streaming runtime. It maps NodoAssist messages/tools,
...
// NodoAssist synthesizes these caps when the provider's real output limit is unknown.
// Keep them out of Bedrock adaptive requests so Bedrock can use its native default.
const NODOASSIST_FALLBACK_MODEL_MAX_TOKENS = new Set([4096, 8192, 16_384]);
...
function resolveAdaptiveBedrockMaxTokens(
  ...
  return NODOASSIST_FALLBACK_MODEL_MAX_TOKENS.has(model.maxTokens) ? undefined : model.maxTokens;
}
...
/** Stream a Bedrock Converse request from the generic NodoAssist stream options. */
 ...
 * Defaults to "short" and uses NODOASSIST_CACHE_RETENTION for backward compatibility.
 */
