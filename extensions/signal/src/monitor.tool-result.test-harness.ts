...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/nodoassist-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("nodoassist/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/security-runtime")>(
    "nodoassist/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      ...
      import("nodoassist/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
