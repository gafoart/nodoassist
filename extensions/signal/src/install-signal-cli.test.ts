...
vi.mock("nodoassist/plugin-sdk/setup-tools", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nodoassist/plugin-sdk/setup-tools")>();
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/run-command", () => ({
...
async function withTempFile(run: (filePath: string) => Promise<void>) {
  const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-signal-download-"));
  try {
...
describe("installSignalCli", () => {
  ...
  it("uses Homebrew on macOS instead of downloading the first GitHub release archive", async () => {
    ...
    const brewPrefix = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-signal-brew-"));
...
describe("extractSignalCliArchive", () => {
  async function withArchiveWorkspace(run: (workDir: string) => Promise<void>) {
    const workDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-signal-install-"));
    try {
      await run(workDir);
    } finally {
      await fs.rm(workDir, { recursive: true, force: true }).catch(() => undefined);
