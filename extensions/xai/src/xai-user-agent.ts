// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatNodoAssistUserAgent`.

import { NODOASSIST_VERSION as PACKAGE_VERSION } from "nodoassist/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "nodoassist";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.NODOASSIST_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.NODOASSIST_SERVICE_VERSION) ??
...
// an empty record so the nodoassist identity is not forwarded to the proxy.
