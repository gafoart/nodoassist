...
function readLegacyGrokFallbackAuth(cfg?: NodoAssistConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: NodoAssistConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function resolveFallbackXaiAuth(cfg?: NodoAssistConfig): XaiFallbackAuth | undefined {
