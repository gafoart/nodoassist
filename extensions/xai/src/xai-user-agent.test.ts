...
describe("xaiUserAgent", () => {
  ...
  it("prefers NODOASSIST_VERSION env over the bundled package version", () => {
    vi.stubEnv("NODOASSIST_VERSION", "2026.3.22");
    expect(xaiUserAgent()).toBe("nodoassist/2026.3.22");
  });

  it("falls back to NODOASSIST_SERVICE_VERSION when NODOASSIST_VERSION is unset", () => {
    vi.stubEnv("NODOASSIST_VERSION", "");
    vi.stubEnv("NODOASSIST_SERVICE_VERSION", "2026.3.99");
    // NODOASSIST_VERSION from the SDK is the bundled VERSION constant. In a dev
    ...
    expect(result.startsWith("nodoassist/")).toBe(true);
    expect(result).not.toBe("nodoassist/");
  });

  it("returns the nodoassist/<version> shape", () => {
    vi.stubEnv("NODOASSIST_VERSION", "2026.5.16");
    expect(xaiUserAgent()).toMatch(/^nodoassist\/\d+\.\d+\.\d+$/u);
  });
});
