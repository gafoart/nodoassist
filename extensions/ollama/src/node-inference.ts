...
import { jsonResult } from "nodoassist/plugin-sdk/channel-actions";
import {
...
type NodeSummary = Awaited<
  ReturnType<NodoAssistPluginApi["runtime"]["nodes"]["list"]>
>["nodes"][number];
...
export function createOllamaNodeInvokePolicy(): NodoAssistPluginNodeInvokePolicy {
  return {
...
export function createOllamaNodeInferenceTool(api: NodoAssistPluginApi): AnyAgentTool {
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (action === "discover") {
        ...
        return jsonResult({
          nodes,
          ...(modelNodes.length === 0 && {
            hint: "No connected node advertises Ollama inference. Start Ollama and `nodoassist node run` on the target machine, then approve any request shown by `nodoassist nodes pending`.",
          }),
        });
      }
