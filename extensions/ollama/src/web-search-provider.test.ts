...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): NodoAssistConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): NodoAssistConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    expect(result.query).toBe("nodoassist docs");
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest nodoassist release" })).rejects.toThrow(
      "ollama signin",
    );
  });
