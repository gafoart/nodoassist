...
`nodoassist.invoke --each`) without adding new NodoAssist code per workflow.
...
This extension depends on NodoAssist internal modules (the embedded agent runner).
It is intended to ship as a **bundled** NodoAssist extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.nodoassist/extensions` as a standalone plugin directory.
