...
import { createAccountListHelpers } from "nodoassist/plugin-sdk/account-helpers";
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "nodoassist/plugin-sdk/account-id";
import { resolveMergedAccountConfig } from "nodoassist/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "nodoassist";
  const botDisplayName = merged.botDisplayName?.trim() || "NodoAssist QA";
  return {
    accountId,
    enabled,
