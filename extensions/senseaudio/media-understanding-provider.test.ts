...
import { runFfmpeg } from "nodoassist/plugin-sdk/media-runtime";
import {
...
describe("transcribeSenseAudioAudio", () => {
  ...
  it("can transcribe generated speech in live mode", async () => {
    if (process.env.NODOASSIST_LIVE_TEST !== "1" || !process.env.SENSEAUDIO_API_KEY) {
      return;
    }
    ...
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "nodoassist-senseaudio-live-"));
    try {
      const aiffPath = path.join(tempDir, "speech.aiff");
      const mp3Path = path.join(tempDir, "speech.mp3");
      const sayResult = spawnSync("say", ["-o", aiffPath, "nodo assist live transcription test"], {
        encoding: "utf8",
      });
