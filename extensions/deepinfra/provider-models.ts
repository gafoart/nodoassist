...
import { asPositiveSafeInteger } from "nodoassist/plugin-sdk/string-coerce-runtime";
...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=nodoassist&filter=with_meta`;
...
// Static fallback. Chat rows live in nodoassist.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=nodoassist&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `NODOASSIST_LIVE_*`
// indirection too).
