...
describe("DEEPINFRA_MODELS_URL", () => {
  it("points at /v1/openai/models with the nodoassist sort + filter=with_meta gate", () => {
...
describe("hasDeepInfraApiKey", () => {
  ...
  it("falls back to the auth-profile store when no env var is set", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/nodoassist-agent" })).toBe(true);
  ...
  it("returns false when env is empty and the auth-profile store has no deepinfra profile", () => {
    ...
    expect(hasDeepInfraApiKey({ env: {}, agentDir: "/tmp/nodoassist-agent" })).toBe(false);
...
describe("discoverDeepInfraModels (chat-only shim)", () => {
  ...
  it("fetches the nodoassist-projection endpoint and parses chat-surface entries when an API key is configured", async () => {
