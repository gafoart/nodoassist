 ...
 * `__NODOASSIST_REDACTED__` literal anywhere in the output throws
 ...
 *
 * @module @nodoassist/oc-path/emit
 */
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__NODOASSIST_REDACTED__")) {
      guardSentinel("__NODOASSIST_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
