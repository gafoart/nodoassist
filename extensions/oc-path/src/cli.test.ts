/**
 * Smoke tests for the `nodoassist path` CLI handlers.
 *
...
describe("nodoassist path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "nodoassist.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://nodoassist.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://nodoassist.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
