...
import { createMockIncomingRequest } from "nodoassist/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "nodoassist/plugin-sdk/webhook-request-guards";
...
vi.mock("nodoassist/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/webhook-ingress")>(
    "nodoassist/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("nodoassist/plugin-sdk/reply-runtime");
    vi.doUnmock("nodoassist/plugin-sdk/runtime-env");
    vi.doUnmock("nodoassist/plugin-sdk/webhook-ingress");
