...
import { loadAuthProfileStoreWithoutExternalProfiles } from "nodoassist/plugin-sdk/agent-runtime";
import { MIGRATION_REASON_TARGET_EXISTS } from "nodoassist/plugin-sdk/migration";
...
describe("Hermes migration file and skill items", () => {
  ...
  it("archives unsupported Hermes state without copying raw auth credentials", async () => {
    ...
    expect(plan.warnings).toEqual([
      "Some Hermes files are archive-only. They will be copied into the migration report for manual review, not loaded into NodoAssist.",
    ]);
