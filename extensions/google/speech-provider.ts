...
function resolveGoogleTtsModelProviderApiKey(cfg?: NodoAssistConfig): string | undefined {
...
function isNodoAssistGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isNodoAssistGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
