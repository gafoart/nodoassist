...
describe("discord doctor", () => {
  ...
  it("removes unsupported Discord realtime wake names", () => {
      ...
      cfg: {
        channels: {
          discord: {
            voice: {
              realtime: {
                wakeNames: ["Claw", "Claw Bot Helper", "Nodo Assist"],
              },
            },
            accounts: {
              work: {
    ...
    expect(result.changes).toEqual([
      ...
      "Removed empty channels.discord.accounts.empty.voice.realtime.wakeNames; unset wake names use the default agent/NodoAssist fallback.",
  ...
  it("repairs safe numeric ids into strings and warns for unsafe lists", () => {
    ...
    const result = maybeRepairDiscordNumericIds(cfg, "nodoassist doctor --fix");
  ...
  it("formats repair guidance for unsafe numeric ids", () => {
    ...
    expect(warnings[1]).toContain("nodoassist doctor --fix");
  });
