...
import { loadWebMediaRaw } from "nodoassist/plugin-sdk/web-media";
import { afterAll, afterEach, beforeAll, beforeEach, describe, expect, it, vi } from "vitest";
import { RateLimitError } from "./internal/discord.js";
import { makeDiscordRest } from "./send.test-harness.js";

vi.mock("nodoassist/plugin-sdk/web-media", async () => {
...
afterAll(() => {
  vi.doUnmock("nodoassist/plugin-sdk/web-media");
});
...
describe("uploadStickerDiscord", () => {
  ...
  it("uploads sticker assets", async () => {
    ...
    await uploadStickerDiscord(
      {
        guildId: "g1",
        name: "nodoassist_wave",
        description: "NodoAssist waving",
        tags: "👋",
    ...
    expect(stickerBody.name).toBe("nodoassist_wave");
    expect(stickerBody.description).toBe("NodoAssist waving");
