...
import type { RealtimeVoiceAgentControlResult } from "nodoassist/plugin-sdk/realtime-voice";
  ...
  return {
    ...
    controlRealtimeVoiceAgentRunMock: vi.fn<() => Promise<RealtimeVoiceAgentControlResult>>(
      async () => ({
        ...
        message: "There is no active NodoAssist run to steer.",
        speak: true,
        show: true,
        suppress: false,
      }),
...
vi.mock("nodoassist/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/routing")>(
    "nodoassist/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/agent-runtime")>(
    "nodoassist/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
    agentCommandFromIngress: agentCommandMock,
    getTtsProvider: vi.fn(() => "openai"),
    resolveAgentDir: vi.fn(() => "/tmp/nodoassist-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/nodoassist-tts.json"),
  };
});

vi.mock("nodoassist/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("nodoassist/plugin-sdk/realtime-bootstrap-context")
  >("nodoassist/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/realtime-voice")>(
    "nodoassist/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("destroys stale tracked voice connections before joining", async () => {
    ...
    expect(getVoiceConnectionMock).toHaveBeenCalledWith("g1", "nodoassist:default");
  ...
  it("isolates voice connections by Discord account", async () => {
    ...
    expect(getVoiceConnectionMock).toHaveBeenNthCalledWith(1, "g1", "nodoassist:first");
    expect(getVoiceConnectionMock).toHaveBeenNthCalledWith(2, "g1", "nodoassist:second");
    expect(joinVoiceChannelMock).toHaveBeenNthCalledWith(
      1,
      expect.objectContaining({ group: "nodoassist:first" }),
    );
    expect(joinVoiceChannelMock).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ group: "nodoassist:second" }),
    );
  });
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same NodoAssist agent");
    expect(bridgeParams?.instructions).toContain("short natural backchannel");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("nodoassist_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("nodoassist_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
    ...
    bridgeParams?.onToolCall?.(
      {
        ...
        args: {
          question: [
            "Speak this exact NodoAssist answer to the Discord voice channel, without adding, removing, or rephrasing words.",
  ...
  it("routes active-run realtime transcripts to voice control before forced consults", async () => {
    ...
    await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active NodoAssist run."));
    expect(textToSpeechMock).not.toHaveBeenCalledWith(
      expect.objectContaining({ text: "Cancelled the active NodoAssist run." }),
    );

    const stopCallsAfterControl = player.stop.mock.calls.length;
    bridgeParams?.onTranscript?.("assistant", "Cancelled the active NodoAssist run.", true);
  ...
  it("accepts NodoAssist as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "nodoassist wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    ownerTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "NodoAssist, how is it going");
    ...
    expect(lastAgentCommandArgs().message).toContain("how is it going");
    expect(lastAgentCommandArgs().message).not.toContain("NodoAssist");
    expectUserMessageIncludes("nodoassist wake answer");
  });
  ...
  it("ignores default agent wake names longer than two words", async () => {
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "NodoAssist, fallback still wakes");
  ...
  it("accepts leading fuzzy wake names before realtime agent-proxy consults", async () => {
    ...
    const nodoAssistTurn = entry.realtime?.beginSpeakerTurn(
      { extraSystemPrompt: undefined, senderIsOwner: true, speakerLabel: "Owner" },
      "u-owner",
    );
    nodoAssistTurn?.sendInputAudio(Buffer.alloc(8));
  ...
  it("does not accept configured realtime wake names longer than two words", async () => {
    ...
    fallbackTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "NodoAssist, ship it");
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("nodoassist_agent_consult");
  ...
  it("reuses forced agent-proxy answers for late matching consult tool calls", async () => {
    ...
    expect(realtimeSessionMock.submitToolResult).toHaveBeenCalledWith(
      "call-late",
      {
        ...
        message: "NodoAssist already delivered this answer to Discord voice.",
      },
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call nodoassist_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("nodoassist_agent_consult");
  ...
  it("adds default bootstrap profile context to realtime voice instructions", async () => {
    ...
    resolveRealtimeBootstrapContextInstructionsMock.mockResolvedValue(
      "NodoAssist realtime voice profile context:\n\n### IDENTITY.md\nName: Wilfred",
    );
    const manager = createManager({
      groupPolicy: "open",
      voice: {
    ...
    const bridgeParams = lastRealtimeBridgeParams() as
      ...
      | undefined;
    expect(bridgeParams?.instructions).toContain("NodoAssist realtime voice profile context");
