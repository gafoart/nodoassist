...
import { createUnionActionGate } from "nodoassist/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "nodoassist/plugin-sdk/channel-contract";
import type { DiscordActionConfig, NodoAssistConfig } from "nodoassist/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: NodoAssistConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: NodoAssistConfig) {
