...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: NodoAssistConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): NodoAssistConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("nodoassist/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/acp-runtime")>(
    "nodoassist/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("nodoassist/plugin-sdk/native-command-config-runtime")
  >("nodoassist/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/error-runtime")>(
    "nodoassist/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
