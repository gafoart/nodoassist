...
import { sleepWithAbort } from "nodoassist/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "nodoassist/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupAccountIds(cfg: NodoAssistConfig): string[] {
...
function resolveDiscordStartupDelayMs(cfg: NodoAssistConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). NodoAssist adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
      ...
      status: createComputedAccountStatusAdapter<ResolvedDiscordAccount, DiscordProbe>({
        ...
        buildCapabilitiesDiagnostics: async ({ account, target, timeoutMs }) => {
          ...
          const statusCfg: NodoAssistConfig = {
            channels: {
              discord: {
                accounts: {
                  [account.accountId]: {
