...
vi.mock("nodoassist/plugin-sdk/channel-outbound", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/channel-outbound")>(
    "nodoassist/plugin-sdk/channel-outbound",
  );
  return {
    ...actual,
...
describe("deliverDiscordReply", () => {
  ...
  it("formats reasoning replies as visible Discord payloads before shared outbound", async () => {
    ...
    expect(firstDeliverParams().payloads).toEqual([{ text: ".nodoassist\n\n_Because it helps_" }]);
  });
      ...
      replies: [
        {
          text: [
            "[tool:exec]",
            "<parameter=command>",
            'cat /proc/mounts 2>/dev/null | grep -i "libra|rav|nodoassist" | head -20',
            "</parameter>",
            "",
            "<function=exec>",
            "<parameter=command>",
  ...
  it("passes media roots and explicit off-mode payload reply tags to shared outbound", async () => {
    ...
    await deliverDiscordReply({
      ...
      textLimit: 2000,
      replyToMode: "off",
      mediaLocalRoots: ["/tmp/nodoassist-media"],
      kind: "final",
    });
    ...
    expect(params.mediaAccess).toEqual({ localRoots: ["/tmp/nodoassist-media"] });
  });
