...
vi.mock("nodoassist/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/media-runtime")>(
    "nodoassist/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
beforeEach(() => {
  ...
  saveRemoteMediaMock.mockImplementation(
    async (options: { fallbackContentType?: string; filePathHint?: string }) => ({
      id: "test-media",
      path: `/tmp/nodoassist-discord-test/${options.filePathHint ?? "media"}`,
      size: 5,
...
function createThreadBinding(
  overrides?: Partial<import("nodoassist/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("nodoassist/plugin-sdk/conversation-runtime").SessionBindingRecord;
}
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("nodoassist/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("preflights direct-message voice notes without mention gating", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hello nodoassist from dm audio");
    ...
    expect(preflight.preflightAudioTranscript).toBe("hello nodoassist from dm audio");
  });
  ...
  it("downloads attachments during preflight, before the message reaches the run queue", async () => {
    ...
    expect(preflight.preparedMedia).toEqual([
      {
        path: "/tmp/nodoassist-discord-test/photo.png",
  ...
  it("suppresses repeated bot messages before downloading attachments (#58789)", async () => {
    ...
    const message = createDiscordMessage({
      id: "m-loop-1",
      channelId,
      content: "chatter <@nodoassist-bot>",
      mentionedUsers: [{ id: "nodoassist-bot" }],
    ...
    const repeatedMessage = createDiscordMessage({
      id: "m-loop-2",
      channelId,
      content: "more chatter <@nodoassist-bot>",
      mentionedUsers: [{ id: "nodoassist-bot" }],
      attachments: [
        {
          id: "att-loop",
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("nodoassist/plugin-sdk/config-contracts").NodoAssistConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("hydrates mention metadata from REST when bot mention syntax is present but mentions are missing", async () => {
    ...
    client.rest = {
      get: vi.fn(async () => ({
        ...
        mentions: [{ id: botId, username: "NodoAssist", bot: true }],
        mention_roles: [],
        mention_everyone: false,
      })),
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@nodoassist-bot> /new incident room",
      mentionedUsers: [{ id: "nodoassist-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("records sticker image media for skipped mention-gated guild history", async () => {
    ...
    saveRemoteMediaMock.mockResolvedValueOnce({
      id: "test-sticker",
      path: "/tmp/nodoassist-discord-test/sticker.png",
      size: 5,
      contentType: "image/png",
    });
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey nodoassist");
    ...
    const preflight = expectPreflightResult(result);
    expect(preflight.wasMentioned).toBe(true);
    expect(preflight.preflightAudioTranscript).toBe("hey nodoassist");
  });
