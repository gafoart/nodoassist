...
vi.mock("nodoassist/plugin-sdk/proxy-capture", () => ({
  ...
  resolveEffectiveDebugProxyUrl: (configuredProxyUrl?: string) =>
    configuredProxyUrl?.trim() || process.env.NODOASSIST_DEBUG_PROXY_URL,
...
describe("createDiscordGatewayPlugin", () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv("NODOASSIST_DEBUG_PROXY_ENABLED", "");
    vi.stubEnv("NODOASSIST_DEBUG_PROXY_URL", "");
  ...
  it("uses env gateway metadata timeout when config is unset", async () => {
    vi.useFakeTimers();
    vi.stubEnv("NODOASSIST_DISCORD_GATEWAY_INFO_TIMEOUT_MS", "6000");
    const runtime = createRuntime();
    globalFetchMock.mockImplementation(() => new Promise(() => {}));
    const plugin = createDiscordGatewayPlugin({
      discordConfig: {},
