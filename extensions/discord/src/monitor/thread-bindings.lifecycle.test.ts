...
const acpRuntime = await import("nodoassist/plugin-sdk/acp-runtime");
...
describe("thread binding lifecycle", () => {
  ...
  it("persists touched activity timestamps across restart when persistence is enabled", async () => {
    ...
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-thread-bindings-"));
  ...
  it("binds current Discord DMs as direct conversation bindings", async () => {
    ...
    const bound = await getSessionBindingService().bind({
      targetSessionKey: "plugin-binding:nodoassist-codex-app-server:dm",
      targetKind: "session",
      conversation: {
        channel: "discord",
        accountId: "default",
