...
vi.mock("nodoassist/plugin-sdk/fetch-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/fetch-runtime")>(
    "nodoassist/plugin-sdk/fetch-runtime",
  );
  return {
    ...actual,
...
describe("sendWebhookMessageDiscord proxy support", () => {
  ...
  it("accepts Discord's no-body webhook response when wait is false", async () => {
    ...
    const result = await sendWebhookMessageDiscord("hello", {
      cfg: { channels: { discord: { token: "Bot test-token" } } } as NodoAssistConfig,
      accountId: "default",
      webhookId: "123",
      webhookToken: "abc",
      wait: false,
