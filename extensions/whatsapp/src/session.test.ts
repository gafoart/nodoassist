...
import { resetLogger, setLoggerOverride } from "nodoassist/plugin-sdk/runtime-env";
...
const TEST_UNDICI_RUNTIME_DEPS_KEY = "__NODOASSIST_TEST_UNDICI_RUNTIME_DEPS__";
...
let NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV: typeof import("./session.js").NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV;
...
function createTempCaFile(contents: string): string {
  const dir = createTempAuthDir("nodoassist-wa-proxy-ca");
...
describe("web session", () => {
  beforeAll(async () => {
    ({
      ...
      formatError,
      logWebSelfId,
      NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV,
      waitForWaConnection,
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("nodoassist-wa-creds-test");
  ...
  it("prints compact terminal QR output when requested", async () => {
    const authDir = createTempAuthDir("nodoassist-wa-terminal-qr");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("nodoassist-wa-creds-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("nodoassist-wa-authdir-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("nodoassist-wa-auth-parent-symlink-runtime");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("nodoassist-wa-creds-symlink-save");
      const targetPath = path.join(authDir, "target-creds.json");
      const credsPath = path.join(authDir, "creds.json");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("nodoassist-wa-creds-parent-symlink-save");
      const targetBaseDir = path.join(rootDir, "target-base");
      const linkedBaseDir = path.join(rootDir, "linked-base");
      const authDir = path.join(linkedBaseDir, "default");
      fsSync.mkdirSync(targetBaseDir);
  ...
  it("uses NODOASSIST_WHATSAPP_WEB_SOCKET_URL as the default Baileys WebSocket URL", async () => {
    vi.stubEnv(NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV, " ws://127.0.0.1:49153/ws/chat ");
  ...
  it("ignores blank Baileys WebSocket URL environment overrides", async () => {
    vi.stubEnv(NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV, " ");
  ...
  it("rejects invalid NODOASSIST_WHATSAPP_WEB_SOCKET_URL values", async () => {
  ...
  it("preserves explicit Baileys WebSocket URL options over invalid environment", async () => {
    vi.stubEnv(NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV, "http://127.0.0.1:49153/ws/chat");
  ...
  it("adds managed proxy CA trust to WhatsApp env proxy agents", async () => {
    ...
    vi.stubEnv("NODOASSIST_PROXY_ACTIVE", "1");
    vi.stubEnv("NODOASSIST_PROXY_CA_FILE", caFile);
  ...
  it("logWebSelfId prints cached E.164 when creds exist", () => {
    const authDir = createTempAuthDir("nodoassist-wa-log-self");
    fsSync.writeFileSync(
  ...
  it("logWebSelfId prints cached lid details when creds include a lid", () => {
    const authDir = createTempAuthDir("nodoassist-wa-log-self-lid");
    fsSync.writeFileSync(
      path.join(authDir, "creds.json"),
      JSON.stringify({
        me: {
  ...
  it("does not clobber creds backup when creds.json is corrupted", async () => {
    const authDir = createTempAuthDir("nodoassist-wa-corrupt-backup");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("nodoassist-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("nodoassist-wa-a");
    const authDirB = createTempAuthDir("nodoassist-wa-b");
    const onError = vi.fn();
  ...
  it("rotates creds backup when creds.json is valid JSON", async () => {
    const authDir = createTempAuthDir("nodoassist-wa-rotate-backup");
    const credsPath = path.join(authDir, "creds.json");
    const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("nodoassist-wa-rotate-backup-symlink");
      const credsPath = path.join(authDir, "creds.json");
      const backupPath = path.join(authDir, "creds.json.bak");
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    try {
      await writeCredsJsonAtomically("/tmp/nodoassist-oauth/whatsapp/default", {
        me: { id: "123@s.whatsapp.net" },
      });

      const write = firstWriteFileCall(openMock.writeFileSpy);
      expect(write.path).toContain(
        path.join("/tmp", "nodoassist-oauth", "whatsapp", "default", ".creds."),
      );
      expect(typeof write.data).toBe("string");
      expect(write.options.mode).toBe(0o600);
      expect(write.options.flag).toBe("wx");
      ...
      expect(chmodSpy).toHaveBeenCalledWith(
        path.join("/tmp", "nodoassist-oauth", "whatsapp", "default", "creds.json"),
        0o600,
      );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("nodoassist-wa-creds-atomic");
    const credsPath = path.join(authDir, "creds.json");
