...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "nodoassist/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("nodoassist/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("nodoassist/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: NodoAssistConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("nodoassist/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `nodoassist-outbound-${crypto.randomUUID()}.log`);
