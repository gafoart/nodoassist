...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "nodoassist-test-web-logout-"));
    previousOAuthDir = process.env.NODOASSIST_OAUTH_DIR;
    process.env.NODOASSIST_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the NodoAssist auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the NodoAssist auth root", async () => {
