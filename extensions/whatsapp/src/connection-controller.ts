...
import { info } from "nodoassist/plugin-sdk/runtime-env";
...
const WHATSAPP_LOGGED_OUT_RELINK_MESSAGE =
  "WhatsApp reported the session is logged out. Cleared cached web session; please rerun nodoassist channels login and scan the QR again.";
...
export function closeWaSocket(
  ...
  try {
    if (typeof sock?.end === "function") {
      sock.end(new Error("NodoAssist WhatsApp socket close"));
      return;
    }
    sock?.ws?.close?.();
  } catch {
