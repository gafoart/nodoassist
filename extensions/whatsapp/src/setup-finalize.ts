...
type WhatsAppConfig = NonNullable<NonNullable<NodoAssistConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: NodoAssistConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: NodoAssistConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: NodoAssistConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: NodoAssistConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("nodoassist channels login"),
      }),
      "WhatsApp",
    );
  }
