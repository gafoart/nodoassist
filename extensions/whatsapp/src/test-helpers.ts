...
import { formatEnvelopeTimestamp } from "nodoassist/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("nodoassist:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("nodoassist:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[nodoassist]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/nodoassist-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("nodoassist:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("nodoassist:lastSocket")];
  if (typeof getter === "function") {
