...
import { createQueuedWizardPrompter } from "nodoassist/plugin-sdk/plugin-test-runtime";
...
vi.mock("nodoassist/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: NodoAssistConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<NodoAssistConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
...
async function runConfigureWithHarness(params: {
  ...
  const result = await finalizeWhatsAppSetup({
    cfg: params.cfg ?? ({} as NodoAssistConfig),
