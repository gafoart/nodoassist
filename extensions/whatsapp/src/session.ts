...
import { VERSION } from "nodoassist/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "nodoassist/plugin-sdk/fetch-runtime";
import { danger, success } from "nodoassist/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "nodoassist/plugin-sdk/runtime-env";
...
// Env var name is an external contract: @openclaw/crabline's whatsapp bridge
// emits OPENCLAW_WHATSAPP_WEB_SOCKET_URL for QA wiring; keep the legacy name.
export const NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV = "OPENCLAW_WHATSAPP_WEB_SOCKET_URL";
...
function resolveEnvWaWebSocketUrl(): string | undefined {
  const value = resolveWaWebSocketUrl(process.env[NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV]);
  if (!value) {
    return undefined;
  }
  let url: URL;
  try {
    ...
    throw new Error(`${NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV} must be a valid URL.`);
  }
  if (url.protocol !== "ws:" && url.protocol !== "wss:") {
    throw new Error(`${NODOASSIST_WHATSAPP_WEB_SOCKET_URL_ENV} must use ws:// or wss://.`);
  }
  return url.toString();
}
...
export async function createWaSocket(
  ...
  const sock = makeWASocket({
    ...
    browser: ["nodoassist", "cli", VERSION],
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("nodoassist channels login")}`,
              ),
            );
          }
        }
