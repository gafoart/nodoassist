...
vi.mock("nodoassist/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/channel-pairing")>(
...
vi.mock("nodoassist/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/media-store")>(
    "nodoassist/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("nodoassist/plugin-sdk/plugin-state-test-runtime")>(
      "nodoassist/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/nodoassist-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `nodoassist-inbound-media-${crypto.randomUUID()}`);
