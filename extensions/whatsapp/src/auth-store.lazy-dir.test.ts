...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NODOASSIST_STATE_DIR", "NODOASSIST_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-wa-profile-"));
  ...
  it("resolves the default web auth dir from NODOASSIST_STATE_DIR at call time", async () => {
