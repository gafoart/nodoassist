...
describe("collectWhatsAppStatusIssues", () => {
  beforeEach(() => {
    vi.stubEnv("NODOASSIST_CONTAINER_HINT", "");
    vi.stubEnv("NODOASSIST_PROFILE", "");
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: nodoassist channels login (scan QR on the gateway host).",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: nodoassist doctor (or restart the gateway). If it persists, relink via channels login and check logs.",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Watch: nodoassist logs --follow and run nodoassist channels status --probe if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.",
      },
    ]);
  });
