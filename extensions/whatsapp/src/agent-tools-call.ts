...
import { createActionGate, stringEnum } from "nodoassist/plugin-sdk/channel-actions";
import type {
  AnyAgentTool,
  NodoAssistPluginApi,
  NodoAssistPluginToolContext,
} from "nodoassist/plugin-sdk/core";
import { mulawToPcm } from "nodoassist/plugin-sdk/realtime-voice";
import { detectBinary } from "nodoassist/plugin-sdk/setup-tools";
import { resolveOAuthDir } from "nodoassist/plugin-sdk/state-paths";
...
async function resolveRequesterE164(params: {
  accountId: string;
  cfg: NonNullable<NodoAssistPluginToolContext["config"]>;
...
function resolveRuntimeConfig(api: NodoAssistPluginApi, context: NodoAssistPluginToolContext) {
    ...
    async execute(_toolCallId, rawParams, signal) {
      ...
      if (params.action === "status") {
        return jsonResult({
          ...
          note: "MeowCaller uses a separate WhatsApp linked-device session; it cannot reuse NodoAssist's Baileys credentials.",
        });
      }
      ...
      if (linkedSelf === target) {
        throw new Error(
          "WhatsApp cannot call the linked account itself; use a dedicated NodoAssist WhatsApp number",
        );
      }
      ...
      try {
        ...
        const tempDir = await fs.mkdtemp(
          path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-whatsapp-call-"),
        );
...
export function registerWhatsAppCallTool(api: NodoAssistPluginApi): void {
