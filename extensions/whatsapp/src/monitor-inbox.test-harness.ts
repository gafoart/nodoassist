...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/nodoassist-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/nodoassist-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("NodoAssist: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "nodoassist-auth-"));
    } else {
      authDir = undefined;
    }
  });
