...
vi.mock("nodoassist/plugin-sdk/logging-core", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/logging-core")>(
    "nodoassist/plugin-sdk/logging-core",
  );
  return {
    ...actual,
    getChildLogger: () => ({
...
describe("web monitor inbox", () => {
  ...
  it("detaches inbound listeners and ends the socket on close()", async () => {
    ...
    expect(closeError?.message).toBe("NodoAssist WhatsApp listener close");
