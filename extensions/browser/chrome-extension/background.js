// NodoAssist extension service worker.
//
// Thin transport between the NodoAssist extension relay (loopback WebSocket) and
...
// NodoAssist tab group in sync. Membership in that group is the user-visible
// consent boundary: only grouped tabs are reported to (and driven by) NodoAssist.
import {
  NODOASSIST_TAB_GROUP_TITLE,
...
async function findNodoAssistGroups() {
  try {
    return await chrome.tabGroups.query({ title: NODOASSIST_TAB_GROUP_TITLE });
  } catch {
    return [];
  }
}
...
async function listSharedTabs() {
  const groups = await findNodoAssistGroups();
  const tabs = [];
...
async function addTabToNodoAssistGroup(tabId) {
  ...
  await chrome.tabGroups.update(groupId, {
    title: NODOASSIST_TAB_GROUP_TITLE,
    color: groupColor,
  });
}

async function removeTabFromNodoAssistGroup(tabId) {
  try {
...
async function attachDebugger(tabId) {
  if (!(await isTabShared(tabId))) {
    throw new Error(`tab ${tabId} is not in the ${NODOASSIST_TAB_GROUP_TITLE} tab group`);
  }
...
chrome.debugger.onDetach.addListener((source, reason) => {
  ...
  if (reason === "canceled_by_user") {
    ...
    void removeTabFromNodoAssistGroup(source.tabId).then(scheduleTabsSync);
  }
});
  ...
  try {
    switch (msg.type) {
      ...
      case "createTab": {
        ...
        await addTabToNodoAssistGroup(tab.id);
        scheduleTabsSync();
    ...
    switch (msg?.type) {
      ...
      case "toggleShareTab": {
        ...
        if (await isTabShared(tabId)) {
          ...
          await removeTabFromNodoAssistGroup(tabId);
          scheduleTabsSync();
          sendResponse({ ok: true, shared: false });
        } else {
          await addTabToNodoAssistGroup(tabId);
          scheduleTabsSync();
...
chrome.alarms.create("nodoassist-relay-watchdog", { periodInMinutes: 0.5 });
chrome.alarms.onAlarm.addListener((alarm) => {
  if (alarm.name === "nodoassist-relay-watchdog") {
    void connectRelay();
  }
});
