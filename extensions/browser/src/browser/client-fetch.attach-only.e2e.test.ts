...
import { clearRuntimeConfigSnapshot } from "nodoassist/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("nodoassist-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".nodoassist", "nodoassist.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to NodoAssist");
      expect(message).toContain("Restarting the NodoAssist gateway will not launch it");
      expect(message).not.toContain("Restart the NodoAssist gateway");
