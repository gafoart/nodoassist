...
import { resolveNodoAssistUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveNodoAssistUserDataDir: vi.fn(() => "/tmp/nodoassist-test/nodoassist/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "nodoassist-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveNodoAssistUserDataDir).mockReturnValue(userDataDir);
