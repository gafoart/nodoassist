...
import { MEDIA_MAX_BYTES } from "nodoassist/plugin-sdk/media-runtime";
...
describe("persistBrowserProxyFiles", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("nodoassist-browser-proxy-files-");
  });
  ...
  it("persists browser proxy files under the shared media store", async () => {
    ...
    expect(path.normalize(savedPath ?? "")).toContain(
      `${path.sep}.nodoassist${path.sep}media${path.sep}browser${path.sep}`,
    );
  ...
  it("rejects browser proxy files that exceed the shared media size limit", async () => {
    ...
    await expect(
      fs.stat(path.join(tempHome.home, ".nodoassist", "media", "browser")),
  ...
  it("rewrites nested download paths after node file persistence", () => {
    ...
    applyBrowserProxyPaths(
      result,
      new Map([["/tmp/nodoassist/downloads/report.pdf", "/tmp/nodoassist-media/report.pdf"]]),
    );

    expect(result.download.path).toBe("/tmp/nodoassist-media/report.pdf");
  });
});
