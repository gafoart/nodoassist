 ...
 */
import { parseBrowserHttpUrl, redactCdpUrl } from "nodoassist/plugin-sdk/browser-config";
  ...
  try {
    ...
    const res = await withManagedProxyForCdpUrl(fetchUrl, () =>
      withNoProxyForCdpUrl(url, async () => {
        ...
        // Loopback CDP is an NodoAssist control plane, not page navigation. Allow
