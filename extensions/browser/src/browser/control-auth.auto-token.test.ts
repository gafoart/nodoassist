...
const mocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => NodoAssistConfig>(),
  writeConfigFile: vi.fn<(cfg: NodoAssistConfig) => Promise<void>>(async (_cfg) => {}),
  replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: NodoAssistConfig }) => {
    await mocks.writeConfigFile(nextConfig);
  }),
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: NodoAssistConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<NodoAssistConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: NodoAssistConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): NodoAssistConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: NodoAssistConfig) {
