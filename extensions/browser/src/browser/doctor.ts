...
export function buildBrowserDoctorReport(params: {
  ...
  checks.push({
    ...
    label: "Profile",
    status: "pass",
    summary: `${status.profile ?? "nodoassist"} via ${transport}`,
  });
  ...
  if (transport === "chrome-mcp") {
    ...
    checks.push({
      ...
      status: status.running ? "pass" : "fail",
      summary: status.running
        ? "NodoAssist Chrome extension is connected"
        : "NodoAssist Chrome extension is not connected",
      ...(status.running
        ? {}
        : {
            fixHint:
              "Install the NodoAssist Chrome extension (nodoassist browser extension path), run nodoassist browser extension pair, and paste the pairing string into the extension popup.",
          }),
    });
  } else {
    checks.push({
    ...
    if (missingDisplay) {
      checks.push({
        ...
        fixHint:
          "Use a desktop session, Xvfb, set NODOASSIST_BROWSER_HEADLESS=1, or remove the headed override.",
      });
    }
    ...
    checks.push({
      ...
      ...(status.cdpHttp || !status.running
        ...
        : {
            fixHint:
              "Run nodoassist browser start or inspect browser.cdpUrl/CDP port reachability.",
          }),
    });
