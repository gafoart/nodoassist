/**
 * NodoAssist-managed Chrome profile decoration.
 *
...
import { loadJsonFile, saveJsonFile } from "nodoassist/plugin-sdk/json-store";
import {
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".nodoassist-profile-decorated");
}
...
export function usesNodoAssistMockKeychain(userDataDir: string): boolean {
  const localState = safeReadJson(path.join(userDataDir, "Local State"));
  return readDefaultProfileInfo(localState)?.nodoassist_mock_keychain === true;
}
 ...
 */
export function decorateNodoAssistProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string; mockKeychain?: boolean },
) {
  const desiredName = opts?.name ?? DEFAULT_NODOASSIST_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_NODOASSIST_BROWSER_COLOR).toUpperCase();
  ...
  if (opts?.mockKeychain) {
    ...
    setDeep(localState, ["profile", "info_cache", "Default", "nodoassist_mock_keychain"], true);
  }
