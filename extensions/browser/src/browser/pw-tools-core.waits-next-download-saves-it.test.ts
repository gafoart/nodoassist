...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredNodoAssistTmpDir: vi.fn(() => "/tmp/nodoassist"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-nodoassist-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-nodoassist-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredNodoAssistTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredNodoAssistTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredNodoAssistTmpDir.mockReturnValue("/tmp/nodoassist");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-browser-download-test-"));
    try {
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredNodoAssistTmpDir.mockReturnValue("/tmp/nodoassist-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "nodoassist-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "nodoassist-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredNodoAssistTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).not.toBe(
      path.resolve(path.join(path.sep, "tmp", "nodoassist-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toContain(path.basename(res.path));
    expect(path.basename(outPath)).toMatch(/\.part$/);
    await expect(fs.readFile(res.path, "utf8")).resolves.toBe("download-content");
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "nodoassist-preferred", "downloads")}${path.sep}`),
    );
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tempDir) => {
        ...
        tmpDirMocks.resolvePreferredNodoAssistTmpDir.mockReturnValue(tempDir);
