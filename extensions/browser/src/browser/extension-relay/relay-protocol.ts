/**
 * Wire protocol between the extension relay server and the NodoAssist Chrome
 * extension. The extension stays a dumb transport: it attaches chrome.debugger,
 * forwards CDP traffic, and manages the NodoAssist tab group. All CDP target
 * semantics (Target.* synthesis for Playwright) live server-side in the bridge.
 */

/** Tab snapshot reported by the extension for tabs shared with NodoAssist. */
...
export type RelayCommandBody =
  ...
  /** Open a new tab inside the NodoAssist tab group. Result: { tabId: number }. */
