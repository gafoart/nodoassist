 ...
 *
 * Loopback-only endpoint that pairs the NodoAssist Chrome extension with the
...
const EXTENSION_RELAY_PROTOCOL = "nodoassist-extension-relay";
const EXTENSION_RELAY_TOKEN_PROTOCOL_PREFIX = "nodoassist-extension-token.";
...
export async function startExtensionRelayServer(params: {
  ...
  const server: Server = http.createServer((req, res) => {
    ...
    if (!isAuthorized(req, params.token)) {
      res.writeHead(401, { "WWW-Authenticate": 'Basic realm="nodoassist-extension-relay"' });
    ...
    if (req.method === "GET" && (path === "/json/version" || path === "/json/version/")) {
      if (!bridge.extensionConnected) {
        ...
        res.end(
          JSON.stringify({
            error:
              "NodoAssist Chrome extension is not connected. Install the extension and pair it with `nodoassist browser extension pair`.",
          }),
        );
        return;
      }
