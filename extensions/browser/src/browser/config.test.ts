...
import {
  ...
  NODOASSIST_BROWSER_HEADLESS_ENV,
...
const prevStateDir = process.env.NODOASSIST_STATE_DIR;
beforeEach(() => {
  isolatedStateDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-cfg-")));
  process.env.NODOASSIST_STATE_DIR = isolatedStateDir;
});
afterEach(() => {
  if (prevStateDir === undefined) {
    delete process.env.NODOASSIST_STATE_DIR;
  } else {
    process.env.NODOASSIST_STATE_DIR = prevStateDir;
  }
...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("nodoassist");
    expect(profile?.driver).toBe("nodoassist");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const nodoassist = resolveProfile(resolved, "nodoassist");
    expect(nodoassist?.driver).toBe("nodoassist");
    expect(nodoassist?.cdpPort).toBe(18800);
    expect(nodoassist?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from NODOASSIST_GATEWAY_PORT when unset", () => {
    withEnv({ NODOASSIST_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const nodoassist = resolveProfile(resolved, "nodoassist");
      expect(nodoassist?.cdpPort).toBe(19012);
      expect(nodoassist?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ NODOASSIST_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const nodoassist = resolveProfile(resolved, "nodoassist");
      expect(nodoassist?.cdpPort).toBe(19022);
      expect(nodoassist?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(nodoassist?.cdpPort).toBe(19000);
    expect(nodoassist?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "nodoassist");
  ...
  describe("managed browser headless mode", () => {
    const noDisplayEnv = {
      ...
      [NODOASSIST_BROWSER_HEADLESS_ENV]: undefined,
    };
    ...
    it("lets explicit profile headless=false beat the Linux no-display fallback", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        profiles: {
          nodoassist: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      });
      const profile = resolveProfile(resolved, "nodoassist")!;
    ...
    it("lets NODOASSIST_BROWSER_HEADLESS override profile/global config", () => {
      const resolved = resolveBrowserConfig({
        profiles: {
          nodoassist: { cdpPort: 18800, color: "#FF4500", headless: false },
        },
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          platform: "linux",
          env: { ...noDisplayEnv, [NODOASSIST_BROWSER_HEADLESS_ENV]: "1" },
        }),
    ...
    it("lets request-local headless override beat env and profile/global config", () => {
      ...
      expect(
        resolveManagedBrowserHeadlessMode(resolved, profile, {
          ...
          platform: "linux",
          env: { ...noDisplayEnv, [NODOASSIST_BROWSER_HEADLESS_ENV]: "0" },
        }),
    ...
    it("returns an actionable error only when headed mode is explicitly selected", () => {
      ...
      const defaultProfile = resolveProfile(defaultResolved, "nodoassist")!;
      expect(
      ...
      const profile = resolveProfile(profileResolved, "nodoassist")!;
      expect(
        getManagedBrowserMissingDisplayError(profileResolved, profile, {
          platform: "linux",
          env: noDisplayEnv,
        }),
      ).toContain("browser.profiles.nodoassist.headless=false");
    });
  });
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "nodoassist")!;
  ...
  describe("default profile preference", () => {
    it("defaults to nodoassist profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("nodoassist");
    });

    it("keeps nodoassist default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("nodoassist");
    ...
    it("keeps nodoassist default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("nodoassist");
    ...
    it("keeps nodoassist default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
