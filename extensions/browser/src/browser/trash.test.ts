...
const resolvePreferredNodoAssistTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/nodoassist"));
...
vi.mock("nodoassist/plugin-sdk/temp-path", () => ({
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    ...
    testRoot = realRealpathSyncNative(
      realMkdtempSync(path.join(os.tmpdir(), "nodoassist-browser-")),
    );
    ...
    resolvePreferredNodoAssistTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "nodoassist-demo");
