...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        nodoassist: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.NODOASSIST_GATEWAY_PORT;
    process.env.NODOASSIST_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.NODOASSIST_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.NODOASSIST_GATEWAY_PASSWORD;
