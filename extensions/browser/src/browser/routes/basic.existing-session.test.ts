    ...
    forProfile: () =>
      ({
        profile: {
          ...
          cdpHost: "127.0.0.1",
          cdpIsLoopback: true,
          userDataDir: "/tmp/nodoassist-profile",
          color: "#FF4500",
          headless: false,
          headlessSource: "default",
          attachOnly: false,
...
describe("basic browser routes", () => {
  ...
  it("passes valid start headless override to local managed profiles", async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual({ ok: true, profile: "nodoassist" });
  ...
  it("rejects start headless override for existing-session profiles", async () => {
    ...
    expect(response.statusCode).toBe(400);
    expect(responseBodyRecord(response).error).toBe(
      'Headless start override is only supported for locally launched nodoassist profiles. Profile "chrome-live" is attach-only, remote, or existing-session.',
    );
    expect(ensureBrowserAvailable).not.toHaveBeenCalled();
  });
