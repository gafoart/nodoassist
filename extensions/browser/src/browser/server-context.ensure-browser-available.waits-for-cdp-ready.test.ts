...
function setupEnsureBrowserAvailableHarness() {
  ...
  const launchNodoAssistChrome = vi.mocked(chromeModule.launchNodoAssistChrome);
  const stopNodoAssistChrome = vi.mocked(chromeModule.stopNodoAssistChrome);
  ...
  const profile = ctx.forProfile("nodoassist");

  return { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady, profile, state };
}
...
function requireFirstLaunchOptions(launchNodoAssistChrome: {
  mock: { calls: unknown[][] };
}): unknown {
  const [call] = launchNodoAssistChrome.mock.calls;
  if (!call) {
...
describe("browser server-context ensureBrowserAvailable", () => {
  it("waits for CDP readiness after launching to avoid follow-up PortInUseError races (#21149)", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValueOnce(false).mockResolvedValue(true);
    mockLaunchedChrome(launchNodoAssistChrome, 123);

    const promise = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(promise).resolves.toBeUndefined();

    expect(launchNodoAssistChrome).toHaveBeenCalledTimes(1);
    expect(isChromeCdpReady).toHaveBeenCalled();
    expect(stopNodoAssistChrome).not.toHaveBeenCalled();
  });
  ...
  it("stops launched chrome when CDP readiness never arrives", async () => {
    ...
    mockLaunchedChrome(launchNodoAssistChrome, 321);
    ...
    expect(launchNodoAssistChrome).toHaveBeenCalledTimes(1);
    expect(stopNodoAssistChrome).toHaveBeenCalledTimes(1);
  });

  it("uses configured local CDP readiness timeout after launching", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady, profile, state } =
      setupEnsureBrowserAvailableHarness();
    state.resolved.localCdpReadyTimeoutMs = 250;
    isChromeCdpReady.mockResolvedValue(false);
    mockLaunchedChrome(launchNodoAssistChrome, 322);
  ...
  it("deduplicates concurrent lazy-start calls to prevent PortInUseError", async () => {
    ...
    mockLaunchedChrome(launchNodoAssistChrome, 456);
  ...
  it("deduplicates concurrent lazy-start calls across fresh profile contexts", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchNodoAssistChrome, 457);

    const firstCtx = createBrowserRouteContext({ getState: () => state });
    const secondCtx = createBrowserRouteContext({ getState: () => state });
    const first = firstCtx.forProfile("nodoassist").ensureBrowserAvailable();
    const second = secondCtx.forProfile("nodoassist").ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(Promise.all([first, second])).resolves.toEqual([undefined, undefined]);
  ...
  it("passes request-local headless override to initial launch", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    mockLaunchedChrome(launchNodoAssistChrome, 654);
    ...
    expect(launchNodoAssistChrome).toHaveBeenCalledTimes(1);
    expect(requireFirstLaunchOptions(launchNodoAssistChrome)).toEqual({ headlessOverride: true });
    expect(stopNodoAssistChrome).not.toHaveBeenCalled();
  });
  ...
  it("passes request-local headless override to the owned restart path", async () => {
    ...
    state.profiles.set("nodoassist", {
      profile: profile.profile,
      running: {
        pid: 111,
        exe: { kind: "chromium", path: "/usr/bin/chromium" },
        userDataDir: "/tmp/nodoassist-test",
        cdpPort: 18800,
        startedAt: Date.now(),
        proc: existingProc,
      },
    ...
    mockLaunchedChrome(launchNodoAssistChrome, 987);
  ...
  it("does not share inflight lazy-start promises across different headless overrides", async () => {
    const { launchNodoAssistChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("clears the concurrent lazy-start guard after launch failure", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady, profile } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchNodoAssistChrome.mockRejectedValueOnce(
    ...
    mockLaunchedChrome(launchNodoAssistChrome, 789);
    const retry = profile.ensureBrowserAvailable();
    await vi.advanceTimersByTimeAsync(100);
    await expect(retry).resolves.toBeUndefined();

    expect(launchNodoAssistChrome).toHaveBeenCalledTimes(2);
    expect(stopNodoAssistChrome).not.toHaveBeenCalled();
  });
  ...
  it("cools down repeated managed Chrome launch failures across route contexts", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchNodoAssistChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));

    for (let attempt = 0; attempt < 3; attempt += 1) {
      const ctx = createBrowserRouteContext({ getState: () => state });
      await expect(ctx.forProfile("nodoassist").ensureBrowserAvailable()).rejects.toThrow(
        "Failed to start Chrome CDP",
      );
    }

    const cooledDownCtx = createBrowserRouteContext({ getState: () => state });
    await expect(cooledDownCtx.forProfile("nodoassist").ensureBrowserAvailable()).rejects.toThrow(
      'Browser launch for profile "nodoassist" is cooling down after 3 consecutive managed Chrome launch failures.',
    );
    await expect(cooledDownCtx.forProfile("nodoassist").ensureBrowserAvailable()).rejects.toThrow(
      "set browser.enabled=false if the browser tool is not needed",
    );

    expect(launchNodoAssistChrome).toHaveBeenCalledTimes(3);
    expect(stopNodoAssistChrome).not.toHaveBeenCalled();
  });

  it("allows one managed Chrome launch attempt after the cooldown expires", async () => {
    const { launchNodoAssistChrome, isChromeCdpReady, state } =
      setupEnsureBrowserAvailableHarness();
    isChromeCdpReady.mockResolvedValue(true);
    launchNodoAssistChrome.mockRejectedValue(new Error("Failed to start Chrome CDP"));
    ...
    await expect(retryCtx.forProfile("nodoassist").ensureBrowserAvailable()).rejects.toThrow(
      "Failed to start Chrome CDP",
    );

    expect(launchNodoAssistChrome).toHaveBeenCalledTimes(4);
  });
  ...
  it("reuses a pre-existing loopback browser after an initial short probe miss", async () => {
    ...
    expect(launchNodoAssistChrome).not.toHaveBeenCalled();
    expect(stopNodoAssistChrome).not.toHaveBeenCalled();
  });
  ...
  it("explains attachOnly for externally managed loopback CDP services", async () => {
    ...
    await expect(promise).rejects.toThrow(
      'Port 18800 is in use for profile "nodoassist" but not by nodoassist.',
    );
    await expect(promise).rejects.toThrow(
      "set browser.profiles.nodoassist.attachOnly=true so NodoAssist attaches without trying to manage the local process",
    );
    await expect(promise).rejects.toThrow(
      "For Browserless Docker, set EXTERNAL to the same WebSocket endpoint NodoAssist can reach via browser.profiles.<name>.cdpUrl.",
    );
  ...
  it("retries remote CDP websocket reachability once before failing", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome, isChromeCdpReady } =
      setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
  ...
  it("treats attachOnly loopback CDP as local control with remote-class probe timeouts", async () => {
    const { launchNodoAssistChrome, stopNodoAssistChrome } = setupEnsureBrowserAvailableHarness();
    const isChromeReachable = vi.mocked(chromeModule.isChromeReachable);
    const isChromeCdpReady = vi.mocked(chromeModule.isChromeCdpReady);
