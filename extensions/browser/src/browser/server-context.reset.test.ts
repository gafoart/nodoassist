...
function localNodoAssistProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "nodoassist",
...
function createLocalNodoAssistResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localNodoAssistProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveNodoAssistUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-reset-"));
    const profileDir = path.join(tempRoot, "nodoassist");
    ...
    const ops = createLocalNodoAssistResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveNodoAssistUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-reset-no-own-"));
    const profileDir = path.join(tempRoot, "nodoassist");
    fs.mkdirSync(profileDir, { recursive: true });
