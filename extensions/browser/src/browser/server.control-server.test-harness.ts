...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "nodoassist" | "existing-session";
      attachOnly?: boolean;
    }
  >;
  tabUrl: string;
...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.NODOASSIST_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "nodoassist",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/nodoassist" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    nodoassist: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/nodoassist.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/nodoassist.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/nodoassist.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchNodoAssistChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveNodoAssistUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopNodoAssistChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "nodoassist";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.NODOASSIST_GATEWAY_PORT;
  process.env.NODOASSIST_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.NODOASSIST_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.NODOASSIST_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.NODOASSIST_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.NODOASSIST_GATEWAY_PASSWORD;
  } else {
    process.env.NODOASSIST_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
