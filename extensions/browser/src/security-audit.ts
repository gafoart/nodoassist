 ...
 */
import type { NodoAssistPluginSecurityAuditContext } from "nodoassist/plugin-sdk/plugin-entry";
...
import {
  ...
  isPrivateIpAddress,
} from "nodoassist/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: NodoAssistPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("nodoassist security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.NODOASSIST_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.NODOASSIST_GATEWAY_PASSWORD) ||
