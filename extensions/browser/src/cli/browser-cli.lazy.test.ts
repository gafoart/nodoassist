...
describe("registerBrowserCli lazy browser subcommands", () => {
  ...
  it("registers only the requested browser group before dispatch", async () => {
    ...
    registerBrowserCli(program, ["node", "nodoassist", "browser", "status"]);
  ...
  it("loads browser doctor from the manage group so --deep is available", async () => {
    ...
    registerBrowserCli(program, ["node", "nodoassist", "browser", "doctor", "--deep"]);
  ...
  it("preserves parent --json while reparsing lazy manage commands", async () => {
    ...
    registerBrowserCli(program, ["node", "nodoassist", "browser", "--json", "open", "about:blank"]);
    ...
    tabsProgram.name("nodoassist");
    registerBrowserCli(tabsProgram, ["node", "nodoassist", "browser", "--json", "tabs"]);
  ...
  it("accepts the shipped trailing browser profile order after lazy loading", async () => {
    const program = new Command().name("nodoassist").enablePositionalOptions();
  ...
  it("can eagerly register all browser groups for compatibility", async () => {
    vi.stubEnv("NODOASSIST_DISABLE_LAZY_SUBCOMMANDS", "1");
    const program = new Command();
    program.name("nodoassist");
