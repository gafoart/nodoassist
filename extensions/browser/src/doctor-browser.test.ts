...
describe("browser doctor readiness", () => {
  ...
  it("warns when managed browser profiles have no local executable", async () => {
    ...
    expect(noteFn).toHaveBeenCalledWith(
      [
        "- NodoAssist-managed browser profile(s) are configured: nodoassist.",
  ...
  it("warns about legacy clawd managed browser profile residue", async () => {
    const noteFn = vi.fn();
    const configDir = "/tmp/nodoassist-home";
    ...
    expect(note).toContain("/tmp/nodoassist-home/browser/clawd");
    expect(note).toContain("/tmp/nodoassist-home/browser/nodoassist/user-data");
    expect(note).toContain("nodoassist doctor --fix");
  });
...
describe("legacy clawd browser profile cleanup", () => {
  it("archives stale clawd residue with the safe trash mover", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/nodoassist-home/browser/.trash/clawd");
    ...
    expect(movePathToTrash).toHaveBeenCalledWith("/tmp/nodoassist-home/browser/clawd");
    expect(result.warnings).toStrictEqual([]);
    expect(result.changes.join("\n")).toContain(
      "Archived legacy clawd managed browser profile residue.",
    );
    expect(result.changes.join("\n")).toContain(
      "/tmp/nodoassist-home/browser/nodoassist/user-data",
    );
  });
