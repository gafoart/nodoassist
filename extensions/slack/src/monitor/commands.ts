...
import type { SlackSlashCommandConfig } from "nodoassist/plugin-sdk/config-contracts";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(
    normalizeOptionalString(raw?.name) ?? "nodoassist",
  );
  const name = normalizedName || "nodoassist";
  return {
