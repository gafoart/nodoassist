...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, /^nodoassist_cmdarg/, "arg-menu action");
  ...
  it("registers options handlers without losing app receiver binding", async () => {
    ...
    expect(
      Array.from(testHarness.actions.keys()).some(
        (key) => key instanceof RegExp && String(key) === String(/^nodoassist_cmdarg/),
      ),
    ).toBe(true);
    expect(testHarness.options.has("nodoassist_cmdarg")).toBe(true);
  ...
  it("shows a button menu when required args are omitted", async () => {
    ...
    expect(actions?.elements?.[0]?.action_id).toBe("nodoassist_cmdarg_0_0");
    expect(actions?.elements?.[1]?.action_id).toBe("nodoassist_cmdarg_0_1");
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("nodoassist_cmdarg");
  ...
  it("shows an external_select menu when choices exceed static_select options max", async () => {
    ...
    expect(element?.action_id).toBe("nodoassist_cmdarg");
    expect(blockId).toContain("nodoassist_cmdarg_ext:");
    const token = (blockId ?? "").slice("nodoassist_cmdarg_ext:".length);
...
describe("slack slash command session metadata", () => {
  ...
  it("refreshes slash routing config between invocations", async () => {
    ...
    const sourceCfg = (harness.ctx as { cfg: NodoAssistConfig }).cfg;
    const runtimeCfg = {
      ...sourceCfg,
      session: { dmScope: "per-channel-peer" },
    } as NodoAssistConfig;
    resolveAgentRouteMock.mockImplementation((params: { cfg: NodoAssistConfig }) => ({
      agentId: "main",
      accountId: "acct",
      sessionKey:
