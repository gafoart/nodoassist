...
import { asOptionalRecord as asRecord } from "nodoassist/plugin-sdk/string-coerce-runtime";
...
const NODOASSIST_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
const NODOASSIST_SLACK_SOCKET_START_FAILED_EVENT = "unable_to_socket_mode_start";
const NODOASSIST_SLACK_NATIVE_RECONNECT_OBSERVER_KEY = "__nodoassistNativeReconnectFailureObserver";
...
function installSlackNativeReconnectFailureObserver(receiver: unknown) {
  ...
  if (Reflect.get(client, NODOASSIST_SLACK_NATIVE_RECONNECT_OBSERVER_KEY)) {
    return;
  }
  ...
  Reflect.set(client, NODOASSIST_SLACK_NATIVE_RECONNECT_OBSERVER_KEY, true);
  Reflect.set(
    client,
    ...
    function patchedDelayReconnectAttempt(this: object, callback: unknown) {
      ...
      const delayMs =
        (Number.isFinite(pingTimeoutMs) && pingTimeoutMs >= 0
          ? pingTimeoutMs
          : NODOASSIST_SLACK_CLIENT_PING_TIMEOUT_MS) * nextFailureCount;
        ...
        setTimeout(() => {
          ...
          Promise.resolve(callback.call(this)).then(resolve, (error: unknown) => {
            if (callback === Reflect.get(this, "start")) {
              emit.call(this, NODOASSIST_SLACK_SOCKET_START_FAILED_EVENT, error);
              resolve(undefined);
              return;
            }
...
export function shouldSkipNodoAssistSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? NODOASSIST_SLACK_CLIENT_PING_TIMEOUT_MS,
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping NodoAssist's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
    ...(receiver ? { receiver } : {}),
  });
  app.use(async (args) => {
    if (shouldSkipNodoAssistSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
