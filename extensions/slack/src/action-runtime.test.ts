...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): NodoAssistConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as NodoAssistConfig;
  ...
  function expectLastSlackSend(content: string, cfg: NodoAssistConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: NodoAssistConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: NodoAssistConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/nodoassist-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/nodoassist-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/nodoassist-media/report.pdf",
      outbound: false,
      contentType: "application/pdf",
    });
  });
