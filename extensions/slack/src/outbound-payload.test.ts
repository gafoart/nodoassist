...
import { installChannelOutboundPayloadContractSuite } from "nodoassist/plugin-sdk/channel-contract-testing";
...
describe("slackOutbound sendPayload", () => {
  ...
  it("offsets presentation controls against native Slack blocks before standalone interactive controls", async () => {
    ...
    expect(blocks?.[2]?.block_id).toBe("nodoassist_reply_buttons_2");
    expect(blocks?.[2]?.elements?.[0]?.action_id).toBe("nodoassist:reply_button:2:1");
    expect(blocks?.[3]?.block_id).toBe("nodoassist_reply_buttons_3");
    expect(blocks?.[3]?.elements?.[0]?.action_id).toBe("nodoassist:reply_button:3:1");
  });
});
