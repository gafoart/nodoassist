...
const SLACK_API_URL_KEYS = ["SLACK_API_URL", "NODOASSIST_SLACK_API_URL"] as const;
const PROXY_KEYS = [
  "HTTPS_PROXY",
  "HTTP_PROXY",
  "https_proxy",
...
function writeTempCa(contents: string): string {
  const dir = mkdtempSync(path.join(os.tmpdir(), "nodoassist-slack-proxy-ca-"));
  tempDirs.push(dir);
...
describe("slack web client config", () => {
  ...
  it("does not read NODOASSIST_SLACK_API_URL as a default Slack Web API root", () => {
    process.env.NODOASSIST_SLACK_API_URL = "http://127.0.0.1:49152/api/";
