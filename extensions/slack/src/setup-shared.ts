...
import { patchChannelConfigForAccount } from "nodoassist/plugin-sdk/setup-runtime";
...
import type { NodoAssistConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

export function buildSlackManifest(botName = "NodoAssist") {
  const safeName = botName.trim() || "NodoAssist";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for NodoAssist`,
    },
    features: {
      bot_user: {
      ...
      assistant_view: {
        assistant_description: `${safeName} connects Slack assistant threads to NodoAssist agents.`,
        suggested_prompts: [
          {
