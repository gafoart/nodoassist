...
import { hasConfiguredSecretInput } from "nodoassist/plugin-sdk/secret-input";
import {
...
function enableSlackAccount(cfg: NodoAssistConfig, accountId: string): NodoAssistConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: NodoAssistConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: NodoAssistConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: NodoAssistConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: NodoAssistConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: NodoAssistConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: NodoAssistConfig; accountId: string }) =>
    ...
    disable: (cfg: NodoAssistConfig) => setSetupChannelEnabled(cfg, channel, false),
