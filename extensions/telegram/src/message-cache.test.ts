...
describe("telegram message cache", () => {
  ...
  it("parses legacy sidecar records for doctor migration only", async () => {
    const storePath = `/tmp/nodoassist-telegram-message-cache-legacy-${process.pid}-${Date.now()}.json`;
  ...
  it("filters conversation context nodes when an include predicate is supplied", async () => {
    ...
    for (const msg of [
      ...
      },
      {
        ...
        message_id: 601,
        date: 1736380660,
        text: "@nodoassist_bot please check this",
    ...
    const context = await buildTelegramConversationContext({
      ...
      recentLimit: 10,
      replyTargetWindowSize: 1,
      includeNode: (node) => node.body?.includes("@nodoassist_bot") === true,
    });
  ...
  it("filters ambient transcript rows from cache-derived group context", async () => {
    ...
    for (const msg of [
      ...
      },
      {
        ...
        text: "@nodoassist_bot what happened?",
  ...
  it("does not select messages before the latest session reset command", async () => {
    ...
    const staleInstruction = "okay so we just flip in nodoassist? if yes do it up";
    const record = (params: {
      id: number;
      text: string;
      timestampMs: number;
