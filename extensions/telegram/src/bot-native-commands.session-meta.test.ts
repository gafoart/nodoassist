...
import { getAgentScopedMediaLocalRoots } from "nodoassist/plugin-sdk/media-runtime";
import { resolveChunkMode } from "nodoassist/plugin-sdk/reply-dispatch-runtime";
...
type ResolveConfiguredBindingRouteFn =
  typeof import("nodoassist/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("nodoassist/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
...
type ResolveCommandArgMenuFn =
  typeof import("nodoassist/plugin-sdk/command-auth-native").resolveCommandArgMenu;
...
type LoadModelCatalogFn = typeof import("nodoassist/plugin-sdk/agent-runtime").loadModelCatalog;
type ResolveDefaultModelForAgentFn =
  typeof import("nodoassist/plugin-sdk/agent-runtime").resolveDefaultModelForAgent;
...
vi.mock("nodoassist/plugin-sdk/session-transcript-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("nodoassist/plugin-sdk/session-transcript-runtime")
  >("nodoassist/plugin-sdk/session-transcript-runtime");
  return {
    ...actual,
    resolveSessionTranscriptLegacyFileTarget: sessionMocks.resolveSessionTranscriptLegacyFileTarget,
  };
});
vi.mock("nodoassist/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/command-auth-native")>(
    "nodoassist/plugin-sdk/command-auth-native",
  );
...
vi.mock("nodoassist/plugin-sdk/plugin-runtime", async () => {
...
function resetSessionMetaMocks() {
  ...
  sessionMocks.resolveSessionTranscriptLegacyFileTarget.mockClear().mockResolvedValue({
    ...
    sessionFile: "/tmp/nodoassist-sessions/sess-topic-topic-42.jsonl",
    targetKind: "runtime-session",
  });
  sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/nodoassist-sessions.json");
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.getSessionEntry).toHaveBeenCalledWith({
      storePath: "/tmp/nodoassist-sessions.json",
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/nodoassist-sessions/sessions.json");
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as NodoAssistConfig,
    ...
    expectRecordFields(
      ...
      ),
      {
        ...
        sessionId: "sess-topic",
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        storePath: "/tmp/nodoassist-sessions/sessions.json",
        threadId: 42,
      },
  ...
  it("passes a resolved transcript file to plugin commands when the entry has no file", async () => {
    ...
    sessionMocks.resolveSessionTranscriptLegacyFileTarget.mockResolvedValue({
      ...
      sessionFile: "/tmp/nodoassist-sessions/sess-main.jsonl",
      sessionId: "sess-main",
      sessionKey: "agent:main:main",
      targetKind: "runtime-session",
    });
