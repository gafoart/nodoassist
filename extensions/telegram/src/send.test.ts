...
const { resetLogger, setLoggerOverride } = await import("nodoassist/plugin-sdk/runtime-env");
const {
  buildInlineKeyboard,
...
function captureInfoLogs(): string {
  logCaptureCounter += 1;
  const logFile = `/tmp/nodoassist-telegram-send-log-${process.pid}-${logCaptureCounter}.jsonl`;
...
describe("sent-message-cache", () => {
  ...
  it("keeps sent-message ownership across restart", async () => {
    const persistedStorePath = `/tmp/nodoassist-telegram-send-tests-${process.pid}-restart.json`;
  ...
  it("keeps expired custom-store cleanup away from the default store", () => {
    const customStorePath = `/tmp/nodoassist-telegram-send-tests-${process.pid}-custom-cleanup.json`;
  ...
  it("keeps default and custom stores isolated while both are loaded", () => {
    const customStorePath = `/tmp/nodoassist-telegram-send-tests-${process.pid}-custom-isolated.json`;
    const customCfg = { session: { store: customStorePath } };
...
describe("sendMessageTelegram", () => {
  ...
  it("records sent text messages into the Telegram prompt context cache", async () => {
    const storePath = `/tmp/nodoassist-telegram-send-context-${process.pid}-${Date.now()}.json`;
  ...
  it("records prompt-context text messages with a transcript timestamp override", async () => {
    const storePath = `/tmp/nodoassist-telegram-send-context-override-${process.pid}-${Date.now()}.json`;
    const cfg = { session: { store: storePath } };
  ...
  it.each([
    {
      name: "local path",
      markdown:
        "See [scripts/yougile.py](/home/user/.nodoassist/workspace/scripts/yougile.py#L41) and [docs](https://example.com/docs)",
      rejectedAnchor: '<a href="/home',
      visibleLabel: "<code>scripts/yougile.py</code>",
    },
    {
      name: "relative path",
      markdown: "Edit [config](./nodoassist.json) or see [docs](https://example.com/docs)",
