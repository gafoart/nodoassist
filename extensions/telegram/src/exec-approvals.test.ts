...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}
...
function buildConfig(
  execApprovals?: NonNullable<
    NonNullable<NodoAssistConfig["channels"]>["telegram"]
  >["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<NodoAssistConfig["channels"]>["telegram"]>>,
): NodoAssistConfig {
  return {
    channels: {
      telegram: {
