...
import { resolvePersistentDedupePluginStateNamespace } from "nodoassist/plugin-sdk/persistent-dedupe";
import {
...
describe("telegram state migrations", () => {
  it("detects legacy bot-info cache import", async () => {
    const dir = await mkdtemp(path.join(os.tmpdir(), "nodoassist-telegram-state-migration-"));
    const env = { ...process.env, NODOASSIST_STATE_DIR: dir };
