...
const { inboundBodyResult, recordInboundSessionMock, resolveStorePathMock } = vi.hoisted(() => {
  ...
  return {
    ...
    resolveStorePathMock: vi.fn<ResolveStorePathFn>(() => "/tmp/nodoassist-session-store.json"),
  };
});
...
const { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
  await import("nodoassist/plugin-sdk/runtime-config-snapshot");
...
afterEach(() => {
  ...
  resolveStorePathMock.mockReturnValue("/tmp/nodoassist-session-store.json");
});
...
describe("buildTelegramMessageContext group sessions without forum", () => {
  ...
  it("does not add a topic-cache store lookup for non-forum group reply threads", async () => {
    const resolveStorePath = vi.fn(() => "/tmp/nodoassist/session-store.json");
  ...
  it("reloads topic name from disk after cache reset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-telegram-topic-name-"));
