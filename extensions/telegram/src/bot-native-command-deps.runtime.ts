...
import { getPluginCommandSpecs } from "nodoassist/plugin-sdk/plugin-runtime";
// Telegram plugin module implements bot native command deps behavior.
import type {
  ModelsAuthLoginFlowOptions,
  ModelsAuthLoginFlowResult,
} from "nodoassist/plugin-sdk/provider-auth-login-flow-runtime";
...
export const defaultTelegramNativeCommandDeps: TelegramNativeCommandDeps = {
  ...
  async runModelsAuthLoginFlow(opts) {
    const { runModelsAuthLoginFlow } =
      await import("nodoassist/plugin-sdk/provider-auth-login-flow-runtime");
