...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("nodoassist/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("nodoassist/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "nodoassist-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as NodoAssistConfig) as TelegramBotDeps["getRuntimeConfig"];
