...
import { resolveRequestUrl } from "nodoassist/plugin-sdk/request-url";
...
// fix for the leak observed in nodoassist#68128 is the transport lifecycle that
...
function resolveNodoAssistProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.NODOASSIST_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveNodoAssistProxyUrlForTelegram() : undefined;
