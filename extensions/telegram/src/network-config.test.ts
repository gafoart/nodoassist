...
import type { TelegramNetworkConfig } from "nodoassist/plugin-sdk/config-contracts";
...
let isWSL2Sync: typeof import("nodoassist/plugin-sdk/runtime-env").isWSL2Sync;
...
async function loadModule() {
  ({ isWSL2Sync } = await import("nodoassist/plugin-sdk/runtime-env"));
  ({
  ...
  it.each([
    {
      ...
      env: {
        NODOASSIST_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1",
        NODOASSIST_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1",
      },
      expected: {
        value: true,
        source: "env:NODOASSIST_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "uses env disable when set",
      env: { NODOASSIST_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY: "1" },
      expected: {
        value: false,
        source: "env:NODOASSIST_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY",
      },
    },
    {
      name: "prefers env enable over config",
      env: { NODOASSIST_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY: "1" },
  ...
  it.each([
    {
      ...
      env: { NODOASSIST_TELEGRAM_DNS_RESULT_ORDER: "verbatim" },
      nodeMajor: 22,
      expected: {
        value: "verbatim",
        source: "env:NODOASSIST_TELEGRAM_DNS_RESULT_ORDER",
      },
    },
    {
      name: "normalizes trimmed env values",
      env: { NODOASSIST_TELEGRAM_DNS_RESULT_ORDER: "  IPV4FIRST  " },
      nodeMajor: 20,
      expected: {
        value: "ipv4first",
        source: "env:NODOASSIST_TELEGRAM_DNS_RESULT_ORDER",
    ...
    },
    {
      ...
      env: { NODOASSIST_TELEGRAM_DNS_RESULT_ORDER: "bogus" },
      network: { dnsResultOrder: "ipv4first" },
      nodeMajor: 20,
      expected: { value: "ipv4first", source: "config" },
    },
