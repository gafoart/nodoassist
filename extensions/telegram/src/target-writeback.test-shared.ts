...
vi.mock("nodoassist/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/config-mutation")>(
    "nodoassist/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("nodoassist/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("nodoassist/plugin-sdk/cron-store-runtime")>(
    "nodoassist/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/nodoassist.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
