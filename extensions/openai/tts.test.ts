...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds NodoAssist attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.["User-Agent"]).toBe("nodoassist/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.NODOASSIST_DEBUG_PROXY_SESSION_ID = "tts-session";
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.NODOASSIST_DEBUG_PROXY_ENABLED = "1";
      process.env.NODOASSIST_STATE_DIR = tempDir;
      process.env.NODOASSIST_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
