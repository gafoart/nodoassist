...
const disableAutoStartKey = Symbol.for("nodoassist.diffs.disableAutoStart");
...
function renderCard(payloadOverride?: string): void {
  ...
  document.body.insertAdjacentHTML(
    ...
    `<section class="oc-diff-card">
      <div data-nodoassist-diff-host></div>
      <script type="application/json" data-nodoassist-diff-payload>${payload}</script>
    </section>`,
  );
}
...
describe("hydrateViewer", () => {
  beforeEach(() => {
    ...
    delete document.documentElement.dataset.nodoassistDiffsError;
    delete document.documentElement.dataset.nodoassistDiffsReady;
    vi.clearAllMocks();
  });
  ...
  it("continues hydrating later cards when one card throws", async () => {
    ...
    expect(document.documentElement.dataset.nodoassistDiffsError).toBeUndefined();
    warn.mockRestore();
  });
...
describe("ensureShadowRoot", () => {
  ...
  it("attaches shadow root from template and removes template element", async () => {
    renderCard();
    const host = document.querySelector<HTMLElement>("[data-nodoassist-diff-host]")!;
