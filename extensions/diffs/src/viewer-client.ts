...
declare const __NODOASSIST_DIFFS_LANGUAGE_PACK__: boolean | undefined;

// Build-time esbuild define; typeof guard keeps the module loadable where the
// define is absent (vitest/node), matching the __NODOASSIST_VERSION__ pattern.
function readInjectedLanguagePackFlag(): boolean | undefined {
  return typeof __NODOASSIST_DIFFS_LANGUAGE_PACK__ === "boolean"
    ? __NODOASSIST_DIFFS_LANGUAGE_PACK__
    : undefined;
}
...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-nodoassist-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-nodoassist-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.nodoassistDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.nodoassistDiffsError = "true";
    console.error("Failed to hydrate diff viewer", error);
  }
}

export const disableAutoStartKey = Symbol.for("nodoassist.diffs.disableAutoStart");
