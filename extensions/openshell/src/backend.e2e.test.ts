...
import { createSandboxTestContext } from "nodoassist/plugin-sdk/test-fixtures";
import {
...
const NODOASSIST_OPENSHELL_E2E = process.env.NODOASSIST_E2E_OPENSHELL === "1";
const NODOASSIST_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const NODOASSIST_OPENSHELL_COMMAND =
  process.env.NODOASSIST_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
const NODOASSIST_OPENSHELL_CONFIG_HOME =
  process.env.NODOASSIST_E2E_OPENSHELL_CONFIG_HOME?.trim() || null;
const NODOASSIST_OPENSHELL_HOST_IP = process.env.NODOASSIST_E2E_OPENSHELL_HOST_IP?.trim() || null;
...
RUN echo "nodoassist-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
async function resolveOpenShellHostIp(): Promise<string> {
  if (NODOASSIST_OPENSHELL_HOST_IP) {
    return NODOASSIST_OPENSHELL_HOST_IP;
  }
  ...
  throw new Error(
    "OpenShell E2E could not resolve the OpenShell Docker network gateway; set NODOASSIST_E2E_OPENSHELL_HOST_IP",
  );
}
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && NODOASSIST_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: NODOASSIST_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        throw new Error("OpenShell E2E requires a working Docker daemon");
      }
      if (!(await commandAvailable(NODOASSIST_OPENSHELL_COMMAND))) {
        throw new Error(`OpenShell CLI is unavailable: ${NODOASSIST_OPENSHELL_COMMAND}`);
      }
      if (!NODOASSIST_OPENSHELL_CONFIG_HOME) {
        throw new Error(
          "OpenShell E2E requires NODOASSIST_E2E_OPENSHELL_CONFIG_HOME because tests isolate HOME and XDG_CONFIG_HOME",
        );
      }
      const openshellConfigHome = NODOASSIST_OPENSHELL_CONFIG_HOME;
      const hostIp = await resolveOpenShellHostIp();
      const gatewayName = await activeOpenShellGateway(NODOASSIST_OPENSHELL_COMMAND, {
        ...process.env,
      ...
      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-openshell-e2e-"));
      ...
      const allowSandboxName = `nodoassist-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: NODOASSIST_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("nodoassist-openshell-e2e");
