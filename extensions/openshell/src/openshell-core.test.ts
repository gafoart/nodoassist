...
import type { CreateSandboxBackendParams } from "nodoassist/plugin-sdk/sandbox";
import {
...
describe("openshell backend manager", () => {
  ...
  it("uploads staged directory snapshots to the managed remote directory itself", () => {
    expect(
      buildOpenShellDirectoryUploadArgs({
        sandboxName: "nodoassist-session",
        localPath: "/tmp/nodoassist-upload/sandbox/seed.txt",
        remotePath: "/sandbox",
      }),
    ).toEqual([
      "sandbox",
      ...
      "--no-git-ignore",
      "nodoassist-session",
      "/tmp/nodoassist-upload/sandbox/seed.txt",
      "/sandbox",
    ]);
    expect(
      buildOpenShellDirectoryUploadArgs({
        sandboxName: "nodoassist-session",
        localPath: "/tmp/nodoassist-upload/project",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await makeTempDir("nodoassist-openshell-real-");
      const root = path.join(realParent, "sandbox");
      const target = path.join(root, ".nodoassist", "sandbox-skills");
  ...
  it("checks runtime status with config override from NodoAssist config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "nodoassist-session-1234",
        backendId: "openshell",
        runtimeLabel: "nodoassist-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "nodoassist-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "nodoassist-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "nodoassist-session-5678",
        backendId: "openshell",
        runtimeLabel: "nodoassist-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "nodoassist",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "nodoassist-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "nodoassist-session-5678"],
    });
  });
  ...
  it("preserves a local sandbox skills shadow when mirror sync crosses filesystems", async () => {
    const workspaceDir = await makeTempDir("nodoassist-openshell-workspace-");
    const shadowFile = path.join(workspaceDir, ".nodoassist", "sandbox-skills", "user-note.txt");
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        ...
        await fs.mkdir(path.join(tmpDir, ".nodoassist", "sandbox-skills", "skills"), {
          recursive: true,
        });
        await fs.writeFile(
          path.join(tmpDir, ".nodoassist", "sandbox-skills", "skills", "generated.txt"),
          "generated",
          "utf8",
        );
      }
    ...
    try {
      ...
      await expectPathMissing(
        path.join(workspaceDir, ".nodoassist", "sandbox-skills", "skills", "generated.txt"),
      );
    } finally {
  ...
  it("drops non-directory materialized sandbox skills from mirror downloads", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.mkdir(path.join(tmpDir, ".nodoassist"), { recursive: true });
        await fs.writeFile(path.join(tmpDir, ".nodoassist", "sandbox-skills"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expectPathMissing(path.join(workspaceDir, ".nodoassist", "sandbox-skills"));
  });
  ...
  it("restores a local sandbox skills shadow when mirror download has a file parent", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = args[4];
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.writeFile(path.join(tmpDir, ".nodoassist"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expect(fs.readFile(shadowFile, "utf8")).resolves.toBe("local shadow");
    expect((await fs.stat(path.join(workspaceDir, ".nodoassist"))).isDirectory()).toBe(true);
  });
});
...
function createOpenShellBackendSandboxConfig(): CreateSandboxBackendParams["cfg"] {
  return {
    ...
    ssh: createSandboxSshConfig("/tmp/nodoassist-sandboxes"),
...
async function makeExecutable(params: { name: string; script: string }): Promise<string> {
  const dir = await makeTempDir("nodoassist-openshell-bin-");
...
describe("openshell fs bridges", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = await makeTempDir("nodoassist-openshell-remote-pin-");
  ...
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("nodoassist-openshell-fs-");
  ...
  it("rejects cross-root mirror renames before the remote backend commit", async () => {
    const workspaceDir = await makeTempDir("nodoassist-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("nodoassist-openshell-agent-fs-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeTempDir("nodoassist-openshell-fs-");
      const outsideDir = await makeTempDir("nodoassist-openshell-outside-");
  ...
  it("reads materialized sandbox skills from the protected skills workspace", async () => {
    const workspaceDir = await makeTempDir("nodoassist-openshell-fs-");
    const skillsWorkspaceDir = await makeTempDir("nodoassist-openshell-skills-");
    ...
    await expect(
      bridge.readFile({
        filePath: "/sandbox/.nodoassist/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.readFile({
        filePath: ".nodoassist/sandbox-skills/skills/demo/SKILL.md",
      }),
    ).resolves.toEqual(Buffer.from("# Demo\nmaterialized\n"));
    await expect(
      bridge.writeFile({
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("nodoassist-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("nodoassist-openshell-agent-");
