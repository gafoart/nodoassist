...
vi.mock("nodoassist/plugin-sdk/sandbox", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nodoassist/plugin-sdk/sandbox")>();
  return {
    ...actual,
...
describe("openshell backend exec workdir validation", () => {
  beforeEach(() => {
    ...
    cliMocks.createOpenShellSshSession.mockResolvedValue({
      command: "ssh",
      configPath: "/tmp/nodoassist-openshell-test-ssh-config",
      host: "openshell-test",
    });
    ...
    sdkMocks.runSshSandboxCommand.mockImplementation(async ({ remoteCommand }) => ({
      stdout: String(remoteCommand).includes("nodoassist-validate-workdir")
