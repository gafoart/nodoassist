...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "nodoassist/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "nodoassist/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: NodoAssistConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: NodoAssistConfig): string | undefined {
...
export function registerMinimaxProviders(api: NodoAssistPluginApi) {
