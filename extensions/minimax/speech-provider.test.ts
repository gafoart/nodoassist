...
describe("buildMinimaxSpeechProvider", () => {
  ...
  describe("isConfigured", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "nodoassist-minimax-tts-auth-"));
      ...
      process.env.NODOASSIST_AGENT_DIR = tempAgentDir;
      clearMinimaxAuthEnv();
      clearRuntimeAuthProfileStoreSnapshots();
    });
  ...
  describe("synthesize", () => {
    ...
    beforeEach(async () => {
      tempStateDir = await mkdtemp(path.join(tmpdir(), "nodoassist-minimax-tts-synth-"));
      tempAgentDir = path.join(tempStateDir, "agents", "main", "agent");
      await mkdir(tempAgentDir, { recursive: true });
      process.env = {
        ...savedEnv,
        NODOASSIST_AGENT_DIR: tempAgentDir,
        NODOASSIST_STATE_DIR: tempStateDir,
      };
      clearMinimaxAuthEnv();
      clearRuntimeAuthProfileStoreSnapshots();
