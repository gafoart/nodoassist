...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__nodoassist_channel_id";
...
type InteractionCallbackConfig = Pick<NodoAssistConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "nodoassist-mattermost-interactions").update(botToken).digest("hex");
}
