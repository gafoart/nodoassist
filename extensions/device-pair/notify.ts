...
import type { NodoAssistPluginService } from "nodoassist/plugin-sdk/core";
import { listDevicePairing } from "nodoassist/plugin-sdk/device-bootstrap";
...
async function readNotifyState(api: NodoAssistPluginApi): Promise<NotifyStateFile> {
...
async function writeNotifyState(api: NodoAssistPluginApi, state: NotifyStateFile): Promise<void> {
  const subscriberStore = openNotifySubscriberStore(api);
...
async function notifyPendingPairingRequests(params: { api: NodoAssistPluginApi }): Promise<void> {
...
export function createPairingNotifierService(api: NodoAssistPluginApi): NodoAssistPluginService {
