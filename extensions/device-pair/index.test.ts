...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredNodoAssistTmpDir: vi.fn(() =>
    path.join(os.tmpdir(), "nodoassist-device-pair-tests"),
  ),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredNodoAssistTmpDir: pluginApiMocks.resolvePreferredNodoAssistTmpDir,
...
function createApi(params?: {
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as NodoAssistPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: NodoAssistPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredNodoAssistTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredNodoAssistTmpDir(), {
      recursive: true,
      force: true,
    });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the NodoAssist iOS app:");
    expect(payload.mediaUrl).toBeUndefined();
    expect(payload.channelData?.nodoassistPairingQr).toEqual({
    ...
    expect(text).not.toContain("![NodoAssist pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the NodoAssist iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://nodoassist.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://nodoassist.local:18789");
  });
