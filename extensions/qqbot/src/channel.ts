...
import { getExecApprovalReplyMetadata } from "nodoassist/plugin-sdk/approval-runtime";
import { buildChannelOutboundSessionRoute } from "nodoassist/plugin-sdk/channel-core";
import {
...
import { toGatewayAccount, writeNodoAssistConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped nodoassist.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // nodoassist.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeNodoAssistConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe nodoassist.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeNodoAssistConfigThroughRuntime(getQQBotRuntime(), nextCfg as NodoAssistConfig);
      }

      const resolved = resolveQQBotAccount(
        (changed ? nextCfg : cfg) as NodoAssistConfig,
        accountId,
      );
