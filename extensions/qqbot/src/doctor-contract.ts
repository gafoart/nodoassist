...
import type { GroupToolPolicyConfig } from "nodoassist/plugin-sdk/channel-policy";
import type { NodoAssistConfig } from "nodoassist/plugin-sdk/config-contracts";
import { asObjectRecord } from "nodoassist/plugin-sdk/runtime-doctor";
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.qqbot.groups.<id>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.groups.<id>.tools instead. Run "nodoassist doctor --fix".',
    match: hasLegacyGroupToolPolicy,
  },
  {
    path: ["channels", "qqbot", "accounts"],
    message:
      'channels.qqbot.accounts.<id>.groups.<groupId>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.accounts.<id>.groups.<groupId>.tools instead. Run "nodoassist doctor --fix".',
...
export function normalizeCompatibilityConfig({
  ...
  return {
    config: {
      ...
      channels: {
        ...cfg.channels,
        qqbot: updated as unknown as NonNullable<NodoAssistConfig["channels"]>["qqbot"],
