...
describe("qqbot local media path remapping", () => {
  ...
  function createNodoAssistTestRoot() {
    const actualHome = getHomeDir();
    const nodoassistDir = path.join(actualHome, ".nodoassist");
    fs.mkdirSync(nodoassistDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(nodoassistDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createNodoAssistTestRoot();
  ...
  it("allows structured payload files inside sibling NodoAssist media subdirectories", () => {
    ...
    // trust the shared `~/.nodoassist/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".nodoassist", "media", "outbound");
...
// when HOME and NODOASSIST_HOME diverge (Docker, multi-user hosts), QQ Bot media
// paths must be anchored on NODOASSIST_HOME so files written under
// `$NODOASSIST_HOME/.nodoassist/media/qqbot/` are accepted by the outbound
// allowlist.
//
...
// home as the baseline and only varies `process.env.NODOASSIST_HOME`.
describe("qqbot media path resolution honors NODOASSIST_HOME (#83562)", () => {
  ...
  function makeFakeNodoAssistHome(): string {
  ...
  it("accepts files under $NODOASSIST_HOME/.nodoassist/media/qqbot when NODOASSIST_HOME differs from HOME", () => {
    const fakeNodoAssistHome = makeFakeNodoAssistHome();
    vi.stubEnv("NODOASSIST_HOME", fakeNodoAssistHome);

    const mediaFile = path.join(fakeNodoAssistHome, ".nodoassist", "media", "qqbot", "repro.png");
    ...
    const oldHomeMediaRoot = path.join(realOsHome, ".nodoassist", "media", "qqbot");
    expect(isPathInsideOrEqual(mediaFile, oldHomeMediaRoot)).toBe(false);
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");

    expect(getQQBotMediaPath()).toBe(
      path.join(fakeNodoAssistHome, ".nodoassist", "media", "qqbot"),
    );
    expect(resolveQQBotPayloadLocalFilePath(mediaFile)).toBe(fs.realpathSync(mediaFile));
  });

  it("expands tilde-prefixed NODOASSIST_HOME against the OS home", () => {
    ...
    vi.stubEnv("NODOASSIST_HOME", `~/${sub}`);

    expect(getQQBotMediaPath()).toBe(path.join(expectedHome, ".nodoassist", "media", "qqbot"));

    const mediaFile = path.join(expectedHome, ".nodoassist", "media", "qqbot", "tilde.png");
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");
  ...
  it("falls back to OS home when NODOASSIST_HOME is unset (no regression)", () => {
    vi.stubEnv("NODOASSIST_HOME", "");

    expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".nodoassist", "media", "qqbot"));
  });

  it("treats sentinel strings 'undefined' and 'null' as unset", () => {
    for (const sentinel of ["undefined", "null"]) {
      vi.stubEnv("NODOASSIST_HOME", sentinel);
      expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".nodoassist", "media", "qqbot"));
    }
  });
  ...
  it("keeps persisted QQ Bot data anchored on the OS home (compatibility)", () => {
    ...
    // operator adds NODOASSIST_HOME — otherwise existing deployments would lose
    // their session state. Only the media root follows NODOASSIST_HOME.
    expect(getQQBotDataPath()).toBe(path.join(realOsHome, ".nodoassist", "qqbot"));
  });

  it("rejects files that live under HOME tree when NODOASSIST_HOME is the active root", () => {
    const fakeNodoAssistHome = makeFakeNodoAssistHome();
    vi.stubEnv("NODOASSIST_HOME", fakeNodoAssistHome);

    // File under the HOME-side mirror — exactly the path that *worked* on
    // current main and *broke* the NODOASSIST_HOME setup. After the fix the
    // active media root is NODOASSIST_HOME, so a file under HOME is no longer
    ...
    const homeOnlyFile = path.join(realOsHome, ".nodoassist", "media", "qqbot", stale);
  ...
  it("remaps workspace paths under either HOME or NODOASSIST_HOME to the NODOASSIST_HOME media root", () => {
    const fakeNodoAssistHome = makeFakeNodoAssistHome();
    vi.stubEnv("NODOASSIST_HOME", fakeNodoAssistHome);

    const baseName = `remap-${process.pid}-${Date.now()}`;

    // Real file lives under the NODOASSIST_HOME media tree.
    const mediaFile = path.join(
      fakeNodoAssistHome,
      ".nodoassist",
      "media",
    ...
    const homeWorkspaceDir = path.join(realOsHome, ".nodoassist", "workspace", "qqbot");
    ...
    // HOME-side path, so nothing to clean up there beyond the NODOASSIST_HOME tree.
    expect(resolveQQBotLocalMediaPath(homeWorkspacePath)).toBe(mediaFile);

    // Same path but under NODOASSIST_HOME should also remap.
    const nodoassistWorkspacePath = path.join(
      fakeNodoAssistHome,
      ".nodoassist",
      "workspace",
      "qqbot",
    ...
    expect(resolveQQBotLocalMediaPath(nodoassistWorkspacePath)).toBe(mediaFile);
  });
});
