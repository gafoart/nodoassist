...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.nodoassist/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".nodoassist", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".nodoassist", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "nodoassist.sqlite");
