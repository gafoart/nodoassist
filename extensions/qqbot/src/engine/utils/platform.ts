 ...
 * silk-wasm availability checks — all without importing `nodoassist/plugin-sdk`.
 ...
 * `NODOASSIST_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective NodoAssist home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of NodoAssist treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `nodoassist` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `NODOASSIST_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveNodoAssistHome(): string {
  const raw = process.env.NODOASSIST_HOME?.trim();
...
/**
 * Return a legacy path under `~/.nodoassist/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".nodoassist", "qqbot", ...subPaths);
}

/** Return a path under `~/.nodoassist/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<nodoassist-home>/.nodoassist/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under NodoAssist's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `NODOASSIST_HOME` (when set) so files written by agents into
 * the NodoAssist-managed media tree are reachable by this plugin even when
 * `HOME` and `NODOASSIST_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveNodoAssistHome(), ".nodoassist", "media", "qqbot", ...subPaths);
}

/** Return a path under `<nodoassist-home>/.nodoassist/media/qqbot`, creating it on demand. */
...
/**
 * Return `<nodoassist-home>/.nodoassist/media`, NodoAssist's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `nodoassist/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `NODOASSIST_HOME`.
 */
function getNodoAssistMediaDir(): string {
  return path.join(resolveNodoAssistHome(), ".nodoassist", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const nodoassistHomeDir = resolveNodoAssistHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When NODOASSIST_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the NodoAssist-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".nodoassist", "workspace", "qqbot"),
      path.join(nodoassistHomeDir, ".nodoassist", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and NodoAssist's shared `~/.nodoassist/media`
  ...
  const allowedRoots = [getNodoAssistMediaDir(), getQQBotMediaPath()];
