...
vi.mock("nodoassist/plugin-sdk/outbound-media", () => ({
...
import { loadOutboundMediaFromUrl } from "nodoassist/plugin-sdk/outbound-media";
import * as securityRuntime from "nodoassist/plugin-sdk/security-runtime";
import {
...
function makeCtx() {
  return {
    ...
    mediaAccess: {
      localRoots: ["/tmp/nodoassist-sandbox"],
      workspaceDir: "/tmp/workspace",
      readFile: async () => Buffer.from("report"),
    },
    mediaLocalRoots: ["/tmp/nodoassist-sandbox"],
...
beforeEach(async () => {
  ...
  nodoassistHome = await fs.realpath(
...
afterEach(async () => {
  ...
  if (nodoassistHome) {
    await fs.rm(nodoassistHome, { recursive: true, force: true });
  }
});
...
describe("resolveOutboundMediaPath", () => {
  ...
  it("does not treat workspaceDir as an allowed host absolute root", () => {
    ...
    ).toEqual(["/tmp/nodoassist-sandbox"]);
  });

  it("maps only authorized virtual workspace roots for host-read loading", () => {
    expect(
      resolveWorkspaceScopedLocalRoots(
        ["/workspace/attachments", "/tmp/nodoassist-sandbox", "/workspace/../media"],
        "/tmp/agent-workspace",
      ),
    ).toEqual(["/tmp/agent-workspace/attachments", "/tmp/nodoassist-sandbox"]);
  });
...
describe("trySendViaHostRead error handling", () => {
  it("returns OutboundResult.error when loadOutboundMediaFromUrl rejects", async () => {
    ...
    const result = await sendPhoto(makeCtx(), "/tmp/nodoassist-sandbox/report.docx");
  ...
  it("falls back to normal local sends for trusted media paths outside host-read roots", async () => {
    const trustedMediaDir = path.join(nodoassistHome, ".nodoassist", "media", "qqbot");
  ...
  it("returns OutboundResult.error when senderSendMedia rejects", async () => {
    ...
    const result = await sendPhoto(makeCtx(), "/tmp/nodoassist-sandbox/chart.png");
  ...
  it("loads virtual-root workspace media through the real outbound loader", async () => {
    const actualOutboundMedia = await vi.importActual<
      typeof import("nodoassist/plugin-sdk/outbound-media")
    >("nodoassist/plugin-sdk/outbound-media");
    mockedLoadOutboundMediaFromUrl.mockImplementation(actualOutboundMedia.loadOutboundMediaFromUrl);
    mockedSenderSendMedia.mockResolvedValue({ id: "media-1", timestamp: 123 });
    const workspaceDir = path.join(nodoassistHome, "agent-workspace");
