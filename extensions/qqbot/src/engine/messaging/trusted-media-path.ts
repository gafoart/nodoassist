import { resolvePreferredNodoAssistTmpDir } from "nodoassist/plugin-sdk/sandbox";
import { resolveLocalPathFromRootsSync } from "nodoassist/plugin-sdk/security-runtime";
...
function trustedNodoAssistTmpRoot(): string | null {
  if (cachedTrustedTmpRoot === undefined) {
    try {
      cachedTrustedTmpRoot = resolvePreferredNodoAssistTmpDir();
    } catch {
      return null;
    }
  }
 ...
 * roots, this also trusts NodoAssist's permission-hardened temp root, where
...
export function resolveTrustedOutboundMediaPath(
  ...
  const tmpRoot = trustedNodoAssistTmpRoot();
  if (!tmpRoot) {
    return null;
  }
  return (
    resolveLocalPathFromRootsSync({
      filePath: p,
      roots: [tmpRoot],
      label: "NodoAssist temp media root",
