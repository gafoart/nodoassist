...
const getRuntimeConfigMock = vi.hoisted(() => vi.fn<() => NodoAssistConfig>());

vi.mock("nodoassist/plugin-sdk/runtime-config-snapshot", () => ({
  getRuntimeConfig: getRuntimeConfigMock,
}));

function asCfg(shape: { bindings: Array<{ id: string }> }): NodoAssistConfig {
  return shape as unknown as NodoAssistConfig;
}
