...
vi.mock("../../utils/platform.js", () => ({
  ...
  getQQBotDataDir: (...subPaths: string[]) => {
    const dir = platformMock.path.join(platformMock.homeDir, ".nodoassist", "qqbot", ...subPaths);
...
describe("buildBotLogsResult", () => {
  ...
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-qqbot-logs-"));
  ...
  it("suffixes same-second log exports instead of overwriting", () => {
    ...
    const logDir = path.join(tempHome, ".nodoassist", "logs");
