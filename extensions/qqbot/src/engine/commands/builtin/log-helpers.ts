...
function getConfiguredLogFiles(): string[] {
  ...
  const files: string[] = [];
  for (const cli of ["nodoassist", "clawdbot", "moltbot"]) {
    try {
...
function collectCandidateLogDirs(): string[] {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (/STATE_DIR$/i.test(key) && /(NODOASSIST|CLAWDBOT|MOLTBOT)/i.test(key)) {
      pushStateDir(value);
    }
  }
  ...
  for (const root of searchRoots) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(nodoassist|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
  ...
  if (!isWindows()) {
    for (const name of ["nodoassist", "clawdbot", "moltbot"]) {
...
function collectRecentLogFiles(logDirs: string[]): LogCandidate[] {
  ...
  for (const dir of logDirs) {
    ...
    pushFile(path.join(dir, "nodoassist.log"), dir);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (!/(gateway|nodoassist|clawdbot|moltbot)/i.test(entry.name)) {
          continue;
        }
