...
import { coerceSecretRef, normalizeSecretInputString } from "nodoassist/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: NodoAssistConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: NodoAssistConfig): string {
...
/** Apply account config updates back into the NodoAssist config object. */
