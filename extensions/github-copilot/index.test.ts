...
import { MAX_DATE_TIMESTAMP_MS, MAX_TIMER_TIMEOUT_MS } from "nodoassist/plugin-sdk/number-runtime";
import type {
  NodoAssistConfig,
  NodoAssistPluginApi,
  ProviderAuthResult,
...
type RegisteredMemoryEmbeddingProvider = Parameters<
  NodoAssistPluginApi["registerMemoryEmbeddingProvider"]
>[0];
type RegisteredProvider = Parameters<NodoAssistPluginApi["registerProvider"]>[0];
type GithubCopilotTestProvider = RegisteredProvider & {
  auth: Array<{
    run: (ctx: unknown) => Promise<ProviderAuthResult | null>;
    runNonInteractive: (ctx: unknown) => Promise<NodoAssistConfig | null>;
  }>;
  catalog: {
...
async function createAgentDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-github-copilot-test-"));
  tempDirs.push(dir);
  return dir;
}
...
function registerProviderAndCatalogWithPluginConfig(pluginConfig: Record<string, unknown>) {
  const registerProviderMock = vi.fn<NodoAssistPluginApi["registerProvider"]>();
  const registerModelCatalogProviderMock =
    vi.fn<NodoAssistPluginApi["registerModelCatalogProvider"]>();
...
describe("github-copilot plugin", () => {
  ...
  it("registers embedding provider", () => {
    const registerMemoryEmbeddingProviderMock =
      vi.fn<NodoAssistPluginApi["registerMemoryEmbeddingProvider"]>();
