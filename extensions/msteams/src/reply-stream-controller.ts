...
/**
 * Bridges nodoassist's reply pipeline callbacks to the SDK's `ctx.stream`.
...
export function createTeamsReplyStreamController(params: {
  ...
  // nodoassist's reply pipeline calls onPartialReply with the cumulative text on
  ...
  return {
    ...
    onPartialReply(payload: { text?: string }): void {
      ...
      // mode, nodoassist's pipeline doesn't deliver tokens — the model output
