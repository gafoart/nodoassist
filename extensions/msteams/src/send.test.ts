...
// pulls from `nodoassist/plugin-sdk/outbound-media` (post-migration). Mock the
// canonical source so the re-export carries our stub through.
vi.mock("nodoassist/plugin-sdk/outbound-media", () => ({
  loadOutboundMediaFromUrl: mockState.loadOutboundMediaFromUrl,
}));

vi.mock("nodoassist/plugin-sdk/markdown-table-runtime", () => ({
  resolveMarkdownTableMode: mockState.resolveMarkdownTableMode,
}));

vi.mock("nodoassist/plugin-sdk/text-chunking", async (importOriginal) => {
  const actual = await importOriginal<typeof import("nodoassist/plugin-sdk/text-chunking")>();
  return {
    ...actual,
