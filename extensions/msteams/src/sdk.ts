...
import { buildNodoAssistUserAgentFragment } from "./user-agent.js";
...
type MSTeamsAppOn = {
  ...
  // delegates back into the SDK handlers after NodoAssist sender policy passes.
...
export async function createMSTeamsApp(
  ...
  // backend can identify NodoAssist traffic for usage telemetry. Teams SDK
  ...
  const appOptions: Record<string, unknown> = {
    client: options?.httpClient ?? {
      headers: { "User-Agent": buildNodoAssistUserAgentFragment() },
    },
