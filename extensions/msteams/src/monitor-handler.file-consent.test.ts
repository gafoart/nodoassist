  ...
  return {
    ...
    state: {
      ...
      resolveStateDir: (env?: NodeJS.ProcessEnv) => {
        ...
        return stateDir ?? path.join(os.homedir(), ".nodoassist");
      },
    },
...
describe("msteams file consent invoke FS fallback", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "nodoassist-msteams-invoke-"));
    process.env.NODOASSIST_STATE_DIR = tmpDir;
