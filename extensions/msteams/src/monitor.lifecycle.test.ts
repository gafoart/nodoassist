...
function createConfig(port: number): NodoAssistConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  ...
  patch: NonNullable<NonNullable<NodoAssistConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): NodoAssistConfig {
  ...
  it("gates poll card votes before recording them", async () => {
    ...
    const response = await cardActionHandler({
      activity: {
        ...
        value: { action: { data: { nodoassistPollId: "poll-1", choices: "0" } } },
      },
    });
