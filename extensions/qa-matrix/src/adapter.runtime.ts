...
import { buildQaTarget } from "nodoassist/plugin-sdk/qa-channel";
...
const MATRIX_SHARED_FLOW_TOPOLOGY = {
  ...
  rooms: [
    ...
    },
    {
      ...
      kind: "group",
      members: ["driver", "observer", "sut"],
      name: "NodoAssist Matrix QA Secondary Room",
      requireMention: true,
    },
    {
      key: "driver-dm",
      kind: "dm",
      members: ["driver", "sut"],
      name: "NodoAssist Matrix QA Driver DM",
    },
    {
      key: "driver-dm-shared",
      kind: "dm",
      members: ["driver", "sut"],
      name: "NodoAssist Matrix QA Shared DM",
    },
  ],
...
export async function createMatrixQaTransportAdapter(
  ...
  try {
    provisioning = await provisionMatrixQaRoom({
      ...
      roomName: `NodoAssist Matrix QA ${suffix}`,
  ...
  return {
    ...
    async sendInbound(input) {
      ...
      const hasPortableMention = input.text.includes("@nodoassist");
      const body = input.text.replaceAll("@nodoassist", provisioning.sut.userId);
    ...
    createGatewayConfig: () =>
      buildMatrixQaConfig({} as NodoAssistConfig, {
    ...
    createRuntimeEnvPatch: () => ({
      NODOASSIST_QA_MATRIX_DRIVER_USER_ID: provisioning.driver.userId,
      NODOASSIST_QA_MATRIX_OBSERVER_USER_ID: provisioning.observer.userId,
    }),
