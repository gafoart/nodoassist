...
import { spawn as startNodoAssistCliProcess, spawnSync } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `nodoassist ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaNodoAssistCliEntryPath(cwd: string): string {
...
function killMatrixQaCliChild(
  child: ReturnType<typeof startNodoAssistCliProcess>,
...
export function startMatrixQaNodoAssistCli(params: {
  ...
  const distEntryPath = resolveMatrixQaNodoAssistCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startNodoAssistCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaNodoAssistCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaNodoAssistCli(params).wait();
}
...
export async function createMatrixQaNodoAssistCliRuntime(params: {
  accountId: string;
  accessToken: string;
