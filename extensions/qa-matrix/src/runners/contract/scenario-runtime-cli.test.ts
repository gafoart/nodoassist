...
describe("Matrix QA CLI runtime", () => {
  it("redacts secret CLI arguments in diagnostic command text", () => {
    ...
    ).toBe("nodoassist matrix verify backup restore --recovery-key [REDACTED]");
    expect(formatMatrixQaCliCommand(["matrix", "account", "add", "--access-token=token-123"])).toBe(
      "nodoassist matrix account add --access-token=[REDACTED]",
    );
    expect(
      formatMatrixQaCliCommand(["matrix", "verify", "device", "abcdef1234567890ghij", "--json"]),
    ).toBe("nodoassist matrix verify device [REDACTED] --json");
  });
  ...
  it("prefers the ESM NodoAssist CLI entrypoint when present", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-entry-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(path.join(root, "dist", "index.mjs"), "");
      expect(resolveMatrixQaNodoAssistCliEntryPath(root)).toBe(
  ...
  it("can preserve expected non-zero CLI output for negative scenarios", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-nonzero-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const result = await runMatrixQaNodoAssistCli({
        allowNonZero: true,
  ...
  it("can pass stdin to CLI commands", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-stdin-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
  ...
  it("can close stdin after interactive CLI prompts", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-interactive-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
      ...
      const session = startMatrixQaNodoAssistCli({
  ...
  it("includes timed-out CLI output in diagnostics", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-timeout-"),
    );
    try {
      await mkdir(path.join(root, "dist"));
      await writeFile(
  ...
  it("kills CLI commands that ignore graceful timeout termination", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-timeout-kill-"),
    );
  ...
  it("preserves timeout diagnostics when wait attaches after timeout", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-late-wait-timeout-"),
    );
    const pidPath = path.join(root, "cli.pid");
    let childPid: number | undefined;
    try {
  ...
  it("settles and kills descendants that keep timed-out CLI stdio open", async () => {
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-timeout-tree-"),
    );
  ...
  it("kills ignored-stdio descendants after a timed-out CLI exits gracefully", async () => {
    ...
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-timeout-ignored-stdio-"),
    );
    const childPidPath = path.join(root, "child.pid");
    const grandchildPidPath = path.join(root, "grandchild.pid");
    let childPid: number | undefined;
    ...
    try {
      ...
      const run = runMatrixQaNodoAssistCli({
        args: ["matrix", "verify", "self"],
        cwd: root,
        env: process.env,
        timeoutMs: 500,
  ...
  it("kills ignored-stdio descendants after manual CLI session kill", async () => {
    ...
    const root = await mkdtemp(
      path.join(resolvePreferredNodoAssistTmpDir(), "matrix-qa-cli-session-kill-ignored-stdio-"),
    );
    const childPidPath = path.join(root, "child.pid");
    const grandchildPidPath = path.join(root, "grandchild.pid");
    let childPid: number | undefined;
