...
describe("matrix qa cli registration", () => {
  const originalDisableForceExit = process.env.NODOASSIST_QA_MATRIX_DISABLE_FORCE_EXIT;
  ...
  afterEach(() => {
    if (originalDisableForceExit === undefined) {
      delete process.env.NODOASSIST_QA_MATRIX_DISABLE_FORCE_EXIT;
    } else {
      process.env.NODOASSIST_QA_MATRIX_DISABLE_FORCE_EXIT = originalDisableForceExit;
    }
    exitSpy.mockRestore();
  ...
  it("passes a non-default selected account into an affected shared flow", async () => {
    process.env.NODOASSIST_QA_MATRIX_DISABLE_FORCE_EXIT = "1";
    const qa = new Command();
    matrixQaCliRegistration.register(qa);
  ...
  it("exits with failure after Matrix artifacts are written for a failed run", async () => {
    ...
    await expect(qa.parseAsync(["node", "nodoassist", "matrix"])).rejects.toThrow(
      "process.exit(1)",
    );
