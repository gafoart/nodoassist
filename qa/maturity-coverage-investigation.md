...
| nodoassist-app-sdk<br>NodoAssist App SDK                                                                                                        | 31           | 1              | 2          | 0               | 29                      | 3.2%      | 6.5%         | 6.5%              |
...
| `qa/scenarios/runtime/package-nodoassist-for-docker.yaml`       | vitest     | `test/e2e/qa-lab/runtime/package-nodoassist-for-docker.e2e.test.ts`        |
...
- Health and Repair (10): `telemetry.background-health-monitor-loop`, `telemetry.core-doctor-checks`, `telemetry.gateway-rpc-health`, `telemetry.nodoassist-health`, `telemetry.per-account-enable-disable-settings`, `telemetry.plugin-sdk-doctor-health-contracts`, `telemetry.restart-logging`, `telemetry.startup-grace`, `telemetry.structured-health-checks`, `windows.nodoassist-status`
- Logging (5): `telemetry.gateway-rpc-logs-tail`, `telemetry.nodoassist-logs`, `telemetry.redaction-patterns-and-sinks`, `telemetry.rolling-gateway-jsonl-file-logs`, `telemetry.trace-correlation-fields`
- Diagnostic Collection (7): `telemetry.bounded-in-process-stability-recorder`, `telemetry.chat-diagnostics`, `telemetry.critical-memory-pressure-snapshot-option`, `telemetry.memory-pressure-events`, `telemetry.nodoassist-gateway-diagnostics-export`, `telemetry.nodoassist-gateway-stability`, `telemetry.nodoassist-gateway-stability-bundle`
...
- Publishing (7): `clawhub.external-code-plugin-package-contract-required`, `clawhub.npm-trusted-publishing-provenance`, `clawhub.nodoassist-owned-package-release-validation-for-clawhub`, `clawhub.package-publishing-owner`, `clawhub.skill-package-metadata`, `clawhub.skill-publishing-flow`, `clawhub.version-bump-gates`
- Catalog Discovery (5): `clawhub.catalog-lookup-failure`, `clawhub.distinction-between-plugin-search`, `clawhub.nodoassist-plugins-search-as-the-clawhub`, `clawhub.search-result-metadata`, `clawhub.skill-catalog-search`
- Compatibility and Trust (12): `clawhub.archive`, `clawhub.built-in-dangerous-code-scanner`, `clawhub.compatibility-docs`, `clawhub.npm-compatibility-fallback-to-the-newest`, `clawhub.npm-integrity-drift`, `clawhub.official-external-plugin-catalog-behavior`, `clawhub.nodoassist-compat-pluginapi`, `clawhub.operator-trust-model-for-installing`, `clawhub.package-compatibility-validation`, `clawhub.publishing-review-hidden-release-behavior-as-upstream`, `clawhub.skill-archive-safety`, `clawhub.skill-audit-signals`
...
- Gateway Service Lifecycle (10): `macos.app-managed-launchagent-handoff`, `macos.gateway-token-env-handling`, `macos.launchagent-labels`, `macos.launchctl-bootstrap`, `macos.managed-service-refresh`, `macos.nodoassist-uninstall`, `macos.nodoassist-update-package-git-handoff`, `macos.per-user-gateway-launchagent-install`, `macos.stale-updater-launchd-job-detection`, `macos.stranded-service-recovery`
- Diagnostics and Observability (4): `macos.gateway-silently-stops-responding`, `macos.launchagent-log-paths`, `macos.nodoassist-gateway-status-deep`, `macos.stale-updater-jobs`
...
- CLI (7): `windows.nodoassist-onboard`, `wsl2.cli-entrypoints`, `wsl2.managed-systemd-gateway-restart`, `wsl2.nodoassist-doctor-status-and-logs`, `wsl2.nodoassist-update`, `wsl2.package-manager-caveats`, `wsl2.service-metadata-refresh`
...
- Diagnostics and Repair (5): `telemetry.nodoassist-logs`, `windows.nodoassist-status`, `wsl2.operator-repair-guidance-after-wsl2-service`, `wsl2.secretref`, `wsl2.wsl-systemd-unavailable-hints`
...
- CLI (9): `windows.command-shims`, `windows.daemon-install-flags`, `windows.local-gateway-config`, `windows.native-vs-wsl-setup-boundary`, `windows.node-and-package-manager-bootstrap`, `windows.npm-global-install`, `windows.nodoassist-onboard`, `windows.packaged-cli-launcher`, `windows.powershell-installer`
- Gateway Management (11): `windows.foreground-runtime-health-readiness`, `windows.gateway-launcher-files`, `windows.nodoassist-gateway`, `windows.nodoassist-gateway-install`, `windows.nodoassist-status`, `windows.post-install-diagnostics`, `windows.scheduled-task-runtime-status`, `windows.service-inspection`, `windows.specific-restart-signal`, `windows.startup-folder-fallback`, `windows.unmanaged-foreground-mode`
...
- Updates (4): `windows.detached-update-handoff`, `windows.managed-gateway-stop-restart`, `windows.nodoassist-update-on-native-windows-package`, `windows.package-locks`
...
- Setup and Compatibility (11): `raspberry-pi.64-bit-arm-boundary`, `raspberry-pi.fallback-build-guidance`, `raspberry-pi.hardware-and-64-bit-os-requirements`, `raspberry-pi.installer-architecture-detection`, `raspberry-pi.node-runtime-setup`, `raspberry-pi.npm-pnpm-bun-install-modes`, `raspberry-pi.nodoassist-install-and-onboarding`, `raspberry-pi.optional-arm-binary-checks`, `raspberry-pi.slow-device-caveats`, `raspberry-pi.supported-pi-model-selection`, `raspberry-pi.unsupported-device-guidance`
...
- Install Handoff (4): `nix.install-discoverability`, `nix.install-overview`, `nix.nodoassist-source-of-truth`, `nix.verification-handoff`
...
- Channel Setup and Operations (5): `whatsapp.baileys-socket-lifecycle`, `whatsapp.channel-config-schema`, `whatsapp.official-nodoassist-whatsapp-plugin-metadata`, `whatsapp.nodoassist-plugin-install-whatsapp`, `whatsapp.operator-troubleshooting`
