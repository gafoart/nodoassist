...
const COMPILE_CACHE_DISABLED_RESPAWNED_ENV = "NODOASSIST_COMPILE_CACHE_DISABLED_RESPAWNED";
...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `nodoassist: Node.js ${SUPPORTED_NODE_RANGE} is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[nodoassist] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.NODOASSIST_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    NODOASSIST_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["nodoassist: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:nodoassist/nodoassist#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g nodoassist@latest`.");
  return lines.join("\n");
};
...
const hasLauncherContainerTarget = (argv) => {
  if (normalizeLauncherMetadataValue(process.env.NODOASSIST_CONTAINER)) {
    return true;
  }
...
const isHelpFastPathDisabled = () =>
  process.env.NODOASSIST_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.NODOASSIST_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.NODOASSIST_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveLauncherUserPath(explicit)];
  }
  const stateOverride = process.env.NODOASSIST_STATE_DIR?.trim();
  if (stateOverride) {
    const stateDir = resolveLauncherUserPath(stateOverride);
    return [path.join(stateDir, "nodoassist.json"), path.join(stateDir, "clawdbot.json")];
  }
  const homeDir = resolveLauncherHomeDir();
  return [
    path.join(homeDir, ".nodoassist", "nodoassist.json"),
    path.join(homeDir, ".nodoassist", "clawdbot.json"),
    path.join(homeDir, ".clawdbot", "nodoassist.json"),
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.NODOASSIST_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.NODOASSIST_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    ...
    process.stdout.write(
      commit ? `NodoAssist ${version} (${commit})\n` : `NodoAssist ${version}\n`,
    );
    return true;
  } catch {
    return false;
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.NODOASSIST_BUNDLED_VERSION) ?? "0.0.0";
}
