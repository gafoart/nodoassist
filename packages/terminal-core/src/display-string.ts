...
/** Resolve the underlying OS home before applying NodoAssist overrides. */
...
/** Resolve raw home with NODOASSIST_HOME tilde expansion. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$NODOASSIST_HOME" } : { home, prefix: "~" };
}
...
/** Replace the effective home path with "~" or "$NODOASSIST_HOME" for terminal display. */
