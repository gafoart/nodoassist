...
import type {
  ...
  HelloOk,
  RequestFrame,
} from "@nodoassist/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@nodoassist/gateway-protocol/client-info";
import {
...
} from "@nodoassist/gateway-protocol/connect-error-details";
import {
  isGatewayEventFrame,
  isGatewayResponseFrame,
} from "@nodoassist/gateway-protocol/frame-guards";
import { resolveGatewayStartupRetryAfterMs } from "@nodoassist/gateway-protocol/startup-unavailable";
import {
  MIN_CLIENT_PROTOCOL_VERSION,
  PROTOCOL_VERSION,
} from "@nodoassist/gateway-protocol/version";
...
// The package stays reusable by depending on host callbacks for NodoAssist-owned
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    this.deps = {
      // Defaults keep the package inert outside NodoAssist; device signing throws
  ...
  start() {
    ...
    const allowPrivateWs =
      (this.opts.env ?? process.env).NODOASSIST_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set NODOASSIST_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `nodoassist doctor --fix` for guidance.",
      );
