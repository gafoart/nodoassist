// NodoAssist SDK module implements client behavior.
...
// High-level NodoAssist SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the NodoAssist SDK client. */
export type NodoAssistOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: NodoAssistTransport;
};

function resolveGatewayUrl(options: NodoAssistOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function assertNoUnsupportedRunOptions(params: AgentRunParams): void {
  ...
  throw new Error(
    `NodoAssist Gateway does not support per-run SDK option${
...
function unsupportedGatewayApi(api: string): never {
  throw new Error(`${api} is not supported by the current NodoAssist Gateway yet`);
}
...
function readChatProjection(event: NodoAssistEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: NodoAssistEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: NodoAssistEvent): boolean {
  return (
...
export class NodoAssist {
  ...
  private readonly transport: NodoAssistTransport;
  private readonly normalizedEvents = new EventHub<NodoAssistEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, NodoAssistEvent[]>();
  ...
  constructor(options: NodoAssistOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: NodoAssistEvent) => boolean): AsyncIterable<NodoAssistEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: NodoAssistEvent) => boolean,
  ): AsyncIterable<NodoAssistEvent> {
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new Error("NodoAssist SDK client is closed");
    }
  }
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: NodoAssistEvent): NodoAssistEvent | undefined => {
    ...
    const matches = (event: NodoAssistEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: NodoAssistEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): NodoAssistEvent[] {
...
export class Agent {
  constructor(
    private readonly client: NodoAssist,
    readonly id: string,
  ) {}
...
export class AgentsNamespace {
  constructor(private readonly client: NodoAssist) {}
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<NodoAssistEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: NodoAssist,
