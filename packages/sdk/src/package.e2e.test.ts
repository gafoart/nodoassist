// NodoAssist SDK tests cover package behavior.
...
const WORKSPACE_PACKAGE_NAMES = [
  "@nodoassist/gateway-protocol",
  "@nodoassist/gateway-client",
  "@nodoassist/sdk",
] as const;
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@nodoassist/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
async function startNodoAssistRegistry(packages: PackedPackage[]): Promise<{
  registryUrl: string;
  close: () => Promise<void>;
}> {
...
describe("NodoAssist SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@nodoassist/sdk")?.tarball ?? "";
    expect(sdkTarball).not.toBe("");
    const registry = await startNodoAssistRegistry(packedPackages);
    ...
    await fs.writeFile(
      ...
      `@nodoassist:registry=${registry.registryUrl}`,
    );
    try {
    ...
    const importScript = `
      import { GatewayClientTransport, NodoAssist, normalizeGatewayEvent } from "@nodoassist/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof NodoAssist !== "function") throw new Error("missing client export");
