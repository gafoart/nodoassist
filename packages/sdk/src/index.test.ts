// NodoAssist SDK tests cover index behavior.
import { describe, expect, it } from "vitest";
import { EventHub, NodoAssist, normalizeGatewayEvent } from "./index.js";
import type {
  GatewayEvent,
...
class FakeTransport implements NodoAssistTransport {
...
class EventsOnlyTransport implements NodoAssistTransport {
...
describe("NodoAssist SDK", () => {
  it("runs an agent through the Gateway agent method", async () => {
    ...
    const oc = new NodoAssist({ transport });
  ...
  it("splits provider-qualified model refs and rejects unsupported run options", async () => {
    ...
    await expect(
      ...
      "NodoAssist Gateway does not support per-run SDK options yet: workspace, runtime, environment, approvals",
    );
  });
  ...
  it("throws explicit unsupported errors for SDK namespaces without Gateway RPCs", async () => {
    ...
    await expect(oc.environments.create({ provider: "testbox" })).rejects.toThrow(
      "oc.environments.create is not supported by the current NodoAssist Gateway yet",
    );
    await expect(oc.environments.delete("environment_123")).rejects.toThrow(
      "oc.environments.delete is not supported by the current NodoAssist Gateway yet",
    );
    expect(transport.calls).toStrictEqual([]);
  });
  ...
  it("keeps close terminal when it races a pending connect", async () => {
    ...
    await expect(connect).rejects.toThrow("NodoAssist SDK client is closed");
    await close;
    await expect(oc.agents.list()).rejects.toThrow("NodoAssist SDK client is closed");
    await expect(oc.events()[Symbol.asyncIterator]().next()).rejects.toThrow(
      "NodoAssist SDK client is closed",
    );
    expect(() => oc.rawEvents()).toThrow("NodoAssist SDK client is closed");
  ...
  it("rejects normalized event streams when the event pump fails before yielding", async () => {
    ...
    let futureIterator: AsyncIterator<NodoAssistEvent> | undefined;
  ...
  it("uses cumulative text for the first replayed chat projection", async () => {
    ...
    let text = "";
    let iterator: AsyncIterator<NodoAssistEvent> | undefined;
