...
/** Minimal transport interface consumed by the NodoAssist SDK client. */
export type NodoAssistTransport = {
  request<T = unknown>(
    method: string,
    params?: unknown,
...
export type ConnectableNodoAssistTransport = NodoAssistTransport & {
...
export type RuntimeSelection =
  | "auto"
  | { type: "embedded"; id: "nodoassist" | "codex" | (string & {}) }
...
export type NodoAssistEventType =
  | "run.created"
  | "run.queued"
  | "run.started"
  | "run.completed"
...
export type NodoAssistEvent<TData = unknown> = {
  version: 1;
  id: string;
  ts: number;
  type: NodoAssistEventType;
  runId?: string;
  sessionId?: string;
  sessionKey?: string;
  taskId?: string;
