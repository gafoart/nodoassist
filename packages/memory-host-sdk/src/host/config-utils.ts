...
export { splitShellArgs } from "./nodoassist-runtime-io.js";

// Shared NodoAssist config helpers used by memory host, QMD, and agent context code.
...
/** Narrow NodoAssist config shape consumed by memory host utilities. */
...
/** Resolve the underlying OS home before applying NodoAssist-specific overrides. */
...
/** Resolve NODOASSIST_HOME or the OS home, falling back to cwd for hermetic tests. */
function resolveRequiredHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const explicitHome = normalizeHomeValue(env.NODOASSIST_HOME);
...
/** Resolve absolute user paths, including "~" against the effective NodoAssist home. */
...
function resolveStateDir(
  ...
  if (env.NODOASSIST_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
  // Existing legacy state remains authoritative until an explicit migration creates .nodoassist.
...
/** Resolve the default agent workspace, partitioned by NODOASSIST_PROFILE when set. */
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  const home = resolveRequiredHomeDir(env, os.homedir);
  const profile = env.NODOASSIST_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".nodoassist", `workspace-${profile}`);
  }
  return path.join(home, ".nodoassist", "workspace");
}

/** Return configured agent entries after dropping nullish placeholders. */
function listAgentEntries(cfg: NodoAssistConfig): AgentConfig[] {
...
function resolveDefaultAgentId(cfg: NodoAssistConfig): string {
...
function resolveAgentConfig(cfg: NodoAssistConfig, agentId: string): AgentConfig | undefined {
