...
import type { NodoAssistConfig } from "./config-utils.js";
...
const rootMemoryConfig = (workspaceDir: string): NodoAssistConfig =>
  ({
    agents: {
...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as NodoAssistConfig;
  ...
  it("preserves unquoted Windows absolute qmd command paths", () => {
    ...
    const cfg = {
      agents: { defaults: { workspace: String.raw`C:\Users\penny\.nodoassist\workspace` } },
      memory: {
        backend: "qmd",
        qmd: {
          command,
