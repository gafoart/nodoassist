...
export type MemorySessionSyncTarget = {
  /** Owning NodoAssist agent. Omit only when the active manager scope already supplies it. */
  agentId?: string;
...
export type MemorySyncParams = {
   ...
   * During the deprecation window only canonical NodoAssist transcript paths are accepted.
   */
  sessionFiles?: string[];
