...
`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"nodoassist":{"emoji":"disk","events":["command:new"]}}');

    const parsed = JSON5.parse(result.metadata);
    expect(parsed.nodoassist?.emoji).toBe("disk");
  });
...
---`;
    const result = parseFrontmatterBlock(content);
    expect(result.metadata).toBe('{"nodoassist":true}');
  });
