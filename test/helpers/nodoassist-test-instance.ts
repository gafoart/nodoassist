// NodoAssist test instance helper spawns isolated NodoAssist processes.
...
import {
  ...
  type NodoAssistTestStateOptions,
} from "../../src/test-utils/nodoassist-test-state.js";
import { sleep } from "../../src/utils.js";

export type NodoAssistTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<NodoAssistTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type NodoAssistTestInstanceCommandResult = {
  code: number | null;
...
type NodoAssistTestProcess = ChildProcessByStdio<null, Readable, Readable>;

export type NodoAssistTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  stateDir: string;
  configPath: string;
  state: NodoAssistTestState;
  stdout: string[];
  stderr: string[];
  child?: NodoAssistTestProcess;
  env: NodeJS.ProcessEnv;
  entrypoint: () => Promise<string[]>;
  cli: (
    args: string[],
    options?: { timeoutMs?: number },
  ) => Promise<NodoAssistTestInstanceCommandResult>;
...
type NodoAssistTestChildProcess = Pick<NodoAssistTestProcess, "kill" | "pid">;
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseNodoAssistTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalNodoAssistTestProcess(child, "SIGKILL");
...
function hasChildExited(child: Pick<NodoAssistTestProcess, "exitCode" | "signalCode">) {
...
export async function createNodoAssistTestInstance(
  options: NodoAssistTestInstanceOptions,
): Promise<NodoAssistTestInstance> {
  ...
  let child: NodoAssistTestProcess | undefined;
  let cleaned = false;

  const instance: NodoAssistTestInstance = {
    name: options.name,
    port,
    ...
    stopGateway: async () => {
      ...
      if (!hasChildExited(child) && !child.killed) {
        try {
          signalNodoAssistTestProcess(child, "SIGTERM");
        } catch {
          // ignore
        }
      }
...
function shouldUseNodoAssistTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalNodoAssistTestProcess(
  child: NodoAssistTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = (pid, nextSignal) =>
    process.kill(pid, nextSignal),
): void {
  if (shouldUseNodoAssistTestProcessGroup() && typeof child.pid === "number") {
    try {
