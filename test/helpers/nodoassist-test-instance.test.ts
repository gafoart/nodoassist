// NodoAssist test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { createNodoAssistTestInstance, testing } from "./nodoassist-test-instance.js";
...
describe("nodoassist test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalNodoAssistTestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createNodoAssistTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".nodoassist"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "nodoassist.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.NODOASSIST_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.NODOASSIST_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.NODOASSIST_SKIP_CRON).toBe("0");
