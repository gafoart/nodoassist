...
const SHARED_TEST_SETUP = Symbol.for("nodoassist.sharedTestSetup");
const EMBEDDED_RUN_STATE = Symbol.for("nodoassist.embeddedRunState");
const REPLY_RUN_REGISTRY = Symbol.for("nodoassist.replyRunRegistry");
...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.NODOASSIST_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.NODOASSIST_TEST_HOME = testHome;
...
function resetNodoAssistGlobalRunState(): void {
...
export default class NodoAssistNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreRealTimers();
    restoreNativeTimerGlobals();
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.NODOASSIST_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
