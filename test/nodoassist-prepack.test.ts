// NodoAssist prepack tests validate package prepack output.
...
import {
  ...
  runPrepackCommand,
} from "../scripts/nodoassist-prepack.ts";
...
describe("resolvePrepackCommandTimeoutMs", () => {
  it("parses only positive integer environment timeouts", () => {
    ...
    expect(resolvePrepackCommandTimeoutMs({ NODOASSIST_PREPACK_COMMAND_TIMEOUT_MS: "" })).toBe(
      30 * 60 * 1000,
    );
    expect(resolvePrepackCommandTimeoutMs({ NODOASSIST_PREPACK_COMMAND_TIMEOUT_MS: "1234" })).toBe(
      1234,
    );

    for (const raw of ["nope", "10m", "1e3", "0", "-1", "9007199254740992"]) {
      expect(() =>
        resolvePrepackCommandTimeoutMs({ NODOASSIST_PREPACK_COMMAND_TIMEOUT_MS: raw }),
      ).toThrow(`invalid NODOASSIST_PREPACK_COMMAND_TIMEOUT_MS: ${raw}`);
    }
  });
});
