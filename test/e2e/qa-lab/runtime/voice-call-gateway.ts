...
async function runVoiceCallProof(options: ProducerOptions): Promise<string> {
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-voice-call-gateway-"));
  ...
  try {
    gateway = await startQaGatewayChild({
      ...
      runtimeEnvPatch: {
        NODOASSIST_QA_VOICE_BRIDGE_CALLS_PATH: fixture.bridgeCallsPath,
        NODOASSIST_QA_VOICE_TOOL_RESULTS_PATH: fixture.toolResultsPath,
      },
      mutateConfig: (config) =>
    ...
    if (!bridgeCalls.some((call) => call.tools?.includes("nodoassist_agent_consult"))) {
      throw new Error(
