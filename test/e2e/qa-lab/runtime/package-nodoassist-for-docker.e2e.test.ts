// Package NodoAssist For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packNodoAssistPackageForDocker,
  parseArgs,
  prepareBundledAiRuntimePackage,
  runCommandForTest,
} from "../../../../scripts/package-nodoassist-for-docker.mjs";
...
describe("package-nodoassist-for-docker", () => {
  it("parses package artifact output options", () => {
    expect(
      parseArgs([
        "--output-dir",
        ".artifacts/docker",
        "--output-name=nodoassist-current.tgz",
        "--pack-json",
  ...
  it("uses build-all with declaration generation for package artifacts", async () => {
    ...
    const previousTimeout = process.env.NODOASSIST_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    const previousSkipDts = process.env.NODOASSIST_RUN_NODE_SKIP_DTS_BUILD;
    process.env.NODOASSIST_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    process.env.NODOASSIST_RUN_NODE_SKIP_DTS_BUILD = "1";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.NODOASSIST_BUILD_ALL_NO_PNPM,
            skipDts: options.env?.NODOASSIST_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.NODOASSIST_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.NODOASSIST_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
      if (previousSkipDts === undefined) {
        delete process.env.NODOASSIST_RUN_NODE_SKIP_DTS_BUILD;
      } else {
        process.env.NODOASSIST_RUN_NODE_SKIP_DTS_BUILD = previousSkipDts;
      }
    }
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.NODOASSIST_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "NODOASSIST_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("bundles and restores the separately packed AI runtime", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-ai-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-ai-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify(
      {
        dependencies: { "@nodoassist/ai": "workspace:*", "dep-a": "1.2.3" },
        files: ["dist"],
        name: "nodoassist",
        version: "2026.6.17",
      },
    ...
    const installedAiPath = path.join(sourceDir, "node_modules", "@nodoassist", "ai");
    ...
    try {
      const cleanup = await prepareBundledAiRuntimePackage(
        ...
        async (command: string, args: string[], cwd: string) => {
          ...
          fs.writeFileSync(path.join(outputDir, "nodoassist-ai-2026.6.17.tgz"), "ai package");
          return "";
        },
        {
      ...
      expect(packageJson.dependencies["@nodoassist/ai"]).toBe("2026.6.17");
      expect(packageJson.bundleDependencies).toContain("@nodoassist/ai");
      ...
      expect(fs.existsSync(path.join(outputDir, "nodoassist-ai-2026.6.17.tgz"))).toBe(false);
    } finally {
  ...
  it("leaves pre-AI-workspace package sources unchanged", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-legacy-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-legacy-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
      dependencies: { "dep-a": "1.2.3" },
      name: "nodoassist",
  ...
  it("rejects incomplete AI workspace package sources", async () => {
    const cases = [
      {
        dependencies: { "@nodoassist/ai": "workspace:*" },
        expected: "@nodoassist/ai dependency requires the packages/ai workspace",
        withWorkspace: false,
      },
      {
        dependencies: {},
        expected: "root package.json must declare @nodoassist/ai as a dependency",
        withWorkspace: true,
      },
    ];

    for (const testCase of cases) {
      const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-invalid-source-"));
      const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-invalid-output-"));
      fs.writeFileSync(
        path.join(sourceDir, "package.json"),
        `${JSON.stringify({ dependencies: testCase.dependencies, name: "nodoassist" })}\n`,
      );
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packNodoAssistPackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (
        ...
        return "nodoassist-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "nodoassist-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare:/repo",
  ...
  it("writes npm pack metadata for renamed package artifacts", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-pack-json-"));
    const packJsonPath = path.join(outputDir, "pack.json");

    try {
      const tarball = await packNodoAssistPackageForDocker("/repo", outputDir, {
        outputName: "nodoassist-current.tgz",
        packJsonPath,
        prepareBundledAiRuntime: skipBundledAiRuntime,
        ...
        runCaptureImpl: async (
          ...
          expect(options.deferForwardedSignalExit).toBe(true);
          fs.writeFileSync(path.join(outputDir, "nodoassist-2026.5.28.tgz"), "package");
          return JSON.stringify([
            {
              entryCount: 1,
              filename: "nodoassist-2026.5.28.tgz",
              size: 7,
              unpackedSize: 7,
              version: "2026.5.28",
            },
      ...
      expect(tarball).toBe(path.join(outputDir, "nodoassist-current.tgz"));
      expect(JSON.parse(fs.readFileSync(packJsonPath, "utf8"))).toEqual([
        {
          entryCount: 1,
          filename: "nodoassist-current.tgz",
          size: 7,
          unpackedSize: 7,
          version: "2026.5.28",
        },
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    for (const filename of [
      ...
      "nodoassist-nested/evil.tgz",
      String.raw`nodoassist-nested\evil.tgz`,
      "nodoassist-C:evil.tgz",
    ]) {
      await expect(
        packNodoAssistPackageForDocker("/repo", "/out", {
          prepareBundledAiRuntime: skipBundledAiRuntime,
          prepareChangelog: async () => {},
          restoreChangelog: async () => {},
  ...
  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-pack-"));
    try {
      fs.writeFileSync(path.join(outputDir, "nodoassist-C:evil.tgz"), "");
      fs.writeFileSync(path.join(outputDir, String.raw`nodoassist-nested\evil.tgz`), "");
      await expect(
        packNodoAssistPackageForDocker("/repo", outputDir, {
          prepareBundledAiRuntime: skipBundledAiRuntime,
          prepareChangelog: async () => {},
          restoreChangelog: async () => {},
          runCaptureImpl: async () => "npm notice\n",
        }),
      ).rejects.toThrow("missing packed NodoAssist tarball");
      ...
      await expect(
        packNodoAssistPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "nodoassist-2026.6.17.tgz"), "");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "nodoassist-2026.6.17.tgz"));
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
    }
  });
  ...
  it("ignores stale package tarballs before fallback scanning npm output", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-docker-pack-stale-"));
    try {
      fs.writeFileSync(path.join(outputDir, "nodoassist-9999.1.1.tgz"), "stale");
      ...
      await expect(
        packNodoAssistPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "nodoassist-2026.6.17.tgz"), "current");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "nodoassist-2026.6.17.tgz"));

      expect(fs.existsSync(path.join(outputDir, "nodoassist-9999.1.1.tgz"))).toBe(false);
      expect(fs.readFileSync(path.join(outputDir, "nodoassist-2026.6.17.tgz"), "utf8")).toBe(
        "current",
      );
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-package-timeout-"));
  ...
  it("clamps oversized kill grace before scheduling", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-package-grace-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-nodoassist-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid;
    try {
