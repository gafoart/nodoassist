// NodoAssist bundle MCP tools Docker harness.
...
server.tool("docker_probe", "Docker NodoAssist MCP tool availability probe", async () => ({
...
async function main() {
  const tempState = await createE2eStateDir("nodoassist-agent-bundle-mcp-");
