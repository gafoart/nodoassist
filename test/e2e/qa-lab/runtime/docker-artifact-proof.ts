...
async function runScheduler(options: ProducerOptions, appendLog: (chunk: unknown) => void) {
  ...
  let packageTgz = process.env.NODOASSIST_CURRENT_PACKAGE_TGZ;
  if (packageTgz) {
    const packageDir = path.join(dockerRunDir, "nodoassist-package");
    const evidencePackageTgz = path.join(packageDir, "nodoassist-current.tgz");
  ...
  return await new Promise<{
    ...
    const child = spawn(process.execPath, ["scripts/test-docker-all.mjs"], {
      ...
      env: {
        ...process.env,
        NODOASSIST_DOCKER_ALL_BUILD: "1",
        NODOASSIST_DOCKER_ALL_DRY_RUN: "0",
        NODOASSIST_DOCKER_ALL_LANES: options.lane,
        NODOASSIST_DOCKER_ALL_LOG_DIR: dockerRunDir,
        NODOASSIST_DOCKER_ALL_PARALLELISM: "1",
        NODOASSIST_DOCKER_ALL_PREFLIGHT: "1",
        NODOASSIST_DOCKER_ALL_TIMINGS_FILE: path.join(dockerRunDir, "lane-timings.json"),
        NODOASSIST_DOCKER_ARTIFACT_IDENTITY_PATH: identityPath,
        ...(packageTgz ? { NODOASSIST_CURRENT_PACKAGE_TGZ: packageTgz } : {}),
      },
...
async function runDockerArtifactProofProducer(
  ...
  try {
    ...
    const packagePath = path.join("docker-run", "nodoassist-package", "nodoassist-current.tgz");
