...
async function createFixturePlugin() {
  // nodoassist-temp-dir: allow standalone producer cleans this root in each scenario finally block
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-mcp-fixture-"));
...
function withFixturePlugin(config: NodoAssistConfig, pluginDir: string): NodoAssistConfig {
    ...
    return {
      args: [
        ...
        [
          ...
          `.then((module) => module.serveNodoAssistChannelMcp({`,
          `gatewayUrl: process.env.NODOASSIST_QA_GATEWAY_URL,`,
          `gatewayToken: process.env.NODOASSIST_QA_GATEWAY_TOKEN,`,
      ...
      cwd: params.repoRoot,
      envPatch: {
        NODOASSIST_QA_GATEWAY_TOKEN: params.gatewayToken,
        NODOASSIST_QA_GATEWAY_URL: params.gatewayUrl,
      },
    };
  }

  throw new Error(
    "NodoAssist channel MCP entry not found: expected dist/index.(m)js or src/mcp/channel-server.ts",
  );
}
...
async function connectChannelMcpClient(params: {
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      NODOASSIST_LOG_LEVEL: "debug",
      NODOASSIST_STATE_DIR: tempState.stateDir,
    },
    stderr: "pipe",
  });
...
async function writePluginToolsConfig(root: string, pluginDir: string) {
  const configPath = path.join(root, "nodoassist.json");
  const config = withFixturePlugin({} as NodoAssistConfig, pluginDir);
...
async function runMcpPluginToolsProof(options: ProducerOptions): Promise<string> {
  const fixture = await createFixturePlugin();
  // nodoassist-temp-dir: allow standalone producer cleans and verifies this root in its finally block
  const runtimeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-plugin-tools-mcp-"));
