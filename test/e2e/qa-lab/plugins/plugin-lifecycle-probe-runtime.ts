...
type ProbeEnv = Pick<NodeJS.ProcessEnv, "HOME" | "NODOASSIST_CONFIG_PATH" | "NODOASSIST_STATE_DIR">;
...
function stateDir(env: ProbeEnv = process.env) {
  return env.NODOASSIST_STATE_DIR || path.join(env.HOME ?? os.homedir(), ".nodoassist");
}

function configPath(env: ProbeEnv = process.env) {
  return env.NODOASSIST_CONFIG_PATH || path.join(stateDir(env), "nodoassist.json");
}
...
function createMatrixStateEnv(resourceDir: string): MatrixEnv {
  ...
  const configFile = path.join(stateDir, "nodoassist.json");
  ...
  return {
    ...
    NODOASSIST_CONFIG_PATH: configFile,
    NODOASSIST_TEST_WORKSPACE_DIR: workspaceDir,
    NODOASSIST_AUTH_PROFILE_SECRET_KEY: randomBytes(32).toString("hex"),
  };
}
...
function packageEntrypoint(prefix: string) {
  ...
  throw new Error(`NodoAssist package entrypoint not found under ${packageRoot}/dist/`);
}
...
async function installNodoAssistPackage(prefix: string, env: MatrixEnv) {
  const packageTgz = env.NODOASSIST_CURRENT_PACKAGE_TGZ;
  assertProbe(packageTgz, "NODOASSIST_CURRENT_PACKAGE_TGZ is required");
  const installLog = "/tmp/nodoassist-plugin-lifecycle-install.log";
  process.stdout.write("Installing mounted NodoAssist package...\n");
  await runCommand(
    "npm",
    ["install", "-g", "--prefix", prefix, packageTgz, "--no-fund", "--no-audit"],
    {
      ...
      timeoutMs: parseDurationMs(env.NODOASSIST_E2E_NPM_INSTALL_TIMEOUT, "600s"),
    },
  );
}
...
async function runPluginLifecycleMatrix() {
  ...
  const packageName = "@nodoassist/lifecycle-claw";
  const resourceDir = tempDirs.make("nodoassist-plugin-lifecycle-matrix-");
  ...
  try {
    await installNodoAssistPackage(npmPrefix, env);
