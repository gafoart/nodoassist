...
const DEFAULT_TARBALL_ENV = "NODOASSIST_QA_RELEASE_CANDIDATE_TARBALL";
const CHECKOUT_BUILD_RESULT_PREFIX = "__NODOASSIST_QA_RELEASE_CANDIDATE_TARBALL__";
...
function parseOptions(
  ...
  let artifactBase = "";
  let buildFromCheckout = env.NODOASSIST_QA_RELEASE_CANDIDATE_BUILD === "1";
...
async function buildCandidateTarballFromCheckout(options: ProducerOptions) {
  ...
  const evalScript = `
    import { packNodoAssist } from "./scripts/e2e/parallels/package-artifact.ts";
    const artifact = await packNodoAssist({ destination: ${JSON.stringify(destination)} });
