...
type OfficialChannelInstall = NonNullable<
  NonNullable<OfficialChannelCatalogEntry["nodoassist"]>["install"]
>;
...
function requireInstall(entry: OfficialChannelCatalogEntry | undefined): OfficialChannelInstall {
  const install = entry?.nodoassist?.install;
  if (!install) {
...
function summarizeCatalogEntry(entry: OfficialChannelCatalogEntry) {
  return {
    ...
    source: entry.source,
    plugin: entry.nodoassist?.plugin,
    channel: entry.nodoassist?.channel,
    install: entry.nodoassist?.install,
  };
}
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("nodoassist-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@nodoassist/whatsapp",
      version: "2026.3.23",
      description: "NodoAssist WhatsApp channel plugin",
      nodoassist: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@wecom/wecom-nodoassist-plugin"),
      ),
    ).toEqual({
      name: "@wecom/wecom-nodoassist-plugin",
      description: "NodoAssist WeCom channel plugin by the Tencent WeCom team.",
      source: "external",
      plugin: {
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "nodoassist-plugin-yuanbao"),
      ),
    ).toEqual({
      name: "nodoassist-plugin-yuanbao",
      description: "NodoAssist Yuanbao channel plugin by the Tencent Yuanbao team.",
      source: "external",
      plugin: {
        id: "nodoassist-plugin-yuanbao",
        label: "Yuanbao",
      },
      channel: {
        id: "yuanbao",
    ...
    expect(
      summarizeCatalogEntry(
        findCatalogEntry(entries, (entry) => entry.name === "@nodoassist/whatsapp"),
      ),
    ).toEqual({
      name: "@nodoassist/whatsapp",
      description: "NodoAssist WhatsApp channel plugin",
  ...
  it("keeps third-party official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("nodoassist-official-channel-catalog-policy-");
    const entries = buildOfficialChannelCatalog({ repoRoot }).entries.filter(
      (entry) => entry.source === "external" && !entry.name?.startsWith("@nodoassist/"),
    );
  ...
  it("allows official NodoAssist channel npm specs without integrity during launch", () => {
    const repoRoot = makeRepoRoot("nodoassist-official-channel-catalog-nodoassist-policy-");
    const twitch = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (entry) => entry.nodoassist?.channel?.id === "twitch",
    );

    expect({
      name: twitch?.name,
      install: twitch?.nodoassist?.install,
    }).toEqual({
  ...
  it("preserves ClawHub specs when generating publishable channel catalog entries", () => {
    const repoRoot = makeRepoRoot("nodoassist-official-channel-catalog-clawhub-");
    writeJson(path.join(repoRoot, "extensions", "storepack-chat", "package.json"), {
      name: "@nodoassist/storepack-chat",
      nodoassist: {
        channel: {
          id: "storepack-chat",
        ...
        install: {
          clawhubSpec: "clawhub:@nodoassist/storepack-chat",
          npmSpec: "@nodoassist/storepack-chat",
          defaultChoice: "clawhub",
        },
        release: {
          publishToNpm: true,
    ...
    const entry = buildOfficialChannelCatalog({ repoRoot }).entries.find(
      (candidate) => candidate.nodoassist?.channel?.id === "storepack-chat",
    );
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("nodoassist-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@nodoassist/whatsapp",
      nodoassist: {
        channel: {
    ...
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "@wecom/wecom-nodoassist-plugin",
    );
    expect(entries.map((entry: { name?: string }) => entry.name)).toContain(
      "nodoassist-plugin-yuanbao",
    );
    const whatsappEntry = findCatalogEntry(
      entries,
      (entry: { nodoassist?: { channel?: { id?: string } } }) =>
        entry.nodoassist?.channel?.id === "whatsapp",
    );
