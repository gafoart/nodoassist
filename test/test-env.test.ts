...
function createTempHome(): string {
  return makeTempDir(tempDirs, "nodoassist-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-nodoassist.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.nodoassist/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.nodoassist/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".nodoassist", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".nodoassist", "external-plugins", "glueclaw", "nodoassist.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".nodoassist", "agents", "main", "agent", "auth-profiles.json"),
    ...
    setTestEnvValue("NODOASSIST_LIVE_TEST", "1");
    setTestEnvValue("NODOASSIST_LIVE_TEST_QUIET", "1");
    setTestEnvValue("NODOASSIST_CONFIG_PATH", "~/custom-nodoassist.json5");
    setTestEnvValue("NODOASSIST_TEST_HOME", priorIsolatedHome);
    setTestEnvValue("NODOASSIST_STATE_DIR", path.join(priorIsolatedHome, ".nodoassist"));
    ...
    expect(process.env.NODOASSIST_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".nodoassist", "nodoassist.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".nodoassist", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".nodoassist", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    setTestEnvValue("USERPROFILE", realHome);
    setTestEnvValue("NODOASSIST_LIVE_TEST", "1");
    setTestEnvValue("NODOASSIST_LIVE_USE_REAL_HOME", "1");
    setTestEnvValue("NODOASSIST_LIVE_TEST_QUIET", "1");
  ...
  it("keeps hermetic mode isolated when live flags request the real HOME", () => {
    const realHome = createTempHome();
    writeFile(path.join(realHome, ".profile"), "export TEST_PROFILE_ONLY=from-profile\n");
    writeFile(path.join(realHome, ".nodoassist", "nodoassist.json"), '{"live":true}\n');
    writeFile(path.join(realHome, ".nodoassist", "credentials", "token.txt"), "secret\n");
    ...
    setTestEnvValue("NODOASSIST_LIVE_TEST", "1");
    setTestEnvValue("NODOASSIST_LIVE_GATEWAY", "1");
    setTestEnvValue("NODOASSIST_LIVE_USE_REAL_HOME", "1");
    const callerPluginDir = path.join(realHome, "caller-plugins");
    setTestEnvValue("NODOASSIST_BUNDLED_PLUGINS_DIR", callerPluginDir);
    setTestEnvValue("NODOASSIST_TEST_TRUST_BUNDLED_PLUGINS_DIR", "1");
    setTestEnvValue("NODOASSIST_DISABLE_BUNDLED_PLUGINS", "1");
    setTestEnvValue("NODOASSIST_HOME", realHome);
    ...
    expect(process.env.NODOASSIST_LIVE_TEST).toBeUndefined();
    expect(process.env.NODOASSIST_LIVE_GATEWAY).toBeUndefined();
    expect(process.env.NODOASSIST_LIVE_USE_REAL_HOME).toBeUndefined();
    expect(process.env.NODOASSIST_BUNDLED_PLUGINS_DIR).not.toBe(callerPluginDir);
    expect(path.basename(process.env.NODOASSIST_BUNDLED_PLUGINS_DIR ?? "")).toBe("extensions");
    expect(process.env.NODOASSIST_TEST_TRUST_BUNDLED_PLUGINS_DIR).toBe("1");
    expect(process.env.NODOASSIST_DISABLE_BUNDLED_PLUGINS).toBeUndefined();
    expect(process.env.NODOASSIST_HOME).toBeUndefined();
    expect(fs.existsSync(path.join(testEnv.tempHome, ".nodoassist", "nodoassist.json"))).toBe(
      false,
    );
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".nodoassist", "credentials", "token.txt")),
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    deleteTestEnvValue("NODOASSIST_LIVE_TEST");
    deleteTestEnvValue("NODOASSIST_LIVE_GATEWAY");
    deleteTestEnvValue("NODOASSIST_LIVE_USE_REAL_HOME");
    deleteTestEnvValue("NODOASSIST_LIVE_TEST_QUIET");
