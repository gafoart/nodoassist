...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.NODOASSIST_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through nodoassist infer", () => {
    const outputBase = path.join(os.tmpdir(), `nodoassist-infer-image-${process.pid}.png`);
