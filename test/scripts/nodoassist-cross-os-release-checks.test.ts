// NodoAssist Cross Os Release Checks tests cover nodoassist cross os release checks script behavior.
...
} from "../../scripts/nodoassist-cross-os-release-checks.ts";
...
describe("scripts/nodoassist-cross-os-release-checks", () => {
  ...
  it("requires dashboard root markers and same-origin asset URLs", () => {
    const html = [
      "<title>NodoAssist Control</title>",
      "<nodoassist-app></nodoassist-app>",
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: nodoassist gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("gives the Windows packaged updater wrapper enough headroom for NodoAssist timeout output", () => {
  ...
  it("rejects malformed cross-OS positive integer environment values", () => {
    expect(parsePositiveIntegerEnv("NODOASSIST_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {})).toBe(
      60,
    );
    expect(
      parsePositiveIntegerEnv("NODOASSIST_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
        NODOASSIST_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: "25",
      }),
    ).toBe(25);
    ...
    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() =>
        parsePositiveIntegerEnv("NODOASSIST_CROSS_OS_COMMAND_HEARTBEAT_SECONDS", 60, {
          NODOASSIST_CROSS_OS_COMMAND_HEARTBEAT_SECONDS: raw,
        }),
      ).toThrow("NODOASSIST_CROSS_OS_COMMAND_HEARTBEAT_SECONDS must be a positive integer");
    }
  });

  it("records packaged-fresh phase timings for release-check summaries", () => {
    const source = readFileSync("scripts/nodoassist-cross-os-release-checks.ts", "utf8");
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-agent-output-"));
    try {
  ...
  it("ignores stale OK markers outside the recent agent log tail", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-agent-output-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("requires explicit opt-in before cross-OS agent turns become optional", () => {
    ...
    expect(resolveCrossOsAgentTurnOptional({ NODOASSIST_CROSS_OS_AGENT_TURN_OPTIONAL: "1" })).toBe(
      true,
    );
    expect(
      resolveCrossOsAgentTurnOptional({ NODOASSIST_CROSS_OS_AGENT_TURN_OPTIONAL: "false" }),
    ).toBe(false);
  });

  it("detects embedded fallback agent turns as non-gateway proof", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-agent-fallback-"));
    const logPath = join(dir, "agent.log");
    expect(
      agentTurnUsedEmbeddedFallback({
  ...
  it("skips optional live agent turns only for model availability failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-agent-skip-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("does not classify stale timeout logs as current optional agent-turn failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-agent-skip-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-agent-skip-retry-"));
    try {
      const logPath = join(dir, "agent.log");
  ...
  it("allows cross-OS provider smoke models to use faster CI overrides", () => {
    expect(
      resolveProviderConfig("openai", {
        NODOASSIST_CROSS_OS_OPENAI_MODEL: "openai/gpt-5.4-mini",
      })?.model,
    ).toBe("openai/gpt-5.4-mini");
    expect(
      resolveProviderConfig("openai", {
        NODOASSIST_CROSS_OS_MODEL: "openai/gpt-5.4-nano",
      })?.model,
  ...
  it("keeps release cross-OS OpenAI smoke on GPT-5.5", () => {
    ...
    const releaseChecks = readFileSync(".github/workflows/nodoassist-release-checks.yml", "utf8");

    expect(workflow).toContain(
      "NODOASSIST_CROSS_OS_OPENAI_MODEL: ${{ inputs.openai_model || vars.NODOASSIST_CROSS_OS_OPENAI_MODEL || 'openai/gpt-5.5' }}",
    );
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("nodoassist@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "nodoassist@2026.5.2",
      "--omit=dev",
  ...
  it("rejects unsafe npm pack tarball filenames before staging release artifacts", () => {
    expect(resolveNpmPackTarballFileName("nodoassist-2026.6.17.tgz")).toBe(
      "nodoassist-2026.6.17.tgz",
    );
  ...
  it("accepts pnpm pack tarballs reported under the requested destination", () => {
    const packDir = resolvePath("/tmp/nodoassist-pack");

    expect(resolvePackDestinationTarball("nodoassist-2026.6.17.tgz", packDir, "pnpm pack")).toEqual(
      {
        fileName: "nodoassist-2026.6.17.tgz",
        path: resolvePath(packDir, "nodoassist-2026.6.17.tgz"),
      },
    );
    expect(
      resolvePackDestinationTarball(
        resolvePath(packDir, "nodoassist-2026.6.17.tgz"),
        packDir,
        "pnpm pack",
      ),
    ).toEqual({
  ...
  it("rejects pnpm pack tarballs outside the requested destination", () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/nodoassist.tgz",
      "nested\\nodoassist.tgz",
      resolvePath(dirname(packDir), "nodoassist.tgz"),
      resolvePath(packDir, "nested", "nodoassist.tgz"),
      "nodoassist\u0000.tgz",
      "nodoassist.tar.gz",
    ];
  ...
  it("falls back to pnpm pack for historical refs without the Docker package helper", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-pack-command-"));
    try {
      ...
      const helperPath = join(dir, "scripts", "package-nodoassist-for-docker.mjs");
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "nodoassist" }');
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'nodoassist.cmd')");
    expect(script).toContain("$cmd = Get-Command nodoassist -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        NODOASSIST_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        NODOASSIST_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        NODOASSIST_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("nodoassist-2026.4.14.tgz")).toBe(
      "application/octet-stream",
    );
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-static-server-"));
    const filePath = join(dir, "nodoassist-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-static-server-close-"));
    const filePath = join(dir, "nodoassist-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("flushes static release artifact logs before close resolves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-static-server-log-flush-"));
    const filePath = join(dir, "nodoassist-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "nodoassist/plugin-sdk/plugin-runtime"');
    expect(script).toContain('overrideEnvVar: "NODOASSIST_BROWSER_CONTROL_MODULE"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/nodoassist/dist/plugin-sdk/plugin-runtime.js",
    );
    expect(installedScript).toContain(
      'from "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/nodoassist/dist/plugin-sdk/plugin-runtime.js"',
    );
    expect(readFileSync("scripts/nodoassist-cross-os-release-checks.ts", "utf8")).toContain(
      "NODOASSIST_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href",
    );
  });
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      resolveCommandSpawnInvocation(
        String.raw`C:\Program Files\nodejs\npm.cmd`,
        ["view", "nodoassist@latest", "version"],
        {
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view nodoassist@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\NodoAssist Prefix`, "nodoassist.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\NodoAssist Prefix\nodoassist.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-run-command-"));
    try {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-run-command-output-"));
    try {
      const logPath = join(dir, "command.log");
  ...
  it("flushes command logs before resolving", async () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-run-command-flush-"));
    try {
  ...
  it("reads npm debug logs from the Windows cache root", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-npm-debug-"));
    try {
  ...
  it("prefers npm configured log directories over cache defaults", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-npm-logs-dir-"));
    try {
      const homeDir = join(dir, "home");
  ...
  it("keeps npm debug log collection best-effort", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-npm-debug-best-effort-"));
    try {
      const homeDir = join(dir, "home");
      const logPath = join(dir, "install.log");
  ...
  it("resolves relative npm log config from the install working directory", () => {
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-npm-relative-logs-"));
    try {
      const homeDir = join(dir, "home");
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-run-command-timeout-"));
    ...
    try {
      ...
      const parentScript = [
        ...
        "fs.writeFileSync(process.env.NODOASSIST_TEST_CHILD_PID, String(child.pid));",
        "setInterval(() => {}, 1000);",
      ].join("");

      const command = runCommand(process.execPath, ["-e", parentScript], {
        cwd: dir,
        env: { ...process.env, NODOASSIST_TEST_CHILD_PID: childPidPath },
        logPath,
        timeoutMs: 500,
      });
  ...
  it("forwards external termination to command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-run-command-signal-"));
    const childPidPath = join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      resolvePath("scripts/nodoassist-cross-os-release-checks.ts"),
    ).href;
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            NODOASSIST_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "200",
            NODOASSIST_TEST_CHILD_PID: childPidPath,
          },
  ...
  it("exits promptly after externally signaled commands close", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-run-command-signal-exit-"));
    const childPidPath = join(dir, "child.pid");
    ...
    try {
      ...
      const runner = spawn(
        ...
        ["--import", "tsx", "--input-type=module", "-e", runnerScript],
        {
          ...
          env: {
            ...process.env,
            NODOASSIST_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS: "3000",
            NODOASSIST_TEST_CHILD_PID: childPidPath,
          },
          stdio: ["ignore", "ignore", "pipe"],
        },
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/nodoassist", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-linux-home-"));
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "nodoassist");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "nodoassist" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "nodoassist"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "nodoassist"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "nodoassist"));
  ...
  it("drops the bundled plugin postinstall disable flag for real updater calls", () => {
    expect(
      buildRealUpdateEnv({
        FOO: "bar",
        NODE_COMPILE_CACHE: "/tmp/stale-nodoassist-cache",
        NODOASSIST_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
      }),
    ).toEqual({
      FOO: "bar",
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[nodoassist] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/nodoassist/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\nodoassist-upgrade-q9DsA7\\prefix\\node_modules\\nodoassist\\nodoassist.mjs update --tag http://127.0.0.1:49951/openclaw-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\nodoassist\\nodoassist.mjs update --tag http://127.0.0.1:49951/openclaw-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node nodoassist.mjs update --tag nodoassist@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.nodoassist-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("nodoassist@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("nodoassist@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-installed-version-"));
    try {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "nodoassist")
          : join(prefixDir, "lib", "node_modules", "nodoassist");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(
          packageRoot,
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "nodoassist-cross-os-local-stamps-"));
    try {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "nodoassist-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
