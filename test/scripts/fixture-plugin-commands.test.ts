...
describe("plugin fixture commands", () => {
  it("writes plugin fixtures with manifest and package metadata", () => {
    const root = makeTempDir(tempDirs, "nodoassist-fixture-plugin-");
    ...
    expect(readJson(path.join(pluginRoot, "nodoassist.plugin.json"))).toMatchObject({
      id: "demo-plugin",
  ...
  it("writes CLI plugin fixtures with local dependency metadata", () => {
    const root = makeTempDir(tempDirs, "nodoassist-fixture-plugin-cli-");
  ...
  it("rejects plugin fixture commands with missing required args", () => {
    const root = makeTempDir(tempDirs, "nodoassist-fixture-plugin-missing-");
