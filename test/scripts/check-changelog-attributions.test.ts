...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "nodoassist-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "nodoassist-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$NODOASSIST_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$NODOASSIST_TEST_CONTRIB"',
    ],
    {
      NODOASSIST_PR_CHANGELOG_SH: changelogScriptPath,
      NODOASSIST_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$NODOASSIST_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      NODOASSIST_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @nodoassist.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @NodoAssist.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "nodoassist", text: "- Org-owned fix. Thanks @nodoassist." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      {
        line: 4,
        handle: "nodoassist",
        text: "- Mixed credit. Thanks @contributor and @NodoAssist.",
      },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @nodoassist and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "nodoassist",
        text: "- Mixed credit (#123). Thanks @nodoassist and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("nodoassist")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("nodoassist-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("nodoassist-clawsweeper[bot]")).toBe(true);
      ...
      try {
        run(
          ...
          [
            ...
            `
set -euo pipefail
source "$NODOASSIST_PR_COMMON_SH"
source "$NODOASSIST_PR_CHANGELOG_SH"
source "$NODOASSIST_PR_GATES_SH"

enter_worktree() { :; }
checkout_prep_branch() { :; }
bootstrap_deps_if_needed() { :; }
require_artifact() { [ -s "$1" ]; }
normalize_pr_changelog_entries() { printf 'normalize\\n' >>"$NODOASSIST_TEST_CALLS"; }
validate_changelog_attribution_policy() { printf 'policy\\n' >>"$NODOASSIST_TEST_CALLS"; }
validate_changelog_merge_hygiene() { printf 'merge-hygiene\\n' >>"$NODOASSIST_TEST_CALLS"; }
validate_changelog_entry_for_pr() { printf 'entry:%s:%s\\n' "$1" "$2" >>"$NODOASSIST_TEST_CALLS"; }
run_quiet_logged() { printf 'gate:%s\\n' "$1" >>"$NODOASSIST_TEST_CALLS"; }

prepare_gates 123
`,
          ],
          {
            NODOASSIST_PR_COMMON_SH: commonScriptPath,
            NODOASSIST_PR_CHANGELOG_SH: changelogScriptPath,
            NODOASSIST_PR_GATES_SH: gatesScriptPath,
            NODOASSIST_TEST_CALLS: callsPath,
            NODOASSIST_TESTBOX: "0",
          },
        );
      } catch (error) {
...
`,
        ...
        {
          NODOASSIST_ALLOW_ROOT_CHANGELOG_PR: "1",
          NODOASSIST_PR_COMMON_SH: commonScriptPath,
          NODOASSIST_PR_CHANGELOG_SH: changelogScriptPath,
          NODOASSIST_PR_GATES_SH: gatesScriptPath,
          NODOASSIST_TEST_CALLS: callsPath,
