...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ClawHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "NODOASSIST_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@nodoassist/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@nodoassist/kitchen-sink@beta");
    expect(script).toContain("clawhub:@nodoassist/kitchen-sink@latest");
    expect(script).toContain("clawhub:@nodoassist/kitchen-sink@beta");
    expect(script).toContain("NODOASSIST_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    ...
    expect(sweepScript).toContain("NODOASSIST_KITCHEN_SINK_PERSONALITY");
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("NODOASSIST_ENTRY=/app/nodoassist.mjs");
    expect(script).toContain("NODOASSIST_KITCHEN_SINK_COMMAND_MAX_RSS_MIB");
    expect(script).toContain("docker_e2e_sample_stats_until_exit");
    expect(script).toContain("scripts/e2e/lib/docker-stats/assert-resource-ceiling.mjs");
    ...
    expect(walkScript).toContain("usesBuiltNodoAssistEntry(runner)");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    expect(lane).toEqual({
      command: "NODOASSIST_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
    ...
    expect(assertionsScript).toContain("assertClawHubExternalInstallContract");
    expect(assertionsScript).toContain('node_modules", "nodoassist');
    expect(fixtureServer).toContain('"is-number": "7.0.0"');
    expect(fixtureServer).toContain('nodoassist: ">=2026.4.11"');
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(manifestEnv).toEqual({
      NODOASSIST_CI_CHECKOUT_REVISION: "${{ steps.checkout_ref.outputs.sha }}",
      NODOASSIST_CI_DOCS_CHANGED:
        "${{ github.event_name == 'workflow_dispatch' && 'true' || steps.docs_scope.outputs.docs_changed }}",
      NODOASSIST_CI_DOCS_ONLY:
        "${{ github.event_name == 'workflow_dispatch' && 'false' || steps.docs_scope.outputs.docs_only }}",
      NODOASSIST_CI_EVENT_NAME: "${{ github.event_name }}",
      NODOASSIST_CI_REPOSITORY: "${{ github.repository }}",
      NODOASSIST_CI_RUN_ANDROID:
    ...
    expect(inspectorRun.env).toEqual({
      NODOASSIST_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      NODOASSIST_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@nodoassist/plugin-inspector@${NODOASSIST_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/nodoassist-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "nodoassist-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
    ...
    expect(dockerPreflightStep?.run).toContain(
      '--build-arg NODOASSIST_EXTENSIONS="diagnostics-otel,codex"',
    );
    expect(
    ...
    expect(runtimeToolCoverage.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          run: expect.stringContaining("pnpm nodoassist qa coverage"),
        }),
      ]),
    );
    expect(runtimeToolCoverage.steps).toEqual(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, NODOASSIST_DISCORD_TOKEN)");
