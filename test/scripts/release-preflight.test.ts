...
function makeFakePnpm(): { binDir: string; logPath: string } {
  const root = makeTempDir(tempDirs, "nodoassist-release-preflight-");
...
appendFileSync(process.env.NODOASSIST_RELEASE_PREFLIGHT_PNPM_LOG, command + "\\n");
const failures = new Set((process.env.NODOASSIST_RELEASE_PREFLIGHT_FAIL_COMMANDS ?? "").split(";").filter(Boolean));
...
function runPreflight(
  ...
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      ...(fakePnpm
        ? {
            NODOASSIST_RELEASE_PREFLIGHT_PNPM_LOG: fakePnpm.logPath,
...
describe("scripts/release-preflight.mjs", () => {
  ...
  it("runs every check command and reports all failed release artifact checks", () => {
    ...
    const result = runPreflight(["--check"], fakePnpm, {
      NODOASSIST_RELEASE_PREFLIGHT_FAIL_COMMANDS: "plugins:sync:check;config:docs:check",
    });
  ...
  it("stops refresh mode at the first failed generator before running checks", () => {
    ...
    const result = spawnSync(process.execPath, [SCRIPT, "--fix"], {
      ...
      env: {
        ...process.env,
        NODOASSIST_RELEASE_PREFLIGHT_FAIL_COMMANDS: "deps:shrinkwrap:changed:generate",
        NODOASSIST_RELEASE_PREFLIGHT_PNPM_LOG: fakePnpm.logPath,
        PATH: `${fakePnpm.binDir}${delimiter}${process.env.PATH ?? ""}`,
      },
    });
