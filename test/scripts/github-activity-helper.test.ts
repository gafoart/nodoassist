...
const helperPath = path.join(
  repoRoot,
  ".agents/skills/nodoassist-pr-maintainer/scripts/github-activity.sh",
);
...
describe("nodoassist-pr-maintainer github activity helper", () => {
  it("counts PRs and issues from one paginated issues response", () => {
    ...
    expect(result.stdout).toContain("nodoassist/nodoassist last 1mo: 2 PRs, 1 issues, 2 commits");
    expect(log.match(/repos\/nodoassist\/nodoassist\/issues/g)).toHaveLength(1);
    expect(log.match(/repos\/nodoassist\/nodoassist\/commits/g)).toHaveLength(1);
