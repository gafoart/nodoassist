...
describe("check-deadcode-unused-files", () => {
  it("parses the compact Knip unused-file section", () => {
    expect(
      parseKnipCompactUnusedFiles(`
> nodoassist@2026.4.27 deadcode:knip /repo
...
`),
  ...
  it("runs Knip through a process-group-aware subprocess", async () => {
    const calls: unknown[] = [];
    const root = mkdtempSync(path.join(os.tmpdir(), "nodoassist-knip-runner-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(os.tmpdir(), "nodoassist-knip-timeout-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = mkdtempSync(path.join(os.tmpdir(), "nodoassist-knip-parent-signal-"));
      const childPidPath = path.join(root, "child.pid");
