...
describe("k8s manifests", () => {
  ...
  it("keeps gateway service selectors and ports aligned with deployment labels", () => {
    ...
    expect(deployment).toMatchObject({
      apiVersion: "apps/v1",
      kind: "Deployment",
      metadata: { name: "nodoassist" },
    });
    expect(matchLabels).toEqual({ app: "nodoassist" });
  ...
  it("keeps deployment mounts, secrets, and security posture deployable", () => {
    ...
    expect(findNamed(env, "NODOASSIST_CONFIG_DIR")).toMatchObject({
      value: "/home/node/.nodoassist",
    });
    expect(findNamed(env, "NODOASSIST_GATEWAY_TOKEN")).toMatchObject({
      valueFrom: { secretKeyRef: { key: "NODOASSIST_GATEWAY_TOKEN", name: "nodoassist-secrets" } },
    });
    expect(findNamed(volumes, "nodoassist-home")).toMatchObject({
      persistentVolumeClaim: { claimName: "nodoassist-home-pvc" },
    });
    expect(findNamed(volumes, "config")).toMatchObject({
      configMap: { name: "nodoassist-config" },
    });
  ...
  it("keeps config and persistence manifests aligned with the gateway", () => {
    ...
    const config = JSON.parse(String(data["nodoassist.json"])) as Record<string, unknown>;
    const gateway = asRecord(config.gateway, "nodoassist config gateway");
    const auth = asRecord(gateway.auth, "nodoassist config auth");
    const agents = asRecord(config.agents, "nodoassist config agents");
    const defaults = asRecord(agents.defaults, "nodoassist config agent defaults");
    ...
    expect(configMap).toMatchObject({
      apiVersion: "v1",
      kind: "ConfigMap",
      metadata: { name: "nodoassist-config" },
    });
    expect(gateway).toMatchObject({ mode: "local", port: 18789 });
    expect(auth).toMatchObject({ mode: "token" });
    expect(defaults).toMatchObject({ workspace: "~/.nodoassist/workspace" });
    expect(data["AGENTS.md"]).toContain("NodoAssist Assistant");
    expect(pvc).toMatchObject({
      apiVersion: "v1",
      kind: "PersistentVolumeClaim",
      metadata: { name: "nodoassist-home-pvc" },
    });
