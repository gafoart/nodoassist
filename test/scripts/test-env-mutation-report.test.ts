...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("nodoassist-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.NODOASSIST_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.NODOASSIST_STATE_DIR = "state";
delete process.env["NODOASSIST_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("NODOASSIST_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, NODOASSIST_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "NODOASSIST_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.NODOASSIST_HOME = "not code"';
process.env.NOT_NODOASSIST = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.NODOASSIST_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.NODOASSIST_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/nodoassist-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active NodoAssist env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
    ...
    ).toEqual([
      {
        ...
        key: "NODOASSIST_CONFIG_PATH",
        operation: "assign",
      },
      {
        file: "src/agents/auth-profiles/oauth-test-utils.ts",
        key: "NODOASSIST_AGENT_DIR",
        operation: "assign",
      },
      { file: "src/example.test.ts", key: "NODOASSIST_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "NODOASSIST_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "NODOASSIST_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "NODOASSIST_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("NodoAssist test env mutation report");
    ...
    expect(rendered).toContain("L2 NODOASSIST_STATE_DIR assign process.env");
    ...
    for (const limit of ["1e3", ""]) {
      expect(() =>
        runTestEnvMutationReport([
          ...
          limit,
          "--repo-root",
          createTempDir("nodoassist-env-limit-"),
        ]),
