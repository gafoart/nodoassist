...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "nodoassist-dist-plist-"));
  tempDirs.push(dir);
...
describe("package-mac-dist plist validation", () => {
  it("fails closed for required Info.plist reads", () => {
    ...
    const readBlock = script.slice(
      ...
      script.indexOf('ZIP="$ROOT_DIR/dist/NodoAssist-$VERSION.zip"'),
    );
  ...
  it("fails on old Swift before reading package metadata", () => {
    const toolsDir = mkdtempSync(path.join(tmpdir(), "nodoassist-dist-swift-tools-"));
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "nodoassist-dist-pnpm-root-"));
    const outerRoot = mkdtempSync(path.join(tmpdir(), "nodoassist-dist-pnpm-outer-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "nodoassist-dist-pnpm-tools-"));
  ...
  it("keeps dependency bootstrap output out of captured Sparkle build values", () => {
    ...
    const dir = mkdtempSync(path.join(tmpdir(), "nodoassist-dist-sparkle-"));
    tempDirs.push(dir);
    ...
    const result = runHelper(`
      ...
      NODOASSIST_ROOT=${JSON.stringify(process.cwd())}
      NODOASSIST_MARKER=${JSON.stringify(marker)}
      PATH=${JSON.stringify(tools)}:/usr/bin:/bin
      export NODOASSIST_MARKER NODOASSIST_ROOT PATH
      ${helpers}
