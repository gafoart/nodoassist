...
function mockClawtributorsFixture() {
  ...
  const execSync = vi.fn((cmd: string) => {
    if (
      cmd === 'gh api "repos/nodoassist/nodoassist/contributors?per_page=100&anon=1" --paginate'
    ) {
    ...
    if (
      cmd ===
      "gh pr list -R nodoassist/nodoassist --state merged --limit 5000 --json author --jq '.[].author.login'"
    ) {
      return "";
    }
