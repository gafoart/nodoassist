...
function successfulRun(name: string, id: number, updatedAt: string) {
  return {
    ...
    head_repository: { full_name: "nodoassist/nodoassist" },
...
describe("verify-pr-hosted-gates", () => {
  ...
  it("parses required CLI arguments", () => {
    ...
    expect(() => parseArgs(["--repo", "nodoassist/nodoassist"])).toThrow("Usage:");
  ...
  it("rejects duplicate hosted gate verifier CLI arguments", () => {
    const duplicateCases = [
      ["--repo", [...requiredCliArgs, "--repo", "fork/nodoassist"]],
  ...
  it("queries the target and recorded pre-rebase SHAs", () => {
    const previousSha = "8d86c44c6144f8f726a460914cddb8c9c201f119";
    expect(
      workflowRunQueryPaths("nodoassist/nodoassist", {
        sha,
        recentSha: previousSha,
      }),
    ).toEqual([
      `repos/nodoassist/nodoassist/actions/runs?head_sha=${sha}&per_page=100&page=1`,
      `repos/nodoassist/nodoassist/actions/runs?head_sha=${previousSha}&per_page=100&page=1`,
    ]);
