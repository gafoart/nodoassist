...
import {
  ...
  isReleaseBefore,
  resolveUpgradeSurvivorNodoAssistCommand,
  runUpgradeSurvivorNodoAssistStep,
...
describe("upgrade survivor config recipe command resolution", () => {
  ...
  it("wraps Windows nodoassist npm shims through cmd.exe", () => {
    expect(
      resolveUpgradeSurvivorNodoAssistCommand(
      ...
      args: [
        ...
        "/s",
        "/c",
        'nodoassist.cmd config set models.providers.openai "{""apiKey"":""sk test""}" --strict-json',
      ],
      command: String.raw`C:\Windows\System32\cmd.exe`,
      commandLabel:
        'nodoassist config set models.providers.openai {"apiKey":"sk test"} --strict-json',
      shell: false,
      windowsVerbatimArguments: true,
    });
  });

  it("keeps POSIX nodoassist invocations direct", () => {
    expect(
      resolveUpgradeSurvivorNodoAssistCommand(["config", "validate"], {
        platform: "linux",
      }),
    ).toEqual({
      args: ["config", "validate"],
      command: "nodoassist",
      commandLabel: "nodoassist config validate",
      shell: false,
    });
  });
  ...
  it("bounds baseline config commands and reports spawn errors", () => {
    const calls: unknown[] = [];
    const timeoutError = Object.assign(new Error("spawnSync nodoassist ETIMEDOUT"), {
      code: "ETIMEDOUT",
    });

    const outcome = runUpgradeSurvivorNodoAssistStep(
      {
    ...
    expect(outcome).toMatchObject({
      command: "nodoassist config validate",
      errorCode: "ETIMEDOUT",
      errorMessage: "spawnSync nodoassist ETIMEDOUT",
      ok: false,
      signal: "SIGTERM",
      status: null,
      stderr: "still validating",
  ...
  it("skips ACPX bridge config on baselines before the bridge field existed", () => {
    const root = mkdtempSync(join(tmpdir(), "nodoassist-upgrade-recipe-acpx-"));
    try {
      const binDir = join(root, "bin");
      const logPath = join(root, "nodoassist-argv.jsonl");
      const summaryPath = join(root, "summary.json");
      mkdirSync(binDir, { recursive: true });
      const nodoassistLogPath = join(binDir, "nodoassist-log.js");
      const nodoassistPath = join(binDir, "nodoassist");
      const nodoassistCmdPath = join(binDir, "nodoassist.cmd");
      writeFileSync(
        nodoassistLogPath,
        `
...
`,
      ...
      writeFileSync(nodoassistPath, `#!/usr/bin/env node\nrequire("./nodoassist-log.js");\n`);
      chmodSync(nodoassistPath, 0o755);
      writeFileSync(
        nodoassistCmdPath,
        `@echo off\r\n"${process.execPath}" "%~dp0nodoassist-log.js" %*\r\n`,
      );
      ...
      expect(summary.skippedIntents).toContain("acpx-nodoassist-tools-bridge");
      expect(loggedArgs).not.toContainEqual(
        expect.arrayContaining([
          "set",
          "plugins",
          expect.stringContaining("nodoAssistToolsMcpBridge"),
        ]),
      );
    } finally {
