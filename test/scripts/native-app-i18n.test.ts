...
describe("native app i18n inventory", () => {
  ...
  it("collects stable Android and Apple UI entries", async () => {
    ...
    expect(
      entries
        ...
        .every((entry) =>
          /^(?:apps\/ios|apps\/macos\/Sources|apps\/shared\/NodoAssistKit\/Sources)\//u.test(
            entry.path,
          ),
        ),
    ).toBe(true);
    ...
    expect(
      entries.some(
        (entry) =>
          ...
          entry.source === "Search NodoAssist",
      ),
    ).toBe(true);
    expect(
      entries.some(
    ...
    expect(entries.some((entry) => entry.source === "Check NodoAssist status")).toBe(true);
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Summarize the current NodoAssist status and tell me what needs attention.",
      ),
    ).toBe(true);
    expect(
      entries.some(
    ...
    expect(entries.some((entry) => entry.source === "ask NodoAssist $prompt")).toBe(true);
    expect(entries.some((entry) => entry.source === "NodoAssist is paused")).toBe(true);
    expect(
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source === 'NodoAssist needs ${labels.joinToString(", ")} permissions to continue.',
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Approve this device on the gateway.\n1) `\\(commandLine)`\n2) `/pair approve` in your NodoAssist chat\n\\(requestLine)\nNodoAssist will also retry automatically when you return to this app.",
      ),
    ).toBe(true);
    ...
    expect(
      entries.some((entry) =>
        entry.source.startsWith(
          "Exec approvals can only be reviewed while NodoAssist is open and connected.",
        ),
      ),
    ).toBe(true);
    expect(entries.some((entry) => entry.source === "$(PRODUCT_BUNDLE_IDENTIFIER)")).toBe(false);
    expect(entries.some((entry) => entry.source === "ai.nodoassist.screenRecord.writer")).toBe(
      false,
    );
    expect(
      entries.some(
  ...
  it("creates a first-run locale artifact and leaves a complete artifact unchanged", async () => {
    ...
    const translationsDir = makeTempDir(tempDirs, "nodoassist-native-i18n-");
