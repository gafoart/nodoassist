...
function makeTempDir(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-secret-provider-proof-"));
  tempDirs.push(root);
  return root;
}
...
function writeStallingNodoAssist(
  root: string,
  options: {
  ...
  const descendantScript = options.gatewayDescendantMarkerPath
    ...
    : "";
  const scriptPath = path.join(root, "fake-nodoassist.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
      ...
      "  process.exit(0);",
      "}",
      "console.error(`unexpected fake nodoassist args: ${args.join(' ')}`);",
      "process.exit(2);",
      "",
    ].join("\n"),
    { mode: 0o755 },
...
function writeLeakingStartupNodoAssist(root: string): string {
  const scriptPath = path.join(root, "fake-leaking-nodoassist.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeSignaledStartupNodoAssist(root: string): string {
  const scriptPath = path.join(root, "fake-signaled-nodoassist.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeNoisySecretsConfigureNodoAssist(root: string): string {
  const scriptPath = path.join(root, "fake-noisy-secrets-configure-nodoassist.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function runProofHarness(
  ...
  return spawnSync(process.execPath, [harnessPath, proofScriptPath, root, mode], {
    ...
    env: {
      ...process.env,
      NODOASSIST_ENTRY: fakeNodoAssist,
      NODOASSIST_SECRET_PROOF_READY_MS: "60",
      NODOASSIST_SECRET_PROOF_RPC_MS: "1000",
      ...envOverrides,
    },
    timeout: 5_000,
  });
...
describe("secret provider integration proof harness", () => {
  it("runs pnpm-backed NodoAssist commands through the repo pnpm runner", async () => {
    ...
    const command = await proof.resolveNodoAssistCommand(
      ["gateway", "status"],
      { ...process.env, NODOASSIST_SECRET_PROOF_SENTINEL: "1" },
      {
        nodeExecPath: "/opt/node/bin/node",
        npmExecPath: fakePnpm,
        runner: { pnpm: true, baseArgs: ["nodoassist"], label: "pnpm nodoassist" },
      },
    );

    expect(command.command).toBe("/opt/node/bin/node");
    expect(command.args).toEqual([fakePnpm, "nodoassist", "gateway", "status"]);
    expect(command.options.env.NODOASSIST_SECRET_PROOF_SENTINEL).toBe("1");
    expect(command.options.shell).toBe(false);
  });

  it("keeps stalled startup health probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeNodoAssist = writeStallingNodoAssist(root);
    const result = runProofHarness(root, fakeNodoAssist, "start");
  ...
  it("fails fast when startup exits by signal", () => {
    const root = makeTempDir();
    const fakeNodoAssist = writeSignaledStartupNodoAssist(root);
    const result = runProofHarness(root, fakeNodoAssist, "start", {
      NODOASSIST_SECRET_PROOF_READY_MS: "2000",
    });
  ...
  it("kills a stalled startup gateway before returning a readiness failure", async () => {
    ...
    const result = runProofHarness(root, fakeNodoAssist, "start", {
      NODOASSIST_SECRET_PROOF_TEARDOWN_GRACE_MS: "100",
    });
  ...
  it("bounds captured command output", async () => {
    const previousLimit = process.env.NODOASSIST_SECRET_PROOF_OUTPUT_BYTES;
    process.env.NODOASSIST_SECRET_PROOF_OUTPUT_BYTES = "1024";
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.NODOASSIST_SECRET_PROOF_OUTPUT_BYTES;
      } else {
        process.env.NODOASSIST_SECRET_PROOF_OUTPUT_BYTES = previousLimit;
      }
    }
  });

  it("clamps oversized command timeout env values before scheduling timers", async () => {
    const previousTimeout = process.env.NODOASSIST_SECRET_PROOF_COMMAND_MS;
    process.env.NODOASSIST_SECRET_PROOF_COMMAND_MS = String(Number.MAX_SAFE_INTEGER);
    try {
      const proof = await import(
      ...
      if (previousTimeout === undefined) {
        delete process.env.NODOASSIST_SECRET_PROOF_COMMAND_MS;
      } else {
        process.env.NODOASSIST_SECRET_PROOF_COMMAND_MS = previousTimeout;
      }
    }
  });
  ...
  it("blocks skipped secret proofs unless local rehearsals explicitly allow skips", async () => {
    const previousAllowSkips = process.env.NODOASSIST_SECRET_PROOF_ALLOW_SKIPS;
    const proof = await import(
    ...
    try {
      delete process.env.NODOASSIST_SECRET_PROOF_ALLOW_SKIPS;
      expect(proof.collectBlockingProofResults(entries)).toEqual(entries);

      process.env.NODOASSIST_SECRET_PROOF_ALLOW_SKIPS = "1";
      expect(proof.collectBlockingProofResults(entries)).toEqual([]);
    } finally {
      if (previousAllowSkips === undefined) {
        delete process.env.NODOASSIST_SECRET_PROOF_ALLOW_SKIPS;
      } else {
        process.env.NODOASSIST_SECRET_PROOF_ALLOW_SKIPS = previousAllowSkips;
      }
    }
  });
  ...
  it.runIf(process.platform !== "win32")("bounds captured PTY configure output", async () => {
    const root = makeTempDir();
    const fakeNodoAssist = writeNoisySecretsConfigureNodoAssist(root);
    const previousLimit = process.env.NODOASSIST_SECRET_PROOF_OUTPUT_BYTES;
    const previousEntry = process.env.NODOASSIST_ENTRY;
    process.env.NODOASSIST_SECRET_PROOF_OUTPUT_BYTES = "128";
    process.env.NODOASSIST_ENTRY = fakeNodoAssist;
    try {
      const proof = await import(
      ...
      if (previousEntry === undefined) {
        delete process.env.NODOASSIST_ENTRY;
      } else {
        process.env.NODOASSIST_ENTRY = previousEntry;
      }
    }
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir();
      const fakeNodoAssist = path.join(root, "fake-nodoassist-pty-timeout.mjs");
  ...
  it.each([
    ["NODOASSIST_SECRET_PROOF_COMMAND_MS", "150ms"],
    ["NODOASSIST_SECRET_PROOF_READY_MS", "0"],
    ["NODOASSIST_SECRET_PROOF_OUTPUT_BYTES", "4mb"],
    ["NODOASSIST_SECRET_PROOF_RESOLVER_STDIN_BYTES", "4mb"],
  ...
  it("bounds generated resolver stdin before reading the secret store", async () => {
    ...
    const previousLimit = process.env.NODOASSIST_SECRET_PROOF_RESOLVER_STDIN_BYTES;
    process.env.NODOASSIST_SECRET_PROOF_RESOLVER_STDIN_BYTES = "64";
    ...
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.NODOASSIST_SECRET_PROOF_RESOLVER_STDIN_BYTES;
      } else {
        process.env.NODOASSIST_SECRET_PROOF_RESOLVER_STDIN_BYTES = previousLimit;
      }
    }
  });
  ...
  it("fails when proof temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        proof.cleanupEnv("/tmp/nodoassist-secret-provider-proof-stuck", {
          attempts: 3,
          retryDelayMs: 1,
        }),
  ...
  it("detects startup secret leaks after the retained output cap", () => {
    const root = makeTempDir();
    const fakeNodoAssist = writeLeakingStartupNodoAssist(root);
    const result = runProofHarness(root, fakeNodoAssist, "startup-fails", {
      NODOASSIST_SECRET_PROOF_OUTPUT_BYTES: "128",
    });
  ...
  it("keeps stalled managed status probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeNodoAssist = writeStallingNodoAssist(root);
    const result = runProofHarness(root, fakeNodoAssist, "status");
