...
import type { DB as NodoAssistStateKyselyDatabase } from "../../src/state/nodoassist-state-db.generated.js";
import {
...
type GatewayRestartIntentDatabase = Pick<NodoAssistStateKyselyDatabase, "gateway_restart_intent">;
...
describe("gateway restart benchmark script", () => {
  ...
  it("prints help without running benchmark cases", () => {
    ...
    expect(helpResult.stdout).toContain("NodoAssist Gateway restart benchmark");
...
`),
  ...
  it("enables both startup and restart trace in the child gateway environment", () => {
    const env = testing.sanitizedEnv("/tmp/nodoassist-bench", "/tmp/nodoassist-bench/config.json", {
      config: {},
      id: "skipChannels",
      name: "gateway restart, skip channels",
    });

    expect(env.NODOASSIST_GATEWAY_STARTUP_TRACE).toBe("1");
    expect(env.NODOASSIST_GATEWAY_RESTART_TRACE).toBe("1");
    expect(env.NODOASSIST_NO_RESPAWN).toBe("1");
    expect(env.NODOASSIST_LOCAL_CHECK).toBeUndefined();
  });

  it("can pin ACPX startup probe policy per benchmark case", () => {
    const probeOffEnv = testing.sanitizedEnv(
      "/tmp/nodoassist-bench",
      "/tmp/nodoassist-bench/config.json",
      {
        config: {},
        env: { NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE: "0" },
        id: "skipChannelsNoAcpxProbe",
        name: "gateway restart, skip channels, ACPX startup probe off",
      },
    );

    expect(probeOffEnv.NODOASSIST_ACPX_RUNTIME_STARTUP_PROBE).toBe("0");
  });
  ...
  it("writes restart intent files for the target gateway pid", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-restart-bench-test-"));
    try {
      const env = { NODOASSIST_STATE_DIR: path.join(root, "state") };
  ...
  it("writes plugin fixtures as a parent load path with explicit startup activation", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-restart-bench-config-test-"));
    try {
      ...
      const manifest = JSON.parse(
        fs.readFileSync(
          path.join(root, "plugins", "bench-plugin-01", "nodoassist.plugin.json"),
          "utf8",
        ),
