...
describe("check-database-first-legacy-stores", () => {
  it("collects JavaScript runtime source files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-db-first-guard-"));
    try {
  ...
  it("flags private file store writes to legacy paths", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import { privateFileStore } from "nodoassist/plugin-sdk/security-runtime";
  ...
  it("flags fs-safe factory aliases writing legacy paths", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import { privateFileStore } from "nodoassist/plugin-sdk/security-runtime";
        import * as fsSafe from "nodoassist/plugin-sdk/security-runtime";
  ...
  it("flags fs-safe root writes to legacy paths", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import { root } from "nodoassist/plugin-sdk/security-runtime";
  ...
  it("flags file access runtime root writes to legacy paths", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import { root } from "nodoassist/plugin-sdk/file-access-runtime";
        const state = await root(stateDir);
        await state.writeJson("thread-bindings.json", {});
      `,
  ...
  it("allows fs copy calls reading from legacy store paths", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import fs from "node:fs/promises";
        import syncFs from "node:fs";
        await fs.copyFile("sessions.json", "state/nodoassist.sqlite.import");
        await fs.cp("cron/jobs.json", "state/nodoassist.sqlite.import");
        syncFs.copyFileSync("auth-profiles.json", "state/nodoassist.sqlite.import");
        syncFs.cpSync("cache/models.json", "state/nodoassist.sqlite.import");
      `,
  ...
  it("flags legacy paths destructured from for-of tuple entries", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import path from "node:path";
        import { root as fsRoot } from "nodoassist/plugin-sdk/security-runtime";
        const CLAIMS_DIGEST_PATH = ".nodoassist-wiki/cache/claims.jsonl";
  ...
  it("flags legacy paths written through regular-file helpers", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import { appendRegularFile as appendSafe } from "nodoassist/plugin-sdk/security-runtime";
  ...
  it("flags legacy paths written through JSON and atomic helpers", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        ...
        import { saveJsonFile, writeJsonFileAtomically } from "nodoassist/plugin-sdk/json-store";
  ...
  it("does not scan unrelated object properties for earlier property defaults", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        ...
        await writePath({
          currentPath: "state/nodoassist.sqlite",
  ...
  it("does not treat aliased top-level helpers as closing over wrapper parameters", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        import { writeTextAtomic } from "../infra/json-files.js";
        const filePath = "not-nodoassist-state.txt";
        function helper() {
          return writeTextAtomic(filePath, "{}\\n");
        }
        function persist(filePath: string) {
  ...
  it("keeps maybe missing nested properties after conditional parent object assignments", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        ...
        const options = { paths: { filePath: "state/nodoassist.sqlite" } };
        if (Math.random() > 0.5) {
          options.paths = {};
        }
        await persist(options);
  ...
  it("does not treat sibling nested properties as wrapper option paths", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        ...
        const options = {
          paths: {
            filePath: "state/nodoassist.sqlite",
            legacyPath: "sessions.json",
          },
        };
        await persist(options);
  ...
  it("allows current legacy-debt writes after harmless line movement", () => {
    const content = [
      ...
      `const relativePath = ".nodoassist-wiki/cache/claims.jsonl";`,
      `const root = await fsRoot(rootDir);`,
      ...Array.from({ length: 8 }, () => ""),
      `await root.write(relativePath, content);`,
    ].join("\n");
  ...
  it("flags stale current legacy-debt allowlist entries during full scans", () => {
    const violations = collectDatabaseFirstLegacyStoreViolations(
      `
        export const CLAIMS_DIGEST_PATH = ".nodoassist-wiki/cache/claims.jsonl";
      `,
      "extensions/memory-wiki/src/compile.ts",
