...
const NO_MEMORY_LIMIT = {
  ...
  procMeminfoPath: "/nodoassist-test-missing-proc-meminfo",
};
...
describe("resolveTsdownBuildInvocation", () => {
  ...
  it("routes Windows tsdown builds through the pnpm runner instead of shell=true", () => {
    const rootDir = createTempDir("nodoassist-pnpm-runner-");
  ...
  it("honors NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB over platform and memory defaults", () => {
    const result = resolveTsdownBuildInvocation({
      nodeExecPath: "/usr/bin/node",
      npmExecPath: "/tmp/pnpm.cjs",
      env: { NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB: "3072" },
      cgroupMemoryLimitBytes: 7 * 1024 * 1024 * 1024,
    });
  ...
  it("keeps memory detection when NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB is blank", () => {
    const result = resolveTsdownBuildInvocation({
      nodeExecPath: "/usr/bin/node",
      npmExecPath: "/tmp/pnpm.cjs",
      env: { NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB: "  " },
      cgroupMemoryLimitBytes: 7 * 1024 * 1024 * 1024,
    });
  ...
  it("uses NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB to normalize inherited NODE_OPTIONS", () => {
    const result = resolveTsdownBuildInvocation({
      platform: "win32",
      nodeExecPath: "C:\\Program Files\\nodejs\\node.exe",
      npmExecPath: "C:\\repo\\pnpm.cjs",
      env: {
        ...
        NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB: "4096",
      },
      ...NO_MEMORY_LIMIT,
    });
  ...
  it("rejects malformed NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB values", () => {
    for (const value of ["0", "-1", "1.5", "1e3", "4096mb", "9007199254740992"]) {
      expect(() =>
        resolveTsdownBuildInvocation({
          nodeExecPath: "/usr/bin/node",
          npmExecPath: "/tmp/pnpm.cjs",
          env: { NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB: value },
          ...NO_MEMORY_LIMIT,
        }),
      ).toThrow("NODOASSIST_TSDOWN_MAX_OLD_SPACE_MB must be");
    }
  });
  ...
  it("can run tsdown without invoking pnpm", () => {
    const result = resolveTsdownBuildInvocation({
      platform: "linux",
      nodeExecPath: "/usr/bin/node",
      env: { NODOASSIST_BUILD_ALL_NO_PNPM: "1" },
      ...NO_MEMORY_LIMIT,
    });
  ...
  it("prunes stale hashed root chunk files but keeps stable aliases and nested assets", async () => {
    const rootDir = createTempDir("nodoassist-tsdown-build-");
  ...
  it("cleans tsdown output roots before using tsdown --no-clean", async () => {
    const rootDir = createTempDir("nodoassist-tsdown-clean-");
  ...
  it("removes CLI startup metadata during default tsdown clean", async () => {
    const rootDir = createTempDir("nodoassist-tsdown-clean-metadata-default-");
  ...
  it("preserves CLI startup metadata across opted-in build-all tsdown clean", async () => {
    const rootDir = createTempDir("nodoassist-tsdown-clean-metadata-");
    const metadataFile = path.join(rootDir, "dist", "cli-startup-metadata.json");
    ...
    cleanTsdownOutputRoots({
      cwd: rootDir,
      env: { NODOASSIST_PRESERVE_CLI_STARTUP_METADATA: "1" },
    });
  ...
  it("preserves existing package declarations when tsdown DTS output is skipped", async () => {
    const rootDir = createTempDir("nodoassist-tsdown-clean-skip-dts-");
    ...
    cleanTsdownOutputRoots({
      cwd: rootDir,
      env: { NODOASSIST_RUN_NODE_SKIP_DTS_BUILD: "1" },
    });
  ...
  it("prunes untracked generated declaration files that shadow source entries", async () => {
    const rootDir = createTempDir("nodoassist-tsdown-source-dts-");
...
describe("createTsdownOutputScanner", () => {
  ...
  it("ignores unresolved imports from bundled plugin and dependency paths", () => {
    ...
    scanner.append(
      "[UNRESOLVED_IMPORT] ../../../../tmp/nodoassist-pnpm-node-modules/baileys/lib/Utils/messages-media.js\n",
    );
...
describe("runTsdownBuildInvocation", () => {
  ...
  it("streams child output while preserving diagnostics for post-run checks", async () => {
    ...
    const result = await runTsdownBuildInvocation(
      ...
      },
      {
        stdout: output.sink,
        stderr: output.sink,
        env: { ...process.env, NODOASSIST_TSDOWN_HEARTBEAT_MS: "0" },
      },
    );
  ...
  it("rejects malformed NODOASSIST_TSDOWN_TIMEOUT_MS values", async () => {
    const invocation = {
      command: process.execPath,
    ...
    for (const value of ["1.5", "1e3", "10ms", "0"]) {
      await expect(
        runTsdownBuildInvocation(invocation, {
          env: {
            ...process.env,
            NODOASSIST_TSDOWN_TIMEOUT_MS: value,
          },
        }),
      ).rejects.toThrow("NODOASSIST_TSDOWN_TIMEOUT_MS must be");
    }
  });

  it("rejects malformed NODOASSIST_TSDOWN_HEARTBEAT_MS values", async () => {
    const invocation = {
      command: process.execPath,
      args: ["-e", "process.exit(0)"],
      options: {
    ...
    for (const value of ["1.5", "1e3", "10ms", "-1"]) {
      await expect(
        runTsdownBuildInvocation(invocation, {
          env: {
            ...process.env,
            NODOASSIST_TSDOWN_HEARTBEAT_MS: value,
          },
        }),
      ).rejects.toThrow("NODOASSIST_TSDOWN_HEARTBEAT_MS must be");
    }
  });

  it("terminates the child when NODOASSIST_TSDOWN_TIMEOUT_MS elapses", async () => {
    const output = createWriteSink();
    const result = await runTsdownBuildInvocation(
      {
        command: process.execPath,
      ...
      },
      {
        ...
        env: {
          ...process.env,
          NODOASSIST_TSDOWN_HEARTBEAT_MS: "0",
          NODOASSIST_TSDOWN_TIMEOUT_MS: "50",
        },
      },
    );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const rootDir = createTempDir("nodoassist-tsdown-timeout-");
        ...
        const runPromise = runTsdownBuildInvocation(
          ...
          },
          {
            ...
            env: {
              ...process.env,
              NODOASSIST_TSDOWN_HEARTBEAT_MS: "0",
              NODOASSIST_TSDOWN_TIMEOUT_MS: String(timeoutMs),
            },
          },
        );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const rootDir = createTempDir("nodoassist-tsdown-timeout-clean-");
        ...
        const runPromise = runTsdownBuildInvocation(
          ...
          },
          {
            ...
            env: {
              ...process.env,
              NODOASSIST_TSDOWN_HEARTBEAT_MS: "0",
              NODOASSIST_TSDOWN_TIMEOUT_MS: "250",
            },
          },
        );
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const rootDir = createTempDir("nodoassist-tsdown-parent-signal-");
      const childPidPath = path.join(rootDir, "child.pid");
      const readyPath = path.join(rootDir, "child.ready");
