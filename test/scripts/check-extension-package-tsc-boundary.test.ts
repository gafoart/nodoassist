...
function createTempExtensionRoot(extensionId = "demo") {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-boundary-canary-"));
...
describe("check-extension-package-tsc-boundary", () => {
  ...
  it("parses extension boundary compile concurrency strictly", () => {
    expect(resolveCompileConcurrency({ NODOASSIST_EXTENSION_BOUNDARY_CONCURRENCY: "4" }, 32)).toBe(
      4,
    );
    ...
    for (const value of ["4x", "0", "1e3"]) {
      expect(() =>
        resolveCompileConcurrency({ NODOASSIST_EXTENSION_BOUNDARY_CONCURRENCY: value }, 32),
      ).toThrow("NODOASSIST_EXTENSION_BOUNDARY_CONCURRENCY must be a positive integer");
    }
  });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-extension-tsc-timeout-"));
      tempRoots.add(root);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-extension-tsc-signal-"));
      tempRoots.add(root);
      const childPidPath = path.join(root, "child.pid");
