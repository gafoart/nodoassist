...
describe("install.ps1 failure handling", () => {
  ...
  beforeAll(() => {
    ...
    const cases = [
      ...
      },
      {
        ...
        source: [
          ...
          "} catch {",
          "  if ($_.Exception.Message -ne 'NodoAssist installation failed with exit code 1.') { throw }",
          "  $caught = $true",
          "}",
      ...
      },
      {
        ...
        source: [
          ...
          "function Check-ExistingNodoAssist { return $false }",
          "function Get-NpmCommandPath { return $null }",
          "function Install-NodoAssistFromGit {",
          "  Write-Output 'pnpm stdout before failure'",
          "  return $false",
          "}",
          "function Ensure-NodoAssistOnPath { throw 'should not continue after failed git install' }",
          "$InstallMethod = 'git'",
          "$GitDir = 'C:\\\\nodoassist-test'",
          "$NoOnboard = $true",
          "$result = Main",
      ...
      },
      {
        ...
        source: [
          ...
          "function Install-NodoAssist { Write-Output 'npm stdout'; return $true }",
          "function Ensure-NodoAssistOnPath { return $true }",
          "function Refresh-GatewayServiceIfLoaded { }",
          "function Invoke-NodoAssistCommand { return 'NodoAssist test-version' }",
          "$NoOnboard = $true",
          "$result = Main",
    ...
    const tempDir = harness.createTempDir("nodoassist-install-ps1-batch-");
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "NodoAssist installation failed with exit code/);
  ...
  it("runs npm install through the resolved command with quiet CI defaults", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-NodoAssist");
  ...
  it("does not force npm or pnpm lifecycle scripts through cmd.exe", () => {
    ...
    const npmInstallBody = extractFunctionBody(source, "Install-NodoAssist");
    const gitInstallBody = extractFunctionBody(source, "Install-NodoAssistFromGit");
  ...
  it("runs Windows command shims from a Windows-local cwd", () => {
    ...
    const nodoAssistPathBody = extractFunctionBody(source, "Ensure-NodoAssistOnPath");
    const ensurePnpmBody = extractFunctionBody(source, "Ensure-Pnpm");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(nodoAssistPathBody).toContain(
    ...
    expect(mainBody).toContain('Invoke-NpmCommand -Arguments @("uninstall", "-g", "nodoassist")');
  ...
  it("rejects NodoAssist GitHub source targets for npm installs", () => {
    const npmInstallBody = extractFunctionBody(source, "Install-NodoAssist");
    const sourceTargetBody = extractFunctionBody(source, "Test-NodoAssistSourcePackageInstallSpec");
    expect(sourceTargetBody).toContain('$normalizedTag -eq "main"');
    expect(sourceTargetBody).toContain("^github:nodoassist/nodoassist");
    expect(npmInstallBody).toContain("Test-NodoAssistSourcePackageInstallSpec -RequestedTag $Tag");
    expect(npmInstallBody).toContain(
      "npm installs do not support NodoAssist GitHub source targets",
    );
  ...
  it("preserves caller-relative local tarball install specs before safe-cwd npm calls", () => {
    const resolveSpecBody = extractFunctionBody(source, "Resolve-NpmNodoAssistInstallSpec");
  ...
  it("falls back to a user-local portable Node.js bootstrap when package managers are absent", () => {
    ...
    const depsRootBody = extractFunctionBody(source, "Get-NodoAssistDepsRoot");
    ...
    expect(depsRootBody).toContain("NodoAssist\\deps");
  ...
  it("persists user-local portable Git for future git-backed updates", () => {
    ...
    expect(portableGitRootBody).toContain("Get-NodoAssistDepsRoot");
  ...
  it("activates the repo-pinned pnpm version for git installs", () => {
    ...
    expect(mainBody).toContain("$gitInstallResults = @(Install-NodoAssistFromGit");
    expect(mainBody).toContain("Test-BooleanSuccessResult -Results $gitInstallResults");
    expect(mainBody).toContain("$npmInstallResults = @(Install-NodoAssist)");
    ...
    expect(gitInstallBody).toContain(
      'Write-Host "[!] NodoAssist build did not produce $entryPath"',
    );
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(
      source,
      "Invoke-InteractiveNodoAssistCommand",
    );
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveNodoAssistCommand onboard");
  });

  runConcurrentIfPowerShell(
    "fails install when interactive onboarding exits non-zero",
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "nodoassist-install-ps1-"));
      ...
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "function Install-NodoAssist { return $true }",
            "function Ensure-NodoAssistOnPath { return $true }",
            "function Add-ToUserPath { param([string]$Path) }",
            "function Get-NodoAssistCommandPath { return 'cmd.exe' }",
        ...
        expect(`${result.stdout}\n${result.stderr}`).toContain(
          "nodoassist onboard failed with exit code 17",
        );
      } finally {
