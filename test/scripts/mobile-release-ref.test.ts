...
function createFixtureRepo(): { cleanup: () => void; remote: string; root: string; sha: string } {
  const root = mkdtempSync(path.join(os.tmpdir(), "nodoassist-mobile-release-ref-"));
...
describe("mobile-release-ref", () => {
  it("renders platform release refs from store identities", () => {
    expect(mobileReleaseRefFor({ platform: "ios", version: "2026.6.10", build: "8" })).toBe(
      "refs/nodoassist/mobile-releases/ios/2026.6.10-8",
    );
    expect(
      mobileReleaseRefFor({
        platform: "android",
    ...
    ).toBe("refs/nodoassist/mobile-releases/android/2026.6.10-2026061008");
  });
  ...
  it("creates, resolves, and idempotently accepts an existing same-SHA ref", () => {
    ...
    try {
      ...
      expect(recordMobileReleaseRef(options)).toMatchObject({
        ref: "refs/nodoassist/mobile-releases/ios/2026.6.10-8",
        sha: fixture.sha,
        status: "created",
      });
  ...
  it("prints the resolved SHA from the CLI", () => {
    ...
    try {
      ...
      expect(stdout).toBe(`${fixture.sha}\trefs/nodoassist/mobile-releases/ios/2026.6.10-9\n`);
    } finally {
      fixture.cleanup();
    }
  });
