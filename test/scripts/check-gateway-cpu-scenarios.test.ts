...
describe("gateway CPU scenario guard", () => {
  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        ...
        if (args.includes("nodoassist") && args.includes("qa")) {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      NODOASSIST_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "nodoassist.json"),
      NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1",
      NODOASSIST_HOME: path.join(outputDir, "qa-state-root", "home"),
      NODOASSIST_RUN_NODE_SKIP_DTS_BUILD: "1",
      NODOASSIST_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      NODOASSIST_TEST_DISABLE_UPDATE_CHECK: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.NODOASSIST_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
  ...
  it("does not prebuild private QA dist when the required entries already exist", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      env: {
        HOME: "/real/user/home",
        NODOASSIST_CONFIG_PATH: "/real/user/.nodoassist/nodoassist.json",
        NODOASSIST_HOME: "/real/user/home",
        NODOASSIST_STATE_DIR: "/real/user/.nodoassist",
      },
      silent: true,
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        calls.push({ args, env: opts?.env });
