...
describe("release upgrade baseline resolver", () => {
  ...
  it("does not pick a newer stable release for a prerelease candidate", () => {
    ...
    ).toBe("nodoassist@2026.6.6");
  });

  it("compares prerelease versions with semver ordering", () => {
    expect(compareNodoAssistVersions("2026.6.7-beta.2", "2026.6.7-beta.10")).toBeLessThan(0);
    expect(compareNodoAssistVersions("2026.6.7", "2026.6.7-beta.10")).toBeGreaterThan(0);
  });
});
