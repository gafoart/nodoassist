...
function makePackage(version: string): { packageDir: string; path: string } {
  const root = mkdtempSync(join(tmpdir(), "nodoassist-plugin-publish-test-"));
  tempDirs.push(root);
  ...
  writeFileSync(
    ...
    JSON.stringify({ name: "@nodoassist/demo", version }),
  );
...
describe("plugin npm publish wrapper", () => {
  ...
  it("requires an explicit artifact directory for real pack mode", () => {
    ...
    expect(result.status).toBe(2);
    expect(result.stdout).toBe("");
    expect(result.stderr.trim()).toBe("--pack requires NODOASSIST_PLUGIN_NPM_PACK_OUTPUT_DIR");
  });
