...
function makeLockRepoDir(): string {
  const dir = makeTempDir("nodoassist-pr-gates-lock-");
  mkdirSync(join(dir, ".git"), { recursive: true });
  return dir;
}

function heavyCheckLockDir(repoDir: string): string {
  return join(repoDir, ".git", "nodoassist-local-checks", "heavy-check.lock");
}
...
function sanitizedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.NODOASSIST_PR_GATES_REMOTE;
  delete env.NODOASSIST_TESTBOX;
  delete env.NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD;
  delete env.NODOASSIST_TSGO_HEAVY_CHECK_LOCK_HELD;
  delete env.NODOASSIST_OXLINT_SKIP_LOCK;
  delete env.NODOASSIST_HEAVY_CHECK_LOCK_TIMEOUT_MS;
  delete env.NODOASSIST_HEAVY_CHECK_LOCK_POLL_MS;
...
function makeRetryRepo(): { repoDir: string; stubBin: string; headSha: string } {
  const dir = makeTempDir("nodoassist-pr-gates-retry-");
...
describe("resolve_pr_gates_remote_mode", () => {
  ...
  ])("resolves NODOASSIST_PR_GATES_REMOTE=$value to $expected", ({ value, expected }) => {
    const env: NodeJS.ProcessEnv = {};
    if (value !== undefined) {
      env.NODOASSIST_PR_GATES_REMOTE = value;
    }
  ...
  it("rejects unsupported values", () => {
    const result = runGatesBash("resolve_pr_gates_remote_mode", {
      env: { NODOASSIST_PR_GATES_REMOTE: "azure" },
    });
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("Unsupported NODOASSIST_PR_GATES_REMOTE=azure");
  });

  it("rejects the hosted-gates conflict before touching the worktree", () => {
    const result = runGatesBash("prepare_gates 424242", {
      env: { NODOASSIST_PR_GATES_REMOTE: "testbox", NODOASSIST_TESTBOX: "1" },
    });
    expect(result.status).toBe(2);
    expect(result.stdout).toContain("conflicts with NODOASSIST_TESTBOX=1");
  });
});

describe("remote testbox gate delegation", () => {
  it("runs the full pnpm test through the worktree crabbox wrapper", () => {
    const dir = makeTempDir("nodoassist-pr-gates-remote-");
    const stubBin = join(dir, "bin");
    mkdirSync(stubBin);
    writeFileSync(
      join(stubBin, "node"),
  ...
  it("extracts the last successful blacksmith-testbox timing stamp", () => {
    const dir = makeTempDir("nodoassist-pr-gates-stamp-");
    ...
    const result = runGatesBash(
      ...
      ].join("\n"),
      {
        ...
        env: {
          ...
          NODOASSIST_PR_GATES_REMOTE: "testbox",
        },
      },
    );
...
describe("prepare gate stamp transitions", () => {
  ...
  it("forwards only the recorded pre-rebase SHA as recent evidence", () => {
    const result = runGatesBash(
      [
        "gh() { if [ \"$1\" = pr ]; then printf 'deadbeef\\n'; else printf 'nodoassist/nodoassist\\n'; fi; }",
  ...
  it("clears remote stamps when hosted gates replace remote proof", () => {
    ...
    const result = runGatesBash(
      ...
      { cwd: repoDir, env: { NODOASSIST_TESTBOX: "1" } },
    );
...
describe("pr-gates-lock helper", () => {
  ...
  it("queues behind an existing holder and acquires after it exits", async () => {
    ...
    const second = spawnGateLockHolder(repoDir, secondStatus, {
      NODOASSIST_HEAVY_CHECK_LOCK_POLL_MS: "50",
    });
  ...
  it("fails instead of holding when the wait times out", async () => {
    ...
    const statusFile = join(repoDir, "status");
    const holder = spawnGateLockHolder(repoDir, statusFile, {
      NODOASSIST_HEAVY_CHECK_LOCK_TIMEOUT_MS: "200",
      NODOASSIST_HEAVY_CHECK_LOCK_POLL_MS: "50",
    });
    await waitForExit(holder);
...
describe("gates.sh gate lock plumbing", () => {
  ...
  it("exports the held-lock contract while holding and clears it on release", () => {
    ...
    const result = runGatesBash(
      [
        ...
        'echo "held=${NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD:-unset},${NODOASSIST_TSGO_HEAVY_CHECK_LOCK_HELD:-unset},${NODOASSIST_OXLINT_SKIP_LOCK:-unset}"',
        "jq -r .tool .git/nodoassist-local-checks/heavy-check.lock/owner.json",
        "release_pr_gates_lock",
        'echo "released=${NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD:-unset}"',
        '[ -d .git/nodoassist-local-checks/heavy-check.lock ] && echo "lock=held" || echo "lock=free"',
      ].join("\n"),
      { cwd: repoDir },
    );
  ...
  it("skips acquisition when a parent already holds the lock", () => {
    ...
    const result = runGatesBash(
      ...
      { cwd: repoDir, env: { NODOASSIST_TEST_HEAVY_CHECK_LOCK_HELD: "1" } },
    );
