...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as NodoAssist-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install.sh --version latest && nodoassist --version");
    expect(workflow).not.toContain("bash /tmp/install.sh --no-prompt --no-onboard");
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/nodoassist");
    expect(workflow).toContain("macos-installer:");
    expect(workflow).toContain("runs-on: macos-15");
    expect(workflow).toContain("node-version: 24");
    expect(workflow).toContain('NODOASSIST_NO_ONBOARD: "1"');
    expect(workflow).toContain('NODOASSIST_NO_PROMPT: "1"');
    expect(workflow).toContain("bash scripts/install.sh --no-onboard --no-prompt --version latest");
    expect(workflow).toContain("nodoassist --version");
  ...
  it("syncs verified scripts to openclaw.ai only after all installer checks pass", () => {
    ...
    expect(workflow).toContain("repository: nodoassist/openclaw.ai");
    expect(workflow).toContain("NODOASSIST_GH_TOKEN: ${{ secrets.NODOASSIST_GH_TOKEN }}");
    expect(workflow).toContain("NODOASSIST_GH_TOKEN is not configured");
    expect(workflow).toContain("token: ${{ env.NODOASSIST_GH_TOKEN }}");
    expect(workflow).toContain("cp nodoassist/scripts/install.sh openclaw.ai/public/install.sh");
    expect(workflow).toContain(
      "cp nodoassist/scripts/install-cli.sh openclaw.ai/public/install-cli.sh",
    );
    expect(workflow).toContain("cp nodoassist/scripts/install.ps1 openclaw.ai/public/install.ps1");
