...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "nodoassist-plistbuddy-"));
  tempDirs.push(dir);
...
function runStopPackagedAppHarness(killZeroStatus: 0 | 1) {
  const root = mkdtempSync(path.join(tmpdir(), "nodoassist-package-stop-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "nodoassist-package-stop-tools-"));
  tempDirs.push(root, toolsDir);

  const appRoot = path.join(root, "dist", "NodoAssist.app");
  const appBinary = path.join(appRoot, "Contents", "MacOS", "NodoAssist");
...
function runSwiftCompatibilityHarness(buildConfig: "debug" | "release") {
  const root = mkdtempSync(path.join(tmpdir(), "nodoassist-package-swift-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "nodoassist-package-swift-tools-"));
  const developerDir = path.join(root, "Xcode.app", "Contents", "Developer");
  const appRoot = path.join(root, "NodoAssist.app");
...
describe("package-mac-app plist stamping", () => {
  ...
  it("falls back to corepack pnpm when the pnpm shim is absent", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "nodoassist-package-pnpm-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "nodoassist-package-pnpm-tools-"));
    ...
    const result = runHelper(`
      ...
      NODOASSIST_TEST_LOG=${JSON.stringify(logPath)}
      export NODOASSIST_TEST_LOG
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      ${helperBlock}
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    const helperBlock = getPackageManagerHelperBlock();
    const tempRoot = mkdtempSync(path.join(tmpdir(), "nodoassist-package-pnpm-root-"));
    const outerRoot = mkdtempSync(path.join(tmpdir(), "nodoassist-package-pnpm-outer-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "nodoassist-package-pnpm-tools-"));
  ...
  it("fails with an actionable error when Swift tools are too old", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("NodoAssist macOS app packaging requires Swift tools 6.2+");
  ...
  it("runs Sparkle build metadata derivation from the repository root", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "nodoassist-package-sparkle-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "nodoassist-package-sparkle-tools-"));
    tempDirs.push(tempRoot, toolsDir);
    ...
    const result = runHelper(`
      set -euo pipefail
      ROOT_DIR=${JSON.stringify(tempRoot)}
      NODOASSIST_ROOT=${JSON.stringify(tempRoot)}
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      export NODOASSIST_ROOT PATH
      cd /tmp
      ${helperBlock}
  ...
  it("does not kill unrelated NodoAssist processes during packaging", () => {
    const script = readFileSync(scriptPath, "utf8");
    ...
    expect(script).not.toContain("killall -q NodoAssist");
    expect(stopBlock).toContain('local app_binary="$APP_ROOT/Contents/MacOS/NodoAssist"');
  ...
  it("fails when the packaged app survives forced shutdown", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("ERROR: Packaged NodoAssist bundle did not exit: 123");
  });
  ...
  it("fails closed when required Swift resources are missing", () => {
    const script = readFileSync(scriptPath, "utf8");
    const nodoAssistKitBlock = script.slice(
      script.indexOf(
        'NODOASSISTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/NodoAssistKit_NodoAssistKit.bundle"',
      ),
      script.indexOf("running_packaged_app_pids()"),
    );
    ...
    expect(nodoAssistKitBlock).toContain("ERROR: NodoAssistKit resource bundle not found");
    expect(nodoAssistKitBlock).toContain("exit 1");
    expect(nodoAssistKitBlock).not.toContain("WARN:");
    expect(nodoAssistKitBlock).not.toContain("continuing");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      ...
      const result = runHelper(`
        ...
        plist_set_string_required ${JSON.stringify(plist)} CFBundleIdentifier 'ai.nodoassist.test'
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain("ai.nodoassist.test");
