// Resolve NodoAssist ref tests cover the release workflow ref resolver script.
import { execFileSync, spawnSync } from "node:child_process";
import { join } from "node:path";
import { afterEach, describe, expect, it } from "vitest";
import { createTempDirTracker } from "../helpers/temp-dir.js";

const SCRIPT_PATH = "scripts/github/resolve-nodoassist-ref.sh";
...
function createRemoteRepo() {
  const repo = tempDirs.make("nodoassist-ref-remote-");
...
describe("scripts/github/resolve-nodoassist-ref.sh", () => {
  ...
  it("writes fallback outputs for unresolved refs when a caller supplies an expected SHA", () => {
    const { repo, sha } = createRemoteRepo();
    const outputPath = join(tempDirs.make("nodoassist-ref-output-"), "github-output.txt");
  ...
  it("does not let fallback mode hide remote lookup failures", () => {
    const missingRemote = join(tempDirs.make("nodoassist-ref-missing-"), "missing.git");
