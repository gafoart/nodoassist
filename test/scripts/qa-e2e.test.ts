...
describe("qa-e2e script", () => {
  it("enables private QA plugin SDK subpaths before loading QA Lab", () => {
    ...
    expect(env.NODOASSIST_BUILD_PRIVATE_QA).toBe("1");
    expect(env.NODOASSIST_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(env.NODOASSIST_DISABLE_BUNDLED_PLUGINS).toBe("0");
  });

  it("overrides inherited environment that would break the private QA self-check", () => {
    const env: NodeJS.ProcessEnv = {
      NODOASSIST_BUILD_PRIVATE_QA: "0",
      NODOASSIST_ENABLE_PRIVATE_QA_CLI: "0",
  ...
  it("prints help before enabling private QA or loading QA Lab", async () => {
    ...
    expect(env.NODOASSIST_BUILD_PRIVATE_QA).toBeUndefined();
  });
