...
describe("generate-dependency-release-evidence", () => {
  ...
  it("collects report counts and renders human summaries", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-release-dependency-evidence-test-"));
    try {
      ...
      const stepSummary = renderDependencyEvidenceStepSummary({
        evidenceArtifactName: "nodoassist-release-dependency-evidence-v2026.5.13",
        baseRef: "v2026.5.1",
        counts,
      });
      expect(stepSummary).toContain(
        "- Evidence artifact: `nodoassist-release-dependency-evidence-v2026.5.13`",
      );
