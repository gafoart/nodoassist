...
function makeApp(plistEntries: string[]): string {
  const dir = mkdtempSync(path.join(tmpdir(), "nodoassist-create-dmg-"));
  tempDirs.push(dir);
  const app = path.join(dir, "NodoAssist.app");
...
function makeFakeDmgTools() {
  const dir = mkdtempSync(path.join(tmpdir(), "nodoassist-create-dmg-tools-"));
  tempDirs.push(dir);
...
describe("create-dmg plist validation", () => {
  ...
  it("keeps temporary DMG artifacts scoped to one run", () => {
    ...
    expect(script).toContain('DMG_TEMP="$(mktemp -d "${TMPDIR:-/tmp}/nodoassist-dmg.XXXXXX")"');
    ...
    expect(script).toContain(
      'DMG_OUTPUT_TEMP="$(mktemp -d "$(dirname "$OUT_PATH")/.nodoassist-dmg.XXXXXX")"',
    );
    ...
    expect(script).not.toContain("/tmp/nodoassist-dmg-limits.txt");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      const app = makeApp(["<key>CFBundleName</key>", "<string>NodoAssist</string>"]);
...
describe.runIf(process.platform === "darwin")("create-dmg ownership boundaries", () => {
  it("uses private intermediate paths without deleting caller-owned siblings", () => {
    ...
    const outputDir = mkdtempSync(path.join(tmpdir(), "nodoassist-create-dmg-output-"));
    tempDirs.push(outputDir);
    const output = path.join(outputDir, "NodoAssist.dmg");
    const sibling = path.join(outputDir, "NodoAssist-rw.dmg");
    ...
    expect(log).toContain(`${outputDir}${path.sep}.nodoassist-dmg.`);
  ...
  it("creates a caller-provided output directory before finalizing the DMG", () => {
    const app = makeValidApp();
    const root = mkdtempSync(path.join(tmpdir(), "nodoassist-create-dmg-output-"));
    tempDirs.push(root);
  ...
  it("styles the private mount without closing unrelated Finder windows", () => {
    ...
    expect(applescript).not.toContain('tell disk "NodoAssist"');
