...
async function writeIpaFixture(root: string): Promise<string> {
  ...
  const ipaPath = path.join(root, "NodoAssist.ipa");
...
async function writeValidFixture(
  ...
  const appDir = path.join(payloadDir, "NodoAssist.app");
  ...
  const infoBody = [
    plistString("CFBundleIdentifier", "ai.nodoassistfoundation.app"),
    plistString("NodoAssistPushMode", options.pushMode ?? "appStore"),
    plistString("NodoAssistPushRelayBaseURL", ""),
    options.legacyKey ? plistString("NodoAssistPushRelayProfile", "production") : "",
  ].join("");
  ...
  writeFileSync(
    ...
    plist(
      [
        plistString("application-identifier", "FWJYW4S8P8.ai.nodoassistfoundation.app"),
        ...
        plistArray("com.apple.security.application-groups", [
          "group.ai.nodoassistfoundation.app.shared",
        ]),
      ].join(""),
    ),
    "utf8",
  ...
  writeFileSync(
    ...
    plist(
      [
        plistString("Name", "NodoAssist App Store ai.nodoassistfoundation.app"),
...
describe("scripts/ios-validate-app-store-ipa.sh", () => {
  ...
  it("accepts an App Store IPA with appStore mode and production entitlements", async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "nodoassist-ios-ipa-"));
    tempDirs.push(root);
