...
function makeTempRoot(): string {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-android-signing-"));
...
function runGit(args: string[], cwd?: string, env: NodeJS.ProcessEnv = {}) {
  execFileSync("git", args, {
    ...
    env: {
      ...process.env,
      GIT_AUTHOR_NAME: "NodoAssist Test",
      GIT_AUTHOR_EMAIL: "test@example.com",
      GIT_COMMITTER_NAME: "NodoAssist Test",
    ...
    `${JSON.stringify(
      {
        ...
        gradlePropertyNames: [
          "NODOASSIST_ANDROID_STORE_FILE",
          "NODOASSIST_ANDROID_STORE_PASSWORD",
          "NODOASSIST_ANDROID_KEY_ALIAS",
          "NODOASSIST_ANDROID_KEY_PASSWORD",
        ],
      },
      null,
      2,
...
function writeSigningSources(tempRoot: string) {
  ...
  fs.writeFileSync(
    propertiesPath,
    [
      `NODOASSIST_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`,
      "NODOASSIST_ANDROID_KEY_ALIAS=nodoassist-upload",
      `NODOASSIST_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`,
      "",
    ].join("\n"),
  );
...
describe("scripts/android-release-signing.mjs", () => {
  ...
  it("documents the canonical Android release signing plan", () => {
    ...
    expect(result.stdout).toContain("Signing repo: git@github.com:nodoassist/apps-signing.git");
    expect(result.stdout).toContain("Signing assets: android/nodoassist");
  ...
  it.runIf(commandAvailable("openssl"))(
    ...
    () => {
      ...
      const encryptedProperties = fs.readFileSync(
        path.join(remoteCheck, "android", "nodoassist", "gradle.properties.enc"),
        "utf8",
      );
      ...
      expect(materializedProperties).toContain(
        `NODOASSIST_ANDROID_STORE_FILE=${path.join(materializedDir, "upload-keystore.jks")}`,
      );
      expect(materializedProperties).toContain(
        `NODOASSIST_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`,
      );
      expect(materializedProperties).toContain("NODOASSIST_ANDROID_KEY_ALIAS=nodoassist-upload");
      expect(materializedProperties).toContain(`NODOASSIST_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`);
