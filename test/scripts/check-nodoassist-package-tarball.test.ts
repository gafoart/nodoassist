// Check NodoAssist Package Tarball tests cover check nodoassist package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-nodoassist-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "nodoassist-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(
      join(packageRoot, "package.json"),
      JSON.stringify({ name: "nodoassist", version, ...options.packageJson }),
    );
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><nodoassist-app></nodoassist-app>",
    ...
    const tarball = join(root, "nodoassist.tgz");
...
describe("check-nodoassist-package-tarball", () => {
  it("prints help before touching tarball state", () => {
    const result = spawnSync("node", [CHECK_SCRIPT, "--help"], { encoding: "utf8" });

    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout).toContain(
      "Usage: node scripts/check-nodoassist-package-tarball.mjs [--require-bundled-workspace-deps] <nodoassist.tgz>",
    );
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown NodoAssist package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("NodoAssist package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "nodoassist.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected NodoAssist package tarball check argument: extra");
    expect(extra.stderr).not.toContain("NodoAssist package tarball does not exist");
  });

  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "nodoassist-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.NODOASSIST_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            NODOASSIST_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("NodoAssist package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
  ...
  it("rejects workspace protocol dependencies in package manifests", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "package.json dependencies.@nodoassist/ai must not use workspace protocol workspace:*",
        );
      },
      "2026.6.11",
      { packageJson: { dependencies: { "@nodoassist/ai": "workspace:*" } } },
    );
  });
  ...
  it("rejects workspace protocol dependencies in shrinkwrap root metadata", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "npm-shrinkwrap.json packages root dependencies.@nodoassist/ai must not use workspace protocol workspace:*",
        );
      },
      "2026.6.11",
      { shrinkwrapRootPackage: { dependencies: { "@nodoassist/ai": "workspace:*" } } },
    );
  });
  ...
  it("accepts separately published private workspace dependencies by default", () => {
    withTarball(
      ...
      "2026.6.11",
      { packageJson: { dependencies: { "@nodoassist/ai": "2026.6.11" } } },
    );
  });
  ...
  it("rejects private workspace dependencies that are not bundled when strict packaging requires it", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.status).not.toBe(0);
        expect(result.stderr).toContain(
          "package.json dependencies.@nodoassist/ai must be listed in bundleDependencies because it is private to the NodoAssist workspace",
        );
        expect(result.stderr).toContain(
          "package.json dependencies.@nodoassist/ai must be bundled in node_modules/@nodoassist/ai",
        );
      },
      "2026.6.11",
      { packageJson: { dependencies: { "@nodoassist/ai": "2026.6.11" } } },
  ...
  it("accepts private workspace dependencies when bundled in the tarball", () => {
    withTarball(
      ...
      {
        "dist/index.js": "export {};\n",
        "node_modules/@nodoassist/ai/package.json": JSON.stringify({
          name: "@nodoassist/ai",
          version: "2026.6.11",
        }),
      },
      ...
      {
        packageJson: {
          dependencies: { "@nodoassist/ai": "2026.6.11" },
          bundleDependencies: ["@nodoassist/ai"],
        },
      },
    );
  });
