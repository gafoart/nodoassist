...
describe("Docker E2E client scripts", () => {
  ...
  it("keeps session runtime-context checks wired to packaged transcript behavior", () => {
    ...
    expect(source).toContain("nodoassist/plugin-sdk/agent-sessions");
    ...
    expect(source).toContain("<<<BEGIN_NODOASSIST_INTERNAL_CONTEXT>>>");
    expect(source).toContain("nodoassist.runtime-context");
