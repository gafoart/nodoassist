...
const tmpFixtureFiles = ["/tmp/nodoassist-codex-inspect.json", "/tmp/nodoassist-plugins-list.json"];
...
function writeAuthProfileStoreSqlite(agentDir: string) {
  ...
  const db = new DatabaseSync(path.join(agentDir, "nodoassist-agent.sqlite"));
  try {
    db.exec(`
...
function runCodexOnDemandAssertions(root: string) {
  return spawnSync(process.execPath, [ASSERTIONS_SCRIPT], {
    ...
    env: {
      ...
      NODOASSIST_CONFIG_PATH: path.join(root, "state", "nodoassist.json"),
...
function createCodexInstallFixture(root: string) {
  ...
  const projectRoot = npmProjectRootForInstalledPackage(installPath, "@nodoassist/codex");
  writeJson(path.join(installPath, "package.json"), { name: "@nodoassist/codex" });
  ...
  writeJson(path.join(stateDir, "nodoassist.json"), {
    ...
    plugins: {
      installs: {
        codex: {
          installPath,
          source: "npm",
          spec: "npm:@nodoassist/codex",
        },
      },
    },
  });
  writeJson("/tmp/nodoassist-codex-inspect.json", {
    plugin: { id: "codex", status: "loaded", agentHarnessIds: ["codex"] },
  });
  writeJson("/tmp/nodoassist-plugins-list.json", {
...
describe("Codex install helpers", () => {
  it("resolves package roots and package manifests inside managed npm installs", () => {
    const root = makeTempDir(tempDirs, "nodoassist-codex-install-utils-");
    ...
    const projectRoot = npmProjectRootForInstalledPackage(packageRoot, "@nodoassist/codex");
  ...
  it("accepts a complete on-demand Codex npm install fixture", () => {
    const root = makeTempDir(tempDirs, "nodoassist-codex-on-demand-");
  ...
  it("rejects on-demand fixtures missing the managed @openai/codex dependency", () => {
    const root = makeTempDir(tempDirs, "nodoassist-codex-on-demand-missing-");
    createCodexInstallFixture(root);
  ...
  it("rejects on-demand fixtures missing the managed Codex executable", () => {
    const root = makeTempDir(tempDirs, "nodoassist-codex-on-demand-missing-bin-");
    createCodexInstallFixture(root);
    rmSync(
      path.join(
        root,
