...
describe("check-memory-fd-repro", () => {
  ...
  it("rejects loose numeric environment limits before generating files", () => {
    expect(
      withEnv(
        {
          NODOASSIST_MEMORY_FD_REPRO_FILES: "17",
          NODOASSIST_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "0",
          NODOASSIST_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "0",
          NODOASSIST_MEMORY_FD_REPRO_SETTLE_DELAY_MS: String(MAX_TIMER_TIMEOUT_MS + 1),
          NODOASSIST_MEMORY_FD_REPRO_TIMEOUT_MS: String(MAX_TIMER_TIMEOUT_MS + 1),
        },
        () => parseArgs([]),
      ),
    ).toMatchObject({
    ...
    expect(() =>
      withEnv({ NODOASSIST_MEMORY_FD_REPRO_FILES: "17files" }, () => parseArgs([])),
    ).toThrow("NODOASSIST_MEMORY_FD_REPRO_FILES must be a non-negative integer");
    expect(() =>
      withEnv({ NODOASSIST_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3" }, () => parseArgs([])),
    ).toThrow("NODOASSIST_MEMORY_FD_REPRO_TIMEOUT_MS must be a non-negative integer");
  });
  ...
  it("lets explicit CLI numeric flags override malformed inherited env defaults", () => {
    expect(
      withEnv(
        {
          NODOASSIST_MEMORY_FD_REPRO_FILES: "17files",
          NODOASSIST_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "4fds",
          NODOASSIST_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3",
          NODOASSIST_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "soon",
          NODOASSIST_MEMORY_FD_REPRO_SETTLE_DELAY_MS: "later",
        },
        () =>
          parseArgs([
            "--files",
  ...
  it("stops parsing options after the argument terminator", () => {
    ...
    expect(
      withEnv({ NODOASSIST_MEMORY_FD_REPRO_FILES: "17" }, () => parseArgs(["--", "--unknown"])),
    ).toMatchObject({
      fileCount: 17,
    });
  });
  ...
  it("writes an offline FTS-only memory search config for repro indexing", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-memory-fd-config-"));
    try {
