...
function makeSkipInventoryFixture(): string {
  const repoRoot = createTempDir("nodoassist-test-skip-inventory-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
...
`,
  ...
  writeRepoFile(
    ...
    `
const LIVE = process.env.NODOASSIST_LIVE_TEST === "1";
...
describe("collectTestSkipInventoryReport", () => {
  ...
  it("renders a compact non-blocking text report", () => {
    ...
    expect(rendered).toContain("NodoAssist test skip inventory");
  ...
  it("rejects missing CLI repo roots and loose limits before scanning", () => {
    ...
    expect(() =>
      runTestSkipInventory([
        ...
        "1e3",
        "--repo-root",
        createTempDir("nodoassist-skip-limit-"),
      ]),
