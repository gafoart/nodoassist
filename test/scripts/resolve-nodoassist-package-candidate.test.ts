// Resolve NodoAssist Package Candidate tests cover resolve nodoassist package candidate script behavior.
...
import {
  ...
  cleanPackedNodoAssistTarballsForTest,
  downloadUrl,
  ...
  validateNodoAssistPackageSpec,
} from "../../scripts/resolve-nodoassist-package-candidate.mjs";
...
describe("resolve-nodoassist-package-candidate", () => {
  it("accepts only NodoAssist release package specs for npm candidates", () => {
    for (const spec of [
      "nodoassist@beta",
      "nodoassist@alpha",
      "nodoassist@extended-stable",
      "nodoassist@latest",
      "nodoassist@2026.4.27",
      "nodoassist@2026.4.27-1",
      "nodoassist@2026.4.27-beta.2",
      "nodoassist@2026.4.27-alpha.2",
    ]) {
      expect(validateNodoAssistPackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateNodoAssistPackageSpec("@evil/nodoassist@1.0.0")).toThrow(
      "package_spec must be nodoassist@alpha",
    );
    expect(() => validateNodoAssistPackageSpec("nodoassist@canary")).toThrow(
      "package_spec must be nodoassist@alpha",
    );
    expect(() => validateNodoAssistPackageSpec("nodoassist@2026.04.27")).toThrow(
      "package_spec must be nodoassist@alpha",
    );
    expect(() => validateNodoAssistPackageSpec("nodoassist@npm:other-package")).toThrow(
      "package_spec must be nodoassist@alpha",
    );
    expect(() => validateNodoAssistPackageSpec("nodoassist@file:../other-package.tgz")).toThrow(
      "package_spec must be nodoassist@alpha",
    );
  });
  ...
  it("parses optional empty workflow inputs without rejecting the command line", () => {
    expect(
      ...
      outputName: "nodoassist-current.tgz",
      packageSha256: "",
      packageRef: "release/2026.4.27",
      packageSpec: "nodoassist@beta",
      packageUrl: "",
      source: "npm",
      trustedSourceId: "",
  ...
  it("rejects package candidate output names that escape the output directory", () => {
    for (const outputName of [
      "../nodoassist-current.tgz",
      "nested/nodoassist-current.tgz",
      "nodoassist-current.zip",
      ".nodoassist-current.tgz",
    ]) {
    ...
    expect(parseArgs(["--output-name", "nodoassist-current.tar.gz"]).outputName).toBe(
      "nodoassist-current.tar.gz",
    );
  });
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "nodoassist@2026.5.26-beta.1",
      "C:\\nodoassist\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack nodoassist@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\nodoassist\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("keeps npm pack filenames inside the package candidate output directory", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "nodoassist-2026.6.17.tgz"), "package");

    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify([{ filename: "nodoassist-2026.6.17.tgz" }]),
        "nodoassist-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "nodoassist-current.tgz"));
    await expect(readFile(path.join(dir, "nodoassist-current.tgz"), "utf8")).resolves.toBe(
      "package",
    );
  });
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/nodoassist-2026.6.17.tgz",
      "nested\\nodoassist-2026.6.17.tgz",
      "/tmp/nodoassist-2026.6.17.tgz",
      "C:\\temp\\nodoassist-2026.6.17.tgz",
      "nodoassist-2026.6.17.tar.gz",
    ];
    ...
    for (const filename of unsafeFilenames) {
      ...
      ).rejects.toThrow("npm pack reported unsafe NodoAssist tarball filename");
    }
  });
  ...
  it("rejects unsafe text npm pack filenames instead of using loose stdout fallback", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "nodoassist-2026.6.17.tgz"), "safe fallback");

    for (const filename of ["../nodoassist-2026.6.17.tgz", "C:nodoassist-2026.6.17.tgz"]) {
      await expect(
        moveNewestPackedTarballForTest(
          dir,
  ...
  it("cleans stale package tarballs before npm fallback scanning", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-npm-pack-stale-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "nodoassist-9999.1.1.tgz"), "stale");
    await writeFile(path.join(dir, "nodoassist-C:evil.tgz"), "unsafe");

    await cleanPackedNodoAssistTarballsForTest(dir);
    await writeFile(path.join(dir, "nodoassist-2026.6.17.tgz"), "current");

    await expect(
      moveNewestPackedTarballForTest(dir, "npm notice\n", "nodoassist-current.tgz"),
    ).resolves.toBe(path.join(dir, "nodoassist-current.tgz"));
    await expect(missing(path.join(dir, "nodoassist-9999.1.1.tgz"))).resolves.toBe(true);
    await expect(readFile(path.join(dir, "nodoassist-C:evil.tgz"), "utf8")).resolves.toBe("unsafe");
    await expect(readFile(path.join(dir, "nodoassist-current.tgz"), "utf8")).resolves.toBe(
      "current",
    );
  });
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("clamps oversized package runner kill grace before scheduling", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-runner-grace-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
  ...
  it("rejects timed-out package runner commands when descendants exit cleanly", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-runner-timeout-clean-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    ...
    const childScript = [
      "const fs = require('node:fs');",
      "process.on('SIGTERM', () => {",
      "  fs.writeFileSync(process.env.NODOASSIST_TEST_CHILD_CLEANUP, 'clean');",
      "  setTimeout(() => process.exit(0), 25);",
      "});",
      "fs.writeFileSync(process.env.NODOASSIST_TEST_CHILD_READY, 'ready');",
      "setInterval(() => {}, 1000);",
    ].join("");
    const parentScript = [
      "const { spawn } = require('node:child_process');",
      ...
      "  env: {",
      "    ...process.env,",
      "    NODOASSIST_TEST_CHILD_CLEANUP: process.env.NODOASSIST_TEST_CHILD_CLEANUP,",
      "    NODOASSIST_TEST_CHILD_READY: process.env.NODOASSIST_TEST_CHILD_READY,",
      "  },",
      "});",
      "fs.writeFileSync(process.env.NODOASSIST_TEST_CHILD_PID, String(child.pid));",
    ...
    const timeoutAssertion = expect(
      runCommandForTest(process.execPath, ["-e", parentScript], {
        env: {
          ...process.env,
          NODOASSIST_TEST_CHILD_CLEANUP: cleanupPath,
          NODOASSIST_TEST_CHILD_PID: childPidPath,
          NODOASSIST_TEST_CHILD_READY: readyPath,
        },
        killAfterMs: 250,
        timeoutMs: 250,
      }),
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-nodoassist-package-candidate.mjs"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/nodoassist-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/nodoassist-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/nodoassist/"],
            ports: [443, 8443],
  ...
  it("rejects unsafe package_url downloads before fetching private targets", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-download-"));
    tempDirs.push(dir);
    const target = path.join(dir, "nodoassist.tgz");
  ...
  it("does not forward trusted package auth headers to redirect hosts", async () => {
    ...
    tempDirs.push(dir);
    const target = path.join(dir, "nodoassist.tgz");
    const previousToken = process.env.NODOASSIST_TRUSTED_PACKAGE_TOKEN;
    process.env.NODOASSIST_TRUSTED_PACKAGE_TOKEN = "token-123";
    const trustedSource = {
      allowPrivateNetwork: true,
      auth: { type: "bearer" },
      hosts: ["packages.internal"],
    ...
    try {
      ...
      if (previousToken === undefined) {
        delete process.env.NODOASSIST_TRUSTED_PACKAGE_TOKEN;
      } else {
        process.env.NODOASSIST_TRUSTED_PACKAGE_TOKEN = previousToken;
      }
    }
  ...
  it("times out stalled package_url response bodies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-download-timeout-"));
    tempDirs.push(dir);
    const target = path.join(dir, "nodoassist.tgz");
    let bodyCancelled = false;
    const startedAt = Date.now();
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("normalizes artifact package source SHAs before workflow output", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-candidate-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("normalizes whitespace-only artifact package source SHAs to absent", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-candidate-empty-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("rejects malformed artifact package source SHAs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-candidate-bad-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("accepts uppercase package artifact SHA-256 metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-sha-"));
    tempDirs.push(dir);
    const file = path.join(dir, "nodoassist.tgz");
    await writeFile(file, "nodoassist package bytes");
  ...
  it("rejects source artifact scans that exceed the filesystem entry limit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-artifact-scan-"));
    tempDirs.push(dir);
    const maxEntries = 3;
  ...
  it("rejects source artifact directories with multiple tarballs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-artifact-duplicates-"));
    tempDirs.push(dir);

    await writeFile(path.join(dir, "nodoassist-a.tgz"), "a");
    ...
    expect(message).toContain("nodoassist-a.tgz");
    expect(message).not.toContain(path.join(dir, "nested.tar.gz"));
    expect(message).not.toContain(path.join(dir, "nodoassist-a.tgz"));
  });

  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "nodoassist-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "nodoassist" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "nodoassist.tgz");
