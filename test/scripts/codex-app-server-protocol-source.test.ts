...
const originalNodoAssistCodexRepo = process.env.NODOASSIST_CODEX_REPO;

afterEach(() => {
  if (originalNodoAssistCodexRepo === undefined) {
    delete process.env.NODOASSIST_CODEX_REPO;
  } else {
    process.env.NODOASSIST_CODEX_REPO = originalNodoAssistCodexRepo;
  }
});
...
`),
  ...
  it("rejects a Codex checkout that differs from the pinned package version", async () => {
    const repoRoot = createTempDir("nodoassist-protocol-version-root-");
    const codexRepo = createTempDir("nodoassist-protocol-version-codex-");
  ...
  it("allows an explicit local disk headroom override", () => {
    expect(
      resolveCodexProtocolMinFreeBytes({ NODOASSIST_CODEX_PROTOCOL_MIN_FREE_BYTES: "0" }),
    ).toBe(0);
    expect(() =>
      validateCodexProtocolGenerationHeadroom({
        freeBytes: 1,
  ...
  it("rejects malformed local disk headroom overrides", () => {
    expect(() =>
      resolveCodexProtocolMinFreeBytes({ NODOASSIST_CODEX_PROTOCOL_MIN_FREE_BYTES: "nope" }),
  ...
  it("uses NODOASSIST_CODEX_REPO when provided", async () => {
    const root = createTempDir("nodoassist-protocol-source-root-");
    const codexRepo = createTempDir("nodoassist-protocol-source-codex-");
    createProtocolSchema(codexRepo);
    process.env.NODOASSIST_CODEX_REPO = codexRepo;
  ...
  it("finds the primary checkout sibling from a git worktree", async () => {
    const parentDir = createTempDir("nodoassist-protocol-source-parent-");
    const primaryNodoAssist = path.join(parentDir, "nodoassist");
    const codexRepo = path.join(parentDir, "codex");
    const worktreeRoot = createTempDir("nodoassist-protocol-source-worktree-");
    fs.mkdirSync(path.join(primaryNodoAssist, ".git", "worktrees", "codex-harness"), {
      recursive: true,
    });
    fs.mkdirSync(worktreeRoot, { recursive: true });
    fs.writeFileSync(
      path.join(worktreeRoot, ".git"),
      `gitdir: ${path.join(primaryNodoAssist, ".git", "worktrees", "codex-harness")}\n`,
    );
    createProtocolSchema(codexRepo);
    delete process.env.NODOASSIST_CODEX_REPO;
