...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "nodoassist-startup-memory-test-"));
  tempRoots.push(root);
  return root;
}
...
describe("check-cli-startup-memory", () => {
  ...
  it("keeps invalid startup memory env values from bypassing budgets", () => {
    expect(() =>
      testing.readPositiveNumberEnv("NODOASSIST_STARTUP_MEMORY_HELP_MB", 100, {
        NODOASSIST_STARTUP_MEMORY_HELP_MB: "abc",
      }),
    ).toThrow("NODOASSIST_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("NODOASSIST_STARTUP_MEMORY_HELP_MB", 100, {
        NODOASSIST_STARTUP_MEMORY_HELP_MB: "1e3",
      }),
    ).toThrow("NODOASSIST_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("NODOASSIST_STARTUP_MEMORY_HELP_MB", 100, {
        NODOASSIST_STARTUP_MEMORY_HELP_MB: "0x10",
      }),
    ).toThrow("NODOASSIST_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("NODOASSIST_STARTUP_MEMORY_HELP_MB", 100, {
        NODOASSIST_STARTUP_MEMORY_HELP_MB: "0",
      }),
    ).toThrow("NODOASSIST_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(
      testing.readPositiveNumberEnv("NODOASSIST_STARTUP_MEMORY_HELP_MB", 100, {
        NODOASSIST_STARTUP_MEMORY_HELP_MB: "125.5",
      }),
    ).toBe(125.5);
  });
  ...
  it("keeps invalid startup memory timeout env values from parsing loosely", () => {
    expect(() =>
      testing.readPositiveIntEnv("NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS must be a positive number");
    expect(() =>
      testing.readPositiveIntEnv("NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS: "1000.5",
      }),
    ).toThrow("NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS must be a positive integer");
    expect(() =>
      testing.readPositiveIntEnv("NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS: String(Number.MAX_SAFE_INTEGER + 1),
      }),
    ).toThrow("NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS must be a positive integer");
    expect(
      testing.readPositiveIntEnv("NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        NODOASSIST_STARTUP_MEMORY_TIMEOUT_MS: "1000",
      }),
    ).toBe(1000);
  });
      ...
      testing.runStartupMemoryCheck(
        ...
        ],
        {
          ...
          spawnSync: () => ({
            signal: null,
            status: 0,
            stderr: "__NODOASSIST_MAX_RSS_KB__=0\n",
            stdout: "",
          }),
        },
      ),
      ...
      ],
      {
        ...
        spawnSync: (_command: string, args: string[]) => {
          ...
          return {
            ...
            signal: null,
            status: 0,
            stderr: "__NODOASSIST_MAX_RSS_KB__=1024\n",
            stdout: "",
          };
        },
      },
