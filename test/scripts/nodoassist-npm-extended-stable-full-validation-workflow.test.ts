...
const releaseChecksPath = ".github/workflows/nodoassist-release-checks.yml";
  ...
  it("lets the exact extended-stable branch reach every child at the target SHA", () => {
    ...
    const childDispatches = [
      ...
      },
      {
        ...
        workflow: "nodoassist-release-checks.yml",
        target: '-f expected_sha="$TARGET_SHA"',
      },
      {
        job: "performance",
        step: "Dispatch and monitor NodoAssist Performance",
        workflow: "nodoassist-performance.yml",
        target: '-f target_ref="$TARGET_SHA"',
      },
    ];
