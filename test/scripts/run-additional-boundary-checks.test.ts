...
describe("run-additional-boundary-checks", () => {
  ...
  it("rejects malformed timeout and output limit integers", () => {
    expect(resolvePositiveInteger("25", 50, "NODOASSIST_ADDITIONAL_BOUNDARY_TIMEOUT_MS")).toBe(25);
    expect(resolvePositiveInteger(undefined, 50, "NODOASSIST_ADDITIONAL_BOUNDARY_TIMEOUT_MS")).toBe(
      50,
    );
    expect(() =>
      resolvePositiveInteger("1000ms", 50, "NODOASSIST_ADDITIONAL_BOUNDARY_TIMEOUT_MS"),
    ).toThrow("NODOASSIST_ADDITIONAL_BOUNDARY_TIMEOUT_MS must be a positive integer; got: 1000ms");
    expect(() =>
      resolvePositiveInteger("1e3", 50, "NODOASSIST_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES"),
    ).toThrow(
      "NODOASSIST_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES must be a positive integer; got: 1e3",
    );
  });
  ...
  it("parses CLI help and shard args before running checks", () => {
    ...
    expect(parseCliArgs([], { NODOASSIST_ADDITIONAL_BOUNDARY_SHARD: "4/4" })).toEqual({
      help: false,
      shardSpec: "4/4",
    });
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-boundary-timeout-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-boundary-signal-"));
      ...
      try {
        ...
        const parentScript = [
          ...
          `const child = spawn(process.execPath, ['-e', ${JSON.stringify(childScript)}], { stdio: 'ignore' });`,
          "fs.writeFileSync(process.env.NODOASSIST_TEST_CHILD_PID, String(child.pid));",
          "fs.writeFileSync(process.env.NODOASSIST_TEST_READY, 'ready');",
...
`;
        ...
        runner = spawn(process.execPath, ["--input-type=module", "--eval", runnerScript], {
          ...
          env: {
            ...
            NODOASSIST_TEST_READY: readyPath,
          },
