...
const AUTH_MONITOR_SERVICE_PATH = "scripts/systemd/nodoassist-auth-monitor.service";
const AUTH_MONITOR_TIMER_PATH = "scripts/systemd/nodoassist-auth-monitor.timer";
...
describe("auth monitoring scripts", () => {
  it("keeps systemd install rendering free of checked-in host paths", () => {
    ...
    expect(service).toContain("ExecStart=@NODOASSIST_AUTH_MONITOR_PATH@");
  ...
  it("keeps public helper scripts free of private host defaults", () => {
    ...
    expect(joined).toContain("Run on the NodoAssist host: ${SCRIPT_DIR}/mobile-reauth.sh");
    for (const script of TERMUX_WIDGET_PATHS.map(readScript)) {
      expect(script).toContain('SERVER="${NODOASSIST_SERVER:-nodoassist-host}"');
    }
    expect(readScript("scripts/termux-sync-widget.sh")).toContain(
      "'$HOME/nodoassist/scripts/sync-claude-code-auth.sh'",
    );
  });
  ...
  it("keeps mobile reauth wired to local auth status and Claude token setup", () => {
    ...
    expect(script).toContain("systemctl --user restart nodoassist");
  });
});
