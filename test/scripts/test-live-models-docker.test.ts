...
describe("scripts/test-live-models-docker.sh", () => {
  it("validates optional live model limits before auth or Docker setup", () => {
    ...
    expect(script).toContain('LIVE_MAX_MODELS="${NODOASSIST_LIVE_MAX_MODELS:-}"');
    expect(script).toContain('[[ -n "$LIVE_MAX_MODELS" && ! "$LIVE_MAX_MODELS" =~ ^\\+?[0-9]+$ ]]');
    expect(script).toContain(
      'nodoassist_live_read_positive_int_env NODOASSIST_LIVE_MODEL_TIMEOUT_MS "$LIVE_MODEL_TIMEOUT_MS"',
    );
    expect(script).toContain('-e NODOASSIST_LIVE_MAX_MODELS="$LIVE_MAX_MODELS"');
    expect(script).toContain('-e NODOASSIST_LIVE_MODEL_TIMEOUT_MS="$LIVE_MODEL_TIMEOUT_MS"');
  });

  it.each([
    ["max models", "NODOASSIST_LIVE_MAX_MODELS", "3models"],
    ["model timeout", "NODOASSIST_LIVE_MODEL_TIMEOUT_MS", "45s"],
