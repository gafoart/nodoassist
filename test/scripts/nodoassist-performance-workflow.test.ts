// NodoAssist Performance Workflow tests cover nodoassist performance workflow script behavior.
import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

const WORKFLOW = ".github/workflows/nodoassist-performance.yml";
...
describe("NodoAssist performance workflow", () => {
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");

    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('NodoAssist Performance {0}', inputs.dispatch_id) || 'NodoAssist Performance' }}",
    );
  ...
  it("resolves dispatch target refs before checkout", () => {
    const resolveTarget = findStep("Resolve NodoAssist target ref");
    const checkout = findStep("Checkout NodoAssist");
  ...
  it("installs local workspace packages beside the OCM root tarball", () => {
    ...
    expect(configure.run).toContain(
      "NODOASSIST_OCM_WORKSPACE_DEPENDENCY_DIRS=$workspace_dependency_dirs",
    );
  });
