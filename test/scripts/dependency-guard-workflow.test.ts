...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectSteps[1].env?.NODOASSIST_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubSteps[3].env?.NODOASSIST_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubSteps[3].env?.NODOASSIST_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalSteps[1].env?.NODOASSIST_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.NODOASSIST_SECURITY_TEAM_SLUG).toBe("nodoassist-secops");
    expect(runStep.env?.NODOASSIST_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");
  ...
  it("blocks package lockfile and manifest graph changes unless secops approves the current head sha", () => {
    ...
    expect(script).toContain("nodoassist-secops");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-guard.yml @nodoassist/nodoassist-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-guard-workflow.test.ts @nodoassist/nodoassist-secops",
    );
    expect(codeowners).toContain(
      "/scripts/github/dependency-guard.mjs @nodoassist/nodoassist-secops",
    );
    expect(codeowners).toContain("/package-lock.json @nodoassist/nodoassist-secops");
    expect(codeowners).toContain("/npm-shrinkwrap.json @nodoassist/nodoassist-secops");
    expect(codeowners).toContain("/extensions/*/package-lock.json @nodoassist/nodoassist-secops");
    expect(codeowners).toContain("/extensions/*/npm-shrinkwrap.json @nodoassist/nodoassist-secops");
  });
});
