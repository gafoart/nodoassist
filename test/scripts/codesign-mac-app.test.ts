...
function entitlementTemps(dir: string): string[] {
  return readdirSync(dir).filter((name) => name.startsWith("nodoassist-entitlements"));
}
...
describe("codesign-mac-app temp file hygiene", () => {
  ...
  it("does not allocate entitlement temp files for help output", () => {
    const tempRoot = makeTempDir("nodoassist-codesign-help-");
  ...
  it("does not allocate entitlement temp files before app validation", () => {
    const tempRoot = makeTempDir("nodoassist-codesign-missing-");
  ...
  it("rejects unknown options before app validation", () => {
    const tempRoot = makeTempDir("nodoassist-codesign-unknown-");
  ...
  it("rejects extra app bundle arguments before signing", () => {
    const tempRoot = makeTempDir("nodoassist-codesign-extra-");
  ...
  it("cleans entitlement temp files when signing fails", () => {
    const tempRoot = makeTempDir("nodoassist-codesign-fail-");
    const app = path.join(tempRoot, "Fake.app");
    mkdirSync(path.join(app, "Contents", "MacOS"), { recursive: true });
  ...
  it("passes generated app entitlements to signing commands and cleans them", () => {
    const tempRoot = makeTempDir("nodoassist-codesign-success-");
    const app = path.join(tempRoot, "Fake.app");
    ...
    writeFileSync(path.join(app, "Contents", "MacOS", "NodoAssist"), "#!/bin/sh\n");
    ...
    expect(signLines[0]).toContain(`${path.join(app, "Contents", "MacOS", "NodoAssist")}\t`);
    ...
    for (const line of signLines) {
      ...
      expect(entitlementPath).toContain("nodoassist-entitlements");
