...
import { buildMarkdown, parseArgs } from "../../scripts/nodoassist-performance-source-summary.mjs";

const tmpRoots: string[] = [];

function mkTmpRoot() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-source-summary-"));
  tmpRoots.push(root);
  return root;
}
...
function runCli(...args: string[]) {
  return spawnSync(
    process.execPath,
    ["scripts/nodoassist-performance-source-summary.mjs", ...args],
    {
      cwd: path.resolve("."),
      encoding: "utf8",
    },
