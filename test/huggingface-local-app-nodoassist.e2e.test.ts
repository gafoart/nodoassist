...
// Hugging Face local-app CLI contract tests cover NodoAssist's snippet-facing commands.
import { createServer, type IncomingMessage, type ServerResponse } from "node:http";
import type { AddressInfo } from "node:net";
import { afterEach, describe, expect, it } from "vitest";
import {
  createNodoAssistTestInstance,
  type NodoAssistTestInstance,
} from "./helpers/nodoassist-test-instance.js";
...
const instances: NodoAssistTestInstance[] = [];
...
describe("Hugging Face NodoAssist local-app CLI contract", () => {
  it.each(HF_LOCAL_APP_CASES)(
    ...
    async ({ providerId, modelId }) => {
      ...
      const instance = await createNodoAssistTestInstance({
...
async function handleFakeOpenAiRequest(
  ...
  writeSse(res, {
    id: "chatcmpl-hf-nodoassist-contract",
