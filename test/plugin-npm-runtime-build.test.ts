...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.nodoassist).toBe(
        plan.packageJson.nodoassist.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.nodoassist.optional).toBe(true);
    }
  });
