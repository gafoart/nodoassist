...
const LIVE_TEST_TRIGGER_ENV_KEYS = [
  "LIVE",
  "NODOASSIST_LIVE_TEST",
  "NODOASSIST_LIVE_GATEWAY",
] as const;
const HERMETIC_TEST_ENV_KEYS = [
  ...LIVE_TEST_TRIGGER_ENV_KEYS,
  "NODOASSIST_LIVE_USE_REAL_HOME",
  "NODOASSIST_BUNDLED_PLUGINS_DIR",
  "NODOASSIST_TEST_TRUST_BUNDLED_PLUGINS_DIR",
...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.NODOASSIST_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    ...
    { key: "NODOASSIST_TEST_FAST", value: process.env.NODOASSIST_TEST_FAST },
    {
      key: "NODOASSIST_STRICT_FAST_REPLY_CONFIG",
      value: process.env.NODOASSIST_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "NODOASSIST_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.NODOASSIST_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "NODOASSIST_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.NODOASSIST_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "NODOASSIST_STATE_DIR", value: process.env.NODOASSIST_STATE_DIR },
    { key: "NODOASSIST_CONFIG_PATH", value: process.env.NODOASSIST_CONFIG_PATH },
    { key: "NODOASSIST_GATEWAY_PORT", value: process.env.NODOASSIST_GATEWAY_PORT },
    { key: "NODOASSIST_BRIDGE_ENABLED", value: process.env.NODOASSIST_BRIDGE_ENABLED },
    { key: "NODOASSIST_BRIDGE_HOST", value: process.env.NODOASSIST_BRIDGE_HOST },
    { key: "NODOASSIST_BRIDGE_PORT", value: process.env.NODOASSIST_BRIDGE_PORT },
    { key: "NODOASSIST_CANVAS_HOST_PORT", value: process.env.NODOASSIST_CANVAS_HOST_PORT },
    { key: "NODOASSIST_TEST_HOME", value: process.env.NODOASSIST_TEST_HOME },
    { key: "NODOASSIST_AGENT_DIR", value: process.env.NODOASSIST_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-test-home-"));

  setTestEnvValue("HOME", tempHome);
  setTestEnvValue("USERPROFILE", tempHome);
  setTestEnvValue("NODOASSIST_TEST_HOME", tempHome);
  setTestEnvValue("NODOASSIST_TEST_FAST", "1");
  setTestEnvValue("NODOASSIST_STRICT_FAST_REPLY_CONFIG", "1");
  deleteTestEnvValue("NODOASSIST_ALLOW_SLOW_REPLY_TESTS");
  ...
  deleteTestEnvValue("NODOASSIST_AGENT_DIR");
  ...
  deleteTestEnvValue("NODOASSIST_BRIDGE_ENABLED");
  deleteTestEnvValue("NODOASSIST_BRIDGE_HOST");
  deleteTestEnvValue("NODOASSIST_BRIDGE_PORT");
  deleteTestEnvValue("NODOASSIST_CANVAS_HOST_PORT");
  ...
  if (process.platform === "win32") {
    setTestEnvValue("NODOASSIST_STATE_DIR", path.join(tempHome, ".nodoassist"));
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.NODOASSIST_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.NODOASSIST_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".nodoassist");
  const priorIsolatedHome = params.env.NODOASSIST_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".nodoassist")
  ) {
    realStateDir = path.join(params.realHome, ".nodoassist");
  }
  const tempStateDir = path.join(params.tempHome, ".nodoassist");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.NODOASSIST_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.NODOASSIST_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "nodoassist.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "nodoassist.json"),
...
export function installTestEnv(options?: InstallTestEnvOptions): {
  ...
  const allowRealHome = !hermetic && isTruthyEnvValue(process.env.NODOASSIST_LIVE_USE_REAL_HOME);
  ...
  if (hermetic) {
    ...
    setTestEnvValue("NODOASSIST_TEST_TRUST_BUNDLED_PLUGINS_DIR", "1");
    setTestEnvValue(
      "NODOASSIST_BUNDLED_PLUGINS_DIR",
