...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${NODOASSIST_CONFIG_PATH:-${NODOASSIST_STATE_DIR:-$HOME/.nodoassist}/nodoassist.json}"
...
```text
${NODOASSIST_STATE_DIR:-$HOME/.nodoassist}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with nodoassist message send if route provided.
```
