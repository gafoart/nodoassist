...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.nodoassist`, `$NODOASSIST_STATE_DIR`, or active NodoAssist state dirs.
...
- Worker must send completion/failure via `nodoassist message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/nodoassist`; use an isolated checkout.
...
nodoassist message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use nodoassist system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t nodoassist-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
