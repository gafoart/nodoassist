...
The NodoAssist macOS app hosts Peekaboo Bridge at
`~/Library/Application Support/NodoAssist/bridge.sock`. Before running Peekaboo
from NodoAssist, select that socket so the CLI uses the app's Screen Recording
and Accessibility grants instead of starting its standalone daemon:

```bash
export PEEKABOO_BRIDGE_SOCKET="${PEEKABOO_BRIDGE_SOCKET:-$HOME/Library/Application Support/NodoAssist/bridge.sock}"
```

Confirm routing with `peekaboo bridge status --json`; `hostKind` must be `gui`
and the socket path must end in `NodoAssist/bridge.sock`.
...
- In NodoAssist subprocesses, set `PEEKABOO_BRIDGE_SOCKET` as shown above. Do not
