---
name: release-nodoassist-announcement
description: "Draft or post NodoAssist beta/stable Discord release announcements from changelog, GitHub release, registry, and validation evidence. Use when announcing a beta, stable release, release candidate, or asking what users should test after an NodoAssist release."
---

# NodoAssist Release Announcement

Use with `release-nodoassist-maintainer` after a beta or stable release is live.
...
- exact version, e.g. `NodoAssist 2026.5.25-beta.1`
...
- update command promoted near the top:
  ```sh
  nodoassist update --channel beta --yes
  nodoassist --version
  ```
- fresh install path:
...
```sh
nodoassist update --channel stable --yes
nodoassist --version
```
...
sending it. Never use NodoAssist channel sends, bots, webhooks, relays, or tokens.
