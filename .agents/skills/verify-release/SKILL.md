---
name: verify-release
description: "Verify an NodoAssist release is fully published across GitHub, npm, plugins, ClawHub, package smoke, and live Gateway agent turns."
---

# Verify Release

Use this when asked whether an NodoAssist release is fully released, published,
promoted, smoke-tested, or live-verified. This is a verification skill, not a
publish skill; use `$release-nodoassist-maintainer` before changing release state.
...
1. GitHub release:
   - `gh release view v<VERSION> --repo nodoassist/nodoassist --json tagName,name,publishedAt,isDraft,isPrerelease,targetCommitish,url,body,assets`
...
2. Root npm:
   - `npm view nodoassist@<VERSION> version dist-tags.latest dist.tarball dist.integrity time.<VERSION> --json`
...
3. Plugin publish set:
   - Get exact tag metadata from GitHub, not the local checkout when dirty:
     ...
     into `/tmp/nodoassist-v<VERSION>-src`.
   - Count `extensions/*/package.json` with
     `nodoassist.release.publishToNpm === true` and
     `nodoassist.release.publishToClawHub === true`.
   - Compare expected counts to workflow job counts:
     `gh api repos/nodoassist/nodoassist/actions/runs/<RUN>/jobs --paginate`.
...
4. ClawHub:
   ...
   - Use NodoAssist itself for live registry proof:
     `nodoassist plugins search <known-plugin> --json`.
   - Install one official plugin from ClawHub in an isolated HOME:
     `nodoassist plugins install clawhub:@nodoassist/matrix --pin`.
     Prefer `matrix` unless that plugin is not in the expected set.
5. Release workflows:
   - Verify conclusions for release notes evidence links:
     Full Release Validation, NodoAssist Release Checks, NodoAssist NPM Release,
     Plugin NPM Release, Plugin ClawHub Release, mac preflight/validation/publish
     when stable mac assets are expected.
   - For stable, verify `NodoAssist Stable Main Closeout` succeeded and its
...
6. Published package smoke:
   - In `/tmp`, isolated HOME:
     `npm exec --yes --package nodoassist@<VERSION> -- nodoassist --version`.
...
7. Dev Gateway live model smoke:
   - Use temp HOME/workspace, not the user's normal state:
     `HOME=/tmp/nodoassist-release-smoke/home NODOASSIST_WORKSPACE=/tmp/nodoassist-release-smoke/work pnpm nodoassist --dev gateway run --auth none --force --verbose`.
   - Health check via CLI: `nodoassist --dev gateway health --json`.
