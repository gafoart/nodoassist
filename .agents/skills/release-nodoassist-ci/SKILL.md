---
name: release-nodoassist-ci
description: "Run, watch, debug, and summarize NodoAssist full release CI, release checks, live provider gates, install/update proofs, and release-secret preflights."
---

# NodoAssist Release CI

Use this with `$release-nodoassist-maintainer` and `$nodoassist-testing` when a release candidate needs full validation, install/update proof, live provider checks, or CI recovery.
...
```bash
node .agents/skills/release-nodoassist-ci/scripts/verify-provider-secrets.mjs --required openai,anthropic,fireworks
...
```bash
gh workflow run nodoassist-performance.yml \
...
Publish with `nodoassist-release-publish.yml` using `release_profile=from-validation`
...
```bash
node .agents/skills/release-nodoassist-ci/scripts/release-ci-summary.mjs <full-release-run-id>
```

Then watch only when useful:

```bash
gh run watch <full-release-run-id> --repo nodoassist/nodoassist --exit-status
```
...
2. List failed jobs only:
   ```bash
   gh run view <child-run-id> --repo nodoassist/nodoassist --json jobs \
