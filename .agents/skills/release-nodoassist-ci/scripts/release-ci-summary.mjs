...
const repo = process.env.NODOASSIST_RELEASE_REPO || "nodoassist/nodoassist";
...
function githubRestJson(pathSuffix) {
  const result = execFileSync(
    ...
    [
      ...
      [
        "set -euo pipefail",
        'token="$("$NODOASSIST_PLAIN_GH_BIN" auth token)"',
    ...
    ],
    {
      ...
      env: {
        ...plainGhEnv(),
        NODOASSIST_PLAIN_GH_BIN: resolvePlainGhBin(),
        NODOASSIST_GITHUB_REST_URL: `https://api.github.com/repos/${repo}/${pathSuffix}`,
      },
