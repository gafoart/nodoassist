...
const repo = "nodoassist/nodoassist";
const commitAssociationQueryBatchSize = 20;
const excludedHandles = new Set(["nodoassist", "clawsweeper", "claude", "codex", "steipete"]);
...
function closingReferencesIn(text) {
  const references = [];
  for (const match of text.matchAll(
    /\b(?:fix(?:es|ed)?|closes?|closed|resolves?|resolved)\s+(?:(?:nodoassist\/nodoassist)?#\d+)(?:\s*(?:,|and)\s*(?:(?:nodoassist\/nodoassist)?#\d+))*/gi,
  )) {
...
function resolveAssociatedPullRequests(commitHashes, targetTimestamp) {
  ...
  for (let index = 0; index < commitHashes.length; index += commitAssociationQueryBatchSize) {
    ...
    const fields = chunk
      .map(
        (hash, offset) =>
          `c${index + offset}: repository(owner: "nodoassist", name: "nodoassist") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map(
        (item, offset) =>
          `c${offset}: repository(owner: "nodoassist", name: "nodoassist") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map((item, offset) => {
        ...
        return `n${offset}: repository(owner: "nodoassist", name: "nodoassist") {
...
function resolveReferences(numbers) {
  ...
  for (let index = 0; index < numbers.length; index += 40) {
    ...
    const fields = chunk
      .map(
        (number) => `n${number}: repository(owner: "nodoassist", name: "nodoassist") {
