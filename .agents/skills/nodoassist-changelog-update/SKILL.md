---
name: nodoassist-changelog-update
description: Regenerate NodoAssist release changelog sections from git history before beta or stable releases.
---

# NodoAssist Changelog Update
...
rerun. Use it with `release-nodoassist-maintainer`; this skill owns changelog
...
3. Generate the complete contribution record and editorial manifest before
   ...
   ```bash
   node .agents/skills/nodoassist-changelog-update/scripts/verify-release-notes.mjs \
     --base <base-tag> \
     --target <target-ref> \
     --version <YYYY.M.PATCH> \
     --manifest /tmp/nodoassist-release-<YYYY.M.PATCH>.json \
     --write-ledger
   ```
...
6. Preserve attribution:
   ...
   - never thank bots, `@claude`, `@nodoassist`, `@clawsweeper`, or `@steipete`
...
- unqualified `#NNN` references resolve against `nodoassist/nodoassist`;
  cross-repository references such as `nodoassist/imsg#141` remain literal
