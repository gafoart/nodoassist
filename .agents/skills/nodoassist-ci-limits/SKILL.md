---
name: nodoassist-ci-limits
description: Manage NodoAssist GitHub Actions and Blacksmith CI capacity, runner-registration budgets, fanout caps, main-push debounce, shard sizing, hosted-runner offload, queue health, and safe ramp-down/ramp-up changes. Use when tuning `.github/workflows/*`, `docs/ci.md`, CI runner labels, matrix `max-parallel`, ClawSweeper/Blacksmith burst protection, CodeQL runner placement, or investigating slow/queued NodoAssist CI.
---

# NodoAssist CI Limits

Use this skill for CI capacity changes, not ordinary test failure triage. The
goal is to keep NodoAssist fast while staying below GitHub's self-hosted runner
...
- The scarce resource is Blacksmith runner registrations, not Blacksmith vCPU
  capacity.
- GitHub runner registrations for `nodoassist` currently report a 10,000 per
  5-minute bucket in `actions_runner_registration`. Verify the live bucket
  before each tuning pass because GitHub can change it. The `nodoassist`
...
ghx run list -R nodoassist/nodoassist --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
ghx run list -R nodoassist/clawsweeper --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
...
- **NodoAssist test runtime:** jobs start quickly but one lane dominates wall time.
  Use `$nodoassist-test-performance` instead of runner tuning.
- **Real failing CI:** one job fails after starting. Use `$github:gh-fix-ci` or
  `$nodoassist-testing`, not this skill.
- **ClawSweeper backlog:** exact-review queue grows while CI is healthy. Tune
  ClawSweeper workers in `nodoassist/clawsweeper`, not NodoAssist CI.
...
ClawSweeper, ClawHub, Clownfish, NodoAssist RTT, and Clawbench.
...
2. Keep the `runner-admission` hosted debounce for canonical `main` pushes.
   Change `NODOASSIST_MAIN_CI_DEBOUNCE_SECONDS` only with evidence.
...
7. Split truly slow tests with `$nodoassist-test-performance`; do not hide a slow
...
- `runner-admission` on `ubuntu-24.04` with
  `NODOASSIST_MAIN_CI_DEBOUNCE_SECONDS=90`.
...
./node_modules/.bin/oxfmt --check .github/workflows/ci.yml .github/workflows/codeql-critical-quality.yml docs/ci.md test/scripts/ci-workflow-guards.test.ts .agents/skills/nodoassist-ci-limits/SKILL.md .agents/skills/nodoassist-ci-limits/agents/openai.yaml
git diff --check
```
...
ghx pr checks <pr> -R nodoassist/nodoassist --watch --interval 15
```
...
NODOASSIST_TESTBOX=1 scripts/pr prepare-run <pr>
```
...
```bash
ghx run list -R nodoassist/nodoassist --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
for repo in nodoassist/clawsweeper nodoassist/clawhub nodoassist/clownfish nodoassist/nodoassist-rtt nodoassist/clawbench; do
