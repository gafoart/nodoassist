---
name: nodoassist-test-heap-leaks
description: Investigate NodoAssist pnpm test memory growth, Vitest OOMs, RSS spikes, and heap snapshot deltas.
---

# NodoAssist Test Heap Leaks
...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && NODOASSIST_TEST_MEMORY_TRACE=1 NODOASSIST_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 NODOASSIST_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap NODOASSIST_TEST_WORKERS=2 NODOASSIST_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `NODOASSIST_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
...
2. Wait for repeated snapshots before concluding anything.
   ...
   - Use `.agents/skills/nodoassist-test-heap-leaks/scripts/heapsnapshot-delta.mjs` to compare either two files directly or the earliest/latest pair per PID in one lane directory.
...
- Direct comparison:
  - `node .agents/skills/nodoassist-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/nodoassist-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir .tmp/heapsnap/unit-fast-batch-2`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
...
```
node .agents/skills/nodoassist-test-heap-leaks/scripts/heapsnapshot-delta.mjs \
