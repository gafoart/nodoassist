---
name: nodoassist-pr-maintainer
description: Use immediately for any pasted NodoAssist GitHub issue or PR URL/number, and for NodoAssist issue/PR review, triage, duplicate search, opener identity/who wrote it, author account age/activity, comments, labels, close, land, or maintainer evidence checks.
---

# NodoAssist PR Maintainer

Use this skill for maintainer-facing GitHub workflow, not for ordinary code changes.

## Start issue and PR triage with gitcrawl

- Use `$gitcrawl` first anytime you inspect NodoAssist issues or PRs.
...
gitcrawl neighbors nodoassist/nodoassist --number <issue-or-pr-number> --limit 12 --json
gitcrawl search nodoassist/nodoassist --query "<scope or title keywords>" --mode hybrid --json
...
When a maintainer asks Codex to review, triage, fix, or land a specific NodoAssist issue/PR, check assignment before deep work.
...
```bash
gh api "repos/nodoassist/nodoassist/issues/<number>/timeline" --paginate \
...
```bash
gh api -X POST "repos/nodoassist/nodoassist/issues/<number>/assignees" -f 'assignees[]=<login>' >/dev/null
```
...
- Report opener identity as one compact line:
  `By: Jane Doe (@jane, acct 2021-04-03) | NodoAssist: 4 PRs, 2 issues, 11 commits/12mo | GitHub: 9 repos, 86 commits, 9 PRs, 3 issues, 12 reviews`
- Always show recent activity in two lanes: NodoAssist-local PRs, issues, and commits in the last 12 months; and general public GitHub activity over the same window. For linked issue-fixing PRs, include both the PR author and issue opener when they differ.
- Prefer the bundled helper for activity lookups:

```bash
.agents/skills/nodoassist-pr-maintainer/scripts/github-activity.sh <login> [other-login...]
.agents/skills/nodoassist-pr-maintainer/scripts/github-activity.sh --global <login>
```
...
- If the contribution graph is misleading or zero but public events/repos show activity, keep it one line, for example:
  `By: pickaxe (@ProspectOre, acct 2019-08-24) | NodoAssist: 5 PRs, 0 issues, 5 commits/12mo | GitHub: 5 repos, 29 recent events, 100 public own-repo commits; graph=0`
...
- Good manual-close categories:
  - blank or mostly untouched PR template with no concrete NodoAssist problem/fix
...
- Use `--repo nodoassist/nodoassist` with `--match title,body` first when using `gh search`.
...
```bash
gh search prs --repo nodoassist/nodoassist --match title,body --limit 50 -- "auto-update"
gh search issues --repo nodoassist/nodoassist --match title,body --limit 50 -- "auto-update"
gh search issues --repo nodoassist/nodoassist --match title,body --limit 50 \
