---
name: release-nodoassist-plugin-testing
description: Plan and run pre-release NodoAssist plugin validation across bundled plugins, package artifacts, lifecycle commands, doctor/fix, config round-trip, gateway startup, SDK compatibility, Docker E2E, Package Acceptance, and Testbox proof.
---

# NodoAssist Pre-Release Plugin Testing
...
release?" It complements `nodoassist-testing`; use that skill too when choosing
...
the main NodoAssist checkout. Do not run `pnpm install` there. For broad or
...
```bash
NODOASSIST_TESTBOX=1 pnpm check:changed
pnpm run test:extensions:package-boundary:canary
pnpm run test:extensions:package-boundary:compile
pnpm test:docker:plugins
NODOASSIST_PLUGINS_E2E_CLAWHUB=0 pnpm test:docker:plugins
...
```bash
NODOASSIST_BUNDLED_PLUGIN_SWEEP_TOTAL=8 \
NODOASSIST_BUNDLED_PLUGIN_SWEEP_INDEX=<0-7> \
...
Use `source=npm -f package_spec=nodoassist@beta` for published beta proof. Keep
...
Start packaged NodoAssist in Docker with clean state:
...
- `nodoassist status --json` includes plugin diagnostics
- `nodoassist plugins inspect --all --json` is parseable
...
`nodoassist/plugin-sdk/*` subpaths. Assert `plugins inspect` sees its tool,
