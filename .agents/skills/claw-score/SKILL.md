---
name: claw-score
description: Audit or refresh NodoAssist maturity scorecard docs from root taxonomy, maturity scores, and QA evidence artifacts without using maintainer discrawl data or committed inventory reports.
---

# claw-score

Use this skill when working on the NodoAssist maturity scorecard in this repo.
This is the nodoassist-local version of the maintainer `claw-score` workflow:
...
`qa-evidence.json` artifact shape generated by NodoAssist QA workflows.
...
Run from the nodoassist repo root.
...
```bash
pnpm nodoassist qa coverage --json
```
...
By default, Completeness measures how fully NodoAssist exposes the intended
...
tree to nodoassist. Evidence-enriched scorecard outputs belong in short-lived
