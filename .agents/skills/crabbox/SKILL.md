---
name: crabbox
description: Use the Crabbox wrapper for NodoAssist remote validation across Linux, macOS, Windows, and WSL2, including delegated Blacksmith Testbox proof. Report the actual provider and id.
---

# Crabbox

NodoAssist agent sessions use the Crabbox wrapper by default for tests and
...
Blacksmith Testbox through the Crabbox wrapper is the default NodoAssist agent
...
`--provider blacksmith-testbox` for the default NodoAssist agent path.
...
- NodoAssist scripts prefer `../crabbox/bin/crabbox` when present. The user PATH
  shim can be stale.
...
- The brokered AWS image is a Linux developer image in `eu-west-1`; the repo
  ...
  region/AZ placement, and FSR state before blaming NodoAssist.
- For trusted NodoAssist agent tests and computationally intensive work, use the
...
- Treat contributor/fork source as untrusted unless a maintainer explicitly
  ...
  `CRABBOX_ENV_ALLOW=CI` to replace the repo's `NODOASSIST_*`/`NODE_OPTIONS`
...
- Run untrusted source only with the sanitized form below. The explicit
  allowlist prevents locally exported `NODOASSIST_*` credentials from crossing
...
- Do not treat inherited shell env as operator intent. In particular,
  `NODOASSIST_LOCAL_CHECK_MODE=throttled` from the local shell is not permission
  to move broad `pnpm check:changed`, `pnpm test:changed`, full `pnpm test`, or
  lint/typecheck fan-out onto the laptop.
- Only use `NODOASSIST_LOCAL_CHECK_MODE=throttled|full` when the user explicitly
...
Use these only when the task needs an existing non-Linux host. NodoAssist broad
...
Do not silently substitute AWS macOS for normal NodoAssist Linux proof. Report
...
Use this for NodoAssist maintainer broad/heavy `pnpm` gates when the prepared CI
...
node scripts/crabbox-wrapper.mjs run \
  --provider blacksmith-testbox \
  --cache-volume pnpm-store=nodoassist-node24-pnpm-lock:/tmp/nodoassist-pnpm-store \
  --timing-json \
  -- \
  corepack pnpm check:changed
```
...
install NodoAssist as the user would, run the same setup/update/CLI/Gateway/API
...
- Prefer `NODOASSIST_CURRENT_PACKAGE_TGZ` with Docker/package lanes when testing a
...
- Prefer deterministic arrow navigation over search typing for Clack-style
  ...
  sequences.
- Isolate mutable state with `NODOASSIST_STATE_DIR=$(mktemp -d)`. Plugin npm
  installs live under that state dir (`npm/node_modules/...`), not under
  `NODOASSIST_CONFIG_DIR`. Verify downloads by checking the state dir, package
  lock, and installed package metadata.
- To test automatic setup installs against local package artifacts, use
  `NODOASSIST_ALLOW_PLUGIN_INSTALL_OVERRIDES=1` plus
  `NODOASSIST_PLUGIN_INSTALL_OVERRIDES='{"plugin-id":"npm-pack:/tmp/plugin.tgz"}'`.
  Pack with `npm pack`, set an isolated `NODOASSIST_STATE_DIR`, and verify the
...
- For OpenAI/Codex onboarding proof, the useful markers are the UI line
  `Installed Codex plugin`, `npm/node_modules/@nodoassist/codex`, and the
...
- Bad local config: inspect `.crabbox.yaml`, `crabbox config show`, and
  `crabbox whoami`; normal NodoAssist agent proof should use Blacksmith Testbox.
...
```sh
blacksmith auth login --non-interactive --organization nodoassist
```
...
Use Blacksmith Testbox through Crabbox by default for NodoAssist agent tests and
...
```sh
brew install nodoassist/tap/crabbox
...
- If the CLI asks for `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, or AWS
  profile setup during normal NodoAssist validation, assume the agent selected
...
- Ask for AWS keys only for explicit direct-provider/account administration,
  not for normal brokered NodoAssist proof.
...
for NodoAssist lanes. Let that config drive normal validation.
...
Do not add NodoAssist-specific setup to Crabbox itself. Put repo setup in the
