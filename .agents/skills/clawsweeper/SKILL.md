---
name: clawsweeper
description: "Use for all ClawSweeper work: NodoAssist issue/PR sweep reports, commit-review reports, repair jobs, cloud fix PRs, @clawsweeper maintainer mention commands, trusted ClawSweeper-reviewed autofix/automerge, GitHub Actions monitoring, permissions, gates, and manual backfills."
---

# ClawSweeper

ClawSweeper lives at `~/Projects/clawsweeper`. It is the one NodoAssist
...
- Target app permissions: read target scan context; write issues and pull
  ...
  inputs; Actions write on `nodoassist/clawsweeper` for comment-router
...
pnpm commit-reports -- --repo nodoassist/nodoassist --author steipete --since 7d
...
```bash
gh workflow run commit-review.yml --repo nodoassist/clawsweeper \
  -f target_repo=nodoassist/nodoassist \
...
```bash
pnpm run repair:create-job -- \
  --from-report ../clawsweeper/records/nodoassist-nodoassist/items/123.md
```
...
```bash
pnpm run repair:validate-job -- jobs/nodoassist/inbox/clawsweeper-nodoassist-nodoassist-123.md
pnpm run repair:dispatch -- jobs/nodoassist/inbox/clawsweeper-nodoassist-nodoassist-123.md \
  --mode autonomous \
...
```bash
gh variable set CLAWSWEEPER_ALLOW_EXECUTE --repo nodoassist/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_FIX_PR --repo nodoassist/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_MERGE --repo nodoassist/clawsweeper --body 1
gh variable set CLAWSWEEPER_ALLOW_AUTOMERGE --repo nodoassist/clawsweeper --body 1
```
...
@nodoassist-clawsweeper fix ci
@nodoassist-clawsweeper[bot] fix ci
```
...
```bash
pnpm run repair:comment-router -- --repo nodoassist/nodoassist --lookback-minutes 180
pnpm run repair:comment-router -- --repo nodoassist/nodoassist --execute --wait-for-capacity
```
...
leave the local NodoAssist checkout on the PR branch. After the PR is created,
...
escalation, and sensitive data exposure to central NodoAssist security handling.
...
```bash
gh run list --repo nodoassist/clawsweeper --workflow "ClawSweeper Commit Review" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo nodoassist/clawsweeper --workflow "repair cluster worker" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
gh run list --repo nodoassist/clawsweeper --workflow "repair comment router" \
  --limit 12 --json databaseId,displayTitle,event,status,conclusion,createdAt,updatedAt,url
```
...
```bash
gh run list --repo nodoassist/nodoassist --workflow "ClawSweeper Dispatch" \
...
```bash
gh api "repos/nodoassist/nodoassist/commits/<sha>/check-runs?per_page=100" \
