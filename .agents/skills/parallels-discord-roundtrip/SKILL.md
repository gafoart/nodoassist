...
```bash
export NODOASSIST_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.nodoassist/nodoassist.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env NODOASSIST_PARALLELS_DISCORD_TOKEN \
...
- Use the `nodoassist` wrapper for guest `message send/read`; `node nodoassist.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/nodoassist-parallels-smoke.*`
...
- host inbound nonce appears in `nodoassist message read` output
