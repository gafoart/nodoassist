---
name: nodoassist-qa-testing
description: Run, watch, debug, extend, or explain NodoAssist qa-lab and qa-channel scenarios, artifacts, and live lanes.
---

# NodoAssist QA Testing
...
5. If the user wants to watch the live UI, find the current `nodoassist-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm nodoassist qa matrix` defaults to the full `all` profile. Use explicit
...
- That item is the first place to look for:
  ...
  - `NODOASSIST_QA_PROVIDER_MODE`
  - `NODOASSIST_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `NodoAssist`
  - item: `NODOASSIST_QA_CONVEX_SITE_URL`
  - item: `NODOASSIST_QA_CONVEX_SECRET_MAINTAINER`
  - item: `NODOASSIST_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `NODOASSIST QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `NODOASSIST_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `NODOASSIST_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `NodoAssist/NODOASSIST_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
NODOASSIST_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
NODOASSIST_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
NODOASSIST_QA_PROVIDER_MODE="mock-openai" \
NODOASSIST_NPM_TELEGRAM_PACKAGE_SPEC="nodoassist@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `NODOASSIST_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `NODOASSIST_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `NODOASSIST_QA_PROVIDER_MODE`, map it explicitly to `NODOASSIST_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `NODOASSIST_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo nodoassist/nodoassist --ref main \
  -f package_spec=nodoassist@YYYY.M.D-beta.N \
  -f package_label=nodoassist@YYYY.M.D-beta.N \
...
```bash
gh api repos/nodoassist/nodoassist/actions/runs/<run-id>/artifacts
```
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal NodoAssist WhatsApp accounts in the shared pool.
...
- Validate with `pnpm nodoassist qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal NodoAssist identity combines with the character.
...
```bash
pnpm nodoassist qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `NODOASSIST_HOME` sandboxed.
