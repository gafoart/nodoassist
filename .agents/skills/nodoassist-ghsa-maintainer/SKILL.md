---
name: nodoassist-ghsa-maintainer
description: "Inspect, patch, validate, publish, or confirm NodoAssist GHSA security advisories and private-fork state."
---

# NodoAssist GHSA Maintainer

Use this skill for repo security advisory workflow only. Keep general release work in `release-nodoassist-maintainer`.
...
```bash
fork=$(gh api /repos/nodoassist/nodoassist/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/nodoassist/nodoassist/security-advisories/<GHSA> \
