---
name: nodoassist-testing
description: Choose, run, rerun, or debug NodoAssist tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
- Treat contributor and fork patches as untrusted unless a maintainer
  ...
  `NODOASSIST_*` allowlist, `--no-hydrate` must block auth-profile hydration, and
...
NODOASSIST_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
NODOASSIST_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- For local release-candidate proof, pack the plugin and install it with
  `nodoassist plugins install npm-pack:<path.tgz> --force`. This uses the managed
...
- Treat missing runtime imports as package-manifest bugs first. Runtime code
  ...
  `npm install` inside `~/.nodoassist/npm/projects/...`.
...
- `NODOASSIST_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `NODOASSIST_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `NodoAssist Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the public `nodoassist/releases` evidence ledger.
Use the manual `NodoAssist Release Evidence`
(`nodoassist-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation nodoassist/nodoassist <run-id> blocking
package-acceptance nodoassist/nodoassist <run-id> blocking
release-checks nodoassist/nodoassist <run-id> blocking
```
...
When `Full Release Validation` completes and `NODOASSIST_RELEASES_DISPATCH_TOKEN`
is configured in the source repo, it requests the public
`NodoAssist Release Evidence From Full Validation` workflow. That workflow reads
...
`NodoAssist Release Checks` (`nodoassist-release-checks.yml`) is the release child
...
```bash
gh workflow run nodoassist-release-checks.yml \
  --repo nodoassist/nodoassist \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`NodoAssist Release Checks` uses the trusted workflow ref to resolve the selected
...
`ghcr.io/nodoassist/nodoassist-live-media-runner:ubuntu-24.04` container so
...
`pnpm nodoassist qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm nodoassist qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `NODOASSIST_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `NodoAssist Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`NodoAssist Live And E2E Checks (Reusable)`
(`nodoassist-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run nodoassist-live-and-e2e-checks-reusable.yml \
  --repo nodoassist/nodoassist \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `NODOASSIST_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`NODOASSIST_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
NODOASSIST_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
NODOASSIST_DOCKER_ALL_DRY_RUN=1 NODOASSIST_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
NODOASSIST_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
NODOASSIST_DOCKER_ALL_LANES=<lane> \
NODOASSIST_DOCKER_ALL_BUILD=0 \
NODOASSIST_DOCKER_ALL_PREFLIGHT=0 \
NODOASSIST_SKIP_DOCKER_BUILD=1 \
NODOASSIST_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
NODOASSIST_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view nodoassist@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/nodoassist-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=nodoassist@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=nodoassist@YYYY.M.D-beta.N` or
  `package_spec=nodoassist@YYYY.M.D`.
- For stable package proof, use `package_spec=nodoassist@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `nodoassist@beta`,
  `nodoassist@latest`, or exact NodoAssist release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `nodoassist update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `nodoassist@beta`, `nodoassist@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an NodoAssist branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-nodoassist-for-docker.mjs` is the
...
from `nodoassist skills search`, installs it, and verifies `.clawhub` origin/lock
