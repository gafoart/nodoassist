---
name: release-nodoassist-maintainer
description: Prepare or verify NodoAssist stable/beta releases, changelogs, release notes, publish commands, and artifacts.
---

# NodoAssist Release Maintainer
...
- Core `nodoassist` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- For a beta release train, keep Full Release Validation as a pre-publish gate
  ...
  stop and report.
- As soon as the release candidate SHA exists, dispatch `NodoAssist Performance`
...
- In maintainer Testbox mode, use `NODOASSIST_TESTBOX=1 scripts/pr prepare-run
<PR>` only after the exact PR head has passed `CI` and every scheduled
  ...
  `NODOASSIST_TESTBOX=1 scripts/pr prepare-sync-head <PR>`, wait for those hosted
...
- If an exact PR-head CI run has no active jobs because Blacksmith capacity is
  ...
  the PR head branch:
  `gh workflow run ci.yml --repo nodoassist/nodoassist --ref <pr-head-branch> -f
target_ref=<full-pr-sha> -f include_android=true -f release_gate=true`.
  ...
  cancelled, or been skipped. Then rerun `NODOASSIST_TESTBOX=1 scripts/pr
prepare-run <PR>`.
- Generate the changelog before every beta, beta rerun, stable release, or
  stable rerun, before version/tag preparation. Use
  `$nodoassist-changelog-update` for the rewrite. Do not continue release prep if
...
5. Run `pnpm release:generated:check`, `pnpm deps:shrinkwrap:check`, and
   `NODOASSIST_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `nodoassist-stable-main-closeout.yml` starts from the `main` push carrying the
...
- Every stable NodoAssist release ships the npm package, macOS app, and signed
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/openclaw/openclaw/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `nodoassist` repo.
...
- Stable Windows Hub release closeout requires the signed
  `NodoAssistCompanion-Setup-x64.exe`, `NodoAssistCompanion-Setup-arm64.exe`, and
  `NodoAssistCompanion-SHA256SUMS.txt` assets on the canonical
  `nodoassist/nodoassist` GitHub Release. Pass the exact signed
  `nodoassist/nodoassist-windows-node` release tag as `windows_node_tag` to
  `NodoAssist Release Publish`, together with the candidate-approved
  ...
  dispatches the public `Windows Node Release` workflow while the NodoAssist
  release is still a draft, carries those pinned source asset digests
  unchanged, verifies the expected NodoAssist Foundation Authenticode signer on
  ...
  `NodoAssistCompanion-*` target asset names, then replaces the expected contract
  assets with the pinned source bytes.
- Website Windows Hub download links should target exact canonical
  `nodoassist/nodoassist/releases/download/vYYYY.M.PATCH/...` assets for the current
...
- Before release branching or tagging, rewrite the target `CHANGELOG.md`
  ...
  release SHA.
- Generate `$nodoassist-changelog-update`'s full contribution manifest before
...
- Before publishing or closing a release, run
  `$nodoassist-changelog-update`'s `verify-release-notes.mjs` with every stable
...
- To update an existing GitHub Release body, resolve the numeric release id and
  ...
  `gh api repos/nodoassist/nodoassist/releases/tags/vYYYY.M.PATCH --jq .id`, then
  `gh api -X PATCH repos/nodoassist/nodoassist/releases/<id> -F body=@/tmp/notes.md`.
  Do not trust `gh release edit --notes-file` or `--input` JSON if verification
  disagrees; verify with `gh api repos/nodoassist/nodoassist/releases/<id>` because
...
- When cutting a mac release with a beta GitHub prerelease:
  ...
  - create a prerelease titled `nodoassist YYYY.M.PATCH-beta.N`
...
- Before tagging, diff publishable plugin package manifests against the last
  ...
  (`nodoassist.release.publishToNpm: true` or `publishToClawHub: true`) whose
...
```bash
  NODOASSIST_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke
```

After npm publish, run:

```bash
node --import tsx scripts/nodoassist-npm-postpublish-verify.ts <published-version>
```
...
- Set `NODOASSIST_LIVE_VIDEO_GENERATION_FULL_MODES=1` only when intentionally
  validating the slower image-to-video and video-to-video transform lanes.

## Check all relevant release builds

- Always validate the NodoAssist npm release path before creating the tag.
...
- Full pre-npm beta test roster:
  ...
  - all Parallels install/update tests:
    ...
    rerun lanes from `nodoassist-parallels-smoke`
...
- Post-published beta verification roster:
  - `node --import tsx scripts/nodoassist-npm-postpublish-verify.ts <beta-version>`
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    from `main` with `package_spec=nodoassist@<beta-version>` and
    ...
    `NODOASSIST_NPM_TELEGRAM_PACKAGE_SPEC` and Convex CI env only as a fallback
    or debugging path.
...
- For beta-style full e2e batteries, hard-cap top-level long lanes instead of letting them run indefinitely. Use host `timeout --foreground`/`gtimeout --foreground` caps such as:
  - `45m` for `NODOASSIST_INSTALL_SMOKE_SKIP_NONROOT=1 pnpm test:install:smoke`
  ...
  - `180m` for local full QA live OpenAI + Anthropic rosters when explicitly
    ...
    `QA-Lab - All Lanes`
  - Parallels caps from the `nodoassist-parallels-smoke` skill
...
- Actual npm install/update phases are capped at 5 minutes. If `npm install -g`, installer package install, or `nodoassist update` takes longer than 300s in release e2e, stop treating the run as healthy progress and debug the installer/updater or harness.
...
- Include mac release readiness in preflight by running the public validation
  workflow in `nodoassist/nodoassist` and the release-ops mac preflight in
  `nodoassist/releases` for every release.
...
- NodoAssist publish uses GitHub trusted publishing.
- Stable npm promotion from `beta` to `latest` uses the restricted release-ops
  `nodoassist/releases/.github/workflows/nodoassist-npm-dist-tags.yml` workflow
...
- Prefer fixing the release-ops workflow token path over any local 1Password
  ...
  repo's `NPM_TOKEN` secret, scoped to the `nodoassist` package with read/write
...
- If the release-ops dist-tag workflow cannot promote because `NPM_TOKEN` is
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add nodoassist@YYYY.M.PATCH latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    `npm view nodoassist dist-tags --json --prefer-online --cache /tmp/nodoassist-npm-cache-verify-$$`
    and `npm view nodoassist@latest version dist.tarball --json --prefer-online`.
...
- npm registry metadata is eventually consistent immediately after trusted
  ...
  steps instead of reading the registry again. If the NodoAssist npm child
...
- `.github/workflows/macos-release.yml` in `nodoassist/nodoassist` is now a
  ...
  artifacts.
- `nodoassist/releases/.github/workflows/nodoassist-macos-validate.yml` is the
...
- Real mac preflight and real mac publish both use
  `nodoassist/releases/.github/workflows/nodoassist-macos-publish.yml`.
...
- The `npm-release` environment must be approved by `@nodoassist/nodoassist-release-managers` before publish continues.
- Mac publish uses
  `nodoassist/releases/.github/workflows/nodoassist-macos-publish.yml` for
...
- Real release-ops mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `nodoassist/nodoassist`
  automatically when `NODOASSIST_PUBLIC_REPO_RELEASE_TOKEN` is present in the
...
- Do not use `NPM_TOKEN` or the plugin OTP flow for the NodoAssist package
...
- `@nodoassist/*` plugin publishes use a separate maintainer-only flow.
...
9. Immediately dispatch Actions > `NodoAssist Performance` from `main` with
...
15. Dispatch Actions > `QA-Lab - All Lanes` against the release tag and wait
    ...
    lanes to pass.
16. Start `.github/workflows/nodoassist-npm-release.yml` from the release branch
...
18. For stable releases, start `.github/workflows/macos-release.yml` in
    `nodoassist/nodoassist` and wait for the public validation-only run to pass.
19. For stable releases, start
    `nodoassist/releases/.github/workflows/nodoassist-macos-validate.yml` with the
    same tag and wait for the release-ops mac validation lane to pass.
20. For stable releases, start
    `nodoassist/releases/.github/workflows/nodoassist-macos-publish.yml` with
...
22. Start `.github/workflows/nodoassist-release-publish.yml` from the same branch with
    ...
    `nodoassist/nodoassist-windows-node` tag as `windows_node_tag` and its
    candidate-approved installer digest map as `windows_node_installer_digests`.
23. Wait for `npm-release` approval from `@nodoassist/nodoassist-release-managers`.
24. Wait for the real publish workflow to run postpublish verification,
    ...
    waited plugin publish or Windows Hub promotion fails after NodoAssist npm
    succeeds, the workflow keeps the release draft with NodoAssist npm evidence
...
30. For stable releases, start
    `nodoassist/releases/.github/workflows/nodoassist-macos-publish.yml` for the
...
- Use `nodoassist-ghsa-maintainer` for GHSA advisory inspection, patch/publish flow, private-fork validation, and GHSA API-specific publish checks.
