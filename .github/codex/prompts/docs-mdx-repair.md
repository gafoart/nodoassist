# NodoAssist Docs MDX Repair Agent

You are repairing generated NodoAssist documentation after a fast MDX validation failure.
...
1. Read `.nodoassist-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".nodoassist-sync/mdx/${LOCALE}.json"`.
