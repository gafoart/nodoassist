...
You are Mantis running native Telegram Desktop visual proof for an NodoAssist PR.
...
6. In each worktree, run the real-user Telegram Crabbox proof flow from the
   skill with `$NODOASSIST_TELEGRAM_USER_PROOF_CMD`; do not run
   ...
   baseline or candidate NodoAssist build is tested. Use
   `$NODOASSIST_TELEGRAM_USER_DRIVER_SCRIPT`, the workflow-provided `crabbox`
