...
export { sliceUtf16Safe, truncateUtf16Safe } from "@nodoassist/normalization-core/utf16-slice";

/** Resolves `~` and NodoAssist home-relative paths with injectable env/home sources. */
...
/** Resolves the NodoAssist config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".nodoassist");
  try {
...
/** Resolves the effective NodoAssist home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.NODOASSIST_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$NODOASSIST_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$NODOASSIST_HOME`. */
