...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.nodoassist.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.nodoassist.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        NODOASSIST_STATE_DIR: "/tmp/nodoassist-service",
        NODOASSIST_CONFIG_PATH: "/tmp/nodoassist-service/nodoassist.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { NODOASSIST_STATE_DIR?: string; NODOASSIST_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.NODOASSIST_STATE_DIR).toBe("/tmp/nodoassist-service");
    expect(handoffEnv?.NODOASSIST_CONFIG_PATH).toBe("/tmp/nodoassist-service/nodoassist.json");
    expect(note).toHaveBeenCalledWith(
      ...
      connections: [
        {
          pid: 4242,
          command: "node",
          commandLine: "/tmp/newer-nodoassist/bin/nodoassist logs --follow",
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "nodoassist-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "nodoassist-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "nodoassist-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("nodoassist gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system NodoAssist gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /etc/systemd/system/nodoassist-gateway.service",
        scope: "system",
        marker: "nodoassist",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        "System-level NodoAssist gateway service detected while the user gateway service is not installed.",
        "- nodoassist-gateway.service (unit: /etc/systemd/system/nodoassist-gateway.service)",
        "NodoAssist will not install a second user-level gateway service automatically.",
        "Run `nodoassist gateway status --deep` or `nodoassist doctor --deep` to inspect duplicate services.",
