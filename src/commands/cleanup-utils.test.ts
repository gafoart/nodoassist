...
describe("buildCleanupPlan", () => {
  test("resolves inside-state flags and workspace dirs", () => {
    ...
    const defaultWorkspace = path.join(tmpRoot, "nodoassist-workspace-default");
    const opsWorkspace = path.join(tmpRoot, "nodoassist-workspace-ops");
    const cfg = {
      agents: {
    ...
    const plan = buildCleanupPlan({
      cfg: cfg as unknown as NodoAssistConfig,
      stateDir: path.join(tmpRoot, "nodoassist-state"),
      configPath: path.join(tmpRoot, "nodoassist-state", "nodoassist.json"),
      oauthDir: path.join(tmpRoot, "nodoassist-oauth"),
    });
  ...
  test("includes implicit per-agent workspaces under the state dir", () => {
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "nodoassist-cleanup-plan");
...
describe("cleanup path removals", () => {
  ...
  it("removes state and only linked paths outside state", async () => {
    ...
    const tmpRoot = path.join(path.parse(process.cwd()).root, "tmp", "nodoassist-cleanup");
    await removeStateAndLinkedPaths(
      {
        stateDir: path.join(tmpRoot, "state"),
        configPath: path.join(tmpRoot, "state", "nodoassist.json"),
    ...
    expect(runtime.log.mock.calls.map(([line]) => line.replaceAll("\\", "/"))).toEqual([
      "[dry-run] remove /tmp/nodoassist-cleanup/state",
      "[dry-run] remove /tmp/nodoassist-cleanup/oauth",
    ]);
  });

  it("preserves nested workspace paths during state-only removal", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cleanup-"));
    const stateDir = path.join(tmpRoot, ".nodoassist");
  ...
  it("removes every workspace directory", async () => {
    const runtime = createRuntimeMock();
    const workspaces = ["/tmp/nodoassist-workspace-1", "/tmp/nodoassist-workspace-2"];

    await removeWorkspaceDirs(workspaces, runtime, { dryRun: true });

    const logs = runtime.log.mock.calls.map(([line]) => line);
    expect(logs).toEqual([
      "[dry-run] remove /tmp/nodoassist-workspace-1",
      "[dry-run] remove /tmp/nodoassist-workspace-2",
    ]);
  });

  it("removes owned legacy workspace attestations", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cleanup-attest-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `nodoassist-workspace-attestation:v1\n${new Date().toISOString()}\n`,
      );
  ...
  it("refuses to remove a directory containing the current working directory", async () => {
    const runtime = createRuntimeMock();
    const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cleanup-cwd-"));
