// Builds overview table rows for `nodoassist status` and `nodoassist status --all`.
...
/** Builds the default `nodoassist status` overview rows from scan, health, memory, and session inputs. */
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRestartValue
        ? [{ Item: "Update restart", Value: params.updateRestartValue }]
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("nodoassist security audit --deep")}` },
    ],
