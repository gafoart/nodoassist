...
describe("describeHeartbeatSessionTargetIssues", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): NodoAssistConfig {
  ...
  function writeStore(cfg: NodoAssistConfig, entries: Record<string, unknown>) {
