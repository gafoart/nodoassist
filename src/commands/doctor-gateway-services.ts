...
import { replaceConfigFile, type NodoAssistConfig } from "../config/config.js";
...
import { NODOASSIST_WRAPPER_ENV_KEY } from "../daemon/program-args.js";
...
function buildGatewayServiceRepairEnv(
  ...
  const wrapperPath = command?.environment?.[NODOASSIST_WRAPPER_ENV_KEY]?.trim();
  if (!wrapperPath || Object.hasOwn(process.env, NODOASSIST_WRAPPER_ENV_KEY)) {
    return process.env;
  }
  return {
    ...process.env,
    [NODOASSIST_WRAPPER_ENV_KEY]: wrapperPath,
  };
}
...
function resolveGatewayServiceWrapperPath(
  ...
  return normalizeOptionalString(command?.environment?.[NODOASSIST_WRAPPER_ENV_KEY]) ?? null;
}
...
function resolveSystemdUnitNameFromServicePath(sourcePath: string | undefined): string {
  ...
  return base.endsWith(".service") ? base : "nodoassist-gateway.service";
}
...
export function extraGatewayServiceToHealthFinding(service: ExtraGatewayService): HealthFinding {
  return {
    ...
    fixHint:
      service.legacy === true
        ? "Run nodoassist doctor --fix to remove legacy gateway services."
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (serviceWrapperPath) {
    note(`Gateway service invokes ${NODOASSIST_WRAPPER_ENV_KEY}: ${serviceWrapperPath}`, "Gateway");
  }
  ...
  const sourceCheckoutWarning = serviceLayout?.entrypointSourceCheckout
    ? [
        ...
        "Run `nodoassist doctor --fix` from the intended package install, or reinstall the gateway service with `nodoassist gateway install --force`.",
      ].join("\n")
    : null;
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service NODOASSIST_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
  ...
  if (serviceRewriteBlocked) {
    note(
      "Gateway service is running; leaving supervisor metadata unchanged. Stop the service first or use `nodoassist gateway install --force` when you want to replace the active launcher.",
      "Gateway service config",
    );
    return cfg;
  }
  ...
  if (
    ...
    note(
      "Update-mode doctor detected gateway service drift but left the live systemd unit unchanged. Review the service file and run `nodoassist gateway install --force` when you want NodoAssist to replace operator-owned systemd directives.",
      "Gateway service config",
    );
    return cfg;
  }
  ...
  if (!repair) {
    if (!emittedSourceCheckoutWarning) {
      note(
        "Run `nodoassist gateway install --force` when you want to replace the gateway service definition.",
        "Gateway service config",
      );
    }
    return cfg;
  ...
  const installedWindowsTaskName = command.environment?.NODOASSIST_WINDOWS_TASK_NAME?.trim();
  const serviceRepairEnv =
    updateRepairWillRewriteWindowsTask && installedWindowsTaskName
      ? {
          ...serviceInstallEnv,
          NODOASSIST_WINDOWS_TASK_NAME: installedWindowsTaskName,
        }
      : serviceInstallEnv;
  ...
  if (
    ...
    const nextCfg: NodoAssistConfig = {
      ...cfg,
      gateway: {
        ...cfg.gateway,
        auth: {
  ...
  try {
    ...
    if (
      ...
      if (installedWindowsTaskName) {
        ...
        restartEnv.NODOASSIST_WINDOWS_TASK_NAME = installedWindowsTaskName;
      }
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    ...
    if (shouldRemove) {
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing NodoAssist gateway next.");
      }
    }
  }
