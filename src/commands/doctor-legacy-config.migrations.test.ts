...
vi.mock("../plugins/setup-registry.js", () => ({
  ...
  runPluginSetupConfigMigrations: ({ config }: { config: NodoAssistConfig }) => ({
    config,
    changes: [],
  }),
}));
...
vi.mock("./doctor/shared/channel-legacy-config-migrate.js", () => ({
  applyChannelDoctorCompatibilityMigrations: (cfg: NodoAssistConfig) => ({
    next: cfg,
    changes: [],
  }),
}));
...
vi.mock("../secrets/target-registry.js", () => {
  ...
  return {
    discoverConfigSecretTargets: (cfg: NodoAssistConfig) => {
      const targets: Array<{
        entry: typeof entry;
        path: string;
        pathSegments: string[];
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  beforeAll(() => {
    previousOauthDir = process.env.NODOASSIST_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-oauth-"));
    process.env.NODOASSIST_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterAll(() => {
    if (previousOauthDir === undefined) {
      delete process.env.NODOASSIST_OAUTH_DIR;
    } else {
      process.env.NODOASSIST_OAUTH_DIR = previousOauthDir;
    }
    fs.rmSync(tempOauthDir, { recursive: true, force: true });
  });
        ...
        list: [
          {
            ...
            models: {
              "anthropic/claude-opus-4-7": { agentRuntime: { id: "nodoassist" } },
            },
          },
        ],
      },
  ...
  it("does not report talk provider normalization for realtime voice aliases", () => {
    ...
    const res = normalizeCompatibilityConfigValues(input as NodoAssistConfig);
