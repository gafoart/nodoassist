...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "nodoassist-tui") {
    return true;
  }
  return executable === "nodoassist" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local NodoAssist TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: NodoAssistConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("nodoassist doctor --fix")}.`,
      );
    }
  }
