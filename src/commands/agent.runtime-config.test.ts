...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: NodoAssistConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => NodoAssistConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: NodoAssistConfig, sourceConfig: NodoAssistConfig) => void>(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "nodoassist-agent-" });
}
...
function mockConfig(home: string, storePath: string): NodoAssistConfig {
  const cfg = {
    agents: {
      defaults: {
