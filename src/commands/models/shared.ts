...
export async function loadValidConfigOrThrow(): Promise<NodoAssistConfig> {
...
export async function updateConfig(
  mutator: (cfg: NodoAssistConfig, context: UpdateConfigContext) => NodoAssistConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: NodoAssistConfig }): {
  provider: string;
  model: string;
} {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("nodoassist agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
