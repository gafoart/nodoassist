...
const mocks = vi.hoisted(() => ({
  ...
  resolveModelsTargetAgent: vi.fn((_cfg: NodoAssistConfig, rawAgentId?: string) => {
    const agentId = rawAgentId ?? "main";
    return { agentDir: `/tmp/nodoassist/agents/${agentId}`, agentId };
  }),
}));

vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: NodoAssistConfig, agentId: string) => `/tmp/nodoassist/agents/${agentId}`,
...
vi.mock("../../agents/auth-profiles.js", () => ({
  ...
  resolveAuthStatePathForDisplay: (agentDir: string) => `${agentDir}/nodoassist-agent.sqlite`,
}));
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as NodoAssistConfig);
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "coder",
        authStatePath: "/tmp/nodoassist/agents/coder/nodoassist-agent.sqlite",
        profiles: [
          {
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "main",
        authStatePath: "/tmp/nodoassist/agents/main/nodoassist-agent.sqlite",
        profiles: [
          {
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state store: /tmp/nodoassist/agents/main/nodoassist-agent.sqlite",
      "Profiles: (none)",
    ]);
  });
