...
vi.mock("../../plugins/provider-auth-choice-helpers.js", async (importOriginal) => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: NodoAssistConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: NodoAssistConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: NodoAssistConfig;
  let lastUpdatedConfig: NodoAssistConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/nodoassist/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/nodoassist/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/nodoassist/workspace");
    ...
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: NodoAssistConfig) => NodoAssistConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/nodoassist/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: NodoAssistConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/nodoassist/agents/coder" : "/tmp/nodoassist/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: NodoAssistConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/nodoassist/workspaces/coder" : "/tmp/nodoassist/workspace",
    );
  ...
  it("runs plugin-owned openai login", async () => {
    ...
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai:user@example.com",
      agentDir: "/tmp/nodoassist/agents/main",
    });
    ...
    expect(upsertCall.agentDir).toBe("/tmp/nodoassist/agents/main");
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(authRunCall.agentDir).toBe("/tmp/nodoassist/agents/coder");
    expect(authRunCall.workspaceDir).toBe("/tmp/nodoassist/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/nodoassist/agents/coder");
  });
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(providerResolutionCall.workspaceDir).toBe("/tmp/nodoassist/workspace");
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/nodoassist/agents/main");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai. Verify plugins via `nodoassist plugins list --json`.',
    );
  });
  ...
  it("writes pasted tokens to the requested agent store", async () => {
    ...
    expect(mocks.upsertAuthProfileWithLock).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/nodoassist/agents/coder",
    });
  });
  ...
  it("rejects OpenAI API keys pasted as OpenAI Codex token material", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like an OpenAI API key. Use nodoassist models auth paste-api-key --provider openai for API-key auth.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects token material pasted into the OpenAI Codex API-key command", async () => {
    ...
    expect(validateMessages).toEqual([
      "That looks like token or OAuth material, not an OpenAI API key. Use nodoassist models auth paste-token --provider openai for token auth material.",
    ]);
    expect(mocks.upsertAuthProfileWithLock).not.toHaveBeenCalled();
    expect(mocks.updateConfig).not.toHaveBeenCalled();
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "nodoassist agents list" to see configured agents.',
    );
  ...
  it("uses the requested agent store for setup-token provider auth", async () => {
    ...
    expect(tokenAuthCall.agentDir).toBe("/tmp/nodoassist/agents/coder");
    expect(tokenAuthCall.workspaceDir).toBe("/tmp/nodoassist/workspaces/coder");
    expect(
      (readMockCallArg(mocks.upsertAuthProfileWithLock) as UpsertAuthProfileCall).agentDir,
    ).toBe("/tmp/nodoassist/agents/coder");
  });
