...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/nodoassist-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/nodoassist-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/nodoassist-agent"}/auth-profiles.json`,
    ),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/nodoassist-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/nodoassist-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/nodoassist-dev/nodoassist.json");
    expect(payload.auth.storePath).toBe("/tmp/nodoassist-agent/auth-profiles.json");
  ...
  it("honors NODOASSIST_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ NODOASSIST_AGENT_DIR: "/tmp/nodoassist-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/nodoassist-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/nodoassist-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/nodoassist-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when NODOASSIST_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
        NODOASSIST_AGENT_DIR: undefined,
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/nodoassist-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/nodoassist-legacy-agent");
  });
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    await withAgentScopeOverrides(
      ...
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/nodoassist-agent-custom");
  ...
  it("reports Gemini CLI OAuth for canonical Google text routed through the CLI runtime", async () => {
    ...
    try {
      ...
      expect(
        ...
        kind: "profiles",
        detail: "/tmp/nodoassist-agent/auth-profiles.json",
      });
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          provider: "google",
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        ...
        options?.workspaceDir === "/tmp/nodoassist-agent/workspace"
          ? {
