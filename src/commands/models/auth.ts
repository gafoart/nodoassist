...
function validateOpenAICodexApiKeyInput(value: string): string | undefined {
  ...
  if (looksLikeJwtToken(trimmed) || looksLikeStructuredCredential(trimmed)) {
    ...
    return `That looks like token or OAuth material, not an OpenAI API key. Use ${formatCliCommand("nodoassist models auth paste-token --provider openai")} for token auth material.`;
  }
...
function resolveRequestedProviderOrThrow(
  ...
  throw new Error(
    `Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("nodoassist plugins list --json")}\`.`,
  );
}
...
async function persistProviderAuthResult(params: {
  ...
  // Auth login owns the credential store. Keep nodoassist.json untouched unless
...
export async function modelsAuthSetupTokenCommand(
  ...
  if (!process.stdin.isTTY) {
    throw new Error(
      `setup-token requires an interactive TTY. In automation, use ${formatCliCommand("nodoassist models auth paste-token --provider <provider>")} instead.`,
    );
  }
  ...
  if (tokenProviders.length === 0) {
    throw new Error(
      `No provider token-auth plugins found. Install one via \`${formatCliCommand("nodoassist plugins install")}\`.`,
    );
  }
  ...
  if (!provider) {
    throw new Error(
      `No token-capable provider is available. Run ${formatCliCommand("nodoassist plugins list")} to verify provider plugins are installed.`,
    );
  }
...
export async function modelsAuthPasteTokenCommand(
  ...
  if (!rawProvider) {
    throw new Error(
      `Missing --provider. Run ${formatCliCommand("nodoassist models status")} or ${formatCliCommand("nodoassist plugins list")} to choose a provider.`,
    );
  }
  ...
  const validateTokenInput = (value: string | undefined): string | undefined => {
    ...
    if (isOpenAIProvider(provider) && looksLikeOpenAIApiKey(trimmed)) {
      return `That looks like an OpenAI API key. Use ${formatCliCommand("nodoassist models auth paste-api-key --provider openai")} for API-key auth.`;
    }
    return undefined;
  };
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in NodoAssist.");
    runtime.log("NodoAssist prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this NodoAssist path is allowed again.");
  }
}
...
export async function modelsAuthAddCommand(opts: { agent?: string }, runtime: RuntimeEnv) {
  ...
  if (providerPlugin) {
    ...
    if (methodId !== "paste") {
      ...
      if (!method) {
        throw new Error(
          `Unknown token auth method "${methodId}". Run ${formatCliCommand("nodoassist models auth login --provider " + providerPlugin.id)} to choose interactively.`,
        );
      }
      await runProviderAuthMethod({
        config,
...
function maybeLogOpenAICodexNativeSearchTip(runtime: RuntimeEnv, providerId: string) {
  ...
  runtime.log(
    "Tip: Codex-capable models can use native Codex web search. Enable it with nodoassist configure --section web (recommended mode: cached). Docs: https://docs.openclaw.ai/tools/web",
  );
}
...
export async function runModelsAuthLoginFlow(
  ...
  if (authProviders.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("nodoassist plugins install")}\`.`,
    );
  }
  ...
  if (!selectedProvider) {
    throw new Error(
      `Unknown provider. Run ${formatCliCommand("nodoassist models status")} or ${formatCliCommand("nodoassist plugins list")} to see available provider plugins.`,
    );
  }
  ...
  if (!chosenMethod) {
    throw new Error(
      `Unknown auth method. Run ${formatCliCommand("nodoassist models auth login --provider " + selectedProvider.id)} without --method to choose interactively.`,
    );
  }
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  if (!process.stdin.isTTY) {
    throw new Error(
      `models auth login requires an interactive TTY. In automation, use ${formatCliCommand("nodoassist models auth paste-token --provider <provider>")} when token auth is available.`,
    );
  }
