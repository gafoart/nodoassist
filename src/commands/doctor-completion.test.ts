...
describe("shell completion health mapping", () => {
  it("checks an explicit shell instead of the detected environment shell", async () => {
    const homeDir = tempDirs.make("nodoassist-completion-home-");
    const stateDir = tempDirs.make("nodoassist-completion-state-");
    setTestEnvValue("HOME", homeDir);
    setTestEnvValue("NODOASSIST_STATE_DIR", stateDir);
    setTestEnvValue("SHELL", "/bin/zsh");

    const current = await checkShellCompletionStatus("nodoassist", { shell: "fish" });

    expect(current.shell).toBe("fish");
    expect(current.cachePath).toBe(path.join(stateDir, "completions", "nodoassist.fish"));
...
async function setupDoctorCompletionTest(usesSlowPattern: boolean) {
  const homeDir = tempDirs.make("nodoassist-doctor-home-");
  const stateDir = tempDirs.make("nodoassist-doctor-state-");
  setTestEnvValue("HOME", homeDir);
  setTestEnvValue("NODOASSIST_STATE_DIR", stateDir);
  ...
  if (usesSlowPattern) {
    await fs.writeFile(
      profilePath,
      '# test bashrc\n[ -f "/tmp/nonexistent" ] && source <(nodoassist completion bash)\n',
      "utf-8",
    );
    const cacheDir = path.join(stateDir, "completions");
    await fs.mkdir(cacheDir, { recursive: true });
    await fs.writeFile(path.join(cacheDir, "nodoassist.bash"), "# completion cache\n", "utf-8");
  }
  return profilePath;
}
...
describe("doctorShellCompletion", () => {
  ...
  it.each([
    ...
    async ({ generationMode, expectedSkipValue }) => {
      ...
      await expect(
        ensureCompletionCacheExists("nodoassist", {
          shell: "powershell",
          generationMode,
        }),
      ).resolves.toBe(true);
