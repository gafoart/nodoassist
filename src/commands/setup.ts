...
async function writeDefaultConfigFile(config: NodoAssistConfig): Promise<void> {
...
async function readConfigFileRaw(configPath: string): Promise<{
  exists: boolean;
  parsed: NodoAssistConfig;
}> {
  try {
    const raw = await fs.readFile(configPath, "utf-8");
    const parsed = safeParseWithSchema(JsonRecordSchema, JSON5.parse(raw));
    return { exists: true, parsed: (parsed ?? {}) as NodoAssistConfig };
  } catch {
...
export async function setupCommand(
  ...
  runtime.log(`Next guided path: ${formatCliCommand("nodoassist onboard")}.`);
  runtime.log(
    `Next targeted changes: ${formatCliCommand("nodoassist configure")} for models, channels, Gateway, plugins, skills, and health checks.`,
  );
  runtime.log(`Add a chat channel later: ${formatCliCommand("nodoassist channels add")}.`);
}
