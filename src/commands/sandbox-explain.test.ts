...
describe("sandbox explain command", () => {
  it("prints JSON shape + fix-it keys", { timeout: SANDBOX_EXPLAIN_TEST_TIMEOUT_MS }, async () => {
    mockCfg = {
      ...
      session: { store: "/tmp/nodoassist-test-sessions-{agentId}.json" },
    };
  ...
  it("reports the effective rw workspace and Docker mount without changing workspaceRoot", async () => {
    mockCfg = {
      agents: {
        ...
        list: [{ id: "builder", workspace: "/tmp/nodoassist-agent-workspace" }],
      },
      session: { store: "/tmp/nodoassist-test-sessions-{agentId}.json" },
    };
    ...
    const parsed = JSON.parse(logs.join(""));
    const agentWorkspace = path.resolve("/tmp/nodoassist-agent-workspace");
    expect(parsed.sandbox.workspaceRoot).toBe("/tmp/nodoassist-sandboxes");
  ...
  it("uses the canonical derived workspace for non-default agents", async () => {
    mockCfg = {
      agents: {
        defaults: {
          workspace: "/tmp/nodoassist-agent-workspaces",
          sandbox: {
            mode: "all",
            scope: "agent",
            workspaceAccess: "rw",
    ...
    expect(parsed.sandbox.effectiveHostWorkspaceRoot).toBe(
      path.resolve("/tmp/nodoassist-agent-workspaces/builder"),
    );
    expect(parsed.sandbox.workspaceMounts[0]).toMatchObject({
      hostRoot: path.resolve("/tmp/nodoassist-agent-workspaces/builder"),
      source: "workspace",
      writable: true,
    });
  });
  ...
  it("reports the generated sandbox workspace for non-rw sessions", async () => {
    ...
    expect(parsed.sandbox.effectiveHostWorkspaceRoot).toMatch(
      /^\/tmp\/nodoassist-sandboxes\/agent-builder-/,
    );
  ...
  it("reports the agent workspace for direct sessions", async () => {
    ...
    const parsed = JSON.parse(logs.join(""));
    expect(parsed.sandbox.effectiveHostWorkspaceRoot).toBe(
      path.resolve("/tmp/nodoassist-agent-workspace"),
    );
    expect(parsed.sandbox.runtimeWorkdir).toBe(path.resolve("/tmp/nodoassist-agent-workspace"));
    expect(parsed.sandbox.workspaceSource).toBe("direct");
    expect(parsed.sandbox.workspaceMounts).toEqual([]);
  });

  it("uses persisted spawned-session workspace and cwd overrides", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sandbox-explain-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        [sessionKey]: {
          ...
          spawnedWorkspaceDir: "/tmp/nodoassist-child-workspace",
          spawnedCwd: "/tmp/nodoassist-child-workspace/task",
        },
      }),
    );
    mockCfg = {
    ...
    try {
      ...
      const parsed = JSON.parse(logs.join(""));
      expect(parsed.sandbox.effectiveHostWorkspaceRoot).toBe(
        path.resolve("/tmp/nodoassist-child-workspace"),
      );
      expect(parsed.sandbox.runtimeWorkdir).toBe("/tmp/nodoassist-child-workspace/task");
      expect(parsed.sandbox.workspaceSource).toBe("direct");
    } finally {
  ...
  it("mounts a persisted spawned workspace for sandboxed sessions", async () => {
    ...
    try {
      ...
      expect(parsed.sandbox.runtimeWorkdir).toBe("/workspace");
      expect(parsed.sandbox.workspaceMounts[0]).toMatchObject({
        hostRoot: path.resolve("/tmp/nodoassist-child-workspace"),
        containerRoot: "/workspace",
        writable: true,
      });
    } finally {
  ...
  it("reports a global main session as direct in non-main mode", async () => {
    mockCfg = {
      agents: {
        ...
        list: [{ id: "main", workspace: "/tmp/nodoassist-main-workspace" }],
      },
      session: {
        scope: "global",
        store: "/tmp/nodoassist-test-sessions-{agentId}.json",
      },
    };
    ...
    expect(parsed.sandbox.effectiveHostWorkspaceRoot).toBe(
      path.resolve("/tmp/nodoassist-main-workspace"),
    );
    expect(parsed.sandbox.workspaceSource).toBe("direct");
    expect(parsed.sandbox.workspaceMounts).toEqual([]);
  });
  ...
  it("uses the configured default agent for global sessions", async () => {
        ...
        list: [
          {
            id: "ops",
            default: true,
            workspace: "/tmp/nodoassist-ops-workspace",
          },
        ],
      },
    ...
    expect(parsed.sandbox.sessionIsSandboxed).toBe(false);
    expect(parsed.sandbox.effectiveHostWorkspaceRoot).toBe(
      path.resolve("/tmp/nodoassist-ops-workspace"),
    );
  });
