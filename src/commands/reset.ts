...
/** CLI options accepted by `nodoassist reset`. */
...
async function stopGatewayIfRunning(runtime: RuntimeEnv) {
  if (isNixMode) {
    // Nix mode owns service lifecycle outside NodoAssist-managed launchd/systemd
...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("nodoassist onboard --install-daemon")}`);
    return;
  }
