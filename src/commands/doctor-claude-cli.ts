...
function usesClaudeCliModelSelection(cfg: NodoAssistConfig): boolean {
...
function resolveClaudeCliCommand(cfg: NodoAssistConfig): string {
...
function formatWorkspaceHealthLine(
  ...
  if (health === "missing") {
    return `- ${label}: ${display} (missing; NodoAssist will create it on first run).`;
  }
...
function resolveClaudeCliAgentIds(cfg: NodoAssistConfig): string[] {
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(
      `- NodoAssist auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`,
    );
    fixHints.push(
    ...
    lines.push(
      `- NodoAssist auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- NodoAssist auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
