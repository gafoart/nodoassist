...
function isTrustedAutoInstallableSkill(skill: { bundled: boolean; source: string }): boolean {
  ...
  return skill.bundled && skill.source === "nodoassist-bundled";
}
...
export async function setupSkills(
  ...
  if (installable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        ...
        `To inspect available skills, run: ${formatCliCommand("nodoassist skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("nodoassist skills check")}`,
      ].join("\n"),
  ...
  if (installable.length > 0) {
    ...
    for (const target of selectedSkills) {
      ...
      // visible through `nodoassist skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("nodoassist doctor")}\` to review skills + requirements.`,
      );
