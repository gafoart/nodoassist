// Implements `nodoassist agents add`, including config mutation, workspace setup, auth copy, and route binding setup.
...
export async function agentsAddCommand(
  ...
  if (nonInteractive) {
    if (!nameInput) {
      runtime.error(
        `Agent name is required in non-interactive mode. Run ${formatCliCommand("nodoassist agents add <id> --workspace <path>")}.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (agentId === DEFAULT_AGENT_ID) {
      runtime.error(
        `"${DEFAULT_AGENT_ID}" is reserved. Choose another name, or run ${formatCliCommand("nodoassist agents list")} to inspect the default agent.`,
      );
      runtime.exit(1);
      return;
    }
    ...
    if (findAgentEntryIndex(listAgentEntries(cfg), agentId) >= 0) {
      runtime.error(
        `Agent "${agentId}" already exists. Run ${formatCliCommand("nodoassist agents list")} to inspect configured agents.`,
      );
      runtime.exit(1);
      return;
    }
  ...
  try {
    await prompter.intro("Add NodoAssist agent");
    const name =
      nameInput ??
      (await prompter.text({
        message: "Agent name",
