...
export async function runPostUpgradeProbes(params: {
  ...
  if (!installs) {
    findings.push({
      ...
      message:
        "Installed plugin index is missing, unreadable, or malformed. Run `nodoassist plugins registry --refresh` to rebuild it before post-upgrade validation.",
    });
  ...
  for (const record of installs.plugins) {
    ...
    if (pkgRelPath) {
      ...
      const entries = pkg.nodoassist?.extensions ?? [];
      if (entries.length > 0) {
    ...
    if (record.manifestPath && record.manifestHash) {
      ...
      if (currentHash && currentHash !== record.manifestHash) {
        findings.push({
          ...
          message: `Plugin ${record.pluginId} manifest hash drifted from installs.json snapshot. Run \`nodoassist plugins registry --refresh\` to re-sync.`,
          plugin: record.pluginId,
        });
      }
    }
