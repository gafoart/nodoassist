...
function isTempBackedNodoAssistRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const nodoassistIndex = lower.lastIndexOf("nodoassist");
  if (nodoassistIndex < 1) {
    return false;
  }
  return lower[nodoassistIndex - 1] === "tmp" || lower[nodoassistIndex - 1] === "temp";
}
  ...
  return (
    lower.some(
      (segment) =>
        ...
        segment.startsWith("nodoassist@"),
    ) || isTempBackedNodoAssistRoot(beforeSkillRoot)
  );
}
...
export function sessionSnapshotIssueToHealthFinding(
  ...
  return {
    ...
    fixHint:
      "To clean up the advisory artifact, run `nodoassist doctor --fix` to rewrite stale cached session metadata paths, or start a fresh session after confirming history can be retired.",
  };
}
