...
function hasConfiguredCodexOAuthProfile(cfg: NodoAssistConfig): boolean {
...
export function noteLegacyCodexProviderOverride(cfg: NodoAssistConfig): void {
...
function listAuthProfileHealthTargets(cfg: NodoAssistConfig): AuthProfileHealthTarget[] {
...
async function resolveAuthIssueHint(
  ...
  if (issue.reasonCode === "malformed_api_key") {
    return "Paste the API key value, not an NodoAssist onboarding command.";
  }
  ...
  return {
    ...
    fixHint:
      ...
      (params.issue.status === "expiring"
        ? "Run `nodoassist doctor --fix` to refresh expiring OAuth profiles, or re-authenticate static tokens."
        : "Run `nodoassist doctor --fix` to refresh OAuth profiles, or re-authenticate this provider."),
  };
}
