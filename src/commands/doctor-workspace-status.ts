...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("nodoassist tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("nodoassist tasks flow cancel <flow-id>")}`,
    ]
...
function pluginVersionDriftToHealthFindings(
  ...
  return drift.drifts.map((entry) => {
    ...
    return {
      ...
      fixHint: `${updateCommand} && ${formatCliCommand("nodoassist gateway restart")}`,
    };
  });
}
...
function taskFlowRecoveryToHealthFinding(finding: TaskFlowRecoveryFinding): HealthFinding {
  return {
    ...
    fixHint: [
      formatCliCommand(`nodoassist tasks flow show ${finding.flowId}`),
      formatCliCommand(`nodoassist tasks flow cancel ${finding.flowId}`),
    ].join(" or "),
  };
}
...
function notePluginVersionDrift(drift: PluginVersionDriftReport | undefined) {
  ...
  const lines = [
    ...
    } not on NodoAssist ${drift.gatewayVersion}`,
    ...
    singleDrift
      ? `Fix: ${updateCommands[0]} && ${formatCliCommand("nodoassist gateway restart")}.`
      : [
          "Fix each drifted plugin:",
          ...updateCommands.map((command) => `- ${command}`),
          `Then run ${formatCliCommand("nodoassist gateway restart")}.`,
        ].join("\n"),
  ];
