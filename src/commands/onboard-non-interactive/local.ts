...
export async function runNonInteractiveLocalSetup(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `nodoassist gateway run` or `nodoassist agent --local`.",
              ]
            : [`Run \`${formatCliCommand("nodoassist gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
    ...
    if (!probe.ok) {
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              "Non-interactive local setup only waits for an already-running gateway unless you pass `--install-daemon` to `nodoassist onboard`.",
              `Fix: start \`${formatCliCommand("nodoassist gateway run")}\`, re-run \`${formatCliCommand("nodoassist onboard --install-daemon")}\`, or use \`${formatCliCommand("nodoassist onboard --skip-health")}\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("nodoassist configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.openclaw.ai/tools/web`,
    );
  }
}
