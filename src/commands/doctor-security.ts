...
import type { NodoAssistConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: NodoAssistConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: NodoAssistConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "NodoAssist default (full)";
  const defaultRequestedAskSource = "NodoAssist default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("nodoassist approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: NodoAssistConfig): string[] {
  void cfg;
  return [];
}

function collectExecFilesystemPolicyWarnings(cfg: NodoAssistConfig): string[] {
...
function collectPlaintextConfigSecretWarnings(cfg: NodoAssistConfig): string[] {
  ...
  return [
    "- WARNING: nodoassist.json contains plaintext secret-bearing config fields.",
    `  Paths: ${pathLine}`,
    "  Agents or workspace tools that can read config files may see these API keys/tokens.",
    `  Migrate them to SecretRefs with ${formatCliCommand("nodoassist secrets configure")} or ${formatCliCommand("nodoassist secrets apply")}, then verify with ${formatCliCommand("nodoassist secrets audit --check")}.`,
  ];
}
...
export async function collectSecurityWarnings(
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      `  Check local policy with: ${formatCliCommand("nodoassist approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("nodoassist configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("nodoassist config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("nodoassist doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("nodoassist config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('nodoassist config set session.dmScope "per-channel-peer"') +
...
export async function noteSecurityWarnings(cfg: NodoAssistConfig) {
  const warnings = await collectSecurityWarnings(cfg);
  const auditHint = `- Run: ${formatCliCommand("nodoassist security audit --deep")}`;
