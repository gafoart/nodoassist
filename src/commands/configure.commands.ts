 ...
 * Mirrors the real `nodoassist config <sub>` surface so the message only ever
...
const CONFIGURE_NON_TTY_HINT = [
  ...
  `  ${formatCliCommand("nodoassist config set <path> <value>")}  write a config entry`,
  `  ${formatCliCommand("nodoassist config get <path>")}          read a config entry`,
  `  ${formatCliCommand("nodoassist config patch")}              apply a JSON patch`,
  `  ${formatCliCommand("nodoassist config validate")}           validate configuration`,
].join("\n");
...
export async function configureCommandFromSectionsArg(
  ...
  // Fail closed once at the shared entry: both `nodoassist configure` and the
  // no-subcommand `nodoassist config` route here, so a single guard keeps them
  ...
  if (invalid.length > 0) {
    runtime.error(
      `Invalid --section: ${invalid.join(", ")}. Expected one of: ${CONFIGURE_WIZARD_SECTIONS.join(", ")}. Run ${formatCliCommand("nodoassist configure")} without --section to use the full wizard.`,
    );
    runtime.exit(1);
    return;
  }
