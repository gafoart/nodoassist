...
import { findStaleNodoAssistUpdateLaunchdJobs } from "../daemon/launchd.js";
...
export function collectMacLaunchAgentOverrideWarning(deps?: {
  ...
  const markerCandidates = [path.join(home, ".nodoassist", "disable-launchagent")];
...
/** Returns a warning for stale NodoAssist updater launchd jobs left after interrupted updates. */
export async function collectMacStaleNodoAssistUpdateLaunchdJobsWarning(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleNodoAssistUpdateLaunchdJobs;
  ...
  const jobs = await (deps?.findJobs ?? findStaleNodoAssistUpdateLaunchdJobs)(scanEnv).catch(
    () => [],
  );
  if (jobs.length === 0) {
    return null;
  ...
  return [
    "- Stale NodoAssist updater launchd job(s) detected.",
    ...jobs.map((job) => {
      const exitStatus =
    ...
    `  ${formatCliCommand("nodoassist gateway restart")}`,
  ].join("\n");
}

/** Emits stale updater launchd job notes using the gateway service environment when available. */
export async function noteMacStaleNodoAssistUpdateLaunchdJobs(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleNodoAssistUpdateLaunchdJobs;
  env?: NodeJS.ProcessEnv;
...
function hasConfigGatewayCreds(cfg: NodoAssistConfig): boolean {
...
export async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  const tokenEntries = [
    ["NODOASSIST_GATEWAY_TOKEN", await getenv("NODOASSIST_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["NODOASSIST_GATEWAY_PASSWORD", await getenv("NODOASSIST_GATEWAY_PASSWORD")],
  ] as const;
  ...
  return [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "NODOASSIST_GATEWAY_PASSWORD"}\` is set; it can make local clients use a different password than gateway.auth.password.`
      : undefined,
...
export async function collectMacGatewayPlatformWarnings(
  ...
  const serviceEnv =
    platform === "darwin" ? await resolveGatewayServiceEnvForPlatformNotes(deps) : deps?.env;
  const staleUpdateWarning = await collectMacStaleNodoAssistUpdateLaunchdJobsWarning({
    env: serviceEnv,
    findJobs: deps?.findJobs,
    platform,
  });
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.NODOASSIST_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- NODOASSIST_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
