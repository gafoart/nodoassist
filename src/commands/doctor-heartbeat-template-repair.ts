...
const DOCS_HEARTBEAT_TEMPLATE_PAGE_AS_TEMPLATE = [
  "# HEARTBEAT.md template",
  "`HEARTBEAT.md` lives in the agent workspace. Keep the file empty, or with only Markdown comments and headings, when you want NodoAssist to skip heartbeat model calls.",
...
function heartbeatTemplateAnalysisToHealthFinding(
  ...
  return {
    ...
    fixHint: 'Run "nodoassist doctor --fix" to replace it with the clean heartbeat template.',
  };
}
  ...
  if (!params.shouldRepair) {
    note(
      [
        ...
        'Run "nodoassist doctor --fix" to replace it with the clean heartbeat template.',
      ].join("\n"),
      "Heartbeat template",
    );
    return;
