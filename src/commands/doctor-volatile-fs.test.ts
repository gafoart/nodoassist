...
describe("detectLinuxVolatileStateDir", () => {
  const TMPFS_MOUNT_INFO = [
    ...
    "35 22 0:35 / /home/user/.nodoassist rw - tmpfs tmpfs rw,size=1048576k",
  ].join("\n");
  const RAMFS_MOUNT_INFO = [
    "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
    "35 22 0:35 / /home/user/.nodoassist rw - ramfs ramfs rw",
  ].join("\n");
  ...
  it.each([
    ...
    const result = detectLinuxVolatileStateDir("/home/user/.nodoassist", {
      platform: "linux",
      mountInfo,
    ...
    expect(result).toMatchObject({
      path: "/home/user/.nodoassist",
      mountPoint: "/home/user/.nodoassist",
      fsType,
    });
  });
  ...
  it("uses the most specific matching mount", () => {
    const mountInfo = [
      ...
      "35 30 0:35 / /home/user/.nodoassist rw - tmpfs tmpfs rw",
    ].join("\n");

    expect(
      detectLinuxVolatileStateDir("/home/user/.nodoassist", {
        platform: "linux",
        mountInfo,
        resolveRealPath: (targetPath) => targetPath,
      }),
  ...
  it("detects a missing state directory through an existing symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-doctor-volatile-"));
    try {
      ...
      const result = detectLinuxVolatileStateDir(path.join(stateLink, "nodoassist"), {
        platform: "linux",
        mountInfo: [
          "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
      ...
      expect(result).toMatchObject({
        path: path.join(resolvedVolatileMount, "nodoassist"),
...
describe("formatLinuxVolatileStateDirWarning", () => {
  it("covers all SQLite state and sidecar files under the volatile state directory", () => {
    const warning = formatLinuxVolatileStateDirWarning("~/.nodoassist", {
      path: "/home/user/.nodoassist",
      mountPoint: "/home/user/.nodoassist",
      fsType: "tmpfs",
    });
    ...
    expect(warning).toContain("NODOASSIST_STATE_DIR");
  });
});
