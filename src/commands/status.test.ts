...
beforeAll(() => {
  envSnapshot = captureEnv(["NODOASSIST_PROFILE"]);
  process.env.NODOASSIST_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByNodoAssist: Boolean(command),
    externallyManaged: !command && runtime?.status === "running",
    loadedText: service.loadedText,
    runtime,
    runtimeShort: runtime?.pid ? `pid ${runtime.pid}` : null,
    wrapperPath: command?.environment?.NODOASSIST_WRAPPER?.trim() || undefined,
  };
}
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/nodoassist/pnpm-lock.yaml",
        markerPath: "/tmp/nodoassist/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.NODOASSIST_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.nodoassist.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.nodoassist.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.NODOASSIST_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/nodoassist-root.js", () => ({
  resolveNodoAssistPackageRoot: vi.fn().mockResolvedValue("/tmp/nodoassist"),
  resolveNodoAssistPackageRootSync: vi.fn(() => "/tmp/nodoassist"),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /nodoassist (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("nodoassist --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("NODOASSIST_GATEWAY_TOKEN", "abcd1234", async () => {
  ...
  it("notes when secret diagnostics may come from a CLI process outside the service wrapper context", async () => {
    ...
    mocks.resolveGatewayService.mockReturnValue({
      ...
      readCommand: async () => ({
        ...
        environment: { NODOASSIST_WRAPPER: wrapperPath },
        sourcePath: "/tmp/Library/LaunchAgents/ai.nodoassist.gateway.plist",
      }),
    });
    mocks.loadConfig.mockReturnValue({
    ...
    await withOptionalEnvVar("NODOASSIST_WRAPPER", undefined, async () => {
      const logs = await runStatusAndGetLogs();
      expectLogsInclude(logs, "Secret diagnostics:");
      expectLogsInclude(logs, "installed gateway service uses NODOASSIST_WRAPPER");
    ...
    await withEnvVar("NODOASSIST_WRAPPER", wrapperPath, async () => {
      const logs = await runStatusAndGetLogs();
      expectLogsInclude(logs, "Secret diagnostics:");
