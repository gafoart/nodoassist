...
import {
  NODOASSIST_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveNodoAssistWrapperPath,
...
function configContainsSecretRef(config: NodoAssistConfig | undefined): boolean {
  if (!config) {
    return false;
  }
...
// they share the NODOASSIST_ prefix that is otherwise stripped from preserved
...
const PRESERVED_NODOASSIST_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "NODOASSIST_CLI_CONTAINER_BYPASS",
  "NODOASSIST_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("NODOASSIST_") && !PRESERVED_NODOASSIST_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[NODOASSIST_WRAPPER_ENV_KEY];
  ...
  if (wrapperPointsAtWindowsTaskScript) {
    params.warn?.(
      `Ignoring ${NODOASSIST_WRAPPER_ENV_KEY} because it points to the Windows task script; using the NodoAssist gateway entrypoint directly to avoid a recursive gateway.cmd wrapper.`,
    );
  }
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveNodoAssistWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [NODOASSIST_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, NODOASSIST_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.NODOASSIST_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("nodoassist gateway install")}\` after fixing the error.`;
}
