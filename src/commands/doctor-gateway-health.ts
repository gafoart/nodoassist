...
function noteCliGatewayVersionSkew(status: StatusSummary | undefined): void {
  ...
  note(
    [
      `This command is NodoAssist ${VERSION}; the running Gateway is NodoAssist ${gatewayVersion}.`,
      "Check `nodoassist --version`, `which nodoassist`, and `nodoassist gateway status --deep`.",
      "If this mismatch is unexpected, update PATH so `nodoassist` points to the version you want, or reinstall the Gateway service from that same NodoAssist install.",
    ].join("\n"),
    "NodoAssist version mismatch",
  );
}
