...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(env.NODOASSIST_PROFILE);
    hints.push(
      `LaunchAgent label cached but plist missing. Clear with: launchctl bootout gui/$UID/${label}`,
    );
    hints.push(`Then reinstall: ${formatCliCommand("nodoassist gateway install", env)}`);
  }
  if (runtime.missingUnit) {
    hints.push(
      `Service not installed. Run: ${formatCliCommand("nodoassist gateway install", env)}`,
    );
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
  ...
  if (runtime.missingGuiSession && platform === "darwin") {
    ...
    hints.push(
      `Sign in to the macOS desktop as this user, then run: ${formatCliCommand("nodoassist gateway restart", env)}`,
    );
    hints.push(
  ...
  if (runtime.missingSupervision && platform === "darwin") {
    hints.push(
      `LaunchAgent installed but not loaded. Run: ${formatCliCommand("nodoassist gateway restart", env)}`,
    );
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
  ...
  if (runtime.status === "stopped") {
    if (platform === "linux" && isSystemdStartLimitHit(runtime)) {
      ...
      hints.push(
        ...
        `Recover with: ${formatCliCommand("nodoassist gateway restart", env)}, then inspect logs if it keeps crashing.`,
      );
    } else {
    ...
    hints.push(
      ...buildPlatformRuntimeLogHints({
        platform,
        env,
        systemdServiceName: resolveGatewaySystemdServiceName(env.NODOASSIST_PROFILE),
        windowsTaskName: resolveGatewayWindowsTaskName(env.NODOASSIST_PROFILE),
      }),
    );
  }
  if (platform === "linux" && isSystemdCgroupHygieneRisk(runtime.systemd)) {
    const unit =
      ...
      `${resolveGatewaySystemdServiceName(env.NODOASSIST_PROFILE)}.service`;
    ...
    if (summary) {
      hints.push(
        ...
        `After reviewing service settings, run: ${formatCliCommand("nodoassist gateway restart", env)}`,
      );
    }
  }
  return hints;
