...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(nodoassist completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `nodoassist doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`nodoassist completion --write-state\` or \`nodoassist doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
