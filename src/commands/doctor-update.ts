...
async function detectNodoAssistGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
async function restartRunningGatewayServiceAfterUpdate(
  ...
  try {
    ...
    note("Restarted the running gateway service after updating NodoAssist.", "Update");
    return true;
  } catch (err) {
...
/** Offers to update NodoAssist before doctor when running interactively from an updatable install. */
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.NODOASSIST_UPDATE_IN_PROGRESS);
  ...
  const git = await detectNodoAssistGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update NodoAssist from git before running doctor?",
      initialValue: true,
    });
    if (!shouldUpdate) {
      return { updated: false };
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("nodoassist update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
