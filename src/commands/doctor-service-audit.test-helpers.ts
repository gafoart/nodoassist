...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.NODOASSIST_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.NODOASSIST_GATEWAY_TOKEN);
}
