...
describe("sessionsTailCommand", () => {
  ...
  beforeEach(() => {
    ...
    previousTrajectoryDir = process.env.NODOASSIST_TRAJECTORY_DIR;
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sessions-tail-"));
    process.env.NODOASSIST_STATE_DIR = path.join(tmpDir, "state");
  ...
  afterEach(() => {
    ...
    if (previousTrajectoryDir === undefined) {
      ...
      process.env.NODOASSIST_TRAJECTORY_DIR = previousTrajectoryDir;
    }
