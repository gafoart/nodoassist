...
const mocks = vi.hoisted(() => ({
  ...
  maybeRepairManagedNpmNodoAssistPeerLinks: vi.fn(),
...
vi.mock("../doctor-plugin-registry.js", () => ({
  maybeRepairManagedNpmNodoAssistPeerLinks: mocks.maybeRepairManagedNpmNodoAssistPeerLinks,
...
vi.mock("./shared/channel-doctor.js", () => ({
  collectChannelDoctorRepairMutations: ({ cfg }: { cfg: NodoAssistConfig }) => {
...
vi.mock("./shared/empty-allowlist-scan.js", () => ({
  scanEmptyAllowlistPolicyWarnings: (cfg: NodoAssistConfig) =>
    cfg.channels?.signal
...
vi.mock("./shared/allowlist-policy-repair.js", () => ({
  maybeRepairAllowlistPolicyAllowFrom: async (cfg: NodoAssistConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/bundled-plugin-load-paths.js", () => ({
  maybeRepairBundledPluginLoadPaths: (cfg: NodoAssistConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/invalid-plugin-config.js", () => ({
  maybeRepairInvalidPluginConfig: (cfg: NodoAssistConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
vi.mock("./shared/legacy-tools-by-sender.js", () => ({
  maybeRepairLegacyToolsBySenderKeys: (cfg: NodoAssistConfig) => {
...
vi.mock("./shared/exec-safe-bins.js", () => ({
  maybeRepairExecSafeBinProfiles: (cfg: NodoAssistConfig) => ({
    config: cfg,
    changes: [],
  }),
}));
...
describe("doctor repair sequencing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.applyPluginAutoEnable.mockImplementation((params: { config: NodoAssistConfig }) => ({
      config: params.config,
      changes: [],
    }));
    mocks.materializePluginAutoEnableCandidates.mockImplementation(
      (params: { config: NodoAssistConfig }) => ({
        config: params.config,
        changes: [],
      }),
    );
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementation((cfg: NodoAssistConfig) => ({
      config: cfg,
      changes: [],
    }));
    mocks.maybeRepairManagedNpmNodoAssistPeerLinks.mockResolvedValue(false);
    ...
    mocks.maybeRepairOpenAICodexAuthConfig.mockImplementation((cfg: NodoAssistConfig) => ({
      changes: [],
      config: cfg,
      warnings: [],
    }));
    ...
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementation((cfg: NodoAssistConfig) => ({
      config: cfg,
      changes: [],
    }));
    ...
    mocks.maybeRepairStalePluginConfig.mockImplementation((cfg: NodoAssistConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("repairs managed npm plugin drift before missing plugin install repair", async () => {
    ...
    mocks.maybeRepairManagedNpmNodoAssistPeerLinks.mockImplementation(async () => {
      events.push("nodoassist-peer-links");
      return true;
    });
    ...
    expect(events).toEqual(["bundled-shadow-cleanup", "nodoassist-peer-links", "missing-installs"]);
    ...
    expect(mocks.maybeRepairManagedNpmNodoAssistPeerLinks).toHaveBeenCalledOnce();
    const peerLinkCall = mocks.maybeRepairManagedNpmNodoAssistPeerLinks.mock.calls[0]?.[0];
  ...
  it("auto-enables newly installed configured plugins after doctor repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "brave" from @nodoassist/brave-plugin.'],
      warnings: [],
    });
    mocks.applyPluginAutoEnable.mockImplementationOnce((params: { config: NodoAssistConfig }) => ({
      config: {
        ...params.config,
        plugins: {
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "brave" from @nodoassist/brave-plugin.',
  ...
  it("explicitly enables plugins repaired from env-only configuration", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: ['Installed missing configured plugin "exa" from @nodoassist/exa-plugin.'],
      warnings: [],
    ...
    expect(result.changeNotes).toStrictEqual([
      'Installed missing configured plugin "exa" from @nodoassist/exa-plugin.',
  ...
  it("surfaces ClawHub notices from successful missing configured plugin repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      ...
      warnings: [],
      notices: [
        'ClawHub trust warning for "@nodoassist/brave-plugin@1.2.3": scan=pending; reasons=pending.',
      ],
    });
    mocks.maybeRepairStalePluginConfig.mockImplementationOnce((cfg: NodoAssistConfig) => ({
      config: {
        ...cfg,
        plugins: {
          ...cfg.plugins,
  ...
  it("moves legacy Codex routes to canonical OpenAI before missing plugin install repair", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockImplementationOnce(
      async (params: { cfg: NodoAssistConfig }) => {
  ...
  it("runs group allowFrom fallback migration after open-policy allowFrom repair", async () => {
    const events: string[] = [];
    mocks.maybeRepairOpenPolicyAllowFrom.mockImplementationOnce((cfg: NodoAssistConfig) => {
    ...
    mocks.maybeRepairGroupAllowFromFallback.mockImplementationOnce((cfg: NodoAssistConfig) => {
  ...
  it("does not remove deferred configured plugins during the package update doctor pass", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [
        'Skipped package-manager repair for configured plugin "brave" during package update; rerun "nodoassist doctor --fix" after the update completes.',
      ],
      warnings: [],
    });
    const result = await runDoctorRepairSequence({
  ...
  it("preserves configured plugins when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured plugin "brave" from @nodoassist/brave-plugin: package install failed',
      ],
  ...
  it("preserves configured channels when their install repair fails", async () => {
    mocks.repairMissingConfiguredPluginInstalls.mockResolvedValueOnce({
      changes: [],
      warnings: [
        'Failed to install missing configured channel plugin "whatsapp" from @nodoassist/whatsapp: package install failed',
      ],
