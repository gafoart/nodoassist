...
export function normalizeLegacyCommandsConfig(
  ...
  return {
    ...cfg,
    commands: commands as NodoAssistConfig["commands"],
  };
}
...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as NodoAssistConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as NodoAssistConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<NodoAssistConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<NodoAssistConfig["models"]>>;
...
function normalizeLegacyCodexCliProviderRuntimePins(
  cfg: NodoAssistConfig,
  changes: string[],
): { config: NodoAssistConfig; changed: boolean } {
  ...
  return changed
    ? {
        config: {
          ...
          models: {
            ...rawModels,
            providers: nextProviders as NonNullable<NodoAssistConfig["models"]>["providers"],
          },
        },
        changed: true,
      }
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  const nextCfg = changed
    ? {
        ...cfgWithProviders,
        agents: nextAgents as NodoAssistConfig["agents"],
      }
    : cfgWithProviders;
  return nextCfg;
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<NodoAssistConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as NodoAssistConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<NodoAssistConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyOllamaNativeNumCtxParams(
  ...
  type ProviderConfigMap = NonNullable<NonNullable<NodoAssistConfig["models"]>["providers"]>;
  for (const [providerId, rawProvider] of Object.entries(rawProviders)) {
    if (!isRecord(rawProvider)) {
      continue;
    }
