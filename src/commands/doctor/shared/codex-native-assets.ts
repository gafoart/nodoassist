...
function isCodexRuntimeConfigured(cfg: NodoAssistConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: NodoAssistConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: NodoAssistConfig, env: NodeJS.ProcessEnv): boolean {
...
export async function collectCodexNativeAssetInfoNotes(params: {
  ...
  return [
    [
      "- Personal Codex CLI assets were found, but native Codex-mode NodoAssist agents use isolated per-agent Codex homes.",
      ...
      "- If the Codex plugin is not installed, run `nodoassist plugins install npm:@nodoassist/codex` first. Then run `nodoassist migrate plan codex` to inventory them. Applying that migration copies skills into the current NodoAssist agent workspace; Codex plugins, hooks, and config stay manual-review only.",
    ].join("\n"),
  ];
}
