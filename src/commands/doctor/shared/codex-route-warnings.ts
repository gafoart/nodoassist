...
import { AGENT_MODEL_CONFIG_KEYS } from "@nodoassist/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@nodoassist/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@nodoassist/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@nodoassist/normalization-core/string-coerce";
...
function resolveImplicitDefaultAgentModelRef(cfg: NodoAssistConfig): string {
...
function collectConfigModelRefs(cfg: NodoAssistConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: NodoAssistConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: NodoAssistConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: NodoAssistConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: NodoAssistConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: NodoAssistConfig; changes: string[] } {
...
function clearConfigLegacyAgentRuntimePolicies(cfg: NodoAssistConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: NodoAssistConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: NodoAssistConfig }): ConfigRouteRepairResult {
...
function formatUnsupportedCompactionWarning(params: {
  ...
  return [
    "- Codex runtime uses native server-side compaction and ignores NodoAssist compaction summarizer overrides.",
    ...params.hits.map(
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `nodoassist doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function formatDisabledCodexPluginWarning(params: {
  ...
  const fixHint = params.blockedOutsideEntry
    ? "- Enable plugin loading and remove `codex` from plugins.deny, or set the affected OpenAI models to an NodoAssist runtime policy."
    : "- Run `nodoassist doctor --fix`: it enables plugins.entries.codex, or set the affected OpenAI models to an NodoAssist runtime policy.";
  return [
...
function collectCodexAppServerCommandWarnings(cfg: NodoAssistConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
  ...
  if (hits.length > 0) {
    warnings.push(
      [
        ...
        "- Run `nodoassist doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    );
  }
  ...
  if (fixableHits.length > 0) {
    warnings.push(
      formatUnsupportedCompactionWarning({
        hits: fixableHits,
        fixHint:
          "- Run `nodoassist doctor --fix`: it removes unsupported Codex compaction overrides.",
      }),
    );
  }
  return warnings;
...
}): { cfg: NodoAssistConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "nodoassist") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "nodoassist") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `nodoassist doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
