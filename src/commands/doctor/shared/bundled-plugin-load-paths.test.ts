...
import { bundledDistPluginRootAt, bundledPluginRootAt } from "nodoassist/plugin-sdk/test-fixtures";
...
describe("bundled plugin load path repair", () => {
  beforeEach(() => {
    const packageRoot = "/app/node_modules/nodoassist";
  ...
  it("detects legacy bundled plugin paths that still point at source extensions", () => {
    const packageRoot = path.resolve("app-node-modules", "nodoassist");
  ...
  it("removes stale bundled paths from old versioned NodoAssist package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "nodoassist");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
      "nodoassist@2026.3.28_@napi-rs+canvas@0.1.97",
      "node_modules",
      "nodoassist",
    );
  ...
  it("removes stale legacy bundled paths from old versioned NodoAssist package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "nodoassist");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("formats a doctor hint for legacy bundled plugin paths", () => {
    ...
    expect(warnings).toEqual([
      `- plugins.load.paths: bundled plugin path "${legacyPath}" still aliases feishu; NodoAssist loads the packaged bundled plugin from "${bundledPath}".`,
      '- Run "nodoassist doctor --fix" to remove these redundant bundled plugin paths.',
    ]);
  });

  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "nodoassist", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
