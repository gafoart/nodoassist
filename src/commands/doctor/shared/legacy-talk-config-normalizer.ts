...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<NodoAssistConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as NodoAssistConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(
  ...
  const normalizedTalk = normalizeTalkSection(rawTalk as NodoAssistConfig["talk"]) ?? {};
