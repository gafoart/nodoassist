...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".nodoassist-runtime-deps.json" ||
    name === ".nodoassist-runtime-deps-stamp.json" ||
    name.startsWith(".nodoassist-runtime-deps-")
  );
}

function isInstallStageDebrisName(name: string): boolean {
  return /^\.nodoassist-install-stage(?:-.+)?$/u.test(name);
}

function isLegacyDependencyDebrisName(name: string): boolean {
  return (
    isRuntimeDependencyMarkerName(name) ||
    name === ".nodoassist-pnpm-store" ||
    name === ".nodoassist-install-backups" ||
...
function collectExplicitStageTargets(env: NodeJS.ProcessEnv): CleanupTarget[] {
  return splitPathList(env.NODOASSIST_PLUGIN_STAGE_DIR).map((entry) => ({
    kind: "explicit-stage",
...
async function hasNodoAssistRenameResidue(root: string): Promise<boolean> {
  const nodeModulesRoot = path.join(root, "node_modules");
  if (await isFile(path.join(nodeModulesRoot, ".nodoassist-rename-tmp"))) {
    return true;
  }
  ...
  for (const entry of entries) {
    ...
    if (await isFile(path.join(entryPath, ".nodoassist-rename-tmp"))) {
      return true;
    }
    ...
    for (const scopedEntry of scopedEntries) {
      ...
      if (await isFile(path.join(entryPath, scopedEntry.name, ".nodoassist-rename-tmp"))) {
        return true;
      }
    }
  }
...
async function hasExplicitStageDebrisProof(root: string): Promise<boolean> {
  ...
  return await hasNodoAssistRenameResidue(root);
}
...
function filterLegacyStaleRootCandidates(
  ...
  for (const target of targets) {
    ...
    if (!cleanupRootPaths.some((rootPath) => isPathInsideRoot(targetPath, rootPath))) {
      warnings.push(
        `Skipped legacy plugin dependency state ${targetPath}: outside NodoAssist cleanup roots`,
      );
      continue;
    }
    safeTargets.push({ ...target, path: targetPath });
...
async function resolveSafeRemovalTarget(
  ...
  if (!cleanupRoots.some((root) => isPathInsideRoot(realPath, root.realPath))) {
    return {
      warning: `Skipped legacy plugin dependency state ${targetPath}: resolved outside NodoAssist cleanup roots`,
    };
  }
  return { target: targetPath };
}
...
/** Remove legacy plugin dependency state under trusted NodoAssist cleanup roots. */
