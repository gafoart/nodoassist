...
function repairBindingsForTest(config: NodoAssistConfig) {
  ...
  it("removes sidecar memory search index paths", () => {
    const res = migrateLegacyConfigForTest({
      memorySearch: {
        provider: "openai",
        store: {
          path: "/tmp/nodoassist-memory-{agentId}.sqlite",
    ...
    const res = migrateLegacyConfigForTest({
      agents: {
        ...
        list: [
          {
            id: "reviewer",
            agentRuntime: { fallback: "nodoassist" },
            embeddedHarness: {
              runtime: "codex",
              fallback: "none",
            },
...
describe("legacy migrate MCP server type aliases", () => {
  it("moves CLI-native http type to NodoAssist streamable HTTP transport", () => {
    const res = migrateLegacyConfigForTest({
      mcp: {
        servers: {
          silo: {
