...
describe("stale OAuth profile shadow doctor repair", () => {
  const envSnapshot = captureEnv(["NODOASSIST_STATE_DIR", "NODOASSIST_HOME"]);
  let tempRoot = "";
  let stateDir = "";

  beforeEach(async () => {
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-stale-oauth-shadow-"));
    stateDir = path.join(tempRoot, "state");
    process.env.NODOASSIST_STATE_DIR = stateDir;
    process.env.NODOASSIST_HOME = stateDir;
  });
  ...
  it("warns about stale local OAuth shadows without modifying the child store", async () => {
    ...
    expect(warnings[0]).toContain("nodoassist doctor --fix");
  ...
  it("uses the injected env for the main auth store", async () => {
    ...
    const injectedEnv = {
      ...process.env,
      NODOASSIST_STATE_DIR: injectedStateDir,
      NODOASSIST_HOME: injectedStateDir,
    };
    saveAuthProfileStore(
      storeWith(
        profileId,
  ...
  it("removes stale child OAuth shadows and local cooldown state", async () => {
    ...
    const result = await repairStaleOAuthProfileShadows({
      cfg: { agents: { list: [{ id: "telegram" }] } } satisfies NodoAssistConfig,
      now,
    });
