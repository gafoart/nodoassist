...
function listAgentRecords(cfg: NodoAssistConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: NodoAssistConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: NodoAssistConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: NodoAssistConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: NodoAssistConfig): boolean {
...
function hasConfiguredSafeBins(cfg: NodoAssistConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: NodoAssistConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: NodoAssistConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: NodoAssistConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
  ...
  if (groupPolicy.value === "message_tool") {
    ...
    warnings.push(
      `- ${groupPolicy.path} is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; NodoAssist falls back to automatic visible replies, so normal replies may post to the source chat. Enable the message tool or set ${groupPolicy.path} to "automatic".`,
    );
  }
  ...
  if (globalVisibleReplies === "message_tool" && groupPolicy.path !== "messages.visibleReplies") {
    ...
    warnings.push(
      `- messages.visibleReplies is set to "message_tool", but the message tool is unavailable for ${formatTargets(
        targets,
      )}; NodoAssist falls back to automatic direct-chat replies, so normal replies may post to the source chat. Enable the message tool or set messages.visibleReplies to "automatic".`,
    );
  }
  return warnings;
}
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: NodoAssistConfig): string[] {
...
export function collectProfileConfiguredToolSectionWarnings(cfg: NodoAssistConfig): string[] {
  const warnings: string[] = [];
...
export async function resolveDoctorChannelPreviewConfig(params: {
  ...
  allowExec?: boolean;
}): Promise<{ cfg: NodoAssistConfig; diagnostics: string[] }> {
