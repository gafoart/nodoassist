...
import { compareNodoAssistVersions } from "../../../config/version.js";
import {
...
function isPluginsGloballyDisabled(cfg: NodoAssistConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: NodoAssistConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: NodoAssistConfig): string[] {
...
function isPluginEntryDisabled(cfg: NodoAssistConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: NodoAssistConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: NodoAssistConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: NodoAssistConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: NodoAssistConfig): string[] {
...
function collectConfiguredChannelIds(cfg: NodoAssistConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectWebSearchPluginIds(cfg: NodoAssistConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: NodoAssistConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: NodoAssistConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: NodoAssistConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: NodoAssistConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: NodoAssistConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: NodoAssistConfig): string[] {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareNodoAssistVersions(
  ...
  const touchedComparedToRelease = compareNodoAssistVersions(params.touchedVersion, releaseVersion);
