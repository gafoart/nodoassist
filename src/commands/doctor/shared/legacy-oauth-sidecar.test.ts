...
async function writeLegacySidecarThatNeedsKeychain(): Promise<{
  ...
  const state = await createNodoAssistTestState({
    layout: "state-only",
    prefix: "nodoassist-legacy-oauth-keychain-warn-",
    env: {
      NODOASSIST_AGENT_DIR: undefined,
      NODOASSIST_AUTH_PROFILE_SECRET_KEY: undefined,
    },
  });
  states.push(state);
...
describe("loadLegacyOAuthSidecarMaterial keychain-only headless warning", () => {
  ...
  function envWithoutVitestSignals(state: NodoAssistTestState): NodeJS.ProcessEnv {
  ...
  it("emits a single doctor-pointer warning when only Keychain can decrypt and prompts are disabled", async () => {
    ...
    expect(String(firstMessage)).toContain("nodoassist doctor --fix");
