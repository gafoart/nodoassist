...
function resolveBundledWorkspaceDir(cfg: NodoAssistConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isNodoAssistNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "nodoassist" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by NodoAssist. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed NodoAssist package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isNodoAssistNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; NodoAssist loads the packaged bundled plugin from "${hit.toPath}".`,
  );
