...
type CronMigrationDatabase = Pick<NodoAssistStateDatabase, "migration_runs" | "migration_sources">;
...
export function hasLegacyCronMigrationReceipt(source: LegacyCronMigrationSource): boolean {
  return hasLegacyCronMigrationReceiptInDatabase(openNodoAssistStateDatabase().db, source);
}
...
export function hasLegacyCronMigrationReceiptReadOnly(source: LegacyCronMigrationSource): boolean {
  const statePath = resolveNodoAssistStateSqlitePath(process.env);
