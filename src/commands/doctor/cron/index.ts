...
function legacyCronStoreFinding(params: {
  ...
  return {
    ...
    fixHint:
      params.fixHint ??
      `Run ${formatCliCommand("nodoassist doctor --fix")} to normalize legacy cron storage.`,
  };
}
  ...
  if (state.legacyStoreDetected) {
    ...
    if (archiveResult.ok) {
      ...
      for (const failure of archiveResult.failures) {
        warnings.push(
          `Migrated cron jobs to SQLite but could not archive the legacy cron file at ${shortenHomePath(failure.path)}: ${failure.reason}. Remove it manually or rerun ${formatCliCommand("nodoassist doctor --fix")} to retry.`,
        );
      }
    }
  ...
  try {
    ...
    return [
      legacyCronStoreFinding({
        ...
        fixHint: [
          `Fix the file's permissions or contents and re-run ${formatCliCommand("nodoassist doctor")}.`,
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  try {
    ...
    note(
      [
        ...
        `- ${reason}`,
        `Fix the file's permissions or contents and re-run ${formatCliCommand("nodoassist doctor")}; later health checks will continue.`,
      ].join("\n"),
      "Cron",
    );
    return;
  ...
  if (rawJobs.length === 0) {
    ...
    note(
      [
        ...
        ...previewLines,
        `Repair with ${formatCliCommand("nodoassist doctor --fix")} to finish the migration.`,
      ].join("\n"),
      "Cron",
    );
  ...
  if (inFlightCount > 0) {
    ...
    note(
      [
        `${pluralize(inFlightCount, "cron job")} ${inFlightCount === 1 ? "is" : "are"} still marked in-flight (\`state.runningAtMs\` is set), so ${formatCliCommand("nodoassist cron list")} shows ${subject} as \`running\`.`,
        ...
        `- Review with ${formatCliCommand("nodoassist cron list")} or ${formatCliCommand("nodoassist cron show <id>")}.`,
      ].join("\n"),
      "Cron",
    );
  }
  ...
  note(
    [
      noteHeading,
      ...previewLines,
      `Repair with ${formatCliCommand("nodoassist doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
