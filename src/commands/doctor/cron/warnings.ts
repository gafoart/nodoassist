...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("nodoassist cron list")} and ${formatCliCommand("nodoassist cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
export function collectCronDeliveryTargetAdvisory(params: {
  ...
  return [
    ...
    `Reactivate the channel plugin or update the job's \`delivery.channel\` after reviewing with ${formatCliCommand("nodoassist cron list")} and ${formatCliCommand("nodoassist cron show <job-id>")}.`,
  ].join("\n");
}
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.nodoassist/bin/ensure-whatsapp.sh` is not maintained by current NodoAssist and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("nodoassist channels status --probe")}, ${formatCliCommand("nodoassist doctor")}, and ${formatCliCommand("nodoassist gateway status")} for current health checks.`,
