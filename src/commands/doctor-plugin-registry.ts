...
import {
  auditNodoAssistPeerDependenciesInManagedNpmRoot,
  type NodoAssistPeerLinkAuditIssue,
...
export type PluginRegistryHealthIssue =
  ...
  | {
      kind: "managed-npm-nodoassist-peer-link";
      packageName: string;
      packageDir: string;
      reason: string;
    };
...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "nodoassist.plugin.json"));
...
function listStaleManagedNpmBundledPlugins(
  ...
  for (const npmRoot of listManagedPluginNpmRoots(params)) {
    ...
    for (const packageName of Object.keys(dependencies).toSorted((left, right) =>
      ...
      if (!packageName.startsWith("@nodoassist/")) {
        continue;
      }
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("nodoassist doctor --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("nodoassist doctor --fix")} to remove stale local install records and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return [];
...
/** Relinks managed npm plugin packages to the current NodoAssist host packages. */
export async function maybeRepairManagedNpmNodoAssistPeerLinks(
  params: PluginRegistryDoctorRepairParams,
): Promise<boolean> {
  const npmRoots = listManagedPluginNpmRoots(params);
  if (!params.prompter.shouldRepair) {
    const audits = await Promise.all(
      npmRoots.map((npmRoot) => auditNodoAssistPeerDependenciesInManagedNpmRoot({ npmRoot })),
    );
    const issues = audits.flatMap((audit) => audit.issues);
    if (issues.length > 0) {
      note(
        [
          "Managed npm NodoAssist host peer links need repair:",
          ...issues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("nodoassist doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (repaired > 0) {
    note(
      `Repaired NodoAssist host peer link(s) for ${repaired} managed npm plugin package(s).`,
      "Plugin registry",
    );
  }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm NodoAssist host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
...
async function listManagedNpmNodoAssistPeerLinkIssues(
  params: PluginRegistryDoctorRepairParams,
): Promise<NodoAssistPeerLinkAuditIssue[]> {
  const audits = await Promise.all(
    listManagedPluginNpmRoots(params).map((npmRoot) =>
      auditNodoAssistPeerDependenciesInManagedNpmRoot({ npmRoot }),
    ),
  );
...
export async function detectPluginRegistryHealthIssues(
  ...
  for (const issue of await listManagedNpmNodoAssistPeerLinkIssues(params)) {
    issues.push({
      kind: "managed-npm-nodoassist-peer-link",
  ...
  switch (issue.kind) {
    case "registry-missing-or-stale":
      return {
        ...
        path: issue.path,
        fixHint:
          "Run `nodoassist doctor --fix` to rebuild the plugin registry from enabled plugins.",
      };
    case "stale-managed-npm-bundled-plugin":
      return {
        ...
        fixHint:
          "Run `nodoassist doctor --fix` to remove stale managed npm packages and rebuild the plugin registry.",
      };
    case "stale-local-bundled-plugin-install-record":
      return {
        ...
        target: issue.pluginId,
        fixHint:
          "Run `nodoassist doctor --fix` to remove stale local install records and rebuild the plugin registry.",
      };
    case "managed-npm-nodoassist-peer-link":
      return {
        checkId: PLUGIN_REGISTRY_CHECK_ID,
        severity: "warning",
        message: `Managed npm package ${issue.packageName} has a broken NodoAssist peer link: ${issue.reason}.`,
        path: issue.packageDir,
        target: issue.packageName,
        fixHint: "Run `nodoassist doctor --fix` to relink managed npm plugin packages.",
      };
  }
...
export function pluginRegistryIssueToRepairEffect(
  ...
  switch (issue.kind) {
    ...
    case "managed-npm-nodoassist-peer-link":
      return {
        kind: "package",
        action: "would-relink-managed-npm-nodoassist-peer",
        target: issue.packageDir,
        dryRunSafe: false,
      };
  }
...
export async function maybeRepairPluginRegistryState(
  ...
  const repairedManagedNpmNodoAssistPeerLinks =
    await maybeRepairManagedNpmNodoAssistPeerLinks(params);
    ...
    if (preflight.action === "migrate") {
      note(
        [
          ...
          `Repair with ${formatCliCommand("nodoassist doctor --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (
    ...
    repairedManagedNpmNodoAssistPeerLinks
  ) {
