...
function renderEstablishedGatewayConnections(connections: PortConnection[]): string {
  return [
    ...
    "If logs show protocol mismatch after rollback, stop stale NodoAssist client processes listed here and rerun doctor.",
  ].join("\n");
}
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.NODOASSIST_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("nodoassist gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
