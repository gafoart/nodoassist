...
/** Prompt for configured channel sections to remove from nodoassist.json. */
export async function removeChannelConfigWizard(
  ...
  while (true) {
    ...
    if (configured.length === 0) {
      note(
        [
          "No channel config found in nodoassist.json.",
          `Tip: \`${formatCliCommand("nodoassist channels status")}\` shows what is configured and enabled.`,
        ].join("\n"),
        "Remove channel",
      );
      return next;
    ...
    if (Object.keys(nextChannels).length) {
      next.channels = nextChannels as NodoAssistConfig["channels"];
    } else {
