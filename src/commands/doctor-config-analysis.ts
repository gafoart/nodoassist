...
import { resolvePrimaryStringValue } from "@nodoassist/normalization-core/string-coerce";
...
function isUpdateInProgress(): boolean {
  const value = process.env.NODOASSIST_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: NodoAssistConfig): {
  ...
  const parsed = NodoAssistSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteOpencodeProviderOverrides(cfg: NodoAssistConfig): void {
...
export function collectImplicitFallbackClobberWarnings(cfg: NodoAssistConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: NodoAssistConfig): void {
