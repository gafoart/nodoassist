...
const replyPayloadModuleLoader = createLazyPromiseLoader(
  () => import("nodoassist/plugin-sdk/reply-payload"),
  { cacheRejections: true },
);
...
async function loadRuntimeConfig(): Promise<NodoAssistConfig> {
...
function missingAgentMessageError(): Error {
  return new Error(
    `Missing message. Use ${formatCliCommand('nodoassist agent --message "..." --agent <id>')} or ${formatCliCommand("nodoassist agent --message-file <path> --agent <id>")}.`,
  );
}
...
function parseTimeoutSeconds(opts: { cfg: NodoAssistConfig; timeout?: string }) {
  const raw =
...
async function agentViaGatewayCommand(
  ...
  if (!opts.to && !opts.sessionId && !opts.agent && !explicitSessionKey) {
    throw new Error(
      `No target session selected. Use --agent <id>, --session-key <key>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("nodoassist agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("nodoassist agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  const dispatchGatewayAgentCall = async (activeCfg: NodoAssistConfig) =>
    await withProgress(
      {
...
export async function agentCliCommand(
  ...
  if (isCompactControlCommand(messageOpts.message)) {
    runtime.error?.(
      "Slash commands cannot be executed via --message from the CLI. Use: nodoassist sessions compact <key>",
    );
    runtime.exit(1);
    return undefined;
  }
