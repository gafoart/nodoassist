...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "NODOASSIST_GATEWAY_TOKEN",
          placeholder: "NODOASSIST_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = normalizeOptionalString(value) ?? "";
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like NODOASSIST_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. NodoAssist will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: NodoAssist trusts user identity from a reverse proxy.",
