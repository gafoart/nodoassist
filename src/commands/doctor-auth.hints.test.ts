...
function doctorFixtureConfig(config: unknown): NodoAssistConfig {
...
describe("resolveUnusableProfileHint", () => {
  ...
  it("formats permanent OAuth refresh failures as reauth-required", () => {
    expect(
      ...
      "- openai-codex:default: re-auth required [refresh_token_reused] — Run `nodoassist models auth login --provider openai`.",
    );
  });
  ...
  it("formats non-permanent OAuth refresh failures as retry-then-reauth guidance", () => {
    expect(
      ...
      "- openai-codex:default: OAuth refresh failed — Try again; if this persists, run `nodoassist models auth login --provider openai`.",
    );
  });
  ...
  it("quotes exact current profile ids in OAuth reauth guidance", () => {
    expect(
      ...
      "- OpenAI Work Profile: re-auth required [invalid_grant] — Run `nodoassist models auth login --provider openai --profile-id 'OpenAI Work Profile'`.",
    );
  });
  ...
  it("drops the provider-specific command when the parsed provider is unsafe", () => {
    expect(
      ...
      "- openai-codex:default: re-auth required [invalid_grant] — Run `nodoassist models auth login --provider openai`.",
    );
  });
