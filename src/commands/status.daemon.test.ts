...
describe("status daemon summary", () => {
  it("preserves service layout diagnostics for status output", async () => {
    mocks.readServiceStatusSummary.mockResolvedValueOnce({
      ...
      installed: true,
      loaded: true,
      managedByNodoAssist: true,
      externallyManaged: false,
      loadedText: "enabled",
      runtime: { status: "running", pid: 1234 },
      layout: {
        execStart: "/usr/bin/node /opt/nodoassist/dist/entry.js gateway",
        sourceScope: "system",
    ...
    expect(summary.layout?.execStart).toBe("/usr/bin/node /opt/nodoassist/dist/entry.js gateway");
