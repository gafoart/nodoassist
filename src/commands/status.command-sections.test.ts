...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:nodoassist security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:nodoassist security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:nodoassist status --all",
      "  Need to debug live? cmd:nodoassist logs --follow",
      "  Fix reachability first: cmd:nodoassist gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:nodoassist devices approve req-123)",
      "muted(Fallback: cmd:nodoassist devices approve --latest)",
      "muted(Inspect: cmd:nodoassist devices list)",
    ]);
  });
