...
const resolveNodoAssistPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadNodoAssistPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/nodoassist.json",
    createConfigIO,
...
vi.mock("nodoassist/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("nodoassist/plugin-sdk/provider-stream-shared", () => ({
...
beforeEach(() => {
  ...
  resolveNodoAssistPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.NODOASSIST_STATE_DIR;
  originalUpdateInProgress = process.env.NODOASSIST_UPDATE_IN_PROGRESS;
  process.env.NODOASSIST_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-doctor-state-"));
  process.env.NODOASSIST_STATE_DIR = tempStateDir;
