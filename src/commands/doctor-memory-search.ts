...
function hasConfiguredAwsSdkAuthForProvider(provider: string, cfg: NodoAssistConfig): boolean {
...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: NodoAssistConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: NodoAssistConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("nodoassist doctor --fix")} or ${formatCliCommand("nodoassist memory status --fix")}`
    : `Verify: ${formatCliCommand("nodoassist memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: NodoAssistConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: NodoAssistConfig): boolean {
 ...
 * Runs as part of `nodoassist doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
        ...
        noteFn(
          [
            ...
            workspaceProbeFailed
              ...
              : `- Set an explicit binary path: ${formatCliCommand("nodoassist config set memory.qmd.command /absolute/path/to/qmd")}`,
            `- Or switch back to builtin memory: ${formatCliCommand("nodoassist config set memory.backend builtin")}`,
            "",
            `Verify: ${formatCliCommand("nodoassist memory status --deep")}`,
          ]
            .filter(Boolean)
      ...
      noteFn(
        [
          ...
          `- Enable QMD session export: ${formatCliCommand(
            "nodoassist config set memory.qmd.sessions.enabled true",
          )}`,
  ...
  if (provider === "local") {
    ...
    noteFn(
      [
        ...
        "",
        "Fix (pick one):",
        `- Install the llama.cpp provider plugin: ${formatCliCommand("nodoassist plugins install @nodoassist/llama-cpp-provider")}`,
        `- Set a local GGUF model path in config`,
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`nodoassist config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    noteFn(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("nodoassist config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("nodoassist memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    noteFn(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("nodoassist config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("nodoassist memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `nodoassist doctor` without
  ...
  noteFn(
    [
      ...
      `- Configure credentials: ${formatCliCommand("nodoassist configure --section model")}`,
      `- To disable: ${formatCliCommand("nodoassist config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("nodoassist memory status --deep")}`,
    ].join("\n"),
    "Memory search",
