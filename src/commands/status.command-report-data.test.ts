...
describe("buildStatusCommandReportData", () => {
  it("builds report inputs from shared status surfaces", async () => {
    ...
    expect(result.taskMaintenanceHint).toBe(
      "Task maintenance: cmd:nodoassist tasks maintenance --apply",
    );
    ...
    expect(result.footerLines.at(-1)).toBe("  Need to test channels? cmd:nodoassist status --deep");
  });
  ...
  it("shows skipped audit text when fast status omits the security audit", async () => {
    ...
    expect(result.securityAuditLines).toEqual([
      "muted(Skipped in fast status. Full report: cmd:nodoassist security audit)",
      "muted(Deep probe: cmd:nodoassist status --deep)",
    ]);
  });
