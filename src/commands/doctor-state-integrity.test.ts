...
function setupSessionState(cfg: NodoAssistConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("NODOASSIST_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: NodoAssistConfig) {
...
async function runStateIntegrityText(cfg: NodoAssistConfig): Promise<string> {
...
describe("structured state integrity findings", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "NODOASSIST_HOME", "NODOASSIST_STATE_DIR"]);
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-doctor-state-integrity-"));
    setTestEnvValue("HOME", tempHome);
    setTestEnvValue("NODOASSIST_HOME", tempHome);
  ...
  it("maps a missing state directory to a structured finding and dry-run effect", () => {
    ...
    expect(issue).toEqual({
      ...
      path: path.join(tempHome, ".nodoassist"),
    });
    ...
    expect(stateIntegrityIssueToRepairEffect(issue)).toEqual({
      ...
      target: path.join(tempHome, ".nodoassist"),
      dryRunSafe: false,
    });
  });
  ...
  it("reports permissive state and config file permissions as structured findings", () => {
    ...
    const configPath = path.join(tempHome, "nodoassist.json");
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    deleteTestEnvValue("NODOASSIST_OAUTH_DIR");
  ...
  it("prompts for oauth dir when NODOASSIST_OAUTH_DIR is explicitly configured", async () => {
    process.env.NODOASSIST_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: NodoAssistConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(hasRepairPromptMessage(confirmRuntimeRepair, "Create OAuth dir at")).toBe(true);
  ...
  it("does not warn when NODOASSIST_AGENT_DIR points at the live compatibility agent dir", async () => {
    createAgentDir("legacy");
    const legacyAgentDir = path.join(
      process.env.NODOASSIST_STATE_DIR ?? "",
      "agents",
      "legacy",
      "agent",
    );
    setTestEnvValue("NODOASSIST_AGENT_DIR", legacyAgentDir);
  ...
  it("warns about tombstoned subagent restart recovery sessions", async () => {
    ...
    expect(text).toContain("nodoassist tasks maintenance --apply");
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.NODOASSIST_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        const symlinkStateDir = path.join(symlinkHome, ".nodoassist");
        setTestEnvValue("HOME", symlinkHome);
        setTestEnvValue("NODOASSIST_HOME", symlinkHome);
        setTestEnvValue("NODOASSIST_STATE_DIR", symlinkStateDir);
  ...
  it("prints nodoassist-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: NodoAssistConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/nodoassist sessions --store ".*sessions\.json"/);
    expect(text).toMatch(
      /nodoassist sessions cleanup --store ".*sessions\.json" --dry-run --fix-missing/,
    );
    expect(text).not.toMatch(
      /nodoassist sessions cleanup --store ".*sessions\.json" --dry-run(?! --fix-missing)/,
    );
    expect(text).toMatch(
      /nodoassist sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
  ...
  it("does not let synthetic heartbeat metadata override mixed transcript history", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-heartbeat-main-mixed-"));
    try {
  ...
  it("does not let heartbeat-looking routing metadata skip mixed transcript checks", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-heartbeat-main-route-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("does not classify transcripts with real user activity after 400 heartbeat messages", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-heartbeat-main-cap-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
  ...
  it("keeps the heartbeat main-session helper conservative", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-heartbeat-main-helper-"));
    try {
      const transcriptPath = path.join(tempDir, "session.jsonl");
      fs.writeFileSync(
        transcriptPath,
  ...
  it("moves store entries and clears matching TUI pointers without touching others", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-tui-pointer-clear-"));
    try {
