...
const originalUpdateInProgress = process.env.NODOASSIST_UPDATE_IN_PROGRESS;
const originalParentSupportsConfigWrite =
  process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE;
const originalParentSupportsGatewayRestart =
  process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART;
const originalParentAllowsGatewayServiceRepair =
  process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR;
const originalParentAllowsGatewayActivation =
  process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION;
...
async function runRepair(cfg: NodoAssistConfig, options: { allowExecSecretRefs?: boolean } = {}) {
...
async function runNonInteractiveRepair(params: {
  ...
  if (params.updateInProgress) {
    process.env.NODOASSIST_UPDATE_IN_PROGRESS = "1";
  } else {
    delete process.env.NODOASSIST_UPDATE_IN_PROGRESS;
  }
    ...
    issues: [
      {
        ...
        message: "Gateway service NODOASSIST_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    ...
    mocks.readWindowsProcessArgsSync.mockReturnValue(["node", "nodoassist.mjs", "update"]);
    mocks.resolveGatewayAuthTokenForService.mockImplementation(
      async (cfg: NodoAssistConfig, env) => {
        const configToken =
          typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
        const envToken = env.NODOASSIST_GATEWAY_TOKEN?.trim() || undefined;
  ...
  afterEach(() => {
    ...
    if (originalUpdateInProgress === undefined) {
      ...
      process.env.NODOASSIST_UPDATE_IN_PROGRESS = originalUpdateInProgress;
    }
    if (originalParentSupportsConfigWrite === undefined) {
      delete process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE;
    } else {
      process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE =
        originalParentSupportsConfigWrite;
    }
    if (originalParentSupportsGatewayRestart === undefined) {
      delete process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART;
    } else {
      process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART =
        originalParentSupportsGatewayRestart;
    }
    if (originalParentAllowsGatewayServiceRepair === undefined) {
      delete process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR;
    } else {
      process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR =
        originalParentAllowsGatewayServiceRepair;
    }
    if (originalParentAllowsGatewayActivation === undefined) {
      delete process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION;
    } else {
      process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION =
  ...
  it("does not duplicate gateway runtime warnings already emitted by the node install plan", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: [nvmNode, "/usr/local/bin/nodoassist", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("passes planned managed env keys into service audit for legacy inline secret detection", async () => {
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...
      workingDirectory: "/tmp",
      environment: {
        NODOASSIST_SERVICE_MANAGED_ENV_KEYS: "TAVILY_API_KEY",
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
  ...
  it("uses NODOASSIST_GATEWAY_TOKEN when config token is missing", async () => {
  ...
  it("does not flag entrypoint mismatch when symlink and realpath match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/nodoassist/dist/index.js",
      installEntrypoint:
        "/Users/test/Library/pnpm/global/5/node_modules/.pnpm/nodoassist@2026.3.12/node_modules/nodoassist/dist/index.js",
      realpath: async (value: string) => {
        const normalized = value.replaceAll("\\", "/").replace(/^[A-Z]:/i, "");
        if (normalized.includes("/global/5/node_modules/nodoassist/")) {
          return normalized.replace(
            "/global/5/node_modules/nodoassist/",
            "/global/5/node_modules/.pnpm/nodoassist@2026.3.12/node_modules/nodoassist/",
          );
        }
        return normalized;
      },
  ...
  it("does not flag entrypoint mismatch when realpath fails but normalized absolute paths match", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/opt/nodoassist/../nodoassist/dist/index.js",
      installEntrypoint: "/opt/nodoassist/dist/index.js",
  ...
  it("keeps wrapper-managed gateway services aligned during entrypoint drift checks", async () => {
    const wrapperPath = "/usr/local/bin/nodoassist-doppler";
    mocks.readCommand.mockResolvedValue({
      programArguments: [wrapperPath, "gateway", "--port", "18789"],
      environment: {
        NODOASSIST_WRAPPER: wrapperPath,
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: true,
    ...
    mocks.buildGatewayInstallPlan.mockImplementation(async ({ env }) => ({
      programArguments: [env.NODOASSIST_WRAPPER, "gateway", "--port", "18789"],
      environment: {
        NODOASSIST_WRAPPER: env.NODOASSIST_WRAPPER,
      },
    }));
    ...
    expect(requireRecord(installPlanOptions.env, "install env").NODOASSIST_WRAPPER).toBe(
      wrapperPath,
    );
    expect(
    ...
    expect(mocks.note).toHaveBeenCalledWith(
      "Gateway service invokes NODOASSIST_WRAPPER: /usr/local/bin/nodoassist-doppler",
      "Gateway",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("still flags entrypoint mismatch when canonicalized paths differ", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint:
        "/Users/test/.nvm/versions/node/v22.0.0/lib/node_modules/nodoassist/dist/index.js",
      installEntrypoint: "/Users/test/Library/pnpm/global/5/node_modules/nodoassist/dist/index.js",
    });
  ...
  it("skips entrypoint rewrites for an active systemd unit", async () => {
    ...
    mocks.readCommand.mockResolvedValue({
      ...createGatewayCommand("/opt/old-nodoassist/dist/index.js"),
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...createGatewayCommand("/opt/new-nodoassist/dist/index.js"),
      workingDirectory: "/tmp",
    });
  ...
  it("leaves all service metadata unchanged when an active unit has command drift plus other issues", async () => {
    mockProcessPlatform("linux");
    mocks.readCommand.mockResolvedValue({
      programArguments: ["/usr/bin/nodoassist", "run"],
      environment: {},
      sourcePath: "/home/test/.config/systemd/user/nodoassist-gateway.service",
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
  ...
  it("skips entrypoint rewrite in non-interactive fix mode", async () => {
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/nodoassist/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/nodoassist/dist/index.js",
    ...
    expectNoteContaining("nodoassist gateway install --force", "Gateway service config");
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  });
  ...
  it("defers systemd service config rewrites during non-interactive update repairs", async () => {
    mockProcessPlatform("linux");
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    setupGatewayEntrypointRepairScenario({
      currentEntrypoint: "/Users/test/Library/npm/node_modules/nodoassist/dist/entry.js",
      installEntrypoint: "/Users/test/Library/npm/node_modules/nodoassist/dist/index.js",
      installWorkingDirectory: "/tmp",
  ...
  it("does not persist or stage embedded service tokens during systemd update repairs", async () => {
    ...
    process.env.NODOASSIST_UPDATE_IN_PROGRESS = "1";
    process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
  ...
  it.each([
    ["update command", ["node", "nodoassist.mjs", "update"]],
    ["--update shorthand", ["node", "nodoassist.mjs", "--update"]],
    ["doctor update prompt", ["node", "nodoassist.mjs", "doctor"]],
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
        NODOASSIST_SERVICE_VERSION: "2026.5.25",
        NODOASSIST_WINDOWS_TASK_NAME: "NodoAssist Gateway Work",
      },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
        {
          ...
          message: "Gateway service was installed by an older NodoAssist version.",
          level: "recommended",
        },
      ],
    });
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      ...
      workingDirectory: "/tmp",
      environment: {
        NODOASSIST_SERVICE_VERSION: "2026.5.26",
      },
    });
    ...
    expectNoteContaining(
      "Gateway service was installed by an older NodoAssist version.",
      "Gateway service config",
    );
    expect(mocks.stage).not.toHaveBeenCalled();
    expect(mocks.install).not.toHaveBeenCalled();
  ...
  it("stages running Windows repairs when the update parent forbids activation", async () => {
    ...
    process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "0";
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
        NODOASSIST_SERVICE_VERSION: "2026.5.25",
  ...
  it("does not rewrite a service when the update parent rejects ownership", async () => {
    mockProcessPlatform("win32");
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "0";
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "0";
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: {
  ...
  it.each([
    {
      ...
      args: ["node", "nodoassist.mjs", "update", "--no-restart"],
    },
    {
    ...
    },
    {
      ...
      args: ["node", "nodoassist.mjs", "update", "wizard"],
    },
    {
    ...
    },
    {
      ...
      args: ["node", "nodoassist.mjs", "gateway"],
    },
    ...
    mocks.readCommand.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      environment: { NODOASSIST_SERVICE_VERSION: "2026.5.25" },
    });
    mocks.auditGatewayServiceConfig.mockResolvedValue({
      ok: false,
      issues: [
    ...
    mocks.buildGatewayInstallPlan.mockResolvedValue({
      programArguments: gatewayProgramArguments,
      workingDirectory: "/tmp",
      environment: { NODOASSIST_SERVICE_VERSION: "2026.5.26" },
    });
    mocks.readRuntime.mockResolvedValue({ status: "running" });
  ...
  it("persists embedded service tokens before Windows update repairs rewrite the task", async () => {
    ...
    process.env.NODOASSIST_UPDATE_IN_PROGRESS = "1";
    process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE = "1";
    process.env.NODOASSIST_UPDATE_PARENT_SUPPORTS_GATEWAY_RESTART = "1";
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR = "1";
    process.env.NODOASSIST_UPDATE_PARENT_ALLOWS_GATEWAY_ACTIVATION = "1";
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            NODOASSIST_GATEWAY_TOKEN: "env-file-token",
          },
  ...
  it("reports service config drift but skips service rewrite when service repair policy is external", async () => {
    await withEnvAsync({ NODOASSIST_SERVICE_REPAIR_POLICY: "external" }, async () => {
      setupGatewayEntrypointRepairScenario({
        currentEntrypoint: "/Users/test/Library/npm/node_modules/nodoassist/dist/entry.js",
        installEntrypoint: "/Users/test/Library/npm/node_modules/nodoassist/dist/index.js",
        installWorkingDirectory: "/tmp",
  ...
  it("warns when the gateway service entrypoint resolves to a source checkout", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-doctor-service-layout-"));
      try {
  ...
  it("does not duplicate Gateway service config panels for a source-checkout entrypoint with audit findings", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "nodoassist-doctor-service-config-dedup-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        const installEntrypoint = "/usr/local/lib/node_modules/nodoassist/dist/index.js";
        setupGatewayEntrypointRepairScenario({
        ...
        const forceMatches = consolidated.match(/nodoassist gateway install --force/g) ?? [];
  ...
  it("keeps the gateway install force hint when a source-checkout warning is suppressed and repair is declined", async () => {
    await withEnvAsync({}, async () => {
      const root = await fs.mkdtemp(
        path.join(os.tmpdir(), "nodoassist-doctor-service-config-force-hint-"),
      );
      try {
        await fs.mkdir(path.join(root, ".git"), { recursive: true });
        await fs.mkdir(path.join(root, "src"), { recursive: true });
        ...
        expect(gatewayServiceConfigNotes[1]?.[0]).toContain("nodoassist gateway install --force");
      } finally {
        await fs.rm(root, { recursive: true, force: true });
      }
    });
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("keeps legacy gateway services warning-level", () => {
    expect(
      extraGatewayServiceToHealthFinding({
        platform: "linux",
        label: "nodoassist-gateway.service",
        detail: "legacy unit",
        scope: "user",
        legacy: true,
      }),
      ...
      expect.objectContaining({
        ...
        severity: "warning",
        source: "linux",
        target: "nodoassist-gateway.service",
      }),
    );
  });
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing NodoAssist gateway next.",
    );
  });
