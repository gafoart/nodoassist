...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    promptRemoteGatewayConfig: vi.fn(async (cfg: NodoAssistConfig) => ({
      ...cfg,
      gateway: { mode: "remote", remote: { url: "wss://gateway.example.test" } },
    })),
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: NodoAssistConfig }) =>
      Boolean(config.auth?.profiles?.["openai:default"]),
    ),
    setupChannels: vi.fn(async (cfg: NodoAssistConfig) => cfg),
  };
});
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.nodoassist/nodoassist.json",
  createConfigIO: () => ({
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.nodoassist/workspace",
  applyWizardMetadata: (cfg: NodoAssistConfig) => cfg,
...
function setupBaseWizardState(config: NodoAssistConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: NodoAssistConfig) => cfg);
    mocks.promptAuthConfig.mockReset();
    mocks.promptAuthConfig.mockImplementation(async (cfg: NodoAssistConfig) => cfg);
    mocks.promptGatewayConfig.mockReset();
    mocks.promptGatewayConfig.mockImplementation(async (cfg: NodoAssistConfig) => ({
      config: cfg,
      port: 18789,
    }));
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: NodoAssistConfig) => {
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(String(defaults.workspace)).toContain("/.nodoassist/workspace");
