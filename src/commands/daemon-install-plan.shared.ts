...
function isNodoAssistCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "nodoassist") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "nodoassist.cmd" ||
      basename === "nodoassist.ps1" ||
      basename === "nodoassist.exe"
    );
  }
  return false;
}
...
/** Resolve the NodoAssist CLI binary directory from argv/PATH for daemon PATH. */
export function resolveDaemonNodoAssistBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isNodoAssistCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "nodoassist.cmd" : "nodoassist");
...
/** Merge Node and NodoAssist binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonNodoAssistBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}
