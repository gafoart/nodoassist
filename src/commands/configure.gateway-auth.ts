...
import type { NodoAssistConfig, GatewayAuthConfig } from "../config/config.js";
...
function hasConfiguredProviderModels(cfg: NodoAssistConfig, provider: string | undefined): boolean {
  if (!provider) {
    return false;
  }
...
function listConfiguredModelProviders(cfg: NodoAssistConfig): string[] {
  return Object.entries(cfg.models?.providers ?? {})
    .filter(([, provider]) => (provider.models?.length ?? 0) > 0)
    .map(([provider]) => provider);
}

function resolveSingleConfiguredProvider(cfg: NodoAssistConfig): string | undefined {
...
function resolveConfiguredProviderFromAuthChange(params: {
  before: NodoAssistConfig;
  after: NodoAssistConfig;
...
export function buildGatewayAuthConfig(params: {
  ...
  if (params.mode === "trusted-proxy") {
    if (!params.trustedProxy) {
      throw new Error(
        `trustedProxy config is required when mode is trusted-proxy. Run ${formatCliCommand("nodoassist configure --section gateway")} to configure Gateway auth interactively.`,
      );
    }
