// Builds the data model for the standard `nodoassist status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("nodoassist security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("nodoassist status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("nodoassist tasks maintenance --apply")}`,
