...
export async function requireValidConfigFileSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`NodoAssist config is invalid: ${snapshot.path}\n${issues}`);
    runtime.error(
    ...
    runtime.error(`Inspect: ${formatCliCommand("nodoassist config validate")}`);
    runtime.exit(1);
    return null;
  }
...
/** Read and return a valid NodoAssist config, or null after reporting validation errors. */
