...
const channelWizardMocks = vi.hoisted(() => {
  ...
  return {
    prompter,
    setupChannels: vi.fn(async (...args: unknown[]) => args[0] as NodoAssistConfig),
  };
});
    ...
    await channelsAddCommand(
      {
        channel: "whatsapp",
        account: "work",
        authDir: "/tmp/nodoassist-wa-auth",
      },
      runtime,
      { hasFlags: true },
    );
