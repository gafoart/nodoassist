...
const states: NodoAssistTestState[] = [];
...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<NodoAssistTestState> {
  const state = await createNodoAssistTestState({
    layout: "state-only",
    prefix: "nodoassist-doctor-oauth-sidecar-",
    env: {
      NODOASSIST_AGENT_DIR: undefined,
      NODOASSIST_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "nodoassist-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans NODOASSIST_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
    const previousAgentDir = process.env.NODOASSIST_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.NODOASSIST_AGENT_DIR = previousAgentDir;
      }
    }
  });
