...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("nodoassist/plugin-sdk/test-node-mocks");
...
describe("doctor WhatsApp responsiveness", () => {
  ...
  it("lists only verified local TUI processes", () => {
    spawnSyncMock.mockReturnValue({
      ...
      stdout: [
        " 101 nodoassist-tui",
        " 102 /usr/bin/node /usr/lib/node_modules/nodoassist/dist/index.js gateway --port 18789",
        " 103 nodoassist channels",
        " 104 nodoassist tui --local",
        " 105 /usr/bin/nodoassist chat",
        " 106 helper --note 'nodoassist tui'",
        " 107 nodoassist-helper nodoassist terminal",
        " 108 nodoassist --flag tui",
      ].join("\n"),
    });
    ...
    if (process.platform === "win32") {
      ...
      expect(listLocalTuiProcesses()).toEqual([
        { pid: 101, command: "nodoassist-tui" },
        { pid: 104, command: "nodoassist tui --local" },
        { pid: 105, command: "/usr/bin/nodoassist chat" },
      ]);
    }
  });
  ...
  it("terminates stale local TUI processes with a kill fallback", async () => {
    ...
    await expect(
      terminateLocalTuiProcesses({
        processes: [{ pid: 101, command: "nodoassist-tui" }],
        controller,
        graceMs: 0,
      }),
  ...
  it("warns and repairs local TUI pressure when WhatsApp is enabled and the gateway is degraded", async () => {
    ...
    const cfg = { channels: { whatsapp: { enabled: true } } } as NodoAssistConfig;
    ...
    await noteWhatsappResponsivenessHealth({
      ...
      shouldRepair: true,
      listLocalTuiProcesses: () => [{ pid: 101, command: "nodoassist-tui" }],
  ...
  it("keeps WhatsApp responsiveness findings quiet without the exact pressure signal", () => {
    ...
    expect(
      collectWhatsappResponsivenessHealthFindings({
        cfg: { channels: { whatsapp: { enabled: false } } } as NodoAssistConfig,
        status: {
          eventLoop: {
            degraded: true,
            reasons: ["event_loop_delay"],
