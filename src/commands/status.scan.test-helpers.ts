...
type StatusScanSharedMocks = {
  ...
  resolveGatewayPort: Mock<(cfg?: NodoAssistConfig) => number>;
...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(
      () => `/tmp/nodoassist-${configPathLabel}-missing-${process.pid}.json`,
    ),
    resolveGatewayPort: vi.fn((cfg?: NodoAssistConfig) => cfg?.gateway?.port ?? 18789),
...
export function createStatusScanConfig<T extends object = NodoAssistConfig>(
  overrides: T = {} as T,
): NodoAssistConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): NodoAssistConfig {
