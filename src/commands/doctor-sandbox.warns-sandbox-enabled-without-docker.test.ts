...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): NodoAssistConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
  ...
  function createSandboxConfigWithDockerNetwork(network: string): NodoAssistConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/nodoassist/sandbox/containers.json",
        shardedDir: "/tmp/nodoassist/sandbox/containers",
        source: "monolithic",
        exists: true,
        valid: true,
        entries: 2,
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers monolithic: /tmp/nodoassist/sandbox/containers.json (2 entries)",
        "Run nodoassist doctor --fix to migrate them to SQLite.",
      ].join("\n"),
      "Sandbox",
    );
  });
  ...
  it("maps legacy registry files to structured findings and dry-run effects", () => {
    ...
    expect(legacySandboxRegistryInspectionToHealthFinding(monolithicFile)).toEqual(
      expect.objectContaining({
        ...
        severity: "warning",
        path: "/tmp/nodoassist/sandbox/containers.json",
        fixHint: expect.stringContaining("nodoassist doctor --fix"),
      }),
    );
    expect(legacySandboxRegistryInspectionToRepairEffect(monolithicFile)).toEqual({
      kind: "state",
      action: "would-migrate-legacy-sandbox-registry",
      target: "/tmp/nodoassist/sandbox/containers.json",
      dryRunSafe: false,
    });
    expect(legacySandboxRegistryInspectionToHealthFinding(shardedFile)).toEqual(
      expect.objectContaining({
        path: "/tmp/nodoassist/sandbox/containers",
        message: expect.stringContaining(
          "- containers sharded: /tmp/nodoassist/sandbox/containers (2 entries)",
        ),
      }),
    );
    expect(legacySandboxRegistryInspectionToRepairEffect(shardedFile)).toEqual(
      expect.objectContaining({
        target: "/tmp/nodoassist/sandbox/containers",
      }),
    );
  });
  ...
  it("maps invalid legacy registry files to quarantine effects", () => {
    expect(
      legacySandboxRegistryInspectionToRepairEffect({
        kind: "browsers",
        registryPath: "/tmp/nodoassist/sandbox/browsers.json",
        shardedDir: "/tmp/nodoassist/sandbox/browsers",
        source: "monolithic",
        exists: true,
        valid: false,
        entries: 0,
      ...
      expect.objectContaining({
        ...
        target: "/tmp/nodoassist/sandbox/browsers.json",
      }),
    );
  });
