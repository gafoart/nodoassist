...
const testConfigStore = new Map<string, NodoAssistConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.NODOASSIST_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.NODOASSIST_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("NODOASSIST_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "nodoassist.json");
}
...
function readTestConfig<T = NodoAssistConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  ...
  resolveGatewayPort: (cfg: NodoAssistConfig) => cfg.gateway?.port ?? 18789,
}));
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/nodoassist-workspace",
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "nodoassist"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    setTestEnvValue("NODOASSIST_SKIP_CHANNELS", "1");
    setTestEnvValue("NODOASSIST_SKIP_GMAIL_WATCHER", "1");
    setTestEnvValue("NODOASSIST_SKIP_CRON", "1");
    setTestEnvValue("NODOASSIST_SKIP_CANVAS_HOST", "1");
    setTestEnvValue("NODOASSIST_SKIP_BROWSER_CONTROL_SERVER", "1");
    deleteTestEnvValue("NODOASSIST_GATEWAY_TOKEN");
    deleteTestEnvValue("NODOASSIST_GATEWAY_PASSWORD");

    tempHome = await makeTempWorkspace("nodoassist-onboard-");
  ...
  it("preserves existing agents.list and bindings on onboard rerun (nodoassist#84692)", async () => {
    await withStateDir("state-preserve-agents-", async (stateDir) => {
      const workspace = path.join(stateDir, "nodoassist");
      const seededAgents = [
  ...
  it("preserves existing agents.list and bindings on remote onboard rerun (nodoassist#84692)", async () => {
  ...
  it("explains local health failure when no daemon was requested", async () => {
    await withStateDir("state-local-health-hint-", async (stateDir) => {
      ...
      await expect(
        ...
        /only waits for an already-running gateway unless you pass `--install-daemon` to `nodoassist onboard`[\s\S]*nodoassist onboard --install-daemon[\s\S]*nodoassist onboard --skip-health/,
      );
    });
  }, 60_000);
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `nodoassist gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `nodoassist gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      setTestEnvValue("NODOASSIST_STATE_DIR", stateDir);
      setTestEnvValue("NODOASSIST_CONFIG_PATH", path.join(stateDir, "nodoassist.json"));
