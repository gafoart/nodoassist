...
describe("status update restart formatting", () => {
  it("surfaces failed update restarts with deep status guidance", () => {
    ...
    expect(value).toBe(
      "warn(failed · managed-service-handoff-failed · run nodoassist gateway status --deep · 60000ms)",
    );
  });
  ...
  it("labels handoff and health-pending sentinels as restart pending", () => {
    ...
    ).toBe("handoff running · gateway restart pending · run nodoassist update status");
    expect(
      formatUpdateRestartStatusValue({
        ...basePayload,
        stats: { ...basePayload.stats, reason: "restart-health-pending" },
      }),
    ).toBe("restart pending health verification · run nodoassist gateway status --deep");
  });
  ...
  it("adds action lines for failed and pending update restarts only", () => {
    ...
    ).toContain("Update restart failed; run nodoassist gateway status --deep.");
    expect(
      formatUpdateRestartActionLines({
        ...basePayload,
        stats: { ...basePayload.stats, reason: "restart-health-pending" },
