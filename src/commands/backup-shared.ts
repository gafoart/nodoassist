...
export function buildBackupArchiveRoot(nowMs = Date.now()): string {
  return `${formatBackupArchiveTimestamp(nowMs)}-nodoassist-backup`;
}
...
/** Resolve the backup plan from the current NodoAssist state/config/workspace paths on disk. */
export async function resolveBackupPlanFromDisk(
  ...
  if (includeWorkspace && configSnapshot.exists && !configSnapshot.valid) {
    throw new Error(
      `Config invalid at ${shortenHomePath(configSnapshot.path)}. NodoAssist cannot reliably discover custom workspaces for backup. Fix the config or rerun with --no-include-workspace for a partial backup.`,
    );
  }
