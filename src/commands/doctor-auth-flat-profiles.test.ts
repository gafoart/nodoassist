...
async function makeTestState(): Promise<NodoAssistTestState> {
  const state = await createNodoAssistTestState({
    layout: "state-only",
    prefix: "nodoassist-doctor-flat-auth-",
    env: {
...
describe("maybeMigrateAuthProfileJsonStoresToSqlite", () => {
  ...
  it("infers config credential provider and mode before stripping config", async () => {
    const cases: Array<{ profileId: string; cfg: NodoAssistConfig; now: number }> = [
      {
...
describe("maybeRepairOpenAICodexAuthConfig", () => {
  it("renames legacy OpenAI Codex config profiles and merges auth order", () => {
    ...
    const migrated = result.config as NodoAssistConfig & {
      agents?: {
        defaults?: {
