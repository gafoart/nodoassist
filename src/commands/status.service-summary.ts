...
export async function readServiceStatusSummary(
  ...
  try {
    ...
    const managedByNodoAssist = state.installed;
    // A running unmanaged process still counts as installed for status display.
    const externallyManaged = !managedByNodoAssist && state.running;
    const installed = managedByNodoAssist || externallyManaged;
