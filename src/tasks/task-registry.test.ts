...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronJobsStorePath: () => "/tmp/nodoassist-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/nodoassist-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "nodoassist-task-registry-" }, async (root) => {
    return await withEnvAsync({ NODOASSIST_STATE_DIR: root }, async () => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through NodoAssist subagent sessions", async () => {
    await withTaskRegistryTempDir(async () => {
      resetTaskRegistryForTests();
      const task = createTaskRecord({
        runtime: "subagent",
  ...
  it("cancels childless copilot-native tasks without routing through NodoAssist subagent sessions", async () => {
    await withTaskRegistryTempDir(async () => {
      resetTaskRegistryForTests();
      const task = createTaskRecord({
        runtime: "subagent",
