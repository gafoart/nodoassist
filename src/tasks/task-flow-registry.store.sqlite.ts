// Persists managed task-flow records through the NodoAssist SQLite state database.
...
type FlowRunsTable = NodoAssistStateKyselyDatabase["flow_runs"];
type FlowRegistryStoreDatabase = Pick<NodoAssistStateKyselyDatabase, "flow_runs">;
...
// SQLite-backed task-flow store mirrors the in-process registry into nodoassist-state.db.
...
function pruneFlowsNotInSnapshot(params: { db: DatabaseSync; ids: readonly string[] }) {
  const tempTableName = "nodoassist_live_flow_ids";
