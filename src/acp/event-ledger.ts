...
import { resolveIntegerOption } from "@nodoassist/acp-core/numeric-options";
...
/** Resolves the legacy file-backed ACP ledger path under the NodoAssist state directory. */
...
export async function migrateFileAcpEventLedgerToSqlite(
  params: { filePath: string; archiveSource?: boolean } & NodoAssistStateDatabaseOptions,
...
export function createSqliteAcpEventLedger(
  params: NodoAssistStateDatabaseOptions & LedgerOptions = {},
): AcpEventLedger {
  ...
  const mutate = (fn: (db: DatabaseSync) => void) =>
    runNodoAssistStateWriteTransaction((database) => fn(database.db), dbOptions);
  const read = <T>(fn: (db: DatabaseSync) => T): T => fn(openNodoAssistStateDatabase(dbOptions).db);
