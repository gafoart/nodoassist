...
import { AcpRuntimeError } from "@nodoassist/acp-core/runtime/errors";
...
export function isAcpEnabledByPolicy(cfg: NodoAssistConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: NodoAssistConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: NodoAssistConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: NodoAssistConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: NodoAssistConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: NodoAssistConfig, agentId: string): boolean {
