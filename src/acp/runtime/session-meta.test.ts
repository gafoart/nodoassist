...
describe("ACP session metadata SQLite store", () => {
  ...
  it("persists ACP metadata in SQLite without writing sessions.json acp blocks", async () => {
    await withTempDir({ prefix: "nodoassist-acp-meta-" }, async (dir) => {
      const storePath = path.join(dir, "sessions.json");
      const databasePath = path.join(dir, "state", "nodoassist.sqlite");
  ...
  it("honors NODOASSIST_STATE_DIR when joining listed SQLite rows to session stores", async () => {
    await withTempDir({ prefix: "nodoassist-acp-meta-" }, async (dir) => {
      const env = { ...process.env, NODOASSIST_STATE_DIR: dir } as NodeJS.ProcessEnv;
