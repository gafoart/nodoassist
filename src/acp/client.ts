/** Interactive stdio ACP client used to connect a terminal session to an NodoAssist ACP server. */
...
async function createAcpClient(opts: AcpClientOptions = {}): Promise<AcpClientHandle> {
  ...
  const defaultServerCommand = entryPath ? process.execPath : "nodoassist";
  ...
  await client.initialize({
    ...
    clientInfo: { name: "nodoassist-acp-client", version: "1.0.0" },
  });
...
export async function runAcpClientInteractive(opts: AcpClientOptions = {}): Promise<void> {
  ...
  console.log("NodoAssist ACP client");
