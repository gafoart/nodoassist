...
const mockState = vi.hoisted(() => ({
  ...
  closeNodoAssistStateDatabase: vi.fn(),
...
vi.mock("../state/nodoassist-state-db.js", () => ({
  closeNodoAssistStateDatabase: () => mockState.closeNodoAssistStateDatabase(),
}));
...
describe("serveAcpGateway startup", () => {
  ...
  beforeEach(async () => {
    ...
    mockState.closeNodoAssistStateDatabase.mockReset();
  ...
  it.each([
    {
      ...
      opts: {},
      expected: ["nodoassist acp: gateway event chat failed\n"],
    },
    {
      name: "verbose logging",
      opts: { verbose: true },
      expected: [
        "nodoassist acp: gateway event chat failed\n",
        "nodoassist acp: gateway event chat error: Error: handler boom\n",
      ],
    },
  ...
  it("closes the shared state database on shutdown", async () => {
    const { signalHandlers, onceSpy } = captureProcessSignalHandlers();
    expect(mockState.closeNodoAssistStateDatabase).not.toHaveBeenCalled();
    ...
    try {
      ...
      expect(mockState.closeNodoAssistStateDatabase).toHaveBeenCalledOnce();
    } finally {
      onceSpy.mockRestore();
    }
  });
  ...
  it("closes a real node:sqlite DatabaseSync handle through serveAcpGateway shutdown", async () => {
    ...
    // the closeNodoAssistStateDatabase helper in isolation.
    const actualStateDb = await vi.importActual<typeof import("../state/nodoassist-state-db.js")>(
      "../state/nodoassist-state-db.js",
    );

    const realDb = actualStateDb.openNodoAssistStateDatabase();
    expect(realDb.db.isOpen).toBe(true);
    expect(actualStateDb.isNodoAssistStateDatabaseOpen()).toBe(true);

    // Wire the test mock so serveAcpGateway's shutdown handler calls the
    // real closeNodoAssistStateDatabase, which closes the handle we opened above.
    mockState.closeNodoAssistStateDatabase.mockImplementation(() => {
      actualStateDb.closeNodoAssistStateDatabase();
    });
    ...
    try {
      ...
      expect(actualStateDb.isNodoAssistStateDatabaseOpen()).toBe(false);
    } finally {
      actualStateDb.closeNodoAssistStateDatabase();
      onceSpy.mockRestore();
    }
