...
import { resolveNodoAssistPackageRoot } from "./nodoassist-root.js";
import {
...
function resolveAutoUpdatePolicy(cfg: NodoAssistConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: NodoAssistConfig): number {
...
async function resolveStartupInstallStatus() {
  const root = await resolveNodoAssistPackageRoot({
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.NODOASSIST_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("nodoassist update")}`,
      );
    ...
    if (channel !== "extended-stable" && auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by NODOASSIST_NO_AUTO_UPDATE", {
