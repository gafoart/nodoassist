...
function createHandoffEnv(): NodeJS.ProcessEnv {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-restart-handoff-"));
  tempDirs.push(dir);
  return {
    ...process.env,
...
function legacyHandoffPath(env: NodeJS.ProcessEnv): string {
  return path.join(env.NODOASSIST_STATE_DIR ?? "", "gateway-supervisor-restart-handoff.json");
}
