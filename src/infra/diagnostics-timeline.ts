...
const NODOASSIST_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "nodoassist.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.NODOASSIST_DIAGNOSTICS)) &&
    typeof env.NODOASSIST_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.NODOASSIST_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: NODOASSIST_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.NODOASSIST_DIAGNOSTICS_RUN_ID ? { runId: env.NODOASSIST_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.NODOASSIST_DIAGNOSTICS_ENV ? { envName: env.NODOASSIST_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.NODOASSIST_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
