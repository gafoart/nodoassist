...
/** Supported package managers for NodoAssist global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "nodoassist";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
/** npm-compatible spec used when the user asks to install the moving main branch. */
export const NODOASSIST_MAIN_PACKAGE_SPEC = "github:nodoassist/nodoassist#main";
const COREPACK_ENABLE_DOWNLOAD_PROMPT_DEFAULT = "0";
const NPM_GLOBAL_INSTALL_QUIET_FLAGS = ["--no-fund", "--no-audit", "--loglevel=error"] as const;
const PNPM_NODOASSIST_BUILD_ALLOWLIST_FLAG = `--allow-build=${PRIMARY_PACKAGE_NAME}`;
...
function isPnpmNodoAssistSourceInstallSpec(spec: string): boolean {
...
/**
 * Extracts a pinned installed version from package specs like `nodoassist@1.2.3`.
...
/**
 * Verifies that a global package root looks like a packaged NodoAssist install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "nodoassist.plugin.json")))
      ) {
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "NodoAssist", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.NODOASSIST_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.NODOASSIST_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return NODOASSIST_MAIN_PACKAGE_SPEC;
  }
 ...
 * Used for staged installs where NodoAssist creates the prefix itself.
 */
...
/**
 * Detects an installed global NodoAssist package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global NodoAssist install.
...
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(isPnpmNodoAssistSourceInstallSpec(spec) ? [PNPM_NODOASSIST_BUILD_ALLOWLIST_FLAG] : []),
      spec,
    ];
  }
  if (resolved.manager === "bun") {
