// Covers NodoAssist package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__nodoassist_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "nodoassist-root");
...
function setPackageRoot(root: string, name = "nodoassist") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./nodoassist-root.js").resolveNodoAssistPackageRootSync,
  asyncResolver: typeof import("./nodoassist-root.js").resolveNodoAssistPackageRoot,
  opts: Parameters<typeof import("./nodoassist-root.js").resolveNodoAssistPackageRootSync>[0],
...
vi.mock("./nodoassist-root.fs.runtime.js", () => ({
  nodoAssistRootFsSync: mockFsModule(),
  nodoAssistRootFs: mockFsPromisesModule(),
}));

describe("resolveNodoAssistPackageRoot", () => {
  let resolveNodoAssistPackageRoot: typeof import("./nodoassist-root.js").resolveNodoAssistPackageRoot;
  let resolveNodoAssistPackageRootSync: typeof import("./nodoassist-root.js").resolveNodoAssistPackageRootSync;
  let clearNodoAssistPackageRootCaches: typeof import("./nodoassist-root.js").testing.clearNodoAssistPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveNodoAssistPackageRoot,
      resolveNodoAssistPackageRootSync,
      testing: { clearNodoAssistPackageRootCaches },
    } = await import("./nodoassist-root.js"));
  });

  beforeEach(() => {
    clearNodoAssistPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "nodoassist");
        const pkgRoot = path.join(project, "node_modules", "nodoassist");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "nodoassist");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "nodoassist.mjs")));
        setPackageRoot(realPkg);
        return { opts: { argv1: bin }, expected: realPkg };
      },
    },
    {
      name: "prefers a symlink target nested under another nodoassist package",
      setup: () => {
        const sourceRoot = fx("nested-symlink-scenario");
        const bin = path.join(sourceRoot, ".artifacts", "prefix", "bin", "nodoassist");
        ...
        state.realpaths.set(abs(bin), abs(path.join(installedRoot, "nodoassist.mjs")));
    ...
    {
      name: "falls through from a non-nodoassist moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-nodoassist");
    ...
    {
      name: "returns null for non-nodoassist package roots",
      setup: () => {
        const pkgRoot = fx("not-nodoassist");
        setPackageRoot(pkgRoot, "not-nodoassist");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "nodoassist.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "nodoassist");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    {
      name: "still resolves the nodoassist package below a node_modules boundary",
      setup: () => {
