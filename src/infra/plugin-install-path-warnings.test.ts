...
const MATRIX_REPO_INSTALL_COMMAND = `nodoassist plugins install ${repoInstallSpec("matrix")}`;
...
describe("plugin install path warnings", () => {
  ...
  it("detects stale custom plugin install paths", async () => {
    const issue = await detectPluginInstallPathIssue({
      ...
      install: {
        source: "path",
        sourcePath: "/tmp/nodoassist-matrix-missing",
        installPath: "/tmp/nodoassist-matrix-missing",
      },
    });
    ...
    expect(issue).toEqual({
      kind: "missing-path",
      pluginId: "matrix",
      path: "/tmp/nodoassist-matrix-missing",
    });
    expect(
      formatPluginInstallPathIssue({
        issue: issue!,
      ...
      "Matrix is installed from a custom path that no longer exists: /tmp/nodoassist-matrix-missing",
      'Reinstall with "nodoassist plugins install @nodoassist/matrix".',
  ...
  it("applies custom command formatting in warning messages", () => {
    expect(
      ...
      'Reinstall with "<nodoassist plugins install @nodoassist/matrix>" when you want to return to the standard Matrix plugin.',
