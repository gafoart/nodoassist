...
async function withPackageManagerRoot<T>(
  ...
  return await withTempDir({ prefix: "nodoassist-detect-pm-" }, async (root) => {
...
async function writePublishedNodoAssistRoot(root: string): Promise<void> {
...
describe("detectPackageManager", () => {
  ...
  it("keeps pnpm-owned direct package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "nodoassist-detect-pm-pnpm-direct-" }, async (base) => {
      const nodeModulesRoot = path.join(base, "pnpm-global", "node_modules");
      const packageRoot = path.join(nodeModulesRoot, "nodoassist");
      await writePublishedNodoAssistRoot(packageRoot);
  ...
  it("keeps pnpm-owned virtual-store package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "nodoassist-detect-pm-pnpm-virtual-" }, async (base) => {
      const nodeModulesRoot = path.join(base, "project", "node_modules");
      const packageRoot = path.join(
        nodeModulesRoot,
        ".pnpm",
        "nodoassist@2026.5.27",
        "node_modules",
        "nodoassist",
      );
      await writePublishedNodoAssistRoot(packageRoot);
  ...
  it("keeps bun-owned global package roots that ship npm-shrinkwrap", async () => {
    await withTempDir({ prefix: "nodoassist-detect-pm-bun-" }, async (base) => {
