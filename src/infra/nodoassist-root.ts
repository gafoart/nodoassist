// Resolves the NodoAssist package root from runtime and package metadata.
import path from "node:path";
import { fileURLToPath } from "node:url";
import { nodoAssistRootFs, nodoAssistRootFsSync } from "./nodoassist-root.fs.runtime.js";

const CORE_PACKAGE_NAMES = new Set(["nodoassist"]);
...
async function readPackageName(dir: string): Promise<string | null> {
  ...
  try {
    const name = parsePackageName(await nodoAssistRootFs.readFile(packageJsonPath, "utf-8"));
...
function readPackageNameSync(dir: string): string | null {
  ...
  try {
    const name = parsePackageName(nodoAssistRootFsSync.readFileSync(packageJsonPath, "utf-8"));
    packageNameCache.set(packageJsonPath, name);
    return name;
  } catch {
    packageNameCache.set(packageJsonPath, null);
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another NodoAssist checkout keeps its own package root.
  try {
    const resolved = nodoAssistRootFsSync.realpathSync(normalized);
...
export async function resolveNodoAssistPackageRoot(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): Promise<string | null> {
...
export function resolveNodoAssistPackageRootSync(opts: {
  cwd?: string;
  argv1?: string;
  moduleUrl?: string;
}): string | null {
...
export const testing = {
  clearNodoAssistPackageRootCaches(): void {
