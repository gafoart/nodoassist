...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "nodoassist-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      NODOASSIST_DIAGNOSTICS: "timeline",
      NODOASSIST_DIAGNOSTICS_RUN_ID: "run-1",
      NODOASSIST_DIAGNOSTICS_ENV: "env-1",
      NODOASSIST_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NODOASSIST_DIAGNOSTICS: "1" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NODOASSIST_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NODOASSIST_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NODOASSIST_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NODOASSIST_DIAGNOSTICS: "*" } })).toBe(
      true,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, NODOASSIST_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, NODOASSIST_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, NODOASSIST_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, NODOASSIST_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.NODOASSIST_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as NodoAssistConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as NodoAssistConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as NodoAssistConfig;
  ...
  it("lets false-like env diagnostics disable config-enabled timeline output", async () => {
    ...
    expect(
      isDiagnosticsTimelineEnabled({
        ...
        env: { ...env, NODOASSIST_DIAGNOSTICS: "0" },
      }),
    ).toBe(false);
  });
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("nodoassist.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.NODOASSIST_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as NodoAssistConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
