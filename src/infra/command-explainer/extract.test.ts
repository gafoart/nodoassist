...
describe("command explainer tree-sitter runtime", () => {
  ...
  it("reports missing parser packages and wasm files with explainer context", () => {
    expect(() =>
      resolvePackageFileForCommandExplanation(
        "definitely-missing-nodoassist-parser-package",
        "parser.wasm",
      ),
    ).toThrow("Unable to resolve definitely-missing-nodoassist-parser-package");

    expect(() =>
      resolvePackageFileForCommandExplanation("web-tree-sitter", "missing-nodoassist-parser.wasm"),
    ).toThrow("Unable to locate missing-nodoassist-parser.wasm in web-tree-sitter");
  });
  ...
  it("detects eval, source, aliases, and carrier shell wrappers", async () => {
    const evalCommand = await explainShellCommand('eval "$NODOASSIST_CMD"');
