...
function resolveGatewayServiceRecovery(
  ...
  if (supervisor === "systemd") {
    const override = env.NODOASSIST_SYSTEMD_UNIT?.trim();
    const unit = override
  ...
  if (supervisor === "launchd") {
    const label =
      env.NODOASSIST_LAUNCHD_LABEL?.trim() ||
      resolveGatewayLaunchAgentLabel(env.NODOASSIST_PROFILE);
  ...
  if (supervisor === "schtasks") {
    const taskName =
      env.NODOASSIST_WINDOWS_TASK_NAME?.trim() ||
      resolveGatewayWindowsTaskName(env.NODOASSIST_PROFILE);
...
function buildSystemdHandoffUnitName(handoffId: string | undefined): string {
  const suffix =
    ...
    "handoff";
  return `nodoassist-update-${suffix}.scope`;
}
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside nodoassist-gateway.service",
    );
  }
...
export async function startManagedServiceUpdateHandoff(params: {
  ...
  const helperParams = {
    ...
    logPath,
    metaPath,
    stateDatabasePath: resolveNodoAssistStateSqlitePath(params.env ?? process.env),
  ...
  const env = {
    ...
    NODOASSIST_UPDATE_RUN_HANDOFF: "1",
  };
