...
const NODOASSIST_DEBUG_PROXY_ENABLED = "NODOASSIST_DEBUG_PROXY_ENABLED";
...
function isManagedProxyActive(): boolean {
  return process.env["NODOASSIST_PROXY_ACTIVE"] === "1";
}
...
async function captureGuardedFetchExchange(params: {
  ...
  if (params.capture === false || !isTruthyEnvValue(process.env[NODOASSIST_DEBUG_PROXY_ENABLED])) {
    return;
  }
