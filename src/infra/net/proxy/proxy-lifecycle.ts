...
import { isHttpUrl, isWebSocketUrl } from "@nodoassist/net-policy/url-protocol";
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    NODOASSIST_PROXY_ACTIVE: process.env["NODOASSIST_PROXY_ACTIVE"],
    NODOASSIST_PROXY_LOOPBACK_MODE: process.env["NODOASSIST_PROXY_LOOPBACK_MODE"],
    NODOASSIST_PROXY_CA_FILE: process.env["NODOASSIST_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["NODOASSIST_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["NODOASSIST_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["NODOASSIST_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["NODOASSIST_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or NODOASSIST_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["NODOASSIST_PROXY_ACTIVE"] !== "1") {
    return;
  }
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    proxyUrl,
    caFileOverride: process.env["NODOASSIST_PROXY_CA_FILE"],
  });
 ...
 * loopback CDP probe to a Chromium instance NodoAssist spawned itself.
 *
