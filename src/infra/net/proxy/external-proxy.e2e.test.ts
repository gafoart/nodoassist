// End-to-end proxy smoke tests run child NodoAssist modules through a local
...
async function withDiscordTlsFixture<T>(
  ...
  return await withTempDir(
    {
      prefix: "nodoassist-discord-tls-",
      parentDir: resolvePreferredNodoAssistTmpDir(),
    },
    async (dir) => {
  ...
  it("routes normal HTTP and WebSocket egress through an operator-managed proxy even when NO_PROXY includes loopback", async () => {
    ...
    const child = await runNodeModule(
      `
        ...
        try {
          const response = await undiciFetch(process.env.NODOASSIST_TEST_TARGET_URL, {
            signal: AbortSignal.timeout(${PROBE_TIMEOUT_MS}),
          });
          const body = await response.text();
          const globalFetchResponse = await fetch(process.env.NODOASSIST_TEST_GLOBAL_FETCH_TARGET_URL, {
            signal: AbortSignal.timeout(${PROBE_TIMEOUT_MS}),
          });
          const globalFetchBody = await globalFetchResponse.text();
          const nodeHttp = await nodeHttpGet(process.env.NODOASSIST_TEST_NODE_HTTP_TARGET_URL);
          const explicitAgent = await nodeHttpGet(process.env.NODOASSIST_TEST_EXPLICIT_AGENT_TARGET_URL, {
            agent: new http.Agent(),
          });
          await expectFailure("node:https", () =>
            nodeHttpsProbe(process.env.NODOASSIST_TEST_NODE_HTTPS_TARGET_URL),
          );
          await websocketProbe(process.env.NODOASSIST_TEST_WS_TARGET_URL);
          await gatewayLoopbackBypassProbe(process.env.NODOASSIST_TEST_GATEWAY_BYPASS_WS_URL);
      ...
      {
        ...process.env,
        NODOASSIST_PROXY_URL: `http://127.0.0.1:${proxyPort}`,
        NODOASSIST_TEST_TARGET_URL: `http://127.0.0.1:${targetPort}/private-metadata`,
        NODOASSIST_TEST_GLOBAL_FETCH_TARGET_URL: `http://127.0.0.1:${globalFetchTargetPort}/global-fetch-metadata`,
        NODOASSIST_TEST_NODE_HTTP_TARGET_URL: `http://127.0.0.1:${targetPort}/node-http-metadata`,
        NODOASSIST_TEST_EXPLICIT_AGENT_TARGET_URL: `http://127.0.0.1:${targetPort}/explicit-agent`,
        NODOASSIST_TEST_NODE_HTTPS_TARGET_URL: `https://127.0.0.1:${httpsLikeTargetPort}/https-connect-proof`,
        NODOASSIST_TEST_WS_TARGET_URL: `ws://127.0.0.1:${wsTargetPort}/websocket-proxied`,
        NODOASSIST_TEST_GATEWAY_BYPASS_WS_URL: `ws://127.0.0.1:${gatewayBypassWsTargetPort}/gateway-bypass`,
  ...
  it("preserves the target TLS hostname for Node HTTPS requests through the managed proxy", async () => {
    await withDiscordTlsFixture(async (tlsFixture) => {
      ...
      const child = await runNodeModule(
        ...
        try {
          const response = await nodeHttpsGet(process.env.NODOASSIST_TEST_DISCORD_TLS_URL);
      ...
      `,
        {
          ...
          NODOASSIST_PROXY_URL: `http://127.0.0.1:${proxyPort}`,
          NODOASSIST_TEST_DISCORD_TLS_URL: `https://discord.com:${tlsTargetPort}/tls-proxy-proof`,
          NO_PROXY: "127.0.0.1,localhost",
          no_proxy: "localhost",
        },
      );
