...
import { isHttpUrl } from "@nodoassist/net-policy/url-protocol";
...
const DENIED_CANARY_HEADER = "x-nodoassist-proxy-validation-canary";
...
export type ResolveProxyValidationConfigOptions = {
  config?: ProxyConfig;
  env?: NodeJS.ProcessEnv | Partial<Record<"NODOASSIST_PROXY_URL", string | undefined>>;
...
function validateProxyUrl(value: string | undefined): string[] {
  if (!value) {
    return ["proxy validation requires proxy.proxyUrl, --proxy-url, or NODOASSIST_PROXY_URL"];
  }
  if (!isHttpUrl(value)) {
...
function validateProxyEnabled(source: ProxyValidationConfigSource, enabled: boolean): string[] {
  ...
  if (source === "env") {
    return ["proxy validation requires proxy.enabled to be true for NODOASSIST_PROXY_URL"];
  }
...
export function resolveProxyValidationConfig(
  ...
  const envUrl = normalizeProxyUrl(options.env?.NODOASSIST_PROXY_URL);
  if (envUrl) {
    const proxyCaFile = resolveManagedProxyCaFileForUrl({
      proxyUrl: envUrl,
      config: options.config,
