// Creates temporary NodoAssist directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared NodoAssist temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_NODOASSIST_TMP_DIR = "/tmp/nodoassist";
...
export type ResolvePreferredNodoAssistTmpDirOptions = {
...
/** Resolves a safe NodoAssist temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredNodoAssistTmpDir(
  options: ResolvePreferredNodoAssistTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "nodoassist" : `nodoassist-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[nodoassist] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback NodoAssist temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback NodoAssist temp dir: ${fallbackPath}`);
    }
  ...
  const preferredDir = POSIX_NODOASSIST_TMP_DIR;
