...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const nodoassistPayload = requireRecord(payload.nodoassist, "nodoassist payload");
    expectRecordFields(nodoassistPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof nodoassistPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.nodoassist.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open NodoAssist to review this request.",
      },
      sound: "default",
      category: "nodoassist.exec-approval",
      "content-available": 1,
    });
    const nodoassistPayload = requireRecord(payload.nodoassist, "nodoassist payload");
    expectRecordFields(nodoassistPayload, {
    ...
    expect(typeof nodoassistPayload.ts).toBe("number");
    expectNoProperties(nodoassistPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.nodoassist, "nodoassist payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
