// Covers NodoAssist's default fs-safe Python helper configuration.
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.NODOASSIST_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in NodoAssist", async () => {
  ...
  it("honors the NodoAssist-specific env mode override", async () => {
    process.env.NODOASSIST_FS_SAFE_PYTHON_MODE = "auto";
