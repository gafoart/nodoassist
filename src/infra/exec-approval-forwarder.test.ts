...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): NodoAssistConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): NodoAssistConfig {
  return {
