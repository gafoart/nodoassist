// Validates the current runtime against NodoAssist's Node engine floor.
...
/** Returns whether a detected runtime meets NodoAssist's minimum runtime contract. */
...
/** Checks a Node version label against NodoAssist's supported Node version range. */
...
export function assertSupportedRuntime(
  ...
  runtime.error(
    [
      "nodoassist requires Node >=22.19.0 <23 or >=23.11.0.",
      `Detected: ${runtimeLabel} (exec: ${execLabel}).`,
      `PATH searched: ${details.pathEnv}`,
      "Install Node: https://nodejs.org/en/download",
      "Upgrade Node and re-run nodoassist.",
    ].join("\n"),
  );
  runtime.exit(1);
}
