// Tests NodoAssist home directory resolution.
...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers NODOASSIST_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands NODOASSIST_HOME when set to ~", () => {
    const env = {
      NODOASSIST_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for NODOASSIST_HOME",
      env: { NODOASSIST_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when NODOASSIST_HOME is tilde-only and no fallback home exists",
      env: { NODOASSIST_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
  ...
  it("fails clearly when both home and cwd are unavailable", () => {
    ...
    try {
      expect(() => resolveRequiredHomeDir({}, noHome)).toThrow(/set NODOASSIST_HOME/i);
...
describe("resolveOsHomeDir", () => {
  it("ignores NODOASSIST_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          NODOASSIST_HOME: "/srv/nodoassist-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/nodoassist-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/nodoassist-home " },
      expected: "/srv/nodoassist-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/nodoassist-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of NODOASSIST_HOME", () => {
    expect(
