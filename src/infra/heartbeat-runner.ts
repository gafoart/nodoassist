...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: NodoAssistConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: NodoAssistConfig) {
...
function resolveHeartbeatAgents(cfg: NodoAssistConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: NodoAssistConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: NodoAssistConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: NodoAssistConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: NodoAssistConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: NodoAssistConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: NodoAssistConfig) {
...
export function startHeartbeatRunner(opts: {
  cfg?: NodoAssistConfig;
  readCurrentConfig?: () => NodoAssistConfig;
  runtime?: RuntimeEnv;
  ...
  const updateConfig = (cfg: NodoAssistConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
