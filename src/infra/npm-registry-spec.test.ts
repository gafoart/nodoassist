...
import {
  ...
  isNodoAssistStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    ...
    "@nodoassist/voice-call@1.2.3",
    "@nodoassist/voice-call@1.2.3-beta.4",
    "@nodoassist/voice-call@latest",
    "@nodoassist/voice-call@beta",
  ])("accepts %s", (spec) => {
  ...
  it.each([
    {
      spec: "@nodoassist/voice-call@^1.2.3",
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    },
    {
      ...
      expected: {
        name: "@nodoassist/voice-call",
        raw: "@nodoassist/voice-call@beta",
        selector: "beta",
        selectorKind: "tag",
        selectorIsPrerelease: false,
      },
    ...
    {
      spec: "@nodoassist/voice-call@2026.5.3-1",
      expected: {
        name: "@nodoassist/voice-call",
        raw: "@nodoassist/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@nodoassist/voice-call@1.2.3-beta.1",
      expected: {
        name: "@nodoassist/voice-call",
        raw: "@nodoassist/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@nodoassist/voice-call", expected: true },
    { spec: "@nodoassist/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@nodoassist/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects NodoAssist-org npm specs for %s", ({ spec, expected }) => {
    expect(isNodoAssistOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects NodoAssist stable correction versions for %s", ({ value, expected }) => {
    expect(isNodoAssistStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares NodoAssist release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareNodoAssistReleaseVersions(left, right)).toBe(expected);
  });
});
...
describe("npm prerelease resolution policy", () => {
  it.each([
    ...
    {
      spec: "@nodoassist/voice-call@latest",
  ...
  it.each([
    {
      spec: "@nodoassist/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@nodoassist/voice-call@beta"`,
    },
    {
      spec: "@nodoassist/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
