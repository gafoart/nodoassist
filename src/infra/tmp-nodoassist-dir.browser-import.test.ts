...
describe("tmp-nodoassist-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__nodoassistTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_NODOASSIST_TMP_DIR, resolvePreferredNodoAssistTmpDir } from "./src/infra/tmp-nodoassist-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_NODOASSIST_TMP_DIR,
            resolverType: typeof resolvePreferredNodoAssistTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-nodoassist-dir-browser-entry.ts",
      },
      write: false,
    });
