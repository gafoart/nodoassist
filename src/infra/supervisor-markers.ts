...
const SUPERVISOR_HINTS = {
  launchd: ["NODOASSIST_LAUNCHD_LABEL"],
  systemd: ["NODOASSIST_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["NODOASSIST_WINDOWS_TASK_NAME"],
} as const;
...
export interface DetectRespawnSupervisorOptions {
  includeLinuxNodoAssistGatewayServiceMarker?: boolean;
}
...
function hasNodoAssistGatewayServiceMarker(env: NodeJS.ProcessEnv): boolean {
  return (
    env.NODOASSIST_SERVICE_MARKER?.trim() === "nodoassist" &&
    env.NODOASSIST_SERVICE_KIND?.trim() === "gateway"
  );
}

function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.NODOASSIST_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "linux") {
    return hasAnyHint(env, SUPERVISOR_HINTS.systemd) ||
      (options.includeLinuxNodoAssistGatewayServiceMarker === true &&
        hasNodoAssistGatewayServiceMarker(env))
      ? "systemd"
      : null;
  }
  if (platform === "win32") {
    ...
    const marker = env.NODOASSIST_SERVICE_MARKER?.trim();
