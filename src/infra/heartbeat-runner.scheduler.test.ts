...
describe("startHeartbeatRunner", () => {
  ...
  function heartbeatConfig(
    list?: NonNullable<NonNullable<NodoAssistConfig["agents"]>["list"]>,
  ): NodoAssistConfig {
    return {
      agents: {
  ...
  it("reads the latest runtime config for heartbeat wakes after no-op reload commits", async () => {
    ...
    expect((options.cfg as NodoAssistConfig).messages?.visibleReplies).toBe("message_tool");
  ...
  it("preserves immediate delivery for repeated bare wake reasons", async () => {
    // 'wake' is the immediate-path reason from `nodoassist system event --mode now`
