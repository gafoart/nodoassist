// Covers NodoAssist CLI PATH construction.
import fs from "node:fs";
import path from "node:path";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { ensureNodoAssistCliOnPath } from "./path-env.js";
...
describe("ensureNodoAssistCliOnPath", () => {
  const envKeys = [
    "PATH",
    "NODOASSIST_PATH_BOOTSTRAPPED",
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/nodoassist-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "nodoassist");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureNodoAssistCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  function resetBootstrapEnv(pathValue = "/usr/bin") {
    process.env.PATH = pathValue;
    delete process.env.NODOASSIST_PATH_BOOTSTRAPPED;
    delete process.env.NODOASSIST_ALLOW_PROJECT_LOCAL_BIN;
  ...
  it("prepends the bundled app bin dir when a sibling nodoassist exists", () => {
  ...
  it("keeps the current runtime directory ahead of system PATH hardening", () => {
    const tmp = abs("/tmp/nodoassist-path/case-runtime-dir");
  ...
  it("is idempotent", () => {
    ...
    process.env.NODOASSIST_PATH_BOOTSTRAPPED = "1";
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "nodoassist");
      ...
      if (envValue === undefined) {
        ...
        process.env.NODOASSIST_ALLOW_PROJECT_LOCAL_BIN = envValue;
      }
  ...
  it("skips project-local bins when the working directory was deleted", () => {
    ...
    const localBinDir = path.join(tmp, "node_modules", ".bin");
    setDir(localBinDir);
    setExe(path.join(localBinDir, "nodoassist"));
    resetBootstrapEnv();
    process.env.NODOASSIST_ALLOW_PROJECT_LOCAL_BIN = "1";
    const cwdSpy = vi.spyOn(process, "cwd").mockImplementation(() => {
      throw new Error("ENOENT: uv_cwd");
    });

    try {
      ensureNodoAssistCliOnPath({ execPath: appCli, homeDir: tmp, platform: "darwin" });
    } finally {
      cwdSpy.mockRestore();
    }
  ...
  it("ignores package-manager env roots derived from the active workspace", () => {
    const homeDir = abs("/tmp/nodoassist-path/home");
  ...
  it.each([
    ...
    },
    {
      ...
      setup: () => {
        const tmp = abs("/tmp/nodoassist-path/case-linuxbrew");
