...
async function withTempNodoAssistStateDir<T>(test: (stateDir: string) => Promise<T>): Promise<T> {
...
describe("runMessageAction media behavior", () => {
  ...
  it("materializes buffer-only send attachments into outbound media paths", async () => {
    ...
    await withTempNodoAssistStateDir(async () => {
  ...
  it("does not stage buffer-only send attachments before target validation passes", async () => {
    ...
    await withTempNodoAssistStateDir(async (stateDir) => {
      await expect(
        runMessageAction({
          cfg: workspaceConfig,
          action: "send",
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before nodoassist#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred NodoAssist tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideNodoAssist = path.join(
          os.tmpdir(),
          "outside-nodoassist",
          "test-media.png",
        );
        await expect(
          runMessageAction({
            ...
            params: {
              channel: "workspace",
              target: "12345678",
              media: hostTmpOutsideNodoAssist,
              message: "",
            },
            sandboxRoot: sandboxDir,
            dryRun: true,
