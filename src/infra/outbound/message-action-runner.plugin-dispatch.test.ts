...
describe("runMessageAction plugin dispatch", () => {
  ...
  describe("alias-based plugin action dispatch", () => {
    ...
    it("routes execution context ids into plugin handleAction", async () => {
      const stateDir = path.join("/tmp", "nodoassist-plugin-dispatch-media-roots");
    ...
    it("applies TTS before gateway-executed plugin sends", async () => {
      ...
      expectRecordFields(
        ...
        {
          message: "",
          media: "file:///tmp/nodoassist-voice.ogg",
          mediaUrl: "file:///tmp/nodoassist-voice.ogg",
          asVoice: true,
          audioAsVoice: true,
        },
  ...
  describe("threaded plugin actions", () => {
    ...
    const cfg = { channels: { forumchat: { enabled: true } } } as NodoAssistConfig;
  ...
  describe("presentation-only send behavior", () => {
    const handleAction = vi.fn(
      async ({ cfg, params }: { cfg: NodoAssistConfig; params: Record<string, unknown> }) => {
