...
import { resolveNonNegativeIntegerOption } from "@nodoassist/normalization-core/number-coercion";
...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: NodoAssistConfig | null = null;
   ...
   */
  get(key: string, cfg: NodoAssistConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: NodoAssistConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
   ...
   */
  clear(cfg?: NodoAssistConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: NodoAssistConfig): void {
