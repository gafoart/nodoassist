...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `nodoassist agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: NodoAssistConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: nodoassist doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run nodoassist channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use nodoassist channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: NodoAssistConfig): Promise<boolean> {
