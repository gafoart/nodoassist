  ...
  if (params.assessment.disposition === "blocked") {
    ...
    const blockedActionLines =
      params.mode === "update"
        ? malicious
          ? [
              `Latest ${noun} version is marked malicious; NodoAssist will not download it.`,
              `Uninstall the installed ${noun} unless you have independently reviewed it.`,
            ]
          : [`Latest ${noun} version is blocked by ClawHub; NodoAssist will not download it.`]
        : [`NodoAssist will not install this ${noun} release from ClawHub.`];
