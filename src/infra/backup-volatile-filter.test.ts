...
const stateDir = "/opt/nodoassist/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/nodoassist/state/sessions/../config.jsonl` resolves to
    // `/opt/nodoassist/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/nodoassist/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
