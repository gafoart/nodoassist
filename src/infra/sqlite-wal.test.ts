...
describe("sqlite WAL maintenance", () => {
  ...
  it("uses rollback journaling for databases on NFS-backed volumes", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-nfs-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("linux");
      ...
      const maintenance = configureSqliteWalMaintenance(db, {
        ...
        databasePath: path.join(tempDir, "missing", "nodoassist.sqlite"),
      });
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-network-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("linux");
      ...
      configureSqliteWalMaintenance(db, {
        checkpointIntervalMs: 0,
        databasePath: path.join(tempDir, "nodoassist.sqlite"),
      });
  ...
  it.each([
    ...
    "//server/share/nodoassist.sqlite",
    "//?/UNC/server/share/nodoassist.sqlite",
  ...
  it("uses rollback journaling for mapped Windows network drives", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\state\nodoassist.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    const realpath = vi
      .spyOn(fs.realpathSync, "native")
      .mockReturnValue(String.raw`\\server\share\state\nodoassist.sqlite`);
  ...
  it("does not treat namespaced Windows local drives as UNC paths", () => {
    const db = createMockDb();
    const databasePath = String.raw`\\?\C:\state\nodoassist.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("uses rollback journaling when Windows cannot classify an opened drive path", () => {
    const db = createMockDb();
    const databasePath = String.raw`Z:\restricted\nodoassist.sqlite`;
    vi.spyOn(process, "platform", "get").mockReturnValue("win32");
  ...
  it("refuses fuse.sshfs mountinfo entries", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-sshfs-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("linux");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
  ...
  it("refuses symlinked paths into fuse.sshfs mounts", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-sshfs-link-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(linkedDir, "nodoassist.sqlite"),
        }),
  ...
  it("matches raw mount paths when the existing path canonicalizes elsewhere", () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-sshfs-prefix-"));
    ...
    try {
      ...
      expect(() =>
        configureSqliteWalMaintenance(createMockDb(), {
          checkpointIntervalMs: 0,
          databasePath: path.join(rawMountDir, "nodoassist.sqlite"),
        }),
      ).toThrow(/SSHFS.*refusing to open/);
    } finally {
      fs.rmSync(tempDir, { recursive: true, force: true });
  ...
  it("uses macOS SMB mount filesystem names", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-smb-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("darwin");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
  ...
  it.each([
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-sshfs-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("darwin");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
  ...
  it("keeps WAL enabled for non-remote macFUSE mounts", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sqlite-macfuse-"));
    try {
      const db = createMockDb();
      vi.spyOn(process, "platform", "get").mockReturnValue("darwin");
      vi.spyOn(fs, "statfsSync").mockReturnValue(statfsFixture(0));
