// Computes git, dependency, and registry update status for NodoAssist installs.
...
import { compareNodoAssistReleaseVersions } from "./npm-registry-spec.js";
...
function packageTargetSpec(params: { target: string; spec?: string }): string {
  ...
  return spec || `nodoassist@${params.target.trim() || "latest"}`;
}

const PUBLIC_NPM_REGISTRY_URL = "https://registry.npmjs.org/";
const PUBLIC_NPM_PACKAGE_NAME = "nodoassist";
...
function resolveExtendedStableRegistryTarget(params: {
  ...
  const packageSpecOverride = env.NODOASSIST_UPDATE_PACKAGE_SPEC?.trim();
...
export function compareSemverStrings(a: string | null, b: string | null): number | null {
  if (a && b) {
    const nodoAssistReleaseCmp = compareNodoAssistReleaseVersions(a, b);
    if (nodoAssistReleaseCmp != null) {
      return nodoAssistReleaseCmp;
    }
  }
