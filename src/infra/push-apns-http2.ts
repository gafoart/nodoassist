  ...
  try {
    return await new Promise<ProbeApnsHttp2ReachabilityViaProxyResult>((resolve, reject) => {
      ...
      const request = session.request({
        ...
        authorization: "bearer intentionally.invalid.nodoassist.proxy.validation",
        "apns-topic": "ai.nodoassist.ios",
      ...
      request.end(JSON.stringify({ aps: { alert: "NodoAssist APNs proxy validation" } }));
    });
  } finally {
