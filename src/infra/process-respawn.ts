...
import { triggerNodoAssistRestart } from "./restart.js";
...
const PNPM_VERSIONED_NODOASSIST_ENTRY_PATTERN =
  /^(.*?)([\\/])node_modules\2\.pnpm\2nodoassist@[^\\/]+\2node_modules\2nodoassist\2.+$/;

function rewritePnpmVersionedNodoAssistEntryPath(entryPath: string): string {
  // pnpm can expose argv[1] as a versioned realpath that self-update removes.
  // Respawn through the stable NodoAssist package wrapper instead.
  return entryPath.replace(
    PNPM_VERSIONED_NODOASSIST_ENTRY_PATTERN,
    "$1$2node_modules$2nodoassist$2nodoassist.mjs",
  );
}
...
function spawnDetachedGatewayProcess(opts: GatewayRespawnOptions = {}): {
  ...
  const args = [
    ...process.execArgv,
    ...(entryArg ? [rewritePnpmVersionedNodoAssistEntryPath(entryArg)] : []),
    ...entryArgs,
  ];
 ...
 * - NODOASSIST_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthy(process.env.NODOASSIST_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerNodoAssistRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthy(process.env.NODOASSIST_NO_RESPAWN)) {
    return { mode: "disabled", detail: "NODOASSIST_NO_RESPAWN" };
  }
  try {
