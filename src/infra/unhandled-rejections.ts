...
// Plugins resolve `nodoassist/plugin-sdk/runtime` through their own staged
...
const HANDLERS_GLOBAL_KEY = Symbol.for("nodoassist.unhandledRejection.handlers");
const EXCEPTION_HANDLERS_GLOBAL_KEY = Symbol.for("nodoassist.uncaughtException.handlers");
...
function isUnhandledRejectionHandled(reason: unknown): boolean {
  for (const handler of handlers) {
    try {
      ...
      console.error(
        "[nodoassist] Unhandled rejection handler failed:",
...
export function isUncaughtExceptionHandled(error: unknown): boolean {
  for (const handler of exceptionHandlers) {
    try {
      ...
      console.error(
        "[nodoassist] Uncaught exception handler failed:",
        err instanceof Error ? (err.stack ?? err.message) : err,
      );
    }
  }
...
export function installUnhandledRejectionHandler(): void {
  ...
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isAbortError(reason)) {
      console.warn("[nodoassist] Suppressed AbortError:", formatUncaughtError(reason));
      return;
    }

    if (isFatalError(reason)) {
      console.error("[nodoassist] FATAL unhandled rejection:", formatUncaughtError(reason));
    ...
    if (isConfigError(reason)) {
      console.error(
        "[nodoassist] CONFIGURATION ERROR - requires fix:",
    ...
    if (isTransientUnhandledRejectionError(reason)) {
      console.warn(
        "[nodoassist] Non-fatal unhandled rejection (continuing):",
        formatUncaughtError(reason),
      );
      return;
    }

    console.error("[nodoassist] Unhandled promise rejection:", formatUncaughtError(reason));
