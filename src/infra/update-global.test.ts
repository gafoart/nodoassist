...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["NODOASSIST_UPDATE_PACKAGE_SPEC"]);
    process.env.NODOASSIST_UPDATE_PACKAGE_SPEC = "file:/tmp/nodoassist.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "nodoassist", tag: "latest" })).toBe(
      "file:/tmp/nodoassist.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "nodoassist",
        tag: "beta",
        env: { NODOASSIST_UPDATE_PACKAGE_SPEC: "nodoassist@next" },
      }),
    ).toBe("nodoassist@next");
  });
  ...
  it("resolves scoped package paths from the package manager global root", async () => {
    ...
    await expect(
      ...
      manager: "npm",
      globalRoot,
      packageRoot: path.join(globalRoot, "@kevins8", "nodoassist"),
    });
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "nodoassist", tag: "main" })).toBe(
      NODOASSIST_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "nodoassist",
        tag: "github:nodoassist/nodoassist#feature/my-branch",
      }),
    ).toBe("github:nodoassist/nodoassist#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "nodoassist",
  ...
  it("resolves portable Git paths from process-local app data only", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "nodoassist-update-portable-git-" }, async (base) => {
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:nodoassist/nodoassist#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/openclaw-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/nodoassist-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("/tmp/nodoassist-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("nodoassist-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:nodoassist/nodoassist#main")).toBe(
      false,
    );
    expect(canResolveRegistryVersionForPackageTarget("/tmp/nodoassist-main.tgz")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "nodoassist-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "nodoassist");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "nodoassist"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "nodoassist"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "nodoassist"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "nodoassist"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "nodoassist-update-npm-prefix-" }, async (base) => {
        ...
        expect(globalInstallArgs("npm", "nodoassist@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "nodoassist@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "nodoassist@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "nodoassist@latest",
  ...
  it("keeps npm ownership but avoids per-Node npm commands for reinstall", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "nodoassist-update-node-version-prefix-" }, async (base) => {
        ...
        for (const layout of layouts) {
          ...
          const pkgRoot = path.join(nodeManagedRoot, "nodoassist");
          ...
          expect(globalInstallArgs("npm", "nodoassist@latest", pkgRoot), layout.name).toEqual([
            "npm",
            "i",
            "-g",
            "nodoassist@latest",
          ...
          expect(
            globalInstallFallbackArgs("npm", "nodoassist@latest", pkgRoot),
            layout.name,
          ).toEqual([
            "npm",
            "i",
  ...
  it("keeps npm self-updates on the running package root when the PATH probe diverges", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "nodoassist-update-ephemeral-probe-" }, async (base) => {
        ...
        const pkgRoot = path.join(nvmRoot, "nodoassist");
  ...
  it("keeps scoped npm self-updates on the running package root", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "nodoassist-update-scoped-probe-" }, async (base) => {
        const nvmPrefix = path.join(base, "home", ".nvm", "versions", "node", "v24.5.0");
        const nvmRoot = path.join(nvmPrefix, "lib", "node_modules");
  ...
  it("keeps the npm probe when the package root is not globally installed", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "nodoassist-update-probe-only-" }, async (base) => {
        const nvmPrefix = path.join(base, "home", ".nvm", "versions", "node", "v24.5.0");
        const nvmRoot = path.join(nvmPrefix, "lib", "node_modules");
        const pkgRoot = path.join(base, "checkout", "node_modules", "nodoassist");
        await fs.mkdir(pkgRoot, { recursive: true });
        ...
        await expect(
          ...
          command: "npm",
          globalRoot: nvmRoot,
          packageRoot: path.join(nvmRoot, "nodoassist"),
        });
      });
    });
  });
  ...
  it("falls back to the running package root when the npm root probe fails", async () => {
    await withMockedPlatform("darwin", async () => {
      await withTempDir({ prefix: "nodoassist-update-probe-failure-" }, async (base) => {
        const globalRoot = path.join(base, "usr", "local", "lib", "node_modules");
        const pkgRoot = path.join(globalRoot, "nodoassist");
        await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "nodoassist-update-npm-missing-bin-" }, async (base) => {
  ...
  it("honors an explicitly selected direct npm node_modules package root", async () => {
    await withTempDir({ prefix: "nodoassist-update-managed-service-root-" }, async (base) => {
      const managedNpmRoot = path.join(base, ".nodoassist", "npm", "node_modules");
      const pkgRoot = path.join(managedNpmRoot, "nodoassist");
      ...
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(otherPnpmRoot, "nodoassist"), { recursive: true });
  ...
  it("preserves bun ownership for direct node_modules package roots", async () => {
    await withTempDir({ prefix: "nodoassist-update-managed-bun-root-" }, async (base) => {
      envSnapshot = captureEnv(["BUN_INSTALL"]);
      process.env.BUN_INSTALL = path.join(base, ".bun");
      const bunRoot = path.join(process.env.BUN_INSTALL, "install", "global", "node_modules");
      const pkgRoot = path.join(bunRoot, "nodoassist");
      const pathNpmRoot = path.join(base, "shell", "lib", "node_modules");
      await fs.mkdir(pkgRoot, { recursive: true });
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    await withMockedWindowsPlatform(async () => {
      await withTempDir({ prefix: "nodoassist-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "nodoassist");
  ...
  it("detects custom pnpm global layouts from the running package root", async () => {
    await withTempDir({ prefix: "nodoassist-update-pnpm-custom-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "nodoassist");
  ...
  it("detects custom pnpm global layouts from virtual-store package roots", async () => {
    await withTempDir({ prefix: "nodoassist-update-pnpm-virtual-root-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(
        ...
        "5",
        ".pnpm",
        "nodoassist@file+..+pack+nodoassist-2026.5.6.tgz",
        "node_modules",
        "nodoassist",
      );
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        command: "pnpm",
        globalRoot: customGlobalRoot,
        packageRoot: path.join(customGlobalRoot, "nodoassist"),
      });
    });
  });

  it("does not infer pnpm ownership without pnpm node_modules metadata", async () => {
    await withTempDir({ prefix: "nodoassist-update-pnpm-shape-only-" }, async (base) => {
      const customGlobalDir = path.join(base, "custom-pnpm");
      const customGlobalRoot = path.join(customGlobalDir, "5", "node_modules");
      const pkgRoot = path.join(customGlobalRoot, "nodoassist");
      const defaultPnpmRoot = path.join(base, "default-pnpm", "5", "node_modules");
      ...
      await expect(
        ...
        packageRoot: path.join(defaultPnpmRoot, "nodoassist"),
      });
    });
  });
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "nodoassist@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "nodoassist@latest",
    ...
    expect(globalInstallArgs("pnpm", "nodoassist@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "nodoassist@latest",
    ]);
    expect(globalInstallArgs("pnpm", "github:nodoassist/nodoassist#release/2026.5.12")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=nodoassist",
      "github:nodoassist/nodoassist#release/2026.5.12",
    ]);
    expect(
      globalInstallArgs("pnpm", "nodoassist@git+https://github.com/openclaw/openclaw.git"),
    ).toEqual([
      "pnpm",
      "add",
      "-g",
      "--allow-build=nodoassist",
      "nodoassist@git+https://github.com/openclaw/openclaw.git",
    ]);
    expect(globalInstallArgs("bun", "nodoassist@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "nodoassist@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "nodoassist@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "nodoassist@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "nodoassist@latest")).toBeNull();
    expect(
      globalInstallArgs(
        { manager: "pnpm", command: "/opt/homebrew/bin/pnpm" },
        "nodoassist@latest",
      ),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "nodoassist@latest"]);
    expect(globalInstallArgs("pnpm", "nodoassist@latest", null, "/opt/pnpm-global")).toEqual([
      "pnpm",
      "add",
      "-g",
      "--global-dir",
  ...
  it("builds npm staged install argv with an explicit prefix", () => {
    expect(globalInstallArgs("npm", "nodoassist@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
    ...
    expect(globalInstallFallbackArgs("npm", "nodoassist@latest", null, "/tmp/stage")).toEqual([
      "npm",
      "i",
      "-g",
      "--prefix",
  ...
  it("resolves npm prefix layouts for normal global roots", () => {
    expect(resolveNpmGlobalPrefixLayoutFromGlobalRoot("/opt/nodoassist/lib/node_modules")).toEqual({
      prefix: "/opt/nodoassist",
      globalRoot: "/opt/nodoassist/lib/node_modules",
      binDir: "/opt/nodoassist/bin",
    });
  ...
  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "nodoassist-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".nodoassist-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".nodoassist-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".nodoassist-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "nodoassist"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".nodoassist-123", ".nodoassist-456"],
      });
      const packageDirStat = await fs.stat(path.join(root, "nodoassist"));
      const markerFileStat = await fs.stat(path.join(root, ".nodoassist-file"));
  ...
  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "nodoassist-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("reports bundled plugin install stages during installed dist verification", async () => {
    await withTempDir({ prefix: "nodoassist-update-global-plugin-stage-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await fs.mkdir(path.join(packageRoot, "dist", "extensions", "brave"), { recursive: true });
      await writePackageDistInventory(packageRoot);

      for (const stageDir of [".nodoassist-install-stage", ".nodoassist-install-stage-retry"]) {
      ...
      await expect(collectInstalledGlobalPackageErrors({ packageRoot })).resolves.toEqual([
        "unexpected packaged dist file dist/extensions/brave/.nodoassist-install-stage-retry/node_modules/typebox/build/compile/code.mjs",
        "unexpected packaged dist file dist/extensions/brave/.nodoassist-install-stage/node_modules/typebox/build/compile/code.mjs",
      ]);
    });
  });

  it("flags global package roots that resolve into source checkouts", async () => {
    await withTempDir({ prefix: "nodoassist-update-global-source-checkout-" }, async (base) => {
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "nodoassist-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "nodoassist-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "nodoassist-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir(
      { prefix: "nodoassist-update-global-legacy-plugin-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot);
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@nodoassist/telegram");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "nodoassist-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "telegram", "@nodoassist/telegram");
        await writePackageDistInventory(packageRoot);
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "nodoassist-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@nodoassist/qa-lab");
        await writePackageDistInventory(packageRoot);
