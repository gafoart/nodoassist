// Runs NodoAssist package update checks, package steps, and restart handoff.
...
const DEFAULT_PACKAGE_NAME = "nodoassist";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR_ENV =
  "NODOASSIST_UPDATE_DEFER_CONFIGURED_PLUGIN_INSTALL_REPAIR";
const UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE_ENV =
  "NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE";
...
const UPDATE_DOCTOR_SERVICE_REPAIR_POLICY_ENV = "NODOASSIST_SERVICE_REPAIR_POLICY";
const EXTERNAL_SERVICE_REPAIR_POLICY_MIN_VERSION = "2026.4.25-beta.1";
const PREFLIGHT_TEMP_PREFIX =
  process.platform === "win32" ? "ocu-pf-" : "nodoassist-update-preflight-";
...
const DEV_PREFLIGHT_LINT_ENV: NodeJS.ProcessEnv = {
  ...
  NODOASSIST_OXLINT_SHARDS_SERIAL: "1",
};
const DEV_PREFLIGHT_LINT_OPT_IN_ENV = "NODOASSIST_UPDATE_PREFLIGHT_LINT";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["nodoassist", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && (await pathsReferToSameLocation(gitRoot, pkgRoot))) {
    ...
    try {
      ...
      const doctorEntry = path.join(gitRoot, "nodoassist.mjs");
      ...
      const doctorStep = await runStep(
        step("nodoassist doctor", doctorArgv, gitRoot, {
