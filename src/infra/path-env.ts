// Builds PATH values for NodoAssist child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current NodoAssist process is on.
  try {
  ...
  // Bundled macOS app: `nodoassist` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "nodoassist");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.NODOASSIST_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin && cwd) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "nodoassist"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `nodoassist`
...
/**
 * Best-effort PATH bootstrap so skills that require the `nodoassist` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureNodoAssistCliOnPath(opts: EnsureNodoAssistPathOpts = {}) {
  if (isTruthyEnvValue(process.env.NODOASSIST_PATH_BOOTSTRAPPED)) {
    return;
  }
