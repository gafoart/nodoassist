// Resolves NodoAssist home and platform-specific config directories.
...
function resolveRawHomeDir(env: NodeJS.ProcessEnv, homedir: () => string): string | undefined {
  const explicitHome = normalize(env.NODOASSIST_HOME);
  if (!explicitHome) {
...
/** Resolves NodoAssist's effective home, honoring NODOASSIST_HOME before OS homes. */
...
/** Resolves the underlying OS user home, ignoring NODOASSIST_HOME overrides. */
...
export function resolveRequiredHomeDir(
  ...
  throw new Error(
    "Unable to resolve an NodoAssist home: set NODOASSIST_HOME, HOME, or USERPROFILE, or run from an existing directory.",
  );
}
...
/** Resolves a user-supplied path against the OS home, ignoring NODOASSIST_HOME. */
