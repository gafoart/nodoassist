...
const { triggerNodoAssistRestart } = await import("./restart.js");
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-nodoassist/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "cnodoassist", `p${stalePidB}`, "cnodoassist-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
  ...
  it("uses explicit port override when provided", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePid}`, "cnodoassist"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerNodoAssistRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    ...
    withEnv(
      ...
      () => {
        ...
        const result = triggerNodoAssistRestart();
        ...
        expect(result).toEqual({
          ...
          tried: [
            ...
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.nodoassist.gateway.plist`,
          ],
        });
      },
    );
      ...
      () => {
        ...
        expect(result).toEqual({
          ...
          tried: [
            `launchctl kickstart -k gui/${uid}/ai.nodoassist.gateway`,
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.nodoassist.gateway.plist`,
            `launchctl kickstart gui/${uid}/ai.nodoassist.gateway`,
          ],
        });
      },
    );
