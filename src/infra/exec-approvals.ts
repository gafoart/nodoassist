...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.nodoassist";
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.NODOASSIST_STATE_DIR?.trim()
    ? `$NODOASSIST_STATE_DIR/${EXEC_APPROVALS_FILE}`
...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "nodoassist" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "nodoassist") {
    return false;
  }
  offset += 1;
