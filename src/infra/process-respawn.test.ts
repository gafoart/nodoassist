...
const triggerNodoAssistRestartMock = vi.hoisted(() => vi.fn());
...
vi.mock("./restart.js", () => ({
  triggerNodoAssistRestart: (...args: unknown[]) => triggerNodoAssistRestartMock(...args),
}));
...
afterEach(() => {
  ...
  spawnMock.mockClear();
  triggerNodoAssistRestartMock.mockClear();
...
function expectLaunchdSupervisedWithoutKickstart(params?: { launchJobLabel?: string }) {
  ...
  process.env.NODOASSIST_LAUNCHD_LABEL = "ai.nodoassist.gateway";
  const result = restartGatewayProcessWithFreshPid();
  expect(result).toEqual({ mode: "supervised" });
  expect(triggerNodoAssistRestartMock).not.toHaveBeenCalled();
  expect(spawnMock).not.toHaveBeenCalled();
}

describe("restartGatewayProcessWithFreshPid", () => {
  it("returns disabled when NODOASSIST_NO_RESPAWN is set", () => {
    process.env.NODOASSIST_NO_RESPAWN = "1";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("disabled");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("keeps NODOASSIST_NO_RESPAWN ahead of inherited supervisor hints", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.NODOASSIST_NO_RESPAWN = "1";
    process.env.LAUNCH_JOB_LABEL = "ai.nodoassist.gateway";
  ...
  it("returns supervised when NodoAssist launchd markers are present on macOS (no kickstart)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart({ launchJobLabel: "ai.nodoassist.gateway" });
  });
  ...
  it("returns supervised for a real gateway XPC launchd job without the injected marker", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.nodoassist.gateway";
  ...
  it("launchd supervisor never returns failed regardless of triggerNodoAssistRestart outcome", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.NODOASSIST_LAUNCHD_LABEL = "ai.nodoassist.gateway";
    // Even if triggerNodoAssistRestart *would* fail, launchd path must not call it.
    triggerNodoAssistRestartMock.mockReturnValue({
      ok: false,
      method: "launchctl",
  ...
  it("does not treat inherited XPC_SERVICE_NAME as launchd supervision", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.nodoassist.mac";
    process.env.NODOASSIST_PROFILE = "mac";
  ...
  it("uses in-process restart on unmanaged Unix so custom supervisors keep the tracked PID", () => {
    delete process.env.NODOASSIST_NO_RESPAWN;
    clearSupervisorHints();
    setPlatform("linux");
  ...
  it("returns supervised when NODOASSIST_LAUNCHD_LABEL is set (stock launchd plist)", () => {
    clearSupervisorHints();
    expectLaunchdSupervisedWithoutKickstart();
  });

  it("returns supervised when NODOASSIST_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.NODOASSIST_SYSTEMD_UNIT = "nodoassist-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("returns supervised when NodoAssist gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.NODOASSIST_SERVICE_MARKER = "nodoassist";
    process.env.NODOASSIST_SERVICE_KIND = "gateway";
    triggerNodoAssistRestartMock.mockReturnValue({ ok: true, method: "schtasks" });
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(triggerNodoAssistRestartMock).toHaveBeenCalledOnce();
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.NODOASSIST_TASK_SCRIPT = "C:\\nodoassist\\node.cmd";
    process.env.NODOASSIST_TASK_SCRIPT_NAME = "node.cmd";
    process.env.NODOASSIST_SERVICE_MARKER = "nodoassist";
    process.env.NODOASSIST_SERVICE_KIND = "node";
...
describe("respawnGatewayProcessForUpdate", () => {
  it("keeps NODOASSIST_NO_RESPAWN semantics for update restarts", () => {
    clearSupervisorHints();
    process.env.NODOASSIST_NO_RESPAWN = "1";

    const result = respawnGatewayProcessForUpdate();

    expect(result).toEqual({ mode: "disabled", detail: "NODOASSIST_NO_RESPAWN" });
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("rewrites a pnpm-versioned NodoAssist entry before detached update respawn", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.execArgv = [];
    process.argv = [
      "/usr/local/bin/node",
      "/app/node_modules/.pnpm/nodoassist@2026.6.5/node_modules/nodoassist/dist/entry.js",
      "gateway",
      "run",
    ];
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      process.execPath,
      ["/app/node_modules/nodoassist/nodoassist.mjs", "gateway", "run"],
      {
        detached: true,
        env: process.env,
        stdio: "inherit",
