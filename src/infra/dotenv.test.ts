...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "NODOASSIST_LIVE_ANTHROPIC_KEYS",
  "NODOASSIST_LIVE_GEMINI_KEY",
  "NODOASSIST_LIVE_OPENAI_KEY",
  "NODOASSIST_GATEWAY_TOKEN",
  "NODOASSIST_GATEWAY_PASSWORD",
  "NODOASSIST_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "NODOASSIST_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "NODOASSIST_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "NODOASSIST_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "NODOASSIST_BUNDLED_SKILLS_DIR=./attacker-skills",
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.nodoassist/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.nodoassist/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        setTestEnvValue("HOME", base);
        const defaultStateDir = path.join(base, ".nodoassist");
        setTestEnvValue("NODOASSIST_STATE_DIR", defaultStateDir);
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "nodoassist", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NODOASSIST_STATE_DIR=./evil-state",
            "NODOASSIST_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.NODOASSIST_STATE_DIR).toBe(stateDir);
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NODOASSIST_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "NODOASSIST_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "NODOASSIST_LIVE_GEMINI_KEY=sk-gemini-live",
            "NODOASSIST_LIVE_OPENAI_KEY=sk-openai-live",
            "NODOASSIST_GATEWAY_TOKEN=attacker-token",
            "NODOASSIST_GATEWAY_PASSWORD=attacker-password",
            "NODOASSIST_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
      ...
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          ...
          [
            ...
            "NODOASSIST_AGENT_DIR=./evil-agent",
            `NODOASSIST_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "NODOASSIST_OAUTH_DIR=./evil-oauth",
  ...
  it("blocks NODOASSIST_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "NODOASSIST_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.NODOASSIST_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.NODOASSIST_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            'NODOASSIST_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );

        delete process.env.NODOASSIST_ALLOW_PLUGIN_INSTALL_OVERRIDES;
        delete process.env.NODOASSIST_PLUGIN_INSTALL_OVERRIDES;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.NODOASSIST_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.NODOASSIST_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "NODOASSIST_PINNED_PYTHON=./attacker-python",
            "NODOASSIST_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.NODOASSIST_PINNED_PYTHON;
        delete process.env.NODOASSIST_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.NODOASSIST_PINNED_PYTHON).toBeUndefined();
        expect(process.env.NODOASSIST_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NODOASSIST_PINNED_PYTHON=/trusted/python",
            "NODOASSIST_PINNED_WRITE_PYTHON=/trusted/write-python",
        ...
        expect(process.env.NODOASSIST_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.NODOASSIST_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "NODOASSIST_LIVE_GEMINI_KEY=sk-gemini-live",
            "NODOASSIST_LIVE_OPENAI_KEY=sk-openai-live",
            "NODOASSIST_GATEWAY_TOKEN=trusted-token",
            "NODOASSIST_GATEWAY_PASSWORD=trusted-password",
            "NODOASSIST_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.NODOASSIST_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.NODOASSIST_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.NODOASSIST_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.NODOASSIST_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.NODOASSIST_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.NODOASSIST_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "NODOASSIST_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks NODOASSIST_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "NODOASSIST_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when NODOASSIST_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("NODOASSIST_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "nodoassist", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.NODOASSIST_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
  ...
  it("blocks workspace .env takeover vars before loading the global fallback", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        expect(process.env.NODOASSIST_STATE_DIR).toBe(stateDir);
        expect(process.env.NODOASSIST_CONFIG_PATH).toBeUndefined();
        expect(process.env.NODOASSIST_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.NODE_OPTIONS).toBeUndefined();
        expect(process.env.NODE_REDIRECT_WARNINGS).toBeUndefined();
        expect(process.env.NODE_REPL_EXTERNAL_MODULE).toBeUndefined();
        expect(process.env.NODE_REPL_HISTORY).toBeUndefined();
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        const plugin: PluginManifestRecord = {
          ...
          manifestPath: "/plugins/runtime-cloud/nodoassist.plugin.json",
          providerAuthEnvVars: {
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "NODOASSIST_MDNS_HOSTNAME",
          "NODOASSIST_SESSION_CACHE_TTL_MS",
          "NODOASSIST_UPDATE_PACKAGE_SPEC",
          ...
          "NODOASSIST_BROWSER_EXECUTABLE_PATH",
          "NODOASSIST_WHATSAPP_WEB_SOCKET_URL",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "NODOASSIST_SKIP_PROVIDERS",
          "NODOASSIST_SKIP_CRON",
          "NODOASSIST_RAW_STREAM",
          "NODOASSIST_RAW_STREAM_PATH",
          "NODOASSIST_CACHE_TRACE",
          "NODOASSIST_CACHE_TRACE_FILE",
          "NODOASSIST_CACHE_TRACE_MESSAGES",
          "NODOASSIST_CACHE_TRACE_PROMPT",
          "NODOASSIST_CACHE_TRACE_SYSTEM",
          "NODOASSIST_SHOW_SECRETS",
          "NODOASSIST_PLUGIN_CATALOG_PATHS",
          "NODOASSIST_MPM_CATALOG_PATHS",
          "NODOASSIST_NODE_EXEC_HOST",
          "NODOASSIST_NODE_EXEC_FALLBACK",
          "NODOASSIST_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("nodoassist/nodoassist");
