...
const fixtureRootTracker = createSuiteTempRootTracker({
  prefix: "nodoassist-npm-managed-root-",
});
...
describe("managed npm root", () => {
  ...
  it("syncs NodoAssist-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads workspace pnpm overrides for managed plugin installs", async () => {
    ...
    await expect(readNodoAssistManagedNpmRootOverrides()).resolves.toEqual(expectedOverrides);
  });
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readNodoAssistManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "managed-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
    ...
    expect(
      resolveManagedNpmRootDependencySpec({
        ...
        resolution: {
          name: "@nodoassist/discord",
          version: "2026.5.2",
          resolvedSpec: "@nodoassist/discord@2026.5.2",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@nodoassist/discord": {
              version: "2026.5.2",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for nodoassist@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            "@nodoassist/discord": "2026.5.2",
            "@nodoassist/voice-call": "2026.5.2",
          },
          devDependencies: {
            fixture: "1.0.0",
          },
  ...
  it("repairs stale managed nodoassist peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "nodoassist", "package.json"),
      `${JSON.stringify({ name: "nodoassist", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nodoassist"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nodoassist.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nodoassist.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootNodoAssistPeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@nodoassist/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.nodoassist).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "nodoassist"));
    for (const binName of ["nodoassist", "nodoassist.cmd", "nodoassist.ps1"]) {
  ...
  it("does not repair the active NodoAssist host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/nodoassist-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "nodoassist", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "nodoassist"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nodoassist"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nodoassist.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "nodoassist.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/nodoassist-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "nodoassist"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/nodoassist"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/nodoassist-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.nodoassist).toBeUndefined();
    for (const binName of ["nodoassist", "nodoassist.cmd", "nodoassist.ps1"]) {
