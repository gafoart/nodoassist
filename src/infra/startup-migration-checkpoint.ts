...
import { withNodoAssistStateStartupMigrationCheckpointDatabase } from "../state/nodoassist-state-db.js";
...
function withStartupMigrationCheckpointDatabase<T>(
  ...
  return withNodoAssistStateStartupMigrationCheckpointDatabase(callback, { env });
}
...
export function acquireStartupMigrationLease(
  ...
  writeStartupMigrationCheckpointDatabase(env, (db) => {
    ...
    if (existing) {
      throw new Error(
        `NodoAssist startup migrations are already running for this state directory; retry after the other gateway finishes or after ${new Date(existing.expiresAt ?? expiresAt).toISOString()}.`,
      );
    }
    executeSqliteQuerySync(
      db,
    ...
    heartbeat: (heartbeatParams = {}) => {
      ...
      writeStartupMigrationCheckpointDatabase(env, (db) => {
        ...
        if (result.numAffectedRows !== 1n) {
          throw new Error(
            "NodoAssist startup migration lease was lost before startup migrations completed; restart the gateway so migrations can run under a fresh lease.",
          );
        }
      });
    },
...
export function recordSuccessfulStartupMigrations(
  ...
  writeStartupMigrationCheckpointDatabase(env, (db) => {
    ...
    if (params.lease) {
      ...
      if (!activeLease) {
        throw new Error(
          "NodoAssist startup migration lease was lost before checkpoint recording; restart the gateway so migrations can run under a fresh lease.",
        );
      }
    }
    executeSqliteQuerySync(
