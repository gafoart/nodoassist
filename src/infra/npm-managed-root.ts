...
import { normalizeOptionalString as readOptionalString } from "@nodoassist/normalization-core/string-coerce";
import { parse as parseYaml } from "yaml";
import { runCommandWithTimeout } from "../process/exec.js";
import { hasErrnoCode } from "./errors.js";
import type { NpmSpecResolution } from "./install-source-utils.js";
import { readJson, readJsonIfExists, writeJson } from "./json-files.js";
import { resolveNodoAssistPackageRootSync } from "./nodoassist-root.js";
...
type ManagedNpmRootNodoAssistMetadata = {
...
type ManagedNpmRootNodoAssistHostState = "none" | "managed-active-host" | "linked-active-host";
...
function buildManagedNodoAssistMetadata(params: {
  current: unknown;
  managedOverrideKeys: string[];
  managedPeerDependencyKeys?: string[];
}): ManagedNpmRootNodoAssistMetadata | undefined {
  const metadata: ManagedNpmRootNodoAssistMetadata = isRecord(params.current)
    ? { ...params.current }
    : {};
...
function applyManagedNpmRootOverrides(params: {
  ...
  for (const key of readManagedOverrideKeys(params.manifest.nodoassist)) {
...
/** Read host NodoAssist pnpm overrides for reuse inside a managed npm root. */
export async function readNodoAssistManagedNpmRootOverrides(params?: {
  argv1?: string;
  cwd?: string;
  moduleUrl?: string;
  ...
  const packageRoot =
    params?.packageRoot ??
    resolveNodoAssistPackageRootSync({
...
export async function upsertManagedNpmRootDependency(params: {
  ...
  const managedDependencyNames = new Set(readManagedPeerDependencyKeys(manifest.nodoassist));
  ...
  const nodoassistMetadata = buildManagedNodoAssistMetadata({
    current: manifest.nodoassist,
    managedOverrideKeys,
  ...
  if (nodoassistMetadata) {
    next.nodoassist = nodoassistMetadata;
  } else {
    delete next.nodoassist;
  }
...
function scrubHostPeerFromLockPackage(value: unknown): boolean {
  ...
  let changed = false;
  if (isRecord(value.peerDependencies) && "nodoassist" in value.peerDependencies) {
    const peerDependencies = { ...value.peerDependencies };
    delete peerDependencies.nodoassist;
  ...
  if (isRecord(value.peerDependenciesMeta) && "nodoassist" in value.peerDependenciesMeta) {
    const peerDependenciesMeta = { ...value.peerDependenciesMeta };
    delete peerDependenciesMeta.nodoassist;
...
function isHostPeerResolutionFailure(
  ...
  return /(^|[^@\w.-])nodoassist(?=$|[@\s:,"'])/i.test(output);
}
...
async function collectNpmResolvedManagedNpmRootPeerDependencyPins(params: {
  ...
  const dependencies = readDependencyRecord(manifest.dependencies);
  const previousManagedPeerDependencies = readManagedPeerDependencyKeys(manifest.nodoassist);
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-managed-peer-plan-"));
  try {
    delete dependencies.nodoassist;
    await writeJson(
    ...
    await copyPathIfExists(
      path.join(params.npmRoot, "_nodoassist-pack-archives"),
      path.join(tempRoot, "_nodoassist-pack-archives"),
    );
...
export async function readManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(path.join(params.npmRoot, "package.json"));
  const dependencies = readDependencyRecord(manifest.dependencies);
  const managedPeerDependencies = readManagedPeerDependencyKeys(manifest.nodoassist).toSorted();
...
export async function restoreManagedNpmRootPeerDependencySnapshot(params: {
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  for (const packageName of readManagedPeerDependencyKeys(manifest.nodoassist)) {
    delete dependencies[packageName];
  }
  Object.assign(dependencies, params.snapshot.dependencies);
  const overrides = readOverrideRecord(manifest.overrides);
  const currentManagedOverrideKeys = readManagedOverrideKeys(manifest.nodoassist);
...
/** Remove stale managed-root nodoassist peer installs while preserving active host links. */
export async function repairManagedNpmRootNodoAssistPeer(params: {
  npmRoot: string;
  packageRoot?: string | null;
  timeoutMs?: number;
  ...
  const activeHostState = await readManagedNpmRootNodoAssistHostState({
    npmRoot: params.npmRoot,
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  const hasManifestDependency = "nodoassist" in dependencies;
  const hasLockDependency = await managedNpmRootLockfileHasNodoAssistPeer(params.npmRoot);
  const hasPackageDir = await pathExists(path.join(params.npmRoot, "node_modules", "nodoassist"));
  ...
  if (preserveActiveHostLink) {
    await scrubManagedNpmRootNodoAssistPeer({
      npmRoot: params.npmRoot,
  ...
  try {
    ...
    if (result.code !== 0) {
      params.logger?.warn?.(
        `npm ${hasManifestDependency ? "uninstall nodoassist" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${result.stderr.trim() || result.stdout.trim()}`,
      );
    }
  } catch (error) {
    params.logger?.warn?.(
      `npm ${hasManifestDependency ? "uninstall nodoassist" : "prune"} failed while repairing managed npm root; falling back to direct cleanup: ${String(error)}`,
    );
  }

  await scrubManagedNpmRootNodoAssistPeer({ npmRoot: params.npmRoot });
  return true;
}

async function readManagedNpmRootNodoAssistHostState(params: {
  npmRoot: string;
  packageRoot?: string | null;
}): Promise<ManagedNpmRootNodoAssistHostState> {
  const packageRoot =
  ...
  const managedNodoAssistPackageDir = path.join(params.npmRoot, "node_modules", "nodoassist");
  const [hostPackageRoot, managedPackageRoot, managedPackageStat] = await Promise.all([
    realpathIfExists(packageRoot),
    realpathIfExists(managedNodoAssistPackageDir),
    lstatIfExists(managedNodoAssistPackageDir),
  ]);
...
async function managedNpmRootLockfileHasNodoAssistPeer(npmRoot: string): Promise<boolean> {
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (
        ...
        "nodoassist" in rootPackage.dependencies
      ) {
        return true;
      }
      if ("node_modules/nodoassist" in parsed.packages) {
        return true;
      }
    }
    return isRecord(parsed.dependencies) && "nodoassist" in parsed.dependencies;
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return false;
    }
...
async function scrubManagedNpmRootNodoAssistPeer(params: {
  npmRoot: string;
  ...
  const manifest = await readManagedNpmRootManifest(manifestPath);
  const dependencies = readDependencyRecord(manifest.dependencies);
  if ("nodoassist" in dependencies) {
    const { nodoassist: _removed, ...nextDependencies } = dependencies;
    await fs.writeFile(
      manifestPath,
  ...
  try {
    ...
    if (isRecord(parsed.packages)) {
      ...
      if (isRecord(rootPackage) && isRecord(rootPackage.dependencies)) {
        ...
        if ("nodoassist" in dependenciesValue) {
          delete dependenciesValue.nodoassist;
          parsed.packages[""] = { ...rootPackage, dependencies: dependenciesValue };
          lockChanged = true;
        }
      }
      if ("node_modules/nodoassist" in parsed.packages) {
        delete parsed.packages["node_modules/nodoassist"];
        lockChanged = true;
      }
    }
    if (isRecord(parsed.dependencies) && "nodoassist" in parsed.dependencies) {
      const dependenciesLocal = { ...parsed.dependencies };
      delete dependenciesLocal.nodoassist;
  ...
  const nodoassistPackageDir = path.join(params.npmRoot, "node_modules", "nodoassist");
  if (!params.preservePackageDir && (await pathExists(nodoassistPackageDir))) {
    await fs.rm(nodoassistPackageDir, { recursive: true, force: true });
  }
  const binDir = path.join(params.npmRoot, "node_modules", ".bin");
  await Promise.all(
    ["nodoassist", "nodoassist.cmd", "nodoassist.ps1"].map((binName) =>
