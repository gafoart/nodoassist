...
describe("clawhub helpers", () => {
  ...
  afterEach(() => {
    delete process.env.NODOASSIST_CLAWHUB_URL;
    delete process.env.NODOASSIST_CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_TOKEN;
    delete process.env.CLAWHUB_AUTH_TOKEN;
    delete process.env.NODOASSIST_CLAWHUB_CONFIG_PATH;
  ...
  it("treats NodoAssist release correction versions as stable plugin API hosts", () => {
  ...
  it("checks min gateway versions with loose host labels", () => {
    ...
    expect(satisfiesGatewayMinimum("NodoAssist 2026.3.22", "2026.3.0")).toBe(true);
  ...
  it("resolves ClawHub auth token from config.json", async () => {
    await withTempDir({ prefix: "nodoassist-clawhub-config-" }, async (configRoot) => {
      const configPath = path.join(configRoot, "clawhub", "config.json");
      process.env.NODOASSIST_CLAWHUB_CONFIG_PATH = configPath;
  ...
  it("resolves ClawHub auth token from the legacy config path override", async () => {
    await withTempDir({ prefix: "nodoassist-clawdhub-config-" }, async (configRoot) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "nodoassist-clawhub-home-" }, async (fakeHome) => {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "nodoassist-clawhub-home-" }, async (fakeHome) => {
        await withTempDir({ prefix: "nodoassist-clawhub-xdg-" }, async (xdgRoot) => {
  ...
  it("injects resolved auth token into ClawHub requests", async () => {
    process.env.NODOASSIST_CLAWHUB_TOKEN = "env-token-123";
  ...
  it("preserves the configured ClawHub base URL path prefix", async () => {
    process.env.NODOASSIST_CLAWHUB_URL = "https://internal.example.com/clawhub";
    let requestedUrl = "";
  ...
  it("fetches typed package artifact resolver reports", async () => {
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40nodoassist%2Fdiagnostics-otel/versions/2026.3.22/artifact",
    );
  });
  ...
  it("fetches typed package security reports", async () => {
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40nodoassist%2Fdiagnostics-otel/versions/2026.3.22/security",
    );
  });
  ...
  it("annotates 429 errors with the reset hint and a sign-in hint when unauthenticated", async () => {
    process.env.NODOASSIST_CLAWHUB_CONFIG_PATH = path.join(
      os.tmpdir(),
      "nodoassist-no-clawhub-config",
    );
    await expect(
      searchClawHubSkills({
        query: "calendar",
