...
import { bundledPluginFile } from "nodoassist/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "nodoassist.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "nodoassist-watch-node-" }, async (cwd) => {
      ...
      expect(spawnEnv.NODOASSIST_WATCH_MODE).toBe("1");
      expect(spawnEnv.NODOASSIST_WATCH_SESSION).toBe("1700000000000-4242");
      expect(spawnEnv.NODOASSIST_NO_RESPAWN).toBe("1");
      expect(spawnEnv.NODOASSIST_WATCH_COMMAND).toBe("gateway --force");
      expect(spawnEnv.NODOASSIST_TRACE_SYNC_IO).toBeUndefined();
  ...
  it("preserves explicit sync I/O trace overrides for gateway watch", async () => {
    ...
    await withTempDir({ prefix: "nodoassist-watch-node-" }, async (cwd) => {
      const runPromise = runWatch({
        ...
        cwd,
        createWatcher,
        env: { NODOASSIST_TRACE_SYNC_IO: "0" },
        lockDisabled: true,
        process: fakeProcess,
        spawn,
      });
      ...
      expect(spawnCall[0]).toBe("/usr/local/bin/node");
      expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
      expect(requireSpawnEnv(spawn, 0).NODOASSIST_TRACE_SYNC_IO).toBe("0");
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { NODOASSIST_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
    const spawnEnv = requireSpawnEnv(spawn, 0);
    expect(spawnEnv.LAUNCH_JOB_LABEL).toBe("ai.nodoassist.gateway");
    expect(spawnEnv.NODOASSIST_NO_RESPAWN).toBe("1");
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/nodoassist/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/nodoassist/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[nodoassist] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[nodoassist] Invalid package config: /tmp/nodoassist/.pnpm/chokidar/package.json"],
        ["[nodoassist] This usually means a file in node_modules is empty or truncated."],
        ["[nodoassist] Recommended recovery:"],
        ["[nodoassist]   rm -rf node_modules"],
        ["[nodoassist]   pnpm store prune"],
        ["[nodoassist]   pnpm install"],
        [""],
        ["[nodoassist] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "nodoassist-watch-node-lock-" }, async (cwd) => {
