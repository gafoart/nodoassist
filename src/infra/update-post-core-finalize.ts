...
// update.
//
// `runGatewayUpdate` (git mode) runs `nodoassist doctor --fix` with
// `NODOASSIST_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `nodoassist update` CLI resumes
...
// binary's hidden `nodoassist update finalize` entrypoint — the designed
...
// re-enters the full `nodoassist update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "nodoassist update finalize",
