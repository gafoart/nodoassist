...
function resolveLegacyDebugProxyCapturePaths(
  ...
  return {
    sourcePath: env.NODOASSIST_DEBUG_PROXY_DB_PATH?.trim() || path.join(rootDir, "capture.sqlite"),
    blobDir: env.NODOASSIST_DEBUG_PROXY_BLOB_DIR?.trim() || path.join(rootDir, "blobs"),
  };
}
...
export function detectLegacyDebugProxyCaptureSidecar(
  ...
  if (
    ...
    path.resolve(resolveNodoAssistStateSqlitePath({ ...env, NODOASSIST_STATE_DIR: stateDir }))
  ) {
