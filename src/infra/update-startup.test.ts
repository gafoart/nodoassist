...
const {
  ...
  startManagedServiceUpdateHandoffMock: vi.fn(async () => ({
    ...
    pid: 12345,
    command: "nodoassist update --yes --channel beta --timeout 2700",
    logPath: "/tmp/nodoassist-handoff.log",
  })),
}));
...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    testState = await createNodoAssistTestState({
      layout: "state-only",
      prefix: "nodoassist-update-check-suite-",
      env: {
        NODOASSIST_NO_AUTO_UPDATE: undefined,
        NODOASSIST_SERVICE_KIND: undefined,
        NODOASSIST_SERVICE_MARKER: undefined,
        NODOASSIST_GATEWAY_SERVICE_PID: undefined,
        NODOASSIST_LAUNCHD_LABEL: undefined,
        NODOASSIST_SYSTEMD_UNIT: undefined,
        NODOASSIST_WINDOWS_TASK_NAME: undefined,
    ...
    if (!loaded) {
      ({ resolveNodoAssistPackageRoot } = await import("./nodoassist-root.js"));
    ...
    vi.mocked(resolveNodoAssistPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveNodoAssistPackageRoot).mockResolvedValue("/opt/nodoassist");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("nodoassist update")}`,
    );
  ...
  it("discovers and deduplicates an exact extended-stable update without auto-applying", async () => {
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (extended-stable): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("nodoassist update")}`,
    );
  ...
  it("does no extended-stable hint or auto work when checkOnStart is false", async () => {
    ...
    expect(resolveNodoAssistPackageRoot).not.toHaveBeenCalled();
    expect(checkUpdateStatus).not.toHaveBeenCalled();
    expect(resolveNpmChannelTag).not.toHaveBeenCalled();
    expect(runAutoUpdate).not.toHaveBeenCalled();
    expect(startManagedServiceUpdateHandoffMock).not.toHaveBeenCalled();
  ...
  it("honors NODOASSIST_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.NODOASSIST_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by NODOASSIST_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by NODOASSIST_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/nodoassist/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(options.env).toEqual({ NODOASSIST_AUTO_UPDATE: "1" });
  });
