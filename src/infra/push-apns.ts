...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open NodoAssist to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "nodoassist.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.NODOASSIST_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.NODOASSIST_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.NODOASSIST_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.NODOASSIST_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.NODOASSIST_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set NODOASSIST_APNS_PRIVATE_KEY_P8 or NODOASSIST_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading NODOASSIST_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
