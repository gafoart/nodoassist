...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.NODOASSIST_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*nodoassist-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "nodoassist";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "nodoassist");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "nodoassist-gw");
  ...
  records.push(`_nodoassist-gw._tcp IN PTR ${instanceLabel}._nodoassist-gw._tcp`);
  records.push(`${instanceLabel}._nodoassist-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._nodoassist-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; nodoassist-content-hash: ${contentHash}\n${contentBody}`;
}
