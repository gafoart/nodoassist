...
vi.mock("./nodoassist-root.js", () => ({
  resolveNodoAssistPackageRoot: vi.fn(async () => null),
  resolveNodoAssistPackageRootSync: vi.fn(() => null),
}));
...
let nodoassistRoot: typeof import("./nodoassist-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    nodoassistRoot = await import("./nodoassist-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/nodoassist");
    const wrapperArgv1 = abs("fixtures/bin/nodoassist");
  ...
  it("uses resolveNodoAssistPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/nodoassist");
    (
      nodoassistRoot.resolveNodoAssistPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/nodoassist"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "nodoassist" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "nodoassist.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-nodoassist");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", () => {
    const pkgRoot = abs("fixtures/nodoassist-bundle");
    (
      nodoassistRoot.resolveNodoAssistPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/NodoAssist.app/Contents/MacOS/NodoAssist");
    const bundledUiDir = abs("fixtures/NodoAssist.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/nodoassist-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
