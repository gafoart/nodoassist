...
describe("renderQrTerminal", () => {
  ...
  it("delegates terminal rendering to qrcode", async () => {
    await expect(renderQrTerminal("nodoassist")).resolves.toBe("ASCII-QR");
    expect(toString).toHaveBeenCalledWith("nodoassist", {
      small: false,
      type: "terminal",
    });
  });
  ...
  it("renders compact QR output without qrcode terminal small mode", async () => {
    const rendered = await renderQrTerminal("nodoassist", { small: true });
    expect(rendered).toContain("▄");
    expect(create).toHaveBeenCalledWith("nodoassist");
