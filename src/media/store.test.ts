...
describe("media store", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("nodoassist-test-home-");
    home = tempHome.home;
  ...
  it.each([
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal17, homeLocal4) => {
          ...
          expect(path.normalize(dir)).toContain(`${path.sep}.nodoassist${path.sep}media`);
    ...
    },
    {
      ...
      run: async () => {
        await withTempStore(async (storeLocal9, homeInner) => {
          ...
          const targetDir = path.join(homeInner, ".nodoassist", "media", "oversized-stream");
  ...
  it("prefers header mime extension when sniffed mime lacks mapping", async () => {
    await withTempStore(async (_store, homeLocal) => {
      vi.doMock("@nodoassist/media-core/mime", async () => {
        const actual = await vi.importActual<typeof import("@nodoassist/media-core/mime")>(
          "@nodoassist/media-core/mime",
        );
        return {
          ...actual,
      ...
      try {
        ...
        vi.doUnmock("@nodoassist/media-core/mime");
      }
    });
  });
