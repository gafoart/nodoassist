...
describe("renderQrPngBase64", () => {
  const tmpRoot = path.join(os.tmpdir(), "nodoassist-qr-image-tests");
  ...
  it("delegates PNG rendering to qrcode", async () => {
    await expect(renderQrPngBase64("nodoassist", { scale: 8, marginModules: 2 })).resolves.toBe(
      MOCK_PNG_BASE64,
    );
    expect(toDataURL).toHaveBeenCalledWith("nodoassist", {
      margin: 2,
      scale: 8,
      type: "image/png",
    });
  ...
  it("uses the default PNG rendering options", async () => {
    await renderQrPngBase64("nodoassist");
    expect(toDataURL).toHaveBeenCalledWith("nodoassist", {
      margin: 4,
      scale: 6,
      type: "image/png",
  ...
  it("floors finite PNG rendering options before delegating", async () => {
    await renderQrPngBase64("nodoassist", { scale: 8.9, marginModules: 2.9 });
    expect(toDataURL).toHaveBeenCalledWith("nodoassist", {
      margin: 2,
      scale: 8,
      type: "image/png",
  ...
  it.each([
    ...
    await expect(renderQrPngBase64("nodoassist", { scale, marginModules })).rejects.toThrow(
      message,
    );
  ...
  it("rejects non-PNG qrcode data URLs", async () => {
    ...
    await expect(renderQrPngBase64("nodoassist")).rejects.toThrow(
  ...
  it("formats QR PNG data URLs", async () => {
    ...
    await expect(renderQrPngDataUrl("nodoassist")).resolves.toBe(
  ...
  it("writes QR PNGs to a scoped temp file", async () => {
    ...
    const result = await writeQrPngTempFile("nodoassist", {
      tmpRoot,
      dirPrefix: "pair-",
