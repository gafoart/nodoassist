...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ NODOASSIST_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "nodoassist-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "nodoassist-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "nodoassist-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "nodoassist-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "nodoassist-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "nodoassist-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps the config-dir media cache root when state and config paths differ", () => {
    const stateDir = path.join("/tmp", "nodoassist-legacy-state");
    const configDir = path.join("/tmp", "nodoassist-current-config");
