...
function resolveNodoAssistHooks(
...
async function installHookPackageFromDir(
  ...
  const hookManifest = resolveNodoAssistHooks(manifest);
  if (!hookManifest.ok) {
    return hookManifest;
  }
  ...
  for (const entry of hookEntries) {
    ...
    if (!runtime.isPathInside(params.packageDir, hookDir)) {
      return {
        ok: false,
        error: `nodoassist.hooks entry escapes package directory: ${entry}`,
      };
    }
    ...
    if (
      ...
      return {
        ok: false,
        error: `nodoassist.hooks entry resolves outside package directory: ${entry}`,
      };
    }
...
export async function installHooksFromNpmSpec(
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "nodoassist-hook-pack-",
    spec,
    timeoutMs,
