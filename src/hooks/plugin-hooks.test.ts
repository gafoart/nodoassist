...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "nodoassist-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.NODOASSIST_BUNDLED_HOOKS_DIR;
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.NODOASSIST_BUNDLED_HOOKS_DIR;
    } else {
      process.env.NODOASSIST_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".nodoassist", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): NodoAssistConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("nodoassist-plugin");
  ...
  it("does not treat Claude hooks.json bundles as NodoAssist hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".nodoassist", "extensions", "claude-bundle");
