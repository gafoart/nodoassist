...
export function hasConfiguredInternalHooks(config: NodoAssistConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: NodoAssistConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
