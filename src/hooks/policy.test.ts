...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "nodoassist-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "nodoassist-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "nodoassist-bundled");
      const plugin = makeHookEntry("shared", "nodoassist-plugin");
      const managed = makeHookEntry("shared", "nodoassist-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("nodoassist-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "nodoassist-managed");
      const workspace = makeHookEntry("shared", "nodoassist-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "nodoassist-workspace");
      const second = makeHookEntry("shared", "nodoassist-workspace");
      second.hook.handlerPath = "/tmp/nodoassist-workspace/shared/handler-2.js";
