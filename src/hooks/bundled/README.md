...
This directory contains hooks that ship with NodoAssist. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.nodoassist/workspace`)
...
```bash
nodoassist hooks enable bootstrap-extra-files
```
...
**Output**: `~/.nodoassist/logs/commands.log`

**Enable**:

```bash
nodoassist hooks enable command-logger
```
...
```bash
nodoassist hooks enable boot-md
```
...
- **Managed hooks**: `~/.nodoassist/hooks/` (shared across workspaces)
...
```bash
nodoassist hooks enable session-memory
nodoassist hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.nodoassist/nodoassist.json`:
...
2. Restart gateway: `pkill -9 -f 'nodoassist.*gateway' && pnpm nodoassist gateway`
3. Enable the hook: `nodoassist hooks enable my-hook`
