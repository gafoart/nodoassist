...
2. **Appends to log file** - Writes a JSON line to `~/.nodoassist/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.nodoassist/logs/commands.log ~/.nodoassist/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/nodoassist`:
   ```
   /home/username/.nodoassist/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.nodoassist/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.nodoassist/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.nodoassist/logs/commands.log | jq .
```
