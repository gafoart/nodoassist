...
// Avoid calling the embedded NodoAssist agent (global command lane); keep this unit test deterministic.
...
beforeAll(async () => {
  ...
  suiteWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-session-memory-"));
});
...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => NodoAssistConfig;
  action?: "new" | "reset";
...
describe("session-memory hook", () => {
  ...
  it("keeps sibling home-prefix paths intact in completion logs", async () => {
    ...
    await withEnvAsync(
      { HOME: fakeHome, USERPROFILE: fakeHome, NODOASSIST_HOME: undefined },
      async () => {
