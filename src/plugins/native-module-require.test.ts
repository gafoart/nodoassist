...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-native-require-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("tryNativeRequireJavaScriptModule", () => {
  ...
  it("declines missing dependency errors when source-transform fallback is available", () => {
    const dir = makeTempDir();
    const modulePath = path.join(dir, "plugin.cjs");
    fs.writeFileSync(modulePath, 'require("nodoassist/plugin-sdk");\n', "utf8");
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  allowWindows: true,",
        `  aliasMap: { "nodoassist/plugin-sdk/channel-outbound": ${JSON.stringify(sdkPath)} },`,
        "});",
        "if (!result.ok) {",
