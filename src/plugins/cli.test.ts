...
const mocks = vi.hoisted(() => ({
  ...
  loadNodoAssistPluginCliRegistry: vi.fn(),
  loadNodoAssistPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadNodoAssistPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadNodoAssistPluginCliRegistry(...args),
  loadNodoAssistPlugins: (...args: unknown[]) => mocks.loadNodoAssistPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  ...
  autoEnabledConfig: NodoAssistConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadNodoAssistPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadNodoAssistPluginCliRegistry.mockReset();
    mocks.loadNodoAssistPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadNodoAssistPlugins.mockReset();
    mocks.loadNodoAssistPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as NodoAssistConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as NodoAssistConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as NodoAssistConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as NodoAssistConfig);

    const loadOptions = getMockCallObject(mocks.loadNodoAssistPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadNodoAssistPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as NodoAssistConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as NodoAssistConfig);

    expect(mocks.loadNodoAssistPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as NodoAssistConfig;
    const configB = { plugins: {} } as NodoAssistConfig;
    const envA = { NODOASSIST_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { NODOASSIST_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadNodoAssistPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadNodoAssistPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadNodoAssistPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadNodoAssistPluginCliRegistry.mockImplementationOnce(
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as NodoAssistConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadNodoAssistPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as NodoAssistConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadNodoAssistPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadNodoAssistPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadNodoAssistPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadNodoAssistPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadNodoAssistPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadNodoAssistPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as NodoAssistConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
