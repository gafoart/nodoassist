...
import type { NodoAssistStateDatabaseOptions } from "../state/nodoassist-state-db.js";
...
function resolveStoreEnv(options: InstalledPluginIndexStoreOptions): NodeJS.ProcessEnv {
  return options.stateDir
    ? { ...(options.env ?? process.env), NODOASSIST_STATE_DIR: options.stateDir }
...
export function resolveInstalledPluginIndexStorePath(
  ...
  return resolveNodoAssistStateSqlitePath(resolveStoreEnv(options));
}
