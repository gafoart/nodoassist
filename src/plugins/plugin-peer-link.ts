...
export type NodoAssistPeerLinkAuditIssue = {
  packageName: string;
  packageDir: string;
  reason: string;
};
...
type AuditManagedNpmRootResult = {
  checked: number;
  broken: number;
  issues: NodoAssistPeerLinkAuditIssue[];
};

type NodoAssistPeerLinkResult = "linked" | "skipped" | "unchanged";
...
async function auditNodoAssistPeerDependency(params: {
  hostRoot: string;
  packageDir: string;
  npmRoot?: string;
  packageName?: string;
}): Promise<NodoAssistPeerLinkAuditIssue | null> {
  const packageName =
    params.packageName ??
    (params.npmRoot
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return {
        packageName,
        packageDir: params.packageDir,
        reason: `missing ${path.join(nodeModulesDir, "nodoassist")}`,
      };
    }
    throw error;
  }

  const linkPath = path.join(nodeModulesDir, "nodoassist");
...
export async function auditNodoAssistPeerDependencyLink(params: {
  packageDir: string;
  packageName?: string;
}): Promise<NodoAssistPeerLinkAuditIssue | null> {
  ...
  if (!hostRoot) {
    return {
      packageName,
      packageDir: params.packageDir,
      reason: "could not locate nodoassist package root",
    };
  }
  return await auditNodoAssistPeerDependency({
    hostRoot,
    packageDir: params.packageDir,
    packageName,
  });
...
async function ensureRealNodeModulesDir(params: {
  ...
  try {
    ...
    if (!existing.isDirectory() || existing.isSymbolicLink()) {
      params.logger.warn?.(
        `Skipping nodoassist peerDependency link because ${nodeModulesDir} is not a real directory.`,
      );
      return null;
    }
...
async function linkNodoAssistPeerDependency(params: {
  hostRoot: string;
  installedDir: string;
  peerName: string;
  logger: PluginPeerLinkLogger;
}): Promise<NodoAssistPeerLinkResult> {
  ...
  try {
    ...
    if (existing) {
      if (!existing.isSymbolicLink()) {
        if (params.peerName === "nodoassist" && existing.isDirectory()) {
          const existingPackageName = await readPackageName(linkPath);
          if (existingPackageName === "nodoassist") {
        ...
        params.logger.warn?.(
          `Skipping nodoassist peerDependency link because ${linkPath} already exists and is not a symlink.`,
        );
        return "skipped";
      }
...
/**
 * Symlink the host nodoassist package for plugins that declare it as a peer.
 ...
 */
export async function linkNodoAssistPeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginPeerLinkLogger;
}): Promise<{ repaired: number; skipped: number }> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "nodoassist");
  if (peers.length === 0) {
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate nodoassist package root to symlink peerDependencies; plugin may fail to resolve nodoassist at runtime.",
    );
  ...
  for (const peerName of peers) {
    const result = await linkNodoAssistPeerDependency({
      hostRoot,
      installedDir: params.installedDir,
      peerName,
      logger: params.logger,
...
export async function relinkNodoAssistPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  logger: PluginPeerLinkLogger;
  ...
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    ...
    checked += 1;
    const result = await linkNodoAssistPeerDependencies({
...
export async function auditNodoAssistPeerDependenciesInManagedNpmRoot(params: {
  npmRoot: string;
  ...
  let checked = 0;
  const issues: NodoAssistPeerLinkAuditIssue[] = [];
  for (const packageDir of await listManagedNpmRootPackageDirs(params.npmRoot)) {
    const peerDependencies = await readPackagePeerDependencies(packageDir);
    if (!Object.hasOwn(peerDependencies, "nodoassist")) {
      continue;
    ...
    checked += 1;
    const issue = await auditNodoAssistPeerDependency({
      hostRoot,
      npmRoot: params.npmRoot,
      packageDir,
    });
