...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "nodoassist-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): NodoAssistConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: NodoAssistConfig, pluginId: string): Record<string, unknown> {
