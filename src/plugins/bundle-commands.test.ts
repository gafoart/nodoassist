...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".nodoassist", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const homeDir = await createTempDir("nodoassist-bundle-commands-home-");
    const workspaceDir = await createTempDir("nodoassist-bundle-commands-workspace-");
    await withEnvAsync(
      {
        HOME: homeDir,
        USERPROFILE: homeDir,
