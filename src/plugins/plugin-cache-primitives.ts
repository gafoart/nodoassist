...
export type ConfigScopedRuntimeCache<T> = WeakMap<NodoAssistConfig, Map<string, T>>;
...
export type ConfigScopedPromiseLoader<T> = {
  load(config?: NodoAssistConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: NodoAssistConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<NodoAssistConfig, Promise<T>>();

  const createPromise = (config?: NodoAssistConfig): Promise<T> => {
  ...
  return {
    async load(config?: NodoAssistConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<NodoAssistConfig, Promise<T>>();
    },
  };
}
