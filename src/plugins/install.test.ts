...
vi.mock("../infra/nodoassist-root.js", () => ({
  resolveNodoAssistPackageRootSync: vi.fn(),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("nodoassist-plugin-install");
...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      nodoassist: { extensions: ["./dist/index.js"] },
...
process.stdin.on("end", () => {
  fs.appendFileSync(process.env.NODOASSIST_POLICY_LOG, input + "\\n");
...
function configWithInstallPolicy(scriptPath: string, logPath: string): NodoAssistConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
        exec: {
          ...
          args: [scriptPath],
          env: { NODOASSIST_POLICY_LOG: logPath },
...
function setPluginMinHostVersion(pluginDir: string, minHostVersion: string) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    nodoassist?: { install?: Record<string, unknown> };
  };
  manifest.nodoassist = {
    ...manifest.nodoassist,
    install: {
      ...manifest.nodoassist?.install,
      minHostVersion,
    },
  };
  fs.writeFileSync(packageJsonPath, JSON.stringify(manifest), "utf-8");
...
function setPluginPackageCompatibility(pluginDir: string, pluginApiRange: unknown) {
  const packageJsonPath = path.join(pluginDir, "package.json");
  const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
    nodoassist?: { compat?: Record<string, unknown> };
  };
  manifest.nodoassist = {
    ...manifest.nodoassist,
    compat: {
      ...manifest.nodoassist?.compat,
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "nodoassist.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@nodoassist/cognee-nodoassist",
      version: "0.0.1",
      nodoassist: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "nodoassist.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(commandOptions.cwd).toContain(".nodoassist-install-stage-");
  });
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@nodoassist/voice-call" });
  ...
  it("rejects native plugin zip archives without nodoassist.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid nodoassist.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@nodoassist/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without nodoassist.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@nodoassist/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("nodoassist.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_NODOASSIST_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when nodoassist.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@nodoassist/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing nodoassist.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_NODOASSIST_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without nodoassist.extensions");
  });

  it("rejects package installs when nodoassist.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_NODOASSIST_EXTENSIONS);
  ...
  it("allows missing TypeScript source entries when an inferred built runtime entry exists", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        nodoassist: { extensions: ["./src/index.ts"] },
      }),
    );
    fs.writeFileSync(path.join(pluginDir, "dist", "index.js"), "export {};\n");
  ...
  it("rejects package installs when nodoassist.extensions contains a blank entry", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_NODOASSIST_EXTENSIONS);
      expect(result.error).toContain("nodoassist.extensions[1]");
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_NODOASSIST_EXTENSIONS);
      expect(result.error).toContain("nodoassist.runtimeExtensions[0]");
      expect(result.error).toContain("non-empty string");
    }
  });
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        nodoassist: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("rejects package installs when an extension entry is a hardlinked alias", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        nodoassist: { extensions: ["./escape.js"] },
      }),
    );
  ...
  it("allows declared package entrypoints with dangerous code under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        nodoassist: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
  ...
  it("does not flag the real qa-matrix plugin as dangerous install code", async () => {
    ...
    vi.mocked(resolveNodoAssistPackageRootSync).mockReturnValue(process.cwd());
    ...
    const scanResult = await installSecurityScan.scanPackageInstallSource({
      ...
      pluginId: "qa-matrix",
      packageName: "@nodoassist/qa-matrix",
  ...
  it("allows extension entry files in hidden directories without built-in scanner warnings", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        nodoassist: { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("emits a redacted security event after installing a plugin directory", async () => {
    ...
    expect(captured.events[0]).toMatchObject({
      ...
      severity: "medium",
      actor: { kind: "operator" },
      target: { kind: "plugin", name: "@nodoassist/test-plugin" },
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.nodoassist).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires NodoAssist >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json nodoassist.install.minHostVersion"],
    },
    {
  ...
  it("rejects plugins whose package plugin API metadata is malformed", async () => {
    ...
    expectFailedInstallResult({
      ...
      messageIncludes: ["nodoassist.compat.pluginApi", "must be a string"],
    });
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("checks package plugin API before current-host extension shape validation", async () => {
    ...
    if (!result.ok) {
      expect(result.error).not.toContain("nodoassist.extensions");
    }
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("rejects bundle package installs whose package plugin API range is newer than the current host", async () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@nodoassist/future-bundle",
        version: "2026.5.27",
        nodoassist: { compat: { pluginApi: ">=2026.5.27" } },
      }),
      "utf-8",
    );
  ...
  it("allows plugins when a beta host is on the package plugin API floor", async () => {
    ...
    expect(result.pluginId).toBe("@nodoassist/test-plugin");
  });

  it("uses nodoassist.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@nodoassist/cognee-nodoassist"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@nodoassist/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkNodoAssistPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveNodoAssistPackageRootSync);
  ...
  it("creates a node_modules/nodoassist symlink when peerDependencies declares nodoassist", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    const run = vi.mocked(runCommandWithTimeout);
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { nodoassist: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "nodoassist");
  ...
  it("keeps the nodoassist peer symlink when a local plugin already has dependencies", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { nodoassist: "*" }, { "is-number": "7.0.0" });
  ...
  it("replaces a copied local nodoassist package with the host peer symlink", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    const fakeHostRoot = suiteTempRootTracker.makeTempDir();
    resolveRootMock.mockReturnValue(fakeHostRoot);

    writePluginWithPeerDeps(pluginDir, { nodoassist: "*" });
    fs.mkdirSync(path.join(pluginDir, "node_modules", "nodoassist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "node_modules", "nodoassist", "package.json"),
      JSON.stringify({ name: "nodoassist", version: "2026.5.31" }),
      "utf-8",
    );
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "nodoassist");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "nodoassist");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("rejects when resolveNodoAssistPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expectWarningIncludes(warnings, "Could not locate nodoassist package root");
  });
});
