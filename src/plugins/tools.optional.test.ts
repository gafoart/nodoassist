...
vi.mock("./loader.js", () => ({
  loadNodoAssistPlugins: (params: unknown) => loadNodoAssistPluginsMock(params),
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadNodoAssistPluginsMock.mockReturnValue(registry);
...
function expectLoaderSelectedOnlyPluginIds(expectedPluginIds: readonly string[]) {
  const selectedPluginIds = loadNodoAssistPluginsMock.mock.calls.map(
...
describe("resolvePluginTools optional tools", () => {
  ...
  beforeEach(() => {
    ...
    resolveRuntimePluginRegistryMock.mockImplementation((params) =>
      loadNodoAssistPluginsMock(params),
    );
  ...
  it("does not load plugin-owned tools whose manifest metadata has no available signal", () => {
    ...
    loadNodoAssistPluginsMock.mockImplementation((params) =>
  ...
  it("loads manifest-gated tools when a named account supplies required config", () => {
    ...
    loadNodoAssistPluginsMock.mockReturnValue(
      createToolRegistry([
        {
          pluginId: "feishu",
          optional: false,
  ...
  it("does not reuse a partial active registry for wildcard-selected plugin tools", () => {
    ...
    loadNodoAssistPluginsMock.mockReturnValue(fullRegistry);
    ...
    const loaderParams = mockCallParams(loadNodoAssistPluginsMock) as {
      activate?: unknown;
      cache?: unknown;
  ...
  it("uses the fresh cold-loaded registry for diagnostics when partial active registries remain incomplete", () => {
    ...
    loadNodoAssistPluginsMock.mockReturnValue(freshRegistry);
  ...
  it("does not reuse a pinned gateway registry for manifest-unavailable tools", () => {
    ...
    loadNodoAssistPluginsMock.mockReturnValue({ tools: [], diagnostics: [] });
    ...
    const config = {
      ...
      secrets: {
        providers: {
          vault: {
            source: "file",
            path: "/tmp/nodoassist-secrets.json",
            mode: "json",
          },
        },
      },
  ...
  it("does not cold-load unrelated manifest-optional plugins when alsoAllow opts into one optional tool", () => {
    ...
    loadNodoAssistPluginsMock.mockReturnValue(createToolRegistry([explicitOptionalEntry]));
    installToolManifestSnapshots({
      config,
      plugins: [
        {
  ...
  it.each([
    {
      ...
      params: {
        env: { NODOASSIST_HOME: "/srv/nodoassist-home" } as NodeJS.ProcessEnv,
        toolAllowlist: ["optional_tool"],
      },
      expectedLoaderCall: {
        env: { NODOASSIST_HOME: "/srv/nodoassist-home" },
      },
    },
    {
  ...
  it("retains cold-loaded plugin tools for cached descriptor execution after active registry replacement", async () => {
    ...
    expect(loadNodoAssistPluginsMock).toHaveBeenCalledTimes(1);
  ...
  it("loads a standalone registry when cached runtime registries lack matching tool entries", () => {
    ...
    resolveRuntimePluginRegistryMock.mockReturnValue(undefined);
    loadNodoAssistPluginsMock.mockReturnValue(loadedRegistry);
  ...
  it("adds enabled non-startup tool plugins to the active tool runtime scope", () => {
    ...
    setActivePluginRegistry(activeRegistry as never, "gateway-startup", "gateway-bindable", "/tmp");
    resolveRuntimePluginRegistryMock.mockReturnValue(activeRegistry);
    loadNodoAssistPluginsMock.mockReturnValue(createToolRegistry([]));
