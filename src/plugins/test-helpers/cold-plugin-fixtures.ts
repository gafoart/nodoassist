...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/nodoassist-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "nodoassist.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): NodoAssistConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    NODOASSIST_HOME: path.join(homeDir, "home"),
    NODOASSIST_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    NODOASSIST_DISABLE_PERSISTED_PLUGIN_REGISTRY:
