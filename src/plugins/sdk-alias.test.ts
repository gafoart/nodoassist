...
const fixtureRoot = makeTrackedTempDir("nodoassist-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createTrustedNodoAssistPackageFixture(version: string) {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "nodoassist.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(
...
function createBundledPluginPackagePublicSurfaceAliasFixture() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@nodoassist/slack", type: "module" }, null, 2),
    "utf-8",
  );
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".nodoassist", "extensions", "demo");
...
function expectPluginSdkAliasTargets(
  ...
  expect(fs.realpathSync(aliases["nodoassist/plugin-sdk"] ?? "")).toBe(
  ...
  if (params.channelRuntimePath) {
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  if (params.pluginEntryPath) {
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/plugin-entry"] ?? "")).toBe(
...
function expectCwdFallbackPluginSdkAliasResolution(params: {
  ...
  const resolved = withCwd(params.fixture.root, () =>
    resolvePluginSdkAlias({
      ...
      distFile: "channel-runtime.js",
      modulePath: "/tmp/tsx-cache/nodoassist-loader.js",
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/nodoassist-loader.js",
      argv1: (root: string) => path.join(root, "nodoassist.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("resolves extension-api aliases from an explicit dev source root", () => {
    ...
    expect(fs.realpathSync(aliases["nodoassist/extension-api"] ?? "")).toBe(
  ...
  it("adds private qa plugin-sdk subpaths for trusted local checkouts when enabled", () => {
    ...
    const subpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it("resolves public QA plugin-sdk aliases without enabling private QA mode", () => {
    ...
    const subpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({ modulePath: sourcePluginEntry }),
    );
    const aliases = withEnv(
      { NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined },
    ...
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/qa-runner-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRunnerPath),
    );
    expect(
      fs.realpathSync(aliases["nodoassist/plugin-sdk/qa-live-transport-scenarios"] ?? ""),
    ).toBe(fs.realpathSync(distQaLiveTransportScenariosPath));
    expect(aliases["nodoassist/plugin-sdk/qa-runtime"]).toBeUndefined();
  });
  ...
  it("adds non-QA private Codex helper subpaths only for trusted Codex plugins", () => {
    ...
    const { packageRoot: installedCodexRoot, pluginEntry: installedCodexEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".nodoassist", "npm"),
        packageName: "@nodoassist/codex",
      });
    const { packageRoot: installedOtherRoot, pluginEntry: installedOtherEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".nodoassist", "npm"),
        packageName: "@nodoassist/demo",
      });
    const shadowCodexRoot = path.join(makeTempDir(), ".nodoassist", "extensions", "codex-shadow");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@nodoassist/codex", type: "module" }, null, 2),
      "utf-8",
    );
    fs.writeFileSync(shadowCodexEntry, 'export const plugin = "shadow";\n', "utf-8");

    const codexSubpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceCodexEntry,
      }),
    );
    const otherSubpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOtherEntry,
      }),
    );
    const installedCodexSubpaths = withCwd(installedCodexRoot, () =>
      withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
        listPluginSdkExportedSubpaths({
          modulePath: installedCodexEntry,
          argv1: path.join(fixture.root, "nodoassist.mjs"),
        }),
      ),
    );
  ...
  it("does not reuse a non-private cached subpath list after private qa gets enabled", () => {
    ...
    const privateSubpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: path.join(fixture.root, "src", "plugins", "loader.ts"),
      }),
    );
  ...
  it.each([
    {
      name: "does not derive plugin-sdk subpaths from cwd fallback when package root is not an NodoAssist root",
      fixture: () =>
        createPluginSdkAliasFixture({
  ...
  it("adds private qa plugin-sdk aliases for source plugins when enabled", () => {
    ...
    const aliases = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined }, () =>
      buildPluginLoaderAliasMap(sourcePluginEntry),
    );
    ...
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/qa-channel"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelPath),
    );
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/qa-channel-protocol"] ?? "")).toBe(
      fs.realpathSync(sourceQaChannelProtocolPath),
    );
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/qa-lab"] ?? "")).toBe(
  ...
  it("aliases non-QA private plugin-sdk subpaths for trusted Codex runtime loading", () => {
    ...
    const devRootAliases = withEnv(
      ...
      () =>
        buildPluginLoaderAliasMap(
          distCodexEntry,
          path.join(fixture.root, "nodoassist.mjs"),
          undefined,
          "dist",
          devFixture.root,
        ),
    ...
    const installedAliases = withCwd(installedCodexRoot, () =>
      withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined, NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(
          installedCodexEntry,
          path.join(fixture.root, "nodoassist.mjs"),
          undefined,
    ...
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/codex-mcp-projection"] ?? "")).toBe(
      fs.realpathSync(sourceCodexMcpProjectionPath),
    );
    expect(fs.realpathSync(aliases["nodoassist/plugin-sdk/codex-native-task-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceCodexNativeTaskRuntimePath),
    );
    expect(
      fs.realpathSync(installedAliases["nodoassist/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(distCodexMcpProjectionPath));
    expect(
      fs.realpathSync(installedAliases["nodoassist/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(distCodexNativeTaskRuntimePath));
    expect(fs.realpathSync(devRootAliases["nodoassist/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
    expect(
      fs.realpathSync(devRootAliases["nodoassist/plugin-sdk/codex-mcp-projection"] ?? ""),
    ).toBe(fs.realpathSync(devCodexMcpProjectionPath));
    expect(
      fs.realpathSync(devRootAliases["nodoassist/plugin-sdk/codex-native-task-runtime"] ?? ""),
    ).toBe(fs.realpathSync(devCodexNativeTaskRuntimePath));
    expect(aliases["nodoassist/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(otherAliases["nodoassist/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(otherAliases["nodoassist/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
    expect(installedOtherAliases["nodoassist/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(
      installedOtherAliases["nodoassist/plugin-sdk/codex-native-task-runtime"],
    ).toBeUndefined();
    expect(shadowCodexAliases["nodoassist/plugin-sdk/codex-mcp-projection"]).toBeUndefined();
    expect(shadowCodexAliases["nodoassist/plugin-sdk/codex-native-task-runtime"]).toBeUndefined();
  });
  ...
  it("aliases the SSRF internal helper only for bundled local IPC owner plugins", async () => {
    ...
    const entryBody = [
      'import { ssrfInternal } from "nodoassist/plugin-sdk/ssrf-runtime-internal";',
    ...
    const { packageRoot: installedOllamaRoot, pluginEntry: installedOllamaEntry } =
      writeInstalledPluginEntry({
        installRoot: path.join(makeTempDir(), ".nodoassist", "npm"),
        packageName: "@nodoassist/ollama",
      });

    const sourceSubpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceOllamaEntry,
      }),
    );
    const sourceBrowserSubpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      listPluginSdkExportedSubpaths({
        modulePath: sourceBrowserEntry,
      }),
    );
    const privateQaOtherSubpaths = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      listPluginSdkExportedSubpaths({
    ...
    const privateQaOtherAliases = withEnv(
      { NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1", NODE_ENV: undefined },
      () => buildPluginLoaderAliasMap(sourceOtherPluginEntry),
    );
    ...
    expect(
      fs.realpathSync(sourceAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(sourceSsrFInternalPath));
    expect(
      fs.realpathSync(sourceBrowserAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(sourceSsrFInternalPath));
    expect(fs.realpathSync(distAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"] ?? "")).toBe(
      fs.realpathSync(distSsrFInternalPath),
    );
    expect(
      fs.realpathSync(distBrowserAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(distRuntimeAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"] ?? ""),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(
      fs.realpathSync(
        distRuntimeBrowserAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"] ?? "",
      ),
    ).toBe(fs.realpathSync(distSsrFInternalPath));
    expect(otherAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(privateQaOtherAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
    expect(installedAliases["nodoassist/plugin-sdk/ssrf-runtime-internal"]).toBeUndefined();
  ...
  it("aliases workspace packages to source when dist artifacts are missing", () => {
    ...
    expect(fs.realpathSync(aliases["@nodoassist/gateway-client"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/gateway-client/timeouts"] ?? "")).toBe(
      fs.realpathSync(gatewayClientTimeouts.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/gateway-protocol"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocol.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/gateway-protocol/schema"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolSchema.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/gateway-protocol/frame-guards"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolFrameGuards.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/markdown-core"] ?? "")).toBe(
      fs.realpathSync(markdownCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/markdown-core/tables"] ?? "")).toBe(
      fs.realpathSync(markdownCoreTables.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/media-generation-core"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/media-generation-core/model-ref"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationModelRef.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/media-core"] ?? "")).toBe(
      fs.realpathSync(mediaCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/media-core/mime"] ?? "")).toBe(
      fs.realpathSync(mediaCoreMime.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/acp-core"] ?? "")).toBe(
      fs.realpathSync(acpCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/acp-core/runtime/types"] ?? "")).toBe(
      fs.realpathSync(acpCoreRuntimeTypes.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/normalization-core"] ?? "")).toBe(
      fs.realpathSync(normalizationCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/normalization-core/boolean-coercion"] ?? "")).toBe(
      fs.realpathSync(normalizationBooleanCoercion.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/normalization-core/string-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationStringCoerce.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/terminal-core"] ?? "")).toBe(
      fs.realpathSync(terminalCore.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/terminal-core/theme"] ?? "")).toBe(
      fs.realpathSync(terminalCoreTheme.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/net-policy"] ?? "")).toBe(
      fs.realpathSync(netPolicy.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/net-policy/ip"] ?? "")).toBe(
      fs.realpathSync(netPolicyIp.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/net-policy/url-protocol"] ?? "")).toBe(
      fs.realpathSync(netPolicyUrlProtocol.srcFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/model-catalog-core/provider-id"] ?? "")).toBe(
  ...
  it("aliases workspace package subpaths to dist when available", () => {
    ...
    expect(fs.realpathSync(aliases["@nodoassist/gateway-client/readiness"] ?? "")).toBe(
      fs.realpathSync(gatewayClient.distFile),
    );
    expect(
      fs.realpathSync(aliases["@nodoassist/gateway-protocol/connect-error-details"] ?? ""),
    ).toBe(fs.realpathSync(gatewayProtocol.distFile));
    expect(fs.realpathSync(aliases["@nodoassist/gateway-protocol/frame-guards"] ?? "")).toBe(
      fs.realpathSync(gatewayProtocolFrameGuards.distFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/markdown-core/render"] ?? "")).toBe(
      fs.realpathSync(markdownCore.distFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/media-generation-core/catalog"] ?? "")).toBe(
      fs.realpathSync(mediaGenerationCore.distFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/acp-core/normalize-text"] ?? "")).toBe(
      fs.realpathSync(acpCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/normalization-core/record-coerce"] ?? "")).toBe(
      fs.realpathSync(normalizationCoreRootDistFile),
    );
    expect(fs.realpathSync(aliases["@nodoassist/terminal-core/links"] ?? "")).toBe(
    ...
    expect(
      fs.realpathSync(aliases["@nodoassist/model-catalog-core/provider-model-id-normalize"] ?? ""),
  ...
  it("derives acp-core aliases from packaged root dist when package metadata is absent", () => {
    ...
    const cwdWithoutNodoAssistPackage = makeTempDir();

    const aliases = withCwd(cwdWithoutNodoAssistPackage, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(sourcePluginEntry, undefined, undefined, "dist"),
      ),
    );

    expect(fs.realpathSync(aliases["@nodoassist/acp-core/runtime/errors"] ?? "")).toBe(
  ...
  it("aliases bundled plugin package public surfaces for source plugin transforms", () => {
    ...
    expect(fs.realpathSync(aliases["@nodoassist/slack/api.js"] ?? "")).toBe(
      fs.realpathSync(sourceApiPath),
    );
    expect(fs.realpathSync(aliases["@nodoassist/slack/runtime-api.js"] ?? "")).toBe(
      fs.realpathSync(sourceRuntimeApiPath),
    );
    expect(aliases["@nodoassist/slack/test-api.js"]).toBeUndefined();
    expect(aliases["@nodoassist/slack/internal.js"]).toBeUndefined();
  });
  ...
  it("aliases bundled plugin package test surfaces only in private QA mode", () => {
    ...
    expect(fs.realpathSync(aliases["@nodoassist/slack/test-api.js"] ?? "")).toBe(
  ...
  it("falls back to source plugin-sdk subpath aliases when dist chunks are stale", () => {
    ...
    expect(fs.realpathSync(distAliases["nodoassist/plugin-sdk/provider-entry"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running nodoassist argv hint", () => {
    const {
      externalPluginEntry,
      externalPluginRoot,
      fixture,
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "nodoassist.mjs")),
      ),
    );
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the nodoassist root
    // (e.g. single-binary distributions or custom process launchers).
    // Use nodoassist.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real nodoassist repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "nodoassist.mjs")).href;
    ...
    // runner binary, inside the nodoassist repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedNodoAssistRootFromArgvHint returns null,
  ...
  it.each([
    {
      name: "does not resolve plugin-sdk alias files from cwd fallback when package root is not an NodoAssist root",
      fixture: () =>
        createPluginSdkAliasFixture({
          srcFile: "channel-runtime.ts",
          distFile: "channel-runtime.js",
  ...
  it("configures the plugin loader native-first boundary to prefer native dist modules", () => {
    ...
    expect(options.nativeModules).toEqual(["nodoassist"]);
  ...
  it("preserves configured jiti native modules while adding nodoassist", () => {
    const options = withEnv(
      { JITI_NATIVE_MODULES: JSON.stringify(["native-addon", "nodoassist"]) },
      () => buildPluginLoaderJitiOptions({}),
    );

    expect(options.nativeModules).toEqual(["native-addon", "nodoassist"]);
  });
  ...
  it("scopes plugin loader module config by plugin-sdk resolution", () => {
    ...
    expect(fs.realpathSync(auto.aliasMap["nodoassist/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(dist.aliasMap["nodoassist/plugin-sdk"] ?? "")).toBe(
...
`,
    ...
    const withAlias = createJiti(sourceLoaderBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "nodoassist/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
        "@nodoassist/plugin-sdk/channel-outbound": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
  ...
  it("falls back to ancestor runtime candidates when package-root markers are unavailable", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const loaderCachePath = path.join(root, ".cache", "tsx", "nodoassist-loader.js");
    mkdirSafeDir(path.dirname(distFile));
    ...
    expect(
      resolvePluginRuntimeModulePath({
        ...
        argv1: path.join(root, "bin", "nodoassist"),
        pluginSdkResolution: "dist",
      }),
    ).toBe(distFile);
  });
  ...
  it("uses the default startup argv hint for runtime fallback when argv1 is omitted", () => {
    ...
    const loaderCachePath = path.join(loaderCacheRoot, "tsx", "nodoassist-loader.js");
    ...
    process.argv[1] = path.join(root, "bin", "nodoassist");
    try {
      expect(
        resolvePluginRuntimeModulePath({
          modulePath: loaderCachePath,
  ...
  it("resolves runtime fallback through symlinked startup argv", () => {
    const root = makeTempDir();
    const distFile = path.join(root, "dist", "plugins", "runtime", "index.js");
    const binFile = path.join(root, "bin", "nodoassist");
    const shimRoot = makeTempDir();
    const shimFile = path.join(shimRoot, "bin", "nodoassist");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "nodoassist-loader.js");
    mkdirSafeDir(path.dirname(distFile));
  ...
  it("resolves runtime fallback through npm .bin startup argv", () => {
    const root = makeTempDir();
    const packageRoot = path.join(root, "node_modules", "nodoassist");
    ...
    const binFile = path.join(root, "node_modules", ".bin", "nodoassist");
    const loaderCachePath = path.join(makeTempDir(), "tsx", "nodoassist-loader.js");
    mkdirSafeDir(path.dirname(distFile));
...
describe("buildPluginLoaderAliasMap memoization", () => {
  ...
  it("returns different references for different modulePath inputs", () => {
    ...
    expect(aliasA["nodoassist/plugin-sdk"]).not.toBe(aliasB["nodoassist/plugin-sdk"]);
  });
  ...
  it("returns different references when an explicit dev source root differs", () => {
    ...
    expect(fs.realpathSync(stableAliases["nodoassist/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(stableRootAlias),
    );
    expect(fs.realpathSync(devAliases["nodoassist/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(devRootAlias),
    );
  });
  ...
  it("does not reuse a public alias map after private qa aliases are enabled", () => {
    ...
    const publicAliases = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: undefined }, () =>
      buildPluginLoaderAliasMap(entry),
    );
    const privateAliases = withEnv({ NODOASSIST_ENABLE_PRIVATE_QA_CLI: "1" }, () =>
      buildPluginLoaderAliasMap(entry),
    );

    expect(publicAliases).not.toBe(privateAliases);
    expect(publicAliases["nodoassist/plugin-sdk/qa-runtime"]).toBeUndefined();
    expect(fs.realpathSync(privateAliases["nodoassist/plugin-sdk/qa-runtime"] ?? "")).toBe(
      fs.realpathSync(sourceQaRuntimePath),
    );
  });
  ...
  it("does not reuse a development alias map in production mode", () => {
    ...
    expect(fs.realpathSync(developmentAliases["nodoassist/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(productionAliases["nodoassist/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
  });
...
describe("buildPluginLoaderJitiOptions", () => {
  it("scopes jiti fs cache by NodoAssist package version and install metadata", () => {
    const root = createTrustedNodoAssistPackageFixture("1.2.3-beta.4");
    ...
    expect(fsCache).toContain(path.join(tmpDir, "jiti", "nodoassist", "1.2.3-beta.4") + path.sep);
  ...
  it("preserves jiti's tmpdir guard when TMPDIR resolves to cwd", () => {
    ...
    expect(guardedFsCache).toContain(path.join("jiti", "nodoassist", "1.2.3-beta.4") + path.sep);
    expect(guardedFsCache.startsWith(path.join(root, "jiti") + path.sep)).toBe(false);
    expect(respectedFsCache).toContain(
      path.join(root, "jiti", "nodoassist", "1.2.3-beta.4") + path.sep,
    );
  });

  it("adds the versioned fs cache directory to plugin loader jiti options", () => {
    const root = createTrustedNodoAssistPackageFixture("2.0.0");
    const tmpDir = path.join(root, "tmp");

    const options = withEnv({ TMPDIR: tmpDir }, () =>
      buildPluginLoaderJitiOptions(
        { "nodoassist/plugin-sdk": path.join(root, "dist", "plugin-sdk", "root-alias.cjs") },
        { modulePath: path.join(root, "dist", "plugins", "loader.js") },
      ),
    );

    expect(options.fsCache).toContain(path.join(tmpDir, "jiti", "nodoassist", "2.0.0"));
  });
  ...
  it("pre-normalizes and marks alias maps for source transforms", () => {
    ...
    const aliasMap = {
      "nodoassist/plugin-sdk/core": "/repo/src/plugin-sdk/core.ts",
      "nodoassist/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
      "@nodoassist/plugin-sdk": "/repo/src/plugin-sdk/root-alias.cjs",
    };
