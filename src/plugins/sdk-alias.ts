...
import { resolveNodoAssistDevSourceRoot } from "./dev-source-root.js";
...
function resolvePluginLoaderJitiNativeModules(): string[] {
  try {
    ...
    const nativeModules = Array.isArray(configured)
      ...
      : [];
    return [...new Set([...nativeModules, "nodoassist"])];
  } catch {
    return ["nodoassist"];
  }
}
...
function hasTrustedNodoAssistRootIndicator(params: {
  packageRoot: string;
  ...
  const hasNodoAssistBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("nodoassist")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.nodoassist === "string");
  const hasNodoAssistEntrypoint = fs.existsSync(path.join(params.packageRoot, "nodoassist.mjs"));
  return hasCliEntryExport || hasNodoAssistBin || hasNodoAssistEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedNodoAssistRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedNodoAssistRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedNodoAssistRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveNodoAssistDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveNodoAssistPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedNodoAssistRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["nodoassist/plugin-sdk", "@nodoassist/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@nodoassist/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@nodoassist/gateway-client",
  ...
  {
    packageName: "@nodoassist/gateway-protocol",
  ...
  {
    packageName: "@nodoassist/media-generation-core",
  ...
  {
    packageName: "@nodoassist/terminal-core",
  ...
  {
    packageName: "@nodoassist/net-policy",
  ...
  {
    packageName: "@nodoassist/model-catalog-core",
...
export function listWorkspacePackageExportAliasEntries(params: {
  ...
  const fallbackPackageRoot = resolveNodoAssistPackageRootSync({ cwd: process.cwd() });
  const packageJson =
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@nodoassist/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `nodoassist/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "nodoassist/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
...
export function buildPluginLoaderJitiOptions(
  ...
  return {
    ...
    tryNative: true,
    // When jiti must transform a plugin entry, keep NodoAssist's own package
...
export function isBundledPluginExtensionPath(params: {
  modulePath: string;
  nodoAssistPackageRoot: string;
  bundledPluginsDir?: string;
}): boolean {
  const normalizedModulePath = path.resolve(params.modulePath);
  const roots = [
    params.bundledPluginsDir ? path.resolve(params.bundledPluginsDir) : null,
    path.join(params.nodoAssistPackageRoot, "extensions"),
    path.join(params.nodoAssistPackageRoot, "dist", "extensions"),
    path.join(params.nodoAssistPackageRoot, "dist-runtime", "extensions"),
