...
function makeNodoAssistDevSourceRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-loader-dev-source-"));
  fs.writeFileSync(
...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadNodoAssistPlugins(loadOptions);

      loadNodoAssistPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadNodoAssistPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadNodoAssistPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadNodoAssistPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    expect(loadNodoAssistPlugins(loadOptions)).not.toBe(registry);
  });
});
