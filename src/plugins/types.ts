...
export type {
  NodoAssistPluginActiveModelContext,
  NodoAssistPluginHookOptions,
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<NodoAssistConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). NodoAssist uses this to expose
 ...
 *
 * Runs after NodoAssist resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common NodoAssist auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and NodoAssist wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before NodoAssist builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. NodoAssist no
...
/**
 * Provider-owned stream wrapper hook after NodoAssist applies its generic
 ...
 *
 * Return `true` or `false` to override NodoAssist's built-in provider cache TTL
 ...
 *
 * Runs only after NodoAssist exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that NodoAssist should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after NodoAssist loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, NodoAssist falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * NodoAssist calls this only from async model resolution paths. After it
   ...
   *
   * Runs after NodoAssist performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before NodoAssist registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic NodoAssist wrappers.
   *
   ...
   *
   * Called after NodoAssist resolves the raw configured credential but before the
   ...
   * hint than NodoAssist's generic auth-store guidance.
   */
   ...
   * NodoAssist's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when NodoAssist should surface a provider-specific hint for
   ...
   * merging. NodoAssist deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. NodoAssist stores the
   ...
   * without replacing the full NodoAssist-owned system prompt.
   */
   ...
   *
   * Runs after NodoAssist's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep NodoAssist's default fallback policy.
   */
   ...
   * rewrites after NodoAssist has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before NodoAssist handles
   ...
   *
   * NodoAssist uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `nodoassist doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `nodoassist doctor` should offer.
   *
   ...
   *
   * NodoAssist calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep NodoAssist's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * NodoAssist falls back to that stored profile.
   */
...
export type NodoAssistPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral NodoAssist session id when available. */
  sessionId?: string;
  /** Transcript file for this NodoAssist session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active NodoAssist session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current NodoAssist configuration */
  config: NodoAssistConfig;
...
export const AGENT_PROMPT_SURFACE_KINDS = [
  "nodoassist_main",
  /** @deprecated Use nodoassist_main. */
  "pi_main",
  "codex_app_server",
  "cli_backend",
  "acp_backend",
...
export type NodoAssistPluginHttpRouteAuth = "gateway" | "plugin";
export type NodoAssistPluginHttpRouteMatch = "exact" | "prefix";
export type NodoAssistPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";
...
export type NodoAssistPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type NodoAssistPluginHttpRouteParams = {
  path: string;
...
export type NodoAssistPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type NodoAssistPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `nodoassist` program. For nested
...
export type NodoAssistPluginCliRegistrar = (
  ctx: NodoAssistPluginCliContext,
) => void | Promise<void>;
 ...
 * If you want NodoAssist to keep a plugin command lazy-loaded while still
 ...
 */
export type NodoAssistPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type NodoAssistPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `nodoassist nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `nodoassist nodes`, so a descriptor named
   * `"camera"` exposes `nodoassist nodes camera`.
   */
...
export type NodoAssistPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type NodoAssistPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type NodoAssistPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: NodoAssistPluginNodeInvokeApprovalDecision | null;
  }>;
};
...
export type NodoAssistPluginNodeInvokePolicyContext = {
  ...
  approvals?: NodoAssistPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<NodoAssistPluginNodeInvokeTransportResult>;
};
...
export type NodoAssistPluginNodeInvokePolicy = {
  ...
  handle: (
    ctx: NodoAssistPluginNodeInvokePolicyContext,
  ) => Promise<NodoAssistPluginNodeInvokePolicyResult> | NodoAssistPluginNodeInvokePolicyResult;
};
...
export type NodoAssistPluginSecurityAuditCollector = (
  ctx: NodoAssistPluginSecurityAuditContext,
...
export type NodoAssistGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: NodoAssistGatewayDiscoveryAdvertiseContext,
...
export type NodoAssistPluginService = {
  id: string;
  start: (ctx: NodoAssistPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: NodoAssistPluginServiceContext) => void | Promise<void>;
};

export type NodoAssistPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type NodoAssistPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: NodoAssistPluginReloadRegistration;
  nodeHostCommands?: NodoAssistPluginNodeHostCommand[];
  securityAuditCollectors?: NodoAssistPluginSecurityAuditCollector[];
  register?: (api: NodoAssistPluginApi) => void;
  activate?: (api: NodoAssistPluginApi) => void;
};

export type NodoAssistPluginModule =
  | NodoAssistPluginDefinition
  | ((api: NodoAssistPluginApi) => void);
...
export type PluginConfigMigration = (config: NodoAssistConfig) =>
  | {
      config: NodoAssistConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `nodoassist migrate`. */
...
export type NodoAssistPluginSessionStateApi = {
...
export type NodoAssistPluginSessionWorkflowApi = {
...
export type NodoAssistPluginSessionControlsApi = {
...
export type NodoAssistPluginSessionApi = {
  state: NodoAssistPluginSessionStateApi;
  workflow: NodoAssistPluginSessionWorkflowApi;
  controls: NodoAssistPluginSessionControlsApi;
};

export type NodoAssistPluginAgentEventsApi = {
...
export type NodoAssistPluginAgentApi = {
  events: NodoAssistPluginAgentEventsApi;
};

export type NodoAssistPluginRunContextApi = {
...
export type NodoAssistPluginLifecycleApi = {
...
export type NodoAssistPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
   ...
   */
  session: NodoAssistPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: NodoAssistPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: NodoAssistPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: NodoAssistPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | NodoAssistPluginToolFactory,
    opts?: NodoAssistPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: NodoAssistPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: NodoAssistPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: NodoAssistPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: NodoAssistPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    ...
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, NodoAssist
  ...
  /**
   * Register a plugin-owned node feature command group under `nodoassist nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: NodoAssistPluginCliRegistrar,
    opts?: NodoAssistPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: NodoAssistPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: NodoAssistPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: NodoAssistPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: NodoAssistPluginSecurityAuditCollector) => void;
  registerService: (service: NodoAssistPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: NodoAssistGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `nodoassist migrate` (migration capability). */
   ...
   */
  registerCommand: (command: NodoAssistPluginCommandDefinition) => void;
