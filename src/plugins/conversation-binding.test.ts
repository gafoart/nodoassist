...
const tempRoot = makeTrackedTempDir("nodoassist-plugin-binding", tempDirs);
...
describe("plugin conversation binding approvals", () => {
  beforeEach(() => {
    process.env.NODOASSIST_STATE_DIR = tempRoot;
  ...
  it("persists plugin-owned binding data on approved plugin bindings", async () => {
    const data = {
      ...
      version: 1,
      sessionFile: "/tmp/nodoassist/session.jsonl",
  ...
  it.each([
    ...
    },
    {
      ...
      existingRecord: {
        ...
        targetSessionKey: "nodoassist-app-server:thread:019ce411-6322-7db2-a821-1a61c530e7d9",
        targetKind: "session" as const,
        conversation: {
          channel: "telegram",
          accountId: "default",
