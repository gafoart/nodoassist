...
const resolveNodoAssistPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/nodoassist-root.js", () => ({
  resolveNodoAssistPackageRootSync: (...args: unknown[]) =>
    resolveNodoAssistPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("nodoassist-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      nodoassist: params.nodoassist ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
...
type MockNpmPackage = {
  ...
  materializesRootNodoAssist?: boolean;
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootNodoAssist) {
      lockPackages["node_modules/nodoassist"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalNodoAssistPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "nodoassist"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.nodoassist ? { nodoassist: viewPackage.nodoassist } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalNodoAssistPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "nodoassist") {
            const nodoassistRoot = path.join(npmRoot, "node_modules", "nodoassist");
            fs.mkdirSync(nodoassistRoot, { recursive: true });
            fs.writeFileSync(
              path.join(nodoassistRoot, "package.json"),
              JSON.stringify({ name: "nodoassist", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootNodoAssist) {
            ...
            fs.writeFileSync(
              path.join(nodoassistRoot, "package.json"),
              JSON.stringify({ name: "nodoassist", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "nodoassist") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "nodoassist"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveNodoAssistPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "nodoassist", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveNodoAssistPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "nodoassist-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@nodoassist/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
    ...
    {
      spec: "@nodoassist/voice-call@0.0.1",
  ...
  const dependencySpec = managedManifest.dependencies?.["@nodoassist/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@nodoassist/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@nodoassist/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("nodoassist-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@nodoassist/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_nodoassist-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_nodoassist-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@nodoassist/pack-demo";
    const archiveV1Path = path.join(stateDir, "nodoassist-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "nodoassist-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@nodoassist/pack-demo";
    const archivePath = path.join(stateDir, "nodoassist-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .nodoassist/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@nodoassist/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@nodoassist/voice-call@0.0.1");
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@nodoassist/codex-fixture";
    ...
    expect(repairCacheDir).toContain("nodoassist-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@nodoassist/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    ...
    const quarantineParent = path.join(npmProjectRoot, "_nodoassist-quarantined-npm-projects");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed nodoassist peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "nodoassist")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/nodoassist");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "nodoassist")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their nodoassist peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveNodoAssistPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@nodoassist/codex@2026.5.7",
      packageName: "@nodoassist/codex",
      version: "2026.5.7",
      pluginId: "@nodoassist/codex",
      npmRoot,
      peerDependencies: { nodoassist: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@nodoassist/codex");
    expect(result.error).toContain("plugin-local node_modules/nodoassist link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate nodoassist package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@nodoassist/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@nodoassist/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("NODOASSIST_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@nodoassist/whatsapp@2026.5.27",
      packageName: "@nodoassist/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { nodoassist: ">=2026.5.27" },
      nodoassist: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@nodoassist", "whatsapp"))).toBe(
      false,
    );
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@nodoassist/whatsapp@2026.5.26",
        packageName: "@nodoassist/whatsapp",
        version: "2026.5.26",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@nodoassist/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @nodoassist/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@nodoassist/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@nodoassist/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@nodoassist/msteams@2026.5.28-beta.3",
        packageName: "@nodoassist/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@nodoassist/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @nodoassist/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@nodoassist/msteams",
    ...
    expect(managedManifest.dependencies?.["@nodoassist/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root nodoassist materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstall({
        ...
        npmRoot,
        peerDependencies: { nodoassist: "^2026.0.0" },
        materializesRootNodoAssist: true,
      });
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "nodoassist"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/nodoassist"]).toBeUndefined();
      expect(
        fs
          .lstatSync(path.join(requiredPeerPluginDir, "node_modules", "nodoassist"))
          .isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed nodoassist root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
    ...
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "nodoassist"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "nodoassist", "package.json"),
      JSON.stringify({
        name: "nodoassist",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@nodoassist/discord@beta",
      packageName: "@nodoassist/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { nodoassist: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("nodoassist");
    expect(manifest.dependencies?.["@nodoassist/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/nodoassist"]).toBeUndefined();
    expect(lockfile.dependencies?.nodoassist).toBeUndefined();
  });

  it("preserves the active host nodoassist runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "nodoassist");
    ...
    resolveNodoAssistPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/nodoassist-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/nodoassist-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.nodoassist).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/nodoassist-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/nodoassist-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/nodoassist-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/nodoassist-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@nodoassist/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "nodoassist"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "nodoassist")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local nodoassist peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@nodoassist/codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@nodoassist/codex": "0.0.1" },
      packages: [
        {
          packageName: "@nodoassist/codex",
          version: "0.0.1",
    ...
    const installedDir = writeInstalledNpmPlugin({
      ...
      packageName: "@nodoassist/codex",
      version: "0.0.1",
      peerDependencies: { nodoassist: "*" },
    });
    const peerLink = path.join(installedDir, "node_modules", "nodoassist");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@nodoassist/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
      ...
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            ...
            nodoassist?: { managedOverrides?: string[] };
          };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.nodoassist?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.nodoassist?.managedOverrides).toEqual(["axios", "nested"]);
        }
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official NodoAssist package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@nodoassist/voice-call");
  ...
  it("allows duplicate npm installs in update mode", async () => {
    ...
    mockNpmViewAndInstall({
      spec: "@nodoassist/voice-call@0.0.2",
      packageName: "@nodoassist/voice-call",
      version: "0.0.2",
      pluginId: "voice-call",
      npmRoot,
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    mockNpmViewAndInstallMany([
      ...
      {
        spec: "@nodoassist/whatsapp@0.0.1",
        packageName: "@nodoassist/whatsapp",
        version: "0.0.1",
        pluginId: "whatsapp",
        npmRoot,
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@nodoassist/voice-call"))).toBe(
      true,
    );
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@nodoassist/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@nodoassist/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@nodoassist/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @nodoassist/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@nodoassist/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@nodoassist/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @nodoassist/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    mockNpmViewAndInstall({
      spec: "@nodoassist/voice-call@beta",
      packageName: "@nodoassist/voice-call",
      version: "0.0.2-beta.1",
      pluginId: "voice-call",
      integrity: "sha512-beta",
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@nodoassist/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@nodoassist/voice-call",
