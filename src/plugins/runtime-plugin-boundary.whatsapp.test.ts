...
function createBundledWhatsAppRuntimeFixture() {
  const rootDir = makeTrackedTempDir("nodoassist-whatsapp-boundary", tempDirs);
  for (const [relativePath, value] of Object.entries({
    ...
    "dist/connection-controller-registry.js": [
      'const key = Symbol.for("nodoassist.whatsapp.connectionControllerRegistry");',
      "const g = globalThis;",
      "if (!g[key]) {",
...
function createExternalTypeScriptRuntimePackageFixture() {
  const rootDir = makeTrackedTempDir("nodoassist-external-boundary-ts", tempDirs);
  writeRuntimeFixtureText(
    rootDir,
    "package.json",
    JSON.stringify(
      {
        name: "nodoassist-external-ts-runtime",
        type: "module",
      },
      null,
      2,
...
describe("runtime plugin boundary whatsapp seam", () => {
  ...
  it("rejects bundled TypeScript runtime modules instead of using the source loader", () => {
    const rootDir = makeTrackedTempDir("nodoassist-bundled-boundary-ts", tempDirs);
