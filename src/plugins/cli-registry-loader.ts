...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly NodoAssistPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: NodoAssistPluginCliContext["program"]) => Promise<void>;
};
...
async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadNodoAssistPluginCliRegistry(
...
async function loadPluginCliCommandRegistryWithContext(params: {
  ...
  return {
    ...params.context,
    registry: loadNodoAssistPlugins(
...
): Promise<NodoAssistPluginCliCommandDescriptor[]> {
  try {
