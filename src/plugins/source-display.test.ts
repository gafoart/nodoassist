...
const PLUGIN_SOURCE_ROOTS = {
  ...
  global: path.resolve(path.sep, "Users", "x", ".nodoassist", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".nodoassist", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "nodoassist-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".nodoassist", "extensions"),
      },
    });
  });
});
