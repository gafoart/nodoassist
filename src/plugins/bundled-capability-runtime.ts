...
import { discoverNodoAssistPlugins, type PluginDiscoveryResult } from "./discovery.js";
...
import type { NodoAssistPluginDefinition, NodoAssistPluginModule } from "./types.js";
...
export function buildVitestCapabilityShimAliasMap(): Record<string, string> {
  return Object.fromEntries(
    CAPABILITY_VITEST_SHIM_ALIASES.flatMap(({ subpath, target }) => {
      ...
      return [
        [`nodoassist/plugin-sdk/${subpath}`, targetPath],
        [`@nodoassist/plugin-sdk/${subpath}`, targetPath],
      ];
    }),
  );
...
function applyVitestCapabilityAliasOverrides(params: {
  ...
  const {
    "nodoassist/plugin-sdk": _ignoredLegacyRootAlias,
    "@nodoassist/plugin-sdk": _ignoredScopedRootAlias,
    ...scopedAliasMap
  } = params.aliasMap;
  return {
    ...scopedAliasMap,
...
function resolvePluginModuleExport(moduleExport: unknown): {
  ...
  if (resolved && typeof resolved === "object") {
    const definition = resolved as NodoAssistPluginDefinition;
    return {
      definition,
...
export function loadBundledCapabilityRuntimeRegistry(params: {
  ...
  const discovery = params.discovery ?? discoverNodoAssistPlugins({ env });
  ...
  for (const candidate of discovery.candidates) {
    ...
    try {
      mod = getModuleLoader(safeSource)(safeSource) as NodoAssistPluginModule;
    } catch (error) {
