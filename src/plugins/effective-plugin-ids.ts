...
function collectExplicitEffectivePluginIds(config: NodoAssistConfig): string[] {
...
function collectSelectedContextEnginePluginIds(config: NodoAssistConfig): string[] {
  const plugins = normalizePluginsConfig(config.plugins);
  if (!plugins.enabled) {
    return [];
  }
