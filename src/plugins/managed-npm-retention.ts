...
const RETAINED_MANAGED_NPM_INSTALL_MARKER_DIR = ".nodoassist-retained-npm-installs";
...
export async function clearRetainedManagedNpmInstallMarker(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    // Best effort: keep the NodoAssist-owned marker directory if it is not empty.
  }
  return true;
}
