...
function hasPackagePluginMetadata(manifest: Record<string, unknown>): boolean {
  const nodoassist = manifest.nodoassist;
  if (!isRecord(nodoassist)) {
    return false;
  }
  const extensions = nodoassist.extensions;
...
function readManifestPluginId(packageDir: string): string | undefined {
  const manifest = readJsonObjectFileSync(path.join(packageDir, "nodoassist.plugin.json"));
