...
import { listNodoAssistPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "nodoassist.plugin.json"),
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/nodoassist.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reuses current metadata when callers omit config", () => {
    const config: NodoAssistConfig = { plugins: { allow: ["normalizer"] } };
    setCurrentPluginMetadataSnapshot(
      createCurrentSnapshot({
        manifestHash: "alpha",
        prefix: "alpha",
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    setTestEnvValue("NODOASSIST_STATE_DIR", stateDirA);
    deleteTestEnvValue("NODOASSIST_HOME");
    setTestEnvValue("NODOASSIST_DISABLE_BUNDLED_PLUGINS", "1");
    deleteTestEnvValue("NODOASSIST_BUNDLED_PLUGINS_DIR");
    ...
    setTestEnvValue("NODOASSIST_STATE_DIR", stateDirB);
    clearPluginMetadataLifecycleCaches();
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    const listNormalizerRecords = () =>
      listNodoAssistPluginManifestMetadata(process.env).filter(
