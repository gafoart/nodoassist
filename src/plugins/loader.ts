...
import { installNodoAssistPluginSdkNativeResolver } from "./plugin-sdk-native-resolver.js";
...
function createGuardedPluginRegistrationApi(api: NodoAssistPluginApi): {
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<NodoAssistPluginDefinition["register"]>>[0],
): void {
...
export const testing = {
  ...
  resolvePluginSdkAliasFile,
  resolvePluginRuntimeModulePath,
  ensureNodoAssistPluginSdkAlias,
  shouldLoadChannelPluginInSetupRuntime,
  shouldPreferNativeModuleLoad,
  toSafeImportPath,
...
function resolvePluginLoadCacheContext(options: PluginLoadOptions = {}) {
  ...
  const devSourceRoot = resolveNodoAssistDevSourceRoot(env);
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadNodoAssistPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadNodoAssistPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: NodoAssistPluginDefinition;
  register?: NodoAssistPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as NodoAssistPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as NodoAssistPluginDefinition;
...
export function loadNodoAssistPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into NodoAssist yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: NodoAssistPluginModule | null = null;
      let moduleLoadMs = 0;
      ...
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => loadPluginModule(safeSource) as NodoAssistPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: NodoAssistPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => loadPluginModule(safeRuntimeSource) as NodoAssistPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'nodoassist plugins inspect <id> --runtime --json' for runtime diagnostics, 'nodoassist plugins list' for registry state, and restart the Gateway after plugin code or load-path changes.`,
        );
      }
    }
...
export async function loadNodoAssistPluginCliRegistry(
  options: PluginLoadOptions = {},
  ...
  for (const candidate of orderedCandidates) {
    ...
    let mod: NodoAssistPluginModule | null;
    try {
      mod = withProfile(
        { pluginId: record.id, source: safeSource },
        "cli-metadata",
