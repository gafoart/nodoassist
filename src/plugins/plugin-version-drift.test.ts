...
function npmRecord(
  ...
  const resolvedName = overrides.resolvedName ?? "@nodoassist/whatsapp";
  return {
    source: "npm",
...
describe("detectPluginVersionDrift", () => {
  it("returns empty drifts when all externalized plugins match the gateway", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4", { resolvedName: "@nodoassist/discord" }),
      },
    });
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3", {
          ...
          spec: "@nodoassist/whatsapp@2026.5.3",
        }),
        discord: npmRecord("2026.5.4", { resolvedName: "@nodoassist/discord" }),
      },
    });
  ...
  it("treats a build-qualifier suffix on either side as matching (2026.5.4-1 ≈ 2026.5.4)", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        discord: npmRecord("2026.5.4-1", { resolvedName: "@nodoassist/discord" }),
      },
    });
  ...
  it("ignores official catalog installs pinned to independent package versions", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        "nodoassist-plugin-yuanbao": npmRecord("2.13.1", {
          resolvedName: "nodoassist-plugin-yuanbao",
          spec: "nodoassist-plugin-yuanbao@2.13.1",
        }),
      },
    });
  ...
  it("ignores exact catalog pins even when the pin matches the gateway version", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        "wecom-nodoassist-plugin": npmRecord("2026.5.6", {
          resolvedName: "@wecom/wecom-nodoassist-plugin",
          spec: "@wecom/wecom-nodoassist-plugin@2026.5.6",
        }),
      },
    });
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        ...
        archive: {
          ...
          spec: "@nodoassist/whatsapp@archive",
        },
        local: {
          source: "path",
          resolvedName: "@nodoassist/whatsapp",
  ...
  it("falls back to the install record's `version` field when `resolvedVersion` is absent", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: {
          source: "npm",
          spec: "@nodoassist/whatsapp@latest",
          resolvedName: "@nodoassist/whatsapp",
          version: "2026.5.3",
        },
      },
  ...
  it("skips plugins with no recorded version (cannot detect drift)", () => {
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: { source: "npm", spec: "@nodoassist/whatsapp@latest" },
      },
    });
  ...
  it("skips plugins that are explicitly disabled in config", () => {
    ...
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@nodoassist/discord" }),
      },
      config,
    });
  ...
  it("includes plugins with no entry in config (default-enabled)", () => {
    const config: NodoAssistConfig = { plugins: { entries: {} } } as NodoAssistConfig;
    const result = detectPluginVersionDrift({
      gatewayVersion: "2026.5.4",
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
  ...
  it("returns drifts sorted by pluginId for deterministic output", () => {
    const result = detectPluginVersionDrift({
      ...
      installRecords: {
        whatsapp: npmRecord("2026.5.3"),
        discord: npmRecord("2026.5.3", { resolvedName: "@nodoassist/discord" }),
        matrix: npmRecord("2026.5.3", { resolvedName: "@nodoassist/matrix" }),
      },
    });
...
describe("resolvePluginVersionDriftUpdateCommand", () => {
  it("uses an exact npm package target when the drifted install is pinned", () => {
    ...
    ).toBe("nodoassist plugins update @nodoassist/brave-plugin@2026.6.10-beta.1");
  });
  ...
  it("prefers the parsed exact npm spec package over inconsistent drift metadata", () => {
    expect(
      resolvePluginVersionDriftUpdateCommand({
        ...
        gatewayVersion: "2026.6.10-beta.1",
        source: "npm",
        packageName: "@nodoassist/other-plugin",
        spec: "@nodoassist/brave-plugin@2026.6.9",
      }),
    ).toBe("nodoassist plugins update @nodoassist/brave-plugin@2026.6.10-beta.1");
  });
  ...
  it("keeps plugin-id updates for floating npm install records", () => {
    ...
    ).toBe("nodoassist plugins update brave");
  });
