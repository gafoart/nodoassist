...
import {
  installNodoAssistPluginSdkNativeResolver,
  resetNodoAssistPluginSdkNativeResolverForTest,
...
function writeFakeNodoAssistPackage(root: string): { distRoot: string; loaderModulePath: string } {
  ...
  fs.writeFileSync(path.join(root, "nodoassist.mjs"), "#!/usr/bin/env node\n", "utf8");
...
describe("installNodoAssistPluginSdkNativeResolver", () => {
  it("resolves installed plugin SDK imports to the dev source root", () => {
    const stableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-stable-"));
    const devRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-dev-source-"));
    const { loaderModulePath } = writeFakeNodoAssistPackage(stableRoot);
    writeFakeNodoAssistPackage(devRoot);
    ...
    const previousDevSourceRoot = process.env.NODOASSIST_DEV_SOURCE_ROOT;
    process.env.NODOASSIST_DEV_SOURCE_ROOT = devRoot;

    try {
      const installedAliases = installNodoAssistPluginSdkNativeResolver({
        modulePath: loaderModulePath,
        pluginModulePath: externalPluginEntry,
      });

      expect(installedAliases).toContain("nodoassist/plugin-sdk/agent-runtime");
      const requireFromPlugin = createRequire(externalPluginEntry);
      expect(
        fs.realpathSync(requireFromPlugin.resolve("nodoassist/plugin-sdk/agent-runtime")),
      ).toBe(fs.realpathSync(path.join(devRoot, "dist", "plugin-sdk", "agent-runtime.js")));
    } finally {
      if (previousDevSourceRoot === undefined) {
        delete process.env.NODOASSIST_DEV_SOURCE_ROOT;
      } else {
        process.env.NODOASSIST_DEV_SOURCE_ROOT = previousDevSourceRoot;
      }
    }
  });
  ...
  it("resolves installed plugin SDK imports to an explicit dev source root", () => {
    ...
    expect(installedAliases).toContain("nodoassist/plugin-sdk/agent-runtime");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(fs.realpathSync(requireFromPlugin.resolve("nodoassist/plugin-sdk/agent-runtime"))).toBe(
  ...
  it("removes stale native SDK aliases when a later dev root omits a subpath", () => {
    ...
    expect(fs.realpathSync(requireFromPlugin.resolve("nodoassist/plugin-sdk/stable-extra"))).toBe(
    ...
    expect(() => requireFromPlugin.resolve("nodoassist/plugin-sdk/stable-extra")).toThrow();
  });

  it("keeps native aliases on JS dist artifacts when source files exist", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-source-resolver-"));
    const { loaderModulePath } = writeFakeNodoAssistPackage(root);
    ...
    expect(installedAliases).toContain("nodoassist/plugin-sdk/channel-outbound");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(requireFromPlugin.resolve("nodoassist/plugin-sdk/channel-outbound")),
    ).toBe(fs.realpathSync(path.join(root, "dist", "plugin-sdk", "channel-outbound.js")));
  });

  it("lets built external plugins resolve NodoAssist SDK subpaths with createRequire", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-resolver-"));
    const { distRoot, loaderModulePath } = writeFakeNodoAssistPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    ...
    try {
      ...
      const sdk = requireFromPlugin("nodoassist/plugin-sdk/channel-outbound") as {
        defineChannelMessageAdapter?: () => string;
      };

      expect(sdk.defineChannelMessageAdapter?.()).toBe("adapter");
      expect(() =>
        requireFromPlugin.resolve("nodoassist/not-plugin-sdk/channel-message"),
      ).toThrow();
    } finally {
      if (process.platform !== "win32") {
  ...
  beforeAll(() => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-esm-resolver-"));
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'import path from "node:path";',
        'import { pathToFileURL } from "node:url";',
        `import { installNodoAssistPluginSdkNativeResolver, resetNodoAssistPluginSdkNativeResolverForTest } from ${JSON.stringify(resolverModuleUrl)};`,
        ...
        "fs.writeFileSync(",
        "  entryPath,",
        '  "import { defineChannelMessageAdapter } from \\"nodoassist/plugin-sdk/channel-outbound\\"; export const eager = defineChannelMessageAdapter(); export const loadLazy = () => import(\\"./lazy.js\\");\\n",',
        '  "utf8",',
        ");",
        "fs.writeFileSync(",
        "  lazyPath,",
        '  "import { defineChannelMessageAdapter } from \\"nodoassist/plugin-sdk/channel-outbound\\"; export const lazy = defineChannelMessageAdapter();\\n",',
        '  "utf8",',
        ");",
        "installNodoAssistPluginSdkNativeResolver({",
        "  modulePath: loaderModulePath,",
  ...
  it("does not resolve SDK aliases for parents outside registered plugin roots", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-guard-"));
    const { loaderModulePath } = writeFakeNodoAssistPackage(root);
    const externalPluginEntry = writeExternalPluginEntry(path.join(root, "external-plugin"));
    const unrelatedRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-outside-"));
    ...
    expect(requireFromPlugin.resolve("nodoassist/plugin-sdk/channel-outbound")).toBeTruthy();
    expect(() => requireFromOutside.resolve("nodoassist/plugin-sdk/channel-outbound")).toThrow();
  });

  it("resolves internal core packages only for NodoAssist-owned source parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-core-internal-"));
    const { loaderModulePath } = writeFakeNodoAssistPackage(root);
    ...
    expect(installedAliases).toContain("@nodoassist/normalization-core/string-coerce");
    expect(installedAliases).toContain("@nodoassist/normalization-core/boolean-coercion");
    expect(installedAliases).toContain("@nodoassist/media-core/mime");
    expect(installedAliases).toContain("@nodoassist/markdown-core/code-spans");
    expect(installedAliases).toContain("@nodoassist/ai/internal/runtime");
    expect(installedAliases).toContain("@nodoassist/acp-core/runtime/types");
    expect(installedAliases).toContain("@nodoassist/llm-core");
    const requireFromCoreSource = createRequire(coreSourceParent);
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(
      fs.realpathSync(
        requireFromCoreSource.resolve("@nodoassist/normalization-core/string-coerce"),
      ),
    ).toBe(fs.realpathSync(normalizationSource));
    expect(
      fs.realpathSync(
        requireFromCoreSource.resolve("@nodoassist/normalization-core/boolean-coercion"),
      ),
    ).toBe(fs.realpathSync(booleanCoercionSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@nodoassist/media-core/mime"))).toBe(
      fs.realpathSync(mediaCoreSource),
    );
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@nodoassist/markdown-core/code-spans")),
    ).toBe(fs.realpathSync(markdownCoreSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@nodoassist/ai/internal/runtime"))).toBe(
      fs.realpathSync(aiRuntimeSource),
    );
    expect(
      fs.realpathSync(requireFromCoreSource.resolve("@nodoassist/acp-core/runtime/types")),
    ).toBe(fs.realpathSync(acpCoreSource));
    expect(fs.realpathSync(requireFromCoreSource.resolve("@nodoassist/llm-core"))).toBe(
      fs.realpathSync(llmCoreSource),
    );
    expect(() =>
      requireFromPlugin.resolve("@nodoassist/normalization-core/string-coerce"),
    ).toThrow();
    expect(() =>
      requireFromPlugin.resolve("@nodoassist/normalization-core/boolean-coercion"),
    ).toThrow();
    expect(() => requireFromPlugin.resolve("@nodoassist/media-core/mime")).toThrow();
    expect(() => requireFromPlugin.resolve("@nodoassist/markdown-core/code-spans")).toThrow();
    expect(() => requireFromPlugin.resolve("@nodoassist/ai/internal/runtime")).toThrow();
    expect(() => requireFromPlugin.resolve("@nodoassist/acp-core/runtime/types")).toThrow();
    expect(() => requireFromPlugin.resolve("@nodoassist/llm-core")).toThrow();
  });

  it("does not register source-only SDK subpaths for native resolution", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-source-only-"));
    const { loaderModulePath } = writeFakeNodoAssistPackage(root);
    ...
    expect(installedAliases).toContain("nodoassist/plugin-sdk/channel-outbound");
    expect(installedAliases).not.toContain("nodoassist/plugin-sdk/source-only");
    const requireFromPlugin = createRequire(externalPluginEntry);
    expect(() => requireFromPlugin.resolve("nodoassist/plugin-sdk/source-only")).toThrow();
  });

  it("scopes private SSRF SDK aliases to bundled local IPC native parents", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sdk-native-ssrf-"));
    const { loaderModulePath } = writeFakeNodoAssistPackage(root);
    ...
    expect(installedAliases).toContain("nodoassist/plugin-sdk/ssrf-runtime-internal");
    const requireFromOllama = createRequire(ollamaEntry);
    expect(
      fs.realpathSync(requireFromOllama.resolve("nodoassist/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeOllama = createRequire(runtimeOllamaEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeOllama.resolve("nodoassist/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromBrowser = createRequire(browserEntry);
    expect(
      fs.realpathSync(requireFromBrowser.resolve("nodoassist/plugin-sdk/ssrf-runtime-internal")),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromRuntimeBrowser = createRequire(runtimeBrowserEntry);
    expect(
      fs.realpathSync(
        requireFromRuntimeBrowser.resolve("nodoassist/plugin-sdk/ssrf-runtime-internal"),
      ),
    ).toBe(fs.realpathSync(internalPath));

    const requireFromOther = createRequire(otherEntry);
    expect(() => requireFromOther.resolve("nodoassist/plugin-sdk/ssrf-runtime-internal")).toThrow();
  });
});
