...
describe("managed npm retention", () => {
  it("cleans retired generations while preserving the active install root", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-retention-"));
    ...
    const oldPackageDir = path.join(oldProjectRoot, "node_modules", "@nodoassist", "codex");
    const activePackageDir = path.join(activeProjectRoot, "node_modules", "@nodoassist", "codex");
  ...
  it("cleans retained packages from the legacy shared npm root", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-retention-"));
    const npmDir = path.join(stateDir, "npm");
    const packageDir = path.join(npmDir, "node_modules", "@nodoassist", "codex");
