...
describe("plugin session actions", () => {
  ...
  it("emits plugin-attributed agent events through the plugin API", () => {
    ...
    const { config, registry } = createPluginRegistryFixture();
    let bundledApi: NodoAssistPluginApi | undefined;
    let workspaceApi: NodoAssistPluginApi | undefined;
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("blocks agent events from stale and non-activating plugin API closures", () => {
    ...
    try {
      ...
      let neverActiveApi: NodoAssistPluginApi | undefined;
      registerTestPlugin({
      ...
      let inactiveApi: NodoAssistPluginApi | undefined;
      registerTestPlugin({
