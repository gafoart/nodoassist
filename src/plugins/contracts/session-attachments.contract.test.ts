...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@nodoassist/media-core/mime";
import {
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: NodoAssistPluginApi }).proofAttachmentApi;
  ...
  it("wires sendSessionAttachment through the plugin API with stale-registry protection", async () => {
    await withSessionStore(async ({ storePath, filePath }) => {
      ...
      let capturedApi: NodoAssistPluginApi | undefined;
      registerTestPlugin({
        registry,
        config,
