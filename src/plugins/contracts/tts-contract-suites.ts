...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "nodoassist/plugin-sdk/speech-core";
...
type TtsRuntimeModule = typeof import("nodoassist/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("nodoassist/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@nodoassist/speech-core",
  artifactBasename: "runtime-api.js",
});

let ttsRuntime: TtsRuntimeModule;
let ttsRuntimeInitialized = false;
let completeSimple: typeof import("nodoassist/plugin-sdk/llm").completeSimple;
...
vi.mock("nodoassist/plugin-sdk/llm", () => {
...
function asLegacyTtsConfig(value: unknown): NodoAssistConfig {
  return value as NodoAssistConfig;
}

function asLegacyNodoAssistConfig(value: Record<string, unknown>): NodoAssistConfig {
  return value as unknown as NodoAssistConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): NodoAssistConfig {
...
const loadTtsCore = createLazyRuntimeModule(() => import("nodoassist/plugin-sdk/speech-core"));
...
function createResolvedSummarizationConfig(cfg: NodoAssistConfig): ResolvedTtsConfig {
  const rawConfig =
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("nodoassist/plugin-sdk/llm"));
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyNodoAssistConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: NodoAssistConfig = asLegacyNodoAssistConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.NODOASSIST_TTS_PREFS;
      process.env.NODOASSIST_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.NODOASSIST_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: NodoAssistConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
