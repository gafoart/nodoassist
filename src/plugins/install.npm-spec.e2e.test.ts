...
async function makeTempDir(label: string): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `nodoassist-${label}-`));
  tempDirs.push(dir);
  return dir;
}

function configWithInstalledPackageTreeBlockPolicy(): NodoAssistConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
            ...
            versions: Object.fromEntries(
              [...pkg.versionsByVersion.entries()].map(([version, entry]) => [
                version,
                {
                  name: pkg.packageName,
                  version,
                  ...(entry.nodoassist ? { nodoassist: entry.nodoassist } : {}),
...
describe("installPluginFromNpmSpec e2e", () => {
  it("installs the newest compatible stable package when npm latest requires a newer plugin API", async () => {
    ...
    const versions = [
      await packPlugin({
        ...
        version: "2026.5.26",
        rootDir,
        nodoassist: compatibleNodoAssist,
      }),
      await packPlugin({
        packageName,
        pluginId: packageName,
        version: "2026.5.27",
        rootDir,
        nodoassist: incompatibleNodoAssist,
      }),
    ];
    const registry = await startStaticRegistry([{ packageName, latest: "2026.5.27", versions }]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    const previousHostVersion = process.env.NODOASSIST_COMPATIBILITY_HOST_VERSION;
    process.env.NODOASSIST_COMPATIBILITY_HOST_VERSION = "2026.5.10-beta.1";
    ...
    try {
      ...
      if (previousHostVersion === undefined) {
        ...
        process.env.NODOASSIST_COMPATIBILITY_HOST_VERSION = previousHostVersion;
      }
    }
  });

  it("scrubs root nodoassist materialized by required npm peers", async () => {
    ...
    const nodoAssistVersions = [
      await packPlugin({
        packageName: "nodoassist",
        pluginId: "registry-nodoassist-copy",
        version: "2026.0.0",
        rootDir,
      }),
    ];
    const registry = await startStaticRegistry([
      ...
      { packageName: "nodoassist", latest: "2026.0.0", versions: nodoAssistVersions },
    ]);
    process.env.NPM_CONFIG_REGISTRY = registry;
    process.env.npm_config_registry = registry;
    ...
    const rawNodoAssistLockEntry = rawLock.packages?.["node_modules/nodoassist"] as
      | { peer?: unknown; version?: unknown }
      | undefined;
    expect(rawNodoAssistLockEntry?.peer).toBe(true);
    expect(rawNodoAssistLockEntry?.version).toBe("2026.0.0");
    ...
    expect(lock.packages?.["node_modules/nodoassist"]).toBeUndefined();
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", "nodoassist")),
    ).rejects.toHaveProperty("code", "ENOENT");
    await expect(
      fs
        .lstat(path.join(result.targetDir, "node_modules", "nodoassist"))
  ...
  it("plans peers from installed optional dependencies", async () => {
    ...
    expect(rootManifest.nodoassist?.managedPeerDependencies ?? []).toContain(runtimePeer);
  });
  ...
  it("leaves legacy flat-root peer dependencies alone during isolated later installs", async () => {
    ...
    expect(rootManifest.nodoassist?.managedPeerDependencies ?? []).not.toContain(runtimePeer);
  });
  ...
  it("falls back to the legacy npm peer mode inside the plugin project when npm cannot plan third-party peers", async () => {
    ...
    expect(rootManifest.nodoassist?.managedPeerDependencies ?? []).not.toContain(missingPeer);
    await expect(
      fs.lstat(path.join(projectRoot, "node_modules", blockedPlugin, "package.json")),
    ).resolves.toBeTruthy();
  });
  ...
  it("does not take ownership of an existing root dependency observed as a peer", async () => {
    ...
    expect(rootManifest.dependencies?.[blockedPlugin]).toBeUndefined();
    expect(rootManifest.dependencies?.[runtimePeer]).toBeUndefined();
    expect(rootManifest.nodoassist?.managedPeerDependencies ?? []).not.toContain(
  ...
  it("scrubs host peers inside each isolated npm project", async () => {
    ...
    const registry = await startStaticRegistry([
      {
        ...
        versions: [
          await packPlugin({
            packageName: codexName,
            peerDependencies: { nodoassist: ">=2026.5.5-beta.2" },
            peerDependenciesMeta: { nodoassist: { optional: true } },
            pluginId: codexName,
            version: "1.0.0",
            rootDir,
          }),
      ...
      },
      {
        ...
        versions: [
          await packPlugin({
            packageName: opikName,
            peerDependencies: { nodoassist: ">=2026.3.2" },
            peerDependenciesMeta: {},
            pluginId: opikName,
            version: "1.0.0",
            rootDir,
    ...
    await expect(
      fs
        .lstat(path.join(first.targetDir, "node_modules", "nodoassist"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
    await expect(
      fs
        .lstat(path.join(second.targetDir, "node_modules", "nodoassist"))
        .then((stat) => stat.isSymbolicLink()),
    ).resolves.toBe(true);
  });

  it("keeps an earlier isolated nodoassist peer link after later plugin installs", async () => {
    ...
    const peerLink = path.join(first.targetDir, "node_modules", "nodoassist");
    ...
    expect(manifest.dependencies?.nodoassist).toBeUndefined();
    const lock = JSON.parse(
