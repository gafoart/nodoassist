...
vi.mock("./loader.js", () => ({
  ...
  loadNodoAssistPlugins: mocks.loadNodoAssistPlugins,
...
describe("web-provider-runtime-shared", () => {
  ...
  it("caches setup web provider plugin loads by default", () => {
    ...
    const mapRegistryProviders = vi.fn(() => ["provider"]);
    mocks.loadNodoAssistPlugins.mockReturnValue(loadedRegistry as never);
    ...
    expect(providers).toEqual(["provider"]);
    expect(mockArg(mocks.loadNodoAssistPlugins).cache).toBe(true);
    expect(mockArg(mocks.loadNodoAssistPlugins).onlyPluginIds).toEqual(["brave"]);
  });
  ...
  it("falls back to plugin loading when bundled runtime artifacts do not cover the scope", () => {
    ...
    mocks.loadNodoAssistPlugins.mockReturnValue(fallbackRegistry as never);
  ...
  it("loads the plugin registry when runtime activation is explicitly requested", () => {
    ...
    expect(mocks.loadNodoAssistPlugins).toHaveBeenCalledTimes(1);
    expect(mockArg(mocks.loadNodoAssistPlugins).activate).toBe(true);
  });
  ...
  it("keeps explicit setup web provider cache opt-outs", () => {
    ...
    expect(mockArg(mocks.loadNodoAssistPlugins).cache).toBe(false);
    expect(mockArg(mocks.loadNodoAssistPlugins).onlyPluginIds).toEqual(["brave"]);
  });
});
