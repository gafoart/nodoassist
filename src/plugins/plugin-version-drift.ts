...
export function resolvePluginVersionDriftUpdateCommand(entry: PluginVersionDriftEntry): string {
  ...
  if (exactNpmPackageName) {
    ...
    if (parseRegistryNpmSpec(exactNpmTarget)?.selectorKind === "exact-version") {
      return `nodoassist plugins update ${exactNpmTarget}`;
    }
  }
  return `nodoassist plugins update ${entry.pluginId}`;
}
...
function isPluginEnabled(config: NodoAssistConfig | undefined, pluginId: string): boolean {
 ...
 *   `version` field of the installed nodoassist package.json).
 ...
 * @param params.config The merged daemon-side NodoAssistConfig (optional).
