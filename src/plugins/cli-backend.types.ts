...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before NodoAssist consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether NodoAssist should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume NodoAssist's bundle
   ...
   *
   * Use this when the backend should consume a canonical NodoAssist auth profile
   ...
   * `profile-only` treats the selected NodoAssist auth profile as the sole auth
   ...
   * as static config, such as mapping NodoAssist thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside NodoAssist's tool
