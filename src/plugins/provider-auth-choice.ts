...
function resolveConfiguredDefaultModelPrimary(cfg: NodoAssistConfig): string | undefined {
...
async function applyDefaultModelFromAuthChoice(params: {
  config: NodoAssistConfig;
  configBeforeProviderAuth?: NodoAssistConfig;
  selectedModel: string;
  selectedModelDisplay?: string;
  preserveExistingDefaultModel: boolean | undefined;
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  workspaceDir?: string;
  runSelectedModelHook: (config: NodoAssistConfig) => Promise<void>;
...
export async function runProviderPluginAuthMethod(params: {
  ...
  allowSecretRefPrompt?: boolean;
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: NodoAssistConfig; defaultModel?: string }> {
...
export async function applyAuthChoiceLoadedPluginProvider(
  ...
  const resolveScopedRuntimeProviders = (config: NodoAssistConfig): ProviderPlugin[] =>
    resolvePluginProviders({
      config,
      workspaceDir,
      env: params.env,
...
export async function applyAuthChoicePluginProvider(
  ...
  if (!provider) {
    await params.prompter.note(
      `${options.label} auth plugin is not available. Install or enable the plugin, then rerun onboarding. If this started after an update, run "nodoassist doctor --fix" first.`,
      options.label,
    );
    return { config: nextConfig };
  }
