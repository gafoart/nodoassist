...
function makePluginDir() {
  return makeTrackedTempDir("nodoassist-manifest-model-catalog", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "nodoassist.plugin.json"), JSON.stringify(manifest), "utf8");
}
