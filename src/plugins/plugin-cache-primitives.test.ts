...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as NodoAssistConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as NodoAssistConfig;
    const load = vi.fn(async (config?: NodoAssistConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: NodoAssistConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
