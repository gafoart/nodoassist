...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "nodoassist-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "NODOASSIST_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.NODOASSIST_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "nodoassist.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolvePackageRootsForBundledPlugins(): string[] {
  const argvRoot = resolveNodoAssistPackageRootSync({ argv1: process.argv[1] });
  const moduleRoot = resolveNodoAssistPackageRootSync({ moduleUrl: import.meta.url });
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveNodoAssistPackageRootSync({ moduleUrl: import.meta.url });
...
function createBundledPluginsDirCacheKey(env: NodeJS.ProcessEnv): string {
  return JSON.stringify({
    disabled: env.NODOASSIST_DISABLE_BUNDLED_PLUGINS ?? "",
    override: env.NODOASSIST_BUNDLED_PLUGINS_DIR ?? "",
    ...
    nodoAssistHome: env.NODOASSIST_HOME ?? "",
    home: env.HOME ?? "",
...
function resolveBundledPluginsDirUncached(env: NodeJS.ProcessEnv): string | undefined {
  ...
  const override = env.NODOASSIST_BUNDLED_PLUGINS_DIR?.trim();
