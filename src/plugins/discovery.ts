...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @nodoassist/voice-call -> voice-call).
...
function pushInvalidPackageExtensionDiagnostic(params: {
  ...
  if (params.resolution.status === "empty") {
    params.diagnostics.push({
      level: "error",
      source: params.source,
      message: "package.json nodoassist.extensions is empty",
    });
    return true;
  }
  return false;
...
function addCandidate(params: {
  ...
  params.candidates.push({
    ...
    rootDir: resolvedRoot,
    origin: params.origin,
    format: params.format ?? "nodoassist",
...
function addLegacyNpmDeclarationDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `legacy npm plugin declaration ignored for "${declaration.pluginId}"; run "nodoassist doctor --fix" to install ${declaration.npmSpec} into the managed plugin root`,
  });
  return true;
}
  ...
  for (const loadPath of params.loadPaths) {
    ...
    if (bundledAlias) {
      params.result.diagnostics.push({
        level: "warn",
        source: trimmed,
        message: `ignored plugins.load.paths entry that points at NodoAssist's ${bundledAlias.kind} bundled plugin directory; remove this redundant path or run nodoassist doctor --fix`,
      });
      continue;
    }
    discoverFromPath({
...
export function discoverNodoAssistPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  const scopedResult = tracePluginLifecyclePhase(
    ...
    () => {
      ...
      if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
        // Keep workspace auto-discovery constrained to the NodoAssist extensions root.
