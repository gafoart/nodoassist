...
import { runNodoAssistStateWriteTransaction } from "../state/nodoassist-state-db.js";
import {
...
function tempStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-metadata-memo-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePersistedIndex(params: {
  ...
  const manifestPath = params.manifestPath ?? path.join(pluginDir, "nodoassist.plugin.json");
...
function writeRecoverableNpmPlugin(params: {
  ...
  writeJson(path.join(packageDir, "nodoassist.plugin.json"), { id: params.pluginId });
}
...
function makeIndex(
  ...
  const manifestPath = options.manifestPath ?? path.join(rootDir, "nodoassist.plugin.json");
  return {
    version: 1,
    hostContractVersion: "test",
    compatRegistryVersion: "test",
...
function makeManifestRegistry(pluginId = "demo"): PluginManifestRegistry {
  const plugin: PluginManifestRecord = {
    ...
    manifestPath: `/plugins/${pluginId}/nodoassist.plugin.json`,
    origin: "global",
  };
...
describe("loadPluginMetadataSnapshot process memo", () => {
  ...
  it("does not emit metadata scan spans for hot memo hits", () => {
    ...
    const env = {
      ...
      NODOASSIST_DIAGNOSTICS_TIMELINE_PATH: timelinePath,
    };
    touchPersistedIndex(stateDir);
    loadPluginRegistrySnapshotWithMetadata.mockReturnValue({
      source: "persisted",
  ...
  it("reuses the expanded freshness fingerprint on hot cache hits", () => {
    const stateDir = tempStateDir();
    const manifestPath = path.join(stateDir, "extensions", "demo", "nodoassist.plugin.json");
  ...
  it.each([
    ["manifest", "nodoassist.plugin.json", "manifestPath"],
  ...
  it("does not fingerprint persisted plugin paths outside the plugin root", () => {
    const stateDir = tempStateDir();
    const outsideManifestPath = path.join(stateDir, "outside", "nodoassist.plugin.json");
