...
const nixEnv: NodeJS.ProcessEnv = { NODOASSIST_NIX_MODE: "1" };
...
describe("plugin hardlink policy", () => {
  ...
  it("does not treat NODOASSIST_NIX_MODE as enough by itself", () => {
    expect(
      shouldRejectHardlinkedPluginFiles({
        origin: "config",
        rootDir: "/tmp/plugin",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      expect(isNixStorePluginRoot("/nix/store/abc-nodoassist-plugin")).toBe(true);
      expect(isNixStorePluginRoot("/tmp/nix/store/abc-nodoassist-plugin")).toBe(false);
      expect(
        shouldRejectHardlinkedPluginFiles({
          origin: "config",
          rootDir: "/nix/store/abc-nodoassist-plugin",
          env: nixEnv,
        }),
      ).toBe(false);
    },
