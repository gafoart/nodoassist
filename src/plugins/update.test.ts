...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-nodoassist",
    targetDir: params?.targetDir ?? "/tmp/opik-nodoassist",
...
function createSuccessfulClawHubUpdateResult(params?: {
  ...
  return {
    ...
    targetDir: params?.targetDir ?? "/tmp/nodoassist-plugins/legacy-chat",
...
function createEnabledDemoClawHubInstallConfig(): NodoAssistConfig {
    ...
    plugins: {
      installs: {
        "nodoassist-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/nodoassist-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createInstalledPackageDir(params: {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createNodoAssistPeerLinkFixtures(
  plugins: Array<{ pluginId: string; packageName: string }>,
) {
  const peerTarget = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-peer-target-"));
  ...
  const installPaths = Object.fromEntries(
    plugins.map(({ pluginId, packageName }) => [
      ...
      createInstalledPackageDir({
        name: packageName,
        version: "2026.5.4",
        peerDependencies: { nodoassist: ">=2026.5.4" },
      }),
    ]),
  );
  const peerLinkPath = (pluginId: string) =>
    path.join(installPaths[pluginId], "node_modules", "nodoassist");
...
function mockNpmViewMetadata(params: {
  ...
  integrity?: string;
  shasum?: string;
  nodoassist?: Record<string, unknown>;
}) {
  runCommandWithTimeoutMock.mockResolvedValueOnce({
    ...
    stdout: JSON.stringify({
      ...
      ...(params.nodoassist ? { nodoassist: params.nodoassist } : {}),
    }),
    stderr: "",
  });
}
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@nodoassist/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  const install = params.result.config.plugins?.installs?.["nodoassist-codex-app-server"];
  expect(install?.source).toBe("npm");
  expect(install?.spec).toBe(params.spec);
  expect(install?.installPath).toBe("/tmp/nodoassist-codex-app-server");
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-nodoassist",
        spec: "@opik/opik-nodoassist",
        integrity: "sha512-old",
        installPath: "/tmp/opik-nodoassist",
      }),
      pluginIds: ["opik-nodoassist"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-nodoassist",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-nodoassist",
        spec: "@opik/opik-nodoassist@0.2.5",
        integrity: "sha512-old",
        installPath: "/tmp/opik-nodoassist",
      }),
      pluginIds: ["opik-nodoassist"],
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        ...
        spec: "nodoassist-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/nodoassist-codex-app-server",
      }),
      pluginIds: ["nodoassist-codex-app-server"],
      specOverrides: {
        "nodoassist-codex-app-server": "nodoassist-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "nodoassist-codex-app-server",
        targetDir: "/tmp/nodoassist-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "nodoassist-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("trusts official catalog npm updates when the installed package matches the catalog", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@nodoassist/acpx",
        installPath,
        resolvedName: "@nodoassist/acpx",
        resolvedSpec: "@nodoassist/acpx@2026.5.2-beta.1",
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/acpx");
    expect(npmInstallCall()?.expectedPluginId).toBe("acpx");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@nodoassist/acpx@2026.5.2-beta.2");
  });
  ...
  it("pins unchanged official npm records during official sync", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@nodoassist/acpx",
        resolvedSpec: "@nodoassist/acpx@2026.5.2",
    ...
    expect(result.config.plugins?.installs?.acpx?.spec).toBe("@nodoassist/acpx@2026.5.2");
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/acpx",
          version: "2026.5.1",
          resolvedSpec: "@nodoassist/acpx@2026.5.1",
        },
      }),
    );
  ...
  it("keeps integrity drift checks for exact prerelease-only official pins", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/voice-call",
          version: "0.0.2-beta.1",
          resolvedSpec: "@nodoassist/voice-call@0.0.2-beta.1",
        },
      }),
    );
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "voice-call",
        spec: "@nodoassist/voice-call@0.0.2-beta.1",
        installPath,
        resolvedName: "@nodoassist/voice-call",
        resolvedSpec: "@nodoassist/voice-call@0.0.2-beta.1",
  ...
  it("keeps integrity drift checks for exact official pins during beta fallback", async () => {
    ...
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error: "No matching version found for @nodoassist/acpx@beta",
    ...
    expect(npmInstallCall(0)?.spec).toBe("@nodoassist/acpx@beta");
    expect(npmInstallCall(0)?.expectedIntegrity).toBeUndefined();
    expect(npmInstallCall(1)?.spec).toBe("@nodoassist/acpx");
  ...
  it("does not skip trusted official default updates when latest resolves to the installed prerelease", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@nodoassist/acpx@2026.5.2-beta.2",
        installPath,
        integrity: "sha512-beta",
        shasum: "beta",
        resolvedName: "@nodoassist/acpx",
  ...
  it("updates trusted official npm plugins when latest resolves to a stable correction release", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/acpx",
          version: "2026.5.3-1",
          resolvedSpec: "@nodoassist/acpx@2026.5.3-1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@nodoassist/acpx",
        resolvedSpec: "@nodoassist/acpx@2026.5.3",
  ...
  it("does not skip unchanged npm plugins when package metadata requires a newer plugin API", async () => {
    vi.stubEnv("NODOASSIST_COMPATIBILITY_HOST_VERSION", "2026.5.28-beta.3");
    const installPath = createInstalledPackageDir({
      name: "@nodoassist/msteams",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/msteams",
          version: "2026.5.28-beta.3",
          resolvedSpec: "@nodoassist/msteams@2026.5.28-beta.3",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        resolvedSpec: "@nodoassist/msteams@2026.5.28-beta.4",
        integrity: "sha512-newer",
        shasum: "newer",
      }),
      pluginIds: ["msteams"],
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/msteams");
  ...
  it("repairs missing nodoassist peer links before skipping unchanged npm plugins", async () => {
    const installPath = createInstalledPackageDir({
      name: "@nodoassist/codex",
      version: "2026.5.3",
      peerDependencies: { nodoassist: ">=2026.5.3" },
    });
    mockNpmViewMetadata({
      name: "@nodoassist/codex",
      version: "2026.5.3",
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/codex",
          version: "2026.5.3",
          resolvedSpec: "@nodoassist/codex@2026.5.3",
        },
      }),
    );
    const config: NodoAssistConfig = {
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/codex");
    expect(npmInstallCall()?.mode).toBe("update");
  ...
  it("skips unchanged npm plugins when the nodoassist peer link already resolves", async () => {
    const installPath = createInstalledPackageDir({
      name: "@nodoassist/codex",
      version: "2026.5.3",
      peerDependencies: { nodoassist: ">=2026.5.3" },
    });
    fs.mkdirSync(path.join(installPath, "node_modules", "nodoassist"), { recursive: true });
    mockNpmViewMetadata({
      name: "@nodoassist/codex",
      version: "2026.5.3",
      integrity: "sha512-same",
  ...
  it("repairs nodoassist peer links after batch npm updates prune earlier plugin links", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@nodoassist/brave-plugin" },
      { pluginId: "codex", packageName: "@nodoassist/codex" },
      { pluginId: "discord", packageName: "@nodoassist/discord" },
    ];
    const { installPaths, peerLinkPath, linkPeer } = createNodoAssistPeerLinkFixtures(plugins);
  ...
  it("repairs sibling nodoassist peer links after a targeted npm update prunes the shared install tree", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@nodoassist/brave-plugin" },
      { pluginId: "codex", packageName: "@nodoassist/codex" },
      { pluginId: "discord", packageName: "@nodoassist/discord" },
      ...
      return Promise.resolve(
        createSuccessfulNpmUpdateResult({
          ...
          npmResolution: {
            name: "@nodoassist/codex",
            version: "2026.5.4",
            resolvedSpec: "@nodoassist/codex@2026.5.4",
          },
        }),
      );
    });
  ...
  it("continues repairing sibling nodoassist peer links after one recorded npm install cannot be relinked", async () => {
    const plugins = [
      { pluginId: "brave", packageName: "@nodoassist/brave-plugin" },
      { pluginId: "codex", packageName: "@nodoassist/codex" },
    ];
      ...
      config: {
        plugins: {
          installs: {
            broken: {
              source: "npm",
              spec: "@nodoassist/broken-plugin",
              installPath: brokenInstallPath,
              resolvedName: "@nodoassist/broken-plugin",
              resolvedVersion: "2026.5.4",
              resolvedSpec: "@nodoassist/broken-plugin@2026.5.4",
            },
            ...Object.fromEntries(
              plugins.map(({ pluginId, packageName }) => [
                pluginId,
    ...
    expect(warnMessages).toEqual([
      `Could not repair nodoassist peer link for "broken" at ${brokenInstallPath}: Skipping nodoassist peerDependency link because ${path.join(brokenInstallPath, "node_modules")} is not a real directory.`,
    ]);
  });
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".nodoassist", "extensions", "lossless-claw");
    ...
    const result = await withEnvAsync({ HOME: home }, () =>
      updateNpmInstalledPlugins({
        config: createNpmInstallConfig({
          pluginId: "lossless-claw",
          spec: "@martian-engineering/lossless-claw",
          installPath: "~/.nodoassist/extensions/lossless-claw",
          resolvedName: "@martian-engineering/lossless-claw",
          resolvedVersion: "0.9.0",
          resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
          integrity: "sha512-same",
  ...
  it("uses failure cleanup when metadata probing fails and disableOnFailure is enabled", async () => {
    ...
    const message =
      'Disabled "lossless-claw" after plugin update failure; NodoAssist will continue without it. Failed to check lossless-claw: npm view failed: registry timeout';
  ...
  it("updates disabled trusted official npm installs from the channel spec when requested", async () => {
      ...
      config: {
        plugins: {
          ...
          installs: {
            codex: {
              source: "npm",
              spec: "@nodoassist/codex@2026.5.3",
              installPath,
            },
          },
        },
    ...
    expectRecordFields(result.config.plugins?.installs?.codex, {
      source: "npm",
      spec: "@nodoassist/codex@2026.5.4",
      version: "2026.5.4",
      resolvedName: "@nodoassist/codex",
      resolvedVersion: "2026.5.4",
      resolvedSpec: "@nodoassist/codex@2026.5.4",
  ...
  it("preserves exact official npm pins when official install sync is not requested", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/codex",
          version: "2026.5.28",
          resolvedSpec: "@nodoassist/codex@2026.5.28",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/codex@2026.5.28");
    expect(npmInstallCall()?.expectedPluginId).toBe("codex");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
    expect(result.changed).toBe(false);
    expectRecordFields(result.outcomes[0], {
  ...
  it("reinstalls missing exact official npm pins without official install sync", async () => {
    const extensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-missing-plugin-"));
  ...
  it.each(["@acme/demo@1.2.3", "@acme/demo@v1.2.3"])(
    ...
    async (spec) => {
      ...
      expectRecordFields(result.outcomes[0], {
        ...
        currentVersion: "1.2.3",
        nextVersion: "1.2.3",
        message: `demo is pinned to ${spec} (installed 1.2.3); registry default resolves to 1.2.4. Pass \`nodoassist plugins update @acme/demo@latest\` to follow the registry default line.`,
      });
    },
  );
  ...
  it("updates disabled trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@nodoassist/diagnostics-otel",
      }),
    );
    ...
    const config = createClawHubInstallConfig({
      ...
      clawhubFamily: "code-plugin",
      clawhubChannel: "official",
      spec: "clawhub:@nodoassist/diagnostics-otel@2026.5.3",
    });
    const result = await updateNpmInstalledPlugins({
      config: {
        ...config,
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@nodoassist/diagnostics-otel");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-otel"], {
      source: "clawhub",
      spec: "clawhub:@nodoassist/diagnostics-otel",
      version: "2026.5.4",
      clawhubPackage: "@nodoassist/diagnostics-otel",
      clawhubChannel: "official",
    });
  ...
  it("updates bare trusted official ClawHub installs through the catalog spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue(
      createSuccessfulClawHubUpdateResult({
        ...
        version: "2026.5.4",
        clawhubPackage: "@nodoassist/diagnostics-prometheus",
      }),
    );
        ...
        plugins: {
          installs: {
            "diagnostics-prometheus": {
              source: "clawhub",
              spec: "clawhub:@nodoassist/diagnostics-prometheus@2026.5.3",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@nodoassist/diagnostics-prometheus");
    ...
    expectRecordFields(result.config.plugins?.installs?.["diagnostics-prometheus"], {
      source: "clawhub",
      spec: "clawhub:@nodoassist/diagnostics-prometheus",
      version: "2026.5.4",
      clawhubPackage: "@nodoassist/diagnostics-prometheus",
      clawhubChannel: "official",
    });
  ...
  it("does not skip a risk-gated ClawHub update when the installed package is missing", async () => {
    ...
    const installPath = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-update-missing-"));
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:demo");
    const message =
      'Disabled "demo" after plugin update failure; NodoAssist will continue without it. Failed to update demo: Update cancelled; rerun with --acknowledge-clawhub-risk to continue after reviewing the warning. (ClawHub clawhub:demo).';
    expect(result.changed).toBe(true);
    expect(result.config.plugins?.entries?.demo).toEqual({
      enabled: false,
      config: { preserved: true },
  ...
  it("disables an existing ClawHub plugin when its current release is blocked", async () => {
    ...
    const message =
      'Disabled "demo" after plugin update failure; NodoAssist will continue without it. Failed to update demo: ClawHub blocked this release; update was not started. (ClawHub clawhub:demo).';
    expect(warn).toHaveBeenCalledWith(message);
    expect(result.outcomes).toEqual([
      {
        pluginId: "demo",
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-nodoassist@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-nodoassist@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-nodoassist" (@opik/opik-nodoassist@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-nodoassist: aborted: npm package integrity drift detected for @opik/opik-nodoassist@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ok: false,
        code: "npm_package_not_found",
        error: "Package not found on npm: @nodoassist/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@nodoassist/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @nodoassist/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedName: "nodoassist-codex-app-server",
        resolvedSpec: "nodoassist-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "nodoassist-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "nodoassist-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "nodoassist-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "nodoassist-codex-app-server",
      }),
      specOverrides: {
        "nodoassist-codex-app-server": "nodoassist-codex-app-server@beta",
      },
      expectedSpec: "nodoassist-codex-app-server@beta",
      expectedRecordSpec: "nodoassist-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "nodoassist-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "nodoassist-codex-app-server",
      });
  ...
  it("preserves explicit official npm tag overrides during manual updates", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/acpx",
          version: "2026.5.3-beta.1",
          resolvedSpec: "@nodoassist/acpx@2026.5.3-beta.1",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      ...
      pluginIds: ["acpx"],
      specOverrides: {
        acpx: "@nodoassist/acpx@beta",
      },
    });

    expectNpmUpdateCall({
      spec: "@nodoassist/acpx@beta",
      expectedPluginId: "acpx",
    });
  ...
  it("targets the exact core version for official extended-stable updates and preserves intent", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/acpx",
          version: "2026.7.33",
          resolvedSpec: "@nodoassist/acpx@2026.7.33",
        },
      }),
    );
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        ...
        installPath,
        resolvedName: "@nodoassist/acpx",
        resolvedSpec: "@nodoassist/acpx@2026.7.21",
    ...
    expectNpmUpdateCall({
      spec: "@nodoassist/acpx@2026.7.33",
      expectedPluginId: "acpx",
    });
    expectRecordFields(result.config.plugins?.installs?.acpx, {
      spec: "@nodoassist/acpx",
  ...
  it("preserves an explicit official pin during extended-stable updates", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@nodoassist/acpx@2026.6.33",
        installPath,
        resolvedName: "@nodoassist/acpx",
        resolvedSpec: "@nodoassist/acpx@2026.6.33",
  ...
  it("lets an explicit bare official spec opt a legacy pin into exact-core tracking", async () => {
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "acpx",
        spec: "@nodoassist/acpx@2026.6.21",
        installPath,
        resolvedName: "@nodoassist/acpx",
        resolvedSpec: "@nodoassist/acpx@2026.6.21",
        resolvedVersion: "2026.6.21",
      }),
      pluginIds: ["acpx"],
      specOverrides: { acpx: "@nodoassist/acpx" },
      syncOfficialPluginInstalls: true,
      officialPluginUpdateChannel: "extended-stable",
      coreVersion: "2026.7.33",
    });
  ...
  it("falls back to the default npm spec when a beta tag is unavailable", async () => {
    installPluginFromNpmSpecMock
      .mockResolvedValueOnce({
        ok: false,
        error:
          "npm ERR! code ETARGET\nnpm ERR! No matching version found for nodoassist-codex-app-server@beta.",
      })
      .mockResolvedValueOnce(
        createSuccessfulNpmUpdateResult({
          pluginId: "nodoassist-codex-app-server",
          ...
          npmResolution: {
            name: "nodoassist-codex-app-server",
            version: "0.2.6",
            resolvedSpec: "nodoassist-codex-app-server@0.2.6",
          },
        }),
      );
    ...
    expect(npmInstallCall(0)?.spec).toBe("nodoassist-codex-app-server@beta");
    expect(npmInstallCall(1)?.spec).toBe("nodoassist-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "nodoassist-codex-app-server" has no beta npm release for nodoassist-codex-app-server@beta; using nodoassist-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "nodoassist-codex-app-server",
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Updated nodoassist-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback used nodoassist-codex-app-server because nodoassist-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback).toEqual({
      requestedSpec: "nodoassist-codex-app-server@beta",
      usedSpec: "nodoassist-codex-app-server",
      requestedLabel: "@beta",
      usedLabel: "@latest",
      reason: "unavailable",
      message:
        "plugin channel fallback: nodoassist-codex-app-server used @latest because @beta was unavailable",
    });
  });
  ...
  it("reports npm beta fallback as tentative during dry-run checks", async () => {
    ...
    expect(result.outcomes[0]?.message).toBe(
      "Would update nodoassist-codex-app-server: unknown -> 0.2.6. (warning: beta channel fallback would use nodoassist-codex-app-server because nodoassist-codex-app-server@beta could not be used).",
    );
    expect(result.outcomes[0]?.channelFallback?.message).toBe(
      "plugin channel fallback: nodoassist-codex-app-server would use @latest because @beta was unavailable",
    );
  });
  ...
  it("falls back to the default npm spec when the beta package exists but is invalid", async () => {
    ...
    expect(npmInstallCall(1)?.spec).toBe("nodoassist-codex-app-server");
    expect(warnMessages).toEqual([
      'Plugin "nodoassist-codex-app-server" failed beta npm update for nodoassist-codex-app-server@beta; using nodoassist-codex-app-server instead. Core update can still complete.',
    ]);
    expectCodexAppServerInstallState({
      result,
      spec: "nodoassist-codex-app-server",
    ...
    expect(result.outcomes[0]?.channelFallback).toMatchObject({
      ...
      reason: "failed",
      message:
        "plugin channel fallback: nodoassist-codex-app-server used @latest after @beta failed",
    });
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update nodoassist-codex-app-server: npm package not found for nodoassist-codex-app-server.",
        channelFallback: {
          requestedSpec: "nodoassist-codex-app-server@beta",
          usedSpec: "nodoassist-codex-app-server",
          requestedLabel: "@beta",
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to check nodoassist-codex-app-server: npm package not found for nodoassist-codex-app-server.",
        channelFallback: {
          requestedSpec: "nodoassist-codex-app-server@beta",
          usedSpec: "nodoassist-codex-app-server",
          requestedLabel: "@beta",
          ...
          reason: "failed",
          message:
            "plugin channel fallback: nodoassist-codex-app-server would use @latest after @beta failed",
        },
      },
    ]);
  });
  ...
  it("preserves explicit npm tags when updating on the beta channel", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: createCodexAppServerInstallConfig({
        spec: "nodoassist-codex-app-server@rc",
      }),
      pluginIds: ["nodoassist-codex-app-server"],
      updateChannel: "beta",
      dryRun: true,
  ...
  it("does not fall back to npm for blocked official ClawHub artifact downloads", async () => {
    ...
    installPluginFromClawHubMock.mockResolvedValueOnce({
      ...
      code: "clawhub_download_blocked",
      error:
        'ClawHub blocked artifact download for "@nodoassist/discord@2026.5.16-beta.5"; install was not started. ClawHub /api/v1/packages/%40nodoassist%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded.',
    ...
    const result = await updateNpmInstalledPlugins({
      config: createClawHubInstallConfig({
        ...
        installPath,
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@nodoassist/discord",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
    ...
    expect(clawHubInstallCall()?.spec).toBe("clawhub:@nodoassist/discord@beta");
    expect(installPluginFromNpmSpecMock).not.toHaveBeenCalled();
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        message:
          'Skipped discord ClawHub update: ClawHub blocked artifact download for "@nodoassist/discord@2026.5.16-beta.5"; install was not started. ClawHub /api/v1/packages/%40nodoassist%2Fdiscord/versions/2026.5.16-beta.5/artifact/download failed (403): Blocked: this package release has been flagged as malicious and cannot be downloaded. Existing installed plugin left unchanged.',
      },
    ]);
  ...
  it("uses the default npm spec when beta ClawHub falls back before an artifact block", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/discord",
          version: "2026.5.16",
          resolvedSpec: "@nodoassist/discord@2026.5.16",
        },
      }),
    );
    ...
    expect(clawHubInstallCall(0)?.spec).toBe("clawhub:@nodoassist/discord@beta");
    expect(clawHubInstallCall(1)?.spec).toBe("clawhub:@nodoassist/discord");
    expect(npmInstallCall()?.spec).toBe("@nodoassist/discord");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
      spec: "@nodoassist/discord@2026.5.16",
      installPath: "/tmp/nodoassist-plugins/discord",
      version: "2026.5.16",
    });
    expect(result.outcomes[0]?.message).toBe(
      "Updated discord: 2026.5.12 -> 2026.5.16. (warning: official ClawHub artifact fallback used @nodoassist/discord).",
    );
    expect(warnMessages).toEqual([
      'Plugin "discord" has no beta ClawHub release for clawhub:@nodoassist/discord@beta; using clawhub:@nodoassist/discord instead. Core update can still complete.',
      'Plugin "discord" could not download official ClawHub artifact for clawhub:@nodoassist/discord; using npm @nodoassist/discord instead. Core update can still complete.',
    ]);
  });
  ...
  it("uses exact-core npm when an official ClawHub install falls back on extended-stable", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/discord",
          version: "2026.7.33",
          resolvedSpec: "@nodoassist/discord@2026.7.33",
        },
      }),
    );
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/discord@2026.7.33");
    expectRecordFields(result.config.plugins?.installs?.discord, {
      source: "npm",
  ...
  it("reports npm dry-run versions for trusted official ClawHub artifact fallback", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@nodoassist/discord",
        version: "2026.5.16-beta.5",
        resolvedSpec: "@nodoassist/discord@2026.5.16-beta.5",
      },
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/discord@beta");
    expect(npmInstallCall()?.dryRun).toBe(true);
    expect(result.outcomes).toEqual([
      {
        pluginId: "discord",
        ...
        message:
          "discord is up to date (2026.5.16-beta.5). (warning: official ClawHub artifact fallback would use @nodoassist/discord@beta).",
      },
    ]);
  });
    ...
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update discord: artifact unavailable (ClawHub clawhub:@nodoassist/discord@beta).",
      },
    ]);
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      targetDir: "/tmp/nodoassist-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/voice-call");
    expect(npmInstallCall()?.expectedPluginId).toBe("voice-call");
    expect(result.config.plugins?.allow).toEqual(["@nodoassist/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@nodoassist/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@nodoassist/voice-call");
    expect(result.config.plugins?.entries?.["@nodoassist/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expectRecordFields(result.config.plugins?.installs?.["@nodoassist/voice-call"], {
      source: "npm",
      spec: "@nodoassist/voice-call",
      installPath: "/tmp/nodoassist-voice-call",
      version: "0.0.2",
    });
  ...
  it("keeps authored plugin config shape when only the install key migrates", async () => {
    ...
    expect(result.config.plugins).toEqual({
      installs: {
        "@nodoassist/voice-call": expect.objectContaining({
          source: "npm",
          spec: "@nodoassist/voice-call",
          installPath: "/tmp/nodoassist-voice-call",
        }),
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@nodoassist/context-engine",
      targetDir: "/tmp/nodoassist-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
      ...
      config: {
        plugins: {
          ...
          installs: {
            "context-engine": {
              source: "npm",
              spec: "@nodoassist/context-engine",
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@nodoassist/context-engine");
    expectRecordFields(result.config.plugins?.installs?.["@nodoassist/context-engine"], {
      source: "npm",
      spec: "@nodoassist/context-engine",
      installPath: "/tmp/nodoassist-context-engine",
      version: "0.0.2",
    });
  ...
  it("forwards dangerous force unsafe install to plugin update installers", async () => {
    ...
    expect(npmInstallCall()?.spec).toBe("nodoassist-codex-app-server@beta");
    expect(npmInstallCall()?.dangerouslyForceUnsafeInstall).toBe(true);
    expect(npmInstallCall()?.expectedPluginId).toBe("nodoassist-codex-app-server");
  });

  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/nodoassist/extensions/demo";
    const extensionsDir = "/var/nodoassist/extensions";
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    resolveBundledPluginSourcesMock.mockReturnValue(new Map());
    const env = { NODOASSIST_HOME: "/srv/nodoassist-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/nodoassist-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/nodoassist-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@nodoassist/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@nodoassist/legacy-chat@2.0.0",
        },
      }),
    );
      ...
      externalizedBundledPluginBridges: [
        {
          ...
          npmSpec: "@nodoassist/legacy-chat",
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/legacy-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
      spec: "@nodoassist/legacy-chat",
      installPath: "/tmp/nodoassist-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@nodoassist/legacy-chat",
  ...
  it("marks official externalized bundled npm installs as trusted", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "voice-call",
        targetDir: "/tmp/nodoassist-plugins/voice-call",
        version: "0.0.2-beta.1",
      }),
    );
  ...
  it("falls back from ClawHub to npm only when the ClawHub package is absent", async () => {
    ...
    expect(result.summary.warnings).toEqual([
      "ClawHub clawhub:legacy-chat@2026.5.1-beta.2 unavailable for legacy-chat; falling back to npm @nodoassist/legacy-chat.",
    ]);
    expect(result.summary.errors).toStrictEqual([]);
    expectRecordFields(result.config.plugins?.installs?.["legacy-chat"], {
      source: "npm",
  ...
  it("uses exact-core npm when an official ClawHub bridge falls back on extended-stable", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        npmResolution: {
          name: "@nodoassist/voice-call",
          version: "2026.7.33",
          resolvedSpec: "@nodoassist/voice-call@2026.7.33",
        },
      }),
    );
      ...
      externalizedBundledPluginBridges: [
        {
          bundledPluginId: "voice-call",
          preferredSource: "clawhub",
          clawhubSpec: "clawhub:@nodoassist/voice-call",
          npmSpec: "@nodoassist/voice-call",
          channelIds: ["voice-call"],
        },
      ],
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/voice-call@2026.7.33");
    expect(npmInstallCall()?.trustedSourceLinkedOfficialInstall).toBe(true);
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/nodoassist-plugins/default-chat",
        version: "2.0.0",
      }),
    );
      ...
      externalizedBundledPluginBridges: [
        {
          ...
          npmSpec: "@nodoassist/default-chat",
    ...
    expect(npmInstallCall()?.spec).toBe("@nodoassist/default-chat");
    expect(npmInstallCall()?.mode).toBe("update");
    ...
    expectRecordFields(result.config.plugins?.installs?.["default-chat"], {
      source: "npm",
      spec: "@nodoassist/default-chat",
      installPath: "/tmp/nodoassist-plugins/default-chat",
      version: "2.0.0",
    });
  });
      ...
      config: {
        ...
        plugins: {
          ...
          installs: {
            "legacy-chat": {
              source: "npm",
              spec: "@nodoassist/legacy-chat@1.2.3",
              resolvedSpec: "@nodoassist/legacy-chat@1.2.3",
              installPath: "/tmp/nodoassist-plugins/legacy-chat",
            },
          },
        },
