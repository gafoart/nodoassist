...
const fixtureRoot = makeTrackedTempDir("nodoassist-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadNodoAssistPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "nodoassist.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("nodoassist plugins inspect <id> --runtime --json");
    expect(summary).toContain("nodoassist plugins list");
  });
});
