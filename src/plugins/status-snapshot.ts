...
/** Control-plane plugin status shape used by `nodoassist plugins status` style surfaces. */
...
function isPluginLifecycleTraceEnabled(): boolean {
  const raw = process.env.NODOASSIST_PLUGIN_LIFECYCLE_TRACE?.trim().toLowerCase();
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "nodoassist";
