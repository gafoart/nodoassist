...
import { normalizeOptionalString as normalizeString } from "@nodoassist/normalization-core/string-coerce";
...
function getConfigPath(config: NodoAssistConfig | undefined, path: string): unknown {
...
function setConfigPath(target: NodoAssistConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: NodoAssistConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: NodoAssistConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: NodoAssistConfig) =>
