...
import { resolveNodoAssistAgentSqlitePath } from "../state/nodoassist-agent-db.paths.js";
import { resolveMemorySearchConfig, resolveMemorySearchSyncConfig } from "./memory-search.js";

const asConfig = (cfg: NodoAssistConfig): NodoAssistConfig => ({
  ...cfg,
...
describe("memory search config", () => {
  ...
  function configWithDefaultProvider(provider: string): NodoAssistConfig {
    return asConfig({
      agents: {
        defaults: {
          memorySearch: {
  ...
  it("defaults provider to openai when unspecified", () => {
    ...
    expect(resolved?.store.databasePath).toBe(
      resolveNodoAssistAgentSqlitePath({ agentId: "main" }),
    );
  });
