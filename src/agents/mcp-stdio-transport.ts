/**
 * NodoAssist stdio transport wrapper for MCP server subprocesses.
 */
...
type NodoAssistStdioServerParameters = {
  command: string;
  args?: string[];
...
export class NodoAssistStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: NodoAssistStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "NodoAssistStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
