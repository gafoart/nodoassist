 ...
 *
 * Registers provider-specific stream functions and rewrites models that need NodoAssist-managed transport semantics.
 */
...
const PROVIDER_SIMPLE_COMPLETION_API_PREFIX = "nodoassist-provider-simple:";
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `nodoassist-anthropic-vertex-simple:${suffix}`;
}
...
function applyProviderSimpleCompletionWrapper(model: Model, cfg?: NodoAssistConfig): Model {
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use NodoAssist's transport so resolved request auth is applied.
  const transportModel = {
    ...model,
    baseUrl: normalizeCodexResponsesBaseUrlForOpenAISdk(model.baseUrl),
  } as Model;
  const api = resolveTransportAwareSimpleApi(model.api);
  const streamFn = createNodoAssistTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
