...
import type { AssistantMessage, ToolResultMessage, UserMessage } from "nodoassist/plugin-sdk/llm";
...
describe("sanitizeSessionMessagesImages", () => {
  ...
  describe("thought_signature stripping", () => {
    it("strips msg_-prefixed thought_signature from assistant message content blocks", async () => {
      // msg_ values are NodoAssist message ids, not provider signatures.
      const input = castAgentMessages([
        {
          role: "assistant",
          content: [
