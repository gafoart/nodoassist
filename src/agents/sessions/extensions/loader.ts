...
import * as bundledLlm from "nodoassist/plugin-sdk/llm";
...
import { installNodoAssistInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "nodoassist/plugin-sdk/agent-core": bundledAgentCore,
  "@nodoassist/plugin-sdk/agent-core": bundledAgentCore,
  "nodoassist/plugin-sdk/llm": bundledLlm,
  "@nodoassist/plugin-sdk/llm": bundledLlm,
  "nodoassist/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@nodoassist/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@nodoassist\/plugin-sdk|nodoassist\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "nodoassist/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@nodoassist/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installNodoAssistInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.nodoassist && typeof pkg.nodoassist === "object") {
      return pkg.nodoassist as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "nodoassist.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "nodoassist" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "nodoassist" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with NodoAssist manifest or index.ts
