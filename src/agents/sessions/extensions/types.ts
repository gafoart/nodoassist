...
export interface ExtensionUIContext {
   ...
   * extend `CustomEditor` from `nodoassist/plugin-sdk/agent-sessions` and call
   * `super.handleInput(data)` for keys you don't handle.
   *
   * @example
   * ```ts
   * import { CustomEditor } from "nodoassist/plugin-sdk/agent-sessions";
   *
...
export interface ExtensionContext {
  ...
  /** Gracefully shut down NodoAssist and exit. Available in all contexts. */
  shutdown(): void;
...
export interface ProviderModelConfig {
  ...
  reasoning: boolean;
  /** Maps NodoAssist thinking levels to provider/model-specific values; null marks a level unsupported. */
