...
type SkillDiscoveryMode = "nodoassist" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { nodoassist?: ResourceManifest };
    return pkg.nodoassist ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "nodoassist");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "nodoassist-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "nodoassist"),
      projectMetadata,
    ...
    // User extensions from ~/.nodoassist/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.nodoassist/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "nodoassist"),
      userMetadata,
