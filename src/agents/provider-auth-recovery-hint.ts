...
function resolveProviderAuthLoginCommand(params: {
  ...
  return formatCliCommand(`nodoassist models auth login --provider ${providerId}`);
}
...
export function buildProviderAuthRecoveryHint(params: {
  ...
  if (params.includeConfigure !== false) {
    parts.push(`\`${formatCliCommand("nodoassist configure")}\``);
  }
  if (params.includeEnvVar) {
    parts.push("set an API key env var");
  }
  if (parts.length === 0) {
    return `Run \`${formatCliCommand("nodoassist configure")}\`.`;
  }
  if (parts.length === 1) {
    return `${parts[0]}.`;
  }
