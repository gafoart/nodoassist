...
function isNodoAssistLoopbackMcpServer(name: string, server: BundleMcpServerConfig): boolean {
  return (
    name === "nodoassist" &&
...
export function normalizeCodexMcpServerConfig(
  ...
  } else if (isNodoAssistLoopbackMcpServer(name, server)) {
    // NodoAssist's loopback MCP exposes local tools; Codex should ask for approval
