...
const CLEANUP_STATE_KEY = Symbol.for("nodoassist.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("nodoassist.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("nodoassist.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "NODOASSIST_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "NODOASSIST_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "NODOASSIST_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.NODOASSIST_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isNodoAssistSessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "nodoassist" || exe.endsWith("/nodoassist") || exe.endsWith("/nodoassist-gateway")) {
    return true;
  }
  if (
    normalized.some(
      (arg) =>
        arg === "nodoassist" ||
        arg.endsWith("/nodoassist") ||
        arg === "nodoassist.mjs" ||
        arg.endsWith("/nodoassist.mjs"),
    )
  ) {
    return true;
  }
  ...
  const hasNodoAssistCommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && hasNodoAssistCommandToken,
  );
}
...
function shouldTreatAsNonNodoAssistOwner(params: {
  payload: LockFilePayload | null;
  ...
  return !isNodoAssistSessionOwnerArgv(args);
}
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    shouldTreatAsNonNodoAssistOwner({
      payload: params.payload,
      inspected,
      heldByThisProcess: params.heldByThisProcess,
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-nodoassist-owner"],
    };
  }
