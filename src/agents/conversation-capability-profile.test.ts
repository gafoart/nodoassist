...
describe("resolveConversationCapabilityProfile", () => {
  it("prepares a direct conversation profile with sender tool restrictions", () => {
    ...
    const profile = resolveConversationCapabilityProfile({
      ...
      modelId: "gpt-5.5",
      modelApi: "responses",
      workspaceDir: "/tmp/nodoassist-direct-profile",
      cwd: "/tmp/nodoassist-direct-profile/task",
      agentDir: "/tmp/nodoassist-agent-direct-profile",
      skillsSnapshot: {
        prompt: "",
        skills: [{ name: "ops" }],
      },
    ...
    expect(profile.workspace).toMatchObject({
      workspaceRoot: "/tmp/nodoassist-direct-profile",
      runtimeRoot: "/tmp/nodoassist-direct-profile/task",
      instructionRoot: "/tmp/nodoassist-agent-direct-profile",
    });
  ...
  it("prepares a shared conversation profile with group per-sender restrictions", () => {
    ...
    const profile = resolveConversationCapabilityProfile({
      ...
      modelProvider: "openai",
      modelId: "gpt-5.5",
      workspaceDir: "/tmp/nodoassist-shared-profile",
    });
  ...
  it("keeps inherited subagent grants out of explicit overrides", () => {
    const storePath = path.join(
      os.tmpdir(),
      `nodoassist-capability-profile-inherited-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    fs.writeFileSync(
      storePath,
      JSON.stringify({
