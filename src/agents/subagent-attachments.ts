...
function resolveAttachmentLimits(config: NodoAssistConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".nodoassist", "attachments");
  const relDir = path.posix.join(".nodoassist", "attachments", attachmentId);
