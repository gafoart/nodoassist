...
async function createBundleMcpFixture(params: {
  ...
  setTestEnvValue("NODOASSIST_DISABLE_PERSISTED_PLUGIN_REGISTRY", "1");
  ...
  const fakeClaudePidPath = params.liveSession
    ...
    : undefined;
  const pluginRoot = path.join(tempHome, ".nodoassist", "extensions", "bundle-probe");
...
describe("CLI bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        tempPrefix: "nodoassist-cli-bundle-mcp-",
      });
  ...
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        liveSession: true,
        tempPrefix: "nodoassist-cli-live-cleanup-",
      });
      const context = await prepareBundleMcpExecutionContext({
