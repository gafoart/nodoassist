...
describe("tool display details", () => {
  it("summarizes tool-search code targets from described tool ids", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: "const tool = await nodoassist.tools.describe('nodoassist:core:exec'); return await nodoassist.tools.call(tool.id, { command: 'echo hi' });",
      }),
    ).toEqual({
      toolName: "nodoassist:core:exec",
  ...
  it("normalizes direct tool-search catalog ids to native display names and args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await nodoassist.tools.call("nodoassist:core:exec", { command: "echo hi" });',
      }),
    ).toEqual({
      toolName: "nodoassist:core:exec",
      displayToolName: "exec",
  ...
  it("preserves JS numeric literals in tool-search call args", () => {
    expect(
      resolveToolSearchCodeDisplayTarget({
        code: 'return await nodoassist.tools.call("web_search", { query: "NodoAssist", count: 1e3, limit: +3, threshold: .5 });',
      })?.displayArgs,
    ).toEqual({
      query: "NodoAssist",
      count: 1000,
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "NodoAssist docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "NodoAssist docs" (top 3)');
  });
  ...
  it("formats Parallel's native objective + search_queries shape", () => {
    expect(
      ...
      'for "Find the NodoAssist repository on GitHub", "nodoassist github", "nodoassist repository" (top 5)',
    );
  });
  ...
  it("summarizes exec commands with context", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          command:
            "set -euo pipefail\ngit -C /Users/adityasingh/.nodoassist/workspace status --short | head -n 3",
          workdir: "/Users/adityasingh/.nodoassist/workspace",
        },
      }),
    );
  ...
  it("uses compact workspace markers for common workspace paths", () => {
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git fetch", workdir: "/Users/peter/mantis-workspace/nodoassist" },
          detailMode: "explain",
        }),
      ),
    ...
    expect(
      formatToolDetail(
        resolveToolDisplay({
          name: "bash",
          args: { command: "git status", workdir: "/Users/peter/Projects/nodoassist" },
          detailMode: "explain",
        }),
      ),
      ...
      formatToolDetail(
        resolveToolDisplay({
          ...
          args: {
            ...
            workdir: "/root/.nodoassist/sandboxes/agent-clawsweeper-sandbox-discor-766423d0",
          },
          detailMode: "explain",
        }),
      ),
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "NodoAssist docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "NodoAssist docs"');
  });
    ...
    const detail = formatToolDetail(
      resolveToolDisplay({
        ...
        args: {
          ...
          workdir: "/Users/example/.nodoassist/workspace",
        },
        detailMode: "explain",
      }),
    );
