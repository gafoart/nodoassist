...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-managed-media-"));
  try {
    return await withEnvAsync({ NODOASSIST_STATE_DIR: stateDir }, async () => {
...
async function withOutsideHardlinkInNodoAssistTmp<T>(
  params: {
    nodoAssistTmpDir: string;
  ...
  const hardlinkPath = path.join(params.nodoAssistTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.nodoAssistTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.nodoAssistTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.nodoAssistTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const nodoAssistTmpDir = resolvePreferredNodoAssistTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred NodoAssist tmp root",
      media: path.join(nodoAssistTmpDir, "image.png"),
      expected: path.join(nodoAssistTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred NodoAssist tmp root",
      media: pathToFileURL(path.join(nodoAssistTmpDir, "photo.png")).href,
      expected: path.join(nodoAssistTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred NodoAssist tmp root",
      media: path.join(nodoAssistTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(nodoAssistTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(nodoAssistTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside nodoassist tmp root",
      media: path.join(os.tmpdir(), "outside-nodoassist", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked NodoAssist tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(nodoAssistTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(nodoAssistTmpDir, { recursive: true });
      const symlinkPath = path.join(nodoAssistTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked NodoAssist tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInNodoAssistTmp(
      {
        nodoAssistTmpDir,
  ...
  it("rejects symlinked NodoAssist tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInNodoAssistTmp(
