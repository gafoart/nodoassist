...
import { listNodoAssistPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
const NODOASSIST_ATTRIBUTION_PRODUCT = "NodoAssist";
const NODOASSIST_ATTRIBUTION_ORIGINATOR = "nodoassist";
...
function formatNodoAssistUserAgent(version: string): string {
  return `${NODOASSIST_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listNodoAssistPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: NODOASSIST_ATTRIBUTION_PRODUCT,
...
function buildOpenRouterAttributionPolicy(
  ...
  return {
    ...
    reviewNote: "Documented app attribution headers. Verified in NodoAssist runtime wrapper.",
    ...identity,
    headers: {
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": NODOASSIST_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildGoogleAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      "x-goog-api-client": `${NODOASSIST_ATTRIBUTION_ORIGINATOR}/${identity.version}`,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: NODOASSIST_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatNodoAssistUserAgent(identity.version),
    },
  };
}
...
function buildXaiAttributionPolicy(
  ...
  return {
    ...
    hook: "request-headers",
    reviewNote:
      "xAI api.x.ai accepts a standard nodoassist User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
    ...identity,
    headers: {
      originator: NODOASSIST_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
