...
function resolveConfiguredAgentIds(cfg: NodoAssistConfig): string[] {
...
async function prepareSubagentSessionContext(params: {
  ...
  try {
    ...
    if (forkedResult.status === "failed" || forkedResult.status === "missing-entry") {
      throw new Error(
        'context="fork" requested but NodoAssist could not fork the requester transcript.',
      );
    }
    ...
    if (params.contextMode === "fork") {
      if (!parentEntry || !forked) {
        ...
        return {
          status: "error",
          error: 'context="fork" requested but NodoAssist could not prepare forked context.',
        };
      }
      return {
        status: "ok",
