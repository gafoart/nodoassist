...
describe("AgentHarness lifecycle runner", () => {
  ...
  it("advertises NodoAssist embedded host capabilities", async () => {
    const harness = createNodoAssistAgentHarness();

    expect(harness.contextEngineHostCapabilities).toEqual(
      NODOASSIST_EMBEDDED_CONTEXT_ENGINE_HOST.capabilities,
    );
  });
