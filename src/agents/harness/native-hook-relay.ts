...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("nodoassist.nativeHookRelay.state");
        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by NodoAssist",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveNodoAssistCliExecutable();
  const argv =
...
function registerNativeHookRelayBridge(registration: ActiveNativeHookRelayRegistration): void {
  ...
  // directory is scoped by OS user (uid) and is shared across all NodoAssist
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `nodoassist-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("nodoassist:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.nodoassist_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `nodoassist-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveNodoAssistCliExecutable(): string {
  const envPath = process.env.NODOASSIST_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "nodoassist.mjs"),
  ...
  throw new Error("Cannot resolve NodoAssist CLI executable path for native hook relay");
}
