...
describe("NodoAssistStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    // Detached POSIX process groups let NodoAssist clean up child tool servers
    // without relying on shell-specific process trees.
    const child = new MockChildProcess();
    spawnMock.mockReturnValue(child);

    const transport = new NodoAssistStdioClientTransport({
      command: "npx",
      args: ["-y", "example-mcp"],
      env: { EXAMPLE: "1" },
      cwd: "/tmp/example",
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new NodoAssistStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
  ...
  it("reports stderr pipe errors without an unhandled error crash", async () => {
    ...
    const transport = new NodoAssistStdioClientTransport({ command: "npx", stderr: "pipe" });
    const onerror = vi.fn();
