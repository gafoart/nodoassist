...
import { resolveOptionalMediaToolFactoryPlan } from "./nodoassist-tools.media-factory-plan.js";
...
type CreateNodoAssistToolsOptions = Parameters<
  typeof import("./nodoassist-tools.js").createNodoAssistTools
>[0];
let createNodoAssistToolsForTestModule: typeof import("./nodoassist-tools.js").createNodoAssistTools;
let legacyComfyToolNames: string[];

async function createNodoAssistToolsForTest(options?: CreateNodoAssistToolsOptions) {
  return createNodoAssistToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/nodoassist.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): NodoAssistConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createNodoAssistTools: createNodoAssistToolsForTestModule } =
      await import("./nodoassist-tools.js"));
    ...
    legacyComfyToolNames = (
      await createNodoAssistToolsForTest({
        config,
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: NodoAssistConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createNodoAssistToolsForTest({
      config,
      agentDir: "/tmp/nodoassist-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: NodoAssistConfig = {};
    const workspaceDir = "/tmp/nodoassist-workspace";
