...
vi.mock("../plugins/provider-runtime.js", () => ({
  applyProviderConfigDefaultsWithPlugin: (config: NodoAssistConfig) => config,
...
let ensureNodoAssistModelsJson: typeof import("./models-config.js").ensureNodoAssistModelsJson;
let resetModelsJsonReadyCacheForTest: typeof import("./models-config.js").resetModelsJsonReadyCacheForTest;
let planNodoAssistModelsJsonWithDeps: typeof import("./models-config.plan.js").planNodoAssistModelsJsonWithDeps;
let readGeneratedModelsJson: typeof import("./models-config.test-utils.js").readGeneratedModelsJson;
const fixtureSuite = createFixtureSuite("nodoassist-models-runtime-source-");

beforeAll(async () => {
  await fixtureSuite.setup();
  ({ clearConfigCache, clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } =
    await import("../config/io.js"));
  ({ ensureNodoAssistModelsJson, resetModelsJsonReadyCacheForTest } =
    await import("./models-config.js"));
  ({ planNodoAssistModelsJsonWithDeps } = await import("./models-config.plan.js"));
...
function createOpenAiApiKeySourceConfig(): NodoAssistConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiApiKeyRuntimeConfig(): NodoAssistConfig {
...
function createCustomProviderApiKeySourceConfig(): NodoAssistConfig {
  return {
    models: {
      providers: {
        litellm: {
          ...
          apiKey: {
            source: "env",
            provider: "default",
            id: "NODOASSIST_MODEL_LITELLM_API_KEY", // pragma: allowlist secret
          },
          api: "openai-completions" as const,
          models: [],
        },
...
function createCustomProviderApiKeyRuntimeConfig(): NodoAssistConfig {
  return {
    models: {
      providers: {
        litellm: {
...
function createOpenAiHeaderSourceConfig(): NodoAssistConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiHeaderRuntimeConfig(): NodoAssistConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createOpenAiSourceConfigWithHeadersAndApiKey(): NodoAssistConfig {
...
function createOpenAiRuntimeConfigWithHeadersAndApiKey(): NodoAssistConfig {
...
function withGatewayTokenMode(config: NodoAssistConfig): NodoAssistConfig {
  return {
    ...config,
    gateway: {
      auth: {
...
async function planGeneratedProviders(params: {
  ...
  sourceConfigForSecrets: NodoAssistConfig;
}) {
  ...
  const plan = await planNodoAssistModelsJsonWithDeps(
    {
      ...
      agentDir: "/tmp/nodoassist-models-plan",
      env: {},
      existingRaw: "",
      existingParsed: null,
    },
...
describe("models-config runtime source snapshot", () => {
  ...
  it("projects cloned runtime configs onto source snapshot when preserving provider auth", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const clonedRuntimeConfig: NodoAssistConfig = {
        ...runtimeConfig,
        agents: {
          defaults: {
      ...
      try {
        ...
        await ensureNodoAssistModelsJson(clonedRuntimeConfig, agentDir);
  ...
  it("preserves source markers for custom-provider api keys after models status secret resolution", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureNodoAssistModelsJson(runtimeConfig, agentDir);
        await expectGeneratedProviderApiKey(
          agentDir,
          "litellm",
          "NODOASSIST_MODEL_LITELLM_API_KEY",
  ...
  it("invalidates cached readiness when projected config changes under the same runtime snapshot", async () => {
    ...
    await withTempEnv(MODELS_CONFIG_IMPLICIT_ENV_VARS, async () => {
      ...
      const sourceConfig = createOpenAiApiKeySourceConfig();
      const runtimeConfig = createOpenAiApiKeyRuntimeConfig();
      const firstCandidate: NodoAssistConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
              ...
              baseUrl: "https://api.openai.com/v1",
              headers: {
                "X-NodoAssist-Test": "one",
              },
            },
          },
        },
      };
      const secondCandidate: NodoAssistConfig = {
        ...runtimeConfig,
        models: {
          providers: {
            openai: {
      ...
      try {
        setRuntimeConfigSnapshot(runtimeConfig, sourceConfig);
        await ensureNodoAssistModelsJson(firstCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.headers?.["X-NodoAssist-Test"]).toBe("one");

        // Header changes still rewrite models.json, but merge mode preserves the existing baseUrl.
        await ensureNodoAssistModelsJson(secondCandidate, agentDir);
        ...
        expect(parsed.providers.openai?.baseUrl).toBe("https://api.openai.com/v1");
        expect(parsed.providers.openai?.apiKey).toBe("OPENAI_API_KEY"); // pragma: allowlist secret
        expect(parsed.providers.openai?.headers?.["X-NodoAssist-Test"]).toBe("two");
      } finally {
        clearRuntimeConfigSnapshot();
        clearConfigCache();
      }
