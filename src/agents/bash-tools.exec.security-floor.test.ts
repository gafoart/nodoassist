...
function writeExecApprovalsFixture(root: string, file: Record<string, unknown>): void {
  const stateDir = process.env.NODOASSIST_STATE_DIR ?? path.join(root, "state");
...
describe("exec security floor", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-exec-security-floor-"));
    setTestEnvValue("HOME", tempRoot);
    setTestEnvValue("USERPROFILE", tempRoot);
    setTestEnvValue("NODOASSIST_HOME", tempRoot);
    setTestEnvValue("NODOASSIST_STATE_DIR", path.join(tempRoot, "state"));
