...
const ORIGINAL_BUILD_PRIVATE_QA = process.env.NODOASSIST_BUILD_PRIVATE_QA;
const ORIGINAL_QA_FORCE_RUNTIME = process.env.NODOASSIST_QA_FORCE_RUNTIME;
...
function restoreEnv(
  name: "NODOASSIST_BUILD_PRIVATE_QA" | "NODOASSIST_QA_FORCE_RUNTIME",
...
function makeProviderRuntimeConfig(runtime: string): NodoAssistConfig {
  return {
    models: {
      providers: {
        openai: {
...
afterEach(() => {
  restoreEnv("NODOASSIST_BUILD_PRIVATE_QA", ORIGINAL_BUILD_PRIVATE_QA);
  restoreEnv("NODOASSIST_QA_FORCE_RUNTIME", ORIGINAL_QA_FORCE_RUNTIME);
});

describe("resolveModelRuntimePolicy", () => {
  it("ignores the QA force-runtime override when the private QA gate is unset", () => {
    deleteTestEnvValue("NODOASSIST_BUILD_PRIVATE_QA");
    setTestEnvValue("NODOASSIST_QA_FORCE_RUNTIME", "nodoassist");
  ...
  it("respects the QA force-runtime override when the private QA gate is set", () => {
    ...
    setTestEnvValue("NODOASSIST_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("NODOASSIST_QA_FORCE_RUNTIME", "nodoassist");
  ...
  it("ignores invalid QA force-runtime values even when the private QA gate is set", () => {
    setTestEnvValue("NODOASSIST_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("NODOASSIST_QA_FORCE_RUNTIME", "bogus");
  ...
  it("honors provider wildcard agent model runtime policy entries", () => {
    const config = {
      agents: {
        defaults: {
          models: {
            "vllm/*": { agentRuntime: { id: "nodoassist" } },
          },
        },
      },
    } as NodoAssistConfig;
  ...
  it("prefers provider-qualified agent entries over bare entries for inferred providers", () => {
    const config = {
      agents: {
        defaults: {
          models: {
            "claude-opus-4-7": { agentRuntime: { id: "nodoassist" } },
