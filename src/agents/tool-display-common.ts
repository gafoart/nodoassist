 ...
 * tool update streams.
 */
import { asOptionalObjectRecord as asRecord } from "@nodoassist/normalization-core/record-coerce";
import {
...
function parseToolSearchCall(code: string): { target: string; args?: string } | undefined {
  ...
  const prefixMatch = code.match(/nodoassist\.tools\.call\s*\(\s*/s);
...
function normalizeToolSearchDisplayToolName(toolName: string | undefined): string | undefined {
  ...
  const catalogIdMatch = value.match(/^(?:nodoassist|mcp|client):[^:]+:(.+)$/s);
...
function collectToolSearchDescribeBindings(code: string): Map<string, string> {
  ...
  const bindingPattern =
    /\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:await\s+)?nodoassist\.tools\.describe\s*\(\s*("[^"]{1,240}"|'[^']{1,240}')\s*(?:,|\))/gs;
...
export function resolveToolSearchCodeDisplayTarget(
  ...
  const describeMatch = code.match(/nodoassist\.tools\.describe\s*\(\s*([^)]+?)\s*(?:,|\))/s);
  if (describeMatch) {
  ...
  const searchMatch = code.match(/nodoassist\.tools\.search\s*\(\s*([^)]+?)\s*(?:,|\))/s);
  if (searchMatch) {
