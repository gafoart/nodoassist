...
import type { ExtensionAPI, ExtensionContext } from "nodoassist/plugin-sdk/agent-sessions";
...
describe("compaction-safeguard recent-turn preservation", () => {
  ...
  it("passes compaction instructions to providers and preserves suffix context", async () => {
    ...
    expect(JSON.stringify(providerMessages)).not.toContain("nodoassist.runtime-context");
...
async function expectWorkspaceSummaryEmptyForAgentsAlias(
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-compaction-summary-"));
...
describe("readWorkspaceContextForSummary", () => {
  ...
  it("reads workspace context from the configured workspace instead of process cwd", async () => {
    const processRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-compaction-cwd-"));
    const workspaceRoot = fs.mkdtempSync(
      path.join(os.tmpdir(), "nodoassist-compaction-workspace-"),
    );
