...
async function withTempExecApprovalsFile(
  ...
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cli-exec-approvals-"));
...
async function withTempNodoAssistHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cli-home-"));
  try {
    await withEnvAsync({ NODOASSIST_HOME: home }, async () => run(home));
  } finally {
    await fs.rm(home, { recursive: true, force: true });
  }
}
...
describe("runCliAgent spawn path", () => {
  ...
  it("includes the NodoAssist skills prompt in CLI system prompts", () => {
  ...
  it("passes Claude system prompts through a file instead of argv", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(systemPromptPath).toContain("nodoassist-cli-system-prompt-");
  ...
  it("resends system prompts through a file for soft-resumed prompt-tool drift", async () => {
    const writeSoftResumeSystemPromptFile = vi.fn(async () => ({
      filePath: "/tmp/nodoassist-soft-resume-system-prompt.md",
      cleanup: async () => {},
    }));
    setCliRunnerExecuteTestDeps({
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(input.argv?.join(" ")).toContain("/tmp/nodoassist-soft-resume-system-prompt.md");
      return createManagedRun({
        reason: "exit",
        exitCode: 0,
        exitSignal: null,
  ...
  it("passes prepared backend env to the spawned CLI process", async () => {
    ...
    await executePreparedCliRun(
      buildPreparedCliRunContext({
        ...
        preparedEnv: {
          GEMINI_CLI_HOME: "/tmp/nodoassist-gemini-profile-home",
          GEMINI_CLI_SYSTEM_SETTINGS_PATH: "/tmp/nodoassist-gemini-system-settings.json",
        },
      }),
    );
    ...
    expect(input.env?.GEMINI_CLI_HOME).toBe("/tmp/nodoassist-gemini-profile-home");
    expect(input.env?.GEMINI_CLI_SYSTEM_SETTINGS_PATH).toBe(
      "/tmp/nodoassist-gemini-system-settings.json",
    );
  });

  it("passes NodoAssist skills to Claude as a session plugin", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cli-skills-"));
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      expect(manifest.name).toBe("nodoassist-skills");
  ...
  it("keeps captured live prepared backend cleanup with the whole-run owner", async () => {
    const mcpConfigDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nodoassist-cli-captured-mcp-config-"),
    );
  ...
  it("preserves completed output when system prompt cleanup fails after delivery", async () => {
    ...
    supervisorSpawnMock.mockImplementationOnce(async (...args: unknown[]) => {
      ...
      const captureHandle = markMcpLoopbackToolCallStarted({
        captureKey: input.env?.NODOASSIST_MCP_CLI_CAPTURE_KEY ?? "",
        toolName: "message",
  ...
  it("preserves Claude resume args when building live session argv", () => {
    ...
    expect(args).not.toContain("nodoassist-session");
...
`,
  ...
  it("does not create exec approvals file while resolving Claude live policy", async () => {
    await withTempNodoAssistHome(async (home) => {
      const approvalsPath = path.join(home, ".nodoassist", "exec-approvals.json");
      let stdoutListener: ((chunk: string) => void) | undefined;
      const stdin = {
        write: vi.fn((_data: string, cb?: (err?: Error | null) => void) => {
          stdoutListener?.(
...
`,
  ...
  it("answers Claude live control_request can_use_tool with allow when NodoAssist exec is YOLO despite raw --permission-mode default", async () => {
    let stdoutListener: ((chunk: string) => void) | undefined;
    const writes: string[] = [];
    const stdin = {
      write: vi.fn((data: string, cb?: (err?: Error | null) => void) => {
...
`,
    ...
    // and NodoAssist policy is authoritative over raw Claude permission-mode
  ...
  it("restarts Claude live sessions when selected skills change", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-live-skills-"));
  ...
  it("can preserve selected clearEnv keys for live CLI backend probes", async () => {
    try {
      process.env.NODOASSIST_LIVE_CLI_BACKEND_PRESERVE_ENV = '["SAFE_CLEAR"]';
      process.env.SAFE_CLEAR = "from-base";
      mockSuccessfulCliRun();
      await executePreparedCliRun(
        buildPreparedCliRunContext({
      ...
      delete process.env.NODOASSIST_LIVE_CLI_BACKEND_PRESERVE_ENV;
  ...
  it("loads workspace bootstrap files into the Claude CLI system prompt", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nodoassist-cli-bootstrap-context-"),
    );
