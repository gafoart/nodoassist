/**
 * NodoAssist built-in and plugin tool assembly.
 *
...
} from "./nodoassist-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./nodoassist-tools.nodes-workspace-guard.js";
import {
  collectPresentNodoAssistTools,
  shouldIncludeUpdatePlanToolForNodoAssistTools,
} from "./nodoassist-tools.registration.js";
...
type NodoAssistToolsDeps = {
  callGateway: typeof callGateway;
  config?: NodoAssistConfig;
};

const defaultNodoAssistToolsDeps: NodoAssistToolsDeps = {
  callGateway,
};

let nodoAssistToolsDeps: NodoAssistToolsDeps = defaultNodoAssistToolsDeps;

export function createNodoAssistTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? nodoAssistToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("nodoassist-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("nodoassist-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("nodoassist-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("nodoassist-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("nodoassist-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("nodoassist-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("nodoassist-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("nodoassist-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("nodoassist-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("nodoassist-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : nodoAssistToolsDeps.callGateway;
  const includeUpdatePlanTool = shouldIncludeUpdatePlanToolForNodoAssistTools({
    config: resolvedConfig,
    agentSessionKey: options?.agentSessionKey,
    agentId: options?.requesterAgentIdOverride,
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentNodoAssistTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentNodoAssistTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: nodoAssistToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentNodoAssistTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("nodoassist-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    options?.recordToolPrepStage?.("nodoassist-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("nodoassist-tools:tool-hooks");
  return allTools
    .map((tool) =>
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<NodoAssistToolsDeps>) {
    nodoAssistToolsDeps = overrides
      ? {
          ...defaultNodoAssistToolsDeps,
          ...overrides,
        }
      : defaultNodoAssistToolsDeps;
  },
