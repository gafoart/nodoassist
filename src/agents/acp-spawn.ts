...
} from "@nodoassist/acp-core/runtime/session-identifiers";
import type { AcpRuntimeSessionMode } from "@nodoassist/acp-core/runtime/types";
import {
...
function resolveHeartbeatConfigForAgent(params: {
  cfg: NodoAssistConfig;
  agentId: string;
}): NonNullable<NonNullable<NodoAssistConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an NodoAssist config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for NodoAssist config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: NodoAssistConfig, agentId: string): boolean {
...
function resolveConfiguredAcpSubagentTargetIds(cfg: NodoAssistConfig): string[] {
