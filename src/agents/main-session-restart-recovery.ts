...
export async function markRestartAbortedMainSessions(params: {
  ...
  additionalCfgs?: Iterable<NodoAssistConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, NODOASSIST_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is NodoAssistConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "NodoAssist was waiting on tool/model work. Continue from the existing " +
