...
const {
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "nodoassist-sandbox-registry-"));
  const previousStateDir = process.env.NODOASSIST_STATE_DIR;
  Reflect.set(process.env, "NODOASSIST_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_NODOASSIST_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_NODOASSIST_STATE_DIR === undefined) {
    deleteTestEnvValue("NODOASSIST_STATE_DIR");
  } else {
    setTestEnvValue("NODOASSIST_STATE_DIR", PREVIOUS_NODOASSIST_STATE_DIR);
  }
});
