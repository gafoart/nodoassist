...
function cfg(mode: "tools" | "off", patterns?: string[]): NodoAssistConfig {
  return {
    logging: {
...
describe("redactTranscriptMessage", () => {
  ...
  it("preserves OpenAI encrypted reasoning inside thinkingSignature", () => {
    ...
    const replayItem = JSON.parse(block.thinkingSignature) as {
      ...
      summary: unknown[];
      content?: unknown[];
      __nodoassist_replay: Record<string, unknown>;
    };
    const blockMetadata = (
      block as unknown as { nodoassistReasoningReplay: Record<string, unknown> }
    ).nodoassistReasoningReplay;
    ...
    expect(replayItem["__nodoassist_replay"]).toEqual(OPENAI_REASONING_REPLAY_METADATA);
  ...
  it.each([
    ...
    {
      api: "nodoassist-openai-completions-transport",
      provider: "google",
      block: {
        type: "toolCall",
        id: "call_1",
  ...
  it.each(["openai-responses", "nodoassist-openai-responses-transport"])(
  ...
  it("redacts nested strings in assistant tool-call arguments", () => {
      ...
      content: [
        {
          ...
          name: "shell",
          arguments: {
            command: "OPENAI_API_KEY=sk-abcdef1234567890xyz nodoassist health",
    ...
    expect(argumentsValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz nodoassist health");
    expect(argumentsValue.env.nested[0]).toBe("token sk-abc…0xyz");
    expect(argumentsValue.count).toBe(1);
    expect(serializedArguments).toContain("nodoassist health");
  ...
  it("redacts structured tool-use input payloads", () => {
    ...
    expect(inputValue.command).toBe("OPENAI_API_KEY=sk-abc…0xyz nodoassist health");
