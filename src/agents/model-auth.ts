...
function hasSecretRefProviderApiKey(cfg: NodoAssistConfig | undefined, provider: string): boolean {
...
function providerConfigMatchesRuntimeSnapshot(params: {
  inputConfig: NodoAssistConfig | undefined;
  ...
  const toComparableConfig = (providerConfig: ModelProviderConfig): NodoAssistConfig => ({
...
export async function resolveApiKeyForProvider(params: {
  ...
  throw new ProviderAuthError(
    ...
    [
      ...
      `Configure auth for this agent (${formatCliCommand("nodoassist agents add <id>")}) or copy only portable static auth profiles from the main agentDir.`,
    ].join(" "),
  );
}
