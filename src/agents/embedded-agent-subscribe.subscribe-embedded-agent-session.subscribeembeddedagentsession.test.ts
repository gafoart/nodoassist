...
describe("subscribeEmbeddedAgentSession", () => {
  ...
  async function captureToolLifecycleLogSubsystems(messageChannel?: string): Promise<string[]> {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-tool-log-attribution-"));
    const logFile = path.join(tempDir, "nodoassist.log");
    try {
      setLoggerOverride({
        level: "debug",
        consoleLevel: "silent",
