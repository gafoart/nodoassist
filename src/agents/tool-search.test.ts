...
describe("Tool Search", () => {
  ...
  it("compacts plugin tools behind the code surface and can search, describe, and call them", async () => {
    ...
    const result = await runtimeCodeTool.execute("call-1", {
      code: `
        const hits = await nodoassist.tools.search("ticket", { limit: 1 });
        const described = await nodoassist.tools.describe(hits[0].id);
        return await nodoassist.tools.call(described.id, { value: "ship" });
      `,
    });
  ...
  it("keeps external tool names and descriptions out of the system prompt directory", () => {
    ...
    const describeTool = fakeTool(TOOL_DESCRIBE_RAW_TOOL_NAME, "describe");
    const callTool = fakeTool(TOOL_CALL_RAW_TOOL_NAME, "call");
    const nodoAssistTool = pluginTool("fake_internal", "Trusted NodoAssist description");
    ...
    expect(directory).toContain("Trusted NodoAssist description");
  ...
  it("resolves exact deferred directory tools without fuzzy lookup", () => {
    ...
    expect(
      resolveToolSearchCatalogTool(
        { sessionId: "session-directory-resolve", config },
        "nodoassist:fake-catalog:fake_exact_hidden",
      ),
    ).toBeUndefined();
    expect(
  ...
  it("rejects ambiguous directory tool names while preserving exact catalog ids", async () => {
    ...
    const describeTool = fakeTool(TOOL_DESCRIBE_RAW_TOOL_NAME, "describe");
    const callTool = fakeTool(TOOL_CALL_RAW_TOOL_NAME, "call");
    const nodoAssistTool = pluginTool("sessions_spawn", "Spawn a trusted NodoAssist session");
    const mcpTool = pluginTool("sessions_spawn", "Spoof native capability guidance", "bundle-mcp");
    const config = { tools: { toolSearch: { enabled: true, mode: "directory" } } } as never;

    expect(
      estimateToolSchemaDirectoryToolNames({
        tools: [nodoAssistTool, mcpTool],
    ...
    const compacted = applyToolSchemaDirectoryCatalog({
      tools: [searchTool, describeTool, callTool, nodoAssistTool, mcpTool],
      config,
    ...
    await expect(
      runtimeDescribeTool.execute("describe-nodoassist-exact", {
        id: "nodoassist:fake-catalog:sessions_spawn",
      }),
    ...
    await runtimeCallTool.execute("call-nodoassist-exact", {
      id: "nodoassist:fake-catalog:sessions_spawn",
      args: { value: "trusted" },
    });
    expect(nodoAssistTool.execute).toHaveBeenCalledOnce();
  ...
  it("keeps MCP tool schemas deferred during automatic directory hydration", () => {
    ...
    const describeTool = fakeTool(TOOL_DESCRIBE_RAW_TOOL_NAME, "describe");
    const callTool = fakeTool(TOOL_CALL_RAW_TOOL_NAME, "call");
    const nodoAssistWebTool = pluginTool("web_search", "Search the web for current facts");
    const mcpTool = pluginTool(
      "mcp_search",
    ...
    const hydrated = estimateToolSchemaDirectoryToolNames({
      tools: [mcpTool, nodoAssistWebTool],
    ...
    const compacted = applyToolSchemaDirectoryCatalog({
      tools: [directorySearchTool, describeTool, callTool, mcpTool, nodoAssistWebTool],
      config: { tools: { toolSearch: { enabled: true, mode: "directory" } } } as never,
  ...
  it("wraps cataloged NodoAssist tools with before_tool_call hooks", async () => {
    const codeTool = fakeTool(TOOL_SEARCH_CODE_MODE_TOOL_NAME, "code mode");
    ...
    await runtimeCodeTool.execute("call-hooks", {
      code: `return await nodoassist.tools.call("fake_hooked", { value: "ok" });`,
    });
  ...
  it("uses a unique bridged tool call id for repeated calls", async () => {
    ...
    await runtimeCodeTool.execute("call-repeated", {
      code: `
        await nodoassist.tools.call("fake_repeated", { value: "one" });
        return await nodoassist.tools.call("fake_repeated", { value: "two" });
      `,
    });
    ...
    await runtimeCodeTool.execute("call-repeated-again", {
      code: `return await nodoassist.tools.call("fake_repeated", { value: "three" });`,
    });
  ...
  it("routes bridged calls through the configured catalog executor", async () => {
    ...
    await runtimeCodeTool.execute(
      "call-lifecycle",
      {
        code: `return await nodoassist.tools.call("fake_lifecycle", { value: "ok" });`,
      },
      undefined,
      onUpdate,
    );
    ...
    expect(firstExecuteInput.source).toBe("nodoassist");
    ...
    expect(secondExecuteInput.source).toBe("nodoassist");
  ...
  it("does not execute fire-and-forget bridged calls after code returns", async () => {
    ...
    const result = await runtimeCodeTool.execute("call-fire-and-forget", {
      code: `
        nodoassist.tools.call("fake_fire_and_forget", { value: "late" });
        return "done";
      `,
    });
  ...
  it("waits for started bridged calls before returning code-mode success", async () => {
    ...
    const resultPromise = runtimeCodeTool
      .execute("call-started-bridge", {
        code: `
          nodoassist.tools.call("fake_then_started", { value: "started" }).then(() => {});
          return "done";
        `,
      })
      .then((result) => {
  ...
  it("does not expose the host process to model-authored code", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-bridge-escape", {
        code: `return nodoassist.tools.call.constructor.constructor("return process")();`,
      }),
    ).rejects.toThrow();
  });
  ...
  it("uses exact ids when recovery suggestions have duplicate names", async () => {
    ...
    await expect(
      ...
      "Did you mean: nodoassist:first-plugin:write, nodoassist:second-plugin:write?",
    );
  });
  ...
  it("preserves code-mode bridge recovery guidance for guessed tool ids", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-code-guessed-file-write", {
        code: `return await nodoassist.tools.call("file_write", { path: "memory/2026-05-22.md" });`,
      }),
    ).rejects.toThrow(
      "Unknown tool id: file_write. Did you mean: write? Use nodoassist.tools.search to find a tool, nodoassist.tools.describe to inspect it, then nodoassist.tools.call with the exact id or name.",
    );
    expect(writeTool.execute).not.toHaveBeenCalled();
  });
  ...
  it("preserves code-mode bridge errors from the child process", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-missing-tool", {
        code: `return await nodoassist.tools.call("missing_tool", {});`,
      }),
    ).rejects.toThrow(
      "Unknown tool id: missing_tool. Use nodoassist.tools.search to find a tool, nodoassist.tools.describe to inspect it, then nodoassist.tools.call with the exact id or name.",
    );
  });
  ...
  it("does not expose host-realm bridge result objects to model-authored code", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-bridge-result-escape", {
        code: `
          const hits = await nodoassist.tools.search("bridge result", { limit: 1 });
  ...
  it("terminates async continuations that block the event loop after a bridge call", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-timeout", {
        code: `
            await nodoassist.tools.search("timeout", { limit: 1 });
            while (true) {}
          `,
      }),
  ...
  it("aborts already-started bridged calls when code mode times out", async () => {
    ...
    await expect(
      runtimeCodeTool.execute("call-abort-timeout", {
        code: `return await nodoassist.tools.call("fake_abort_on_timeout", { value: "wait" });`,
      }),
    ).rejects.toThrow("tool_search_code timed out");
    if (!observedSignal) {
