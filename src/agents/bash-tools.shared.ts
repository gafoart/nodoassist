 ...
 */
import { parseStrictInteger } from "@nodoassist/normalization-core/number-coercion";
...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via NODOASSIST_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `NODOASSIST_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${NODOASSIST_PREPEND_PATH}:$PATH"; unset NODOASSIST_PREPEND_PATH; '
    : "";
