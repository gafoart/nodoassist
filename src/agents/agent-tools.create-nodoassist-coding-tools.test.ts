/**
 * Broad coverage for createNodoAssistCodingTools.
...
import "./test-helpers/fast-nodoassist-tools.js";
import { wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call.js";
import { createNodoAssistCodingTools } from "./agent-tools.js";
import type { AuthProfileStore } from "./auth-profiles/types.js";
import * as nodoAssistPluginTools from "./nodoassist-plugin-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createNodoAssistCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createNodoAssistCodingTools>) {
...
type NodoAssistCodingTool = ReturnType<typeof createNodoAssistCodingTools>[number];
type NodoAssistToolsOptions = NonNullable<Parameters<typeof createNodoAssistTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: NodoAssistCodingTool[], name: string): NodoAssistCodingTool {
...
function requireToolExecute(
  tool: NodoAssistCodingTool,
): NonNullable<NodoAssistCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateNodoAssistToolsOptions(): NodoAssistToolsOptions {
  const calls = vi.mocked(createNodoAssistTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createNodoAssistTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: NodoAssistToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createNodoAssistCodingTools", () => {
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createNodoAssistCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createNodoAssistTools).mockReturnValueOnce([wrapped as never]);

    const tools = createNodoAssistCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core NodoAssist tools are not materialized", () => {
    const createNodoAssistToolsMock = vi.mocked(createNodoAssistTools);
    createNodoAssistToolsMock.mockClear();
    ...
    expect(createNodoAssistToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    const options = latestCreateNodoAssistToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:nodoassist"], ["*"]]) {
      const tools = createNodoAssistCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to NodoAssist tool construction", () => {
    const createNodoAssistToolsMock = vi.mocked(createNodoAssistTools);
    createNodoAssistToolsMock.mockClear();
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNodoAssistToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into NodoAssist tool construction plans", () => {
    const createNodoAssistToolsMock = vi.mocked(createNodoAssistTools);
    createNodoAssistToolsMock.mockClear();
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNodoAssistToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the NodoAssist core factory", () => {
    const createNodoAssistToolsMock = vi.mocked(createNodoAssistTools);
    createNodoAssistToolsMock.mockClear();
  ...
  it("forwards active model metadata to plugin-only tool construction", () => {
    ...
    const resolvePluginToolsSpy = vi
      .spyOn(nodoAssistPluginTools, "resolveNodoAssistPluginToolsForOptions")
      .mockReturnValue([]);
  ...
  it("uses tools.alsoAllow for optional plugin discovery without widening to all plugins", () => {
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateNodoAssistToolsOptions().pluginToolAllowlist).toStrictEqual([
      "lobster",
      DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY,
    ]);
  });

  it("passes explicit denylist entries to NodoAssist tool factory planning", () => {
    const createNodoAssistToolsMock = vi.mocked(createNodoAssistTools);
    createNodoAssistToolsMock.mockClear();
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateNodoAssistToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to NodoAssist plugin discovery", async () => {
    const createNodoAssistToolsMock = vi.mocked(createNodoAssistTools);
    createNodoAssistToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `nodoassist-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateNodoAssistToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateNodoAssistToolsOptions().inheritedToolAllowlist;
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createNodoAssistToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateNodoAssistToolsOptions().cronCreatorToolAllowlist;
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      NodoAssistToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("nodoassist-tools:test-helper")).toBeLessThan(
      stages.indexOf("nodoassist-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createNodoAssistCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createNodoAssistCodingTools({
      config: testConfig,
    ...
    const codexTools = createNodoAssistCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: NodoAssistConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createNodoAssistCodingTools({
    ...
    const anthropicTools = createNodoAssistCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: NodoAssistConfig = {
      tools: {
        exec: {
    ...
    const allowed = createNodoAssistCodingTools({
    ...
    const denied = createNodoAssistCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createNodoAssistCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createNodoAssistCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createNodoAssistTools).mockClear();
    ...
    expect(latestCreateNodoAssistToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createNodoAssistCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createNodoAssistCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createNodoAssistCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createNodoAssistCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createNodoAssistCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createNodoAssistCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createNodoAssistCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createNodoAssistCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createNodoAssistCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-read-"));
    try {
      ...
      const contents = "Hello from nodoassist read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "nodoassist-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "nodoassist-workspace"),
    ...
    const tools = createNodoAssistCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-canonical-"));
    try {
      const tools = createNodoAssistCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-legacy-alias-"));
    try {
      const tools = createNodoAssistCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-structured-write-"));
    try {
      const tools = createNodoAssistCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-structured-edit-"));
    try {
