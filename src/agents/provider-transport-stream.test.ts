...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to NodoAssist transport aliases when needed.
    const cases = [
      {
        ...
        alias: "nodoassist-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "nodoassist-openai-completions-transport",
      },
      {
        ...
        alias: "nodoassist-azure-openai-responses-transport",
      },
      {
        ...
        alias: "nodoassist-anthropic-messages-transport",
      },
      {
        ...
        alias: "nodoassist-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on NodoAssist transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createNodoAssistTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the NodoAssist simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("nodoassist-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the NodoAssist transport until NodoAssist preserves attribution", () => {
