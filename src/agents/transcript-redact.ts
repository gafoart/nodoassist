...
} from "@nodoassist/media-core/inline-image-data-url";
...
function redactTranscriptOptions(cfg?: NodoAssistConfig) {
...
function isTranscriptRedactionDisabled(cfg?: NodoAssistConfig): boolean {
  return (cfg?.logging?.redactSensitive ?? readLoggingConfig()?.redactSensitive) === "off";
}

function redactTranscriptText(value: string, cfg?: NodoAssistConfig): string {
...
const OPENAI_RESPONSES_APIS = new Set([
  ...
  "nodoassist-azure-openai-responses-transport",
]);
...
const OPENAI_COMPLETIONS_APIS = new Set([
  "openai-completions",
  "nodoassist-openai-completions-transport",
]);
...
function isGitHubCopilotResponsesRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return (
    (route?.api === "openai-responses" || route?.api === "nodoassist-openai-responses-transport") &&
...
const OPENAI_REASONING_REPLAY_METADATA_KEY = "__nodoassist_replay";
