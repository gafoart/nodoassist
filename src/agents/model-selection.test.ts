...
function resolveAnthropicOpusThinking(cfg: NodoAssistConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: NodoAssistConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicOpus48Thinking(cfg: NodoAssistConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicVertexOpus48Thinking(cfg: NodoAssistConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic-vertex",
    model: "claude-opus-4-8",
...
function resolveClaudeCliOpus48Thinking(cfg: NodoAssistConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "claude-cli",
    model: "claude-opus-4-8",
...
function resolveConfiguredRefForTest(cfg: Partial<NodoAssistConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as NodoAssistConfig,
...
describe("model-selection", () => {
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<NodoAssistConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("nodoassist-model-selection-test");
      try {
        const cfg: Partial<NodoAssistConfig> = {
          agents: {
            defaults: {
