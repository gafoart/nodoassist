...
const parseNodoAssistChannelsLoginShellCommand = testing.parseNodoAssistChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("nodoassist-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive NodoAssist channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseNodoAssistChannelsLoginShellCommand("nodoassist channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseNodoAssistChannelsLoginShellCommand(
        "pnpm exec nodoassist channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseNodoAssistChannelsLoginShellCommand("nodoassist channels status --deep")).toBe(
      false,
    );
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-nodoassist-channel-login", {
        command: "nodoassist channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive NodoAssist channel login commands/);
    await expect(
      tool.execute("call-wrapped-nodoassist-channel-login", {
        command: "sudo -u nodoassist bash -lc 'nodoassist channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive NodoAssist channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'nodoassist channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive NodoAssist channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env nodoassist channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive NodoAssist channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'nodoassist channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive NodoAssist channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("nodoassist-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("nodoassist-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("nodoassist-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("nodoassist-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__nodoassist_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
