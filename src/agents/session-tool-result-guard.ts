...
import { isTranscriptOnlyNodoAssistAssistantModel } from "../shared/transcript-only-nodoassist-assistant.js";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000NODOASSIST_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function redactPersistedDetailString(
  ...
  const redactedPrefix =
    boundaryIndex >= 0
      ...
      : "[NodoAssist persisted detail redacted: boundary marker removed]";
  ...
  const persistedPrefix =
    ...
    PARTIAL_PRIVATE_KEY_BLOCK_RE.test(initialPersistedPrefix)
      ? "[NodoAssist persisted detail redacted: partial secret span omitted]"
      : initialPersistedPrefix;
  const boundaryNotice = "[NodoAssist persisted detail redacted: boundary overlap omitted]";
  return `${persistedPrefix}${persistedPrefix ? "\n" : ""}${boundaryNotice}\n\n[NodoAssist persisted detail truncated: ${Math.max(
    0,
...
function redactPersistedDetailValue(
  ...
  if (depth >= 8) {
    return "[NodoAssist persisted detail redacted: max depth exceeded]";
  }
...
function isTranscriptOnlyNodoAssistAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlyNodoAssistAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage, callerOptions?: AppendMessageOptions) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyNodoAssistAssistantMessage(nextMessage);
    if (
