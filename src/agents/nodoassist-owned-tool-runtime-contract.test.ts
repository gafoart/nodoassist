// Verifies NodoAssist-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "nodoassist/plugin-sdk/agent-core";
import {
  installNodoAssistOwnedToolHooks,
  resetNodoAssistOwnedToolHooks,
  textToolResult,
...
describe("NodoAssist-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetNodoAssistOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installNodoAssistOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installNodoAssistOwnedToolHooks();
    ...
    const originalParams = {
      ...
      mediaUrl: "/tmp/nodoassist-reply.png",
      provider: "telegram",
      to: "chat-1",
    };
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/nodoassist-reply.png"]);
    expect(
    ...
    ).toEqual([
      {
        ...
        mediaUrls: ["/tmp/nodoassist-reply.png"],
      },
    ]);
    const [afterPayload, afterContext] = await waitForAfterToolCall(hooks);
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installNodoAssistOwnedToolHooks({ blockReason: "blocked by policy" });
