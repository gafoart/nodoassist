...
import { createReadTool } from "nodoassist/plugin-sdk/agent-sessions";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

vi.mock("nodoassist/plugin-sdk/llm", async () => {
  const original = await vi.importActual<typeof import("nodoassist/plugin-sdk/llm")>(
...
describe("FS tools with workspaceOnly=false", () => {
  ...
  const toolsFor = (workspaceOnly: boolean | undefined): AnyAgentTool[] => {
    const read = createNodoAssistReadTool(createReadTool(workspaceDir) as unknown as AnyAgentTool);
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-test-"));
