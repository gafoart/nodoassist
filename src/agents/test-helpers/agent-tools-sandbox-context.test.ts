...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("nodoassist-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("nodoassist-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("nodoassist-sbx-");
