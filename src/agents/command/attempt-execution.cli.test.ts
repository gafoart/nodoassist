...
function firstEmbeddedAgentArg(callIndex = 0) {
  return requireMockArg(runEmbeddedAgentMock, callIndex, "embedded NodoAssist agent argument");
}
...
describe("CLI attempt execution", () => {
  ...
  async function runNodoAssistEmbeddedAttemptForTest(overrides?: {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cli-attempt-"));
  ...
  function makeClaudeCliSessionEntry(
    nodoassistSessionId: string,
    cliSessionId: string,
  ): SessionEntry {
    return {
      sessionId: nodoassistSessionId,
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
  ...
  it("does not pass --resume when the stored Claude CLI transcript is missing", async () => {
    ...
    setTestEnvValue("HOME", homeDir);
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-123",
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
  ...
  it("keeps Claude CLI resume when the stored transcript has assistant content", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-456",
      updatedAt: Date.now(),
      cliSessionBindings: {
        "claude-cli": {
          sessionId: cliSessionId,
  ...
  it("checks Claude CLI transcript content under the process cwd", async () => {
    ...
    const sessionEntry = makeClaudeCliSessionEntry("nodoassist-session-cwd", cliSessionId);
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runCliAgentMock.mockResolvedValueOnce(makeCliResult("resumed cli response"));
  ...
  it("passes session-bound OpenAI Codex auth profile to codex-cli aliases", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-codex",
      updatedAt: Date.now(),
  ...
  it("skips auto auth-profile resolution for CLI-owned transport", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-codex-owned",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
  ...
  it("selects a google-gemini-cli auth profile for canonical Google models routed through Gemini CLI", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-gemini",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards pinned canonical Google API-key profiles to Google models routed through Gemini CLI", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-gemini-api-key",
      updatedAt: Date.now(),
  ...
  it("forwards incompatible pinned profiles to Gemini CLI for fail-closed backend validation", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-gemini-incompatible-auth",
      updatedAt: Date.now(),
  ...
  it("ignores stale auto-selected profiles when resolving Gemini CLI auth order", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-gemini-stale-auto-auth",
      updatedAt: Date.now(),
      authProfileOverride: "openai:work",
  ...
  it("selects canonical Google API-key auth order for Google models routed through Gemini CLI", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-gemini-api-key-order",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("embedded assistant gap-fill skips trailing nodoassist.cache-ttl custom entries (regression for #83427)", async () => {
    ...
    await fs.appendFile(
      ...
      `${JSON.stringify({
        type: "custom",
        customType: "nodoassist.cache-ttl",
  ...
  it("forwards separate user trigger, channel, and provider context to CLI runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-channel",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards message-tool-only policy and requires explicit subagent targets", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-cli-message-policy",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("does not pass auth-order profiles to CLI backends that do not stage them", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-claude-auth-order",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("does not pass auth-order profiles to configured CLI runtimes that do not stage them", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-anthropic-claude-runtime-auth-order",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards runtime toolsAllow into CLI attempts so the CLI harness can fail closed", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-cli-tools-allow",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("stamps CLI prompts with current timestamp context", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-cli-timestamp",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    await runAgentAttempt({
      ...
      originalProvider: "claude-cli",
      modelOverride: "opus",
      cfg: { agents: { defaults: { userTimezone: "UTC" } } } as NodoAssistConfig,
      sessionEntry,
      sessionId: sessionEntry.sessionId,
      sessionKey,
      sessionAgentId: "main",
  ...
  it("routes canonical Anthropic models through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-canonical-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    ...
    const fallbackArg = await runNodoAssistEmbeddedAttemptForTest({
  ...
  it("routes provider-qualified Anthropic shorthand through the configured Claude CLI runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-shorthand-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("routes canonical OpenAI models through the configured embedded Codex runtime", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-canonical-codex-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
    runEmbeddedAgentMock.mockResolvedValueOnce({
      ...
      meta: {
        ...
        executionTrace: { runner: "nodoassist" },
      },
    });
  ...
  it("keeps live stream output for visible subagent lane runs", async () => {
    const embeddedArg = await runNodoAssistEmbeddedAttemptForTest({
      opts: { lane: "subagent" },
  ...
  it("records raw CLI-shaped model runs as embedded origins", async () => {
    ...
    const firstArg = await runNodoAssistEmbeddedAttemptForTest({
    ...
    const retryArg = await runNodoAssistEmbeddedAttemptForTest({
  ...
  it("forwards selected auth profiles through metadata-scoped provider aliases", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-metadata-auth-alias",
      updatedAt: Date.now(),
      authProfileOverride: "openai:work",
      authProfileOverrideSource: "user",
    };
  ...
  it("forwards user-pinned OpenAI API-key backup profiles to Codex harness runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-openai-chatgpt-api-key",
      updatedAt: Date.now(),
  ...
  it("keeps one-shot model runs on the raw embedded provider path", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-model-run-raw",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
  ...
  it("forwards trusted elevated defaults to embedded agent runs", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-elevated-followup",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    const bashElevated = {
  ...
  it("forwards one-shot CLI cleanup to CLI providers", async () => {
    ...
    const sessionEntry: SessionEntry = {
      sessionId: "nodoassist-session-cleanup-cli",
      updatedAt: Date.now(),
    };
    const sessionStore: Record<string, SessionEntry> = { [sessionKey]: sessionEntry };
    await fs.writeFile(storePath, JSON.stringify(sessionStore, null, 2), "utf-8");
...
describe("embedded attempt harness pinning", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-embedded-attempt-"));
    runCliAgentMock.mockReset();
    runEmbeddedAgentMock.mockReset();
  });
  ...
  it("ignores stale OpenAI sessions pinned to NodoAssist and relies on default Codex routing", async () => {
    const sessionEntry: SessionEntry = {
  ...
  it("routes explicit OpenAI native runs with legacy Codex OAuth through NodoAssist", async () => {
    const sessionEntry: SessionEntry = {
