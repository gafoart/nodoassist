...
import { NODOASSIST_AGENT_RUNTIME_ID } from "../agent-runtime-id.js";
...
type CliCompactionDeps = {
  ...
  resolveContextEngine: (cfg: NodoAssistConfig) => Promise<ContextEngine>;
...
function isNativeHarnessCompactionSession(
  ...
  if (!harnessId || normalizeOptionalAgentRuntimeId(harnessId) === NODOASSIST_AGENT_RUNTIME_ID) {
    return false;
  }
