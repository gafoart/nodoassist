...
async function writeSessionFile(params: { sessionFile: string; sessionId: string }) {
  // The lifecycle compacts canonical NodoAssist session JSONL, so tests write the
...
describe("runCliTurnCompactionLifecycle", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cli-compaction-"));
  ...
  it("compacts over-budget CLI transcripts and clears external CLI resume state", async () => {
    ...
    // Once NodoAssist rewrites the transcript, external CLI resume ids are stale
  ...
  it("ignores stale native harness ids when the active provider no longer matches", async () => {
    const sessionKey = "agent:main:nodoassist-after-codex";
    const sessionId = "session-nodoassist-after-codex";
    const sessionFile = path.join(tmpDir, "session-nodoassist-after-codex.jsonl");
    const storePath = path.join(tmpDir, "sessions-nodoassist-after-codex.json");
    await writeSessionFile({ sessionFile, sessionId });
  ...
  it("bounds a hung CLI context-engine compaction and leaves resume state intact", async () => {
    ...
    const pending = runCliTurnCompactionLifecycle({
      cfg: { agents: { defaults: { compaction: { timeoutSeconds: 1 } } } } as NodoAssistConfig,
      sessionId,
      sessionKey,
      sessionEntry,
      sessionStore,
