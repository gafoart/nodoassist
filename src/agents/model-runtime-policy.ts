...
export function resolveModelRuntimePolicy(params: {
  ...
  if (process.env.NODOASSIST_BUILD_PRIVATE_QA === "1") {
    const forcedRuntime = process.env.NODOASSIST_QA_FORCE_RUNTIME?.trim().toLowerCase();
    if (forcedRuntime === "nodoassist" || forcedRuntime === "codex") {
