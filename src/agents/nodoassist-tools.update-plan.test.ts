// Verifies update_plan registration gates and base NodoAssist tool inclusion policy.
...
import { shouldIncludeUpdatePlanToolForNodoAssistTools } from "./nodoassist-tools.registration.js";
import { createUpdatePlanTool } from "./tools/update-plan-tool.js";

type UpdatePlanGatingParams = Parameters<typeof shouldIncludeUpdatePlanToolForNodoAssistTools>[0];
type CreateNodoAssistToolsOptions = NonNullable<Parameters<typeof createNodoAssistTools>[0]>;

function expectUpdatePlanEnabled(params: UpdatePlanGatingParams, expected: boolean): void {
  expect(shouldIncludeUpdatePlanToolForNodoAssistTools(params)).toBe(expected);
}

function toolNames(tools: ReturnType<typeof createNodoAssistTools>): string[] {
  return tools.map((tool) => tool.name);
}

function createFastToolNames(options: CreateNodoAssistToolsOptions): string[] {
...
function expectToolNamed(
  tools: ReturnType<typeof createNodoAssistTools>,
  name: string,
): ReturnType<typeof createNodoAssistTools>[number] {
...
describe("nodoassist-tools update_plan gating", () => {
  afterEach(() => {
    setEmbeddedMode(false);
  });

  it("keeps update_plan disabled by default", () => {
    expectUpdatePlanEnabled({ config: {} as NodoAssistConfig }, false);
  });
  ...
  it("does not expose update_plan from default tool construction", () => {
    ...
    expect(shouldIncludeUpdatePlanToolForNodoAssistTools(emptyAllowlistParams)).toBe(false);
  });

  it("wraps constructed tools with before-tool-call hooks by default", () => {
    const tools = createNodoAssistTools({
      config: {} as NodoAssistConfig,
      disablePluginTools: true,
    });
    const unwrappedTools = createNodoAssistTools({
      config: {} as NodoAssistConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
  ...
  it("requires explicit transcripts enablement before registering the transcripts tool", () => {
    ...
    const enabledTools = createFastToolNames({
      config: { transcripts: { enabled: true } } as NodoAssistConfig,
    });
  ...
  it("keeps explicitly allowed message tool in embedded completions", () => {
    setEmbeddedMode(true);
    const fromRuntimeAllowlist = createNodoAssistTools({
      config: {} as NodoAssistConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
      wrapBeforeToolCallHook: false,
    });
    const fromGlobalAlsoAllow = createNodoAssistTools({
      config: { tools: { profile: "minimal", alsoAllow: ["message"] } } as NodoAssistConfig,
      disablePluginTools: true,
      wrapBeforeToolCallHook: false,
    });
    const denied = createNodoAssistTools({
      config: {} as NodoAssistConfig,
      disablePluginTools: true,
      pluginToolAllowlist: ["message"],
  ...
  it("includes update_plan when a config allowlist group includes it", () => {
    const includeUpdatePlan = shouldIncludeUpdatePlanToolForNodoAssistTools({
      config: { tools: { allow: ["group:agents"] } } as NodoAssistConfig,
      modelProvider: "anthropic",
      modelId: "claude-sonnet-4-6",
    });
