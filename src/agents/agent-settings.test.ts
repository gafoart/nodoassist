...
describe("isSilentOverflowProneModel", () => {
  ...
  // nodoassist#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable NodoAssist runtime's secondary compaction path;
  // NodoAssist's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of nodoassist#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering NodoAssist runtime's
  ...
  // state.messages intact; NodoAssist's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep NodoAssist runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove NodoAssist runtime's
