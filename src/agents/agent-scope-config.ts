...
export function listAgentEntries(cfg: NodoAssistConfig): AgentEntry[] {
...
export function listAgentIds(cfg: NodoAssistConfig): string[] {
...
export function resolveDefaultAgentId(cfg: NodoAssistConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: NodoAssistConfig, agentId: string): AgentEntry | undefined {
