...
function createClaudeCliOverrideConfig(config: CliBackendConfig): NodoAssistConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from NodoAssist's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__nodoassist__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__nodoassist__*");
  ...
  it("keeps Claude permission mode unset when NodoAssist exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
