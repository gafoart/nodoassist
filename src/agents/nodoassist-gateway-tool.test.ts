// Verifies NodoAssist gateway tool schema, restart signaling, and config mutations.
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-test-"));

    try {
      await withEnvAsync(
        { NODOASSIST_STATE_DIR: stateDir, NODOASSIST_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          config: {
            tools: {
              exec: {
                ...
                safeBinTrustedDirs: ["/tmp/nodoassist-bin"],
  ...
  it("rejects config.apply when it changes safe bin trusted directories", async () => {
    ...
    await expect(
      tool.execute("call-protected-safe-bin-trust-apply", {
        action: "config.apply",
        raw: '{ tools: { exec: { ask: "on-miss", security: "allowlist", safeBinTrustedDirs: ["/tmp/nodoassist-bin"] } } }',
      }),
    ).rejects.toThrow(
