 ...
 * provider-specific payload policy before converting SDK streams into NodoAssist assistant events.
 */
...
} from "@nodoassist/ai/internal/openai";
import {
...
const OPENAI_RESPONSES_REASONING_REPLAY_META_KEY = "__nodoassist_replay";
const OPENAI_RESPONSES_REASONING_REPLAY_BLOCK_META_KEY = "nodoassistReasoningReplay";
...
type BaseStreamOptions = {
  ...
  nodoassistCodeModeToolSurface?: boolean;
...
async function processResponsesStream(
  ...
  const guardedStream = withFirstStreamEventTimeout(openaiStream, {
    ...
    hint: "The provider may be stalled while parsing the tool payload; retry with a smaller tool surface or enable NODOASSIST_DEBUG_MODEL_PAYLOAD=tools to inspect exposed tools.",
  });
  ...
  return (model, context, options) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (
          (options as { nodoassistCodeModeToolSurface?: unknown } | undefined)
            ?.nodoassistCodeModeToolSurface === true
        ) {
...
function resolveCacheRetention(cacheRetention: string | undefined): "short" | "long" | "none" {
  ...
  if (typeof process !== "undefined" && process.env.NODOASSIST_CACHE_RETENTION === "long") {
    return "long";
  }
  return "short";
}
...
function isOpenAICodexResponsesModel(model: Model): boolean {
  return (
    ...
    (model.api === "openai-chatgpt-responses" ||
      model.api === "nodoassist-openai-responses-transport")
  );
}
...
function getCompletionsContentDeltas(content: unknown): CompletionsReasoningDelta[] {
  ...
  // Preserve provider reasoning as NodoAssist thinking blocks so channel/UI
