...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: NodoAssistConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/nodoassist" }],
      },
    };
        ...
        list: [
          {
            ...
            agentDir: "~/.nodoassist/agents/main",
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: NodoAssistConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: NodoAssistConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: NodoAssistConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: NodoAssistConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: NodoAssistConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: NodoAssistConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: NodoAssistConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: NodoAssistConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: NodoAssistConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: NodoAssistConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: NodoAssistConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/nodoassist-restricted",
            tools: {
              allow: ["read"],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/nodoassist");
  });

  it("uses NODOASSIST_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "nodoassist-home");
    withEnv({ NODOASSIST_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as NodoAssistConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".nodoassist", "workspace"));
    });
  });

  it("uses NODOASSIST_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses NODOASSIST_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "nodoassist-home");
    withEnv({ NODOASSIST_HOME: home, NODOASSIST_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as NodoAssistConfig, "main");
      expect(agentDir).toBe(
        path.join(path.resolve(home), ".nodoassist", "agents", "main", "agent"),
      );
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ NODOASSIST_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ NODOASSIST_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/nodoassist-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/nodoassist-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-agent-scope-"));
