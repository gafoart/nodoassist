// Verifies OpenAI model selections route between NodoAssist and Codex runtimes.
...
describe("OpenAI runtime routing policy", () => {
  ...
  it("keeps explicit NodoAssist plus Codex auth profile under the unified OpenAI provider", () => {
    // OpenAI auth now stays canonical even when the runtime is not Codex.
    expect(
      listOpenAIAuthProfileProvidersForAgentRuntime({
        provider: "openai",
        harnessRuntime: "nodoassist",
      }),
    ).toEqual(["openai"]);
    expect(
      resolveOpenAIRuntimeProvider({
  ...
  it("keeps explicit OpenAI NodoAssist API-key auth order ahead of Codex backups", () => {
    const config = {
      auth: {
        order: {
  ...
  it("does not route custom OpenAI-compatible NodoAssist configs through Codex auth order", () => {
    const config = {
      models: {
        providers: {
          openai: {
