...
type NodoAssistReadToolOptions = {
...
function resolveAdaptiveReadMaxBytes(options?: NodoAssistReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".nodoassist/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.nodoassist/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with NodoAssist result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createNodoAssistReadTool(base, {
...
/** Wrap the base read tool with NodoAssist paging, MIME, and image handling. */
export function createNodoAssistReadTool(
  base: AnyAgentTool,
  options?: NodoAssistReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
