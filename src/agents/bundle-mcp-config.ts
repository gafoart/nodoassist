...
const NODOASSIST_TRANSPORT_TO_CLI_BUNDLE_TYPE: Record<string, string> = {
...
/**
 * User config stores NodoAssist MCP transport names, while CLI backends such as
 * Claude Code and Gemini expect a downstream `type` field. Keep this adapter
 * out of the generic merge path because embedded NodoAssist still consumes the raw
 * NodoAssist `transport` shape directly.
 */
export function toCliBundleMcpServerConfig(server: BundleMcpServerConfig): BundleMcpServerConfig {
  ...
  if (typeof rawTransport === "string") {
    const mapped = NODOASSIST_TRANSPORT_TO_CLI_BUNDLE_TYPE[rawTransport];
    if (mapped) {
      next.type = mapped;
    }
  }
...
export function loadMergedBundleMcpConfig(params: {
  ...
  return {
    config: {
      // NodoAssist config is the owner-managed layer, so it overrides bundle defaults.
      mcpServers: {
        ...enabledBundleMcp,
        ...Object.fromEntries(
