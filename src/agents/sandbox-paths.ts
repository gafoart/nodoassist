...
import { isPassThroughRemoteMediaSource } from "@nodoassist/media-core/media-source-url";
...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const nodoAssistTmpDir = path.resolve(resolvePreferredNodoAssistTmpDir());
  if (!isPathInside(nodoAssistTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: nodoAssistTmpDir });
  return resolved;
}
