...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["NODOASSIST_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("nodoassist-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir(
      "nodoassist-cli-bundle-mcp-workspace-",
    );
    const emptyBundledDir = await tempHarness.createTempDir("nodoassist-cli-bundle-mcp-bundled-");
    setTestEnvValue("NODOASSIST_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): NodoAssistConfig {
  return {
    plugins: {
      entries: {
