...
describe("buildCliAgentSystemPrompt", () => {
  ...
  it("uses CLI backend tool fallback instead of NodoAssist tool assumptions", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/nodoassist",
      tools: [],
      modelDisplay: "test/model",
    ...
    expect(prompt).not.toContain("NodoAssist lists the standard tools above");
    expect(prompt).not.toContain("This runtime enables:");
    expect(prompt).not.toContain("For long waits, avoid rapid poll loops");
    expect(prompt).not.toContain("Larger work: use `sessions_spawn`");
    expect(prompt).not.toContain("Do not poll `subagents list` / `sessions_list` in a loop");
    expect(prompt).toContain("No NodoAssist tool list is injected");
  });

  it("uses cwd, not bootstrap workspace, for CLI workspace guidance", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/nodoassist-agent",
      cwd: "/tmp/task-repo",
      tools: [],
      modelDisplay: "test/model",
    });
    ...
    expect(prompt).not.toContain("Your working directory is: /tmp/nodoassist-agent");
  });
  ...
  it("includes CLI-scoped plugin command guidance", () => {
    ...
    // NodoAssist-main command text into external CLI backends.
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "NodoAssist-only command guidance.",
    ...
    expect(prompt).not.toContain("NodoAssist-only command guidance.");
  });
