...
import { extensionForMime } from "@nodoassist/media-core/mime";
import {
...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".nodoassist-cli-images");
  }
  return path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const workspace = await tempWorkspace({
    ...
    prefix: "nodoassist-cli-system-prompt-",
  });
