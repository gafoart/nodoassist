/** Tests projecting NodoAssist user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as NodoAssistConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as NodoAssistConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as NodoAssistConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by NodoAssist agent id", () => {
    // Agent-scoped MCP servers should follow the active NodoAssist agent, while
  ...
  it("omits scoped Codex MCP servers when no NodoAssist agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
