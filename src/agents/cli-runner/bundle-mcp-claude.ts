/**
 * Claude CLI argument helpers for NodoAssist-managed bundle MCP config.
 */
...
export function findClaudeMcpConfigPaths(args?: string[]): string[] {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--mcp-config") {
      ...
      // as positional prompts after NodoAssist injects its strict overlay.
...
/** Return Claude args with NodoAssist's strict MCP config path injected. */
...
/** Writes the active per-attempt capture token into NodoAssist's generated Claude MCP config. */
export async function writeClaudeMcpCaptureConfig(params: {
  ...
  const nodoassist = isRecord(mcpServers.nodoassist) ? mcpServers.nodoassist : undefined;
  if (!nodoassist) {
    throw new Error("Claude MCP capture requires an nodoassist server config");
  }
  const headers = isRecord(nodoassist.headers) ? nodoassist.headers : {};
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        ...
        mcpServers: {
          ...
          nodoassist: {
            ...
            headers: {
              ...headers,
              "x-nodoassist-cli-capture-key": params.captureKey,
            },
          },
        },
      },
