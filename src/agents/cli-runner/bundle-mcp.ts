...
function normalizeNodoAssistLoopbackUrl(value: string): string {
  const match =
    /^(http:\/\/(?:127\.0\.0\.1|localhost|\[::1\])):\d+(\/mcp)$/.exec(value.trim()) ?? undefined;
  if (!match) {
    return value;
  }
  return `${match[1]}:<nodoassist-loopback>${match[2]}`;
}

function canonicalizeBundleMcpConfigForResume(config: BundleMcpConfig): BundleMcpConfig {
  // The NodoAssist loopback MCP port changes across runs. Replace it before
  ...
  const canonicalServers = Object.fromEntries(
    Object.entries(config.mcpServers).map(([name, server]) => {
      if (name !== "nodoassist" || typeof server.url !== "string") {
      ...
      return [
        ...
        sortJsonValue({
          ...server,
          url: normalizeNodoAssistLoopbackUrl(server.url),
        }),
      ];
    }),
...
const NODOASSIST_MCP_ENV_TEMPLATE_PATTERN = /\$\{(NODOASSIST_MCP_[A-Z0-9_]+)\}/g;

function resolveNodoAssistMcpEnvTemplates(value: unknown, env?: Record<string, string>): unknown {
  if (!env) {
    return value;
  }
  if (typeof value === "string") {
    return value.replace(NODOASSIST_MCP_ENV_TEMPLATE_PATTERN, (match, name: string) => {
      return Object.hasOwn(env, name) ? env[name] : match;
    });
  }
  if (Array.isArray(value)) {
    return value.map((entry) => resolveNodoAssistMcpEnvTemplates(entry, env));
  }
  if (!isRecord(value)) {
    return value;
  }
  return Object.fromEntries(
    Object.entries(value).map(([key, entry]) => [
      key,
      resolveNodoAssistMcpEnvTemplates(entry, env),
    ]),
  );
}
...
async function prepareModeSpecificBundleMcpConfig(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cli-mcp-"));
  const mcpConfigPath = path.join(tempDir, "mcp.json");
  const runtimeConfig = resolveNodoAssistMcpEnvTemplates(
    params.mergedConfig,
    params.env,
  ) as BundleMcpConfig;
...
export async function prepareCliBundleMcpConfig(params: {
   ...
   * crestodian MCP server instead of the normal nodoassist tool surface.
   */
...
export async function prepareCliBundleMcpCaptureAttempt(params: {
  ...
  return {
    env: {
      ...params.env,
      NODOASSIST_MCP_CLI_CAPTURE_KEY: params.captureKey,
    },
  };
}
