/**
 * Materializes selected NodoAssist skills as a temporary Claude CLI plugin.
 */
...
const NODOASSIST_CLAUDE_PLUGIN_NAME = "nodoassist-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, NODOASSIST_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: NODOASSIST_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped NodoAssist skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
