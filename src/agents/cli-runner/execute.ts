...
const NODOASSIST_MCP_TOOL_PREFIX = "mcp__nodoassist__";

function normalizeCliMessagingToolName(toolName: string): string {
  return toolName.startsWith(NODOASSIST_MCP_TOOL_PREFIX)
    ? toolName.slice(NODOASSIST_MCP_TOOL_PREFIX.length)
    : toolName;
}
...
const CLI_BACKEND_PRESERVE_ENV = "NODOASSIST_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.NODOASSIST_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.NODOASSIST_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.NODOASSIST_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.NODOASSIST_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.NODOASSIST_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
...
export async function executePreparedCliRun(
  ...
  const basePrompt = cliSessionIdToUse
    ? params.prompt
    : (context.nodoAssistHistoryPrompt ?? params.prompt);
      ...
      try {
        cliBackendLog.info(
          buildCliExecLogLine({
            ...
            hasHistoryPrompt: Boolean(context.nodoAssistHistoryPrompt),
          }),
        );
        const logOutputText =
        ...
        if (logOutputText) {
          ...
          if (
            env.NODOASSIST_MCP_TOKEN ||
            env.NODOASSIST_MCP_SESSION_KEY ||
            env.NODOASSIST_MCP_AGENT_ID
          ) {
          ...
          if (result.exitCode !== 0 || result.reason !== "exit") {
            if (result.reason === "no-output-timeout" || result.noOutputTimedOut) {
              ...
              const deferWatchdogNoticeForFreshRetry =
                ...
                Boolean(context.nodoAssistHistoryPrompt) &&
