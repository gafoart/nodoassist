...
const LIVE = isLiveTestEnabled(["NODOASSIST_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "nodoassist-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("nodoassist_live_probe", "NodoAssist Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an NodoAssist-configured streamable-http server", async () => {
    const geminiCommand = process.env.NODOASSIST_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("nodoassistLiveProbe");
