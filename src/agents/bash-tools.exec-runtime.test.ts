...
describe("runExecProcess POSIX command wrapper", () => {
  ...
  it("wraps command with PATH export if NODOASSIST_PREPEND_PATH is present", async () => {
    ...
    expect(commandStr).toContain(
      'export PATH="${NODOASSIST_PREPEND_PATH}${PATH:+:$PATH}"; unset NODOASSIST_PREPEND_PATH; echo test',
    );
  });
