...
const CAPTURE_MARKER = "__NODOASSIST_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__NODOASSIST_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // NODOASSIST_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.NODOASSIST_SHELL === "exec") {
    env.NODOASSIST_SHELL = "exec";
  }
  return env;
}
