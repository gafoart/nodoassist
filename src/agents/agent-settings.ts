...
function resolveCompactionReserveTokensFloor(cfg?: NodoAssistConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: NodoAssistConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers NodoAssist runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (nodoassist#75799).
 *
...
/**
 * Disable NodoAssist runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when NodoAssist or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (nodoassist#75799).
 * Default-mode runs against ordinary providers keep NodoAssist runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores NodoAssist runtime's
