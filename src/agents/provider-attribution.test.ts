...
vi.mock("../plugins/manifest-metadata-scan.js", () => ({
  listNodoAssistPluginManifestMetadata: () =>
...
describe("provider attribution", () => {
  it("resolves the canonical NodoAssist product and runtime version", () => {
  ...
  it("returns a documented Google Gemini attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      version: "2026.3.22",
      headers: {
        "x-goog-api-client": "nodoassist/2026.3.22",
      },
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { NODOASSIST_VERSION: "2026.3.22" })).toEqual(
      {
        provider: "openai",
        enabledByDefault: true,
    ...
    expect(
      resolveProviderAttributionPolicy("openai", { NODOASSIST_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "nodoassist",
      version: "2026.3.22",
      "User-Agent": "nodoassist/2026.3.22",
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { NODOASSIST_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
    ...
    expect(
      resolveProviderAttributionPolicy("xai", { NODOASSIST_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "nodoassist",
      version: "2026.3.22",
      "User-Agent": "nodoassist/2026.3.22",
  ...
  it("lists the current attribution support matrix", () => {
    ...
    expect(
      listProviderAttributionPolicies({ NODOASSIST_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
