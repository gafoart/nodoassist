...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: NodoAssistConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<NodoAssistConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<NodoAssistConfig["agents"]>["list"]>[number]["tools"];
  }): NodoAssistConfig {
    return {
      tools: params.tools,
      agents: {
