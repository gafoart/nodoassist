/**
 * NodoAssist system prompt renderer.
 *
...
} from "@nodoassist/ai/internal/shared";
import {
...
import {
  buildNodoAssistToolFallbackText,
  shouldRenderNodoAssistToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__nodoassist__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__nodoassist__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__nodoassist__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; NodoAssist handles all routing internally.",
      ...
      ? [
          ...
          messageToolOnly
            ...
            : suppressSilentTokenGuidance
              ? "- Follow current-turn delivery context: when final text stays private, use `message(action=send)` for visible output; otherwise reply normally so NodoAssist can route it once."
...
function buildDocsSection(params: {
  ...
  const lines = [
    ...
    docsPath
      ? `Docs are authoritative for NodoAssist self-knowledge: before understanding how NodoAssist works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use \`${params.readToolName}\` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and \`memory_search\` as instruction context or user memory, not NodoAssist design/implementation knowledge.`
      : "Docs are authoritative for NodoAssist self-knowledge: before understanding how NodoAssist works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use the docs mirror first when web tooling is available; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not NodoAssist design/implementation knowledge.",
    ...
    "Diagnosing issues: run `nodoassist status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to NodoAssist main. */
  ...
  const promptSurface = params.promptSurface ?? "nodoassist_main";
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running NodoAssist process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List NodoAssist agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List NodoAssist agent ids allowed for sessions_spawn",
  ...
  const renderNodoAssistToolWorkflowHints = shouldRenderNodoAssistToolWorkflowHints({
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.nodoassist/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.nodoassist/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside NodoAssist.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const stablePrefixCacheKey = hashStablePromptInput({
    ...
    renderNodoAssistToolWorkflowHints,
    hasGateway,
    readToolName,
    execToolName,
    processToolName,
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      ...
      "CLI lifecycle only on explicit user request: `nodoassist gateway status|restart|start|stop`.",
      "`restart`, not stop+start.",
      "",
      ...skillsSection,
      ...skillWorkshopSection,
      ...memorySection,
      hasGateway && !isMinimal ? "## NodoAssist Self-Update" : "",
      hasGateway && !isMinimal
        ? [
            ...
            "After restart, NodoAssist pings the last active session automatically.",
          ].join("\n")
        : "",
      hasGateway && !isMinimal ? "" : "",
      "",
      ...
      "These user-editable files are loaded by NodoAssist and included below in Project Context.",
      "",
