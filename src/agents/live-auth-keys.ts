...
const GOOGLE_LIVE_SINGLE_KEY = "NODOASSIST_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "NODOASSIST_LIVE_ANTHROPIC_KEY",
    listVar: "NODOASSIST_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "NODOASSIST_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `NODOASSIST_LIVE_${base}_KEY`;
...
export function collectProviderApiKeys(
  ...
  const forcedSingle = config.liveSingle
    ...
    : undefined;
  if (forcedSingle) {
    // NODOASSIST_LIVE_*_KEY pins a single key so retries do not rotate fixtures.
    return [forcedSingle];
  }
