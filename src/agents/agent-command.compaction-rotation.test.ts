...
const state = vi.hoisted(() => ({
  cfg: undefined as NodoAssistConfig | undefined,
...
vi.mock("./agent-scope.js", async () => {
  ...
  return {
    ...
    resolveAgentDir: () => state.agentDir ?? "/tmp/nodoassist-agent",
    resolveDefaultAgentId: () => "main",
    resolveEffectiveModelFallbacks: () => undefined,
    resolveSessionAgentId: () => "main",
    resolveAgentWorkspaceDir: () => state.workspaceDir ?? "/tmp/nodoassist-workspace",
  };
});
...
beforeEach(async () => {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-rotation-e2e-"));
