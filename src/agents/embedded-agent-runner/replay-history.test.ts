...
function nodoassistTranscriptAssistant(
...
describe("normalizeAssistantReplayContent", () => {
  ...
  it("filters nodoassist delivery-mirror and gateway-injected assistant messages from replay", () => {
    ...
    const messages = [
      userMessage("hello"),
      nodoassistTranscriptAssistant("delivery-mirror"),
      bedrockAssistant([{ type: "text", text: "real reply" }]),
      nodoassistTranscriptAssistant("gateway-injected"),
    ];
    const out = normalizeAssistantReplayContent(messages);
