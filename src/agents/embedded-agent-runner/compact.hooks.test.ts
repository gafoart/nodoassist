...
describe("compactEmbeddedAgentSessionDirect hooks", () => {
  ...
  it("preserves full sender identity when building compaction tools", async () => {
    ...
    expectRecordFields(mockCallArg(createNodoAssistCodingToolsMock), {
      senderId: "sender-1",
      senderName: "Alice",
      senderUsername: "alice_u",
      senderE164: "+15551234567",
  ...
  it("skips runtime tool construction when the compaction model does not support tools", async () => {
    ...
    expect(createNodoAssistCodingToolsMock).not.toHaveBeenCalled();
  });
  ...
  it("quarantines unsupported tool schemas before creating the compaction model session", async () => {
    ...
    createNodoAssistCodingToolsMock.mockReturnValueOnce([
      {
        name: "healthy_lookup",
  ...
  it("preserves direct OpenAI API-key compaction when NodoAssist runtime is active", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({ runtime: "nodoassist" });
    resolveModelMock.mockImplementation((provider = "openai", modelId = "fake") => ({
      model: { provider, api: "responses", id: modelId, input: [] },
      error: null,
      authStorage: { setRuntimeApiKey: vi.fn() },
    ...
    const result = await compactEmbeddedAgentSessionDirect({
      ...
      config: {
        ...
        agents: { defaults: { embeddedHarness: { runtime: "nodoassist" } } },
      } as never,
    });
...
describe("compactEmbeddedAgentSession hooks (ownsCompaction engine)", () => {
  ...
  it("preserves concrete NodoAssist pins over explicit Codex policy for queued compaction", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({
      runtime: "codex",
      runtimeSource: "model",
    } as never);
