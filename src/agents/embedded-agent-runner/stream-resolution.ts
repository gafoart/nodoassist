...
import { stripSystemPromptCacheBoundary } from "@nodoassist/ai/internal/shared";
...
let nodoAssistNativeCodexResponsesStreamFnForTest: StreamFn | undefined;
...
function isDefaultNodoAssistStreamFnForModel(
...
function resolveNodoAssistNativeCodexResponsesStreamFn(params: {
  model: EmbeddedRunAttemptParams["model"];
  ...
  if (!isDefaultNodoAssistStreamFnForModel(params.model, params.currentStreamFn)) {
    return undefined;
  }
  return nodoAssistNativeCodexResponsesStreamFnForTest ?? params.currentStreamFn ?? streamSimple;
}
...
export function describeEmbeddedAgentStreamStrategy(params: {
  ...
  if (
    resolveNodoAssistNativeCodexResponsesStreamFn({
      model: params.model,
      currentStreamFn: params.currentStreamFn,
    })
  ) {
    return "nodoassist-native-codex-responses";
  }
  if (isDefaultNodoAssistStreamFnForModel(params.model, params.currentStreamFn)) {
...
export function resolveEmbeddedAgentStreamFn(params: {
  ...
  const nodoAssistNativeCodexResponsesStreamFn = resolveNodoAssistNativeCodexResponsesStreamFn({
    model: params.model,
    currentStreamFn: params.currentStreamFn,
  });
  if (nodoAssistNativeCodexResponsesStreamFn) {
    return wrapEmbeddedAgentStreamFn(nodoAssistNativeCodexResponsesStreamFn, {
      runSignal: params.signal,
      resolvedApiKey: params.resolvedApiKey,
      authProfileId: params.authProfileId,
      authStorage: params.authStorage,
  ...
  if (
    isDefaultNodoAssistStreamFnForModel(params.model, params.currentStreamFn) ||
    hasResolvedRuntimeApiKey(params.resolvedApiKey) ||
    ...
    if (boundaryAwareStreamFn) {
      // Some NodoAssist session factories return a provider-specific stream wrapper
      // once runtime auth is resolved. Keep transport-supported APIs on
      // NodoAssist's HTTP transport so provider-specific auth/header semantics
...
export const testing = {
  setNodoAssistNativeCodexResponsesStreamFnForTest(streamFn: StreamFn | undefined): void {
    nodoAssistNativeCodexResponsesStreamFnForTest = streamFn;
  },
  resetNodoAssistNativeCodexResponsesStreamFnForTest(): void {
    nodoAssistNativeCodexResponsesStreamFnForTest = undefined;
  },
};
