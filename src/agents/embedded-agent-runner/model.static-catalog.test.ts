...
vi.mock("../../plugins/manifest-metadata-scan.js", () => ({
  listNodoAssistPluginManifestMetadata: manifestMocks.listNodoAssistPluginManifestMetadata,
}));
...
function setManifestPlugins(plugins: unknown[]) {
  ...
  manifestMocks.listNodoAssistPluginManifestMetadata.mockReturnValue(
  ...
  manifestMocks.loadPluginManifest.mockImplementation((pluginDir: string) => {
    ...
    return plugin
      ...
      : {
          ...
          manifestPath: `${pluginDir}/nodoassist.plugin.json`,
        };
  });
}
...
beforeEach(() => {
  manifestMocks.listNodoAssistPluginManifestMetadata.mockReset();
...
describe("resolveBundledStaticCatalogModel", () => {
  it("reuses one manifest scan across prepared lookups", () => {
    ...
    expect(manifestMocks.listNodoAssistPluginManifestMetadata).toHaveBeenCalledTimes(1);
  });
