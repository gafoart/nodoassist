...
export function createEmbeddedRunAuthController(params: {
  ...
  const applyApiKeyInfo = async (candidate?: string): Promise<void> => {
    ...
    if (!apiKeyInfo.apiKey) {
      ...
      // NodoAssist runtime's authStorage considers the provider authenticated.  Try
      ...
      // sentinel so NodoAssist runtime's hasConfiguredAuth() passes and the AWS SDK default
