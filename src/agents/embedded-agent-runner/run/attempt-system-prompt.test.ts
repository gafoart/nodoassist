  ...
  it("injects workspace identity context", () => {
    ...
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        userTimezone: "UTC",
        contextFiles: [
          { path: "/tmp/nodoassist/SOUL.md", content: "SOUL_CONTEXT_MARKER" },
          { path: "/tmp/nodoassist/IDENTITY.md", content: "IDENTITY_CONTEXT_MARKER" },
          { path: "/tmp/nodoassist/USER.md", content: "USER_CONTEXT_MARKER" },
        ],
      },
    ...
    expect(result.systemPrompt).toContain("## /tmp/nodoassist/SOUL.md");
    expect(result.systemPrompt).toContain("SOUL_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/nodoassist/IDENTITY.md");
    expect(result.systemPrompt).toContain("IDENTITY_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/nodoassist/USER.md");
  ...
  it("preserves bootstrap Project Context", () => {
    const result = buildAttemptSystemPrompt({
      ...
      embeddedSystemPrompt: {
        ...
        contextFiles: [
          {
            path: "/tmp/nodoassist/BOOTSTRAP.md",
            content: "Reply with BOOTSTRAP_OK.",
          },
          {
            path: "/tmp/nodoassist/SOUL.md",
            content: "SOUL_CONTEXT_MARKER",
          },
          {
            path: "/tmp/nodoassist/IDENTITY.md",
            content: "IDENTITY_CONTEXT_MARKER",
          },
          {
            path: "/tmp/nodoassist/USER.md",
    ...
    expect(result.systemPrompt).toContain("## /tmp/nodoassist/USER.md");
    expect(result.systemPrompt).toContain("USER_CONTEXT_MARKER");
    expect(result.systemPrompt).toContain("## /tmp/nodoassist/BOOTSTRAP.md");
