...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __nodoassistSessionLockPromptReleaseInstalled?: boolean;
};
...
function parsePromptReleasedMessageLine(
  ...
  try {
    ...
    const isNodoAssistTranscriptOnlyAssistant = isTranscriptOnlyNodoAssistAssistantMessage(message);
    if (
      typeof message.role !== "string" ||
      (!options?.allowAnyMessage && !isNodoAssistTranscriptOnlyAssistant)
    ) {
      return undefined;
    }
    return {
...
// Controllers in the same NodoAssist process can legitimately take turns writing
// the same session file while another attempt is released for model I/O. Track
// only fingerprints that changed while NodoAssist held the write lock so the
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "nodoassist.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__nodoassistSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__nodoassistSessionLockPromptReleaseInstalled"] = true;
