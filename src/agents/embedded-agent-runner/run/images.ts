...
function isNodoAssistCliImageCachePath(filePath: string): boolean {
  const parts = filePath.replaceAll("\\", "/").split("/");
  return parts.some((part, index) => {
    if (part === ".nodoassist-cli-images") {
      return true;
    }
    const parent = parts[index - 1] ?? "";
    return part === "nodoassist-cli-images" && /^nodoassist(?:-\d+)?$/.test(parent);
  });
}
...
export function detectImageReferences(prompt: string): DetectedImageRef[] {
  ...
  const addPathRef = (raw: string) => {
    ...
    if (isNodoAssistCliImageCachePath(resolved)) {
      return;
    }
    seen.add(dedupeKey);
