...
import type { NodoAssistCodingToolConstructionPlan } from "../../agent-tools.js";
...
// Names here must be emitted directly by createNodoAssistTools(). Catalog entries
...
const NODOASSIST_TOOL_FACTORY_NAMES = new Set([
  "agents_list",
  "crestodian",
  "canvas",
  "cron",
...
const ALL_CODING_TOOL_CONSTRUCTION_PLAN: NodoAssistCodingToolConstructionPlan = {
...
const NO_CODING_TOOL_CONSTRUCTION_PLAN: NodoAssistCodingToolConstructionPlan = {
...
function cloneCodingToolConstructionPlan(
  plan: NodoAssistCodingToolConstructionPlan,
): NodoAssistCodingToolConstructionPlan {
  return { ...plan };
}
...
function isKnownLocalCodingToolName(normalized: string): boolean {
  ...
  return (
    ...
    NODOASSIST_TOOL_FACTORY_NAMES.has(normalized)
  );
}
...
function resolveCodingToolConstructionPlanForAllowlist(
  ...
  const includeNodoAssistTools = normalized.some((name) => NODOASSIST_TOOL_FACTORY_NAMES.has(name));
...
export function resolveEmbeddedAttemptToolConstructionPlan(params: {
  ...
  codingToolConstructionPlan: NodoAssistCodingToolConstructionPlan;
} {
  ...
  const includeCoreTools =
    ...
    codingToolConstructionPlan.includeNodoAssistTools;
