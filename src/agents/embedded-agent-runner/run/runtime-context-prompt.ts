...
import {
  ...
  NODOASSIST_RUNTIME_EVENT_HEADER,
} from "../../internal-runtime-context.js";
import type { CurrentInboundPromptContext } from "./params.js";

const NODOASSIST_RUNTIME_EVENT_USER_PROMPT = "Continue the NodoAssist runtime event.";
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt.trim()) {
    return runtimeContext
      ? {
          prompt: NODOASSIST_RUNTIME_EVENT_USER_PROMPT,
          ...(modelPromptText.trim() && modelPromptText !== NODOASSIST_RUNTIME_EVENT_USER_PROMPT
            ? { modelPrompt: modelPromptText }
            : {}),
          runtimeContext,
          runtimeOnly: true,
