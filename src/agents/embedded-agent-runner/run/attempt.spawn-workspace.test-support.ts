...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createNodoAssistCodingToolsMock: UnknownMock;
...
const hoisted = vi.hoisted((): AttemptSpawnWorkspaceHoisted => {
  ...
  return {
    ...
    createNodoAssistCodingToolsMock,
...
vi.mock("../../docs-path.js", () => ({
  resolveNodoAssistReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createNodoAssistCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createNodoAssistCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("nodoassist.cache-ttl", data),
  ...
  readLastCacheTtlTimestamp: (
    ...
    for (let index = calls.length - 1; index >= 0; index -= 1) {
      ...
      if (customType !== "nodoassist.cache-ttl") {
        continue;
      }
      const entry = data as
        | {
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createNodoAssistCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.NODOASSIST_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.NODOASSIST_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.NODOASSIST_TRAJECTORY;
    } else {
      process.env.NODOASSIST_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
