...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const nodoassist = (message as unknown as Record<string, unknown>)["__nodoassist"];
    if (!nodoassist || typeof nodoassist !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (nodoassist as { beforeAgentRunBlocked?: unknown })
    ...
    const nextNodoAssist = {
      ...(nodoassist as Record<string, unknown>),
      beforeAgentRunBlocked: safeBlocked,
    };
    changed = true;
    return {
      ...(message as unknown as Record<string, unknown>),
      __nodoassist: nextNodoAssist,
