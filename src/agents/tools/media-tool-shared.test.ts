...
describe("resolveMediaToolLocalRoots", () => {
  it("does not widen default local roots from media sources", () => {
    const stateDir = path.join("/tmp", "nodoassist-media-tool-roots-state");
    const picturesDir =
    ...
    const roots = withEnv({ NODOASSIST_STATE_DIR: stateDir }, () =>
  ...
  it("keeps channel inbound attachment roots separate from local roots", () => {
    ...
    const accountRoot = path.join("/tmp", "nodoassist-imessage-work");
    const sharedRoot = path.join("/tmp", "nodoassist-imessage-shared");
    const cfg = {
      channels: {
        imessage: {
