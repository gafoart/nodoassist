...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.NODOASSIST_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      setTestEnvValue("NODOASSIST_CONFIG_PATH", previousConfigPath);
    }
    if (tempDir) {
  ...
  it("uses the oldest visible message for pagination after tool messages are filtered", async () => {
    const tool = createSessionsHistoryTool({
      ...
      callGateway: async <T = Record<string, unknown>>(): Promise<T> =>
        ({
          messages: [
            { role: "tool", content: "hidden", __nodoassist: { seq: 6 } },
            { role: "assistant", content: "visible", __nodoassist: { seq: 7 } },
            { role: "assistant", content: "latest", __nodoassist: { seq: 8 } },
          ],
          offset: 0,
          nextOffset: 5,
          hasMore: true,
