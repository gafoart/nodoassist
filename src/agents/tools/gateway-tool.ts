...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `nodoassist security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as NodoAssistConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as NodoAssistConfig);
