...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    nodoassist: process.env.NODOASSIST_GATEWAY_TOKEN,
    gatewayUrl: process.env.NODOASSIST_GATEWAY_URL,
  };
  ...
  afterAll(() => {
    if (envSnapshot.nodoassist === undefined) {
      delete process.env.NODOASSIST_GATEWAY_TOKEN;
    } else {
      process.env.NODOASSIST_GATEWAY_TOKEN = envSnapshot.nodoassist;
    }
    if (envSnapshot.gatewayUrl === undefined) {
      delete process.env.NODOASSIST_GATEWAY_URL;
    } else {
      process.env.NODOASSIST_GATEWAY_URL = envSnapshot.gatewayUrl;
    }
  });
  ...
  it("uses NODOASSIST_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    ...
    process.env.NODOASSIST_GATEWAY_TOKEN = "local-env-token";
    mocks.configState.value = {
      gateway: {
  ...
  it("explains stale gateway cron connection metadata rejections", async () => {
    ...
    await expect(
      ...
      "The running Gateway is from an older NodoAssist build and rejected current agent cron connection metadata. Restart the Gateway with `nodoassist gateway restart`, then retry.",
    );
  ...
  it("does not send the local approval runtime token to env-selected gateways", async () => {
    process.env.NODOASSIST_GATEWAY_URL = "wss://gateway.example";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateways", async () => {
    process.env.NODOASSIST_GATEWAY_URL = "ws://127.0.0.1:18789";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
  ...
  it("does not send the local approval runtime token to loopback env-selected gateway paths", async () => {
    process.env.NODOASSIST_GATEWAY_URL = "ws://127.0.0.1:18789/ws";
    mocks.callGateway.mockResolvedValueOnce({ decision: "allow-once" });
