...
import { resolveImageToolFactoryAvailable } from "../nodoassist-tools.media-factory-plan.js";
...
function createUserReportedConfig(params?: { includeApiKey?: boolean }): NodoAssistConfig {
...
function createBedrockSdkConfig(): NodoAssistConfig {
  return {
...
async function withEmptyAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-image-auth-regression-"));
  try {
