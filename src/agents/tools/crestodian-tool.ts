...
/** Validate nodoassist.json after a write so the agent can fix mistakes in-loop. */
...
export function createCrestodianTool(options: CrestodianToolOptions): AnyAgentTool {
  return {
    ...
    label: "Crestodian",
    description: [
      "Ring-zero NodoAssist setup and repair. Read actions (status/models/agents/channels/config_get/config_schema/gateway_status/plugin_search/validate_config/doctor/audit) run immediately.",
