...
type AcpSessionIdentifiersRuntime =
  typeof import("@nodoassist/acp-core/runtime/session-identifiers");
...
async function prepareAgentCommandExecution(opts: AgentCommandOpts, runtime: RuntimeEnv) {
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("nodoassist agents list")}" to see configured agents.`,
      );
    }
  }
 ...
 * this every diagnostics consumer (Langfuse bridge, @nodoassist/diagnostics-otel,
