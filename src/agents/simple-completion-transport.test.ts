import { registerApiProvider, unregisterApiProviders } from "@nodoassist/ai/internal/runtime";
...
const createNodoAssistTransportStreamFnForModel = vi.fn();
...
describe("prepareModelForSimpleCompletion", () => {
  ...
  beforeEach(() => {
    ...
    createNodoAssistTransportStreamFnForModel.mockReset();
    ...
    createNodoAssistTransportStreamFnForModel.mockReturnValue(undefined);
  ...
  it("routes provider-owned simple-completion wrappers through an internal API alias", () => {
    ...
    expect(result.api).toBe(
      "nodoassist-provider-simple:moonshot:kimi-k2.7-code:moonshot-simple-source:https%3A%2F%2Fapi.moonshot.ai%2Fv1",
    );
  ...
  it("uses a custom api alias for Anthropic Vertex simple completions", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "nodoassist-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
      "vertex-stream",
    );
    expect(result).toEqual({
      ...model,
      api: "nodoassist-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
    });
  });
  ...
  it("uses a transport-aware custom api alias when llm request transport overrides are present", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "nodoassist-openai-responses-transport",
      "transport-stream",
    );
    expect(result).toEqual({
      ...model,
  ...
  it("uses the Google simple-completion sanitizer alias after transport checks pass through", () => {
    ...
    prepareGoogleSimpleCompletionModel.mockImplementationOnce((m: unknown) => ({
      ...
      api: "nodoassist-google-generative-ai-simple",
    }));
    resolveProviderStreamFn.mockReturnValueOnce(undefined);
  ...
  it.each([
    ...
    "uses NodoAssist transport for OpenAI Codex-response simple completions with baseUrl %s",
    (baseUrl, expectedBaseUrl) => {
      ...
      resolveProviderStreamFn.mockReturnValueOnce(undefined);
      createNodoAssistTransportStreamFnForModel.mockReturnValueOnce("codex-transport-stream");
      resolveTransportAwareSimpleApi.mockReturnValueOnce("nodoassist-openai-responses-transport");
      ...
      expect(createNodoAssistTransportStreamFnForModel).toHaveBeenCalledWith(
        {
          ...model,
          baseUrl: expectedBaseUrl,
        },
