...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("nodoassist-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("nodoassist-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("nodoassist-tilde-test-mkdir-");
  ...
  it("ignores NODOASSIST_HOME for write operations", async () => {
    const nodoassistHome = await createTempDir("nodoassist-home-override-", os.tmpdir());
    const dir = await createTempDir("nodoassist-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ NODOASSIST_HOME: nodoassistHome }, async () => {
      createHostWorkspaceWriteTool(nodoassistHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(nodoassistHome, path.basename(testFile))));
    });
  });

  it("ignores NODOASSIST_HOME for mkdir operations", async () => {
    const nodoassistHome = await createTempDir("nodoassist-home-override-", os.tmpdir());
    const dir = await createTempDir("nodoassist-tilde-test-mkdir-");
    ...
    await withEnvAsync({ NODOASSIST_HOME: nodoassistHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(nodoassistHome, path.basename(newDir))));
    });
  });

  it("ignores NODOASSIST_HOME for readFile operations", async () => {
    const nodoassistHome = await createTempDir("nodoassist-home-override-", os.tmpdir());
    const dir = await createTempDir("nodoassist-tilde-test-edit-");
    ...
    await withEnvAsync({ NODOASSIST_HOME: nodoassistHome }, async () => {
      createHostWorkspaceEditTool(nodoassistHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores NODOASSIST_HOME for access operations", async () => {
    const nodoassistHome = await createTempDir("nodoassist-home-override-", os.tmpdir());
    const dir = await createTempDir("nodoassist-tilde-test-edit-");
