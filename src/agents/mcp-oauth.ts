 ...
 * private NodoAssist state directory with one hashed file per MCP server URL.
 */
...
/** Creates the MCP SDK OAuth provider backed by NodoAssist's private store. */
export function createMcpOAuthClientProvider(params: {
  ...
  const assertAuthorizationRedirectAllowed = () => {
    if (!allowAuthorizationRedirect) {
      throw new Error(
        `MCP server "${params.serverName}" requires OAuth authorization. Run nodoassist mcp login ${params.serverName}.`,
      );
    }
  };
  return {
