...
describe("reconcileSessionStoreCompactionCountAfterSuccess", () => {
  it("raises the stored compaction count to the observed value", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-reconcile-"));
  ...
  it("does not double count when the store is already at or above the observed value", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-idempotent-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
...
describe("compaction lifecycle logging", () => {
  it("logs lifecycle events at info level for gateway watch visibility", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-log-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
  ...
  it("logs manual compaction as incomplete when no result is produced", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-incomplete-log-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
  ...
  it("defaults legacy synthetic compaction events to threshold logs", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-legacy-log-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
...
describe("handleCompactionEnd", () => {
  it("reconciles the session store after a successful compaction end event", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-handler-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    await seedSessionStore({
      storePath,
  ...
  it("clears stale assistant usage before compaction and preserves fresh usage after it", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-usage-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const ctx = createCompactionContext({
      storePath,
  ...
  it("uses the compaction timestamp for summary-first transcripts", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-summary-first-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const ctx = createCompactionContext({
      storePath,
  ...
  it("uses index fallback only for legacy transcripts without timestamps", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-legacy-usage-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const ctx = createCompactionContext({
      storePath,
  ...
  it("clears assistant usage when final compaction has no summary marker", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-no-summary-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const ctx = createCompactionContext({
      storePath,
  ...
  it("does not let legacy index fallback erase timestamp-fresh usage", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-compaction-timestamp-fresh-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const ctx = createCompactionContext({
      storePath,
