    ...
    return await withEnvAsync(
      {
        ...
        ...(options.clearOAuthDir ? { NODOASSIST_OAUTH_DIR: undefined } : {}),
      },
      async () =>
        await run({
          stateDir,
...
describe("promoteAuthProfileInOrder", () => {
  it("marks newly saved runtime snapshot profiles as persisted", async () => {
    await withAuthProfileTestState(
      "nodoassist-auth-profile-runtime-persisted-",
      async ({ agentDir }) => {
  ...
  it("normalizes copied secrets when using the locked upsert path", async () => {
    await withAuthProfileTestState(
      "nodoassist-auth-profile-upsert-",
      async ({ agentDir }) => {
        fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("persists openai oauth credentials inline", async () => {
    await withAuthProfileTestState("nodoassist-auth-profile-metadata-", ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("preserves access-only openai oauth credentials inline", async () => {
    await withAuthProfileTestState("nodoassist-auth-profile-access-only-", ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      const profileId = "openai:default";
      const expires = Date.now() + 60 * 60 * 1000;
      saveAuthProfileStore(
  ...
  it("keeps copied openai oauth profiles inline", async () => {
    await withAuthProfileTestState("nodoassist-auth-profile-copy-ref-", ({ agentDirFor }) => {
  ...
  it("moves a relogin profile to the front of an existing per-agent provider order", async () => {
    await withAuthProfileTestState("nodoassist-auth-order-promote-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("creates a per-agent provider order when relogin has no existing order", async () => {
    await withAuthProfileTestState("nodoassist-auth-order-create-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("preserves config-only fallback ids when creating a relogin order", async () => {
    await withAuthProfileTestState("nodoassist-auth-order-config-only-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      const newProfileId = "openai:new-login";
  ...
  it("keeps implicit round-robin when relogin has no existing order by default", async () => {
    await withAuthProfileTestState("nodoassist-auth-order-implicit-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
      const newProfileId = "openai:new-login";
      saveAuthProfileStore(
        {
  ...
  it("clears matching lastGood after a stale refresh_token_reused profile", async () => {
    await withAuthProfileTestState("nodoassist-auth-clear-lastgood-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
  ...
  it("does not clear lastGood when the failed profile is not the stored profile", async () => {
    await withAuthProfileTestState("nodoassist-auth-clear-lastgood-keep-", async ({ agentDir }) => {
      fs.mkdirSync(agentDir, { recursive: true });
