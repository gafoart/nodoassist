...
import type { DB as NodoAssistAgentKyselyDatabase } from "../../state/nodoassist-agent-db.generated.js";
import {
  runNodoAssistAgentWriteTransaction,
  type NodoAssistAgentDatabase,
} from "../../state/nodoassist-agent-db.js";
import { NODOASSIST_SQLITE_BUSY_TIMEOUT_MS } from "../../state/nodoassist-state-db.js";
import { resolveUserPath } from "../../utils.js";
import { resolveRegisteredAgentIdForDir } from "../agent-dir-registry.js";
import { resolveDefaultAgentDir } from "../agent-scope-config.js";

type AuthProfileDatabase = Pick<
  NodoAssistAgentKyselyDatabase,
...
// The auth database lives in the agent dir and shares the nodoassist-agent schema
...
function resolveAuthProfileDatabaseOptions(agentDir?: string) {
  ...
  return {
    ...
    path: path.join(dir, "nodoassist-agent.sqlite"),
  };
}
...
export function readPersistedAuthProfileStoreRaw(
  agentDir?: string,
  database?: NodoAssistAgentDatabase,
): unknown {
  if (database) {
...
export function writePersistedAuthProfileStoreRaw(
  ...
  const write = (target: NodoAssistAgentDatabase) => {
  ...
  runNodoAssistAgentWriteTransaction(write, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function deletePersistedAuthProfileStoreRaw(
  ...
  const remove = (target: NodoAssistAgentDatabase) => {
    const db = getAuthProfileKysely(target.db);
    executeSqliteQuerySync(
      target.db,
  ...
  runNodoAssistAgentWriteTransaction(remove, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function runAuthProfileWriteTransaction<T>(
  ...
  return runNodoAssistAgentWriteTransaction(operation, resolveAuthProfileDatabaseOptions(agentDir));
}
