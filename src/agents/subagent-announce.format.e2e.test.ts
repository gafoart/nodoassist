...
let configOverride: NodoAssistConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: NodoAssistConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    previousFastTestEnv = process.env.NODOASSIST_TEST_FAST;
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // NODOASSIST_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("NodoAssist runtime context (internal):");
