...
beforeAll(async () => {
  sandboxFixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sandbox-context-"));
});
...
describe("resolveSandboxContext", () => {
  it("does not sandbox the agent main session in non-main mode", async () => {
    ...
    const result = await resolveSandboxContext({
      ...
      workspaceDir: "/tmp/nodoassist-test",
    });
  ...
  it("materializes skills into a hidden read-only workspace for writable sandboxes", async () => {
      ...
      agents: {
        defaults: {
          sandbox: {
            ...
            scope: "session",
            workspaceAccess: "rw",
            workspaceRoot: path.join(workspaceDir, ".nodoassist", "sandboxes"),
          },
        },
      },
    };
    ...
    expect(syncOptions?.targetWorkspaceDir).toContain(
      path.join(".nodoassist", "sandbox", "skills-workspaces"),
    );
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]agent-main-main-[a-f0-9]{8}[\\/]\.nodoassist[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".nodoassist", "sandbox-skills"),
    );
    expect(
      syncOptions?.targetWorkspaceDir?.startsWith(path.join(workspaceDir, ".nodoassist")),
    ).toBe(false);
    expect(syncOptions?.config).toBe(cfg);
    expect(syncOptions?.agentId).toBe("main");
    expect(syncOptions?.eligibility).toEqual({ remote: { note: "test-remote" } });
  ...
  it("uses the SSH backend remote workspace for sandbox workspace info", async () => {
    ...
    expect(result?.containerWorkdir).toMatch(
      /^\/remote\/nodoassist\/nodoassist-ssh-agent-main-main-[a-f0-9]{8}\/workspace$/,
    );
  ...
  it("materializes skills for shared writable sandboxes even when roots match", async () => {
    ...
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]shared-[a-f0-9]{8}[\\/]\.nodoassist[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".nodoassist", "sandbox-skills"),
    );
