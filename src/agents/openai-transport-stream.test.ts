...
import type { Api, Model } from "nodoassist/plugin-sdk/llm";
...
import {
  ...
  createNodoAssistTransportStreamFnForModel,
...
describe("openai transport stream", () => {
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    const thinkingBlock = output.content[0] as {
      ...
      nodoassistReasoningReplay?: unknown;
    };
    ...
    expect(replayItem).not.toHaveProperty("__nodoassist_replay");
    expect(thinkingBlock.nodoassistReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.NODOASSIST_DEBUG_MODEL_PAYLOAD;
    process.env.NODOASSIST_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.NODOASSIST_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.NODOASSIST_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.NODOASSIST_DEBUG_MODEL_PAYLOAD;
    process.env.NODOASSIST_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave NodoAssist", () => {
    const payload = {
      tools: [
        { type: "function", name: "exec" },
  ...
  it("adds NodoAssist attribution to native OpenAI transport headers and protects it from provider overrides", () => {
  ...
  it("adds NodoAssist attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("NODOASSIST_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      {
        id: "gpt-5.4-codex",
  ...
  it("adds SSE Accept only to native ChatGPT/Codex Responses stream requests", () => {
    ...
    const transportAliasModel = {
      ...codexModel,
      api: "nodoassist-openai-responses-transport" as Api,
    } satisfies Model;
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("nodoassist-openai-responses-transport");
    expectRecordFields(prepared, {
      api: "nodoassist-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "nodoassist-anthropic-messages-transport",
    );
    expectRecordFields(prepared, {
      api: "nodoassist-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "nodoassist-google-generative-ai-transport",
    );
  });
  ...
  it("uses top-level instructions for Codex responses and preserves prompt cache identity", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        nodoassist_session_id: "session-123",
        nodoassist_turn_id: "turn-123",
      },
    ) as Record<string, unknown> & {
  ...
  it("keeps Codex response shaping when simple completions use the NodoAssist transport alias", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.5",
        name: "GPT-5.5",
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripping cache identity", () => {
    const payload = {
      ...
      metadata: { nodoassist_session_id: "session-123" },
  ...
  it("omits prior Responses replay item ids when store is disabled for custom Codex-compatible responses", () => {
            ...
            content: [
              {
                ...
                nodoassistReasoningReplay: testing.buildOpenAIResponsesReasoningReplayMetadata(
                  model,
                  {
                    authProfileId: "openai:oauth",
                    sessionId: "session-123",
    ...
    expect(reasoningItem?.id).toBeUndefined();
    expect(reasoningItem).not.toHaveProperty("__nodoassist_replay");
    const assistantMessage = params.input?.find(
      (item) => item.type === "message" && item.role === "assistant",
    );
    expectRecordFields(assistantMessage, {
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        nodoassist_session_id: "session-123",
        nodoassist_turn_id: "turn-123",
        nodoassist_turn_attempt: "1",
        nodoassist_transport: "stream",
      },
