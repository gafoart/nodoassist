...
const SNAPSHOT_REF_PREFIX = "refs/nodoassist/snapshots";
const NODOASSIST_LOCK_PATTERN = /^nodoassist pid=(\d+)$/;
...
async function runSetupScript(repoRoot: string, worktreePath: string): Promise<void> {
  const setupScript = path.join(repoRoot, ".nodoassist", "worktree-setup.sh");
  ...
  const result = await runCommandWithTimeout([setupScript], {
    ...
    cwd: worktreePath,
    env: {
      NODOASSIST_SOURCE_TREE_PATH: repoRoot,
      NODOASSIST_WORKTREE_PATH: worktreePath,
    },
  });
  if (result.code !== 0) {
    throw new Error(
...
async function lockState(record: ManagedWorktreeRecord): Promise<LockState> {
  ...
  const match = NODOASSIST_LOCK_PATTERN.exec(entry.lockedReason);
  if (!match) {
...
async function snapshotWorktree(record: ManagedWorktreeRecord, reason: string): Promise<string> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-worktree-index-"));
  const indexPath = path.join(tempDir, "index");
  const snapshotRef = `${SNAPSHOT_REF_PREFIX}/${record.id}`;
  const env: NodeJS.ProcessEnv = {
    GIT_INDEX_FILE: indexPath,
    GIT_AUTHOR_NAME: "NodoAssist",
    GIT_AUTHOR_EMAIL: "nodoassist@localhost",
    GIT_COMMITTER_NAME: "NodoAssist",
    GIT_COMMITTER_EMAIL: "nodoassist@localhost",
  };
  try {
    ...
    const commit = await requireGit(
      record.path,
      ["commit-tree", tree, "-p", parent, "-m", `NodoAssist worktree snapshot: ${reason}`],
      { env },
    );
...
export class ManagedWorktreeService {
  ...
  async remove(params: {
    ...
    if ((state.kind === "live" || state.kind === "foreign") && !params.force) {
      throw new Error(
        state.kind === "live"
          ? `worktree is locked by live NodoAssist pid ${state.pid}`
