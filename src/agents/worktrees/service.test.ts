...
async function initializeRepository(root: string, name = "repo"): Promise<string> {
  ...
  await git(repo, "config", "user.name", "NodoAssist Test");
  await git(repo, "config", "user.email", "nodoassist-test@example.invalid");
...
describe("ManagedWorktreeService", () => {
  ...
  beforeEach(async () => {
    ...
    root = await fs.mkdtemp(path.join(tempRoot, "nodoassist-managed-worktrees-"));
    repo = await initializeRepository(root);
    env = { ...process.env, NODOASSIST_STATE_DIR: path.join(root, "nodoassist-state") };
  ...
  it("creates from origin HEAD and returns the existing live named worktree", async () => {
    ...
    expect(created.branch).toBe("nodoassist/remote-task");
  ...
  it("does not remove a concurrent successful create during remote fallback", async () => {
    ...
    expect(await git(created.path, "branch", "--show-current")).toBe("nodoassist/concurrent");
  });
  ...
  it("preserves a pre-existing branch when a managed name collides", async () => {
    await addRemote(root, repo);
    await git(repo, "branch", "nodoassist/existing-name", "HEAD");
    const branchTip = await git(repo, "rev-parse", "nodoassist/existing-name");

    await expect(service.create({ repoRoot: repo, name: "existing-name" })).rejects.toThrow(
      "branch already exists",
    );

    expect(await git(repo, "rev-parse", "nodoassist/existing-name")).toBe(branchTip);
  });
  ...
  it("runs an executable setup script with source and worktree paths", async () => {
    await fs.mkdir(path.join(repo, ".nodoassist"));
    const script = path.join(repo, ".nodoassist", "worktree-setup.sh");
    await fs.writeFile(
      script,
      '#!/bin/sh\nprintf "%s\\n%s\\n" "$NODOASSIST_SOURCE_TREE_PATH" "$NODOASSIST_WORKTREE_PATH" > setup-paths.txt\n',
      { mode: 0o755 },
    );
  ...
  it("removes the worktree and branch when setup fails", async () => {
    ...
    expect(await git(repo, "branch", "--list", "nodoassist/broken-setup")).toBe("");
  });
  ...
  it("exempts manual worktrees and garbage collects idle run-owned worktrees", async () => {
    ...
    await git(repo, "worktree", "lock", "--reason", "nodoassist pid=999999", created.path);
    now += IDLE_GC_MS + 1;
