...
describe("managed worktree registry", () => {
  ...
  beforeEach(async () => {
    ...
    root = await fs.mkdtemp(path.join(tempRoot, "nodoassist-worktree-registry-"));
    env = { ...process.env, NODOASSIST_STATE_DIR: path.join(root, "state") };
  });
  ...
  it("persists, orders, updates, and deletes worktree rows through Kysely", () => {
    ...
    updateRegistryWorktree(env, "first", {
      lastActiveAt: 30,
      removedAt: 40,
      snapshotRef: "refs/nodoassist/snapshots/first",
    });
