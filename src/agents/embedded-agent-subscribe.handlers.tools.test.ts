...
import type { AgentEvent } from "nodoassist/plugin-sdk/agent-core";
...
describe("handleToolExecutionEnd cron mutation tracking", () => {
  ...
  it.each([
    ["exec", "nodoassist cron add --at +1h --message 'follow up' --name reminder"],
    ["exec", "npx nodoassist cron add --at=+1h --message 'follow up'"],
    ["exec", "bunx nodoassist cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm exec nodoassist cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx nodoassist cron add --at +1h --message 'follow up'"],
    ["exec", "npx -y nodoassist cron add --at +1h --message 'follow up'"],
    ["exec", "bunx --bun nodoassist cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx nodoassist@latest cron add --at +1h --message 'follow up'"],
    ["exec", "npx nodoassist@latest cron add --at +1h --message 'follow up'"],
    ["exec", "bunx nodoassist@latest cron add --at +1h --message 'follow up'"],
    ["exec", "/usr/local/bin/nodoassist cron add --at +1h --message 'follow up'"],
    ["bash", "corepack pnpm exec nodoassist cron add --at +1h --message 'follow up'"],
    ["exec", "env NODOASSIST_PROFILE=test nodoassist cron add --at +1h --message 'follow up'"],
    ["exec", "nodoassist cron create --at +1h --message 'follow up'"],
    ["exec", "nodoassist --profile work cron create --at +1h --message 'follow up'"],
    ["exec", "nodoassist --dev cron add --at +1h --message 'follow up'"],
    ["exec", "nodoassist --log-level debug --no-color cron add --at +1h --message 'follow up'"],
    ["exec", "nodoassist --container helper cron add --at +1h --message 'follow up'"],
    ["exec", "nodoassist cron add --at +1h --message 'follow up || wait'"],
    ["exec", "nodoassist cron add --at +1h --message 'follow up' 2>&1"],
  ...
  it("does not increment successfulCronAdds when shell cron add fails", async () => {
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: {
          command: "nodoassist cron add --at +1h --message 'follow up' --name reminder",
        },
      } as never,
    );
  ...
  it.each([
    ["nodoassist cron list --json", "a different cron action"],
    ["echo nodoassist cron add --at +1h", "a command that only mentions cron add"],
    ["nodoassist cron add --at '+1h", "an unterminated shell argument"],
    ["cd /tmp && nodoassist cron add --at +1h", "a compound command"],
    ["nodoassist cron add --help", "the add command help"],
    ["nodoassist cron create -h", "the create alias help"],
    ["nodoassist cron add --bad||true", "a masked cron failure"],
    ["nodoassist cron add --at +1h; true", "a semicolon suffix"],
    ["nodoassist cron add --at +1h | cat", "a pipeline suffix"],
    ["nodoassist cron add --at +1h & true", "a background suffix"],
    ["nodoassist cron add --at +1h\ntrue", "a newline-separated suffix"],
    ["nodoassist cron add --bad # ignored\ntrue", "a comment-masked cron failure"],
    ["npx -y echo nodoassist cron add --at +1h", "a package runner for another executable"],
    ["pnpm nodoassist cron add --at +1h", "a bare pnpm package script"],
    ["corepack pnpm nodoassist cron add --at +1h", "a corepack pnpm package script"],
    ["nodoassist@latest cron add --at +1h", "a package spec without a package runner"],
    ["pnpm exec nodoassist@latest cron add --at +1h", "a package spec passed to pnpm exec"],
    ["nodoassist cron add --bad &>/tmp/cron.log", "a bash-only combined redirection"],
    ["nodoassist cron add --bad &>>/tmp/cron.log", "a bash-only append redirection"],
  ...
  it("preserves compact cwd labels in semantic raw exec metadata payload warnings", async () => {
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: { command: "git status", workdir: "/Users/agent/Projects/NodoAssist" },
      } as never,
    );
  ...
  it("redacts secrets in exec aggregated stdout before emitting command_output", async () => {
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: { command: "cat ~/.nodoassist/nodoassist.json" },
      } as never,
    );
