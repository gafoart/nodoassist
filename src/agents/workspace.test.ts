...
beforeEach(async () => {
  testState = await createNodoAssistTestState({
    layout: "state-only",
    prefix: "nodoassist-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses NODOASSIST_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NODOASSIST_HOME: "/srv/nodoassist-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/nodoassist-home"), ".nodoassist", "workspace"));
  });

  it("prefers NODOASSIST_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      NODOASSIST_WORKSPACE_DIR: "/srv/nodoassist-workspace",
      NODOASSIST_HOME: "/srv/nodoassist-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/nodoassist-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = ["nodoassist-workspace-state.json"] as const;
const LEGACY_WORKSPACE_STATE_PATH_SEGMENTS = [".nodoassist", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("nodoassist-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not nodoassist state\n";
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".nodoassist"), { recursive: true });
  ...
  it("does not overwrite a sibling file that is not an NodoAssist attestation marker", async () => {
    const tempDir = await makeTempWorkspace("nodoassist-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-workspace-hardlink-"));
    try {
