...
type CoreToolDefinition = {
  ...
  includeInNodoAssistGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const nodoassistTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInNodoAssistGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:nodoassist": nodoassistTools,
