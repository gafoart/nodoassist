...
const planNodoAssistModelsJsonMock = vi.fn();
...
function planParamsAt(callIndex: number): {
  ...
  // Planner call shape is the contract between ensureNodoAssistModelsJson and planning.
  const call = planNodoAssistModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planNodoAssistModelsJson: (...args: unknown[]) => planNodoAssistModelsJsonMock(...args),
  }));
  ...
  ({ ensureNodoAssistModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureNodoAssistModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureNodoAssistModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes implicit models.json into the configured default agent dir", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureNodoAssistModelsJson(cfg);

      expect(result.agentDir).toBe(path.join(home, ".nodoassist", "agents", "ops", "agent"));
      await expect(fs.access(path.join(result.agentDir, "models.json"))).resolves.toBeUndefined();
      await expectMissingPath(
        fs.access(path.join(home, ".nodoassist", "agents", "main", "agent", "models.json")),
      );
    });
  });
  ...
  it("writes plugin-owned model catalogs beside the agent plugin state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planNodoAssistModelsJsonMock.mockImplementation(async () => ({
        action: "write",
  ...
  it("removes stale plugin-owned model catalogs", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureNodoAssistModelsJson({}, agentDir);
  ...
  it("keeps generated plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planNodoAssistModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planNodoAssistModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureNodoAssistModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureNodoAssistModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planNodoAssistModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planNodoAssistModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureNodoAssistModelsJson(first);
      await ensureNodoAssistModelsJson(second);
      await ensureNodoAssistModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureNodoAssistModelsJson(first),
        ensureNodoAssistModelsJson(second),
      ]);
