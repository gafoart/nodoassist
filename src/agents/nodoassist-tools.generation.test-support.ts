// Shared registration assertions for optional media-generation NodoAssist tools.
...
export function describeNodoAssistGenerationToolRegistration(params: {
  suiteName: string;
  toolName: string;
  toolLabel: string;
}) {
  describe(params.suiteName, () => {
    it(`registers ${params.toolName} when ${params.toolLabel} is present`, () => {
      ...
      expect(collectPresentNodoAssistTools([tool])).toEqual([tool]);
    });

    it(`omits ${params.toolName} when ${params.toolLabel} is absent`, () => {
      expect(collectPresentNodoAssistTools([null]).map((tool) => tool.name)).not.toContain(
        params.toolName,
      );
    });
  });
