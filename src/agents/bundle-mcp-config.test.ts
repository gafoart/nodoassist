/** Tests merging bundled MCP defaults with NodoAssist user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets NodoAssist mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps NodoAssist transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled NodoAssist MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled NodoAssist MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
