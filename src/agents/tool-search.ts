 ...
 *
 * Presents large NodoAssist/MCP/client tool inventories through search, describe, call, and optional code-mode tools.
 */
...
type CatalogSource = "nodoassist" | "mcp" | "client";
...
function buildModelScriptSource(code) {
  return "(async (nodoassist, console) => {\n" + code + "\n})(nodoassist, console)";
}
...
function buildControllerSource() {
  ...
  return (
    ...
    "});\n" +
    "const nodoassist = Object.freeze({\n" +
    "  tools: Object.freeze({\n" +
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    nodoassist: { value: controller.nodoassist, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("nodoassist.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: NodoAssistConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: NodoAssistConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "nodoassist", sourceName: pluginId };
  }
  return { source: "nodoassist", sourceName: "core" };
}
...
function formatUnknownToolIdError(
  ...
  const recoveryText =
    options.recoverySurface === "code-mode"
      ? "Use nodoassist.tools.search to find a tool, nodoassist.tools.describe to inspect it, then nodoassist.tools.call with the exact id or name."
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with nodoassist.tools.search, nodoassist.tools.describe, and nodoassist.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The nodoassist.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call a selected Tool Search catalog entry through NodoAssist.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
