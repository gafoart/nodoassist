...
import { resolveClaudeFable5ModelIdentity } from "@nodoassist/llm-core";
...
import {
  ...
  prepareNodoAssistModelsJsonSource,
...
let manifestModelCatalogCache = new WeakMap<NodoAssistConfig, ManifestModelCatalogCacheEntry>();
...
function shouldLogModelCatalogTiming(): boolean {
  return process.env.NODOASSIST_DEBUG_INGRESS_TIMING === "1";
}
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: NodoAssistConfig): boolean {
...
export async function loadModelCatalog(params?: {
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      if (!readOnly) {
        const preparedSource = await prepareNodoAssistModelsJsonSource(cfg, agentDir, {
          pluginMetadataSnapshot: params?.metadataSnapshot,
          workspaceDir,
        });
