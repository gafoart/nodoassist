...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        NODOASSIST_BASE_ENV: "base",
        NODOASSIST_TO_REMOVE: undefined,
      },
      env: {
        NODOASSIST_TEST_ENV: "ok",
      },
    });

    expect(resolved.NODOASSIST_BASE_ENV).toBe("base");
    expect(resolved.NODOASSIST_TEST_ENV).toBe("ok");
    expect(resolved.NODOASSIST_TO_REMOVE).toBeUndefined();
    expect(resolved.NODOASSIST_CLI).toBe(NODOASSIST_CLI_ENV_VALUE);
  });
