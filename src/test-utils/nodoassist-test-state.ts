// Creates isolated NodoAssist state directories for integration-style tests.
...
type NodoAssistTestStateLayout = "home" | "state-only" | "split";

type NodoAssistTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type NodoAssistTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: NodoAssistTestStateLayout;
  scenario?: NodoAssistTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
export type NodoAssistTestState = {
  root: string;
  home: string;
  stateDir: string;
  configPath: string;
...
const DEFAULT_PREFIX = "nodoassist-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
  ...
  "NODOASSIST_SERVICE_REPAIR_POLICY",
] as const;
...
function resolveLayout(
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "nodoassist.json"),
...
function scenarioConfig(options: NodoAssistTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "nodoassist-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: NodoAssistTestStateOptions): Record<string, string | undefined> {
...
export async function createNodoAssistTestState(
  options: NodoAssistTestStateOptions = {},
): Promise<NodoAssistTestState> {
  ...
  const state: NodoAssistTestState = {
    root,
    ...paths,
    env,
    envVars,
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed NodoAssist env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withNodoAssistTestState<T>(
  options: NodoAssistTestStateOptions,
  fn: (state: NodoAssistTestState) => Promise<T>,
): Promise<T> {
  const state = await createNodoAssistTestState(options);
  try {
