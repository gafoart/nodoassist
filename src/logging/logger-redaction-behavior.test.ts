...
const logPathTracker = createSuiteLogPathTracker("nodoassist-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ NODOASSIST_CONFIG_PATH: configPath, NODOASSIST_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-nodoassist.log")).toBe(
        path.join(home, "custom-nodoassist.log"),
      );
    });
  });
