...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-log-tail-redaction-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("readConfiguredLogTail redaction", () => {
  it("redacts raw auth headers before returning log lines", async () => {
    ...
    const logFile = path.join(dir, "nodoassist.log");
    const configFile = path.join(dir, "nodoassist.json");
    ...
    expect(text).toContain("X-NodoAssist-Token: supers…7890");
    ...
    expect(text).not.toContain(nodoAssistToken);
