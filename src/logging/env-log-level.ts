...
/** Resolves NODOASSIST_LOG_LEVEL once per value, warning only when the invalid value changes. */
export function resolveEnvLogLevelOverride(): LogLevel | undefined {
  const trimmed = normalizeOptionalString(process.env.NODOASSIST_LOG_LEVEL) ?? "";
  if (!trimmed) {
  ...
  if (loggingState.invalidEnvLogLevelValue !== trimmed) {
    ...
    process.stderr.write(
      `[nodoassist] Ignoring invalid NODOASSIST_LOG_LEVEL="${trimmed}" (allowed: ${ALLOWED_LOG_LEVELS.join("|")}).\n`,
    );
  }
  return undefined;
}
