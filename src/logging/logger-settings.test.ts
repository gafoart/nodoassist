...
beforeEach(() => {
  envSnapshot = captureEnv(["NODOASSIST_TEST_FILE_LOG", "NODOASSIST_LOG_LEVEL"]);
  delete process.env.NODOASSIST_TEST_FILE_LOG;
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    logging.setLoggerConfigLoaderForTests(() => ({
      level: "debug",
      file: "/tmp/nodoassist-configured.log",
      maxFileBytes: 2048,
    }));
    ...
    expect(settings.file).toBe("/tmp/nodoassist-configured.log");
  ...
  it("uses defaults when no logging config is available", () => {
    ...
    expect(path.basename(settings.file)).toMatch(/^nodoassist-vitest-\d+-\d{4}-\d{2}-\d{2}\.log$/);
    expect(settings.file).not.toBe(
      `/tmp/nodoassist/nodoassist-${new Date().toISOString().slice(0, 10)}.log`,
    );
  });
});
