...
const SUPPORT_EXPORT_PREFIX = "nodoassist-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.NODOASSIST_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$NODOASSIST_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/nodoassist-sanitized.jsonl`: sanitized log summaries and metadata",
