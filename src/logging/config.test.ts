...
function writeConfig(source: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-logging-config-"));
  tempDirs.push(dir);
...
describe("readLoggingConfig", () => {
  ...
  it("skips mutating config loads for config schema", () => {
    process.argv = ["node", "nodoassist", "config", "schema"];
  ...
  it("reads logging config directly from the active config path", () => {
    const configPath = writeConfig(`{
      logging: {
        level: "debug",
        file: "/tmp/nodoassist-custom.log",
        maxFileBytes: 1234,
      },
    }`);
  ...
  it("supports JSON5 comments and trailing commas", () => {
    const configPath = writeConfig(`{
      // users commonly keep comments in nodoassist.json
      logging: {
        consoleLevel: "warn",
      },
    }`);
  ...
  it("returns undefined for missing or malformed config files", () => {
    withEnv(
      { NODOASSIST_CONFIG_PATH: path.join(os.tmpdir(), "nodoassist-missing-config.json") },
      () => {
        expect(readLoggingConfig()).toBeUndefined();
      },
    );
