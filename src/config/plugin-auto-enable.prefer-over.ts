...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.nodoassist) || !isRecord(entry.nodoassist.channel)) {
      continue;
    }
    const channel = entry.nodoassist.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: NodoAssistConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: NodoAssistConfig, pluginId: string) => boolean;
