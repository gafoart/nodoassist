...
import { resetConfigRuntimeState, type NodoAssistConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        NODOASSIST_MPM_CATALOG_PATHS: undefined,
...
export async function writeNodoAssistConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.NODOASSIST_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected NODOASSIST_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeNodoAssistConfig(home, config);
