...
function createRenameAuditRecord(home: string) {
  return finalizeConfigWriteAuditRecord({
    base: createAuditRecordBase(path.join(home, ".nodoassist", "nodoassist.json")),
    result: "rename",
    nextMetadata: {
      dev: "12",
      ino: "13",
...
function readAuditLog(home: string): unknown[] {
  const auditPath = path.join(home, ".nodoassist", "logs", "config-audit.jsonl");
  return fs
...
describe("config io audit helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "nodoassist-config-audit-" });
  ...
  it('ignores literal "undefined" home env values when choosing the audit log path', async () => {
    ...
    expect(auditPath).toBe(path.join(home, ".nodoassist", "logs", "config-audit.jsonl"));
  ...
  it("formats overwrite warnings with hash transition and backup path", () => {
    expect(
      ...
      "Config overwrite: /tmp/nodoassist.json (sha256 prev-hash -> next-hash, backup=/tmp/nodoassist.json.bak, changedPaths=3)",
    );
  });
  ...
  it("captures watch markers and next stat metadata for successful writes", () => {
    const base = createConfigWriteAuditRecordBase({
      configPath: "/tmp/nodoassist.json",
      env: {
        NODOASSIST_WATCH_MODE: "1",
        NODOASSIST_WATCH_SESSION: "watch-session-1",
        NODOASSIST_WATCH_COMMAND: "gateway --force",
      } as NodeJS.ProcessEnv,
      existsBefore: true,
      previousHash: "prev-hash",
      nextHash: "next-hash",
  ...
  it("drops next-file metadata and preserves error details for failed writes", () => {
    const base = createAuditRecordBase("/tmp/nodoassist.json");
  ...
  it("redacts structured audit records before persistence", async () => {
    ...
    const record = finalizeConfigWriteAuditRecord({
      base: {
        ...createAuditRecordBase(path.join(home, ".nodoassist", "nodoassist.json")),
        suspicious: [
    ...
    const raw = fs.readFileSync(
      path.join(home, ".nodoassist", "logs", "config-audit.jsonl"),
      "utf-8",
    );
  ...
  it("redacts the value half of `--flag=value` for secret flags", () => {
    const argv = ["nodoassist", "--token=ghp_realgithubtoken1234567890ABCD", "--port=8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["nodoassist", "--token=***", "--port=8080"]);
  });
  ...
  it("redacts standalone token shapes via the shared logging redaction patterns", () => {
    ...
    expect(result[1]).toBe("nodoassist");
  ...
  it("leaves non-secret arguments untouched", () => {
    const argv = ["node", "nodoassist", "gateway", "--port", "8080", "--bind", "lan"];
  ...
  it("masks the next arg after a secret flag even when it looks like another option", () => {
    const argv = ["nodoassist", "--token", "--port", "8080"];
    expect(redactConfigAuditArgv(argv)).toEqual(["nodoassist", "--token", "***", "8080"]);
  });

  it("redacts dash-leading secret values after bare secret flags", () => {
    const argv = ["nodoassist", "--password", "-secret-value"];
    expect(redactConfigAuditArgv(argv)).toEqual(["nodoassist", "--password", "***"]);
  });

  it("does not mask when a secret flag is the final arg with no value", () => {
    const argv = ["nodoassist", "--token"];
    expect(redactConfigAuditArgv(argv)).toEqual(["nodoassist", "--token"]);
  });
  ...
  it("redacts processInfo.argv when explicitly supplied to createConfigWriteAuditRecordBase", () => {
    const base = createConfigWriteAuditRecordBase({
      ...
      processInfo: {
        ...
        ppid: 1,
        cwd: "/work",
        argv: ["node", "nodoassist", "--token", "leaked-but-not-anymore-12345"],
        execArgv: [],
      },
    });
    expect(base.argv).toEqual(["node", "nodoassist", "--token", "***"]);
  });
  ...
  it("rewrites historical config-audit entries through redactConfigAuditArgv and preserves 0600 mode", async () => {
    ...
    const unredactedRecord = {
      ...
      source: "config-io",
      event: "config.write",
      configPath: path.join(home, ".nodoassist", "nodoassist.json"),
      pid: 1590563,
      ppid: 1590548,
      cwd: home,
      argv: [
        "/usr/bin/node",
        "/usr/local/bin/nodoassist.mjs",
        "config",
        "set",
    ...
    const alreadyRedactedRecord = {
      ...
      ppid: 1,
      cwd: home,
      argv: ["/usr/bin/node", "/usr/local/bin/nodoassist.mjs", "config", "set", "ui.theme", "dark"],
      execArgv: ["--disable-warning=ExperimentalWarning"],
      suspicious: [],
      result: "rename",
    };
  ...
  it("preserves malformed lines verbatim and counts them as skipped", async () => {
    ...
    const validUnredacted = {
      ts: "2026-05-02T00:03:48.471Z",
      argv: ["node", "nodoassist.mjs", "config", "set", "x", "xoxb-bad-token-1234567890abcdef"],
    };
  ...
  it("aborts without overwriting when the audit log is appended to after temp write", async () => {
    ...
    const appended = {
      ...
      argv: ["node", "nodoassist.mjs", "config", "set", "theme", "dark"],
      execArgv: [],
    };
    const original = `${JSON.stringify(unredacted)}\n`;
