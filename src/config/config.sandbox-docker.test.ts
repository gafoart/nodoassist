    ...
    const res = validateConfigObject({
      agents: {
        defaults: {
          sandbox: {
            docker: {
              binds: ["D:/data/nodoassist/src:/src:ro", "D:\\data\\nodoassist\\output:/output:rw"],
            },
          },
        },
      },
...
describe("sandbox browser binds config", () => {
          ...
          sandbox: {
            browser: {
              binds: [
                ...
                "D:/data/nodoassist/chrome:/data/chrome-windows:rw",
              ],
            },
          },
        },
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("nodoassist-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "nodoassist-sandbox-browser-global" },
      agentBrowser: { network: "nodoassist-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("nodoassist-sandbox-browser-agent");
  });
