...
/**
 * Nix mode detection: When NODOASSIST_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
export function resolveIsNixMode(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.NODOASSIST_NIX_MODE === "1";
}
...
const NEW_STATE_DIRNAME = ".nodoassist";
const CONFIG_FILENAME = "nodoassist.json";
...
/** Build a homedir thunk that respects NODOASSIST_HOME for the given env. */
 ...
 * Can be overridden via NODOASSIST_STATE_DIR.
 * Default: ~/.nodoassist
 */
export function resolveStateDir(
  ...
  const override = env.NODOASSIST_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, effectiveHomedir);
  }
  const newDir = newStateDir(effectiveHomedir);
  if (env.NODOASSIST_TEST_FAST === "1") {
    return newDir;
  }
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const nodoassistOverride = env.NODOASSIST_STATE_DIR?.trim();
  if (nodoassistOverride) {
    env.NODOASSIST_STATE_DIR = resolveUserPath(nodoassistOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `NODOASSIST_INCLUDE_ROOTS` as a
 ...
 * directory containing `nodoassist.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.NODOASSIST_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
 ...
 * Can be overridden via NODOASSIST_CONFIG_PATH.
 * Default: ~/.nodoassist/nodoassist.json (or $NODOASSIST_STATE_DIR/nodoassist.json)
 */
function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, envHomedir(env)),
): string {
  const override = env.NODOASSIST_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveConfigPath(
  ...
  const stateOverride = env.NODOASSIST_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.NODOASSIST_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const nodoassistStateDir = env.NODOASSIST_STATE_DIR?.trim();
  if (nodoassistStateDir) {
    const resolved = resolveUserPath(nodoassistStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/nodoassist-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `nodoassist-${uid}` : "nodoassist";
 ...
 *
 * Precedence:
 * - `NODOASSIST_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.NODOASSIST_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.NODOASSIST_GATEWAY_PORT?.trim();
