...
const mockReadSourceConfigSnapshot = vi.hoisted(() => async () => {
  ...
  const configPath = pathValue.join(process.env.NODOASSIST_STATE_DIR ?? "", "nodoassist.json");
  try {
...
const mockReplaceConfigFile = vi.hoisted(() => async ({ nextConfig }: { nextConfig: unknown }) => {
  ...
  const configPath = pathLocal.join(process.env.NODOASSIST_STATE_DIR ?? "", "nodoassist.json");
