...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.NODOASSIST_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.NODOASSIST_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.NODOASSIST_WATCH_COMMAND),
...
export type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `nodoassist doctor --fix` once the
