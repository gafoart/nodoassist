...
import { withTempHome, writeNodoAssistConfig } from "./test-helpers.js";
...
describe("readBestEffortConfig", () => {
  ...
  it("can read snapshots without updating config observation state", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeNodoAssistConfig(home, {
        gateway: { mode: "local" },
      });

      await readConfigFileSnapshot({ observe: false });

      const healthPath = `${home}/.nodoassist/logs/config-health.json`;
  ...
  it("can read snapshots without applying config env vars to the process", async () => {
    await withTempHome(async (home) => {
      const key = "NODOASSIST_ISOLATED_CONFIG_READ_TEST";
      await withEnvAsync({ [key]: undefined }, async () => {
        await writeNodoAssistConfig(home, {
  ...
  it("resolves config env above exact lower-precedence values in isolated snapshots", async () => {
    await withTempHome(async (home) => {
      const key = "NODOASSIST_GATEWAY_TOKEN";
  ...
  it("can read best-effort config without applying env vars or recording observation", async () => {
    await withTempHome(async (home) => {
      const key = "NODOASSIST_ISOLATED_BEST_EFFORT_CONFIG_TEST";
      await withEnvAsync({ [key]: undefined }, async () => {
        await writeNodoAssistConfig(home, {
          env: { vars: { [key]: "from-config" } },
          gateway: { mode: "local" },
        ...
        expect(process.env[key]).toBeUndefined();
        await expect(fs.stat(`${home}/.nodoassist/logs/config-health.json`)).rejects.toMatchObject({
          code: "ENOENT",
        });
      });
    });
  ...
  it("preserves Windows case-insensitive env lookup in isolated reads", async () => {
    await withTempHome(async (home) => {
      const mixedCaseKey = "NodoAssist_Config_Path";
      const customConfigPath = `${home}/custom-nodoassist.json`;
  ...
  it("does not restore suspicious direct edits from .bak during ordinary reads", async () => {
    await withTempHome(async (home) => {
      ...
      const entries = await fs.readdir(`${home}/.nodoassist`);
      expect(entries.some((entry) => entry.startsWith("nodoassist.json.clobbered."))).toBe(false);
    });
  });
