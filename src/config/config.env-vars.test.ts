...
import type { NodoAssistConfig } from "./types.js";
...
describe("config env vars", () => {
  ...
  it("applies config env above normalized lower-precedence aliases", () => {
    ...
    applyConfigEnvVars({ env: { vars: { Z_AI_API_KEY: "config-key" } } } as NodoAssistConfig, env, {
  ...
  it("preserves a higher-precedence normalized alias", () => {
    ...
    applyConfigEnvVars({ env: { vars: { ZAI_API_KEY: "config-key" } } } as NodoAssistConfig, env, {
      lowerPrecedenceEnv: { ZAI_API_KEY: "shell-key" },
    });
  ...
  it.runIf(process.platform !== "win32")("keeps unrelated POSIX env casing distinct", () => {
    ...
    applyConfigEnvVars({ env: { vars: { foo: "config-key" } } } as NodoAssistConfig, env);
  ...
  it("applies env vars from env.vars when missing", async () => {
    await withEnvOverride({ GROQ_API_KEY: undefined }, async () => {
      applyConfigEnvVars({ env: { vars: { GROQ_API_KEY: "gsk-config" } } } as NodoAssistConfig);
  ...
  it("preserves Windows case-insensitive env precedence in merged runtime env", () => {
    ...
    try {
      const merged = createConfigRuntimeEnv(
        { env: { vars: { NODOASSIST_LOAD_SHELL_ENV: "1" } } } as NodoAssistConfig,
        { NodoAssist_Load_Shell_Env: "0" },
      );

      expect(merged.NODOASSIST_LOAD_SHELL_ENV).toBe("0");
      expect(Object.keys(merged)).toEqual(["NodoAssist_Load_Shell_Env"]);
    } finally {
  ...
  it("blocks dangerous startup env vars from config env", async () => {
    await withEnvOverride(
      ...
      async () => {
        const config = {
          env: {
            vars: {
              ...
              NODOASSIST_INCLUDE_ROOTS: "/tmp/evil-include-root",
              nodoassist_allow_older_binary_destructive_actions: "1",
              OPENROUTER_API_KEY: "config-key",
            },
          },
        };
        const entries = collectConfigRuntimeEnvVars(config as NodoAssistConfig);
        ...
        expect(entries.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
        expect(entries.NODOASSIST_INCLUDE_ROOTS).toBeUndefined();
        expect(entries.nodoassist_allow_older_binary_destructive_actions).toBeUndefined();
        expect(entries.OPENROUTER_API_KEY).toBe("config-key");

        applyConfigEnvVars(config as NodoAssistConfig);
  ...
  it("loads ${VAR} substitutions from ~/.nodoassist/.env on repeated runtime loads", async () => {
    await withTempHome(async (_home) => {
      await withEnvOverride({ BRAVE_API_KEY: undefined }, async () => {
        const stateDir = process.env.NODOASSIST_STATE_DIR?.trim();
        if (!stateDir) {
          throw new Error("Expected NODOASSIST_STATE_DIR to be set by withTempHome");
        }
        ...
        const first = resolveConfigEnvVars(config, process.env) as NodoAssistConfig;
        expect(first.tools?.web?.search?.apiKey).toBe("from-dotenv");

        delete process.env.BRAVE_API_KEY;
        loadDotEnv({ quiet: true });
        const second = resolveConfigEnvVars(config, process.env) as NodoAssistConfig;
  ...
  it("drops dangerous and empty values from the state-dir .env file", async () => {
    await withTempHome(async (_home) => {
      ...
      expect(vars.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
      expect(vars.EMPTY).toBeUndefined();
      expect(vars.VALID).toBe("ok");
    });
  });

  it("respects NODOASSIST_STATE_DIR when reading state-dir .env vars", async () => {
    await withTempHome(async (_home) => {
      const customStateDir = path.join(process.env.NODOASSIST_STATE_DIR ?? "", "custom-state");
      await writeStateDirDotEnv("CUSTOM_KEY=from-override\n", {
        stateDir: customStateDir,
      });
      expect(
        readStateDirDotEnvVars({
          NODOASSIST_STATE_DIR: customStateDir,
        }).CUSTOM_KEY,
      ).toBe("from-override");
    });
  });
