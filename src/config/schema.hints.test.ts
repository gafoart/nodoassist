...
import { isSensitiveUrlConfigPath } from "@nodoassist/net-policy/redact-sensitive-url";
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    const schema = NodoAssistSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "NodoAssistConfig";
    const hints = mapSensitivePaths(NodoAssistSchema, "", {});
...
describe("collectMatchingSchemaPaths", () => {
  it("finds base-config URL fields that may embed secrets", () => {
    const paths = collectMatchingSchemaPaths(NodoAssistSchema, "", isSensitiveUrlConfigPath);
