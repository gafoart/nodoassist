...
// Keep integration tests deterministic: never read a real nodoassist.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "nodoassist-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.NODOASSIST_SESSION_CACHE_TTL_MS;
    process.env.NODOASSIST_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  });
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.NODOASSIST_SESSION_CACHE_TTL_MS;
    } else {
      process.env.NODOASSIST_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"nodoassist-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"nodoassist-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
        traceSchema: "nodoassist-trajectory-pointer",
        schemaVersion: 1,
        sessionId: staleSessionId,
