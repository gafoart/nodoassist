...
export function listConfiguredSessionStoreAgentIds(cfg: NodoAssistConfig): string[] {
...
export function resolveSessionStoreTargets(
  ...
  if (hasAgent) {
    ...
    if (!knownAgents.includes(requested)) {
      throw new Error(
        `Unknown agent id "${opts.agent}". Use "nodoassist agents list" to see configured agents.`,
      );
    }
    return [
      {
