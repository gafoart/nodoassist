...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, NodoAssist env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still NodoAssist-managed keys:
 ...
 * literal references for keys NodoAssist previously managed.
 */
...
/**
 * Read and parse `~/.nodoassist/.env` (or `$NODOASSIST_STATE_DIR/.env`), returning
