...
const NIX_NODOASSIST_AGENT_FIRST_URL = "https://github.com/openclaw/nix-openclaw#quick-start";
/** Public NodoAssist Nix overview shown with immutable-config errors. */
const NODOASSIST_NIX_OVERVIEW_URL = "https://docs.openclaw.ai/install/nix";
...
export class NixModeConfigMutationError extends Error {
  readonly code = "NODOASSIST_NIX_MODE_CONFIG_IMMUTABLE";
...
function formatNixModeConfigMutationMessage(params: { configPath?: string } = {}): string {
  return [
    ...
    "This usually means nix-nodoassist, the first-party Nix distribution, or another Nix-managed package set this mode.",
    ...
    "Edit the Nix source for this install instead. For nix-nodoassist, edit `programs.nodoassist.config` or `instances.<name>.config`, then rebuild with Home Manager or NixOS.",
    `Agent-first Nix setup: ${NIX_NODOASSIST_AGENT_FIRST_URL}`,
    `NodoAssist Nix overview: ${NODOASSIST_NIX_OVERVIEW_URL}`,
  ].join("\n");
}

/** Throw when the current environment marks NodoAssist config as Nix-managed and immutable. */
