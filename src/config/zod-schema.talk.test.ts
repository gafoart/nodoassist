...
describe("NodoAssistSchema talk validation", () => {
  ...
  it.each([
    ...
    expect(() => NodoAssistSchema.parse({ talk: { realtime } })).toThrow();
  });
  ...
  it.each(["constructor", "__proto__"])(
    ...
    (provider) => {
      ...
      expect(NodoAssistSchema.safeParse({ talk: { provider, providers } }).success).toBe(false);
      expect(
        NodoAssistSchema.safeParse({ talk: { realtime: { provider, providers } } }).success,
      ).toBe(false);
    },
  );
