...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ NODOASSIST_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-nodoassist",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-nodoassist#general");
  });
  ...
  it("derives session transcripts dir from NODOASSIST_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { NODOASSIST_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when NODOASSIST_STATE_DIR differs from stored paths", () => {
