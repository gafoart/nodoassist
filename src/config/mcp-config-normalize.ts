...
type NodoAssistMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_NODOASSIST_TRANSPORT: Record<string, NodoAssistMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to NodoAssist HTTP transport names. */
export function resolveNodoAssistMcpTransportAlias(
  value: unknown,
): NodoAssistMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_NODOASSIST_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias NodoAssist can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_NODOASSIST_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveNodoAssistMcpTransportAlias(next.type);
  // `transport` is NodoAssist's canonical field; legacy `type` only fills a gap.
