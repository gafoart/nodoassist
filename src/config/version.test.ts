...
import {
  compareNodoAssistVersions,
  isSameNodoAssistStableFamily,
  parseNodoAssistVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseNodoAssistVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseNodoAssistVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseNodoAssistVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseNodoAssistVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseNodoAssistVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseNodoAssistVersion("2026.3")).toBeNull();
    expect(parseNodoAssistVersion("latest")).toBeNull();
  });
});

describe("compareNodoAssistVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareNodoAssistVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareNodoAssistVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareNodoAssistVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareNodoAssistVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareNodoAssistVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareNodoAssistVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameNodoAssistStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameNodoAssistStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameNodoAssistStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameNodoAssistStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameNodoAssistStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
