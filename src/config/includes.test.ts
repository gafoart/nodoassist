...
const ETC_NODOASSIST_DIR = path.join(ROOT_DIR, "etc", "nodoassist");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "nodoassist.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcNodoAssistPath(...parts: string[]) {
  return path.join(ETC_NODOASSIST_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcNodoAssistPath("agents.json");
    ...
    expectResolveIncludeError(
      () =>
        resolve(
          ...
          configPath("sub", "nodoassist.json"),
        ),
...
describe("resolveConfigIncludeWritePath", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "nodoassist-include-write-path-" }, async (tempRoot) => {
        ...
        expect(
          resolveConfigIncludeWritePath({
            configPath: path.join(configDir, "nodoassist.json"),
...
describe("security: path traversal protection (CWE-22)", () => {
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      await withTempDir({ prefix: "nodoassist-includes-symlink-" }, async (tempRoot) => {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "nodoassist.json"),
        );
    ...
    it("rejects include files that are hardlinked aliases", async () => {
      ...
      await withTempDir({ prefix: "nodoassist-includes-hardlink-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
    ...
    it("rejects oversized include files", async () => {
      await withTempDir({ prefix: "nodoassist-includes-big-" }, async (tempRoot) => {
        const configDir = path.join(tempRoot, "config");
        await fs.mkdir(configDir, { recursive: true });
...
describe("NODOASSIST_INCLUDE_ROOTS allowlist", () => {
  ...
  it("still rejects include paths that fall outside every allowed root", () => {
    const obj = { $include: etcNodoAssistPath("agents.json") };
    expect(() =>
  ...
  it("resolves a symlinked include whose realpath lands inside an allowed root", async () => {
    await withTempDir({ prefix: "nodoassist-includes-allowed-symlink-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
  ...
  it("rejects a symlinked include that escapes both the config directory and every allowed root", async () => {
    await withTempDir({ prefix: "nodoassist-includes-allowed-escape-" }, async (tempRoot) => {
      const configDir = path.join(tempRoot, "config");
      const allowedDir = path.join(tempRoot, "allowed");
