...
describe("config io write prepare", () => {
      ...
      resolvePersistCandidateForWrite({
        ...
        sourceConfig: {
          plugins: {
            entries: {},
            installs: {
              "nodoassist-web-search": {
                source: "npm",
                spec: "@ollama/nodoassist-web-search",
                installPath: "/tmp/nodoassist-web-search",
                resolvedName: "@ollama/nodoassist-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "nodoassist-web-search": {
                source: "npm",
                spec: "@ollama/nodoassist-web-search@0.2.2",
                installPath: "/tmp/nodoassist-web-search",
                resolvedName: "@ollama/nodoassist-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("nodoassist config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('nodoassist config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/nodoassist.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: NodoAssistConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
