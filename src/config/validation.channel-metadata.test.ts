...
describe("validateConfigObjectRawWithPlugins channel metadata", () => {
    ...
    if (!result.ok) {
      expect(result.issues).toContainEqual(
        expect.objectContaining({
          ...
          message:
            'invalid config for plugin nodoassist-lark: must not have additional properties: "unsupportedField"',
        }),
      );
    }
  });
  ...
  it("sanitizes the schema owner in validation diagnostics", () => {
    const unsafeId = `nodoassist${String.fromCharCode(10)}${String.fromCharCode(27)}[31m-lark`;
    const registry = createExternalFeishuSchemaRegistry();
    registry.plugins[0] = {
      ...registry.plugins[0],
      id: unsafeId,
  ...
  it("keeps raw channel validation diagnostics plugin-agnostic", () => {
    ...
    if (!result.ok) {
      ...
      expect(result.issues[0]?.message).not.toContain("nodoassist doctor --fix");
    }
  });
});
