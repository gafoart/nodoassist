...
function isBlockedConfigEnvVar(key: string): boolean {
  return (
    ...
    key.toUpperCase() === "NODOASSIST_INCLUDE_ROOTS" ||
...
function collectConfigEnvVarsByTarget(cfg?: NodoAssistConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: NodoAssistConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: NodoAssistConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.NODOASSIST_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
