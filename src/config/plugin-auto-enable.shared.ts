...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: NodoAssistConfig): boolean {
...
function isProviderConfigured(cfg: NodoAssistConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: NodoAssistConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: NodoAssistConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: NodoAssistConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: NodoAssistConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: NodoAssistConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: NodoAssistConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: NodoAssistConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: NodoAssistConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: NodoAssistConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: NodoAssistConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: NodoAssistConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: NodoAssistConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: NodoAssistConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: NodoAssistConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: NodoAssistConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: NodoAssistConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: NodoAssistConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: NodoAssistConfig): boolean {
  return (
...
function hasPluginEntries(cfg: NodoAssistConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: NodoAssistConfig): boolean {
  if (
...
function arePluginsGloballyDisabled(cfg: NodoAssistConfig): boolean {
...
function isPluginExplicitlyDisabled(cfg: NodoAssistConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: NodoAssistConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: NodoAssistConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: NodoAssistConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
