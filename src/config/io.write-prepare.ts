...
import { normalizeConfiguredProviderCatalogModelId } from "@nodoassist/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  nodoassist config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  nodoassist config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: NodoAssistConfig,
  pathSegments: string[],
): { changed: boolean; next: NodoAssistConfig } {
