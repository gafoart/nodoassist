// Defines the top-level NodoAssist configuration type.
...
/** Top-level NodoAssist config as read from user/project config files. */
export type NodoAssistConfig = {
  /** JSON schema URL used by editors and generated config files. */
  $schema?: string;
  meta?: {
    /** Last NodoAssist version that wrote this config. */
  ...
  wizard?: {
    ...
    lastRunAt?: string;
    /** NodoAssist version used by the last completed wizard run. */
  ...
  ui?: {
    /** Accent color for NodoAssist UI chrome (hex). */
    seamColor?: string;
    assistant?: {
...
export type NodoAssistConfigInput = Omit<NodoAssistConfig, "models"> & {
  models?: ModelsConfigInput;
};

declare const nodoAssistConfigStateBrand: unique symbol;

type BrandedConfigState<TState extends string> = NodoAssistConfig & {
  readonly [nodoAssistConfigStateBrand]?: TState;
};
