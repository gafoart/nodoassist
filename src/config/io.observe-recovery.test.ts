...
describe("config observe recovery", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "nodoassist-config-observe-recovery-"));
  });
  ...
  function makeDeps(
    ...
    return {
      ...
      configPath,
      auditPath: path.join(home, ".nodoassist", "logs", "config-audit.jsonl"),
      warn,
    };
  }
  ...
  it("loadConfig clears env vars from the discarded clobbered config before rereading backup", async () => {
    await withSuiteHome(async (home) => {
      ...
      await writeConfigRaw(configPath, {
        meta: { lastTouchedVersion: "2026.5.28" },
        env: { vars: { NODOASSIST_CLOBBER_ONLY: "bad" } },
      });

      const config = io.loadConfig();

      expect(config.gateway?.mode).toBe("local");
      expect(env.NODOASSIST_CLOBBER_ONLY).toBeUndefined();
    });
  });
  ...
  it("validates backup candidates without leaking their env into live state", async () => {
    await withSuiteHome(async (home) => {
      ...
      await seedConfigBackup(configPath, {
        gateway: { mode: "local" },
        env: { vars: { NODOASSIST_BACKUP_ONLY: "stale" } },
        agents: { defaults: { model: 123 } },
      });
      await writeConfigRaw(configPath, {
        meta: { lastTouchedVersion: "2026.5.28" },
      ...
      expect(env.NODOASSIST_BACKUP_ONLY).toBeUndefined();
    });
  });
  ...
  it("logs async health-state write failures", async () => {
    await withSuiteHome(async (home) => {
      ...
      await expectPathMissing(path.join(home, ".nodoassist", "logs", "config-health.json"));
