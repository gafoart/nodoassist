...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "NodoAssist ignores provider credentials and protected runtime controls from workspace `.env`",
    );
    expect(markdown).toContain("~/.nodoassist/.env");
    expect(markdown).toContain("$NODOASSIST_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("NODOASSIST_LOAD_SHELL_ENV=1");
