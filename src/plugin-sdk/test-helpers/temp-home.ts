// Temp home test helpers create isolated NodoAssist home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked NODOASSIST_HOME.
  deleteTestEnvValue("NODOASSIST_HOME");
  setTestEnvValue("NODOASSIST_STATE_DIR", path.join(base, ".nodoassist"));
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "nodoassist-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".nodoassist", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
