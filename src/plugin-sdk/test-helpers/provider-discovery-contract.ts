...
import type { AuthProfileStore, NodoAssistConfig } from "../provider-auth.js";
import {
...
function installDiscoveryHooks(state: DiscoveryState, options: DiscoveryContractOptions) {
  beforeAll(async () => {
    vi.resetModules();
    vi.doMock("nodoassist/plugin-sdk/agent-runtime", () => {
      return {
        ensureAuthProfileStore: ensureAuthProfileStoreMock,
        listProfilesForProvider: listProfilesForProviderMock,
      };
    });
    vi.doMock("nodoassist/plugin-sdk/provider-auth", () => {
      return {
        DEFAULT_COPILOT_API_BASE_URL: "https://api.individual.githubcopilot.com",
        MINIMAX_OAUTH_MARKER: "minimax-oauth",
        applyAuthProfileConfig: (config: NodoAssistConfig) => config,
