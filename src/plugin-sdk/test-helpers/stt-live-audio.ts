...
export const NODOASSIST_LIVE_TRANSCRIPT_MARKER_RE = /open(?:claw|cl|flaw|clar|core)/;

export function expectNodoAssistLiveTranscriptMarker(value: string): void {
  expect(normalizeTranscriptForMatch(value)).toMatch(NODOASSIST_LIVE_TRANSCRIPT_MARKER_RE);
}
...
export async function runRealtimeSttLiveTest(params: {
  ...
  const errors: Error[] = [];
  const expected = params.expectedNormalizedText ?? NODOASSIST_LIVE_TRANSCRIPT_MARKER_RE;
