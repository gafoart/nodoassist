 ...
 */
export function resolveNodoAssistAgentDir(env: NodeJS.ProcessEnv = process.env): string {
  const override = env.NODOASSIST_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
