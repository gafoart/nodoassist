...
export { VERSION as NODOASSIST_VERSION } from "../version.js";
...
export { resolveNodoAssistAgentDir } from "./agent-dir-compat.js";
export {
  resolveAgentDir,
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  config?: import("../config/types.nodoassist.js").NodoAssistConfig;
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@nodoassist/media-core/constants"),
    ]);
 ...
 * nodoassist/plugin-sdk/session-transcript-runtime so transcript writes target a
 ...
 * nodoassist/plugin-sdk/session-transcript-runtime so transcript updates target
...
/**
 * Derive the same compact user-facing tool detail that embedded NodoAssist uses for progress logs.
 */
 ...
 * preserve the same NodoAssist-owned fallback signals as the built-in NodoAssist path
