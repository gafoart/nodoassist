...
const pluginSdkPackageNames = ["nodoassist/plugin-sdk", "@nodoassist/plugin-sdk"];
...
const DIAGNOSTIC_EVENTS_STATE_KEY = Symbol.for("nodoassist.diagnosticEvents.state.v1");
...
const shouldPreferSourceGraph =
  ...
  (process.env.NODE_ENV !== "production" ||
    ...
    process.env.NODOASSIST_PLUGIN_SDK_SOURCE_IN_TESTS === "1");
...
function listPluginSdkExportedSubpaths() {
  ...
  const cacheKey = `${packageRoot}::privateQa=${process.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI === "1" ? "1" : "0"}`;
...
function listPrivateLocalOnlyPluginSdkSubpaths() {
  if (process.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI !== "1") {
    return [];
  }
  try {
...
function getModuleLoader(tryNative) {
  ...
  const moduleLoader = createJiti(__filename, {
    ...
    // so local plugins do not create a second transpiled NodoAssist core graph.
    tryNative,
