/**
 * Shared realtime voice controls for active NodoAssist agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME = "nodoassist_agent_control";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal NodoAssist voice control result.",
    "Do not call nodoassist_agent_consult or any other tool for this message.",
    "Speak this exact NodoAssist status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "NodoAssist is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "NodoAssist has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "NodoAssist could not queue that follow-up."
    : "NodoAssist could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "NodoAssist finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `NodoAssist is starting ${name}.` : "NodoAssist is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `NodoAssist finished ${name} and is continuing.`
        : "NodoAssist finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `NodoAssist is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "NodoAssist is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `NodoAssist is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "NodoAssist is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "NodoAssist is working on the current voice request.";
  }
