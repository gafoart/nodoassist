...
describe("realtime voice activation names", () => {
  it("normalizes and validates one- or two-word activation names", () => {
    expect(normalizeSupportedRealtimeVoiceActivationName("  NodoAssist  ")).toBe("nodoassist");
    expect(normalizeSupportedRealtimeVoiceActivationName("Nodo Assist")).toBe("nodo assist");
  ...
  it("prefers longer activation names first", () => {
    expect(sortRealtimeVoiceActivationNames(["claw", "claw bot", "nodoassist"])).toEqual([
      "claw bot",
      "nodoassist",
      "claw",
    ]);
