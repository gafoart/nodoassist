...
declare const __NODOASSIST_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "nodoassist";
...
function readInjectedVersion(): string | undefined {
  return typeof __NODOASSIST_VERSION__ === "string" ? __NODOASSIST_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["NODOASSIST_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["NODOASSIST_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["NODOASSIST_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.NODOASSIST_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current NodoAssist version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.NODOASSIST_BUNDLED_VERSION,
});
