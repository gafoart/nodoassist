...
function listAgents(cfg: NodoAssistConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: NodoAssistConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<NodoAssistConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: NodoAssistConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: NodoAssistConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: NodoAssistConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<NodoAssistConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  NodoAssistConfig,
  {
    bindingsRef: NodoAssistConfig["bindings"];
    agentsRef: NodoAssistConfig["agents"];
    sessionRef: NodoAssistConfig["session"];
...
function resolveRouteCacheForConfig(cfg: NodoAssistConfig): Map<string, ResolvedAgentRoute> {
