...
function resetGatewayCallMocks() {
  ...
  connectError = null;
  const loadConfigForTests = getRuntimeConfig as unknown as () => NodoAssistConfig;
...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    deleteTestEnvValue("NODOASSIST_ALLOW_INSECURE_PRIVATE_WS");
  ...
  it("allows Tailscale-authenticated backend calls without client-side credentials", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        remote: { url: "wss://nodoassist.example.test" },
    ...
    expect(lastClientOptions?.url).toBe("wss://nodoassist.example.test");
    expect(lastClientOptions?.token).toBeUndefined();
  ...
  it("keeps CLI device identity when an ambient token is inactive under auth mode none", async () => {
    ...
    setGatewayNetworkDefaults();
    process.env.NODOASSIST_GATEWAY_TOKEN = "inactive-env-token";
  ...
  it("uses NODOASSIST_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.NODOASSIST_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.NODOASSIST_GATEWAY_TOKEN = "env-token";
  ...
  it("lets an explicit local port override bypass gateway env URL and port", async () => {
    ...
    resolveGatewayPort.mockImplementation((_config?: unknown, env?: unknown) => {
      ...
      return Number(candidateEnv?.NODOASSIST_GATEWAY_PORT ?? 18789);
    });
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.NODOASSIST_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.NODOASSIST_GATEWAY_PORT = "19001";
    process.env.NODOASSIST_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("lets probe details local port override bypass gateway env URL and port", async () => {
    ...
    testing.setDepsForTests({
      ...
      resolveGatewayPort: (_config?: unknown, env?: NodeJS.ProcessEnv) =>
        Number(env?.NODOASSIST_GATEWAY_PORT ?? 18789),
    });
    const prevUrl = process.env.NODOASSIST_GATEWAY_URL;
    const prevPort = process.env.NODOASSIST_GATEWAY_PORT;
    try {
      process.env.NODOASSIST_GATEWAY_URL = "wss://env-gateway.example/ws";
      process.env.NODOASSIST_GATEWAY_PORT = "19001";
      ...
      if (prevUrl === undefined) {
        delete process.env.NODOASSIST_GATEWAY_URL;
      } else {
        process.env.NODOASSIST_GATEWAY_URL = prevUrl;
      }
      if (prevPort === undefined) {
        delete process.env.NODOASSIST_GATEWAY_PORT;
      } else {
        process.env.NODOASSIST_GATEWAY_PORT = prevPort;
      }
    }
  });
  ...
  it("uses env NODOASSIST_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.NODOASSIST_GATEWAY_URL;
    try {
      process.env.NODOASSIST_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env NODOASSIST_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.NODOASSIST_GATEWAY_URL;
  ...
  it("falls back to the default config loader when test deps drift", () => {
    const tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-gateway-call-"));
    setTestEnvValue("NODOASSIST_STATE_DIR", tempStateDir);
    setTestEnvValue("NODOASSIST_CONFIG_PATH", path.join(tempStateDir, "missing-config.json"));
    try {
      getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
      resolveGatewayPort.mockReturnValue(18800);
      testing.setDepsForTests({
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("nodoassist doctor --fix");
  });
  ...
  it("allows ws:// hostname remote URLs when NODOASSIST_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.NODOASSIST_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
...
describe("callGateway error details", () => {
  ...
  it("does not over-claim a gateway crash on a 1006 abnormal close", async () => {
    ...
    expect(message).toContain("Run `nodoassist doctor`");
  });
  ...
  it("keeps the default wrapper timeout aligned with env handshake timeout", async () => {
    const envSnapshot = captureEnv(["NODOASSIST_HANDSHAKE_TIMEOUT_MS"]);
    try {
      process.env.NODOASSIST_HANDSHAKE_TIMEOUT_MS = "30000";
      startMode = "silent";
      setLocalLoopbackGatewayConfig();
  ...
  it("disables the request and wrapper deadline when timeout is null", async () => {
    ...
    testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => NodoAssistConfig,
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.NODOASSIST_GATEWAY_TOKEN = "env-token";
    process.env.NODOASSIST_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.NODOASSIST_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
    ...
    await expect(callGateway({ method: "health" })).rejects.toThrow(
      /NODOASSIST_GATEWAY_TOKEN or NODOASSIST_GATEWAY_PASSWORD/i,
    );
  });
});
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "NODOASSIST_GATEWAY_PASSWORD",
      envValue: "from-env",
    ...
    },
    {
      ...
      envKey: "NODOASSIST_GATEWAY_TOKEN",
      envValue: "env-token",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.NODOASSIST_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.NODOASSIST_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
