...
async function withGatewayChatHarness(
  ...
  const createSessionDir = async () => {
    const sessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gw-"));
  ...
  try {
    ...
    if (process.env.NODOASSIST_CONFIG_PATH) {
      await fs.rm(process.env.NODOASSIST_CONFIG_PATH, { force: true });
    }
    clearConfigCache();
...
function readNodoAssistSeq(message: unknown): number | undefined {
  if (!message || typeof message !== "object" || Array.isArray(message)) {
    return undefined;
  }
  const metadata = (message as Record<string, unknown>)["__nodoassist"];
...
test("chat.send replays a cached result after the session is archived", async () => {
  const sessionDir = autoCleanupTempDirs.make("nodoassist-gw-");
  try {
...
describe("gateway server chat", () => {
  ...
  test("chat.send starts the next WebChat turn after the prior internal run finishes", async () => {
    ...
    try {
      ...
      expect(dispatchOptions[0]?.promptCacheKey).toEqual(
        expect.stringMatching(/^nodoassist-webchat-[a-f0-9]{32}$/u),
      );
  ...
  test("chat.history offset pages overread context before filtering stale announce replies", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      const page = await rpcReq<{
        messages?: Array<{ __nodoassist?: { seq?: number } }>;
        nextOffset?: number;
        hasMore?: boolean;
      }>(ws, "chat.history", {
        sessionKey: "main",
  ...
  test("chat.send diagnostics timeline carries run correlation attributes", async () => {
    const timelineDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-chat-timeline-"));
  ...
  test("chat.history hard-caps single oversized nested payloads", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      expect(messages[0]).toMatchObject({
        __nodoassist: { id: "msg-huge", truncated: true, reason: "oversized" },
      });
  ...
  test("chat.history offset pagination advances from the projected first-page boundary", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      expect(firstPage.payload?.messages?.map(readNodoAssistSeq)).toEqual([3, 5]);
      ...
      expect(secondPage.payload?.messages?.map(readNodoAssistSeq)).toEqual([1, 2]);
  ...
  test("chat.history offset pagination advances from the final budgeted page", async () => {
    await withGatewayChatHarness(async ({ ws, createSessionDir }) => {
      ...
      expect(firstPage.ok).toBe(true);
      expect(firstPage.payload?.messages?.map(readNodoAssistSeq)).toEqual([2, 3]);
