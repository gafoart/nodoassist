...
// Emits a one-shot notice for ignored pre-NodoAssist environment prefixes.
...
export function warnLegacyNodoAssistEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but NodoAssist only reads NODOASSIST_* names now.`,
      "Rename them by replacing the legacy prefix with NODOASSIST_; the old names are ignored.",
    ].join("\n"),
    { code: "NODOASSIST_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

/** Resets the one-shot legacy env warning latch for tests. */
export function resetLegacyNodoAssistEnvWarningForTest(): void {
  warned = false;
}
