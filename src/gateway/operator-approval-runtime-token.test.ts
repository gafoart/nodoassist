...
const envSnapshot = captureEnv(["HOME", "NODOASSIST_HOME"]);

const tempHomes: string[] = [];

function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-approval-runtime-"));
  tempHomes.push(home);
...
function writeExecApprovalsToken(home: string, token: string): void {
  fs.mkdirSync(path.join(home, ".nodoassist"), { recursive: true });
  fs.writeFileSync(
