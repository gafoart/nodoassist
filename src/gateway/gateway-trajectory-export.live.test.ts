...
const CODEX_HARNESS_LIVE = process.env.NODOASSIST_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.NODOASSIST_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["NODOASSIST_TRAJECTORY", "NODOASSIST_TRAJECTORY_DIR"]);
}
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(
        path.join(process.cwd(), ".tmp-nodoassist-trajectory-live-"),
      );
      cleanup.push(async () => {
      ...
      process.env.NODOASSIST_TRAJECTORY = "1";
      process.env.NODOASSIST_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".nodoassist", "trajectory-exports", "bundle");
