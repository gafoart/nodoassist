...
import type { NodoAssistPluginNodeInvokePolicyContext } from "../plugins/types.js";
...
function createApprovalRequestPolicy(params?: {
  ...
  return createDemoPolicy(async (ctx: NodoAssistPluginNodeInvokePolicyContext) => {
...
describe("applyPluginNodeInvokePolicy", () => {
  ...
  it("uses a matching plugin policy when one is registered", async () => {
    setDangerousDemoCommandRegistry([
      createDemoPolicy((ctx: NodoAssistPluginNodeInvokePolicyContext) => ctx.invokeNode()),
    ]);
    const { context, invoke } = createContext();
