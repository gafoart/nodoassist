...
const WRITE_SCOPE_HEADER = { "x-nodoassist-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-nodoassist-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("nodoassist/default");
  ...
  it("rejects explicit unknown agent ids", async () => {
    try {
      ...
      const header = await postEmbeddings(
        { model: "nodoassist/default", input: "hello" },
        { "x-nodoassist-agent-id": "missing-agent" },
      );
      await expectInvalidEmbeddingRequest(header, "Unknown agent 'missing-agent'.");

      const model = await postEmbeddings({ model: "nodoassist/missing-agent", input: "hello" });
  ...
  it("rejects disallowed x-nodoassist-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "nodoassist/default",
        input: "hello",
      },
      { "x-nodoassist-model": "ollama/nomic-embed-text" },
    );
    await expectInvalidEmbeddingRequest(
      res,
  ...
  it("rejects x-nodoassist-model for trusted write-only callers", async () => {
