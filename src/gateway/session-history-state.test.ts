...
describe("SessionHistorySseState", () => {
  ...
  test("carries inline user idempotency keys into history metadata", () => {
    ...
    expect(
      (
        appended!.message as {
          __nodoassist?: { id?: string; idempotencyKey?: string; seq?: number };
        }
      )["__nodoassist"],
    ).toMatchObject({
      id: "message-user-2",
  ...
  test("reuses one canonical array for items and messages", () => {
    ...
    expect(snapshot.history.messages[0]?.["__nodoassist"]?.seq).toBe(2);
  ...
  test("uses carried sequence for inline SSE appends", () => {
    ...
    expect(state.snapshot().messages.at(-1)?.["__nodoassist"]?.seq).toBe(9);
  });
  ...
  test("emits message-tool mirror when silent control reply completes inline append", () => {
    ...
    expect(
      (
        appended?.message as {
          content?: Array<{ text?: string }>;
          nodoassistMessageToolMirror?: unknown;
        }
      )?.content?.[0]?.text,
    ).toBe("Still the current chat.");
    expect(
      Boolean(
        (appended?.message as { nodoassistMessageToolMirror?: unknown } | undefined)
  ...
  test("keeps cursors when a paginated history page starts with a message-tool mirror", () => {
    ...
    expect(snapshot.history.messages[0]?.["__nodoassist"]?.seq).toBe(3);
    expect(
  ...
  test("does not coerce partial cursor values", () => {
    ...
    expect(snapshot.history.messages.map((message) => message["__nodoassist"]?.seq)).toEqual([
      1, 2,
    ]);
  });
  ...
  test("requests refresh when inline TTS supplement merges into an existing assistant message", () => {
    ...
    const appended = state.appendInlineMessage({
      message: {
        ...
        nodoassistTtsSupplement: { textSha256, spokenText: visibleText },
      },
      messageSeq: 3,
    });
  ...
  test("requests refresh for non-monotonic carried inline sequence", () => {
    ...
    expect(appended).toEqual({ shouldRefresh: true });
    expect(state.snapshot().messages).toHaveLength(1);
    expect(state.snapshot().messages.at(-1)?.["__nodoassist"]?.seq).toBe(5);
  });
