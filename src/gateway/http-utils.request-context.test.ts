...
describe("resolveGatewayRequestContext", () => {
  it("uses normalized x-nodoassist-message-channel when enabled", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-nodoassist-message-channel": " Custom-Channel " }),
      model: "nodoassist",
  ...
  it("preserves normal explicit session-key overrides", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-nodoassist-session-key": "customer-case-42" }),
      model: "nodoassist",
      sessionPrefix: "openai",
      defaultMessageChannel: "webchat",
    });
  ...
  it.each([
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-nodoassist-session-key": sessionKey }),
        model: "nodoassist",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
  ...
  it("does not build session state for explicit unknown agent ids", () => {
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-nodoassist-agent-id": "missing-agent" }),
        model: "nodoassist",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq(),
        model: "nodoassist/missing-agent",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
    ).toThrow(/Unknown agent/);
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-nodoassist-agent-id": "!!!" }),
        model: "nodoassist",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
...
describe("resolveHttpSenderIsOwner", () => {
  it("requires operator.admin on a trusted HTTP scope-bearing request", () => {
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-nodoassist-scopes": "operator.admin" }), noneAuth),
    ).toBe(true);
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-nodoassist-scopes": "operator.write" }), noneAuth),
    ).toBe(false);
  });
  ...
  it("returns false for bearer requests even with operator.admin in headers", () => {
    expect(
      resolveHttpSenderIsOwner(
        createReq({
          authorization: "Bearer secret",
          "x-nodoassist-scopes": "operator.admin",
        }),
        tokenAuth,
      ),
    ).toBe(false);
...
describe("resolveOpenAiCompatibleHttpSenderIsOwner", () => {
  ...
  it("still requires operator.admin for trusted scope-bearing requests", () => {
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-nodoassist-scopes": "operator.write" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(false);
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-nodoassist-scopes": "operator.admin" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(true);
  });
...
describe("authorizeOpenAiCompatibleHttpModelOverride", () => {
  it("allows shared-secret bearer callers to use x-nodoassist-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({ authorization: "Bearer secret", "x-nodoassist-model": "openai/gpt-5.4" }),
        { authMethod: "token", trustDeclaredOperatorScopes: false },
      ),
    ).toEqual({ allowed: true });
  });

  it("allows trusted admin callers to use x-nodoassist-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-nodoassist-scopes": "operator.admin, operator.write",
  ...
  it("rejects trusted write-only callers that try to use x-nodoassist-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-nodoassist-scopes": "operator.write",
