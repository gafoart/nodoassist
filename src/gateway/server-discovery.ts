...
/** Formats the Bonjour instance name while preserving user-provided NodoAssist names. */
export function formatBonjourInstanceName(displayName: string) {
  const trimmed = displayName.trim();
  if (!trimmed) {
    return "NodoAssist";
  }
  if (/nodoassist/i.test(trimmed)) {
    return trimmed;
  }
  return `${trimmed} (NodoAssist)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const envPath = env.NODOASSIST_CLI_PATH?.trim();
  if (envPath) {
    return envPath;
  }
  ...
  const binCli = path.join(cwd, "bin", "nodoassist");
  if (isFile(binCli)) {
    return binCli;
  }
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.NODOASSIST_TAILNET_DNS?.trim();
