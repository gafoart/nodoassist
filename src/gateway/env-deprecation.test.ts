...
import {
  resetLegacyNodoAssistEnvWarningForTest,
  warnLegacyNodoAssistEnvVars,
} from "./env-deprecation.js";

describe("warnLegacyNodoAssistEnvVars", () => {
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("replacing the legacy prefix with NODOASSIST_");
    ...
    expect(options).toEqual({
      code: "NODOASSIST_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current NODOASSIST names", () => {
    warnLegacyNodoAssistEnvVars({ NODOASSIST_GATEWAY_TOKEN: "token" });

    expect(emitWarning).not.toHaveBeenCalled();
  });

  it("warns only once after a successful emit", () => {
    warnLegacyNodoAssistEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
    warnLegacyNodoAssistEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacyNodoAssistEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacyNodoAssistEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
