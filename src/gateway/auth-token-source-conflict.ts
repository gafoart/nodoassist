...
const GATEWAY_ENV_TOKEN = "NODOASSIST_GATEWAY_TOKEN";
const GATEWAY_SERVICE_KIND = "gateway";

// Doctor/startup warning shape for shells where NODOASSIST_GATEWAY_TOKEN would
...
export function resolveGatewayAuthTokenSourceConflict(params: {
  ...
  const envToken = normalizeOptionalString(params.env.NODOASSIST_GATEWAY_TOKEN);
  if (!envToken) {
    return null;
  }

  if (params.env.NODOASSIST_SERVICE_KIND?.trim() === GATEWAY_SERVICE_KIND) {
  ...
  const remediation =
    `Remove ${GATEWAY_ENV_TOKEN} from the shell, ~/.nodoassist/.env, or launchctl env if gateway.auth.token is intended, ` +
