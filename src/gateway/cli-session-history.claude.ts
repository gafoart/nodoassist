...
// Converts Claude project JSONL into NodoAssist transcript-compatible messages.
...
import { attachNodoAssistTranscriptMeta } from "./session-transcript-readers.js";
...
function normalizeClaudeCliContent(
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      // Claude stores tool calls as `tool_use` with `input`; NodoAssist history
...
function parseClaudeCliHistoryEntry(
  ...
  if (type === "user") {
    ...
    return attachNodoAssistTranscriptMeta(
      {
        role: "user",
        content,
