...
function configWithDefaultEnvProvider(gateway: NonNullable<NodoAssistConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: NodoAssistConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: NodoAssistConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("NODOASSIST_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        NODOASSIST_GATEWAY_TOKEN: "test-token-from-env",
