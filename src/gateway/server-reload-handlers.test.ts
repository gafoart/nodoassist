...
const hoisted = vi.hoisted(() => ({
  ...
  runtimeConfig: { value: { session: { store: "/tmp/active-sessions.json" } } as NodoAssistConfig },
  reloadEvents: [] as string[],
  loadModelCatalog: vi.fn(async (_params: { config: NodoAssistConfig }) => []),
  resetModelCatalogCache: vi.fn(() => {}),
  refreshContextWindowCache: vi.fn(async (_cfg: NodoAssistConfig) => {}),
  clearCurrentProviderAuthState: vi.fn(() => {}),
  warmCurrentProviderAuthStateOffMainThread: vi.fn(async (_cfg: NodoAssistConfig) => {}),
...
vi.mock("../agents/model-catalog.js", () => ({
  loadModelCatalog: (params: { config: NodoAssistConfig }) => {
...
vi.mock("../agents/context.js", () => ({
  refreshContextWindowCache: async (cfg: NodoAssistConfig) => {
...
vi.mock("../agents/model-provider-auth.js", () => ({
  ...
  warmCurrentProviderAuthStateOffMainThread: async (cfg: NodoAssistConfig) => {
...
// set NODOASSIST_SKIP_CHANNELS / NODOASSIST_SKIP_PROVIDERS at module load. When a
...
describe("gateway hot reload model state", () => {
  ...
  it("resets prepared model runtime state for every hot reload and rewarms after plugin reload", async () => {
    ...
    const nextConfig = { plugins: { enabled: true } } as NodoAssistConfig;
    await applyHotReload(
      {
  ...
  it("disposes cached MCP runtimes on MCP config hot reloads", async () => {
    ...
    const nextConfig = { mcp: { servers: {} } } as NodoAssistConfig;
  ...
  it.each([
    ...
    const { applyHotReload } = createReloadHandlersForTest();
    const previousConfig = testCase.previousConfig as NodoAssistConfig;
    const nextConfig = testCase.nextConfig as NodoAssistConfig;
...
describe("gateway Gmail hot reload handlers", () => {
  ...
  function createGmailConfig(account: string): NodoAssistConfig {
    return {
  ...
  it("commits runtime secrets for managed no-op config reloads", async () => {
    ...
    const initialConfig: NodoAssistConfig = {
      gateway: { reload: { debounceMs: 0 } },
    ...
    const activateRuntimeSecrets = vi.fn(async (config: NodoAssistConfig) => ({
      sourceConfig: config,
      config,
      authStores: [],
      warnings: [],
  ...
  it("aborts an in-flight managed Gmail restart when the reloader stops", async () => {
    ...
    const reloader = startManagedGatewayConfigReloader({
      ...
      logReload: { info: vi.fn(), warn: vi.fn(), error: vi.fn() },
      channelManager: {} as never,
      activateRuntimeSecrets: vi.fn(async (config: NodoAssistConfig) => ({
        sourceConfig: config,
        config,
        authStores: [],
        warnings: [],
