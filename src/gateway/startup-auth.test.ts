...
const mocks = vi.hoisted(() => ({
  replaceConfigFile: vi.fn(async (_params: { nextConfig: NodoAssistConfig }) => {}),
}));
...
type GatewayAuthConfig = NonNullable<NonNullable<NodoAssistConfig["gateway"]>["auth"]>;
...
function gatewayAuthConfig(auth: GatewayAuthConfig): NodoAssistConfig {
  return {
    gateway: { auth },
  };
}

function gatewayAuthConfigWithDefaultEnvProvider(auth: GatewayAuthConfig): NodoAssistConfig {
  return {
...
describe("mergeGatewayTailscaleConfig", () => {
  ...
  it("preserves explicit serviceName overrides", () => {
    expect(
      mergeGatewayTailscaleConfig(
        { mode: "serve", serviceName: "svc:old-nodoassist", resetOnExit: false },
        { serviceName: "svc:nodoassist" },
      ),
    ).toEqual({ mode: "serve", serviceName: "svc:nodoassist", resetOnExit: false });
  });
});
...
describe("ensureGatewayStartupAuth", () => {
  ...
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: NodoAssistConfig) {
  ...
  async function expectNoTokenGeneration(cfg: NodoAssistConfig, mode: string) {
    const result = await runStartupAuth({
      cfg,
      persist: true,
    });
  ...
  function createMissingGatewayTokenSecretRefConfig(): NodoAssistConfig {
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    await expectResolvedToken({
      cfg: gatewayAuthConfig({ mode: "token", token: "${NODOASSIST_GATEWAY_TOKEN}" }),
      env: {
        NODOASSIST_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${NODOASSIST_GATEWAY_TOKEN}",
    });
  });

  it("uses NODOASSIST_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        NODOASSIST_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
  ...
  it("uses NODOASSIST_GATEWAY_PASSWORD without resolving configured password SecretRef", async () => {
    const result = await runStartupAuth({
      cfg: gatewayAuthConfigWithDefaultEnvProvider({
        mode: "password",
      ...
      env: {
        NODOASSIST_GATEWAY_PASSWORD: "password-from-env", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      persist: true,
    });
  ...
  it("keeps startup non-breaking when hooks token reuses gateway token resolved from env", async () => {
    ...
    const result = await runStartupAuth({
      ...
      env: {
        NODOASSIST_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
      } as NodeJS.ProcessEnv,
      warn,
    });
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("nodoassist security audit"));
  });
