...
describe("gateway server models + voicewake", () => {
  ...
  const withTempHome = async <T>(fn: (homeDir: string) => Promise<T>): Promise<T> => {
    const tempHome = await createTempHomeEnv("nodoassist-home-");
    try {
  ...
  test(
    ...
    async () => {
      await withTempHome(async (homeDir) => {
        ...
        expect(initial.payload?.triggers).toEqual(["nodoassist", "claude", "computer"]);
        ...
        await expect(
          fs.readFile(path.join(homeDir, ".nodoassist", "settings", "voicewake.json"), "utf8"),
  ...
  test("voicewake.routing.get/set persists and broadcasts", { timeout: 60_000 }, async () => {
    await withTempHome(async (homeDir) => {
      ...
      await expect(
        fs.readFile(
          path.join(homeDir, ".nodoassist", "settings", "voicewake-routing.json"),
          "utf8",
        ),
      ).rejects.toThrow(/ENOENT/u);
