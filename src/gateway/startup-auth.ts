...
const HOOKS_GATEWAY_AUTH_REUSE_WARNING =
  "Security warning: hooks.token matches active Gateway shared-secret auth. Startup continues for compatibility; rotate hooks.token or Gateway auth. Run nodoassist security audit for a full report, and run nodoassist doctor --fix when the reused hooks.token is persisted in config.";
...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken = trimToUndefined(params.env.NODOASSIST_GATEWAY_TOKEN);
  if (envToken) {
    return true;
  }
  if (
