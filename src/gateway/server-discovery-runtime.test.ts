...
const mocks = vi.hoisted(() => ({
  ...
  writeWideAreaGatewayZone: vi.fn<WriteWideAreaGatewayZone>(async () => ({
    changed: true,
    zonePath: "/tmp/nodoassist.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (NodoAssist)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/nodoassist"),
...
async function expectSshPortOmitted(rawPort: string) {
  ...
  process.env.NODOASSIST_SSH_PORT = rawPort;
...
function startStuckDiscovery(timeoutMs: string) {
  vi.useFakeTimers();
  useDevelopmentDiscoveryEnv();
  process.env.NODOASSIST_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS = timeoutMs;
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.NODOASSIST_SSH_PORT = "2222";
    ...
    expect(bonjour.service.advertise).toHaveBeenCalledWith({
      ...
      cliPath: "/usr/local/bin/nodoassist",
      minimal: false,
    });
  ...
  it("skips local discovery services for truthy NODOASSIST_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.NODOASSIST_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "nodoassist.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
    ...
    expect(zoneParams.domain).toBe("nodoassist.internal.");
    expect(zoneParams.gatewayPort).toBe(18789);
    expect(zoneParams.gatewayDirectReachable).toBe(true);
    expect(zoneParams.displayName).toBe("Lab Mac (NodoAssist)");
    ...
    expect(logs.info.mock.calls).toEqual([
      ["wide-area DNS-SD updated (nodoassist.internal. → /tmp/nodoassist.internal.db)"],
    ]);
    expect(result.bonjourStop).toBeNull();
  });
