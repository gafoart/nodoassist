...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "nodoassist-test-owner-display-secret",
    },
  };
}

/** Writes a temp NodoAssist config, installs it as runtime state, then restores globals. */
export async function withTempConfig(params: {
  cfg: unknown;
  run: () => Promise<void>;
  prefix?: string;
}): Promise<void> {
  const prevConfigPath = process.env.NODOASSIST_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as NodoAssistConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "nodoassist-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.NODOASSIST_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
