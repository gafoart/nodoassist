 ...
 *
 * Implements the OpenResponses `/v1/responses` endpoint for NodoAssist Gateway.
 *
...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    requestedSessionKey: getHeader(params.req, "x-nodoassist-session-key"),
  });
}
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText =
          accumulatedText || bufferedReplaceableAssistantContent || "No response from NodoAssist.";
