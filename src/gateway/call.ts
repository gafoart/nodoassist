...
import { isLoopbackIpAddress } from "@nodoassist/net-policy/ip";
...
type CallGatewayBaseOptions = {
   ...
   * Bypasses NODOASSIST_GATEWAY_URL and NODOASSIST_GATEWAY_PORT for this call only.
   */
...
type GatewayRuntimeConfigLoader = () => NodoAssistConfig | Promise<NodoAssistConfig>;
const defaultGetRuntimeConfig = async (): Promise<NodoAssistConfig> =>
...
async function loadGatewayConfig(): Promise<NodoAssistConfig> {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: NodoAssistConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: NodoAssistConfig) {
...
export function ensureExplicitGatewayAuth(params: {
  ...
  const sourceHint =
    params.urlOverrideSource === "env"
      ? "Set NODOASSIST_GATEWAY_TOKEN or NODOASSIST_GATEWAY_PASSWORD alongside NODOASSIST_GATEWAY_URL; config credentials are intentionally not reused."
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.NODOASSIST_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.NODOASSIST_TEST_HANDSHAKE_TIMEOUT_MS);
...
async function resolveGatewayCallContext(
  ...
  const envUrlOverride =
    cliUrlOverride || opts.localPortOverride !== undefined
      ? undefined
      : trimToUndefined(process.env.NODOASSIST_GATEWAY_URL);
  ...
  const config =
    opts.config ?? (canSkipConfigLoad ? ({} as NodoAssistConfig) : await loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `nodoassist doctor` for diagnostics.";
  }
  return message;
}
