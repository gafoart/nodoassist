...
const gatewayEnv = {
  NODOASSIST_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  NODOASSIST_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
} as NodeJS.ProcessEnv;

function makeRemoteGatewayConfig(remote: { token?: string; password?: string }): NodoAssistConfig {
  return {
    gateway: {
      mode: "remote",
      remote,
...
function withGatewayAuthEnv<T>(env: NodeJS.ProcessEnv, fn: () => T): T {
  return withEnv(
    {
      NODOASSIST_GATEWAY_TOKEN: env.NODOASSIST_GATEWAY_TOKEN,
      NODOASSIST_GATEWAY_PASSWORD: env.NODOASSIST_GATEWAY_PASSWORD,
      NODOASSIST_SERVICE_KIND: env.NODOASSIST_SERVICE_KIND,
    },
    fn,
  );
}
