...
const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: NodoAssistConfig }) => ({
...
vi.mock("../config/paths.js", () => ({
  ...
  resolveStateDir: vi.fn(() => "/tmp/nodoassist-state"),
}));

vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: NodoAssistConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: NodoAssistConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/nodoassist-startup-recovery.json";
...
function buildRuntimeSnapshot(
  ...
  runtimeConfig: NodoAssistConfig = sourceConfig,
): ConfigFileSnapshot {
  return {
...
function expectPluginAutoEnableFor(config: NodoAssistConfig) {
...
function mockRuntimeAutoEnable(config: NodoAssistConfig) {
...
function buildInvalidConfigSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: false,
    config: params.config ?? (params.rawConfig as NodoAssistConfig),
...
describe("gateway startup config validation", () => {
  ...
  it("does not suggest doctor repair for plugin packaging compiled-output failures", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as NodoAssistConfig,
      issues: [
        {
    ...
    await start.catch((error: unknown) => {
      expect(String(error)).not.toContain("nodoassist doctor --fix");
    });
  });
  ...
  it("keeps doctor repair guidance for mixed plugin packaging and core invalidity", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as unknown as NodoAssistConfig,
      issues: [
        {
          path: "plugins.slots.memory",
          message: "plugin not found: source-only-pack",
    ...
    await expectStartupRejects('Run "nodoassist doctor --fix" to repair, then retry.');
  });
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires NodoAssist >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
    });
    vi.mocked(configIo.readConfigFileSnapshot).mockResolvedValueOnce(invalidSnapshot);
