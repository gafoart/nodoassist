...
function readMessageSenderIsOwner(message: unknown): boolean | undefined {
  ...
  const nodoassist = (message as Record<string, unknown>)["__nodoassist"];
  if (!nodoassist || typeof nodoassist !== "object" || Array.isArray(nodoassist)) {
    return undefined;
  }
  const value = (nodoassist as Record<string, unknown>).senderIsOwner;
...
async function handleTranscriptUpdateBroadcast(
  ...
  const rawMessage = attachNodoAssistTranscriptMeta(update.message, {
