...
export const NODOASSIST_MODEL_ID = "nodoassist";
/** Default OpenAI-compatible model alias that targets the default NodoAssist agent. */
export const NODOASSIST_DEFAULT_MODEL_ID = "nodoassist/default";
...
export class GatewaySessionKeyOverrideError extends Error {
  constructor() {
    super("`x-nodoassist-session-key` cannot use reserved internal session namespaces.");
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-nodoassist-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-nodoassist-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === NODOASSIST_MODEL_ID || lowered === NODOASSIST_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^nodoassist[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Validates and resolves the `x-nodoassist-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `nodoassist` or `nodoassist/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-nodoassist-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-nodoassist-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-nodoassist-session-key")?.trim();
  if (explicit) {
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-nodoassist-message-channel")) ??
