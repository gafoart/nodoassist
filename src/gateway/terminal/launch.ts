...
type TerminalLaunchPolicy = {
  ...
  isEnabled: () => boolean;
  prepareConfig: (config: NodoAssistConfig, options: { restartPending: boolean }) => void;
  commitConfig: () => void;
};
...
export function createTerminalLaunchPolicy(initialConfig: NodoAssistConfig): TerminalLaunchPolicy {
  let activeConfig = initialConfig;
  let hasPendingRestart = false;
  let terminalDisabledUntilRestart = false;
  let preparedConfig: NodoAssistConfig | null = null;
  ...
  const preserveTerminalConfig = (config: NodoAssistConfig, owner: NodoAssistConfig) => {
  ...
  const resolveForConfig = (config: NodoAssistConfig, agentId?: string) => {
  ...
  const accumulateRestartRestrictions = (config: NodoAssistConfig) => {
  ...
  const accumulateCommitRestrictions = (config: NodoAssistConfig) => {
    if (config.gateway?.terminal?.enabled !== true) {
...
export function buildTerminalEnv(baseEnv: NodeJS.ProcessEnv): Record<string, string> {
  ...
  // Lets shells and prompts detect that they are inside an NodoAssist terminal.
  env.NODOASSIST_TERMINAL = "1";
  return env;
}
