...
async function writeConfig(config: NodoAssistConfig): Promise<void> {
...
describe("gateway startup web fetch config", () => {
  ...
  beforeAll(async () => {
    previousMinimal = process.env.NODOASSIST_TEST_MINIMAL_GATEWAY;
    process.env.NODOASSIST_TEST_MINIMAL_GATEWAY = "0";
    await writeConfig({
      gateway: {
        mode: "local",
        bind: "loopback",
  ...
  afterAll(async () => {
    ...
    if (previousMinimal === undefined) {
      ...
      process.env.NODOASSIST_TEST_MINIMAL_GATEWAY = previousMinimal;
    }
  });
