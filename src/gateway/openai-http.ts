...
// Translates OpenAI chat requests to NodoAssist agent runs and SSE/JSON responses.
...
function resolveAgentResponseText(result: unknown): string {
  ...
  if (!Array.isArray(payloads) || payloads.length === 0) {
    return "No response from NodoAssist.";
  }
  const content = payloads
    .map((p) => (typeof p.text === "string" ? p.text : ""))
    .filter(Boolean)
    .join("\n\n");
  return content || "No response from NodoAssist.";
}
...
export async function handleOpenAiHttpRequest(
  ...
  const model = typeof payload.model === "string" ? payload.model : "nodoassist";
