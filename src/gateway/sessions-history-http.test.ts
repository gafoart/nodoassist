...
const READ_SCOPE_HEADER = { "x-nodoassist-scopes": "operator.read" };
...
async function createSessionStoreFile(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-session-history-"));
...
type SessionHistoryMessage = {
  ...
  __nodoassist?: { id?: string; seq?: number };
};
...
function expectNodoAssistMetadata(
...
async function expectMessageEventMatch(
  ...
  if (params.id !== undefined) {
    expectNodoAssistMetadata(
      (event.data as { message?: { __nodoassist?: { id?: string; seq?: number } } }).message?.[
        "__nodoassist"
      ],
      {
        id: params.id,
...
describe("session history HTTP endpoints", () => {
  test("returns session history over direct REST", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectNodoAssistMetadata(body.messages?.[0]?.["__nodoassist"], {
        seq: 1,
      });
    });
  });
  ...
  test("supports cursor pagination over direct REST while preserving the messages field", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expect(firstBody.messages?.map((message) => message["__nodoassist"]?.seq)).toEqual([2, 3]);
      ...
      expect(secondBody.messages?.map((message) => message["__nodoassist"]?.seq)).toEqual([1]);
  ...
  test("streams bounded history windows over SSE", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      expectNodoAssistMetadata(nextData.messages?.[0]?.["__nodoassist"], {
        id: thirdMessageId,
        seq: 3,
      });
  ...
  test("seeds bounded SSE windows from visible history when transcript refreshes are silent", async () => {
    ...
    await withGatewayHarness(async (harness) => {
      ...
      const refreshData = refreshEvent.data as {
        messages?: Array<{ content?: Array<{ text?: string }>; __nodoassist?: { seq?: number } }>;
      };
      expect(refreshData.messages?.[0]?.content?.[0]?.text).toBe("second message");
      expect(refreshData.messages?.[0]?.["__nodoassist"]?.seq).toBe(2);
