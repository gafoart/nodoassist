...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.NODOASSIST_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.NODOASSIST_DISABLE_BONJOUR) &&
  ...
  const sshPort = mdnsMinimal
    ? undefined
    : (parseTcpPort(process.env.NODOASSIST_SSH_PORT) ?? undefined);
