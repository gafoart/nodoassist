...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: NodoAssistConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride || options.localPortOverride !== undefined
      ? undefined
      : normalizeOptionalString(process.env.NODOASSIST_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.NODOASSIST_ALLOW_INSECURE_PRIVATE_WS === "1";
