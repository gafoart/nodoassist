...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "NODOASSIST_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.NODOASSIST_CONFIG_PATH) {
    configPaths.add(process.env.NODOASSIST_CONFIG_PATH);
  }
  if (process.env.NODOASSIST_STATE_DIR) {
    configPaths.add(path.join(process.env.NODOASSIST_STATE_DIR, "nodoassist.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.NODOASSIST_STATE_DIR = path.join(tempHome, ".nodoassist");
  delete process.env.NODOASSIST_CONFIG_PATH;
...
function applyGatewaySkipEnv() {
  ...
  process.env.NODOASSIST_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "nodoassist-test-no-bundled-extensions")
    : "nodoassist-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".nodoassist-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".nodoassist-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".nodoassist-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>nodoassist-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__nodoassistTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__nodoassistTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["NODOASSIST_GATEWAY_TOKEN"]);
  const prev = process.env.NODOASSIST_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.NODOASSIST_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.NODOASSIST_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
