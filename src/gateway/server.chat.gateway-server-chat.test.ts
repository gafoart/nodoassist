...
describe("gateway server chat", () => {
  ...
  const withMainSessionStore = async <T>(
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gw-"));
    try {
  ...
  test("sessions.send accepts dashboard messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sessions-send-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.steer accepts dashboard follow-up messages for existing sessions", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sessions-steer-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort stops active dashboard runs", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sessions-abort-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("sessions.abort resolves active runs by runId without a caller session key", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-sessions-abort-runid-"));
    testState.sessionStorePath = path.join(dir, "sessions.json");
    try {
      await writeSessionStore({
        entries: {
  ...
  test("handles chat send and history flows", async () => {
    ...
    try {
      ...
      const sendPolicyDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gw-"));
      ...
      const agentBlockedDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gw-"));
      ...
      const historyDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gw-"));
  ...
  test("chat.history mirrors current-session message tool sends before NO_REPLY", async () => {
    ...
    expect(
      historyMessages.some((message) => {
        ...
        const entry = message as { role?: unknown; nodoassistMessageToolMirror?: unknown };
        return entry.role === "assistant" && Boolean(entry.nodoassistMessageToolMirror);
      }),
    ).toBe(true);
  });
    ...
    expect(
      historyMessages.some(
        (message) =>
          ...
          Boolean(
            (message as { nodoassistMessageToolMirror?: unknown }).nodoassistMessageToolMirror,
          ),
      ),
    ).toBe(true);
    expect(historyMessages).not.toContainEqual(
      expect.objectContaining({ provider: "nodoassist", model: "delivery-mirror" }),
    );
  });
