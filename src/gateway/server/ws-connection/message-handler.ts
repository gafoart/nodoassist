...
/**
 * Lazily resolve the local node host's nodeId from ~/.nodoassist/node.json.
 * Process-stable: only changes on `nodoassist node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-nodoassist-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.nodoassist/node.json nodeId). SSH-tunneled remote
