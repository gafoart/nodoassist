...
vi.mock("../agents/agent-scope.js", () => ({
  listAgentEntries: (cfg: NodoAssistConfig) => cfg.agents?.list ?? [],
  listAgentIds: (cfg: NodoAssistConfig) => cfg.agents?.list?.map((entry) => entry.id) ?? ["main"],
  resolveDefaultAgentId: (cfg: NodoAssistConfig) =>
    cfg.agents?.list?.find((entry) => entry.default)?.id ?? "main",
}));

vi.mock("../agents/memory-search.js", () => ({
  resolveMemorySearchConfig: (cfg: NodoAssistConfig, agentId: string) => {
...
async function startMemoryBackendForTest(cfg: NodoAssistConfig) {
  const log = createGatewayLogMock();
  await startGatewayMemoryBackend({ cfg, log });
  return log;
}

async function startQmdBackendWithManager(cfg: NodoAssistConfig) {
...
function expectQmdManagerRequests(cfg: NodoAssistConfig, agentIds: string[]) {
