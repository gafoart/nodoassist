...
type ConnectionAuthCase = {
  name: string;
  cfgLocal: NodoAssistConfig;
  env: NodeJS.ProcessEnv;
  options?: Partial<Omit<GatewayConnectionAuthOptions, "config" | "env">>;
  expected: ResolvedAuth;
};

function cfg(input: Partial<NodoAssistConfig>): NodoAssistConfig {
  return input as NodoAssistConfig;
}
...
const DEFAULT_ENV = {
  ...
  NODOASSIST_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when NODOASSIST env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when NODOASSIST env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when NODOASSIST env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
