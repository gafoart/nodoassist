...
import type { loadNodoAssistPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "NODOASSIST_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: NodoAssistConfig): GatewayMemoryStartupPolicy {
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadNodoAssistPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { normalizeProviderId } = await import("@nodoassist/model-catalog-core/provider-id");
  if (
  ...
  const { ensureNodoAssistModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureNodoAssistModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: NodoAssistConfig;
  pluginRegistry: ReturnType<typeof loadNodoAssistPlugins>;
      ...
      await measureStartup(params.startupTrace, "sidecars.acp.identity-reconcile", async () => {
        const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] =
          await Promise.all([
            ...
            import("@nodoassist/acp-core/runtime/session-identifiers"),
          ]);
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: NodoAssistConfig;
