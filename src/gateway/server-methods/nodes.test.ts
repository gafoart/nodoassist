...
const createdStates: NodoAssistTestState[] = [];

async function createState(label: string): Promise<NodoAssistTestState> {
  const state = await createNodoAssistTestState({ label, layout: "state-only" });
