...
function requestModelsList(params: {
  ...
  const request = modelsHandlers["models.list"]({
    ...
    context: {
      getRuntimeConfig: () => params.runtimeConfig ?? ({} as NodoAssistConfig),
...
describe("models.list", () => {
  ...
  it("marks legacy OpenAI Codex aliases available through ChatGPT OAuth", async () => {
    await withNodoAssistTestState(
      {
        layout: "state-only",
        prefix: "nodoassist-models-list-codex-alias-",
        agentEnv: "main",
      },
      async (state) => {
  ...
  it("marks catalog models available through their configured CLI runtime", async () => {
    await withEnvAsync({ ANTHROPIC_API_KEY: undefined }, async () => {
      await withNodoAssistTestState(
        {
          layout: "state-only",
          prefix: "nodoassist-models-list-cli-runtime-",
          agentEnv: "main",
        },
        async (state) => {
          await state.writeAuthProfiles({
  ...
  it("does not mark catalog rows available from expired OAuth profiles", async () => {
    await withNodoAssistTestState(
      {
        layout: "state-only",
        prefix: "nodoassist-models-list-expired-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles(
  ...
  it("uses refreshed persisted OAuth when the runtime auth snapshot is stale", async () => {
    await withNodoAssistTestState(
      {
        layout: "state-only",
        prefix: "nodoassist-models-list-stale-runtime-profile-",
        agentEnv: "main",
      },
      async (state) => {
  ...
  it("marks env SecretRef-backed auth profiles available", async () => {
    await withNodoAssistTestState(
      {
        layout: "state-only",
        prefix: "nodoassist-models-list-env-profile-",
        agentEnv: "main",
        env: {
  ...
  it("keeps non-env SecretRef-backed auth profile availability unknown", async () => {
    await withNodoAssistTestState(
      {
        layout: "state-only",
        prefix: "nodoassist-models-list-file-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles({
  ...
  it("marks auth profiles available even when provider config uses non-env SecretRef markers", async () => {
    for (const fixture of [
      ...
      await withNodoAssistTestState(
        {
          layout: "state-only",
          prefix: `nodoassist-models-list-provider-${fixture.name}-profile-`,
          agentEnv: "main",
          env: {
            NODOASSIST_TEST_PROFILE_API_KEY: "test-token",
            ...
            profiles: {
              "vllm:env": {
                ...
                keyRef: {
                  source: "env",
                  provider: "default",
                  id: "NODOASSIST_TEST_PROFILE_API_KEY",
                },
              },
            },
          });
