...
import { isAudioFileName } from "@nodoassist/media-core/mime";
...
function resolveWebchatPromptCacheKey(params: {
  ...
  const digest = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 32);
  return `nodoassist-webchat-${digest}`;
}
...
function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  const rawMetadata =
    message && typeof message === "object"
      ? (message as Record<string, unknown>)["__nodoassist"]
      : undefined;
  const metadata =
...
function findSourceReplyTranscriptMirrorByIdempotencyKeyInEvents(
  ...
  if (found?.message.provider !== "nodoassist" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
...
function sendGlobalAwareNodeChatPayload(params: {
  ...
  const deliveryKeys = resolveGlobalAwareNodeChatDeliveryKeys({
    cfg: params.context.getRuntimeConfig?.() ?? ({} as NodoAssistConfig),
    sessionKey: params.sessionKey,
    agentId: params.agentId,
  });
...
function readChatHistoryMessageId(message: unknown): string | undefined {
  const metadata = asOptionalRecord(asOptionalRecord(message)?.["__nodoassist"]);
...
async function handleChatHistoryRequest({
  ...
  const requestedAgentId = resolveRequestedChatAgentId({
    cfg: (context as { getRuntimeConfig?: () => NodoAssistConfig }).getRuntimeConfig?.(),
...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    const sessionRoutingChanged = (candidateConfig: NodoAssistConfig) =>
      ...
      try {
        await measureDiagnosticsTimelineSpan(
          ...
          async () => {
            ...
            // attachments even when the parent NodoAssist session model is text-only.
        ...
        .then(async () => {
          ...
          await measureDiagnosticsTimelineSpan(
            ...
            async () => {
              ...
              // through NodoAssist runtime's SessionManager; this dispatcher only owns live delivery payloads.
                  ...
                  if (shouldAppendAssistantTranscript) {
                    ...
                    if (appended.ok) {
                      ...
                      message = {
                        ...
                        timestamp: nowValue,
                        ...(ttsSupplementMarker
                          ? { nodoassistTtsSupplement: ttsSupplementMarker }
                          : {}),
