...
vi.mock("../../skills/security/clawhub-verdicts.js", () => ({
  ...
  fetchNodoAssistSkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    testState = await createNodoAssistTestState({
      layout: "state-only",
      prefix: "nodoassist-skills-proposals-gateway-state-",
    });
    ...
    mocks.workspaceDir = await tempDirs.make("nodoassist-skills-proposals-gateway-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("nodoassist-skills-proposals-gateway-second-");
