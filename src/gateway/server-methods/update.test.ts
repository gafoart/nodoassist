...
const startManagedServiceUpdateHandoffMock = vi.fn(async () => ({
  ...
  pid: 12345,
  command: "nodoassist update --yes --timeout 1800",
  logPath: "/tmp/nodoassist-update-run-handoff/handoff.log",
}));
...
vi.mock("../../infra/nodoassist-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/nodoassist-root.js")>(
    "../../infra/nodoassist-root.js",
  );
  return {
    ...actual,
    resolveNodoAssistPackageRoot: async () => "/tmp/nodoassist",
  };
});
...
vi.mock("../../infra/update-managed-service-handoff.js", () => ({
  ...
  formatManagedServiceUpdateCommand: (params?: { timeoutMs?: number; channel?: UpdateChannel }) => {
    const args = ["nodoassist", "update", "--yes"];
    if (params?.channel) {
  ...
  buildManagedServiceHandoffUnavailableMessage: (command: string) =>
    [
      "NodoAssist updates cannot safely run inside the live gateway process without a managed-service handoff.",
...
beforeEach(() => {
  ...
  readConfigFileSnapshotMock.mockResolvedValue({
    ...
    raw: "{}",
    parsed: {},
    resolved: {} as NodoAssistConfig,
    sourceConfig: {} as NodoAssistConfig,
    valid: true,
    config: {} as NodoAssistConfig,
    runtimeConfig: {} as NodoAssistConfig,
    issues: [],
    warnings: [],
    legacyIssues: [],
  });
...
async function invokeUpdateRun(
  ...
  runtimeConfig: NodoAssistConfig = { update: {} },
) {
...
function mockGlobalInstallSurface() {
  resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
    kind: "global",
    mode: "npm",
    root: "/tmp/nodoassist-global",
    packageRoot: "/tmp/nodoassist-global",
  });
}
...
describe("update.run restart scheduling", () => {
  ...
  it("preserves an indefinite restart drain for managed updates", async () => {
    ...
    await withProcessEnv({ NODOASSIST_LAUNCHD_LABEL: "ai.nodoassist.gateway" }, () =>
  ...
  it("keeps a startup grace before restarting after systemd handoff spawn", async () => {
    ...
    await withProcessEnv({ NODOASSIST_SYSTEMD_UNIT: "nodoassist-gateway.service" }, () =>
  ...
  it("hands supervised git/dev updates to the CLI path instead of rebuilding live dist in-process", async () => {
    detectRespawnSupervisorMock.mockReturnValueOnce("launchd");
    mockGitInstallSurface("/tmp/nodoassist-git");
    const payload = await withProcessEnv(
      { NODOASSIST_LAUNCHD_LABEL: "ai.nodoassist.gateway" },
      () => captureUpdateRunPayload(),
    );
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        "NodoAssist updates cannot safely run inside the live gateway process without a managed-service handoff.\n" +
        "Run `nodoassist update --yes --timeout 1800` from a shell outside the gateway service, or restart/update from the host UI.",
    });
  });
...
describe("update.run post-core plugin finalize", () => {
  ...
  it("resumes official plugin convergence after a git/source core update", async () => {
    runPostCoreFinalizeAfterGatewayUpdateMock.mockResolvedValueOnce({
      status: "ok",
      entrypoint: "/tmp/nodoassist-git/dist/index.mjs",
    });
    mockGitOkUpdate("/tmp/nodoassist-git");
  ...
  it("carries the pre-doctor source config into the git finalizer", async () => {
    ...
    const [finalizeParams] = firstMockCall(
      ...
      { preUpdateConfig?: { sourceConfig?: NodoAssistConfig; authoredConfig?: NodoAssistConfig } },
    ];
