...
export function resolveHooksConfig(cfg: NodoAssistConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: NodoAssistConfig, defaultAgentId: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-nodoassist-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-nodoassist-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-nodoassist-idempotency-key"]) ||
