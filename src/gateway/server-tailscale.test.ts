...
describe("startGatewayTailscaleExposure preserveFunnel", () => {
  ...
  it("passes serviceName through to Tailscale Serve setup and cleanup", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(18789, undefined, "svc:nodoassist");
    expect(logTailscale.info).toHaveBeenCalledWith(
      "serve enabled for svc:nodoassist: https://openclaw.tailnet.ts.net/ (WS via wss://nodoassist.tailnet.ts.net)",
    );

    await cleanup?.();

    expect(mocks.disableTailscaleServe).toHaveBeenCalledWith(undefined, "svc:nodoassist");
  });
