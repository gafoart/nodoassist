...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: NodoAssistConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: NodoAssistConfig) => {
...
function hasActiveGatewayAuthSecretRef(config: NodoAssistConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: NodoAssistConfig): NodoAssistConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: NodoAssistConfig): void {
