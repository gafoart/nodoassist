...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-nodoassist-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-nodoassist-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-nodoassist-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-nodoassist-thread-id"));
