...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.NODOASSIST_GATEWAY_STARTUP_TRACE);
  let timelineConfig: NodoAssistConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.NODOASSIST_DIAGNOSTICS_EVENT_LOOP);
  ...
  return {
    setConfig(config: NodoAssistConfig) {
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`nodoassist config set gateway.auth.mode token\` and \`nodoassist config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
    "In Nix mode, set gateway.auth.token in your Nix-managed NodoAssist config and rebuild.",
...
export type GatewayServerOptions = {
   ...
   * reparsing nodoassist.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.NODOASSIST_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.NODOASSIST_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "NODOASSIST_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "NODOASSIST_RAW_STREAM_PATH",
  ...
  let cfgAtStart: NodoAssistConfig;
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: NodoAssistConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: NodoAssistConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({
