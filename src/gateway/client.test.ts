...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("nodoassist doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    envSnapshot.restore();
    delete process.env.NODOASSIST_ALLOW_INSECURE_PRIVATE_WS;
    delete process.env.NODOASSIST_PROXY_ACTIVE;
    delete process.env.NODOASSIST_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with NODOASSIST_ALLOW_INSECURE_PRIVATE_WS=1", () => {
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { NODOASSIST_HOME: "/tmp/custom-nodoassist-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      NODOASSIST_STATE_DIR: "/tmp/nodoassist-client-service-state",
