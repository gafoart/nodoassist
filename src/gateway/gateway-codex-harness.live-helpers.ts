...
export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current NodoAssist session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsNodoAssistStatus =
    normalized.includes("nodoassist is running on") ||
    /nodoassist\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("nodoassist status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsNodoAssistStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from nodoassist status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live nodoassist config shows") ||
