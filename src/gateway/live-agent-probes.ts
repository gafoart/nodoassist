...
const NODOASSIST_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const NODOASSIST_CLI_CHILD_TIMEOUT_MS = NODOASSIST_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export function buildLiveCronProbeMessage(params: {
  ...
  if (params.attempt === 0) {
    return (
      "Use the NodoAssist MCP cron tool from server `nodoassist`. " +
      "If it is not already visible, search/load MCP tools for `nodoassist cron` or `cron`, " +
      "then call the matching NodoAssist MCP tool; Claude-style names may appear as `mcp__nodoassist__cron`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not NodoAssist proof. " +
      ...
      "Do the actual tool call; I will verify externally with the NodoAssist cron CLI. " +
      `After the cron job is created, reply exactly: ${params.exactReply}`
    );
  }
  if (claudeLike) {
    return (
      "Retry the NodoAssist MCP cron tool from server `nodoassist` now. " +
      "If it is not already visible, search/load MCP tools for `nodoassist cron` or `cron`, " +
      "then call the matching NodoAssist MCP tool; Claude-style names may appear as `mcp__nodoassist__cron`. " +
      "Do not use Claude native `CronCreate`, `CronList`, or `CronDelete`; those are not NodoAssist proof. " +
      ...
      "I will verify externally with the NodoAssist cron CLI."
    );
  }
  return (
    "Your previous NodoAssist cron MCP tool call was cancelled before the job was created. " +
    "Retry the NodoAssist MCP cron tool from server `nodoassist` now. " +
    "If the harness shows Claude-style MCP names, use `mcp__nodoassist__cron`. " +
    `Use these exact JSON arguments: ${params.argsJson}. ` +
    "Preserve job.sessionTarget and job.sessionKey exactly as provided. " +
    `If the cron job is created, reply exactly: ${params.exactReply}. ` +
    "If the tool call is cancelled, the job is not created, or you cannot confirm creation, " +
...
export async function runNodoAssistCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(NODOASSIST_CLI_GATEWAY_TIMEOUT_MS)];
  const { stdout, stderr } = await execFileAsync(process.execPath, ["nodoassist.mjs", ...cliArgs], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: NODOASSIST_CLI_CHILD_TIMEOUT_MS,
    maxBuffer: 1024 * 1024,
  });
  ...
  if (!trimmed) {
    throw new Error(
      [
        `nodoassist ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `nodoassist ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runNodoAssistCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
