...
let lastCreateNodoAssistToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/nodoassist-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateNodoAssistToolsContext?.agentTo,
          agentThreadId: lastCreateNodoAssistToolsContext?.agentThreadId,
        },
        inheritedToolDenylist: lastCreateNodoAssistToolsContext?.inheritedToolDenylist,
      }),
    },
    {
      name: "sessions_send",
  ...
  return {
    createNodoAssistTools: (ctx: Record<string, unknown>) => {
      lastCreateNodoAssistToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateNodoAssistToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-nodoassist-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-nodoassist-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateNodoAssistToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateNodoAssistToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateNodoAssistToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateNodoAssistToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateNodoAssistToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateNodoAssistToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateNodoAssistToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-nodoassist-message-to": "channel:24514",
        "x-nodoassist-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("keeps shared-secret bearer auth as owner for explicitly allowed owner-only tools", async () => {
    ...
    expect(lastCreateNodoAssistToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateNodoAssistToolsContext?.senderIsOwner).toBe(false);
