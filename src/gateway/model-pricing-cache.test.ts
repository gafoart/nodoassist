...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listNodoAssistPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...
    listNodoAssistPluginManifestMetadata: (
      params?: Parameters<typeof actual.listNodoAssistPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listNodoAssistPluginManifestMetadata(params);
      return actual.listNodoAssistPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listNodoAssistPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as NodoAssistConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listNodoAssistPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/nodoassist.plugin.json",
    ...overrides,
  };
}
