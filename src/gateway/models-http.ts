// OpenAI-compatible `/v1/models` HTTP route backed by configured NodoAssist agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([NODOASSIST_MODEL_ID, NODOASSIST_DEFAULT_MODEL_ID]);
  ids.add(`nodoassist/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`nodoassist/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== NODOASSIST_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
