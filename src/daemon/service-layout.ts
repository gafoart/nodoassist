/** Summarizes installed service command paths and NodoAssist package layout. */
...
async function resolveNodoAssistPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal
    ? await resolveNodoAssistPackageRoot(entrypointReal)
    : undefined;
