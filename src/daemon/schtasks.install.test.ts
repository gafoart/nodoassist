...
describe("installScheduledTask", () => {
  ...
  async function withUserProfileDir(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-schtasks-install-"));
    const env = {
      USERPROFILE: tmpDir,
  ...
  function expectInitialTaskQueries(taskName = "NodoAssist Gateway"): void {
    expect(schtasksCalls[0]).toEqual(["/Query"]);
    expect(schtasksCalls[1]).toEqual(["/Query", "/TN", taskName]);
  }

  function expectTaskRunCall(index: number, taskName = "NodoAssist Gateway"): void {
  ...
  it("writes quoted set assignments and escapes metacharacters", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      expect(schtasksCalls[0]).toEqual(["/Query"]);
      expect(schtasksCalls[1]).toEqual(["/Query", "/TN", "NodoAssist Gateway"]);
      ...
      expect(schtasksCalls[4]).toEqual(["/Run", "/TN", "NodoAssist Gateway"]);
    });
  });
  ...
  it("uses the hidden launcher for generated Windows gateway service installs", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      const callerEnv: Record<string, string | undefined> = {
        ...
        USERDOMAIN: "WORKSTATION",
        USERNAME: "alice",
        NODOASSIST_WINDOWS_TASK_NAME: "NodoAssist Custom Gateway",
      };
      ...
      expect(callerEnv.NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER).toBeUndefined();
      expect(gatewayEnv.NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
      expect(gatewayEnv.NODOASSIST_WINDOWS_TASK_NAME).toBe("NodoAssist Gateway");
      ...
      expect(schtasksCalls[2]?.slice(0, 5)).toEqual([
        ...
        "/F",
        "/TN",
        "NodoAssist Custom Gateway",
        "/XML",
      ]);
      expect(schtasksCalls[2]?.slice(6)).toEqual(["/RU", "WORKSTATION\\alice", "/NP"]);
      const captured = xmlPayloadCaptures.find((entry) => entry.index === 2);
      expect(captured?.xml).toContain("gateway.vbs</Command>");
      expect(script).toContain('set "NODOASSIST_WINDOWS_TASK_NAME=NodoAssist Custom Gateway"');
      expect(launcher).toContain("WScript.Shell");
      expect(launcher).toContain(`Run """${scriptPath}""", 0, False`);
      expectTaskRunCall(3, "NodoAssist Custom Gateway");
    });
  });
  ...
  it("updates existing tasks to use the hidden launcher when requested", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      const { scriptPath } = await installDefaultGatewayTask({
        ...
        USERDOMAIN: "WORKSTATION",
        USERNAME: "alice",
        NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER: "true",
      });
      const launcherPath = scriptPath.replace(/\.cmd$/i, ".vbs");
  ...
  it("does not persist a frozen PATH snapshot into the generated task script", async () => {
    await withUserProfileDir(async (_tmpDir, env) => {
      ...
      expect(script).toContain('set "NODOASSIST_GATEWAY_PORT=18789"');
    });
  });
