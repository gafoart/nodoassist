...
function resolveGatewayLogPrefix(env: GatewayServiceEnv): string {
  return env.NODOASSIST_LOG_PREFIX?.trim() || "gateway";
}

function resolveMacLaunchAgentLogPrefix(env: GatewayServiceEnv): string {
  return (
    env.NODOASSIST_LOG_PREFIX?.trim() ||
    `gateway${resolveGatewayProfileSuffix(env.NODOASSIST_PROFILE)}`
  );
}
...
function resolveMacLaunchAgentLogPaths(env: GatewayServiceEnv): GatewayLogPaths {
  ...
  const logDir = path.posix.join(home, "Library", "Logs", "nodoassist");
...
export function resolveGatewaySupervisorLogPaths(
  ...
  // NodoAssist state dir so generated service users can create the directory.
...
export function renderCmdRestartLogSetup(env: GatewayServiceEnv): {
  ...
  return {
    ...
    lines: [
      ...
      `>> ${quotedLogPath} 2>&1 echo [%DATE% %TIME%] nodoassist restart log initialized`,
    ],
  };
}
