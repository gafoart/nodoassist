...
type LaunchdRestartLogEnv = {
  HOME?: string;
  USERPROFILE?: string;
  NODOASSIST_STATE_DIR?: string;
  NODOASSIST_PROFILE?: string;
};
...
function collectRestartLogEnv(env?: Record<string, string | undefined>): LaunchdRestartLogEnv {
  ...
  return {
    ...
    NODOASSIST_STATE_DIR: source.NODOASSIST_STATE_DIR,
    NODOASSIST_PROFILE: source.NODOASSIST_PROFILE,
  };
}

function resolveLaunchAgentLabel(env?: Record<string, string | undefined>): string {
  const envLabel = normalizeOptionalString(env?.NODOASSIST_LAUNCHD_LABEL);
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(env?.NODOASSIST_PROFILE));
}
...
printf '[%s] nodoassist restart attempt source=launchd-handoff mode=${mode} target=%s waitPid=%s\\n' "$(date -u +%FT%TZ)" "$service_target" "$wait_pid" >&2
...
if [ "$status" -eq 0 ]; then
  printf '[%s] nodoassist restart done source=launchd-handoff mode=${mode}\\n' "$(date -u +%FT%TZ)" >&2
else
  printf '[%s] nodoassist restart failed source=launchd-handoff mode=${mode} status=%s\\n' "$(date -u +%FT%TZ)" "$status" >&2
fi
exit "$status"
`;
  }
...
while [ "$print_retry_count" -gt 0 ]; do
  if launchctl print "$service_target" >/dev/null 2>&1; then
    printf '[%s] nodoassist restart done source=launchd-handoff mode=${mode} reason=launchd-auto-reload\\n' "$(date -u +%FT%TZ)" >&2
    exit 0
  fi
  ...
  try {
    const child = spawn(
      ...
      [
        ...
        "nodoassist-launchd-restart-handoff",
