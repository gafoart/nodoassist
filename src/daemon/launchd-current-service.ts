...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.NODOASSIST_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.NODOASSIST_SERVICE_MARKER) === "nodoassist" &&
    Boolean(normalizeOptionalString(env.NODOASSIST_SERVICE_KIND))
  ) {
