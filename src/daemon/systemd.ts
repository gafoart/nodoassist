...
function resolveSystemdServiceName(env: GatewayServiceEnv): string {
  ...
  return resolveGatewaySystemdServiceName(env.NODOASSIST_PROFILE);
}
...
function sanitizeSystemdUnitBackupContent(params: {
  ...
  // Backups should not retain file-managed secrets that NodoAssist moved into the
...
function resolveSystemdEnvironmentFilePath(params: {
  ...
  const serviceKind = params.environment?.NODOASSIST_SERVICE_KIND?.trim();
  const filename =
...
function resolveLegacyNodeSystemdEnvironmentFilePath(params: {
  ...
  if (params.environment?.NODOASSIST_SERVICE_KIND?.trim() !== "node") {
    return null;
  }
...
function isNodeSystemdEnvironment(env: GatewayServiceEnv): boolean {
  return env.NODOASSIST_SERVICE_KIND?.trim() === "node";
}
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** NodoAssist-managed keys that must not be preserved from an old env file; stale file values
  ...
  fileManagedKeys?: ReadonlySet<string>;
  /** State-dir .env keys NodoAssist previously managed but is now skipping (unresolved shell
  ...
  // NodoAssist-managed keys (identified by inlineManagedKeys) are excluded: a stale
...
async function removeNodeSystemdManagedEnvironmentKeys(env: GatewayServiceEnv): Promise<void> {
  ...
  const managedKeys = new Set([normalizeSystemdEnvironmentKey("NODOASSIST_GATEWAY_TOKEN")]);
