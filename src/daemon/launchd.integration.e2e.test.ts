...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `nodoassist-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      NODOASSIST_LAUNCHD_LABEL: `ai.nodoassist.launchd-int-${testId}`,
      NODOASSIST_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
