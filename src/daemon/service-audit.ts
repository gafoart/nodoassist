...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds NODOASSIST_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `nodoassist gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service NODOASSIST_GATEWAY_TOKEN does not match gateway.auth.token in nodoassist.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.NODOASSIST_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.NODOASSIST_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.NODOASSIST_SERVICE_VERSION?.trim();
  ...
  issues.push({
    ...
    message: `Gateway service was installed by NodoAssist ${serviceVersion}; current CLI is ${VERSION}.`,
