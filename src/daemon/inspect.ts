/** Inspects installed platform services for extra NodoAssist or legacy gateway jobs. */
...
const EXTRA_MARKERS = ["nodoassist", "clawdbot"] as const;
...
export function renderGatewayServiceCleanupHints(
  ...
  const profile = env.NODOASSIST_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  ...
  const markerKeys = ["nodoassist_service_marker"];
  const kindKeys = ["nodoassist_service_kind"];
...
function isNodoAssistGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (detectLaunchdGatewayExecutionMarker(contents) !== "nodoassist") {
    return false;
  }
  return label.startsWith("ai.nodoassist.");
}

function isNodoAssistGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("nodoassist-gateway")) {
    return false;
  }
  return normalizeLowercaseStringOrEmpty(contents).includes("gateway");
}

function isNodoAssistGatewayTaskName(name: string): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(name);
  if (!normalized) {
    return false;
  }
  // Windows schtasks /Query returns task names prefixed with \ (e.g.
  // \NodoAssist Gateway for root-folder tasks). Strip the leading
  ...
  return stripped === defaultName || /^nodoassist gateway \(.+\)$/.test(stripped);
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    const marker =
      hasGatewayServiceMarker(contents) || executionMarker === "nodoassist"
        ? "nodoassist"
    ...
    if (marker === "nodoassist" && isNodoAssistGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "nodoassist" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  dir: string;
  scope: "user" | "system";
  includeManagedNodoAssist?: boolean;
}): Promise<ExtraGatewayService[]> {
  const results: ExtraGatewayService[] = [];
  const candidates = await collectServiceFiles({
    dir: params.dir,
    extension: ".service",
    isIgnoredName: params.includeManagedNodoAssist ? () => false : isIgnoredSystemdName,
  });
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (
      !params.includeManagedNodoAssist &&
      marker === "nodoassist" &&
      isNodoAssistGatewaySystemdService(name, contents)
    ) {
      continue;
    }
    results.push({
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isNodoAssistGatewayTaskName(name)) {
        continue;
      }
