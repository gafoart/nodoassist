...
function isLaunchAgentServiceEnvironment(params: {
  ...
  return (
    ...
    Boolean(params.serviceEnvironment.NODOASSIST_LAUNCHD_LABEL?.trim())
  );
}
...
export function applyManagedServiceEnvRenderPolicy(params: {
  ...
  if (params.plan.environment.NODOASSIST_SERVICE_MANAGED_ENV_KEYS) {
    params.plan.environmentValueSources.NODOASSIST_SERVICE_MANAGED_ENV_KEYS = "inline";
  }
  const managedKeys = readManagedServiceEnvKeysFromEnvironment({
    NODOASSIST_SERVICE_MANAGED_ENV_KEYS: params.managedServiceEnvKeys,
  });
