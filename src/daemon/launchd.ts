...
const NODOASSIST_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.nodoassist.update.";
const NODOASSIST_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.nodoassist\.manual-update\.\d+$/;
...
export type StaleNodoAssistUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeNodoAssistUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(NODOASSIST_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.nodoassist labels.
  return NODOASSIST_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.NODOASSIST_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.NODOASSIST_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.NODOASSIST_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.NODOASSIST_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

function resolveCurrentNodoAssistUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.NODOASSIST_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeNodoAssistUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.NODOASSIST_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.NODOASSIST_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by NodoAssist. Do not edit while the gateway service is installed.",
...
async function resolveLaunchAgentEnvironmentWrapperOverwriteWarnings(params: {
  ...
  return [
    `Existing generated LaunchAgent env wrapper at ${params.wrapperPath} contains custom behavior and will be overwritten; move custom behavior to nodoassist gateway install --wrapper <path> or NODOASSIST_WRAPPER.`,
  ];
}
...
export function parseLaunchctlListNodoAssistUpdateJobs(
  output: string,
): StaleNodoAssistUpdateLaunchdJob[] {
  const jobs: StaleNodoAssistUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeNodoAssistUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleNodoAssistUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleNodoAssistUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListNodoAssistUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function disableNodoAssistUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeNodoAssistUpdateLaunchdLabel(label);
...
export async function disableCurrentNodoAssistUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentNodoAssistUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableNodoAssistUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.NODOASSIST_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.NODOASSIST_GATEWAY_PORT ?? "");
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `nodoassist gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.NODOASSIST_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "nodoassist gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "nodoassist gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `nodoassist gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "nodoassist gateway restart",
    });
