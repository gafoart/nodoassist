...
export const NODOASSIST_WRAPPER_ENV_KEY = "NODOASSIST_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/nodoassist -> .pnpm/nodoassist@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // nodoassist from node_modules/.bin points at the package root sibling.
...
export async function resolveNodoAssistWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${NODOASSIST_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveNodoAssistWrapperPath(params.wrapperPath);
  if (wrapperPath) {
