...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.NODOASSIST_GATEWAY_PORT).toBe("18789");
    expect(env.NODOASSIST_GATEWAY_TOKEN).toBeUndefined();
    expect(env.NODOASSIST_SERVICE_MARKER).toBe("nodoassist");
    expect(env.NODOASSIST_SERVICE_KIND).toBe("gateway");
    expect(typeof env.NODOASSIST_SERVICE_VERSION).toBe("string");
    expect(env.NODOASSIST_SYSTEMD_UNIT).toBe("nodoassist-gateway.service");
    expect(env.NODOASSIST_WINDOWS_TASK_NAME).toBe("NodoAssist Gateway");
    expect(env.NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    if (process.platform === "darwin") {
      expect(env.NODOASSIST_LAUNCHD_LABEL).toBe("ai.nodoassist.gateway");
    }
  });

  it("sets the NodoAssist-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through NODOASSIST_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.NODOASSIST_WRAPPER).toBe("/usr/local/bin/nodoassist-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".nodoassist", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", NODOASSIST_PROFILE: "work" },
      port: 18789,
    });
    expect(env.NODOASSIST_SYSTEMD_UNIT).toBe("nodoassist-gateway-work.service");
    expect(env.NODOASSIST_WINDOWS_TASK_NAME).toBe("NodoAssist Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.NODOASSIST_LAUNCHD_LABEL).toBe("ai.nodoassist.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        ...
        NODOASSIST_SYSTEMD_UNIT: "nodoassist-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.NODOASSIST_SYSTEMD_UNIT).toBe("nodoassist-gateway-maintenance.service");
  });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", NODOASSIST_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        NODOASSIST_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.NODOASSIST_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the NodoAssist-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.NODOASSIST_LAUNCHD_LABEL).toBe("ai.nodoassist.node");
  });

  it("passes through NODOASSIST_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", NODOASSIST_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.NODOASSIST_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through NODOASSIST_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", NODOASSIST_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.NODOASSIST_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits NODOASSIST_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.NODOASSIST_WINDOWS_TASK_NAME).toBe("NodoAssist Node");
    expect(env.NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.NODOASSIST_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".nodoassist"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", NODOASSIST_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".nodoassist-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", NODOASSIST_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".nodoassist"));
  });

  it("uses NODOASSIST_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", NODOASSIST_STATE_DIR: "/var/lib/nodoassist" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/nodoassist"));
  });

  it("expands ~ in NODOASSIST_STATE_DIR", () => {
    const env = { HOME: "/Users/test", NODOASSIST_STATE_DIR: "~/nodoassist-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/nodoassist-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { NODOASSIST_STATE_DIR: "C:\\State\\nodoassist" };
