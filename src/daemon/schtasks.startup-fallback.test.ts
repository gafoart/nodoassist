...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.NODOASSIST_WINDOWS_TASK_NAME ?? "NodoAssist Gateway";
  return path.join(
    env.APPDATA,
    "Microsoft",
    "Windows",
...
async function writeNodeScript(env: Record<string, string>, port = "18789") {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "NODOASSIST_SERVICE_KIND=node"`,
      `set "NODOASSIST_GATEWAY_PORT=${port}"`,
      `"C:\\bin\\nodoassist.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
function makeNodeServiceEnv(env: Record<string, string>): Record<string, string> {
  return {
    ...
    NODOASSIST_WINDOWS_TASK_NAME: "NodoAssist Node",
  };
}
        ...
        stdout: JSON.stringify(
          processAlive
            ? [
                {
                  ProcessId: processId,
                  CommandLine: "C:\\bin\\nodoassist.cmd node run --host 127.0.0.1 --port 18789",
                },
...
function expectStartupFallbackSpawn() {
  ...
  expect((options.env as Record<string, string> | undefined)?.NODOASSIST_GATEWAY_PORT).toBe(
    "18789",
  );
...
function installGatewayScheduledTask(
  ...
  return installScheduledTask({
    ...
    environment: { NODOASSIST_GATEWAY_PORT: port },
...
describe("Windows startup fallback", () => {
  it("skips task ownership probes when no Startup fallback exists", async () => {
    await withWindowsEnv("nodoassist-win-startup-", async ({ env }) => {
  ...
  it("migrates an exact persisted wrapper that owns the replacement port", async () => {
    await withWindowsEnv("nodoassist-win-startup-", async ({ env }) => {
      ...
      await fs.writeFile(
        ...
        [
          "@echo off",
          'set "NODOASSIST_GATEWAY_PORT=18789"',
          '"C:\\bin\\nodoassist-doppler.exe" gateway --port 18789',
          "",
        ].join("\r\n"),
        "utf8",
      );
      ...
      inspectPortUsage
        .mockResolvedValueOnce({
          ...
          listeners: [
            {
              pid: 4242,
              command: "nodoassist-doppler.exe",
              commandLine: '"C:\\bin\\nodoassist-doppler.exe" gateway --port 18789',
            },
          ],
          hints: [],
        ...
        if (
          ...
          return makeSpawnSyncResult({
            stdout: JSON.stringify([
              {
                ProcessId: 3131,
                CommandLine: "C:\\manual\\nodoassist.cmd gateway --port 18789",
              },
              {
                ProcessId: 4242,
                CommandLine:
  ...
  it("probes the old fallback port before replacing a drifted task script", async () => {
    ...
    await withWindowsEnv("nodoassist-win-startup-", async ({ env }) => {
      ...
      env.NODOASSIST_GATEWAY_PORT = "19433";
      inspectPortUsage.mockImplementation(async (port) => ({
        port,
  ...
  it("accepts a clean hidden-launcher exit when its gateway listener is running", async () => {
    await withWindowsEnv("nodoassist-win-startup-", async ({ env }) => {
      const hiddenEnv = { ...env, NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER: "1" };
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 5151,
                CommandLine: "node nodoassist node run --host 127.0.0.1 --port 18789",
              },
            ]),
            stderr: "",
            status: 0,
  ...
  it("does not report a node task as running from a gateway listener", async () => {
    await withWindowsEnv("nodoassist-win-startup-", async ({ env }) => {
      env.NODOASSIST_SERVICE_KIND = "node";
      env.NODOASSIST_WINDOWS_TASK_NAME = "NodoAssist Node";
      findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4242]);
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
        { code: 0, stdout: notYetRunTaskQueryOutput(), stderr: "" },
  ...
  it("kills the Startup fallback runtime even when the CLI env omits the gateway port", async () => {
    ...
    await withWindowsEnv("nodoassist-win-startup-", async ({ env }) => {
      ...
      delete envWithoutPort.NODOASSIST_GATEWAY_PORT;
