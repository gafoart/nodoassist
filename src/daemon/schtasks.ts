...
async function writeTaskXmlTempFile(xml: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-task-xml-"));
...
function shouldUseHiddenWindowsTaskLauncher(env: GatewayServiceEnv): boolean {
  const value = normalizeLowercaseStringOrEmpty(env.NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER);
...
async function resolveScheduledTaskProcess(
  ...
  const port =
    ...
    parsePositivePort(command?.environment?.NODOASSIST_GATEWAY_PORT) ??
  ...
  // Match the full persisted argv so another NodoAssist process on the same port
...
function shouldManageGatewayListenerPort(env: GatewayServiceEnv): boolean {
  return normalizeLowercaseStringOrEmpty(env.NODOASSIST_SERVICE_KIND) !== NODE_SERVICE_KIND;
}
...
async function assertReplacementPortAvailableForTakeover(params: {
  ...
  const port =
    ...
    parsePositivePort(params.environment?.NODOASSIST_GATEWAY_PORT) ??
...
const CALLER_OWNED_SERVICE_IDENTITY_KEYS = [
  ...
  "NODOASSIST_SYSTEMD_UNIT",
  "NODOASSIST_WINDOWS_TASK_NAME",
] as const;
...
const SCHEDULED_TASK_ACTIVATION_KEYS = [
  "NODOASSIST_WINDOWS_TASK_HIDDEN_LAUNCHER",
  "NODOASSIST_TASK_SCRIPT_NAME",
  "NODOASSIST_TASK_SCRIPT",
...
async function updateExistingScheduledTask(params: {
  ...
  const upgradeXmlPath = await writeTaskXmlTempFile(
    buildScheduledTaskXml({
      taskDescription: params.description ?? "NodoAssist Gateway",
...
async function activateScheduledTask(params: {
  ...
  const taskDescription = params.description ?? "NodoAssist Gateway";
