...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded NodoAssist agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<NodoAssistConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-commitment-runtime-"));
    tmpDirs.push(tmpDir);
    stateDirEnvSnapshot ??= captureEnv(["NODOASSIST_STATE_DIR"]);
