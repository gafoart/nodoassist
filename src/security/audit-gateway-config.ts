...
type CollectDangerousConfigFlags = (cfg: NodoAssistConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.NODOASSIST_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.NODOASSIST_GATEWAY_PASSWORD);
