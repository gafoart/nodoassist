...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-security-audit-async-"));
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        nodoassist: { extensions: ["../outside.js"] },
      }),
    );
  ...
  it("ignores install backup and debris dirs when scanning installed plugin roots", async () => {
    ...
    try {
      ...
      expect(findings.map((f) => f.title).join("\n")).not.toContain(".nodoassist-install-backups");
    } finally {
      scanSpy.mockRestore();
    }
  });
  ...
  it("audits canonical auth profile SQLite store permissions", async () => {
    ...
    const databasePath = path.join(agentDir, "nodoassist-agent.sqlite");
    ...
    expect(readableAuthTargets).toEqual(
      expect.arrayContaining([
        expect.stringContaining("nodoassist-agent.sqlite"),
        expect.stringContaining("nodoassist-agent.sqlite-wal"),
        expect.stringContaining("nodoassist-agent.sqlite-shm"),
        expect.stringContaining("nodoassist-agent.sqlite-journal"),
      ]),
    );
  });
});
