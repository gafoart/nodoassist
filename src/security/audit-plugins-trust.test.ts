...
describe("security audit install metadata findings", () => {
  ...
  const runInstallMetadataAudit = async (cfg: NodoAssistConfig, stateDir: string) => {
  ...
  const writePluginIndexInstallRecords = async (
    ...
    const index: InstalledPluginIndex = {
      ...
      plugins: Object.keys(records).map((pluginId) => ({
        pluginId,
        manifestPath: path.join(stateDir, "extensions", pluginId, "nodoassist.plugin.json"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-security-install-"));
  });
              ...
              hooks: {
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@nodoassist/test-hooks@1.2.3",
        ...
        run: async () => {
          ...
          await writePluginIndexInstallRecords(stateDir, {
            "voice-call": {
              source: "npm",
              spec: "@nodoassist/voice-call",
              resolvedSpec: "@nodoassist/voice-call@1.2.3",
              integrity: "sha512-plugin",
            },
          });
          return runInstallMetadataAudit(
                ...
                internal: {
                  installs: {
                    "test-hooks": {
                      source: "npm",
                      spec: "@nodoassist/test-hooks",
                      resolvedSpec: "@nodoassist/test-hooks@1.2.3",
                      integrity: "sha512-hook",
                    },
                  },
                },
...
describe("security audit extension tool reachability findings", () => {
  ...
  const runSharedExtensionsAudit = async (config: NodoAssistConfig) => {
  ...
  beforeAll(async () => {
    ...
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-security-extensions-"));
    isolatedHome = path.join(fixtureRoot, "home");
    const isolatedEnv = createPathResolutionEnv(isolatedHome, { NODOASSIST_HOME: isolatedHome });
    ...
    const cases = [
      {
        ...
        cfg: {} satisfies NodoAssistConfig,
