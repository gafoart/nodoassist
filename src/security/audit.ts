...
import { asNullableRecord } from "@nodoassist/normalization-core/record-coerce";
import {
...
import type { ConfigFileSnapshot, NodoAssistConfig } from "../config/config.js";
import { resolveConfigPath, resolveStateDir } from "../config/paths.js";
import type { CliBackendConfig } from "../config/types.agent-defaults.js";
import type { GatewayAuthConfig } from "../config/types.gateway.js";
import type { SecurityAuditSuppression } from "../config/types.nodoassist.js";
import {
...
async function materializeAuditGatewayAuthRefs(params: {
  ...
  const materializeParams = {
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(params.env.NODOASSIST_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(params.env.NODOASSIST_GATEWAY_PASSWORD)),
  };
...
function shouldMaterializeHooksGatewayAuthRefs(cfg: NodoAssistConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your NodoAssist state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your NodoAssist state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
export function collectLoggingFindings(cfg: NodoAssistConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: NodoAssistConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectYoloExecScopeIds(cfg: NodoAssistConfig, approvals: ExecApprovalsFile): string[] {
...
export function collectExecRuntimeFindings(cfg: NodoAssistConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  ...
  if (claudePermissionModeHits.length > 0 && yoloExecScopeIds.length > 0) {
    findings.push({
      ...
      detail: `claude-cli sets ${claudePermissionModeHits.map((hit) => `${hit.argSet}=${hit.mode}`).join(", ")}, but NodoAssist exec is YOLO for: ${yoloExecScopeIds.join(", ")}. Managed Claude live sessions use --permission-mode bypassPermissions.`,
      remediation:
        "Restrict NodoAssist tools.exec.security/tools.exec.ask, or remove the Claude --permission-mode override.",
    });
  }
...
function listConfiguredMcpServerNames(cfg: NodoAssistConfig): string[] {
...
function collectAgentSkillMcpBoundaryScopes(cfg: NodoAssistConfig): AgentSkillMcpBoundaryScope[] {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  return [
    {
      ...
      detail:
        ...
        `\nMCP server registries visible to the gateway configuration/state:\n${sources
          ...
          .join("\n")}\n` +
        "agents.*.skills filters NodoAssist skill visibility and snapshots; it is not a shell-time authorization boundary. " +
...
function collectOpenExecSurfacePaths(cfg: NodoAssistConfig): string[] {
