...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: NodoAssistConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when NODOASSIST_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: NodoAssistConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from NODOASSIST_GATEWAY_TOKEN", () => {
    const cfg: NodoAssistConfig = {
