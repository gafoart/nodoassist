...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("nodoassist status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("nodoassist security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set NODOASSIST_GATEWAY_TOKEN/NODOASSIST_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("nodoassist security audit --deep")}".`,
    });
  }
  return findings;
}
