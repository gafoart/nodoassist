...
describe("security audit exec surface findings", () => {
  // Redirect the NodoAssist home (NODOASSIST_HOME wins over HOME/USERPROFILE in
  // `resolveRawHomeDir`) to a per-test tempdir so `saveExecApprovals` never
  // touches the real `~/.nodoassist/exec-approvals.json` on the host running
  // the suite.
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-exec-approvals-"));
  });

  beforeEach(async () => {
    envSnapshot = captureEnv(["NODOASSIST_HOME", "HOME", "USERPROFILE"]);
    const tempDir = path.join(tempRoot, `case-${++tempCaseIndex}`);
    await fs.mkdir(path.join(tempDir, ".nodoassist"), { recursive: true });
    // NODOASSIST_HOME takes precedence over HOME/USERPROFILE in resolveRawHomeDir,
    // so all three must point at the tempdir to neutralize whichever the host
    // happens to have set.
    process.env.NODOASSIST_HOME = tempDir;
  ...
  it("warns when YOLO exec overrides restrictive Claude permission mode", () => {
    ...
    expect(finding?.detail).toContain("NodoAssist exec is YOLO");
  });
  ...
  it("does not warn for restrictive Claude permission mode when NodoAssist exec is restrictive", () => {
    const findings = collectExecRuntimeFindings({
