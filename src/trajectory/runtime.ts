...
async function replaceTrajectoryWindow(params: {
  ...
  await writeSiblingTempFile({
    ...
    chmodDir: false,
    mode: 0o600,
    tempPrefix: ".nodoassist-trajectory-",
...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with NODOASSIST_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.NODOASSIST_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }
