...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/nodoassist.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.nodoassist/nodoassist.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          NODOASSIST_STATE_DIR: "/Users/tester/.nodoassist",
        },
      ...
      expect(harness.invocation).toEqual([
        "node",
        "~/project/nodoassist.js",
        "--api-key",
        "<redacted>",
        "--config=$NODOASSIST_STATE_DIR/nodoassist.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/nodoassist.js");
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.nodoassist" };
