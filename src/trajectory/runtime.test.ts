...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-trajectory-runtime-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("trajectory runtime", () => {
  ...
  it("sanitizes session ids when resolving an override directory", () => {
    expect(
      resolveTrajectoryFilePath({
        env: { NODOASSIST_TRAJECTORY_DIR: "/tmp/traces" },
  ...
  it("writes a session-adjacent pointer when using an override directory", () => {
    ...
    const recorder = createTrajectoryRuntimeRecorder({
      env: { NODOASSIST_TRAJECTORY_DIR: trajectoryDir },
      sessionId: "session-1",
      sessionFile,
      writer: {
