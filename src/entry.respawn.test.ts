...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(respawnPlan.env[NODOASSIST_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(respawnPlan.env[NODOASSIST_NODE_OPTIONS_READY]).toBe("1");
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      const respawnPlan = expectCliRespawnPlan(plan);
      expect(respawnPlan.argv).toEqual(["nodoassist", command]);
      expect(respawnPlan.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(respawnPlan.env[NODOASSIST_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(respawnPlan.env[NODOASSIST_NODE_OPTIONS_READY]).toBeUndefined();
  ...
  it("keeps bare-root startup respawns attached to the terminal", () => {
    ...
    const respawnPlan = expectCliRespawnPlan(plan);
    expect(respawnPlan.argv).toEqual([EXPERIMENTAL_WARNING_FLAG, "nodoassist"]);
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("does not respawn interactive commands for warning suppression only", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "nodoassist", "tui"],
        env: { [NODOASSIST_NODE_EXTRA_CA_CERTS_READY]: "1" },
        execArgv: [],
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "nodoassist", "status"],
        env: {
          [NODOASSIST_NODE_EXTRA_CA_CERTS_READY]: "1",
          [NODOASSIST_NODE_OPTIONS_READY]: "1",
        },
  ...
  it("adds a larger V8 stack size on Windows", () => {
    ...
    expect(respawnPlan.env[NODOASSIST_NODE_EXTRA_CA_CERTS_READY]).toBeUndefined();
    expect(respawnPlan.env[NODOASSIST_NODE_OPTIONS_READY]).toBeUndefined();
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/nodoassist", "status"],
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/nodoassist/dist/entry.js", "status"],
  ...
  it("force-kills a signaled respawn child that does not exit", () => {
    ...
    try {
      runCliRespawnPlan(
        {
          command: "/usr/bin/node",
          argv: ["/repo/nodoassist/dist/entry.js", "tui"],
          env: {},
