...
import { validateIPv4AddressInput } from "@nodoassist/net-policy/ipv4";
...
import type { GatewayBindMode, GatewayTailscaleMode, NodoAssistConfig } from "../config/config.js";
    ...
    if (tokenMode === "ref") {
      if (flow === "quickstart" && quickstartTokenRef) {
        ...
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "NODOASSIST_GATEWAY_TOKEN",
          copy: {
            sourceMessage: t("wizard.gateway.authTokenStoredMessage"),
            envVarPlaceholder: "NODOASSIST_GATEWAY_TOKEN",
          },
        });
      ...
      gatewayToken =
        (quickstartTokenString ??
          normalizeGatewayTokenInput(process.env.NODOASSIST_GATEWAY_TOKEN)) ||
        randomToken();
      gatewayTokenInput = gatewayToken;
    } else {
      const existingToken =
        quickstartTokenString ?? normalizeGatewayTokenInput(process.env.NODOASSIST_GATEWAY_TOKEN);
    ...
    if (!password) {
      ...
      if (selectedMode === "ref") {
        const resolved = await promptSecretRefForSetup({
          ...
          config: nextConfig,
          prompter,
          preferredEnvVar: "NODOASSIST_GATEWAY_PASSWORD",
          copy: {
            sourceMessage: t("wizard.gateway.authPasswordStoredMessage"),
            envVarPlaceholder: "NODOASSIST_GATEWAY_PASSWORD",
          },
        });
