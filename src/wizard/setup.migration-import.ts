...
function hasMeaningfulConfig(config: NodoAssistConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.NODOASSIST_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh NodoAssist setup.",
...
async function resolveSetupMigrationProvider(params: {
  ...
  runtime: RuntimeEnv;
  workspaceDir: string;
}): Promise<{ provider: MigrationProviderPlugin; baseConfig: NodoAssistConfig }> {
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: NodoAssistConfig) => Promise<NodoAssistConfig>;
  ...
  // can reference a concrete NodoAssist config target.
