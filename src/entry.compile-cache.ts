...
export function shouldEnableNodoAssistCompileCache(params: {
...
export function resolveNodoAssistCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
type NodoAssistCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
...
type NodoAssistCompileCacheRespawnRuntime = RespawnChildRuntime & {
  writeError: (message: string) => void;
};

export function buildNodoAssistCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
  ...
  nodeVersion?: string;
  platform?: NodeJS.Platform;
}): NodoAssistCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
...
export function respawnWithoutNodoAssistCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runNodoAssistCompileCacheRespawnPlan(plan);
  return true;
}

export function runNodoAssistCompileCacheRespawnPlan(
  plan: NodoAssistCompileCacheRespawnPlan,
  runtime: NodoAssistCompileCacheRespawnRuntime = {
    spawn,
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        `[nodoassist] Failed to respawn CLI without compile cache: ${
...
export function enableNodoAssistCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableNodoAssistCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveNodoAssistCompileCacheDirectory(params));
  } catch {
