...
describe("audit event worker", () => {
  it("keeps shutdown beyond the supported SQLite contention window", () => {
    expect(testApi.auditWriterShutdownTimeoutMs).toBeGreaterThan(NODOASSIST_SQLITE_BUSY_TIMEOUT_MS);
  });

  it("returns immediately under SQLite contention and flushes before stop", async () => {
    const stateDir = makeTempDir(tempDirs, "nodoassist-audit-writer-");
    const database = { env: { NODOASSIST_STATE_DIR: stateDir } };
    const errors: string[] = [];
