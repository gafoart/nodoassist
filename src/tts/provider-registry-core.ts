...
export type SpeechProviderRegistryResolver = {
  getProvider: (providerId: string, cfg?: NodoAssistConfig) => SpeechProviderPlugin | undefined;
  listProviders: (cfg?: NodoAssistConfig) => SpeechProviderPlugin[];
};
...
export function createSpeechProviderRegistry(resolver: SpeechProviderRegistryResolver) {
  const buildResolvedProviderMaps = (cfg?: NodoAssistConfig) =>
    buildCapabilityProviderMaps(resolver.listProviders(cfg));

  const listProviders = (cfg?: NodoAssistConfig): SpeechProviderPlugin[] => [
