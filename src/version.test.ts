...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "nodoassist-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __NODOASSIST_VERSION__");
    expect(source).toContain("? __NODOASSIST_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "nodoassist", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "nodoassist", version: "2.3.4" });
  ...
  it("ignores non-nodoassist package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers NODOASSIST_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        NODOASSIST_VERSION: "9.9.9",
        NODOASSIST_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale NODOASSIST_VERSION for compatibility checks", () => {
    const previous = process.env.NODOASSIST_VERSION;
    const previousService = process.env.NODOASSIST_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.NODOASSIST_VERSION = "2026.3.25";
      process.env.NODOASSIST_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("NODOASSIST_VERSION", previous);
      restoreEnvValue("NODOASSIST_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        NODOASSIST_VERSION: "2026.3.99",
        NODOASSIST_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        NODOASSIST_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        NODOASSIST_VERSION: "2026.3.99",
        NODOASSIST_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores unusable env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        NODOASSIST_VERSION: "   ",
        NODOASSIST_SERVICE_VERSION: "  2.0.0  ",
    ...
    expect(
      resolveRuntimeServiceVersion({
        ...
        NODOASSIST_SERVICE_VERSION: "null",
        npm_package_version: "1.0.0-package",
      }),
    ).toBe(VERSION);
  });
