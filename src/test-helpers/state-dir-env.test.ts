...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    nodoassist: process.env.NODOASSIST_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.NODOASSIST_STATE_DIR).toBe(snapshot.nodoassist);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips NODOASSIST_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/nodoassist-state-dir-test");
    expect(process.env.NODOASSIST_STATE_DIR).toBe("/tmp/nodoassist-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("nodoassist-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("nodoassist-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
