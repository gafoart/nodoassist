...
export function writeClaudeMcpConfig(mcpConfig: AttachGrant["mcpConfig"]): {
  ...
  const dir = mkdtempSync(join(tmpdir(), "nodoassist-attach-"));
...
export async function registerAttachCli(program: Command, _argv: string[] = process.argv) {
  program
    ...
    .addHelpText(
      "after",
      "\nExamples:\n  nodoassist attach                       Attach Claude Code to the main session\n  nodoassist attach --session agent:main:telegram:123 --ttl 600000\n  nodoassist attach --print-config        Set up the grant + config and print how to launch it yourself\n",
    )
