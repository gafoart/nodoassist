...
function shouldShowStack(argv: string[] | undefined, env: NodeJS.ProcessEnv): boolean {
  return hasDebugArg(argv) || isTruthyEnvValue(env.NODOASSIST_DEBUG);
}

function pushPrefixed(out: string[], value: string): void {
  for (const line of value.split("\n")) {
    if (line.trim().length > 0) {
      out.push(`[nodoassist] ${line}`);
    }
  }
}
...
export function formatCliFailureLines(options: FormatCliFailureOptions): string[] {
  ...
  const lines = [
    `[nodoassist] ${options.title}`,
    `[nodoassist] Reason: ${formatErrorMessage(options.error)}`,
  ];

  if (shouldShowStack(options.argv, env)) {
    lines.push("[nodoassist] Stack:");
    pushPrefixed(lines, formatUncaughtError(options.error));
  } else {
    lines.push("[nodoassist] Debug: set NODOASSIST_DEBUG=1 to include the stack trace.");
  }

  if (options.includeDoctorHint !== false) {
    lines.push(`[nodoassist] Try: ${formatCliCommand("nodoassist doctor", env)}`);
  }
  lines.push(`[nodoassist] Help: ${formatCliCommand("nodoassist --help", env)}`);
  return lines;
}
