...
import { isRecord as isPlainRecord } from "@nodoassist/normalization-core/record-coerce";
...
function normalizeModelProviderRefsForConfigMutation(
  providers: NonNullable<NodoAssistConfig["models"]>["providers"] | undefined,
): unknown {
...
function normalizeConfigMutationModelRefs(cfg: NodoAssistConfig): NodoAssistConfig {
...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "nodoassist config set secrets.providers.vault --provider-source file --provider-path /etc/nodoassist/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("nodoassist config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("nodoassist doctor --fix")}\` ${message}`;
}
...
function formatConfigUnsetMissingPathMessage(params: {
  ...
  if (params.runtimeOnly) {
    return `Config path not found in authored config: ${params.path}. It only exists after runtime defaults are applied, so there is nothing for config unset to remove. Use ${formatCliCommand("nodoassist config set <path> <value>")} to override the inherited value.`;
  }
  return `Config path not found: ${params.path}. Nothing was changed. Run ${formatCliCommand("nodoassist config get <path>")} first if you are unsure of the path.`;
}
...
async function loadValidConfig(runtime: RuntimeEnv = defaultRuntime) {
  ...
  runtime.error(`NodoAssist config is invalid: ${shortenHomePath(snapshot.path)}`);
...
function configApplyHintForPaths(paths: string[], afterConfig: NodoAssistConfig): string {
  if (paths.length === 0) {
    return RESTART_HINT;
  }
...
function configApplyHintForOperations(
  ...
  beforeConfig: NodoAssistConfig,
  afterConfig: NodoAssistConfig,
): string {
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("nodoassist plugins install <spec>")}`,
    `  ${formatCliCommand("nodoassist plugins update <plugin-id>")}`,
    `  ${formatCliCommand("nodoassist plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
function formatAutoManagedMetaError(paths: readonly PathSegment[][]): string {
  ...
  return [
    `${subject} is auto-managed by NodoAssist and cannot be edited; the value would be overwritten on the next config write.`,
    "",
    "These fields are stamped on every config write to record the NodoAssist version and timestamp that produced the file.",
  ].join("\n");
}
...
function collectDryRunSchemaErrors(params: { config: NodoAssistConfig }): ConfigSetDryRunError[] {
...
async function runConfigOperations(params: {
  ...
  const currentConfigForApplyHint = normalizeConfigMutationModelRefs(
    structuredClone(snapshot.resolved) as NodoAssistConfig,
  );
  ...
  const nextConfig = normalizeConfigMutationModelRefs(next as NodoAssistConfig);
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("nodoassist config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    const hint = configApplyHintForOperations(
      ...
      normalizeConfigMutationModelRefs(structuredClone(next) as NodoAssistConfig),
    );
...
async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "nodoassist.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("nodoassist onboard")} or run ${formatCliCommand("nodoassist doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(danger(`NodoAssist config is invalid: ${shortPath}`));
        ...
        runtime.error(`Inspect with ${formatCliCommand("nodoassist config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing nodoassist.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing nodoassist.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for nodoassist.json")
    .action(async () => {
