...
const DEFAULT_CLI_NAME = "nodoassist";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(nodoassist)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `nodoassist`. */
...
/** Replace a leading `nodoassist` command prefix with the active CLI name. */
