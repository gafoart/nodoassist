...
function loadRuntimeKindReportForPlugins(config: NodoAssistConfig, pluginIds: readonly string[]) {
...
export function applySlotSelectionForPlugin(
  config: NodoAssistConfig,
  pluginId: string,
): { config: NodoAssistConfig; warnings: string[] } {
...
export function formatPluginInstallWithHookFallbackError(
  ...
  if (hookFallback.code === HOOK_INSTALL_ERROR_CODE.MISSING_NODOASSIST_HOOKS) {
    return formattedPluginError;
  }
