...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "NODOASSIST_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# NodoAssist Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# NodoAssist Completion\n${sourceLine}`;
  const next = trimmed ? `${trimmed}\n\n${block}\n` : `${block}\n`;
  return { next, changed: next !== content, hadExisting };
}

/** Resolves the shell startup profile path that should contain the NodoAssist completion block. */
...
/** Returns whether a shell profile already contains an NodoAssist completion block or source line. */
 ...
 * Returns true if profile has `source <(nodoassist completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "nodoassist") {
