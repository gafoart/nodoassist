...
const loadConfig = vi.fn<() => NodoAssistConfig>(() => ({
  gateway: {
    auth: {
      token: "config-token",
    },
...
function stubServiceGatewayTokenEnv() {
  service.readCommand.mockResolvedValue({
    programArguments: [],
    environment: {
      NODOASSIST_GATEWAY_TOKEN: "service-token",
...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { NODOASSIST_GATEWAY_TOKEN: "service-token" },
    });
    stubEmptyGatewayEnv();
  });
    ...
    await withUnsupportedGatewayService(async (unsupportedService) => {
      await expect(
        runServiceStart({
          ...
          renderStartHints: () => ["nodoassist gateway install"],
          opts: { json: true },
          onNotLoaded,
        }),
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("NODOASSIST_CONTAINER_HINT", "nodoassist-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for nodoassist-demo-container.",
    );
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: [],
      environment: { NODOASSIST_GATEWAY_TOKEN: "env-token" },
    });
  ...
  it("repairs stale loaded services during start before reporting success", async () => {
    service.readCommand.mockResolvedValue({
      programArguments: ["nodoassist", "gateway"],
    ...
    const repairLoadedService = vi.fn<RepairLoadedService>(async (ctx) => {
      ...
      return {
        ...
        warnings: ["service was installed by NodoAssist 2026.4.24, current CLI is 2026.5.2"],
        loaded: true,
      };
    });
    ...
    expect(payload.warnings).toEqual(
      expect.arrayContaining([
        expect.stringContaining("service was installed by NodoAssist"),
  ...
  it("fails start with an install hint when a stale loaded service has no repair callback", async () => {
    ...
    expect(payload.hints).toEqual(["nodoassist gateway install --force"]);
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    expect(payload.hints?.includes("nodoassist gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "nodoassist gateway install",
      ),
    ).toBe(true);
    expect(service.restart).not.toHaveBeenCalled();
  });
