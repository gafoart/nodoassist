  ...
  vi.fn(
    async (params: {
      ...
      return {
        ...
        workingDirectory: "/tmp/nodoassist",
        environment: {
...
const resolveNodoAssistWrapperPathMock = vi.hoisted(() => vi.fn());
...
vi.mock("../../daemon/program-args.js", () => ({
  ...
  resolveNodoAssistWrapperPath: resolveNodoAssistWrapperPathMock,
}));
...
describe("repairLoadedGatewayServiceForStart", () => {
  beforeEach(() => {
    ...
    resolveNodoAssistWrapperPathMock.mockReset();
    ...
    resolveNodoAssistWrapperPathMock.mockResolvedValue("/usr/bin/nodoassist");
  ...
  it("forwards existing env value-source metadata when repairing stale service definitions", async () => {
    ...
    const existingEnvironmentValueSources = {
      NODOASSIST_SERVICE_VERSION: "inline" as const,
