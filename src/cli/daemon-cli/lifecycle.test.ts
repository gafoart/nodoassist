...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["NODOASSIST_CONTAINER_HINT", "NODOASSIST_PROFILE"]);
    delete process.env.NODOASSIST_CONTAINER_HINT;
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["nodoassist", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("repairs stale loaded service definitions from gateway start", async () => {
    ...
    runServiceStart.mockImplementation(
      async (params: {
        ...
        await params.repairLoadedService?.({
          json: true,
          stdout: process.stdout,
          state: { command: { environment: { NODOASSIST_SERVICE_VERSION: "2026.4.24" } } },
    ...
    expect(repairParams.state?.command?.environment).toEqual({
      NODOASSIST_SERVICE_VERSION: "2026.4.24",
    });
  ...
  it("does not fall back to unmanaged restart when launchd repair reports headless GUI bootstrap failure", async () => {
    ...
    recoverInstalledLaunchAgent.mockRejectedValue(
      new Error("LaunchAgent nodoassist gateway restart requires a logged-in macOS GUI session"),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
  ...
  it("delegates system-scope restart to systemctl without unmanaged signaling when root (nodoassist#87577)", async () => {
  ...
  it("surfaces systemd sudo guidance and never signals when restarting a system-scope unit as non-root (nodoassist#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "nodoassist.service",
    ...
    restartSystemdService.mockRejectedValue(
      new Error(
        "nodoassist.service is a system-scope unit (/etc/systemd/system/nodoassist.service); run `sudo systemctl restart nodoassist.service` to restart it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    mockUnmanagedRestart();
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toThrow(
      /sudo systemctl restart nodoassist\.service/,
    );
  ...
  it("delegates system-scope stop to systemctl without unmanaged signaling when root (nodoassist#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "nodoassist-gateway.service",
  ...
  it("surfaces systemd sudo guidance and never signals when stopping a system-scope unit as non-root (nodoassist#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "nodoassist-gateway.service",
    ...
    stopSystemdService.mockRejectedValue(
      new Error(
        "nodoassist-gateway.service is a system-scope unit (/etc/systemd/system/nodoassist-gateway.service); run `sudo systemctl stop nodoassist-gateway.service` to stop it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    runServiceStop.mockImplementation(async (params: { onNotLoaded?: () => Promise<unknown> }) => {
