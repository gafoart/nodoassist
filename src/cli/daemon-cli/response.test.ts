...
describe("buildDaemonHintItems", () => {
  it("classifies common daemon hint kinds", () => {
    expect(
      ...
      { kind: "install", text: "nodoassist gateway install" },
      {
        kind: "container-restart",
        text: "Restart the container or the service that manages it for nodoassist-demo-container.",
      },
      {
      ...
      },
      {
        ...
        text: "If you're in a container, run the gateway in the foreground instead of `nodoassist gateway`.",
      },
      {
        kind: "wsl-systemd",
