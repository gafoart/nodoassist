...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("nodoassist-daemon-install-int-");
    configPath = path.join(tempHome, "nodoassist.json");
    process.env.HOME = tempHome;
    process.env.NODOASSIST_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer NodoAssist", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.NODOASSIST_GATEWAY_TOKEN).toBeUndefined();
  });
});
