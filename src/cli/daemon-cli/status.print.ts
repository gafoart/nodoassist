...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("nodoassist doctor")}" (or "${formatCliCommand("nodoassist doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("nodoassist gateway install --force")}\` from the same --profile / NODOASSIST_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this NodoAssist command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `nodoassist --version`, `which nodoassist`, and `nodoassist gateway status --deep`; if this mismatch is unexpected, update PATH so `nodoassist` points to the version you want, or reinstall the Gateway service from that same NodoAssist install.",
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("nodoassist gateway restart")} and re-check with ${formatCliCommand("nodoassist gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale NodoAssist client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.NODOASSIST_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("nodoassist gateway install")}`),
    );
    spacer();
  }

  if (service.staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale NodoAssist updater launchd job(s) detected."));
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("nodoassist gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.NODOASSIST_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      ...
      if (updateCommands.length === 1) {
        defaultRuntime.log(
          `${label("Fix:")} ${updateCommands[0]} && ${formatCliCommand("nodoassist gateway restart")}.`,
        );
      } else {
        ...
        defaultRuntime.log(`Then run ${formatCliCommand("nodoassist gateway restart")}.`);
      }
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("nodoassist gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("nodoassist status")}`);
