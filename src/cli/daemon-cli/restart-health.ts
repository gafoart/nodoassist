...
async function confirmGatewayReachable(params: {
  ...
  const token = normalizeOptionalString(params.auth?.token ?? process.env.NODOASSIST_GATEWAY_TOKEN);
  const password = normalizeOptionalString(
    params.auth?.password ?? process.env.NODOASSIST_GATEWAY_PASSWORD,
  );
