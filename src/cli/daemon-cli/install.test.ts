...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: {
    ...
    if (params?.wrapperPath || params?.env?.NODOASSIST_WRAPPER) {
      environment.NODOASSIST_WRAPPER = params.wrapperPath ?? params.env?.NODOASSIST_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/nodoassist.json" },
  })),
}));
...
function mockResolvedGatewayTokenSecretRef() {
  resolveSecretInputRefMock.mockReturnValue({
    ref: { source: "env", provider: "default", id: "NODOASSIST_GATEWAY_TOKEN" },
  });
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:NODOASSIST_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${NODOASSIST_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds NODOASSIST_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["nodoassist", "gateway", "run"],
      environment: {
        NODOASSIST_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service NODOASSIST_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["nodoassist", "gateway", "run"],
      environment: {
        NODOASSIST_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
    buildGatewayInstallPlanMock.mockResolvedValueOnce({
      programArguments: ["nodoassist", "gateway", "run"],
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/nodoassist-doppler", "gateway", "run"],
      environment: {
    ...
    expectFields(installPlanArg, { wrapperPath: "/usr/local/bin/nodoassist-doppler" });
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/nodoassist-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service NODOASSIST_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when NODOASSIST_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["nodoassist", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["nodoassist", "gateway", "run"],
      environment: {
        NODOASSIST_STATE_DIR: "/tmp/nodoassist-doctor-manual",
        NODOASSIST_CONFIG_PATH: "/tmp/nodoassist-doctor-manual/nodoassist.json",
        NODOASSIST_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.NODOASSIST_STATE_DIR).toBeUndefined();
      expect(env.NODOASSIST_CONFIG_PATH).toBeUndefined();
