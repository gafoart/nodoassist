...
vi.mock("../../daemon/restart-logs.js", () => ({
  ...
  resolveGatewaySupervisorLogPaths: () => ({
    logDir: "/Users/test/Library/Logs/nodoassist",
    stdoutPath: "/Users/test/Library/Logs/nodoassist/gateway.log",
    stderrPath: "/Users/test/Library/Logs/nodoassist/gateway.err.log",
  }),
...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expectMockLineContains(runtime.error, formatCliCommand("nodoassist gateway restart"));
  });
  ...
  it("prints established gateway client guidance gathered by deep status", () => {
    ...
    expectMockLineContains(runtime.log, "newer-nodoassist");
  ...
  it("prints stale updater launchd job guidance", () => {
    ...
    expectMockLineContains(runtime.error, "Stale NodoAssist updater launchd job(s) detected.");
    expectMockLineContains(runtime.error, "ai.nodoassist.update.2026.5.12");
    expectMockLineContains(runtime.error, "ai.nodoassist.manual-update.1717168800");
  ...
  it("prints macOS launchd stdout and suppressed stderr when gateway is not listening", () => {
    ...
    expectMockLineContains(runtime.error, "/Users/test/Library/Logs/nodoassist/gateway.log");
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.4.23",
          entrypoint: "/usr/local/bin/nodoassist",
        },
        service: {
          label: "LaunchAgent",
          loaded: true,
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/nodoassist)");
    expectMockLineContains(runtime.log, "Gateway version: 2026.5.6");
    expectMockLineContains(runtime.error, "this NodoAssist command is version 2026.4.23");
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `nodoassist` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/nodoassist-cli/nodoassist.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/nodoassist-daemon/nodoassist.json",
            exists: true,
            valid: true,
    ...
    printDaemonStatus(
      {
        ...
        extraServices: [{ label: "ai.nodoassist.gateway.rescue", scope: "user", detail: "loaded" }],
      },
      { json: false },
    );

    expectMockLineContains(runtime.log, "Other gateway-like services detected");
    expectMockLineContains(runtime.log, "ai.nodoassist.gateway.rescue");
  ...
  it("prints a terse plugin drift warning outside deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "nodoassist gateway status --deep");
  ...
  it("prints detailed plugin drift entries in deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "nodoassist plugins update whatsapp");
    expectMockLineContains(runtime.log, "nodoassist gateway restart");
  });
  ...
  it("prints exact package update commands for pinned npm plugin drift in deep mode", () => {
          ...
          drifts: [
            {
              ...
              gatewayVersion: "2026.6.10-beta.1",
              source: "npm",
              packageName: "@nodoassist/brave-plugin",
              spec: "@nodoassist/brave-plugin@2026.6.9",
            },
          ],
        },
        extraServices: [],
    ...
    expectMockLineContains(
      runtime.log,
      "nodoassist plugins update @nodoassist/brave-plugin@2026.6.10-beta.1",
    );
    expectMockLineContains(runtime.log, "nodoassist gateway restart");
  });
