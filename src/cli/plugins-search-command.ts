...
/** Options accepted by `nodoassist plugins search`. */
...
function formatPackageSearchLine(entry: ClawHubPackageSearchResult): string {
  ...
  return `${pkg.name}  ${theme.muted(flags.join(" | "))}${summary}\n  ${theme.muted(`Install: nodoassist plugins install clawhub:${pkg.name}`)}`;
}
...
export async function runPluginsSearchCommand(
  ...
  if (!query) {
    runtime.error("Usage: nodoassist plugins search <query>");
