...
export function registerNodeCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "nodoassist node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["nodoassist node status", "Check node host service status."],
          ["nodoassist node install", "Install the node host service."],
          ["nodoassist node start", "Start the installed node host service."],
          ["nodoassist node restart", "Restart the installed node host service."],
  ...
  node
    ...
    .option("--context-path <path>", "Gateway WebSocket context path (e.g. /nodoassist-gw)")
