...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nodoassist channels list", "List configured channels."],
          ["nodoassist channels list --all", "Show configured, bundled, and installable channels."],
          ["nodoassist channels add", "Open guided channel setup."],
          ["nodoassist channels status --probe", "Run channel status checks and probes."],
          [
            "nodoassist channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["nodoassist channels login --channel whatsapp", "Link a WhatsApp Web account."],
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nodoassist channels add", "Open guided setup for available chat channels."],
          [
            "nodoassist channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["nodoassist channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
