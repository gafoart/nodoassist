...
const NODOASSIST_REPO_URL = "https://github.com/openclaw/openclaw.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "nodoassist";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);

/** Normalize a CLI tag/version/spec into the npm target form accepted by update flows. */
export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["nodoassist", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.NODOASSIST_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "nodoassist");
  }
...
/** Locate the installed NodoAssist package root that should receive update operations. */
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveNodoAssistPackageRoot({
...
/** Ensure the configured source-update directory exists and points at an NodoAssist checkout. */
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    ...
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", NODOASSIST_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `NODOASSIST_GIT_DIR points at a non-git directory: ${params.dir}. Set NODOASSIST_GIT_DIR to an empty folder or an nodoassist checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`NODOASSIST_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }

  return null;
}

/** Detect the package manager that owns a global/package NodoAssist install. */
...
const COMPLETION_CACHE_MANUAL_REFRESH_HINT =
  "Shell tab-completion may be stale; refresh manually with: nodoassist completion --write-state";

/** Best-effort refresh of shell completion state after a successful update. */
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "nodoassist.mjs");
