...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-NodoAssistScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-NodoAssistSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "nodoassist restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-NodoAssistListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-NodoAssistSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'nodoassist-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `nodoassist-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("nodoassist-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^nodoassist-restart-/);
    ...
    it("uses NODOASSIST_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        NODOASSIST_PROFILE: "default",
        NODOASSIST_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("nodoassist-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        NODOASSIST_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        NODOASSIST_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped nodoassist gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart nodoassist-gateway.service");
      expect(calls).toContain("--user is-active --quiet nodoassist-gateway.service");
      expect(calls).toContain("is-active --quiet nodoassist-gateway.service");
      expect(calls).not.toContain("--user restart nodoassist-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.nodoassist.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.nodoassist.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.nodoassist/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain(
        "exec >>'/Users/testuser/.nodoassist/logs/gateway-restart.log' 2>&1",
      );
    ...
    it("uses NODOASSIST_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/nodoassist-state/logs' 2>/dev/null && : >>'/tmp/nodoassist-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/nodoassist-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain(
        "nodoassist restart attempt source=update target=ai.nodoassist.gateway",
      );
      expect(log).toContain("launchctl kickstart -k gui/501/ai.nodoassist.gateway");
      expect(log).toContain("nodoassist restart failed source=update status=42");
      expect(log).not.toContain("nodoassist restart done source=update");
    });
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        NODOASSIST_PROFILE: "default",
        HOME: path.join(tmpDir, "home"),
        NODOASSIST_STATE_DIR: stateFile,
      });
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        NODOASSIST_LAUNCHD_LABEL: "ai.nodoassist.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        NODOASSIST_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.nodoassist.$(echo injected)");
      expect(log).not.toContain("target=ai.nodoassist.injected");
    });

    it("uses NODOASSIST_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        NODOASSIST_PROFILE: "default",
        NODOASSIST_LAUNCHD_LABEL: "com.custom.nodoassist",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.nodoassist'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'NodoAssist Gateway'");
      expect(content).toContain("function Invoke-NodoAssistSchtasksWithTimeout");
      expect(content).toContain("function Get-NodoAssistScheduledTaskState");
      expect(content).toContain("function Invoke-NodoAssistStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("nodoassist restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".nodoassist\\gateway.cmd"',
      );
      expect(content).toContain("nodoassist restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%NODOASSIST_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses NODOASSIST_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        NODOASSIST_PROFILE: "default",
        NODOASSIST_WINDOWS_TASK_NAME: "NodoAssist Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'NodoAssist Gateway (custom)'");
      expect(content).toContain("Get-NodoAssistScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-NodoAssistSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-NodoAssistStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        NODOASSIST_PROFILE: "production",
      });
      expect(content).toContain("nodoassist-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        NODOASSIST_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.nodoassist.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'NodoAssist Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("escapes single quotes in profile names for shell scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        NODOASSIST_PROFILE: "it's-a-test",
      });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        NODOASSIST_LAUNCHD_LABEL: "ai.nodoassist.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.nodoassist.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
    ...
    it("rejects unsafe batch profile names on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });
      const scriptPath = await prepareRestartScript({
        NODOASSIST_PROFILE: "test&whoami",
      });
