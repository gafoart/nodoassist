...
import { disableCurrentNodoAssistUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const POST_CORE_UPDATE_ENV = "NODOASSIST_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "NODOASSIST_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "NODOASSIST_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "NODOASSIST_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV =
  "NODOASSIST_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_STARTED_AT_ENV = "NODOASSIST_UPDATE_POST_CORE_STARTED_AT_MS";
...
function restoreDroppedPreUpdateChannels(
  ...
  const authoredChannels = resolveRestoredAuthoredChannels({
    ...
    currentAuthoredChannels: isRecord(snapshot.parsed)
      ? (snapshot.parsed as NodoAssistConfig).channels
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run nodoassist plugins inspect ${pluginId} --runtime --json for details.`;
}
...
export function buildInvalidConfigPostCoreUpdateResult(): {
  ...
  const guidance = [
    "Run `nodoassist doctor` to inspect the config validation errors.",
    "Once the config parses, rerun `nodoassist update repair`.",
  ];
  const message =
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("nodoassist gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("nodoassist gateway install --force"),
    CLI_NAME,
  );
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall NodoAssist ${beforeVersion} with the same package manager, then rerun \`${replaceCliName(formatCliCommand("nodoassist gateway install --force"), CLI_NAME)}\`.`,
    );
  }
  return lines;
}
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `nodoassist update detected it is running inside the gateway process tree.
Gateway PID ${pid} is an ancestor of this process, so this updater cannot safely stop or restart the gateway that owns it.
Run \`${replaceCliName(formatCliCommand("nodoassist update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`;
}
...
async function maybeStopManagedServiceBeforeMutableUpdate(params: {
  ...
  if (serviceMatchesMutationRoot === false) {
    if (!params.jsonMode) {
      defaultRuntime.log(
        theme.muted(
          `Managed gateway service points at a different NodoAssist root; leaving it running during this ${params.updateInstallKind} update.`,
        ),
      );
    }
    return {
...
function isRunningInsideGatewayService(
  ...
  if (env.NODOASSIST_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.NODOASSIST_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `${runtimeLabel} is too old for nodoassist@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    runtime.nodeRunner
      ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `nodoassist update`."
      : "Upgrade to Node 22.19 or newer 22.x, Node 23.11+, or Node 24+, then rerun `nodoassist update`.",
    "Bare `npm i -g nodoassist` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g nodoassist@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.NODOASSIST_SERVICE_MARKER;
  delete resolvedEnv.NODOASSIST_SERVICE_KIND;
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("nodoassist completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
        ...
        const doctorStep = await runUpdateStep({
          ...
          env: {
            ...
            ...(doctorPolicy.serviceRepairPolicy
              ? { NODOASSIST_SERVICE_REPAIR_POLICY: doctorPolicy.serviceRepairPolicy }
              : {}),
            [UPDATE_POST_INSTALL_DOCTOR_RESULT_PATH_ENV]: doctorResultPath,
            ...(candidateHostVersion === null
              ? {}
              : { NODOASSIST_COMPATIBILITY_HOST_VERSION: candidateHostVersion }),
          },
          timeoutMs: params.timeoutMs,
        });
...
export async function updatePluginsAfterCoreUpdate(params: {
  ...
  if (pluginsChanged) {
    ...
    if (params.restoredAuthoredChannels !== undefined) {
      nextConfig = {
        ...nextConfig,
        channels: structuredClone(params.restoredAuthoredChannels) as NodoAssistConfig["channels"],
      };
    }
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("nodoassist gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("nodoassist gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("nodoassist doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("nodoassist gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("nodoassist gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
function withUpdateFinalizationEnv<T>(run: () => Promise<T>): Promise<T> {
  const previousUpdateInProgress = process.env.NODOASSIST_UPDATE_IN_PROGRESS;
  ...
  return run().finally(() => {
    if (previousUpdateInProgress === undefined) {
      ...
      process.env.NODOASSIST_UPDATE_IN_PROGRESS = previousUpdateInProgress;
    }
    ...
    (configSnapshot.valid
      ? {
          ...
          authoredConfig: isRecord(configSnapshot.parsed)
            ? (configSnapshot.parsed as NodoAssistConfig)
...
function normalizePreUpdateConfigRestoreInput(
  ...
  if (isRecord(sourceConfig) && isRecord(authoredConfig)) {
    return {
      sourceConfig: sourceConfig as NodoAssistConfig,
      authoredConfig: authoredConfig as NodoAssistConfig,
    };
  }
  const authored = parsed as NodoAssistConfig;
  return {
...
function resolvePreUpdateSourceConfigFromAuthored(
  ...
  try {
    ...
    return isRecord(resolved) ? (resolved as NodoAssistConfig) : authoredConfig;
  } catch {
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-update-post-core-"));
    ...
    const child = spawn(params.nodeRunner ?? resolveNodeRunner(), argv, {
      ...
      env: {
        ...
        ...(postCoreHostVersion === null
          ? {}
          : { NODOASSIST_COMPATIBILITY_HOST_VERSION: postCoreHostVersion }),
...
async function updateCommandInternal(
  ...
  if (opts.dryRun !== true) {
    try {
      ...
      await disableCurrentNodoAssistUpdateLaunchdJob().catch(() => undefined);
      throw err;
    }
  }
  ...
  if (postCoreUpdateResume) {
    ...
    process.env.NODOASSIST_COMPATIBILITY_HOST_VERSION = (await readPackageVersion(root)) ?? VERSION;
  ...
  const devTargetRef =
    channel === "dev"
      ? process.env.NODOASSIST_UPDATE_DEV_TARGET_REF?.trim() || undefined
      : undefined;
  ...
  if (updateInstallKind === "package") {
    ...
    if (managedServiceRootRedirect) {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.warn(
            `Shell NodoAssist root differs from the managed gateway service root: ${managedServiceRootRedirect.previousRoot}`,
          ),
        );
        defaultRuntime.log(
          theme.muted(
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating NodoAssist..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `nodoassist update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this NodoAssist install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("nodoassist doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("nodoassist gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g nodoassist@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g nodoassist@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (!pluginsUpdatedInFreshProcess) {
    ...
    const previousCompatibilityHostVersion = process.env.NODOASSIST_COMPATIBILITY_HOST_VERSION;
    if (compatibilityDowngradeTarget) {
      process.env.NODOASSIST_COMPATIBILITY_HOST_VERSION = compatibilityDowngradeTarget;
    }
    try {
      ...
      if (compatibilityDowngradeTarget) {
        if (previousCompatibilityHostVersion === undefined) {
          ...
          process.env.NODOASSIST_COMPATIBILITY_HOST_VERSION = previousCompatibilityHostVersion;
        }
      }
    }
  }
