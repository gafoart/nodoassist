...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-payload-smoke-"));
  });
  afterEach(async () => {
  ...
  function resolveTestHostRoot(): string {
    const hostRoot = resolveNodoAssistPackageRootSync({
  ...
  async function linkNodoAssistPeerToHost(dir: string): Promise<void> {
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(
      resolveTestHostRoot(),
      path.join(dir, "node_modules", "nodoassist"),
      "junction",
    );
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@nodoassist/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@nodoassist/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (NodoAssist plugins commonly use `exports` or `nodoassist.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@nodoassist/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares an existing `nodoassist.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("reports a failure when `nodoassist.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@nodoassist/brave-plugin",
      nodoassist: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json nodoassist.extensions[1] must be a non-empty string",
      },
    ]);
  });
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json nodoassist.extensions is empty",
      },
    ]);
  });
  ...
  it("accepts a packaged TypeScript extension entry when compiled runtime output exists", async () => {
    ...
    await writePackage(dir, {
      ...
      nodoassist: { extensions: ["./index.ts"] },
    });
  ...
  it("reports a failure when an nodoassist peer link is missing", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
        name: "@nodoassist/codex",
        main: "dist/index.js",
        peerDependencies: { nodoassist: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        pluginId: "codex",
        installPath: dir,
        reason: "missing-nodoassist-peer-link",
        detail: `Plugin declares peerDependency "nodoassist" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
          "nodoassist",
        )}.`,
  ...
  it("reports a failure when an nodoassist peer link is a stale real directory", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "nodoassist");
  ...
  it("reports a failure when an nodoassist peer link points at the wrong package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    const wrongHostRoot = path.join(tmpRoot, "old-nodoassist");
    await fs.mkdir(wrongHostRoot, { recursive: true });
    await fs.mkdir(path.join(dir, "node_modules"), { recursive: true });
    await fs.symlink(wrongHostRoot, path.join(dir, "node_modules", "nodoassist"), "junction");
  ...
  it("accepts an nodoassist peer link when it resolves to the host package root", async () => {
    const dir = path.join(tmpRoot, "codex");
    await writePackage(
      dir,
      {
    ...
    await linkNodoAssistPeerToHost(dir);
  ...
  it("reports a failure when an `nodoassist.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@nodoassist/brave-plugin",
