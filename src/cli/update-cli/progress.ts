...
const STEP_LABELS: Record<string, string> = {
  ...
  "nodoassist doctor entry": "Checking doctor entrypoint",
  "nodoassist doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g nodoassist@latest");
    hints.push(
      "System install outline: nodoassist gateway stop -> sudo <system-npm> i -g nodoassist@latest -> nodoassist gateway install --force -> nodoassist gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g nodoassist@latest --omit=optional");
  }
