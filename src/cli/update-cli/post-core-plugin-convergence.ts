...
import { relinkNodoAssistPeerDependenciesInManagedNpmRoot } from "../../plugins/plugin-peer-link.js";
...
const REPAIR_GUIDANCE = "Run `nodoassist update repair` to retry plugin repair.";
const inspectGuidance = (pluginId: string) =>
  `Run \`nodoassist plugins inspect ${pluginId} --runtime --json\` for details.`;

async function repairManagedNpmNodoAssistPeerLinks(params: {
  ...
  try {
    ...
    const message = `Failed to repair managed npm NodoAssist host peer links: ${err instanceof Error ? err.message : String(err)}`;
    return {
      changes: [],
      warnings: [
        {
...
export async function runPostCorePluginConvergence(params: {
  ...
  const peerLinkRepair = await repairManagedNpmNodoAssistPeerLinks({ env });
