...
import { repoInstallSpec } from "nodoassist/plugin-sdk/test-fixtures";
...
beforeEach(() => {
  ...
  originalHideBanner = process.env.NODOASSIST_HIDE_BANNER;
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.NODOASSIST_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const programLocal = new Command().name("nodoassist");
    programLocal
      .command("agent")
  ...
  it("handles debug mode and config-only command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "nodoassist", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("nodoassist-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "nodoassist", "agents", "list"],
    });
  ...
  it("passes the gateway config recheck to the state migration boundary", async () => {
    await runPreAction({
      parseArgv: ["gateway", "run"],
      processArgv: ["node", "nodoassist", "gateway", "run"],
    });
  ...
  it("loads plugins for text local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "nodoassist", "agent", "--local", "--message", "hi"],
    });
  ...
  it("loads plugins for json local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "nodoassist", "agent", "--local", "--message", "hi", "--json"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "nodoassist", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "channels", "add"],
    });
  ...
  it("skips startup bootstrap for parent default help actions", async () => {
    await runPreAction({
      parseArgv: ["channels"],
      processArgv: ["node", "nodoassist", "channels"],
    });
  ...
  it("lets configure own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["configure"],
      processArgv: ["node", "nodoassist", "configure"],
    });
  ...
  it("lets bare config own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "nodoassist", "config"],
    });
  ...
  it("lets guided config sections own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "nodoassist", "config", "--section", "models"],
    });
  ...
  it("skips the config guard and plugin loading for doctor lint", async () => {
    await runPreAction({
      parseArgv: ["doctor"],
      processArgv: ["node", "nodoassist", "doctor", "--lint"],
    });
  ...
  it("only allows invalid config for explicit official recovery reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@nodoassist/discord"],
      processArgv: ["node", "nodoassist", "plugins", "install", "@nodoassist/discord"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@nodoassist/discord@2026.5.22"],
      processArgv: ["node", "nodoassist", "plugins", "install", "@nodoassist/discord@2026.5.22"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@nodoassist/brave-plugin"],
      processArgv: ["node", "nodoassist", "plugins", "install", "@nodoassist/brave-plugin"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@nodoassist/slack"],
      processArgv: ["node", "nodoassist", "plugins", "install", "@nodoassist/slack"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "plugins", "install", DISCORD_REPO_INSTALL_SPEC],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@nodoassist/discord", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "nodoassist",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "nodoassist", "--version"],
    });
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "nodoassist", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "nodoassist", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "channels", "send", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
  ...
  it("uses the Commander action path for protocol stdout ownership", async () => {
    await runPreAction({
      parseArgv: ["acp"],
      processArgv: ["node", "nodoassist", "acp", "--token", "-secret"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "acp", "--verbose", "client"],
    });
    ...
    await runPreAction({
      parseArgv: ["mcp", "serve"],
      processArgv: ["node", "nodoassist", "mcp", "serve"],
    });
  ...
  it("does not preload plugins for agents list JSON output", async () => {
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "nodoassist", "agents", "list", "--json"],
    });
  ...
  it("does not preload plugins for remote agent JSON output", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "nodoassist", "agent", "--message", "hi", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "nodoassist", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "nodoassist", "backup", "create", "--json"],
    });
