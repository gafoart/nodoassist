...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.NODOASSIST_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.NODOASSIST_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.NODOASSIST_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.NODOASSIST_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "nodoassist", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "nodoassist"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "nodoassist", "nodes", "list"], "nodoassist");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "nodoassist", "infer"], "nodoassist");
  ...
  it("registers the exec-approvals placeholder and dispatches through the approvals registrar", async () => {
    const program = createRegisteredProgram(["node", "nodoassist", "exec-approvals"], "nodoassist");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "nodoassist", "acp", "--help"], "nodoassist");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "nodoassist", "gateway", "--force"];
    process.argv = argv;
    const program = new Command().name("nodoassist");
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "nodoassist", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("nodoassist");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "nodoassist", "completion", "--write-state"];
    const program = new Command().name("nodoassist");
  ...
  it.each([
    ...
    process.argv = ["node", "nodoassist", ...args];
    const program = new Command().name("nodoassist");
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "nodoassist", "plugins"];
    const program = new Command().name("nodoassist");
