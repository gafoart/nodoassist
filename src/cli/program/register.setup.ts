...
export function registerSetupCommand(program: Command): void {
  const command = program
    .command("setup")
    .description("Alias for nodoassist onboard")
    .addHelpText(
      "after",
      () =>
        `\n${theme.heading("Examples:")}\n` +
        `  ${theme.command("nodoassist setup")}\n` +
        `    ${theme.muted("Run full onboarding for auth, models, Gateway, and channels.")}\n\n` +
        `${theme.muted("Docs:")} ${formatDocsLink("/cli/setup", "docs.openclaw.ai/cli/setup")}\n`,
    )
    .option(
      "--workspace <dir>",
      "Agent workspace directory (default: ~/.nodoassist/workspace; stored as agents.defaults.workspace)",
    )
