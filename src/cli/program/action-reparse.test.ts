...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "nodoassist", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    setRawArgs(program, ["node", "nodoassist", "status", "--json"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      ...
      rawArgs: ["node", "nodoassist", "status", "--json"],
      fallbackArgv: ["status", "--json"],
    });
    expect(parseAsync).toHaveBeenCalledWith(["node", "nodoassist", "status"]);
  });

  it("falls back to action args without command name when action has no name", async () => {
    const program = new Command().name("nodoassist");
    setRawArgs(program, ["node", "nodoassist"]);
    const parseAsync = vi.spyOn(program, "parseAsync").mockResolvedValue(program);
    const actionCommand = {
      name: () => "",
      parent: program,
  ...
  it("uses root raw args and reparses the root for nested lazy commands", async () => {
    const root = new Command().name("nodoassist");
    setRawArgs(root, ["node", "nodoassist", "workspaces", "audit", "export", "--since", "1"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "nodoassist",
      rawArgs: ["node", "nodoassist", "workspaces", "audit", "export", "--since", "1"],
  ...
  it("hoists a trailing lazy-parent option before the loaded command", async () => {
    ...
    await expectReparseArgv({
      parent: browser,
      action: tabs,
      argv: ["node", "nodoassist", "browser", "tabs", "--browser-profile", "remote"],
      expected: ["node", "nodoassist", "browser", "--browser-profile", "remote", "tabs"],
    });
  });

  it("skips root option values that match the parent command name", async () => {
    const root = new Command().name("nodoassist").option("--profile <name>");
    const browser = root.command("browser").option("--browser-profile <name>");
    const tabs = browser.command("tabs");
    await expectReparseArgv({
      parent: browser,
  ...
  it("hoists parent options after nested lazy commands", async () => {
    ...
    await expectReparseArgv({
      parent: browser,
      action: tab,
      argv: ["node", "nodoassist", "browser", "tab", "new", "--browser-profile", "work"],
      expected: ["node", "nodoassist", "browser", "--browser-profile", "work", "tab", "new"],
    });
  });
  ...
  it("leaves a child-owned option collision after the child command", async () => {
    ...
    const argv = ["node", "nodoassist", "browser", "extension", "pair", "--json"];
  ...
  it("hoists a parent option when only a sibling command owns the same flag", async () => {
    ...
    await expectReparseArgv({
      parent: browser,
      action: cookies,
      argv: ["node", "nodoassist", "browser", "cookies", "list", "--url", "ws://gateway"],
      expected: ["node", "nodoassist", "browser", "--url", "ws://gateway", "cookies", "list"],
    });
  });
  ...
  it("keeps a missing parent option value after the loaded command", async () => {
    ...
    const browser = root.command("browser").option("--browser-profile <name>");
    const tabs = browser.command("tabs");
    const argv = ["node", "nodoassist", "browser", "tabs", "--browser-profile"];
