...
function hasLegacyExecApprovalsMigrationInput(stateDir: string): boolean {
  if (!process.env.NODOASSIST_STATE_DIR?.trim()) {
    return false;
  }
  const homeDir = resolveRequiredHomeDir(process.env, os.homedir);
  const sourcePath = path.join(homeDir, ".nodoassist", "exec-approvals.json");
...
function hasLegacyStateMigrationInputs(): boolean {
  ...
  if (
    !process.env.NODOASSIST_STATE_DIR?.trim() &&
...
export async function ensureConfigReady(params: {
  ...
  params.runtime.error(heading("NodoAssist config is invalid"));
  ...
  const fixHint = isPluginPackagingRuntimeOutputInvalidConfigSnapshot(snapshot)
    ? formatPluginPackagingRuntimeOutputRecoveryHint()
    : commandText(formatCliCommand("nodoassist doctor --fix"));
  params.runtime.error(`${muted("Fix:")} ${fixHint}`);
  params.runtime.error(
    `${muted("Inspect:")} ${commandText(formatCliCommand("nodoassist config validate"))}`,
  );
  params.runtime.error(
    muted(
