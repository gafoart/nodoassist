...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/nodoassist-stability.json",
  path: "/tmp/nodoassist-stability.json",
}));
...
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/nodoassist-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/nodoassist-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/nodoassist-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/nodoassist-control-ui";
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ NODOASSIST_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
          shellEnv: { enabled: true, timeoutMs: 1234 },
          vars: { NODOASSIST_GATEWAY_TOKEN: "config-token" },
        },
        gateway: {
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            NODOASSIST_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.NODOASSIST_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["NODOASSIST_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { NODOASSIST_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["NODOASSIST_GATEWAY_TOKEN"]);
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              ...
              NODOASSIST_SHELL_ENV_TIMEOUT_MS: "4321",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        };
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.NODOASSIST_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.NODOASSIST_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.NODOASSIST_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.NODOASSIST_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              NODOASSIST_DEFER_SHELL_ENV_FALLBACK: "1",
              NODOASSIST_LOAD_SHELL_ENV: "1",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.NODOASSIST_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { NODOASSIST_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("rejects an invalid final config after a prepared config selected runtime paths", async () => {
    const selectedStateDir = "/tmp/nodoassist-prepared-selected-state";
    await withEnvAsync({ NODOASSIST_STATE_DIR: undefined }, async () => {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        NODOASSIST_GATEWAY_TOKEN: undefined,
        NODOASSIST_PROXY_URL: undefined,
        NODOASSIST_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              NODOASSIST_PROXY_URL: "http://127.0.0.1:19876",
        ...
        const newConfig = {
          env: { vars: { NODOASSIST_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.NODOASSIST_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.NODOASSIST_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.NODOASSIST_PROXY_URL).toBeUndefined();
        expect(process.env.NODOASSIST_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("marks descendants when the final config supplies the service marker", async () => {
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: { vars: { NODOASSIST_SERVICE_MARKER: "nodoassist" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: finalConfig,
        ...
        expect(process.env.NODOASSIST_SERVICE_MARKER).toBe("nodoassist");
        expect(process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV]).toBe(String(process.pid));
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.NODOASSIST_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.NODOASSIST_HOME).toBeUndefined();
          expect(process.env.NODOASSIST_PROFILE).toBe("dev");
          expect(process.env.NODOASSIST_STATE_DIR).toBeUndefined();
          expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.NODOASSIST_WORKSPACE_DIR).toBe("/tmp/nodoassist-reset-workspace");
          configState.snapshot = {
            exists: true,
            valid: true,
            config: { gateway: { mode: "local" } },
        ...
        loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
          process.env.NODOASSIST_GATEWAY_TOKEN ??= "trusted-token";
          process.env.NODOASSIST_PROFILE ??= "dev";
          if (process.env.NODOASSIST_WORKSPACE_DIR === undefined) {
            setTestEnvValue("NODOASSIST_WORKSPACE_DIR", "/tmp/nodoassist-reset-workspace");
          }
        });
        ...
        expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("trusted-token");
  ...
  it("refuses dev reset if trusted dotenv retargets after pre-bootstrap", async () => {
    await withEnvAsync({ NODOASSIST_STATE_DIR: "/tmp/nodoassist-reset-original" }, async () => {
      configState.snapshot = {
        config: { gateway: { mode: "local" } },
        exists: true,
        path: "/tmp/nodoassist-reset-original/nodoassist.json",
        sourceConfig: { gateway: { mode: "local" } },
        valid: true,
      };
      await prepareGatewayReset();
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        setTestEnvValue("NODOASSIST_STATE_DIR", "/tmp/nodoassist-reset-retargeted");
        return {
          gatewayEnvAppliedKeys: [],
          stateEnvAppliedKeys: ["NODOASSIST_STATE_DIR"],
        };
      });
      ...
      expect(ensureDevGatewayConfig).not.toHaveBeenCalled();
      expect(process.env.NODOASSIST_STATE_DIR).toBe("/tmp/nodoassist-reset-original");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it.each([
    ...
    "NODOASSIST_INCLUDE_ROOTS",
    "NODOASSIST_NIX_MODE",
    "NODOASSIST_OAUTH_DIR",
    "NODOASSIST_PACKAGE_DIR",
    "NODOASSIST_PROFILE",
    ...
    await withEnvAsync({ [selector]: "/tmp/nodoassist-reset-value" }, async () => {
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        setTestEnvValue(selector, "/tmp/nodoassist-reset-retargeted");
      });
      ...
      expect(process.env[selector]).toBe("/tmp/nodoassist-reset-value");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it("blocks a final startup snapshot that changes guarded config selection", async () => {
    await withEnvAsync({ NODOASSIST_STATE_DIR: undefined }, async () => {
      configState.snapshot = {
        ...
        config: { gateway: { mode: "local" } },
        sourceConfig: {
          env: { vars: { NODOASSIST_STATE_DIR: "/tmp/nodoassist-late-selection" } },
          gateway: { mode: "local" },
        },
      };
  ...
  it("blocks a final startup snapshot that changes an already-selected config selector", async () => {
    await withEnvAsync({ NODOASSIST_STATE_DIR: undefined }, async () => {
      const guardedConfig = {
        env: { vars: { NODOASSIST_STATE_DIR: "/tmp/nodoassist-guarded-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
      ...
      await selectGatewayRunEnvironment({ opts: {}, runtime: defaultRuntime });
      await prepareGatewayRunBootstrap({ opts: {}, runtime: defaultRuntime });
      expect(process.env.NODOASSIST_STATE_DIR).toBe("/tmp/nodoassist-guarded-state");

      const finalConfig = {
        env: { vars: { NODOASSIST_STATE_DIR: "/tmp/nodoassist-final-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
        config: finalConfig,
  ...
  it.each([
    ...
    delete process.env.NODOASSIST_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.NODOASSIST_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `nodoassist onboard --mode local` or `nodoassist setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/nodoassist-test-missing-config.json",
      config: {},
      parsed: null,
