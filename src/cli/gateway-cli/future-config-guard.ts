...
type GatewayRunFutureConfigGuardParams = {
  ...
  config?: Pick<NodoAssistConfig, "env" | "meta"> | null;
};

function resolveGatewayRunFutureConfigBlock(params: GatewayRunFutureConfigGuardParams) {
  const processServiceMode = Boolean(process.env.NODOASSIST_SERVICE_MARKER?.trim());
  ...
  const candidateServiceMode =
    ...
    Boolean(
      candidateConfig
        ? createConfigRuntimeEnv(candidateConfig, process.env).NODOASSIST_SERVICE_MARKER?.trim()
        : undefined,
    );
