// Runtime implementations for `nodoassist plugins` subcommands. Heavy plugin modules stay
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "nodoassist"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "nodoassist doctor --fix" to install ${installSpec}, or install it manually with "nodoassist plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsEnableCommand(idInput: string): Promise<void> {
  ...
  const cfg = (snapshot.sourceConfig ?? snapshot.config) as NodoAssistConfig;
  ...
  let next: NodoAssistConfig = enableResult.config;
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(
      `${theme.muted("Repair:")} ${theme.command("nodoassist plugins registry --refresh")}`,
    );
  }
...
export async function runPluginsDoctorCommand(): Promise<void> {
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    nodoassist plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    nodoassist plugins registry --refresh");
      lines.push("    nodoassist gateway restart --force");
    }
  }
...
/** List entries from the configured NodoAssist marketplace feed. */
...
/** Refresh the configured NodoAssist marketplace feed snapshot. */
