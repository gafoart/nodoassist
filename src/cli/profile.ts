...
export function applyCliProfileEnv(params: {
  ...
  env.NODOASSIST_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.NODOASSIST_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.NODOASSIST_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.NODOASSIST_CONFIG_PATH)) {
    env.NODOASSIST_CONFIG_PATH = path.join(stateDir, "nodoassist.json");
  }

  if (profile === "dev" && !env.NODOASSIST_GATEWAY_PORT?.trim()) {
    env.NODOASSIST_GATEWAY_PORT = "19001";
  }
}
