...
describe("proxy cli runtime", () => {
  const envKeys = [
    ...
    "NODOASSIST_DEBUG_PROXY_CERT_DIR",
    "NODOASSIST_DEBUG_PROXY_SESSION_ID",
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "nodoassist-proxy-cli-runtime-"));
    process.env.NODOASSIST_STATE_DIR = tempDir;
    process.env.NODOASSIST_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.NODOASSIST_DEBUG_PROXY_ENABLED;
    delete process.env.NODOASSIST_DEBUG_PROXY_SESSION_ID;
  ...
  afterEach(async () => {
    ...
    const { closeNodoAssistStateDatabaseForTest } = await import("../state/nodoassist-state-db.js");
  ...
  it("prints actionable disabled proxy config output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Enable proxy.enabled with proxy.proxyUrl or NODOASSIST_PROXY_URL, or pass --proxy-url for an explicit one-off validation.\n",
    );
  });
  ...
  it("prints actionable output when proxy config is disabled and missing", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        ...
        source: "disabled",
        errors: [
          "proxy validation requires proxy.enabled=true with proxy.proxyUrl or NODOASSIST_PROXY_URL, or --proxy-url",
        ],
      },
      checks: [],
    });
  ...
  it("redacts malformed proxy URLs in text output", async () => {
    ...
    expect(process.stdout["write"]).toHaveBeenCalledWith(
      "Proxy validation failed\n\n" +
        ...
        "Next steps\n" +
        "  Fix proxy.proxyUrl, NODOASSIST_PROXY_URL, or --proxy-url so it uses a reachable http:// or https:// proxy.\n",
    );
  });
  ...
  it("prints proxy validation JSON and sets exit code on failure", async () => {
    runProxyValidationMock.mockResolvedValueOnce({
      ...
      config: {
        enabled: true,
        source: "missing",
        errors: ["proxy validation requires proxy.proxyUrl, --proxy-url, or NODOASSIST_PROXY_URL"],
      },
      checks: [],
    });
    const { runProxyValidateCommand } = await import("./proxy-cli.runtime.js");
      ...
      `${JSON.stringify(
        {
          ...
          config: {
            ...
            source: "missing",
            errors: [
              "proxy validation requires proxy.proxyUrl, --proxy-url, or NODOASSIST_PROXY_URL",
            ],
          },
          checks: [],
        },
