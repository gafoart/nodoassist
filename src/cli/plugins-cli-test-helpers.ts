...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as NodoAssistConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: NodoAssistConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/nodoassist-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: NodoAssistConfig) =>
    invokeMock<
      [NodoAssistConfig],
...
vi.mock("../hooks/install.js", () => ({
  HOOK_INSTALL_ERROR_CODE: {
    MISSING_NODOASSIST_HOOKS: "missing_nodoassist_hooks",
    EMPTY_NODOASSIST_HOOKS: "empty_nodoassist_hooks",
  },
...
export function resetPluginsCliTestState() {
  ...
  replaceConfigFile.mockImplementation(
    (async (params: { nextConfig: NodoAssistConfig }) =>
      await writeConfigFile(params.nextConfig)) as (...args: unknown[]) => Promise<unknown>,
  );
  resolveStateDir.mockReturnValue("/tmp/nodoassist-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation(((cfg: NodoAssistConfig, pluginId: string) => ({
    config: cfg,
    enabled: true,
    pluginId,
  })) as (...args: unknown[]) => unknown);
  recordPluginInstall.mockImplementation(
    ((cfg: NodoAssistConfig) => cfg) as (...args: unknown[]) => unknown,
  );
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: NodoAssistConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
