...
vi.mock("../infra/path-env.js", () => ({
  ensureNodoAssistCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.NODOASSIST_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.NODOASSIST_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "nodoassist", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "nodoassist", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "nodoassist", "agent", "--local"]);
  ...
  it("flushes requested one-shot exits after asynchronous teardown", async () => {
    ...
    await runCli(["node", "nodoassist", "models", "status", "--probe"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "nodoassist", "config"]);

    expect(createCliProgressMock).toHaveBeenCalledWith({
      label: "Loading NodoAssist CLI…",
      indeterminate: true,
      delayMs: 0,
    });
  ...
  it("suppresses startup progress for json output commands before full CLI parsing", async () => {
    ...
    await runCli(["node", "nodoassist", "sessions", "--json", "--limit", "all"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "nodoassist", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "nodoassist", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "nodoassist", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "nodoassist", "gateway", "--force"],
    });
  ...
  it("rechecks the effective guarded config before automatic startup migrations", async () => {
    ...
    await runCli(["node", "nodoassist", "gateway"]);
    const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
      | { beforeRun?: (opts: { reset?: boolean }) => Promise<void> }
      | undefined;
    await hooks?.beforeRun?.({});
  ...
  it.each([
    ...
    const previousMarker = process.env.NODOASSIST_SERVICE_MARKER;
    const previousOverride = process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    if (params.marker) {
      process.env.NODOASSIST_SERVICE_MARKER = params.marker;
    } else {
      delete process.env.NODOASSIST_SERVICE_MARKER;
    }
    if (params.override) {
      process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = params.override;
    } else {
      delete process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    }
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const exitSpy = vi.spyOn(process, "exit").mockImplementation(((code?: number) => {
      throw new Error(`exit:${String(code)}`);
    ...
    try {
      await expect(runCli(["node", "nodoassist", "gateway", ...params.flags])).rejects.toThrow(
      ...
      if (params.marker) {
        expect(process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
      }
    } finally {
      exitSpy.mockRestore();
      errorSpy.mockRestore();
      if (previousMarker === undefined) {
        ...
        process.env.NODOASSIST_SERVICE_MARKER = previousMarker;
      }
      if (previousOverride === undefined) {
        delete process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
      } else {
        process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = previousOverride;
      }
    }
  });
  ...
  it("blocks and revokes the destructive override when selected config declares service mode", async () => {
    readConfigFileSnapshotMock.mockResolvedValue({
      ...
      valid: true,
      sourceConfig: {
        env: { vars: { NODOASSIST_SERVICE_MARKER: "gateway" } },
    ...
    try {
      await withEnvAsync(
        {
          NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: "1",
          NODOASSIST_SERVICE_MARKER: undefined,
        },
        async () => {
          await expect(runCli(["node", "nodoassist", "gateway"])).rejects.toThrow("exit:78");
          expect(process.env.NODOASSIST_SERVICE_MARKER).toBeUndefined();
          expect(process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
          expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
        },
      );
  ...
  it("ignores service mode declared by an invalid selected config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(process.env.NODOASSIST_SERVICE_MARKER).toBeUndefined();
        expect(process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBe("1");
      },
    );
  });
  ...
  it("guards the config selected by trusted global dotenv before the default config", async () => {
    const homeDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nodoassist-gateway-global-selection-"),
    );
    const stateDir = path.join(homeDir, ".nodoassist");
    ...
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
          NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: undefined,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.NODOASSIST_CONFIG_PATH === selectedConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { gateway: { mode: "local" } },
          ...
          expect(process.env.NODOASSIST_CONFIG_PATH).toBe(selectedConfigPath);
          expect(process.env.NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
  ...
  it("loads state dotenv before a custom config-root fallback", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-config-env-"));
    const stateDir = path.join(homeDir, ".nodoassist");
    const configDir = path.join(homeDir, "profile");
    const configPath = path.join(configDir, "nodoassist.json");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.mkdir(configDir, { recursive: true });
    await fs.writeFile(path.join(stateDir, ".env"), "NODOASSIST_GATEWAY_TOKEN=state-token\n");
    await fs.writeFile(
      path.join(configDir, ".env"),
      [
        "NODOASSIST_GATEWAY_PASSWORD=config-root-password",
        "NODOASSIST_GATEWAY_TOKEN=config-root-token",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        {
          ...
          NODOASSIST_GATEWAY_PASSWORD: undefined,
        ...
        async () => {
          ...
          expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("state-token");
          expect(process.env.NODOASSIST_GATEWAY_PASSWORD).toBe("config-root-password");
        },
      );
    } finally {
      await fs.rm(homeDir, { recursive: true, force: true });
  ...
  it("loads and repins a legacy state dotenv after automatic state migration", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-legacy-env-"));
    const legacyStateDir = path.join(homeDir, ".clawdbot");
    const newStateDir = path.join(homeDir, ".nodoassist");
    await fs.mkdir(legacyStateDir, { recursive: true });
    await fs.writeFile(
      path.join(legacyStateDir, ".env"),
      "NODOASSIST_GATEWAY_TOKEN=legacy-token\n",
    );
    try {
      await withEnvAsync(
        {
          ...
          NODOASSIST_HOME: homeDir,
          NODOASSIST_STATE_DIR: undefined,
          NODOASSIST_TEST_FAST: undefined,
        },
        async () => {
          ...
          expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("legacy-token");
  ...
  it("blocks a future-config recovery candidate before destructive gateway reset", async () => {
    ...
    try {
      await runCli(["node", "nodoassist", "gateway", "--dev", "--reset"]);
      const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
        | { beforeRun?: (opts: { reset?: boolean }) => Promise<void> }
        | undefined;
  ...
  it("re-guards config env path selection until the gateway config is stable", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-selection-"));
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.NODOASSIST_CONFIG_PATH === "/tmp/nodoassist-chain-c.json") {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            if (process.env.NODOASSIST_STATE_DIR === "/tmp/nodoassist-chain-b") {
              return {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { NODOASSIST_CONFIG_PATH: "/tmp/nodoassist-chain-c.json" } },
                  gateway: { mode: "local" },
                },
              };
            }
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { NODOASSIST_STATE_DIR: "/tmp/nodoassist-chain-b" } },
                gateway: { mode: "local" },
              },
            };
          });
          ...
          try {
            await expect(runCli(["node", "nodoassist", "gateway"])).rejects.toThrow("exit:1");
            expect(errorSpy).toHaveBeenCalledWith(
              expect.stringContaining("run automatic gateway startup migrations"),
            );
            expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
  ...
  it("drops credentials from configs superseded during state selection", async () => {
    await withEnvAsync(
      ...
      async () => {
        readConfigFileSnapshotMock.mockImplementation(async () =>
          process.env.NODOASSIST_STATE_DIR === "/tmp/nodoassist-selected-state"
            ? {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { NODOASSIST_GATEWAY_TOKEN: "selected-token" } },
                  gateway: { mode: "local" },
                },
              }
            : {
                ...
                sourceConfig: {
                  env: {
                    vars: {
                      NODOASSIST_GATEWAY_TOKEN: "superseded-token",
                      NODOASSIST_STATE_DIR: "/tmp/nodoassist-selected-state",
                    },
                  },
                  gateway: { mode: "local" },
                },
        ...
        expect(process.env.NODOASSIST_STATE_DIR).toBe("/tmp/nodoassist-selected-state");
        expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("selected-token");
  ...
  it("re-guards config selection from a newly selected state dotenv", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-dotenv-"));
    const futureConfigPath = path.join(stateDir, "future.json");
    await fs.writeFile(
      path.join(stateDir, ".env"),
      [
        `NODOASSIST_CONFIG_PATH=${futureConfigPath}`,
        "NODOASSIST_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.NODOASSIST_CONFIG_PATH === futureConfigPath) {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { NODOASSIST_STATE_DIR: stateDir } },
                gateway: { mode: "local" },
              },
            };
          });
  ...
  it("re-inspects recovery after recovery changes config selection", async () => {
    await withEnvAsync({ NODOASSIST_CONFIG_PATH: undefined }, async () => {
      const selectedConfigPath = "/tmp/nodoassist-recovered-selection.json";
      const currentSnapshot = {
        exists: true,
        valid: true,
        sourceConfig: { gateway: { mode: "local" } },
      ...
      readConfigFileSnapshotMock.mockImplementation(async (options) => {
        ...
        if (recoveryReads === 1) {
          const recoveredSnapshot = {
            ...
            valid: true,
            sourceConfig: {
              env: { vars: { NODOASSIST_CONFIG_PATH: selectedConfigPath } },
              gateway: { mode: "local" },
            },
          };
          await options.allowSuspiciousRecovery?.(
  ...
  it("discards env from a config replaced by suspicious recovery", async () => {
    await withEnvAsync(
      { NODOASSIST_GATEWAY_TOKEN: undefined, NODOASSIST_PROXY_ACTIVE: undefined },
      async () => {
        const clobberedSnapshot = {
          exists: true,
          valid: true,
          sourceConfig: {
            env: { vars: { NODOASSIST_GATEWAY_TOKEN: "discarded-token" } },
            gateway: { mode: "local" },
          },
          hash: "clobbered",
          path: "/tmp/nodoassist.json",
        ...
        process.env.NODOASSIST_PROXY_ACTIVE = "1";
        const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
          | { beforeRun?: (opts: { force?: boolean }) => Promise<void> }
          | undefined;
        await hooks?.beforeRun?.({});

        expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.NODOASSIST_PROXY_ACTIVE).toBe("1");
        expect(ensureCliExecutionBootstrapMock).toHaveBeenCalledOnce();
      },
    );
  });
  ...
  it("does not apply environment variables from invalid config snapshots", async () => {
    await withEnvAsync({ NODOASSIST_INCLUDE_ROOTS: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: false,
        sourceConfig: {
          env: { vars: { NODOASSIST_INCLUDE_ROOTS: "/tmp/nodoassist-includes" } },
          gateway: { mode: "local" },
        },
      });
      ...
      expect(process.env.NODOASSIST_INCLUDE_ROOTS).toBeUndefined();
      expect(readConfigFileSnapshotMock.mock.calls).toEqual([
        [{ isolateEnv: true, observe: false }],
        [{ isolateEnv: true, observe: false }],
      ]);
  ...
  it("loads selected state dotenv before config env and environment normalization", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-selected-env-"));
        ...
        async () => {
          readConfigFileSnapshotMock.mockResolvedValue({
            ...
            sourceConfig: {
              env: {
                vars: {
                  NODOASSIST_GATEWAY_TOKEN: "config-token",
          ...
          normalizeEnvMock.mockImplementation(() => {
            tokenAtNormalize = process.env.NODOASSIST_GATEWAY_TOKEN;
          });
  ...
  it("drops credentials from a trusted dotenv superseded by state selection", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-dotenv-hop-"));
    const defaultStateDir = path.join(homeDir, ".nodoassist");
    ...
    await fs.writeFile(
      ...
      "NODOASSIST_GATEWAY_TOKEN=selected-token\n",
    );
    try {
      await withEnvAsync(
        {
        ...
        async () => {
          ...
          expect(process.env.NODOASSIST_STATE_DIR).toBe(selectedStateDir);
          expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("selected-token");
        },
      );
    } finally {
  ...
  it("drops gateway.env selectors when the default state dotenv selects a custom state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-fallback-hop-"));
    const defaultStateDir = path.join(homeDir, ".nodoassist");
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "nodoassist");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
    ...
    await fs.writeFile(
      ...
      [
        "NODOASSIST_CONFIG_PATH=/tmp/wrong-nodoassist.json",
        "NODOASSIST_GATEWAY_TOKEN=fallback-token",
        "",
      ].join("\n"),
    );
    await fs.writeFile(
      ...
      [
        "NODOASSIST_GATEWAY_TOKEN=selected-token",
        "NODOASSIST_INCLUDE_ROOTS=/tmp/untrusted-include-root",
  ...
  it("preserves gateway.env selectors when the compatibility fallback selects the target", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-fallback-select-"));
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "nodoassist");
    await fs.mkdir(selectedStateDir, { recursive: true });
    await fs.mkdir(gatewayEnvDir, { recursive: true });
      ...
      await withEnvAsync(
        {
          ...
          NODOASSIST_GATEWAY_TOKEN: undefined,
          NODOASSIST_HOME: homeDir,
          NODOASSIST_INCLUDE_ROOTS: undefined,
          NODOASSIST_STATE_DIR: undefined,
          NODE_OPTIONS: undefined,
        },
        async () => {
  ...
  it("drops old state dotenv credentials when config selects another state", async () => {
    const homeDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "nodoassist-gateway-config-state-hop-"),
    );
    const defaultStateDir = path.join(homeDir, ".nodoassist");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
      ...
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              process.env.NODOASSIST_STATE_DIR === selectedStateDir
                ? { gateway: { mode: "local" } }
                : {
                    env: { vars: { NODOASSIST_STATE_DIR: selectedStateDir } },
                    gateway: { mode: "local" },
                  },
          }));
  ...
  it("drops early target credentials when a later guard selects another state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-late-state-hop-"));
    const defaultStateDir = path.join(homeDir, ".nodoassist");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
    await fs.writeFile(
      path.join(defaultStateDir, ".env"),
      "NODOASSIST_GATEWAY_TOKEN=early-token\n",
    );
    await fs.writeFile(
      path.join(selectedStateDir, ".env"),
      "NODOASSIST_GATEWAY_TOKEN=selected-token\n",
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              selectLateState && process.env.NODOASSIST_STATE_DIR !== selectedStateDir
                ? {
                    env: { vars: { NODOASSIST_STATE_DIR: selectedStateDir } },
                    gateway: { mode: "local" },
                  }
          ...
          await runCli(["node", "nodoassist", "gateway"]);
          expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBe("early-token");
  ...
  it("does not let gateway.env authorize automatic mutations of a selected future config", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-gateway-global-env-"));
    const gatewayEnvDir = path.join(homeDir, ".config", "nodoassist");
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.NODOASSIST_CONFIG_PATH === futureConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
              ...
              : {
                  ...
                  valid: true,
                  sourceConfig: {
                    env: { vars: { NODOASSIST_CONFIG_PATH: futureConfigPath } },
                    gateway: { mode: "local" },
                  },
                },
          );
  ...
  it("does not treat gateway option values as bootstrap command paths", async () => {
    await runCli(["node", "nodoassist", "gateway", "--raw-stream-path", "status"]);
  ...
  it("retains selected config paths and invocation reset targets", async () => {
    await withEnvAsync(
      {
        NODOASSIST_CONFIG_PATH: "/tmp/nodoassist-invocation/nodoassist.json",
        NODOASSIST_GATEWAY_TOKEN: undefined,
        NODOASSIST_HOME: "/tmp/nodoassist-invocation-home",
        NODOASSIST_INCLUDE_ROOTS: undefined,
        NODOASSIST_PROFILE: undefined,
        NODOASSIST_STATE_DIR: "/tmp/nodoassist-invocation-state",
        NODOASSIST_TEST_FAST: "1",
        NODOASSIST_WORKSPACE_DIR: "/tmp/nodoassist-invocation-workspace",
      },
      async () => {
        readConfigFileSnapshotMock.mockResolvedValue({
          exists: true,
          ...
          sourceConfig: {
            env: {
              vars: {
                NODOASSIST_CONFIG_PATH: "/tmp/nodoassist-reset/nodoassist.json",
                NODOASSIST_GATEWAY_TOKEN: "old-token",
                NODOASSIST_HOME: "/tmp/nodoassist-reset-home",
                NODOASSIST_INCLUDE_ROOTS: "/tmp/nodoassist-reset-includes",
                NODOASSIST_PROFILE: "config-dev",
                NODOASSIST_STATE_DIR: "/tmp/nodoassist-reset",
                NODOASSIST_TEST_FAST: "0",
                NODOASSIST_WORKSPACE_DIR: "/tmp/nodoassist-reset-workspace",
              },
            },
            gateway: { mode: "local" },
          },
        ...
        expect(process.env.NODOASSIST_CONFIG_PATH).toBe(
          "/tmp/nodoassist-invocation/nodoassist.json",
        );
        expect(process.env.NODOASSIST_HOME).toBe("/tmp/nodoassist-invocation-home");
        expect(process.env.NODOASSIST_PROFILE).toBeUndefined();
        expect(process.env.NODOASSIST_STATE_DIR).toBe("/tmp/nodoassist-invocation-state");
        expect(process.env.NODOASSIST_TEST_FAST).toBe("1");
        expect(process.env.NODOASSIST_WORKSPACE_DIR).toBe("/tmp/nodoassist-invocation-workspace");
        expect(process.env.NODOASSIST_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.NODOASSIST_INCLUDE_ROOTS).toBeUndefined();
        expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
      },
  ...
  it("does not let config env authorize or retarget an explicit reset", async () => {
    await withEnvAsync(
      { NODOASSIST_PROFILE: undefined, NODOASSIST_WORKSPACE_DIR: undefined },
      async () => {
        readConfigFileSnapshotMock.mockResolvedValue({
          exists: true,
          valid: true,
          sourceConfig: {
            env: {
              vars: {
                ...
                NODOASSIST_WORKSPACE_DIR: "/tmp/nodoassist-config-workspace",
              },
            },
            gateway: { mode: "local" },
          },
        ...
        await runCli(["node", "nodoassist", "gateway", "--reset"]);

        expect(process.env.NODOASSIST_PROFILE).toBeUndefined();
        expect(process.env.NODOASSIST_WORKSPACE_DIR).toBeUndefined();
      },
    );
  });

  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.NODOASSIST_HIDE_BANNER = "1";

    await runCli(["node", "nodoassist", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "nodoassist", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "nodoassist", "browser", "--help"]);
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "nodoassist", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "nodoassist", "nodes", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "nodoassist", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "nodoassist", "doctor", "--help"]);
  ...
  it("keeps root help on the precomputed path without proxy bootstrap", async () => {
    ...
    await runCli(["node", "nodoassist", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "nodoassist", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "nodoassist", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "nodoassist", "gateway", "run"]],
    ["bare gateway runtime", ["node", "nodoassist", "gateway"]],
    ["node runtime", ["node", "nodoassist", "node", "run"]],
    ["local agent runtime", ["node", "nodoassist", "agent", "--local"]],
    ["provider inference", ["node", "nodoassist", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "nodoassist", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "nodoassist", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "nodoassist", "skills", "search", "browser"]],
    ["update command", ["node", "nodoassist", "update", "check"]],
    ["channel probe", ["node", "nodoassist", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "nodoassist", "channels", "capabilities"]],
    ["directory plugin command", ["node", "nodoassist", "directory", "peers", "list"]],
    ["message plugin command", ["node", "nodoassist", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "nodoassist", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "nodoassist", "--help"]],
    ["root version", ["node", "nodoassist", "--version"]],
    ["gateway help", ["node", "nodoassist", "gateway", "--help"]],
    ["gateway run help", ["node", "nodoassist", "gateway", "run", "--help"]],
    ["status", ["node", "nodoassist", "status"]],
    ["health", ["node", "nodoassist", "health"]],
    ["gateway status", ["node", "nodoassist", "gateway", "status"]],
    ["gateway health", ["node", "nodoassist", "gateway", "health"]],
    ["remote agent control-plane", ["node", "nodoassist", "agent", "run"]],
    ["chat control-plane", ["node", "nodoassist", "chat"]],
    ["terminal control-plane", ["node", "nodoassist", "terminal"]],
    ["config", ["node", "nodoassist", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "nodoassist", "channels"]],
    ["completion", ["node", "nodoassist", "completion", "zsh"]],
    ["debug proxy cli", ["node", "nodoassist", "proxy", "start"]],
    ["agents list", ["node", "nodoassist", "agents", "list"]],
    ["models list", ["node", "nodoassist", "models", "list"]],
    ["models status without live probe", ["node", "nodoassist", "models", "status"]],
    ["skills check", ["node", "nodoassist", "skills", "check"]],
    ["skills info", ["node", "nodoassist", "skills", "info", "weather"]],
    ["skills list", ["node", "nodoassist", "skills", "list"]],
    ["tasks list", ["node", "nodoassist", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "nodoassist", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "nodoassist", "tools", "effective"]],
    ["migrate", ["node", "nodoassist", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "nodoassist", "plugins", "marketplace", "list"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it.each([
    ["fast path", ["node", "nodoassist", "gateway", "run"]],
    [
      "full Commander path with root options",
      ["node", "nodoassist", "--log-level", "debug", "gateway", "run"],
    ],
  ...
  it("validates the runtime before selecting gateway config", async () => {
    await runCli(["node", "nodoassist", "gateway", "run"]);
  ...
  it("selects gateway config env before starting its managed proxy", async () => {
    await withEnvAsync({ NODOASSIST_TEST_PROXY_SELECTION: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: true,
        sourceConfig: {
          env: { vars: { NODOASSIST_TEST_PROXY_SELECTION: "selected" } },
          gateway: { mode: "local" },
        },
      });
      loadConfigMock.mockImplementationOnce(() => ({
        proxy: { selected: process.env.NODOASSIST_TEST_PROXY_SELECTION },
      }));
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "nodoassist", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "nodoassist", "foo"])).rejects.toThrow(
  ...
  it("suggests close known commands for unowned command roots before proxy startup", async () => {
    await expect(runCli(["node", "nodoassist", "upate"])).rejects.toThrow(
      "Did you mean this?\n  nodoassist update",
    );
  ...
  it("keeps suggestions out of plugin-policy diagnostics", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "nodoassist", "codex"]);
    } catch (caught) {
      error = caught;
    }
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "nodoassist", "foo", "--help"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "nodoassist", "foo", "--version"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "nodoassist", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
    expect(error).toBeInstanceOf(Error);
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "nodoassist", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "nodoassist", "lcm_recent"])).rejects.toThrow(
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "nodoassist", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "nodoassist", "auth", "--help"]],
    ["tool", ["node", "nodoassist", "tool", "image_generate"]],
    ["tools", ["node", "nodoassist", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "nodoassist", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "nodoassist", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "nodoassist", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "nodoassist", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts the gateway-backed TUI for bare root invocations when config already exists", async () => {
    ...
    await withEnvAsync({ NODOASSIST_GATEWAY_PASSWORD: "gateway-ref-password" }, async () => {
      await withInteractiveTty(async () => {
        await runCli(["node", "nodoassist"]);
      });
    });
  ...
  it("uses gateway env credentials for bare root gateway preflight", async () => {
    ...
    await withEnvAsync({ NODOASSIST_GATEWAY_TOKEN: "env-token" }, async () => {
      await withInteractiveTty(async () => {
        await runCli(["node", "nodoassist"]);
      });
    });
  ...
  it("does not probe unsafe remote gateway URLs with configured credentials", async () => {
    ...
    await withEnvAsync({ NODOASSIST_ALLOW_INSECURE_PRIVATE_WS: undefined }, async () => {
      await withInteractiveTty(async () => {
        await runCli(["node", "nodoassist"]);
      });
    });
  ...
  it("rejects configured bare root TUI startup without an interactive TTY", async () => {
    ...
    try {
      ...
      expect(process.exitCode).toBe(1);
      expect(errorSpy).toHaveBeenCalledWith(
        "NodoAssist TUI needs an interactive TTY. Use `nodoassist agent --local ...` for automation.",
      );
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "nodoassist", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "nodoassist", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "nodoassist", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "nodoassist", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "nodoassist", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "nodoassist", "doctor", "--help"]],
    ]);
  });
  ...
  it("restores terminal state before uncaught CLI exits", async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "[nodoassist] NodoAssist hit an unexpected runtime error.",
      );
      expect(consoleErrorSpy).toHaveBeenCalledWith("[nodoassist] Reason: boom");
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[nodoassist] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
