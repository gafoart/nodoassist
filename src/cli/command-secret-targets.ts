...
function resolveFetchConfig(config: NodoAssistConfig): Record<string, unknown> | undefined {
...
function resolveSearchConfig(config: NodoAssistConfig): Record<string, unknown> | undefined {
...
function isScopedChannelSecretTargetEntry(params: {
  ...
  return (
    ...
    params.entry.configFile === "nodoassist.json" &&
...
type CapabilityWebCommandSecretTargetParams = {
  ...
  autoDetectTargets: (config: NodoAssistConfig) => CommandSecretTargetScope;
  selectedProviderTargetIds: (
    config: NodoAssistConfig,
    providerId?: string | null,
  ) => SelectedProviderTargetIds;
