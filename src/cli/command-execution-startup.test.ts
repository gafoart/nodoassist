...
describe("command-execution-startup", () => {
  ...
  it("uses process env banner suppression when startup env is omitted", () => {
    const originalHideBanner = process.env.NODOASSIST_HIDE_BANNER;
    try {
  ...
  it("skips local plugin bootstrap for JSON gateway agent calls", () => {
    expect(
      mod.resolveCliExecutionStartupContext({
        argv: ["node", "nodoassist", "agent", "--agent", "main", "--message", "hi", "--json"],
        jsonOutputMode: true,
    ...
    expect(
      mod.resolveCliExecutionStartupContext({
        argv: ["node", "nodoassist", "agent", "--agent", "main", "--message", "hi"],
        jsonOutputMode: false,
      }).startupPolicy.loadPlugins,
    ).toBe(true);
  });
  ...
  it("uses the resolved action command path for protocol startup policy", () => {
    expect(
      mod.resolveCliExecutionStartupContext({
        argv: ["node", "nodoassist", "acp", "--token", "-secret"],
    ...
    expect(
      mod.resolveCliExecutionStartupContext({
        argv: ["node", "nodoassist", "acp", "--verbose", "client"],
