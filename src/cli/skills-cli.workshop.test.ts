...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createNodoAssistTestState({
      layout: "state-only",
      prefix: "nodoassist-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("nodoassist-skills-cli-workshop-");
  ...
  it("scopes list and inspect to the selected workspace", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("nodoassist-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
