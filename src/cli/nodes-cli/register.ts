    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nodoassist nodes status", "List known nodes with live status."],
          ["nodoassist nodes pairing pending", "Show pending node pairing requests."],
          ["nodoassist nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'nodoassist nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["nodoassist nodes camera snap --node <id>", "Capture a photo from a node camera."],
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `nodoassist nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
