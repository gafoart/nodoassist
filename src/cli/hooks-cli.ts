...
function buildHooksReport(config: NodoAssistConfig): HookStatusReport {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("nodoassist hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("nodoassist hooks list")}\` to see available hooks.`;
  }
...
async function enableHook(hookName: string): Promise<void> {
  ...
  const config = (snapshot.sourceConfig ?? snapshot.config) as NodoAssistConfig;
...
export function registerHooksCli(program: Command): void {
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `nodoassist plugins install`")
    ...
    .action(async (raw: string, opts: { link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`nodoassist hooks install` is deprecated; use `nodoassist plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `nodoassist plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`nodoassist hooks update` is deprecated; use `nodoassist plugins update`."),
      );
