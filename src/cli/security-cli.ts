...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["nodoassist security audit", "Run a local security audit."],
          [
          ...
          ["nodoassist security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["nodoassist security audit --json", "Output machine-readable JSON."],
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("NodoAssist security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("nodoassist security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("nodoassist security audit --fix")}`));
        if (!fixResult) {
