...
const mocks = vi.hoisted(() => {
  ...
  let configState: NodoAssistConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    setConfig: (next: NodoAssistConfig) => {
      configState = next;
    },
    ...
    replaceConfigFile: vi.fn(
      async ({ nextConfig }: { nextConfig: NodoAssistConfig; baseHash?: string }) => {
    ...
    readConfigFileSnapshot: vi.fn<
      () => Promise<{ path: string; hash: string; config: NodoAssistConfig }>
    >(async () => ({
      path: "/tmp/nodoassist.json",
      hash: "config-hash-1",
      config: configState,
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: NodoAssistConfig) => void }) => {
  ...
  it("sanitizes terminal control content before rendering the text table", async () => {
    ...
    expect(output).toContain("/tmp/nodoassist.json");
    ...
    expect(output).not.toContain("/tmp/nodoassist.json\nforged");
