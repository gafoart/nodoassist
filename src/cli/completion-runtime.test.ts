...
describe("completion-runtime", () => {
  ...
  it("installs PowerShell completion into the concrete profile path", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-completion-state-bob's-"));

    try {
      await withEnvAsync({ HOME: homeDir, NODOASSIST_STATE_DIR: stateDir }, async () => {
        const cachePath = resolveCompletionCachePath("powershell", "nodoassist");
        await fs.mkdir(path.dirname(cachePath), { recursive: true });
        await fs.writeFile(cachePath, "# powershell completion\n", "utf-8");

        await installCompletion("powershell", true, "nodoassist");

        const profilePath = resolveCompletionProfilePath("powershell");
        const profile = await fs.readFile(profilePath, "utf-8");
        expect(profile).toBe(`# NodoAssist Completion\n. '${cachePath.replace(/'/g, "''")}'\n`);
      });
    } finally {
  ...
  it("rejects install when the completion cache is missing", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-completion-home-"));
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-completion-state-"));

    try {
      await withEnvAsync({ HOME: homeDir, NODOASSIST_STATE_DIR: stateDir }, async () => {
        await expect(installCompletion("zsh", true, "nodoassist")).rejects.toThrow(
