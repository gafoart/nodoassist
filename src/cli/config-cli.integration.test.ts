...
async function withExecDryRunConfigHarness(
  ...
  const envSnapshot = captureEnv(["NODOASSIST_CONFIG_PATH", "NODOASSIST_TEST_FAST"]);
  try {
    fs.writeFileSync(
      configPath,
      `${JSON.stringify(
...
describe("config cli integration", () => {
  beforeAll(async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-config-cli-warmup-"));
    const configPath = path.join(tempDir, "nodoassist.json");
    const envSnapshot = captureEnv(["NODOASSIST_CONFIG_PATH", "NODOASSIST_TEST_FAST"]);
    try {
  ...
  it("accepts plugin hook conversation-access policy via config set", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-config-cli-plugin-hooks-"));
    const configPath = path.join(tempDir, "nodoassist.json");
    const envSnapshot = captureEnv(["NODOASSIST_CONFIG_PATH", "NODOASSIST_TEST_FAST"]);
    try {
      fs.writeFileSync(
      ...
      await runConfigSet({
        path: "plugins.entries.nodoassist-mem0.hooks.allowConversationAccess",
        value: "true",
        cliOptions: {},
      ...
      expect(afterWrite.plugins?.entries?.["nodoassist-mem0"]?.hooks).toEqual({
  ...
  it("supports batch-file dry-run and then writes real config changes", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-config-cli-int-"));
    const configPath = path.join(tempDir, "nodoassist.json");
    const batchPath = path.join(tempDir, "batch.json");
    const envSnapshot = captureEnv([
      "NODOASSIST_CONFIG_PATH",
      "NODOASSIST_TEST_FAST",
      "DISCORD_BOT_TOKEN",
    ]);
    try {
      fs.writeFileSync(
  ...
  it("keeps file unchanged when real-file dry-run fails and reports JSON error payload", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-config-cli-int-fail-"));
    const configPath = path.join(tempDir, "nodoassist.json");
    const envSnapshot = captureEnv([
      "NODOASSIST_CONFIG_PATH",
      "NODOASSIST_TEST_FAST",
  ...
  it("skips exec provider execution during dry-run by default", async () => {
    await withExecDryRunConfigHarness("nodoassist-config-cli-int-exec-skip-", async (params) => {
  ...
  it("executes exec providers during dry-run when --allow-exec is set", async () => {
    await withExecDryRunConfigHarness("nodoassist-config-cli-int-exec-allow-", async (params) => {
      const before = fs.readFileSync(params.configPath, "utf8");
      await runConfigSet({
        cliOptions: {
          batchFile: params.batchPath,
