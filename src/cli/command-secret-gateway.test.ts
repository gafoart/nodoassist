...
describe("resolveCommandSecretRefsViaGateway", () => {
  function makeTalkProviderApiKeySecretRefConfig(envKey: string): NodoAssistConfig {
  ...
  async function createExecProviderConfig(refId: string): Promise<{
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-command-secret-exec-"));
    tempRoots.add(root);
    ...
    const resolverScript = [
      ...
      "  fs.writeFileSync(process.env.NODOASSIST_EXEC_MARKER, 'executed');",
        ...
        secrets: {
          providers: {
            default: {
              ...
              env: { NODOASSIST_EXEC_MARKER: markerPath },
