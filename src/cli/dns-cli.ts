...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. nodoassist.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log(
        theme.heading(
          "Recommended config ($NODOASSIST_CONFIG_PATH, default ~/.nodoassist/nodoassist.json):",
        ),
      );
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by nodoassist dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in the active NodoAssist config ($NODOASSIST_CONFIG_PATH, default ~/.nodoassist/nodoassist.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
