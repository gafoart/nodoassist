...
async function resolveReachableGateway(config: NodoAssistConfig): Promise<ReachableGateway | null> {
...
async function resolveGatewayProbeAuth(
  ...
  const token = configToken ?? normalizeOptionalString(process.env.NODOASSIST_GATEWAY_TOKEN);
  const password =
    configPassword ?? normalizeOptionalString(process.env.NODOASSIST_GATEWAY_PASSWORD);
  if (token) {
...
async function resolveGatewayProbeTargets(config: NodoAssistConfig): Promise<GatewayProbeTarget[]> {
...
function isSafeGatewayProbeTarget(target: GatewayProbeTarget): boolean {
  ...
  return isSecureWebSocketUrl(target.url, {
    allowPrivateWs: process.env.NODOASSIST_ALLOW_INSECURE_PRIVATE_WS === "1",
  });
}
...
async function resolveValidatedRemoteGatewayUrl(config: NodoAssistConfig): Promise<string | null> {
  try {
...
async function resolveLocalGatewayWebSocketUrls(config: NodoAssistConfig): Promise<string[]> {
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.NODOASSIST_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.NODOASSIST_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
function createAllowlistAgnosticCliLookupConfig(config: NodoAssistConfig): NodoAssistConfig {
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return [
    `Unknown command: nodoassist ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`,
    suggestion,
  ]
    .filter(Boolean)
    .join("\n");
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.NODOASSIST_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<NodoAssistConfig> | null = null;
  const isolateProxyConfigEnv = isGatewayRunInvocation;
  const readBestEffortCliConfig = async (): Promise<NodoAssistConfig> => {
  ...
  const replaceStartedProxy = async (config: NodoAssistConfig["proxy"]) => {
  ...
  try {
    ...
    // `nodoassist <typo> --help` surfaces the same Unknown command error as
    // `nodoassist <typo>` instead of silently showing generic top-level help.
    ...
    if (bareRootLaunchTarget) {
      ...
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `nodoassist crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[nodoassist] Non-fatal uncaught exception (continuing):",
            formatUncaughtError(error),
          );
          return;
        }
        for (const line of formatCliFailureLines({
          title: "NodoAssist hit an unexpected runtime error.",
          error,
          argv: normalizedArgv,
        })) {
          console.error(line);
        ...
        for (const message of runFatalErrorHooks({ reason: "uncaught_exception", error })) {
          console.error("[nodoassist]", message);
        }
