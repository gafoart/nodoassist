...
const CLI_STATE_ROOT = "/tmp/nodoassist-state";
const ORIGINAL_NODOASSIST_STATE_DIR = process.env.NODOASSIST_STATE_DIR;
const ORIGINAL_NODOASSIST_NIX_MODE = process.env.NODOASSIST_NIX_MODE;
const PROFILE_STATE_ROOT = "/tmp/nodoassist-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.NODOASSIST_STATE_DIR = PROFILE_STATE_ROOT;
  return path.resolve(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): NodoAssistConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): NodoAssistConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createNpmPackPluginInstallResult(
  ...
  return {
    ...
    manifestName: `@nodoassist/${pluginId}`,
    npmTarballName: `nodoassist-${pluginId}-1.2.3.tgz`,
    npmResolution: {
      name: `@nodoassist/${pluginId}`,
      version: "1.2.3",
      resolvedSpec: `@nodoassist/${pluginId}@1.2.3`,
...
function createPathHookPackInstalledConfig(tmpRoot: string): NodoAssistConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): NodoAssistConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing nodoassist.plugin.json",
  });
...
function replaceConfigCall(callIndex = 0): { baseHash?: string; nextConfig?: NodoAssistConfig } {
...
function primeBlockedPluginConfigMutation(
  params: { blockHooks?: boolean; config?: NodoAssistConfig } = {},
): void {
  const configPath = path.join(process.cwd(), "nodoassist.json5");
  ...
  const config = params.config ?? ({} as NodoAssistConfig);
  const parsed = {
...
function primeNestedPluginConfigMutation(tempRoot: string): void {
  ...
  const config = { plugins: { entries: {} } } as NodoAssistConfig;
...
function primeBlockedRootConfigMutation(config = {} as NodoAssistConfig): void {
  const configPath = path.join(process.cwd(), "nodoassist.json5");
  loadConfig.mockReturnValue(config);
  readConfigFileSnapshotForWrite.mockResolvedValue({
    snapshot: {
...
function primeBlockedHookConfigMutation(config = {} as NodoAssistConfig): void {
  const configPath = path.join(process.cwd(), "nodoassist.json5");
  const externalHooksPath = path.join(
    path.parse(process.cwd()).root,
    "external-nodoassist",
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_NODOASSIST_STATE_DIR === undefined) {
      delete process.env.NODOASSIST_STATE_DIR;
    } else {
      process.env.NODOASSIST_STATE_DIR = ORIGINAL_NODOASSIST_STATE_DIR;
    }
  ...
  it("blocks local package inspection when plugin and hook config are include-owned", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-hook-pack-"));
    primeBlockedPluginConfigMutation({ blockHooks: true });
    installHooksFromPath.mockResolvedValue(createHookPackInstallResult(localPath));
    installPluginFromPath.mockResolvedValue({
      ok: false,
      error: "package.json missing nodoassist.extensions",
      code: "missing_nodoassist_extensions",
    });
  ...
  it("fails closed for ambiguous local plugins when the whole config is include-owned", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-demo-plugin-"));
    primeBlockedRootConfigMutation();
  ...
  it("fails closed before installing a blocked ambiguous local plugin", async () => {
    const archivePath = path.join(os.tmpdir(), `nodoassist-plugin-${process.pid}.tgz`);
  ...
  it("fails closed when a local hook probe finds a plugin-capable package", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-dual-package-"));
    primeBlockedPluginConfigMutation();
    installHooksFromPath.mockResolvedValue({
  ...
  it("fails closed for a local bundle plugin instead of installing its hooks", async () => {
    const localPath = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-bundle-plugin-"));
    primeBlockedPluginConfigMutation();
    installHooksFromPath.mockResolvedValue({
      ...createHookPackInstallResult(localPath),
      packageKind: "plugin-capable",
  ...
  it("fails closed when a blocked-config local hook probe throws", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-local-plugin-"));
    primeBlockedPluginConfigMutation();
  ...
  it("blocks explicit plugins through nested include config before installer side effects", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-nested-"));
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/nodoassist-config.json5",
      exists: true,
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `nodoassist doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("resolves exact official external plugin ids through their npm package", async () => {
    ...
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
    expect(npmInstallCall().spec).toBe("@nodoassist/brave-plugin");
    ...
    expect(record.spec).toBe("@nodoassist/brave-plugin");
  ...
  it("passes third-party external catalog integrity with catalog install trust", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("wecom-nodoassist-plugin");
    loadConfig.mockReturnValue(cfg);
    findBundledPluginSourceMock.mockReturnValue(undefined);
    installPluginFromNpmSpec.mockResolvedValue(
      createNpmPluginInstallResult("wecom-nodoassist-plugin"),
    );
    enablePluginInConfig.mockReturnValue({ config: enabledCfg });
    applyExclusiveSlotSelection.mockReturnValue({
      config: enabledCfg,
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-nodoassist-plugin@2026.5.7");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-nodoassist-plugin");
  ...
  it("passes third-party external catalog integrity to hook-pack fallback", async () => {
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "aborted: npm package integrity drift detected for @wecom/wecom-nodoassist-plugin@2026.5.7",
    });

    await expect(runPluginsCommand(["plugins", "install", "wecom"])).rejects.toThrow("__exit__:1");

    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(hookNpmInstallCall().spec).toBe("@wecom/wecom-nodoassist-plugin@2026.5.7");
  ...
  it("installs npm-pack archives through npm install semantics", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
    const archivePath = "/tmp/nodoassist-demo-1.2.3.tgz";
    ...
    const record = persistedInstallRecord("demo");
    expect(record.source).toBe("npm");
    expect(record.spec).toBe("@nodoassist/demo@1.2.3");
    ...
    expect(record.npmTarballName).toBe("nodoassist-demo-1.2.3.tgz");
    expect(writeConfigFile).toHaveBeenCalledWith(enabledCfg);
  });
  ...
  it("marks explicit official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "npm:@nodoassist/discord"]);
  ...
  it("marks scoped official npm package installs as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@nodoassist/discord"]);
  ...
  it("uses bundled NodoAssist package specs instead of pinning stale managed npm overrides", async () => {
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("discord");
    ...
    findBundledPluginSourceMock.mockImplementation((params: unknown) => {
      ...
      return lookup.kind === "npmSpec" && lookup.value === "@nodoassist/discord"
        ? {
            pluginId: "discord",
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@nodoassist/discord@2026.5.20" },
    });
    expect(findBundledPluginSourceMock).toHaveBeenCalledWith({
      lookup: { kind: "npmSpec", value: "@nodoassist/discord" },
    });
    const record = persistedInstallRecord("discord");
    expect(record.source).toBe("path");
    expect(record.spec).toBe("@nodoassist/discord@2026.5.20");
    expect(record.sourcePath).toBe(bundledPath);
    expect(record.installPath).toBe(bundledPath);
    expect(runtimeLogsContain("ships with the current NodoAssist build")).toBe(true);
    expect(runtimeLogsContain("npm:@nodoassist/discord@2026.5.20")).toBe(true);
  });
  ...
  it("marks catalog npm package installs with alternate selectors as trusted", async () => {
    ...
    await runPluginsCommand(["plugins", "install", "@wecom/wecom-nodoassist-plugin@latest"]);
    ...
    expect(npmInstallCall().spec).toBe("@wecom/wecom-nodoassist-plugin@latest");
    expect(npmInstallCall().expectedPluginId).toBe("wecom-nodoassist-plugin");
    expect(npmInstallCall().trustedSourceLinkedOfficialInstall).toBe(true);
    expect(npmInstallCall().expectedIntegrity).toBeUndefined();
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("adds a Git PATH hint when npm plugin dependency install cannot spawn git", async () => {
    ...
    await expect(
      runPluginsCommand(["plugins", "install", "npm:@nodoassist/whatsapp"]),
    ).rejects.toThrow("__exit__:1");
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as NodoAssistConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("rejects --pin for git installs and points at git refs", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain("nodoassist plugins install git:<repo>@<ref>");
  });
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/nodoassist/.nodoassist/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing nodoassist.hooks",
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `nodoassist plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-local-hook-pack-"));
    const cfg = {} as NodoAssistConfig;
    const installedCfg = {
      hooks: {
        internal: {
  ...
  it("does not fall back to npm when explicit ClawHub rejects a real package", async () => {
    ...
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "nodoassist skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "nodoassist skills install demo" instead.');
  });
