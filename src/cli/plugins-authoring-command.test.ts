...
describe("plugin authoring commands", () => {
  beforeAll(async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-source-warm-"));
    try {
      const entryPath = writeSourceToolPluginProject({
        tmpDir,
        packageName: "nodoassist-plugin-source-warm",
  ...
  it("preserves manifest-owned metadata while updating generated fields", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        entry: "./src/index.ts",
        manifest,
        packageManifest: { version: "1.2.3", nodoassist: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([]);
  });
  ...
  it("drops stale manifest-owned tool metadata when no generated metadata remains", () => {
    const metadata = createDemoMetadata();
    const packageManifest = { version: "1.2.3", nodoassist: { extensions: ["./src/index.ts"] } };
    const manifest = buildToolPluginManifest({
      metadata,
      packageManifest,
      existingManifest: {
  ...
  it("aligns package metadata with the selected runtime extension entry", () => {
    expect(
      buildToolPluginPackageManifest({
        packageManifest: {
          name: "demo",
          nodoassist: { setupEntry: "./setup.ts", extensions: ["./src/other.ts"] },
        },
        entry: "./src/index.ts",
      }),
    ).toEqual({
  ...
  it("reports stale manifest contracts", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        packageManifest: { nodoassist: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([
      "nodoassist.plugin.json generated metadata is stale. Run nodoassist plugins build.",
      "nodoassist.plugin.json contracts.tools is missing: demo_echo",
      "nodoassist.plugin.json contracts.tools has no matching defineToolPlugin tool: other_tool",
    ]);
  });

  it("reports missing entries with an author-facing path", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-missing-"));
  ...
  it("loads source entries that import the NodoAssist plugin SDK package subpath", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-source-"));
    const entryPath = writeSourceToolPluginProject({
      tmpDir,
      packageName: "nodoassist-plugin-source-demo",
  ...
  it("scaffolds a dist-entry tool plugin project", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-plugin-init-"));
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(projectDir, "nodoassist.plugin.json"), "utf8")),
    ).toMatchObject({
      id: "stock-quotes",
      name: 'Stock "Quotes"',
      configSchema: {
  ...
  it("scaffolds a provider plugin project with ClawHub validation and release metadata", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-provider-init-"));
    ...
    expect(packageManifest).toMatchObject({
      name: "nodoassist-plugin-plugin-init-test",
      scripts: {
      ...
      nodoassist: {
        extensions: ["./dist/index.js"],
        install: {
          clawhubSpec: "clawhub:nodoassist-plugin-plugin-init-test",
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(projectDir, "nodoassist.plugin.json"), "utf8"),
    );
    expect(manifest).toMatchObject({
      id: "plugin-init-test",
      name: "Plugin Init Test",
    ...
    expect(workflow).toContain(
      "nodoassist/clawhub/.github/workflows/package-publish.yml@9d49df109d4ad3dc8a6ecf05d26b39f46d294721",
    );
  });
});
