...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-bundled-skills-test-"));
    process.env.NODOASSIST_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
