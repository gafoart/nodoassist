...
describe("commitConfigWithPendingPluginInstalls", () => {
  ...
  it("marks replaced managed npm generations when install records are committed", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-record-commit-"));
  ...
  it("does not mark arbitrary npm paths outside the managed npm root", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-record-commit-"));
    const outsideRoot = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-record-outside-"));
    const previousInstallPath = path.join(
      outsideRoot,
      "npm",
      "projects",
        ...
        await commitPluginInstallRecordsWithConfig({
          ...
          nextInstallRecords: {
            "@nodoassist/voice-call": {
              source: "npm",
              spec: "@nodoassist/voice-call@2.0.0",
              installPath: nextInstallPath,
            },
          },
          nextConfig: {},
  ...
  it("removes earlier retained markers when a later marker creation fails", async () => {
    ...
    fs.writeFileSync(
      path.join(stateDir, "npm", "projects", "voice-call-v1", ".nodoassist-retained-npm-installs"),
      "not a directory",
      "utf8",
    );
