...
describe("proxy cli", () => {
  ...
  it.each([
    ...
    expect(() => program.parse(["node", "nodoassist", ...args])).toThrow(expected);
  });

  it("normalizes signed decimal numeric options through the shared parser", async () => {
    const program = createProgram();

    await program.parseAsync(["node", "nodoassist", "proxy", "start", "--port", "+08080"]);
    await program.parseAsync(["node", "nodoassist", "proxy", "validate", "--timeout-ms", "+01000"]);
    await program.parseAsync(["node", "nodoassist", "proxy", "sessions", "--limit", "+05"]);
