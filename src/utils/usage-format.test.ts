...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["NODOASSIST_AGENT_DIR", "NODOASSIST_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-usage-format-"));
  ...
  it("prefers models.json pricing over nodoassist config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to nodoassist config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
