...
async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.NODOASSIST_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.NODOASSIST_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      const startupDelayMs = Number(process.env.NODOASSIST_TUI_PTY_STARTUP_DELAY_MS ?? 0);
      ...
      let fastMode = process.env.NODOASSIST_TUI_PTY_FAST_MODE === "true";
      ...
      async function main() {
        await runTui({
          ...
          deliver: false,
          historyLimit: 5,
          title: "nodoassist tui pty fixture",
        });
      }
...
async function startTuiFixture(opts: { env?: NodeJS.ProcessEnv } = {}) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "nodoassist-tui-pty-"));
  ...
  const run = startPty(process.execPath, ["--import", "tsx", scriptPath], {
    ...
    env: {
      ...
      NODOASSIST_TUI_PTY_LOG_PATH: logPath,
      NO_COLOR: undefined,
      ...opts.env,
    },
  ...
  it(
    ...
    async () => {
      const slow = await startTuiFixture({
        env: { NODOASSIST_TUI_PTY_STARTUP_DELAY_MS: "400" },
      });
      try {
