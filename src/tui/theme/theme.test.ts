...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      NODOASSIST_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when NODOASSIST_THEME=light", async () => {
    const mod = await importThemeWithEnv({ NODOASSIST_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when NODOASSIST_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ NODOASSIST_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats NODOASSIST_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ NODOASSIST_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("NODOASSIST_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ NODOASSIST_THEME: "light" });
    const darkMod = await importThemeWithEnv({ NODOASSIST_THEME: "dark" });
