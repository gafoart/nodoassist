...
async function withModeExecProviderFixture(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), `nodoassist-tui-mode-${label}-`));
...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "NODOASSIST_TUI_SETUP_AUTH_SOURCE",
    ]);
    ...
    resolveStateDir.mockImplementation(
      (env: NodeJS.ProcessEnv) => env.NODOASSIST_STATE_DIR ?? "/tmp/nodoassist",
    );
    resolveConfigPath.mockImplementation(
      (env: NodeJS.ProcessEnv, stateDir: string) =>
        env.NODOASSIST_CONFIG_PATH ?? `${stateDir}/nodoassist.json`,
    );
    ...
    delete process.env.NODOASSIST_TUI_SETUP_AUTH_SOURCE;
  });
  ...
  it("keeps an explicit Gateway port ahead of active lock metadata", async () => {
    ...
    await withEnvAsync({ NODOASSIST_GATEWAY_PORT: "19001" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("falls back to NODOASSIST_GATEWAY_TOKEN when config token is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("keeps normal TUI local password mode env precedence by default", async () => {
    ...
    await withEnvAsync({ NODOASSIST_GATEWAY_PASSWORD: "env-password" }, async () => {
      const result = await resolveGatewayConnection({});
  ...
  it("uses configured local password for setup-launched TUI despite stale gateway password env", async () => {
    ...
    await withEnvAsync(
      {
        NODOASSIST_GATEWAY_PASSWORD: "stale-env-password", // pragma: allowlist secret
        NODOASSIST_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
        expect(result.password).toBe("config-password");
  ...
  it("still resolves env SecretRefs for setup-launched TUI config auth", async () => {
    ...
    await withEnvAsync(
      {
        NODOASSIST_GATEWAY_PASSWORD: "resolved-ref-password", // pragma: allowlist secret
        NODOASSIST_TUI_SETUP_AUTH_SOURCE: "config",
      },
      async () => {
        const result = await resolveGatewayConnection({});
  ...
  it("prefers NODOASSIST_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    const gatewayPasswordEnv = "NODOASSIST_GATEWAY_PASSWORD"; // pragma: allowlist secret
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-tui-file-secret-"));
