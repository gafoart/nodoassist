...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  const envSnapshot = captureEnv(["NODOASSIST_HOME", "HOME"]);

  afterEach(() => {
    envSnapshot.restore();
  });

  it("uses NODOASSIST_HOME for tilde expansion", () => {
    setTestEnvValue("NODOASSIST_HOME", "/srv/nodoassist-home");
    setTestEnvValue("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/nodoassist-home", "cron", "jobs.json"));
  });
});
