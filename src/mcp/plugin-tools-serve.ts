/**
 * Standalone MCP server that exposes NodoAssist plugin-registered tools
...
function resolvePluginToolPolicy(config: NodoAssistConfig): {
...
function resolveTools(config: NodoAssistConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "nodoassist-plugin-tools", tools });
}
