...
/**
 * Runtime bridge between MCP tools and the NodoAssist Gateway channel APIs.
 *
...
export class NodoAssistChannelBridge {
  ...
  constructor(
    private readonly cfg: NodoAssistConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "NodoAssist MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`nodoassist mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`nodoassist mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `nodoassist mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
  ...
  private async dispatchGatewayEvent(event: EventFrame): Promise<void> {
    try {
      ...
      process.stderr.write(`nodoassist mcp: gateway event ${event.event} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `nodoassist mcp: gateway event ${event.event} error: ${String(error)}\n`,
        );
      }
    }
  }
