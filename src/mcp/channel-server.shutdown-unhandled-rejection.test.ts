...
vi.mock("./channel-bridge.js", () => ({
  NodoAssistChannelBridge: class MockNodoAssistChannelBridge {
...
describe("serveNodoAssistChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveNodoAssistChannelMcp } = await import("./channel-server.js");

    const servePromise = serveNodoAssistChannelMcp({ verbose: false });
