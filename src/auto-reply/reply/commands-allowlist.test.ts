...
async function withTempConfigPath<T>(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-allowlist-config-"));
  const configPath = path.join(dir, "nodoassist.json");
  const previous = process.env.NODOASSIST_CONFIG_PATH;
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      setTestEnvValue("NODOASSIST_CONFIG_PATH", previous);
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<NodoAssistConfig>(configPath);
      ...
      createTestRegistry([
        {
          ...
          plugin: {
            ...
            config: {
              ...
              defaultAccountId: (cfg: NodoAssistConfig) =>
                (cfg.channels?.telegram as TelegramTestSectionConfig | undefined)?.defaultAccount ??
                DEFAULT_ACCOUNT_ID,
            },
          },
