...
const mocks = vi.hoisted(() => ({
  ...
  triggerNodoAssistRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerNodoAssistRestart: mocks.triggerNodoAssistRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerNodoAssistRestart.mockReset();
    mocks.triggerNodoAssistRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerNodoAssistRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerNodoAssistRestart).not.toHaveBeenCalled();
  ...
  it("clears the success sentinel when fallback restart fails", async () => {
    mocks.triggerNodoAssistRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
