...
const policyMocks = vi.hoisted(() => ({
  resolveAcpDispatchPolicyError: vi.fn<(cfg: NodoAssistConfig) => AcpRuntimeError | null>(
    () => null,
  ),
  resolveAcpAgentPolicyError: vi.fn<
    (cfg: NodoAssistConfig, agent: string) => AcpRuntimeError | null
  >(() => null),
}));
...
const sessionMetaMocks = vi.hoisted(() => ({
  readAcpSessionEntry: vi.fn<
    (params: { sessionKey: string; cfg?: NodoAssistConfig }) => AcpSessionStoreEntry | null
  >(() => null),
}));
...
vi.mock("../../acp/policy.js", () => ({
  resolveAcpDispatchPolicyError: (cfg: NodoAssistConfig) =>
    policyMocks.resolveAcpDispatchPolicyError(cfg),
  resolveAcpAgentPolicyError: (cfg: NodoAssistConfig, agent: string) =>
...
function createAcpConfigWithVisibleToolTags(): NodoAssistConfig {
...
describe("tryDispatchAcpReply", () => {
  ...
  it("forwards media-understanding PDF page images alongside current image attachments", async () => {
    setReadyAcpResolution();
    const currentPath = "/tmp/nodoassist-current-image.png";
  ...
  it("preserves chat.send inline image attachments over recent history images", async () => {
    ...
    const historyPath = "/tmp/nodoassist-history-inline.png";
  ...
  it("marks accumulated ACP block TTS finals as trusted local media for WebChat", async () => {
    ...
    ttsMocks.resolveTtsConfig.mockReturnValue({ mode: "final" });
    queueTtsReplies({
      mediaUrl: "/tmp/nodoassist-media/acp-tts.ogg",
      audioAsVoice: true,
    } as MockTtsReply);
    ...
    expect(finalPayload.mediaUrl).toBe("/tmp/nodoassist-media/acp-tts.ogg");
