...
export function resolveAcpInstallCommandHint(cfg: NodoAssistConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `nodoassist plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^nodoassist plugins install /u, "");
    ...
    return "nodoassist plugins install acpx";
  }
