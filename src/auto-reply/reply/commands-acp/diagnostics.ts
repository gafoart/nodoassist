...
import { formatAcpRuntimeErrorText } from "@nodoassist/acp-core/runtime/error-text";
import {
...
export async function handleAcpDoctorAction(
  ...
  try {
    ...
    lines.push(`next: nodoassist config set plugins.entries.${backendId}.enabled true`);
...
export function handleAcpInstallAction(
  ...
  const lines = [
    ...
    `run: ${installHint}`,
    `then: nodoassist config set plugins.entries.${backendId}.enabled true`,
    "then: /acp doctor",
  ];
  return stopWithText(lines.join("\n"));
}
