...
describe("runPreparedReply media-only handling", () => {
  ...
  it("hydrates current image MediaPaths by extension when MediaTypes are missing", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "nodoassist-followup-image-"));
  ...
  it.each([
    ...
    if (shouldPersist) {
      ...
      expect(message).not.toHaveProperty("__nodoassist.senderId");
      expect(message).not.toHaveProperty("__nodoassist.senderName");
      expect(message).not.toHaveProperty("__nodoassist.senderUsername");
    }
  });
  ...
  it("refreshes goal context after interrupt admission waits", async () => {
    ...
    const runPromise = runPreparedReply(
      baseParams({
        ...
        storePath: "/tmp/nodoassist-session-store.json",
      }),
    );
  ...
  it("runs room events as contextual events instead of direct user prompts", async () => {
    ...
    const call = requireLastRunReplyAgentCall();
    expect(call?.commandBody).toBe("[NodoAssist room event]");
    expect(call?.transcriptCommandBody).toBe("#35676 Keśava: No wtf");
    expect(call?.followupRun.prompt).toBe("[NodoAssist room event]");
    ...
    expect(call?.followupRun.userTurnTranscriptRecorder?.message).toEqual({
      ...
      __nodoassist: { senderIsOwner: false, senderName: "Keśava" },
    });
    ...
    expect(call?.followupRun.currentInboundContext?.text).toContain("[NodoAssist room event]");
    expect(call?.followupRun.currentInboundContext?.text).toContain(
  ...
  it("keeps heartbeat prompts out of visible transcript prompt", async () => {
    ...
    expect(call?.transcriptCommandBody).toBe("[NodoAssist heartbeat poll]");
    expect(call?.followupRun.transcriptPrompt).toBe("[NodoAssist heartbeat poll]");
  });
  ...
  it.each([
    ...
    async (commandText, startupAction) => {
      ...
      expect(call?.transcriptCommandBody).toBe(`[NodoAssist session ${startupAction}]`);
      expect(call?.followupRun.transcriptPrompt).toBe(`[NodoAssist session ${startupAction}]`);
