...
const workspaceHarness = createCommandWorkspaceHarness("nodoassist-command-plugins-install-");
...
function expectPersistedInstall(pluginId: string, expectedInstall: Record<string, unknown>): void {
  ...
  expectObjectFields(persisted.snapshot, {
    writeOptions: expect.objectContaining({
      ...
      expectedConfigPath: expect.stringContaining("nodoassist.json"),
      ownedConfigPathForWrite: expect.stringContaining("nodoassist.json"),
    }),
  });
...
describe("handleCommands /plugins install", () => {
  ...
  it("passes the active config to npm install policy preflight", async () => {
    const policyConfig: NodoAssistConfig = {
      commands: {
        text: true,
        plugins: true,
      },
    ...
    await withTempHome("nodoassist-command-plugins-home-", async (home) => {
      await fs.writeFile(
        path.join(home, ".nodoassist", "nodoassist.json"),
  ...
  it("installs a plugin from a local path", async () => {
    ...
    await withTempHome("nodoassist-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
  ...
  it("installs from an explicit clawhub: spec", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.2.3",
      extensions: ["index.js"],
      packageName: "@nodoassist/clawhub-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@nodoassist/clawhub-demo",
    ...
    await withTempHome("nodoassist-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install clawhub:@nodoassist/clawhub-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@nodoassist/clawhub-demo@1.2.3",
      );
      expectPersistedInstall("clawhub-demo", {
        source: "clawhub",
        spec: "clawhub:@nodoassist/clawhub-demo@1.2.3",
  ...
  it("includes non-blocking ClawHub warnings in successful chat install replies", async () => {
    const warning =
      'ClawHub trust warning for "@nodoassist/clawhub-demo@1.2.3": scan=pending; reasons=pending.';
  ...
  it("reports risky ClawHub install failures without persisting install metadata", async () => {
    const warning =
      'ClawHub trust warning for "@nodoassist/risky-demo@1.2.3": scan=suspicious; moderation=none; blockedFromDownload=false; pending=false; stale=false; reasons=payload_string. Risk signals: scan status suspicious, payload_string.';
    installPluginFromClawHubMock.mockResolvedValue({
      ok: false,
      code: "clawhub_risk_acknowledgement_required",
      error:
        'ClawHub release "@nodoassist/risky-demo@1.2.3" has trust warnings. Review the package and rerun with --acknowledge-clawhub-risk to continue.',
      warning,
    });
    ...
    await withTempHome("nodoassist-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install clawhub:@nodoassist/risky-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(result.reply?.text).toContain("local nodoassist plugins install command");
      expect(result.reply?.text).toContain("trusted shell");
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@nodoassist/risky-demo@1.2.3",
      );
      expect(persistPluginInstallMock).not.toHaveBeenCalled();
    });
  });
  ...
  it("includes ClawHub trust details for blocked chat install failures", async () => {
    const warning =
      'ClawHub trust warning for "@nodoassist/blocked-demo@1.2.3": scan=suspicious; moderation=blocked; blockedFromDownload=true; pending=false; stale=false; reasons=payload_string. Risk signals: blocked from download, scan status suspicious, moderation state blocked, payload_string.';
    installPluginFromClawHubMock.mockResolvedValue({
      ok: false,
      code: "clawhub_download_blocked",
      error:
        'ClawHub release "@nodoassist/blocked-demo@1.2.3" is blocked from download by ClawHub.',
      warning,
    });
    ...
    await withTempHome("nodoassist-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install clawhub:@nodoassist/blocked-demo@1.2.3",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
  ...
  it("refuses plugin installs in Nix mode before package installer side effects", async () => {
    const previousNixMode = process.env.NODOASSIST_NIX_MODE;
    ...
    try {
      await withTempHome("nodoassist-command-plugins-home-", async () => {
        ...
        expect(result.reply?.text).toContain("NODOASSIST_NIX_MODE=1");
        expect(result.reply?.text).toContain("nix-nodoassist#quick-start");
      ...
      if (previousNixMode === undefined) {
        ...
        process.env.NODOASSIST_NIX_MODE = previousNixMode;
      }
    }
  });
  ...
  it("refuses installs through a root include before package installer side effects", async () => {
    await withTempHome("nodoassist-command-plugins-home-", async (home) => {
      const sharedConfigPath = path.join(home, ".nodoassist", "shared.json5");
  ...
  it("treats /plugin add as an install alias", async () => {
    installPluginFromClawHubMock.mockResolvedValue({
      ...
      version: "1.0.0",
      extensions: ["index.js"],
      packageName: "@nodoassist/alias-demo",
      clawhub: {
        source: "clawhub",
        clawhubUrl: "https://clawhub.ai",
        clawhubPackage: "@nodoassist/alias-demo",
        clawhubFamily: "code-plugin",
        clawhubChannel: "official",
        version: "1.0.0",
    ...
    await withTempHome("nodoassist-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugin add clawhub:@nodoassist/alias-demo@1.0.0",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(mockFirstObjectArg(installPluginFromClawHubMock).spec).toBe(
        "clawhub:@nodoassist/alias-demo@1.0.0",
      );
    });
  });
  ...
  it("trusts catalog npm package installs with alternate selectors", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "wecom-nodoassist-plugin",
      targetDir: "/tmp/wecom-nodoassist-plugin",
      version: "2026.4.23",
      extensions: ["index.js"],
      npmResolution: {
    ...
    await withTempHome("nodoassist-command-plugins-home-", async () => {
      const workspaceDir = await workspaceHarness.createWorkspace();
      const params = buildPluginsParams(
        "/plugins install @wecom/wecom-nodoassist-plugin@latest",
        workspaceDir,
      );
      const result = await handlePluginsCommand(params, true);
      if (result === null) {
      ...
      expect(result.reply?.text).toContain('Installed plugin "wecom-nodoassist-plugin"');
      const npmInstallArgs = mockFirstObjectArg(installPluginFromNpmSpecMock);
      expectObjectFields(npmInstallArgs, {
        spec: "@wecom/wecom-nodoassist-plugin@latest",
        expectedPluginId: "wecom-nodoassist-plugin",
        trustedSourceLinkedOfficialInstall: true,
      });
      expect(npmInstallArgs.expectedIntegrity).toBeUndefined();
      expectPersistedInstall("wecom-nodoassist-plugin", {
        source: "npm",
        spec: "@wecom/wecom-nodoassist-plugin@latest",
        installPath: "/tmp/wecom-nodoassist-plugin",
        version: "2026.4.23",
        resolvedName: "@wecom/wecom-nodoassist-plugin",
