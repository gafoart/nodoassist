...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("nodoassist.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("nodoassist.reply.full-runtime");

type ReplyConfigWithMarker = NodoAssistConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.NODOASSIST_ALLOW_SLOW_REPLY_TESTS === "1" || env.NODOASSIST_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends NodoAssistConfig>(
  config: T,
...
export function withFastReplyConfig<T extends NodoAssistConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends NodoAssistConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

function isCompleteReplyConfig(config: unknown): config is NodoAssistConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set NODOASSIST_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as NodoAssistConfig;
}
