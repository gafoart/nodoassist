...
const ROOM_EVENT_PROMPT = "[NodoAssist room event]";
...
export function buildReplyPromptEnvelopeBase(
  ...
  // conversation, while the NodoAssist marker remains current-turn context only.
  const transcriptBody = params.isHeartbeat
    ? HEARTBEAT_TRANSCRIPT_PROMPT
    : params.isBareSessionReset
      ? softResetTail || `[NodoAssist session ${params.startupAction}]`
      : isRoomEvent
