...
export async function stageSandboxMedia(params: {
  ...
  // For remote attachments without sandbox, use ~/.nodoassist/media (not agent workspace for privacy).
  // Managed local inbound refs are already in NodoAssist's media store; when no sandbox is
  ...
  const hostWorkspaceStagingDir =
    !sandbox && !ctx.MediaRemoteHost
      ? path.join("media", "inbound", `nodoassist-staged-${crypto.randomUUID()}`)
      : undefined;
