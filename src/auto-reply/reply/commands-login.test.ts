...
function buildLoginParams(
  ...
  const params = buildCommandTestParams(
    ...
    { workspaceDir: "/tmp/nodoassist-login-test" },
  );
...
describe("handleLoginCommand", () => {
  ...
  it("rejects grouped shared-channel login before emitting a device code", async () => {
    ...
    expect(result).toEqual({
      shouldContinue: false,
      reply: {
        text: "Codex login codes are only sent in a private chat or Web UI session. Open a private chat with NodoAssist and send `/login codex` there.",
      },
    });
  ...
  it("rejects non-owner senders before starting login", async () => {
    ...
    expect(result).toEqual({
      shouldContinue: false,
      reply: {
        text: "Only a configured NodoAssist owner/admin can start Codex login from this channel.",
      },
    });
    expect(runModelsAuthLoginFlowMock).not.toHaveBeenCalled();
  });
