...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-export-command-"));
  tempDirs.push(dir);
  return dir;
}
...
function makeParams(workspaceDir = makeTempDir()): HandleCommandsParams {
  return {
    cfg: {
      session: {
        store: "/tmp/nodoassist-sessions.json",
      },
    },
    ctx: {
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "nodoassist sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(execCall.defaults.sessionStore).toBe("/tmp/nodoassist-sessions.json");
    ...
    expect(command).not.toContain("nodoassist sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("nodoassist-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("nodoassist sessions export-trajectory");
