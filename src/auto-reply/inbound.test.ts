...
describe("mention helpers", () => {
  it("builds regexes and skips invalid or unsafe patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\bnodoassist\\b", "(invalid", "(a+)+$"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("nodoassist")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("open\u200bclaw")).toBe("nodoassist");
  });

  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\bnodoassist\\b"] } },
    });
    expect(matchesMentionPatterns("NODOASSIST: hi", regexes)).toBe(true);
  });
  ...
  it("scopes configured mention patterns by provider conversation policy", () => {
    ...
    expect(matchesMentionPatterns("nodoassist: hi", allowed)).toBe(true);
    expect(matchesMentionPatterns("nodoassist: hi", denied)).toBe(false);
  });
  ...
  it("preserves mention patterns for callers without scoped policy facts", () => {
    ...
    expect(matchesMentionPatterns("nodoassist", regexes)).toBe(true);
  });
  ...
  it("strips safe mention patterns and ignores unsafe ones", () => {
    const stripped = stripMentions("nodoassist " + "a".repeat(28) + "!", {} as MsgContext, {
      messages: {
        groupChat: { mentionPatterns: ["\\bnodoassist\\b", "(a+)+$"] },
      },
    });
