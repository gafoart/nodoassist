#!/usr/bin/env node
// Boots the NodoAssist CLI entry point under Node.
// CLI process entrypoint for NodoAssist command execution.
...
import {
  enableNodoAssistCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutNodoAssistCompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { normalizeEnv } from "./infra/env.js";
import { isMainModule } from "./infra/is-main.js";
import { ensureNodoAssistExecMarkerOnProcess } from "./infra/nodoassist-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "nodoassist.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "nodoassist.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutNodoAssistCompileCacheIfNeeded({
    currentFile: entryFile,
    installRoot,
  });
  if (!waitingForCompileCacheRespawn) {
    process.title = "nodoassist";
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.NODOASSIST_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[nodoassist] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[nodoassist] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[nodoassist] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        "[nodoassist] Failed to display help:",
...
export async function tryHandlePrecomputedCommandHelpFastPath(
  ...
  if (env.NODOASSIST_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
