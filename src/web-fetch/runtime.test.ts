...
function getFirecrawlApiKey(config?: NodoAssistConfig): unknown {
...
function createFirecrawlPluginConfig(apiKey: unknown): NodoAssistConfig {
  return {
    plugins: {
      entries: {
        firecrawl: {
...
describe("web fetch runtime", () => {
  ...
  it("invalidates provider discovery when the same config object changes", () => {
    ...
    } as NodoAssistConfig & { tools: { web: { fetch: { provider: string } } } };
