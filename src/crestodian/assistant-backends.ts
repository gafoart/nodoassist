...
type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => NodoAssistConfig;
};
...
export function buildCliPlannerConfig(workspaceDir: string, modelRef: string): NodoAssistConfig {
  return {
    agents: {
      defaults: {
...
export function buildCodexAppServerPlannerConfig(workspaceDir: string): NodoAssistConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
