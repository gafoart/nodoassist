...
function useTempStateDir(): string {
  ...
  tempDirs.push(dir);
  vi.stubEnv("NODOASSIST_STATE_DIR", dir);
  return dir;
}
...
describe("CrestodianChatEngine", () => {
  ...
  it("routes sensitive CLI wizard prompts to the masked channel setup flow", async () => {
    ...
    expect(reply.text).toContain("nodoassist channels add --channel telegram");
...
function fakeOverviewLoader(
  ...
  return async () =>
    ({
      config: { path: "/tmp/nodoassist.json", exists: false, valid: true, issues: [], hash: null },
      agents: [],
      defaultAgentId: "main",
