...
function formatUnsupportedRemoteOperation(operation: CrestodianOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      ...
      "Use local `nodoassist` for agent handoff, or ask for status, doctor, config, gateway, agents, or models.",
    ].join(" ");
  }
  if (operation.kind === "channel-setup") {
    return [
      "Crestodian rescue cannot host the interactive channel setup from a message channel.",
      "Run `nodoassist crestodian` locally and say `connect " + operation.channel + "` instead.",
    ].join(" ");
  }
  if (operation.kind === "model-setup") {
    return [
      "Crestodian rescue cannot host model-provider credential setup from a message channel.",
      "Run `nodoassist crestodian` locally and say `configure model provider` instead.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      ...
      "Use local `nodoassist crestodian` or `nodoassist plugins install` instead.",
    ].join(" ");
  }
  return null;
}
