...
const runLive =
  truthy(process.env.NODOASSIST_LIVE_TEST) &&
  truthy(process.env.NODOASSIST_LIVE_CRESTODIAN_RESCUE_CHANNEL);
const describeLive = runLive ? describe : describe.skip;

function commandContext(channel = process.env.NODOASSIST_LIVE_CRESTODIAN_CHANNEL ?? "whatsapp") {
  return {
    surface: channel,
    channel,
    channelId: channel,
...
describeLive("Crestodian live rescue channel smoke", () => {
  afterEach(() => {
    ...
    if (originalStateDir === undefined) {
      ...
      setTestEnvValue("NODOASSIST_STATE_DIR", originalStateDir);
    }
