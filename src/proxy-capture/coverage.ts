...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[nodoassist proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[nodoassist proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`nodoassist proxy coverage\` for details.`,
  );
}
