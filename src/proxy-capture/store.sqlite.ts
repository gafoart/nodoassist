...
import { normalizeNullableString as normalizeObservedValue } from "@nodoassist/normalization-core/string-coerce";
...
class DebugProxyCaptureStoreImpl {
  ...
  constructor(
    ...
    const database = openNodoAssistStateDatabase({ env: optionsOrDbPath.env });
    this.db = database.db;
...
function resolveDebugProxyCaptureStoreKey(
  ...
  return typeof optionsOrDbPath === "string"
    ...
    : `shared:${openNodoAssistStateDatabase({ env: optionsOrDbPath.env }).path}`;
}
