// NodoAssist agent database tests cover agent-scoped DB storage and migrations.
...
type AgentDbTestDatabase = Pick<NodoAssistAgentKyselyDatabase, "schema_meta">;
...
function createTempStateDir(): string {
  return makeTempDir(agentDbTempDirs, "nodoassist-agent-db-");
}

function listRegisteredAgentDatabasesForTest(options: { env?: NodeJS.ProcessEnv } = {}) {
  const rows = openNodoAssistStateDatabase(options)
    .db.prepare(
...
describe("nodoassist agent database", () => {
  it("resolves under the per-agent state directory", () => {
    ...
    expect(
      resolveNodoAssistAgentSqlitePath({
        agentId: "Worker-1",
        env: { NODOASSIST_STATE_DIR: stateDir },
      }),
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "nodoassist-agent.sqlite"));
  });
  ...
  it("creates the per-agent schema and registers it globally", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./nodoassist-agent-schema.sql", import.meta.url)),
    );
    expect(database.agentId).toBe("worker-1");
    expect(database.path).toBe(
      path.join(stateDir, "agents", "worker-1", "agent", "nodoassist-agent.sqlite"),
    );
  ...
  it("keeps multiple registered paths for the same agent", () => {
    ...
    const relocated = openNodoAssistAgentDatabase({
      agentId: "worker-1",
      env,
      path: relocatedPath,
    });
    const defaultDatabase = openNodoAssistAgentDatabase({
      agentId: "worker-1",
      env,
    });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    const stateDir = createTempStateDir();
    const env = { NODOASSIST_STATE_DIR: stateDir };
    const stateDatabasePath = path.join(stateDir, "state", "nodoassist.sqlite");
    ...
    legacyDb.exec(`
      ...
      INSERT INTO agent_databases (
        ...
        'worker-1',
        '/legacy/worker-1/nodoassist-agent.sqlite',
        1,
        10,
        20
      );
    ...
    ).toThrow(/run nodoassist doctor --fix/);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const agentModuleUrl = new URL("./nodoassist-agent-db.ts", import.meta.url).href;
    const stateModuleUrl = new URL("./nodoassist-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-agent-db-state-"));
          const env = { NODOASSIST_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-agent-db-relative-"));
          ...
          try {
            ...
            const first = openNodoAssistAgentDatabase({
              agentId: "worker-1",
              env,
              path: "agent.sqlite",
            });
            ...
            const second = openNodoAssistAgentDatabase({
              agentId: "worker-1",
              env,
              path: "agent.sqlite",
            });
            ...
            console.log(JSON.stringify({
              ...
              registeredPaths: openNodoAssistStateDatabase({ env }).db
  ...
  it("rejects explicit paths that point at the global state database", () => {
    const stateDir = createTempStateDir();
    const env = { NODOASSIST_STATE_DIR: stateDir };
    const databasePath = path.join(stateDir, "state", "nodoassist.sqlite");
    const stateDatabase = openNodoAssistStateDatabase({
      env,
      path: databasePath,
    });
    closeNodoAssistStateDatabaseForTest();
    ...
    const reopenedStateDatabase = openNodoAssistStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { NODOASSIST_STATE_DIR: parentDir };
  ...
  it("closes cached handles on normal process exit so no stale WAL remains", () => {
    ...
    const output = execFileSync(
      ...
      [
        ...
        `
          import fs from "node:fs";
          import { openNodoAssistAgentDatabase } from ${JSON.stringify(agentModuleUrl)};

          const database = openNodoAssistAgentDatabase({
            agentId: "worker-1",
            env: { NODOASSIST_STATE_DIR: process.env.NODOASSIST_AGENT_DB_EXIT_TEST_DIR },
          });
      ...
      {
        encoding: "utf8",
        env: { ...process.env, NODOASSIST_AGENT_DB_EXIT_TEST_DIR: stateDir },
      },
    );
    const result = JSON.parse(output) as {
