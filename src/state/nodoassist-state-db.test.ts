// NodoAssist state database tests cover state DB migrations and persistence.
...
import { resolveNodoAssistStateSqlitePath } from "./nodoassist-state-db.paths.js";
import {
...
function createTempStateDir(): string {
  return makeTempDir(stateDbTempDirs, "nodoassist-state-db-");
}
...
describe("nodoassist state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveNodoAssistStateSqlitePath({ NODOASSIST_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "nodoassist.sqlite"),
    );
  });
  ...
  it("creates the shared state schema from the committed SQL shape", () => {
    ...
    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./nodoassist-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "nodoassist.sqlite"));
  });

  it("creates the bounded skill curator tables", () => {
    const stateDir = createTempStateDir();
    const database = openNodoAssistStateDatabase({ env: { NODOASSIST_STATE_DIR: stateDir } });
  ...
  it.runIf(process.platform === "linux")("closes the database when initialization fails", () => {
    const databasePath = path.join(createTempStateDir(), "nodoassist.sqlite");
    fs.writeFileSync(databasePath, "not a sqlite database");

    expect(() => openNodoAssistStateDatabase({ path: databasePath })).toThrow(
  ...
  it("adds gateway boot lifecycle startup markers to existing state databases", () => {
    ...
    const reopened = openNodoAssistStateDatabase({
      env: { NODOASSIST_STATE_DIR: stateDir },
    });
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const currentReopened = openNodoAssistStateDatabase({
      env: { NODOASSIST_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `nodoassist-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openNodoAssistStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openNodoAssistStateDatabase({ path: firstPath });
    const second = openNodoAssistStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openNodoAssistStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(1);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const moduleUrl = new URL("./nodoassist-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-state-db-relative-"));
          ...
          try {
            ...
            const first = openNodoAssistStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openNodoAssistStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    const stateDir = createTempStateDir();
    const options = { env: { NODOASSIST_STATE_DIR: stateDir } };
    ...
    runNodoAssistStateWriteTransaction((database) => {
      ...
      expect(() =>
        runNodoAssistStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runNodoAssistStateWriteTransaction(async () => {
        return "not sync";
      }, options),
