// NodoAssist state database manages shared persisted state and migrations.
...
import {
  resolveNodoAssistStateSqliteDir,
  resolveNodoAssistStateSqlitePath,
} from "./nodoassist-state-db.paths.js";
import { NODOASSIST_STATE_SCHEMA_SQL } from "./nodoassist-state-schema.generated.js";

/**
 * Shared NodoAssist SQLite state database lifecycle and metadata writers.
 *
 ...
 */
export const NODOASSIST_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const NODOASSIST_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const NODOASSIST_STATE_DIR_MODE = 0o700;
const NODOASSIST_STATE_FILE_MODE = 0o600;

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type NodoAssistStateDatabase = {
  db: DatabaseSync;
  path: string;
...
export type NodoAssistStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

export type NodoAssistStateDatabaseSchemaMigration = {
  kind: "agent-databases-composite-primary-key";
  path: string;
};

const cachedDatabases = new Map<string, NodoAssistStateDatabase>();

type NodoAssistStateMetadataDatabase = Pick<NodoAssistStateKyselyDatabase, "schema_meta">;

function assertSupportedSchemaVersion(db: DatabaseSync, pathname: string): void {
  const userVersion = readSqliteUserVersion(db);
  if (userVersion > NODOASSIST_STATE_SCHEMA_VERSION) {
    throw new Error(
      `NodoAssist state database ${pathname} uses newer schema version ${userVersion}; this NodoAssist build supports ${NODOASSIST_STATE_SCHEMA_VERSION}.`,
    );
  }
}
...
export function ensureNodoAssistStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveNodoAssistStateSqliteDir(env);
  const isDefaultStateDatabase =
    path.resolve(pathname) === path.resolve(resolveNodoAssistStateSqlitePath(env));
  if (isDefaultStateDatabase && dir !== defaultDir) {
    throw new Error(`NodoAssist state database path resolved outside its state dir: ${pathname}`);
  }
  const dirExisted = existsSync(dir);
  mkdirSync(dir, { recursive: true, mode: NODOASSIST_STATE_DIR_MODE });
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, NODOASSIST_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, NODOASSIST_STATE_FILE_MODE);
    }
  }
}
...
function assertCanonicalStateSchemaShape(db: DatabaseSync, pathname: string): void {
  ...
  throw new Error(
    `NodoAssist state database ${pathname} has a legacy agent database registry schema; run nodoassist doctor --fix to migrate it.`,
  );
}

export function detectNodoAssistStateDatabaseSchemaMigrations(
  options: NodoAssistStateDatabaseOptions = {},
): NodoAssistStateDatabaseSchemaMigration[] {
...
export function repairNodoAssistStateDatabaseSchema(options: NodoAssistStateDatabaseOptions = {}): {
  changes: string[];
  warnings: string[];
} {
  ...
  ensureNodoAssistStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  const db = new sqlite.DatabaseSync(pathname);
  db.exec(`PRAGMA busy_timeout = ${NODOASSIST_SQLITE_BUSY_TIMEOUT_MS};`);
  try {
...
export function withNodoAssistStateStartupMigrationCheckpointDatabase<T>(
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(NODOASSIST_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${NODOASSIST_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<NodoAssistStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: NODOASSIST_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: NodoAssistStateDatabaseOptions = {}): string {
  return path.resolve(options.path ?? resolveNodoAssistStateSqlitePath(options.env ?? process.env));
}

/** Open or return a cached shared state database after schema and migration checks. */
export function openNodoAssistStateDatabase(
  options: NodoAssistStateDatabaseOptions = {},
): NodoAssistStateDatabase {
  const env = options.env ?? process.env;
  const pathname = resolveDatabasePath(options);
  ...
  const walMaintenance = (() => {
    ...
    try {
      maintenance = configureSqliteConnectionPragmas(db, {
        ...
        databaseLabel: "nodoassist-state",
...
export function runNodoAssistStateWriteTransaction<T>(
  operation: (database: NodoAssistStateDatabase) => T,
  options: NodoAssistStateDatabaseOptions = {},
): T {
  const database = openNodoAssistStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureNodoAssistStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function closeNodoAssistStateDatabase(): void {
...
export function isNodoAssistStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Test alias for closing shared state handles from teardown code. */
export const closeNodoAssistStateDatabaseForTest = closeNodoAssistStateDatabase;
