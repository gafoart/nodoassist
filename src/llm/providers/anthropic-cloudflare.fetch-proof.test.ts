import type { Context, Model } from "@nodoassist/ai";
...
// ever reached. This proves the stream facade installs NodoAssist's guarded
...
// Importing the facade installs the NodoAssist AI transport host ports.
import "../stream.js";
...
describe("Anthropic Cloudflare guard-specific SSRF blocking proof", () => {
  ...
  it("blocks a private-IP request before globalThis.fetch is called (guard-specific behavior)", async () => {
    ...
    const { streamAnthropic } = await import("@nodoassist/ai/internal/anthropic");
