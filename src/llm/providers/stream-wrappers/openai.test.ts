...
describe("createCodexNativeWebSearchWrapper", () => {
  ...
  it("does not enable code-mode transport enforcement when config is on but controls are inactive", () => {
    ...
    expect(observedOptions[0]?.nodoassistCodeModeToolSurface).toBeUndefined();
  ...
  it("enforces the code-mode transport surface when the run enables it at agent scope", () => {
    ...
    expect(observedOptions[0]?.nodoassistCodeModeToolSurface).toBe(true);
    expect(payloads[0]?.tools).toEqual([
      { type: "function", name: "exec" },
      { type: "function", name: "wait" },
    ]);
...
describe("createOpenAIAttributionHeadersWrapper", () => {
  it("routes native Codex traffic through the NodoAssist transport so attribution survives NodoAssist defaults", () => {
    let codexCalls = 0;
    ...
    expect(capturedHeaders?.originator).toBe("nodoassist");
    expect(capturedHeaders?.["User-Agent"]).toMatch(/^nodoassist\//);
  });
  ...
  it("keeps existing wrapped Codex streams so runtime OAuth injection is preserved", () => {
    ...
    expect(capturedOptions?.headers?.originator).toBe("nodoassist");
    expect(capturedOptions?.headers?.["User-Agent"]).toMatch(/^nodoassist\//);
  });
});
