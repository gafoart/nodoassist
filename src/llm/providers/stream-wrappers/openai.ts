...
type NodoAssistSimpleStreamOptions = SimpleStreamOptions & {
...
function isCodeModeEnabled(config?: NodoAssistConfig): boolean {
...
export function createCodexNativeWebSearchWrapper(
  ...
  return (model, context, options) => {
    if (
      ...
      const originalOnPayload = options?.onPayload;
      const codeModeOptions: NodoAssistSimpleStreamOptions = {
        ...options,
