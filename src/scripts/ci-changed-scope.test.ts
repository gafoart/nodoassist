...
describe("detectChangedScope", () => {
  it("routes only native i18n-owned paths to the native inventory job", () => {
    for (const changedPath of [
      ...
      "apps/android/app/src/main/java/ai/nodoassist/app/MainActivity.kt",
      "apps/ios/Sources/RootTabs.swift",
      "apps/macos/Sources/NodoAssist/Settings.swift",
      "apps/shared/NodoAssistKit/Sources/NodoAssistKit/Client.swift",
  ...
  it("keeps node lane off for native-only changes", () => {
    ...
    expect(
      detectChangedScope(["apps/macos-mlx-tts/Sources/NodoAssistMLXTTSHelper/main.swift"]),
    ).toEqual({
      runNode: false,
      runMacos: true,
      runIosBuild: false,
    ...
    expect(detectChangedScope(["apps/shared/NodoAssistKit/Sources/Foo.swift"])).toEqual({
      runNode: false,
      runMacos: true,
      runIosBuild: true,
      runAndroid: true,
  ...
  it("treats base and head as literal git args", () => {
    const markerPath = path.join(
      os.tmpdir(),
      `nodoassist-ci-changed-scope-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`,
    );
  ...
  it("uses the merge commit first parent instead of a stale PR payload base", () => {
    const { repoDir, staleBase } = createSyntheticMergeRepo("nodoassist-ci-scope-merge-");
  ...
  it("keeps direct CLI preflight empty diffs as no-op scope", () => {
    const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "nodoassist-ci-scope-empty-"));
    tempDirs.push(repoDir);
