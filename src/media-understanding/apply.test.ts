...
const TEMP_MEDIA_PREFIX = "nodoassist-media-";
...
function createGroqAudioConfig(): NodoAssistConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): NodoAssistConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): NodoAssistConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      NODOASSIST_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: NodoAssistConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredNodoAssistTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: NodoAssistConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("nodoassist-antigravity-probe-"),
    });
