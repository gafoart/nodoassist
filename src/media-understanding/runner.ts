...
import { mergeInboundPathRoots } from "@nodoassist/media-core/inbound-path-policy";
...
async function probeAntigravityCliCandidate(command: string): Promise<string | null> {
  ...
  const probeDir = await fs.mkdtemp(
    path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-antigravity-probe-"),
  );
  try {
...
async function resolveAntigravityCliBinary(): Promise<string | null> {
  ...
  const resolved = (async () => {
    const configured = process.env.NODOASSIST_ANTIGRAVITY_CLI?.trim();
