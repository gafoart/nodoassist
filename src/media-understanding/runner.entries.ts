...
import type { ModelProviderConfig, NodoAssistConfig } from "../config/types.js";
import type {
...
function resolveAudioProviderPrompt(params: {
  ...
  // match the audio language; omit NodoAssist's English default for non-English
...
export function formatMissingProviderHint(providerId: string): string {
  ...
  return ` Install the official external plugin with: ${formatCliCommand(catalogHint.installCommand)}, then run ${formatCliCommand("nodoassist plugins registry --refresh")} and stop and start the gateway service, or run ${formatCliCommand(catalogHint.doctorFixCommand)} to repair automatically.`;
}
...
export async function runCliEntry(params: {
  ...
  const outputDir = await fs.mkdtemp(
    path.join(resolvePreferredNodoAssistTmpDir(), "nodoassist-media-cli-"),
  );
