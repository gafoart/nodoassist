...
const hoisted = vi.hoisted(() => ({
  completeMock: vi.fn(),
  ensureNodoAssistModelsJsonMock: vi.fn(async () => {}),
...
const {
  completeMock,
  ensureNodoAssistModelsJsonMock,
...
vi.mock("../agents/models-config.js", async () => ({
  ...
  ensureNodoAssistModelsJson: ensureNodoAssistModelsJsonMock,
}));
...
describe("describeImageWithModel", () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv("NODOASSIST_BUNDLED_PLUGINS_DIR", path.join(process.cwd(), "extensions"));
  ...
  it("resolves configured image models when discovery has not registered the provider", async () => {
    ...
    expect(resolveRequest.agentDir).toBe("/tmp/nodoassist-agent");
    expect(
      requireRecord(
        requireRecord(
