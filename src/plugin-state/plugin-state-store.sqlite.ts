// Plugin state SQLite helpers persist plugin state in the NodoAssist state database.
...
type PluginStateEntriesTable = NodoAssistStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<NodoAssistStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveNodoAssistStateSqlitePath(process.env),
): PluginStateStoreError {
...
function parseStoredJson(raw: string, operation: PluginStateStoreOperation): unknown {
  try {
    ...
    throw createPluginStateError({
      ...
      path: resolveNodoAssistStateSqlitePath(process.env),
      cause: error,
    });
  }
}
...
function openPluginStateDatabase(
  ...
  const pathname = resolveNodoAssistStateSqlitePath(env);
...
function envOptions(env?: NodeJS.ProcessEnv): NodoAssistStateDatabaseOptions {
  return env ? { env } : {};
}
...
function runWriteTransaction<T>(
  ...
  return runNodoAssistStateWriteTransaction(() => {
...
export function probePluginStateStore(): PluginStateStoreProbeResult {
  ...
  const stateWasOpen = isNodoAssistStateDatabaseOpen();
  ...
  try {
    ...
    openNodoAssistStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {
