...
beforeAll(async () => {
  testState = await createNodoAssistTestState({ label: "plugin-state-store" });
  rmSync(path.dirname(resolveNodoAssistStateSqlitePath()), { recursive: true, force: true });
});
...
describe("plugin state keyed store", () => {
      ...
      async (stateA) => {
        await withNodoAssistTestState(
          ...
          async (stateB) => {
            ...
            expect(resolveNodoAssistStateSqlitePath(stateA.env)).not.toBe(
              resolveNodoAssistStateSqlitePath(stateB.env),
            );
          },
        );
      },
  ...
  it("reopens after the shared state DB cache closes its handle", async () => {
    await withPluginStateTestState(async () => {
      ...
      const secondary = await createNodoAssistTestState({
        label: "plugin-state-cache-secondary",
        applyEnv: false,
      });
      try {
        openNodoAssistStateDatabase({ env: secondary.env });
        testState?.applyEnv();
        await expect(store.lookup("k")).resolves.toEqual({ ok: true });
      } finally {
  ...
  it.runIf(process.platform !== "win32")("hardens DB directory and file permissions", async () => {
    await withPluginStateTestState(async () => {
      ...
      const databasePath = resolveNodoAssistStateSqlitePath();
