...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: NodoAssistConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "nodoassist.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.NODOASSIST_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_NODOASSIST_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.NODOASSIST_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.NODOASSIST_BUNDLED_PLUGINS_DIR ??= "extensions";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.NODOASSIST_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function addCoveragePluginLoadPath(config: NodoAssistConfig, pluginId: string): void {
...
function collectNodoAssistCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "nodoassist.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_NODOASSIST_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForNodoAssistTarget(
...
async function expectNodoAssistCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("NODOASSIST_SECRET_TARGET", entry.id);
    ...
    applyConfigForNodoAssistTarget(config, entry, envId, wildcardToken);
  }
...
const NODOASSIST_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectNodoAssistCoverageEntries({ includePluginEntries: false }),
);
const NODOASSIST_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectNodoAssistCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = NODOASSIST_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectNodoAssistCoverageBatchResolved("nodoassist.json core", googleChatBatch);
    }
    const webProviderBatch = NODOASSIST_PLUGIN_COVERAGE_BATCHES.find((batch) =>
    ...
    if (webProviderBatch) {
      ...
      await expectNodoAssistCoverageBatchResolved("nodoassist.json plugins", webProviderBatch);
    }
  });

  describe("nodoassist.json core and channel registry targets", () => {
    test.each(NODOASSIST_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectNodoAssistCoverageBatchResolved("nodoassist.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("nodoassist.json plugin registry targets", () => {
    test.each(NODOASSIST_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectNodoAssistCoverageBatchResolved("nodoassist.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("NODOASSIST_AUTH_SECRET_TARGET", entry.id);
