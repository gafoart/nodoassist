...
import { resolveStateDir, type NodoAssistConfig } from "../config/config.js";
...
/** Stable finding codes emitted by `nodoassist secrets audit`. */
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as NodoAssistConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
