...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-secrets-audit-"));
...
describe("secrets audit", () => {
  ...
  it("scans active agent-dir override models.json even when outside state dir", async () => {
    ...
    const report = await runSecretsAudit({
      env: {
        ...fixture.env,
        NODOASSIST_AGENT_DIR: externalAgentDir,
      },
    });
    expect(
      hasFinding(
  ...
  it("does not flag non-sensitive routing headers in nodoassist config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
  ...
  it("keeps request headers in nodoassist config covered by plaintext audit", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
  ...
  it("does not flag nodoassist.json model provider apiKey marker values as plaintext", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          lmstudio: {
