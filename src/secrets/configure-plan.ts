/** Builds the interactive `nodoassist secrets configure` target list and apply plan. */
...
/** Credential target shown by `nodoassist secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: NodoAssistConfig): Record<string, SecretProviderConfig> {
...
/** Builds configure candidates for the current NodoAssist config only. */
export function buildConfigureCandidates(config: NodoAssistConfig): ConfigureCandidate[] {
...
function configureCandidateSortKey(candidate: ConfigureCandidate): string {
  ...
  return `nodoassist:${candidate.path}`;
}
...
/** Builds configure candidates for NodoAssist config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: NodoAssistConfig;
  authoredNodoAssistConfig?: NodoAssistConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredNodoAssistConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const nodoassistCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `nodoassist.json` as const,
  ...
  return [...nodoassistCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "nodoassist secrets configure",
