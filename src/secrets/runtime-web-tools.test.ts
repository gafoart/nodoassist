...
function setConfiguredFetchProviderKey(configTarget: NodoAssistConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: NodoAssistConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: NodoAssistConfig, provider: ProviderUnderTest): unknown {
...
describe("runtime web tools resolution", () => {
  ...
  it("uses runtime web search discovery when the managed plugin index install records is populated", async () => {
    loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
      "external-search": {
        source: "npm",
        spec: "@nodoassist/external-search",
      },
    });
  ...
  it("resolves SecretRefs for verified installed Firecrawl fetch config", async () => {
    loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
      firecrawl: {
        source: "npm",
        spec: "@nodoassist/firecrawl-plugin",
      },
    });
  ...
  describe("when brave is installed as an external plugin and explicitly configured", () => {
    ...
    beforeEach(() => {
      loadInstalledPluginIndexInstallRecordsSyncMock.mockReturnValue({
        brave: { source: "npm", spec: "@nodoassist/brave-search" },
      });
