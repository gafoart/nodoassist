...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `nodoassist.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built nodoassist
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.NODOASSIST_DEBUG_CHANNEL_CONTRACT_API === "1") {
