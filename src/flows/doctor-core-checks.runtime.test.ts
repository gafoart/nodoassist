...
const mocks = vi.hoisted(() => ({
  ...
  createNodoAssistCodingTools: vi.fn(),
  ...
  resolveGatewayService: vi.fn(() => ({ label: "nodoassist-gateway" })),
...
vi.mock("../agents/agent-tools.js", () => ({
  createNodoAssistCodingTools: mocks.createNodoAssistCodingTools,
}));
...
describe("doctor runtime tool schema checks", () => {
  beforeEach(() => {
    mocks.createNodoAssistCodingTools.mockReset().mockReturnValue([]);
    ...
    mocks.readGatewayServiceState.mockReset().mockResolvedValue({
      ...
      running: true,
      env: {},
      command: { programArguments: ["nodoassist", "gateway"], sourcePath: "/tmp/gateway.service" },
  ...
  it("preserves direct OpenAI catalog transport while building doctor runtime models", async () => {
    ...
    mocks.createNodoAssistCodingTools.mockReturnValueOnce([
  ...
  it("reports unsupported schemas exposed only to a non-default configured agent", async () => {
    mocks.createNodoAssistCodingTools.mockImplementation((options) =>
    ...
    expect(mocks.createNodoAssistCodingTools).toHaveBeenCalledWith(
  ...
  it("skips ACP-only agents because they do not use embedded tool projection", async () => {
    ...
    expect(mocks.createNodoAssistCodingTools).toHaveBeenCalledTimes(1);
    expect(mocks.createNodoAssistCodingTools).toHaveBeenCalledWith(
  ...
  it("loads bundled MCP runtime once per distinct agent workspace", async () => {
    mocks.createNodoAssistCodingTools.mockReturnValue([]);
    mocks.createBundleMcpToolRuntime.mockImplementation(
      async (options: { workspaceDir: string }) => ({
...
describe("doctor gateway runtime checks", () => {
  beforeEach(() => {
    ...
    mocks.resolveGatewayService.mockReset().mockReturnValue({ label: "nodoassist-gateway" });
  });
  ...
  it("reports unreachable gateway health probes", async () => {
    ...
    await expect(
      ...
      fixHint:
        "Start the Gateway service or run `nodoassist doctor --fix` for service repair prompts.",
    });
  });
  ...
  it("reports missing local gateway daemon service", async () => {
    ...
    await expect(
      ...
      path: "gateway.mode",
      target: "nodoassist-gateway",
      fixHint: "Run `nodoassist doctor --fix` or `nodoassist gateway install` to install it.",
    });
  });
