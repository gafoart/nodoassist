...
import { createNodoAssistCodingTools } from "../agents/agent-tools.js";
...
export function detectUnavailableSkills(cfg: NodoAssistConfig): SkillStatusEntry[] {
    ...
    return [
      {
        ...
        fixHint:
          "Fix Gateway connection configuration, then rerun `nodoassist doctor --lint --only core/doctor/gateway-health`.",
      },
    ];
  }
...
export async function collectGatewayDaemonFindings(
  ...
  if (!state.loaded) {
    findings.push({
      ...
      target: service.label,
      fixHint: "Run `nodoassist doctor --fix` or `nodoassist gateway start` to load it.",
    });
  }
  ...
  if (state.loaded && !state.running) {
    findings.push({
      ...
      target: service.label,
      fixHint:
        "Run `nodoassist gateway status --deep` or `nodoassist doctor --fix` for repair hints.",
    });
  }
...
function collectAgentRuntimeToolSchemaFindings(params: {
  ...
  try {
    tools = createNodoAssistCodingTools({
      agentId: params.agentId,
      workspaceDir: params.workspaceDir,
      config: params.cfg,
      modelProvider: params.modelRef.provider,
...
function isAcpRuntimeAgent(cfg: NodoAssistConfig, agentId: string): boolean {
