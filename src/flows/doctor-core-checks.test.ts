...
function createSkill(overrides: Partial<SkillStatusEntry> = {}): SkillStatusEntry {
  return {
    ...
    source: "workspace",
    bundled: false,
    filePath: "/tmp/nodoassist-test-workspace/skills/missing-tool/SKILL.md",
    baseDir: "/tmp/nodoassist-test-workspace/skills/missing-tool",
    ...
    requirements: {
      bins: ["nodoassist-test-missing-skill-bin"],
      anyBins: [],
      env: [],
      config: [],
      os: [],
...
describe("CORE_HEALTH_CHECKS", () => {
  ...
  it("converts unavailable skills into repair-capable health findings", async () => {
    ...
    const cfg: NodoAssistConfig = {
      agents: {
        defaults: {
          workspace: "/tmp/nodoassist-test-workspace",
          skills: ["missing-tool"],
        },
      },
    };
    ...
    const findings = await check.detect({
      ...
      runtime,
      cfg,
      cwd: "/tmp/nodoassist-test-workspace",
    });
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves in lint checks", async () => {
    ...
    await withEnvAsync({ NODOASSIST_TEST_GATEWAY_TOKEN: "resolved-test-token" }, async () => {
      const findings = await check?.detect({
        mode: "lint",
          ...
          gateway: {
            ...
            auth: {
              ...
              token: {
                source: "env",
                provider: "default",
                id: "NODOASSIST_TEST_GATEWAY_TOKEN",
              },
            },
          },
          secrets: {
  ...
  it("reports unresolved SecretRefs even when NODOASSIST_GATEWAY_TOKEN is set", async () => {
    const check = CORE_HEALTH_CHECKS.find((entry) => entry.id === "core/doctor/gateway-auth");
    await withEnvAsync(
      {
        NODOASSIST_GATEWAY_TOKEN: "fallback-token",
        NODOASSIST_MISSING_GATEWAY_REF_TOKEN: undefined,
      },
      async () => {
        const findings = await check?.detect({
          mode: "lint",
  ...
  it("does not execute or warn for valid exec SecretRefs during default gateway auth lint checks", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "nodoassist-health-exec-ref-"));
  ...
  it("reports exec SecretRef failures when gateway auth lint explicitly allows exec checks", async () => {
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        message: expect.stringContaining("Gateway token SecretRef could not be resolved:"),
        fixHint:
          "Run `nodoassist doctor --allow-exec` to verify exec SecretRefs during doctor, or `nodoassist secrets audit --allow-exec` to audit all exec SecretRefs.",
      }),
    );
  });
...
describe("core/doctor/bootstrap-size", () => {
  ...
  it("honors the per-agent bootstrapMaxChars override in health findings", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "nodoassist-health-bootstrap-"));
