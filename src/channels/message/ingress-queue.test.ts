...
type ChannelIngressTestDatabase = Pick<NodoAssistStateKyselyDatabase, "channel_ingress_events">;

async function withTempState<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-ingress-queue-"));
  try {
...
describe("channel ingress queue", () => {
  ...
  it("opens a custom state database without copying the full process env", async () => {
    ...
    await withTempState(async (stateDir) => {
      ...
      expect(env.NODOASSIST_STATE_DIR).toBe(stateDir);
      expect(env.HOME).toBe("/home/nodoassist");
      expect(Object.getPrototypeOf(env)).toBe(inheritedEnv);
      expect(Object.keys(env)).toEqual(["NODOASSIST_STATE_DIR"]);

      const database = openNodoAssistStateDatabase({ env });
