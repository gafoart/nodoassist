...
function listBuiltPluginLoaderModuleCandidateUrls(importerUrl: string): URL[] {
  ...
  // ../../plugins candidates would escape the installed nodoassist package there.
...
function getChannelConfigRecord(cfg: NodoAssistConfig, channelId: string): Record<string, unknown> {
...
function listManifestChannelAccountIds(cfg: NodoAssistConfig, channelId: string): string[] {
...
function resolveManifestChannelDefaultAccountId(cfg: NodoAssistConfig, channelId: string): string {
  const channelConfig = getChannelConfigRecord(cfg, channelId);
...
function rebindChannelPluginConfig(
  ...
  const rebind = (cfg: NodoAssistConfig) =>
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupFallbackPlugins === true) {
      ...
      const registry = loadPluginLoaderModule().loadNodoAssistPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
