...
import type { NodoAssistPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["NODOASSIST_PLUGIN_CATALOG_PATHS", "NODOASSIST_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = uniqueStrings(
    [
      resolveNodoAssistPackageRootSync({ cwd: process.cwd() }),
      resolveNodoAssistPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<NodoAssistPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
