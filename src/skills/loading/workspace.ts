...
import { resolveNodoAssistMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".nodoassist", "source-origin.json");
...
function isContainerStateHomeWherePromptTildeEscapes(home: string): boolean {
  ...
  return (
    home === "/data" &&
    (configDir === "/data/.nodoassist" || isPathInside("/data/.nodoassist", configDir))
  );
}
...
function resolveSkillsLimits(config?: NodoAssistConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "nodoassist-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "nodoassist-bundled") {
    return {
...
function resolveSkillEntryMetadata(params: {
  ...
  const metadata = resolveNodoAssistMetadata(params.frontmatter);
...
function loadGeneratedPluginSkillRecords(params: {
  ...
  for (const name of childDirs) {
    ...
    // Plugin skills live as symlinks under ~/.nodoassist/plugin-skills/, so
  ...
  const loadSkills = (params: { dir: string; source: string }): LoadedSkillRecord[] => {
    ...
    for (const candidate of scanQueue) {
      ...
      const maxGroupedDepth =
        params.source === "nodoassist-extra" &&
...
function buildSkillsLimitNote(params: {
  ...
  if (params.truncated) {
    return `⚠️ Skills truncated: included ${params.included} of ${params.total}${params.compact ? " (compact format, descriptions omitted)" : ""}. Run \`nodoassist skills check\` to audit.`;
  }
  if (params.compact) {
    return `⚠️ Skills catalog using compact format (descriptions omitted). Run \`nodoassist skills check\` to audit.`;
  }
  return "";
}
