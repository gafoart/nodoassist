...
function resolveWatchTargets(workspaceDir: string, config?: NodoAssistConfig): WatchTarget[] {
  const baseRoots: Array<{ path: string; source: string }> = [];
  if (workspaceDir.trim()) {
    baseRoots.push({ path: path.join(workspaceDir, "skills"), source: "nodoassist-workspace" });
    baseRoots.push({
      path: path.join(workspaceDir, ".agents", "skills"),
      source: "agents-skills-project",
    });
  }
  baseRoots.push({ path: path.join(CONFIG_DIR, "skills"), source: "nodoassist-managed" });
  baseRoots.push({
...
function isTrustedSymlinkSkillTarget(
  ...
  if (source === "nodoassist-managed" || source === "agents-skills-personal") {
    return true;
  }
  return (
...
function resolveWatchDebounceMs(config?: NodoAssistConfig): number {
...
export function ensureSkillsWatcher(params: { workspaceDir: string; config?: NodoAssistConfig }) {
