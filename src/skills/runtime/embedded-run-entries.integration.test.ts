...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("nodoassist-bundled-");
  const workspaceDir = await createTempDir("nodoassist-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: NodoAssistConfig) {
