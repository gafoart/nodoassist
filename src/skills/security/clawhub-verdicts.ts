...
type NodoAssistSkillSecurityVerdictItem = Omit<
...
function projectClawHubVerdictItem(
  ...
  registry: string,
): NodoAssistSkillSecurityVerdictItem {
  const projected: NodoAssistSkillSecurityVerdictItem = {
    registry,
    ok: item.ok,
  ...
  if (item.error) {
    const error: NodoAssistSkillSecurityVerdictItem["error"] = {};
...
export async function fetchNodoAssistSkillSecurityVerdicts(
  targets: Array<{ registry: string; slug: string; version: string }>,
): Promise<NodoAssistSkillSecurityVerdictItem[]> {
  ...
  const items: NodoAssistSkillSecurityVerdictItem[] = [];
