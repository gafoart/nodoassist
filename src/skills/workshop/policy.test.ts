...
beforeEach(async () => {
  testState = await createNodoAssistTestState({
    layout: "state-only",
    prefix: "nodoassist-skill-workshop-policy-",
  });
});
...
describe("resolveSkillWorkshopToolApproval", () => {
  it("describes the target proposal and bounds the approval wait", async () => {
    const workspaceDir = await tempDirs.make("nodoassist-skill-workshop-policy-workspace-");
  ...
  it("bounds approval metadata without dropping required proposal facts", async () => {
    const workspaceDir = await tempDirs.make("nodoassist-skill-workshop-policy-long-name-");
    const description = "d".repeat(160);
    const proposal = await proposeCreateSkill({
      workspaceDir,
      name: "n".repeat(240),
  ...
  it("renders proposal-controlled fields without approval-line injection", async () => {
    const workspaceDir = await tempDirs.make("nodoassist-skill-workshop-policy-controls-");
    const proposal = await proposeCreateSkill({
      workspaceDir,
      name: "Line\nBreak\u202eSpoof",
      description:
  ...
  it("falls back to the action description when the proposal cannot be resolved", async () => {
    const workspaceDir = await tempDirs.make("nodoassist-skill-workshop-policy-missing-");
