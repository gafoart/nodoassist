...
import type { DB as NodoAssistStateDatabase } from "../../state/nodoassist-state-db.generated.js";
import {
...
type CuratorOptions = NodoAssistStateDatabaseOptions & {
  nowMs?: number;
};

function curatorDb(options: NodoAssistStateDatabaseOptions = {}) {
...
function writeSweepAttempt(nowMs: number, options: NodoAssistStateDatabaseOptions): void {
  runNodoAssistStateWriteTransaction(({ db }) => {
    const kysely = getNodeSqliteKysely<CuratorDatabase>(db);
    executeSqliteQuerySync(
      db,
...
export async function runSkillCuratorSweep(
  ...
  try {
    ...
    const result = runNodoAssistStateWriteTransaction(({ db }) => {
      const kysely = getNodeSqliteKysely<CuratorDatabase>(db);
...
function updateLifecyclePin(
  ...
  const firstSkillFile = runNodoAssistStateWriteTransaction(({ db }) => {
    const kysely = getNodeSqliteKysely<CuratorDatabase>(db);
...
export function pinCuratedSkill(skill: string, options: NodoAssistStateDatabaseOptions = {}) {
  return updateLifecyclePin(skill, true, options);
}

export function unpinCuratedSkill(skill: string, options: NodoAssistStateDatabaseOptions = {}) {
