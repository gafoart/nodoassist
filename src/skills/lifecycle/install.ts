...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "nodoassist");
  }
  return path.join(homedir(), ".nodoassist");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "nodoassist-bundled"
        ? { kind: "bundled", authority: "nodoassist", mutable: false, network: false }
        : skillSource === "nodoassist-managed" || skillSource === "nodoassist-extra"
          ? { kind: "managed", authority: "nodoassist", mutable: false, network: false }
