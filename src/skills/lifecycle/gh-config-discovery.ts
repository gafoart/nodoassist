...
// NodoAssist process is running with a different HOME (e.g. the per-agent
...
export type GhConfigDirMismatch = {
  ...
  // Suggested env value the operator should set on the NodoAssist service to
...
export function formatGhConfigDirMismatchHint(mismatch: GhConfigDirMismatch): string[] {
  ...
  lines.push(
    `  Fix: set GH_CONFIG_DIR=${mismatch.suggestedEnvValue} on the NodoAssist service environment, then restart the gateway.`,
  );
  return lines;
}
