...
function makeEntry(params: {
  ...
  return {
    skill: createCanonicalFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "nodoassist-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "nodoassist-bundled-"));
    ...
    await withEnvAsync({ NODOASSIST_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill.source).toBe("nodoassist-workspace");
