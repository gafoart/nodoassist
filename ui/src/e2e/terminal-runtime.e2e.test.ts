  ...
  it("does not reuse freed terminal cells in the next tab", async () => {
    ...
    try {
      ...
      await page.addScriptTag({
        content: `globalThis.nodoassistTerminalRuntimeModule = import(${JSON.stringify(moduleUrl)});`,
        type: "module",
      });
        ...
        async ({ staleText }) => {
          const runtimeModule = await (
            window as unknown as Window & {
              nodoassistTerminalRuntimeModule: Promise<{
                createIsolatedGhosttyTerminal: BrowserTerminalFactory;
              }>;
            }
          ).nodoassistTerminalRuntimeModule;
