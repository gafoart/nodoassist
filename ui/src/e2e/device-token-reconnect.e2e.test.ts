...
const proofDir = process.env.NODOASSIST_UI_E2E_ARTIFACT_DIR?.trim();
...
async function selectGatewayOnNextLoad(
  ...
  const settingsKey = `nodoassist.control.settings.v1:${normalizeGatewayTokenScope(gatewayUrl)}`;
  const selectionKey =
    `nodoassist.control.currentGateway.v1:` +
...
async function openGatewayPage(params: {
  ...
  await page.locator("nodoassist-app-shell").waitFor();
...
describeControlUiE2e("Control UI device-token reconnect E2E", () => {
  ...
  it("isolates device tokens across gateways, origins, and revocation", async () => {
    ...
    expect(await rositaReconnect.page.locator("nodoassist-login-gate").count()).toBe(0);
    ...
    expect(await wilfredReconnect.page.locator("nodoassist-login-gate").count()).toBe(0);

    const identity = await wilfredSource.page.evaluate(() => {
      const raw = localStorage.getItem("nodoassist-device-identity-v1");
    ...
    const wilfredStoreKey =
      `nodoassist.device.auth.v1:` + normalizeGatewayCredentialScope(WILFRED_GATEWAY_URL);
    await expect
      .poll(() =>
