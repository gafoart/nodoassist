...
describeControlUiE2e("embedded terminal document", () => {
  ...
  it("renders only the terminal while native authentication connects", async () => {
    ...
    await page.addInitScript(() => {
      (
        window as Window & {
          ["__NODOASSIST_NATIVE_CONTROL_AUTH__"]?: {
            gatewayUrl: string;
            token: string;
          };
        }
    ...
    try {
      ...
      expect(await page.locator("nodoassist-login-gate").count()).toBe(0);
      expect(await page.locator("nodoassist-terminal-panel").count()).toBe(1);
