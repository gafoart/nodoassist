...
export async function startControlUiE2eServer(): Promise<ControlUiE2eServer> {
  ...
  const server = await createServer({
    ...
    configFile: false,
    define: {
      NODOASSIST_CONTROL_UI_BUILD_ID: JSON.stringify("e2e"),
    },
    logLevel: "error",
    optimizeDeps: {
      include: [
...
function normalizeScenario(
  ...
  return {
    ...
    assistantName: scenario.assistantName?.trim() || "NodoAssist",
...
function installControlUiMockGateway(input: {
  ...
  type WindowWithGateway = Window & {
    nodoassistControlUiE2eGateway?: ExposedGateway;
  };
  ...
  (window as WindowWithGateway).nodoassistControlUiE2eGateway = exposed;
