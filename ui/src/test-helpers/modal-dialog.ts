...
import type { NodoAssistModalDialog } from "../components/modal-dialog.ts";
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<NodoAssistModalDialog>("nodoassist-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected nodoassist-modal-dialog");
  }
