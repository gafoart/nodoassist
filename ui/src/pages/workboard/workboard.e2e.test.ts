...
function workboardConfigSnapshot() {
  ...
  return {
    ...
    path: "/tmp/nodoassist-e2e/nodoassist.json",
...
describeControlUiE2e("Control UI Workboard mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install chromium\`, or set NODOASSIST_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
