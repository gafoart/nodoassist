...
function streamFallbackReplacementText(message: unknown): string | null {
  ...
  const fallback = (message as { nodoassistStreamFallback?: unknown }).nodoassistStreamFallback;
...
function terminalMessageReplacesStreamFallback(message: unknown, fallback: unknown): boolean {
  ...
  const metadata = (fallback as { nodoassistStreamFallback?: unknown }).nodoassistStreamFallback;
  const source =
