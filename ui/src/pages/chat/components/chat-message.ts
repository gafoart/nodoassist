...
function extractImages(message: unknown): ImageBlock[] {
  ...
  if (Array.isArray(content)) {
    for (const block of content) {
      ...
      } else if (b.type === "nodoassist_pairing_qr") {
...
function extractPairingQrExpiryNotices(
  ...
  for (const block of content) {
    ...
    const b = block as Record<string, unknown>;
    if (b.type === "nodoassist_pairing_qr" && isExpiredPairingQrBlock(b, nowMs)) {
      notices.push({
...
function resolveNearestFuturePairingQrExpiresAtMs(
  ...
  for (const block of content) {
    ...
    const b = block as Record<string, unknown>;
    if (b.type !== "nodoassist_pairing_qr") {
      continue;
    }
...
const SKIP_DELETE_CONFIRM_KEY = "nodoassist:skipDeleteConfirm";
...
function renderDeleteButton(onDelete: () => void, side: DeleteConfirmSide) {
  return html`
    <span class="chat-delete-wrap">
      <nodoassist-tooltip content="Delete">
        <button
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__nodoassist__|media|api\/chat\/media\/outgoing)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__nodoassist__/assistant-media?${params.toString()}`;
}
  ...
  if (!pending) {
    pending = (async () => {
      ...
      if (requesterSessionKey) {
        headers.set("x-nodoassist-requester-session-key", requesterSessionKey);
      }
...
function renderExpandButton(
  ...
  return html`
    <nodoassist-tooltip content="Open in canvas">
      <button
...
function resolveMessageActionDetails(
  ...
  return {
    ...
    shouldFetchFullMessage: Boolean(
      onOpenSidebar &&
      messageId &&
      !record.nodoassistMessageToolMirror &&
