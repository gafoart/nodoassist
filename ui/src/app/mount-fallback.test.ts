...
async function readIndexHtmlWithDelay(delayMs: number): Promise<string> {
  ...
  return html.replace(
    'data-nodoassist-mount-timeout-ms="12000"',
    `data-nodoassist-mount-timeout-ms="${delayMs}"`,
  );
}
...
function installFallbackShell(window: TestWindow, html: string): void {
  ...
  const sentinel = Array.from(parsed.querySelectorAll<HTMLScriptElement>("script:not([src])")).find(
    (script) => script.textContent?.includes("nodoassist-mount-fallback"),
  );
...
describe("Control UI mount fallback", () => {
  ...
  it("shows the static troubleshooting panel when the app element is never registered", async () => {
    ...
    expect(frameWindow.customElements.get("nodoassist-app")).toBeUndefined();
    ...
    const fallback = requireElementById(
      frameWindow,
      "nodoassist-mount-fallback",
      frameWindow.HTMLElement,
    );
    expect(fallback.hidden).toBe(false);
    expect([...frameWindow.document.body.classList]).toEqual(["nodoassist-mount-fallback-active"]);
  ...
  it("keeps the fallback hidden when the app element registers before the timeout", async () => {
    ...
    if (!frameWindow.customElements.get("nodoassist-app")) {
      frameWindow.customElements.define("nodoassist-app", class extends frameWindow.HTMLElement {});
    }
    await frameWindow.customElements.whenDefined("nodoassist-app");
  ...
  it("probes a cache-busted current document when the original bundle did not start", async () => {
    ...
    expect(fetch).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining("nodoassist_mount_recovery="),
      {
        cache: "no-store",
  ...
  it("bounds automatic recovery attempts while the gateway is unavailable", async () => {
    ...
    expect(
      requireElementById(
        frameWindow,
        "nodoassist-mount-fallback-summary",
