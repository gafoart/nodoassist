  ...
  const guidance =
    {
      ...
      "not-git-install":
        "Not a git checkout. Run `nodoassist update` from the CLI for a global reinstall.",
      "not-nodoassist-root":
        "Run the update from an NodoAssist checkout or use the CLI global reinstall path.",
      ...
      "doctor-failed": "Doctor repair failed. Run `nodoassist doctor --non-interactive` and retry.",
...
function resolvePendingUpdateHandoffTimeoutBanner(): ApplicationStatusBanner {
  return {
    tone: "danger",
    text: "Update handoff started, but completion was not reported after reconnect. Run `nodoassist update status` for the final result.",
  };
}
