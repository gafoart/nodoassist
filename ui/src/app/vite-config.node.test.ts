...
describe("Control UI Vite config", () => {
  it("resolves root tsconfig package aliases for source imports", () => {
    expect(findStringAlias("@nodoassist/net-policy/ip")?.replacement).toBe(
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@nodoassist/normalization-core/string-coerce"),
    )?.toEqual({
  ...
  it("resolves published NodoAssist packages before the broad plugin alias", () => {
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const netPolicyIpIndex = aliases.findIndex(
      (alias) => alias.find === "@nodoassist/net-policy/ip",
    );
    const netPolicyPackageIndex = aliases.findIndex(
      (alias) => alias.find === "@nodoassist/net-policy",
    );
    ...
    const broadNodoAssistWildcardIndex = aliases.findIndex(
    ...
    expect(broadNodoAssistWildcardIndex).toBeGreaterThanOrEqual(0);
    expect(netPolicyIpIndex).toBeLessThan(netPolicyPackageIndex);
    expect(netPolicyWildcardIndex).toBeLessThan(broadNodoAssistWildcardIndex);
  });
