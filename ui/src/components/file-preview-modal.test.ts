...
import type { NodoAssistFilePreviewModal } from "./file-preview-modal.ts";
...
async function renderPreview(options: RenderPreviewOptions = {}) {
  ...
  const modal = container.querySelector<NodoAssistFilePreviewModal>(
    "nodoassist-file-preview-modal",
  );
...
function shadowText(modal: NodoAssistFilePreviewModal): string {
  return modal.shadowRoot?.textContent ?? "";
}

describe("nodoassist-file-preview-modal", () => {
  beforeEach(() => {
