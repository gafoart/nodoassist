...
import type { NodoAssistModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: NodoAssistModalDialog, id: string): HTMLElement {
...
describe("nodoassist-modal-dialog", () => {
  beforeEach(() => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("nodoassist-modal-dialog-label");
    expect(descriptionId).toBe("nodoassist-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "nodoassist-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "nodoassist-modal-dialog-description").textContent).toBe(
