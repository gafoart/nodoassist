// NodoAssist Control – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "nodoassist-control-";
...
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__NODOASSIST_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    data = { title: "NodoAssist", body: event.data.text() };
  }

  const title = data.title || "NodoAssist";
  const options = {
    body: data.body || "",
    icon: "./apple-touch-icon.png",
    badge: "./favicon-32.png",
    tag: data.tag || "nodoassist-notification",
    data: { url: data.url || "./" },
  };
