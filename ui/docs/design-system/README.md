...
NodoAssist's visual language is a **dark-first, glass-surface system** built around a deep navy base (`#0b0e14`), the NODO brand green accent (`#9ed77b`), and layered frosted-glass surfaces that create depth without solid panels. Motion is crisp and purposeful — fast micro-interactions (100ms) with spring-loaded expansions. All interactive elements meet WCAG 2.1 AA contrast requirements on dark backgrounds.
...
3. **Accent with restraint** — Brand green (`--accent: #9ed77b`) for primary actions only; the deeper green (`--accent-2`) for secondary/status.
