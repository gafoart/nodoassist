...
NodoAssist is the AI that actually does things.
...
NodoAssist started as a personal playground to learn AI and build something genuinely useful:
an assistant that can run real tasks on a real computer.
It evolved through several names and shells: Warelay -> Clawdbot -> Moltbot -> NodoAssist.
...
NodoAssist runtime code reads the current configuration schema only.
...
`nodoassist doctor --fix` should detect the old shape, explain it, back it up when needed, and rewrite it to the canonical format.
...
Security in NodoAssist is a deliberate tradeoff: strong defaults without killing capability.
...
NodoAssist has an extensive plugin API.
...
- Code plugins run NodoAssist plugin code and are appropriate for deeper runtime extension.
...
NodoAssist docs should document core extension points; plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins.
...
NodoAssist supports MCP as both a server and a runtime integration surface.
...
NodoAssist is currently terminal-first by design.
...
NodoAssist is primarily an orchestration system: prompts, tools, protocols, and integrations.
TypeScript was chosen to keep NodoAssist hackable by default.
